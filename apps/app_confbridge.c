...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 249894 $")
...
TRIS_APP_OPTIONS(app_opts,{
	TRIS_APP_OPTION('A', OPTION_MARKEDUSER),
	TRIS_APP_OPTION('a', OPTION_ADMIN),
	TRIS_APP_OPTION('c', OPTION_ANNOUNCEUSERCOUNT),
	TRIS_APP_OPTION('m', OPTION_STARTMUTED),
	TRIS_APP_OPTION_ARG('M', OPTION_MUSICONHOLD, OPTION_MUSICONHOLD_CLASS),
	TRIS_APP_OPTION('1', OPTION_NOONLYPERSON),
	TRIS_APP_OPTION('s', OPTION_MENU),
	TRIS_APP_OPTION('w', OPTION_WAITMARKED),
...
struct conference_bridge {
	...
	struct tris_bridge *bridge;                                        /*!< Bridge structure doing the mixing */
	unsigned int users;                                               /*!< Number of users present */
	unsigned int markedusers;                                         /*!< Number of marked users present */
	unsigned int locked:1;                                            /*!< Is this conference bridge locked? */
	TRIS_LIST_HEAD_NOLOCK(, conference_bridge_user) users_list;        /*!< List of users participating in the conference bridge */
	struct tris_channel *playback_chan;                                /*!< Channel used for playback into the conference bridge */
	tris_mutex_t playback_lock;                                        /*!< Lock used for playback channel */
};
...
struct conference_bridge_user {
	...
	struct tris_channel *chan;                    /*!< Trismedia channel participating */
	struct tris_flags flags;                      /*!< Flags passed in when the application was called */
	char *opt_args[OPTION_ARRAY_SIZE];           /*!< Arguments to options passed when application was called */
	struct tris_bridge_features features;         /*!< Bridge features structure */
	unsigned int kicked:1;                       /*!< User has been kicked from the conference */
	TRIS_LIST_ENTRY(conference_bridge_user) list; /*!< Linked list information */
};
...
static int conference_bridge_hash_cb(const void *obj, const int flags)
{
	...
	return tris_str_case_hash(conference_bridge->name);
}
...
static void announce_user_count(struct conference_bridge *conference_bridge, struct conference_bridge_user *conference_bridge_user)
{
	if (conference_bridge->users == 1) {
		...
		if (tris_stream_and_wait(conference_bridge_user->chan, "conf-onlyone", "")) {
			return;
		}
	} else {
		/* Alas multiple others in here */
		if (tris_stream_and_wait(conference_bridge_user->chan, "conf-thereare", "")) {
			return;
		}
		if (tris_say_number(conference_bridge_user->chan, conference_bridge->users - 1, "", conference_bridge_user->chan->language, NULL)) {
			return;
		}
		if (tris_stream_and_wait(conference_bridge_user->chan, "conf-otherinparty", "")) {
			return;
		}
	}
}
 ...
 */
static void play_prompt_to_channel(struct conference_bridge *conference_bridge, struct tris_channel *chan, const char *file)
{
	ao2_unlock(conference_bridge);
	tris_stream_and_wait(chan, file, "");
...
{
	if (tris_test_flag(&conference_bridge_user->flags, OPTION_MARKEDUSER)) {
		...
		TRIS_LIST_TRAVERSE(&conference_bridge->users_list, other_conference_bridge_user, list) {
			if (other_conference_bridge_user == conference_bridge_user) {
				continue;
			}
			if (tris_test_flag(&other_conference_bridge_user->flags, OPTION_MUSICONHOLD) && !tris_bridge_suspend(conference_bridge->bridge, other_conference_bridge_user->chan)) {
				tris_moh_stop(other_conference_bridge_user->chan);
				tris_bridge_unsuspend(conference_bridge->bridge, other_conference_bridge_user->chan);
			}
		}

		/* Next play the audio file stating they are going to be placed into the conference */
		if (!tris_test_flag(&conference_bridge_user->flags, OPTION_QUIET)) {
			ao2_unlock(conference_bridge);
			tris_autoservice_start(conference_bridge_user->chan);
			play_sound_file(conference_bridge, "conf-placeintoconf");
			tris_autoservice_stop(conference_bridge_user->chan);
			ao2_lock(conference_bridge);
		}
		 ...
		 */
		if (!conference_bridge->markedusers && tris_test_flag(&conference_bridge_user->flags, OPTION_MUSICONHOLD)) {
			tris_moh_start(conference_bridge_user->chan, conference_bridge_user->opt_args[OPTION_MUSICONHOLD_CLASS], NULL);
		}
	}
}
...
static void post_join_unmarked(struct conference_bridge *conference_bridge, struct conference_bridge_user *conference_bridge_user)
{
	...
	if (conference_bridge->users == 1) {
		...
		if (!tris_test_flag(&conference_bridge_user->flags, OPTION_QUIET | OPTION_NOONLYPERSON)) {
		 ...
		 */
		if (conference_bridge->users == 1 && tris_test_flag(&conference_bridge_user->flags, OPTION_MUSICONHOLD)) {
			tris_moh_start(conference_bridge_user->chan, conference_bridge_user->opt_args[OPTION_MUSICONHOLD_CLASS], NULL);
		}
		return;
	}
	...
	if (tris_test_flag(&conference_bridge_user->flags, OPTION_ANNOUNCEUSERCOUNT)) {
		ao2_unlock(conference_bridge);
	...
	if (conference_bridge->users == 2) {
		struct conference_bridge_user *first_participant = TRIS_LIST_FIRST(&conference_bridge->users_list);

		/* Temporarily suspend the above participant from the bridge so we have control to stop MOH if needed */
		if (tris_test_flag(&first_participant->flags, OPTION_MUSICONHOLD) && !tris_bridge_suspend(conference_bridge->bridge, first_participant->chan)) {
			tris_moh_stop(first_participant->chan);
			tris_bridge_unsuspend(conference_bridge->bridge, first_participant->chan);
		}
	}
}
...
static void destroy_conference_bridge(void *obj)
{
	...
	tris_debug(1, "Destroying conference bridge '%s'\n", conference_bridge->name);

	tris_mutex_destroy(&conference_bridge->playback_lock);

	if (conference_bridge->playback_chan) {
		struct tris_channel *underlying_channel = conference_bridge->playback_chan->tech->bridged_channel(conference_bridge->playback_chan, NULL);
		tris_hangup(underlying_channel);
		tris_hangup(conference_bridge->playback_chan);
	...
	if (conference_bridge->bridge) {
		tris_bridge_destroy(conference_bridge->bridge);
...
static struct conference_bridge *join_conference_bridge(const char *name, struct conference_bridge_user *conference_bridge_user)
{
	...
	tris_debug(1, "Trying to find conference bridge '%s'\n", name);
	...
	if (conference_bridge && conference_bridge->locked && !tris_test_flag(&conference_bridge_user->flags, OPTION_ADMIN)) {
		ao2_unlock(conference_bridges);
		ao2_ref(conference_bridge, -1);
		tris_debug(1, "Conference bridge '%s' is locked and caller is not an admin\n", name);
		tris_stream_and_wait(conference_bridge_user->chan, "conf-locked", "");
		return NULL;
	}
	...
	if (!conference_bridge) {
		...
		if (!(conference_bridge = ao2_alloc(sizeof(*conference_bridge), destroy_conference_bridge))) {
			ao2_unlock(conference_bridges);
			tris_log(LOG_ERROR, "Conference bridge '%s' does not exist.\n", name);
			return NULL;
		}

		/* Setup conference bridge parameters */
		tris_copy_string(conference_bridge->name, name, sizeof(conference_bridge->name));

		/* Create an actual bridge that will do the audio mixing */
		if (!(conference_bridge->bridge = tris_bridge_new(TRIS_BRIDGE_CAPABILITY_1TO1MIX, TRIS_BRIDGE_FLAG_SMART))) {
			ao2_ref(conference_bridge, -1);
			conference_bridge = NULL;
			ao2_unlock(conference_bridges);
			tris_log(LOG_ERROR, "Conference bridge '%s' could not be created.\n", name);
			return NULL;
		}

		/* Setup lock for playback channel */
		tris_mutex_init(&conference_bridge->playback_lock);

		/* Link it into the conference bridges container */
		ao2_link(conference_bridges, conference_bridge);

		tris_debug(1, "Created conference bridge '%s' and linked to container '%p'\n", name, conference_bridges);
	}
	...
	TRIS_LIST_INSERT_TAIL(&conference_bridge->users_list, conference_bridge_user, list);
	...
	if (tris_test_flag(&conference_bridge_user->flags, OPTION_MARKEDUSER | OPTION_WAITMARKED)) {
...
static void  leave_conference_bridge(struct conference_bridge *conference_bridge, struct conference_bridge_user *conference_bridge_user)
{
	...
	TRIS_LIST_REMOVE(&conference_bridge->users_list, conference_bridge_user, list);
	...
	if (conference_bridge->users) {
		if (tris_test_flag(&conference_bridge_user->flags, OPTION_MARKEDUSER) && !conference_bridge->markedusers) {
			struct conference_bridge_user *other_participant = NULL;

			/* Start out with muting everyone */
			TRIS_LIST_TRAVERSE(&conference_bridge->users_list, other_participant, list) {
			...
			TRIS_LIST_TRAVERSE(&conference_bridge->users_list, other_participant, list) {
				if (tris_test_flag(&other_participant->flags, OPTION_MUSICONHOLD) && !tris_bridge_suspend(conference_bridge->bridge, other_participant->chan)) {
					tris_moh_start(other_participant->chan, other_participant->opt_args[OPTION_MUSICONHOLD_CLASS], NULL);
					tris_bridge_unsuspend(conference_bridge->bridge, other_participant->chan);
				}
			}
			...
			if (tris_test_flag(&first_participant->flags, OPTION_MUSICONHOLD) && !tris_bridge_suspend(conference_bridge->bridge, first_participant->chan)) {
				tris_moh_start(first_participant->chan, first_participant->opt_args[OPTION_MUSICONHOLD_CLASS], NULL);
				tris_bridge_unsuspend(conference_bridge->bridge, first_participant->chan);
			}
		}
	} else {
...
static int play_sound_file(struct conference_bridge *conference_bridge, const char *filename)
{
	struct tris_channel *underlying_channel;

	tris_mutex_lock(&conference_bridge->playback_lock);

	if (!(conference_bridge->playback_chan)) {
		int cause;

		if (!(conference_bridge->playback_chan = tris_request("Bridge", TRIS_FORMAT_SLINEAR, "", &cause, 0))) {
			tris_mutex_unlock(&conference_bridge->playback_lock);
			return -1;
		}

		conference_bridge->playback_chan->bridge = conference_bridge->bridge;

		if (tris_call(conference_bridge->playback_chan, "", 0)) {
			tris_hangup(conference_bridge->playback_chan);
			conference_bridge->playback_chan = NULL;
			tris_mutex_unlock(&conference_bridge->playback_lock);
			return -1;
		...
		tris_debug(1, "Created a playback channel to conference bridge '%s'\n", conference_bridge->name);
		...
		underlying_channel = conference_bridge->playback_chan->tech->bridged_channel(conference_bridge->playback_chan, NULL);
		tris_bridge_impart(conference_bridge->bridge, underlying_channel, NULL, NULL);
	}

	/* The channel is all under our control, in goes the prompt */
	tris_stream_and_wait(conference_bridge->playback_chan, filename, "");

	tris_debug(1, "Departing underlying channel '%s' from bridge '%p'\n", underlying_channel->name, conference_bridge->bridge);
	tris_bridge_depart(conference_bridge->bridge, underlying_channel);
 ...
 * \retval -1 failure
 */
static int menu_callback(struct tris_bridge *bridge, struct tris_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	int digit, res = 0, isadmin = tris_test_flag(&conference_bridge_user->flags, OPTION_ADMIN);

	/* See if music on hold is playing */
	ao2_lock(conference_bridge);
	if (conference_bridge->users == 1 && tris_test_flag(&conference_bridge_user->flags, OPTION_MUSICONHOLD)) {
		/* Just us so MOH is probably indeed going, let's stop it */
		tris_moh_stop(bridge_channel->chan);
	}
	ao2_unlock(conference_bridge);

	/* Try to play back the user menu, if it fails pass this back up so the bridging core will act on it */
	if (tris_streamfile(bridge_channel->chan, (isadmin ? "conf-adminmenu" : "conf-usermenu"), bridge_channel->chan->language)) {
		res = -1;
		goto finished;
	}

	/* Wait for them to enter a digit from the user menu options */
	digit = tris_waitstream(bridge_channel->chan, TRIS_DIGIT_ANY);
	tris_stopstream(bridge_channel->chan);
	...
	if (digit == '1') {
		...
		if (!tris_test_flag(&conference_bridge_user->flags, OPTION_WAITMARKED) || conference_bridge->markedusers) {
			conference_bridge_user->features.mute = (!conference_bridge_user->features.mute ? 1 : 0);
		}
		res = tris_stream_and_wait(bridge_channel->chan, (conference_bridge_user->features.mute ? "conf-muted" : "conf-unmuted"), "");
	} else if (isadmin && digit == '2') {
		/* 2 - Unlock or lock conference */
		conference_bridge->locked = (!conference_bridge->locked ? 1 : 0);
		res = tris_stream_and_wait(bridge_channel->chan, (conference_bridge->locked ? "conf-lockednow" : "conf-unlockednow"), "");
	} else if (isadmin && digit == '3') {
		/* 3 - Eject last user */
		struct conference_bridge_user *last_participant = NULL;

		ao2_lock(conference_bridge);
		if (((last_participant = TRIS_LIST_LAST(&conference_bridge->users_list)) == conference_bridge_user) || (tris_test_flag(&last_participant->flags, OPTION_ADMIN))) {
			ao2_unlock(conference_bridge);
			res = tris_stream_and_wait(bridge_channel->chan, "conf-errormenu", "");
		} else {
			last_participant->kicked = 1;
			tris_bridge_remove(conference_bridge->bridge, last_participant->chan);
			ao2_unlock(conference_bridge);
		}
	} else if (digit == '4') {
		/* 4 - Decrease listening volume */
		tris_audiohook_volume_adjust(conference_bridge_user->chan, TRIS_AUDIOHOOK_DIRECTION_WRITE, -1);
	} else if (digit == '6') {
		/* 6 - Increase listening volume */
		tris_audiohook_volume_adjust(conference_bridge_user->chan, TRIS_AUDIOHOOK_DIRECTION_WRITE, 1);
	} else if (digit == '7') {
		/* 7 - Decrease talking volume */
		tris_audiohook_volume_adjust(conference_bridge_user->chan, TRIS_AUDIOHOOK_DIRECTION_READ, -1);
	} else if (digit == '8') {
		/* 8 - Exit the IVR */
	} else if (digit == '9') {
 ...
 finished:
	...
	ao2_lock(conference_bridge);
	if (conference_bridge->users == 1 && tris_test_flag(&conference_bridge_user->flags, OPTION_MUSICONHOLD)) {
		tris_moh_start(bridge_channel->chan, conference_bridge_user->opt_args[OPTION_MUSICONHOLD_CLASS], NULL);
	}
	ao2_unlock(conference_bridge);

	bridge_channel->state = TRIS_BRIDGE_CHANNEL_STATE_WAIT;

	return res;
}

/*! \brief The ConfBridge application */
static int confbridge_exec(struct tris_channel *chan, void *data)
{
	...
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(conf_name);
		TRIS_APP_ARG(options);
	);

	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "%s requires an argument (conference name[,options])\n", app);
		return -1;
	}
	...
	if (args.argc == 2) {
		tris_app_parse_options(app_opts, &conference_bridge_user.flags, conference_bridge_user.opt_args, args.options);
	}
	...
	volume_adjustments[0] = tris_audiohook_volume_get(chan, TRIS_AUDIOHOOK_DIRECTION_READ);
	volume_adjustments[1] = tris_audiohook_volume_get(chan, TRIS_AUDIOHOOK_DIRECTION_WRITE);

	/* Always initialize the features structure, we are in most cases always going to need it. */
	tris_bridge_features_init(&conference_bridge_user.features);

	/* If the menu option is enabled provide a user or admin menu as a custom feature hook */
	if (tris_test_flag(&conference_bridge_user.flags, OPTION_MENU)) {
		tris_bridge_features_hook(&conference_bridge_user.features, "#", menu_callback, &conference_bridge_user);
	}

	/* If the caller should be joined already muted, make it so */
	if (tris_test_flag(&conference_bridge_user.flags, OPTION_STARTMUTED)) {
		conference_bridge_user.features.mute = 1;
	}
	...
	if (!tris_test_flag(&conference_bridge_user.flags, OPTION_QUIET) && !tris_strlen_zero(join_sound) && conference_bridge->users >= 2) {
	...
	tris_bridge_join(conference_bridge->bridge, chan, NULL, &conference_bridge_user.features);

	/* If there is 1 or more people (not including us) already in the conference then play our leave sound unless overridden */
	if (!tris_test_flag(&conference_bridge_user.flags, OPTION_QUIET) && !tris_strlen_zero(leave_sound) && conference_bridge->users >= 2) {
		tris_autoservice_start(chan);
	...
	tris_bridge_features_cleanup(&conference_bridge_user.features);

	/* If the user was kicked from the conference play back the audio prompt for it */
	if (!tris_test_flag(&conference_bridge_user.flags, OPTION_QUIET) && conference_bridge_user.kicked) {
		res = tris_stream_and_wait(chan, "conf-kicked", "");
	}

	/* Restore volume adjustments to previous values in case they were changed */
	if (volume_adjustments[0]) {
		tris_audiohook_volume_set(chan, TRIS_AUDIOHOOK_DIRECTION_READ, volume_adjustments[0]);
	}
	if (volume_adjustments[1]) {
		tris_audiohook_volume_set(chan, TRIS_AUDIOHOOK_DIRECTION_WRITE, volume_adjustments[1]);
	}
...
static int load_module(void)
{
	...
	if (tris_register_application_xml(app, confbridge_exec)) {
		ao2_ref(conference_bridges, -1);
		return TRIS_MODULE_LOAD_DECLINE;
	}
...
TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Conference Bridge Application");
