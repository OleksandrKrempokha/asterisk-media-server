		...
		<syntax>
			...
			<parameter name="options" required="false">
			   <optionlist>
				...
				<option name="i">
					<para>Trismedia will ignore any forwarding requests it may receive on this dial attempt.</para>
				</option>
				<option name="k">
...
TRIS_APP_OPTIONS(dial_exec_options, BEGIN_OPTIONS
...
static void handle_cause(int cause, struct cause_args *num)
{
	...
	case TRIS_CAUSE_NORMAL_CLEARING:
		break;
	case TRIS_CAUSE_FACILITY_REJECTED:
		if (cdr)
			tris_cdr_failed(cdr);
		num->rejected++;
		break;
	case TRIS_CAUSE_NORMAL_UNSPECIFIED:
		if (cdr)
			tris_cdr_failed(cdr);
		num->tempunavail++;
		break;
...
static void senddialevent(struct tris_channel *src, struct tris_channel *dst, const char *dialstring)
{
...
static void senddialendevent(const struct tris_channel *src, const char *dialstatus)
{
	manager_event(EVENT_FLAG_CALL, "Dial",
		"SubEvent: End\r\n"
		"Channel: %s\r\n"
	...
	struct cause_args *num, struct tris_flags64 *peerflags, int single)
{
	...
	if (!c) {
		tris_clear_flag64(o, DIAL_STILLGOING);
	...
	const struct cause_args *num_in, int *result)
{
	...
	int transfer = 0, notifycaller = 1, sendnotify = 0;
	/* single is set if only one destination is enabled */
	int single = outgoing && !outgoing->next && !tris_test_flag64(outgoing, OPT_MUSICBACK | OPT_RINGBACK);
#ifdef HAVE_EPOLL
	struct chanlist *epollo;
#endif
	const char* errcode = 0, *busy_peer = 0, *notifychan = 0;

	if (!in || tris_check_hangup(in)) {
		*to = -1;
		strcpy(pa->status, "CANCEL");
		return NULL;
	}

	notifychan = pbx_builtin_getvar_helper(in, "notifycaller");
	...
	while (*to && !peer) {
		...
		for (o = outgoing; o; o = o->next) {
			...
			if (tris_test_flag64(o, DIAL_STILLGOING) && o->chan)
				watchers[pos++] = o->chan;
			numlines++;
		}
		if (pos == 1) { /* only the input channel is available */
			if (numlines == (num.busy + num.congestion + num.nochan)) {
				...
				else if (num.rejected)
					strcpy(pa->status, "REJECTED");
				else if (num.tempunavail)
					strcpy(pa->status, "TEMPUNAVAIL");
				else if (num.timeout)
					strcpy(pa->status, "NOANSWER");
				else if (num.routefail)
					strcpy(pa->status, "ROUTEFAIL");
				else if (num.nochan) {
					if (in->hangupcause == TRIS_CAUSE_CHANNEL_UNACCEPTABLE)
						strcpy(pa->status, "ROUTEFAIL");
					else if (in->hangupcause == TRIS_CAUSE_FACILITY_REJECTED)
						strcpy(pa->status, "REJECTED");
					else if (in->hangupcause == TRIS_CAUSE_NORMAL_UNSPECIFIED)
						strcpy(pa->status, "TEMPUNAVAIL");
					else if (in->hangupcause == TRIS_CAUSE_CALL_REJECTED)
						strcpy(pa->status, "FORBIDDEN");
					else
		...
		for (o = outgoing; o; o = o->next) {
			...
			if (c == NULL)
				continue;
			if (tris_test_flag64(o, DIAL_STILLGOING) && c->_state == TRIS_STATE_UP) {
				if (!peer) {
			...
			if (f->frametype == TRIS_FRAME_CONTROL) {
				...
				case TRIS_CONTROL_ANSWER:
					...
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_ANSWER);
					}
					break;
				case TRIS_CONTROL_BUSY:
					...
					errcode = pbx_builtin_getvar_helper(c, "Error-Info");
					if (transfer) {
						if (errcode && !tris_strlen_zero(errcode))
							busy_peer = strchr(errcode, ',');
						if (busy_peer)
							pbx_builtin_setvar_helper(in, "Busy-Peer", busy_peer+1);
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_BUSY);
						if (notifycaller) {
							if (busy_peer) {
								if (!strncmp(errcode,  "480", 3) || !strncmp(errcode,  "486", 3)) {
									tris_play_and_wait(in, "dial/is_used");
									tris_play_and_wait(in, "dial/dial-exten-num-is");
									tris_say_digit_str(in, busy_peer+1, "", in->language);
									tris_play_and_wait(in, "dial/dial-is");
								}
							} else {
				...
				case TRIS_CONTROL_ROUTEFAIL:
					...
					c = o->chan = NULL;
					tris_clear_flag64(o, DIAL_STILLGOING);
					handle_cause(TRIS_CAUSE_DESTINATION_OUT_OF_ORDER, &num);
					strcpy(pa->status, "ROUTEFAIL");
					pbx_builtin_setvar_helper(in, "TRANSFERSTATUS", "CONGEST");
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_CIRCUITS);
						if (notifycaller)
							tris_stream_and_wait(in, "dial/pbx-busy", TRIS_DIGIT_ANY);
					}
					break;
				case TRIS_CONTROL_REJECTED:
					...
					c = o->chan = NULL;
					tris_clear_flag64(o, DIAL_STILLGOING);
					handle_cause(TRIS_CAUSE_FACILITY_REJECTED, &num);
					strcpy(pa->status, "REJECTED");
					pbx_builtin_setvar_helper(in, "TRANSFERSTATUS", "CONGEST");
					/*if (transfer)
						tris_stream_and_wait(in, "dial/pbx-busy", TRIS_DIGIT_ANY);*/
				...
				case TRIS_CONTROL_UNAVAILABLE:
					...
					c = o->chan = NULL;
					tris_clear_flag64(o, DIAL_STILLGOING);
					handle_cause(TRIS_CAUSE_NORMAL_UNSPECIFIED, &num);
					strcpy(pa->status, "TEMPUNAVAIL");
					pbx_builtin_setvar_helper(in, "TRANSFERSTATUS", "CONGEST");
					if (transfer) {
						if (sendnotify)
				...
				case TRIS_CONTROL_FORBIDDEN:
					...
					c = o->chan = NULL;
					tris_clear_flag64(o, DIAL_STILLGOING);
					handle_cause(TRIS_CAUSE_CALL_REJECTED, &num);
					strcpy(pa->status, "FORBIDDEN");
					pbx_builtin_setvar_helper(in, "TRANSFERSTATUS", "FORBIDDEN");
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_FORBIDDEN);
						if (notifycaller)
				...
				case TRIS_CONTROL_OFFHOOK:
					...
					strcpy(pa->status, "OFFHOOK");
					pbx_builtin_setvar_helper(in, "TRANSFERSTATUS", "NOTFOUND");
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_OFFHOOK);
						if (notifycaller)
				...
				case TRIS_CONTROL_TAKEOFFHOOK:
					...
					strcpy(pa->status, "TAKEOFFHOOK");
					pbx_builtin_setvar_helper(in, "TRANSFERSTATUS", "UNREGISTERED");
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_TAKEOFFHOOK);
						if (notifycaller)
				...
				case TRIS_CONTROL_TIMEOUT:
					...
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_TIMEOUT);
						if (notifycaller)
				...
				case TRIS_CONTROL_RINGING:
					...
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_RINGING);
					}
					break;
				case TRIS_CONTROL_PROGRESS:
					...
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_PROGRESS);
					}
					break;
				...
				case TRIS_CONTROL_PROCEEDING:
					...
					if (transfer) {
						if (sendnotify)
							tris_indicate(in, TRIS_CONTROL_NOTIFY_PROCEEDING);
					}
					break;
				...
				switch (f->frametype) {
					...
					case TRIS_FRAME_HTML:
						if (!tris_test_flag64(outgoing, DIAL_NOFORWARDHTML) && tris_channel_sendhtml(in, f->subclass, f->data.ptr, f->datalen) == -1) {
		...
		if (winner == in) {
			...
			/* Forward HTML stuff */
			if (single && (f->frametype == TRIS_FRAME_HTML) && !tris_test_flag64(outgoing, DIAL_NOFORWARDHTML))
	...
	char *parse, struct timeval *calldurationlimit)
{
	...
	if (!config->timelimit) {
		tris_log(LOG_WARNING, "Dial does not accept L(%s), hanging up.\n", limit_str);
	...
	struct tris_flags64 *opts, char *opt_args[], struct tris_channel *chan)
{
	...
	if (pa->privdb_val == TRIS_PRIVACY_DENY) {
		tris_verb(3, "Privacy DB reports PRIVACY_DENY for this callerid. Dial reports unavailable\n");
		tris_copy_string(pa->status, "NOANSWER", sizeof(pa->status));
		return 0;
		...
		if (tris_fileexists(pa->privintro, NULL, NULL ) > 0 && strncmp(pa->privcid, "NOCALLERID", 10) != 0) {
			...
			*/
			...
			if (!tris_streamfile(chan, "voicemail/vm-dialout", chan->language) )
...
static int get_monitor_fn(struct tris_channel * chan, char *s, int len)
{
	...
	if (!strncmp(chan->exten, "VOOLGW", 6))
		strcpy(exten, chan->exten+8);
	else
		strcpy(exten, chan->exten);
	tris_localtime(&t, &tm, NULL);
	return snprintf(s, len, "%04d%02d%02d-%02d%02d%02d-%s-%s", 
		tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, 
		tm.tm_sec, S_OR(chan->cid.cid_num, ""), S_OR(exten,""));
}

static int check_mark(struct tris_channel *chan, char *num)
{
	char result[80], sql[256];
	char number[256], *tmp = NULL, *start = NULL;
	char exten[256];
	...
	if (!strncmp(chan->exten, "VOOLGW", 6))
		...
		strcpy(exten, chan->exten);
	snprintf(sql, sizeof(sql), "SELECT extension FROM user_info WHERE tapstart = '1' and (uid = '%s' or extension = '%s' or uid = '%s' or extension = '%s') ",
		chan->cid.cid_num, chan->cid.cid_num, exten, exten);
	sql_select_query_execute(result, sql);
	if (!tris_strlen_zero(result))
		return 1;

	snprintf(sql, sizeof(sql), "SELECT pattern FROM mark_pattern WHERE pattern = '%s' or pattern = '%s'", chan->cid.cid_num, exten);
	sql_select_query_execute(result, sql);
	if (!tris_strlen_zero(result))
		return 1;
	...
	snprintf(number, sizeof(number), "%s", num);
	start = strchr(number, '/');
	if (start) {
		*start = '\0';
		start++;
	...
	snprintf(sql, sizeof(sql), "SELECT pattern FROM mark_pattern WHERE pattern = '%s'", start);
	sql_select_query_execute(result, sql);
	if (!tris_strlen_zero(result))
		return 1;
...
static void exec_monitor(struct tris_channel *chan, char *num) 
{
...
static int dial_exec_full(struct tris_channel *chan, void *data, struct tris_flags64 *peerflags, int *continue_exec)
{
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Dial requires an argument (technology/number)\n");
	...
	if (!tris_strlen_zero(args.options) &&
		tris_app_parse_options64(dial_exec_options, &opts, opt_args, args.options)) {
		pbx_builtin_setvar_helper(chan, "DIALSTATUS", pa.status);
		goto done;
	}
	...
	if (tris_test_flag64(&opts, OPT_DURATION_STOP) && !tris_strlen_zero(opt_args[OPT_ARG_DURATION_STOP])) {
		...
		if (!calldurationlimit.tv_sec) {
			tris_log(LOG_WARNING, "Dial does not accept S(%s), hanging up.\n", opt_args[OPT_ARG_DURATION_STOP]);
			pbx_builtin_setvar_helper(chan, "DIALSTATUS", pa.status);
			goto done;
		}
		tris_verb(3, "Setting call duration limit to %.3lf seconds.\n", calldurationlimit.tv_sec + calldurationlimit.tv_usec / 1000000.0);
	...
	while ((cur = strsep(&rest, "&")) ) {
		...
		num_dialed++;
		if (!number) {
			tris_log(LOG_WARNING, "Dial argument takes format (technology/[device:]number1)\n");
			goto out;
		}

		if (!chan->monitor)
			exec_monitor(chan, number);
		...
		if (opts.flags) {
			...
			tris_set2_flag64(tmp, args.url, DIAL_NOFORWARDHTML);
		}
		...
		if (datastore)
			...
			if (!(datastore = tris_datastore_alloc(&dialed_interface_info, NULL))) {
				tris_log(LOG_WARNING, "Unable to create channel datastore for dialed interfaces. Aborting!\n");
				tris_free(tmp);
				goto out;
			}
		...
		TRIS_LIST_TRAVERSE(dialed_interfaces, di, list) {
			if (!strcasecmp(di->interface, interface)) {
				tris_log(LOG_WARNING, "Skipping dialing interface '%s' again since it has already been dialed\n",
					di->interface);
				break;
			}
		}
		...
		const char* vtmp;
		vtmp = pbx_builtin_getvar_helper(chan, "RealApplication");
		if (vtmp && !strcmp(vtmp, "AppQueue")) {
			tc->appl = "AppQueue";
			snprintf(callinfo, sizeof(callinfo), "Call-Info: MP,queue,1");
			the_app = pbx_findapp("SWITCHAddHeader");
			if (the_app)
				pbx_exec(tc, the_app, callinfo);
		} else {
			tc->appl = "AppDial";
		}
		...
		/* Put them in the list of outgoing thingies...  We're ready now.
		   ...
		   hung up XXX */
		tris_set_flag64(tmp, DIAL_STILLGOING);
		tmp->chan = tc;
		tmp->next = outgoing;
		outgoing = tmp;
	...
	if (!outgoing) {
		if (chan->hangupcause == TRIS_CAUSE_CHANNEL_UNACCEPTABLE)
			strcpy(pa.status, "ROUTEFAIL");
		else
			strcpy(pa.status, "CHANUNAVAIL");
	...
	if (!peer) {
		...
		if (!tris_strlen_zero(args.url) && tris_channel_supports_html(peer) ) {
			tris_debug(1, "app_dial: sendurl=%s.\n", args.url);
		...
		if (tris_test_flag64(&opts, OPT_CALLEE_GOSUB) && !tris_strlen_zero(opt_args[OPT_ARG_CALLEE_GOSUB])) {
			...
			if (theapp && !res9) {
				...
				tris_copy_string(peer->context, "app_dial_gosub_virtual_context", sizeof(peer->context));
		...
		if (res) { /* some error */
			...
			if (chan && chan->exten && !tris_strlen_zero(chan->exten)) {
				memset(result, 0, sizeof(result));
				snprintf(sql, sizeof(sql), "select operator from user_info where uid='%s' or extension='%s';", chan->exten, chan->exten);
				sql_select_query_execute(result, sql);
				if (!tris_strlen_zero(result) && result[0] == '1') {
					if (tris_test_flag64(peerflags, OPT_CALLEE_TRANSFER))
						tris_set_flag(&(config.features_callee), TRIS_FEATURE_REDIRECT);
				}
			}
			if (chan && chan->cid.cid_num && !tris_strlen_zero(chan->exten)) {
				memset(result, 0, sizeof(result));
				snprintf(sql, sizeof(sql), "select operator from user_info where uid='%s' or extension='%s';", chan->cid.cid_num, chan->cid.cid_num);
				sql_select_query_execute(result, sql);
				if (!tris_strlen_zero(result) && result[0] == '1') {
		...
		if (!tris_check_hangup(peer)&& !tris_check_hangup(chan))
			snprintf(peer->context, sizeof(peer->context), "%s", chan->context);
...
out:
	...
	tris_debug(1, "Exiting with DIALSTATUS=%s.\n", pa.status);
...
static int dial_exec(struct tris_channel *chan, void *data)
{
	...
	if (!chan->monitor) {
		exec_monitor(chan, NULL);
	}

	return dial_exec_full(chan, data, &peerflags, NULL);
}

static int retrydial_exec(struct tris_channel *chan, void *data)
{
	char *parse;
	...
	TRIS_DECLARE_APP_ARGS(args,
		...
		TRIS_APP_ARG(dialdata);
	);

	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "RetryDial requires an argument!\n");
		return -1;
	}
	...
	if (!args.dialdata) {
		tris_log(LOG_ERROR, "%s requires a 4th argument (dialdata)\n", rapp);
		goto done;
	}
			...
			if (tris_test_flag64(&peerflags, OPT_DTMF_EXIT)) {
				if (!tris_strlen_zero(args.announce)) {
					if (tris_fileexists(args.announce, NULL, chan->language) > 0) {
						...
						tris_log(LOG_WARNING, "Announce file \"%s\" specified in Retrydial does not exist\n", args.announce);
				}
				if (!res && sleepms) {
					if (!tris_test_flag(chan, TRIS_FLAG_MOH))
						tris_moh_start(chan, NULL, NULL);
...
static int unload_module(void)
{
	...
	if ((con = tris_context_find("app_dial_gosub_virtual_context"))) {
		tris_context_remove_extension2(con, "s", 1, NULL, 0);
		tris_context_destroy(con, "app_dial"); /* leave nothing behind */
	}
...
static int load_module(void)
{
	...
	con = tris_context_find_or_create(NULL, NULL, "app_dial_gosub_virtual_context", "app_dial");
	if (!con)
		tris_log(LOG_ERROR, "Dial virtual context 'app_dial_gosub_virtual_context' does not exist and unable to create\n");
	else
		tris_add_extension2(con, 1, "s", 1, NULL, NULL, "NoOp", tris_strdup(""), tris_free_ptr, "app_dial");

	res = tris_register_application_xml(app, dial_exec);
	res |= tris_register_application_xml(rapp, retrydial_exec);

	return res;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Dialing Application");
