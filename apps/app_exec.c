...
static int exec_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
...
static int tryexec_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *s, *appname, *endargs, args[MAXRESULT];
	struct tris_app *app;
...
static int execif_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *truedata = NULL, *falsedata = NULL, *end, *firstcomma, *firstquestion;
	struct tris_app *app = NULL;
	TRIS_DECLARE_APP_ARGS(expr,
		TRIS_APP_ARG(expr);
		TRIS_APP_ARG(remainder);
	);
	TRIS_DECLARE_APP_ARGS(apps,
		TRIS_APP_ARG(t);
		TRIS_APP_ARG(f);
	);
	...
	if ((firstcomma != NULL && firstquestion != NULL && firstcomma < firstquestion) || (firstquestion == NULL)) {
		/* Deprecated syntax */
		TRIS_DECLARE_APP_ARGS(depr,
			TRIS_APP_ARG(expr);
			TRIS_APP_ARG(appname);
			TRIS_APP_ARG(appargs);
		);
		TRIS_STANDARD_APP_ARGS(depr, parse);

		tris_log(LOG_WARNING, "Deprecated syntax found.  Please upgrade to using ExecIf(<expr>?%s(%s))\n", depr.appname, depr.appargs);
		...
		TRIS_NONSTANDARD_APP_ARGS(expr, parse, '?');
		if (tris_strlen_zero(expr.remainder)) {
			tris_log(LOG_ERROR, "Usage: ExecIf(<expr>?<appiftrue>(<args>)[:<appiffalse>(<args)])\n");
			return -1;
		}

		TRIS_NONSTANDARD_APP_ARGS(apps, expr.remainder, ':');
	...
	if (pbx_checkcondition(expr.expr)) {
		if (!tris_strlen_zero(apps.t) && (app = pbx_findapp(apps.t))) {
			res = pbx_exec(chan, app, S_OR(truedata, ""));
		} else {
			tris_log(LOG_WARNING, "Could not find application! (%s)\n", apps.t);
			res = -1;
		}
	} else if (!tris_strlen_zero(apps.f)) {
...
static int unload_module(void)
{
	...
	res = tris_unregister_application(app_exec);
	res |= tris_unregister_application(app_tryexec);
	res |= tris_unregister_application(app_execif);
...
{
	int res = tris_register_application_xml(app_exec, exec_exec);
	res |= tris_register_application_xml(app_tryexec, tryexec_exec);
	res |= tris_register_application_xml(app_execif, execif_exec);
	return res;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Executes dialplan applications");
