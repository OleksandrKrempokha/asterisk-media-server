 ...
 * 
 * 2007-2008, Dmitry Andrianov <trismedia@dima.spb.ru>
 *
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 245732 $")
...
typedef struct {
	...
	enum tris_t38_state t38state;	/* T38 state of the channel */
	int direction;			/* Fax direction: 0 - receiving, 1 - sending */
	int caller_mode;
	char *file_name;
	struct tris_control_t38_parameters t38parameters;
...
static void span_message(int level, const char *msg)
{
	if (level == SPAN_LOG_ERROR) {
		tris_log(LOG_ERROR, "%s", msg);
	} else if (level == SPAN_LOG_WARNING) {
		tris_log(LOG_WARNING, "%s", msg);
	} else {
		tris_log(LOG_DEBUG, "%s", msg);
	}
}

static int t38_tx_packet_handler(t38_core_state_t *s, void *user_data, const uint8_t *buf, int len, int count)
{
	struct tris_channel *chan = (struct tris_channel *) user_data;

	struct tris_frame outf = {
		.frametype = TRIS_FRAME_MODEM,
		.subclass = TRIS_MODEM_T38,
		.src = __FUNCTION__,
	};

	/* TODO: Trismedia does not provide means of resending the same packet multiple
	  times so count is ignored at the moment */

	TRIS_FRAME_SET_BUFFER(&outf, buf, 0, len);

	if (tris_write(chan, &outf) < 0) {
...
static void phase_e_handler(t30_state_t *f, void *user_data, int result)
{
	...
	tris_debug(1, "Fax phase E handler. result=%d\n", result);
	...
	if (result != T30_ERR_OK) {
		...
		tris_log(LOG_WARNING, "Error transmitting fax. result=%d: %s.\n", result, t30_completion_code_to_str(result));
	...
	tris_debug(1, "Fax transmitted successfully.\n");
	tris_debug(1, "  Remote station ID: %s\n", far_ident);
	tris_debug(1, "  Pages transferred: %d\n", pages_transferred);
	tris_debug(1, "  Image resolution:  %d x %d\n", stat.x_resolution, stat.y_resolution);
	tris_debug(1, "  Transfer Rate:     %d\n", stat.bit_rate);
...
/* Setup SPAN logging according to Trismedia debug level */
...
static void *fax_generator_alloc(struct tris_channel *chan, void *params)
{
	return params;
}

static int fax_generator_generate(struct tris_channel *chan, void *data, int len, int samples)
{
	fax_state_t *fax = (fax_state_t*) data;
	uint8_t buffer[TRIS_FRIENDLY_OFFSET + MAX_SAMPLES * sizeof(uint16_t)];
	int16_t *buf = (int16_t *) (buffer + TRIS_FRIENDLY_OFFSET);
	...
	if (samples > MAX_SAMPLES) {
		tris_log(LOG_WARNING, "Only generating %d samples, where %d requested\n", MAX_SAMPLES, samples);
		samples = MAX_SAMPLES;
	}
	
	if ((len = fax_tx(fax, buf, samples)) > 0) {
		outf.samples = len;
		TRIS_FRAME_SET_BUFFER(&outf, buffer, TRIS_FRIENDLY_OFFSET, len * sizeof(int16_t));
...
struct tris_generator generator = {
...
{
	int res = -1;
	int original_read_fmt = TRIS_FORMAT_SLINEAR;
	int original_write_fmt = TRIS_FORMAT_SLINEAR;
	fax_state_t fax;
	t30_state_t *t30state;
	struct tris_frame *inf = NULL;
	int last_state = 0;
	struct timeval now, start, state_change;
	enum tris_t38_state t38_state;
	struct tris_control_t38_parameters t38_parameters = { .version = 0,
							     .max_ifp = 800,
							     .rate = TRIS_T38_RATE_14400,
							     .rate_management = TRIS_T38_RATE_MANAGEMENT_TRANSFERRED_TCF,
							     .fill_bit_removal = 1,
/*
...
*/
	...
	if (s->caller_mode == FALSE) {
		 ...
		 */
		if ((t38_state = tris_channel_get_t38_state(s->chan)) == T38_STATE_NEGOTIATED) {
			return 1;
		} else if ((t38_state != T38_STATE_UNAVAILABLE) &&
			   (t38_parameters.request_response = TRIS_T38_REQUEST_NEGOTIATE,
			    (tris_indicate_data(s->chan, TRIS_CONTROL_T38_PARAMETERS, &t38_parameters, sizeof(t38_parameters)) == 0))) {
			/* wait up to five seconds for negotiation to complete */
			unsigned int timeout = 5000;
			int ms;
			
			tris_debug(1, "Negotiating T.38 for receive on %s\n", s->chan->name);
			while (timeout > 0) {
				ms = tris_waitfor(s->chan, 1000);
				if (ms < 0) {
					tris_log(LOG_WARNING, "something bad happened while channel '%s' was polling.\n", s->chan->name);
					return -1;
				}
				if (!ms) {
					/* nothing happened */
					if (timeout > 0) {
						...
						tris_log(LOG_WARNING, "channel '%s' timed-out during the T.38 negotiation.\n", s->chan->name);
						break;
					}
				}
				if (!(inf = tris_read(s->chan))) {
					return -1;
				}
				if ((inf->frametype == TRIS_FRAME_CONTROL) &&
				    (inf->subclass == TRIS_CONTROL_T38_PARAMETERS) &&
				    (inf->datalen == sizeof(t38_parameters))) {
					struct tris_control_t38_parameters *parameters = inf->data.ptr;
					
					switch (parameters->request_response) {
					case TRIS_T38_NEGOTIATED:
						tris_debug(1, "Negotiated T.38 for receive on %s\n", s->chan->name);
						res = 1;
						break;
					case TRIS_T38_REFUSED:
						tris_log(LOG_WARNING, "channel '%s' refused to negotiate T.38\n", s->chan->name);
						break;
					default:
						tris_log(LOG_ERROR, "channel '%s' failed to negotiate T.38\n", s->chan->name);
						break;
					}
					tris_frfree(inf);
					if (res == 1) {
	...
	if (original_read_fmt != TRIS_FORMAT_SLINEAR) {
		res = tris_set_read_format(s->chan, TRIS_FORMAT_SLINEAR);
		if (res < 0) {
			tris_log(LOG_WARNING, "Unable to set to linear read mode, giving up\n");
			goto done;
		}
	}

	original_write_fmt = s->chan->writeformat;
	if (original_write_fmt != TRIS_FORMAT_SLINEAR) {
		res = tris_set_write_format(s->chan, TRIS_FORMAT_SLINEAR);
		if (res < 0) {
			tris_log(LOG_WARNING, "Unable to set to linear write mode, giving up\n");
			goto done;
		}
	}
	...
	start = state_change = tris_tvnow();

	tris_activate_generator(s->chan, &generator, &fax);

	while (!s->finished) {
		inf = NULL;

		if ((res = tris_waitfor(s->chan, 25)) < 0) {
			tris_debug(1, "Error waiting for a frame\n");
			break;
		}

		/* Watchdog */
		now = tris_tvnow();
		if (tris_tvdiff_sec(now, start) > WATCHDOG_TOTAL_TIMEOUT || tris_tvdiff_sec(now, state_change) > WATCHDOG_STATE_TIMEOUT) {
			tris_log(LOG_WARNING, "It looks like we hung. Aborting.\n");
			res = -1;
			break;
		}
		...
		if (!(inf = tris_read(s->chan))) {
			tris_debug(1, "Channel hangup\n");
			res = -1;
			break;
		}

		tris_debug(10, "frame %d/%d, len=%d\n", inf->frametype, inf->subclass, inf->datalen);

		/* Check the frame type. Format also must be checked because there is a chance
		   that a frame in old format was already queued before we set channel format
		   to slinear so it will still be received by tris_read */
		if (inf->frametype == TRIS_FRAME_VOICE && inf->subclass == TRIS_FORMAT_SLINEAR) {
			if (fax_rx(&fax, inf->data.ptr, inf->samples) < 0) {
				/* I know fax_rx never returns errors. The check here is for good style only */
				tris_log(LOG_WARNING, "fax_rx returned error\n");
				res = -1;
				break;
			}
			if (last_state != t30state->state) {
				state_change = tris_tvnow();
				last_state = t30state->state;
			}
		} else if ((inf->frametype == TRIS_FRAME_CONTROL) &&
			   (inf->subclass == TRIS_CONTROL_T38_PARAMETERS)) {
			struct tris_control_t38_parameters *parameters = inf->data.ptr;

			if (parameters->request_response == TRIS_T38_NEGOTIATED) {
				/* T38 switchover completed */
				s->t38parameters = *parameters;
				tris_debug(1, "T38 negotiated, finishing audio loop\n");
				res = 1;
				break;
			} else if (parameters->request_response == TRIS_T38_REQUEST_NEGOTIATE) {
				t38_parameters.request_response = TRIS_T38_NEGOTIATED;
				tris_debug(1, "T38 request received, accepting\n");
				/* Complete T38 switchover */
				tris_indicate_data(s->chan, TRIS_CONTROL_T38_PARAMETERS, &t38_parameters, sizeof(t38_parameters));
				/* Do not break audio loop, wait until channel driver finally acks switchover
				 * with TRIS_T38_NEGOTIATED
				 */
			}
		}
	...
	tris_debug(1, "Loop finished, res=%d\n", res);

	if (inf)
		tris_frfree(inf);

	tris_deactivate_generator(s->chan);
	...
	if (original_write_fmt != TRIS_FORMAT_SLINEAR) {
		if (tris_set_write_format(s->chan, original_write_fmt) < 0)
			tris_log(LOG_WARNING, "Unable to restore write format on '%s'\n", s->chan->name);
	}

	if (original_read_fmt != TRIS_FORMAT_SLINEAR) {
		if (tris_set_read_format(s->chan, original_read_fmt) < 0)
...
static int transmit_t38(fax_session *s)
{
	...
	if (t38_terminal_init(&t38, s->caller_mode, t38_tx_packet_handler, s->chan) == NULL) {
		tris_log(LOG_WARNING, "Unable to start T.38 termination.\n");
		res = -1;
		goto disable_t38;
	}
	...
	now = start = state_change = tris_tvnow();

	while (!s->finished) {
		inf = NULL;
		if ((res = tris_waitfor(s->chan, 20)) < 0) {
			break;
		}
		...
		if (res == 0) {
			if (tris_tvdiff_sec(now, start) > WATCHDOG_TOTAL_TIMEOUT || tris_tvdiff_sec(now, state_change) > WATCHDOG_STATE_TIMEOUT) {
				 ...
				 * again
				 */
				t38_terminal_send_timeout(&t38, tris_tvdiff_us(now, last_frame) / (1000000 / 8000));
				continue;
			}
		}
		...
		if (inf->frametype == TRIS_FRAME_MODEM && inf->subclass == TRIS_MODEM_T38) {
			t38_core_rx_ifp_packet(t38state, inf->data.ptr, inf->datalen, inf->seqno);
			if (last_state != t30state->state) {
				state_change = tris_tvnow();
				last_state = t30state->state;
			}
		} else if (inf->frametype == TRIS_FRAME_CONTROL && inf->subclass == TRIS_CONTROL_T38_PARAMETERS) {
			struct tris_control_t38_parameters *parameters = inf->data.ptr;
			if (parameters->request_response == TRIS_T38_TERMINATED) {
				tris_debug(1, "T38 down, finishing\n");
				break;
			}
		}
...
disable_t38:
	 ...
	 */
	if ((s->caller_mode == FALSE) &&
	    (tris_channel_get_t38_state(s->chan) == T38_STATE_NEGOTIATED)) {
		struct tris_control_t38_parameters t38_parameters = { .request_response = TRIS_T38_REQUEST_TERMINATE, };

		if (tris_indicate_data(s->chan, TRIS_CONTROL_T38_PARAMETERS, &t38_parameters, sizeof(t38_parameters)) == 0) {
			/* wait up to five seconds for negotiation to complete */
			unsigned int timeout = 5000;
			int ms;
			
			tris_debug(1, "Shutting down T.38 on %s\n", s->chan->name);
			while (timeout > 0) {
				ms = tris_waitfor(s->chan, 1000);
				if (ms < 0) {
					tris_log(LOG_WARNING, "something bad happened while channel '%s' was polling.\n", s->chan->name);
				...
				if (!ms) {
					...
					if (timeout > 0) {
						...
						tris_log(LOG_WARNING, "channel '%s' timed-out during the T.38 shutdown.\n", s->chan->name);
						break;
					}
				}
				if (!(inf = tris_read(s->chan))) {
				...
				if ((inf->frametype == TRIS_FRAME_CONTROL) &&
					...
					case TRIS_T38_TERMINATED:
						tris_debug(1, "Shut down T.38 on %s\n", s->chan->name);
						break;
					case TRIS_T38_REFUSED:
						tris_log(LOG_WARNING, "channel '%s' refused to disable T.38\n", s->chan->name);
						break;
					default:
						tris_log(LOG_ERROR, "channel '%s' failed to disable T.38\n", s->chan->name);
						break;
					}
					tris_frfree(inf);
					break;
...
static int transmit(fax_session *s)
{
	...
	if (s->chan->_state != TRIS_STATE_UP) {
		/* Shouldn't need this, but checking to see if channel is already answered
		 * Theoretically trismedia should already have answered before running the app */
		res = tris_answer(s->chan);
		if (res) {
	...
	s->t38state = tris_channel_get_t38_state(s->chan);
	if (s->t38state != T38_STATE_NEGOTIATED) {
		...
		if (res > 0) {
			...
			s->t38state = tris_channel_get_t38_state(s->chan);
			if (s->t38state != T38_STATE_NEGOTIATED) {
				tris_log(LOG_ERROR, "Audio loop reports T38 switchover but t38state != T38_STATE_NEGOTIATED\n");
			}
		}
	}
	...
	if (res) {
		tris_log(LOG_WARNING, "Transmission error\n");
		res = -1;
	} else if (s->finished < 0) {
		tris_log(LOG_WARNING, "Transmission failed\n");
	} else if (s->finished > 0) {
		tris_debug(1, "Transmission finished Ok\n");
	}
...
static int sndfax_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *parse;
	...
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(file_name);
		TRIS_APP_ARG(options);
	);

	if (chan == NULL) {
		tris_log(LOG_ERROR, "Fax channel is NULL. Giving up.\n");
		return -1;
	}
	...
	if (tris_strlen_zero(data)) {
		...
		tris_log(LOG_ERROR, "SendFAX requires an argument (filename)\n");
		return -1;
	}
...
static int rcvfax_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *parse;
	fax_session session;
	...
	if (tris_strlen_zero(data)) {
		/* No data implies no filename or anything is present */
		tris_log(LOG_ERROR, "ReceiveFAX requires an argument (filename)\n");
		return -1;
	}
...
static int unload_module(void)
{
	...
	res = tris_unregister_application(app_sndfax_name);	
	res |= tris_unregister_application(app_rcvfax_name);	
...
static int load_module(void)
{
	...
	res = tris_register_application_xml(app_sndfax_name, sndfax_exec);
	res |= tris_register_application_xml(app_rcvfax_name, rcvfax_exec);
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Simple FAX Application",
		.load = load_module,
