...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 153543 $")
...
static int flash_exec(struct tris_channel *chan, void *data)
{
	int res = -1;
	int x;
	...
	if (strcasecmp(chan->tech->type, "DAHDI")) {
		tris_log(LOG_WARNING, "%s is not a DAHDI channel\n", chan->name);
		return -1;
	}
	...
	if (!res) {
		if (dahdip.sigtype & __DAHDI_SIG_FXS) {
			...
			if (!res || (errno == EINPROGRESS)) {
				...
				tris_verb(3, "Flashed channel %s\n", chan->name);
			} else
				tris_log(LOG_WARNING, "Unable to flash channel %s: %s\n", chan->name, strerror(errno));
		} else
			tris_log(LOG_WARNING, "%s is not an FXO Channel\n", chan->name);
	} else
		tris_log(LOG_WARNING, "Unable to get parameters of %s: %s\n", chan->name, strerror(errno));
...
{
	return tris_register_application_xml(app, flash_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Flash channel application");
