...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 209594 $")
...
/*** DOCUMENTATION
	<application name="ForkCDR" language="en_US">
		...
		<description>
			...
			will be affected by events transpiring within Trismedia, with the previously noted exceptions.</para>
		</description>
		<see-also>
...
TRIS_APP_OPTIONS(forkcdr_exec_options, {
	TRIS_APP_OPTION('a', OPT_SETANS),
	TRIS_APP_OPTION('A', OPT_ANSLOCK),
	TRIS_APP_OPTION('d', OPT_SETDISP),
	TRIS_APP_OPTION('D', OPT_RESETDEST),
	TRIS_APP_OPTION('e', OPT_ENDCDR),
	TRIS_APP_OPTION('R', OPT_NORESET),
	TRIS_APP_OPTION_ARG('s', OPT_VARSET, OPT_ARG_VARSET),
	TRIS_APP_OPTION('T', OPT_DONTOUCH),
	TRIS_APP_OPTION('v', OPT_KEEPVARS),
});

static void tris_cdr_fork(struct tris_channel *chan, struct tris_flags optflags, char *set) 
{
	struct tris_cdr *cdr;
	struct tris_cdr *newcdr;
	struct tris_flags flags = { TRIS_CDR_FLAG_KEEP_VARS };
	...
	if (!(newcdr = tris_cdr_dup(cdr)))
		return;
	
	tris_cdr_append(cdr, newcdr);

	if (!tris_test_flag(&optflags, OPT_NORESET))
		tris_cdr_reset(newcdr, &flags);
		
	if (!tris_test_flag(cdr, TRIS_CDR_FLAG_KEEP_VARS))
		tris_cdr_free_vars(cdr, 0);
	
	if (!tris_strlen_zero(set)) {
		char *varname = tris_strdupa(set), *varval;
		varval = strchr(varname,'=');
		if (varval) {
			*varval = 0;
			varval++;
			tris_cdr_setvar(cdr, varname, varval, 0);
		}
	}
	
	if (tris_test_flag(&optflags, OPT_SETANS) && !tris_tvzero(cdr->answer))
		newcdr->answer = newcdr->start;

	if (tris_test_flag(&optflags, OPT_SETDISP))
		newcdr->disposition = cdr->disposition;
	
	if (tris_test_flag(&optflags, OPT_RESETDEST))
		newcdr->dstchannel[0] = 0;
	
	if (tris_test_flag(&optflags, OPT_ENDCDR))
		tris_cdr_end(cdr);

	if (tris_test_flag(&optflags, OPT_ANSLOCK))
		tris_set_flag(cdr, TRIS_CDR_FLAG_ANSLOCKED);
	
	if (tris_test_flag(&optflags, OPT_DONTOUCH))
		tris_set_flag(cdr, TRIS_CDR_FLAG_DONT_TOUCH);
		
	tris_set_flag(cdr, TRIS_CDR_FLAG_CHILD | TRIS_CDR_FLAG_LOCKED);
}

static int forkcdr_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *argcopy = NULL;
	...
	if (!chan->cdr) {
		tris_log(LOG_WARNING, "Channel does not have a CDR\n");
		return 0;
	}
	...
	if (!tris_strlen_zero(arglist.options))
		tris_app_parse_options(forkcdr_exec_options, &flags, opts, arglist.options);

	if (!tris_strlen_zero(data)) {
		int keepvars = tris_test_flag(&flags, OPT_KEEPVARS) ? 1 : 0;
		tris_set2_flag(chan->cdr, keepvars, TRIS_CDR_FLAG_KEEP_VARS);
	}
	
	tris_cdr_fork(chan, flags, opts[OPT_ARG_VARSET]);
...
{
	return tris_register_application_xml(app, forkcdr_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Fork The CDR into 2 separate entities");
