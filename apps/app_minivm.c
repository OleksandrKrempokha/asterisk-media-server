...
/*! \file
 *
 * \brief MiniVoiceMail - A Minimal Voicemail System for Trismedia
 *
 ...
 *
 * \page App_minivm	Trismedia Mini-voicemail - A minimal voicemail system
 *	
 ...
 *	\note Don't use language codes like the rest of Trismedia, two letter countrycode. Use
 ...
 */
/*! \page Minivm_directories Trismedia Mini-Voicemail Directory structure
 *
 *	The directory structure for storing voicemail
 *		- TRIS_SPOOL_DIR - usually /var/spool/trismedia (configurable in trismedia.conf)
 *		- MVM_SPOOL_DIR - should be configurable, usually TRIS_SPOOL_DIR/voicemail
 ...
 *		\b /var/spool/trismedia/voicemail/localdomain.xx/anita
 *
...
/*! \page App_minivm_todo Trismedia Mini-Voicemail - todo
 *	- configure accounts from AMI?
 *	- test, test, test, test
 *	- fix "voicemail/vm-theextensionis.gsm" voiceprompt from Allison in various formats
...
#include "trismedia/paths.h"	/* use various paths */
...
#define SOUND_INTRO		"voicemail/vm-intro"
...
#define TRISMEDIA_USERNAME "trismedia"	/*!< Default username for sending mail is trismedia\@localhost */
...
TRIS_APP_OPTIONS(minivm_app_options, {
...
TRIS_APP_OPTIONS(minivm_accmess_options, {
	TRIS_APP_OPTION('b', OPT_BUSY_GREETING),
	TRIS_APP_OPTION('u', OPT_UNAVAIL_GREETING),
	TRIS_APP_OPTION('t', OPT_TEMP_GREETING),
	TRIS_APP_OPTION('n', OPT_NAME_GREETING),
});

/*! \brief Structure for linked list of Mini-Voicemail users: \ref minivm_accounts */
struct minivm_account {
	char username[TRIS_MAX_CONTEXT];	/*!< Mailbox username */
	char domain[TRIS_MAX_CONTEXT];	/*!< Voicemail domain */
	...
	char accountcode[TRIS_MAX_ACCOUNT_CODE];	/*!< Voicemail account account code */
	...
	struct tris_variable *chanvars;	/*!< Variables for e-mail template */
	double volgain;			/*!< Volume gain for voicemails sent via e-mail */
	TRIS_LIST_ENTRY(minivm_account) list;	
};

/*! \brief The list of e-mail accounts */
static TRIS_LIST_HEAD_STATIC(minivm_accounts, minivm_account);
...
struct minivm_template {
	...
	TRIS_LIST_ENTRY(minivm_template) list;	/*!< List mechanics */
};

/*! \brief The list of e-mail templates */
static TRIS_LIST_HEAD_STATIC(message_templates, minivm_template);
...
struct minivm_zone {
	...
	TRIS_LIST_ENTRY(minivm_zone) list;	/*!< List mechanics */
};

/*! \brief The list of e-mail time zones */
static TRIS_LIST_HEAD_STATIC(minivm_zones, minivm_zone);
...
TRIS_MUTEX_DEFINE_STATIC(minivmlock);	/*!< Lock to protect voicemail system */
TRIS_MUTEX_DEFINE_STATIC(minivmloglock);	/*!< Lock to protect voicemail system log file */
...
static struct tris_flags globalflags = {0};	/*!< Global voicemail flags */
...
static int create_vmaccount(char *name, struct tris_variable *var, int realtime);
static struct minivm_account *find_user_realtime(const char *domain, const char *username);
static char *handle_minivm_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
static struct minivm_template *message_template_create(const char *name)
{
	...
	template = tris_calloc(1, sizeof(*template));
	if (!template)
		return NULL;

	/* Set some defaults for templates */
	tris_copy_string(template->name, name, sizeof(template->name));
	tris_copy_string(template->dateformat, DEFAULT_DATEFORMAT, sizeof(template->dateformat));
	tris_copy_string(template->charset, DEFAULT_CHARSET, sizeof(template->charset));
	tris_copy_string(template->subject, "New message in mailbox ${MVM_USERNAME}@${MVM_DOMAIN}", sizeof(template->subject));
...
{
	if (template->body)
		tris_free(template->body);

	tris_free (template);
}

/*! \brief Build message template from configuration */
static int message_template_build(const char *name, struct tris_variable *var)
{
	struct minivm_template *template;
	int error = 0;
	...
	if (!template) {
		tris_log(LOG_ERROR, "Out of memory, can't allocate message template object %s.\n", name);
		return -1;
	}

	while (var) {
		tris_debug(3, "Configuring template option %s = \"%s\" for template %s\n", var->name, var->value, name);
		if (!strcasecmp(var->name, "fromaddress")) {
			tris_copy_string(template->fromaddress, var->value, sizeof(template->fromaddress));
		} else if (!strcasecmp(var->name, "fromemail")) {
			tris_copy_string(template->serveremail, var->value, sizeof(template->serveremail));
		} else if (!strcasecmp(var->name, "subject")) {
			tris_copy_string(template->subject, var->value, sizeof(template->subject));
		} else if (!strcasecmp(var->name, "locale")) {
			tris_copy_string(template->locale, var->value, sizeof(template->locale));
		} else if (!strcasecmp(var->name, "attachmedia")) {
			template->attachment = tris_true(var->value);
		} else if (!strcasecmp(var->name, "dateformat")) {
			tris_copy_string(template->dateformat, var->value, sizeof(template->dateformat));
		} else if (!strcasecmp(var->name, "charset")) {
			tris_copy_string(template->charset, var->value, sizeof(template->charset));
		} else if (!strcasecmp(var->name, "templatefile")) {
			if (template->body) 
				tris_free(template->body);
			template->body = message_template_parse_filebody(var->value);
			if (!template->body) {
				tris_log(LOG_ERROR, "Error reading message body definition file %s\n", var->value);
				error++;
			}
			...
			if (!template->body) {
				tris_log(LOG_ERROR, "Error parsing message body definition:\n          %s\n", var->value);
				error++;
			}
		} else {
			tris_log(LOG_ERROR, "Unknown message template configuration option \"%s=%s\"\n", var->name, var->value);
			error++;
		}
		var = var->next;
	}
	if (error)
		tris_log(LOG_ERROR, "-- %d errors found parsing message template definition %s\n", error, name);

	TRIS_LIST_LOCK(&message_templates);
	TRIS_LIST_INSERT_TAIL(&message_templates, template, list);
	TRIS_LIST_UNLOCK(&message_templates);
...
static struct minivm_template *message_template_find(const char *name)
{
	...
	TRIS_LIST_LOCK(&message_templates);
	TRIS_LIST_TRAVERSE(&message_templates, this, list) {
...
static void message_destroy_list(void)
{
	...
	TRIS_LIST_LOCK(&message_templates);
	while ((this = TRIS_LIST_REMOVE_HEAD(&message_templates, list))) 
...
static int get_date(char *s, int len)
{
	...
	return tris_strftime(s, len, "%a %b %e %r %Z %Y", &tm);
}
...
{
	if (vmu->chanvars)
		tris_variables_destroy(vmu->chanvars);
	tris_free(vmu);
}
...
/*! \brief Prepare for voicemail template by adding channel variables 
	to the channel
*/
static void prep_email_sub_vars(struct tris_channel *channel, const struct minivm_account *vmu, const char *cidnum, const char *cidname, const char *dur, const char *date, const char *counter)
{
	char callerid[256];
	struct tris_variable *var;
	
	if (!channel) {
		tris_log(LOG_ERROR, "No allocated channel, giving up...\n");
		return;
	}
	...
	pbx_builtin_setvar_helper(channel, "MVM_CALLERID", tris_callerid_merge(callerid, sizeof(callerid), cidname, cidnum, "Unknown Caller"));
...
static void populate_defaults(struct minivm_account *vmu)
{
	...
	tris_copy_string(vmu->attachfmt, default_vmformat, sizeof(vmu->attachfmt));
...
static void vmaccounts_destroy_list(void)
{
	...
	TRIS_LIST_LOCK(&minivm_accounts);
	while ((this = TRIS_LIST_REMOVE_HEAD(&minivm_accounts, list))) 
		tris_free(this);
	TRIS_LIST_UNLOCK(&minivm_accounts);
}
...
static struct minivm_account *find_account(const char *domain, const char *username, int createtemp)
{
	...
	if (tris_strlen_zero(domain) || tris_strlen_zero(username)) {
		tris_log(LOG_NOTICE, "No username or domain? \n");
		return NULL;
	}
	tris_debug(3, "Looking for voicemail user %s in domain %s\n", username, domain);

	TRIS_LIST_LOCK(&minivm_accounts);
	TRIS_LIST_TRAVERSE(&minivm_accounts, cur, list) {
	...
	if (cur) {
		tris_debug(3, "Found account for %s@%s\n", username, domain);
		vmu = cur;
	...
	if (createtemp && !vmu) {
		...
		vmu = mvm_user_alloc();
		tris_set2_flag(vmu, TRUE, MVM_ALLOCED);	
		if (vmu) {
			tris_copy_string(vmu->username, username, sizeof(vmu->username));
			tris_copy_string(vmu->domain, domain, sizeof(vmu->domain));
			tris_debug(1, "Created temporary account\n");
		}
...
static struct minivm_account *find_user_realtime(const char *domain, const char *username)
{
	...
	if (username) 
		tris_copy_string(retval->username, username, sizeof(retval->username));

	populate_defaults(retval);
	var = tris_load_realtime("minivm", "username", username, "domain", domain, SENTINEL);
...
static int sendmail(struct minivm_template *template, struct minivm_account *vmu, char *cidnum, char *cidname, const char *filename, char *format, int duration, int attach_user_voicemail, enum mvm_messagetype type, const char *counter)
{
	...
	if (type == MVM_MESSAGE_EMAIL) {
		if (vmu && !tris_strlen_zero(vmu->email)) {
			tris_copy_string(email, vmu->email, sizeof(email));	
		} else if (!tris_strlen_zero(vmu->username) && !tris_strlen_zero(vmu->domain))
			snprintf(email, sizeof(email), "%s@%s", vmu->username, vmu->domain);
	} else if (type == MVM_MESSAGE_PAGE) {
		tris_copy_string(email, vmu->pager, sizeof(email));
	}

	if (tris_strlen_zero(email)) {
		tris_log(LOG_WARNING, "No address to send message to.\n");
		return -1;	
	}

	tris_debug(3, "Sending mail to %s@%s - Using template %s\n", vmu->username, vmu->domain, template->name);
	...
	if (template->attachment)
		tris_debug(1, "Attaching file '%s', format '%s', uservm is '%d'\n", finalfilename, format, attach_user_voicemail);
	...
	if (pfd > -1) {
		...
		tris_debug(1, "Opening temp file for e-mail: %s\n", tmp);
	}
	if (!p) {
		tris_log(LOG_WARNING, "Unable to open temporary file '%s'\n", tmp);
		return -1;
	}
	/* Allocate channel used for chanvar substitution */
	ast = tris_channel_alloc(0, TRIS_STATE_DOWN, 0, 0, "", "", "", 0, "%s", "");
	...
	if (!tris_strlen_zero(vmu->zonetag)) {
		...
		TRIS_LIST_LOCK(&minivm_zones);
		TRIS_LIST_TRAVERSE(&minivm_zones, z, list) {
			if (strcmp(z->name, vmu->zonetag)) 
				continue;
			the_zone = z;
		}
		TRIS_LIST_UNLOCK(&minivm_zones);
	}

	now = tris_tvnow();
	tris_localtime(&now, &tm, the_zone ? the_zone->timezone : NULL);
	tris_strftime(date, sizeof(date), "%a, %d %b %Y %H:%M:%S %z", &tm);

	/* Start printing the email to the temporary file */
	fprintf(p, "Date: %s\n", date);

	/* Set date format for voicemail mail */
	tris_strftime(date, sizeof(date), template->dateformat, &tm);
	...
	fromemail = tris_strlen_zero(vmu->serveremail) ?  template->serveremail : vmu->serveremail;

	/* Find name to user for server e-mail */
	fromaddress = tris_strlen_zero(template->fromaddress) ? "" : template->fromaddress;

	/* If needed, add hostname as domain */
	if (tris_strlen_zero(fromemail))
		fromemail = "trismedia";

	if (strchr(fromemail, '@'))
		tris_copy_string(who, fromemail, sizeof(who));
	else  {
	...
	if (tris_strlen_zero(fromaddress)) {
		fprintf(p, "From: Trismedia <%s>\n", who);
	} else {
		/* Allocate a buffer big enough for variable substitution */
		int vmlen = strlen(fromaddress) * 3 + 200;

		tris_debug(4, "Fromaddress template: %s\n", fromaddress);
		if ((passdata = alloca(vmlen))) {
			...
			tris_log(LOG_WARNING, "Cannot allocate workspace for variable substitution\n");
			fclose(p);
			return -1;	
		}
	} 
	tris_debug(4, "Fromstring now: %s\n", tris_strlen_zero(passdata) ? "-default-" : passdata);

	fprintf(p, "Message-ID: <Trismedia-%d-%s-%d-%s>\n", (unsigned int)tris_random(), vmu->username, (int)getpid(), who);
	...
	if (!tris_strlen_zero(template->subject)) {
		char *pass_data;
		...
		tris_debug(4, "Subject now: %s\n", pass_data);

	} else  {
		fprintf(p, "Subject: New message in mailbox %s@%s\n", vmu->username, vmu->domain);
		tris_debug(1, "Using default subject for this email \n");
	}


	if (option_debug > 2)
		fprintf(p, "X-Trismedia-debug: template %s user account %s@%s\n", template->name, vmu->username, vmu->domain);
	fprintf(p, "MIME-Version: 1.0\n");

	/* Something unique. */
	snprintf(bound, sizeof(bound), "voicemail_%s%d%d", vmu->username, (int)getpid(), (unsigned int)tris_random());

	fprintf(p, "Content-Type: multipart/mixed; boundary=\"%s\"\n\n\n", bound);

	fprintf(p, "--%s\n", bound);
	fprintf(p, "Content-Type: text/plain; charset=%s\nContent-Transfer-Encoding: 8bit\n\n", global_charset);
	if (!tris_strlen_zero(template->body)) {
		char *pass_data;
		int vmlen = strlen(template->body)*3 + 200;
		if ((pass_data = alloca(vmlen))) {
			pbx_substitute_variables_helper(ast, template->body, pass_data, vmlen);
			tris_debug(3, "Message now: %s\n-----\n", pass_data);
			fprintf(p, "%s\n", pass_data);
		} else
			tris_log(LOG_WARNING, "Cannot allocate workspace for variable substitution\n");
	} else {
		...
		tris_debug(3, "Using default message body (no template)\n-----\n");
	}
	/* Eww. We want formats to tell us their own MIME type */
	if (template->attachment) {
		char *ctype = "audio/x-";
		tris_debug(3, "Attaching file to message: %s\n", fname);
	...
	tris_debug(1, "Sent message to %s with command '%s' - %s\n", vmu->email, global_mailcmd, template->attachment ? "(media attachment)" : "");
	tris_debug(3, "Actual command used: %s\n", tmp2);
	if (ast)
...
{
	return snprintf(dest, len, "%s%s/%s%s%s", MVM_SPOOL_DIR, domain, username, tris_strlen_zero(folder) ? "" : "/", folder ? folder : "");
}
...
static int create_dirpath(char *dest, int len, char *domain, char *username, char *folder)
{
	...
	if ((res = tris_mkdir(dest, 0777))) {
		tris_log(LOG_WARNING, "tris_mkdir '%s' failed: %s\n", dest, strerror(res));
		return -1;
	}
	tris_debug(2, "Creating directory for %s@%s folder %s : %s\n", username, domain, folder, dest);
	return 0;
}
...
*/
static int invent_message(struct tris_channel *chan, char *domain, char *username, int busy, char *ecodes)
{
	int res;
	char fn[PATH_MAX];

	tris_debug(2, "Still preparing to play message ...\n");

	snprintf(fn, sizeof(fn), "%s%s/%s/greet", MVM_SPOOL_DIR, domain, username);

	if (tris_fileexists(fn, NULL, NULL) > 0) {
		...
		tris_debug(2, "No personal prompts. Using default prompt set for language\n");
		
		while (*i)  {
			tris_debug(2, "Numeric? Checking %c\n", *i);
			if (!isdigit(*i)) {
		...
		if (numericusername) {
			if(tris_streamfile(chan, "voicemail/vm-theperson", chan->language))
				return -1;
			if ((res = tris_waitstream(chan, ecodes)))
				return res;
	
			res = tris_say_digit_str(chan, username, ecodes, chan->language);
			if (res)
				return res;
		} else {
			if(tris_streamfile(chan, "voicemail/vm-theextensionis", chan->language))
				return -1;
			if ((res = tris_waitstream(chan, ecodes)))
				return res;
		}
	...
	res = tris_streamfile(chan, busy ? "voicemail/vm-isonphone" : "voicemail/vm-isunavail", chan->language);
	if (res)
		return -1;
	res = tris_waitstream(chan, ecodes);
	return res;
...
static int vm_delete(char *file)
{
	...
	tris_debug(1, "Deleting voicemail file %s\n", file);

	res = unlink(file);	/* Remove the meta data file */
	res |=  tris_filedelete(file, NULL);	/* remove the media file */
	return res;
}


/*! \brief Record voicemail message & let caller review or re-record it, or set options if applicable */
static int play_record_review(struct tris_channel *chan, char *playfile, char *recordfile, int maxtime, char *fmt,
			      ...
			      signed char record_gain)
{
	...
	while ((cmd >= 0) && (cmd != 't')) {
		...
		case '1':
 		...
 		case '2':
			...
			tris_verb(3, "Reviewing the message\n");
 			tris_streamfile(chan, recordfile, chan->language);
 			cmd = tris_waitstream(chan, TRIS_DIGIT_ANY);
 			break;
 		case '3':
 			message_exists = 0;
 			/* Record */
			...
			if (tris_test_flag(vmu, MVM_OPERATOR))
				canceldtmf = "0";
			cmd = tris_play_and_record_full(chan, playfile, recordfile, maxtime, fmt, duration, global_silencethreshold, global_maxsilence, unlockdir, acceptdtmf, canceldtmf);
			if (record_gain)
		...
		case '#':
 		...
 		default:
			/* If the caller is an ouside caller, and the review option is enabled,
			   ...
			   their OGM's */
			if (outsidecaller && !tris_test_flag(vmu, MVM_REVIEW))
				return cmd;
 			if (message_exists) {
 				cmd = tris_play_and_wait(chan, "voicemail/vm-review");
 			} else {
 				cmd = tris_play_and_wait(chan, "voicemail/vm-torerecord");
 				if (!cmd)
 					cmd = tris_waitfordigit(chan, 600);
 			}
 			
 			if (!cmd && outsidecaller && tris_test_flag(vmu, MVM_OPERATOR)) {
 	...
 	if (outsidecaller)  
		tris_play_and_wait(chan, "voicemail/vm-goodbye");
 	if (cmd == 't')
 		cmd = 0;
 	return cmd;
}
...
static void run_externnotify(struct tris_channel *chan, struct minivm_account *vmu)
{
	char arguments[BUFSIZ];

	if (tris_strlen_zero(vmu->externnotify) && tris_strlen_zero(global_externnotify))
		return;

	snprintf(arguments, sizeof(arguments), "%s %s@%s %s %s&", 
		tris_strlen_zero(vmu->externnotify) ? global_externnotify : vmu->externnotify, 
		vmu->username, vmu->domain,
...
static int notify_new_message(struct tris_channel *chan, const char *templatename, struct minivm_account *vmu, const char *filename, long duration, const char *format, char *cidnum, char *cidname)
{
	char *stringp;
	...
	stringp = messageformat = tris_strdupa(format);
	strsep(&stringp, "|");

	if (!tris_strlen_zero(etemplate->locale)) {
		char *new_locale;
		tris_copy_string(oldlocale, setlocale(LC_TIME, NULL), sizeof(oldlocale));
		tris_debug(2, "Changing locale from %s to %s\n", oldlocale, etemplate->locale);
		new_locale = setlocale(LC_TIME, etemplate->locale);
		if (new_locale == NULL) {
			tris_log(LOG_WARNING, "-_-_- Changing to new locale did not work. Locale: %s\n", etemplate->locale);
		}
	}
...
static int leave_voicemail(struct tris_channel *chan, char *username, struct leave_vm_options *options)
{
	...
	tris_copy_string(tmp, username, sizeof(tmp));
	username = tmp;
	domain = strchr(tmp, '@');
	if (domain) {
		*domain = '\0';
	...
	if (!(vmu = find_account(domain, username, TRUE))) {
		...
		tris_log(LOG_ERROR, "Can't allocate temporary account for '%s@%s'\n", username, domain);
	...
	if (strcmp(vmu->domain, "localhost"))
		...
		tris_copy_string(ext_context, vmu->domain, sizeof(ext_context));

	/* The meat of recording the message...  All the announcements and beeps have been played*/
	if (tris_strlen_zero(vmu->attachfmt))
		tris_copy_string(fmt, default_vmformat, sizeof(fmt));
	else
		tris_copy_string(fmt, vmu->attachfmt, sizeof(fmt));

	if (tris_strlen_zero(fmt)) {
		tris_log(LOG_WARNING, "No format for saving voicemail? Default %s\n", default_vmformat);
		pbx_builtin_setvar_helper(chan, "MVM_RECORD_STATUS", "FAILED");
		return res;
	}
	msgnum = 0;
	...
	if (!userdir) {
		...
		tris_debug(3, "Creating temporary directory %s\n", tmpdir);
	}
	...
	if (txtdes < 0) {
		tris_log(LOG_ERROR, "Unable to create message file %s: %s\n", tmptxtfile, strerror(errno));
	...
	if (!txt) {
		...
		tris_localtime(&now, &tm, NULL);
		tris_strftime(timebuf, sizeof(timebuf), "%H:%M:%S", &tm);
		...
		snprintf(logbuf, sizeof(logbuf),
			...
			chan->priority,
			chan->name,
			tris_callerid_merge(callerid, sizeof(callerid), chan->cid.cid_name, chan->cid.cid_num, "Unknown"),
			date, 
			timebuf,
			duration,
		...
		if (minivmlogfile) {
			tris_mutex_lock(&minivmloglock);
			fprintf(minivmlogfile, "%s", logbuf);
			tris_mutex_unlock(&minivmloglock);
		}

		if (duration < global_vmminmessage) {
			tris_verb(3, "Recording was %d seconds long but needs to be at least %d - abandoning\n", duration, global_vmminmessage);
			fclose(txt);
			tris_filedelete(tmptxtfile, NULL);
			unlink(tmptxtfile);
			pbx_builtin_setvar_helper(chan, "MVM_RECORD_STATUS", "FAILED");
			return 0;
		} 
		fclose(txt); /* Close log file */
		if (tris_fileexists(tmptxtfile, NULL, NULL) <= 0) {
			tris_debug(1, "The recorded media file is gone, so we should remove the .txt file too!\n");
			unlink(tmptxtfile);
			pbx_builtin_setvar_helper(chan, "MVM_RECORD_STATUS", "FAILED");
			if(tris_test_flag(vmu, MVM_ALLOCED))
				free_user(vmu);
			return 0;
		}
	...
	global_stats.lastreceived = tris_tvnow();
	global_stats.receivedmessages++;
//	/* Go ahead and delete audio files from system, they're not needed any more */
//	if (tris_fileexists(tmptxtfile, NULL, NULL) <= 0) {
//		tris_filedelete(tmptxtfile, NULL);
//		 /* Even not being used at the moment, it's better to convert tris_log to tris_debug anyway */
//		tris_debug(2, "-_-_- Deleted audio file after notification :: %s \n", tmptxtfile);
//	}
...
static void queue_mwi_event(const char *mbx, const char *ctx, int urgent, int new, int old)
{
	...
	mailbox = tris_strdupa(mbx);
	context = tris_strdupa(ctx);
	...
	if (!(event = tris_event_new(TRIS_EVENT_MWI,
			...
			TRIS_EVENT_IE_NEWMSGS, TRIS_EVENT_IE_PLTYPE_UINT, (new+urgent),
...
/*! \brief Send MWI using interal Trismedia event subsystem */
static int minivm_mwi_exec(struct tris_channel *chan, void *data)
{
	int argc;
	char *argv[4];
	int res = 0;
	...
	if (tris_strlen_zero(data))  {
                tris_log(LOG_ERROR, "Minivm needs at least an account argument \n");
                return -1;
        }
        tmpptr = tris_strdupa((char *)data);
        if (!tmpptr) {
                tris_log(LOG_ERROR, "Out of memory\n");
                return -1;
        }
        argc = tris_app_separate_args(tmpptr, ',', argv, ARRAY_LEN(argv));
	if (argc < 4) {
		tris_log(LOG_ERROR, "%d arguments passed to MiniVM_MWI, need 4.\n", argc);
		return -1;
	}
        tris_copy_string(tmp, argv[0], sizeof(tmp));
        mailbox = tmp;
        domain = strchr(tmp, '@');
        if (domain) {
                *domain = '\0';
        ...
        if (tris_strlen_zero(domain) || tris_strlen_zero(mailbox)) {
                tris_log(LOG_ERROR, "Need mailbox@context as argument. Sorry. Argument 0 %s\n", argv[0]);
                return -1;
        }
...
static int minivm_notify_exec(struct tris_channel *chan, void *data)
{
	int argc;
	char *argv[2];
	int res = 0;
	...
	if (argc == 2 && !tris_strlen_zero(argv[1]))
		template = argv[1];
	...
	if (tris_strlen_zero(domain) || tris_strlen_zero(username)) {
		tris_log(LOG_ERROR, "Need username@domain as argument. Sorry. Argument 0 %s\n", argv[0]);
		return -1;
	}

	if(!(vmu = find_account(domain, username, TRUE))) {
		/* We could not find user, let's exit */
		tris_log(LOG_WARNING, "Could not allocate temporary memory for '%s@%s'\n", username, domain);
	...
	if ((filename = pbx_builtin_getvar_helper(chan, "MVM_FILENAME"))) {
		filename = tris_strdupa(filename);
	}
	tris_channel_unlock(chan);
	...
	if (!tris_strlen_zero(filename)) {
		...
		if ((format = pbx_builtin_getvar_helper(chan, "MVM_FORMAT"))) {
			format = tris_strdupa(format);
		}
		if ((duration_string = pbx_builtin_getvar_helper(chan, "MVM_DURATION"))) {
			duration_string = tris_strdupa(duration_string);
		}
		tris_channel_unlock(chan);
...
static int minivm_record_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *tmp;
	...
	tmp = tris_strdupa((char *)data);
	if (!tmp) {
		tris_log(LOG_ERROR, "Out of memory\n");
		return -1;
	}
	argc = tris_app_separate_args(tmp, ',', argv, ARRAY_LEN(argv));
	if (argc == 2) {
		if (tris_app_parse_options(minivm_app_options, &flags, opts, argv[1])) {
			return -1;
		}
		tris_copy_flags(&leave_options, &flags, OPT_SILENT | OPT_BUSY_GREETING | OPT_UNAVAIL_GREETING );
...
static int minivm_greet_exec(struct tris_channel *chan, void *data)
{
	...
	if (tris_strlen_zero(domain) || tris_strlen_zero(username)) {
		tris_log(LOG_ERROR, "Need username@domain as argument. Sorry. Argument:  %s\n", argv[0]);
		return -1;
	}
	tris_debug(1, "Trying to find configuration for user %s in domain %s\n", username, domain);

	if (!(vmu = find_account(domain, username, TRUE))) {
		tris_log(LOG_ERROR, "Could not allocate memory. \n");
		return -1;
	}
	...
	if (tris_test_flag(&leave_options, OPT_BUSY_GREETING)) {
	...
	} else if (tris_test_flag(&leave_options, OPT_UNAVAIL_GREETING)) {
	...
	if (!(res = check_dirpath(dest, sizeof(dest), vmu->domain, username, "temp"))) {
		tris_debug(2, "Temporary message directory does not exist, using default (%s)\n", tempfile);
		tris_copy_string(prefile, tempfile, sizeof(prefile));
	}
	tris_debug(2, "Preparing to play message ...\n");

	/* Check current or macro-calling context for special extensions */
	if (tris_test_flag(vmu, MVM_OPERATOR)) {
		if (!tris_strlen_zero(vmu->exit)) {
			if (tris_exists_extension(chan, vmu->exit, "o", 1, chan->cid.cid_num)) {
				strncat(ecodes, "0", sizeof(ecodes) - strlen(ecodes) - 1);
				ouseexten = 1;
			}
		} else if (tris_exists_extension(chan, chan->context, "o", 1, chan->cid.cid_num)) {
			strncat(ecodes, "0", sizeof(ecodes) - strlen(ecodes) - 1);
			ouseexten = 1;
		}
		else if (!tris_strlen_zero(chan->macrocontext) && tris_exists_extension(chan, chan->macrocontext, "o", 1, chan->cid.cid_num)) {
			strncat(ecodes, "0", sizeof(ecodes) - strlen(ecodes) - 1);
			ousemacro = 1;
		}
	}
	...
	if (!tris_strlen_zero(vmu->exit)) {
		if (tris_exists_extension(chan, vmu->exit, "a", 1, chan->cid.cid_num))
			strncat(ecodes, "*", sizeof(ecodes) -  strlen(ecodes) - 1);
	} else if (tris_exists_extension(chan, chan->context, "a", 1, chan->cid.cid_num))
		strncat(ecodes, "*", sizeof(ecodes) -  strlen(ecodes) - 1);
	else if (!tris_strlen_zero(chan->macrocontext) && tris_exists_extension(chan, chan->macrocontext, "a", 1, chan->cid.cid_num)) {
		strncat(ecodes, "*", sizeof(ecodes) -  strlen(ecodes) - 1);
		ausemacro = 1;
	}
	...
	if (!tris_strlen_zero(prefile)) {
		if (tris_streamfile(chan, prefile, chan->language) > -1) 
			res = tris_waitstream(chan, ecodes);
	} else {
		tris_debug(2, "%s doesn't exist, doing what we can\n", prefile);
		res = invent_message(chan, vmu->domain, username, tris_test_flag(&leave_options, OPT_BUSY_GREETING), ecodes);
	}
	if (res < 0) {
		tris_debug(2, "Hang up during prefile playback\n");
	...
	if (res == '#') {
		...
		tris_set_flag(&leave_options, OPT_SILENT);
		res = 0;
	}
	if (!res && !tris_test_flag(&leave_options, OPT_SILENT)) {
		res = tris_streamfile(chan, SOUND_INTRO, chan->language);
		if (!res)
			res = tris_waitstream(chan, ecodes);
		if (res == '#') {
			tris_set_flag(&leave_options, OPT_SILENT);
	...
	if (res == '*') {
		...
		chan->exten[1] = '\0';
		if (!tris_strlen_zero(vmu->exit)) {
			tris_copy_string(chan->context, vmu->exit, sizeof(chan->context));
		} else if (ausemacro && !tris_strlen_zero(chan->macrocontext)) {
			tris_copy_string(chan->context, chan->macrocontext, sizeof(chan->context));
		}
		chan->priority = 0;
		...
		if(ouseexten || ousemacro) {
			...
			if (!tris_strlen_zero(vmu->exit)) {
				tris_copy_string(chan->context, vmu->exit, sizeof(chan->context));
			} else if (ousemacro && !tris_strlen_zero(chan->macrocontext)) {
				tris_copy_string(chan->context, chan->macrocontext, sizeof(chan->context));
			}
			tris_play_and_wait(chan, "transfer");
			chan->priority = 0;
			pbx_builtin_setvar_helper(chan, "MVM_GREET_STATUS", "USEREXIT");
		}
		res =  0;
...
static int minivm_delete_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	...
	if (!tris_strlen_zero(data)) {
		...
		tris_channel_lock(chan);
		tris_copy_string(filename, pbx_builtin_getvar_helper(chan, "MVM_FILENAME"), sizeof(filename));
		tris_channel_unlock(chan);
	}

	if (tris_strlen_zero(filename)) {
		tris_log(LOG_ERROR, "No filename given in application arguments or channel variable MVM_FILENAME\n");
		return res;
	} 
...
static int minivm_accmess_exec(struct tris_channel *chan, void *data)
{
	int argc = 0;
	char *argv[2];
	...
	if (tris_strlen_zero(data))  {
		tris_log(LOG_ERROR, "MinivmAccmess needs at least two arguments: account and option\n");
		error = TRUE;
	} else 
		tmpptr = tris_strdupa((char *)data);
	if (!error) {
	...
	if (!error && strlen(argv[1]) > 1) {
		tris_log(LOG_ERROR, "MinivmAccmess can only handle one option at a time. Bad option string: %s\n", argv[1]);
		error = TRUE;
	}

	if (!error && tris_app_parse_options(minivm_accmess_options, &flags, opts, argv[1])) {
		tris_log(LOG_ERROR, "Can't parse option %s\n", argv[1]);
		error = TRUE;
	}
	...
	if (tris_test_flag(&flags, OPT_BUSY_GREETING)) {
		message = "busy";
		prompt = "voicemail/vm-rec-busy";
	} else if (tris_test_flag(&flags, OPT_UNAVAIL_GREETING)) {
		message = "unavailable";
		prompt = "voicemail/vm-rec-unv";
	} else if (tris_test_flag(&flags, OPT_TEMP_GREETING)) {
		message = "temp";
		prompt = "voicemail/vm-rec-temp";
	} else if (tris_test_flag(&flags, OPT_NAME_GREETING)) {
		message = "greet";
		prompt = "voicemail/vm-rec-name";
	}
	...
	tris_debug(1, "Recorded new %s message in %s (duration %d)\n", message, filename, duration);
...
static int create_vmaccount(char *name, struct tris_variable *var, int realtime)
{
	...
	tris_debug(3, "Creating %s account for [%s]\n", realtime ? "realtime" : "static", name);

	tris_copy_string(accbuf, name, sizeof(accbuf));
	username = accbuf;
	...
	if (tris_strlen_zero(domain)) {
		tris_log(LOG_ERROR, "No domain given for mini-voicemail account %s. Not configured.\n", name);
		return 0;
	}

	tris_debug(3, "Creating static account for user %s domain %s\n", username, domain);
	...
	tris_debug(3, "...Configuring account %s\n", name);

	while (var) {
		tris_debug(3, "Configuring %s = \"%s\" for account %s\n", var->name, var->value, name);
		if (!strcasecmp(var->name, "serveremail")) {
			tris_copy_string(vmu->serveremail, var->value, sizeof(vmu->serveremail));
		} else if (!strcasecmp(var->name, "email")) {
			tris_copy_string(vmu->email, var->value, sizeof(vmu->email));
		} else if (!strcasecmp(var->name, "accountcode")) {
			tris_copy_string(vmu->accountcode, var->value, sizeof(vmu->accountcode));
		} else if (!strcasecmp(var->name, "pincode")) {
			tris_copy_string(vmu->pincode, var->value, sizeof(vmu->pincode));
		} else if (!strcasecmp(var->name, "domain")) {
			tris_copy_string(vmu->domain, var->value, sizeof(vmu->domain));
		} else if (!strcasecmp(var->name, "language")) {
			tris_copy_string(vmu->language, var->value, sizeof(vmu->language));
		} else if (!strcasecmp(var->name, "timezone")) {
			tris_copy_string(vmu->zonetag, var->value, sizeof(vmu->zonetag));
		} else if (!strcasecmp(var->name, "externnotify")) {
			tris_copy_string(vmu->externnotify, var->value, sizeof(vmu->externnotify));
		} else if (!strcasecmp(var->name, "etemplate")) {
			tris_copy_string(vmu->etemplate, var->value, sizeof(vmu->etemplate));
		} else if (!strcasecmp(var->name, "ptemplate")) {
			tris_copy_string(vmu->ptemplate, var->value, sizeof(vmu->ptemplate));
		} else if (!strcasecmp(var->name, "fullname")) {
			tris_copy_string(vmu->fullname, var->value, sizeof(vmu->fullname));
		} else if (!strcasecmp(var->name, "setvar")) {
			char *varval;
			char *varname = tris_strdupa(var->value);
			...
			tris_copy_string(vmu->pager, var->value, sizeof(vmu->pager));
		} else if (!strcasecmp(var->name, "volgain")) {
			sscanf(var->value, "%30lf", &vmu->volgain);
		} else {
			tris_log(LOG_ERROR, "Unknown configuration option for minivm account %s : %s\n", name, var->name);
		}
		var = var->next;
	}
	tris_debug(3, "...Linking account %s\n", name);
	
	TRIS_LIST_LOCK(&minivm_accounts);
	TRIS_LIST_INSERT_TAIL(&minivm_accounts, vmu, list);
	TRIS_LIST_UNLOCK(&minivm_accounts);

	global_stats.voicemailaccounts++;

	tris_debug(2, "MVM :: Created account %s@%s - tz %s etemplate %s %s\n", username, domain, tris_strlen_zero(vmu->zonetag) ? "" : vmu->zonetag, tris_strlen_zero(vmu->etemplate) ? "" : vmu->etemplate, realtime ? "(realtime)" : "");
	return 0;
}
...
static void timezone_destroy_list(void)
{
	...
	TRIS_LIST_LOCK(&minivm_zones);
	while ((this = TRIS_LIST_REMOVE_HEAD(&minivm_zones, list))) 
		free_zone(this);
...
static int timezone_add(const char *zonename, const char *config)
{
	...
	newzone = tris_calloc(1, sizeof(*newzone));
	if (newzone == NULL)
		return 0;

	msg_format = tris_strdupa(config);
	if (msg_format == NULL) {
	...
	if (!msg_format) {
		tris_log(LOG_WARNING, "Invalid timezone definition : %s\n", zonename);
		tris_free(newzone);
		return 0;
	}
			
	tris_copy_string(newzone->name, zonename, sizeof(newzone->name));
	tris_copy_string(newzone->timezone, timezone_str, sizeof(newzone->timezone));
	tris_copy_string(newzone->msg_format, msg_format, sizeof(newzone->msg_format));

	TRIS_LIST_LOCK(&minivm_zones);
	TRIS_LIST_INSERT_TAIL(&minivm_zones, newzone, list);
	TRIS_LIST_UNLOCK(&minivm_zones);
...
static char *message_template_parse_filebody(const char *filename) {
	...
	if (tris_strlen_zero(filename))
		return NULL;
	if (*filename == '/') 
		tris_copy_string(filenamebuf, filename, sizeof(filenamebuf));
	else 
		snprintf(filenamebuf, sizeof(filenamebuf), "%s/%s", tris_config_TRIS_CONFIG_DIR, filename);

	if (!(fi = fopen(filenamebuf, "r"))) {
		tris_log(LOG_ERROR, "Can't read message template from file: %s\n", filenamebuf);
		return NULL;
	}
	writepos = buf;
	while (fgets(readbuf, sizeof(readbuf), fi)) {
		...
		tris_copy_string(writepos, readbuf, sizeof(buf) - (writepos - buf));
		writepos += strlen(readbuf) - 1;
	}
	fclose(fi);
	messagebody = tris_calloc(1, strlen(buf + 1));
	tris_copy_string(messagebody, buf, strlen(buf) + 1);
	tris_debug(4, "---> Size of allocation %d\n", (int) strlen(buf + 1) );
	tris_debug(4, "---> Done reading message template : \n%s\n---- END message template--- \n", messagebody);
...
static char *message_template_parse_emailbody(const char *configuration)
{
	...
	char *emailbody = tris_strdup(configuration);
...
static int apply_general_options(struct tris_variable *var)
{
	int error = 0;
	...
	while (var) {
		...
		if (!strcmp(var->name, "mailcmd")) {
			tris_copy_string(global_mailcmd, var->value, sizeof(global_mailcmd)); /* User setting */
			...
			if (!tris_strlen_zero(var->value) ) {
				if(*(var->value) == '/')
					tris_copy_string(global_logfile, var->value, sizeof(global_logfile));
				else
					snprintf(global_logfile, sizeof(global_logfile), "%s/%s", tris_config_TRIS_LOG_DIR, var->value);
			}
		} else if (!strcmp(var->name, "externnotify")) {
			/* External voicemail notify application */
			tris_copy_string(global_externnotify, var->value, sizeof(global_externnotify));
			...
			tris_copy_string(default_vmformat, var->value, sizeof(default_vmformat));
		} else if (!strcmp(var->name, "review")) {
			tris_set2_flag((&globalflags), tris_true(var->value), MVM_REVIEW);	
		} else if (!strcmp(var->name, "operator")) {
			tris_set2_flag((&globalflags), tris_true(var->value), MVM_OPERATOR);	
		}
		var = var->next;
	}
	return error;
...
static int load_config(int reload)
{
	...
	if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		...
		tris_log(LOG_ERROR, "Config file " VOICEMAIL_CONFIG " is in an invalid format.  Aborting.\n");
		return 0;
	}

	tris_mutex_lock(&minivmlock);
	...
	tris_debug(2, "Destroyed memory objects...\n");
	...
	tris_copy_string(default_vmformat, "wav", sizeof(default_vmformat));
	tris_set2_flag((&globalflags), FALSE, MVM_REVIEW);	
	tris_set2_flag((&globalflags), FALSE, MVM_OPERATOR);	
	strcpy(global_charset, "ISO-8859-1");
	/* Reset statistics */
	memset(&global_stats, 0, sizeof(global_stats));
	global_stats.reset = tris_tvnow();

	global_silencethreshold = tris_dsp_get_threshold_from_settings(THRESHOLD_SILENCE);

	/* Make sure we could load configuration file */
	if (!cfg) {
		tris_log(LOG_WARNING, "Failed to load configuration file. Module activated with default settings.\n");
		tris_mutex_unlock(&minivmlock);
		return 0;
	}

	tris_debug(2, "Loaded configuration file, now parsing\n");

	/* General settings */

	cat = tris_category_browse(cfg, NULL);
	while (cat) {
		tris_debug(3, "Found configuration section [%s]\n", cat);
		if (!strcasecmp(cat, "general")) {
			/* Nothing right now */
			error += apply_general_options(tris_variable_browse(cfg, cat));
		} else if (!strncasecmp(cat, "template-", 9))  {
			/* Template */
			char *name = cat + 9;

			/* Now build and link template to list */
			error += message_template_build(name, tris_variable_browse(cfg, cat));
		} else {
	...
	if ((chanvar = tris_variable_retrieve(cfg, "general", "emaildateformat"))) 
		tris_copy_string(template->dateformat, chanvar, sizeof(template->dateformat));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "emailfromstring")))
		tris_copy_string(template->fromaddress, chanvar, sizeof(template->fromaddress));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "emailaaddress")))
		tris_copy_string(template->serveremail, chanvar, sizeof(template->serveremail));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "emailcharset")))
		tris_copy_string(template->charset, chanvar, sizeof(template->charset));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "emailsubject"))) 
		tris_copy_string(template->subject, chanvar, sizeof(template->subject));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "emailbody"))) 
	...
	if ((chanvar = tris_variable_retrieve(cfg, "general", "pagerfromstring")))
		tris_copy_string(template->fromaddress, chanvar, sizeof(template->fromaddress));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "pageraddress")))
		tris_copy_string(template->serveremail, chanvar, sizeof(template->serveremail));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "pagercharset")))
		tris_copy_string(template->charset, chanvar, sizeof(template->charset));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "pagersubject")))
		tris_copy_string(template->subject, chanvar,sizeof(template->subject));
	if ((chanvar = tris_variable_retrieve(cfg, "general", "pagerbody"))) 
		template->body = message_template_parse_emailbody(chanvar);
	template->attachment = FALSE;

	if (error)
		tris_log(LOG_ERROR, "--- A total of %d errors found in mini-voicemail configuration\n", error);
	...
	if(!tris_strlen_zero(global_logfile)) {
		minivmlogfile = fopen(global_logfile, "a");
		if(!minivmlogfile)
			tris_log(LOG_ERROR, "Failed to open minivm log file %s : %s\n", global_logfile, strerror(errno));
		if (minivmlogfile)
			tris_debug(3, "Opened log file %s \n", global_logfile);
	}
...
static char *handle_minivm_list_templates(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct minivm_template *this;
#define HVLT_OUTPUT_FORMAT "%-15s %-10s %-10s %-15.15s %-50s\n"
	...
	TRIS_LIST_LOCK(&message_templates);
	if (TRIS_LIST_EMPTY(&message_templates)) {
		tris_cli(a->fd, "There are no message templates defined\n");
		TRIS_LIST_UNLOCK(&message_templates);
		return CLI_FAILURE;
	}
	tris_cli(a->fd, HVLT_OUTPUT_FORMAT, "Template name", "Charset", "Locale", "Attach media", "Subject");
	tris_cli(a->fd, HVLT_OUTPUT_FORMAT, "-------------", "-------", "------", "------------", "-------");
	TRIS_LIST_TRAVERSE(&message_templates, this, list) {
		tris_cli(a->fd, HVLT_OUTPUT_FORMAT, this->name, 
...
static char *handle_minivm_show_users(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct minivm_account *vmu;
#define HMSU_OUTPUT_FORMAT "%-23s %-15s %-15s %-10s %-10s %-50s\n"
	...
	TRIS_LIST_LOCK(&minivm_accounts);
	if (TRIS_LIST_EMPTY(&minivm_accounts)) {
		tris_cli(a->fd, "There are no voicemail users currently defined\n");
		TRIS_LIST_UNLOCK(&minivm_accounts);
		return CLI_FAILURE;
	}
	tris_cli(a->fd, HMSU_OUTPUT_FORMAT, "User", "E-Template", "P-template", "Zone", "Format", "Full name");
	tris_cli(a->fd, HMSU_OUTPUT_FORMAT, "----", "----------", "----------", "----", "------", "---------");
	TRIS_LIST_TRAVERSE(&minivm_accounts, vmu, list) {
		char tmp[256] = "";
		if ((a->argc == 3) || ((a->argc == 5) && !strcmp(a->argv[4], vmu->domain))) {
			count++;
			snprintf(tmp, sizeof(tmp), "%s@%s", vmu->username, vmu->domain);
			tris_cli(a->fd, HMSU_OUTPUT_FORMAT, tmp, vmu->etemplate ? vmu->etemplate : "-", 
...
static char *handle_minivm_show_zones(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define HMSZ_OUTPUT_FORMAT "%-15s %-20s %-45s\n"
	...
	TRIS_LIST_LOCK(&minivm_zones);
	if (!TRIS_LIST_EMPTY(&minivm_zones)) {
		tris_cli(a->fd, HMSZ_OUTPUT_FORMAT, "Zone", "Timezone", "Message Format");
		tris_cli(a->fd, HMSZ_OUTPUT_FORMAT, "----", "--------", "--------------");
		TRIS_LIST_TRAVERSE(&minivm_zones, zone, list) {
			tris_cli(a->fd, HMSZ_OUTPUT_FORMAT, zone->name, zone->timezone, zone->msg_format);
		}
	} else {
...
static char *handle_minivm_show_settings(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	tris_cli(a->fd, "* Mini-Voicemail general settings\n");
	tris_cli(a->fd, "  -------------------------------\n");
	tris_cli(a->fd, "\n");
	tris_cli(a->fd, "  Mail command (shell):               %s\n", global_mailcmd);
	tris_cli(a->fd, "  Max silence:                        %d\n", global_maxsilence);
	tris_cli(a->fd, "  Silence threshold:                  %d\n", global_silencethreshold);
	tris_cli(a->fd, "  Max message length (secs):          %d\n", global_vmmaxmessage);
	tris_cli(a->fd, "  Min message length (secs):          %d\n", global_vmminmessage);
	tris_cli(a->fd, "  Default format:                     %s\n", default_vmformat);
	tris_cli(a->fd, "  Extern notify (shell):              %s\n", global_externnotify);
	tris_cli(a->fd, "  Logfile:                            %s\n", global_logfile[0] ? global_logfile : "<disabled>");
	tris_cli(a->fd, "  Operator exit:                      %s\n", tris_test_flag(&globalflags, MVM_OPERATOR) ? "Yes" : "No");
	tris_cli(a->fd, "  Message review:                     %s\n", tris_test_flag(&globalflags, MVM_REVIEW) ? "Yes" : "No");
...
/*! \brief Show stats */
static char *handle_minivm_show_stats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct tris_tm timebuf;
	char buf[BUFSIZ];
	...
	tris_cli(a->fd, "* Mini-Voicemail statistics\n");
	tris_cli(a->fd, "  -------------------------\n");
	tris_cli(a->fd, "\n");
	tris_cli(a->fd, "  Voicemail accounts:                  %5d\n", global_stats.voicemailaccounts);
	tris_cli(a->fd, "  Templates:                           %5d\n", global_stats.templates);
	tris_cli(a->fd, "  Timezones:                           %5d\n", global_stats.timezones);
	if (global_stats.receivedmessages == 0) {
		tris_cli(a->fd, "  Received messages since last reset:  <none>\n");
	} else {
		tris_cli(a->fd, "  Received messages since last reset:  %d\n", global_stats.receivedmessages);
		tris_localtime(&global_stats.lastreceived, &timebuf, NULL);
		tris_strftime(buf, sizeof(buf), "%a %b %e %r %Z %Y", &timebuf);
		tris_cli(a->fd, "  Last received voicemail:             %s\n", buf);
	}
	tris_localtime(&global_stats.reset, &timebuf, NULL);
	tris_strftime(buf, sizeof(buf), "%a %b %e %r %Z %Y", &timebuf);
	tris_cli(a->fd, "  Last reset:                          %s\n", buf);
...
static int minivm_account_func_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct minivm_account *vmu;
	char *username, *domain, *colname;

	if (!(username = tris_strdupa(data))) {
		tris_log(LOG_ERROR, "Memory Error!\n");
		return -1;
	}
	...
	if (tris_strlen_zero(username) || tris_strlen_zero(domain)) {
		tris_log(LOG_ERROR, "This function needs a username and a domain: username@domain\n");
		return 0;
	}
	...
	if (!strcasecmp(colname, "hasaccount")) {
		tris_copy_string(buf, (tris_test_flag(vmu, MVM_ALLOCED) ? "0" : "1"), len);
	} else  if (!strcasecmp(colname, "fullname")) { 
		tris_copy_string(buf, vmu->fullname, len);
	} else  if (!strcasecmp(colname, "email")) { 
		if (!tris_strlen_zero(vmu->email))
			tris_copy_string(buf, vmu->email, len);
		else
			snprintf(buf, len, "%s@%s", vmu->username, vmu->domain);
	} else  if (!strcasecmp(colname, "pager")) { 
		tris_copy_string(buf, vmu->pager, len);
	} else  if (!strcasecmp(colname, "etemplate")) { 
		if (!tris_strlen_zero(vmu->etemplate))
			tris_copy_string(buf, vmu->etemplate, len);
		else
			tris_copy_string(buf, "email-default", len);
	} else  if (!strcasecmp(colname, "language")) { 
		tris_copy_string(buf, vmu->language, len);
	} else  if (!strcasecmp(colname, "timezone")) { 
		tris_copy_string(buf, vmu->zonetag, len);
	} else  if (!strcasecmp(colname, "ptemplate")) { 
		if (!tris_strlen_zero(vmu->ptemplate))
			tris_copy_string(buf, vmu->ptemplate, len);
		else
			tris_copy_string(buf, "email-default", len);
	} else  if (!strcasecmp(colname, "accountcode")) {
		tris_copy_string(buf, vmu->accountcode, len);
	} else  if (!strcasecmp(colname, "pincode")) {
		tris_copy_string(buf, vmu->pincode, len);
...
static int access_counter_file(char *directory, char *countername, int value, int operand)
{
	...
	if (operand != 1) {
		...
		if (counterfile) {
			if(fgets(readbuf, sizeof(readbuf), counterfile)) {
				tris_debug(3, "Read this string from counter file: %s\n", readbuf);
	...
	switch (operand) {
	case 0:	/* Read only */
		tris_unlock_path(directory);
		tris_debug(2, "MINIVM Counter %s/%s: Value %d\n", directory, countername, counter);
		return counter;
		break;
	...
	if (!counterfile) {
		tris_log(LOG_ERROR, "Could not open counter file for writing : %s - %s\n", filename, strerror(errno));
		tris_unlock_path(directory);
	...
	fclose(counterfile);
	tris_unlock_path(directory);
	tris_debug(2, "MINIVM Counter %s/%s: Old value %d New value %d\n", directory, countername, old, counter);
	return counter;
}

/*! \brief  ${MINIVMCOUNTER()} Dialplan function - read counters */
static int minivm_counter_func_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	*buf = '\0';
	...
	if (!(username = tris_strdupa(data))) {	/* Copy indata to local buffer */
		tris_log(LOG_WARNING, "Memory error!\n");
		return -1;
	}
	...
	if (tris_strlen_zero(username) && tris_strlen_zero(domain)) {
		tris_log(LOG_ERROR, "No account given\n");
		return -1;
	}

	if (tris_strlen_zero(countername)) {
		tris_log(LOG_ERROR, "This function needs two arguments: Account:countername\n");
		return -1;
	}
	...
	if (!tris_strlen_zero(username) && !(vmu = find_account(domain, username, FALSE))) {
		tris_log(LOG_ERROR, "Minivm account does not exist: %s@%s\n", username, domain);
		return 0;
	}
...
static int minivm_counter_func_write(struct tris_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (tris_strlen_zero(operand) || tris_strlen_zero(countername)) {
		tris_log(LOG_ERROR, "Writing to this function requires three arguments: Account:countername:operand\n");
		return -1;
	}
	...
	if (*operand == 'i') /* Increment */
		...
		tris_log(LOG_ERROR, "Unknown operator: %s\n", operand);
		return -1;
	}
...
static struct tris_cli_entry cli_minivm[] = {
	TRIS_CLI_DEFINE(handle_minivm_show_users, "List defined mini-voicemail boxes"),
	TRIS_CLI_DEFINE(handle_minivm_show_zones, "List zone message formats"),
	TRIS_CLI_DEFINE(handle_minivm_list_templates, "List message templates"),	
	TRIS_CLI_DEFINE(handle_minivm_reload, "Reload Mini-voicemail configuration"),
	TRIS_CLI_DEFINE(handle_minivm_show_stats, "Show some mini-voicemail statistics"),
	TRIS_CLI_DEFINE(handle_minivm_show_settings, "Show mini-voicemail general settings"),
};

static struct tris_custom_function minivm_counter_function = {
	.name = "MINIVMCOUNTER",
	...
	"realtime functions if you are using a database to operate your Trismedia\n",
};

static struct tris_custom_function minivm_account_function = {
	.name = "MINIVMACCOUNT",
...
static int load_module(void)
{
	...
	res = tris_register_application_xml(app_minivm_record, minivm_record_exec);
	res = tris_register_application_xml(app_minivm_greet, minivm_greet_exec);
	res = tris_register_application_xml(app_minivm_notify, minivm_notify_exec);
	res = tris_register_application_xml(app_minivm_delete, minivm_delete_exec);
	res = tris_register_application_xml(app_minivm_accmess, minivm_accmess_exec);
	res = tris_register_application_xml(app_minivm_mwi, minivm_mwi_exec);

	tris_custom_function_register(&minivm_account_function);
	tris_custom_function_register(&minivm_counter_function);
	if (res)
		return(res);
	...
	tris_cli_register_multiple(cli_minivm, ARRAY_LEN(cli_minivm));

	/* compute the location of the voicemail spool directory */
	snprintf(MVM_SPOOL_DIR, sizeof(MVM_SPOOL_DIR), "%s/voicemail/", tris_config_TRIS_SPOOL_DIR);
...
static int unload_module(void)
{
	...
	res = tris_unregister_application(app_minivm_record);
	res |= tris_unregister_application(app_minivm_greet);
	res |= tris_unregister_application(app_minivm_notify);
	res |= tris_unregister_application(app_minivm_delete);
	res |= tris_unregister_application(app_minivm_accmess);
	res |= tris_unregister_application(app_minivm_mwi);

	tris_cli_unregister_multiple(cli_minivm, ARRAY_LEN(cli_minivm));
	tris_custom_function_unregister(&minivm_account_function);
	tris_custom_function_unregister(&minivm_counter_function);
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Mini VoiceMail (A minimal Voicemail e-mail System)",
		.load = load_module,
