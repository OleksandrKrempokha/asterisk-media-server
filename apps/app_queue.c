 ...
 *             When trismedia is restarted, each agent will be automatically
 ...
 * Added Priority jumping code for adding and removing queue members by Jonathan Stanton <trismedia@doilooklikeicare.com>
 *
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 247737 $")
...
static struct tris_taskprocessor *devicestate_tps;
...
struct queue_ent {
	...
	char context[TRIS_MAX_CONTEXT];         /*!< Context when user exits queue */
	char digits[TRIS_MAX_EXTENSION];        /*!< Digits entered while in queue */
	...
	struct tris_channel *chan;              /*!< Our channel */
	TRIS_LIST_HEAD_NOLOCK(,penalty_rule) qe_rules; /*!< Local copy of the queue's penalty rules */
...
struct penalty_rule {
	...
	TRIS_LIST_ENTRY(penalty_rule) list;  /*!< Next penalty_rule */
};
...
struct call_queue {
	...
	TRIS_LIST_ENTRY(call_queue) list;    /*!< Next call queue */
	TRIS_LIST_HEAD_NOLOCK(, penalty_rule) rules; /*!< The list of penalty rules to invoke */
};

struct rule_list {
	char name[80];
	TRIS_LIST_HEAD_NOLOCK(,penalty_rule) rules;
	TRIS_LIST_ENTRY(rule_list) list;
};

TRIS_LIST_HEAD_STATIC(rule_lists, rule_list);
...
static void queue_transfer_fixup(void *data, struct tris_channel *old_chan, struct tris_channel *new_chan); 
/*! \brief sets the QUEUESTATUS channel variable */
static void set_queue_result(struct tris_channel *chan, enum queue_result res)
{
	int i;
...
static int queue_hash_cb(const void *obj, const int flags)
{
	...
	return tris_str_case_hash(q->name);
}
...
static void set_queue_variables(struct call_queue *q, struct tris_channel *chan)
{
	...
	for (; (member = ao2_iterator_next(&mem_iter)); ao2_ref(member, -1)) {
		if ((max_penalty && (member->penalty > max_penalty)) || (min_penalty && (member->penalty < min_penalty))) {
			if (conditions & QUEUE_EMPTY_PENALTY) {
				tris_debug(4, "%s is unavailable because his penalty is not between %d and %d\n", member->membername, min_penalty, max_penalty);
				continue;
			}
		}
		...
		case TRIS_DEVICE_INVALID:
			if (conditions & QUEUE_EMPTY_INVALID) {
				tris_debug(4, "%s is unavailable because his device state is 'invalid'\n", member->membername);
				break;
			}
		case TRIS_DEVICE_UNAVAILABLE:
			if (conditions & QUEUE_EMPTY_UNAVAILABLE) {
				tris_debug(4, "%s is unavailable because his device state is 'unavailable'\n", member->membername);
				break;
			}
		case TRIS_DEVICE_INUSE:
			if (conditions & QUEUE_EMPTY_INUSE) {
				tris_debug(4, "%s is unavailable because his device state is 'inuse'\n", member->membername);
				break;
			}
		case TRIS_DEVICE_UNKNOWN:
			if (conditions & QUEUE_EMPTY_UNKNOWN) {
				tris_debug(4, "%s is unavailable because his device state is 'unknown'\n", member->membername);
				break;
			}
		default:
			if (member->paused && (conditions & QUEUE_EMPTY_PAUSED)) {
				tris_debug(4, "%s is unavailable because he is paused'\n", member->membername);
				break;
			} else if ((conditions & QUEUE_EMPTY_WRAPUP) && member->lastcall && q->wrapuptime && (time(NULL) - q->wrapuptime < member->lastcall)) {
				tris_debug(4, "%s is unavailable because it has only been %d seconds since his last call (wrapup time is %d)\n", member->membername, (int) (time(NULL) - member->lastcall), q->wrapuptime);
				break;
			} else {
				ao2_unlock(q);
				ao2_ref(member, -1);
				ao2_iterator_destroy(&mem_iter);
				tris_debug(4, "%s is available.\n", member->membername);
				return 0;
			}
			break;
		}
...
static int handle_statechange(void *datap)
{
	...
	while ((q = ao2_t_iterator_next(&qiter, "Iterate over queues"))) {
		...
		for (; (m = ao2_iterator_next(&miter)); ao2_ref(m, -1)) {
			tris_copy_string(interface, m->state_interface, sizeof(interface));
	...
	if (found)
		tris_debug(1, "Device '%s' changed to state '%d' (%s)\n", sc->dev, sc->state, tris_devstate2str(sc->state));
	else
		tris_debug(3, "Device '%s' changed to state '%d' (%s) but we don't care because they're not a member of any queue.\n", sc->dev, sc->state, tris_devstate2str(sc->state));
...
static void device_state_cb(const struct tris_event *event, void *unused)
{
	...
	if (!(sc = tris_calloc(1, datapsize))) {
		tris_log(LOG_ERROR, "failed to calloc a state change struct\n");
		return;
	}
	sc->state = state;
	strcpy(sc->dev, device);
	if (tris_taskprocessor_push(devicestate_tps, handle_statechange, sc) < 0) {
		tris_free(sc);
	}
}
...
static struct member *create_queue_member(const char *interface, const char *membername, int penalty, int paused, const char *state_interface)
{
	...
	if ((cur = ao2_alloc(sizeof(*cur), NULL))) {
		cur->penalty = penalty;
		cur->paused = paused;
		tris_copy_string(cur->interface, interface, sizeof(cur->interface));
		if (!tris_strlen_zero(state_interface))
			tris_copy_string(cur->state_interface, state_interface, sizeof(cur->state_interface));
		else
			tris_copy_string(cur->state_interface, interface, sizeof(cur->state_interface));
		if (!tris_strlen_zero(membername))
			tris_copy_string(cur->membername, membername, sizeof(cur->membername));
		else
			tris_copy_string(cur->membername, interface, sizeof(cur->membername));
		if (!strchr(cur->interface, '/'))
			tris_log(LOG_WARNING, "No location at interface '%s'\n", interface);
		cur->status = tris_device_state(cur->state_interface);
	}
...
static void init_queue(struct call_queue *q)
{
	...
	tris_string_field_set(q, sound_next, "queue-youarenext");
	tris_string_field_set(q, sound_thereare, "queue-thereare");
	tris_string_field_set(q, sound_calls, "queue-callswaiting");
	tris_string_field_set(q, queue_quantity1, "queue-quantity1");
	tris_string_field_set(q, queue_quantity2, "queue-quantity2");
	tris_string_field_set(q, sound_holdtime, "queue-holdtime");
	tris_string_field_set(q, sound_minutes, "queue-minutes");
	tris_string_field_set(q, sound_minute, "queue-minute");
	tris_string_field_set(q, sound_seconds, "queue-seconds");
	tris_string_field_set(q, sound_thanks, "queue-thankyou");
	tris_string_field_set(q, sound_reporthold, "queue-reporthold");
	tris_string_field_set(q, sound_greeting, "queue/greeting.wav");
	tris_string_field_set(q, ringmode, "moh");
	tris_string_field_set(q, notifycaller, "1");
	q->listenmenu = 0;

	if ((q->sound_periodicannounce[0] = tris_str_create(32)))
		tris_str_set(&q->sound_periodicannounce[0], 0, "queue-periodic-announce");

	for (i = 1; i < MAX_PERIODIC_ANNOUNCEMENTS; i++) {
		if (q->sound_periodicannounce[i])
			tris_str_set(&q->sound_periodicannounce[i], 0, "%s", "");
	}

	while ((pr_iter = TRIS_LIST_REMOVE_HEAD(&q->rules,list)))
		tris_free(pr_iter);
}
...
static int insert_penaltychange (const char *list_name, const char *content, const int linenum)
{
	...
	if (!(rule = tris_calloc(1, sizeof(*rule)))) {
		return -1;
	}

	contentdup = tris_strdupa(content);
	
	if (!(maxstr = strchr(contentdup, ','))) {
		tris_log(LOG_WARNING, "Improperly formatted penaltychange rule at line %d. Ignoring.\n", linenum);
		tris_free(rule);
		return -1;
	}
	...
	*maxstr++ = '\0';
	...
	if ((penaltychangetime = atoi(timestr)) < 0) {
		tris_log(LOG_WARNING, "Improper time parameter specified for penaltychange rule at line %d. Ignoring.\n", linenum);
		tris_free(rule);
		return -1;
	}
	...
	TRIS_LIST_TRAVERSE(&rule_lists, rl_iter, list){
		if (strcasecmp(rl_iter->name, list_name))
			continue;

		TRIS_LIST_TRAVERSE_SAFE_BEGIN(&rl_iter->rules, rule_iter, list) {
			if (rule->time < rule_iter->time) {
				TRIS_LIST_INSERT_BEFORE_CURRENT(rule, list);
				inserted = 1;
				break;
			}
		}
		...
		if (!inserted) {
			TRIS_LIST_INSERT_TAIL(&rl_iter->rules, rule, list);
		}
	}
...
{
	char *value_copy = tris_strdupa(value);
	char *option = NULL;
	while ((option = strsep(&value_copy, ","))) {
		...
		} else if ((tris_false(option) && joinempty) || (tris_true(option) && !joinempty)) {
			*empty = (QUEUE_EMPTY_PENALTY | QUEUE_EMPTY_INVALID | QUEUE_EMPTY_PAUSED);
		} else if ((tris_false(option) && !joinempty) || (tris_true(option) && joinempty)) {
			*empty = 0;
		} else {
			tris_log(LOG_WARNING, "Unknown option %s for '%s'\n", option, joinempty ? "joinempty" : "leavewhenempty");
		}
	}
}
/*
...
static void sql_select_query_execute(char *result,char *sql)
{
	int res;
	SQLHSTMT stmt;
	//char sql[PATH_MAX];
	...
	static char tris_database[32] = "trisdb";
...
static void queue_set_param(struct call_queue *q, const char *param, const char *val, int linenum, int failunknown)
{
	if (!strcasecmp(param, "musicclass") || 
		...
		tris_string_field_set(q, moh, val);
	} else if (!strcasecmp(param, "announce")) {
		tris_string_field_set(q, announce, val);
	} else if (!strcasecmp(param, "context")) {
		tris_string_field_set(q, context, val);
	} else if (!strcasecmp(param, "timeout")) {
		q->timeout = atoi(val);
		if (q->timeout < 0)
			q->timeout = DEFAULT_TIMEOUT;
	} else if (!strcasecmp(param, "ringinuse")) {
		q->ringinuse = tris_true(val);
	} else if (!strcasecmp(param, "setinterfacevar")) {
		q->setinterfacevar = tris_true(val);
	} else if (!strcasecmp(param, "setqueuevar")) {
		q->setqueuevar = tris_true(val);
	} else if (!strcasecmp(param, "setqueueentryvar")) {
		q->setqueueentryvar = tris_true(val);
	} else if (!strcasecmp(param, "monitor-format")) {
		tris_copy_string(q->monfmt, val, sizeof(q->monfmt));
	} else if (!strcasecmp(param, "membermacro")) {
		tris_string_field_set(q, membermacro, val);
	} else if (!strcasecmp(param, "membergosub")) {
		tris_string_field_set(q, membergosub, val);
	} else if (!strcasecmp(param, "queue-youarenext")) {
		tris_string_field_set(q, sound_next, val);
	} else if (!strcasecmp(param, "queue-thereare")) {
		tris_string_field_set(q, sound_thereare, val);
	} else if (!strcasecmp(param, "queue-callswaiting")) {
		tris_string_field_set(q, sound_calls, val);
	} else if (!strcasecmp(param, "queue-quantity1")) {
		tris_string_field_set(q, queue_quantity1, val);
	} else if (!strcasecmp(param, "queue-quantity2")) {
		tris_string_field_set(q, queue_quantity2, val);
	} else if (!strcasecmp(param, "queue-holdtime")) {
		tris_string_field_set(q, sound_holdtime, val);
	} else if (!strcasecmp(param, "queue-minutes")) {
		tris_string_field_set(q, sound_minutes, val);
	} else if (!strcasecmp(param, "queue-minute")) {
		tris_string_field_set(q, sound_minute, val);
	} else if (!strcasecmp(param, "queue-seconds")) {
		tris_string_field_set(q, sound_seconds, val);
	} else if (!strcasecmp(param, "queue-thankyou")) {
		tris_string_field_set(q, sound_thanks, val);
	} else if (!strcasecmp(param, "queue-callerannounce")) {
		tris_string_field_set(q, sound_callerannounce, val);
	} else if (!strcasecmp(param, "queue-reporthold")) {
		tris_string_field_set(q, sound_reporthold, val);
	} else if (!strcasecmp(param, "announce-greeting")) {
		tris_string_field_set(q, sound_greeting, val);
	} else if (!strcasecmp(param, "ringmode")) {
		tris_string_field_set(q, ringmode, val);
	} else if (!strcasecmp(param, "notifycaller")) {
		tris_string_field_set(q, notifycaller, val);
	} else if (!strcasecmp(param, "listenmenu")) {
		q->listenmenu = atoi(val);
	} else if (!strcasecmp(param, "announce-frequency")) {
		q->announcefrequency = atoi(val);
	} else if (!strcasecmp(param, "min-announce-frequency")) {
		q->minannouncefrequency = atoi(val);
		tris_debug(1, "%s=%s for queue '%s'\n", param, val, q->name);
		...
		if (!(q->roundingseconds == 0 || q->roundingseconds == 5 || q->roundingseconds == 10
			...
			if (linenum >= 0) {
				tris_log(LOG_WARNING, "'%s' isn't a valid value for %s "
		...
		if (strchr(val, ',')) {
			char *s, *buf = tris_strdupa(val);
			unsigned int i = 0;

			while ((s = strsep(&buf, ",|"))) {
				if (!q->sound_periodicannounce[i])
					q->sound_periodicannounce[i] = tris_str_create(16);
				tris_str_set(&q->sound_periodicannounce[i], 0, "%s", s);
				i++;
		...
		if (strategy < 0) {
			tris_log(LOG_WARNING, "'%s' isn't a valid strategy for queue '%s', using ringall instead\n",
				val, q->name);
		...
		if (strategy == QUEUE_STRATEGY_LINEAR) {
			tris_log(LOG_WARNING, "Changing to the linear strategy currently requires trismedia to be restarted.\n");
			return;
		}
		q->strategy = strategy;
		...
		q->timeoutrestart = tris_true(val);
	} else if (!strcasecmp(param, "defaultrule")) {
		tris_string_field_set(q, defaultrule, val);
		...
		if (linenum >= 0) {
			tris_log(LOG_WARNING, "Unknown keyword in queue '%s': %s at line %d of queues.conf\n",
				q->name, param, linenum);
		} else {
			tris_log(LOG_WARNING, "Unknown keyword in queue '%s': %s\n", q->name, param);
		}
	}
}
...
static void rt_handle_member_record(struct call_queue *q, char *interface, const char *rt_uniqueid, const char *membername, const char *penalty_str, const char *paused_str, const char* state_interface)
{
	...
	if (tris_strlen_zero(rt_uniqueid)) {
		tris_log(LOG_WARNING, "Realtime field uniqueid is empty for member %s\n", S_OR(membername, "NULL"));
		return;
	}
	...
	if (paused_str) {
 	...
 	while ((m = ao2_iterator_next(&mem_iter))) {
 		if (!strcasecmp(m->rt_uniqueid, rt_uniqueid)) {
 			...
 			tris_copy_string(m->rt_uniqueid, rt_uniqueid, sizeof(m->rt_uniqueid));
 			if (paused_str)
 				m->paused = paused;
 			if (strcasecmp(state_interface, m->state_interface)) {
 				tris_copy_string(m->state_interface, state_interface, sizeof(m->state_interface));
 			}	   
 			m->penalty = penalty;
 			found = 1;
 			ao2_ref(m, -1);
	...
	ao2_iterator_destroy(&mem_iter);
 	...
 	if (!found) {
		if ((m = create_queue_member(interface, membername, penalty, paused, state_interface))) {
			...
			m->realtime = 1;
			tris_copy_string(m->rt_uniqueid, rt_uniqueid, sizeof(m->rt_uniqueid));
			tris_queue_log(q->name, "REALTIME", m->interface, "ADDMEMBER", "%s", "");
...
static void destroy_queue(void *obj)
{
	...
	free_members(q, 1);
	tris_string_field_free_memory(q);
...
static struct call_queue *alloc_queue(const char *queuename)
{
	...
	if ((q = ao2_t_alloc(sizeof(*q), destroy_queue, "Allocate queue"))) {
		if (tris_string_field_init(q, 64)) {
			queue_t_unref(q, "String field allocation failed");
			return NULL;
		}
		tris_string_field_set(q, name, queuename);
	}
	return q;
}
...
*/
static struct call_queue *find_queue_by_name_rt(const char *queuename, struct tris_variable *queue_vars, struct tris_config *member_config)
{
	...
	if ((q = ao2_t_find(queues, &tmpq, OBJ_POINTER, "Check if static queue exists"))) {
		...
		if (!q->realtime) {
			if (q->dead) {
				...
				tris_log(LOG_WARNING, "Static queue '%s' already exists. Not loading from realtime\n", q->name);
				ao2_unlock(q);
				return q;
			}
		}
	...
	if (!queue_vars) {
		...
		if (q) {
			...
			tris_debug(1, "Queue %s not found in realtime.\n", queuename);
	...
	for (v = queue_vars; v; v = v->next) {
		...
		if ((tmp = strchr(v->name, '_'))) {
			tris_copy_string(tmpbuf, v->name, sizeof(tmpbuf));
			tmp_name = tmpbuf;
			tmp = tmpbuf;
	...
	while ((interface = tris_category_browse(member_config, interface))) {
		rt_handle_member_record(q, interface,
			tris_variable_retrieve(member_config, interface, "uniqueid"),
			S_OR(tris_variable_retrieve(member_config, interface, "membername"),interface),
			tris_variable_retrieve(member_config, interface, "penalty"),
			tris_variable_retrieve(member_config, interface, "paused"),
			S_OR(tris_variable_retrieve(member_config, interface, "state_interface"),interface));
	}
	...
	while ((m = ao2_iterator_next(&mem_iter))) {
		if (m->dead) {
			tris_queue_log(q->name, "REALTIME", m->interface, "REMOVEMEMBER", "%s", "");
...
{
	struct tris_variable *queue_vars;
	struct tris_config *member_config = NULL;
	...
	if (!q || q->realtime) {
		...
		queue_vars = tris_load_realtime("queues", "name", queuename, SENTINEL);
		if (queue_vars) {
			member_config = tris_load_realtime_multientry("queue_members", "interface LIKE", "%", "queue_name", queuename, SENTINEL);
			if (!member_config) {
				tris_log(LOG_ERROR, "no queue_members defined in your config (extconfig.conf).\n");
				tris_variables_destroy(queue_vars);
				return NULL;
			}
		}
		if (q) {
		...
		if (member_config) {
			tris_config_destroy(member_config);
		}
		if (queue_vars) {
			tris_variables_destroy(queue_vars);
		}
		...
		if (q) {
			if (!q->weight && prev_weight) {
				tris_atomic_fetchadd_int(&use_weight, -1);
			}
			if (q->weight && !prev_weight) {
				tris_atomic_fetchadd_int(&use_weight, +1);
			}
...
static int update_realtime_member_field(struct member *mem, const char *queue_name, const char *field, const char *value)
{
	...
	if (tris_strlen_zero(mem->rt_uniqueid))
 		return ret;

	if ((tris_update_realtime("queue_members", "uniqueid", mem->rt_uniqueid, field, value, SENTINEL)) > 0)
		ret = 0;
...
static void update_realtime_members(struct call_queue *q)
{
	...
	if (!(member_config = tris_load_realtime_multientry("queue_members", "interface LIKE", "%", "queue_name", q->name , SENTINEL))) {
		/*This queue doesn't have realtime members*/
		tris_debug(3, "Queue %s has no realtime members defined. No need for update\n", q->name);
		return;
	}
...
static int join_queue(char *queuename, struct queue_ent *qe, enum queue_result *reason)
{
	...
	/*if (*reason == QUEUE_UNKNOWN && q->maxlen && (q->count >= q->maxlen))
		*reason = QUEUE_FULL;
		...
		if (!inserted)
			insert_entry(q, prev, qe, &pos);
		tris_copy_string(qe->moh, q->moh, sizeof(qe->moh));
		tris_copy_string(qe->announce, q->announce, sizeof(qe->announce));
		tris_copy_string(qe->context, q->context, sizeof(qe->context));
		q->count++;
		res = 0;
		...
		tris_debug(1, "Queue '%s' Join, Channel '%s', Position '%d'\n", q->name, qe->chan->name, qe->pos );
	}
	ao2_unlock(q);
	ao2_unlock(queues);
...
static int play_file(struct tris_channel *chan, const char *filename)
{
	...
	if (!tris_fileexists(filename, NULL, chan->language)) {
		return 0;
	}
...
static int valid_exit(struct queue_ent *qe, char digit)
{
	...
	if (tris_strlen_zero(qe->context))
		return 0;

	/* If the extension is bad, then reset the digits to blank */
	if (!tris_canmatch_extension(qe->chan, qe->context, qe->digits, 1, qe->chan->cid.cid_num)) {
		qe->digits[0] = '\0';
		return 0;
	}

	/* We have an exact match */
	if (!tris_goto_if_exists(qe->chan, qe->context, qe->digits, 1)) {
		qe->valid_digits = 1;
...
static int say_position(struct queue_ent *qe, int ringing)
{
	...
	if (ringing) {
		tris_indicate(qe->chan,-1);
	} else {
		tris_moh_stop(qe->chan);
	}
	...
	if (announceposition == 1) {
		...
		if (qe->pos == 1) {
			...
			if (qe->parent->announceposition == ANNOUNCEPOSITION_MORE_THAN && qe->pos > qe->parent->announcepositionlimit){
				...
				if (res)
					goto playout;
				res = tris_say_number(qe->chan, qe->parent->announcepositionlimit, TRIS_DIGIT_ANY, qe->chan->language, NULL); /* Needs gender */
				if (res)
					goto playout;
			} else {
				/* Normal Case */
				...
				if (res)
					goto playout;
				res = tris_say_number(qe->chan, qe->pos, TRIS_DIGIT_ANY, qe->chan->language, NULL); /* Needs gender */
				if (res)
					goto playout;
			}
			if (qe->parent->announceposition == ANNOUNCEPOSITION_MORE_THAN && qe->pos > qe->parent->announcepositionlimit){
	...
	tris_verb(3, "Hold time for %s is %d minute(s) %d seconds\n", qe->parent->name, avgholdmins, avgholdsecs);
    ...
    if ((avgholdmins+avgholdsecs) > 0 && qe->parent->announceholdtime &&
		...
		if (avgholdmins >= 1) {
			res = tris_say_number(qe->chan, avgholdmins, TRIS_DIGIT_ANY, qe->chan->language, NULL);
			if (res)
				goto playout;
		...
		if (avgholdsecs >= 1) {
			res = tris_say_number(qe->chan, avgholdmins > 1 ? avgholdsecs : avgholdmins * 60 + avgholdsecs, TRIS_DIGIT_ANY, qe->chan->language, NULL);
			if (res)
				goto playout;
...
posout:
	if (qe->parent->announceposition) {
		tris_verb(3, "Told %s in %s their queue position (which was %d)\n",
...
playout:
	...
	if (!res) {
		if (ringing) {
			tris_indicate(qe->chan, TRIS_CONTROL_RINGING);
		} else {
			tris_moh_start(qe->chan, qe->moh, NULL);
		}
	}
	return res;
}
...
static void leave_queue(struct queue_ent *qe)
{
	...
	for (current = q->head; current; current = current->next) {
		if (current == qe) {
			...
			tris_debug(1, "Queue '%s' Leave, Channel '%s'\n", q->name, qe->chan->name );
			/* Take us out of the queue */
			if (prev)
				...
				q->head = current->next;
			/* Free penalty rules */
			while ((pr_iter = TRIS_LIST_REMOVE_HEAD(&qe->qe_rules, list)))
				tris_free(pr_iter);
		} else {
	...
	if (q->realtime) {
		...
		if (!(var = tris_load_realtime("queues", "name", q->name, SENTINEL))) {
			q->dead = 1;
		} else {
...
static void hangupcalls(struct callattempt *outgoing, struct tris_channel *exception, int cancel_answered_elsewhere)
{
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		if (q->count && q->members) {
			if ((mem = ao2_find(q->members, member, OBJ_POINTER))) {
				tris_debug(1, "Found matching member %s in queue '%s'\n", mem->interface, q->name);
				if (q->weight > rq->weight && q->count >= num_available_members(q)) {
					tris_debug(1, "Queue '%s' (weight %d, calls %d) is preferred over '%s' (weight %d, calls %d)\n", q->name, q->weight, q->count, rq->name, rq->weight, rq->count);
					found = 1;
				}
				ao2_ref(mem, -1);
			}
...
static char *vars2manager(struct tris_channel *chan, char *vars, size_t len)
{
	struct tris_str *buf = tris_str_thread_get(&tris_str_thread_global_buf, len + 1);
	char *tmp;
	...
	if (pbx_builtin_serialize_variables(chan, &buf)) {
		...
		for (i = 0, j = 10; (i < len - 1) && (j < len - 1); i++, j++) {
			...
			if (tmp[i] == '\n') {
				...
				tris_copy_string(&(vars[j]), "Variable: ", len - j);
				j += 9;
			}
		}
		if (j > len - 3)
...
static int ring_entry(struct queue_ent *qe, struct callattempt *tmp, int *busies)
{
	...
	char tmpcid[256], *amark;

	/* on entry here, we know that tmp->chan == NULL */
	if ((tmp->lastqueue && tmp->lastqueue->wrapuptime && (time(NULL) - tmp->lastcall < tmp->lastqueue->wrapuptime)) ||
		(!tmp->lastqueue && qe->parent->wrapuptime && (time(NULL) - tmp->lastcall < qe->parent->wrapuptime))) {
		tris_debug(1, "Wrapuptime not yet expired on queue %s for %s\n", 
				(tmp->lastqueue ? tmp->lastqueue->name : qe->parent->name), tmp->interface);
		if (qe->chan->cdr)
			tris_cdr_busy(qe->chan->cdr);
		tmp->stillgoing = 0;
		(*busies)++;
		return 0;
	}

	if (!qe->parent->ringinuse && (tmp->member->status != TRIS_DEVICE_NOT_INUSE) && (tmp->member->status != TRIS_DEVICE_UNKNOWN)) {
		tris_debug(1, "%s in use, can't receive call\n", tmp->interface);
		if (qe->chan->cdr)
			tris_cdr_busy(qe->chan->cdr);
		tmp->stillgoing = 0;
		return 0;
	...
	if (tmp->member->paused) {
		tris_debug(1, "%s paused, can't receive call\n", tmp->interface);
		if (qe->chan->cdr)
			tris_cdr_busy(qe->chan->cdr);
		tmp->stillgoing = 0;
		return 0;
	...
	if (use_weight && compare_weight(qe->parent,tmp->member)) {
		tris_debug(1, "Priority queue delaying call to %s:%s\n", qe->parent->name, tmp->interface);
		if (qe->chan->cdr)
			tris_cdr_busy(qe->chan->cdr);
		tmp->stillgoing = 0;
		(*busies)++;
	...
	tris_copy_string(tech, tmp->interface, sizeof(tech));
	...
	/* Request the peer */
	tmp->chan = tris_request(tech, qe->chan->nativeformats, location, &status, 0);
	if (!tmp->chan) {			/* If we can't, just go on to the next call */
		...
		ao2_lock(qe->parent);
		update_status(qe->parent, tmp->member, tris_device_state(tmp->member->state_interface));
		qe->parent->rrpos++;
		qe->linpos++;
		ao2_unlock(qe->parent);
	...
	if (qe->cancel_answered_elsewhere) {
		tris_set_flag(tmp->chan, TRIS_FLAG_ANSWERED_ELSEWHERE);
	}
	tmp->chan->appl = "AppQueue";
	tmp->chan->data = "(Outgoing Line)";
	memset(&tmp->chan->whentohangup, 0, sizeof(tmp->chan->whentohangup));
	if (tmp->chan->cid.cid_num)
		tris_free(tmp->chan->cid.cid_num);
	tmp->chan->cid.cid_num = tris_strdup(qe->chan->cid.cid_num);
	if (tmp->chan->cid.cid_name)
		tris_free(tmp->chan->cid.cid_name);
	tmp->chan->cid.cid_name = tris_strdup(qe->chan->cid.cid_name);
	if (tmp->chan->cid.cid_ani)
		tris_free(tmp->chan->cid.cid_ani);
	tmp->chan->cid.cid_ani = tris_strdup(qe->chan->cid.cid_ani);

	/* Inherit specially named variables from parent channel */
	tris_channel_inherit_variables(qe->chan, tmp->chan);
	tris_channel_datastore_inherit(qe->chan, tmp->chan);

	/* Presense of ADSI CPE on outgoing channel follows ours */
	tmp->chan->adsicpe = qe->chan->adsicpe;

	/* Inherit context and extension */
	tris_channel_lock(qe->chan);
	macrocontext = pbx_builtin_getvar_helper(qe->chan, "MACRO_CONTEXT");
	tris_string_field_set(tmp->chan, dialcontext, tris_strlen_zero(macrocontext) ? qe->chan->context : macrocontext);
	macroexten = pbx_builtin_getvar_helper(qe->chan, "MACRO_EXTEN");
	if (!tris_strlen_zero(macroexten))
		tris_copy_string(tmp->chan->exten, macroexten, sizeof(tmp->chan->exten));
	else
		tris_copy_string(tmp->chan->exten, qe->chan->exten, sizeof(tmp->chan->exten));
	if (tris_cdr_isset_unanswered()) {
		/* they want to see the unanswered dial attempts! */
		/* set up the CDR fields on all the CDRs to give sensical information */
		tris_cdr_setdestchan(tmp->chan->cdr, tmp->chan->name);
	...
	tris_channel_unlock(qe->chan);

	/* Place the call, but don't wait on the answer */
	if ((res = tris_call(tmp->chan, location, 0))) {
	...
	snprintf(tmpcid, sizeof(tmpcid), "%s", location);
	amark = strchr(tmpcid, '@');
	if (amark)
		*amark = '\0';
	if (!tris_strlen_zero(tmpcid)) {
		if (tmp->chan->cid.cid_num)
			tris_free(tmp->chan->cid.cid_num);
		tmp->chan->cid.cid_num = tris_strdup(tmpcid);
	}
	return 1;
}
...
static int ring_one(struct queue_ent *qe, struct callattempt *outgoing, int *busies)
{
	...
	while (ret == 0) {
		...
		if (!best) {
			tris_debug(1, "Nobody left to try ringing in queue\n");
			break;
		}
		if (qe->parent->strategy == QUEUE_STRATEGY_RINGALL) {
			...
			for (cur = outgoing; cur; cur = cur->q_next) {
				if (cur->stillgoing && !cur->chan && cur->metric <= best->metric) {
					tris_debug(1, "(Parallel) Trying '%s' with metric %d\n", cur->interface, cur->metric);
					ret |= ring_entry(qe, cur, busies);
				}
			}
		} else {
			/* Ring just the best channel */
			tris_debug(1, "Trying '%s' with metric %d\n", best->interface, best->metric);
			ret = ring_entry(qe, best, busies);
		}
		
		/* If we have timed out, break out */
		if (qe->expire && (time(NULL) >= qe->expire)) {
			tris_debug(1, "Queue timed out while ringing members.\n");
			ret = 0;
			break;
		}
	}
...
static int store_next_rr(struct queue_ent *qe, struct callattempt *outgoing)
{
	...
	if (best) {
		/* Ring just the best channel */
		tris_debug(1, "Next is '%s' with metric %d\n", best->interface, best->metric);
...
static int say_periodic_announcement(struct queue_ent *qe, int ringing)
{
	...
	tris_verb(3, "Playing periodic announcement\n");
	
	if (qe->parent->randomperiodicannounce && qe->parent->numperiodicannounce) {
		qe->last_periodic_announce_sound = ((unsigned long) tris_random()) % qe->parent->numperiodicannounce;
	} else if (qe->last_periodic_announce_sound >= qe->parent->numperiodicannounce || 
		tris_str_strlen(qe->parent->sound_periodicannounce[qe->last_periodic_announce_sound]) == 0) {
		qe->last_periodic_announce_sound = 0;
	}
	
	/* play the announcement */
	res = play_file(qe->chan, tris_str_buffer(qe->parent->sound_periodicannounce[qe->last_periodic_announce_sound]));
...
static void rna(int rnatime, struct queue_ent *qe, char *interface, char *membername, int pause)
{
	...
	tris_queue_log(qe->parent->name, qe->chan->uniqueid, membername, "RINGNOANSWER", "%d", rnatime);
	if (qe->parent->autopause && pause) {
		if (!set_member_paused(qe->parent->name, interface, "Auto-Pause", 1)) {
			tris_verb(3, "Auto-Pausing Queue Member %s in queue %s since they failed to answer.\n", interface, qe->parent->name);
		} else {
			tris_verb(3, "Failed to pause Queue Member %s in queue %s!\n", interface, qe->parent->name);
		}
	}
	return;
}
...
static struct callattempt *wait_for_answer(struct queue_ent *qe, struct callattempt *outgoing, int *to, char *digit, int prebusies, int caller_disconnect, int forwardsallowed)
{
	...
	struct tris_channel *in = qe->chan;
	char on[80] = "";
	char membername[80] = "";
	long starttime = 0;
	long endtime = 0;
	...
	while (*to && !peer) {
		...
		if (pos == 1 /* not found */) {
			if (numlines == (numbusies + numnochan)) {
				tris_debug(1, "Everyone is busy at this time\n");
			} else {
				tris_debug(3, "No one is answering queue '%s' (%d numlines / %d busies / %d failed channels)\n", queue, numlines, numbusies, numnochan);
			}
			*to = 0;
			return NULL;
		}
		...
		for (o = start; o; o = o->call_next) {
			if (o->stillgoing && (o->chan) &&  (o->chan->_state == TRIS_STATE_UP)) {
				if (!peer) {
				...
				tris_copy_string(on, o->member->interface, sizeof(on));
				tris_copy_string(membername, o->member->membername, sizeof(membername));

				if (!tris_strlen_zero(o->chan->call_forward) && !forwardsallowed) {
					tris_verb(3, "Forwarding %s to '%s' prevented.\n", in->name, o->chan->call_forward);
					numnochan++;
					do_hang(o);
					winner = NULL;
					continue;
				} else if (!tris_strlen_zero(o->chan->call_forward)) {
					char tmpchan[256];
					char *stuff;
					char *tech;

					tris_copy_string(tmpchan, o->chan->call_forward, sizeof(tmpchan));
					...
					/* Setup parameters */
					o->chan = tris_request(tech, in->nativeformats, stuff, &status, 0);
					if (!o->chan) {
						tris_log(LOG_NOTICE, "Unable to create local channel for call forward to '%s/%s'\n", tech, stuff);
						o->stillgoing = 0;
						numnochan++;
					} else {
						...
						if (o->chan->cid.cid_num)
							tris_free(o->chan->cid.cid_num);
						o->chan->cid.cid_num = tris_strdup(in->cid.cid_num);

						if (o->chan->cid.cid_name)
							tris_free(o->chan->cid.cid_name);
						o->chan->cid.cid_name = tris_strdup(in->cid.cid_name);

						tris_string_field_set(o->chan, accountcode, in->accountcode);
						o->chan->cdrflags = in->cdrflags;

						if (in->cid.cid_ani) {
							if (o->chan->cid.cid_ani)
								tris_free(o->chan->cid.cid_ani);
							o->chan->cid.cid_ani = tris_strdup(in->cid.cid_ani);
						}
						if (o->chan->cid.cid_rdnis)
							tris_free(o->chan->cid.cid_rdnis);
						o->chan->cid.cid_rdnis = tris_strdup(S_OR(in->macroexten, in->exten));
						if (tris_call(o->chan, tmpchan, 0)) {
							tris_log(LOG_NOTICE, "Failed to dial on local channel for call forward to '%s'\n", tmpchan);
							do_hang(o);
							numnochan++;
						}
					}
				...
				if (f) {
					if (f->frametype == TRIS_FRAME_CONTROL) {
						...
						case TRIS_CONTROL_PROGRESS:
							tris_verb(3, "%s is progress\n", o->chan->name);
							break;
						case TRIS_CONTROL_OFFHOOK:
							/* Ignore going off hook */
							break;
						default:
							tris_verb(3, "Dunno what to do with control type %d\n", f->subclass);
						}
					}
					tris_frfree(f);
				} else { /* tris_read() returned NULL */
		...
		if (winner == in) {
			...
			if ((f->frametype == TRIS_FRAME_DTMF) && caller_disconnect && (f->subclass == '*')) {
				tris_verb(3, "User hit %c to disconnect call.\n", f->subclass);
				*to = 0;
				tris_frfree(f);
				return NULL;
			}
			if ((f->frametype == TRIS_FRAME_DTMF) && valid_exit(qe, f->subclass)) {
				tris_verb(3, "User pressed digit: %c\n", f->subclass);
				*to = 0;
				*digit = f->subclass;
				tris_frfree(f);
				return NULL;
...
static int is_our_turn(struct queue_ent *qe)
{
	...
	tris_debug(1, "There %s %d available %s.\n", avl != 1 ? "are" : "is", avl, avl != 1 ? "members" : "member");
	...
	if (ch && idx < avl && (qe->parent->autofill || qe->pos == 1)) {
		tris_debug(1, "It's our turn (%s).\n", qe->chan->name);
		res = 1;
	} else {
		tris_debug(1, "It's not our turn (%s).\n", qe->chan->name);
		res = 0;
	}
...
static void update_qe_rule(struct queue_ent *qe)
{
	...
	tris_debug(3, "Setting max penalty to %d and min penalty to %d for caller %s since %d seconds have elapsed\n", qe->max_penalty, qe->min_penalty, qe->chan->name, qe->pr->time);
	qe->pr = TRIS_LIST_NEXT(qe->pr, list);
}
...
static int wait_our_turn(struct queue_ent *qe, int ringing, enum queue_result *reason)
{
	...
	for (;;) {
		...
		if (qe->parent->leavewhenempty) {
			...
			if ((status = get_member_status(qe->parent, qe->max_penalty, qe->min_penalty, qe->parent->leavewhenempty))) {
				*reason = QUEUE_LEAVEEMPTY;
				tris_queue_log(qe->parent->name, qe->chan->uniqueid, "NONE", "EXITEMPTY", "%d|%d|%ld", qe->pos, qe->opos, (long) time(NULL) - qe->start);
				leave_queue(qe);
				break;
			}
		}
		...
		if ((res = tris_waitfordigit(qe->chan, RECHECK * 1000))) {
			if (res > 0 && !valid_exit(qe, res))
				res = 0;
			else
				break;
...
static int calc_metric(struct call_queue *q, struct member *mem, int pos, struct queue_ent *qe, struct callattempt *tmp)
{
	...
	case QUEUE_STRATEGY_RANDOM:
		tmp->metric = tris_random() % 1000;
		tmp->metric += mem->penalty * 1000000;
		break;
	case QUEUE_STRATEGY_WRANDOM:
		tmp->metric = tris_random() % ((1 + mem->penalty) * 1000);
		break;
	...
	case QUEUE_STRATEGY_LEASTRECENT:
		...
		break;
	default:
		tris_log(LOG_WARNING, "Can't calculate metric for unknown strategy %d\n", q->strategy);
		break;
	}
	return 0;
}
...
static void send_agent_complete(const struct queue_ent *qe, const char *queuename,
	const struct tris_channel *peer, const struct member *member, time_t callstart,
 ...
 */
static const struct tris_datastore_info queue_transfer_info = {
	.type = "queue_transfer",
...
static void queue_transfer_fixup(void *data, struct tris_channel *old_chan, struct tris_channel *new_chan)
{
	...
	tris_queue_log(qe->parent->name, qe->chan->uniqueid, member->membername, "TRANSFER", "%s|%s|%ld|%ld|%d",
				new_chan->exten, new_chan->context, (long) (callstart - qe->start),
				(long) (time(NULL) - callstart), qe->opos);

	update_queue(qe->parent, member, callcompletedinsl, (time(NULL) - callstart));
	
	/* No need to lock the channels because they are already locked in tris_do_masquerade */
	if ((datastore = tris_channel_datastore_find(old_chan, &queue_transfer_info, NULL))) {
		tris_channel_datastore_remove(old_chan, datastore);
	} else {
		tris_log(LOG_WARNING, "Can't find the queue_transfer datastore.\n");
	}
}
 ...
 */
static int attended_transfer_occurred(struct tris_channel *chan)
{
	return tris_channel_datastore_find(chan, &queue_transfer_info, NULL) ? 0 : 1;
}

/*! \brief create a datastore for storing relevant info to log attended transfers in the queue_log
 */
static struct tris_datastore *setup_transfer_datastore(struct queue_ent *qe, struct member *member, time_t starttime, int callcompletedinsl)
{
	struct tris_datastore *ds;
	struct queue_transfer_ds *qtds = tris_calloc(1, sizeof(*qtds));

	if (!qtds) {
		tris_log(LOG_WARNING, "Memory allocation error!\n");
		return NULL;
	}

	tris_channel_lock(qe->chan);
	if (!(ds = tris_datastore_alloc(&queue_transfer_info, NULL))) {
		tris_channel_unlock(qe->chan);
		tris_log(LOG_WARNING, "Unable to create transfer datastore. queue_log will not show attended transfer\n");
		return NULL;
	}
	...
	ds->data = qtds;
	tris_channel_datastore_add(qe->chan, ds);
	tris_channel_unlock(qe->chan);
	return ds;
}
...
static void end_bridge_callback(void *data)
{
	...
	struct tris_channel *chan = qeb->chan;
...
static int try_calling(struct queue_ent *qe, const char *options, char *announceoverride, const char *url, int *tries, int *noption, const char *agi, const char *macro, const char *gosub, int ringing)
{
	...
	int to, orig;
	char oldexten[TRIS_MAX_EXTENSION]="";
	char oldcontext[TRIS_MAX_CONTEXT]="";
	char queuename[256]="";
	char interfacevar[256]="";
	struct tris_channel *peer;
	struct tris_channel *which;
	struct callattempt *lpeer;
	struct member *member;
	struct tris_app *application;
	int res = 0, bridge = 0;
	int numbusies = 0;
	int x=0;
	char *announce = NULL;
	...
	struct tris_bridge_config bridge_config;
	...
	struct tris_app *mixmonapp = NULL;
	char *p;
	char vars[2048];
	int forwardsallowed = 1;
	int callcompletedinsl;
	struct ao2_iterator memi;
	struct tris_datastore *datastore, *transfer_ds;
	struct queue_end_bridge *queue_end_bridge = NULL;
	const int need_weight = use_weight;

	tris_channel_lock(qe->chan);
	datastore = tris_channel_datastore_find(qe->chan, &dialed_interface_info, NULL);
	tris_channel_unlock(qe->chan);
	...
	for (; options && *options; options++)
		switch (*options) {
		case 't':
			tris_set_flag(&(bridge_config.features_callee), TRIS_FEATURE_REDIRECT);
			break;
		case 'T':
			tris_set_flag(&(bridge_config.features_caller), TRIS_FEATURE_REDIRECT);
			break;
		case 'w':
			tris_set_flag(&(bridge_config.features_callee), TRIS_FEATURE_AUTOMON);
			break;
		case 'W':
			tris_set_flag(&(bridge_config.features_caller), TRIS_FEATURE_AUTOMON);
			break;
		case 'c':
			tris_set_flag(&(bridge_config.features_caller), TRIS_FEATURE_NO_H_EXTEN);
			break;
		case 'd':
			nondataquality = 0;
			break;
		case 'h':
			tris_set_flag(&(bridge_config.features_callee), TRIS_FEATURE_DISCONNECT);
			break;
		case 'H':
			tris_set_flag(&(bridge_config.features_caller), TRIS_FEATURE_DISCONNECT);
			break;
		case 'k':
			tris_set_flag(&(bridge_config.features_callee), TRIS_FEATURE_PARKCALL);
			break;
		case 'K':
			tris_set_flag(&(bridge_config.features_caller), TRIS_FEATURE_PARKCALL);
			break;
		case 'n':
		...
		case 'i':
			...
			break;
		case 'x':
			tris_set_flag(&(bridge_config.features_callee), TRIS_FEATURE_AUTOMIXMON);
			break;
		case 'X':
			tris_set_flag(&(bridge_config.features_caller), TRIS_FEATURE_AUTOMIXMON);
			break;
		case 'C':
	...
	*/
	if (tris_test_flag(qe->chan, TRIS_FLAG_ANSWERED_ELSEWHERE)) {
		qe->cancel_answered_elsewhere = 1;
	}
	...
	if (need_weight)
		ao2_lock(queues);
	ao2_lock(qe->parent);
	tris_debug(1, "%s is trying to call a queue member.\n",
							qe->chan->name);
	tris_copy_string(queuename, qe->parent->name, sizeof(queuename));
	if (!tris_strlen_zero(qe->announce))
		announce = qe->announce;
	if (!tris_strlen_zero(announceoverride))
		announce = announceoverride;

	memi = ao2_iterator_init(qe->parent->members, 0);
	while ((cur = ao2_iterator_next(&memi))) {
		struct callattempt *tmp = tris_calloc(1, sizeof(*tmp));
		struct tris_dialed_interface *di;
		TRIS_LIST_HEAD(, tris_dialed_interface) *dialed_interfaces;
		if (!tmp) {
			ao2_ref(cur, -1);
			ao2_unlock(qe->parent);
		...
		if (!datastore) {
			...
			if (!(dialed_interfaces = tris_calloc(1, sizeof(*dialed_interfaces)))) {
				ao2_ref(cur, -1);
				ao2_unlock(&qe->parent);
				ao2_iterator_destroy(&memi);
				if (need_weight)
			...
			TRIS_LIST_HEAD_INIT(dialed_interfaces);

			tris_channel_lock(qe->chan);
			tris_channel_datastore_add(qe->chan, datastore);
			tris_channel_unlock(qe->chan);
		} else
			dialed_interfaces = datastore->data;

		TRIS_LIST_LOCK(dialed_interfaces);
		TRIS_LIST_TRAVERSE(dialed_interfaces, di, list) {
			if (!strcasecmp(cur->interface, di->interface)) {
				tris_debug(1, "Skipping dialing interface '%s' since it has already been dialed\n", 
					di->interface);
				break;
			}
		}
		TRIS_LIST_UNLOCK(dialed_interfaces);
		...
		if (strncasecmp(cur->interface, "Local/", 6)) {
			if (!(di = tris_calloc(1, sizeof(*di) + strlen(cur->interface)))) {
				ao2_ref(cur, -1);
				ao2_unlock(qe->parent);
				ao2_iterator_destroy(&memi);
				if (need_weight)
			...
			TRIS_LIST_LOCK(dialed_interfaces);
			TRIS_LIST_INSERT_TAIL(dialed_interfaces, di, list);
			TRIS_LIST_UNLOCK(dialed_interfaces);
		}
		...
		tris_copy_string(tmp->interface, cur->interface, sizeof(tmp->interface));
		...
		if (!calc_metric(qe->parent, cur, x++, qe, tmp)) {
			...
			if (outgoing->chan && (outgoing->chan->_state == TRIS_STATE_UP))
				break;
		} else {
			ao2_ref(cur, -1);
			tris_free(tmp);
	...
	tris_localtime(&nowval, &tm, NULL);
	tris_strftime(currenttime, sizeof(currenttime), DATE_FORMAT, &tm);
	
	char itemid[10];
	snprintf(itemid, sizeof(itemid), "%08lx", tris_random());
	
	//char result[32], sql[256];
	//snprintf(sql, sizeof(sql), "INSERT INTO queue_recording(caller,date) values ('%s','%s')",qe->chan->cid.cid_num,currenttime);
	//sql_select_query_execute(result, sql);
	//sql_select_query_execute(result, "SELECT LAST_INSERT_ID()");

	int qcount = 0;
	struct callattempt *tmp1;
	for (tmp1 = outgoing; tmp1; tmp1 = tmp1->q_next) {
		if (tmp1->chan)
			qcount++;
	}

	lpeer = wait_for_answer(qe, outgoing, &to, &digit, numbusies, tris_test_flag(&(bridge_config.features_caller), TRIS_FEATURE_DISCONNECT), forwardsallowed);

	if (!lpeer && qcount == qcount_n) {
		play_file(qe->chan, "queue/unavailable");
		return -1;
	}
	 ...
	 */
	tris_channel_lock(qe->chan);
	if (datastore && !tris_channel_datastore_remove(qe->chan, datastore)) {
	...
	if (!peer) {
		...
		if (res == -1)
			tris_debug(1, "%s: Nobody answered.\n", qe->chan->name);
		if (tris_cdr_isset_unanswered()) {
			...
			for (o = outgoing; o; o = o->q_next) {
				...
				if (strcmp(o->chan->cdr->dstchannel, qe->chan->cdr->dstchannel) == 0) {
					tris_set_flag(o->chan->cdr, TRIS_CDR_FLAG_POST_DISABLED);
					break;
				}
			}
		}
		...
		if (!strcmp(qe->chan->tech->type, "DAHDI"))
			tris_channel_setoption(qe->chan, TRIS_OPTION_TONE_VERIFY, &nondataquality, sizeof(nondataquality), 0);
		if (!strcmp(peer->tech->type, "DAHDI"))
			tris_channel_setoption(peer, TRIS_OPTION_TONE_VERIFY, &nondataquality, sizeof(nondataquality), 0);
		...
		if (announce || qe->parent->reportholdtime || qe->parent->memberdelay) {
			...
			res2 = tris_autoservice_start(qe->chan);
			if (!res2) {
				if (qe->parent->memberdelay) {
					tris_log(LOG_NOTICE, "Delaying member connect for %d seconds\n", qe->parent->memberdelay);
					res2 |= tris_safe_sleep(peer, qe->parent->memberdelay * 1000);
				}
				if (!res2 && announce) {
				...
				if (!res2 && qe->parent->reportholdtime) {
					if (!play_file(peer, qe->parent->sound_reporthold)) {
						...
						if (holdtime > 0) {
							tris_say_number(peer, holdtime, TRIS_DIGIT_ANY, peer->language, NULL);
							play_file(peer, qe->parent->sound_minutes);
						}
						if (holdtimesecs > 1) {
							tris_say_number(peer, holdtimesecs, TRIS_DIGIT_ANY, peer->language, NULL);
							play_file(peer, qe->parent->sound_seconds);
						}
					}
				}
			}
			res2 |= tris_autoservice_stop(qe->chan);
			if (tris_check_hangup(peer)) {
				/* Agent must have hung up */
				tris_log(LOG_WARNING, "Agent on %s hungup on the customer.\n", peer->name);
				tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "AGENTDUMP", "%s", "");
				if (qe->parent->eventwhencalled)
				...
				tris_log(LOG_NOTICE, "Caller was about to talk to agent on %s but the caller hungup.\n", peer->name);
				tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "ABANDON", "%d|%d|%ld", qe->pos, qe->opos, (long) time(NULL) - qe->start);
				record_abandoned(qe);
				tris_cdr_noanswer(qe->chan->cdr);
				tris_hangup(peer);
				ao2_ref(member, -1);
				return -1;
			}
		...
		if (qe->chan->cdr)
			tris_cdr_setdestchan(qe->chan->cdr, peer->name);
		/* Make sure channels are compatible */
		res = tris_channel_make_compatible(qe->chan, peer);
		if (res < 0) {
			tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "SYSCOMPAT", "%s", "");
			tris_log(LOG_WARNING, "Had to drop call because I couldn't make %s compatible with %s\n", qe->chan->name, peer->name);
			record_abandoned(qe);
			tris_cdr_failed(qe->chan->cdr);
			tris_hangup(peer);
			ao2_ref(member, -1);
			return -1;
		}
		...
		if (!tris_strlen_zero(qe->parent->sound_callerannounce)) {
			if (play_file(qe->chan, qe->parent->sound_callerannounce))
				tris_log(LOG_WARNING, "Announcement file '%s' is unavailable, continuing anyway...\n", qe->parent->sound_callerannounce);
		}
		...
		deststr1 = tris_strdupa(member->interface);
		
		deststr2 = strsep(&deststr1,"/");
		deststr2 = strsep(&deststr1,"@");

		char tmpmonitor[256],recordingfile[256],filename[256];
		snprintf(filename,256,"%s_%s_%s",qe->chan->cid.cid_num,deststr2,currenttime);
		snprintf(tmpmonitor,256,"128/in-%s",filename);
		snprintf(recordingfile,256,"/var/spool/trismedia/monitor/128/out-%s",filename);
		...
		if (qe->parent->monfmt && *qe->parent->monfmt) {
			if (!qe->parent->montype) {
				...
				tris_debug(1, "Starting Monitor as requested.\n");
				tris_channel_lock(qe->chan);
				if ((monexec = pbx_builtin_getvar_helper(qe->chan, "MONITOR_EXEC")) || (monargs = pbx_builtin_getvar_helper(qe->chan, "MONITOR_EXEC_ARGS"))) {
					which = qe->chan;
					monexec = monexec ? tris_strdupa(monexec) : NULL;
				}
				else
					which = peer;
				tris_channel_unlock(qe->chan);
				if (tris_monitor_start) {
					if (!tris_strlen_zero(tmpmonitor)) {
						tris_monitor_start(which, qe->parent->monfmt, tmpmonitor, 1, X_REC_IN | X_REC_OUT);
					} else if (qe->chan->cdr && tris_monitor_start) {
						tris_monitor_start(which, qe->parent->monfmt, qe->chan->cdr->uniqueid, 1, X_REC_IN | X_REC_OUT);
					} else if (tris_monitor_start) {
						/* Last ditch effort -- no CDR, make up something */
						snprintf(tmpid, sizeof(tmpid), "chan-%lx", tris_random());
						tris_monitor_start(which, qe->parent->monfmt, tmpid, 1, X_REC_IN | X_REC_OUT);
					}
				}
				if (!tris_strlen_zero(monexec) && tris_monitor_setjoinfiles) {
					tris_monitor_setjoinfiles(which, 1);
				}
			} else {
				mixmonapp = pbx_findapp("MixMonitor");
				
				if (mixmonapp) {
					tris_debug(1, "Starting MixMonitor as requested.\n");
					if (tris_strlen_zero(tmpmonitor)) {
						if (qe->chan->cdr)
							tris_copy_string(tmpid, qe->chan->cdr->uniqueid, sizeof(tmpid));
						else
							snprintf(tmpid, sizeof(tmpid), "chan-%lx", tris_random());
					} else {
						const char *m = tmpmonitor;
					...
					if ((monitor_exec = pbx_builtin_getvar_helper(qe->chan, "MONITOR_EXEC"))) {
							monitor_exec = tris_strdupa(monitor_exec);
					}
					if ((monitor_options = pbx_builtin_getvar_helper(qe->chan, "MONITOR_OPTIONS"))) {
							monitor_options = tris_strdupa(monitor_options);
					} else {
						monitor_options = "";
					}
					tris_channel_unlock(qe->chan);
					...
					if (!tris_strlen_zero(monitor_exec))
					...
					tris_debug(1, "Arguments being passed to MixMonitor: %s\n", mixmonargs);
					/* We purposely lock the CDR so that pbx_exec does not update the application data */
					if (qe->chan->cdr)
						tris_set_flag(qe->chan->cdr, TRIS_CDR_FLAG_LOCKED);
					ret = pbx_exec(qe->chan, mixmonapp, mixmonargs);
					if (qe->chan->cdr)
						tris_clear_flag(qe->chan->cdr, TRIS_CDR_FLAG_LOCKED);

				} else {
					tris_log(LOG_WARNING, "Asked to run MixMonitor on this call, but cannot find the MixMonitor app!\n");
				}
			}
		}
		}
		...
		leave_queue(qe);			
		if (!tris_strlen_zero(url) && tris_channel_supports_html(peer)) {
			tris_debug(1, "app_queue: sendurl=%s.\n", url);
			tris_channel_sendurl(peer, url);
		}
		...
		if (!tris_strlen_zero(macro)) {
				macroexec = tris_strdupa(macro);
		} else {
			if (qe->parent->membermacro)
				macroexec = tris_strdupa(qe->parent->membermacro);
		}

		if (!tris_strlen_zero(macroexec)) {
			tris_debug(1, "app_queue: macro=%s.\n", macroexec);
			
			res = tris_autoservice_start(qe->chan);
			if (res) {
			...
			if (tris_autoservice_stop(qe->chan) < 0) {
		...
		if (!tris_strlen_zero(gosub)) {
				gosubexec = tris_strdupa(gosub);
		} else {
			if (qe->parent->membergosub)
				gosubexec = tris_strdupa(qe->parent->membergosub);
		}

		if (!tris_strlen_zero(gosubexec)) {
			tris_debug(1, "app_queue: gosub=%s.\n", gosubexec);
			...
			if (application) {
				...
				tris_copy_string(peer->context, "app_queue_gosub_virtual_context", sizeof(peer->context));
		...
		if (!tris_strlen_zero(agi)) {
			tris_debug(1, "app_queue: agi=%s.\n", agi);
			application = pbx_findapp("agi");
			if (application) {
				agiexec = tris_strdupa(agi);
				ret = pbx_exec(qe->chan, application, agiexec);
			} else
				tris_log(LOG_WARNING, "Asked to execute an AGI on this channel, but could not find application (agi)!\n");
		}
		qe->handled++;
		tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "CONNECT", "%ld|%s|%ld", (long) time(NULL) - qe->start, peer->uniqueid,
													(long)(orig - to > 0 ? (orig - to) / 1000 : 0));
		if (update_cdr && qe->chan->cdr) 
			tris_copy_string(qe->chan->cdr->dstchannel, member->membername, sizeof(qe->chan->cdr->dstchannel));
		...
		if (qe->parent->eventwhencalled)
			manager_event(EVENT_FLAG_AGENT, "AgentConnect",
					...
					qe->parent->eventwhencalled == QUEUE_EVENT_VARIABLES ? vars2manager(qe->chan, vars, sizeof(vars)) : "");
		tris_copy_string(oldcontext, qe->chan->context, sizeof(oldcontext));
		tris_copy_string(oldexten, qe->chan->exten, sizeof(oldexten));
		...
		pbx_builtin_setvar_helper(qe->chan, "announce-greeting", qe->parent->sound_greeting);
		pbx_builtin_setvar_helper(qe->chan, "ringmode", qe->parent->ringmode);
		pbx_builtin_setvar_helper(qe->chan, "notifycaller", qe->parent->notifycaller);
		if (!tris_strlen_zero(qe->parent->sound_greeting) && tris_fileexists(qe->parent->sound_greeting, NULL, qe->chan->language)) {
			if (qe->parent->listenmenu) {
				res = tris_streamfile(peer, qe->parent->sound_greeting, qe->chan->language);
				tris_autoservice_start(qe->chan);
				tris_streamfile(qe->chan, qe->parent->sound_greeting, qe->chan->language);
				if (!res)
					res = tris_waitstream(peer, TRIS_DIGIT_ANY);
				tris_autoservice_stop(qe->chan);
				tris_stopstream(qe->chan);
				tris_stopstream(peer);
			} else {
				if (play_file(qe->chan, qe->parent->sound_greeting))
					tris_log(LOG_WARNING, "Announcement file 2 '%s' is unavailable, continuing anyway...\n", qe->parent->sound_greeting);
			}
		}
		bridge = tris_bridge_call(qe->chan,peer, &bridge_config);
		...
		if (!attended_transfer_occurred(qe->chan)) {
			struct tris_datastore *tds;

			/* detect a blind transfer */
			if (!(qe->chan->_softhangup | peer->_softhangup) && (strcasecmp(oldcontext, qe->chan->context) || strcasecmp(oldexten, qe->chan->exten))) {
				tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "TRANSFER", "%s|%s|%ld|%ld|%d",
					qe->chan->exten, qe->chan->context, (long) (callstart - qe->start),
					(long) (time(NULL) - callstart), qe->opos);
				send_agent_complete(qe, queuename, peer, member, callstart, vars, sizeof(vars), TRANSFER);
			} else if (tris_check_hangup(qe->chan)) {
				tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "COMPLETECALLER", "%ld|%ld|%d",
					(long) (callstart - qe->start), (long) (time(NULL) - callstart), qe->opos);
				send_agent_complete(qe, queuename, peer, member, callstart, vars, sizeof(vars), CALLER);
			} else {
				tris_queue_log(queuename, qe->chan->uniqueid, member->membername, "COMPLETEAGENT", "%ld|%ld|%d",
					(long) (callstart - qe->start), (long) (time(NULL) - callstart), qe->opos);
				send_agent_complete(qe, queuename, peer, member, callstart, vars, sizeof(vars), AGENT);
			}
			if ((tds = tris_channel_datastore_find(qe->chan, &queue_transfer_info, NULL))) {	
				tris_channel_datastore_remove(qe->chan, tds);
			}
			update_queue(qe->parent, member, callcompletedinsl, (time(NULL) - callstart));
		}

		if (transfer_ds) {
			tris_datastore_free(transfer_ds);
		}
		tris_channel_unlock(qe->chan);
		tris_hangup(peer);
		res = bridge ? bridge : 1;
		...
		//snprintf(sql, sizeof(sql), "UPDATE queue_recording SET callee='%s',date='%s',calltime='%ld',filename='%s' WHERE id='%s'",deststr2,currenttime,(long)(time(NULL) - callstart),filename,result);
		//sql_select_query_execute(result, sql);	
...
static int wait_a_bit(struct queue_ent *qe)
{
	...
	int res = tris_waitfordigit(qe->chan, retrywait);
	if (res > 0 && !valid_exit(qe, res))
		res = 0;
...
static void dump_queue_members(struct call_queue *pm_queue)
{
	...
	while ((cur_member = ao2_iterator_next(&mem_iter))) {
		...
		if (res != strlen(value + value_len)) {
			tris_log(LOG_WARNING, "Could not create persistent member string, out of space\n");
			break;
		}
		value_len += res;
	}
	...
	if (value_len && !cur_member) {
		if (tris_db_put(pm_family, pm_queue->name, value))
			tris_log(LOG_WARNING, "failed to create persistent dynamic entry!\n");
	} else
		/* Delete the entry if the queue is empty or there is an error */
		tris_db_del(pm_family, pm_queue->name);
}
...
static int remove_from_queue(const char *queuename, const char *interface)
{
	...
	tris_copy_string(tmpmem.interface, interface, sizeof(tmpmem.interface));
...
static int set_member_paused(const char *queuename, const char *interface, const char *reason, int paused)
{
	...
	if (tris_strlen_zero(queuename))
		tris_queue_log("NONE", "NONE", interface, (paused ? "PAUSEALL" : "UNPAUSEALL"), "%s", "");

	queue_iter = ao2_iterator_init(queues, 0);
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate over queues"))) {
		ao2_lock(q);
		if (tris_strlen_zero(queuename) || !strcasecmp(q->name, queuename)) {
			if ((mem = interface_exists(q, interface))) {
				if (mem->paused == paused) {
					tris_debug(1, "%spausing already-%spaused queue member %s:%s\n", (paused ? "" : "un"), (paused ? "" : "un"), q->name, interface);
				}
				...
				if (failed) {
					tris_log(LOG_WARNING, "Failed %spausing realtime queue member %s:%s\n", (paused ? "" : "un"), q->name, interface);
					ao2_ref(mem, -1);
					ao2_unlock(q);
					queue_t_unref(q, "Done with iterator");
					continue;
				...
				tris_queue_log(q->name, "NONE", mem->membername, (paused ? "PAUSE" : "UNPAUSE"), "%s", S_OR(reason, ""));
		...
		if (!tris_strlen_zero(queuename) && !strcasecmp(queuename, q->name)) {
			ao2_unlock(q);
			queue_t_unref(q, "Done with iterator");
			break;
		}
...
static int set_member_penalty(char *queuename, char *interface, int penalty)
{
	...
	if (penalty < 0) {
		tris_log(LOG_ERROR, "Invalid penalty (%d)\n", penalty);
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		if (tris_strlen_zero(queuename) || !strcasecmp(q->name, queuename)) {
			...
			if ((mem = interface_exists(q, interface))) {
				...
				tris_queue_log(q->name, "NONE", interface, "PENALTY", "%d", penalty);
	...
	if (foundinterface) {
		...
		tris_log (LOG_ERROR, "Invalid queuename\n"); 
	} else {
		tris_log (LOG_ERROR, "Invalid interface\n");
	}	
...
static void reload_queue_members(void)
{
	...
	db_tree = tris_db_gettree(pm_family, NULL);
	for (entry = db_tree; entry; entry = entry->next) {
		...
		if (!cur_queue) {
			 ...
			 * database */
			tris_log(LOG_WARNING, "Error loading persistent queue: '%s': it does not exist\n", queue_name);
			tris_db_del(pm_family, queue_name);
			continue;
		} 

		if (tris_db_get(pm_family, queue_name, queue_data, PM_MAX_LEN)) {
		...
		while ((member = strsep(&cur_ptr, ",|"))) {
			...
			if (!penalty_tok) {
				tris_log(LOG_WARNING, "Error parsing persistent member string for '%s' (penalty)\n", queue_name);
				break;
			}
			penalty = strtol(penalty_tok, NULL, 10);
			if (errno == ERANGE) {
				tris_log(LOG_WARNING, "Error converting penalty: %s: Out of range.\n", penalty_tok);
				break;
			}
			
			if (!paused_tok) {
				tris_log(LOG_WARNING, "Error parsing persistent member string for '%s' (paused)\n", queue_name);
				break;
			}
			paused = strtol(paused_tok, NULL, 10);
			if ((errno == ERANGE) || paused < 0 || paused > 1) {
				tris_log(LOG_WARNING, "Error converting paused: %s: Expected 0 or 1.\n", paused_tok);
				break;
			}

			tris_debug(1, "Reload Members: Queue: %s  Member: %s  Name: %s  Penalty: %d  Paused: %d\n", queue_name, interface, membername, penalty, paused);
			
			if (add_to_queue(queue_name, interface, membername, penalty, paused, 0, state_interface) == RES_OUTOFMEMORY) {
				tris_log(LOG_ERROR, "Out of Memory when reloading persistent queue member\n");
				break;
			}
		}
		queue_t_unref(cur_queue, "Expire reload reference");
	...
	ao2_unlock(queues);
	if (db_tree) {
		tris_log(LOG_NOTICE, "Queue members successfully reloaded from database.\n");
		tris_db_freetree(db_tree);
	}
}

/*! \brief PauseQueueMember application */
static int pqm_exec(struct tris_channel *chan, void *data)
{
	char *parse;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(queuename);
		TRIS_APP_ARG(interface);
		TRIS_APP_ARG(options);
		TRIS_APP_ARG(reason);
	);

	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "PauseQueueMember requires an argument ([queuename],interface[,options][,reason])\n");
		return -1;
	}
	...
	if (tris_strlen_zero(args.interface)) {
		tris_log(LOG_WARNING, "Missing interface argument to PauseQueueMember ([queuename],interface[,options[,reason]])\n");
		return -1;
	}

	if (set_member_paused(args.queuename, args.interface, args.reason, 1)) {
		tris_log(LOG_WARNING, "Attempt to pause interface %s, not found\n", args.interface);
...
static int upqm_exec(struct tris_channel *chan, void *data)
{
	char *parse;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(queuename);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "UnpauseQueueMember requires an argument ([queuename],interface[,options[,reason]])\n");
		return -1;
	}
	...
	if (set_member_paused(args.queuename, args.interface, args.reason, 0)) {
		tris_log(LOG_WARNING, "Attempt to unpause interface %s, not found\n", args.interface);
...
static int rqm_exec(struct tris_channel *chan, void *data)
{
	int res=-1;
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "RemoveQueueMember requires an argument (queuename[,interface[,options]])\n");
		return -1;
	}
	...
	if (tris_strlen_zero(args.interface)) {
		args.interface = tris_strdupa(chan->name);
		temppos = strrchr(args.interface, '-');
		if (temppos)
			*temppos = '\0';
	}

	tris_debug(1, "queue: %s, member: %s\n", args.queuename, args.interface);

	switch (remove_from_queue(args.queuename, args.interface)) {
	case RES_OKAY:
		tris_queue_log(args.queuename, chan->uniqueid, args.interface, "REMOVEMEMBER", "%s", "");
		tris_log(LOG_NOTICE, "Removed interface '%s' from queue '%s'\n", args.interface, args.queuename);
		pbx_builtin_setvar_helper(chan, "RQMSTATUS", "REMOVED");
		res = 0;
		break;
	case RES_EXISTS:
		tris_debug(1, "Unable to remove interface '%s' from queue '%s': Not there\n", args.interface, args.queuename);
		pbx_builtin_setvar_helper(chan, "RQMSTATUS", "NOTINQUEUE");
		res = 0;
		break;
	case RES_NOSUCHQUEUE:
		tris_log(LOG_WARNING, "Unable to remove interface from queue '%s': No such queue\n", args.queuename);
		pbx_builtin_setvar_helper(chan, "RQMSTATUS", "NOSUCHQUEUE");
		res = 0;
		break;
	case RES_NOT_DYNAMIC:
		tris_log(LOG_WARNING, "Unable to remove interface from queue '%s': '%s' is not a dynamic member\n", args.queuename, args.interface);
...
static int aqm_exec(struct tris_channel *chan, void *data)
{
	int res=-1;
	char *parse, *temppos = NULL;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(queuename);
		TRIS_APP_ARG(interface);
		TRIS_APP_ARG(penalty);
		TRIS_APP_ARG(options);
		TRIS_APP_ARG(membername);
		TRIS_APP_ARG(state_interface);
	);
	int penalty = 0;

	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "AddQueueMember requires an argument (queuename[,interface[,penalty[,options[,membername[,stateinterface]]]]])\n");
		return -1;
	}
	...
	if (!tris_strlen_zero(args.penalty)) {
		if ((sscanf(args.penalty, "%30d", &penalty) != 1) || penalty < 0) {
			tris_log(LOG_WARNING, "Penalty '%s' is invalid, must be an integer >= 0\n", args.penalty);
			penalty = 0;
		}
	}
	...
	case RES_OKAY:
		tris_queue_log(args.queuename, chan->uniqueid, args.interface, "ADDMEMBER", "%s", "");
		tris_log(LOG_NOTICE, "Added interface '%s' to queue '%s'\n", args.interface, args.queuename);
		pbx_builtin_setvar_helper(chan, "AQMSTATUS", "ADDED");
		res = 0;
		break;
	case RES_EXISTS:
		tris_log(LOG_WARNING, "Unable to add interface '%s' to queue '%s': Already there\n", args.interface, args.queuename);
		pbx_builtin_setvar_helper(chan, "AQMSTATUS", "MEMBERALREADY");
		res = 0;
		break;
	case RES_NOSUCHQUEUE:
		tris_log(LOG_WARNING, "Unable to add interface to queue '%s': No such queue\n", args.queuename);
		pbx_builtin_setvar_helper(chan, "AQMSTATUS", "NOSUCHQUEUE");
		res = 0;
		break;
	case RES_OUTOFMEMORY:
		tris_log(LOG_ERROR, "Out of memory adding member %s to queue %s\n", args.interface, args.queuename);
		break;
	}
...
static int ql_exec(struct tris_channel *chan, void *data)
{
	char *parse;

	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(queuename);
		TRIS_APP_ARG(uniqueid);
		TRIS_APP_ARG(membername);
		TRIS_APP_ARG(event);
		TRIS_APP_ARG(params);
	);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "QueueLog requires arguments (queuename,uniqueid,membername,event[,additionalinfo]\n");
		return -1;
	}
	...
	if (tris_strlen_zero(args.queuename) || tris_strlen_zero(args.uniqueid)
	    || tris_strlen_zero(args.membername) || tris_strlen_zero(args.event)) {
		tris_log(LOG_WARNING, "QueueLog requires arguments (queuename,uniqueid,membername,event[,additionalinfo])\n");
		return -1;
	}

	tris_queue_log(args.queuename, args.uniqueid, args.membername, args.event, 
...
static void copy_rules(struct queue_ent *qe, const char *rulename)
{
	struct penalty_rule *pr_iter;
	struct rule_list *rl_iter;
	const char *tmp = tris_strlen_zero(rulename) ? qe->parent->defaultrule : rulename;
	TRIS_LIST_LOCK(&rule_lists);
	TRIS_LIST_TRAVERSE(&rule_lists, rl_iter, list) {
		if (!strcasecmp(rl_iter->name, tmp))
			break;
	}
	if (rl_iter) {
		TRIS_LIST_TRAVERSE(&rl_iter->rules, pr_iter, list) {
			struct penalty_rule *new_pr = tris_calloc(1, sizeof(*new_pr));
			if (!new_pr) {
				tris_log(LOG_ERROR, "Memory allocation error when copying penalty rules! Aborting!\n");
				TRIS_LIST_UNLOCK(&rule_lists);
				break;
			}
			...
			TRIS_LIST_INSERT_TAIL(&qe->qe_rules, new_pr, list);
		}
	}
	TRIS_LIST_UNLOCK(&rule_lists);
}
 ...
 *    exit the queue.
 */
static int queue_exec(struct tris_channel *chan, void *data)
{
	int res=-1;
	int ringing=0;
	...
	TRIS_DECLARE_APP_ARGS(args,
		...
		TRIS_APP_ARG(options);
		TRIS_APP_ARG(url);
		TRIS_APP_ARG(announceoverride);
		TRIS_APP_ARG(queuetimeoutstr);
		TRIS_APP_ARG(agi);
		TRIS_APP_ARG(macro);
		TRIS_APP_ARG(gosub);
		TRIS_APP_ARG(rule);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Queue requires an argument: queuename[,options[,URL[,announceoverride[,timeout[,agi[,macro[,gosub[,rule]]]]]]]]\n");
		return -1;
	}
	...
	if (!tris_strlen_zero(args.queuetimeoutstr))
	...
	if (user_priority) {
		if (sscanf(user_priority, "%30d", &prio) == 1) {
			tris_debug(1, "%s: Got priority %d from ${QUEUE_PRIO}.\n", chan->name, prio);
		} else {
			tris_log(LOG_WARNING, "${QUEUE_PRIO}: Invalid value (%s), channel %s.\n",
				user_priority, chan->name);
			prio = 0;
		}
	} else {
		tris_debug(3, "NO QUEUE_PRIO variable found. Using default.\n");
		prio = 0;
	}
	...
	if ((max_penalty_str = pbx_builtin_getvar_helper(chan, "QUEUE_MAX_PENALTY"))) {
		if (sscanf(max_penalty_str, "%30d", &max_penalty) == 1) {
			tris_debug(1, "%s: Got max penalty %d from ${QUEUE_MAX_PENALTY}.\n", chan->name, max_penalty);
		} else {
			tris_log(LOG_WARNING, "${QUEUE_MAX_PENALTY}: Invalid value (%s), channel %s.\n",
	...
	if ((min_penalty_str = pbx_builtin_getvar_helper(chan, "QUEUE_MIN_PENALTY"))) {
		if (sscanf(min_penalty_str, "%30d", &min_penalty) == 1) {
			tris_debug(1, "%s: Got min penalty %d from ${QUEUE_MIN_PENALTY}.\n", chan->name, min_penalty);
		} else {
			tris_log(LOG_WARNING, "${QUEUE_MIN_PENALTY}: Invalid value (%s), channel %s.\n",
	...
	tris_debug(1, "queue: %s, options: %s, url: %s, announce: %s, expires: %ld, priority: %d\n",
	...
	if (join_queue(args.queuename, &qe, &reason)) {
		tris_log(LOG_WARNING, "Unable to join queue '%s' reason = %d\n", args.queuename,reason);
		set_queue_result(chan, reason);
		return 0;
	}
	tris_queue_log(args.queuename, chan->uniqueid, "NONE", "ENTERQUEUE", "%s|%s", S_OR(args.url, ""),
		S_OR(chan->cid.cid_num, ""));
	copy_rules(&qe, args.rule);
	qe.pr = TRIS_LIST_FIRST(&qe.qe_rules);
	if (strcmp(qe.parent->ringmode, "moh"))
		ringing = 1;
check_turns:
	if (ringing) {
		tris_indicate(chan, TRIS_CONTROL_RINGING);
	} else {
		tris_moh_start(chan, qe.moh, NULL);
	}
	...
	for (;;) {
		...
		if (qe.expire && (time(NULL) >= qe.expire)) {
			...
			reason = QUEUE_TIMEOUT;
			res = 0;
			tris_queue_log(args.queuename, chan->uniqueid,"NONE", "EXITWITHTIMEOUT", "%d|%d|%ld", 
		...
		if (qe.expire && (time(NULL) >= qe.expire)) {
			...
			reason = QUEUE_TIMEOUT;
			res = 0;
			tris_queue_log(args.queuename, chan->uniqueid, "NONE", "EXITWITHTIMEOUT", "%d", qe.pos);
			break;
		}
		...
		if (qe.parent->leavewhenempty) {
			...
			if ((status = get_member_status(qe.parent, qe.max_penalty, qe.min_penalty, qe.parent->leavewhenempty))) {
				...
				tris_cdr_noanswer(qe.chan->cdr);
				reason = QUEUE_LEAVEEMPTY;
				tris_queue_log(args.queuename, chan->uniqueid, "NONE", "EXITEMPTY", "%d|%d|%ld", qe.pos, qe.opos, (long)(time(NULL) - qe.start));
				res = 0;
				break;
			}
		}
		...
		if (noption && tries >= qe.parent->membercount) {
			tris_verb(3, "Exiting on time-out cycle\n");
			tris_queue_log(args.queuename, chan->uniqueid, "NONE", "EXITWITHTIMEOUT", "%d", qe.pos);
			record_abandoned(&qe);
			tris_cdr_noanswer(qe.chan->cdr);
			reason = QUEUE_TIMEOUT;
		...
		if (qe.expire && (time(NULL) >= qe.expire)) {
			...
			reason = QUEUE_TIMEOUT;
			res = 0;
			tris_queue_log(qe.parent->name, qe.chan->uniqueid,"NONE", "EXITWITHTIMEOUT", "%d|%d|%ld", qe.pos, qe.opos, (long) time(NULL) - qe.start);
			break;
		}
		 ...
		 */
		if (!is_our_turn(&qe)) {
			tris_debug(1, "Darn priorities, going back in queue (%s)!\n", qe.chan->name);
			goto check_turns;
		}
	}
	...
	if (res) {
		if (res < 0) {
			if (!qe.handled) {
				record_abandoned(&qe);
				tris_cdr_noanswer(qe.chan->cdr);
				tris_queue_log(args.queuename, chan->uniqueid, "NONE", "ABANDON",
			...
			tris_queue_log(args.queuename, chan->uniqueid, "NONE", "EXITWITHKEY",
 ...
 * \retval -1 on error
*/
static int queue_function_var(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	int res = -1;
	struct call_queue *q, tmpq = {
		.name = data,	
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_ERROR, "%s requires an argument: queuename\n", cmd);
		return -1;
	}
	...
	if ((q = ao2_t_find(queues, &tmpq, OBJ_POINTER, "Find for QUEUE() function"))) {
		...
		tris_log(LOG_WARNING, "queue %s was not found\n", data);
	}
 ...
 * \retval -1 on error
*/
static int queue_function_qac(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	int count = 0;
	struct member *m;
	struct ao2_iterator mem_iter;
	...
	if ((q = load_realtime_queue(data))) {
		...
		if (!strcasecmp(option, "logged")) {
			...
			while ((m = ao2_iterator_next(&mem_iter))) {
				...
				if ((m->status != TRIS_DEVICE_UNAVAILABLE) && (m->status != TRIS_DEVICE_INVALID)) {
					count++;
				}
				ao2_ref(m, -1);
			}
			...
			while ((m = ao2_iterator_next(&mem_iter))) {
				/* Count the agents who are logged in and presently answering calls */
				if ((m->status == TRIS_DEVICE_NOT_INUSE) && (!m->paused)) {
					count++;
				}
				ao2_ref(m, -1);
			}
 ...
 * \retval -1 on error 
*/
static int queue_function_qac_dep(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	int count = 0;
	struct member *m;
	struct call_queue *q;
	...
	if (depflag) {
		depflag = 0;
		tris_log(LOG_NOTICE, "The function QUEUE_MEMBER_COUNT has been deprecated in favor of the QUEUE_MEMBER function and will not be in further releases.\n");
	}
...
static int queue_function_queuewaitingcount(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	int count = 0;
	struct call_queue *q, tmpq = {
		.name = data,	
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_ERROR, "QUEUE_WAITING_COUNT requires an argument: queuename\n");
		return -1;
	}
	...
	} else if ((var = tris_load_realtime("queues", "name", data, SENTINEL))) {
...
static int queue_function_queuememberlist(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct call_queue *q, tmpq = {
		.name = data,	
	};
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_ERROR, "QUEUE_MEMBER_LIST requires an argument: queuename\n");
		return -1;
	}
	...
	if ((q = ao2_t_find(queues, &tmpq, OBJ_POINTER, "Find for QUEUE_MEMBER_LIST()"))) {
		...
		while ((m = ao2_iterator_next(&mem_iter))) {
			...
			if (buflen >= len - 2) {
				ao2_ref(m, -1);
				tris_log(LOG_WARNING, "Truncating list\n");
				break;
			}
			ao2_ref(m, -1);
		}
...
static int queue_function_memberpenalty_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	int penalty;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(queuename);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_ERROR, "Missing argument. QUEUE_MEMBER_PENALTY(<queuename>,<interface>)\n");
		return -1;
	}
...
static int queue_function_memberpenalty_write(struct tris_channel *chan, const char *cmd, char *data, const char *value)
{
	int penalty;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(queuename);
	...
	if (tris_strlen_zero(args.interface)) {
		tris_log (LOG_ERROR, "<interface> parameter can't be null\n");
		return -1;
	}
	...
	if (set_member_penalty(args.queuename, args.interface, penalty)) {
		tris_log(LOG_ERROR, "Invalid interface, queue or penalty\n");
		return -1;
	}
...
static struct tris_custom_function queuevar_function = {
	.name = "QUEUE_VARIABLES",
	.read = queue_function_var,
};

static struct tris_custom_function queuemembercount_function = {
	.name = "QUEUE_MEMBER",
	.read = queue_function_qac,
};

static struct tris_custom_function queuemembercount_dep = {
	.name = "QUEUE_MEMBER_COUNT",
	.read = queue_function_qac_dep,
};

static struct tris_custom_function queuewaitingcount_function = {
	.name = "QUEUE_WAITING_COUNT",
	.read = queue_function_queuewaitingcount,
};

static struct tris_custom_function queuememberlist_function = {
	.name = "QUEUE_MEMBER_LIST",
	.read = queue_function_queuememberlist,
};

static struct tris_custom_function queuememberpenalty_function = {
 ...
 * \return Always returns TRIS_MODULE_LOAD_SUCCESS
 */
static int reload_queue_rules(int reload)
{
	...
	struct penalty_rule *pr_iter;
	char *rulecat = NULL;
	struct tris_variable *rulevar = NULL;
	struct tris_flags config_flags = { reload ? CONFIG_FLAG_FILEUNCHANGED : 0 };
	
	if (!(cfg = tris_config_load("queuerules.conf", config_flags))) {
		tris_log(LOG_NOTICE, "No queuerules.conf file found, queues will not follow penalty rules\n");
		return TRIS_MODULE_LOAD_SUCCESS;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		tris_log(LOG_NOTICE, "queuerules.conf has not changed since it was last loaded. Not taking any action.\n");
		return TRIS_MODULE_LOAD_SUCCESS;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_ERROR, "Config file queuerules.conf is in an invalid format.  Aborting.\n");
		return TRIS_MODULE_LOAD_SUCCESS;
	}

	TRIS_LIST_LOCK(&rule_lists);
	while ((rl_iter = TRIS_LIST_REMOVE_HEAD(&rule_lists, list))) {
		while ((pr_iter = TRIS_LIST_REMOVE_HEAD(&rl_iter->rules, list)))
			tris_free(pr_iter);
		tris_free(rl_iter);
	}
	while ((rulecat = tris_category_browse(cfg, rulecat))) {
		if (!(new_rl = tris_calloc(1, sizeof(*new_rl)))) {
			TRIS_LIST_UNLOCK(&rule_lists);
			return TRIS_MODULE_LOAD_FAILURE;
		} else {
			tris_copy_string(new_rl->name, rulecat, sizeof(new_rl->name));
			TRIS_LIST_INSERT_TAIL(&rule_lists, new_rl, list);
			for (rulevar = tris_variable_browse(cfg, rulecat); rulevar; rulevar = rulevar->next)
				if(!strcasecmp(rulevar->name, "penaltychange"))
					insert_penaltychange(new_rl->name, rulevar->value, rulevar->lineno);
				else
					tris_log(LOG_WARNING, "Don't know how to handle rule type '%s' on line %d\n", rulevar->name, rulevar->lineno);
		}
	}
	TRIS_LIST_UNLOCK(&rule_lists);
...
static void queue_set_global_params(struct tris_config *cfg)
{
	const char *general_val = NULL;
	queue_persistent_members = 0;
	if ((general_val = tris_variable_retrieve(cfg, "general", "persistentmembers")))
		queue_persistent_members = tris_true(general_val);
	autofill_default = 0;
	if ((general_val = tris_variable_retrieve(cfg, "general", "autofill")))
		autofill_default = tris_true(general_val);
	montype_default = 0;
	if ((general_val = tris_variable_retrieve(cfg, "general", "monitor-type"))) {
		if (!strcasecmp(general_val, "mixmonitor"))
			montype_default = 1;
	}
	update_cdr = 0;
	if ((general_val = tris_variable_retrieve(cfg, "general", "updatecdr")))
		update_cdr = tris_true(general_val);
	shared_lastcall = 0;
	if ((general_val = tris_variable_retrieve(cfg, "general", "shared_lastcall")))
		shared_lastcall = tris_true(general_val);
}
...
static void reload_single_member(const char *memberdata, struct call_queue *q)
{
	...
	if (tris_strlen_zero(memberdata)) {
		tris_log(LOG_WARNING, "Empty queue member definition. Moving on!\n");
		return;
	}

	/* Add a new member */
	parse = tris_strdupa(memberdata);
	...
	if (!tris_strlen_zero(args.membername)) {
		membername = args.membername;
		tris_strip(membername);
	} else {
		membername = interface;
	}

	if (!tris_strlen_zero(args.state_interface)) {
		state_interface = args.state_interface;
		tris_strip(state_interface);
	} else {
...
static int kill_dead_members(void *obj, void *arg, int flags)
{
	...
	if (!member->delme) {
		...
		member->status = tris_device_state(member->state_interface);
		return 0;
	} else {
		q->membercount--;
		return CMP_MATCH;
 ...
 * \retval void
 */
static void reload_single_queue(struct tris_config *cfg, struct tris_flags *mask, const char *queuename)
{
	int new;
	...
	const char *tmpvar;
	const int queue_reload = tris_test_flag(mask, QUEUE_RELOAD_PARAMETERS);
	const int member_reload = tris_test_flag(mask, QUEUE_RELOAD_MEMBER);
	int prev_weight = 0;
	struct tris_variable *var;
	...
	if (q->found) {
		tris_log(LOG_WARNING, "Queue '%s' already defined! Skipping!\n", queuename);
		for (var = tris_variable_browse(cfg, queuename); var; var = var->next) {
			if (queue_reload) {
	 ...
	 */
	if (queue_reload) {
		if ((tmpvar = tris_variable_retrieve(cfg, queuename, "strategy"))) {
...
static int mark_dead_and_unfound(void *obj, void *arg, int flags)
{
	struct call_queue *q = obj;
	char *queuename = arg;
	if (!q->realtime && (tris_strlen_zero(queuename) || !strcasecmp(queuename, q->name))) {
		q->dead = 1;
		q->found = 0;
	}
	return 0;
...
static int kill_dead_queues(void *obj, void *arg, int flags)
{
	struct call_queue *q = obj;
	char *queuename = arg;
	if ((tris_strlen_zero(queuename) || !strcasecmp(queuename, q->name)) && q->dead) {
		return CMP_MATCH;
	} else {
		return 0;
	}
 ...
 * \retval 0 All clear!
 */
static int reload_queues(int reload, struct tris_flags *mask, const char *queuename)
{
	struct tris_config *cfg;
	char *cat;
	struct tris_flags config_flags = { reload ? CONFIG_FLAG_FILEUNCHANGED : 0 };
	...
	if (!(cfg = tris_config_load("queues.conf", config_flags))) {
		tris_log(LOG_NOTICE, "No call queueing config file (queues.conf), so no call queues\n");
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		return 0;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_ERROR, "Config file queues.conf is in an invalid format.  Aborting.\n");
		return -1;
	}
	...
	while ((cat = tris_category_browse(cfg, cat)) ) {
		...
		if (tris_strlen_zero(queuename) || !strcasecmp(cat, queuename))
 ...
 */
static int reload_handler(int reload, struct tris_flags *mask, const char *queuename)
{
	int res = 0;

	if (tris_test_flag(mask, QUEUE_RELOAD_RULES)) {
		res |= reload_queue_rules(reload);
	}
	if (tris_test_flag(mask, QUEUE_RESET_STATS)) {
		res |= clear_stats(queuename);
	}
	if (tris_test_flag(mask, (QUEUE_RELOAD_PARAMETERS | QUEUE_RELOAD_MEMBER))) {
...
{
	struct call_queue *q;
	struct tris_str *out = tris_str_alloca(240);
	int found = 0;
	time_t now = time(NULL);
	struct ao2_iterator queue_iter;
	struct ao2_iterator mem_iter;
	...
	} else if (tris_check_realtime("queues")) {
		/* This block is to find any queues which are defined in realtime but
		 * which have not yet been added to the in-core container
		 */
		struct tris_config *cfg = tris_load_realtime_multientry("queues", "name LIKE", "%", SENTINEL);
		char *queuename;
		if (cfg) {
			for (queuename = tris_category_browse(cfg, NULL); !tris_strlen_zero(queuename); queuename = tris_category_browse(cfg, queuename)) {
				if ((q = load_realtime_queue(queuename))) {
					queue_t_unref(q, "Done with temporary pointer");
				}
			}
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		tris_str_set(&out, 0, "%s has %d calls (max ", q->name, q->count);
		if (q->maxlen)
			tris_str_append(&out, 0, "%d", q->maxlen);
		else
			tris_str_append(&out, 0, "unlimited");
		sl = 0;
		if (q->callscompleted > 0)
			sl = 100 * ((float) q->callscompletedinsl / (float) q->callscompleted);
		tris_str_append(&out, 0, ") in '%s' strategy (%ds holdtime, %ds talktime), W:%d, C:%d, A:%d, SL:%2.1f%% within %ds",
			int2strat(q->strategy), q->holdtime, q->talktime, q->weight,
			q->callscompleted, q->callsabandoned,sl,q->servicelevel);
		do_print(s, fd, tris_str_buffer(out));
		if (!ao2_container_count(q->members))
			...
			mem_iter = ao2_iterator_init(q->members, 0);
			while ((mem = ao2_iterator_next(&mem_iter))) {
				tris_str_set(&out, 0, "      %s", mem->membername);
				if (strcasecmp(mem->membername, mem->interface)) {
					tris_str_append(&out, 0, " (%s)", mem->interface);
				}
				if (mem->penalty)
					tris_str_append(&out, 0, " with penalty %d", mem->penalty);
				tris_str_append(&out, 0, "%s%s%s (%s)",
					mem->dynamic ? " (dynamic)" : "",
					mem->realtime ? " (realtime)" : "",
					mem->paused ? " (paused)" : "",
					tris_devstate2str(mem->status));
				if (mem->calls)
					tris_str_append(&out, 0, " has taken %d calls (last was %ld secs ago)",
						mem->calls, (long) (time(NULL) - mem->lastcall));
				else
					tris_str_append(&out, 0, " has taken no calls yet");
				do_print(s, fd, tris_str_buffer(out));
				ao2_ref(mem, -1);
			}
			ao2_iterator_destroy(&mem_iter);
		...
		if (!q->head)
			...
			for (qe = q->head; qe; qe = qe->next) {
				tris_str_set(&out, 0, "      %d. %s (wait: %ld:%2.2ld, prio: %d)",
	...
	if (!found) {
		if (argc == 3)
			tris_str_set(&out, 0, "No such queue: %s.", argv[2]);
		else
			tris_str_set(&out, 0, "No queues.");
		do_print(s, fd, tris_str_buffer(out));
	}
	return CLI_SUCCESS;
}
...
static char *queue_show(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch ( cmd ) {
	case CLI_INIT:
...
static int manager_queues_summary(struct mansession *s, const struct message *m)
{
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		if (tris_strlen_zero(queuefilter) || !strcmp(q->name, queuefilter)) {
			...
			mem_iter = ao2_iterator_init(q->members, 0);
			while ((mem = ao2_iterator_next(&mem_iter))) {
				if ((mem->status != TRIS_DEVICE_UNAVAILABLE) && (mem->status != TRIS_DEVICE_INVALID)) {
					++qmemcount;
					if (((mem->status == TRIS_DEVICE_NOT_INUSE) || (mem->status == TRIS_DEVICE_UNKNOWN)) && !(mem->paused)) {
						++qmemavail;
					}
				}
				ao2_ref(mem, -1);
...
static int manager_queues_status(struct mansession *s, const struct message *m)
{
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		if (tris_strlen_zero(queuefilter) || !strcmp(q->name, queuefilter)) {
			...
			mem_iter = ao2_iterator_init(q->members, 0);
			while ((mem = ao2_iterator_next(&mem_iter))) {
				if (tris_strlen_zero(memberfilter) || !strcmp(mem->interface, memberfilter) || !strcmp(mem->membername, memberfilter)) {
...
static int manager_add_queue_member(struct mansession *s, const struct message *m)
{
	...
	if (tris_strlen_zero(interface)) {
		astman_send_error(s, m, "'Interface' not specified.");
		return 0;
	}

	if (tris_strlen_zero(penalty_s))
		penalty = 0;
	else if (sscanf(penalty_s, "%30d", &penalty) != 1 || penalty < 0)
		penalty = 0;

	if (tris_strlen_zero(paused_s))
		paused = 0;
	else
		paused = abs(tris_true(paused_s));

	switch (add_to_queue(queuename, interface, membername, penalty, paused, queue_persistent_members, state_interface)) {
	case RES_OKAY:
		tris_queue_log(queuename, "MANAGER", interface, "ADDMEMBER", "%s", "");
...
static int manager_remove_queue_member(struct mansession *s, const struct message *m)
{
	...
	if (tris_strlen_zero(queuename) || tris_strlen_zero(interface)) {
	...
	case RES_OKAY:
		tris_queue_log(queuename, "MANAGER", interface, "REMOVEMEMBER", "%s", "");
...
static int manager_pause_queue_member(struct mansession *s, const struct message *m)
{
	...
	if (tris_strlen_zero(interface) || tris_strlen_zero(paused_s)) {
...
static int manager_queue_log_custom(struct mansession *s, const struct message *m)
{
	...
	if (tris_strlen_zero(queuename) || tris_strlen_zero(event)) {
		astman_send_error(s, m, "Need 'Queue' and 'Event' parameters.");
		return 0;
	}

	tris_queue_log(queuename, S_OR(uniqueid, "NONE"), interface, event, "%s", message);
...
{
	struct tris_flags mask = {0,};
	const char *queuename = NULL;
	int header_found = 0;

	queuename = astman_get_header(m, "Queue");
	if (!strcasecmp(S_OR(astman_get_header(m, "Members"), ""), "yes")) {
		tris_set_flag(&mask, QUEUE_RELOAD_MEMBER);
		header_found = 1;
	}
	if (!strcasecmp(S_OR(astman_get_header(m, "Rules"), ""), "yes")) {
		tris_set_flag(&mask, QUEUE_RELOAD_RULES);
		header_found = 1;
	}
	if (!strcasecmp(S_OR(astman_get_header(m, "Parameters"), ""), "yes")) {
		tris_set_flag(&mask, QUEUE_RELOAD_PARAMETERS);
		header_found = 1;
	}

	if (!header_found) {
		tris_set_flag(&mask, TRIS_FLAGS_ALL);
	}
...
{
	const char *queuename = NULL;
	struct tris_flags mask = {QUEUE_RESET_STATS,};
...
static char *complete_queue_add_member(const char *line, const char *word, int pos, int state)
{
	...
	case 4: /* only one possible match, "to" */
		return state == 0 ? tris_strdup("to") : NULL;
	case 5: /* <queue> */
		return complete_queue(line, word, pos, state);
	case 6: /* only one possible match, "penalty" */
		return state == 0 ? tris_strdup("penalty") : NULL;
	case 7:
		if (state < 100) {      /* 0-99 */
			char *num;
			if ((num = tris_malloc(3))) {
				sprintf(num, "%d", state);
			}
			return num;
		} else {
	...
	case 8: /* only one possible match, "as" */
		return state == 0 ? tris_strdup("as") : NULL;
...
static int manager_queue_member_penalty(struct mansession *s, const struct message *m)
{
	...
	if (tris_strlen_zero(interface) || tris_strlen_zero(penalty_s)) {
...
static char *handle_queue_add_member(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (a->argc >= 8) {
		if (sscanf(a->argv[7], "%30d", &penalty) == 1) {
			if (penalty < 0) {
				tris_cli(a->fd, "Penalty must be >= 0\n");
				penalty = 0;
			}
		} else {
			tris_cli(a->fd, "Penalty must be an integer >= 0\n");
			penalty = 0;
		}
	} else {
		penalty = 0;
	...
	case RES_OKAY:
		tris_queue_log(queuename, "CLI", interface, "ADDMEMBER", "%s", "");
		tris_cli(a->fd, "Added interface '%s' to queue '%s'\n", interface, queuename);
		return CLI_SUCCESS;
	case RES_EXISTS:
		tris_cli(a->fd, "Unable to add interface '%s' to queue '%s': Already there\n", interface, queuename);
		return CLI_FAILURE;
	case RES_NOSUCHQUEUE:
		tris_cli(a->fd, "Unable to add interface to queue '%s': No such queue\n", queuename);
		return CLI_FAILURE;
	case RES_OUTOFMEMORY:
		tris_cli(a->fd, "Out of memory\n");
		return CLI_FAILURE;
	case RES_NOT_DYNAMIC:
		tris_cli(a->fd, "Member not dynamic\n");
		return CLI_FAILURE;
	default:
		return CLI_FAILURE;
	}
...
static char *complete_queue_remove_member(const char *line, const char *word, int pos, int state)
{
	...
	if (pos == 4)   /* only one possible match, 'from' */
		return (state == 0 ? tris_strdup("from") : NULL);
...
static char *handle_queue_remove_member(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (remove_from_queue(queuename, interface)) {
	case RES_OKAY:
		tris_queue_log(queuename, "CLI", interface, "REMOVEMEMBER", "%s", "");
		tris_cli(a->fd, "Removed interface '%s' from queue '%s'\n", interface, queuename);
		return CLI_SUCCESS;
	case RES_EXISTS:
		tris_cli(a->fd, "Unable to remove interface '%s' from queue '%s': Not there\n", interface, queuename);
		return CLI_FAILURE;
	case RES_NOSUCHQUEUE:
		tris_cli(a->fd, "Unable to remove interface from queue '%s': No such queue\n", queuename);
		return CLI_FAILURE;
	case RES_OUTOFMEMORY:
		tris_cli(a->fd, "Out of memory\n");
		return CLI_FAILURE;
	case RES_NOT_DYNAMIC:
		tris_cli(a->fd, "Unable to remove interface '%s' from queue '%s': Member is not dynamic\n", interface, queuename);
		return CLI_FAILURE;
	default:
		return CLI_FAILURE;
	}
...
static char *complete_queue_pause_member(const char *line, const char *word, int pos, int state)
{
	...
	case 4:	/* only one possible match, "queue" */
		return state == 0 ? tris_strdup("queue") : NULL;
	case 5:	/* <queue> */
		return complete_queue(line, word, pos, state);
	case 6: /* "reason" */
		return state == 0 ? tris_strdup("reason") : NULL;
...
static char *handle_queue_pause_member(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (set_member_paused(queuename, interface, reason, paused) == RESULT_SUCCESS) {
		tris_cli(a->fd, "%spaused interface '%s'", paused ? "" : "un", interface);
		if (!tris_strlen_zero(queuename))
			tris_cli(a->fd, " in queue '%s'", queuename);
		if (!tris_strlen_zero(reason))
			tris_cli(a->fd, " for reason '%s'", reason);
		tris_cli(a->fd, "\n");
		return CLI_SUCCESS;
	} else {
		tris_cli(a->fd, "Unable to %spause interface '%s'", paused ? "" : "un", interface);
		if (!tris_strlen_zero(queuename))
			tris_cli(a->fd, " in queue '%s'", queuename);
		if (!tris_strlen_zero(reason))
			tris_cli(a->fd, " for reason '%s'", reason);
...
static char *handle_queue_set_member_penalty(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	case RESULT_SUCCESS:
		tris_cli(a->fd, "Set penalty on interface '%s' from queue '%s'\n", interface, queuename);
		return CLI_SUCCESS;
	case RESULT_FAILURE:
		tris_cli(a->fd, "Failed to set penalty on interface '%s' from queue '%s'\n", interface, queuename);
		return CLI_FAILURE;
	default:
		return CLI_FAILURE;
	}
...
static char *handle_queue_rule_show(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *rule;
	struct rule_list *rl_iter;
	struct penalty_rule *pr_iter;
	...
	TRIS_LIST_LOCK(&rule_lists);
	TRIS_LIST_TRAVERSE(&rule_lists, rl_iter, list) {
		if (tris_strlen_zero(rule) || !strcasecmp(rl_iter->name, rule)) {
			tris_cli(a->fd, "Rule: %s\n", rl_iter->name);
			TRIS_LIST_TRAVERSE(&rl_iter->rules, pr_iter, list) {
				tris_cli(a->fd, "\tAfter %d seconds, adjust QUEUE_MAX_PENALTY %s %d and adjust QUEUE_MIN_PENALTY %s %d\n", pr_iter->time, pr_iter->max_relative ? "by" : "to", pr_iter->max_value, pr_iter->min_relative ? "by" : "to", pr_iter->min_value);
			}
		}
	}
	TRIS_LIST_UNLOCK(&rule_lists);
...
static char *handle_queue_reset(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct tris_flags mask = {QUEUE_RESET_STATS,};
	int i;
...
static char *handle_queue_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct tris_flags mask = {0,};
	int i;
...
static struct tris_cli_entry cli_queue[] = {
	TRIS_CLI_DEFINE(queue_show, "Show status of a specified queue"),
	TRIS_CLI_DEFINE(handle_queue_add_member, "Add a channel to a specified queue"),
	TRIS_CLI_DEFINE(handle_queue_remove_member, "Removes a channel from a specified queue"),
	TRIS_CLI_DEFINE(handle_queue_pause_member, "Pause or unpause a queue member"),
	TRIS_CLI_DEFINE(handle_queue_set_member_penalty, "Set penalty for a channel of a specified queue"),
	TRIS_CLI_DEFINE(handle_queue_rule_show, "Show the rules defined in queuerules.conf"),
	TRIS_CLI_DEFINE(handle_queue_reload, "Reload queues, members, queue rules, or parameters"),
	TRIS_CLI_DEFINE(handle_queue_reset, "Reset statistics for a queue"),
};
...
static int unload_module(void)
{
	...
	tris_cli_unregister_multiple(cli_queue, ARRAY_LEN(cli_queue));
	res = tris_manager_unregister("QueueStatus");
	res |= tris_manager_unregister("Queues");
	res |= tris_manager_unregister("QueueRule");
	res |= tris_manager_unregister("QueueSummary");
	res |= tris_manager_unregister("QueueAdd");
	res |= tris_manager_unregister("QueueRemove");
	res |= tris_manager_unregister("QueuePause");
	res |= tris_manager_unregister("QueueLog");
	res |= tris_manager_unregister("QueuePenalty");
	res |= tris_unregister_application(app_aqm);
	res |= tris_unregister_application(app_rqm);
	res |= tris_unregister_application(app_pqm);
	res |= tris_unregister_application(app_upqm);
	res |= tris_unregister_application(app_ql);
	res |= tris_unregister_application(app);
	res |= tris_custom_function_unregister(&queuevar_function);
	res |= tris_custom_function_unregister(&queuemembercount_function);
	res |= tris_custom_function_unregister(&queuemembercount_dep);
	res |= tris_custom_function_unregister(&queuememberlist_function);
	res |= tris_custom_function_unregister(&queuewaitingcount_function);
	res |= tris_custom_function_unregister(&queuememberpenalty_function);

	if (device_state_sub)
		tris_event_unsubscribe(device_state_sub);

	if ((con = tris_context_find("app_queue_gosub_virtual_context"))) {
		tris_context_remove_extension2(con, "s", 1, NULL, 0);
		tris_context_destroy(con, "app_queue"); /* leave no trace */
	}
	...
	ao2_ref(queues, -1);
	devicestate_tps = tris_taskprocessor_unreference(devicestate_tps);
	tris_unload_realtime("queue_members");
	return res;
}
...
static int load_module(void)
{
	int res;
	struct tris_context *con;
	struct tris_flags mask = {TRIS_FLAGS_ALL, };
	...
	con = tris_context_find_or_create(NULL, NULL, "app_queue_gosub_virtual_context", "app_queue");
	if (!con)
		tris_log(LOG_ERROR, "Queue virtual context 'app_queue_gosub_virtual_context' does not exist and unable to create\n");
	else
		tris_add_extension2(con, 1, "s", 1, NULL, NULL, "NoOp", tris_strdup(""), tris_free_ptr, "app_queue");

	if (queue_persistent_members)
		reload_queue_members();

	tris_cli_register_multiple(cli_queue, ARRAY_LEN(cli_queue));
	res = tris_register_application_xml(app, queue_exec);
	res |= tris_register_application_xml(app_aqm, aqm_exec);
	res |= tris_register_application_xml(app_rqm, rqm_exec);
	res |= tris_register_application_xml(app_pqm, pqm_exec);
	res |= tris_register_application_xml(app_upqm, upqm_exec);
	res |= tris_register_application_xml(app_ql, ql_exec);
	res |= tris_manager_register("Queues", 0, manager_queues_show, "Queues");
	res |= tris_manager_register("QueueStatus", 0, manager_queues_status, "Queue Status");
	res |= tris_manager_register("QueueSummary", 0, manager_queues_summary, "Queue Summary");
	res |= tris_manager_register("QueueAdd", EVENT_FLAG_AGENT, manager_add_queue_member, "Add interface to queue.");
	res |= tris_manager_register("QueueRemove", EVENT_FLAG_AGENT, manager_remove_queue_member, "Remove interface from queue.");
	res |= tris_manager_register("QueuePause", EVENT_FLAG_AGENT, manager_pause_queue_member, "Makes a queue member temporarily unavailable");
	res |= tris_manager_register("QueueLog", EVENT_FLAG_AGENT, manager_queue_log_custom, "Adds custom entry in queue_log");
	res |= tris_manager_register("QueuePenalty", EVENT_FLAG_AGENT, manager_queue_member_penalty, "Set the penalty for a queue member"); 
	res |= tris_manager_register("QueueRule", 0, manager_queue_rule_show, "Queue Rules");
	res |= tris_manager_register("QueueReload", 0, manager_queue_reload, "Reload a queue, queues, or any sub-section of a queue or queues");
	res |= tris_manager_register("QueueReset", 0, manager_queue_reset, "Reset queue statistics");
	res |= tris_custom_function_register(&queuevar_function);
	res |= tris_custom_function_register(&queuemembercount_function);
	res |= tris_custom_function_register(&queuemembercount_dep);
	res |= tris_custom_function_register(&queuememberlist_function);
	res |= tris_custom_function_register(&queuewaitingcount_function);
	res |= tris_custom_function_register(&queuememberpenalty_function);

	if (!(devicestate_tps = tris_taskprocessor_get("app_queue", 0))) {
		tris_log(LOG_WARNING, "devicestate taskprocessor reference failed - devicestate notifications will not occur\n");
	}

	if (!(device_state_sub = tris_event_subscribe(TRIS_EVENT_DEVICE_STATE, device_state_cb, NULL, TRIS_EVENT_IE_END))) {
		res = -1;
	}

	tris_realtime_require_field("queue_members", "paused", RQ_INTEGER1, 1, "uniqueid", RQ_UINTEGER2, 5, SENTINEL);

	return res ? TRIS_MODULE_LOAD_DECLINE : 0;
}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "True Call Queueing",
		.load = load_module,
