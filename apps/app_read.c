...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 180032 $")
...
TRIS_APP_OPTIONS(read_app_options, {
...
#define tris_next_data(instr,ptr,delim) if((ptr=strchr(instr,delim))) { *(ptr) = '\0' ; ptr++;}

static int read_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char tmp[256] = "";
	int maxdigits = 255;
	...
	if (!tris_strlen_zero(arglist.options)) {
		tris_app_parse_options(read_app_options, &flags, NULL, arglist.options);
	}
	
	if (!tris_strlen_zero(arglist.attempts)) {
	...
	if (!tris_strlen_zero(arglist.maxdigits)) {
		...
		if ((maxdigits < 1) || (maxdigits > 255)) {
			...
			tris_verb(3, "Accepting a maximum of %d digits.\n", maxdigits);
	}
	if (tris_strlen_zero(arglist.variable)) {
		tris_log(LOG_WARNING, "Invalid! Usage: Read(variable[,filename][,maxdigits][,option][,attempts][,timeout])\n\n");
		return 0;
	}
	if (tris_test_flag(&flags, OPT_INDICATION)) {
		if (!tris_strlen_zero(arglist.filename)) {
	...
	if (!res) {
		while (tries && !res) {
			...
			if (ts && ts->data[0]) {
				...
				res = tris_app_getdata(chan, arglist.filename, tmp, maxdigits, to);
				if (res == TRIS_GETDATA_COMPLETE || res == TRIS_GETDATA_EMPTY_END_TERMINATED)
					status = "OK";
				else if (res == TRIS_GETDATA_TIMEOUT)
					status = "TIMEOUT";
				else if (res == TRIS_GETDATA_INTERRUPTED)
					status = "INTERRUPTED";
			}
			if (res > -1) {
				...
				if (!tris_strlen_zero(tmp)) {
					...
					tries--;
					if (tries)
						tris_verb(3, "User entered nothing, %d chance%s left\n", tries, (tries != 1) ? "s" : "");
					else
...
{
	return tris_register_application_xml(app, read_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Read Variable Application");
