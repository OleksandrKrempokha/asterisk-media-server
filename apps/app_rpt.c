#define	NEW_TRISMEDIA
/* #define OLD_TRISMEDIA */
/*
 ...
 *
 *  To send an trismedia (*) while dialing or talking on phone,
...
#define	FUNCCHAR '*'
#define	ENDCHAR '#'
#define	EXTNODEFILE "/var/lib/trismedia/rpt_extnodes"
...
struct rpt_link
{
	...
	struct	tris_frame *lastf1,*lastf2;
	...
	char newkey;
#ifdef OLD_TRISMEDIA
        TRIS_LIST_HEAD(, tris_frame) rxq;
#else
	TRIS_LIST_HEAD_NOLOCK(, tris_frame) rxq;
#endif
} ;
...
{
	tris_mutex_t lock;
	tris_mutex_t remlock;
	tris_mutex_t statpost_lock;
	...
	struct tris_channel *rxchannel,*txchannel, *monchannel, *parrotchannel;
	struct tris_channel *pchannel,*txpchannel, *dahdirxchannel, *dahditxchannel;
	struct tris_channel *voxchannel;
	struct tris_frame *lastf1,*lastf2;
	...
	struct tris_filestream *monstream,*parrotstream;
	char	loginuser[50];
	char	loginlevel[10];
	long	authtelltimer;
	long	authtimer;
	...
	char inpadtest;
#ifdef OLD_TRISMEDIA
	TRIS_LIST_HEAD(, tris_frame) txq;
#else
	TRIS_LIST_HEAD_NOLOCK(, tris_frame) txq;
#endif
	char txrealkeyed;
#ifdef	__RPT_NOTCH
	struct rptfilter
...
TRIS_MUTEX_DEFINE_STATIC(nodeloglock);

TRIS_MUTEX_DEFINE_STATIC(nodelookuplock);
...
TRIS_MUTEX_DEFINE_STATIC(locklock);
...
static void rpt_mutex_spew(void)
{
	...
	for(i = 0 ; i < 32 ; i++)
	{
		...
		{
			tris_log(LOG_NOTICE,"LOCKDEBUG [#%d] UNLOCK app_rpt.c:%d node %s pid %x diff %lld us at %s.%06d\n",
		...
		{
			tris_log(LOG_NOTICE,"LOCKDEBUG [#%d] LOCK app_rpt.c:%d node %s pid %x diff %lld us at %s.%06d\n",
				i - 31,lock_ring_copy[j].line,lock_ring_copy[j].rpt->name,(int) lock_ring_copy[j].lockthread.id,diff,a,(int)lock_ring_copy[j].tv.tv_usec);
		}
	}
}
...
static void _rpt_mutex_lock(tris_mutex_t *lockp, struct rpt *myrpt, int line)
{
struct lockthread *t;
pthread_t id;
	...
	if (t->lockcount)
	{
		...
		tris_mutex_unlock(&locklock);
		tris_log(LOG_NOTICE,"rpt_mutex_lock: Double lock request line %d node %s pid %x, last lock was line %d\n",line,myrpt->name,(int) t->id,lastline);
		rpt_mutex_spew();
		return;
	}
	t->lastlock = line;
	...
	if(lock_ring_index == 32)
		lock_ring_index = 0;
	tris_mutex_unlock(&locklock);
	tris_mutex_lock(lockp);
}


static void _rpt_mutex_unlock(tris_mutex_t *lockp, struct rpt *myrpt, int line)
{
struct lockthread *t;
pthread_t id;
	...
	if (!t->lockcount)
	{
		...
		tris_mutex_unlock(&locklock);
		tris_log(LOG_NOTICE,"rpt_mutex_lock: Double un-lock request line %d node %s pid %x, last un-lock was line %d\n",line,myrpt->name,(int) t->id,lastline);
		rpt_mutex_spew();
		return;
	}
	t->lastunlock = line;
	...
	if(lock_ring_index == 32)
		lock_ring_index = 0;
	tris_mutex_unlock(&locklock);
	tris_mutex_unlock(lockp);
}

#else  /* APP_RPT_LOCK_DEBUG */

#define rpt_mutex_lock(x) tris_mutex_lock(x)
#define rpt_mutex_unlock(x) tris_mutex_unlock(x)
...
struct	statfs statfsbuf;
	...
	{
		tris_log(LOG_WARNING,"Cannot get filesystem size for %s node %s\n",
...
static void flush_telem(struct rpt *myrpt)
{
	...
	if(debug > 2)
		tris_log(LOG_NOTICE, "flush_telem()!!");
	rpt_mutex_lock(&myrpt->lock);
	telem = myrpt->tele.next;
	while(telem != &myrpt->tele)
	{
		if (telem->mode != SETREMOTE) tris_softhangup(telem->chan,TRIS_SOFTHANGUP_DEV);
		telem = telem->next;
	}
...
*/
static int priority_jump(struct rpt *myrpt, struct tris_channel *chan)
{
	int res=0;

	// if (tris_test_flag(&flags,OPT_JUMP) && tris_goto_if_exists(chan, chan->context, chan->exten, chan->priority + 101) == 0){
	if (tris_goto_if_exists(chan, chan->context, chan->exten, chan->priority + 101) == 0){
		res = 0;
	} else {
		res = -1;
	}
...
static int linkcount(struct rpt *myrpt)
{
	...
	tris_log(LOG_NOTICE, "numoflinks=%i\n",numoflinks);
	return numoflinks;
}
/*
...
static int retreive_memory(struct rpt *myrpt, char *memory)
{
	...
	if (debug)tris_log(LOG_NOTICE, "memory=%s block=%s\n",memory,myrpt->p.memory);

	val = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->p.memory, memory);
	if (!val){
		return -1;
	}			
...
static void birdbath(struct rpt *myrpt)
{
	struct rpt_tele *telem;
	if(debug > 2)
		tris_log(LOG_NOTICE, "birdbath!!");
	rpt_mutex_lock(&myrpt->lock);
	telem = myrpt->tele.next;
	while(telem != &myrpt->tele)
	{
		if (telem->mode == PARROT) tris_softhangup(telem->chan,TRIS_SOFTHANGUP_DEV);
		telem = telem->next;
	}
	rpt_mutex_unlock(&myrpt->lock);
}
...
#ifdef	NEW_TRISMEDIA
               if (l->chan) tris_senddigit(l->chan,c,0);
#else
               if (l->chan) tris_senddigit(l->chan,c);
#endif
               l = l->next;
       }
       return;
...
char	datestr[100];
	...
	nodep = (struct nodelog *)tris_malloc(sizeof(struct nodelog));
	if (nodep == NULL)
	{
		tris_log(LOG_ERROR,"Cannot get memory for node log");
		return;
	}
	...
	tris_mutex_lock(&nodeloglock);
	insque((struct qelem *) nodep, (struct qelem *) nodelog.prev);
	tris_mutex_unlock(&nodeloglock);
}
...
static const char* dtmf_tones[] = {
	...
	{
		if(debug > 6)
			tris_log(LOG_NOTICE,"time out dtmf_local_timer=%i\n",myrpt->dtmf_local_timer);
		...
		if (myrpt->dtmf_local_str[0])
		{
			...
			rpt_mutex_unlock(&myrpt->lock);
			if (digit >= '0' && digit <='9')
				tris_playtones_start(myrpt->txchannel, 0, dtmf_tones[digit-'0'], 0);
			else if (digit >= 'A' && digit <= 'D')
				tris_playtones_start(myrpt->txchannel, 0, dtmf_tones[digit-'A'+10], 0);
			else if (digit == '*')
				tris_playtones_start(myrpt->txchannel, 0, dtmf_tones[14], 0);
			else if (digit == '#')
				tris_playtones_start(myrpt->txchannel, 0, dtmf_tones[15], 0);
			else {
				/* not handled */
				tris_log(LOG_DEBUG, "Unable to generate DTMF tone '%c' for '%s'\n", digit, myrpt->txchannel->name);
			}
			rpt_mutex_lock(&myrpt->lock);
		}
		else
...
struct termios mode;
	...
	if (tcgetattr(fd, &mode)) {
		tris_log(LOG_WARNING, "Unable to get serial parameters for dtr: %s\n", strerror(errno));
		return -1;
	}
	if (enable)
	{
	...
	if (tcsetattr(fd, TCSADRAIN, &mode)) {
		tris_log(LOG_WARNING, "Unable to set serial parameters for dtr: %s\n", strerror(errno));
		return -1;
	}
...
static int openserial(struct rpt *myrpt,char *fname)
{
	...
	if (fd == -1)
	{
		tris_log(LOG_WARNING,"Cannot open serial port %s\n",fname);
		return -1;
	}
	memset(&mode, 0, sizeof(mode));
	if (tcgetattr(fd, &mode)) {
		tris_log(LOG_WARNING, "Unable to get serial parameters on %s: %s\n", fname, strerror(errno));
		return -1;
	}
#ifndef	SOLARIS
	cfmakeraw(&mode);
	...
	if (tcsetattr(fd, TCSANOW, &mode)) 
		tris_log(LOG_WARNING, "Unable to set serial parameters on %s: %s\n", fname, strerror(errno));
	if(!strcmp(myrpt->remoterig, remote_rig_kenwood)) setdtr(fd,0); 
	usleep(100000);
	if (debug)tris_log(LOG_NOTICE,"Opened serial port %s\n",fname);
	return(fd);	
}
...
static void mdc1200_notify(struct rpt *myrpt,char *fromnode, unsigned int unit)
{
	if (!fromnode)
	{
		tris_verbose("Got MDC-1200 ID %04X from local system (%s)\n",
			unit,myrpt->name);
	}
	else
	{
		tris_verbose("Got MDC-1200 ID %04X from node %s (%s)\n",
...
char	str[200];
	...
	while(l != &myrpt->links)
	{
		...
		wf.data = str;
		if (l->chan) tris_write(l->chan,&wf); 
		l = l->next;
	}
	return;
}
...
/*
	send trismedia frame text message on the current tx channel
*/
static int send_usb_txt(struct rpt *myrpt, char *txt) 
{
	struct tris_frame wf;
 
	if (debug)tris_log(LOG_NOTICE, "send_usb_txt %s\n",txt);
	wf.frametype = TRIS_FRAME_TEXT;
	wf.subclass = 0;
	wf.offset = 0;
	wf.mallocd = 0;
...
unsigned int seq;
	...
	str = tris_malloc(strlen(pairs) + strlen(myrpt->p.statpost_url) + 200);
	astr = tris_strdup(myrpt->p.statpost_program);
	if ((!str) || (!astr)) return;
	n = finddelim(astr,astrs,100);
	if (n < 1) return;
	tris_mutex_lock(&myrpt->statpost_lock);
	seq = ++myrpt->statpost_seqno;
	tris_mutex_unlock(&myrpt->statpost_lock);
	astrs[n++] = str;
	astrs[n] = NULL;
	time(&now);
	...
	if (!(pid = tris_safe_fork(0)))
	{
		execv(astrs[0],astrs);
		tris_log(LOG_ERROR, "exec of %s failed.\n", astrs[0]);
		perror("trismedia");
		exit(0);
	}
	tris_free(astr);
...
static struct tris_config *ourcfg = NULL;
struct tris_variable *vp;

	/* try to look it up locally first */
	val = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->p.nodes, digitbuf);
	if (val) return(val);
	tris_mutex_lock(&nodelookuplock);
	/* if file does not exist */
	if (stat(myrpt->p.extnodefile,&mystat) == -1)
	{
		if (ourcfg) tris_config_destroy(ourcfg);
		ourcfg = NULL;
		tris_mutex_unlock(&nodelookuplock);
		return(NULL);
	}
	...
	if (mystat.st_mtime > last)
	{
		if (ourcfg) tris_config_destroy(ourcfg);
#ifdef	NEW_TRISMEDIA
		ourcfg = tris_config_load(myrpt->p.extnodefile,config_flags);
#else
		ourcfg = tris_config_load(myrpt->p.extnodefile);
#endif
		...
		longestnode = 0;
		vp = tris_variable_browse(myrpt->cfg, myrpt->p.nodes);
		while(vp){
			j = strlen(vp->name);
			if (j > longestnode)
				longestnode = j;
		...
		vp = tris_variable_browse(ourcfg, myrpt->p.extnodes);
		while(vp){
			j = strlen(vp->name);
			if (j > longestnode)
				longestnode = j;
	...
	val = NULL;
	if (ourcfg)
		val = (char *) tris_variable_retrieve(ourcfg, myrpt->p.extnodes, digitbuf);
	tris_mutex_unlock(&nodelookuplock);
	return(val);
}
...
/*
 ...
 Note: Trismedia requires a copy of localtime
 ...
 If OLD_TRISMEDIA is defined, then the older localtime_r
 function will be used. The /etc/localtime file is not
 required in this case. This provides backward compatibility
 with Trismedia 1.2 systems.

*/

#ifdef	NEW_TRISMEDIA
static void rpt_localtime( time_t * t, struct tris_tm *lt)
{
	struct timeval when;

	when.tv_sec = *t;
	when.tv_usec = 0;
	tris_localtime(&when, lt, NULL);
}
...
static int retrieve_astcfgint(struct rpt *myrpt,char *category, char *name, int min, int max, int defl)
{
	...
	if(min < 0){ /* If min is negative, this means include 0 as a valid entry */
        ...
        var = (char *) tris_variable_retrieve(myrpt->cfg, category, name);
        if(var){
                ret = myatoi(var);
		if(include_zero && !ret)
			return 0;
...
static char *cs_keywords[] = {"rptena","rptdis","apena","apdis","lnkena","lnkdis","totena","totdis","skena","skdis",
	...
	if (option_verbose > 2)
		tris_verbose(VERBOSE_PREFIX_3 "%s config for repeater %s\n",
			(init) ? "Loading initial" : "Re-Loading",rpt_vars[n].name);
	tris_mutex_lock(&rpt_vars[n].lock);
	if (rpt_vars[n].cfg) tris_config_destroy(rpt_vars[n].cfg);
#ifdef	NEW_TRISMEDIA
	cfg = tris_config_load("rpt.conf",config_flags);
#else
	cfg = tris_config_load("rpt.conf");
#endif
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		tris_mutex_unlock(&rpt_vars[n].lock);
 		tris_log(LOG_NOTICE, "Unable to open radio repeater configuration rpt.conf.  Radio Repeater disabled.\n");
		pthread_exit(NULL);
	}
	rpt_vars[n].cfg = cfg; 
	...
	if (init)
	{
		...
		rpt_vars[n].rpt_thread = TRIS_PTHREADT_NULL;
	...
	val = (char *) tris_variable_retrieve(cfg,this,"tonezone");
	if (val) rpt_vars[n].p.tonezone = val;
	rpt_vars[n].p.tailmessages[0] = 0;
	rpt_vars[n].p.tailmessagemax = 0;
	val = (char *) tris_variable_retrieve(cfg,this,"tailmessagelist");
	if (val) rpt_vars[n].p.tailmessagemax = finddelim(val, rpt_vars[n].p.tailmessages, 500);
	val = (char *) tris_variable_retrieve(cfg,this,"memory");
	if (!val) val = MEMORY;
	rpt_vars[n].p.memory = val;
	val = (char *) tris_variable_retrieve(cfg,this,"macro");
	if (!val) val = MACRO;
	rpt_vars[n].p.macro = val;
	val = (char *) tris_variable_retrieve(cfg,this,"tonemacro");
	if (!val) val = TONEMACRO;
	rpt_vars[n].p.tonemacro = val;
	val = (char *) tris_variable_retrieve(cfg,this,"startup_macro");
	if (val) rpt_vars[n].p.startupmacro = val;
	val = (char *) tris_variable_retrieve(cfg,this,"iobase");
	/* do not use atoi() here, we need to be able to have
		the input specified in hex or decimal so we use
		sscanf with a %i */
	if ((!val) || (sscanf(val,"%30i",&rpt_vars[n].p.iobase) != 1))
		rpt_vars[n].p.iobase = DEFAULT_IOBASE;
	val = (char *) tris_variable_retrieve(cfg,this,"ioport");
	rpt_vars[n].p.ioport = val;
	val = (char *) tris_variable_retrieve(cfg,this,"functions");
	if (!val)
		{
			val = FUNCTIONS;
	...
	val =  (char *) tris_variable_retrieve(cfg,this,"link_functions");
	if (val) rpt_vars[n].p.link_functions = val;
	else 
		rpt_vars[n].p.link_functions = rpt_vars[n].p.functions;
	val = (char *) tris_variable_retrieve(cfg,this,"phone_functions");
	if (val) rpt_vars[n].p.phone_functions = val;
	val = (char *) tris_variable_retrieve(cfg,this,"dphone_functions");
	if (val) rpt_vars[n].p.dphone_functions = val;
	val = (char *) tris_variable_retrieve(cfg,this,"alt_functions");
	if (val) rpt_vars[n].p.alt_functions = val;
	val = (char *) tris_variable_retrieve(cfg,this,"funcchar");
	if (!val) rpt_vars[n].p.funcchar = FUNCCHAR; else 
		rpt_vars[n].p.funcchar = *val;		
	val = (char *) tris_variable_retrieve(cfg,this,"endchar");
	if (!val) rpt_vars[n].p.endchar = ENDCHAR; else 
		rpt_vars[n].p.endchar = *val;		
	val = (char *) tris_variable_retrieve(cfg,this,"nobusyout");
	if (val) rpt_vars[n].p.nobusyout = tris_true(val);
	val = (char *) tris_variable_retrieve(cfg,this,"notelemtx");
	if (val) rpt_vars[n].p.notelemtx = tris_true(val);
	val = (char *) tris_variable_retrieve(cfg,this,"propagate_dtmf");
	if (val) rpt_vars[n].p.propagate_dtmf = tris_true(val);
	val = (char *) tris_variable_retrieve(cfg,this,"propagate_phonedtmf");
	if (val) rpt_vars[n].p.propagate_phonedtmf = tris_true(val);
	val = (char *) tris_variable_retrieve(cfg,this,"linktolink");
	if (val) rpt_vars[n].p.linktolink = tris_true(val);
	val = (char *) tris_variable_retrieve(cfg,this,"nodes");
	if (!val) val = NODES;
	rpt_vars[n].p.nodes = val;
	val = (char *) tris_variable_retrieve(cfg,this,"extnodes");
	if (!val) val = EXTNODES;
	rpt_vars[n].p.extnodes = val;
	val = (char *) tris_variable_retrieve(cfg,this,"extnodefile");
	if (!val) val = EXTNODEFILE;
	rpt_vars[n].p.extnodefile = val;
	val = (char *) tris_variable_retrieve(cfg,this,"archivedir");
	if (val) rpt_vars[n].p.archivedir = val;
	val = (char *) tris_variable_retrieve(cfg,this,"authlevel");
	if (val) rpt_vars[n].p.authlevel = atoi(val); 
	else rpt_vars[n].p.authlevel = 0;
	val = (char *) tris_variable_retrieve(cfg,this,"parrot");
	if (val) rpt_vars[n].p.parrotmode = tris_true(val) * 2;
	else rpt_vars[n].p.parrotmode = 0;
	val = (char *) tris_variable_retrieve(cfg,this,"parrottime");
	if (val) rpt_vars[n].p.parrottime = atoi(val); 
	else rpt_vars[n].p.parrottime = PARROTTIME;
	val = (char *) tris_variable_retrieve(cfg,this,"rptnode");
	rpt_vars[n].p.rptnode = val;
	val = (char *) tris_variable_retrieve(cfg,this,"mars");
	if (val) rpt_vars[n].p.remote_mars = atoi(val); 
	else rpt_vars[n].p.remote_mars = 0;
	val = (char *) tris_variable_retrieve(cfg,this,"monminblocks");
	if (val) rpt_vars[n].p.monminblocks = atol(val); 
	else rpt_vars[n].p.monminblocks = DEFAULT_MONITOR_MIN_DISK_BLOCKS;
	val = (char *) tris_variable_retrieve(cfg,this,"remote_inact_timeout");
	if (val) rpt_vars[n].p.remoteinacttimeout = atoi(val); 
	else rpt_vars[n].p.remoteinacttimeout = DEFAULT_REMOTE_INACT_TIMEOUT;
	val = (char *) tris_variable_retrieve(cfg,this,"civaddr");
	if (val) rpt_vars[n].p.civaddr = atoi(val); 
	else rpt_vars[n].p.civaddr = DEFAULT_CIV_ADDR;
	val = (char *) tris_variable_retrieve(cfg,this,"remote_timeout");
	if (val) rpt_vars[n].p.remotetimeout = atoi(val); 
	else rpt_vars[n].p.remotetimeout = DEFAULT_REMOTE_TIMEOUT;
	val = (char *) tris_variable_retrieve(cfg,this,"remote_timeout_warning");
	if (val) rpt_vars[n].p.remotetimeoutwarning = atoi(val); 
	else rpt_vars[n].p.remotetimeoutwarning = DEFAULT_REMOTE_TIMEOUT_WARNING;
	val = (char *) tris_variable_retrieve(cfg,this,"remote_timeout_warning_freq");
...
#ifdef	__RPT_NOTCH
	val = (char *) tris_variable_retrieve(cfg,this,"rxnotch");
	if (val) {
...
#endif
	val = (char *) tris_variable_retrieve(cfg,this,"inxlat");
	if (val) {
	...
	val = (char *) tris_variable_retrieve(cfg,this,"outxlat");
	if (val) {
	...
	val = (char *) tris_variable_retrieve(cfg,this,"controlstates");
	rpt_vars[n].p.csstanzaname = val;
		
	/* retreive the stanza name for the scheduler if there is one */
	val = (char *) tris_variable_retrieve(cfg,this,"scheduler");
	rpt_vars[n].p.skedstanzaname = val;

	/* retreive the stanza name for the txlimits */
	val = (char *) tris_variable_retrieve(cfg,this,"txlimits");
	rpt_vars[n].p.txlimitsstanzaname = val;

	longestnode = 0;

	vp = tris_variable_browse(cfg, rpt_vars[n].p.nodes);
	...
	*/
	...
	vp = tris_variable_browse(cfg, rpt_vars[n].p.functions);
	while(vp){
		j = strlen(vp->name);
	...
	*/
	...
	vp = tris_variable_browse(cfg, rpt_vars[n].p.link_functions);
	while(vp){
		j = strlen(vp->name);
	...
	{
		vp = tris_variable_browse(cfg, rpt_vars[n].p.phone_functions);
		while(vp){
			j = strlen(vp->name);
	...
	{
		vp = tris_variable_browse(cfg, rpt_vars[n].p.dphone_functions);
		while(vp){
			j = strlen(vp->name);
	...
	{
		vp = tris_variable_browse(cfg, rpt_vars[n].p.alt_functions);
		while(vp){
			j = strlen(vp->name);
	...
	vp = tris_variable_browse(cfg, rpt_vars[n].p.macro);
	while(vp){
		j = strlen(vp->name);
	...
	if(rpt_vars[n].p.csstanzaname)
		vp = tris_variable_browse(cfg, rpt_vars[n].p.csstanzaname);
	else
		vp = NULL;
...
static int rpt_do_debug(int fd, int argc, char *argv[])
{
	int newlevel;
        ...
        if((newlevel < 0) || (newlevel > 7))
                return RESULT_SHOWUSAGE;
        if(newlevel)
                tris_cli(fd, "app_rpt Debugging enabled, previous level: %d, new level: %d\n", debug, newlevel);
        else
                tris_cli(fd, "app_rpt Debugging disabled\n");
	...
	for(i = 0; i < nrpts; i++)
	{
		if (!strcmp(argv[2],rpt_vars[i].name))
		{
			rpt_vars[i].disgorgetime = time(NULL) + 10; /* Do it 10 seconds later */
		        tris_cli(fd, "app_rpt struct dump requested for node %s\n",argv[2]);
		        return RESULT_SUCCESS;
		}
	}
	...
	for(i = 0; i < nrpts; i++)
	{
		if (!strcmp(argv[2],rpt_vars[i].name)){
			...
			while(l && (l != &myrpt->links)){
				...
				listoflinks[numoflinks] = tris_strdup(l->name);
			...
			if(strlen(myrpt->exten)){
				called_number = tris_strdup(myrpt->exten);
			}

			if(strlen(myrpt->lastdtmfcommand)){
				lastdtmfcommand = tris_strdup(myrpt->lastdtmfcommand);
			}
			rpt_mutex_unlock(&myrpt->lock); /* UNLOCK */

			tris_cli(fd, "************************ NODE %s STATISTICS *************************\n\n", myrpt->name);
			tris_cli(fd, "Selected system state............................: %d\n", myrpt->p.sysstate_cur);
			tris_cli(fd, "Signal on input..................................: %s\n", input_signal);
			tris_cli(fd, "System...........................................: %s\n", sys_ena);
			tris_cli(fd, "Parrot Mode......................................: %s\n", parrot_ena);
			tris_cli(fd, "Scheduler........................................: %s\n", sch_ena);
			tris_cli(fd, "Tail Time........................................: %s\n", tail_type);
			tris_cli(fd, "Time out timer...................................: %s\n", tot_ena);
			tris_cli(fd, "Time out timer state.............................: %s\n", tot_state);
			tris_cli(fd, "Time outs since system initialization............: %d\n", timeouts);
			tris_cli(fd, "Identifier state.................................: %s\n", ider_state);
			tris_cli(fd, "Kerchunks today..................................: %d\n", dailykerchunks);
			tris_cli(fd, "Kerchunks since system initialization............: %d\n", totalkerchunks);
			tris_cli(fd, "Keyups today.....................................: %d\n", dailykeyups);
			tris_cli(fd, "Keyups since system initialization...............: %d\n", totalkeyups);
			tris_cli(fd, "DTMF commands today..............................: %d\n", dailyexecdcommands);
			tris_cli(fd, "DTMF commands since system initialization........: %d\n", totalexecdcommands);
			tris_cli(fd, "Last DTMF command executed.......................: %s\n", 
			...
			tris_cli(fd, "TX time today....................................: %02d:%02d:%02d.%d\n",
			...
			tris_cli(fd, "TX time since system initialization..............: %02d:%02d:%02d.%d\n",
				 hours, minutes, seconds, (int) totaltxtime);
                        ...
                        tris_cli(fd, "Uptime...........................................: %02d:%02d:%02d\n",
                                hours, minutes, uptime);

			tris_cli(fd, "Nodes currently connected to us..................: ");
                        if(!numoflinks){
  	                      tris_cli(fd,"<NONE>");
                        }
			else{
				for(j = 0 ;j < numoflinks; j++){
					tris_cli(fd, "%s", listoflinks[j]);
					if(j % 4 == 3){
						tris_cli(fd, "\n");
						tris_cli(fd, "                                                 : ");
					}	
			...
			tris_cli(fd, "Autopatch........................................: %s\n", patch_ena);
			tris_cli(fd, "Autopatch state..................................: %s\n", patch_state);
			tris_cli(fd, "Autopatch called number..........................: %s\n",
			(called_number && strlen(called_number)) ? called_number : not_applicable);
			tris_cli(fd, "Reverse patch/IAXRPT connected...................: %s\n", reverse_patch_state);
			tris_cli(fd, "User linking commands............................: %s\n", link_ena);
			tris_cli(fd, "User functions...................................: %s\n\n", user_funs);

			for(j = 0; j < numoflinks; j++){ /* tris_free() all link names */
				tris_free(listoflinks[j]);
			}
			if(called_number){
				tris_free(called_number);
			}
			if(lastdtmfcommand){
				tris_free(lastdtmfcommand);
			}
		        return RESULT_SUCCESS;
		}
	}
	...
	for(i = 0; i < nrpts; i++)
	{
		if (!strcmp(argv[2],rpt_vars[i].name)){
			...
			while(l && (l != &myrpt->links)){
				...
				if((s = (struct rpt_lstat *) tris_malloc(sizeof(struct rpt_lstat))) == NULL){
					tris_log(LOG_ERROR, "Malloc failed in rpt_do_lstats\n");
					rpt_mutex_unlock(&myrpt->lock); /* UNLOCK */
					return RESULT_FAILURE;
				}
			...
			rpt_mutex_unlock(&myrpt->lock); /* UNLOCK */
			tris_cli(fd, "NODE      PEER                RECONNECTS  DIRECTION  CONNECT TIME        CONNECT STATE\n");
			tris_cli(fd, "----      ----                ----------  ---------  ------------        -------------\n");
			...
			for(s = s_head.next; s != &s_head; s = s->next){
				...
				tris_cli(fd, "%-10s%-20s%-12d%-11s%-20s%-20s\n",
...
int	i;
	...
	for(i = 0; i < nrpts; i++)
	{
		if (rpt_vars[i].rxchannel) tris_softhangup(rpt_vars[i].rxchannel,TRIS_SOFTHANGUP_DEV);
	}
	return RESULT_FAILURE;
}
...
static int rpt_do_fun(int fd, int argc, char *argv[])
{
	...
	if(busy){
		tris_cli(fd, "Function decoder busy");
	}
	return RESULT_FAILURE;
}
/*
...
static int rpt_push_alt_macro(struct rpt *myrpt, char *sptr)
{
	...
	if(!busy){
		int x;
		if (debug)tris_log(LOG_NOTICE, "rpt_push_alt_macro %s\n",sptr);
		myrpt->macrotimer = MACROTIME;
		for(x = 0; *(sptr + x); x++)
	...
	if(busy)tris_log(LOG_WARNING, "Function decoder busy on app_rpt command macro.\n");
...
static int rpt_do_cmd(int fd, int argc, char *argv[])
{
	...
	if (thisRpt < 0)
	{
		tris_cli(fd, "Unknown node number %s.\n", argv[2]);
		return RESULT_FAILURE;
	} /* if thisRpt < 0 */
	...
	if (thisAction < 0)
	{
		tris_cli(fd, "Unknown action name %s.\n", argv[3]);
		return RESULT_FAILURE;
	} /* if thisAction < 0 */
...
static int play_tone_pair(struct tris_channel *chan, int f1, int f2, int duration, int amplitude)
{
	int res;

        if ((res = tris_tonepair_start(chan, f1, f2, duration, amplitude)))
                return res;
                                                                                                                                            
        while(chan->generatordata) {
		if (tris_safe_sleep(chan,1)) return -1;
	}

        return 0;
}

static int play_tone(struct tris_channel *chan, int freq, int duration, int amplitude)
{
	return play_tone_pair(chan, freq, 0, duration, amplitude);
}

static int play_silence(struct tris_channel *chan, int duration)
{
...
static char *handle_cli_debug(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_dump(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_stats(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_nodes(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_local_nodes(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_lstats(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_reload(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_restart(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_fun(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_fun1(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_cli_cmd(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static struct tris_cli_entry rpt_cli[] = {
	TRIS_CLI_DEFINE(handle_cli_debug,"Enable app_rpt debugging"),
	TRIS_CLI_DEFINE(handle_cli_dump,"Dump app_rpt structs for debugging"),
	TRIS_CLI_DEFINE(handle_cli_stats,"Dump node statistics"),
	TRIS_CLI_DEFINE(handle_cli_nodes,"Dump node list"),
	TRIS_CLI_DEFINE(handle_cli_local_nodes,	"Dump list of local node numbers"),
	TRIS_CLI_DEFINE(handle_cli_lstats,"Dump link statistics"),
	TRIS_CLI_DEFINE(handle_cli_reload,"Reload app_rpt config"),
	TRIS_CLI_DEFINE(handle_cli_restart,"Restart app_rpt"),
	TRIS_CLI_DEFINE(handle_cli_fun,"Execute a DTMF function"),
	TRIS_CLI_DEFINE(handle_cli_fun1,"Execute a DTMF function"),
	TRIS_CLI_DEFINE(handle_cli_cmd,"Execute a DTMF function")
};

#endif

static int send_morse(struct tris_channel *chan, char *string, int speed, int freq, int amplitude)
{
...
static int send_tone_telemetry(struct tris_channel *chan, char *tonestring)
{
	char *p,*stringp;
	char *tonesubset;
	int f1,f2;
	...
	p = stringp = tris_strdup(tonestring);
...
static int sayfile(struct tris_channel *mychannel,char *fname)
{
int	res;

	res = tris_streamfile(mychannel, fname, mychannel->language);
	if (!res) 
		res = tris_waitstream(mychannel, "");
	else
		 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
	tris_stopstream(mychannel);
	return res;
}

static int saycharstr(struct tris_channel *mychannel,char *str)
{
int	res;

	res = tris_say_character_str(mychannel,str,NULL,mychannel->language);
	if (!res) 
		res = tris_waitstream(mychannel, "");
	else
		 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
...
static int saynum(struct tris_channel *mychannel, int num)
{
	int res;
	res = tris_say_number(mychannel, num, NULL, mychannel->language, NULL);
	if(!res)
		res = tris_waitstream(mychannel, "");
	else
		tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
...
static int saynode(struct rpt *myrpt, struct tris_channel *mychannel, char *name)
{
int	res;
char	*val,fname[300];

	val = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->name, "nodenames");
	if (!val) val = NODENAMES;
	snprintf(fname,sizeof(fname) - 1,"%s/%s",val,name);
	if (tris_fileexists(fname,NULL,mychannel->language) > 0)
		return(sayfile(mychannel,fname));
	res = sayfile(mychannel,"rpt/node");
	if (!res) 
		res = tris_say_character_str(mychannel,name,NULL,mychannel->language);
	return res;
}

static int telem_any(struct rpt *myrpt,struct tris_channel *chan, char *entry)
{
	int res;
	char c;
...
static int telem_lookup(struct rpt *myrpt,struct tris_channel *chan, char *node, char *name)
{
	...
	telemetry = (char *) tris_variable_retrieve(myrpt->cfg, node, TELEMETRY);
	if(telemetry ){
		telemetry_save = tris_strdup(telemetry);
		if(!telemetry_save){
			tris_log(LOG_WARNING,"tris_strdup() failed in telem_lookup()\n");
			return res;
		}
		entry = (char *) tris_variable_retrieve(myrpt->cfg, telemetry_save, name);
	}
...
static int get_wait_interval(struct rpt *myrpt, int type)
{
        ...
        wait_times = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->name, "wait_times");
                                                                                                                  
        if(wait_times){
                wait_times_save = tris_strdup(wait_times);
                if(!wait_times_save)
			return 0;
...
*/
static void wait_interval(struct rpt *myrpt, int type, struct tris_channel *chan)
{
	int interval;
	interval = get_wait_interval(myrpt, type);
	if(debug)
		tris_log(LOG_NOTICE,"Delay interval = %d\n", interval);
	if(interval)
		tris_safe_sleep(chan,interval);
	if(debug)
		tris_log(LOG_NOTICE,"Delay complete\n");
	return;
}
...
struct	tris_channel *mychannel;
int vmajor, vminor, m;
char *p,*ct,*ct_copy,*ident, *nodename,*cp;
time_t t;
#ifdef	NEW_TRISMEDIA
struct tris_tm localtm;
#else
struct tm localtm;
#endif
char lbuf[MAXLINKLIST],*strs[MAXLINKLIST];
...
struct dahdi_params par;
	...
	rpt_mutex_lock(&myrpt->lock);
	nodename = tris_strdup(myrpt->name);
	if(!nodename)
	{
	    fprintf(stderr,"rpt:Sorry unable strdup nodename\n");
	    rpt_mutex_lock(&myrpt->lock);
	    remque((struct qelem *)mytele);
	    tris_log(LOG_NOTICE,"Telemetry thread aborted at line %d, mode: %d\n",__LINE__, mytele->mode); /*@@@@@@@@@@@*/
	    rpt_mutex_unlock(&myrpt->lock);
	    tris_free(mytele);
	    pthread_exit(NULL);
	}
	...
	if (myrpt->p.ident){
		ident = tris_strdup(myrpt->p.ident);
        	if(!ident)
		{
	...
	/* allocate a pseudo-channel thru trismedia */
	mychannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!mychannel)
	{
...
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (mychannel->cdr) 
		tris_set_flag(mychannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	rpt_mutex_lock(&myrpt->lock);
	...
	{
		tris_log(LOG_WARNING, "Unable to set conference mode to Announce\n");
		rpt_mutex_lock(&myrpt->lock);
	...
	switch(mytele->mode)
	{
	    ...
	    case TAILMSG:
		res = tris_streamfile(mychannel, myrpt->p.tailmessages[myrpt->tailmessagen], mychannel->language); 
		break;
		
	    case IDTALKOVER:
	    	p = (char *) tris_variable_retrieve(myrpt->cfg, nodename, "idtalkover");
	    	if(p)
		...
		if(res < 0){ /* Then default message */
			res = tris_streamfile(mychannel, "rpt/callproceeding", mychannel->language);
		}
		break;
	    case TERM:
		/* wait a little bit longer */
		...
		if(res < 0){ /* Then default message */
			res = tris_streamfile(mychannel, "rpt/callterminated", mychannel->language);
		}
		break;
	    case COMPLETE:
		/* wait a little bit */
		...
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/macro_notfound", mychannel->language);
		break;
	    case MACRO_BUSY:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/macro_busy", mychannel->language);
		break;
	    case UNKEY:
		...
		*/
		...
		while(myrpt->unkeytocttimer)
		{
			...
			tris_safe_sleep(mychannel, ctint);
			rpt_mutex_lock(&myrpt->lock);
		...
		*/
		...
		if (haslink)
		{
			...
			if(res)
				tris_log(LOG_WARNING, "telem_lookup:remotexx failed on %s\n", mychannel->name);
			
		
			/* if in remote cmd mode, indicate it */
			if (myrpt->cmdnode[0])
			{
				tris_safe_sleep(mychannel,200);
				res = telem_lookup(myrpt,mychannel, myrpt->name, "cmdmode");
				if(res)
				 	tris_log(LOG_WARNING, "telem_lookup:cmdmode failed on %s\n", mychannel->name);
				tris_stopstream(mychannel);
			}
		}
		else if((ct = (char *) tris_variable_retrieve(myrpt->cfg, nodename, "unlinkedct"))){ /* Unlinked Courtesy Tone */
			ct_copy = tris_strdup(ct);
			if(ct_copy)
			{
				...
				res = -1;
			if(res)
			 	tris_log(LOG_WARNING, "telem_lookup:ctx failed on %s\n", mychannel->name);		
		}	
		if (hasremote && (!myrpt->cmdnode[0]))
		{
			...
			if((ct = (char *) tris_variable_retrieve(myrpt->cfg, nodename, "remotect"))){ /* Unlinked Courtesy Tone */
				tris_safe_sleep(mychannel,200);
				ct_copy = tris_strdup(ct);
				if(ct_copy)
				{
		...
		if (myrpt->lastunit)
		{
			...
			myrpt->lastunit = 0;
			tris_say_character_str(mychannel,mystr,NULL,mychannel->language);
			break;
		}
#endif
		imdone = 1;
		...
		*/
		...
		if((ct = (char *) tris_variable_retrieve(myrpt->cfg, nodename, "linkunkeyct"))){ /* Unlinked Courtesy Tone */
			ct_copy = tris_strdup(ct);
			if(ct_copy){
				res = telem_lookup(myrpt,mychannel, myrpt->name, ct_copy);
				tris_free(ct_copy);
		...
		if (!res) 
		    res = tris_streamfile(mychannel, ((mytele->mylink.hasconnected) ? 
			"rpt/remote_disc" : "rpt/remote_busy"), mychannel->language);
		break;
	    case REMALREADY:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/remote_already", mychannel->language);
		break;
	    case REMNOTFOUND:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/remote_notfound", mychannel->language);
		break;
	    case REMGO:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/remote_go", mychannel->language);
		break;
	    case CONNECTED:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM,  mychannel);
		res = saynode(myrpt,mychannel,mytele->mylink.name);
		if (!res)
		    res = tris_streamfile(mychannel, "rpt/connected", mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		else
			 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		tris_stopstream(mychannel);
		res = tris_streamfile(mychannel, "digits/2", mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		else
			 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		res = saynode(myrpt,mychannel,mytele->mylink.name);
		if (!res) 
		    res = tris_streamfile(mychannel, "rpt/connection_failed", mychannel->language);
		break;
	    case MEMNOTFOUND:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/memory_notfound", mychannel->language);
		break;
	    case PLAYBACK:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, mytele->param, mychannel->language);
		break;
	    case TOPKEY:
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		...
		else if(!strcmp(myrpt->remoterig, remote_rig_rbi)||!strcmp(myrpt->remoterig, remote_rig_ppp16))
		{
			...
			{
				rpt_mutex_unlock(&myrpt->lock);
				tris_log(LOG_WARNING, "Cant get io permission on IO port %x hex\n",myrpt->p.iobase);
				res = -1;
			}
		...
		else if(!strcmp(myrpt->remoterig, remote_rig_kenwood))
		{
			...
			if (tris_safe_sleep(mychannel,200) == -1)
			{
				tris_mutex_unlock(&myrpt->remlock);
				res = -1;
				break;
			}
			if (myrpt->iofd < 0)
			{
				...
				{
					tris_mutex_unlock(&myrpt->remlock);
					tris_log(LOG_ERROR,"Cant flush events");
					res = -1;
					break;
				}
				...
				{
					tris_mutex_unlock(&myrpt->remlock);
					tris_log(LOG_ERROR,"Cant get params");
					res = -1;
					break;
				}
				myrpt->remoterx = 
		...
		/* wait a little bit */
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/invalid-freq", mychannel->language);
		break;
	    case REMMODE:
		cp = 0;
		wait_interval(myrpt, DLY_TELEM, mychannel);
		...
		if (myrpt->hfscanstop)
		{
			...
			tris_mutex_unlock(&myrpt->remlock);
			if (tris_safe_sleep(mychannel,1000) == -1) break;
		...
		while(l != &myrpt->links)
		{
			...
			l1 = tris_malloc(sizeof(struct rpt_link));
			if (!l1)
			{
				tris_log(LOG_WARNING, "Cannot alloc memory on %s\n", mychannel->name);
				remque((struct qelem *)mytele);
				myrpt->active_telem = NULL;
				rpt_mutex_unlock(&myrpt->lock);
				tris_log(LOG_NOTICE,"Telemetry thread aborted at line %d, mode: %d\n",__LINE__, mytele->mode); /*@@@@@@@@@@@*/
		...
		{
			hastx = 1;
			res = tris_streamfile(mychannel, "rpt/autopatch_on", mychannel->language);
			if (!res) 
				res = tris_waitstream(mychannel, "");
			else
				 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		while(l != &linkbase)
		{
			...
			res = tris_streamfile(mychannel, s, mychannel->language);
			if (!res) 
				res = tris_waitstream(mychannel, "");
			else
				tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		if (!hastx)
		{
			res = tris_streamfile(mychannel, "rpt/repeat_only", mychannel->language);
			if (!res) 
				res = tris_waitstream(mychannel, "");
			else
				 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		if(myrpt->lastnodewhichkeyedusup){
			p = tris_strdup(myrpt->lastnodewhichkeyedusup); /* Make a local copy of the node name */
			if(!p){
				tris_log(LOG_WARNING, "tris_strdup failed in telemetery LASTNODEKEY");
				imdone = 1;
				break;
			}
		}
		...
		wait_interval(myrpt, DLY_TELEM, mychannel);
		res = tris_streamfile(mychannel, "rpt/unauthtx", mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		else
			 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		if (tris_fileexists(mystr,NULL,mychannel->language) <= 0)
		{
			imdone = 1;
			myrpt->parrotstate = 0;
			break;
		...
		sprintf(mystr,PARROTFILE,myrpt->name,(unsigned int)mytele->parrot);
		res = tris_streamfile(mychannel, mystr, mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		else
			 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		res = saynode(myrpt,mychannel,myrpt->name);
		if (!res)
		   res = tris_streamfile(mychannel, "rpt/timeout", mychannel->language);
		break;
		...
		res = saynode(myrpt,mychannel,myrpt->name);
		if (!res)
		   res = tris_streamfile(mychannel, "rpt/timeout-warning", mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		else
			 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		...
		if(!res) /* Say number of seconds */
			tris_say_number(mychannel, myrpt->p.remotetimeout - 
			    (t - myrpt->last_activity_time), 
				"", mychannel->language, (char *) NULL);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		tris_stopstream(mychannel);	
		res = tris_streamfile(mychannel, "queue-seconds", mychannel->language);
		break;
		...
		res = saynode(myrpt,mychannel,myrpt->name);
		if (!res)
		    res = tris_streamfile(mychannel, "rpt/act-timeout-warning", mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		else
			 tris_log(LOG_WARNING, "tris_streamfile failed on %s\n", mychannel->name);
		tris_stopstream(mychannel);
		if(!res) /* Say number of seconds */
			tris_say_number(mychannel, myrpt->p.remoteinacttimeout - 
			    (t - myrpt->last_activity_time), 
				"", mychannel->language, (char *) NULL);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		...
		/* Say the time */				
	    	res = tris_say_time(mychannel, t, "", mychannel->language);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		tris_stopstream(mychannel);		
		imdone = 1;
		...
		if(!res) /* Say "X" */
			tris_say_number(mychannel, vmajor, "", mychannel->language, (char *) NULL);
		if (!res) 
			res = tris_waitstream(mychannel, "");
		tris_stopstream(mychannel);	
		...
		if(!res) /* Say "Y" */
			tris_say_number(mychannel, vminor, "", mychannel->language, (char *) NULL);
		if (!res){
			res = tris_waitstream(mychannel, "");
			tris_stopstream(mychannel);
		}	
	    ...
	    case REV_PATCH:
			...
			tpl_working = tris_strdup(mytele->param);
			...
			for(idx=0; idx<looptemp; idx++) {
				if(!strcmp(tmp[idx], "PARKED")) {
					tris_say_digits(mychannel, atoi(myparm), "", mychannel->language);
				} else if(!strcmp(tmp[idx], "NODE")) {
					tris_say_digits(mychannel, atoi(myrpt->name), "", mychannel->language);
				} else {
					dres = tris_streamfile(mychannel, tmp[idx], mychannel->language);
					if(!dres) {
						dres = tris_waitstream(mychannel, "");
					} else {
		...
		myrpt->stopgen = -1;
	        if ((res = tris_tonepair_start(mychannel, 1004.0, 0, 99999999, 7200.0))) 
		{
			myrpt->stopgen = 0;
			break;
		}
	        while(mychannel->generatordata && (myrpt->stopgen <= 0)) {
			if (tris_safe_sleep(mychannel,1)) break;
		    	imdone = 1;
			}
		myrpt->stopgen = 0;
		break;
...
char *v1, *v2;
	...
	if(debug > 6)
		tris_log(LOG_NOTICE,"mode=%i  data=%s\n",mode, (char *)data);
	...
	switch(mode)
	{
		...
		v1 = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->name, 
			"unlinkedct");
		v2 = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->name, 
			"remotect");
		...
		break;
	    case LINKUNKEY:
		if (!tris_variable_retrieve(myrpt->cfg, myrpt->name, "linkunkeyct"))
			return;
		break;
	    default:
		break;
	}
	tele = tris_malloc(sizeof(struct rpt_tele));
	if (!tele)
	{
		tris_log(LOG_WARNING, "Unable to allocate memory\n");
		pthread_exit(NULL);
		return;
	}
	/* zero it out */
	...
	res = tris_pthread_create(&tele->threadid,&attr,rpt_tele_thread,(void *) tele);
	if(res < 0){
		rpt_mutex_lock(&myrpt->lock);
		remque((struct qlem *) tele); /* We don't like stuck transmitters, remove it from the queue */
		rpt_mutex_unlock(&myrpt->lock);	
		tris_log(LOG_WARNING, "Could not create telemetry thread: %s",strerror(res));
	}
	return;
}
...
struct tris_channel *mychannel,*genchannel;
	...
	/* allocate a pseudo-channel thru trismedia */
	genchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!genchannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		tris_hangup(mychannel);
...
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (genchannel->cdr)
		tris_set_flag(genchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	ci.chan = 0;
	ci.confno = myrpt->conf;
	ci.confmode = DAHDI_CONF_REALANDPSEUDO | DAHDI_CONF_TALKER | DAHDI_CONF_LISTENER
	...
	{
		tris_log(LOG_WARNING, "Cannot start dialtone\n");
		tris_hangup(mychannel);
		tris_hangup(genchannel);
		myrpt->callmode = 0;
		pthread_exit(NULL);
	...
	while ((myrpt->callmode == 1) || (myrpt->callmode == 4))
	{
		...
		if((myrpt->patchdialtime)&&(dialtimer >= myrpt->patchdialtime)){ 
		    if(debug)
		    	tris_log(LOG_NOTICE, "dialtimer %i > patchdialtime %i\n", dialtimer,myrpt->patchdialtime);
			rpt_mutex_lock(&myrpt->lock);
			aborted = 1;
			myrpt->callmode = 0;
			rpt_mutex_unlock(&myrpt->lock);
		...
		res = tris_safe_sleep(mychannel, MSWAIT);
		if (res < 0)
		{
		    if(debug)
		    	tris_log(LOG_NOTICE, "tris_safe_sleep=%i\n", res);
			tris_hangup(mychannel);
			tris_hangup(genchannel);
			rpt_mutex_lock(&myrpt->lock);
			myrpt->callmode = 0;
	...
	{
		if(debug)
			tris_log(LOG_NOTICE, "callmode==0\n");
		tris_hangup(mychannel);
		tris_hangup(genchannel);
		rpt_mutex_lock(&myrpt->lock);
		myrpt->callmode = 0;
	...
	if (myrpt->p.ourcallerid && *myrpt->p.ourcallerid){
		char *name, *loc, *instr;
		instr = tris_strdup(myrpt->p.ourcallerid);
		if(instr){
			tris_callerid_parse(instr, &name, &loc);
			if(loc){
				if(mychannel->cid.cid_num)
					tris_free(mychannel->cid.cid_num);
				mychannel->cid.cid_num = tris_strdup(loc);
			}
			if(name){
				if(mychannel->cid.cid_name)
					tris_free(mychannel->cid.cid_name);
				mychannel->cid.cid_name = tris_strdup(name);
			}
			tris_free(instr);
		}
	}

	tris_copy_string(mychannel->exten, myrpt->exten, sizeof(mychannel->exten) - 1);
	tris_copy_string(mychannel->context, myrpt->patchcontext, sizeof(mychannel->context) - 1);
	
	if (myrpt->p.acctcode)
		tris_cdr_setaccount(mychannel,myrpt->p.acctcode);
	mychannel->priority = 1;
	tris_channel_undefer_dtmf(mychannel);
	if (tris_pbx_start(mychannel) < 0)
	{
		tris_log(LOG_WARNING, "Unable to start PBX!!\n");
		tris_hangup(mychannel);
		tris_hangup(genchannel);
		rpt_mutex_lock(&myrpt->lock);
	 	myrpt->callmode = 0;
	...
	{
		tris_log(LOG_WARNING, "Unable to get autopatch channel number\n");
		tris_hangup(mychannel);
		myrpt->callmode = 0;
		pthread_exit(NULL);
	}
	...
	while(myrpt->callmode)
	{
		if ((!mychannel->pbx) && (myrpt->callmode != 4))
		{
			...
			if(myrpt->patchfarenddisconnect || (myrpt->p.duplex < 2)){ 
				if(debug)tris_log(LOG_NOTICE,"callmode=%i, patchfarenddisconnect=%i, duplex=%i\n",\
		...
		if (myrpt->mydtmf)
		{
			struct tris_frame wf = {TRIS_FRAME_DTMF, } ;
			wf.subclass = myrpt->mydtmf;
			rpt_mutex_unlock(&myrpt->lock);
			tris_queue_frame(mychannel,&wf);
#ifdef	NEW_TRISMEDIA
			tris_senddigit(genchannel,myrpt->mydtmf,0);
#else
			tris_senddigit(genchannel,myrpt->mydtmf);
#endif
			rpt_mutex_lock(&myrpt->lock);
			myrpt->mydtmf = 0;
		}
	...
	if(debug)
		tris_log(LOG_NOTICE, "exit channel loop\n");
	rpt_mutex_unlock(&myrpt->lock);
	tone_zone_play_tone(genchannel->fds[0],-1);
	if (mychannel->pbx) tris_softhangup(mychannel,TRIS_SOFTHANGUP_DEV);
	tris_hangup(genchannel);
	rpt_mutex_lock(&myrpt->lock);
	myrpt->callmode = 0;
	myrpt->macropatch=0;
...
static void send_newkey(struct tris_channel *chan)
{

	/* tris_safe_sleep(chan,10); */
	tris_sendtext(chan,newkeystr);
	return;
}
...
static int connect_link(struct rpt *myrpt, char* node, int mode, int perma)
{
	...
	if(debug > 3){
		tris_log(LOG_NOTICE,"Connect attempt to node %s\n", node);
		tris_log(LOG_NOTICE,"Mode: %s\n",(mode)?"Transceive":"Monitor");
		tris_log(LOG_NOTICE,"Connection type: %s\n",(perma)?"Permalink":"Normal");
	}
	...
	/* if already in this mode, just ignore */
		...
		rpt_mutex_unlock(&myrpt->lock);
		if (l->chan) tris_softhangup(l->chan, TRIS_SOFTHANGUP_DEV);
	...
	/* establish call */
	l = tris_malloc(sizeof(struct rpt_link));
	if (!l)
	{
		tris_log(LOG_WARNING, "Unable to malloc\n");
		return -1;
	}
	...
	tele = strchr(deststr, '/');
	if (!tele){
		tris_log(LOG_WARNING,"link3:Dial number (%s) must be in format tech/number\n",deststr);
		tris_free(l);
		return -1;
	}
	*tele++ = 0;
	l->chan = tris_request(deststr, TRIS_FORMAT_SLINEAR, tele,NULL, 0);
	if (l->chan){
		tris_set_read_format(l->chan, TRIS_FORMAT_SLINEAR);
		tris_set_write_format(l->chan, TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
		if (l->chan->cdr)
			tris_set_flag(l->chan->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
#ifndef	NEW_TRISMEDIA
		...
		if (debug > 3)
			tris_log(LOG_NOTICE, "rpt (remote) initiating call to %s/%s on %s\n",
		deststr, tele, l->chan->name);
		if(l->chan->cid.cid_num)
			tris_free(l->chan->cid.cid_num);
		l->chan->cid.cid_num = tris_strdup(myrpt->name);
		tris_call(l->chan,tele,999);
	}
	else {
		if(debug > 3) 
			tris_log(LOG_NOTICE, "Unable to place call to %s/%s on %s\n",
		deststr,tele,l->chan->name);
	...
	/* allocate a pseudo-channel thru trismedia */
	l->pchan = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!l->pchan){
		tris_log(LOG_WARNING,"rpt connect: Sorry unable to obtain pseudo channel\n");
		tris_hangup(l->chan);
		tris_free(l);
		return -1;
	}
	tris_set_read_format(l->pchan, TRIS_FORMAT_SLINEAR);
	tris_set_write_format(l->pchan, TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (l->pchan->cdr)
		tris_set_flag(l->pchan->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	/* make a conference for the tx */
	ci.chan = 0;
	ci.confno = myrpt->conf;
	...
	switch(myatoi(param)){
		...
		case 1: /* Link off */
			...
			if (l != &myrpt->links){ /* if found */
				...
				if (l->chan)
				{
					tris_write(l->chan,&wf);
					if (tris_safe_sleep(l->chan,250) == -1) return DC_ERROR;
					tris_softhangup(l->chan,TRIS_SOFTHANGUP_DEV);
				}
		...
		case 6: /* All Links Off, including permalinks */
				...
				if(strlen(tmp) + strlen(myrpt->savednodes) + 1 < MAXNODESTR){ 
					...
					strcat(myrpt->savednodes, tmp);
                           	...
                           	l->retries = l->max_retries + 1;
                                ...
                                rpt_mutex_unlock(&myrpt->lock);
				/* tris_log(LOG_NOTICE,"dumping link %s\n",l->name); */
                                ...
                                {
                                        tris_write(l->chan,&wf);
                                        tris_safe_sleep(l->chan,250); /* It's dead already, why check the return value? */
                                        tris_softhangup(l->chan,TRIS_SOFTHANGUP_DEV);
                                }
				rpt_mutex_lock(&myrpt->lock);
                                l = l->next;
			...
			rpt_mutex_unlock(&myrpt->lock);
			if(debug > 3)
				tris_log(LOG_NOTICE,"Nodes disconnected: %s\n",myrpt->savednodes);
                        rpt_telemetry(myrpt, COMPLETE, NULL);
			return DC_COMPLETE;
...
static int function_autopatchup(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	if(!myrpt->callmode){
		...
		if(param){
			...
			lparam = tris_strdup(param);
			if(!lparam){
				tris_log(LOG_ERROR,"App_rpt out of memory on line %d\n",__LINE__);
				return DC_ERROR;	
			}
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
	tris_pthread_create(&myrpt->rpt_call_thread,&attr,rpt_call,(void *) myrpt);
	return DC_COMPLETE;
}
...
int	i;
	...
	else val = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->p.macro, digitbuf);
...
static int function_playback(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	if (tris_fileexists(param,NULL,myrpt->rxchannel->language) <= 0)
		return DC_ERROR;
...
static int function_cop(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	switch(myatoi(param)){
		case 1: /* System reset */
			res = system("killall -9 trismedia");
			return DC_COMPLETE;
		...
		case 32: /* Touch Tone Pad Test */
			...
			if(!i){
				if(debug > 3)
				tris_log(LOG_NOTICE,"Padtest entered");
				myrpt->inpadtest = 1;
			}
			else{
				if(debug > 3)
					tris_log(LOG_NOTICE,"Padtest len= %d digits=%s",i,digitbuf);
				if(digitbuf[i-1] != myrpt->p.endchar)
					break;
				rpt_telemetry(myrpt, ARB_ALPHA, digitbuf);
				myrpt->inpadtest = 0;
				if(debug > 3)
					tris_log(LOG_NOTICE,"Padtest exited");
				return DC_COMPLETE;
			}
	}	
	return DC_INDETERMINATE;
	...
	int command_source, struct rpt_link *mylink)
{
	...
	if (debug > 6) tris_log(LOG_NOTICE,"digits=%s  source=%d\n",digits, command_source);
	...
	vp = tris_variable_browse(myrpt->cfg, function_table_name);
	while(vp) {
	...
	if (debug > 6) tris_log(LOG_NOTICE,"rv=%i\n",rv);
	return(rv);
}
...
struct	tris_frame wf;
	...
	strncpy(tmp,str,sizeof(tmp) - 1);
		...
		mylink->retries = mylink->max_retries + 1;
                tris_softhangup(mylink->chan,TRIS_SOFTHANGUP_DEV);
                return;
        }
	...
	if (tmp[0] == 'L')
	{
		...
		rpt_mutex_unlock(&myrpt->lock);
		if (debug > 6) tris_log(LOG_NOTICE,"@@@@ node %s received node list %s from node %s\n",
	...
	if (tmp[0] == 'K')
	{
		...
		{
			tris_log(LOG_WARNING, "Unable to parse keying string %s\n",str);
			return;
		}
		if (dest[0] == '0')
		{
		...
		if (cmd[1] == '?')
		{
			...
			if (mylink->chan) tris_write(mylink->chan,&wf); 
			return;
		}
	...
	if (tmp[0] == 'I')
	{
		...
		{
			tris_log(LOG_WARNING, "Unable to parse ident string %s\n",str);
			return;
		}
	...
	else
	{
		...
		{
			tris_log(LOG_WARNING, "Unable to parse link string %s\n",str);
			return;
		}
		if (strcmp(cmd,"D"))
		{
	...
	if (myrpt->callmode == 1)
	{
		...
		myrpt->exten[myrpt->cidx] = 0;
		/* if this exists */
		if (tris_exists_extension(myrpt->pchannel,myrpt->patchcontext,myrpt->exten,1,NULL))
		{
			/* if this really it, end now */
			if (!tris_matchmore_extension(myrpt->pchannel,myrpt->patchcontext,
				myrpt->exten,1,NULL)) 
			{
				myrpt->callmode = 2;
				if(!myrpt->patchquiet)
		...
		if (!tris_canmatch_extension(myrpt->pchannel,myrpt->patchcontext,myrpt->exten,1,NULL))
		{
...
struct dahdi_radio_param r;
	...
	if (ioctl(myrpt->dahdirxchannel->fds[0],DAHDI_RADIO_SETPARAM,&r) == -1)
	{
		tris_log(LOG_WARNING,"Cannot send RBI command for channel %s\n",myrpt->dahdirxchannel->name);
		return;
	}
}
	...
	unsigned char *rxbuf, int rxmaxbytes, int asciiflag)
{
	...
	char c;
	 ...
	 if(debug) {
	    tris_log(LOG_NOTICE, "ioport=%s  iofd=0x%x\n",myrpt->p.ioport,myrpt->iofd);
...
tris_log(LOG_NOTICE,"Sent to kenwood: %s\n",txstr);
...
tris_log(LOG_NOTICE,"Got from kenwood: %s\n",rxstr);
	return(i);
}
...
static int check_freq_ic706(int m, int d, int *defmode, char mars)
{
	...
	if(debug > 1)
		tris_log(LOG_NOTICE,"(%i,%i,%i,%i) returning %i\n",m,d,*defmode,mars,rv);
...
static int set_ctcss_mode_ic706(struct rpt *myrpt, char txplon, char rxplon)
{
	...
	if(debug > 6)
		tris_log(LOG_NOTICE,"txplon=%i  rxplon=%i \n",txplon,rxplon);
...
static int set_ctcss_freq_ic706(struct rpt *myrpt, char *txtone, char *rxtone)
{
	...
	if(debug > 6)
		tris_log(LOG_NOTICE,"txtone=%s  rxtone=%s \n",txtone,rxtone);
...
static int set_ic706(struct rpt *myrpt)
{
	...
	if(debug)tris_log(LOG_NOTICE, "Set to VFO A iobase=%i\n",myrpt->p.iobase);
...
printf("FREQ,%s,%s,%s,%s,%s,%s,%d,%d\n",myrpt->freq,
	...
	if (res < 0) tris_log(LOG_ERROR,"Unable to send remote command on node %s\n",myrpt->name);
...
static char check_tx_freq(struct rpt *myrpt)
{
	...
	struct tris_variable *limitlist;
	
	if(debug > 3){
		tris_log(LOG_NOTICE, "myrpt->freq = %s\n", myrpt->freq);
	}
	...
	if(!myrpt->p.txlimitsstanzaname || !myrpt->loginuser[0] || !myrpt->loginlevel[0]){
		if(debug > 3){
			tris_log(LOG_NOTICE, "No tx band table defined, or no user logged in. rv=1\n");
		}
		rv=1;
	...
	limitlist = tris_variable_browse(myrpt->cfg, myrpt->p.txlimitsstanzaname);

	if(!limitlist){
		tris_log(LOG_WARNING, "No entries in %s band table stanza. rv=0\n", myrpt->p.txlimitsstanzaname);
		rv=0;
		return 0;
	}
	...
	if(debug > 3){
		tris_log(LOG_NOTICE, "Login User = %s, login level = %s\n", myrpt->loginuser, myrpt->loginlevel);
	}
	...
	if(!limitlist){
		tris_log(LOG_WARNING, "Can't find %s entry in band table stanza %s. rv=0\n", myrpt->loginlevel, myrpt->p.txlimitsstanzaname);
		rv=0;
	    return 0;
	}
	
	if(debug > 3){
		tris_log(LOG_NOTICE, "Auth: %s = %s\n", limitlist->name, limitlist->value);
	}
	...
	for(i = 0; i < 40 && limit_ranges[i] ; i++){
		...
		range[39] = 0;
        if(debug > 3) 
        	tris_log(LOG_NOTICE, "Check %s within %s\n", myrpt->freq, range);
	
		r = strchr(range, '-');
		if(!r){
			tris_log(LOG_WARNING, "Malformed range in %s tx band table entry. rv=0\n", limitlist->name);
			rv=0;
			break;
		}
		*r++ = 0;
		...
		if((radio_mhz >= llimit_mhz) && (radio_mhz <= ulimit_mhz)){
			if(radio_mhz == llimit_mhz){ /* CASE 1: TX freq is in llimit mhz portion of band */
				if(radio_decimals >= llimit_decimals){ /* Cannot be below llimit decimals */
					if(llimit_mhz == ulimit_mhz){ /* If bandwidth < 1Mhz, check ulimit decimals */
						if(radio_decimals <= ulimit_decimals){
							...
							if(debug > 3)
								tris_log(LOG_NOTICE, "Invalid TX frequency, debug msg 1\n");
							rv=0;
							break;
						}
					}
					...
					if(debug > 3)
						tris_log(LOG_NOTICE, "Invalid TX frequency, debug msg 2\n");
					rv=0;
					break;
				}
			}
				...
				if(radio_decimals <= ulimit_decimals){
					if(debug > 3)
						tris_log(LOG_NOTICE, "radio_decimals <= ulimit_decimals\n");
					rv=1;
					break;
				}
				else{ /* Is above ulimit decimals */
					if(debug > 3)
						tris_log(LOG_NOTICE, "Invalid TX frequency, debug msg 3\n");
					rv=0;
					break;
				}
			}
				...
				if(debug > 3)
						tris_log(LOG_NOTICE, "Valid TX freq within a multi-Mhz band and ok.\n");
				rv=1;
				break;
		}
	}
	if(debug > 3)  
		tris_log(LOG_NOTICE, "rv=%i\n",rv);
...
{
	int res=0;
	if(debug)tris_log(LOG_NOTICE," digitbuf=%s\n", digitbuf);
	res = retreive_memory(myrpt, digitbuf);
	if(!res)res=setrem(myrpt);	
	if(debug)tris_log(LOG_NOTICE," freq=%s  res=%i\n", myrpt->freq, res);
	return res;
}
/*
...
static int channel_steer(struct rpt *myrpt, char *data)
{
	...
	if(debug)tris_log(LOG_NOTICE,"remoterig=%s, data=%s\n",myrpt->remoterig,data);
	if (!myrpt->remoterig) return(0);
	if(data<=0)
	{
		res=-1;
	...
	if(debug)tris_log(LOG_NOTICE,"nowchan=%i  res=%i\n",myrpt->nowchan, res);
	return res;
}
/*
*/
...
{
	int res=0;
	if(debug)tris_log(LOG_NOTICE,"remoterig=%s, nowchan=%02d, waschan=%02d\n",myrpt->remoterig,myrpt->nowchan,myrpt->waschan);
	if (!myrpt->remoterig) return(0);
	if(myrpt->nowchan!=myrpt->waschan)
	{
		char data[8];
        if(debug)tris_log(LOG_NOTICE,"reverting.\n");
...
static int function_remote(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
    ...
    if(debug > 6) {
    	tris_log(LOG_NOTICE,"%s param=%s digitbuf=%s source=%i\n",myrpt->name,param,digitbuf,command_source);
	}
	...
	switch(p){
		...
		case 2:  /* set freq and offset */
			...
			if(debug)
				tris_log(LOG_NOTICE, "New frequency: %s\n", freq);
...
invalid_freq:
		...
		case 4: /* set tx PL tone */
			...
			*/
			...
			{
				if(debug)
					tris_log(LOG_WARNING,"Setting IC706 Tx CTCSS Code Not Supported. Set Rx Code for both.\n");
				return DC_ERROR;
			}
	    	for(i = 0, j = 0, k = 0, l = 0 ; digitbuf[i] ; i++){ /* look for N+*N */
				if(digitbuf[i] == '*'){
		...
		case 99:
			...
			*myrpt->loginuser = 0;
			...
			cp = tris_strdup(param);
			cp1 = strchr(cp,',');
			tris_mutex_lock(&myrpt->lock);
			if (cp1) 
			{
				*cp1 = 0;
				cp2 = strchr(cp1 + 1,',');
		...
		case 119: /* Tune Request */
			if(debug > 3)
				tris_log(LOG_NOTICE,"TUNE REQUEST\n");
			...
			if((!myrpt->tunerequest) && 
				...
				myrpt->remotetx = 0;
				tris_indicate(myrpt->txchannel,TRIS_CONTROL_RADIO_UNKEY);
				myrpt->tunerequest = 1;
...
int	ret,res = 0,src;
	...
	if(debug > 6)
		tris_log(LOG_NOTICE,"c=%c  phonemode=%i  dtmfidx=%i\n",c,phonemode,myrpt->dtmfidx);
...
static int attempt_reconnect(struct rpt *myrpt, struct rpt_link *l)
{
	...
	*tele++ = 0;
	...
	if (l->chan){
		...
		l->chan->data = "(Remote Rx)";
		if (option_verbose > 2)
			tris_verbose(VERBOSE_PREFIX_3 "rpt (attempt_reconnect) initiating call to %s/%s on %s\n",
				deststr, tele, l->chan->name);
		if(l->chan->cid.cid_num)
			tris_free(l->chan->cid.cid_num);
		l->chan->cid.cid_num = tris_strdup(myrpt->name);
	...
	{
		if (option_verbose > 2)
			tris_verbose(VERBOSE_PREFIX_3 "Unable to place call to %s/%s on %s\n",
				deststr,tele,l->chan->name);
		return -1;
	}
	rpt_mutex_lock(&myrpt->lock);
	...
	insque((struct qelem *)l,(struct qelem *)myrpt->links.next);
	rpt_mutex_unlock(&myrpt->lock);
	tris_log(LOG_WARNING,"Reconnect Attempt to %s in process\n",l->name);
	if (!l->phonemode) send_newkey(l->chan);
	return 0;
}
...
char	cmd[MAXDTMF+1] = "",c;
	...
	/* if in simple mode, kill autopatch */
		...
		{   
			if(debug)
				tris_log(LOG_WARNING, "simple mode autopatch kill\n");
			rpt_mutex_lock(&myrpt->lock);
			myrpt->callmode = 0;
			myrpt->macropatch=0;
			channel_revert(myrpt);
...
static void do_scheduler(struct rpt *myrpt)
{
	...
	/* Don't schedule if disabled */
        ...
        if(myrpt->p.s[myrpt->p.sysstate_cur].schedulerdisable){
		if(debug > 6)
			tris_log(LOG_NOTICE, "Scheduler disabled\n");
		return;
	}

	if(!myrpt->p.skedstanzaname){ /* No stanza means we do nothing */
		if(debug > 6)
			tris_log(LOG_NOTICE,"No stanza for scheduler in rpt.conf\n");
		return;
	}

    /* get pointer to linked list of scheduler entries */
    skedlist = tris_variable_browse(myrpt->cfg, myrpt->p.skedstanzaname);

	if(debug > 6){
		tris_log(LOG_NOTICE, "Time now: %02d:%02d %02d %02d %02d\n",
			tmnow.tm_hour,tmnow.tm_min,tmnow.tm_mday,tmnow.tm_mon + 1, tmnow.tm_wday); 
	}
	/* walk the list */
	for(; skedlist; skedlist = skedlist->next){
		if(debug > 6)
			tris_log(LOG_NOTICE, "Scheduler entry %s = %s being considered\n",skedlist->name, skedlist->value);
		...
		if(debug > 6)
			tris_log(LOG_NOTICE, "i = %d, min = %s, hour = %s, mday=%s, mon=%s, wday=%s\n",i,
 		...
 		if(i == 5){
			...
			if((*strs[4] != '*')&&(atoi(strs[4]) != tmnow.tm_wday))
				continue;
			if(debug)
				tris_log(LOG_NOTICE, "Executing scheduler entry %s = %s\n", skedlist->name, skedlist->value);
			if(atoi(skedlist->name) == 0)
				return; /* Zero is reserved for the startup macro */
			val = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->p.macro, skedlist->name);
			if (!val){
				tris_log(LOG_WARNING,"Scheduler could not find macro %s\n",skedlist->name);
				return; /* Macro not found */
			}
			if ((MAXMACRO - strlen(myrpt->macrobuf)) < strlen(val)){
				tris_log(LOG_WARNING, "Scheduler could not execute macro %s: Macro buffer full\n",
					skedlist->name);
			...
			tris_log(LOG_WARNING,"Malformed scheduler entry in rpt.conf: %s = %s\n",
				skedlist->name, skedlist->value);
		}
	}
...
char tmpstr[300],lstr[MAXLINKLIST];
	...
	while(telem != &myrpt->tele)
	{
		tris_softhangup(telem->chan,TRIS_SOFTHANGUP_DEV);
		telem = telem->next;
	}
	rpt_mutex_unlock(&myrpt->lock);
	  ...
	  (ioperm(myrpt->p.iobase,1,1) == -1))
	{
		rpt_mutex_unlock(&myrpt->lock);
		tris_log(LOG_WARNING, "Cant get io permission on IO port %x hex\n",myrpt->p.iobase);
		myrpt->rpt_thread = TRIS_PTHREADT_STOP;
		pthread_exit(NULL);
	}
#endif
	...
	*tele++ = 0;
	myrpt->rxchannel = tris_request(tmpstr,TRIS_FORMAT_SLINEAR,tele,NULL, 0);
	myrpt->dahdirxchannel = NULL;
	if (!strcasecmp(tmpstr,"DAHDI"))
		myrpt->dahdirxchannel = myrpt->rxchannel;
	if (myrpt->rxchannel)
	{
		if (myrpt->rxchannel->_state == TRIS_STATE_BUSY)
		{
			fprintf(stderr,"rpt:Sorry unable to obtain Rx channel\n");
			rpt_mutex_unlock(&myrpt->lock);
			tris_hangup(myrpt->rxchannel);
			myrpt->rpt_thread = TRIS_PTHREADT_STOP;
			pthread_exit(NULL);
		}
		tris_set_read_format(myrpt->rxchannel,TRIS_FORMAT_SLINEAR);
		tris_set_write_format(myrpt->rxchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
		if (myrpt->rxchannel->cdr)
			tris_set_flag(myrpt->rxchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
#ifndef	NEW_TRISMEDIA
		...
		if (option_verbose > 2)
			tris_verbose(VERBOSE_PREFIX_3 "rpt (Rx) initiating call to %s/%s on %s\n",
				tmpstr,tele,myrpt->rxchannel->name);
		tris_call(myrpt->rxchannel,tele,999);
		if (myrpt->rxchannel->_state != TRIS_STATE_UP)
		{
			rpt_mutex_unlock(&myrpt->lock);
			tris_hangup(myrpt->rxchannel);
			myrpt->rpt_thread = TRIS_PTHREADT_STOP;
	...
	if (myrpt->txchanname)
	{
		...
		*tele++ = 0;
		myrpt->txchannel = tris_request(tmpstr,TRIS_FORMAT_SLINEAR,tele,NULL, 0);
		if (!strcasecmp(tmpstr,"DAHDI"))
			myrpt->dahditxchannel = myrpt->txchannel;
		if (myrpt->txchannel)
		{
			if (myrpt->txchannel->_state == TRIS_STATE_BUSY)
			{
				fprintf(stderr,"rpt:Sorry unable to obtain Tx channel\n");
				rpt_mutex_unlock(&myrpt->lock);
				tris_hangup(myrpt->txchannel);
				tris_hangup(myrpt->rxchannel);
				myrpt->rpt_thread = TRIS_PTHREADT_STOP;
				pthread_exit(NULL);
			}			
			tris_set_read_format(myrpt->txchannel,TRIS_FORMAT_SLINEAR);
			tris_set_write_format(myrpt->txchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
			if (myrpt->txchannel->cdr)
				tris_set_flag(myrpt->txchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
#ifndef	NEW_TRISMEDIA
			...
			if (option_verbose > 2)
				tris_verbose(VERBOSE_PREFIX_3 "rpt (Tx) initiating call to %s/%s on %s\n",
					tmpstr,tele,myrpt->txchannel->name);
			tris_call(myrpt->txchannel,tele,999);
			if (myrpt->rxchannel->_state != TRIS_STATE_UP)
			{
				rpt_mutex_unlock(&myrpt->lock);
				tris_hangup(myrpt->rxchannel);
	...
	tris_indicate(myrpt->txchannel,TRIS_CONTROL_RADIO_KEY);
	tris_indicate(myrpt->txchannel,TRIS_CONTROL_RADIO_UNKEY);
	/* allocate a pseudo-channel thru trismedia */
	myrpt->pchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!myrpt->pchannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		rpt_mutex_unlock(&myrpt->lock);
	...
	if (myrpt->pchannel->cdr)
		tris_set_flag(myrpt->pchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	if (!myrpt->dahdirxchannel) myrpt->dahdirxchannel = myrpt->pchannel;
	if (!myrpt->dahditxchannel)
	{
		/* allocate a pseudo-channel thru trismedia */
		myrpt->dahditxchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
		if (!myrpt->dahditxchannel)
		{
			fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
			rpt_mutex_unlock(&myrpt->lock);
		...
		tris_set_read_format(myrpt->dahditxchannel,TRIS_FORMAT_SLINEAR);
		tris_set_write_format(myrpt->dahditxchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
		if (myrpt->dahditxchannel->cdr)
			tris_set_flag(myrpt->dahditxchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	}
	/* allocate a pseudo-channel thru trismedia */
	myrpt->monchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!myrpt->monchannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		rpt_mutex_unlock(&myrpt->lock);
	...
	tris_set_read_format(myrpt->monchannel,TRIS_FORMAT_SLINEAR);
	tris_set_write_format(myrpt->monchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (myrpt->monchannel->cdr)
		tris_set_flag(myrpt->monchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	/* make a conference for the tx */
	ci.chan = 0;
	...
	if (ioctl(myrpt->dahditxchannel->fds[0],DAHDI_SETCONF,&ci) == -1)
	{
		tris_log(LOG_WARNING, "Unable to set conference mode to Announce\n");
		rpt_mutex_unlock(&myrpt->lock);
		tris_hangup(myrpt->pchannel);
		tris_hangup(myrpt->monchannel);
		if (myrpt->txchannel != myrpt->rxchannel) 
			tris_hangup(myrpt->txchannel);
		tris_hangup(myrpt->rxchannel);
		myrpt->rpt_thread = TRIS_PTHREADT_STOP;
		...
		(myrpt->dahditxchannel == myrpt->txchannel))
	{
		...
		{
			tris_log(LOG_WARNING, "Unable to set tx channel's chan number\n");
			rpt_mutex_unlock(&myrpt->lock);
			tris_hangup(myrpt->pchannel);
			tris_hangup(myrpt->monchannel);
			if (myrpt->txchannel != myrpt->rxchannel) 
	...
	{
		tris_log(LOG_WARNING, "Unable to set conference mode for monitor\n");
		rpt_mutex_unlock(&myrpt->lock);
		tris_hangup(myrpt->pchannel);
		tris_hangup(myrpt->monchannel);
		if (myrpt->txchannel != myrpt->rxchannel) 
	...
	/* allocate a pseudo-channel thru trismedia */
	myrpt->parrotchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	...
	tris_set_read_format(myrpt->parrotchannel,TRIS_FORMAT_SLINEAR);
	tris_set_write_format(myrpt->parrotchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (myrpt->parrotchannel->cdr)
		tris_set_flag(myrpt->parrotchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	/* allocate a pseudo-channel thru trismedia */
	myrpt->voxchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!myrpt->voxchannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		rpt_mutex_unlock(&myrpt->lock);
	...
	tris_set_read_format(myrpt->voxchannel,TRIS_FORMAT_SLINEAR);
	tris_set_write_format(myrpt->voxchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (myrpt->voxchannel->cdr)
		tris_set_flag(myrpt->voxchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	/* allocate a pseudo-channel thru trismedia */
	myrpt->txpchannel = tris_request("DAHDI",TRIS_FORMAT_SLINEAR,"pseudo",NULL, 0);
	if (!myrpt->txpchannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		rpt_mutex_unlock(&myrpt->lock);
	...
	if (myrpt->txpchannel->cdr)
		tris_set_flag(myrpt->txpchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	/* make a conference for the tx */
	ci.chan = 0;
	ci.confno = myrpt->txconf;
	...
	{
		tris_log(LOG_ERROR, "Unable to open %s\n",myrpt->p.ioport);
		rpt_mutex_unlock(&myrpt->lock);
		tris_hangup(myrpt->pchannel);
		if (myrpt->txchannel != myrpt->rxchannel) 
			tris_hangup(myrpt->txchannel);
	...
	idtalkover = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->name, "idtalkover");
	myrpt->dtmfidx = -1;
	myrpt->dtmfbuf[0] = 0;
	myrpt->rem_dtmfidx = -1;
	myrpt->rem_dtmfbuf[0] = 0;
	...
	rpt_mutex_unlock(&myrpt->lock);
	val = 1;
	tris_channel_setoption(myrpt->rxchannel,TRIS_OPTION_RELAXDTMF,&val,sizeof(char),0);
	val = 1;
	tris_channel_setoption(myrpt->rxchannel,TRIS_OPTION_TONE_VERIFY,&val,sizeof(char),0);
	...
	while (ms >= 0)
	{
		struct tris_frame *f,*f1,*f2;
		struct tris_channel *cs[300],*cs1[300];
		...
		if((myrpt->disgorgetime) && (time(NULL) >= myrpt->disgorgetime)){
			...
			myrpt->disgorgetime = 0;
			tris_log(LOG_NOTICE,"********** Variable Dump Start (app_rpt) **********\n");
			tris_log(LOG_NOTICE,"totx = %d\n",totx);
			tris_log(LOG_NOTICE,"remrx = %d\n",remrx);
			tris_log(LOG_NOTICE,"lasttx = %d\n",lasttx);
			tris_log(LOG_NOTICE,"elap = %d\n",elap);
			tris_log(LOG_NOTICE,"toexit = %d\n",toexit);

			tris_log(LOG_NOTICE,"myrpt->keyed = %d\n",myrpt->keyed);
			tris_log(LOG_NOTICE,"myrpt->localtx = %d\n",myrpt->localtx);
			tris_log(LOG_NOTICE,"myrpt->callmode = %d\n",myrpt->callmode);
			tris_log(LOG_NOTICE,"myrpt->mustid = %d\n",myrpt->mustid);
			tris_log(LOG_NOTICE,"myrpt->tounkeyed = %d\n",myrpt->tounkeyed);
			tris_log(LOG_NOTICE,"myrpt->tonotify = %d\n",myrpt->tonotify);
			tris_log(LOG_NOTICE,"myrpt->retxtimer = %ld\n",myrpt->retxtimer);
			tris_log(LOG_NOTICE,"myrpt->totimer = %d\n",myrpt->totimer);
			tris_log(LOG_NOTICE,"myrpt->tailtimer = %d\n",myrpt->tailtimer);
			tris_log(LOG_NOTICE,"myrpt->tailevent = %d\n",myrpt->tailevent);

			dl = myrpt->links.next;
              		while(dl != &myrpt->links){
				tris_log(LOG_NOTICE,"*** Link Name: %s ***\n",dl->name);
				tris_log(LOG_NOTICE,"        link->lasttx %d\n",dl->lasttx);
				tris_log(LOG_NOTICE,"        link->lastrx %d\n",dl->lastrx);
				tris_log(LOG_NOTICE,"        link->connected %d\n",dl->connected);
				tris_log(LOG_NOTICE,"        link->hasconnected %d\n",dl->hasconnected);
				tris_log(LOG_NOTICE,"        link->outbound %d\n",dl->outbound);
				tris_log(LOG_NOTICE,"        link->disced %d\n",dl->disced);
				tris_log(LOG_NOTICE,"        link->killme %d\n",dl->killme);
				tris_log(LOG_NOTICE,"        link->disctime %ld\n",dl->disctime);
				tris_log(LOG_NOTICE,"        link->retrytimer %ld\n",dl->retrytimer);
				tris_log(LOG_NOTICE,"        link->retries = %d\n",dl->retries);
				tris_log(LOG_NOTICE,"        link->reconnects = %d\n",dl->reconnects);
				tris_log(LOG_NOTICE,"        link->newkey = %d\n",dl->newkey);
                        	dl = dl->next;
                	}
                                                                                                                               
			dt = myrpt->tele.next;
			if(dt != &myrpt->tele)
				tris_log(LOG_NOTICE,"*** Telemetry Queue ***\n");
              		while(dt != &myrpt->tele){
				tris_log(LOG_NOTICE,"        Telemetry mode: %d\n",dt->mode);
                        	dt = dt->next;
                	}
			tris_log(LOG_NOTICE,"******* Variable Dump End (app_rpt) *******\n");
		...
		if (myrpt->reload)
		{
			...
			{
				tris_softhangup(inner_telem->chan,TRIS_SOFTHANGUP_DEV);
		...
		rpt_mutex_lock(&myrpt->lock);
		if (tris_check_hangup(myrpt->rxchannel)) break;
		if (tris_check_hangup(myrpt->txchannel)) break;
		if (tris_check_hangup(myrpt->pchannel)) break;
		if (tris_check_hangup(myrpt->monchannel)) break;
		if (myrpt->parrotchannel && 
			tris_check_hangup(myrpt->parrotchannel)) break;
		if (myrpt->voxchannel && 
			tris_check_hangup(myrpt->voxchannel)) break;
		if (tris_check_hangup(myrpt->txpchannel)) break;
		if (myrpt->dahditxchannel && tris_check_hangup(myrpt->dahditxchannel)) break;
		...
		{
		    if(debug)
				tris_log(LOG_NOTICE, "timed-out and in circuit busy after call\n");
			myrpt->callmode = 0;
			myrpt->macropatch=0;
			channel_revert(myrpt);
		}
		...
		if ((myrpt->keyed || remrx) && ((identqueued && idtalkover) || (tailmessagequeued))) {
			...
			while(telem != &myrpt->tele){
				if(telem->mode == ID){
					if (telem->chan) tris_softhangup(telem->chan, TRIS_SOFTHANGUP_DEV); /* Whoosh! */
					hasid = 1;
				}
		...
		totx = totx || (!TRIS_LIST_EMPTY(&myrpt->txq));
		if (totx && (!lasttx))
		{
			char mydate[100],myfname[100];
			time_t myt;

			if (myrpt->monstream) tris_closestream(myrpt->monstream);
			if (myrpt->p.archivedir)
			{
				long blocksleft;
				...
				sprintf(myfname,"%s/%s/%s",myrpt->p.archivedir,
					myrpt->name,mydate);
				myrpt->monstream = tris_writefile(myfname,"wav49",
			...
			myrpt->p.parrotmode && (!myrpt->parrotstate))
		{
			...
			{
				tris_log(LOG_WARNING, "Unable to set conference mode for parrot\n");
				break;
			}
			...
			if (myrpt->parrotstream) 
				tris_closestream(myrpt->parrotstream);
			myrpt->parrotstream = NULL;
			myrpt->parrotstream = tris_writefile(myfname,"wav",
		...
		while(l != &myrpt->links)
		{
			...
			{
				myrx = myrx || (!TRIS_LIST_EMPTY(&l->rxq));
				if (l->voxtotimer <= 0)
				{
					if (l->voxtostate)
					{
			...
			{
				struct	tris_frame lf;

				memset(&lf,0,sizeof(lf));
				lf.frametype = TRIS_FRAME_TEXT;
				lf.subclass = 0;
				lf.offset = 0;
				lf.mallocd = 0;
				lf.samples = 0;
				...
				if (l->chan)
				{
					...
					lf.data.ptr = lstr;
					tris_write(l->chan,&lf);
					if (debug > 6) tris_log(LOG_NOTICE,
			...
			if (l->newkey)
			{
				if ((l->retxtimer += elap) >= REDUNDANT_TX_TIME)
				{
					...
					{
						if (l->lasttx)
							tris_indicate(l->chan,TRIS_CONTROL_RADIO_KEY);
						else
							tris_indicate(l->chan,TRIS_CONTROL_RADIO_UNKEY);
					}
				}
			...
			if ((l->elaptime > MAXCONNECTTIME) && 
			   ((!l->chan) || (l->chan->_state != TRIS_STATE_UP)))
			{
				l->elaptime = 0;
				rpt_mutex_unlock(&myrpt->lock);
				if (l->chan) tris_softhangup(l->chan,TRIS_SOFTHANGUP_DEV);
            ...
            {
				if(debug)tris_log(LOG_NOTICE, "LINKDISC AA\n");
                /* remove from queue */
                remque((struct qelem *) l);
		...
		if (myrpt->linkposttimer <= 0)
		{
			...
			str = tris_malloc(nstr + 256);
			if (!str)
			{
				tris_log(LOG_NOTICE,"Cannot tris_malloc()\n");
				break;
			}
			nstr = 0;
			strcpy(str,"nodes=");
		...
		if (who == myrpt->rxchannel) /* if it was a read from rx */
		{
			...
			if (f->frametype == TRIS_FRAME_VOICE)
			{
				...
				if (n == 1)
				{
						...
						if (debug > 2)
						{
							tris_log(LOG_NOTICE,"Got (single-length) packet:\n");
							tris_log(LOG_NOTICE,"op: %02x, arg: %02x, UnitID: %04x\n",
				...
				if ((debug > 2) && (i == 2))
				{
					...
					mdc_decoder_get_double_packet(myrpt->mdc,&op,&arg,&unitID,
						&ex1,&ex2,&ex3,&ex4);
					tris_log(LOG_NOTICE,"Got (double-length) packet:\n");
					tris_log(LOG_NOTICE,"op: %02x, arg: %02x, UnitID: %04x\n",
						op & 255,arg & 255,unitID);
					tris_log(LOG_NOTICE,"ex1: %02x, ex2: %02x, ex3: %02x, ex4: %02x\n",
				...
				if (f1)
				{
					tris_write(myrpt->pchannel,f1);
					tris_frfree(f1);
				}
			}
#ifndef	OLD_TRISMEDIA
			...
			else if (f->frametype == TRIS_FRAME_CONTROL)
			{
				...
				/* if RX key */
				if (f->subclass == TRIS_CONTROL_RADIO_KEY)
				{
					...
					if (f->datalen && f->data.ptr)
					{
						...
						if (debug) tris_log(LOG_NOTICE,"Got PL %s on node %s\n",(char *)f->data.ptr,myrpt->name);
						...
						{
							char *value = (char *) tris_variable_retrieve(myrpt->cfg, myrpt->p.tonemacro, (char *)f->data.ptr);
							if (value)
							{
								if (debug) tris_log(LOG_NOTICE,"Tone %s doing %s on node %s\n",(char *) f->data.ptr,value,myrpt->name);
								rpt_mutex_lock(&myrpt->lock);
								if ((MAXMACRO - strlen(myrpt->macrobuf)) < strlen(value)){
									rpt_mutex_unlock(&myrpt->lock);
									busy=1;
				...
				/* if RX un-key */
				if (f->subclass == TRIS_CONTROL_RADIO_UNKEY)
				{
					if ((!lasttx) || (myrpt->p.duplex > 1) || (myrpt->p.linktolink))
					{
						if (debug == 7) printf("@@@@ rx un-key\n");
		...
		{
			f = tris_read(myrpt->dahditxchannel);
			if (!f)
			{
				if (debug) printf("@@@@ rpt:Hung Up\n");
				break;
			...
			if (f->frametype == TRIS_FRAME_VOICE)
			{
				...
				if (myrpt->p.duplex < 2)
				{
					if (myrpt->txrealkeyed) 
					{
						...
						{
						    x = 0;
						    TRIS_LIST_TRAVERSE(&myrpt->txq, vframe,
							frame_list) x++;
						    for(;x < myrpt->p.simplexpatchdelay; x++)
						    {
								vframe = tris_frdup(f);
								memset(vframe->data.ptr,0,vframe->datalen);
								TRIS_LIST_INSERT_TAIL(&myrpt->txq,vframe,frame_list);
						    }
						    myfirst = 1;
						}
						vframe = tris_frdup(f);
						TRIS_LIST_INSERT_TAIL(&myrpt->txq,
							vframe,frame_list);
					} else myfirst = 0;
					x = 0;
					TRIS_LIST_TRAVERSE(&myrpt->txq, vframe,
					...
					{
						tris_frfree(f);
						f = TRIS_LIST_REMOVE_HEAD(&myrpt->txq,
							frame_list);
					}
				}
				else
				{
					while((vframe = TRIS_LIST_REMOVE_HEAD(&myrpt->txq,
						frame_list))) tris_frfree(vframe);
				}
				tris_write(myrpt->txchannel,f);
			}
			if (f->frametype == TRIS_FRAME_CONTROL)
			{
				if (f->subclass == TRIS_CONTROL_HANGUP)
		...
		while(l != &myrpt->links)
		{
			...
			if ((who == l->chan) || (!l->lastlinktv.tv_sec) ||
				(tris_tvdiff_ms(now,l->lastlinktv) >= 19))
			{
				l->lastlinktv = now;
			...
			if (who == l->chan) /* if it was a read from rx */
			{
				...
				if (!f)
				{
					...
					if (l->lastf1) tris_frfree(l->lastf1);
					l->lastf1 = NULL;
					if (l->lastf2) tris_frfree(l->lastf2);
					l->lastf2 = NULL;
					/* hang-up on call to device */
					tris_hangup(l->chan);
					tris_hangup(l->pchan);
				...
				if (f->frametype == TRIS_FRAME_VOICE)
				{
					...
					if ((l->phonemode) && (l->phonevox))
					{
						...
						if (n1 != l->wasvox)
						{
							if (debug)tris_log(LOG_DEBUG,"Link Node %s, vox %d\n",l->name,n1);
							l->wasvox = n1;
							l->voxtostate = 0;
						...
						if (l->lastrealrx || n1)
						{
							...
							{
							    x = 0;
							    TRIS_LIST_TRAVERSE(&l->rxq, f1,
								frame_list) x++;
							    for(;x < myrpt->p.simplexphonedelay; x++)
								{
									f1 = tris_frdup(f);
									memset(f1->data.ptr,0,f1->datalen);
									TRIS_LIST_INSERT_TAIL(&l->rxq,
										f1,frame_list);
							    }
							    myfirst = 1;
							}
							f1 = tris_frdup(f);
							TRIS_LIST_INSERT_TAIL(&l->rxq,f1,frame_list);
						} else myfirst = 0; 
						x = 0;
						TRIS_LIST_TRAVERSE(&l->rxq, f1,frame_list) x++;
						if (!x)
						{
							memset(f->data.ptr,0,f->datalen);
						}
						...
						{
							tris_frfree(f);
							f = TRIS_LIST_REMOVE_HEAD(&l->rxq,frame_list);
						}
...
#endif
				if (f->frametype == TRIS_FRAME_TEXT)
				{
		...
		if (who == myrpt->monchannel) 
		{
			...
			{
				if (myrpt->monstream) 
					tris_writestream(myrpt->monstream,f);
			}
			if (f->frametype == TRIS_FRAME_CONTROL)
			{
				if (f->subclass == TRIS_CONTROL_HANGUP)
		...
		{
			f = tris_read(myrpt->parrotchannel);
			if (!f)
			{
				if (debug) printf("@@@@ rpt:Hung Up\n");
				break;
			...
			{
				if (myrpt->parrotstream) 
					tris_writestream(myrpt->parrotstream,f);
			}
			if (f->frametype == TRIS_FRAME_CONTROL)
			{
				if (f->subclass == TRIS_CONTROL_HANGUP)
		...
		if (myrpt->voxchannel && (who == myrpt->voxchannel))
		{
			...
			if (f->frametype == TRIS_FRAME_VOICE)
			{
				...
				if (n != myrpt->wasvox)
				{
					if (debug) tris_log(LOG_DEBUG,"Node %s, vox %d\n",myrpt->name,n);
					myrpt->wasvox = n;
					myrpt->voxtostate = 0;
	...
	tris_hangup(myrpt->pchannel);
	tris_hangup(myrpt->monchannel);
	if (myrpt->parrotchannel) tris_hangup(myrpt->parrotchannel);
	myrpt->parrotstate = 0;
	if (myrpt->voxchannel) tris_hangup(myrpt->voxchannel);
	tris_hangup(myrpt->txpchannel);
	if (myrpt->txchannel != myrpt->rxchannel) tris_hangup(myrpt->txchannel);
	if (myrpt->dahditxchannel != myrpt->txchannel) tris_hangup(myrpt->dahditxchannel);
	if (myrpt->lastf1) tris_frfree(myrpt->lastf1);
	myrpt->lastf1 = NULL;
	if (myrpt->lastf2) tris_frfree(myrpt->lastf2);
	myrpt->lastf2 = NULL;
	tris_hangup(myrpt->rxchannel);
	rpt_mutex_lock(&myrpt->lock);
	l = myrpt->links.next;
...
char *this,*val;
	...
	n = 0;
#ifndef OLD_TRISMEDIA
	/* wait until trismedia starts */
        while(!tris_test_flag(&tris_options,TRIS_OPT_FLAG_FULLY_BOOTED))
                usleep(250000);
#endif
#ifdef	NEW_TRISMEDIA
	rpt_vars[n].cfg = tris_config_load("rpt.conf",config_flags);
#else
	rpt_vars[n].cfg = tris_config_load("rpt.conf");
#endif
	cfg = rpt_vars[n].cfg;
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_NOTICE, "Unable to open radio repeater configuration rpt.conf.  Radio Repeater disabled.\n");
	...
	while((this = tris_category_browse(cfg,this)) != NULL)
	{
		...
		rpt_vars[n].name = tris_strdup(this);
		val = (char *) tris_variable_retrieve(cfg,this,"rxchannel");
		if (val) rpt_vars[n].rxchanname = tris_strdup(val);
		val = (char *) tris_variable_retrieve(cfg,this,"txchannel");
		if (val) rpt_vars[n].txchanname = tris_strdup(val);
		rpt_vars[n].remote = 0;
		rpt_vars[n].remoterig = "";
		val = (char *) tris_variable_retrieve(cfg,this,"remote");
		if (val) 
		{
			rpt_vars[n].remoterig = tris_strdup(val);
			rpt_vars[n].remote = 1;
		}
		val = (char *) tris_variable_retrieve(cfg,this,"radiotype");
		if (val) rpt_vars[n].remoterig = tris_strdup(val);
		tris_mutex_init(&rpt_vars[n].lock);
		tris_mutex_init(&rpt_vars[n].remlock);
		tris_mutex_init(&rpt_vars[n].statpost_lock);
		rpt_vars[n].tele.next = &rpt_vars[n].tele;
		rpt_vars[n].tele.prev = &rpt_vars[n].tele;
		rpt_vars[n].rpt_thread = TRIS_PTHREADT_NULL;
		rpt_vars[n].tailmessagen = 0;
	...
	for(i = 0; i < n; i++)
	{
		...
		else /* is a normal repeater */
		{
			...
			tris_log(LOG_NOTICE,"Normal Repeater Init  %s  %s  %s\n",rpt_vars[i].name, rpt_vars[i].remoterig, rpt_vars[i].freq);
		}
		if (!rpt_vars[i].p.ident)
		{
			tris_log(LOG_WARNING,"Did not specify ident for node %s\n",rpt_vars[i].name);
			tris_config_destroy(cfg);
			pthread_exit(NULL);
		}
	        pthread_attr_init(&attr);
	        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
		tris_pthread_create(&rpt_vars[i].rpt_thread,&attr,rpt,(void *) &rpt_vars[i]);
	}
	usleep(500000);
	time(&starttime);
	for(;;)
	{
		...
		{ 
			...
			if (rpt_vars[i].rpt_thread == TRIS_PTHREADT_STOP) 
				rv = -1;
			else
			...
			if (rv)
			{
				if(time(NULL) - rpt_vars[i].lastthreadrestarttime <= 15)
				{
					...
					{
						tris_log(LOG_ERROR,"Continual RPT thread restarts, killing Trismedia\n");
						exit(1); /* Stuck in a restart loop, kill Trismedia and start over */
					}
					else
					{
						tris_log(LOG_NOTICE,"RPT thread restarted on %s\n",rpt_vars[i].name);
				...
				tris_pthread_create(&rpt_vars[i].rpt_thread,&attr,rpt,(void *) &rpt_vars[i]);
				/* if (!rpt_vars[i].xlink) */
					tris_log(LOG_WARNING, "rpt_thread restarted on node %s\n", rpt_vars[i].name);
			}
		...
		for(;;)
		{
			...
			*space = 0;
			...
			if (fd == -1)
			{
				tris_log(LOG_ERROR,"Cannot open node log file %s for write",space + 1);
				tris_free(nodep);
				continue;
			}
			if (write(fd,space + 1,strlen(space + 1)) !=
				strlen(space + 1))
			{
				tris_log(LOG_ERROR,"Cannot write node log file %s for write",space + 1);
				tris_free(nodep);
				continue;
			}
			close(fd);
...
static int rpt_exec(struct tris_channel *chan, void *data)
{
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Rpt requires an argument (system node)\n");
		return -1;
	}
	...
	{
		tris_log(LOG_NOTICE,"Node %s rejecting call: too soon!\n",tmp);
		tris_safe_sleep(chan,3000);
		return -1;
	}

	tris_log(LOG_NOTICE,"parsing argument=%s \n",tmp);
	...
	tris_log(LOG_NOTICE,"options=%s \n",options);
	if(memp>0)tris_log(LOG_NOTICE,"memp=%s \n",memp);
	if(altp>0)tris_log(LOG_NOTICE,"altp=%s \n",altp);
	...
	if (myrpt == NULL)
	{
		...
		tris_log(LOG_WARNING, "Cannot find specified system node %s\n",tmp);
	...
	if (options && ((*options == 'P') || (*options == 'D') || (*options == 'R') || (*options == 'S')))
	{
		...
		if(memp>0)
		{
			...
			if(numlinks>0 && radiochan!=myrpt->nowchan && !myrpt->bargechan)
			{
				...
				tris_log(LOG_NOTICE, "Radio Channel Busy.\n");
				return (priority_jump(myrpt,chan));
			}
		...
		tris_set_callerid(chan,"0","app_rpt user","0");
		val = 1;
		tris_channel_setoption(chan,TRIS_OPTION_TONE_VERIFY,&val,sizeof(char),0);
	...
	else
	{
	        ...
	        if ( (strncmp(chan->name,"IAX2",4)) && (strncmp(chan->name,"Local",5)) ) {
	            tris_log(LOG_WARNING, "We only accept links via IAX2 or Local!!\n");
	            return -1;
	        }
#else
		if (strncmp(chan->name,"IAX2",4))
		{
			tris_log(LOG_WARNING, "We only accept links via IAX2!!\n");
			return -1;
		}
#endif
	        if(myrpt->p.s[myrpt->p.sysstate_cur].txdisable){ /* Do not allow incoming radio connections if disabled */
        	        tris_log(LOG_NOTICE, "Connect attempt to node %s  with tx disabled", myrpt->name);
                	return -1;
        	}	
	}
	if (options && (*options == 'R'))
	{
		...
		if ((!myrpt->p.nobusyout) && m)
		{
			...
			while(tris_safe_sleep(chan,10000) != -1);
			return -1;
		}
		...
		template=strsep(&s,"|");
		if(!template) {
			tris_log(LOG_WARNING, "An announce template must be defined\n");
			tris_free(orig_s);
			return -1;
		} 
		...
		if(atoi(priority) < 0) {
			tris_log(LOG_WARNING, "Priority '%s' must be a number > 0\n", priority);
			tris_free(orig_s);
			return -1;
		}
		...
		if(option_verbose > 2) {
			tris_verbose( VERBOSE_PREFIX_3 "Return Context: (%s,%s,%d) ID: %s\n", chan->context,chan->exten, chan->priority, chan->cid.cid_num);
			if(!tris_exists_extension(chan, chan->context, chan->exten, chan->priority, chan->cid.cid_num)) {
				tris_verbose( VERBOSE_PREFIX_3 "Warning: Return Context Invalid, call will return to default|s\n");
			}
		}
		...
		tris_masq_park_call(chan, NULL, timeout, &lot);

		if (option_verbose > 2) tris_verbose( VERBOSE_PREFIX_3 "Call Parking Called, lot: %d, timeout: %d, context: %s\n", lot, timeout, return_context);
	...
	if (!options)
	{
		...
		{
			tris_log(LOG_WARNING, "Does not have callerid on %s\n",tmp);
			return -1;
		}
		...
		if (!hisip[0])
		{
			tris_log(LOG_WARNING, "Link IP address cannot be determined!!\n");
			return -1;
		}
		
		tris_callerid_parse(chan->cid.cid_num,&b,&b1);
		tris_shrink_phone_number(b1);
		if (!strcmp(myrpt->name,b1))
		{
			tris_log(LOG_WARNING, "Trying to link to self!!\n");
			return -1;
		}

		if (*b1 < '1')
		{
			tris_log(LOG_WARNING, "Node %s Invalid for connection here!!\n",b1);
			return -1;
		}
		...
		if (!val)
		{
			tris_log(LOG_WARNING, "Reported node %s cannot be found!!\n",b1);
			return -1;
		}
		strncpy(tmp,val,sizeof(tmp) - 1);
		s = tmp;
		...
		if (!s2)
		{
			tris_log(LOG_WARNING, "Reported node %s not in correct format!!\n",b1);
			return -1;
		}
                if (strcmp(s2,"NONE")) {
			hp = tris_gethostbyname(s2, &ahp);
			if (!hp)
			{
				tris_log(LOG_WARNING, "Reported node %s, name %s cannot be found!!\n",b1,s2);
				return -1;
			}
			memcpy(&ia,hp->h_addr,sizeof(in_addr_t));
#ifdef	OLD_TRISMEDIA
			tris_inet_ntoa(nodeip,sizeof(nodeip) - 1,ia);
#else
			strncpy(nodeip,tris_inet_ntoa(ia),sizeof(nodeip) - 1);
#endif
			s3 = strchr(hisip,':');
			if (s3) *s3 = 0;
			if (strcmp(hisip,nodeip))
			{
				...
				if (strcmp(hisip,nodeip))
				{
					tris_log(LOG_WARNING, "Node %s IP %s does not match link IP %s!!\n",b1,nodeip,hisip);
					return -1;
				}
			}
		}
	...
	if (!myrpt->remote)
	{
		...
		if (i)
		{
			tris_log(LOG_WARNING, "Cannot connect to node %s, system busy\n",myrpt->name);
			return -1;
		}
		/* look at callerid to see what node this comes from */
		if (!chan->cid.cid_num) /* if doesn't have caller id */
		{
			tris_log(LOG_WARNING, "Doesnt have callerid on %s\n",tmp);
			return -1;
		}
	...
	if (myrpt->remoteon)
	{
		...
		if (myrpt->remoteon)
		{
			tris_log(LOG_WARNING, "Trying to use busy link on %s\n",tmp);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
			if (chan->cdr)
	...
	if (myrpt->p.rptnode)
	{
		...
		for(i = 0; i < nrpts; i++)
		{
			if (!strcasecmp(rpt_vars[i].name,myrpt->p.rptnode))
			{
				...
				{
					rpt_mutex_unlock(&myrpt->lock);
					tris_log(LOG_WARNING, "Trying to use busy link (repeater node %s) on %s\n",rpt_vars[i].name,tmp);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
					if (chan->cdr)
						tris_set_flag(chan->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
				...
				while(rpt_vars[i].xlink != 3)
				{
					if (!killedit)
					{
						tris_softhangup(rpt_vars[i].rxchannel,TRIS_SOFTHANGUP_DEV);
						rpt_vars[i].xlink = 1;
						killedit = 1;
					}
					rpt_mutex_unlock(&myrpt->lock);
					if (tris_safe_sleep(chan,500) == -1)
					{
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
						if (chan->cdr)
							tris_set_flag(chan->cdr,TRIS_CDR_FLAG_POST_DISABLED);
	...
	{
		rpt_mutex_unlock(&myrpt->lock);
		tris_log(LOG_WARNING, "Can't get io permission on IO port %x hex\n",myrpt->p.iobase);
		return -1;
	}
#endif
	myrpt->remoteon = 1;
	...
	*tele++ = 0;
	myrpt->rxchannel = tris_request(myrpt->rxchanname,TRIS_FORMAT_SLINEAR,tele,NULL, 0);
	myrpt->dahdirxchannel = NULL;
	...
	*--tele = '/';
	...
	if (myrpt->txchanname)
	{
		...
		*tele++ = 0;
		myrpt->txchannel = tris_request(myrpt->txchanname,TRIS_FORMAT_SLINEAR,tele,NULL, 0);
	...
	tris_set_read_format(myrpt->pchannel,TRIS_FORMAT_SLINEAR);
	tris_set_write_format(myrpt->pchannel,TRIS_FORMAT_SLINEAR);
#ifdef	TRIS_CDR_FLAG_POST_DISABLED
	if (myrpt->pchannel->cdr)
		tris_set_flag(myrpt->pchannel->cdr,TRIS_CDR_FLAG_POST_DISABLED);
#endif
	...
	if ((myrpt->iofd < 1) && (myrpt->txchannel == myrpt->dahditxchannel))
	{
		...
		if ((!res) && (!strcmp(myrpt->remoterig,remote_rig_kenwood)))
		{
			...
			{
				tris_log(LOG_ERROR,"Cannot set UIOMODE\n");
				return -1;
			}
			...
			if (ioctl(myrpt->dahditxchannel->fds[0],DAHDI_RADIO_SETPARAM,&z) == -1)
			{
				tris_log(LOG_ERROR,"Cannot set UIODATA\n");
				return -1;
			}
			i = DAHDI_OFFHOOK;
			if (ioctl(myrpt->dahditxchannel->fds[0],DAHDI_HOOK,&i) == -1)
			{
				tris_log(LOG_ERROR,"Cannot set hook\n");
				return -1;
			}
			iskenwood_pci4 = 1;
		}
	...
	myrpt->newkey = 0;
	rpt_mutex_unlock(&myrpt->lock);
	tris_set_write_format(chan, TRIS_FORMAT_SLINEAR);
	tris_set_read_format(chan, TRIS_FORMAT_SLINEAR);
	rem_rx = 0;
	remkeyed = 0;
	...
	if (myrpt->rxchannel == myrpt->dahdirxchannel)
	{
		if (ioctl(myrpt->dahdirxchannel->fds[0],DAHDI_GET_PARAMS,&par) != -1)
		{
			if (par.rxisoffhook)
			{
				tris_indicate(chan,TRIS_CONTROL_RADIO_KEY);
				myrpt->remoterx = 1;
				remkeyed = 1;
			}
		}
	...
	if (myrpt->p.archivedir)
	{
		...
		if (myrpt->p.monminblocks)
		{
			...
			if (blocksleft >= myrpt->p.monminblocks)
				tris_cli_command(nullfd,mycmd);
		} else tris_cli_command(nullfd,mycmd);
		/* look at callerid to see what node this comes from */
		if (!chan->cid.cid_num) /* if doesn't have caller id */
		{
			b1 = "0";
	...
	for(;;) 
	{
		if (tris_check_hangup(chan)) break;
		if (tris_check_hangup(myrpt->rxchannel)) break;
		notremming = 0;
		setting = 0;
		reming = 0;
		...
		//
	    if((debug > 6) && rem_totx) {
	    	tris_log(LOG_NOTICE,"Set rem_totx=%i.  dtmf_local_timer=%i phone_mode=%i keyed=%i tunerequest=%i\n",rem_totx,myrpt->dtmf_local_timer,phone_mode,keyed,myrpt->tunerequest);
		}
		if (keyed && (!keyed1))
		{
			keyed1 = 1;
		...
		{
			myrpt->remoterx = 0;
			tris_indicate(chan,TRIS_CONTROL_RADIO_UNKEY);
		}
		...
		if (rem_totx && (!myrpt->remotetx))
		{
			...
			{
				if(debug > 6)
					tris_log(LOG_NOTICE,"Handle rem_totx=%i.  dtmf_local_timer=%i  tunerequest=%i\n",rem_totx,myrpt->dtmf_local_timer,myrpt->tunerequest);
	...
	myrpt->iofd = -1;
	tris_hangup(myrpt->pchannel);
	if (myrpt->rxchannel != myrpt->txchannel) tris_hangup(myrpt->txchannel);
	tris_hangup(myrpt->rxchannel);
	closerem(myrpt);
	if (myrpt->p.rptnode)
	{
		...
		if ((node)&&(!strcmp(node,rpt_vars[i].name))){
			...
			if(myrpt->remote){ /* Remote base ? */
				...
				if((remoteon = myrpt->remoteon)){
					if(!tris_strlen_zero(myrpt->loginuser))
						loginuser = tris_strdup(myrpt->loginuser);
					if(!tris_strlen_zero(myrpt->loginlevel))
						loginlevel = tris_strdup(myrpt->loginlevel);
					if(!tris_strlen_zero(myrpt->freq))
						freq = tris_strdup(myrpt->freq);
					if(!tris_strlen_zero(myrpt->rxpl))
						rxpl = tris_strdup(myrpt->rxpl);
					if(!tris_strlen_zero(myrpt->txpl))
						txpl = tris_strdup(myrpt->txpl);
...
#endif
{
	...
	for(i = 0; i < nrpts; i++) {
		if (!strcmp(rpt_vars[i].name,rpt_vars[i].p.nodes)) continue;
                tris_mutex_destroy(&rpt_vars[i].lock);
                tris_mutex_destroy(&rpt_vars[i].remlock);
	}
	res = tris_unregister_application(app);

#ifdef	NEW_TRISMEDIA
	tris_cli_unregister_multiple(rpt_cli, ARRAY_LEN(rpt_cli));
#else
	/* Unregister cli extensions */
	tris_cli_unregister(&cli_debug);
	tris_cli_unregister(&cli_dump);
	tris_cli_unregister(&cli_stats);
	tris_cli_unregister(&cli_lstats);
	tris_cli_unregister(&cli_nodes);
	tris_cli_unregister(&cli_local_nodes);
	tris_cli_unregister(&cli_reload);
	tris_cli_unregister(&cli_restart);
	tris_cli_unregister(&cli_fun);
	tris_cli_unregister(&cli_fun1);
	res |= tris_cli_unregister(&cli_cmd);
#endif
#ifndef OLD_TRISMEDIA
	res |= tris_manager_unregister("RptLocalNodes");
	res |= tris_manager_unregister("RptStatus");
#endif
	return res;
}
...
{
	int res;
	tris_pthread_create(&rpt_master_thread,NULL,rpt_master,NULL);

#ifdef	NEW_TRISMEDIA
	tris_cli_register_multiple(rpt_cli, ARRAY_LEN(rpt_cli));
	res = 0;
#else
	/* Register cli extensions */
	tris_cli_register(&cli_debug);
	tris_cli_register(&cli_dump);
	tris_cli_register(&cli_stats);
	tris_cli_register(&cli_lstats);
	tris_cli_register(&cli_nodes);
	tris_cli_register(&cli_local_nodes);
	tris_cli_register(&cli_reload);
	tris_cli_register(&cli_restart);
	tris_cli_register(&cli_fun);
	tris_cli_register(&cli_fun1);
	res = tris_cli_register(&cli_cmd);
#endif
#ifndef OLD_TRISMEDIA
	res |= tris_manager_register("RptLocalNodes", 0, manager_rpt_local_nodes, "List local node numbers");
	res |= tris_manager_register("RptStatus", 0, manager_rpt_status, "Return Rpt Status for CGI");

#endif
	res |= tris_register_application(app, rpt_exec, synopsis, descrip);
	return res;
}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Radio Repeater/Remote Base Application",
		.load = load_module,
