...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 187366 $")
...
static int sendtext_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	...
	/* NOT tris_strlen_zero, because some protocols (e.g. SIP) MUST be able to
	 * send a zero-length message. */
	if (!data) {
		tris_log(LOG_WARNING, "SendText requires an argument (text)\n");
		return -1;
	} else
	...
	status = "FAILURE";
	tris_channel_unlock(chan);
	res = tris_sendtext(chan, args.text);
	if (!res)
		status = "SUCCESS";
	pbx_builtin_setvar_helper(chan, "SENDTEXTSTATUS", status);
	return 0;
...
{
	return tris_register_application_xml(app, sendtext_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Send Text Applications");
