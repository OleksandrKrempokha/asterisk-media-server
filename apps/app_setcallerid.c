...
static int setcallerid_pres_exec(struct tris_channel *chan, void *data)
{
	int pres = -1;
	...
	if (!deprecated) {
		deprecated = 1;
		tris_log(LOG_WARNING, "SetCallerPres is deprecated.  Please use Set(CALLERPRES()=%s) instead.\n", (char *)data);
	}
	...
	if (sscanf(data, "%30d", &pres) != 1 || pres < 0 || pres > 255 || (pres & 0x9c)) {
		pres = tris_parse_caller_presentation(data);
	}

	if (pres < 0) {
		tris_log(LOG_WARNING, "'%s' is not a valid presentation (see 'show application SetCallerPres')\n",
			(char *) data);
		return 0;
	}
...
{
	return tris_unregister_application(app2);
}

static int load_module(void)
{
	return tris_register_application_xml(app2, setcallerid_pres_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Set CallerID Presentation Application");
