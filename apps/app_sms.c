...
#include "trismedia/paths.h"  /* use tris_config_TRIS_SPOOL_DIR and LOG_DIR */
	...
	<application name="SMS" language="en_US">
		...
		<syntax>
			<parameter name="name" required="true">
				<para>The name of the queue used in <filename>/var/spool/trismedia/sms</filename></para>
			</parameter>
...
static char *isodate(time_t t, char *buf, int len)
{
	...
	tris_localtime(&local, &tm, NULL);
	tris_strftime(buf, len, "%Y-%m-%dT%H:%M:%S", &tm);
	return buf;
}
...
static void sms_log(sms_t * h, char status)
{
	...
	o = open(log_file, O_CREAT | O_APPEND | O_WRONLY, TRIS_FILE_MODE);
	if (o >= 0) {
			...
			if (*p == '=') {
				*p++ = 0;
				if (!strcmp(line, "ud")) {  /* parse message (UTF-8) */
					...
					h->udl = o;
					if (*p) {
						tris_log(LOG_WARNING, "UD too long in %s\n", fn);
					}
				} else {
					while (isspace (*p)) {
						p++;
					...
					if (!strcmp(line, "oa") && strlen(p) < sizeof(h->oa)) {
						...
						int Y, m, d, H, M, S;
						/* XXX Why aren't we using tris_strptime here? */
						if (sscanf(p, "%4d-%2d-%2dT%2d:%2d:%2d", &Y, &m, &d, &H, &M, &S) == 6) {
							...
							t.tm_sec = S;
							t.tm_isdst = -1;
							h->scts = tris_mktime(&t, NULL);
							if (h->scts.tv_sec == 0) {
								tris_log(LOG_WARNING, "Bad date/timein %s: %s", fn, p);
							}
						}
					} else {
						tris_log(LOG_WARNING, "Cannot parse in %s: %s=%si\n", fn, line, p);
					}
				}
				...
				*p++ = 0;
				if (*p == '#') {
					...
					if (!strcmp(line, "ud")) {        /* user data */
						...
						h->udl = o;
						if (*p)
							tris_log(LOG_WARNING, "UD too long / invalid UCS-2 hex in %s\n", fn);
					} else
						tris_log(LOG_WARNING, "Only ud can use ## format, %s\n", fn);
					...
					h->udl = o;
					if (*p) {
						tris_log(LOG_WARNING, "UD too long / invalid UCS-1 hex in %s\n", fn);
					}
					...
					h->udhl = o;
					if (*p) {
						tris_log(LOG_WARNING, "UDH too long / invalid hex in %s\n", fn);
					}
				} else {
					tris_log(LOG_WARNING, "Only ud and udh can use # format, %s\n", fn);
				}
			} else {
				tris_log(LOG_WARNING, "Cannot parse in %s: %s\n", fn, line);
			}
		}
		fclose(s);
		if (!dcsset && packsms7(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			if (packsms8(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
				if (packsms16(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
					tris_log(LOG_WARNING, "Invalid UTF-8 message even for UCS-2 (%s)\n", fn);
				} else {
					h->dcs = 0x08;          /* default to 16 bit */
					tris_log(LOG_WARNING, "Sending in 16 bit format(%s)\n", fn);
				}
			} else {
				h->dcs = 0xF5;              /* default to 8 bit */
				tris_log(LOG_WARNING, "Sending in 8 bit format(%s)\n", fn);
			}
		}
		if (is7bit(h->dcs) && packsms7(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			tris_log(LOG_WARNING, "Invalid 7 bit GSM data %s\n", fn);
		}
		if (is8bit(h->dcs) && packsms8(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			tris_log(LOG_WARNING, "Invalid 8 bit data %s\n", fn);
		}
		if (is16bit(h->dcs) && packsms16(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			tris_log(LOG_WARNING, "Invalid 16 bit data %s\n", fn);
		}
	}
}
...
static void sms_writefile(sms_t * h)
{
	...
	if (tris_tvzero(h->scts)) {
		h->scts = tris_tvnow();
	}
	snprintf(fn, sizeof(fn), "%s/sms/%s", tris_config_TRIS_SPOOL_DIR, h->smsc ? h->rx ? "morx" : "mttx" : h->rx ? "mtrx" : "motx");
	tris_mkdir(fn, 0777);                    /* ensure it exists */
	tris_copy_string(fn2, fn, sizeof(fn2));
	...
	if (rename(fn, fn2)) {
		...
		tris_log(LOG_EVENT, "Received to %s\n", fn2);
	}
}
...
static unsigned char sms_handleincoming (sms_t * h)
{
	...
	if (h->smsc) {                          /* SMSC */
		if ((h->imsg[2] & 3) == 1) {        /* SMS-SUBMIT */
			...
			tris_copy_string(h->oa, h->cli, sizeof(h->oa));
			h->scts = tris_tvnow();
			h->mr = h->imsg[p++];
			...
			if (p != h->imsg[1] + 2) {
				tris_log(LOG_WARNING, "Mismatch receive unpacking %d/%d\n", p, h->imsg[1] + 2);
				return 0xFF;		  /* duh! */
			}
		} else {
			tris_log(LOG_WARNING, "Unknown message type %02X\n", h->imsg[2]);
			return 0xFF;
		}
	} else {                                /* client */
...
static int sms_handleincoming_proto2(sms_t *h)
{
	...
	sz = h->imsg[1] + 2;
	/* tris_verb(3, "SMS-P2 Frame: %s\n", sms_hexdump(h->imsg, sz, debug_buf)); */

	/* Parse message body (called payload) */
	now = h->scts = tris_tvnow();
	for (f = 4; f < sz; ) {
		msg = h->imsg[f++];
		msgsz = h->imsg[f++];
		msgsz += (h->imsg[f++] * 256);
		...
		case 0x14:                          /* Date SCTS */
			...
			tm.tm_sec = 0;
			h->scts = tris_mktime(&tm, NULL);
			tris_verb(3, "SMS-P2 Date#%02X=%02d/%02d %02d:%02d\n", msg, tm.tm_mday, tm.tm_mon + 1, tm.tm_hour, tm.tm_min);
			break;
		case 0x15:                          /* Calling line (from SMSC) */
			if (msgsz >= 20) {
				msgsz = 20 - 1;
			}
			tris_verb(3, "SMS-P2 Origin#%02X=[%.*s]\n", msg, msgsz, &h->imsg[f]);
			tris_copy_string(h->oa, (char *)(&h->imsg[f]), msgsz + 1);
			break;
		case 0x18:                          /* Destination(from TE/phone) */
			if (msgsz >= 20) {
				msgsz = 20 - 1;
			}
			tris_verb(3, "SMS-P2 Destination#%02X=[%.*s]\n", msg, msgsz, &h->imsg[f]);
			tris_copy_string(h->da, (char *)(&h->imsg[f]), msgsz + 1);
			break;
		case 0x1C:                          /* Notify */
			tris_verb(3, "SMS-P2 Notify#%02X=%s\n", msg, sms_hexdump(&h->imsg[f], 3, debug_buf));
			break;
		default:
			tris_verb(3, "SMS-P2 Par#%02X [%d]: %s\n", msg, msgsz, sms_hexdump(&h->imsg[f], msgsz, debug_buf));
			break;
		}
...
#define DLL2_ACK(h) ((h->framenumber & 1) ? DLL2_SMS_ACK1: DLL2_SMS_ACK1)
	...
	case DLL2_SMS_ACK1:
		...
		if ( (h->omsg[0] & DLL_SMS_MASK) == DLL2_SMS_REL) {
			...
			tris_log(LOG_NOTICE, "SMS_SUBMIT or SMS_DELIVERY");
			sms_nextoutgoing (h);
		}
		break;
...
{	  
	...
	*h->da = *h->oa = '\0';                 /* clear destinations */
	...
	snprintf(fn, sizeof(fn), "%s/sms/%s", tris_config_TRIS_SPOOL_DIR, h->smsc ? "mttx" : "motx");
	tris_mkdir(fn, 0777);                    /* ensure it exists */
	d = opendir(fn);
	if (d) {
 ...
 */
static int sms_generate(struct tris_channel *chan, void *data, int len, int samples)
{
...
#define MAXSAMPLES (800)
	...
	len = samples * sizeof(*buf) + TRIS_FRIENDLY_OFFSET;
	buf = alloca(len);
 ...
 */
static void *sms_alloc(struct tris_channel *chan, void *sms_t_ptr)
{
	return sms_t_ptr;
}

static void sms_release(struct tris_channel *chan, void *data)
{
	return;	/* nothing to do here. */
}

static struct tris_generator smsgen = {
	.alloc = sms_alloc,
	.release = sms_release,
	...
	for ( ; samples-- ; data++) {
		...
		if (h->imag <= 500) {               /* below [arbitrary] threahold: lost carrier */
			if (h->idle++ == 80000) {       /* nothing happening */
				tris_log(LOG_NOTICE, "No data, hanging up\n");
				h->hangup = 1;
				h->err = 1;
			}
			if (h->ierr) {                  /* error */
				tris_log(LOG_NOTICE, "Error %d, hanging up\n", h->ierr);
				/* Protocol 1 */
				h->err = 1;
				h->omsg[0] = 0x92;          /* error */
				h->omsg[1] = 1;
		...
		if (bit && h->ibitc == 200) {       /* sync, restart message */
			...
			if (h->framenumber < 0 && h->ibytec >= 160 && !memcmp(h->imsg, "UUUUUUUUUUUUUUUUUUUU", 20)) {
				h->framenumber = 1;
				tris_verb(3, "SMS protocol 2 detected\n");
				h->protocol = 2;
		...
		if (h->ibitn) {
			...
			if (h->iphasep >= 80) {         /* next bit */
				...
				if (h->ibitn++ == 9) {      /* end of byte */
					if (!bit) {             /* bad stop bit */
						tris_log(LOG_NOTICE, "bad stop bit");
						...
						if (h->ibytep < sizeof(h->imsg)) {
							...
							tris_log(LOG_NOTICE, "msg too large");
						...
						if (h->ibytep > 1 && h->ibytep == 3 + h->imsg[1] && !h->ierr) {
							if (!h->ibytec) {
								...
								tris_log(LOG_NOTICE, "bad checksum");
 ...
 *	- TRIS_APP_OPTIONS() to drive the parsing routine
 *	- in the function, TRIS_DECLARE_APP_ARGS(...) for the arguments.
 */
enum {
...
TRIS_APP_OPTIONS(sms_options, {
	TRIS_APP_OPTION('s', OPTION_BE_SMSC),
	TRIS_APP_OPTION('a', OPTION_ANSWER),
	TRIS_APP_OPTION('t', OPTION_TWO),
	TRIS_APP_OPTION('r', OPTION_SRR),
	TRIS_APP_OPTION('o', OPTION_DCS),
	TRIS_APP_OPTION_ARG('p', OPTION_PAUSE, OPTION_ARG_PAUSE),
	} );

static int sms_exec(struct tris_channel *chan, void *data)
{
	int res = -1;
	sms_t h = { 0 };
	...
	char *p;
	TRIS_DECLARE_APP_ARGS(sms_args,
		TRIS_APP_ARG(queue);
	...
	if (!data) {
		tris_log(LOG_ERROR, "Requires queue name at least\n");
		return -1;
	}

	parse = tris_strdupa(data);              /* create a local copy */
	TRIS_STANDARD_APP_ARGS(sms_args, parse);
	if (sms_args.argc > 1) {
		tris_app_parse_options(sms_options, &flags, sms_opts, sms_args.options);
	}

	tris_verb(1, "sms argc %d queue <%s> opts <%s> addr <%s> body <%s>\n",
	...
	if (chan->cid.cid_num)
		tris_copy_string(h.cli, chan->cid.cid_num, sizeof(h.cli));

	if (tris_strlen_zero(sms_args.queue)) {
		tris_log(LOG_ERROR, "Requires queue name\n");
		goto done;
	}
	if (strlen(sms_args.queue) >= sizeof(h.queue)) {
		tris_log(LOG_ERROR, "Queue name too long\n");
		goto done;
	}
	tris_copy_string(h.queue, sms_args.queue, sizeof(h.queue));
	...
	h.smsc = tris_test_flag(&flags, OPTION_BE_SMSC);
	h.protocol = tris_test_flag(&flags, OPTION_TWO) ? 2 : 1;
	if (!tris_strlen_zero(sms_opts[OPTION_ARG_PAUSE])) {
		h.opause_0 = atoi(sms_opts[OPTION_ARG_PAUSE]);
	}
	if (h.opause_0 < 25 || h.opause_0 > 2000) {
		h.opause_0 = 300;                   /* default 300ms */
	}
	tris_verb(1, "initial delay %dms\n", h.opause_0);


	/* the following apply if there is an arg3/4 and apply to the created message file */
	if (tris_test_flag(&flags, OPTION_SRR)) {
		h.srr = 1;
	}
	if (tris_test_flag(&flags, OPTION_DCS)) {
		h.dcs = 1;
	}
#if 0	
		case '1':
	...
	if (sms_args.argc > 2) {
		...
		h.scts = tris_tvnow();
		if (tris_strlen_zero(sms_args.addr) || strlen(sms_args.addr) >= sizeof(h.oa)) {
			tris_log(LOG_ERROR, "Address too long %s\n", sms_args.addr);
			goto done;
		}
		if (h.smsc) {
			tris_copy_string(h.oa, sms_args.addr, sizeof(h.oa));
		} else {
			tris_copy_string(h.da, sms_args.addr, sizeof(h.da));
			tris_copy_string(h.oa, h.cli, sizeof(h.oa));
		}
		h.udl = 0;
		if (tris_strlen_zero(sms_args.body)) {
			tris_log(LOG_ERROR, "Missing body for %s\n", sms_args.addr);
			goto done;
		}
		...
		if (is7bit(h.dcs) && packsms7(0, h.udhl, h.udh, h.udl, h.ud) < 0) {
			tris_log(LOG_WARNING, "Invalid 7 bit GSM data\n");
			goto done;
		}
		if (is8bit(h.dcs) && packsms8(0, h.udhl, h.udh, h.udl, h.ud) < 0) {
			tris_log(LOG_WARNING, "Invalid 8 bit data\n");
			goto done;
		}
		if (is16bit(h.dcs) && packsms16(0, h.udhl, h.udh, h.udl, h.ud) < 0) {
			tris_log(LOG_WARNING, "Invalid 16 bit data\n");
			goto done;
		}
		h.rx = 0;                           /* sent message */
		h.mr = -1;
	...
	if (chan->_state != TRIS_STATE_UP) {		/* make sure channel is answered before any TX */
		tris_answer(chan);
	}

	if (tris_test_flag(&flags, OPTION_ANSWER)) {
	...
	res = tris_set_write_format(chan, __OUT_FMT);
	if (res >= 0) {
		res = tris_set_read_format(chan, TRIS_FORMAT_SLINEAR);
	}
	if (res < 0) {
		tris_log(LOG_ERROR, "Unable to set to linear mode, giving up\n");
		goto done;
	}

	if ( (res = tris_activate_generator(chan, &smsgen, &h)) < 0) {
		tris_log(LOG_ERROR, "Failed to activate generator on '%s'\n", chan->name);
		goto done;
	}
	...
	for (;;) {
		struct tris_frame *f;
		int i = tris_waitfor(chan, -1);
		if (i < 0) {
			tris_log(LOG_NOTICE, "waitfor failed\n");
			break;
		}
		if (h.hangup) {
			tris_log(LOG_NOTICE, "channel hangup\n");
			break;
		}
		f = tris_read(chan);
		if (!f) {
			tris_log(LOG_NOTICE, "tris_read failed\n");
			break;
		}
...
static int load_module(void)
{
	...
	int p;
	for (p = 0; p < 80; p++) {
		wavea[p] = TRIS_LIN2A(wave[p]);
	}
#endif
	snprintf(log_file, sizeof(log_file), "%s/sms", tris_config_TRIS_LOG_DIR);
	return tris_register_application_xml(app, sms_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "SMS/PSTN handler");
