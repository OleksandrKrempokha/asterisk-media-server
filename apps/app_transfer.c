...
static int transfer_exec(struct tris_channel *chan, void *data)
{
	int res;
	int len;
	char *slash;
	...
	if (tris_strlen_zero((char *)data)) {
		tris_log(LOG_WARNING, "Transfer requires an argument ([Tech/]destination)\n");
...
{
	return tris_register_application_xml(app, transfer_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Transfers a caller to another extension");
