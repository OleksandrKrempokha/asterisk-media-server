...
TRIS_APP_OPTIONS(app_opts,{
	TRIS_APP_OPTION('w', OPTION_WAIT),
});

static int sendurl_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	char *tmp;
	struct tris_frame *f;
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "SendURL requires an argument (URL)\n");
	...
	if (tris_test_flag(&flags, OPTION_WAIT)) {
		for(;;) {
			...
			if (f->frametype == TRIS_FRAME_HTML) {
				switch(f->subclass) {
				...
				case TRIS_HTML_NOSUPPORT:
					...
					break;
				default:
					tris_log(LOG_WARNING, "Don't know what to do with HTML subclass %d\n", f->subclass);
				};
			}
			tris_frfree(f);
		}
...
{
	return tris_register_application_xml(app, sendurl_exec);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Send URL Applications");
