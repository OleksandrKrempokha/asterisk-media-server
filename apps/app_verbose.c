...
static int verbose_exec(struct tris_channel *chan, void *data)
{
	int vsize;
	char *parse;
	...
	if (sscanf(args.level, "%30d", &vsize) != 1) {
		vsize = 0;
		tris_log(LOG_WARNING, "'%s' is not a verboser number\n", args.level);
	}
	if (option_verbose >= vsize) {
		...
		case 0:
			...
			break;
		case 1:
			tris_verbose(VERBOSE_PREFIX_1 "%s\n", args.msg);
			break;
		case 2:
			tris_verbose(VERBOSE_PREFIX_2 "%s\n", args.msg);
			break;
		case 3:
			tris_verbose(VERBOSE_PREFIX_3 "%s\n", args.msg);
			break;
		default:
			tris_verbose(VERBOSE_PREFIX_4 "%s\n", args.msg);
		}
	}
...
static int log_exec(struct tris_channel *chan, void *data)
{
	char *parse;
	int lnum = -1;
	char extension[TRIS_MAX_EXTENSION + 5], context[TRIS_MAX_EXTENSION + 2];
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(level);
		TRIS_APP_ARG(msg);
	);
	...
	if (!strcasecmp(args.level, "ERROR")) {
		...
		tris_log(LOG_ERROR, "Unknown log level: '%s'\n", args.level);
	}
	...
	if (lnum > -1) {
		...
		tris_log(lnum, extension, chan->priority, context, "%s\n", args.msg);
	}
...
static int unload_module(void)
{
	...
	res = tris_unregister_application(app_verbose);
	res |= tris_unregister_application(app_log);
...
static int load_module(void)
{
	...
	res = tris_register_application_xml(app_log, log_exec);
	res |= tris_register_application_xml(app_verbose, verbose_exec);

	return res;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Send verbose output");
