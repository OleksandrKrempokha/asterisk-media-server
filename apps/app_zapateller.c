...
static int zapateller_exec(struct tris_channel *chan, void *data)
{
	int res = 0;
	int i, answer = 0, nocallerid = 0;
	char *parse = tris_strdupa((char *)data);
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(options)[2];
	);
	...
	if (!tris_strlen_zero(chan->cid.cid_num) && nocallerid)
		return res;

	if (!res) 
		res = tris_tonepair(chan, 950, 0, 330, 0);
	if (!res) 
		res = tris_tonepair(chan, 1400, 0, 330, 0);
	if (!res) 
		res = tris_tonepair(chan, 1800, 0, 330, 0);
	if (!res) 
		res = tris_tonepair(chan, 0, 0, 1000, 0);
...
{
	return ((tris_register_application_xml(app, zapateller_exec)) ? TRIS_MODULE_LOAD_FAILURE : TRIS_MODULE_LOAD_SUCCESS);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Block Telemarketers with Special Information Tone");
