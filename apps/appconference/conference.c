...
//			tris_log ( LOG_WARNING, "base = %d,%d: conf->delivery_time = %d,%d\n",base.tv_sec,base.tv_usec, conf->delivery_time.tv_sec, conf->delivery_time.tv_usec);
		     ...
		     video_source_member = video_source_member->next)
		{
				...
				for (member = conf->memberlist; member != NULL; member = member->next)
				{
					...
					} else
					{
						...
						} else if(member->vad_switch)
						{
							...
							if(video_source_member->receiver_channel_name != NULL && 
								!strcmp(member->channel_name, video_source_member->receiver_channel_name)) {
								queue_outgoing_video_frame(member, cfr->fr, conf->delivery_time);
							}
						}
		 ...
		 struct tris_conf_member * default_member = get_default_member(conf);
		 // loop over the incoming frames and send to all outgoing
		 if (default_member) {
			 while ((cfr = get_incoming_desktop_frame( default_member )))
			 {
				 for (member = conf->memberlist; member != NULL; member = member->next)
				 {
					 // skip members that are not ready or are not supposed to receive video
					 if ( !member->ready_for_outgoing || member->norecv_desktop)
						 continue ;
		 
					 queue_outgoing_desktop_frame(member, cfr->fr, conf->delivery_time);
				 }
				 // Garbage collection
				 delete_conf_frame(cfr);
			 }
  ...
  int c;
	...
	while ( conf_current != NULL )
	{
		...
		if ( conf_current == conf )
		{
			...
			manager_event(EVENT_FLAG_CALL, "ConferenceEnd", "Confno: %s\r\n", conf_current->name);
			free( conf_current ) ;
			conf_current = NULL ;
...
void add_member( struct tris_conf_member *member, struct tris_conference *conf )
{
	...
	// we send notify request when new member enter.
	if (!member->isbargein3conf)
		notify_members(conf, member, 1);

	// release the conference lock
	tris_mutex_unlock( &conf->lock ) ;

	// we determine to send re-invite if member has not drtp
	determin_reinvite(conf, member);
...
int remove_member( struct tris_conf_member* member, struct tris_conference* conf )
{
	...
	while ( member_list != NULL )
	{
		...
		else
		{
			...
			if ( member_is_moderator || ((found && conf->membercount <= 1) || (!found && conf->membercount <= 2)))
			{
				tris_mutex_lock( &member_list->lock ) ;
...
int show_conference_stats ( int fd )
{
	...
	if ( conflist == NULL )
	{
		tris_log( TRIS_CONF_DEBUG, "conflist has not yet been initialized.\n") ;
		return 0 ;
	}
	...
	tris_cli( fd, "%-20.20s  %-40.40s\n", "Name", "Members") ;

	// loop through conf list
	while ( conf != NULL )
	{
		tris_cli( fd, "%-20.20s %3d\n", conf->name, conf->membercount ) ;
		conf = conf->next ;
	}
...
int show_conference_list ( int fd, const char *name )
{
	...
	while ( conf != NULL )
	{
		if ( strncasecmp( (const char*)&(conf->name), name, 80 ) == 0 )
		{
			...
			while ( member != NULL )
			{
				tris_cli( fd, "User#: %d ", member->id ) ;
				tris_cli( fd, "CallerID: %s ", member->callerid ) ;
				tris_cli( fd, "Channel: %s ", member->channel_name ) ;

				tris_cli( fd, "Flags:");
				if ( member->mute_video ) tris_cli( fd, "C");
				if ( member->norecv_video ) tris_cli( fd, "c");
				if ( member->mute_audio ) tris_cli( fd, "L");
				if ( member->norecv_audio ) tris_cli( fd, "l");
				if ( member->vad_flag ) tris_cli( fd, "V");
				if ( member->denoise_flag ) tris_cli( fd, "D");
				if ( member->agc_flag ) tris_cli( fd, "A");
				if ( member->dtmf_switch ) tris_cli( fd, "X");
				if ( member->dtmf_relay ) tris_cli( fd, "R");
				if ( member->vad_switch ) tris_cli( fd, "S");
				if ( member->ismoderator ) tris_cli( fd, "M");
				if ( member->is_admin ) tris_cli(fd, "a");
				if ( member->no_camera ) tris_cli( fd, "N");
				if ( member->does_text ) tris_cli( fd, "t");
				if ( member->via_telephone ) tris_cli( fd, "T");
				tris_cli( fd, " " );
				...
				{
					tris_cli(fd, "Driving:%s(%d) ", member->driven_member->channel_name, member->driven_member->id);
				}
...
int kick_channel ( const char *confname, const char *channel)
{
	...
	while ( conf != NULL )
	{
		if ( strncasecmp( (const char*)&(conf->name), confname, 80 ) == 0 )
		{
			...
			while ( member != NULL )
			{
				if ( strncasecmp( member->channel_name, channel, 80 ) == 0 || strncasecmp( member->callerid, channel, 80 ) == 0 )
				{
					tris_mutex_lock( &member->lock ) ;
					member->kick_flag = 1;
					//tris_soft_hangup(member->chan);
			    ...
			    if (member->id == user_id)
			      {
				      ...
				      member->mute_audio = 1;
				      tris_mutex_unlock( &member->lock ) ;
					manager_event(EVENT_FLAG_CALL, "ConferenceMute", "ConferenceName: %s\r\nUserID: %d\r\n", conf->name, member->id);
				      res = 1;
			      }
			    member = member->next;
			  }
			    ...
			    if (member->id == user_id)
			      {
				      ...
				      member->mute_audio = 0;
				      tris_mutex_unlock( &member->lock ) ;
					manager_event(EVENT_FLAG_CALL, "ConferenceUnmute", "ConferenceName: %s\r\nUserID: %d\r\n", conf->name, member->id);
				      res = 1;
			      }
			    member = member->next;
			  }
...
int get_conference_stats( tris_conference_stats* stats, int requested )
{
	// no conferences exist
	if ( conflist == NULL )
	{
		tris_log( TRIS_CONF_DEBUG, "conflist has not yet been initialize\n" ) ;
		return 0 ;
	}
...
int get_conference_stats_by_name( tris_conference_stats* stats, const char* name )
{
	// no conferences exist
	if ( conflist == NULL )
	{
	...
	return ( stats == NULL ) ? 0 : conf->membercount;
}

struct tris_conf_member *find_member2 (const char *confname, const char *chan, int lock)
{
	...
	while ( conf != NULL && !found )
	{
		if ( strncasecmp( (const char*)&(conf->name), confname, 80 ) == 0 )
		{
			...
			while (member != NULL)
			{
			    if(!strcmp(member->channel_name, chan) || !strcmp(member->callerid, chan)) {
				found = member;
				if(lock)
				    tris_mutex_lock(&member->lock);
				break;
...
struct tris_conf_member * get_default_member(struct tris_conference * conf)
{
	struct tris_conf_member *member = NULL;
...
int drive(const char *conference, int src_member_id, int dst_member_id)
{
	...
	struct tris_conference *conf;
	struct tris_conf_member *member;
	struct tris_conf_member *src;
	struct tris_conf_member *dst;
...
{
	struct tris_conf_member *member;
	struct tris_conf_soundq *newsound;
	struct tris_conf_soundq **q;

	member = find_member(channel, 1);
	if( !member )
	{
		tris_cli(fd, "Member %s not found\n", channel);
		return 0;
	}

	newsound = calloc(1, sizeof(struct tris_conf_soundq));
	newsound->stream = tris_openstream(member->chan, file, NULL);
	if( !newsound->stream )
	{
		tris_cli(fd, "Sound %s not found\n", file);
		free(newsound);
		tris_mutex_unlock(&member->lock);
		return 0;
	}
	...
	newsound->muted = mute;
	tris_copy_string(newsound->name, file, sizeof(newsound->name));
	...
	*q = newsound;
	...
	tris_cli(fd, "Playing sound %s to member %s %s\n",
...
int stop_sound_channel(int fd, const char *channel)
{
	...
	tris_cli( fd, "Stopped sounds to member %s\n", channel);
	return 1;
}
