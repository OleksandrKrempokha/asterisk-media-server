...
# TRIS_EXT_LIB_SETUP([package], [short description], [configure option name], [long description])
...
# TRIS_EXT_LIB_SETUP_DEPENDENT([package symbol name], [package friendly name], [master package symbol name], [master package option name])

AC_DEFUN([TRIS_EXT_LIB_SETUP_DEPENDENT],
[
$1_DESCRIP="$2"
...
# TRIS_EXT_LIB_CHECK([package], [library], [function], [header],
...
if test "x${PBX_$1}" != "x1" -a "${USE_$1}" != "no"; then
   ...
   if test "x${pbxfuncname}" = "x" ; then   # empty lib, assume only headers
      ...
      AC_CHECK_LIB([$2], [${pbxfuncname}], [TRIS_$1_FOUND=yes], [TRIS_$1_FOUND=no], ${pbxlibdir} $5)
   fi
