...
struct multiplexed_thread {
	...
	struct tris_channel *chans[MULTIPLEXED_MAX_CHANNELS];
...
static int multiplexed_bridge_create(struct tris_bridge *bridge)
{
	...
	if (!(multiplexed_thread = ao2_callback(multiplexed_threads, 0, find_multiplexed_thread, NULL))) {
		...
		if (!(multiplexed_thread = ao2_alloc(sizeof(*multiplexed_thread), destroy_multiplexed_thread))) {
			tris_debug(1, "Failed to find or create a new multiplexed thread for bridge '%p'\n", bridge);
		...
		if (pipe(multiplexed_thread->pipe)) {
			tris_debug(1, "Failed to create a pipe for poking a multiplexed thread for bridge '%p'\n", bridge);
		...
		if (fcntl(multiplexed_thread->pipe[0], F_SETFL, flags | O_NONBLOCK) < 0) {
			tris_log(LOG_WARNING, "Failed to setup first nudge pipe for non-blocking operation on %p (%d: %s)\n", bridge, errno, strerror(errno));
			ao2_ref(multiplexed_thread, -1);
			ao2_unlock(multiplexed_threads);
			return -1;
		}
		...
		if (fcntl(multiplexed_thread->pipe[1], F_SETFL, flags | O_NONBLOCK) < 0) {
			tris_log(LOG_WARNING, "Failed to setup second nudge pipe for non-blocking operation on %p (%d: %s)\n", bridge, errno, strerror(errno));
			ao2_ref(multiplexed_thread, -1);
			ao2_unlock(multiplexed_threads);
			return -1;
		}
		...
		multiplexed_thread->thread = TRIS_PTHREADT_NULL;

		/* Finally link us into the container so others may find us */
		ao2_link(multiplexed_threads, multiplexed_thread);
		tris_debug(1, "Created multiplexed thread '%p' for bridge '%p'\n", multiplexed_thread, bridge);
	} else {
		tris_debug(1, "Found multiplexed thread '%p' for bridge '%p'\n", multiplexed_thread, bridge);
	}
...
static void multiplexed_nudge(struct multiplexed_thread *multiplexed_thread)
{
	...
	if (multiplexed_thread->thread == TRIS_PTHREADT_NULL) {
		return;
	}

	if (write(multiplexed_thread->pipe[1], &nudge, sizeof(nudge)) != sizeof(nudge)) {
		tris_log(LOG_ERROR, "We couldn't poke multiplexed thread '%p'... something is VERY wrong\n", multiplexed_thread);
	}
...
static int multiplexed_bridge_destroy(struct tris_bridge *bridge)
{
	...
	if (!multiplexed_thread->count) {
		tris_debug(1, "Unlinking multiplexed thread '%p' since nobody is using it anymore\n", multiplexed_thread);
...
static void *multiplexed_thread_function(void *data)
{
	...
	tris_debug(1, "Starting actual thread for multiplexed thread '%p'\n", multiplexed_thread);

	while (multiplexed_thread->thread != TRIS_PTHREADT_STOP) {
		struct tris_channel *winner = NULL, *first = multiplexed_thread->chans[0];
		int to = -1, outfd = -1;

		/* Move channels around so not just the first one gets priority */
		memmove(multiplexed_thread->chans, multiplexed_thread->chans + 1, sizeof(struct tris_channel *) * (multiplexed_thread->service_count - 1));
		...
		winner = tris_waitfor_nandfds(multiplexed_thread->chans, multiplexed_thread->service_count, &fds, 1, NULL, &outfd, &to);
		...
		if (outfd > -1) {
			...
			if (read(multiplexed_thread->pipe[0], &nudge, sizeof(nudge)) < 0) {
				if (errno != EINTR && errno != EAGAIN) {
					tris_log(LOG_WARNING, "read() failed for pipe on multiplexed thread '%p': %s\n", multiplexed_thread, strerror(errno));
				}
			}
		}
		if (winner && winner->bridge) {
			tris_bridge_handle_trip(winner->bridge, NULL, winner, -1);
		}
	}

	multiplexed_thread->thread = TRIS_PTHREADT_NULL;

	tris_debug(1, "Stopping actual thread for multiplexed thread '%p'\n", multiplexed_thread);
...
static void multiplexed_add_or_remove(struct multiplexed_thread *multiplexed_thread, struct tris_channel *chan, int add)
{
	int i, removed = 0;
	pthread_t thread = TRIS_PTHREADT_NULL;
	...
	if (multiplexed_thread->service_count && multiplexed_thread->thread == TRIS_PTHREADT_NULL) {
		ao2_ref(multiplexed_thread, +1);
		if (tris_pthread_create(&multiplexed_thread->thread, NULL, multiplexed_thread_function, multiplexed_thread)) {
			ao2_ref(multiplexed_thread, -1);
			tris_debug(1, "Failed to create an actual thread for multiplexed thread '%p', trying next time\n", multiplexed_thread);
		}
	} else if (!multiplexed_thread->service_count && multiplexed_thread->thread != TRIS_PTHREADT_NULL) {
		thread = multiplexed_thread->thread;
		multiplexed_thread->thread = TRIS_PTHREADT_STOP;
	} else if (!add && removed) {
		memmove(multiplexed_thread->chans + i, multiplexed_thread->chans + i + 1, sizeof(struct tris_channel *) * (MULTIPLEXED_MAX_CHANNELS - (i + 1)));
	}

	ao2_unlock(multiplexed_thread);

	if (thread != TRIS_PTHREADT_NULL) {
...
static int multiplexed_bridge_join(struct tris_bridge *bridge, struct tris_bridge_channel *bridge_channel)
{
	struct tris_channel *c0 = TRIS_LIST_FIRST(&bridge->channels)->chan, *c1 = TRIS_LIST_LAST(&bridge->channels)->chan;
	struct multiplexed_thread *multiplexed_thread = bridge->bridge_pvt;

	tris_debug(1, "Adding channel '%s' to multiplexed thread '%p' for monitoring\n", bridge_channel->chan->name, multiplexed_thread);
...
static int multiplexed_bridge_leave(struct tris_bridge *bridge, struct tris_bridge_channel *bridge_channel)
{
	struct multiplexed_thread *multiplexed_thread = bridge->bridge_pvt;

	tris_debug(1, "Removing channel '%s' from multiplexed thread '%p'\n", bridge_channel->chan->name, multiplexed_thread);
...
static void multiplexed_bridge_suspend(struct tris_bridge *bridge, struct tris_bridge_channel *bridge_channel)
{
	struct multiplexed_thread *multiplexed_thread = bridge->bridge_pvt;

	tris_debug(1, "Suspending channel '%s' from multiplexed thread '%p'\n", bridge_channel->chan->name, multiplexed_thread);
...
static void multiplexed_bridge_unsuspend(struct tris_bridge *bridge, struct tris_bridge_channel *bridge_channel)
{
	struct multiplexed_thread *multiplexed_thread = bridge->bridge_pvt;

	tris_debug(1, "Unsuspending channel '%s' from multiplexed thread '%p'\n", bridge_channel->chan->name, multiplexed_thread);
...
static enum tris_bridge_write_result multiplexed_bridge_write(struct tris_bridge *bridge, struct tris_bridge_channel *bridge_channel, struct tris_frame *frame)
{
	struct tris_bridge_channel *other;
...
static struct tris_bridge_technology multiplexed_bridge = {
	.name = "multiplexed_bridge",
	.capabilities = TRIS_BRIDGE_CAPABILITY_1TO1MIX,
	.preference = TRIS_BRIDGE_PREFERENCE_HIGH,
...
{
	int res = tris_bridge_technology_unregister(&multiplexed_bridge);
...
static int load_module(void)
{
	...
	return tris_bridge_technology_register(&multiplexed_bridge);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Multiplexed two channel bridging module");
