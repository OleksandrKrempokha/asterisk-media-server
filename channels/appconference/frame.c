...
conf_frame* mix_single_speaker( conf_frame* frames_in )
{
	...
	if (frames_in->fr->promoter) {
		frames_in->fr = convert_frame_to_slinear(
			frames_in->member->to_slinear_P,
			frames_in->fr
		) ;
	} else {
		frames_in->fr = convert_frame_to_slinear(
			frames_in->member->to_slinear_S,
			frames_in->fr
		) ;
	}
...
)
{
	...
	while ( cf_spoken != NULL )
	{
		...
		else
		{
			...
			if (cf_spoken->fr->promoter) {
				cf_spoken->fr = convert_frame_to_slinear(
					cf_spoken->member->to_slinear_P,
					cf_spoken->fr
				) ;
			} else {
				cf_spoken->fr = convert_frame_to_slinear(
					cf_spoken->member->to_slinear_S,
					cf_spoken->fr
				) ;
			}
