 ...
 *
 * 320 samples (20 ms) is the most common frame size in Trismedia.  So, the code
 * in this module reads 320 sample frames from the portaudio stream and queues
 * them up on the Trismedia channel.  Frames of any size can be written to a
...
static struct console_pvt {
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(input_device);
		TRIS_STRING_FIELD(output_device);
...
static struct tris_channel *console_request(const char *type, int format, 
	void *data, int *cause, struct tris_channel* src);
static int console_digit_begin(struct tris_channel *c, char digit);
static int console_digit_end(struct tris_channel *c, char digit, unsigned int duration);
static int console_text(struct tris_channel *c, const char *text);
static int console_hangup(struct tris_channel *c);
static int console_answer(struct tris_channel *c);
static struct tris_frame *console_read(struct tris_channel *chan);
static int console_call(struct tris_channel *c, char *dest, int timeout);
static int console_write(struct tris_channel *chan, struct tris_frame *f);
static int console_indicate(struct tris_channel *chan, int cond, 
	const void *data, size_t datalen);
static int console_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
/*! @} */

/*!
 * \brief Formats natively supported by this module.
 */
#define SUPPORTED_FORMATS ( TRIS_FORMAT_SLINEAR16 )

static const struct tris_channel_tech console_tech = {
	.type = "Console",
 ...
 * be read from the Trismedia channel.
 */
static void *stream_monitor(void *data)
{
	...
	struct tris_frame f = {
		...
		.subclass = TRIS_FORMAT_SLINEAR16,
...
static int open_stream(struct console_pvt *pvt)
{
	...
	if (!strcasecmp(pvt->input_device, "default") && 
		...
		if (input_params.device == paNoDevice)
			tris_log(LOG_ERROR, "No input device found for console device '%s'\n", pvt->name);
		if (output_params.device == paNoDevice)
			tris_log(LOG_ERROR, "No output device found for console device '%s'\n", pvt->name);
...
static int start_stream(struct console_pvt *pvt)
{
	...
	pvt->streamstate = 1;
	tris_debug(1, "Starting stream\n");

	res = open_stream(pvt);
	if (res != paNoError) {
		tris_log(LOG_WARNING, "Failed to open stream - (%d) %s\n",
	...
	if (res != paNoError) {
		tris_log(LOG_WARNING, "Failed to start stream - (%d) %s\n",
			res, Pa_GetErrorText(res));
		ret_val = -1;
		goto return_unlock;
	}

	if (tris_pthread_create_background(&pvt->thread, NULL, stream_monitor, pvt)) {
		tris_log(LOG_ERROR, "Failed to start stream monitor thread\n");
		ret_val = -1;
	}
...
{
	if (!pvt->streamstate || pvt->thread == TRIS_PTHREADT_NULL)
		return 0;
 ...
 */
static struct tris_channel *console_new(struct console_pvt *pvt, const char *ext, const char *ctx, int state)
{
	struct tris_channel *chan;

	if (!(chan = tris_channel_alloc(1, state, pvt->cid_num, pvt->cid_name, NULL, 
		ext, ctx, 0, "Console/%s", pvt->name))) {
		return NULL;
	}

	chan->tech = &console_tech;
	chan->nativeformats = TRIS_FORMAT_SLINEAR16;
	chan->readformat = TRIS_FORMAT_SLINEAR16;
	chan->writeformat = TRIS_FORMAT_SLINEAR16;
	chan->tech_pvt = ref_pvt(pvt);

	pvt->owner = chan;

	if (!tris_strlen_zero(pvt->language))
		tris_string_field_set(chan, language, pvt->language);

	tris_jb_configure(chan, &global_jbconf);
...
static struct tris_channel *console_request(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	...
	if (!(pvt = find_pvt(data))) {
		tris_log(LOG_ERROR, "Console device '%s' not found\n", (char *) data);
		return NULL;
	}

	format &= SUPPORTED_FORMATS;
	if (!format) {
		tris_log(LOG_NOTICE, "Channel requested with unsupported format(s): '%d'\n", oldformat);
		goto return_unref;
	}

	if (pvt->owner) {
		tris_log(LOG_NOTICE, "Console channel already active!\n");
		*cause = TRIS_CAUSE_BUSY;
		goto return_unref;
	}

	console_pvt_lock(pvt);
	chan = console_new(pvt, NULL, NULL, TRIS_STATE_DOWN);
	console_pvt_unlock(pvt);

	if (!chan)
		tris_log(LOG_WARNING, "Unable to create new Console channel!\n");
...
static int console_digit_begin(struct tris_channel *c, char digit)
{
	tris_verb(1, V_BEGIN "Console Received Beginning of Digit %c" V_END, digit);
...
static int console_digit_end(struct tris_channel *c, char digit, unsigned int duration)
{
	tris_verb(1, V_BEGIN "Console Received End of Digit %c (duration %u)" V_END, 
		digit, duration);
...
static int console_text(struct tris_channel *c, const char *text)
{
	tris_verb(1, V_BEGIN "Console Received Text '%s'" V_END, text);
...
static int console_hangup(struct tris_channel *c)
{
	...
	tris_verb(1, V_BEGIN "Hangup on Console" V_END);
...
static int console_answer(struct tris_channel *c)
{
	...
	tris_verb(1, V_BEGIN "Call from Console has been Answered" V_END);
...
/*
 * \brief Implementation of the tris_channel_tech read() callback
 *
 ...
 * wrapped in an tris_frame and queued onto the channel using tris_queue_frame().
 *
 ...
 * in the fds array on the tris_channel which the core will poll() on.  When the
...
static struct tris_frame *console_read(struct tris_channel *chan)
{
	tris_debug(1, "I should not be called ...\n");
...
static int console_call(struct tris_channel *c, char *dest, int timeout)
{
	...
	tris_verb(1, V_BEGIN "Call to device '%s' on console from '%s' <%s>" V_END,
	...
	if (pvt->autoanswer) {
		pvt->hookstate = 1;
		console_pvt_unlock(pvt);
		tris_verb(1, V_BEGIN "Auto-answered" V_END);
		f.frametype = TRIS_FRAME_CONTROL;
		f.subclass = TRIS_CONTROL_ANSWER;
	} else {
		console_pvt_unlock(pvt);
		tris_verb(1, V_BEGIN "Type 'console answer' to answer, or use the 'autoanswer' option "
				"for future calls" V_END);
		f.frametype = TRIS_FRAME_CONTROL;
		f.subclass = TRIS_CONTROL_RINGING;
		tris_indicate(c, TRIS_CONTROL_RINGING);
	}
...
static int console_indicate(struct tris_channel *chan, int cond, const void *data, size_t datalen)
{
	struct console_pvt *pvt = chan->tech_pvt;
	int res = 0;
	...
	case TRIS_CONTROL_HOLD:
		tris_verb(1, V_BEGIN "Console Has Been Placed on Hold" V_END);
		tris_moh_start(chan, data, pvt->mohinterpret);
		break;
	case TRIS_CONTROL_UNHOLD:
		tris_verb(1, V_BEGIN "Console Has Been Retrieved from Hold" V_END);
		tris_moh_stop(chan);
		break;
	default:
 ...
 */
static char *tris_ext_ctx(struct console_pvt *pvt, const char *src, char **ext, char **ctx)
{
...
static struct console_pvt *get_active_pvt(void)
{
	...
	tris_rwlock_rdlock(&active_lock);
	pvt = ref_pvt(active_pvt);	
	tris_rwlock_unlock(&active_lock);

	return pvt;
}

static char *cli_console_autoanswer(struct tris_cli_entry *e, int cmd, 
	struct tris_cli_args *a)
{
	...
	if (!pvt) {
		tris_cli(a->fd, "No console device is set as active.\n");
		return CLI_FAILURE;
	}
...
static char *cli_console_flash(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (!pvt->owner) {
		tris_cli(a->fd, "No call to flash\n");
		unref_pvt(pvt);
		return CLI_FAILURE;
	}
...
static char *cli_console_dial(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *s = NULL;
	...
	if (!pvt) {
		tris_cli(a->fd, "No console device is currently set as active\n");
		return CLI_FAILURE;
	}
	...
	if (pvt->owner) {	/* already in a call */
		int i;
		struct tris_frame f = { TRIS_FRAME_DTMF, 0 };

		if (a->argc == e->args) {	/* argument is mandatory here */
			tris_cli(a->fd, "Already in a call. You can only dial digits until you hangup.\n");
			unref_pvt(pvt);
			return CLI_FAILURE;
		}
		s = a->argv[e->args];
	...
	if (a->argc == e->args + 1) {
		...
		s = tris_ext_ctx(pvt, a->argv[e->args], &ext, &con);
		tris_debug(1, "provided '%s', exten '%s' context '%s'\n", 
	...
	if (tris_strlen_zero(mye))
		mye = pvt->exten;
	if (tris_strlen_zero(myc))
		myc = pvt->context;

	if (tris_exists_extension(NULL, myc, mye, 1, NULL)) {
		console_pvt_lock(pvt);
		pvt->hookstate = 1;
		console_new(pvt, mye, myc, TRIS_STATE_RINGING);
		console_pvt_unlock(pvt);
	} else
...
static char *cli_console_hangup(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct console_pvt *pvt = get_active_pvt();
	...
	if (!pvt->owner && !pvt->hookstate) {
		tris_cli(a->fd, "No call to hang up\n");
		unref_pvt(pvt);
		return CLI_FAILURE;
	}
...
static char *cli_console_mute(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *s;
	struct console_pvt *pvt = get_active_pvt();
	char *res = CLI_SUCCESS;
	...
	tris_verb(1, V_BEGIN "The Console is now %s" V_END, 
...
static char *cli_list_available(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		tris_cli(a->fd, "=== ---------------------------------------------------------\n"
		               "=== Device Name: %s\n", dev->name);
		if (dev->maxInputChannels)
			tris_cli(a->fd, "=== ---> %sInput Device\n", (idx == def_input) ? "Default " : "");
		if (dev->maxOutputChannels)
			tris_cli(a->fd, "=== ---> %sOutput Device\n", (idx == def_output) ? "Default " : "");
		tris_cli(a->fd, "=== ---------------------------------------------------------\n===\n");
	}
...
static char *cli_list_devices(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
 ...
 */
static char *cli_console_answer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
 ...
 */
static char *cli_console_sendtext(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char buf[TEXT_SIZE];
	struct console_pvt *pvt = get_active_pvt();
	struct tris_frame f = {
		.frametype = TRIS_FRAME_TEXT,
		.data.ptr = buf,
	...
	if (!pvt->owner) {
		tris_cli(a->fd, "Not in a call\n");
		unref_pvt(pvt);
		return CLI_FAILURE;
	}

	tris_join(buf, sizeof(buf) - 1, a->argv + e->args);
...
{
	if (pvt == &globals) {
		tris_log(LOG_ERROR, "active is only valid as a per-device setting\n");
		return;
	}
	...
	tris_rwlock_wrlock(&active_lock);
	if (active_pvt)
...
static char *cli_console_active(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct console_pvt *pvt;
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Set or show the active console device for the Trismedia CLI.\n";
		return NULL;
	case CLI_GENERATE:
		if (a->pos == e->args) {
			struct ao2_iterator i;
	...
	if (a->argc == 3) {
		...
		if (!pvt)
			tris_cli(a->fd, "No device is currently set as the active console device.\n");
		else {
			console_pvt_lock(pvt);
			tris_cli(a->fd, "The active console device is '%s'.\n", pvt->name);
			console_pvt_unlock(pvt);
			pvt = unref_pvt(pvt);
		}
	...
	if (!(pvt = find_pvt(a->argv[e->args]))) {
		tris_cli(a->fd, "Could not find a device called '%s'.\n", a->argv[e->args]);
		return CLI_FAILURE;
	}
	...
	console_pvt_lock(pvt);
	tris_cli(a->fd, "The active console device has been set to '%s'\n", pvt->name);
	console_pvt_unlock(pvt);
...
static struct tris_cli_entry cli_console[] = {
	TRIS_CLI_DEFINE(cli_console_dial,       "Dial an extension from the console"),
	TRIS_CLI_DEFINE(cli_console_hangup,     "Hangup a call on the console"),
	TRIS_CLI_DEFINE(cli_console_mute,       "Disable/Enable mic input"),
	TRIS_CLI_DEFINE(cli_console_answer,     "Answer an incoming console call"),
	TRIS_CLI_DEFINE(cli_console_sendtext,   "Send text to a connected party"),
	TRIS_CLI_DEFINE(cli_console_flash,      "Send a flash to the connected party"),
	TRIS_CLI_DEFINE(cli_console_autoanswer, "Turn autoanswer on or off"),
	TRIS_CLI_DEFINE(cli_list_available,     "List available devices"),
	TRIS_CLI_DEFINE(cli_list_devices,       "List configured devices"),
	TRIS_CLI_DEFINE(cli_console_active,     "View or Set the active console device"),
};
...
{
	if (pvt == &globals) {
		tris_string_field_set(pvt, mohinterpret, "default");
		tris_string_field_set(pvt, context, "default");
		tris_string_field_set(pvt, exten, "s");
		tris_string_field_set(pvt, language, "");
		tris_string_field_set(pvt, cid_num, "");
		tris_string_field_set(pvt, cid_name, "");
		tris_string_field_set(pvt, parkinglot, "");
		...
		tris_string_field_set(pvt, mohinterpret, globals.mohinterpret);
		tris_string_field_set(pvt, context, globals.context);
		tris_string_field_set(pvt, exten, globals.exten);
		tris_string_field_set(pvt, language, globals.language);
		tris_string_field_set(pvt, cid_num, globals.cid_num);
		tris_string_field_set(pvt, cid_name, globals.cid_name);
		tris_string_field_set(pvt, parkinglot, globals.parkinglot);
...
static void store_callerid(struct console_pvt *pvt, const char *value)
{
	...
	tris_callerid_split(value, cid_name, sizeof(cid_name), 
		cid_num, sizeof(cid_num));

	tris_string_field_set(pvt, cid_name, cid_name);
	tris_string_field_set(pvt, cid_num, cid_num);
}
...
{
	if (pvt == &globals && !tris_jb_read_conf(&global_jbconf, var, value))
		return;
	...
	tris_log(LOG_WARNING, "Unknown option '%s'\n", var);
...
{
	pvt->thread = TRIS_PTHREADT_NULL;

	if (tris_string_field_init(pvt, 32))
		return -1;

	tris_string_field_set(pvt, name, S_OR(name, ""));

	return 0;
}

static void build_device(struct tris_config *cfg, const char *name)
{
	...
	for (v = tris_variable_browse(cfg, name); v; v = v->next)
...
static int load_config(int reload)
{
	...
	if (!(cfg = tris_config_load(config_file, config_flags))) {
		tris_log(LOG_NOTICE, "Unable to open configuration file %s!\n", config_file);
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_NOTICE, "Config file %s has an invalid format\n", config_file);
		return -1;
	}
	...
	while ((context = tris_category_browse(cfg, context))) {
...
static int pvt_hash_cb(const void *obj, const int flags)
{
	...
	return tris_str_case_hash(pvt->name);
}
...
{
	tris_channel_unregister(&console_tech);
	tris_cli_unregister_multiple(cli_console, ARRAY_LEN(cli_console));
...
static int load_module(void)
{
	...
	res = Pa_Initialize();
	if (res != paNoError) {
		tris_log(LOG_WARNING, "Failed to initialize audio system - (%d) %s\n",
			res, Pa_GetErrorText(res));
		goto return_error_pa_init;
	}

	if (tris_channel_register(&console_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel type 'Console'\n");
		goto return_error_chan_reg;
	}

	if (tris_cli_register_multiple(cli_console, ARRAY_LEN(cli_console)))
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Console Channel Driver",
		.load = load_module,
