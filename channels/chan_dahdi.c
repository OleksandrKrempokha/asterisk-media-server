...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 250484 $")
...
#define TRIS_LAW(p) (((p)->law == DAHDI_LAW_ALAW) ? TRIS_FORMAT_ALAW : TRIS_FORMAT_ULAW)
...
#endif
#ifdef HAVE_OPENR2
	#if defined(HAVE_PRI) || defined(HAVE_SS7) || defined(HAVE_DPNSS)
	" & MFC/R2"
	#else
	"MFC/R2"
	#endif
...
#define SIG_DPNSS     (0x8000000 | DAHDI_SIG_CLEAR)
...
static char parkinglot[TRIS_MAX_EXTENSION] = "";		/*!< Default parking lot for this channel */
...
#ifdef HAVE_PRI
static struct tris_channel inuse;
...
#ifdef HAVE_PRI
TRIS_MUTEX_DEFINE_STATIC(pridebugfdlock);
#endif
...
static tris_cond_t ss_thread_complete;
TRIS_MUTEX_DEFINE_STATIC(ss_thread_lock);
TRIS_MUTEX_DEFINE_STATIC(restart_lock);
...
static enum tris_bridge_result dahdi_bridge(struct tris_channel *c0, struct tris_channel *c1, int flags, struct tris_frame **fo, struct tris_channel **rc, int timeoutms);

static int dahdi_sendtext(struct tris_channel *c, const char *text);
...
struct dahdi_dpnss{
	...
	int dfd;
	struct dpnss *dpnss;
	unsigned int overlap_dial:1;
	struct dahdi_pvt *pvts[MAX_CHANNELS];     /*!< Member channel pvt structs */
	int numchans;
};


static struct dahdi_dpnss dpnsslinks[NUM_SPANS];
...
struct dahdi_pri {
	pthread_t master;						/*!< Thread of master */
	tris_mutex_t lock;						/*!< Mutex */
	char idleext[TRIS_MAX_EXTENSION];				/*!< Where to idle extra calls */
	char idlecontext[TRIS_MAX_CONTEXT];				/*!< What context to use for idle */
	char idledial[TRIS_MAX_EXTENSION];				/*!< What to dial before dumping */
...
struct ringContextData {
	char contextData[TRIS_MAX_CONTEXT];
};
...
struct dahdi_subchannel {
	...
	int chan;
	short buffer[TRIS_FRIENDLY_OFFSET/2 + READ_SIZE];
	struct tris_frame f;		/*!< One frame for each channel.  How did this ever work before? */
...
static struct dahdi_pvt {
	tris_mutex_t lock;
	struct tris_channel *owner;			/*!< Our current active owner (if applicable) */
	 ...
	 * from the Trismedia channel for outgoing calls.
	...
	struct tris_smdi_interface *smdi_iface;
	 ...
	 */
	char defcontext[TRIS_MAX_CONTEXT];
	 ...
	 */
	int busy_tonelength[5];
	/*!
	 * \brief Length of "busy" tone off time.
	 * \note Set from the "busypattern" value read in from chan_dahdi.conf
	 */
	int busy_quietlength[5];
	/*!
	...
	char call_forward[TRIS_MAX_EXTENSION];
	/*!
	...
	int dpnsscall;
	int dpnsscrbit;
	struct dahdi_dpnss *dpnss;
	struct dpnss_chan *dpnsschan;
#endif
...
static struct tris_channel *dahdi_request(const char *type, int format, void *data, int *cause, struct tris_channel* src);
static int dahdi_digit_begin(struct tris_channel *ast, char digit);
static int dahdi_digit_end(struct tris_channel *ast, char digit, unsigned int duration);
static int dahdi_sendtext(struct tris_channel *c, const char *text);
static int dahdi_call(struct tris_channel *ast, char *rdest, int timeout);
static int dahdi_hangup(struct tris_channel *ast);
static int dahdi_answer(struct tris_channel *ast);
static struct tris_frame *dahdi_read(struct tris_channel *ast);
static int dahdi_write(struct tris_channel *ast, struct tris_frame *frame);
static struct tris_frame *dahdi_exception(struct tris_channel *ast);
static int dahdi_indicate(struct tris_channel *chan, int condition, const void *data, size_t datalen);
static int dahdi_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
static int dahdi_setoption(struct tris_channel *chan, int option, void *data, int datalen);
static int dahdi_func_read(struct tris_channel *chan, const char *function, char *data, char *buf, size_t len);
static struct dahdi_pvt *handle_init_event(struct dahdi_pvt *i, int event);

static const struct tris_channel_tech dahdi_tech = {
	.type = "DAHDI",
	.description = tdesc,
	.capabilities = TRIS_FORMAT_SLINEAR | TRIS_FORMAT_ULAW | TRIS_FORMAT_ALAW,
...
static inline int pri_grab(struct dahdi_pvt *pvt, struct dahdi_pri *pri)
{
	...
	/* Grab the lock first */
	do {
		res = tris_mutex_trylock(&pri->lock);
		if (res) {
			DEADLOCK_AVOIDANCE(&pvt->lock);
		}
	} while (res);
	/* Then break the poll */
	if (pri->master != TRIS_PTHREADT_NULL)
...
#endif	/* defined(HAVE_SS7) */
#if defined(HAVE_DPNSS)
static inline void dpnss_rel(struct dahdi_dpnss *dpnss)
{
	tris_mutex_unlock(&dpnss->lock);
}
#endif	/* defined(HAVE_SS7) */

#if defined(HAVE_DPNSS)
static inline int dpnss_grab(struct dahdi_pvt *pvt, struct dahdi_dpnss *dpnss)
{
	int res;
	/* Grab the lock first */
	do {
		res = tris_mutex_trylock(&dpnss->lock);
		if (res) {
			DEADLOCK_AVOIDANCE(&pvt->lock);
		}
	} while (res);
	/* Then break the poll */
	if (dpnss->master != TRIS_PTHREADT_NULL)
		pthread_kill(dpnss->master, SIGURG);
	return 0;
}
#endif	/* defined(HAVE_DPNSS) */
...
static int dahdi_get_index(struct tris_channel *ast, struct dahdi_pvt *p, int nullok)
{
	int res;
	if (p->subs[SUB_REAL].owner == ast)
		...
		res = -1;
		if (!nullok)
			tris_log(LOG_WARNING, "Unable to get index, and nullok is not asserted\n");
	}
	return res;
}
...
static void wakeup_sub(struct dahdi_pvt *p, int a, struct dahdi_pri *pri)
{
	...
	for (;;) {
		if (p->subs[a].owner) {
			if (tris_channel_trylock(p->subs[a].owner)) {
				DEADLOCK_AVOIDANCE(&p->lock);
			} else {
				tris_queue_frame(p->subs[a].owner, &tris_null_frame);
				tris_channel_unlock(p->subs[a].owner);
				break;
			}
		} else
			break;
...
#ifdef HAVE_PRI
	if (pri)
		tris_mutex_lock(&pri->lock);
#endif
}

static void dahdi_queue_frame(struct dahdi_pvt *p, struct tris_frame *f, void *data)
{
#ifdef HAVE_PRI
...
#ifdef HAVE_DPNSS
	struct dahdi_dpnss *dpnss = (struct dahdi_dpnss*)data;
#endif
...
#ifdef HAVE_SS7
		case SIG_SS7:
			tris_mutex_lock(&ss7->lock);
			break;
#endif
#ifdef HAVE_DPNSS
		case SIG_DPNSS:
			tris_mutex_lock(&dpnss->lock);
			break;
#endif
		default:
			break;
...
static struct tris_channel *dahdi_new(struct dahdi_pvt *, int, int, int, int, int);
#ifdef HAVE_OPENR2
...
static int dahdi_r2_answer(struct dahdi_pvt *p)
{
	...
	*  */
	...
	int wants_double_answer = tris_true(double_answer) ? 1 : 0;
	if (!double_answer) {
...
/* should be called with the tris_channel locked */
static openr2_calling_party_category_t dahdi_r2_get_channel_category(struct tris_channel *c)
{
	...
	if (tris_strlen_zero(catstr)) {
		tris_debug(1, "No MFC/R2 category specified for chan %s, using default %s\n",
	...
	if ((cat = openr2_proto_get_category(catstr)) == OR2_CALLING_PARTY_CATEGORY_UNKNOWN) {
		tris_log(LOG_WARNING, "Invalid category specified '%s' for chan %s, using default %s\n",
				catstr, c->name, openr2_proto_get_category_string(p->mfcr2_category));
		return p->mfcr2_category;
	}
	tris_debug(1, "Using category %s\n", catstr);
	return cat;
}
...
static void dahdi_r2_on_call_init(openr2_chan_t *r2chan)
{
	...
	if (p->mfcr2call) {
		...
		tris_log(LOG_ERROR, "Collision of calls on chan %d detected!.\n", openr2_chan_get_number(r2chan));
		return;
	}
	p->mfcr2call = 1;
	...
	tris_mutex_unlock(&p->lock);
	tris_verbose("New MFC/R2 call detected on chan %d.\n", openr2_chan_get_number(r2chan));
}
...
static void dahdi_r2_on_hardware_alarm(openr2_chan_t *r2chan, int alarm)
{
	...
	if (p->inalarm) {
		...
		tris_log(LOG_NOTICE, "Alarm cleared on channel %d\n", p->channel);
...
{
	tris_log(LOG_ERROR, "OS error on chan %d: %s\n", openr2_chan_get_number(r2chan), strerror(errorcode));
}

static void dahdi_r2_on_protocol_error(openr2_chan_t *r2chan, openr2_protocol_error_t reason)
{
	struct dahdi_pvt *p = openr2_chan_get_client_data(r2chan);
	tris_log(LOG_ERROR, "MFC/R2 protocol error on chan %d: %s\n", openr2_chan_get_number(r2chan), openr2_proto_get_error(reason));
	if (p->owner) {
		p->owner->hangupcause = TRIS_CAUSE_PROTOCOL_ERROR;
...
static void dahdi_r2_disconnect_call(struct dahdi_pvt *p, openr2_call_disconnect_cause_t cause)
{
	if (openr2_chan_disconnect_call(p->r2chan, cause)) {
		tris_log(LOG_NOTICE, "Bad! failed to disconnect call on channel %d with reason %s, hope for the best!\n",
...
static void dahdi_r2_on_call_offered(openr2_chan_t *r2chan, const char *ani, const char *dnis, openr2_calling_party_category_t category)
{
	...
	tris_verbose("MFC/R2 call offered on chan %d. ANI = %s, DNIS = %s, Category = %s\n",
	...
	if (!p->mfcr2_allow_collect_calls && category == OR2_CALLING_PARTY_CATEGORY_COLLECT_CALL) {
		tris_log(LOG_NOTICE, "Rejecting MFC/R2 collect call\n");
	...
	if (!p->use_callerid) {
		tris_log(LOG_DEBUG, "No CID allowed in configuration, CID is being cleared!\n");
		p->cid_num[0] = 0;
		p->cid_name[0] = 0;
	}
	...
	if (p->immediate || !openr2_context_get_max_dnis(openr2_chan_get_context(r2chan))) {
		tris_log(LOG_DEBUG, "Setting exten => s because of immediate or 0 DNIS configured\n");
		p->exten[0] = 's';
		p->exten[1] = 0;
	}
	tris_mutex_unlock(&p->lock);
	if (!tris_exists_extension(NULL, p->context, p->exten, 1, p->cid_num)) {
		tris_log(LOG_NOTICE, "MFC/R2 call on channel %d requested non-existent extension '%s' in context '%s'. Rejecting call.\n",
	...
	if (!p->mfcr2_accept_on_offer) {
		...
		c = dahdi_new(p, TRIS_STATE_RING, 1, SUB_REAL, DAHDI_LAW_ALAW, 0);
		if (c) {
		...
		tris_log(LOG_WARNING, "Unable to create PBX channel in DAHDI channel %d\n", p->channel);
		dahdi_r2_disconnect_call(p, OR2_CAUSE_OUT_OF_ORDER);
	} else if (p->mfcr2_charge_calls) {
		tris_log(LOG_DEBUG, "Accepting MFC/R2 call with charge on chan %d\n", p->channel);
		openr2_chan_accept_call(r2chan, OR2_CALL_WITH_CHARGE);
	} else {
		tris_log(LOG_DEBUG, "Accepting MFC/R2 call with no charge on chan %d\n", p->channel);
...
{
	struct dahdi_pvt *p = openr2_chan_get_client_data(r2chan);
	tris_verbose("MFC/R2 call end on channel %d\n", p->channel);
	tris_mutex_lock(&p->lock);
	p->mfcr2call = 0;
	tris_mutex_unlock(&p->lock);
}
...
static void dahdi_r2_on_call_accepted(openr2_chan_t *r2chan, openr2_call_mode_t mode)
{
	...
	if (OR2_DIR_BACKWARD == openr2_chan_get_direction(r2chan)) {
		tris_verbose("MFC/R2 call has been accepted on backward channel %d\n", openr2_chan_get_number(r2chan));
		...
		if (!p->mfcr2_accept_on_offer) {
			...
			if (p->mfcr2_answer_pending) {
				tris_log(LOG_DEBUG, "Answering MFC/R2 call after accepting it on chan %d\n", openr2_chan_get_number(r2chan));
				dahdi_r2_answer(p);
			}
			return;
		}
	...
	tris_verbose("MFC/R2 call has been accepted on forward channel %d\n", p->channel);
...
{
	struct dahdi_pvt *p = openr2_chan_get_client_data(r2chan);
	tris_verbose("MFC/R2 call has been answered on channel %d\n", openr2_chan_get_number(r2chan));
	p->subs[SUB_REAL].needanswer = 1;
}

static void dahdi_r2_on_call_read(openr2_chan_t *r2chan, const unsigned char *buf, int buflen)
{
	/*tris_log(LOG_DEBUG, "Read data from dahdi channel %d\n", openr2_chan_get_number(r2chan));*/
}

static int dahdi_r2_cause_to_tris_cause(openr2_call_disconnect_cause_t cause)
{
	switch (cause) {
...
{
	struct dahdi_pvt *p = openr2_chan_get_client_data(r2chan);
	tris_verbose("MFC/R2 call disconnected on channel %d\n", openr2_chan_get_number(r2chan));
	tris_mutex_lock(&p->lock);
	if (!p->owner) {
		tris_mutex_unlock(&p->lock);
	...
	if (p->owner->_state == TRIS_STATE_UP) {
		...
		/* TODO: what about doing this same thing when were TRIS_STATE_UP? */
		tris_queue_hangup_with_cause(p->owner, dahdi_r2_cause_to_tris_cause(cause));
	}
}
...
static void dahdi_r2_write_log(openr2_log_level_t level, char *logmessage)
{
	switch (level) {
	case OR2_LOG_NOTICE:
		tris_verbose("%s", logmessage);
		break;
	case OR2_LOG_WARNING:
		tris_log(LOG_WARNING, "%s", logmessage);
		break;
	case OR2_LOG_ERROR:
		tris_log(LOG_ERROR, "%s", logmessage);
		break;
	...
	case OR2_LOG_EX_DEBUG:
		tris_log(LOG_DEBUG, "%s", logmessage);
		break;
	default:
		tris_log(LOG_WARNING, "We should handle logging level %d here.\n", level);
		tris_log(LOG_DEBUG, "%s", logmessage);
		break;
	}
}
...
static void dahdi_r2_on_line_blocked(openr2_chan_t *r2chan)
{
	...
	p->remotelyblocked = 1;
	tris_mutex_unlock(&p->lock);
	tris_log(LOG_NOTICE, "Far end blocked on chan %d\n", openr2_chan_get_number(r2chan));
}
...
static void dahdi_r2_on_line_idle(openr2_chan_t *r2chan)
{
	...
	p->remotelyblocked = 0;
	tris_mutex_unlock(&p->lock);
	tris_log(LOG_NOTICE, "Far end unblocked on chan %d\n", openr2_chan_get_number(r2chan));
}
...
//	tris_log(LOG_WARNING,"dnis matched = %d\n",p->mfcr2_dnis_matched);
	if ((p->mfcr2_dnis_matched ||
	    (tris_exists_extension(NULL, p->context, p->exten, 1, p->cid_num) && (p->mfcr2_dnis_matched = 1))) &&
	    !tris_matchmore_extension(NULL, p->context, p->exten, 1, p->cid_num)) {
		return 0;
	}
...
{
	tris_verbose("MFC/R2 billing pulse received on channel %d\n", openr2_chan_get_number(r2chan));
}
...
static void swap_subs(struct dahdi_pvt *p, int a, int b)
{
	int tchan;
	int tinthreeway;
	struct tris_channel *towner;

	tris_debug(1, "Swapping %d and %d\n", a, b);
	...
	if (p->subs[a].owner)
		tris_channel_set_fd(p->subs[a].owner, 0, p->subs[a].dfd);
	if (p->subs[b].owner)
		tris_channel_set_fd(p->subs[b].owner, 0, p->subs[b].dfd);
	wakeup_sub(p, a, NULL);
	wakeup_sub(p, b, NULL);
}
...
static int dahdi_open(char *fn)
{
	...
	if (isnum) {
		chan = atoi(fn);
		if (chan < 1) {
			tris_log(LOG_WARNING, "Invalid channel number '%s'\n", fn);
			return -1;
		}
	...
	if (ioctl(fd, DAHDI_SET_BLOCKSIZE, &bs) == -1) {
		tris_log(LOG_WARNING, "Unable to set blocksize '%d': %s\n", bs,  strerror(errno));
		x = errno;
		close(fd);
		errno = x;
		return -1;
...
static int alloc_sub(struct dahdi_pvt *p, int x)
{
	...
	int res;
	if (p->subs[x].dfd >= 0) {
		tris_log(LOG_WARNING, "%s subchannel of %d already in use\n", subnames[x], p->channel);
		return -1;
	}
	...
	if (!res) {
		...
		if (res < 0) {
			tris_log(LOG_WARNING, "Unable to set buffer policy on channel %d: %s\n", x, strerror(errno));
		}
	} else
		tris_log(LOG_WARNING, "Unable to check buffer policy on channel %d: %s\n", x, strerror(errno));

	if (ioctl(p->subs[x].dfd, DAHDI_CHANNO, &p->subs[x].chan) == 1) {
		tris_log(LOG_WARNING, "Unable to get channel number for pseudo channel on FD %d: %s\n", p->subs[x].dfd, strerror(errno));
		dahdi_close_sub(p, x);
		p->subs[x].dfd = -1;
		return -1;
	}
	tris_debug(1, "Allocated %s subchannel on FD %d channel %d\n", subnames[x], p->subs[x].dfd, p->subs[x].chan);
	return 0;
}
...
{
	if (!x) {
		tris_log(LOG_WARNING, "Trying to unalloc the real channel %d?!?\n", p->channel);
		return -1;
	}
	tris_debug(1, "Released sub %d of channel %d\n", x, p->channel);
	dahdi_close_sub(p, x);
	p->subs[x].linear = 0;
	p->subs[x].chan = 0;
	p->subs[x].owner = NULL;
...
static int dahdi_digit_begin(struct tris_channel *chan, char digit)
{
	...
	if (((pvt->sig == SIG_PRI) || (pvt->sig == SIG_BRI) || (pvt->sig == SIG_BRI_PTMP))
			&& (chan->_state == TRIS_STATE_DIALING) && !pvt->proceeding) {
		if (pvt->setup_ack) {
			if (!pri_grab(pvt, pvt->pri)) {
				pri_information(pvt->pri->pri, pvt->call, digit);
				pri_rel(pvt->pri);
			} else
				tris_log(LOG_WARNING, "Unable to grab PRI on span %d\n", pvt->span);
		} else if (strlen(pvt->dialdest) < sizeof(pvt->dialdest) - 1) {
			int res;
			tris_debug(1, "Queueing digit '%c' since setup_ack not yet received\n", digit);
	...
	if (pvt->pulse || ioctl(pvt->subs[SUB_REAL].dfd, DAHDI_SENDTONE, &dtmf)) {
		...
		if ((res = ioctl(pvt->subs[SUB_REAL].dfd, DAHDI_DIAL, &zo)))
			tris_log(LOG_WARNING, "Couldn't dial digit %c: %s\n", digit, strerror(errno));
		else
			pvt->dialing = 1;
	} else {
		tris_debug(1, "Started VLDTMF digit '%c'\n", digit);
		pvt->dialing = 1;
...
static int dahdi_digit_end(struct tris_channel *chan, char digit, unsigned int duration)
{
	struct dahdi_pvt *pvt;
	int res = 0;
	int idx;
	...
	if (pvt->begindigit) {
		x = -1;
		tris_debug(1, "Ending VLDTMF digit '%c'\n", digit);
...
static int conf_add(struct dahdi_pvt *p, struct dahdi_subchannel *c, int idx, int slavechannel)
{
	...
	if (ioctl(c->dfd, DAHDI_SETCONF, &zi)) {
		tris_log(LOG_WARNING, "Failed to add %d to conference %d/%d: %s\n", c->dfd, zi.confmode, zi.confno, strerror(errno));
		return -1;
	}
	if (slavechannel < 1) {
		p->confno = zi.confno;
	...
	tris_debug(1, "Added %d to conference %d/%d\n", c->dfd, c->curconf.confmode, c->curconf.confno);
	return 0;
}
...
static int conf_del(struct dahdi_pvt *p, struct dahdi_subchannel *c, int idx)
{
	...
	memset(&zi, 0, sizeof(zi));
	if (ioctl(c->dfd, DAHDI_SETCONF, &zi)) {
		tris_log(LOG_WARNING, "Failed to drop %d from conference %d/%d: %s\n", c->dfd, c->curconf.confmode, c->curconf.confno, strerror(errno));
		return -1;
	}
	tris_debug(1, "Removed %d from conference %d/%d\n", c->dfd, c->curconf.confmode, c->curconf.confno);
	memcpy(&c->curconf, &zi, sizeof(c->curconf));
	return 0;
}
...
static int reset_conf(struct dahdi_pvt *p)
{
	...
	if (p->subs[SUB_REAL].dfd > -1) {
		...
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_SETCONF, &zi))
			tris_log(LOG_WARNING, "Failed to reset conferencing on channel %d: %s\n", p->channel, strerror(errno));
	}
	return 0;
}
...
static int update_conf(struct dahdi_pvt *p)
{
	...
	tris_debug(1, "Updated conferencing on %d, with %d conference users\n", p->channel, needconf);
	return 0;
}
...
static void dahdi_enable_ec(struct dahdi_pvt *p)
{
	...
	if (!p)
		return;
	if (p->echocanon) {
		tris_debug(1, "Echo cancellation already on\n");
		return;
	}
	if (p->digital) {
		tris_debug(1, "Echo cancellation isn't required on digital connection\n");
		return;
	}
	if (p->echocancel.head.tap_length) {
		if ((p->sig == SIG_BRI) || (p->sig == SIG_BRI_PTMP) || (p->sig == SIG_PRI) || (p->sig == SIG_SS7) || (p->sig == SIG_DPNSS)) {
			x = 1;
			res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_AUDIOMODE, &x);
			if (res)
				tris_log(LOG_WARNING, "Unable to enable audio mode on channel %d (%s)\n", p->channel, strerror(errno));
		}
		res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_ECHOCANCEL_PARAMS, &p->echocancel);
		if (res) {
			tris_log(LOG_WARNING, "Unable to enable echo cancellation on channel %d (%s)\n", p->channel, strerror(errno));
		} else {
			p->echocanon = 1;
			tris_debug(1, "Enabled echo cancellation on channel %d\n", p->channel);
		}
	} else
		tris_debug(1, "No echo cancellation requested\n");
}
...
static void dahdi_train_ec(struct dahdi_pvt *p)
{
	...
	if (p && p->echocanon && p->echotraining) {
		...
		if (res)
			tris_log(LOG_WARNING, "Unable to request echo training on channel %d: %s\n", p->channel, strerror(errno));
		else
			tris_debug(1, "Engaged echo training on channel %d\n", p->channel);
	} else {
		tris_debug(1, "No echo training requested\n");
	}
}
...
static void dahdi_disable_ec(struct dahdi_pvt *p)
{
	...
	if (p->echocanon) {
		...
		if (res)
			tris_log(LOG_WARNING, "Unable to disable echo cancellation on channel %d: %s\n", p->channel, strerror(errno));
		else
			tris_debug(1, "Disabled echo cancellation on channel %d\n", p->channel);
	}
...
static void fill_txgain(struct dahdi_gains *g, float gain, int law)
{
	...
	case DAHDI_LAW_ALAW:
		for (j = 0; j < ARRAY_LEN(g->txgain); j++) {
			if (gain) {
				k = (int) (((float) TRIS_ALAW(j)) * linear_gain);
				if (k > 32767) k = 32767;
				if (k < -32767) k = -32767;
				g->txgain[j] = TRIS_LIN2A(k);
			} else {
				g->txgain[j] = j;
			}
		}
	...
	case DAHDI_LAW_MULAW:
		for (j = 0; j < ARRAY_LEN(g->txgain); j++) {
			if (gain) {
				k = (int) (((float) TRIS_MULAW(j)) * linear_gain);
				if (k > 32767) k = 32767;
				if (k < -32767) k = -32767;
				g->txgain[j] = TRIS_LIN2MU(k);
			} else {
				g->txgain[j] = j;
			}
		}
...
static int set_actual_txgain(int fd, int chan, float gain, int law)
{
	...
	if (res) {
		tris_debug(1, "Failed to read gains: %s\n", strerror(errno));
		return res;
	}
...
static int bump_gains(struct dahdi_pvt *p)
{
	...
	if (res) {
		tris_log(LOG_WARNING, "Unable to bump gain: %s\n", strerror(errno));
		return -1;
	}
...
static int restore_gains(struct dahdi_pvt *p)
{
	...
	if (res) {
		tris_log(LOG_WARNING, "Unable to restore gains: %s\n", strerror(errno));
		return -1;
	}
...
static inline int dahdi_set_hook(int fd, int hs)
{
	...
	if (res < 0) {
		if (errno == EINPROGRESS)
			return 0;
		tris_log(LOG_WARNING, "DAHDI hook failed returned %d (trying %d): %s\n", res, hs, strerror(errno));
...
static inline int dahdi_confmute(struct dahdi_pvt *p, int muted)
{
	int x, y, res;
	x = muted;
	if ((p->sig == SIG_PRI) || (p->sig == SIG_SS7) || (p->sig == SIG_DPNSS) || (p->sig == SIG_BRI) || (p->sig == SIG_BRI_PTMP)) {
		y = 1;
		res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_AUDIOMODE, &y);
		if (res)
			tris_log(LOG_WARNING, "Unable to set audio mode on %d: %s\n", p->channel, strerror(errno));
	}
	res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_CONFMUTE, &x);
	if (res < 0)
		tris_log(LOG_WARNING, "DAHDI confmute(%d) failed on channel %d: %s\n", muted, p->channel, strerror(errno));
	return res;
}
...
static int save_conference(struct dahdi_pvt *p)
{
	...
	if (p->saveconf.confmode) {
		tris_log(LOG_WARNING, "Can't save conference -- already in use\n");
		return -1;
	}
	p->saveconf.chan = 0;
	res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_GETCONF, &p->saveconf);
	if (res) {
		tris_log(LOG_WARNING, "Unable to get conference info: %s\n", strerror(errno));
	...
	if (res) {
		tris_log(LOG_WARNING, "Unable to set conference info: %s\n", strerror(errno));
		return -1;
	}
	tris_debug(1, "Disabled conferencing\n");
	return 0;
}
 ...
 *
 * 1) It generates an internal Trismedia event notifying any other module that
...
static void notify_message(char *mailbox_full, int thereornot)
{
	...
	/* Strip off @default */
	context = mailbox = tris_strdupa(mailbox_full);
	strsep(&context, "@");
	...
	if (!(event = tris_event_new(TRIS_EVENT_MWI,
			...
			TRIS_EVENT_IE_NEWMSGS, TRIS_EVENT_IE_PLTYPE_UINT, thereornot,
			TRIS_EVENT_IE_OLDMSGS, TRIS_EVENT_IE_PLTYPE_UINT, thereornot,
			TRIS_EVENT_IE_END))) {
		return;
	}
	...
	if (!tris_strlen_zero(mailbox) && !tris_strlen_zero(mwimonitornotify)) {
...
static int restore_conference(struct dahdi_pvt *p)
{
	...
	if (p->saveconf.confmode) {
		...
		p->saveconf.confmode = 0;
		if (res) {
			tris_log(LOG_WARNING, "Unable to restore conference info: %s\n", strerror(errno));
			return -1;
		}
	}
	tris_debug(1, "Restored conferencing\n");
	return 0;
}
...
static int send_cwcidspill(struct dahdi_pvt *p)
{
	p->callwaitcas = 0;
	p->cidcwexpire = 0;
	if (!(p->cidspill = tris_malloc(MAX_CALLERID_SIZE)))
		return -1;
	p->cidlen = tris_callerid_callwaiting_generate(p->cidspill, p->callwait_name, p->callwait_num, TRIS_LAW(p));
	/* Make sure we account for the end */
	p->cidlen += READ_SIZE * 4;
	p->cidpos = 0;
	send_callerid(p);
	tris_verb(3, "CPE supports Call Waiting Caller*ID.  Sending '%s/%s'\n", p->callwait_name, p->callwait_num);
	return 0;
}
...
static int has_voicemail(struct dahdi_pvt *p)
{
	...
	mailbox = context = tris_strdupa(p->mailbox);
	strsep(&context, "@");
	if (tris_strlen_zero(context))
		context = "default";
...
static int dahdi_callwait(struct tris_channel *ast)
{
	struct dahdi_pvt *p = ast->tech_pvt;
	p->callwaitingrepeat = CALLWAITING_REPEAT_SAMPLES;
	if (p->cidspill) {
		tris_log(LOG_WARNING, "Spill already exists?!?\n");
		tris_free(p->cidspill);
	}
	if (!(p->cidspill = tris_malloc(2400 /* SAS */ + 680 /* CAS */ + READ_SIZE * 4)))
		return -1;
	save_conference(p);
	/* Silence */
	memset(p->cidspill, 0x7f, 2400 + 600 + READ_SIZE * 4);
	if (!p->callwaitrings && p->callwaitingcallerid) {
		tris_gen_cas(p->cidspill, 1, 2400 + 680, TRIS_LAW(p));
		p->callwaitcas = 1;
		p->cidlen = 2400 + 680 + READ_SIZE * 4;
	} else {
		tris_gen_cas(p->cidspill, 1, 2400, TRIS_LAW(p));
		p->callwaitcas = 0;
...
static int dahdi_call(struct tris_channel *ast, char *rdest, int timeout)
{
	...
	while (restarting) {
		tris_log(LOG_DEBUG, "It's restarting\n");
		usleep(1);
	}

	tris_mutex_lock(&p->lock);
	tris_copy_string(dest, rdest, sizeof(dest));
	tris_copy_string(p->dialdest, rdest, sizeof(p->dialdest));
	if ((ast->_state == TRIS_STATE_BUSY)) {
		p->subs[SUB_REAL].needbusy = 1;
		tris_mutex_unlock(&p->lock);
		return 0;
	}
	if ((ast->_state != TRIS_STATE_DOWN) && (ast->_state != TRIS_STATE_RESERVED)) {
		tris_log(LOG_WARNING, "dahdi_call called on %s, neither down nor reserved\n", ast->name);
		tris_mutex_unlock(&p->lock);
		return -1;
	}
	...
	if (res)
		tris_log(LOG_WARNING, "Unable to flush input on channel %d: %s\n", p->channel, strerror(errno));
	p->outgoing = 1;
	...
	case SIG_FXOKS:
		if (p->owner == ast) {
			...
			if (p->use_callerid) {
				...
				if (p->cidspill) {
					tris_log(LOG_WARNING, "cidspill already exists??\n");
					tris_free(p->cidspill);
				}
				p->callwaitcas = 0;
				if ((p->cidspill = tris_malloc(MAX_CALLERID_SIZE))) {
					p->cidlen = tris_callerid_generate(p->cidspill, ast->cid.cid_name, ast->cid.cid_num, TRIS_LAW(p));
					p->cidpos = 0;
					send_callerid(p);
				}
			}
			...
			if ((p->distinctivering > 0) && (p->distinctivering <= num_cadence)) {
				if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_SETCADENCE, &cadences[p->distinctivering - 1]))
					tris_log(LOG_WARNING, "Unable to set distinctive ring cadence %d on '%s': %s\n", p->distinctivering, ast->name, strerror(errno));
				p->cidrings = cidrings[p->distinctivering - 1];
			} else {
				if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_SETCADENCE, NULL))
					tris_log(LOG_WARNING, "Unable to reset default ring on '%s': %s\n", ast->name, strerror(errno));
			...
			if (c && (strlen(c) < p->stripmsd)) {
				tris_log(LOG_WARNING, "Number '%s' is shorter than stripmsd (%d)\n", c, p->stripmsd);
				c = NULL;
			}
			if (c) {
				p->dop.op = DAHDI_DIAL_OP_REPLACE;
				snprintf(p->dop.dialstr, sizeof(p->dop.dialstr), "Tw%s", c);
				tris_debug(1, "FXO: setup deferred dialstring: %s\n", c);
			} else {
				p->dop.dialstr[0] = '\0';
			}
			x = DAHDI_RING;
			if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_HOOK, &x) && (errno != EINPROGRESS)) {
				tris_log(LOG_WARNING, "Unable to ring phone: %s\n", strerror(errno));
				tris_mutex_unlock(&p->lock);
				return -1;
			}
			p->dialing = 1;
			...
			p->callwaitrings = 0;
			if (ast->cid.cid_num)
				tris_copy_string(p->callwait_num, ast->cid.cid_num, sizeof(p->callwait_num));
			else
				p->callwait_num[0] = '\0';
			if (ast->cid.cid_name)
				tris_copy_string(p->callwait_name, ast->cid.cid_name, sizeof(p->callwait_name));
			else
				p->callwait_name[0] = '\0';
			...
			if (tone_zone_play_tone(p->subs[SUB_CALLWAIT].dfd, DAHDI_TONE_RINGTONE))
				tris_log(LOG_WARNING, "Unable to generate call-wait ring-back on channel %s\n", ast->name);
		}
		n = ast->cid.cid_name;
		l = ast->cid.cid_num;
		if (l)
			tris_copy_string(p->lastcid_num, l, sizeof(p->lastcid_num));
		else
			p->lastcid_num[0] = '\0';
		if (n)
			tris_copy_string(p->lastcid_name, n, sizeof(p->lastcid_name));
		else
			p->lastcid_name[0] = '\0';
	...
	case SIG_FXSKS:
		if (p->answeronpolarityswitch || p->hanguponpolarityswitch) {
			tris_debug(1, "Ignore possible polarity reversal on line seizure\n");
			p->polaritydelaytv = tris_tvnow();
		}
		/* fall through */
	case SIG_EMWINK:
	case SIG_EM:
	...
	case SIG_SF_FEATB:
		...
		if (!p->pri) {
			...
			if (res < 0) {
				if (errno != EINPROGRESS) {
					tris_log(LOG_WARNING, "Unable to start channel: %s\n", strerror(errno));
					tris_mutex_unlock(&p->lock);
					return -1;
				}
			}
		...
		case SIG_FEATDMF_TA:
		{
			...
			if (!cic)
				cic = defaultcic;
			if (!ozz || !cic) {
				tris_log(LOG_WARNING, "Unable to dial channel of type feature group D MF tandem access without CIC or OZZ set\n");
				tris_mutex_unlock(&p->lock);
				return -1;
			}
			...
			break;
		case SIG_E911:
			tris_copy_string(p->dop.dialstr, "M*911#", sizeof(p->dop.dialstr));
			break;
		case SIG_FGC_CAMA:
		...
		if (!p->pri) {
			...
			if( p->waitfordialtone && CANPROGRESSDETECT(p) && p->dsp ) {
				tris_log(LOG_DEBUG, "Defer dialling for %dms or dialtone\n", p->waitfordialtone);
				gettimeofday(&p->waitingfordt,NULL);
				tris_setstate(ast, TRIS_STATE_OFFHOOK);
				break;
			}
#ifdef HAVE_PRI
		}
		...
		if (!res) {
			if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_DIAL, &p->dop)) {
				...
				tris_log(LOG_WARNING, "Dialing failed on channel %d: %s\n", p->channel, strerror(saveerr));
				tris_mutex_unlock(&p->lock);
				return -1;
			}
		} else
			tris_debug(1, "Deferring dialing...\n");

		p->dialing = 1;
		if (tris_strlen_zero(c))
			p->dialednone = 1;
		tris_setstate(ast, TRIS_STATE_DIALING);
		break;
	case 0:
		/* Special pseudo -- automatically up*/
		tris_setstate(ast, TRIS_STATE_UP);
	...
	case SIG_MFCR2:
		...
		break;
	default:
		tris_debug(1, "not yet implemented\n");
		tris_mutex_unlock(&p->lock);
		return -1;
	}
#ifdef HAVE_SS7
	if (p->ss7) {
		...
		if (ss7_grab(p, p->ss7)) {
			tris_log(LOG_WARNING, "Failed to grab SS7!\n");
			tris_mutex_unlock(&p->lock);
			return -1;
		}
		...
		if (!p->ss7call) {
			ss7_rel(p->ss7);
			tris_mutex_unlock(&p->lock);
			tris_log(LOG_ERROR, "Unable to allocate new SS7 call!\n");
			return -1;
		}
	...
	if (p->dpnss) {
		...
		if (dpnss_grab(p, p->dpnss)) {
			tris_log(LOG_WARNING, "Failed to grab DPNSS!\n");
			tris_mutex_unlock(&p->lock);
			return -1;
		}
		...
		dpnss_chan_set_ani(p->dpnss->dpnss, p->dpnsschan, l,strlen(l));
		dpnss_chan_set_dni(p->dpnss->dpnss,p->dpnsschan,(c + p->stripmsd),strlen(c + p->stripmsd));
		dpnss_chan_send_isrm(p->dpnss->dpnss, p->dpnsschan, maxtxlen);
	...
	if (p->mfcr2) {
		...
		if (-1 == callres) {
			tris_mutex_unlock(&p->lock);
			tris_log(LOG_ERROR, "unable to make new MFC/R2 call!\n");
			return -1;
		}
		tris_setstate(ast, TRIS_STATE_DIALING);
	}
	...
	if (p->pri) {
		...
		if (pri_grab(p, p->pri)) {
			tris_log(LOG_WARNING, "Failed to grab PRI!\n");
			tris_mutex_unlock(&p->lock);
			return -1;
		}
		if (!(p->call = pri_new_call(p->pri->pri))) {
			tris_log(LOG_WARNING, "Unable to create call on channel %d\n", p->channel);
			pri_rel(p->pri);
			tris_mutex_unlock(&p->lock);
			return -1;
		}
		if (!(sr = pri_sr_new())) {
			tris_log(LOG_WARNING, "Failed to allocate setup request channel %d\n", p->channel);
			pri_rel(p->pri);
			tris_mutex_unlock(&p->lock);
		}
		if (p->bearer || (mysig == SIG_FXSKS)) {
			if (p->bearer) {
				tris_debug(1, "Oooh, I have a bearer on %d (%d:%d)\n", PVT_TO_CHANNEL(p->bearer), p->bearer->logicalspan, p->bearer->channel);
				p->bearer->call = p->call;
			} else
				tris_debug(1, "I'm being setup with no bearer right now...\n");
		...
		tris_verb(3, "Requested transfer capability: 0x%.2x - %s\n", ast->transfercapability, tris_transfercapability2str(ast->transfercapability));
		...
		while (c[p->stripmsd] > '9' && c[p->stripmsd] != '*' && c[p->stripmsd] != '#') {
			...
			default:
				if (isalpha(c[p->stripmsd])) {
					tris_log(LOG_WARNING, "Unrecognized pridialplan %s modifier: %c\n",
		...
		if (pri_setup(p->pri->pri, p->call, sr)) {
 			tris_log(LOG_WARNING, "Unable to setup call to %s (using %s)\n",
...
static void destroy_dahdi_pvt(struct dahdi_pvt **pvt)
{
	...
	if (p->next)
		p->next->prev = p->prev;
	if (p->use_smdi)
		tris_smdi_interface_unref(p->smdi_iface);
	if (p->mwi_event_sub)
		tris_event_unsubscribe(p->mwi_event_sub);
	if (p->vars) {
		tris_variables_destroy(p->vars);
	}
...
static void destroy_all_channels(void)
{
	...
	while (p) {
		...
		if (option_verbose > 2)
			tris_verbose(VERBOSE_PREFIX_2 "Unregistered channel %d\n", x);
	}
	iflist = NULL;
	ifcount = 0;
...
static int dahdi_send_keypad_facility_exec(struct tris_channel *chan, void *data)
{
	/* Data will be our digit string */
	struct dahdi_pvt *p;
	char *digits = (char *) data;

	if (tris_strlen_zero(digits)) {
		tris_debug(1, "No digit string sent to application!\n");
		return -1;
	}
	...
	if (!p) {
		tris_debug(1, "Unable to find technology private\n");
		return -1;
	}
	...
	if (!p->pri || !p->call) {
		tris_debug(1, "Unable to find pri or call on channel!\n");
		tris_mutex_unlock(&p->lock);
		return -1;
	}
	...
	if (!pri_grab(p, p->pri)) {
		...
		tris_debug(1, "Unable to grab pri to send keypad facility!\n");
		tris_mutex_unlock(&p->lock);
		return -1;
	}
...
static int dahdi_send_callrerouting_facility_exec(struct tris_channel *chan, void *data)
{
	/* Data will be our digit string */
	struct dahdi_pvt *p;
	char *parse;
	...
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(destination);
		TRIS_APP_ARG(original);
		TRIS_APP_ARG(reason);
	);

	if (tris_strlen_zero(data)) {
		tris_log(LOG_DEBUG, "No data sent to application!\n");
		return -1;
	}
	...
	if (!p) {
		tris_log(LOG_DEBUG, "Unable to find technology private\n");
		return -1;
	}
	...
	if (tris_strlen_zero(args.destination)) {
		tris_log(LOG_WARNING, "callrerouting facility requires at least destination number argument\n");
		return -1;
	}

	if (tris_strlen_zero(args.original)) {
		tris_log(LOG_WARNING, "Callrerouting Facility without original called number argument\n");
		args.original = NULL;
	}

	if (tris_strlen_zero(args.reason)) {
		tris_log(LOG_NOTICE, "Callrerouting Facility without diversion reason argument, defaulting to unknown\n");
		args.reason = NULL;
	}
	...
	if (!p->pri || !p->call) {
		tris_log(LOG_DEBUG, "Unable to find pri or call on channel!\n");
		tris_mutex_unlock(&p->lock);
		return -1;
	}
	...
	case SIG_PRI:
		if (!pri_grab(p, p->pri)) {
			if (chan->_state == TRIS_STATE_RING) {
				res = pri_callrerouting_facility(p->pri->pri, p->call, args.destination, args.original, args.reason);
			}
			pri_rel(p->pri);
		} else {
			tris_log(LOG_DEBUG, "Unable to grab pri to send callrerouting facility on span %d!\n", p->span);
			tris_mutex_unlock(&p->lock);
			return -1;
		}
		break;
...
static int pri_assign_bearer(struct dahdi_pvt *crv, struct dahdi_pri *pri, struct dahdi_pvt *bearer)
{
	...
	if (crv->subs[SUB_REAL].owner)
		tris_channel_set_fd(crv->subs[SUB_REAL].owner, 0, crv->subs[SUB_REAL].dfd);
	crv->bearer = bearer;
...
static int pri_find_dchan(struct dahdi_pri *pri)
{
	...
	if (newslot < 0) {
		...
		if (pri->sig != SIG_BRI_PTMP) {
			tris_log(LOG_WARNING, "No D-channels available!  Using Primary channel %d as D-channel anyway!\n",
				pri->dchannels[newslot]);
		}
	}
	if (old && (oldslot != newslot))
		tris_log(LOG_NOTICE, "Switching from from d-channel %d to channel %d!\n",
...
static int dahdi_accept_r2_call_exec(struct tris_channel *chan, void *data)
{
	...
	if (chan->tech != &dahdi_tech) {
		tris_log(LOG_DEBUG, "Only DAHDI technology accepted!\n");
		return -1;
	}
	...
	if (tris_strlen_zero(args.charge)) {
		tris_log(LOG_WARNING, "DAHDIAcceptR2Call requires 'yes' or 'no' for the charge parameter\n");
		return -1;
	}
	...
	if (!p->mfcr2 || !p->mfcr2call) {
		tris_mutex_unlock(&p->lock);
		tris_log(LOG_DEBUG, "Channel %s does not seems to be an R2 active channel!\n", chan->name);
		return -1;
	}

	if (p->mfcr2_call_accepted) {
		tris_mutex_unlock(&p->lock);
		tris_log(LOG_DEBUG, "MFC/R2 call already accepted on channel %s!\n", chan->name);
		return 0;
	}
	accept_mode = tris_true(args.charge) ? OR2_CALL_WITH_CHARGE : OR2_CALL_NO_CHARGE;
	if (openr2_chan_accept_call(p->r2chan, accept_mode)) {
		tris_mutex_unlock(&p->lock);
		tris_log(LOG_WARNING, "Failed to accept MFC/R2 call!\n");
		return -1;
	}
	tris_mutex_unlock(&p->lock);
	...
	while (maxloops > 0) {
		...
		if (res < 0) {
			tris_log(LOG_DEBUG, "tris_waitfor failed on channel %s, going out ...\n", chan->name);
			res = -1;
			break;
		}
		if (res == 0) {
		...
		if (p->mfcr2_call_accepted) {
			tris_mutex_unlock(&p->lock);
			tris_log(LOG_DEBUG, "Accepted MFC/R2 call!\n");
			break;
		}
		tris_mutex_unlock(&p->lock);
	}
...
static openr2_call_disconnect_cause_t dahdi_tris_cause_to_r2_cause(int cause)
{
	openr2_call_disconnect_cause_t r2cause = OR2_CAUSE_NORMAL_CLEARING;
	switch (cause) {
	case TRIS_CAUSE_USER_BUSY:
	case TRIS_CAUSE_CALL_REJECTED:
	case TRIS_CAUSE_INTERWORKING: /* I don't know wtf is this but is used sometimes when ekiga rejects a call */
		r2cause = OR2_CAUSE_BUSY_NUMBER;
		break;

	case TRIS_CAUSE_NORMAL_CIRCUIT_CONGESTION:
	case TRIS_CAUSE_SWITCH_CONGESTION:
		r2cause = OR2_CAUSE_NETWORK_CONGESTION;
		break;

	case TRIS_CAUSE_UNALLOCATED:
		r2cause = OR2_CAUSE_UNALLOCATED_NUMBER;
		break;

	case TRIS_CAUSE_NETWORK_OUT_OF_ORDER:
	case TRIS_CAUSE_DESTINATION_OUT_OF_ORDER:
		r2cause = OR2_CAUSE_OUT_OF_ORDER;
		break;

	case TRIS_CAUSE_NO_ANSWER:
	case TRIS_CAUSE_NO_USER_RESPONSE:
	...
	tris_log(LOG_DEBUG, "ast cause %d resulted in openr2 cause %d/%s\n",
...
static enum dpnss_cause_code dpnss_get_cause_from_ast(int cause){
	switch(cause){
		case TRIS_CAUSE_UNALLOCATED:
			return DPNSS_CAUSE_NU;
		case TRIS_CAUSE_BUSY:
			return DPNSS_CAUSE_BUSY;
		case TRIS_CAUSE_NORMAL:
			return DPNSS_CAUSE_CT;
		case TRIS_CAUSE_FAILURE:
			return DPNSS_CAUSE_NT;
		case TRIS_CAUSE_RECOVERY_ON_TIMER_EXPIRE:
			return DPNSS_CAUSE_ROS;
		default:
			return DPNSS_CAUSE_CON;
	}
}

static int dpnss_get_cause_from_dpnss(enum dpnss_cause_code cause){
	switch(cause){
		case DPNSS_CAUSE_NU:
			return TRIS_CAUSE_UNALLOCATED;
		case DPNSS_CAUSE_BUSY:
			return TRIS_CAUSE_BUSY;
		case DPNSS_CAUSE_CT:
			return TRIS_CAUSE_NORMAL;
		case DPNSS_CAUSE_NT:
			return TRIS_CAUSE_FAILURE;
		case DPNSS_CAUSE_ROS:
			return TRIS_CAUSE_FAILURE;
		default:
			return TRIS_CAUSE_CONGESTION;
	}
...
static int dahdi_hangup(struct tris_channel *ast)
{
	...
	if ((p->sig == SIG_PRI) ||(p->sig == SIG_DPNSS) || (p->sig == SIG_SS7) || (p->sig == SIG_BRI) || (p->sig == SIG_BRI_PTMP)) {
		x = 1;
		tris_channel_setoption(ast,TRIS_OPTION_AUDIO_MODE,&x,sizeof(char),0);
		p->cid_num[0] = '\0';
		p->cid_name[0] = '\0';
	}
	...
	restore_gains(p);
	if (p->origcid_num) {
		tris_copy_string(p->cid_num, p->origcid_num, sizeof(p->cid_num));
		tris_free(p->origcid_num);
		p->origcid_num = NULL;
	}
	if (p->origcid_name) {
		tris_copy_string(p->cid_name, p->origcid_name, sizeof(p->cid_name));
		tris_free(p->origcid_name);
		p->origcid_name = NULL;
	}
	if (p->dsp)
		tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_DTMF | p->dtmfrelax);
	p->exten[0] = '\0';

	tris_debug(1, "Hangup: channel: %d index = %d, normal = %d, callwait = %d, thirdcall = %d\n",
	...
	if (idx > -1) {
		...
		if (idx == SUB_REAL) {
			if ((p->subs[SUB_CALLWAIT].dfd > -1) && (p->subs[SUB_THREEWAY].dfd > -1)) {
				tris_debug(1, "Normal call hung up with both three way call and a call waiting call in place?\n");
				if (p->subs[SUB_CALLWAIT].inthreeway) {
					/* We had flipped over to answer a callwait and now it's gone */
					tris_debug(1, "We were flipped over to the callwait, moving back and unowning.\n");
					...
					tris_debug(1, "We were in the threeway and have a callwait still.  Ditching the threeway.\n");
					swap_subs(p, SUB_THREEWAY, SUB_REAL);
					unalloc_sub(p, SUB_THREEWAY);
					if (p->subs[SUB_REAL].inthreeway) {
						/* This was part of a three way call.  Immediately make way for
						   another call */
						tris_debug(1, "Call was complete, setting owner to former third call\n");
						p->owner = p->subs[SUB_REAL].owner;
					} else {
						/* This call hasn't been completed yet...  Set owner to NULL */
						tris_debug(1, "Call was incomplete, setting owner to NULL\n");
						p->owner = NULL;
					}
				...
				if (p->owner->_state != TRIS_STATE_UP)
					p->subs[SUB_REAL].needanswer = 1;
				if (tris_bridged_channel(p->subs[SUB_REAL].owner))
					tris_queue_control(p->subs[SUB_REAL].owner, TRIS_CONTROL_UNHOLD);
			...
			if (p->subs[SUB_CALLWAIT].inthreeway) {
				...
				if (p->subs[SUB_THREEWAY].owner && tris_bridged_channel(p->subs[SUB_THREEWAY].owner)) {
					tris_queue_control_data(p->subs[SUB_THREEWAY].owner, TRIS_CONTROL_HOLD,
			...
			if (p->subs[SUB_CALLWAIT].inthreeway) {
				...
				if (p->subs[SUB_CALLWAIT].owner && tris_bridged_channel(p->subs[SUB_CALLWAIT].owner)) {
					tris_queue_control_data(p->subs[SUB_CALLWAIT].owner, TRIS_CONTROL_HOLD,
						S_OR(p->mohsuggest, NULL),
						!tris_strlen_zero(p->mohsuggest) ? strlen(p->mohsuggest) + 1 : 0);
				}
			...
			tris_log(LOG_WARNING, "Index found but not any type of call?\n");
		}
	}
	...
	if (!p->subs[SUB_REAL].owner && !p->subs[SUB_CALLWAIT].owner && !p->subs[SUB_THREEWAY].owner) {
		...
		if (p->faxbuffersinuse) {
			...
			if ((bpres = ioctl(p->subs[SUB_REAL].dfd, DAHDI_SET_BUFINFO, &bi)) < 0) {
				tris_log(LOG_WARNING, "Channel '%s' unable to revert faxbuffer policy: %s\n", ast->name, strerror(errno));
			}
			p->faxbuffersinuse = 0;
		}
		...
		if (res < 0)
			tris_log(LOG_WARNING, "Unable to set law on channel %d to default: %s\n", p->channel, strerror(errno));
			...
			if (p->ss7call) {
				if (!ss7_grab(p, p->ss7)) {
					if (!p->alreadyhungup) {
						...
						tris_log(LOG_WARNING, "Trying to hangup twice!\n");
				} else {
					tris_log(LOG_WARNING, "Unable to grab SS7 on CIC %d\n", p->cic);
					res = -1;
				}
			}
		}
		...
		if (p->dpnss) {
			if (p->dpnsscall) {
				if (!dpnss_grab(p, p->dpnss)) {
					if (!p->alreadyhungup) {
						int icause = ast->hangupcause ? ast->hangupcause : -1;
						dpnss_chan_set_cause(p->dpnss->dpnss,p->dpnsschan,dpnss_get_cause_from_ast(icause));
						dpnss_chan_send_crm(p->dpnss->dpnss,p->dpnsschan);
						dpnss_rel(p->dpnss);
						p->alreadyhungup = 1;
					} else
						tris_log(LOG_WARNING, "Trying to hangup twice!\n");
				} else {
					tris_log(LOG_WARNING, "Unable to grab DPNSS Channel %d\n", p->channel);
					res = -1;
				}
			}
		}
		...
		if (p->mfcr2 && p->mfcr2call && openr2_chan_get_direction(p->r2chan) != OR2_DIR_STOPPED) {
			p->cid_num[0] = '\0';
			p->cid_name[0] = '\0';
			tris_log(LOG_DEBUG, "disconnecting MFC/R2 call on chan %d\n", p->channel);
			...
			if (openr2_chan_get_direction(p->r2chan) == OR2_DIR_BACKWARD && p->mfcr2_forced_release) {
				...
				openr2_call_disconnect_cause_t r2cause = r2cause_user ? dahdi_tris_cause_to_r2_cause(r2cause_user)
					                                              : dahdi_tris_cause_to_r2_cause(ast->hangupcause);
				dahdi_r2_disconnect_call(p, r2cause);
			}
		} else if (p->mfcr2call) {
			tris_log(LOG_DEBUG, "Clearing call request on channel %d\n", p->channel);
			/* since tris_request() was called but not tris_call() we have not yet dialed
		...
		if (p->pri) {
			...
			if (p->call && (!p->bearer || (p->bearer->call == p->call))) {
				if (!pri_grab(p, p->pri)) {
					if (p->alreadyhungup) {
						tris_debug(1, "Already hungup...  Calling hangup once, and clearing call\n");
						...
						int icause = ast->hangupcause ? ast->hangupcause : -1;
						tris_debug(1, "Not yet hungup...  Calling hangup once with icause, and clearing call\n");
					...
					if (res < 0)
						tris_log(LOG_WARNING, "pri_disconnect failed\n");
					pri_rel(p->pri);
				} else {
					tris_log(LOG_WARNING, "Unable to grab PRI on span %d\n", p->span);
					res = -1;
				...
				if (p->bearer)
					tris_debug(1, "Bearer call is %p, while ours is still %p\n", p->bearer->call, p->call);
				p->call = NULL;
				res = 0;
			}
		}
#endif
		if (p->sig && ((p->sig != SIG_PRI) && (p->sig != SIG_SS7) && (p->sig != SIG_DPNSS)
			&& (p->sig != SIG_BRI)
			&& (p->sig != SIG_BRI_PTMP))
			&& (p->sig != SIG_MFCR2))
			res = dahdi_set_hook(p->subs[SUB_REAL].dfd, DAHDI_ONHOOK);
		if (res < 0) {
			tris_log(LOG_WARNING, "Unable to hangup line %s\n", ast->name);
		}
		switch (p->sig) {
		case SIG_FXOGS:
		case SIG_FXOLS:
		case SIG_FXOKS:
			...
			if (!res) {
#if 0
				tris_debug(1, "Hanging up channel %d, offhook = %d\n", p->channel, par.rxisoffhook);
#endif
		...
		case SIG_FXSKS:
			...
			if (ast->_state != TRIS_STATE_RESERVED) {
				time(&p->guardtime);
				p->guardtime += 2;
			}
			break;
		...
		if (p->sig)
			dahdi_disable_ec(p);
		x = 0;
		tris_channel_setoption(ast,TRIS_OPTION_TONE_VERIFY,&x,sizeof(char),0);
		tris_channel_setoption(ast,TRIS_OPTION_TDD,&x,sizeof(char),0);
		p->didtdd = 0;
		p->cidspill = NULL;
		p->callwaitcas = 0;
...
#ifdef HAVE_PRI
		if (p->bearer) {
			tris_debug(1, "Freeing up bearer channel %d\n", p->bearer->channel);
...
static int dahdi_answer(struct tris_channel *ast)
{
	...
	case SIG_FXOKS:
		...
		if ((idx == SUB_REAL) && p->subs[SUB_THREEWAY].inthreeway) {
			if (oldstate == TRIS_STATE_RINGING) {
				tris_debug(1, "Finally swapping real and threeway\n");
	...
	case SIG_SS7:
		if (!ss7_grab(p, p->ss7)) {
			...
			tris_log(LOG_WARNING, "Unable to grab SS7 on span %d\n", p->span);
			res = -1;
		}
		break;
#endif
	...
	case SIG_DPNSS:
		if (!dpnss_grab(p, p->dpnss)) {
			p->proceeding = 1;
			dpnss_chan_send_ccm(p->dpnss->dpnss, p->dpnsschan);
			usleep(1);
			dpnss_chan_send_ccm(p->dpnss->dpnss, p->dpnsschan);
			dpnss_rel(p->dpnss);
		} else {
			tris_log(LOG_WARNING, "Unable to grab DPNSS on span %d\n", p->span);
			res = -1;
		}
		break;
#endif
	...
	case SIG_MFCR2:
		if (!p->mfcr2_call_accepted) {
			...
			if (p->mfcr2_charge_calls) {
				tris_log(LOG_DEBUG, "Accepting MFC/R2 call with charge before answering on chan %d\n", p->channel);
				openr2_chan_accept_call(p->r2chan, OR2_CALL_WITH_CHARGE);
			} else {
				tris_log(LOG_DEBUG, "Accepting MFC/R2 call with no charge before answering on chan %d\n", p->channel);
				openr2_chan_accept_call(p->r2chan, OR2_CALL_NO_CHARGE);
			}
		} else {
			tris_log(LOG_DEBUG, "Answering MFC/R2 call on chan %d\n", p->channel);
			dahdi_r2_answer(p);
		}
		break;
#endif
	case 0:
		...
		return 0;
	default:
		tris_log(LOG_WARNING, "Don't know how to answer signalling %d (channel %d)\n", p->sig, p->channel);
		res = -1;
	}
	tris_mutex_unlock(&p->lock);
	return res;
...
static int dahdi_setoption(struct tris_channel *chan, int option, void *data, int datalen)
{
	...
	switch (option) {
	case TRIS_OPTION_TXGAIN:
		scp = (signed char *) data;
		idx = dahdi_get_index(chan, p, 0);
		if (idx < 0) {
			tris_log(LOG_WARNING, "No index in TXGAIN?\n");
			return -1;
		}
		tris_debug(1, "Setting actual tx gain on %s to %f\n", chan->name, p->txgain + (float) *scp);
		return set_actual_txgain(p->subs[idx].dfd, 0, p->txgain + (float) *scp, p->law);
	case TRIS_OPTION_RXGAIN:
		scp = (signed char *) data;
		idx = dahdi_get_index(chan, p, 0);
		if (idx < 0) {
			tris_log(LOG_WARNING, "No index in RXGAIN?\n");
			return -1;
		}
		tris_debug(1, "Setting actual rx gain on %s to %f\n", chan->name, p->rxgain + (float) *scp);
		return set_actual_rxgain(p->subs[idx].dfd, 0, p->rxgain + (float) *scp, p->law);
	case TRIS_OPTION_TONE_VERIFY:
		if (!p->dsp)
			break;
		cp = (char *) data;
		switch (*cp) {
		case 1:
			tris_debug(1, "Set option TONE VERIFY, mode: MUTECONF(1) on %s\n",chan->name);
			tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_MUTECONF | p->dtmfrelax);  /* set mute mode if desired */
			break;
		case 2:
			tris_debug(1, "Set option TONE VERIFY, mode: MUTECONF/MAX(2) on %s\n",chan->name);
			tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_MUTECONF | DSP_DIGITMODE_MUTEMAX | p->dtmfrelax);  /* set mute mode if desired */
			break;
		default:
			tris_debug(1, "Set option TONE VERIFY, mode: OFF(0) on %s\n",chan->name);
			tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_DTMF | p->dtmfrelax);  /* set mute mode if desired */
			break;
		}
		break;
	case TRIS_OPTION_TDD:
		/* turn on or off TDD */
		cp = (char *) data;
		p->mate = 0;
		if (!*cp) { /* turn it off */
			tris_debug(1, "Set option TDD MODE, value: OFF(0) on %s\n",chan->name);
			if (p->tdd)
				tdd_free(p->tdd);
			p->tdd = 0;
			break;
		}
		tris_debug(1, "Set option TDD MODE, value: %s(%d) on %s\n",
		...
		if (!p->didtdd) { /* if havent done it yet */
			...
			tris_tdd_gen_ecdisa(buf + 16000, 16000);  /* put in tone */
			len = 40000;
			idx = dahdi_get_index(chan, p, 0);
			if (idx < 0) {
				tris_log(LOG_WARNING, "No index in TDD?\n");
				return -1;
			}
			fd = p->subs[idx].dfd;
			while (len) {
				...
				res = poll(fds, 1, -1);
				if (!res) {
					tris_debug(1, "poll (for write) ret. 0 on channel %d\n", p->channel);
					continue;
				}
				/* if got exception */
				...
				if (!(fds[0].revents & POLLOUT)) {
					tris_debug(1, "write fd not ready on channel %d\n", p->channel);
					continue;
				}
				res = write(fd, buf, size);
				if (res != size) {
					if (res == -1) return -1;
					tris_debug(1, "Write returned %d (%s) on channel %d\n", res, strerror(errno), p->channel);
					break;
				}
				len -= size;
				buf += size;
		...
		break;
	case TRIS_OPTION_RELAXDTMF:  /* Relax DTMF decoding (or not) */
		if (!p->dsp)
			break;
		cp = (char *) data;
		tris_debug(1, "Set option RELAX DTMF, value: %s(%d) on %s\n",
			*cp ? "ON" : "OFF", (int) *cp, chan->name);
		tris_dsp_set_digitmode(p->dsp, ((*cp) ? DSP_DIGITMODE_RELAXDTMF : DSP_DIGITMODE_DTMF) | p->dtmfrelax);
		break;
	case TRIS_OPTION_AUDIO_MODE:  /* Set AUDIO mode (or not) */
		cp = (char *) data;
		if (!*cp) {
			tris_debug(1, "Set option AUDIO MODE, value: OFF(0) on %s\n", chan->name);
			x = 0;
			dahdi_disable_ec(p);
		} else {
			tris_debug(1, "Set option AUDIO MODE, value: ON(1) on %s\n", chan->name);
			x = 1;
		}
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_AUDIOMODE, &x) == -1)
			tris_log(LOG_WARNING, "Unable to set audio mode on channel %d to %d: %s\n", p->channel, x, strerror(errno));
		break;
	case TRIS_OPTION_OPRMODE:  /* Operator services mode */
		...
		if (strcasecmp(chan->tech->type, oprmode->peer->tech->type)) {
			tris_log(LOG_NOTICE, "Operator mode not supported on %s to %s calls.\n",
		...
		tris_debug(1, "Set Operator Services mode, value: %d on %s/%s\n",
			oprmode->mode, chan->name,oprmode->peer->name);
		break;
	case TRIS_OPTION_ECHOCAN:
		cp = (char *) data;
		if (*cp) {
			tris_debug(1, "Enabling echo cancelation on %s\n", chan->name);
			dahdi_enable_ec(p);
		} else {
			tris_debug(1, "Disabling echo cancelation on %s\n", chan->name);
			dahdi_disable_ec(p);
		}
		break;
	}
...
static void dahdi_unlink(struct dahdi_pvt *slave, struct dahdi_pvt *master, int needlock)
{
	...
	if (!master)
		return;
	if (needlock) {
		tris_mutex_lock(&master->lock);
		if (slave) {
			while (tris_mutex_trylock(&slave->lock)) {
	...
	for (x = 0; x < MAX_SLAVES; x++) {
		if (master->slaves[x]) {
			if (!slave || (master->slaves[x] == slave)) {
				...
				tris_debug(1, "Unlinking slave %d from %d\n", master->slaves[x]->channel, master->channel);
	...
	if (needlock) {
		if (slave)
			tris_mutex_unlock(&slave->lock);
		tris_mutex_unlock(&master->lock);
	}
}
...
static void dahdi_link(struct dahdi_pvt *slave, struct dahdi_pvt *master) {
	int x;
	if (!slave || !master) {
		tris_log(LOG_WARNING, "Tried to link to/from NULL??\n");
		return;
	}
	for (x = 0; x < MAX_SLAVES; x++) {
		if (!master->slaves[x]) {
	...
	if (x >= MAX_SLAVES) {
		tris_log(LOG_WARNING, "Replacing slave %d with new slave, %d\n", master->slaves[MAX_SLAVES - 1]->channel, slave->channel);
		master->slaves[MAX_SLAVES - 1] = slave;
	}
	if (slave->master)
		tris_log(LOG_WARNING, "Replacing master %d with new master, %d\n", slave->master->channel, master->channel);
	slave->master = master;

	tris_debug(1, "Making %d slave to master %d at %d\n", slave->channel, master->channel, x);
}
...
static void disable_dtmf_detect(struct dahdi_pvt *p)
{
	...
	if (!p->hardwaredtmf && p->dsp) {
		...
		tris_dsp_set_features(p->dsp, p->dsp_features);
	}
}
...
static enum tris_bridge_result dahdi_bridge(struct tris_channel *c0, struct tris_channel *c1, int flags, struct tris_frame **fo, struct tris_channel **rc, int timeoutms)
{
	...
	int os0 = -1, os1 = -1;
	int priority = 0;
	struct tris_channel *oc0, *oc1;
	enum tris_bridge_result res;
	...
	*/
	...
	if (flags & (TRIS_BRIDGE_DTMF_CHANNEL_0 | TRIS_BRIDGE_DTMF_CHANNEL_1))
	...
	if (tris_mutex_trylock(&p0->lock)) {
		/* Don't block, due to potential for deadlock */
		tris_channel_unlock(c0);
		tris_channel_unlock(c1);
		tris_log(LOG_NOTICE, "Avoiding deadlock...\n");
		return TRIS_BRIDGE_RETRY;
	}
	if (tris_mutex_trylock(&p1->lock)) {
		/* Don't block, due to potential for deadlock */
	...
	if ((oi0 == SUB_REAL) && (oi1 == SUB_REAL)) {
		if (p0->owner && p1->owner) {
			...
			if (!p0->subs[SUB_CALLWAIT].inthreeway && !p1->subs[SUB_REAL].inthreeway) {
				...
				tris_log(LOG_WARNING, "Huh?  Both calls are callwaits or 3-ways?  That's clever...?\n");
				tris_log(LOG_WARNING, "p0: chan %d/%d/CW%d/3W%d, p1: chan %d/%d/CW%d/3W%d\n",
					p0->channel,
	...
	tris_debug(1, "master: %d, slave: %d, nothingok: %d\n",
	...
	if (master && slave) {
		...
		if ((oi1 == SUB_THREEWAY) &&
			...
			p1->subs[SUB_REAL].inthreeway &&
			(p1->subs[SUB_REAL].owner->_state == TRIS_STATE_RINGING)) {
			tris_debug(1, "Playing ringback on %s since %s is in a ringing three-way\n", c0->name, c1->name);
			tone_zone_play_tone(p0->subs[oi0].dfd, DAHDI_TONE_RINGTONE);
			os1 = p1->subs[SUB_REAL].owner->_state;
		} else {
			tris_debug(1, "Stopping tones on %d/%d talking to %d/%d\n", p0->channel, oi0, p1->channel, oi1);
		...
		if ((oi0 == SUB_THREEWAY) &&
			...
			(p0->subs[SUB_REAL].owner->_state == TRIS_STATE_RINGING)) {
			tris_debug(1, "Playing ringback on %s since %s is in a ringing three-way\n", c1->name, c0->name);
		...
		tris_log(LOG_WARNING, "Can't link %d/%s with %d/%s\n", p0->channel, subnames[oi0], p1->channel, subnames[oi1]);
	...
	if (!(flags & TRIS_BRIDGE_DTMF_CHANNEL_0) && (oi0 == SUB_REAL))
		disable_dtmf_detect(op0);

	if (!(flags & TRIS_BRIDGE_DTMF_CHANNEL_1) && (oi1 == SUB_REAL))
		disable_dtmf_detect(op1);

	for (;;) {
		struct tris_channel *c0_priority[2] = {c0, c1};
		struct tris_channel *c1_priority[2] = {c1, c0};
		...
		if (!timeoutms ||
			...
			(oi0 != i0) ||
			(oi1 != i1)) {
			tris_debug(1, "Something changed out on %d/%d to %d/%d, returning -3 to restart\n",
		...
		who = tris_waitfor_n(priority ? c0_priority : c1_priority, 2, &timeoutms);
		if (!who) {
...
static int dahdi_fixup(struct tris_channel *oldchan, struct tris_channel *newchan)
{
	...
	int x;
	tris_mutex_lock(&p->lock);
	tris_debug(1, "New owner for channel %d is %s\n", p->channel, newchan->name);
	if (p->owner == oldchan) {
		p->owner = newchan;
	}
	for (x = 0; x < 3; x++)
	...
	if (newchan->_state == TRIS_STATE_RINGING)
		dahdi_indicate(newchan, TRIS_CONTROL_RINGING, NULL, 0);
	update_conf(p);
	tris_mutex_unlock(&p->lock);
	return 0;
}
	...
	do {
		...
		if (res) {
			...
			case EINPROGRESS:
				...
				break;
			default:
				tris_log(LOG_WARNING, "Couldn't ring the phone: %s\n", strerror(errno));
				res = 0;
			}
		}
	} while (res);
...
static int attempt_transfer(struct dahdi_pvt *p)
{
	...
	if (tris_bridged_channel(p->subs[SUB_REAL].owner)) {
		...
		if (tris_bridged_channel(p->subs[SUB_THREEWAY].owner))
			tris_queue_control(p->subs[SUB_THREEWAY].owner, TRIS_CONTROL_UNHOLD);
		if (p->subs[SUB_REAL].owner->_state == TRIS_STATE_RINGING) {
			tris_indicate(tris_bridged_channel(p->subs[SUB_REAL].owner), TRIS_CONTROL_RINGING);
		}
		if (p->subs[SUB_THREEWAY].owner->_state == TRIS_STATE_RING) {
			tone_zone_play_tone(p->subs[SUB_THREEWAY].dfd, DAHDI_TONE_RINGTONE);
		}
		 if (tris_channel_masquerade(p->subs[SUB_THREEWAY].owner, tris_bridged_channel(p->subs[SUB_REAL].owner))) {
			tris_log(LOG_WARNING, "Unable to masquerade %s as %s\n",
					tris_bridged_channel(p->subs[SUB_REAL].owner)->name, p->subs[SUB_THREEWAY].owner->name);
			return -1;
		}
		/* Orphan the channel after releasing the lock */
		tris_channel_unlock(p->subs[SUB_THREEWAY].owner);
		unalloc_sub(p, SUB_THREEWAY);
	} else if (tris_bridged_channel(p->subs[SUB_THREEWAY].owner)) {
		tris_queue_control(p->subs[SUB_REAL].owner, TRIS_CONTROL_UNHOLD);
		if (p->subs[SUB_THREEWAY].owner->_state == TRIS_STATE_RINGING) {
			tris_indicate(tris_bridged_channel(p->subs[SUB_THREEWAY].owner), TRIS_CONTROL_RINGING);
		}
		if (p->subs[SUB_REAL].owner->_state == TRIS_STATE_RING) {
			tone_zone_play_tone(p->subs[SUB_REAL].dfd, DAHDI_TONE_RINGTONE);
		}
		if (tris_channel_masquerade(p->subs[SUB_REAL].owner, tris_bridged_channel(p->subs[SUB_THREEWAY].owner))) {
			tris_log(LOG_WARNING, "Unable to masquerade %s as %s\n",
					tris_bridged_channel(p->subs[SUB_THREEWAY].owner)->name, p->subs[SUB_REAL].owner->name);
			return -1;
		}
		/* Three-way is now the REAL */
		swap_subs(p, SUB_THREEWAY, SUB_REAL);
		tris_channel_unlock(p->subs[SUB_REAL].owner);
		unalloc_sub(p, SUB_THREEWAY);
		...
		p->subs[SUB_THREEWAY].owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
		return -1;
	}
	return 0;
}
...
static int check_for_conference(struct dahdi_pvt *p)
{
	...
	if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_GETCONF, &ci)) {
		tris_log(LOG_WARNING, "Failed to get conference info on channel %d: %s\n", p->channel, strerror(errno));
		return 0;
	}
	...
	if ((p->subs[SUB_REAL].curconf.confno != ci.confno) || (p->subs[SUB_REAL].curconf.confmode != ci.confmode)) {
		tris_verb(3, "Avoiding 3-way call when in an external conference\n");
		return 1;
	}
	return 0;
}
...
static int get_alarms(struct dahdi_pvt *p)
{
	...
	if ((res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_SPANSTAT, &zi)) >= 0) {
		...
		tris_log(LOG_WARNING, "Unable to determine alarm on channel %d: %s\n", p->channel, strerror(errno));
		return 0;
	}
	...
	tris_log(LOG_WARNING, "Unable to determine alarm on channel %d\n", p->channel);

	return DAHDI_ALARM_NONE;
}

static void dahdi_handle_dtmfup(struct tris_channel *ast, int idx, struct tris_frame **dest)
{
	struct dahdi_pvt *p = ast->tech_pvt;
	struct tris_frame *f = *dest;

	tris_debug(1, "DTMF digit: %c on %s\n", f->subclass, ast->name);

	if (p->confirmanswer) {
		tris_debug(1, "Confirm answer on %s!\n", ast->name);
		/* Upon receiving a DTMF digit, consider this an answer confirmation instead
		   of a DTMF digit */
		p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
		p->subs[idx].f.subclass = TRIS_CONTROL_ANSWER;
		*dest = &p->subs[idx].f;
		/* Reset confirmanswer so DTMF's will behave properly for the duration of the call */
		p->confirmanswer = 0;
	} else if (p->callwaitcas) {
		if ((f->subclass == 'A') || (f->subclass == 'D')) {
			tris_debug(1, "Got some DTMF, but it's for the CAS\n");
			if (p->cidspill)
				tris_free(p->cidspill);
			send_cwcidspill(p);
		}
		p->callwaitcas = 0;
		p->subs[idx].f.frametype = TRIS_FRAME_NULL;
		...
		if ((p->callprogress & CALLPROGRESS_FAX) && !p->faxhandled) {
			...
			if (p->usefaxbuffers && !p->faxbuffersinuse) {
				...
				if ((res = ioctl(p->subs[idx].dfd, DAHDI_SET_BUFINFO, &bi)) < 0) {
					tris_log(LOG_WARNING, "Channel '%s' unable to set faxbuffer policy, reason: %s\n", ast->name, strerror(errno));
				} else {
					p->faxbuffersinuse = 1;
				}
			}
			...
			if (strcmp(ast->exten, "fax")) {
				...
				/* We need to unlock 'ast' here because tris_exists_extension has the
...
static void handle_alarms(struct dahdi_pvt *p, int alms)
{
	...
	tris_log(LOG_WARNING, "Detected alarm on channel %d: %s\n", p->channel, alarm_str);
...
static struct tris_frame *dahdi_handle_event(struct tris_channel *ast)
{
	int res, x;
	int idx, mysig;
	char *c;
...
#endif
			dahdi_confmute(p, 0);
			p->subs[idx].f.frametype = TRIS_FRAME_DTMF_END;
	...
	if (res & DAHDI_EVENT_DTMFDOWN) {
		tris_debug(1, "DTMF Down '%c'\n", res & 0xff);
		/* Mute conference */
		dahdi_confmute(p, 1);
		p->subs[idx].f.frametype = TRIS_FRAME_DTMF_BEGIN;
		p->subs[idx].f.subclass = res & 0xff;
		return &p->subs[idx].f;
	}
	...
	switch (res) {
		case DAHDI_EVENT_EC_DISABLED:
			tris_verb(3, "Channel %d echo canceler disabled due to CED detection\n", p->channel);
			p->echocanon = 0;
			break;
		case DAHDI_EVENT_BITSCHANGED:
#ifdef HAVE_OPENR2
			if (p->sig != SIG_MFCR2) {
				tris_log(LOG_WARNING, "Received bits changed on %s signalling?\n", sig2str(p->sig));
			} else {
				tris_log(LOG_DEBUG, "bits changed in chan %d\n", p->channel);
		...
		case DAHDI_EVENT_DIALCOMPLETE:
#ifdef HAVE_OPENR2
			if ((p->sig & SIG_MFCR2) && p->r2chan && ast->_state != TRIS_STATE_UP) {
			...
			if (ioctl(p->subs[idx].dfd,DAHDI_DIALING,&x) == -1) {
				tris_log(LOG_DEBUG, "DAHDI_DIALING ioctl failed on %s: %s\n",ast->name, strerror(errno));
				return NULL;
			}
			if (!x) { /* if not still dialing in driver */
				...
				if (p->echobreak) {
					dahdi_train_ec(p);
					tris_copy_string(p->dop.dialstr, p->echorest, sizeof(p->dop.dialstr));
					p->dop.op = DAHDI_DIAL_OP_REPLACE;
					...
					if ((mysig == SIG_E911) || (mysig == SIG_FGC_CAMA) || (mysig == SIG_FGC_CAMAMF)) {
						...
						if (ast->_state == TRIS_STATE_DIALING_OFFHOOK) {
							tris_setstate(ast, TRIS_STATE_UP);
							p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
							p->subs[idx].f.subclass = TRIS_CONTROL_ANSWER;
							break;
							...
							tris_setstate(ast,TRIS_STATE_DIALING_OFFHOOK);
						}
					}
					if (ast->_state == TRIS_STATE_DIALING) {
						if ((p->callprogress & CALLPROGRESS_PROGRESS) && CANPROGRESSDETECT(p) && p->dsp && p->outgoing) {
							tris_debug(1, "Done dialing, but waiting for progress detection before doing more...\n");
				...
				if (!p->pri || !p->pri->pri || (pri_get_timer(p->pri->pri, PRI_TIMER_T309) < 0)) {
					...
					if (p->call) {
						if (p->pri && p->pri->pri) {
							...
							tris_log(LOG_WARNING, "The PRI Call has not been destroyed\n");
					}
					if (p->owner)
						p->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
				}
		...
		case DAHDI_EVENT_ONHOOK:
			if (p->radio) {
				p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
				p->subs[idx].f.subclass = TRIS_CONTROL_RADIO_UNKEY;
				break;
			}
			if (p->oprmode < 0)
			{
			...
			case SIG_FXOKS:
				...
				if (idx == SUB_REAL) {
					...
					if (p->subs[SUB_CALLWAIT].owner) {
						...
						swap_subs(p, SUB_CALLWAIT, SUB_REAL);
						tris_verb(3, "Channel %d still has (callwait) call, ringing phone\n", p->channel);
						unalloc_sub(p, SUB_CALLWAIT);
#if 0
						p->subs[idx].needanswer = 0;
						p->subs[idx].needringing = 0;
						...
						if (p->subs[SUB_REAL].owner->_state != TRIS_STATE_UP)
							p->dialing = 1;
						dahdi_ring_phone(p);
						...
						while (p->subs[SUB_THREEWAY].owner && tris_channel_trylock(p->subs[SUB_THREEWAY].owner)) {
							...
							DLA_LOCK(&p->lock);
							if (p->owner != ast) {
								tris_log(LOG_WARNING, "This isn't good...\n");
								return NULL;
							}
						}
						if (!p->subs[SUB_THREEWAY].owner) {
							tris_log(LOG_NOTICE, "Whoa, threeway disappeared kinda randomly.\n");
							return NULL;
						}
						mssinceflash = tris_tvdiff_ms(tris_tvnow(), p->flashtime);
						tris_debug(1, "Last flash was %d ms ago\n", mssinceflash);
						if (mssinceflash < MIN_MS_SINCE_FLASH) {
							/* It hasn't been long enough since the last flashook.  This is probably a bounce on
							   hanging up.  Hangup both channels now */
							if (p->subs[SUB_THREEWAY].owner)
								tris_queue_hangup_with_cause(p->subs[SUB_THREEWAY].owner, TRIS_CAUSE_NO_ANSWER);
							p->subs[SUB_THREEWAY].owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
							tris_debug(1, "Looks like a bounced flash, hanging up both calls on %d\n", p->channel);
							tris_channel_unlock(p->subs[SUB_THREEWAY].owner);
						} else if ((ast->pbx) || (ast->_state == TRIS_STATE_UP)) {
							if (p->transfer) {
								/* In any case this isn't a threeway call anymore */
								p->subs[SUB_REAL].inthreeway = 0;
								p->subs[SUB_THREEWAY].inthreeway = 0;
								/* Only attempt transfer if the phone is ringing; why transfer to busy tone eh? */
								if (!p->transfertobusy && ast->_state == TRIS_STATE_BUSY) {
									tris_channel_unlock(p->subs[SUB_THREEWAY].owner);
					...
					tris_log(LOG_WARNING, "Got a hangup and my index is %d?\n", idx);
				}
				/* Fall through */
			default:
				dahdi_disable_ec(p);
		...
		case DAHDI_EVENT_RINGOFFHOOK:
			...
			{
				p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
				p->subs[idx].f.subclass = TRIS_CONTROL_RADIO_KEY;
				break;
 			}
			/* for E911, its supposed to wait for offhook then dial
			   the second half of the dial string */
			if (((mysig == SIG_E911) || (mysig == SIG_FGC_CAMA) || (mysig == SIG_FGC_CAMAMF)) && (ast->_state == TRIS_STATE_DIALING_OFFHOOK)) {
				c = strchr(p->dialdest, '/');
				if (c)
					c++;
				else
				...
				else tris_copy_string(p->dop.dialstr,"M*2#", sizeof(p->dop.dialstr));
			...
			case SIG_FXOKS:
				...
				case TRIS_STATE_RINGING:
					...
					tris_debug(1, "channel %d answered\n", p->channel);
					if (p->cidspill) {
					...
					if (p->confirmanswer) {
						...
						p->subs[idx].f.frametype = TRIS_FRAME_NULL;
						p->subs[idx].f.subclass = 0;
					} else if (!tris_strlen_zero(p->dop.dialstr)) {
						/* nick@dccinc.com 4/3/03 - fxo should be able to do deferred dialing */
						res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_DIAL, &p->dop);
						if (res < 0) {
							tris_log(LOG_WARNING, "Unable to initiate dialing on trunk channel %d: %s\n", p->channel, strerror(errno));
							p->dop.dialstr[0] = '\0';
							return NULL;
						} else {
							tris_debug(1, "Sent FXO deferred digit string: %s\n", p->dop.dialstr);
							p->subs[idx].f.frametype = TRIS_FRAME_NULL;
							p->subs[idx].f.subclass = 0;
							p->dialing = 1;
						}
				...
				case TRIS_STATE_DOWN:
					...
					ast->rings = 1;
					p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
					p->subs[idx].f.subclass = TRIS_CONTROL_OFFHOOK;
					tris_debug(1, "channel %d picked up\n", p->channel);
					return &p->subs[idx].f;
				case TRIS_STATE_UP:
					/* Make sure it stops ringing */
					dahdi_set_hook(p->subs[idx].dfd, DAHDI_OFFHOOK);
				...
				case TRIS_STATE_RESERVED:
					...
					break;
				default:
					tris_log(LOG_WARNING, "FXO phone off hook in weird state %d??\n", ast->_state);
				}
				break;
			case SIG_FXSLS:
			case SIG_FXSGS:
			case SIG_FXSKS:
				...
				tris_debug(1, "Setting IDLE polarity due "
			...
			case SIG_SF_FEATDMF:
			case SIG_SF_FEATB:
				if (ast->_state == TRIS_STATE_PRERING)
					tris_setstate(ast, TRIS_STATE_RING);
				if ((ast->_state == TRIS_STATE_DOWN) || (ast->_state == TRIS_STATE_RING)) {
					tris_debug(1, "Ring detected\n");
					p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
					p->subs[idx].f.subclass = TRIS_CONTROL_RING;
				} else if (p->outgoing && ((ast->_state == TRIS_STATE_RINGING) || (ast->_state == TRIS_STATE_DIALING))) {
					tris_debug(1, "Line answered\n");
					if (p->confirmanswer) {
						p->subs[idx].f.frametype = TRIS_FRAME_NULL;
						p->subs[idx].f.subclass = 0;
					} else {
				...
				} else if (ast->_state != TRIS_STATE_RING)
					tris_log(LOG_WARNING, "Ring/Off-hook in strange state %d on channel %d\n", ast->_state, p->channel);
				break;
			default:
				tris_log(LOG_WARNING, "Don't know how to handle ring/off hook for signalling %d\n", p->sig);
			}
			break;
		...
		case DAHDI_EVENT_RINGEROFF:
			...
			if ((ast->rings > p->cidrings) && (p->cidspill)) {
				tris_log(LOG_WARNING, "Didn't finish Caller-ID spill.  Cancelling.\n");
				tris_free(p->cidspill);
				p->cidspill = NULL;
				p->callwaitcas = 0;
			}
			p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
			if (p->sig == SIG_FXOLS)
				p->subs[idx].f.subclass = TRIS_CONTROL_PROGRESS;
			else
				p->subs[idx].f.subclass = TRIS_CONTROL_RINGING;
			break;
		...
		case DAHDI_EVENT_WINKFLASH:
			...
			p->flashtime = tris_tvnow();
			switch (mysig) {
			case SIG_FXOLS:
			case SIG_FXOGS:
			case SIG_FXOKS:
				tris_debug(1, "Winkflash, index: %d, normal: %d, callwait: %d, thirdcall: %d\n",
					idx, p->subs[SUB_REAL].dfd, p->subs[SUB_CALLWAIT].dfd, p->subs[SUB_THREEWAY].dfd);
				p->callwaitcas = 0;

				if (idx != SUB_REAL) {
					tris_log(LOG_WARNING, "Got flash hook with index %d on channel %d?!?\n", idx, p->channel);
					goto winkflashdone;
				}
				...
				if (p->subs[SUB_CALLWAIT].owner) {
					...
					tone_zone_play_tone(p->subs[SUB_REAL].dfd, -1);
					p->owner = p->subs[SUB_REAL].owner;
					tris_debug(1, "Making %s the new owner\n", p->owner->name);
					if (p->owner->_state == TRIS_STATE_RINGING) {
						tris_setstate(p->owner, TRIS_STATE_UP);
						p->subs[SUB_REAL].needanswer = 1;
					}
					p->callwaitingrepeat = 0;
					p->cidcwexpire = 0;
					/* Start music on hold if appropriate */
					if (!p->subs[SUB_CALLWAIT].inthreeway && tris_bridged_channel(p->subs[SUB_CALLWAIT].owner)) {
						tris_queue_control_data(p->subs[SUB_CALLWAIT].owner, TRIS_CONTROL_HOLD,
							S_OR(p->mohsuggest, NULL),
							!tris_strlen_zero(p->mohsuggest) ? strlen(p->mohsuggest) + 1 : 0);
					}
					...
					if (tris_bridged_channel(p->subs[SUB_REAL].owner)) {
						tris_queue_control_data(p->subs[SUB_REAL].owner, TRIS_CONTROL_HOLD,
							S_OR(p->mohsuggest, NULL),
							!tris_strlen_zero(p->mohsuggest) ? strlen(p->mohsuggest) + 1 : 0);
					}
					...
					if (!p->threewaycalling) {
						...
						if (p->dahditrcallerid && p->owner) {
							if (p->owner->cid.cid_num)
								tris_copy_string(cid_num, p->owner->cid.cid_num, sizeof(cid_num));
							if (p->owner->cid.cid_name)
								tris_copy_string(cid_name, p->owner->cid.cid_name, sizeof(cid_name));
						}
						/* XXX This section needs much more error checking!!! XXX */
						/* Start a 3-way call if feasible */
						if (!((ast->pbx) ||
							(ast->_state == TRIS_STATE_UP) ||
							(ast->_state == TRIS_STATE_RING))) {
							tris_debug(1, "Flash when call not up or ringing\n");
							goto winkflashdone;
						}
						if (alloc_sub(p, SUB_THREEWAY)) {
							tris_log(LOG_WARNING, "Unable to allocate three-way subchannel\n");
							goto winkflashdone;
						}
						/* Make new channel */
						chan = dahdi_new(p, TRIS_STATE_RESERVED, 0, SUB_THREEWAY, 0, 0);
						if (p->dahditrcallerid) {
							if (!p->origcid_num)
								p->origcid_num = tris_strdup(p->cid_num);
							if (!p->origcid_name)
								p->origcid_name = tris_strdup(p->cid_name);
							tris_copy_string(p->cid_num, cid_num, sizeof(p->cid_num));
							tris_copy_string(p->cid_name, cid_name, sizeof(p->cid_name));
						}
						...
						if (res)
							tris_log(LOG_WARNING, "Unable to start dial recall tone on channel %d\n", p->channel);
						p->owner = chan;
						if (!chan) {
							tris_log(LOG_WARNING, "Cannot allocate new structure on channel %d\n", p->channel);
						} else if (tris_pthread_create_detached(&threadid, NULL, ss_thread, chan)) {
							tris_log(LOG_WARNING, "Unable to start simple switch on channel %d\n", p->channel);
							res = tone_zone_play_tone(p->subs[SUB_REAL].dfd, DAHDI_TONE_CONGESTION);
							dahdi_enable_ec(p);
							tris_hangup(chan);
						} else {
 							struct tris_channel *other = tris_bridged_channel(p->subs[SUB_THREEWAY].owner);
 							int way3bridge = 0, cdr3way = 0;

 							if (!other) {
 								other = tris_bridged_channel(p->subs[SUB_REAL].owner);
 							} else
 								way3bridge = 1;
							...
							tris_verb(3, "Started three way call on channel %d\n", p->channel);
					...
					if (p->subs[SUB_THREEWAY].inthreeway) {
						...
						tris_debug(1, "Got flash with three way call up, dropping last call on %d\n", p->channel);
						/* If the primary call isn't answered yet, use it */
						if ((p->subs[SUB_REAL].owner->_state != TRIS_STATE_UP) && (p->subs[SUB_THREEWAY].owner->_state == TRIS_STATE_UP)) {
							/* Swap back -- we're dropping the real 3-way that isn't finished yet*/
							swap_subs(p, SUB_THREEWAY, SUB_REAL);
							p->owner = p->subs[SUB_REAL].owner;
						}
						/* Drop the last call and stop the conference */
						tris_verb(3, "Dropping three-way call on %s\n", p->subs[SUB_THREEWAY].owner->name);
						p->subs[SUB_THREEWAY].owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
						p->subs[SUB_REAL].inthreeway = 0;
						p->subs[SUB_THREEWAY].inthreeway = 0;
					} else {
						/* Lets see what we're up to */
						if (((ast->pbx) || (ast->_state == TRIS_STATE_UP)) &&
							(p->transfertobusy || (ast->_state != TRIS_STATE_BUSY))) {
							...
							tris_verb(3, "Building conference on call on %s and %s\n", p->subs[SUB_THREEWAY].owner->name, p->subs[SUB_REAL].owner->name);
							...
							if (p->subs[otherindex].owner && tris_bridged_channel(p->subs[otherindex].owner))
								tris_queue_control(p->subs[otherindex].owner, TRIS_CONTROL_UNHOLD);
							p->subs[otherindex].needunhold = 1;
							p->owner = p->subs[SUB_REAL].owner;
							if (ast->_state == TRIS_STATE_RINGING) {
								tris_debug(1, "Enabling ringtone on real and threeway\n");
							...
							tris_verb(3, "Dumping incomplete call on on %s\n", p->subs[SUB_THREEWAY].owner->name);
							swap_subs(p, SUB_THREEWAY, SUB_REAL);
							p->subs[SUB_THREEWAY].owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
							p->owner = p->subs[SUB_REAL].owner;
							if (p->subs[SUB_REAL].owner && tris_bridged_channel(p->subs[SUB_REAL].owner))
								tris_queue_control(p->subs[SUB_REAL].owner, TRIS_CONTROL_UNHOLD);
							p->subs[SUB_REAL].needunhold = 1;
							dahdi_enable_ec(p);
						}
...
winkflashdone:
			...
			case SIG_FXSGS:
				if (option_debug) {
					if (p->dialing)
						tris_debug(1, "Ignoring wink on channel %d\n", p->channel);
					else
						tris_debug(1, "Got wink in weird state %d on channel %d\n", ast->_state, p->channel);
				}
				break;
			case SIG_FEATDMF_TA:
				switch (p->whichwink) {
				case 0:
					tris_debug(1, "ANI2 set to '%d' and ANI is '%s'\n", p->owner->cid.cid_ani2, p->owner->cid.cid_ani);
					snprintf(p->dop.dialstr, sizeof(p->dop.dialstr), "M*%d%s#", p->owner->cid.cid_ani2, p->owner->cid.cid_ani);
					break;
				case 1:
					tris_copy_string(p->dop.dialstr, p->finaldial, sizeof(p->dop.dialstr));
					break;
				case 2:
					tris_log(LOG_WARNING, "Received unexpected wink on channel of type SIG_FEATDMF_TA\n");
					return NULL;
				}
				p->whichwink++;
				/* Fall through */
			...
			case SIG_EMWINK:
				...
				if (!tris_strlen_zero(p->dop.dialstr)) {
					res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_DIAL, &p->dop);
					if (res < 0) {
						tris_log(LOG_WARNING, "Unable to initiate dialing on trunk channel %d: %s\n", p->channel, strerror(errno));
						p->dop.dialstr[0] = '\0';
						...
						tris_debug(1, "Sent deferred digit string: %s\n", p->dop.dialstr);
				}
				p->dop.dialstr[0] = '\0';
				break;
			default:
		...
		case DAHDI_EVENT_HOOKCOMPLETE:
			...
			case SIG_SF_FEATDMF:
			case SIG_SF_FEATB:
				tris_debug(1, "Got hook complete in MF FGD, waiting for wink now on channel %d\n",p->channel);
				break;
			default:
				break;
			}
		...
		case DAHDI_EVENT_POLARITY:
			...
			if (p->polarityonanswerdelay > 0) {
				...
				if (tris_tvdiff_ms(tris_tvnow(), p->polaritydelaytv) > p->polarityonanswerdelay) {
					switch (ast->_state) {
					case TRIS_STATE_DIALING:			/*!< Digits (or equivalent) have been dialed */
					case TRIS_STATE_RINGING:			/*!< Remote end is ringing */
						if (p->answeronpolarityswitch) {
							tris_debug(1, "Answering on polarity switch! channel %d\n", p->channel);
							tris_setstate(p->owner, TRIS_STATE_UP);
							p->polarity = POLARITY_REV;
							if (p->hanguponpolarityswitch) {
								p->polaritydelaytv = tris_tvnow();
							...
							tris_debug(1, "Ignore Answer on polarity switch, channel %d\n", p->channel);
						}
						break;

					case TRIS_STATE_UP:			/*!< Line is up */
					case TRIS_STATE_RING:			/*!< Line is ringing */
						if (p->hanguponpolarityswitch) {
							tris_debug(1, "HangingUp on polarity switch! channel %d\n", p->channel);
							tris_softhangup(p->owner, TRIS_SOFTHANGUP_EXPLICIT);
							p->polarity = POLARITY_IDLE;
						} else {
							tris_debug(1, "Ignore Hangup on polarity switch, channel %d\n", p->channel);
						}
						break;

					case TRIS_STATE_DOWN:			/*!< Channel is down and available */
					case TRIS_STATE_RESERVED:		/*!< Channel is down, but reserved */
					case TRIS_STATE_OFFHOOK:			/*!< Channel is off hook */
					case TRIS_STATE_BUSY:			/*!< Line is busy */
					case TRIS_STATE_DIALING_OFFHOOK:		/*!< Digits (or equivalent) have been dialed while offhook */
					case TRIS_STATE_PRERING:			/*!< Channel has detected an incoming call and is waiting for ring */
					default:
						if (p->answeronpolarityswitch || p->hanguponpolarityswitch) {
							tris_debug(1, "Ignoring Polarity switch on channel %d, state %d\n", p->channel, ast->_state);
						}
					}
					...
					case TRIS_STATE_RINGING:			/*!< Remote end is ringing */
						if (p->answeronpolarityswitch) {
							tris_debug(1, "Polarity switch detected but NOT answering (too close to OffHook event) on channel %d, state %d\n", p->channel, ast->_state);
						}
						break;
					...
					case TRIS_STATE_RING:			/*!< Line is ringing */
						if (p->hanguponpolarityswitch) {
							tris_debug(1, "Polarity switch detected but NOT hanging up (too close to Answer event) on channel %d, state %d\n", p->channel, ast->_state);
						}
						break;

					default:	
						if (p->answeronpolarityswitch || p->hanguponpolarityswitch) {
							tris_debug(1, "Polarity switch detected (too close to previous event) on channel %d, state %d\n", p->channel, ast->_state);
						}
					}
				}
			}
			...
			tris_debug(1, "Polarity Reversal event occured - DEBUG 2: channel %d, state %d, pol= %d, aonp= %d, honp= %d, pdelay= %d, tv= %d\n", p->channel, ast->_state, p->polarity, p->answeronpolarityswitch, p->hanguponpolarityswitch, p->polarityonanswerdelay, tris_tvdiff_ms(tris_tvnow(), p->polaritydelaytv) );
			break;
		default:
			tris_debug(1, "Dunno what to do with event %d on channel %d\n", res, p->channel);
	}
	return &p->subs[idx].f;
}

static struct tris_frame *__dahdi_exception(struct tris_channel *ast)
{
	struct dahdi_pvt *p = ast->tech_pvt;
	int res;
	int usedindex=-1;
	...
	p->subs[idx].f.offset = 0;
	p->subs[idx].f.subclass = 0;
	p->subs[idx].f.delivery = tris_tv(0,0);
	...
	if ((!p->owner) && (!(p->radio || (p->oprmode < 0)))) {
		...
		if ((res != DAHDI_EVENT_RINGEROFF) && (res != DAHDI_EVENT_RINGERON) &&
			...
			tris_debug(1, "Restoring owner of channel %d on event %d\n", p->channel, res);
			p->owner = p->subs[SUB_REAL].owner;
		...
		case DAHDI_EVENT_ONHOOK:
			dahdi_disable_ec(p);
			if (p->owner) {
				tris_verb(3, "Channel %s still has call, ringing phone\n", p->owner->name);
				dahdi_ring_phone(p);
				p->callwaitingrepeat = 0;
				p->cidcwexpire = 0;
			} else
				tris_log(LOG_WARNING, "Absorbed on hook, but nobody is left!?!?\n");
			update_conf(p);
			break;
		case DAHDI_EVENT_RINGOFFHOOK:
			dahdi_enable_ec(p);
			dahdi_set_hook(p->subs[SUB_REAL].dfd, DAHDI_OFFHOOK);
			if (p->owner && (p->owner->_state == TRIS_STATE_RINGING)) {
				p->subs[SUB_REAL].needanswer = 1;
				p->dialing = 0;
			}
			break;
		...
		case DAHDI_EVENT_WINKFLASH:
			p->flashtime = tris_tvnow();
			if (p->owner) {
				tris_verb(3, "Channel %d flashed to other channel %s\n", p->channel, p->owner->name);
				if (p->owner->_state != TRIS_STATE_UP) {
					/* Answer if necessary */
			...
			break;
		default:
			tris_log(LOG_WARNING, "Don't know how to absorb event %s\n", event2str(res));
		}
		f = &p->subs[idx].f;
		return f;
	}
	if (!(p->radio || (p->oprmode < 0)))
		tris_debug(1, "Exception on %d, channel %d\n", ast->fds[0],p->channel);
	/* If it's not us, return NULL immediately */
	if (ast != p->owner) {
		tris_log(LOG_WARNING, "We're %s, not %s\n", ast->name, p->owner->name);
		f = &p->subs[idx].f;
		return f;
	}
...
static struct tris_frame *dahdi_read(struct tris_channel *ast)
{
	...
	if (idx < 0)	{
		tris_log(LOG_WARNING, "We dont exist?\n");
		tris_mutex_unlock(&p->lock);
		return NULL;
	}
	...
	if (p->subs[idx].needbusy) {
		...
		p->subs[idx].needbusy = 0;
		p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
		p->subs[idx].f.subclass = TRIS_CONTROL_BUSY;
		tris_mutex_unlock(&p->lock);
		return &p->subs[idx].f;
	}
	...
	if (p->subs[idx].needcongestion) {
		...
		p->subs[idx].needcongestion = 0;
		p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
		p->subs[idx].f.subclass = TRIS_CONTROL_CONGESTION;
		tris_mutex_unlock(&p->lock);
		return &p->subs[idx].f;
	}

	if (p->subs[idx].needcallerid) {
		tris_set_callerid(ast, S_OR(p->lastcid_num, NULL),
	...
	if (p->subs[idx].needflash) {
		...
		p->subs[idx].needflash = 0;
		p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
		p->subs[idx].f.subclass = TRIS_CONTROL_FLASH;
		tris_mutex_unlock(&p->lock);
		return &p->subs[idx].f;
	}
	...
	if (p->subs[idx].needhold) {
		...
		p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
		p->subs[idx].f.subclass = TRIS_CONTROL_HOLD;
		tris_mutex_unlock(&p->lock);
		tris_debug(1, "Sending hold on '%s'\n", ast->name);
		return &p->subs[idx].f;
	}
	...
	if (p->subs[idx].needunhold) {
		...
		p->subs[idx].f.frametype = TRIS_FRAME_CONTROL;
		p->subs[idx].f.subclass = TRIS_CONTROL_UNHOLD;
		tris_mutex_unlock(&p->lock);
		tris_debug(1, "Sending unhold on '%s'\n", ast->name);
		return &p->subs[idx].f;
	}

	if (ast->rawreadformat == TRIS_FORMAT_SLINEAR) {
		if (!p->subs[idx].linear) {
			p->subs[idx].linear = 1;
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				tris_log(LOG_WARNING, "Unable to set channel %d (index %d) to linear mode.\n", p->channel, idx);
		}
	} else if ((ast->rawreadformat == TRIS_FORMAT_ULAW) ||
		(ast->rawreadformat == TRIS_FORMAT_ALAW)) {
		if (p->subs[idx].linear) {
			p->subs[idx].linear = 0;
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				tris_log(LOG_WARNING, "Unable to set channel %d (index %d) to companded mode.\n", p->channel, idx);
		}
	} else {
		tris_log(LOG_WARNING, "Don't know how to read frames in format %s\n", tris_getformatname(ast->rawreadformat));
		tris_mutex_unlock(&p->lock);
		return NULL;
	}
	readbuf = ((unsigned char *)p->subs[idx].buffer) + TRIS_FRIENDLY_OFFSET;
	CHECK_BLOCKING(ast);
	res = read(p->subs[idx].dfd, readbuf, p->subs[idx].linear ? READ_SIZE * 2 : READ_SIZE);
	tris_clear_flag(ast, TRIS_FLAG_BLOCKING);
	/* Check for hangup */
	if (res < 0) {
		f = NULL;
		if (res == -1) {
			if (errno == EAGAIN) {
				...
				tris_log(LOG_WARNING, "dahdi_rec: %s\n", strerror(errno));
		}
		tris_mutex_unlock(&p->lock);
		return f;
	}
	if (res != (p->subs[idx].linear ? READ_SIZE * 2 : READ_SIZE)) {
		tris_debug(1, "Short read (%d/%d), must be an event...\n", res, p->subs[idx].linear ? READ_SIZE * 2 : READ_SIZE);
		f = __dahdi_exception(ast);
		tris_mutex_unlock(&p->lock);
		return f;
	}
	if (p->tdd) { /* if in TDD mode, see if we receive that */
		...
		if (c < 0) {
			tris_debug(1,"tdd_feed failed\n");
			tris_mutex_unlock(&p->lock);
			return NULL;
		}
		if (c) { /* if a char to return */
			p->subs[idx].f.subclass = 0;
			p->subs[idx].f.frametype = TRIS_FRAME_TEXT;
			p->subs[idx].f.mallocd = 0;
			p->subs[idx].f.offset = TRIS_FRIENDLY_OFFSET;
			p->subs[idx].f.data.ptr = p->subs[idx].buffer + TRIS_FRIENDLY_OFFSET;
	...
	/* Expire CID/CW */
	if (p->cidcwexpire == 1) {
		tris_verb(3, "CPE does not support Call Waiting Caller*ID.\n");
		restore_conference(p);
	}
	if (p->subs[idx].linear) {
	...
	if ((p->owner == ast) && p->cidspill &&((ast->_state == TRIS_STATE_UP) || (ast->rings == p->cidrings))) {
		send_callerid(p);
	}

	p->subs[idx].f.frametype = TRIS_FRAME_VOICE;
	p->subs[idx].f.subclass = ast->rawreadformat;
	p->subs[idx].f.samples = READ_SIZE;
	p->subs[idx].f.mallocd = 0;
	p->subs[idx].f.offset = TRIS_FRIENDLY_OFFSET;
	p->subs[idx].f.data.ptr = p->subs[idx].buffer + TRIS_FRIENDLY_OFFSET / sizeof(p->subs[idx].buffer[0]);
#if 0
	tris_debug(1, "Read %d of voice on %s\n", p->subs[idx].f.datalen, ast->name);
#endif
	if (p->dialing || /* Transmitting something */
		(idx && (ast->_state != TRIS_STATE_UP)) || /* Three-way or callwait that isn't up */
	...
	if (p->dsp && (!p->ignoredtmf || p->callwaitcas || p->busydetect || p->callprogress || p->waitingfordt.tv_sec) && !idx) {
		...
		if (f) {
			if ((f->frametype == TRIS_FRAME_CONTROL) && (f->subclass == TRIS_CONTROL_BUSY)) {
				if ((ast->_state == TRIS_STATE_UP) && !p->outgoing) {
				...
				if (tris_tvdiff_ms(tris_tvnow(), p->waitingfordt) >= p->waitfordialtone ) {
					p->waitingfordt.tv_sec = 0;
					tris_log(LOG_WARNING, "Never saw dialtone on channel %d\n", p->channel);
					f=NULL;
				} else if (f->frametype == TRIS_FRAME_VOICE) {
					f->frametype = TRIS_FRAME_NULL;
					f->subclass = 0;
					if ((tris_dsp_get_tstate(p->dsp) == DSP_TONE_STATE_DIALTONE || tris_dsp_get_tstate(p->dsp) == DSP_TONE_STATE_RINGING) && tris_dsp_get_tcount(p->dsp) > 9) {
						p->waitingfordt.tv_sec = 0;
						p->dsp_features &= ~DSP_FEATURE_WAITDIALTONE;
						tris_dsp_set_features(p->dsp, p->dsp_features);
						tris_log(LOG_DEBUG, "Got 10 samples of dialtone!\n");
						if (!tris_strlen_zero(p->dop.dialstr)) { /* Dial deferred digits */
							res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_DIAL, &p->dop);
							if (res < 0) {
								tris_log(LOG_WARNING, "Unable to initiate dialing on trunk channel %d\n", p->channel);
								p->dop.dialstr[0] = '\0';
								tris_mutex_unlock(&p->lock);
								return NULL;
							} else {
								tris_log(LOG_DEBUG, "Sent deferred digit string: %s\n", p->dop.dialstr);
								p->dialing = 1;
								p->dop.dialstr[0] = '\0';
								p->dop.op = DAHDI_DIAL_OP_REPLACE;
								tris_setstate(ast, TRIS_STATE_DIALING);
	...
	if (p->fake_event)
		tris_set_flag(ast, TRIS_FLAG_EXCEPTION);
...
static int dahdi_write(struct tris_channel *ast, struct tris_frame *frame)
{
	...
	idx = dahdi_get_index(ast, p, 0);
	if (idx < 0) {
		tris_log(LOG_WARNING, "%s doesn't really exist?\n", ast->name);
		return -1;
	}
	...
	if ((frame->subclass != TRIS_FORMAT_SLINEAR) &&
		(frame->subclass != TRIS_FORMAT_ULAW) &&
		(frame->subclass != TRIS_FORMAT_ALAW)) {
		tris_log(LOG_WARNING, "Cannot handle frames in %d format\n", frame->subclass);
		return -1;
	}
	if (p->dialing) {
		tris_debug(1, "Dropping frame since I'm still dialing on %s...\n",ast->name);
		return 0;
	}
	if (!p->owner) {
		tris_debug(1, "Dropping frame since there is no active owner on %s...\n",ast->name);
		return 0;
	}
	if (p->cidspill) {
		tris_debug(1, "Dropping frame since I've still got a callerid spill\n");
		return 0;
	}
	...
	if (frame->subclass == TRIS_FORMAT_SLINEAR) {
		if (!p->subs[idx].linear) {
			...
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				tris_log(LOG_WARNING, "Unable to set linear mode on channel %d\n", p->channel);
		}
		...
		if (p->subs[idx].linear) {
			...
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				tris_log(LOG_WARNING, "Unable to set companded mode on channel %d\n", p->channel);
		}
...
static int dahdi_indicate(struct tris_channel *chan, int condition, const void *data, size_t datalen)
{
	...
	tris_mutex_lock(&p->lock);
	idx = dahdi_get_index(chan, p, 0);
	tris_debug(1, "Requested indication %d on channel %s\n", condition, chan->name);
#ifdef HAVE_OPENR2
	...
	if (idx == SUB_REAL) {
		...
		case TRIS_CONTROL_BUSY:
			...
			if (p->priindication_oob && ((p->sig == SIG_PRI) || (p->sig == SIG_BRI) || (p->sig == SIG_BRI_PTMP))) {
				chan->hangupcause = TRIS_CAUSE_USER_BUSY;
		...
		case TRIS_CONTROL_RINGING:
			...
			if ((!p->alerting) && ((p->sig == SIG_PRI) || (p->sig == SIG_BRI) || (p->sig == SIG_BRI_PTMP))
					&& p->pri && !p->outgoing && (chan->_state != TRIS_STATE_UP)) {
				if (p->pri->pri) {
					if (!pri_grab(p, p->pri)) {
...
#endif
#ifdef HAVE_SS7
			if ((!p->alerting) && (p->sig == SIG_SS7) && p->ss7 && !p->outgoing && (chan->_state != TRIS_STATE_UP)) {
				if (p->ss7->ss7) {
					ss7_grab(p, p->ss7);
			...
			if (chan->_state != TRIS_STATE_UP) {
				if ((chan->_state != TRIS_STATE_RING) ||
				 ...
				 (p->sig != SIG_FXSLS) &&
				 (p->sig != SIG_FXSGS)))
				tris_setstate(chan, TRIS_STATE_RINGING);
			}
			break;
		case TRIS_CONTROL_PROCEEDING:
			tris_debug(1,"Received TRIS_CONTROL_PROCEEDING on %s\n",chan->name);
#ifdef HAVE_PRI
			...
			if (chan->_state == TRIS_STATE_UP && (p->rlt != 1) && (p->sig == SIG_SS7)){
				if ((isup_far(p->ss7->ss7, p->ss7call)) != -1)
					p->rlt = 1;
			}
...
#endif
			/* don't continue in tris_indicate */
			res = 0;
			break;
		case TRIS_CONTROL_PROGRESS:
			tris_debug(1,"Received TRIS_CONTROL_PROGRESS on %s\n",chan->name);
#ifdef HAVE_PRI
...
#endif
#ifdef HAVE_DPNSS
			if (!p->progress && p->sig==SIG_DPNSS && p->dpnss && !p->outgoing) {
				if (p->dpnss->dpnss) {
					dpnss_grab(p, p->dpnss);
					p->progress = 1;
					dpnss_chan_send_ccm(p->dpnss->dpnss, p->dpnsschan);
					dpnss_rel(p->dpnss);
					dahdi_enable_ec(p);
		...
		case TRIS_CONTROL_CONGESTION:
			chan->hangupcause = TRIS_CAUSE_CONGESTION;
#ifdef HAVE_PRI
			if (p->priindication_oob && ((p->sig == SIG_PRI) || (p->sig == SIG_BRI) || (p->sig == SIG_BRI_PTMP))) {
				chan->hangupcause = TRIS_CAUSE_SWITCH_CONGESTION;
				chan->_softhangup |= TRIS_SOFTHANGUP_DEV;
				res = 0;
...
#endif
				tris_moh_start(chan, data, p->mohinterpret);
			break;
		...
		case TRIS_CONTROL_FLASH:
			...
			if (ISTRUNK(p) && (p->sig != SIG_PRI)) {
				...
				if ((ioctl(p->subs[SUB_REAL].dfd,DAHDI_HOOK,&func) == -1) && (errno != EINPROGRESS)) {
					tris_log(LOG_WARNING, "Unable to flash external trunk on channel %s: %s\n",
...
static struct tris_channel *dahdi_new(struct dahdi_pvt *i, int state, int startpbx, int idx, int law, int transfercapability)
{
	...
	int x,y;
	int features;
	struct tris_str *chan_name;
	struct tris_variable *v;
	struct dahdi_params ps;
	if (i->subs[idx].owner) {
		tris_log(LOG_WARNING, "Channel %d already has a %s call\n", i->channel,subnames[idx]);
		return NULL;
	}
	y = 1;
	chan_name = tris_str_alloca(32);
	do {
#ifdef HAVE_PRI
		if (i->bearer || (i->pri && (i->sig == SIG_FXSKS)))
			tris_str_set(&chan_name, 0, "%d:%d-%d", i->pri->trunkgroup, i->channel, y);
		else
#endif
		if (i->channel == CHAN_PSEUDO)
			tris_str_set(&chan_name, 0, "pseudo-%ld", tris_random());
		else
			tris_str_set(&chan_name, 0, "%d-%d", i->channel, y);
		for (x = 0; x < 3; x++) {
			if ((idx != x) && i->subs[x].owner && !strcasecmp(tris_str_buffer(chan_name), i->subs[x].owner->name + 6))
				break;
		}
		y++;
	} while (x < 3);
	tmp = tris_channel_alloc(0, state, i->cid_num, i->cid_name, i->accountcode, i->exten, i->context, i->amaflags, "DAHDI/%s", tris_str_buffer(chan_name));
	if (!tmp)
		return NULL;
	tmp->tech = &dahdi_tech;
	memset(&ps, 0, sizeof(ps));
	...
	if (res) {
		tris_log(LOG_WARNING, "Unable to get parameters, assuming MULAW: %s\n", strerror(errno));
		ps.curlaw = DAHDI_LAW_MULAW;
	}
	if (ps.curlaw == DAHDI_LAW_ALAW)
		deflaw = TRIS_FORMAT_ALAW;
	else
		deflaw = TRIS_FORMAT_ULAW;
	if (law) {
	...
	tris_channel_set_fd(tmp, 0, i->subs[idx].dfd);
	...
	if (features) {
		if (i->dsp) {
			tris_debug(1, "Already have a dsp on %s?\n", tmp->name);
		} else {
			if (i->channel != CHAN_PSEUDO)
				i->dsp = tris_dsp_new();
			else
			...
			if (i->dsp) {
				...
				if (i->outgoing && ((i->sig == SIG_PRI) || (i->sig == SIG_BRI) || (i->sig == SIG_BRI_PTMP) || (i->sig == SIG_SS7) ||  (i->sig == SIG_DPNSS))) {
				...
				tris_dsp_set_digitmode(i->dsp, DSP_DIGITMODE_DTMF | i->dtmfrelax);
				if (!tris_strlen_zero(progzone))
					tris_dsp_set_call_progress_zone(i->dsp, progzone);
				if (i->busydetect && CANBUSYDETECT(i)) {
					tris_dsp_set_busy_count(i->dsp, i->busycount);
					tris_dsp_set_busy_pattern(i->dsp, i->busy_tonelength, i->busy_quietlength);
				}
			}
		}
	}
	...
	pbx_builtin_setvar_helper(tmp, "TRANSFERCAPABILITY", tris_transfercapability2str(transfercapability));
	if (transfercapability & TRIS_TRANS_CAP_DIGITAL)
		i->digital = 1;
	/* Assume calls are not idle calls unless we're told differently */
	i->isidlecall = 0;
	i->alreadyhungup = 0;
#endif
	/* clear the fake event in case we posted one before we had tris_channel */
	i->fake_event = 0;
	...
	tris_devstate_changed_literal(tris_state_chan2dev(state), tmp->name);
...
static int my_getsigstr(struct tris_channel *chan, char *str, const char *term, int ms)
{
	char c;
...
static void dahdi_dnd(struct dahdi_pvt *dahdichan, int on)
{
	/* Do not disturb */
	dahdichan->dnd = on;
	tris_verb(3, "%s DND on channel %d\n",
			on? "Enabled" : "Disabled",
			dahdichan->channel);
...
static void *ss_thread(void *data)
{
	...
	struct dahdi_pvt *p = chan->tech_pvt;
	char exten[TRIS_MAX_EXTENSION] = "";
	char exten2[TRIS_MAX_EXTENSION] = "";
	...
	int counter;
	int samples = 0;
	struct tris_smdi_md_message *smdi_msg = NULL;
	int flags = 0;
	int i;
	int timeout;
	int getforward = 0;
	...
	tris_mutex_lock(&ss_thread_lock);
	ss_thread_count++;
	tris_mutex_unlock(&ss_thread_lock);
	/* in the bizarre case where the channel has become a zombie before we
	   even get started here, abort safely
	*/
	if (!p) {
		tris_log(LOG_WARNING, "Channel became a zombie before simple switch could be started (%s)\n", chan->name);
		tris_hangup(chan);
		goto quit;
	}
	tris_verb(3, "Starting simple switch on '%s'\n", chan->name);
	...
	case SIG_BRI_PTMP:
		...
		tris_copy_string(exten, p->exten, sizeof(exten));
		len = strlen(exten);
		res = 0;
		while ((len < TRIS_MAX_EXTENSION-1) && tris_matchmore_extension(chan, chan->context, exten, 1, p->cid_num)) {
			if (len && !tris_ignore_pattern(chan->context, exten))
				tone_zone_play_tone(p->subs[idx].dfd, -1);
			else
				tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALTONE);
			if (tris_exists_extension(chan, chan->context, exten, 1, p->cid_num))
				timeout = matchdigittimeout;
			else
				timeout = gendigittimeout;
			res = tris_waitfordigit(chan, timeout);
			if (res < 0) {
				tris_debug(1, "waitfordigit returned < 0...\n");
				tris_hangup(chan);
				goto quit;
			} else if (res) {
				exten[len++] = res;
		...
		if (tris_strlen_zero(exten)) {
			tris_verb(3, "Going to extension s|1 because of empty extension received on overlap call\n");
			exten[0] = 's';
			exten[1] = '\0';
		}
		tone_zone_play_tone(p->subs[idx].dfd, -1);
		if (tris_exists_extension(chan, chan->context, exten, 1, p->cid_num)) {
			...
			tris_debug(1, "No such possible extension '%s' in context '%s'\n", exten, chan->context);
			chan->hangupcause = TRIS_CAUSE_UNALLOCATED;
			tris_hangup(chan);
			p->exten[0] = '\0';
	...
	case SIG_FGC_CAMA:
		...
		if (p->dsp) {
			if (NEED_MFDETECT(p))
				tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_MF | p->dtmfrelax);
			else
				tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_DTMF | p->dtmfrelax);
		}
		...
		if (res > 0) {
			...
			case SIG_SF_FEATD:
				...
				if ((res < 1) && (p->dsp)) tris_dsp_digitreset(p->dsp);
				break;
			case SIG_FEATDMF_TA:
				res = my_getsigstr(chan, dtmfbuf + 1, "#", 3000);
				if ((res < 1) && (p->dsp)) tris_dsp_digitreset(p->dsp);
			...
			default:
				...
				len = 1;
				dtmfbuf[len] = '\0';
				while ((len < TRIS_MAX_EXTENSION-1) && tris_matchmore_extension(chan, chan->context, dtmfbuf, 1, p->cid_num)) {
					if (tris_exists_extension(chan, chan->context, dtmfbuf, 1, p->cid_num)) {
						timeout = matchdigittimeout;
					} else {
						timeout = gendigittimeout;
					}
		...
		if (res == -1) {
			tris_log(LOG_WARNING, "getdtmf on channel %d: %s\n", p->channel, strerror(errno));
			tris_hangup(chan);
			goto quit;
		} else if (res < 0) {
			tris_debug(1, "Got hung up before digits finished\n");
			tris_hangup(chan);
			goto quit;
		}
		...
		if (p->sig == SIG_FGC_CAMA) {
			...
			if (tris_safe_sleep(chan,1000) == -1) {
				tris_hangup(chan);
				goto quit;
			}
			dahdi_set_hook(p->subs[SUB_REAL].dfd, DAHDI_OFFHOOK);
			...
			if ((res > 0) && (strlen(anibuf) > 2)) {
				...
				tris_set_callerid(chan, anibuf + 2, NULL, anibuf + 2);
			}
			tris_dsp_set_digitmode(p->dsp, DSP_DIGITMODE_DTMF | p->dtmfrelax);
		}

		tris_copy_string(exten, dtmfbuf, sizeof(exten));
		if (tris_strlen_zero(exten))
			tris_copy_string(exten, "s", sizeof(exten));
		if (p->sig == SIG_FEATD || p->sig == SIG_EMWINK) {
			/* Look for Feature Group D on all E&M Wink and Feature Group D trunks */
			if (exten[0] == '*') {
				char *stringp=NULL;
				tris_copy_string(exten2, exten, sizeof(exten2));
				/* Parse out extension and callerid */
				stringp=exten2 +1;
				s1 = strsep(&stringp, "*");
				s2 = strsep(&stringp, "*");
				if (s2) {
					if (!tris_strlen_zero(p->cid_num))
						tris_set_callerid(chan, p->cid_num, NULL, p->cid_num);
					else
						tris_set_callerid(chan, s1, NULL, s1);
					tris_copy_string(exten, s2, sizeof(exten));
				} else
					tris_copy_string(exten, s1, sizeof(exten));
			} else if (p->sig == SIG_FEATD)
				tris_log(LOG_WARNING, "Got a non-Feature Group D input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
			...
			if (exten[0] == '*') {
				...
				if (s2) {
					if (!tris_strlen_zero(p->cid_num))
						...
						if (*(s1 + 2))
							tris_set_callerid(chan, s1 + 2, NULL, s1 + 2);
					tris_copy_string(exten, s2 + 1, sizeof(exten));
				} else
					tris_copy_string(exten, s1 + 2, sizeof(exten));
			} else
				tris_log(LOG_WARNING, "Got a non-Feature Group D input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
		if ((p->sig == SIG_E911) || (p->sig == SIG_FGC_CAMAMF)) {
			if (exten[0] == '*') {
				...
				if (s2 && (*(s2 + 1) == '0')) {
					if (*(s2 + 2))
						tris_set_callerid(chan, s2 + 2, NULL, s2 + 2);
				}
				if (s1)	tris_copy_string(exten, s1, sizeof(exten));
				else tris_copy_string(exten, "911", sizeof(exten));
			} else
				tris_log(LOG_WARNING, "Got a non-E911/FGC CAMA input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
		if (p->sig == SIG_FEATB) {
			if (exten[0] == '*') {
				char *stringp=NULL;
				...
				stringp=exten2 +1;
				s1 = strsep(&stringp, "#");
				tris_copy_string(exten, exten2 + 1, sizeof(exten));
			} else
				tris_log(LOG_WARNING, "Got a non-Feature Group B input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
		if ((p->sig == SIG_FEATDMF) || (p->sig == SIG_FEATDMF_TA)) {
			dahdi_wink(p, idx);
			...
			if (tris_safe_sleep(chan,100)) goto quit;
		}
		dahdi_enable_ec(p);
		if (NEED_MFDETECT(p)) {
			if (p->dsp) {
		...
		if (tris_exists_extension(chan, chan->context, exten, 1, chan->cid.cid_num)) {
			tris_copy_string(chan->exten, exten, sizeof(chan->exten));
			if (p->dsp) tris_dsp_digitreset(p->dsp);
			res = tris_pbx_run(chan);
			if (res) {
				tris_log(LOG_WARNING, "PBX exited non-zero\n");
				res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_CONGESTION);
			...
			tris_verb(2, "Unknown extension '%s' in context '%s' requested\n", exten, chan->context);
			sleep(2);
			res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_INFO);
			if (res < 0)
				tris_log(LOG_WARNING, "Unable to start special tone on %d\n", p->channel);
			else
				sleep(1);
			res = tris_streamfile(chan, "ss-noservice", chan->language);
			if (res >= 0)
	...
	case SIG_FXOKS:
		...
		while (len < TRIS_MAX_EXTENSION-1) {
			...
			if (res < 0) {
				...
				tris_debug(1,"waitfordigit returned '%c' (%d), timeout = %d\n", res, res, timeout);
				exten[len++]=res;
				exten[len] = '\0';
			}
			if (!tris_ignore_pattern(chan->context, exten))
				...
				tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALTONE);
			if (tris_exists_extension(chan, chan->context, exten, 1, p->cid_num) && strcmp(exten, tris_parking_ext())) {
				if (!res || !tris_matchmore_extension(chan, chan->context, exten, 1, p->cid_num)) {
					if (getforward) {
						/* Record this as the forwarding extension */
						tris_copy_string(p->call_forward, exten, sizeof(p->call_forward));
						tris_verb(3, "Setting call forward to '%s' on channel %d\n", p->call_forward, p->channel);
						...
						if (!tris_strlen_zero(p->cid_num)) {
							if (!p->hidecallerid)
								...
								tris_set_callerid(chan, NULL, NULL, p->cid_num);
						}
						if (!tris_strlen_zero(p->cid_name)) {
							if (!p->hidecallerid)
								tris_set_callerid(chan, NULL, p->cid_name, NULL);
						}
						tris_setstate(chan, TRIS_STATE_RING);
						dahdi_enable_ec(p);
						res = tris_pbx_run(chan);
				...
				res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALRECALL);
				if (res) {
					tris_log(LOG_WARNING, "Unable to do dial recall on channel %s: %s\n",
						chan->name, strerror(errno));
				}
				len = 0;
			...
			} else if (!strcmp(exten,tris_pickup_ext())) {
				...
				if (idx == SUB_REAL) {
					...
					tris_log(LOG_WARNING, "Huh?  Got *8# on call not on real\n");
					tris_hangup(chan);
					goto quit;
				}
				...
				res = 0;
				if (!tris_strlen_zero(p->lastcid_num)) {
					res = tris_say_digit_str(chan, p->lastcid_num, "", chan->language);
				}
				if (!res)
					res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALRECALL);
				break;
				...
				tris_verb(3, "Cancelling call forwarding on channel %d\n", p->channel);
				res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALRECALL);
				memset(p->call_forward, 0, sizeof(p->call_forward));
				getforward = 0;
				memset(exten, 0, sizeof(exten));
				len = 0;
			} else if ((p->transfer || p->canpark) && !strcmp(exten, tris_parking_ext()) &&
						p->subs[SUB_THREEWAY].owner &&
						tris_bridged_channel(p->subs[SUB_THREEWAY].owner)) {
				/* This is a three way call, the main call being a real channel,
					and we're parking the first call. */
				tris_masq_park_call(tris_bridged_channel(p->subs[SUB_THREEWAY].owner), chan, 0, NULL);
				tris_verb(3, "Parking call to '%s'\n", chan->name);
				break;
			} else if (!tris_strlen_zero(p->lastcid_num) && !strcmp(exten, "*60")) {
				tris_verb(3, "Blacklisting number %s\n", p->lastcid_num);
				res = tris_db_put("blacklist", p->lastcid_num, "1");
				if (!res) {
					res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALRECALL);
					memset(exten, 0, sizeof(exten));
					len = 0;
				...
				struct tris_channel *nbridge =
					p->subs[SUB_THREEWAY].owner;
				struct dahdi_pvt *pbridge = NULL;
				/* set up the private struct of the bridged one, if any */
				if (nbridge && tris_bridged_channel(nbridge))
					pbridge = tris_bridged_channel(nbridge)->tech_pvt;
				if (nbridge && pbridge &&
					(nbridge->tech == &dahdi_tech) &&
					(tris_bridged_channel(nbridge)->tech == &dahdi_tech) &&
					ISTRUNK(pbridge)) {
					int func = DAHDI_FLASH;
					/* Clear out the dial buffer */
					p->dop.dialstr[0] = '\0';
			...
			} else if (!tris_canmatch_extension(chan, chan->context, exten, 1, chan->cid.cid_num) &&
	...
	case SIG_FXSKS:
		...
		if (p->pri) {
			...
			while (time(NULL) < start + 3) {
				...
				if (res) {
					f = tris_read(chan);
					if (!f) {
						tris_log(LOG_WARNING, "Whoa, hangup while waiting for first ring!\n");
						tris_hangup(chan);
						goto quit;
					} else if ((f->frametype == TRIS_FRAME_CONTROL) && (f->subclass == TRIS_CONTROL_RING)) {
						res = 1;
					} else
						res = 0;
					tris_frfree(f);
					if (res) {
						tris_debug(1, "Got ring!\n");
						res = 0;
						break;
					}
				}
		...
		if (p->use_smdi && p->smdi_iface) {
			smdi_msg = tris_smdi_md_message_wait(p->smdi_iface, SMDI_MD_WAIT_TIMEOUT);

			if (smdi_msg != NULL) {
				tris_copy_string(chan->exten, smdi_msg->fwd_st, sizeof(chan->exten));
				...
				tris_debug(1, "Received SMDI message on %s\n", chan->name);
			} else {
				tris_log(LOG_WARNING, "SMDI enabled but no SMDI message present\n");
			}
		}
		 ...
		 * grab the caller id and wait for ringing to start... */
		} else if (p->use_callerid && (chan->_state == TRIS_STATE_PRERING && (p->cid_start == CID_START_POLARITY || p->cid_start == CID_START_POLARITY_IN))) {
			/* If set to use DTMF CID signalling, listen for DTMF */
			if (p->cid_signalling == CID_SIG_DTMF) {
				int k = 0;
				cs = NULL;
				tris_debug(1, "Receiving DTMF cid on "
					"channel %s\n", chan->name);
				dahdi_setlinear(p->subs[idx].dfd, 0);
				res = 2000;
				for (;;) {
					...
					if (res <= 0) {
						tris_log(LOG_WARNING, "DTMFCID timed out waiting for ring. "
					...
					if (f->frametype == TRIS_FRAME_DTMF) {
						...
						tris_debug(1, "CID got digit '%c'\n", f->subclass);
						res = 2000;
					}
					tris_frfree(f);
					if (chan->_state == TRIS_STATE_RING ||
						chan->_state == TRIS_STATE_RINGING)
						break; /* Got ring */
				}
				dtmfbuf[k] = '\0';
				dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
				/* Got cid and ring. */
				tris_debug(1, "CID got string '%s'\n", dtmfbuf);
				callerid_get_dtmf(dtmfbuf, dtmfcid, &flags);
				tris_debug(1, "CID is '%s', flags %d\n",
					dtmfcid, flags);
				/* If first byte is NULL, we have no cid */
				if (!tris_strlen_zero(dtmfcid))
					number = dtmfcid;
				else
					number = NULL;
				...
				if (cs) {
					...
					for (;;) {
						...
						if ((res = ioctl(p->subs[idx].dfd, DAHDI_IOMUX, &i))) {
							tris_log(LOG_WARNING, "I/O MUX failed: %s\n", strerror(errno));
							callerid_free(cs);
							tris_hangup(chan);
							goto quit;
						}
						if (i & DAHDI_IOMUX_SIGEVENT) {
							res = dahdi_get_event(p->subs[idx].dfd);
							tris_log(LOG_NOTICE, "Got event %d (%s)...\n", res, event2str(res));
							...
							if (p->cid_signalling == CID_SIG_V23_JP) {
								res = callerid_feed_jp(cs, buf, res, TRIS_LAW(p));
							} else {
								res = callerid_feed(cs, buf, res, TRIS_LAW(p));
							}

							if (res < 0) {
								tris_log(LOG_WARNING, "CallerID feed failed on channel '%s'\n", chan->name);
								break;
							} else if (res)
								break;
					...
					if (res == 1) {
						...
						tris_log(LOG_NOTICE, "CallerID number: %s, name: %s, flags=%d\n", number, name, flags);
					}
					...
					for (;;) {
						...
						res = tris_waitfor(chan, res);
						if (res <= 0) {
							tris_log(LOG_WARNING, "CID timed out waiting for ring. "
								"Exiting simple switch\n");
							tris_hangup(chan);
							goto quit;
						}
						if (!(f = tris_read(chan))) {
							tris_log(LOG_WARNING, "Hangup received waiting for ring. Exiting simple switch\n");
							tris_hangup(chan);
							goto quit;
						}
						tris_frfree(f);
					...
					if (p->usedistinctiveringdetection) {
						...
						if (strcmp(p->context,p->defcontext) != 0) {
							tris_copy_string(p->context, p->defcontext, sizeof(p->context));
							tris_copy_string(chan->context,p->defcontext,sizeof(chan->context));
						}
						...
						for (counter = 0; counter < 3; counter++) {
							...
							for (counter1 = 0; counter1 < 3; counter1++) {
								...
								if (p->drings.ringnum[counter].ring[counter1] == -1) {
									...
									tris_verb(3, "Ring pattern matched in range: %d to %d\n",
							...
							if (distMatches == 3) {
								...
								tris_copy_string(p->context, p->drings.ringContext[counter].contextData, sizeof(p->context));
								tris_copy_string(chan->context, p->drings.ringContext[counter].contextData, sizeof(chan->context));
								tris_verb(3, "Distinctive Ring matched context %s\n",p->context);
								break;
							}
						}
					}
					...
					tris_log(LOG_WARNING, "Unable to get caller ID space\n");
			} else {
				tris_log(LOG_WARNING, "Channel %s in prering "
			...
			if (p->cid_signalling == CID_SIG_DTMF) {
				...
				for (;;) {
					...
					if (f->frametype == TRIS_FRAME_DTMF) {
						dtmfbuf[k++] = f->subclass;
						tris_log(LOG_DEBUG, "CID got digit '%c'\n", f->subclass);
						res = 2000;
					}
					tris_frfree(f);
				...
				/* Got cid and ring. */
				callerid_get_dtmf(dtmfbuf, dtmfcid, &flags);
				tris_log(LOG_DEBUG, "CID is '%s', flags %d\n",
					dtmfcid, flags);
				/* If first byte is NULL, we have no cid */
				if (!tris_strlen_zero(dtmfcid))
					number = dtmfcid;
				...
				if (cs) {
					...
					for (;;) {
						...
						if (i & DAHDI_IOMUX_SIGEVENT) {
							...
							if ( res == DAHDI_EVENT_POLARITY && p->hanguponpolarityswitch && p->polarity == POLARITY_REV) {
								tris_log(LOG_DEBUG, "Hanging up due to polarity reversal on channel %d while detecting callerid\n", p->channel);
								p->polarity = POLARITY_IDLE;
								callerid_free(cs);
								tris_hangup(chan);
								goto quit;
							...
							res = callerid_feed(cs, buf, res, TRIS_LAW(p));
							if (res < 0) {
								tris_log(LOG_WARNING, "CallerID feed failed: %s\n", strerror(errno));
								break;
							} else if (res)
								break;
							else if (samples > (8000 * 10))
					...
					if (res == 1) {
						callerid_get(cs, &name, &number, &flags);
						tris_debug(1, "CallerID number: %s, name: %s, flags=%d\n", number, name, flags);
					}
					if (distinctiveringaftercid == 1) {
						...
						receivedRingT = 0;
						tris_verb(3, "Detecting post-CID distinctive ring\n");
						for (;;) {
							i = DAHDI_IOMUX_READ | DAHDI_IOMUX_SIGEVENT;
							if ((res = ioctl(p->subs[idx].dfd, DAHDI_IOMUX, &i))) {
								tris_log(LOG_WARNING, "I/O MUX failed: %s\n", strerror(errno));
...
#endif
					if (res < 0) {
						tris_log(LOG_WARNING, "CallerID returned with error on channel '%s'\n", chan->name);
					}
				} else
					tris_log(LOG_WARNING, "Unable to get caller ID space\n");
			}
		...
		tris_set_callerid(chan, number, name, number);

		if (smdi_msg)
			ASTOBJ_UNREF(smdi_msg, tris_smdi_md_message_destroy);

		if (cs)
			callerid_free(cs);
		/* If the CID had Message waiting payload, assume that this for MWI only and hangup the call */
		if (flags & CID_MSGWAITING) {
			tris_log(LOG_NOTICE, "MWI: Channel %d message waiting!\n", p->channel);
			...
			tris_log(LOG_NOTICE, "MWI: Channel %d no message waiting!\n", p->channel);
		...
		goto quit;
	default:
		tris_log(LOG_WARNING, "Don't know how to handle simple switch with signalling %s on channel %d\n", sig2str(p->sig), p->channel);
		res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_CONGESTION);
		if (res < 0)
				tris_log(LOG_WARNING, "Unable to play congestion tone on channel %d\n", p->channel);
	}
	res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_CONGESTION);
	if (res < 0)
			tris_log(LOG_WARNING, "Unable to play congestion tone on channel %d\n", p->channel);
...
quit:
	tris_mutex_lock(&ss_thread_lock);
	ss_thread_count--;
	tris_cond_signal(&ss_thread_complete);
	tris_mutex_unlock(&ss_thread_lock);
	return NULL;
}
...
static int calc_energy(const unsigned char *buf, int len, int law)
{
	...
	for (x = 0; x < len; x++)
		sum += abs(law == TRIS_FORMAT_ULAW ? TRIS_MULAW(buf[x]) : TRIS_ALAW(buf[x]));
...
static void *mwi_thread(void *data)
{
	...
	callerid_feed(cs, mtd->buf, mtd->len, TRIS_LAW(mtd->pvt));
	...
	for (;;) {
		...
		if (i & DAHDI_IOMUX_SIGEVENT) {
			...
			default:
				tris_log(LOG_NOTICE, "Got event %d (%s)...  Passing along to ss_thread\n", res, event2str(res));
				callerid_free(cs);

				restore_gains(mtd->pvt);
				mtd->pvt->ringt = mtd->pvt->ringt_base;

				if ((chan = dahdi_new(mtd->pvt, TRIS_STATE_RING, 0, SUB_REAL, 0, 0))) {
					if (tris_pthread_create_detached(&threadid, NULL, ss_thread, chan)) {
						tris_log(LOG_WARNING, "Unable to start simple switch thread on channel %d\n", mtd->pvt->channel);
					...
					tris_log(LOG_WARNING, "Could not create channel to handle call\n");
				}
			}
		} else if (i & DAHDI_IOMUX_READ) {
			...
			samples += res;
			if (!spill_done) {
				if ((spill_result = callerid_feed(cs, mtd->buf, res, TRIS_LAW(mtd->pvt))) < 0) {
					tris_log(LOG_WARNING, "CallerID feed failed: %s\n", strerror(errno));
					break;
				...
				*/
				if (calc_energy(mtd->buf, res, TRIS_LAW(mtd->pvt)) <= mwilevel)
					break;
			}
	...
	if (spill_result == 1) {
		callerid_get(cs, &name, &number, &flags);
		if (flags & CID_MSGWAITING) {
			tris_log(LOG_NOTICE, "mwi: Have Messages on channel %d\n", mtd->pvt->channel);
			notify_message(mtd->pvt->mailbox, 1);
		} else if (flags & CID_NOMSGWAITING) {
			tris_log(LOG_NOTICE, "mwi: No Messages on channel %d\n", mtd->pvt->channel);
			notify_message(mtd->pvt->mailbox, 0);
		} else {
			tris_log(LOG_NOTICE, "mwi: Status unknown on channel %d\n", mtd->pvt->channel);
		}
	}
...
static int mwi_send_init(struct dahdi_pvt * pvt)
{
	...
	if (pvt->cidspill) {
		tris_log(LOG_WARNING, "cidspill already exists when trying to send FSK MWI\n");
		tris_free(pvt->cidspill);
		pvt->cidspill = NULL;
		pvt->cidpos = 0;
		pvt->cidlen = 0;
	}
	pvt->cidspill = tris_calloc(1, MAX_CALLERID_SIZE);
	if (!pvt->cidspill) {
		pvt->mwisendactive = 0;
		return -1;
	}
	...
	if (pvt->mwisend_fsk) {
		...
		pvt->cidlen = vmwi_generate(pvt->cidspill, has_voicemail(pvt), CID_MWI_TYPE_MDMF_FULL,
								TRIS_LAW(pvt), pvt->cid_name, pvt->cid_num, 0);
		pvt->cidpos = 0;
#ifdef HAVE_DAHDI_LINEREVERSE_VMWI
	}
#endif
	...
	*/
	if (MWI_SEND_DONE != pvt->mwisend_data.mwisend_current && !pvt->cidspill) {
		...
		case MWI_SEND_SA:
			...
			if (res) {
				tris_log(LOG_WARNING, "Unable to set RP-AS ring cadence: %s\n", strerror(errno));
				goto quit;
			}
		...
		case MWI_SEND_SPILL:
			...
			if(0 < num_read) {
				...
				if (res > 0) {
					...
					tris_log(LOG_WARNING, "MWI FSK Send Write failed: %s\n", strerror(errno));
					goto quit;
				}
			}
			break;
		...
		case DAHDI_EVENT_RINGEROFF:
			if(pvt->mwisend_data.mwisend_current == MWI_SEND_SA_WAIT) {
				...
				if (dahdi_set_hook(pvt->subs[SUB_REAL].dfd, DAHDI_RINGOFF) ) {
					tris_log(LOG_WARNING, "Unable to finsh RP-AS: %s mwi send aborted\n", strerror(errno));
					if(pvt->cidspill) {
						tris_free(pvt->cidspill);
						pvt->cidspill = NULL;
					}
...
static struct dahdi_pvt *handle_init_event(struct dahdi_pvt *i, int event)
{
	...
	case DAHDI_EVENT_RINGOFFHOOK:
		...
		case SIG_FXOKS:
			...
			if (i->immediate) {
				...
				chan = dahdi_new(i, TRIS_STATE_RING, 1, SUB_REAL, 0, 0);
				if (!chan) {
					tris_log(LOG_WARNING, "Unable to start PBX on channel %d\n", i->channel);
				...
				chan = dahdi_new(i, TRIS_STATE_RESERVED, 0, SUB_REAL, 0, 0);
				if (chan) {
					if (has_voicemail(i))
					...
					if (res < 0)
						tris_log(LOG_WARNING, "Unable to play dialtone on channel %d, do you have defaultzone and loadzone defined?\n", i->channel);
					if (tris_pthread_create_detached(&threadid, NULL, ss_thread, chan)) {
						tris_log(LOG_WARNING, "Unable to start simple switch thread on channel %d\n", i->channel);
						res = tone_zone_play_tone(i->subs[SUB_REAL].dfd, DAHDI_TONE_CONGESTION);
						if (res < 0)
					...
					tris_log(LOG_WARNING, "Unable to create channel\n");
			}
			break;
		case SIG_FXSLS:
		case SIG_FXSGS:
		...
		case SIG_SF:
			...
			if (i->cid_start == CID_START_POLARITY_IN) {
				chan = dahdi_new(i, TRIS_STATE_PRERING, 0, SUB_REAL, 0, 0);
			} else {
				chan = dahdi_new(i, TRIS_STATE_RING, 0, SUB_REAL, 0, 0);
			}
			...
			break;
		default:
			tris_log(LOG_WARNING, "Don't know how to handle ring/answer with signalling %s on channel %d\n", sig2str(i->sig), i->channel);
			res = tone_zone_play_tone(i->subs[SUB_REAL].dfd, DAHDI_TONE_CONGESTION);
			if (res < 0)
				tris_log(LOG_WARNING, "Unable to play congestion tone on channel %d\n", i->channel);
			return NULL;
	...
	case DAHDI_EVENT_ONHOOK:
		...
		case SIG_BRI_PTMP:
			...
			break;
		default:
			tris_log(LOG_WARNING, "Don't know how to handle on hook with signalling %s on channel %d\n", sig2str(i->sig), i->channel);
			res = tone_zone_play_tone(i->subs[SUB_REAL].dfd, -1);
			return NULL;
		}
	...
	case DAHDI_EVENT_POLARITY:
		...
		case SIG_FXSGS:
			...
			if (i->cid_start == CID_START_POLARITY || i->cid_start == CID_START_POLARITY_IN) {
				i->polarity = POLARITY_REV;
				tris_verb(2, "Starting post polarity "
			...
			break;
		default:
			tris_log(LOG_WARNING, "handle_init_event detected "
	...
	case DAHDI_EVENT_NEONMWI_ACTIVE:
		if (i->mwimonitor_neon) {
			...
			tris_log(LOG_NOTICE, "NEON MWI set for channel %d, mailbox %s \n", i->channel, i->mailbox);
		}
		break;
	case DAHDI_EVENT_NEONMWI_INACTIVE:
		if (i->mwimonitor_neon) {
			notify_message(i->mailbox, 0);
			tris_log(LOG_NOTICE, "NEON MWI cleared for channel %d, mailbox %s\n", i->channel, i->mailbox);
		}
		break;
	}
	return NULL;
...
static void *do_monitor(void *data)
{
	...
	if (pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL)) {
		tris_log(LOG_WARNING, "Unable to set cancel type to asynchronous\n");
		return NULL;
	}
	tris_debug(1, "Monitor starting...\n");
#endif
	...
	for (;;) {
		...
		if (!pfds || (lastalloc != ifcount)) {
			...
			if (ifcount) {
				if (!(pfds = tris_calloc(1, ifcount * sizeof(*pfds)))) {
					tris_mutex_unlock(&iflock);
					return NULL;
				}
			}
		...
		if (res < 0) {
			if ((errno != EAGAIN) && (errno != EINTR))
				tris_log(LOG_WARNING, "poll return %d: %s\n", res, strerror(errno));
			continue;
		}
		...
		for (i = iflist;; i = i->next) {
			if (doomed) {
				...
				if (!res) {
					tris_log(LOG_WARNING, "Couldn't find channel to destroy, hopefully another destroy operation just happened.\n");
				}
				doomed = NULL;
			}
			if (!i) {
				...
				if (!found && ((i == last) || ((i == iflist) && !last))) {
					...
					if (last) {
						...
						if (!last->mwisendactive &&	last->sig & __DAHDI_SIG_FXO &&
								...
								!tris_strlen_zero(last->mailbox) && (thispass - last->onhooktime > 3)) {
							...
							if (last->msgstate != res) {
								...
								if (res2) {
									...
									tris_debug(3, "Unable to control message waiting led on channel %d: %s\n", last->channel, strerror(errno));
								}
								/* If enabled for FSK spill then initiate it */
								if (mwi_send_init(last)) {
									tris_log(LOG_WARNING, "Unable to initiate mwi send sequence on channel %d\n", last->channel);
								}
								last->msgstate = res;
								found ++;
							}
			...
			if ((i->subs[SUB_REAL].dfd > -1) && i->sig) {
				if (i->radio && !i->owner)
				{
					...
					if (res)
					{
						tris_debug(1, "Monitor doohicky got event %s on radio channel %d\n", event2str(res), i->channel);
				...
				pollres = tris_fdisset(pfds, i->subs[SUB_REAL].dfd, count, &spoint);
				if (pollres & POLLIN) {
					if (i->owner || i->subs[SUB_REAL].owner) {
#ifdef HAVE_PRI
						if (!i->pri)
#endif
							tris_log(LOG_WARNING, "Whoa....  I'm owned but found (%d) in read...\n", i->subs[SUB_REAL].dfd);
						continue;
					}
					if (!i->mwimonitor_fsk && !i->mwisendactive) {
						tris_log(LOG_WARNING, "Whoa....  I'm not looking for MWI or sending MWI but am reading (%d)...\n", i->subs[SUB_REAL].dfd);
						continue;
					}
					res = read(i->subs[SUB_REAL].dfd, buf, sizeof(buf));
					if (res > 0) {
						if (i->mwimonitor_fsk) {
							if (calc_energy((unsigned char *) buf, res, TRIS_LAW(i)) > mwilevel) {
								pthread_attr_t attr;
								pthread_t threadid;
								...
								tris_log(LOG_DEBUG, "Maybe some MWI on port %d!\n", i->channel);
								if ((mtd = tris_calloc(1, sizeof(*mtd)))) {
									mtd->pvt = i;
									memcpy(mtd->buf, buf, res);
									mtd->len = res;
									if (tris_pthread_create_background(&threadid, &attr, mwi_thread, mtd)) {
										tris_log(LOG_WARNING, "Unable to start mwi thread on channel %d\n", i->channel);
										tris_free(mtd);
									}
						...
						tris_log(LOG_WARNING, "Read failed with %d: %s\n", res, strerror(errno));
					}
				}
				if (pollres & POLLPRI) {
					if (i->owner || i->subs[SUB_REAL].owner) {
						...
						if (!i->pri)
#endif
							tris_log(LOG_WARNING, "Whoa....  I'm owned but found (%d)...\n", i->subs[SUB_REAL].dfd);
						continue;
					}
					res = dahdi_get_event(i->subs[SUB_REAL].dfd);
					tris_debug(1, "Monitor doohicky got event %s on channel %d\n", event2str(res), i->channel);
					/* Don't hold iflock while handling init events */
					tris_mutex_unlock(&iflock);
...
static int pri_resolve_span(int *span, int channel, int offset, struct dahdi_spaninfo *si)
{
	...
	if (trunkgroup) {
		...
		tris_log(LOG_WARNING, "Channel %d on span %d configured to use nonexistent trunk group %d\n", channel, *span, trunkgroup);
		*span = -1;
	} else {
		if (pris[*span].trunkgroup) {
			tris_log(LOG_WARNING, "Unable to use span %d implicitly since it is trunk group %d (please use spanmap)\n", *span, pris[*span].trunkgroup);
			*span = -1;
		} else if (pris[*span].mastertrunkgroup) {
			tris_log(LOG_WARNING, "Unable to use span %d implicitly since it is already part of trunk group %d\n", *span, pris[*span].mastertrunkgroup);
			*span = -1;
		} else {
			if (si->totalchans == 31) {
				/* E1 */
				...
				tris_log(LOG_WARNING, "Unable to use span %d, since the D-channel cannot be located (unexpected span size of %d channels)\n", *span, si->totalchans);
				*span = -1;
				return 0;
			}
...
static int pri_create_trunkgroup(int trunkgroup, int *channels)
{
	...
	for (x = 0; x < NUM_SPANS; x++) {
		if (pris[x].trunkgroup == trunkgroup) {
			tris_log(LOG_WARNING, "Trunk group %d already exists on span %d, Primary d-channel %d\n", trunkgroup, x + 1, pris[x].dchannels[0]);
			return -1;
		}
	}
	for (y = 0; y < NUM_DCHANS; y++) {
		...
		if (fd < 0) {
			tris_log(LOG_WARNING, "Failed to open channel: %s\n", strerror(errno));
			return -1;
		}
		x = channels[y];
		if (ioctl(fd, DAHDI_SPECIFY, &x)) {
			tris_log(LOG_WARNING, "Failed to specify channel %d: %s\n", channels[y], strerror(errno));
			close(fd);
			return -1;
		}
		if (ioctl(fd, DAHDI_GET_PARAMS, &p)) {
			tris_log(LOG_WARNING, "Failed to get channel parameters for channel %d: %s\n", channels[y], strerror(errno));
			return -1;
		}
		if (ioctl(fd, DAHDI_SPANSTAT, &si)) {
			tris_log(LOG_WARNING, "Failed go get span information on channel %d (span %d): %s\n", channels[y], p.spanno, strerror(errno));
			close(fd);
			return -1;
		}
		span = p.spanno - 1;
		if (pris[span].trunkgroup) {
			tris_log(LOG_WARNING, "Span %d is already provisioned for trunk group %d\n", span + 1, pris[span].trunkgroup);
			close(fd);
			return -1;
		}
		if (pris[span].pvts[0]) {
			tris_log(LOG_WARNING, "Span %d is already provisioned with channels (implicit PRI maybe?)\n", span + 1);
			close(fd);
			return -1;
		}
		if (!y) {
...
static int pri_create_spanmap(int span, int trunkgroup, int logicalspan)
{
	if (pris[span].mastertrunkgroup) {
		tris_log(LOG_WARNING, "Span %d is already part of trunk group %d, cannot add to trunk group %d\n", span + 1, pris[span].mastertrunkgroup, trunkgroup);
		return -1;
	}
...
static void dpnss_start_call(struct dahdi_pvt *p, struct dahdi_dpnss *linkset)
{
	struct dpnss *dpnss = linkset->dpnss;
	int res;
	int law = 1;
	struct tris_channel *c;
	...
	if (res < 0)
		tris_log(LOG_WARNING, "Unable to set law on channel %d\n", p->channel);

	p->proceeding = 0;

	p->dpnsscall = 1;
	
	dpnss_chan_send_nam(dpnss, p->dpnsschan);

	tris_mutex_unlock(&linkset->lock);
	
	c = dahdi_new(p, TRIS_STATE_RING, 1, SUB_REAL, law, 0);

	if (!c) {
		tris_log(LOG_WARNING, "Unable to start PBX on CHANNEL %d\n", p->channel);
		/* Holding this lock is assumed entering the function */
		tris_mutex_lock(&linkset->lock);
		return;
	} else
		tris_verb(3, "Accepting call to '%s' on CHANNEL %d\n", p->exten, p->channel);
...
static void dpnss_process_isrm(struct dpnss *dpnss,struct dpnss_chan *chan){

	struct dahdi_pvt *p = (struct dahdi_pvt*)(dpnss_chan_get_client_data(dpnss, chan));

	char *ani  = (char*)dpnss_chan_get_ani(dpnss,chan);
	char *dni = (char*)dpnss_chan_get_dni(dpnss,chan);
	int completed = dpnss_chan_get_completed(dpnss,chan);

	tris_mutex_lock(&p->lock);

	tris_log(LOG_DEBUG, "Got DPNSS ISRM for CHANNEL %d and called number %s, calling number %s\n", p->channel,dni,ani);
	
	p->dpnsscall = 1;
	
	if ((p->use_callerid) && (!tris_strlen_zero(ani))) {
		memset(p->cid_num,0,sizeof(p->cid_num));
		memcpy(p->cid_num,ani,strlen(ani));
	} else
		p->cid_num[0] = 0;
	
	if (!tris_strlen_zero(dni)){
		memset(p->exten,0,sizeof(p->exten));
		memcpy(p->exten,dni,strlen(dni));
	}else{
	    tris_mutex_unlock(&p->lock);
	    return;
	}
	
	if (tris_canmatch_extension(NULL, p->context, p->exten, 1, p->cid_num)){
		if (completed || !(tris_matchmore_extension(NULL, p->context, p->exten,1, p->cid_num))){
			dpnss_start_call(p,p->dpnss);
		}
	}else{
		tris_debug(1,"Call on Channel for unconfigured extension %s\n",p->exten);
		p->alreadyhungup = 1;
		dpnss_chan_set_cause(dpnss,chan,DPNSS_CAUSE_NU);
		dpnss_chan_send_crm(dpnss,chan);
	}
...
static void dpnss_process_nam(struct dpnss *dpnss,struct dpnss_chan *chan){

	struct tris_frame f = { TRIS_FRAME_CONTROL, TRIS_CONTROL_PROCEEDING, };
	struct dahdi_pvt *p = (struct dahdi_pvt*)(dpnss_chan_get_client_data(dpnss, chan));
	
	tris_mutex_lock(&p->lock);
	tris_debug(1, "Got DPNSS NAM for CHANNEL %d \n", p->channel);

	dahdi_queue_frame(p, &f, p->dpnss);
...
static void dpnss_process_ccm(struct dpnss *dpnss,struct dpnss_chan *chan){
	struct dahdi_pvt *p = (struct dahdi_pvt*)(dpnss_chan_get_client_data(dpnss, chan));
...
static void dpnss_process_nim(struct dpnss *dpnss,struct dpnss_chan *chan){
}


static void dpnss_process_crm(struct dpnss *dpnss,struct dpnss_chan *chan){
	struct dahdi_pvt *p = (struct dahdi_pvt*)(dpnss_chan_get_client_data(dpnss, chan));

	enum dpnss_cause_code cause = dpnss_chan_get_cause(dpnss,chan);

	tris_mutex_lock(&p->lock);

	tris_log(LOG_DEBUG,"GOT DPNSS Release Request Channel %d\n",p->channel);
	
	if (p->owner) {
		p->owner->hangupcause = dpnss_get_cause_from_dpnss(cause);
		p->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
	} else if (!p->restartpending){
//		tris_log(LOG_WARNING, "REL on channel (CHANNEL %d) without owner!\n", p->channel);
	}
...
static void dpnss_process_cim(struct dpnss *dpnss,struct dpnss_chan *chan){
	struct dahdi_pvt *p = (struct dahdi_pvt*)(dpnss_chan_get_client_data(dpnss, chan));

	tris_mutex_lock(&p->lock);

	tris_log(LOG_DEBUG,"GOT DPNSS Release Ack Channel %d\n",p->channel);
...
static void dpnss_process_release(struct dpnss *dpnss,struct dpnss_chan *chan){

	struct dahdi_pvt *p = (struct dahdi_pvt*)(dpnss_chan_get_client_data(dpnss,chan));

	tris_mutex_lock(&p->lock);

	tris_log(LOG_DEBUG,"DPNSS Protocol Error. Releasing %d\n",p->channel);

	if (p->owner) {
		p->owner->hangupcause = dpnss_get_cause_from_dpnss(DPNSS_CAUSE_CON);
		p->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
	}
	p->dpnsscall = 0;
...
static struct dpnss_msg_handler dpnss_msg_handler ={
	dpnss_process_isrm,  
	dpnss_process_nam, 
	dpnss_process_ccm, 
	dpnss_process_nim, 
	dpnss_process_crm, 
	dpnss_process_cim, 
	dpnss_process_release
};

static struct dpnss_timer dpnss_timer={500,15000,15000,15000};

static void dahdi_close_dpnss_fd(struct dahdi_dpnss *link){
	dahdi_close(link->dfd);
	link->dfd = -1;
}
static int dahdi_dpnss_set_links(struct dahdi_dpnss *link,int dchannel){

	struct dahdi_params p;
	struct dahdi_bufferinfo bi;
	int res = 0;
	
	link->dfd = open("/dev/dahdi/channel", O_RDWR, 0600);

	if ((link->dfd < 0) || (ioctl(link->dfd,DAHDI_SPECIFY,&dchannel) == -1)) {
		tris_log(LOG_ERROR, "Unable to open DPNSS sigchan %d (%s)\n", dchannel, strerror(errno));
		return -1;
	}
	
	memset(&p, 0, sizeof(p));
	res = ioctl(link->dfd, DAHDI_GET_PARAMS, &p);
	if (res) {
		dahdi_close_dpnss_fd(link);
		tris_log(LOG_ERROR, "Unable to get parameters for sigchan %d (%s)\n", dchannel, strerror(errno));
		return -1;
	}
	if (p.sigtype != DAHDI_SIG_HDLCFCS) {
		dahdi_close_dpnss_fd(link);
		tris_log(LOG_ERROR, "sigchan %d is not in HDLC/FCS mode.\n", dchannel);
		return -1;
	}
	...
	if (ioctl(link->dfd, DAHDI_SET_BUFINFO, &bi)) {
		dahdi_close_dpnss_fd(link);
		tris_log(LOG_ERROR, "Unable to set appropriate buffering on channel %d: %s\n", dchannel, strerror(errno));
		return -1;
	}

	link->dpnss = dpnss_new(&dpnss_msg_handler,64, &dpnss_timer,link->dfd);

	if (!link->dpnss){
		dahdi_close_dpnss_fd(link);
		tris_log(LOG_ERROR,"Unabled to create dpnss link\n");
		return -1;		
	}
	link->numchans = 0;
	return 0;
...
static void dahdi_r2_destroy_links(void)
{
	...
	for (; i < r2links_count; i++) {
		if (r2links[i]->r2master != TRIS_PTHREADT_NULL) {
...
static struct dahdi_mfcr2 *dahdi_r2_get_link(void)
{
	...
	if (!r2links_count || (r2links[r2links_count - 1]->monitored_count == R2_LINK_CAPACITY)) {
		new_r2link = tris_calloc(1, sizeof(**r2links));
		if (!new_r2link) {
			tris_log(LOG_ERROR, "Cannot allocate R2 link!\n");
			return NULL;
		}
		new_r2links = tris_realloc(r2links, ((r2links_count + 1) * sizeof(*r2links)));
		if (!new_r2links) {
			tris_log(LOG_ERROR, "Cannot allocate R2 link!\n");
			tris_free(new_r2link);
			return NULL;
		}
		r2links = new_r2links;
		new_r2link->r2master = TRIS_PTHREADT_NULL;
		r2links[r2links_count] = new_r2link;
		r2links_count++;
		tris_log(LOG_DEBUG, "Created new R2 link!\n");
	}
...
static int dahdi_r2_set_context(struct dahdi_mfcr2 *r2_link, const struct dahdi_chan_conf *conf)
{
	...
	if (tris_strlen_zero(conf->mfcr2.logdir)) {
		if (openr2_context_set_log_directory(r2_link->protocol_context, tmplogdir)) {
			tris_log(LOG_ERROR, "Failed setting default MFC/R2 log directory %s\n", tmplogdir);
		}
	} else {
		snres = snprintf(logdir, sizeof(logdir), "%s/%s/%s", tris_config_TRIS_LOG_DIR, "mfcr2", conf->mfcr2.logdir);
		if (snres >= sizeof(logdir)) {
			tris_log(LOG_ERROR, "MFC/R2 logging directory truncated, using %s\n", tmplogdir);
			if (openr2_context_set_log_directory(r2_link->protocol_context, tmplogdir)) {
				tris_log(LOG_ERROR, "Failed setting default MFC/R2 log directory %s\n", tmplogdir);
			}
		} else {
			if (openr2_context_set_log_directory(r2_link->protocol_context, logdir)) {
				tris_log(LOG_ERROR, "Failed setting MFC/R2 log directory %s\n", logdir);
			}
		}
	}
	if (!tris_strlen_zero(conf->mfcr2.r2proto_file)) {
		if (openr2_context_configure_from_advanced_file(r2_link->protocol_context, conf->mfcr2.r2proto_file)) {
			tris_log(LOG_ERROR, "Failed to configure r2context from advanced configuration file %s\n", conf->mfcr2.r2proto_file);
		}
	}
...
static struct dahdi_pvt *mkintf(int channel, const struct dahdi_chan_conf *conf, struct dahdi_pri *pri, int reloading)
{
	...
	if (tmp) {
		...
		if (!here) {
			if ((channel != CHAN_PSEUDO) && !pri) {
				...
				if (tmp->subs[SUB_REAL].dfd < 0) {
					tris_log(LOG_ERROR, "Unable to open channel %d: %s\nhere = %d, tmp->channel = %d, channel = %d\n", channel, strerror(errno), here, tmp->channel, channel);
				...
				if (res < 0) {
					tris_log(LOG_ERROR, "Unable to get parameters: %s\n", strerror(errno));
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				if (conf->is_sig_auto)
				...
				if (p.sigtype != (chan_sig & 0x3ffff)) {
					tris_log(LOG_ERROR, "Signalling requested on channel %d is %s but line is in %s signalling\n", channel, sig2str(chan_sig), sig2str(p.sigtype));
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				tmp->law = p.curlaw;
				...
				if (channel == CHAN_PSEUDO)
					...
					tris_log(LOG_ERROR, "CRV's must use FXO/FXS Kewl Start (fxo_ks/fxs_ks) signalling only.\n");
					return NULL;
				}
			}
#ifdef HAVE_DPNSS
			if (chan_sig == SIG_DPNSS){
				struct dahdi_spaninfo si;
				struct dpnss_chan *dpnss_tmp;
				struct dahdi_dpnss *dahdi_dpnss;
				int clear = 0;
				int dchannel = 0;
				if (ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_AUDIOMODE, &clear)) {
					tris_log(LOG_ERROR, "Unable to set clear mode on clear channel %d of span %d: %s\n", channel, p.spanno, strerror(errno));
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				
				if (span >= NUM_SPANS) {
					tris_log(LOG_ERROR, "Channel %d does not lie on a span I know of (%d)\n", channel, span);
					destroy_dahdi_pvt(&tmp);
					return NULL;
				} else {
					si.spanno = 0;
					if (ioctl(tmp->subs[SUB_REAL].dfd,DAHDI_SPANSTAT,&si) == -1) {
						tris_log(LOG_ERROR, "Unable to get span status: %s\n", strerror(errno));
						destroy_dahdi_pvt(&tmp);
						return NULL;
					}

					dahdi_dpnss = &dpnsslinks[si.spanno-1];
					
					if (!dahdi_dpnss->dpnss){
						if (si.totalchans == 31) {
							//E1
							dchannel = 16 + tmp->channel - p.chanpos;
						} else if (si.totalchans == 24) {
							//T1/J1
							dchannel = 24 + tmp->channel - p.chanpos;
						}else{
							tris_log(LOG_WARNING, "Unable to use span %d, since the D-channel cannot be located (unexpected span size of %d channels)\n", si.spanno, si.totalchans);
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}

						if (dahdi_dpnss_set_links(dahdi_dpnss,dchannel) < 0){
							tris_log(LOG_WARNING,"Unable to create dpnss link upto Span No:%d \n",si.spanno);
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
					}
				...
				dpnss_tmp = dpnss_init_chan(dahdi_dpnss->dpnss,p.chanpos,conf->chan.dpnsscrbit);

				if (!dpnss_tmp){
					tris_log(LOG_WARNING,"Unable to create dpnss channel :%d \n",p.channo);
					destroy_dahdi_pvt(&tmp);
					return NULL;	
				}

				tmp->dpnsscall = 0;
				tmp->dpnss = dahdi_dpnss;
				tmp->dpnsschan = dpnss_tmp;
			
				dpnss_chan_set_client_data(dpnss_tmp,tmp);

				dahdi_dpnss->pvts[dahdi_dpnss->numchans] = tmp;
				dahdi_dpnss->numchans ++;
			...
			if (chan_sig == SIG_SS7) {
				...
				if (!ss7) {
					tris_log(LOG_ERROR, "Unable to find linkset %d\n", cur_linkset);
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				if (cur_cicbeginswith < 0) {
					tris_log(LOG_ERROR, "Need to set cicbeginswith for the channels!\n");
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				...
				tris_copy_string(ss7->internationalprefix, conf->ss7.internationalprefix, sizeof(ss7->internationalprefix));
				tris_copy_string(ss7->nationalprefix, conf->ss7.nationalprefix, sizeof(ss7->nationalprefix));
				tris_copy_string(ss7->subscriberprefix, conf->ss7.subscriberprefix, sizeof(ss7->subscriberprefix));
				tris_copy_string(ss7->unknownprefix, conf->ss7.unknownprefix, sizeof(ss7->unknownprefix));
			...
			if (chan_sig == SIG_MFCR2 && reloading != 1) {
				...
				if (!r2_link) {
					tris_log(LOG_WARNING, "Cannot get another R2 DAHDI context!\n");
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				if (!r2_link->protocol_context && dahdi_r2_set_context(r2_link, conf)) {
					tris_log(LOG_ERROR, "Cannot create OpenR2 protocol context.\n");
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				if (r2_link->numchans == (sizeof(r2_link->pvts)/sizeof(r2_link->pvts[0]))) {
					tris_log(LOG_ERROR, "Cannot add more channels to this link!\n");
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				...
				if (!tmp->r2chan) {
					...
					tris_log(LOG_ERROR, "Cannot create OpenR2 channel: %s\n", openr2_context_error_string(err));
					destroy_dahdi_pvt(&tmp);
					return NULL;
				}
				tmp->mfcr2 = r2_link;
			...
			if ((chan_sig == SIG_PRI) || (chan_sig == SIG_BRI) || (chan_sig == SIG_BRI_PTMP) || (chan_sig == SIG_GR303FXOKS) || (chan_sig == SIG_GR303FXSKS)) {
				...
				if (span >= NUM_SPANS) {
					...
					if (span < 0) {
						tris_log(LOG_WARNING, "Channel %d: Unable to find locate channel/trunk group!\n", channel);
						destroy_dahdi_pvt(&tmp);
						return NULL;
					}
					if ((chan_sig == SIG_PRI) ||
					...
					if (!matchesdchan) {
						if (pris[span].nodetype && (pris[span].nodetype != conf->pri.nodetype)) {
							tris_log(LOG_ERROR, "Span %d is already a %s node\n", span + 1, pri_node2str(pris[span].nodetype));
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if (pris[span].switchtype && (pris[span].switchtype != myswitchtype)) {
							tris_log(LOG_ERROR, "Span %d is already a %s switch\n", span + 1, pri_switch2str(pris[span].switchtype));
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if ((pris[span].dialplan) && (pris[span].dialplan != conf->pri.dialplan)) {
							tris_log(LOG_ERROR, "Span %d is already a %s dialing plan\n", span + 1, dialplan2str(pris[span].dialplan));
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if (!tris_strlen_zero(pris[span].idledial) && strcmp(pris[span].idledial, conf->pri.idledial)) {
							tris_log(LOG_ERROR, "Span %d already has idledial '%s'.\n", span + 1, conf->pri.idledial);
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if (!tris_strlen_zero(pris[span].idleext) && strcmp(pris[span].idleext, conf->pri.idleext)) {
							tris_log(LOG_ERROR, "Span %d already has idleext '%s'.\n", span + 1, conf->pri.idleext);
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if (pris[span].minunused && (pris[span].minunused != conf->pri.minunused)) {
							tris_log(LOG_ERROR, "Span %d already has minunused of %d.\n", span + 1, conf->pri.minunused);
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if (pris[span].minidle && (pris[span].minidle != conf->pri.minidle)) {
							tris_log(LOG_ERROR, "Span %d already has minidle of %d.\n", span + 1, conf->pri.minidle);
							destroy_dahdi_pvt(&tmp);
							return NULL;
						}
						if (pris[span].numchans >= MAX_CHANNELS) {
							tris_log(LOG_ERROR, "Unable to add channel %d: Too many channels in trunk group %d!\n", channel,
						...
						tris_copy_string(pris[span].idledial, conf->pri.idledial, sizeof(pris[span].idledial));
						tris_copy_string(pris[span].idleext, conf->pri.idleext, sizeof(pris[span].idleext));
						tris_copy_string(pris[span].internationalprefix, conf->pri.internationalprefix, sizeof(pris[span].internationalprefix));
						tris_copy_string(pris[span].nationalprefix, conf->pri.nationalprefix, sizeof(pris[span].nationalprefix));
						tris_copy_string(pris[span].localprefix, conf->pri.localprefix, sizeof(pris[span].localprefix));
						tris_copy_string(pris[span].privateprefix, conf->pri.privateprefix, sizeof(pris[span].privateprefix));
						tris_copy_string(pris[span].unknownprefix, conf->pri.unknownprefix, sizeof(pris[span].unknownprefix));
						...
						tris_log(LOG_ERROR, "Channel %d is reserved for D-channel.\n", offset);
						destroy_dahdi_pvt(&tmp);
						return NULL;
					}
				}
		...
		if (tmp->subs[SUB_REAL].dfd >= 0)
		{
			...
			if (res < 0) {
				tris_log(LOG_ERROR, "Unable to set parameters: %s\n", strerror(errno));
				destroy_dahdi_pvt(&tmp);
				return NULL;
			}
		}
		...
		if (tmp->echocancel.head.tap_length) {
			...
			if (conf->chan.echocanbridged)
				tris_log(LOG_NOTICE, "echocancelwhenbridged requires echocancel to be enabled; ignoring\n");
			tmp->echocanbridged = 0;
		}
		tmp->busydetect = conf->chan.busydetect;
		tmp->busycount = conf->chan.busycount;
		for(i=0; i<5; i++) {
			tmp->busy_tonelength[i] = conf->chan.busy_tonelength[i];
			tmp->busy_quietlength[i] = conf->chan.busy_quietlength[i];
		...
		if (tmp->usedistinctiveringdetection) {
			if (!tmp->use_callerid) {
				tris_log(LOG_NOTICE, "Distinctive Ring detect requires 'usecallerid' be on\n");
				tmp->use_callerid = 1;
			}
		}
		...
		if (tmp->cid_signalling == CID_SIG_SMDI) {
			if (!tmp->use_smdi) {
				tris_log(LOG_WARNING, "SMDI callerid requires SMDI to be enabled, enabling...\n");
				tmp->use_smdi = 1;
			}
		}
		if (tmp->use_smdi) {
			tmp->smdi_iface = tris_smdi_interface_find(conf->smdi_port);
			if (!(tmp->smdi_iface)) {
				tris_log(LOG_ERROR, "Invalid SMDI port specfied, disabling SMDI support\n");
				tmp->use_smdi = 0;
			}
		}

		tris_copy_string(tmp->accountcode, conf->chan.accountcode, sizeof(tmp->accountcode));
		...
		tris_copy_string(tmp->defcontext,conf->chan.context,sizeof(tmp->defcontext));
		tris_copy_string(tmp->language, conf->chan.language, sizeof(tmp->language));
		tris_copy_string(tmp->mohinterpret, conf->chan.mohinterpret, sizeof(tmp->mohinterpret));
		tris_copy_string(tmp->mohsuggest, conf->chan.mohsuggest, sizeof(tmp->mohsuggest));
		tris_copy_string(tmp->context, conf->chan.context, sizeof(tmp->context));
		tris_copy_string(tmp->parkinglot, conf->chan.parkinglot, sizeof(tmp->parkinglot));
		tmp->cid_ton = 0;
		if ((tmp->sig != SIG_PRI) || (tmp->sig != SIG_SS7) || (tmp->sig != SIG_BRI) || (tmp->sig != SIG_BRI_PTMP) || (tmp->sig != SIG_MFCR2)) {
			tris_copy_string(tmp->cid_num, conf->chan.cid_num, sizeof(tmp->cid_num));
			tris_copy_string(tmp->cid_name, conf->chan.cid_name, sizeof(tmp->cid_name));
		} else {
			tmp->cid_num[0] = '\0';
			tmp->cid_name[0] = '\0';
		}
		tris_copy_string(tmp->mailbox, conf->chan.mailbox, sizeof(tmp->mailbox));
		if (channel != CHAN_PSEUDO && !tris_strlen_zero(tmp->mailbox)) {
			char *mailbox, *context;
			mailbox = context = tris_strdupa(tmp->mailbox);
			strsep(&context, "@");
			if (tris_strlen_zero(context))
				context = "default";
			tmp->mwi_event_sub = tris_event_subscribe(TRIS_EVENT_MWI, mwi_event_cb, NULL,
				TRIS_EVENT_IE_MAILBOX, TRIS_EVENT_IE_PLTYPE_STR, mailbox,
				TRIS_EVENT_IE_CONTEXT, TRIS_EVENT_IE_PLTYPE_STR, context,
		...
		if (conf->chan.vars) {
			struct tris_variable *v, *tmpvar;
	                for (v = conf->chan.vars ; v ; v = v->next) {
        	                if ((tmpvar = tris_variable_new(v->name, v->value, v->file))) {
		...
		if (tmp->subs[SUB_REAL].dfd > -1) {
			...
			if (tmp->dsp)
				tris_dsp_set_digitmode(tmp->dsp, DSP_DIGITMODE_DTMF | tmp->dtmfrelax);
			update_conf(tmp);
			if (!here) {
				if ((chan_sig != SIG_BRI) && (chan_sig != SIG_BRI_PTMP) && (chan_sig != SIG_PRI)
				    && (chan_sig != SIG_SS7) && (chan_sig != SIG_MFCR2) && (chan_sig != SIG_DPNSS))
...
static inline int available(struct dahdi_pvt *p, int channelmatch, tris_group_t groupmatch, int *busy, int *channelmatched, int *groupmatched)
{
	int res;
	struct dahdi_params par;
	...
	if (!p->owner) {
		...
		if (!(p->radio || (p->oprmode < 0)))
		{
			...
			if (res) {
				tris_log(LOG_WARNING, "Unable to check hook state on channel %d: %s\n", p->channel, strerror(errno));
				...
				tris_debug(1, "Channel %d off hook, can't use\n", p->channel);
	...
	if ((p->owner->_state != TRIS_STATE_UP) &&
		((p->owner->_state != TRIS_STATE_RINGING) || p->outgoing)) {
...
static struct dahdi_pvt *duplicate_pseudo(struct dahdi_pvt *src)
{
	...
	if ((p = tris_malloc(sizeof(*p)))) {
		...
		if (p->subs[SUB_REAL].dfd < 0) {
			tris_log(LOG_ERROR, "Unable to dup channel: %s\n", strerror(errno));
			destroy_dahdi_pvt(&p);
			return NULL;
		}
		...
		if (!res) {
			...
			if (res < 0) {
				tris_log(LOG_WARNING, "Unable to set buffer policy on dup channel: %s\n", strerror(errno));
			}
		} else
			tris_log(LOG_WARNING, "Unable to check buffer policy on dup channel: %s\n", strerror(errno));
	}
	p->destroy = 1;
	p->next = iflist;
	p->prev = NULL;
...
static int pri_find_empty_chan(struct dahdi_pri *pri, int backwards)
{
	...
	for (;;) {
		...
		if (pri->pvts[x] && !pri->pvts[x]->inalarm && !pri->pvts[x]->owner) {
			tris_debug(1, "Found empty available channel %d/%d\n",
...
static struct tris_channel *dahdi_request(const char *type, int format, void *data, int *cause,struct tris_channel* src)
{
	...
	if (data) {
		dest = tris_strdupa((char *)data);
	} else {
		tris_log(LOG_WARNING, "Channel requested with no data\n");
		return NULL;
	}
	if (toupper(dest[0]) == 'G' || toupper(dest[0])=='R') {
		...
		if ((res = sscanf(s, "%30d%1c%30d", &x, &opt, &y)) < 1) {
			tris_log(LOG_WARNING, "Unable to determine group for data %s\n", (char *)data);
			return NULL;
		}
		groupmatch = ((tris_group_t) 1 << x);
		if (toupper(dest[0]) == 'G') {
			if (dest[0] == 'G') {
				backwards = 1;
				p = ifend;
		...
		if (!strcasecmp(s, "pseudo")) {
			...
			if ((trunkgroup < 1) || (crv < 1)) {
				tris_log(LOG_WARNING, "Unable to determine trunk group and CRV for data %s\n", (char *)data);
				return NULL;
			}
			res--;
			for (x = 0; x < NUM_SPANS; x++) {
			...
			if (!pri) {
				tris_log(LOG_WARNING, "Unable to find trunk group %d\n", trunkgroup);
				return NULL;
			}
			channelmatch = crv;
			p = pris[x].crvs;
			...
			tris_log(LOG_WARNING, "Unable to determine channel for data %s\n", (char *)data);
			return NULL;
		} else {
			channelmatch = x;
		}
		...
		if (p && available(p, channelmatch, groupmatch, &busy, &channelmatched, &groupmatched)) {
			...
			if (pri && (p->subs[SUB_REAL].dfd < 0)) {
				if (p->sig != SIG_FXSKS) {
					...
					if (bearer < 0) {
						tris_log(LOG_NOTICE, "Out of bearer channels on span %d for call to CRV %d:%d\n", pri->span, trunkgroup, crv);
						p = NULL;
						break;
					}
					...
					if (alloc_sub(p, 0)) {
						tris_log(LOG_NOTICE, "Failed to allocate place holder pseudo channel!\n");
						p = NULL;
						break;
					} else
						tris_debug(1, "Allocated placeholder pseudo channel\n");
			...
			if (p->dpnss) {
				...
				if (p->dpnsscall) {
					tris_mutex_unlock(&p->lock);
					tris_log(LOG_DEBUG, "Yay!, someone just beat us in the race for channel %d.\n", p->channel);
					goto next;
				}
				p->dpnsscall = 1;
				tris_mutex_unlock(&p->lock);
			...
			p->outgoing = 1;
			tmp = dahdi_new(p, TRIS_STATE_RESERVED, 0, p->owner ? SUB_CALLWAIT : SUB_REAL, 0, 0);
			if (!tmp) {
				p->outgoing = 0;
			}
#ifdef HAVE_PRI
			...
			if (res > 1) {
				if (opt == 'c') {
					...
					/* Distinctive ring */
					if (res < 3)
						tris_log(LOG_WARNING, "Distinctive ring missing identifier in '%s'\n", (char *)data);
					else
						p->distinctivering = y;
				} else if (opt == 'd') {
					...
					p->digital = 1;
					if (tmp)
						tmp->transfercapability = TRIS_TRANS_CAP_DIGITAL;
				} else {
					tris_log(LOG_WARNING, "Unknown option '%c' in '%s'\n", opt, (char *)data);
				}
			}
			/* Note if the call is a call waiting call */
			if (tmp && callwait)
				tmp->cdrflags |= TRIS_CDR_CALLWAIT;
			break;
		}
next:
		if (backwards) {
...
static void ss7_handle_cqm(struct dahdi_ss7 *linkset, int startcic, int endcic, unsigned int dpc)
{
	...
	if (p)
		...
		tris_log(LOG_WARNING, "Could not find any equipped circuits within CQM CICs\n");
...
static inline void ss7_hangup_cics(struct dahdi_ss7 *linkset, int startcic, int endcic, unsigned int dpc)
{
	...
	for (i = 0; i < linkset->numchans; i++) {
		if (linkset->pvts[i] && (linkset->pvts[i]->dpc == dpc && ((linkset->pvts[i]->cic >= startcic) && (linkset->pvts[i]->cic <= endcic)))) {
			tris_mutex_lock(&linkset->pvts[i]->lock);
			if (linkset->pvts[i]->owner)
				linkset->pvts[i]->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
			tris_mutex_unlock(&linkset->pvts[i]->lock);
		}
	}
}
#endif	/* defined(HAVE_SS7) */
...
static void ss7_reset_linkset(struct dahdi_ss7 *linkset)
{
	...
	for (i = 0; i < linkset->numchans; i++) {
		if (linkset->pvts[i+1] && linkset->pvts[i+1]->dpc == dpc && ((linkset->pvts[i+1]->cic - linkset->pvts[i]->cic) == 1) && (linkset->pvts[i]->cic - startcic < 31)) {
			...
			tris_verbose("Resetting CICs %d to %d\n", startcic, endcic);
...
static void dahdi_loopback(struct dahdi_pvt *p, int enable)
{
	if (p->loopedback != enable) {
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_LOOPBACK, &enable)) {
			tris_log(LOG_WARNING, "Unable to set loopback on channel %d: %s\n", p->channel, strerror(errno));
			return;
		}
		p->loopedback = enable;
	}
...
static void ss7_start_call(struct dahdi_pvt *p, struct dahdi_ss7 *linkset)
{
	...
	if (!c) {
		tris_log(LOG_WARNING, "Unable to start PBX on CIC %d\n", p->cic);
		/* Holding this lock is assumed entering the function */
		tris_mutex_lock(&linkset->lock);
		return;
	} else
		tris_verb(3, "Accepting call to '%s' on CIC %d\n", p->exten, p->cic);
	...
	if (!tris_strlen_zero(p->lspi_ident)) {
...
{
	if (tris_strlen_zero(number)) { /* make sure a number exists so prefix isn't placed on an empty string */
		if (size) {
			*buf = '\0';
		}
		return;
	...
	while(1) {
		...
		for (i = 0; i < linkset->numsigchans; i++) {
			if (pollers[i].revents & POLLPRI) {
				...
				if (ioctl(pollers[i].fd, DAHDI_GETEVENT, &x)) {
					tris_log(LOG_ERROR, "Error in exception retrieval!\n");
				}
				switch (x) {
				case DAHDI_EVENT_OVERRUN:
					tris_debug(1, "Overrun detected!\n");
					break;
				case DAHDI_EVENT_BADFCS:
					tris_debug(1, "Bad FCS\n");
					break;
				case DAHDI_EVENT_ABORT:
					tris_debug(1, "HDLC Abort\n");
					break;
				case DAHDI_EVENT_ALARM:
					tris_log(LOG_ERROR, "Alarm on link!\n");
					linkset->linkstate[i] |= (LINKSTATE_DOWN | LINKSTATE_INALARM);
					linkset->linkstate[i] &= ~LINKSTATE_UP;
					ss7_link_alarm(ss7, pollers[i].fd);
					break;
				case DAHDI_EVENT_NOALARM:
					tris_log(LOG_ERROR, "Alarm cleared on link\n");
					...
					break;
				default:
					tris_log(LOG_ERROR, "Got exception %d!\n", x);
					break;
				}
			}
		...
		while ((e = ss7_check_event(ss7))) {
			...
			case SS7_EVENT_UP:
				...
				tris_alarm(AL_TDMTRUNK_RECOVERY, NULL, NULL, NULL, NULL, NULL);
				linkset->state = LINKSET_STATE_UP;
				break;
			case SS7_EVENT_DOWN:
				tris_verbose("--- SS7 Down ---\n");
				tris_alarm(AL_TDMTRUNK_FAULT, NULL, NULL, NULL, NULL, NULL);
				...
				break;
			case MTP2_LINK_UP:
				tris_verbose("MTP2 link up (SLC %d)\n", e->gen.data);
				break;
			case MTP2_LINK_DOWN:
				tris_log(LOG_WARNING, "MTP2 link down (SLC %d)\n", e->gen.data);
				break;
			case ISUP_EVENT_CPG:
				chanpos = ss7_find_cic(linkset, e->cpg.cic, e->cpg.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "CPG on unconfigured CIC %d\n", e->cpg.cic);
					break;
				}
				...
				case CPG_EVENT_INBANDINFO:
					{
						struct tris_frame f = { TRIS_FRAME_CONTROL, TRIS_CONTROL_PROGRESS, };
						tris_debug(1, "Queuing frame PROGRESS on CIC %d\n", p->cic);
					...
					break;
				default:
					tris_debug(1, "Do not handle CPG with event type 0x%x\n", e->cpg.event);
				}

				tris_mutex_unlock(&p->lock);
				break;
			case ISUP_EVENT_RSC:
				tris_verbose("Resetting CIC %d\n", e->rsc.cic);
				chanpos = ss7_find_cic(linkset, e->rsc.cic, e->rsc.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "RSC on unconfigured CIC %d\n", e->rsc.cic);
					break;
				}
				p = linkset->pvts[chanpos];
				tris_mutex_lock(&p->lock);
				...
				break;
			case ISUP_EVENT_GRS:
				tris_debug(1, "Got Reset for CICs %d to %d: Acknowledging\n", e->grs.startcic, e->grs.endcic);
				chanpos = ss7_find_cic(linkset, e->grs.startcic, e->grs.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "GRS on unconfigured CIC %d\n", e->grs.startcic);
					break;
				}
				p = linkset->pvts[chanpos];
				...
				break;
			case ISUP_EVENT_CQM:
				tris_debug(1, "Got Circuit group query message from CICs %d to %d\n", e->cqm.startcic, e->cqm.endcic);
				ss7_handle_cqm(linkset, e->cqm.startcic, e->cqm.endcic, e->cqm.opc);
				break;
			case ISUP_EVENT_GRA:
				tris_verbose("Got reset acknowledgement from CIC %d to %d.\n", e->gra.startcic, e->gra.endcic);
				...
				break;
			case ISUP_EVENT_IAM:
 				tris_debug(1, "Got IAM for CIC %d and called number %s, calling number %s\n", e->iam.cic, e->iam.called_party_num, e->iam.calling_party_num);
				chanpos = ss7_find_cic(linkset, e->iam.cic, e->iam.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "IAM on unconfigured CIC %d\n", e->iam.cic);
				...
				if (p->owner) {
					if (p->ss7call == e->iam.call) {
						tris_mutex_unlock(&p->lock);
						tris_log(LOG_WARNING, "Duplicate IAM requested on CIC %d\n", e->iam.cic);
						break;
					} else {
						tris_mutex_unlock(&p->lock);
						tris_log(LOG_WARNING, "Ring requested on CIC %d already in use!\n", e->iam.cic);
						break;
					}
				}
				...
				if ((p->use_callerid) && (!tris_strlen_zero(e->iam.calling_party_num))) {
					char *st;
				...
				if (p->immediate) {
					p->exten[0] = 's';
					p->exten[1] = '\0';
				} else if (!tris_strlen_zero(e->iam.called_party_num)) {
					char *st;
				...
				p->cid_ani[0] = '\0';
				if ((p->use_callerid) && (!tris_strlen_zero(e->iam.generic_name)))
					tris_copy_string(p->cid_name, e->iam.generic_name, sizeof(p->cid_name));
				else
					p->cid_name[0] = '\0';

				p->cid_ani2 = e->iam.oli_ani2;
				p->cid_ton = 0;
				tris_copy_string(p->charge_number, e->iam.charge_number, sizeof(p->charge_number));
				tris_copy_string(p->gen_add_number, e->iam.gen_add_number, sizeof(p->gen_add_number));
				...
				tris_copy_string(p->gen_dig_number, e->iam.gen_dig_number, sizeof(p->gen_dig_number));
				p->gen_dig_type = e->iam.gen_dig_type;
				p->gen_dig_scheme = e->iam.gen_dig_scheme;
				tris_copy_string(p->jip_number, e->iam.jip_number, sizeof(p->jip_number));
				tris_copy_string(p->orig_called_num, e->iam.orig_called_num, sizeof(p->orig_called_num));
				tris_copy_string(p->redirecting_num, e->iam.redirecting_num, sizeof(p->redirecting_num));
				tris_copy_string(p->generic_name, e->iam.generic_name, sizeof(p->generic_name));
				p->calling_party_cat = e->iam.calling_party_cat;

				/* Set DNID */
				if (!tris_strlen_zero(e->iam.called_party_num))
				...
				if (tris_exists_extension(NULL, p->context, p->exten, 1, p->cid_num)) {
					...
					tris_debug(1, "Call on CIC for unconfigured extension %s\n", p->exten);
					p->alreadyhungup = 1;
					isup_rel(ss7, e->iam.call, TRIS_CAUSE_UNALLOCATED);
				}
				tris_mutex_unlock(&p->lock);
				break;
			case ISUP_EVENT_COT:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "COT on unconfigured CIC %d\n", e->cot.cic);
				...
				break;
			case ISUP_EVENT_CCR:
				tris_debug(1, "Got CCR request on CIC %d\n", e->ccr.cic);
				chanpos = ss7_find_cic(linkset, e->ccr.cic, e->ccr.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "CCR on unconfigured CIC %d\n", e->ccr.cic);
					break;
				}
				...
				break;
			case ISUP_EVENT_CVT:
				tris_debug(1, "Got CVT request on CIC %d\n", e->cvt.cic);
				chanpos = ss7_find_cic(linkset, e->cvt.cic, e->cvt.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "CVT on unconfigured CIC %d\n", e->cvt.cic);
					break;
				}
			...
			case ISUP_EVENT_REL:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "REL on unconfigured CIC %d\n", e->rel.cic);
					break;
				}
				p = linkset->pvts[chanpos];
				tris_mutex_lock(&p->lock);
				if (p->owner) {
					...
					tris_log(LOG_WARNING, "REL on channel (CIC %d) without owner!\n", p->cic);
			...
			case ISUP_EVENT_ACM:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "ACM on unconfigured CIC %d\n", e->acm.cic);
					...
					tris_debug(1, "Queueing frame from SS7_EVENT_ACM on CIC %d\n", p->cic);
			...
			case ISUP_EVENT_CGB:
 				...
 				if (chanpos < 0) {
 					tris_log(LOG_WARNING, "CGB on unconfigured CIC %d\n", e->cgb.startcic);
 					break;
 				}
 				p = linkset->pvts[chanpos];
			...
			case ISUP_EVENT_CGU:
 				...
 				if (chanpos < 0) {
 					tris_log(LOG_WARNING, "CGU on unconfigured CIC %d\n", e->cgu.startcic);
 					break;
 				}
 				p = linkset->pvts[chanpos];
			...
			case ISUP_EVENT_UCIC:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "UCIC on unconfigured CIC %d\n", e->ucic.cic);
					break;
				}
				p = linkset->pvts[chanpos];
				tris_debug(1, "Unequiped Circuit Id Code on CIC %d\n", e->ucic.cic);
				tris_mutex_lock(&p->lock);
				p->remotelyblocked = 1;
				p->inservice = 0;
				tris_mutex_unlock(&p->lock);			//doesn't require a SS7 acknowledgement
				break;
			case ISUP_EVENT_BLO:
				chanpos = ss7_find_cic(linkset, e->blo.cic, e->blo.opc);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "BLO on unconfigured CIC %d\n", e->blo.cic);
					break;
				}
				p = linkset->pvts[chanpos];
				tris_debug(1, "Blocking CIC %d\n", e->blo.cic);
				tris_mutex_lock(&p->lock);
				p->remotelyblocked = 1;
				tris_mutex_unlock(&p->lock);
			...
			case ISUP_EVENT_BLA:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "BLA on unconfigured CIC %d\n", e->bla.cic);
					break;
				}
				tris_debug(1, "Blocking CIC %d\n", e->bla.cic);
				p = linkset->pvts[chanpos];
			...
			case ISUP_EVENT_UBL:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "UBL on unconfigured CIC %d\n", e->ubl.cic);
					break;
				}
				p = linkset->pvts[chanpos];
				tris_debug(1, "Unblocking CIC %d\n", e->ubl.cic);
				tris_mutex_lock(&p->lock);
				p->remotelyblocked = 0;
				tris_mutex_unlock(&p->lock);
			...
			case ISUP_EVENT_UBA:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "UBA on unconfigured CIC %d\n", e->uba.cic);
					break;
				}
				p = linkset->pvts[chanpos];
				tris_debug(1, "Unblocking CIC %d\n", e->uba.cic);
			...
			case ISUP_EVENT_ANM:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "ANM/CON on unconfigured CIC %d\n", cic);
			...
			case ISUP_EVENT_RLC:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "RLC on unconfigured CIC %d\n", e->rlc.cic);
					break;
				} else {
					p = linkset->pvts[chanpos];
					tris_mutex_lock(&p->lock);
					if (p->alreadyhungup)
						...
						tris_log(LOG_NOTICE, "Received RLC out and we haven't sent REL.  Ignoring.\n");
					tris_mutex_unlock(&p->lock);
					}
					break;
			case ISUP_EVENT_FAA:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "FAA on unconfigured CIC %d\n", e->faa.cic);
					break;
				} else {
					p = linkset->pvts[chanpos];
					tris_debug(1, "FAA received on CIC %d\n", e->faa.cic);
					tris_mutex_lock(&p->lock);
					if (p->alreadyhungup){
						p->ss7call = NULL;
						tris_log(LOG_NOTICE, "Received FAA and we haven't sent FAR.  Ignoring.\n");
					}
					tris_mutex_unlock(&p->lock);
				}
				break;
			default:
				tris_debug(1, "Unknown event %s\n", ss7_event2str(e->e));
				break;
			}
		}
		tris_mutex_unlock(&linkset->lock);
...
static void dahdi_ss7_message(struct ss7 *ss7, char *s)
{
	...
	tris_verbose("[%d] %s", i+1, s);
#else
	tris_log(LOG_TRACE, "%s", s);
	tris_verb(7, "%s", s);
#endif
}
#endif	/* defined(HAVE_SS7) */
...
static void *dpnss_linkset(void *data)
{
	int res;
	struct timeval *next = NULL, tv;
	struct dahdi_dpnss *linkset = (struct dahdi_dpnss *) data;
	struct dpnss *dpnss = linkset->dpnss;
	...
	while(1) {
		tris_mutex_lock(&linkset->lock);
		if ((next = dpnss_schedule_next(dpnss))) {
			tv = tris_tvnow();
			tv.tv_sec = next->tv_sec - tv.tv_sec;
			tv.tv_usec = next->tv_usec - tv.tv_usec;
			if (tv.tv_usec < 0) {
		...
		pollers[0].fd = linkset->dfd;
		pollers[0].events = dpnss_pollflags(dpnss);
		pollers[0].revents = 0;

		res = poll(pollers,1, nextms);
		
		if ((res < 0) && (errno != EINTR)) {
			tris_log(LOG_ERROR, "poll(%s)\n", strerror(errno));
		} else if (!res) {
			tris_mutex_lock(&linkset->lock);
			dpnss_schedule_run(dpnss);
			tris_mutex_unlock(&linkset->lock);
			continue;
		}
		...
		if (pollers[0].revents & POLLPRI) {
			int x;
			if (ioctl(pollers[0].fd, DAHDI_GETEVENT, &x)) {
				tris_log(LOG_ERROR, "Error in exception retrieval!\n");
			}
			switch (x) {
				case DAHDI_EVENT_OVERRUN:
		...
		if (pollers[0].revents & POLLIN) {
			res = dpnss_read(dpnss);
		}
		if (pollers[0].revents & POLLOUT) {
			res = dpnss_write(dpnss);
			if (res < 0) {
				tris_debug(1, "Error in write %s\n", strerror(errno));
			}
		}
...
static void dahdi_dpnss_message(struct dpnss *dpnss,char *s){
	tris_log(LOG_TRACE, "%s", s);
	tris_verb(7, "%s", s);
}
...
static void *mfcr2_monitor(void *data)
{
	...
	while (1) {
		...
		for (i = 0; i < mfcr2->numchans; i++) {
			...
			if (!mfcr2->pvts[i]->r2chan) {
				tris_log(LOG_DEBUG, "Wow, no r2chan on channel %d\n", mfcr2->pvts[i]->channel);
				quit_loop = 1;
				break;
			}
		...
		if (pollsize == 0) {
			if (!was_idle) {
				tris_log(LOG_DEBUG, "Monitor thread going idle since everybody has an owner\n");
				was_idle = 1;
			}
		...
		pthread_testcancel();
		if ((res < 0) && (errno != EINTR)) {
			tris_log(LOG_ERROR, "going out, poll failed: %s\n", strerror(errno));
			break;
		}
	...
	tris_log(LOG_NOTICE, "Quitting MFC/R2 monitor thread\n");
	return 0;
}
#endif /* HAVE_OPENR2 */
...
static int pri_fixup_principle(struct dahdi_pri *pri, int principle, q931_call *c)
{
	...
	for (x = 0; x < pri->numchans; x++) {
		...
		if (pri->pvts[x]->call == c) {
			...
			if (principle != x) {
				...
				tris_verb(3, "Moving call from channel %d to channel %d\n",
					 old->channel, new->channel);
				if (new->owner) {
					tris_log(LOG_WARNING, "Can't fix up channel from %d to %d because %d is already in use\n",
				...
				old->owner = NULL;
				if (new->owner) {
					tris_string_field_build(new->owner, name,
						"DAHDI/%d:%d-%d", pri->trunkgroup,
						new->channel, 1);
					new->owner->tech_pvt = new;
					tris_channel_set_fd(new->owner, 0, new->subs[SUB_REAL].dfd);
					new->subs[SUB_REAL].owner = old->subs[SUB_REAL].owner;
					old->subs[SUB_REAL].owner = NULL;
				} else
					tris_log(LOG_WARNING, "Whoa, there's no  owner, and we're having to fix up channel %d to channel %d\n", old->channel, new->channel);
				new->call = old->call;
				old->call = NULL;
	...
	while (crv) {
		if (crv->call == c) {
			...
			if (crv->bearer)
				tris_log(LOG_WARNING, "Trying to fix up call which already has a bearer which isn't the one we think it is\n");
			else if (pri->pvts[principle]->owner)
				tris_log(LOG_WARNING, "Tring to fix up a call to a bearer which already has an owner!\n");
			else {
				...
				tris_debug(1, "Assigning bearer %d/%d to CRV %d:%d\n",
	...
	tris_log(LOG_WARNING, "Call specified, but not found?\n");
	return -1;
}
#endif	/* defined(HAVE_PRI) */
...
{
	struct tris_channel *chan = vchan;
	struct dahdi_pvt *pvt = chan->tech_pvt;
	struct tris_frame *f;
	char ex[80];
	/* Wait up to 30 seconds for an answer */
	int newms, ms = 30000;
	tris_verb(3, "Initiating idle call on channel %s\n", chan->name);
	snprintf(ex, sizeof(ex), "%d/%s", pvt->channel, pvt->pri->idledial);
	if (tris_call(chan, ex, 0)) {
		tris_log(LOG_WARNING, "Idle dial failed on '%s' to '%s'\n", chan->name, ex);
		tris_hangup(chan);
		return NULL;
	}
	while ((newms = tris_waitfor(chan, ms)) > 0) {
		f = tris_read(chan);
		if (!f) {
			/* Got hangup */
			break;
		...
		if (f->frametype == TRIS_FRAME_CONTROL) {
			...
			case TRIS_CONTROL_ANSWER:
				/* Launch the PBX */
				tris_copy_string(chan->exten, pvt->pri->idleext, sizeof(chan->exten));
				tris_copy_string(chan->context, pvt->pri->idlecontext, sizeof(chan->context));
				chan->priority = 1;
				tris_verb(4, "Idle channel '%s' answered, sending to %s@%s\n", chan->name, chan->exten, chan->context);
				tris_pbx_run(chan);
				/* It's already hungup, return immediately */
				return NULL;
			case TRIS_CONTROL_BUSY:
				tris_verb(4, "Idle channel '%s' busy, waiting...\n", chan->name);
				break;
			case TRIS_CONTROL_CONGESTION:
				tris_verb(4, "Idle channel '%s' congested, waiting...\n", chan->name);
				break;
			};
		}
		tris_frfree(f);
...
static void dahdi_pri_message(struct pri *pri, char *s)
{
	...
	if (pri) {
		...
		if (dchancount > 1 && (span > -1)) {
			tris_log(LOG_TRACE, "[Span %d D-Channel %d]%s", span, dchan, s);
			tris_verb(7, "[Span %d D-Channel %d]%s", span, dchan, s);
		} else {
			tris_log(LOG_TRACE, "%s", s);
			tris_verb(7, "%s", s);
		}
	...
	tris_mutex_lock(&pridebugfdlock);
	...
	tris_mutex_unlock(&pridebugfdlock);
}
#endif	/* defined(HAVE_PRI) */
...
static void dahdi_pri_error(struct pri *pri, char *s)
{
	...
	if (pri) {
		...
		if ((dchancount > 1) && (span > -1))
			tris_log(LOG_ERROR, "[Span %d D-Channel %d] PRI: %s", span, dchan, s);
		else
			tris_log(LOG_ERROR, "%s", s);
	} else
		tris_log(LOG_ERROR, "%s", s);
...
static int pri_hangup_all(struct dahdi_pvt *p, struct dahdi_pri *pri)
{
	...
	do {
		redo = 0;
		for (x = 0; x < 3; x++) {
			while (p->subs[x].owner && tris_channel_trylock(p->subs[x].owner)) {
				redo++;
				DEADLOCK_AVOIDANCE(&p->lock);
			}
			if (p->subs[x].owner) {
				tris_queue_hangup_with_cause(p->subs[x].owner, TRIS_CAUSE_PRE_EMPTED);
				tris_channel_unlock(p->subs[x].owner);
			}
		}
	} while (redo);
	tris_mutex_unlock(&p->lock);
...
static void *pri_dchannel(void *vpri)
{
	...
	struct timeval lastidle = tris_tvnow();
	int doidling=0;
	char *cc;
	char idlen[80];
	struct tris_channel *idle;
	pthread_t p;
	time_t t;
	int i, which=-1;
	int numdchans;
	...
	if (!tris_strlen_zero(pri->idledial) && !tris_strlen_zero(pri->idleext)) {
		/* Need to do idle dialing, check to be sure though */
		cc = strchr(pri->idleext, '@');
		if (cc) {
			*cc = '\0';
			cc++;
			tris_copy_string(pri->idlecontext, cc, sizeof(pri->idlecontext));
#if 0
			/* Extensions may not be loaded yet */
			if (!tris_exists_extension(NULL, pri->idlecontext, pri->idleext, 1, NULL))
				tris_log(LOG_WARNING, "Extension '%s @ %s' does not exist\n", pri->idleext, pri->idlecontext);
			else
#endif
				doidling = 1;
		} else
			tris_log(LOG_WARNING, "Idle dial string '%s' lacks '@context'\n", pri->idleext);
	}
	for (;;) {
		...
		if (doidling && pri_is_up(pri)) {
			...
			if (nextidle > -1) {
				if (tris_tvdiff_ms(tris_tvnow(), lastidle) > 1000) {
					/* Don't create a new idle call more than once per second */
					snprintf(idlen, sizeof(idlen), "%d/%s", pri->pvts[nextidle]->channel, pri->idledial);
					idle = dahdi_request("DAHDI", TRIS_FORMAT_ULAW, idlen, &cause,0);
					if (idle) {
						pri->pvts[nextidle]->isidlecall = 1;
						if (tris_pthread_create_background(&p, NULL, do_idle_thread, idle)) {
							tris_log(LOG_WARNING, "Unable to start new thread for idle channel '%s'\n", idle->name);
							dahdi_hangup(idle);
						}
					} else
						tris_log(LOG_WARNING, "Unable to request channel 'DAHDI/%s' for idle call\n", idlen);
					lastidle = tris_tvnow();
				}
				...
				for (x = pri->numchans; x >= 0; x--) {
					...
					if (pri->pvts[x] && pri->pvts[x]->owner && pri->pvts[x]->isidlecall) {
						pri->pvts[x]->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
						haveidles++;
		...
		if (!res) {
			...
			for (which = 0; which < NUM_DCHANS; which++) {
				...
				if (fds[which].revents & POLLPRI) {
					...
					if (x) {
						tris_log(LOG_NOTICE, "PRI got event: %s (%d) on %s D-channel of span %d\n", event2str(x), x, pri_order(which), pri->span);
					...
					tris_debug(1, "Got event %s (%d) on D-channel for span %d\n", event2str(x), x, pri->span);
			...
			tris_log(LOG_WARNING, "pri_event returned error %d (%s)\n", errno, strerror(errno));
		...
		if (e) {
			...
			if (e->e != PRI_EVENT_DCHAN_DOWN) {
				if (!(pri->dchanavail[which] & DCHAN_UP)) {
					tris_verb(2, "%s D-Channel on span %d up\n", pri_order(which), pri->span);
				}
				pri->dchanavail[which] |= DCHAN_UP;
			} else if (pri->sig != SIG_BRI_PTMP) {
				if (pri->dchanavail[which] & DCHAN_UP) {
					tris_verb(2, "%s D-Channel on span %d down\n", pri_order(which), pri->span);
				}
				pri->dchanavail[which] &= ~DCHAN_UP;
			}
							...
							if (!p->pri || !p->pri->pri || pri_get_timer(p->pri->pri, PRI_TIMER_T309) < 0) {
								...
								if (p->call) {
									if (p->pri && p->pri->pri) {
										...
										tris_log(LOG_WARNING, "The PRI Call have not been destroyed\n");
								}
								if (p->realcall) {
			...
			case PRI_EVENT_RESTART:
				if (e->restart.channel > -1) {
					...
					if (chanpos < 0)
						tris_log(LOG_WARNING, "Restart requested on odd/unavailable channel number %d/%d on span %d\n",
							PRI_SPAN(e->restart.channel), PRI_CHANNEL(e->restart.channel), pri->span);
					else {
						tris_verb(3, "B-channel %d/%d restarted on span %d\n",
								PRI_SPAN(e->restart.channel), PRI_CHANNEL(e->restart.channel), pri->span);
						tris_mutex_lock(&pri->pvts[chanpos]->lock);
						...
						if (pri->pvts[chanpos]->realcall)
							...
							pri->pvts[chanpos]->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
						tris_mutex_unlock(&pri->pvts[chanpos]->lock);
					}
				} else {
					tris_verb(3, "Restart on requested on entire span %d\n", pri->span);
					for (x = 0; x < pri->numchans; x++)
						if (pri->pvts[x]) {
							tris_mutex_lock(&pri->pvts[x]->lock);
							if (pri->pvts[x]->call) {
							...
							if (pri->pvts[x]->realcall)
								...
								pri->pvts[x]->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
							tris_mutex_unlock(&pri->pvts[x]->lock);
						}
				}
				break;
			case PRI_EVENT_KEYPAD_DIGIT:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "KEYPAD_DIGITs received on unconfigured channel %d/%d span %d\n",
					...
					if (chanpos > -1) {
						...
						if ((pri->overlapdial & DAHDI_OVERLAPDIAL_INCOMING) && pri->pvts[chanpos]->call==e->digit.call && pri->pvts[chanpos]->owner) {
							...
							for (i = 0; i < digitlen; i++) {
								...
								{
									struct tris_frame f = { TRIS_FRAME_DTMF, digit, };
			...
			case PRI_EVENT_INFO_RECEIVED:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "INFO received on unconfigured channel %d/%d span %d\n",
			...
			case PRI_EVENT_RING:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Ring requested on unconfigured channel %d/%d span %d\n",
						PRI_SPAN(e->ring.channel), PRI_CHANNEL(e->ring.channel), pri->span);
				} else {
					tris_mutex_lock(&pri->pvts[chanpos]->lock);
					if (pri->pvts[chanpos]->owner) {
						if (pri->pvts[chanpos]->call == e->ring.call) {
							tris_log(LOG_WARNING, "Duplicate setup requested on channel %d/%d already in use on span %d\n",
								PRI_SPAN(e->ring.channel), PRI_CHANNEL(e->ring.channel), pri->span);
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
							break;
						} else {
							/* This is where we handle initial glare */
							tris_debug(1, "Ring requested on channel %d/%d already in use or previously requested on span %d.  Attempting to renegotiate channel.\n",
							PRI_SPAN(e->ring.channel), PRI_CHANNEL(e->ring.channel), pri->span);
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
							chanpos = -1;
						}
				...
				if (chanpos > -1) {
					...
					if (pri->switchtype == PRI_SWITCH_GR303_TMC) {
						...
						if (crv)
							tris_mutex_lock(&crv->lock);
						if (!crv || crv->owner) {
							pri->pvts[chanpos]->call = NULL;
							if (crv) {
								if (crv->owner)
									crv->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
								tris_log(LOG_WARNING, "Call received for busy CRV %d on span %d\n", pri_get_crv(pri->pri, e->ring.call, NULL), pri->span);
							} else
								tris_log(LOG_NOTICE, "Call received for unconfigured CRV %d on span %d\n", pri_get_crv(pri->pri, e->ring.call, NULL), pri->span);
							pri_hangup(pri->pri, e->ring.call, PRI_CAUSE_INVALID_CALL_REFERENCE);
							if (crv)
								tris_mutex_unlock(&crv->lock);
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
							break;
						}
					}
					...
					if (pri->pvts[chanpos]->use_callerid) {
						tris_shrink_phone_number(plancallingnum);
						tris_copy_string(pri->pvts[chanpos]->cid_num, plancallingnum, sizeof(pri->pvts[chanpos]->cid_num));
#ifdef PRI_ANI
						if (!tris_strlen_zero(e->ring.callingani)) {
							apply_plan_to_number(plancallingani, sizeof(plancallingani), pri, e->ring.callingani, e->ring.callingplanani);
							tris_shrink_phone_number(plancallingani);
							tris_copy_string(pri->pvts[chanpos]->cid_ani, plancallingani, sizeof(pri->pvts[chanpos]->cid_ani));
						} else {
							pri->pvts[chanpos]->cid_ani[0] = '\0';
						}
#endif
						tris_copy_string(pri->pvts[chanpos]->cid_name, e->ring.callingname, sizeof(pri->pvts[chanpos]->cid_name));
					...
					if (pri->pvts[chanpos]->immediate) {
						tris_verb(3, "Going to extension s|1 because of immediate=yes\n");
						pri->pvts[chanpos]->exten[0] = 's';
						pri->pvts[chanpos]->exten[1] = '\0';
					}
					/* Get called number */
					else if (!tris_strlen_zero(e->ring.callednum)) {
						tris_copy_string(pri->pvts[chanpos]->exten, e->ring.callednum, sizeof(pri->pvts[chanpos]->exten));
						tris_copy_string(pri->pvts[chanpos]->dnid, e->ring.callednum, sizeof(pri->pvts[chanpos]->dnid));
					...
					if (!tris_strlen_zero(e->ring.callednum))
						tris_copy_string(pri->pvts[chanpos]->dnid, e->ring.callednum, sizeof(pri->pvts[chanpos]->dnid));
					/* No number yet, but received "sending complete"? */
					if (e->ring.complete && (tris_strlen_zero(e->ring.callednum))) {
						tris_verb(3, "Going to extension s|1 because of Complete received\n");
						pri->pvts[chanpos]->exten[0] = 's';
						pri->pvts[chanpos]->exten[1] = '\0';
					}

					/* Make sure extension exists (or in overlap dial mode, can exist) */
					if (((pri->overlapdial & DAHDI_OVERLAPDIAL_INCOMING) && tris_canmatch_extension(NULL, pri->pvts[chanpos]->context, pri->pvts[chanpos]->exten, 1, pri->pvts[chanpos]->cid_num)) ||
						tris_exists_extension(NULL, pri->pvts[chanpos]->context, pri->pvts[chanpos]->exten, 1, pri->pvts[chanpos]->cid_num)) {
						/* Setup law */
						int law;
						if (pri->switchtype != PRI_SWITCH_GR303_TMC) {
						...
						if (res < 0)
							tris_log(LOG_WARNING, "Unable to set gains on channel %d\n", pri->pvts[chanpos]->channel);
						...
						if (!e->ring.complete
							...
							&& tris_matchmore_extension(NULL, pri->pvts[chanpos]->context, pri->pvts[chanpos]->exten, 1, pri->pvts[chanpos]->cid_num)) {
							/*
							...
							if (crv) {
								...
								c = dahdi_new(crv, TRIS_STATE_RESERVED, 0, SUB_REAL, law, e->ring.ctype);
								pri->pvts[chanpos]->owner = &inuse;
								tris_debug(1, "Started up crv %d:%d on bearer channel %d\n", pri->trunkgroup, crv->channel, crv->bearer->channel);
							} else {
								c = dahdi_new(pri->pvts[chanpos], TRIS_STATE_RESERVED, 0, SUB_REAL, law, e->ring.ctype);
							}
							tris_mutex_lock(&pri->lock);
							if (c) {
								if (!tris_strlen_zero(e->ring.callingsubaddr)) {
...
#ifdef SUPPORT_USERUSER
								if (!tris_strlen_zero(e->ring.useruserinfo)) {
							...
							if (c && !tris_pthread_create_detached(&threadid, NULL, ss_thread, c)) {
								tris_verb(3, "Accepting overlap call from '%s' to '%s' on channel %d/%d, span %d\n",
									plancallingnum, S_OR(pri->pvts[chanpos]->exten, "<unspecified>"),
									pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset, pri->span);
							} else {
								tris_log(LOG_WARNING, "Unable to start PBX on channel %d/%d, span %d\n",
									pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset, pri->span);
								if (c)
									tris_hangup(c);
								else {
							 ...
							 */
							tris_mutex_unlock(&pri->lock);
							c = dahdi_new(pri->pvts[chanpos], TRIS_STATE_RING, 0, SUB_REAL, law, e->ring.ctype);
							tris_mutex_lock(&pri->lock);
							if (c) {
								/*
							...
							if (c && !tris_pbx_start(c)) {
								tris_verb(3, "Accepting call from '%s' to '%s' on channel %d/%d, span %d\n",
						...
						tris_verb(3, "Extension '%s' in context '%s' from '%s' does not exist.  Rejecting call on channel %d/%d, span %d\n",
			...
			case PRI_EVENT_RINGING:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Ringing requested on unconfigured channel %d/%d span %d\n",
						PRI_SPAN(e->ringing.channel), PRI_CHANNEL(e->ringing.channel), pri->span);
				} else {
					chanpos = pri_fixup_principle(pri, chanpos, e->ringing.call);
					if (chanpos < 0) {
						tris_log(LOG_WARNING, "Ringing requested on channel %d/%d not in use on span %d\n",
							PRI_SPAN(e->ringing.channel), PRI_CHANNEL(e->ringing.channel), pri->span);
					} else {
						tris_mutex_lock(&pri->pvts[chanpos]->lock);
						if (tris_strlen_zero(pri->pvts[chanpos]->dop.dialstr)) {
							dahdi_enable_ec(pri->pvts[chanpos]);
							pri->pvts[chanpos]->subs[SUB_REAL].needringing = 1;
							pri->pvts[chanpos]->alerting = 1;
						} else
							tris_debug(1, "Deferring ringing notification because of extra digits to dial...\n");
						...
						if (
							...
							if (pri->pvts[chanpos]->dsp && pri->pvts[chanpos]->dsp_features) {
								...
								tris_dsp_set_features(pri->pvts[chanpos]->dsp, pri->pvts[chanpos]->dsp_features & ~DSP_PROGRESS_RINGING);
								pri->pvts[chanpos]->dsp_features = 0;
							}
						}

#ifdef SUPPORT_USERUSER
						if (!tris_strlen_zero(e->ringing.useruserinfo)) {
							struct tris_channel *owner = pri->pvts[chanpos]->owner;
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
				...
				if (chanpos > -1) {
					if ((!pri->pvts[chanpos]->progress)
						...
						if (e->proceeding.cause > -1) {
							tris_verb(3, "PROGRESS with cause code %d received\n", e->proceeding.cause);

							/* Work around broken, out of spec USER_BUSY cause in a progress message */
							if (e->proceeding.cause == TRIS_CAUSE_USER_BUSY) {
								if (pri->pvts[chanpos]->owner) {
									tris_verb(3, "PROGRESS with 'user busy' received, signalling TRIS_CONTROL_BUSY instead of TRIS_CONTROL_PROGRESS\n");
						...
						tris_mutex_lock(&pri->pvts[chanpos]->lock);
						tris_debug(1, "Queuing frame from PRI_EVENT_PROGRESS on channel %d/%d span %d\n",
							pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset,pri->span);
						dahdi_queue_frame(pri->pvts[chanpos], &f, pri);
						if (
#ifdef PRI_PROGRESS_MASK
							...
							/* Now we can do call progress detection */
							if (pri->pvts[chanpos]->dsp && pri->pvts[chanpos]->dsp_features) {
								tris_dsp_set_features(pri->pvts[chanpos]->dsp, pri->pvts[chanpos]->dsp_features);
								pri->pvts[chanpos]->dsp_features = 0;
							}
							/* Bring voice path up */
							f.subclass = TRIS_CONTROL_PROGRESS;
							dahdi_queue_frame(pri->pvts[chanpos], &f, pri);
						}
			...
			case PRI_EVENT_PROCEEDING:
				...
				if (chanpos > -1) {
					if (!pri->pvts[chanpos]->proceeding) {
						...
						tris_mutex_lock(&pri->pvts[chanpos]->lock);
						tris_debug(1, "Queuing frame from PRI_EVENT_PROCEEDING on channel %d/%d span %d\n",
							pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset,pri->span);
						dahdi_queue_frame(pri->pvts[chanpos], &f, pri);
						if (
#ifdef PRI_PROGRESS_MASK
			...
			case PRI_EVENT_FACNAME:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Facility Name requested on unconfigured channel %d/%d span %d\n",
						PRI_SPAN(e->facname.channel), PRI_CHANNEL(e->facname.channel), pri->span);
				} else {
					chanpos = pri_fixup_principle(pri, chanpos, e->facname.call);
					if (chanpos < 0) {
						tris_log(LOG_WARNING, "Facility Name requested on channel %d/%d not in use on span %d\n",
							PRI_SPAN(e->facname.channel), PRI_CHANNEL(e->facname.channel), pri->span);
					} else {
						/* Re-use *69 field for PRI */
						tris_mutex_lock(&pri->pvts[chanpos]->lock);
						tris_copy_string(pri->pvts[chanpos]->lastcid_num, e->facname.callingnum, sizeof(pri->pvts[chanpos]->lastcid_num));
						tris_copy_string(pri->pvts[chanpos]->lastcid_name, e->facname.callingname, sizeof(pri->pvts[chanpos]->lastcid_name));
			...
			case PRI_EVENT_ANSWER:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Answer on unconfigured channel %d/%d span %d\n",
						PRI_SPAN(e->answer.channel), PRI_CHANNEL(e->answer.channel), pri->span);
				} else {
					chanpos = pri_fixup_principle(pri, chanpos, e->answer.call);
					if (chanpos < 0) {
						tris_log(LOG_WARNING, "Answer requested on channel %d/%d not in use on span %d\n",
							PRI_SPAN(e->answer.channel), PRI_CHANNEL(e->answer.channel), pri->span);
					} else {
						tris_mutex_lock(&pri->pvts[chanpos]->lock);
						/* Now we can do call progress detection */
						...
						if (pri->pvts[chanpos]->realcall && (pri->pvts[chanpos]->realcall->sig == SIG_FXSKS)) {
							tris_debug(1, "Starting up GR-303 trunk now that we got CONNECT...\n");
							x = DAHDI_START;
						...
						} else if (!tris_strlen_zero(pri->pvts[chanpos]->dop.dialstr)) {
							...
							tris_debug(1, "Waiting on answer confirmation on channel %d!\n", pri->pvts[chanpos]->channel);
						} else {
							pri->pvts[chanpos]->dialing = 0;
...
#ifdef SUPPORT_USERUSER
						if (!tris_strlen_zero(e->answer.useruserinfo)) {
							struct tris_channel *owner = pri->pvts[chanpos]->owner;
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
			...
			case PRI_EVENT_HANGUP:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Hangup requested on unconfigured channel %d/%d span %d\n",
					...
					if (chanpos > -1) {
						...
						if (!pri->pvts[chanpos]->alreadyhungup) {
							...
							tris_verb(3, "Channel %d/%d, span %d got hangup, cause %d\n",
						...
						if (e->hangup.cause == PRI_CAUSE_REQUESTED_CHAN_UNAVAIL) {
							tris_verb(3, "Forcing restart of channel %d/%d on span %d since channel reported in use\n",
									PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
							pri_reset(pri->pri, PVT_TO_CHANNEL(pri->pvts[chanpos]));
							pri->pvts[chanpos]->resetting = 1;
						}
						if (e->hangup.aoc_units > -1)
							tris_verb(3, "Channel %d/%d, span %d received AOC-E charging %d unit%s\n",
									pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset, pri->span, (int)e->hangup.aoc_units, (e->hangup.aoc_units == 1) ? "" : "s");

#ifdef SUPPORT_USERUSER
						if (pri->pvts[chanpos]->owner && !tris_strlen_zero(e->hangup.useruserinfo)) {
							struct tris_channel *owner = pri->pvts[chanpos]->owner;
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
						...
						tris_log(LOG_WARNING, "Hangup on bad channel %d/%d on span %d\n",
							PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
					}
				}
				break;
...
#error please update libpri
			...
			case PRI_EVENT_HANGUP_REQ:
				chanpos = pri_find_principle(pri, e->hangup.channel);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Hangup REQ requested on unconfigured channel %d/%d span %d\n",
						PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
				} else {
					chanpos = pri_fixup_principle(pri, chanpos, e->hangup.call);
					if (chanpos > -1) {
						...
						if (pri->pvts[chanpos]->realcall)
							...
							tris_verb(3, "Channel %d/%d, span %d got hangup request, cause %d\n", PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span, e->hangup.cause);
							if (e->hangup.aoc_units > -1)
								tris_verb(3, "Channel %d/%d, span %d received AOC-E charging %d unit%s\n",
										pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset, pri->span, (int)e->hangup.aoc_units, (e->hangup.aoc_units == 1) ? "" : "s");
						} else {
						...
						if (e->hangup.cause == PRI_CAUSE_REQUESTED_CHAN_UNAVAIL) {
							tris_verb(3, "Forcing restart of channel %d/%d span %d since channel reported in use\n",
									PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
							pri_reset(pri->pri, PVT_TO_CHANNEL(pri->pvts[chanpos]));
							pri->pvts[chanpos]->resetting = 1;
						}
...
#ifdef SUPPORT_USERUSER
						if (!tris_strlen_zero(e->hangup.useruserinfo)) {
							struct tris_channel *owner = pri->pvts[chanpos]->owner;
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
							pbx_builtin_setvar_helper(owner, "USERUSERINFO", e->hangup.useruserinfo);
							tris_mutex_lock(&pri->pvts[chanpos]->lock);
						...
						tris_log(LOG_WARNING, "Hangup REQ on bad channel %d/%d on span %d\n", PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
					}
				}
				break;
			case PRI_EVENT_HANGUP_ACK:
				chanpos = pri_find_principle(pri, e->hangup.channel);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Hangup ACK requested on unconfigured channel number %d/%d span %d\n",
						PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
				} else {
					chanpos = pri_fixup_principle(pri, chanpos, e->hangup.call);
					if (chanpos > -1) {
						...
						if (pri->pvts[chanpos]->owner) {
							tris_verb(3, "Channel %d/%d, span %d got hangup ACK\n", PRI_SPAN(e->hangup.channel), PRI_CHANNEL(e->hangup.channel), pri->span);
						}
			...
			case PRI_EVENT_CONFIG_ERR:
				tris_log(LOG_WARNING, "PRI Error on span %d: %s\n", pri->trunkgroup, e->err.err);
				break;
				...
				if (chanpos < 0) {
					...
					for (x = 0; x < pri->numchans; x++) {
						if (pri->pvts[x] && pri->pvts[x]->resetting) {
							chanpos = x;
							tris_mutex_lock(&pri->pvts[chanpos]->lock);
							tris_debug(1, "Assuming restart ack is really for channel %d/%d span %d\n", pri->pvts[chanpos]->logicalspan,
								pri->pvts[chanpos]->prioffset, pri->span);
							if (pri->pvts[chanpos]->realcall)
								pri_hangup_all(pri->pvts[chanpos]->realcall, pri);
							else if (pri->pvts[chanpos]->owner) {
								tris_log(LOG_WARNING, "Got restart ack on channel %d/%d with owner on span %d\n", pri->pvts[chanpos]->logicalspan,
									pri->pvts[chanpos]->prioffset, pri->span);
								pri->pvts[chanpos]->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
							}
							pri->pvts[chanpos]->resetting = 0;
							tris_verb(3, "B-channel %d/%d successfully restarted on span %d\n", pri->pvts[chanpos]->logicalspan,
									pri->pvts[chanpos]->prioffset, pri->span);
							tris_mutex_unlock(&pri->pvts[chanpos]->lock);
							if (pri->resetting)
								pri_check_restart(pri);
					...
					if (chanpos < 0) {
						tris_log(LOG_WARNING, "Restart ACK requested on strange channel %d/%d span %d\n",
					...
					if (pri->pvts[chanpos]) {
						...
						if (pri->pvts[chanpos]->realcall)
							...
							tris_log(LOG_WARNING, "Got restart ack on channel %d/%d span %d with owner\n",
								PRI_SPAN(e->restartack.channel), PRI_CHANNEL(e->restartack.channel), pri->span);
							pri->pvts[chanpos]->owner->_softhangup |= TRIS_SOFTHANGUP_DEV;
						}
						pri->pvts[chanpos]->resetting = 0;
			...
			case PRI_EVENT_SETUP_ACK:
				...
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Received SETUP_ACKNOWLEDGE on unconfigured channel %d/%d span %d\n",
					...
					if (chanpos > -1) {
						...
						for (x = 0;x < strlen(pri->pvts[chanpos]->dialdest); x++) {
							tris_debug(1, "Sending pending digit '%c'\n", pri->pvts[chanpos]->dialdest[x]);
							pri_information(pri->pri, pri->pvts[chanpos]->call,
								pri->pvts[chanpos]->dialdest[x]);
						}
						tris_mutex_unlock(&pri->pvts[chanpos]->lock);
					} else
						tris_log(LOG_WARNING, "Unable to move channel %d!\n", e->setup_ack.channel);
				}
				break;
			case PRI_EVENT_NOTIFY:
				chanpos = pri_find_principle(pri, e->notify.channel);
				if (chanpos < 0) {
					tris_log(LOG_WARNING, "Received NOTIFY on unconfigured channel %d/%d span %d\n",
...
static int start_pri(struct dahdi_pri *pri)
{
	...
	for (i = 0; i < NUM_DCHANS; i++) {
		...
		if ((pri->fds[i] < 0) || (ioctl(pri->fds[i],DAHDI_SPECIFY,&x) == -1)) {
			tris_log(LOG_ERROR, "Unable to open D-channel %d (%s)\n", x, strerror(errno));
			return -1;
		}
		memset(&p, 0, sizeof(p));
		...
		if (res) {
			...
			tris_log(LOG_ERROR, "Unable to get parameters for D-channel %d (%s)\n", x, strerror(errno));
			return -1;
		}
		if ((p.sigtype != DAHDI_SIG_HDLCFCS) && (p.sigtype != DAHDI_SIG_HARDHDLC)) {
			dahdi_close_pri_fd(pri, i);
			tris_log(LOG_ERROR, "D-channel %d is not in HDLC/FCS mode.\n", x);
			return -1;
		}
		memset(&si, 0, sizeof(si));
		...
		if (res) {
			dahdi_close_pri_fd(pri, i);
			tris_log(LOG_ERROR, "Unable to get span state for D-channel %d (%s)\n", x, strerror(errno));
		}
		if (!si.alarms)
		...
		if (!pri->dchans[i]) {
			dahdi_close_pri_fd(pri, i);
			tris_log(LOG_ERROR, "Unable to create PRI structure\n");
			return -1;
		}
	...
	pri->resetpos = -1;
	if (tris_pthread_create_background(&pri->master, NULL, pri_dchannel, pri)) {
		for (i = 0; i < NUM_DCHANS; i++) {
			if (!pri->dchannels[i])
				break;
			dahdi_close_pri_fd(pri, i);
		}
		tris_log(LOG_ERROR, "Unable to spawn D-channel: %s\n", strerror(errno));
		return -1;
	}
	return 0;
}
...
#if defined(HAVE_PRI)
static char *handle_pri_set_debug_file(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int myfd;
	switch (cmd) {
	case CLI_INIT:
	...
	if (tris_strlen_zero(a->argv[4]))
		return CLI_SHOWUSAGE;

	myfd = open(a->argv[4], O_CREAT|O_WRONLY, TRIS_FILE_MODE);
	if (myfd < 0) {
		tris_cli(a->fd, "Unable to open '%s' for writing\n", a->argv[4]);
		return CLI_SUCCESS;
	}
	...
	pridebugfd = myfd;
	tris_copy_string(pridebugfilename,a->argv[4],sizeof(pridebugfilename));
	tris_mutex_unlock(&pridebugfdlock);
	tris_cli(a->fd, "PRI debug output will be sent to '%s'\n", a->argv[4]);
	return CLI_SUCCESS;
}
#endif	/* defined(HAVE_PRI) */

#if defined(HAVE_PRI)
static char *handle_pri_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int span;
	int x;
	int level = 0;
	...
	if ((span < 1) || (span > NUM_SPANS)) {
		tris_cli(a->fd, "Invalid span %s.  Should be a number %d to %d\n", a->argv[5], 1, NUM_SPANS);
		return CLI_SUCCESS;
	}
	if (!pris[span-1].pri) {
		tris_cli(a->fd, "No PRI running on span %d\n", span);
		return CLI_SUCCESS;
	}
	for (x = 0; x < NUM_DCHANS; x++) {
		if (pris[span-1].dchans[x]) {
			if (level == 1) {
				...
				tris_cli(a->fd, "Enabled debugging on span %d\n", span);
			} else if (level == 0) {
				...
				close(pridebugfd);
				pridebugfd = -1;
				tris_cli(a->fd, "PRI debug output to file disabled\n");
				tris_mutex_unlock(&pridebugfdlock);
			} else {
				pri_set_debug(pris[span-1].dchans[x], PRI_DEBUG_APDU |
					PRI_DEBUG_Q931_DUMP | PRI_DEBUG_Q931_STATE |
...
#if defined(HAVE_PRI)
static char *handle_pri_show_spans(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int span;
	int x;
	char status[256];
...
#if defined(HAVE_PRI)
static char *handle_pri_show_span(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int span;
	int x;
	char status[256];
	...
	span = atoi(a->argv[3]);
	if ((span < 1) || (span > NUM_SPANS)) {
		tris_cli(a->fd, "Invalid span '%s'.  Should be a number from %d to %d\n", a->argv[3], 1, NUM_SPANS);
		return CLI_SUCCESS;
	}
	if (!pris[span-1].pri) {
		tris_cli(a->fd, "No PRI running on span %d\n", span);
...
#if defined(HAVE_PRI)
static char *handle_pri_show_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int x;
	int span;
	int count=0;
		...
		if (pris[span].pri) {
			for (x = 0; x < NUM_DCHANS; x++) {
				...
				if (pris[span].dchans[x]) {
					...
					tris_cli(a->fd, "Span %d: Debug: %s\tIntense: %s\n", span+1, (debug&PRI_DEBUG_Q931_STATE)? "Yes" : "No" ,(debug&PRI_DEBUG_Q921_RAW)? "Yes" : "No" );
					count++;
				}
			}
		}
	...
	tris_mutex_lock(&pridebugfdlock);
	if (pridebugfd >= 0)
		tris_cli(a->fd, "Logging PRI debug to file %s\n", pridebugfilename);
	tris_mutex_unlock(&pridebugfdlock);

	if (!count)
		tris_cli(a->fd, "No debug set or no PRI running\n");
	return CLI_SUCCESS;
}
#endif	/* defined(HAVE_PRI) */

#if defined(HAVE_PRI)
static char *handle_pri_version(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	tris_cli(a->fd, "libpri version: %s\n", pri_get_version());
...
#if defined(HAVE_PRI)
static struct tris_cli_entry dahdi_pri_cli[] = {
	TRIS_CLI_DEFINE(handle_pri_debug, "Enables PRI debugging on a span"),
	TRIS_CLI_DEFINE(handle_pri_show_spans, "Displays PRI Information"),
	TRIS_CLI_DEFINE(handle_pri_show_span, "Displays PRI Information"),
	TRIS_CLI_DEFINE(handle_pri_show_debug, "Displays current PRI debug settings"),
	TRIS_CLI_DEFINE(handle_pri_set_debug_file, "Sends PRI debug output to the specified file"),
	TRIS_CLI_DEFINE(handle_pri_version, "Displays libpri version"),
};
#endif	/* defined(HAVE_PRI) */

#ifdef HAVE_OPENR2

static char *handle_mfcr2_version(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	tris_cli(a->fd, "OpenR2 version: %s, revision: %s\n", openr2_get_version(), openr2_get_revision());
	return CLI_SUCCESS;
}

static char *handle_mfcr2_show_variants(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
#define FORMAT "%4s %40s\n"
	...
	if (!(variants = openr2_proto_get_variant_list(&numvariants))) {
		tris_cli(a->fd, "Failed to get list of variants.\n");
		return CLI_FAILURE;
	}
	tris_cli(a->fd, FORMAT, "Variant Code", "Country");
	for (i = 0; i < numvariants; i++) {
		tris_cli(a->fd, FORMAT, variants[i].name, variants[i].country);
	}
	return CLI_SUCCESS;
#undef FORMAT
}

static char *handle_mfcr2_show_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
#define FORMAT "%4s %-7.7s %-7.7s %-8.8s %-9.9s %-16.16s %-8.8s %-8.8s\n"
	...
	tris_cli(a->fd, FORMAT, "Chan", "Variant", "Max ANI", "Max DNIS", "ANI First", "Immediate Accept", "Tx CAS", "Rx CAS");
	tris_mutex_lock(&iflock);
	p = iflist;
	for (p = iflist; p; p = p->next) {
		...
		tris_cli(a->fd, FORMAT, channo, openr2_proto_get_variant_string(r2variant),
...
static char *handle_mfcr2_set_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct dahdi_pvt *p = NULL;
	int channo = 0;
	char *toklevel = NULL;
	...
	if (-1 == (tmplevel = openr2_log_get_level(toklevel))) {
		...
		while ((toklevel = strtok_r(NULL, ",", &saveptr))) {
			if (-1 == (tmplevel = openr2_log_get_level(toklevel))) {
				tris_cli(a->fd, "Ignoring invalid logging level: '%s'.\n", toklevel);
				continue;
			}
			loglevel |= tmplevel;
		}
	...
	for (p = iflist; p; p = p->next) {
		...
		if (channo != -1) {
			tris_cli(a->fd, "MFC/R2 debugging set to '%s' for channel %d.\n", a->argv[3], p->channel);
			break;
		}
	}
	if ((channo != -1) && !p) {
		tris_cli(a->fd, "MFC/R2 channel %d not found.\n", channo);
	}
	if (channo == -1) {
		tris_cli(a->fd, "MFC/R2 debugging set to '%s' for all channels.\n", a->argv[3]);
	}
	tris_mutex_unlock(&iflock);
	return CLI_SUCCESS;
}

static char *handle_mfcr2_call_files(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct dahdi_pvt *p = NULL;
	int channo = 0;
	switch (cmd) {
	...
	for (p = iflist; p; p = p->next) {
		...
		if (channo != -1) {
			if (tris_true(a->argv[3])) {
				tris_cli(a->fd, "MFC/R2 call files enabled for channel %d.\n", p->channel);
			} else {
				tris_cli(a->fd, "MFC/R2 call files disabled for channel %d.\n", p->channel);
			}
			break;
		}
	}
	...
	if (channo == -1) {
		if (tris_true(a->argv[3])) {
			tris_cli(a->fd, "MFC/R2 Call files enabled for all channels.\n");
		} else {
			tris_cli(a->fd, "MFC/R2 Call files disabled for all channels.\n");
		}
	}
	tris_mutex_unlock(&iflock);
	return CLI_SUCCESS;
...
static char *handle_mfcr2_set_idle(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct dahdi_pvt *p = NULL;
	int channo = 0;
	switch (cmd) {
...
static char *handle_mfcr2_set_blocked(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct dahdi_pvt *p = NULL;
	int channo = 0;
	switch (cmd) {
...
static struct tris_cli_entry dahdi_mfcr2_cli[] = {
	TRIS_CLI_DEFINE(handle_mfcr2_version, "Show OpenR2 library version"),
	TRIS_CLI_DEFINE(handle_mfcr2_show_variants, "Show supported MFC/R2 variants"),
	TRIS_CLI_DEFINE(handle_mfcr2_show_channels, "Show MFC/R2 channels"),
	TRIS_CLI_DEFINE(handle_mfcr2_set_debug, "Set MFC/R2 channel logging level"),
	TRIS_CLI_DEFINE(handle_mfcr2_call_files, "Enable/Disable MFC/R2 call files"),
	TRIS_CLI_DEFINE(handle_mfcr2_set_idle, "Reset MFC/R2 channel forcing it to IDLE"),
	TRIS_CLI_DEFINE(handle_mfcr2_set_blocked, "Reset MFC/R2 channel forcing it to BLOCKED"),
};

#endif /* HAVE_OPENR2 */

static char *dahdi_destroy_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int channel;
	int ret;
	switch (cmd) {
...
retry:
	...
	for (p = iflist; p; p = p->next) {
		...
		if (p->owner && !p->restartpending) {
			if (tris_channel_trylock(p->owner)) {
				if (option_debug > 2)
					tris_verbose("Avoiding deadlock\n");
			...
			if (option_debug > 2)
				tris_verbose("Softhanging up on %s\n", p->owner->name);
			tris_softhangup_nolock(p->owner, TRIS_SOFTHANGUP_EXPLICIT);
			p->restartpending = 1;
			num_restart_pending++;
			tris_channel_unlock(p->owner);
		}
...
static int dahdi_restart(void)
{
	...
	tris_mutex_lock(&restart_lock);
	restarting = 1;
	tris_verb(1, "Destroying channels and reloading DAHDI configuration.\n");
	dahdi_softhangup_all();
	tris_verb(4, "Initial softhangup of all DAHDI channels complete.\n");
#ifdef HAVE_OPENR2
...
#if defined(HAVE_PRI)
	for (i = 0; i < NUM_SPANS; i++) {
		if (pris[i].master && (pris[i].master != TRIS_PTHREADT_NULL)) {
			cancel_code = pthread_cancel(pris[i].master);
			pthread_kill(pris[i].master, SIGURG);
			tris_debug(4, "Waiting to join thread of span %d with pid=%p, cancel_code=%d\n", i, (void *) pris[i].master, cancel_code);
			pthread_join(pris[i].master, NULL);
			tris_debug(4, "Joined thread of span %d\n", i);
		}
	}
#endif
...
#if defined(HAVE_SS7)
	for (i = 0; i < NUM_SPANS; i++) {
		if (linksets[i].master && (linksets[i].master != TRIS_PTHREADT_NULL)) {
	...
	j = 0;
	for (i = 0; i < NUM_SPANS; i++) {
		if (dpnsslinks[i].master && (dpnsslinks[i].master != TRIS_PTHREADT_NULL)) {
			cancel_code = pthread_cancel(dpnsslinks[i].master);
			pthread_kill(dpnsslinks[i].master, SIGURG);
			tris_debug(4, "Waiting to join thread of span %d with pid=%p, cancel_code=%d\n", i, (void *) dpnsslinks[i].master, cancel_code);
			pthread_join(dpnsslinks[i].master, NULL);
			tris_debug(4, "Joined thread of span %d\n", i);
		}
		dpnss_destory(dpnsslinks[i].dpnss);
		dpnsslinks[i].dpnss = NULL;
	}
#endif
	...
	if (monitor_thread && (monitor_thread != TRIS_PTHREADT_STOP) && (monitor_thread != TRIS_PTHREADT_NULL)) {
		...
		pthread_kill(monitor_thread, SIGURG);
		tris_debug(4, "Waiting to join monitor thread with pid=%p, cancel_code=%d\n", (void *) monitor_thread, cancel_code);
		pthread_join(monitor_thread, NULL);
		tris_debug(4, "Joined monitor thread\n");
	}
	monitor_thread = TRIS_PTHREADT_NULL; /* prepare to restart thread in setup_dahdi once channels are reconfigured */
	tris_mutex_unlock(&monlock);

	tris_mutex_lock(&ss_thread_lock);
	while (ss_thread_count > 0) { /* let ss_threads finish and run dahdi_hangup before dahvi_pvts are destroyed */
		int x = DAHDI_FLASH;
		tris_debug(3, "Waiting on %d ss_thread(s) to finish\n", ss_thread_count);
		...
		for (p = iflist; p; p = p->next) {
			...
			tris_cond_wait(&ss_thread_complete, &ss_thread_lock);
		}

	/* ensure any created channels before monitor threads were stopped are hungup */
	dahdi_softhangup_all();
	tris_verb(4, "Final softhangup of all DAHDI channels complete.\n");
	destroy_all_channels();
	tris_debug(1, "Channels destroyed. Now re-reading config. %d active channels remaining.\n", tris_active_channels());
	...
	for (i = 0; i < NUM_SPANS; i++) {
		tris_mutex_init(&pris[i].lock);
		pris[i].offset = -1;
		pris[i].master = TRIS_PTHREADT_NULL;
		for (j = 0; j < NUM_DCHANS; j++)
			pris[i].fds[j] = -1;
		}
	...
	for (i = 0; i < NUM_SPANS; i++) {
		tris_mutex_init(&linksets[i].lock);
		linksets[i].master = TRIS_PTHREADT_NULL;
		for (j = 0; j < NUM_DCHANS; j++)
			linksets[i].fds[j] = -1;
	}
...
#ifdef HAVE_DPNSS
	for (i = 0; i < NUM_SPANS; i++) {
		dahdi_close_dpnss_fd(&(dpnsslinks[i]));
	}

	memset(dpnsslinks, 0, sizeof(dpnsslinks));
	for (i = 0; i < NUM_SPANS; i++) {
		tris_mutex_init(&dpnsslinks[i].lock);
		dpnsslinks[i].master = TRIS_PTHREADT_NULL;
		dpnsslinks[i].dfd = -1;
	}
	dpnss_set_message(dahdi_dpnss_message);
#endif

	if (setup_dahdi(2) != 0) {
		tris_log(LOG_WARNING, "Reload channels from dahdi config failed!\n");
		tris_mutex_unlock(&ss_thread_lock);
		return 1;
	}
	tris_mutex_unlock(&ss_thread_lock);
	restarting = 0;
	tris_mutex_unlock(&restart_lock);
	return 0;
}

static char *dahdi_restart_cmd(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *dahdi_show_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT2 "%7s %-10.10s %-15.15s %-10.10s %-20.20s %-10.10s %-10.10s\n"
	...
	if (a->argc == 5) {
		...
		if (!strcasecmp(a->argv[3], "trunkgroup")) {
			...
			if (pri) {
				...
				tris_cli(a->fd, "No such trunk group %d\n", trunkgroup);
				return CLI_FAILURE;
			}
		} else
#endif
	...
	tris_mutex_lock(lock);
#ifdef HAVE_PRI
	tris_cli(a->fd, FORMAT2, pri ? "CRV" : "Chan", "Extension", "Context", "Language", "MOH Interpret", "Blocked", "State");
#else
	tris_cli(a->fd, FORMAT2, "Chan", "Extension", "Context", "Language", "MOH Interpret", "Blocked", "State");
#endif
	...
	while (tmp) {
		...
		if (tmp->channel > 0) {
			...
			tris_copy_string(tmps, "pseudo", sizeof(tmps));
		...
		tris_cli(a->fd, FORMAT, tmps, tmp->exten, tmp->context, tmp->language, tmp->mohinterpret, blockstr, statestr);
		tmp = tmp->next;
	}
	tris_mutex_unlock(lock);
	return CLI_SUCCESS;
...
static char *dahdi_show_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int channel;
	struct dahdi_pvt *tmp = NULL;
	struct dahdi_confinfo ci;
	...
	while (tmp) {
		if (tmp->channel == channel) {
#ifdef HAVE_PRI
			if (pri)
				tris_cli(a->fd, "Trunk/CRV: %d/%d\n", trunkgroup, tmp->channel);
			else
#endif
			tris_cli(a->fd, "Channel: %d\n", tmp->channel);
			tris_cli(a->fd, "File Descriptor: %d\n", tmp->subs[SUB_REAL].dfd);
			tris_cli(a->fd, "Span: %d\n", tmp->span);
			tris_cli(a->fd, "Extension: %s\n", tmp->exten);
			tris_cli(a->fd, "Dialing: %s\n", tmp->dialing ? "yes" : "no");
			tris_cli(a->fd, "Context: %s\n", tmp->context);
			tris_cli(a->fd, "Caller ID: %s\n", tmp->cid_num);
			tris_cli(a->fd, "Calling TON: %d\n", tmp->cid_ton);
			tris_cli(a->fd, "Caller ID name: %s\n", tmp->cid_name);
			tris_cli(a->fd, "Mailbox: %s\n", S_OR(tmp->mailbox, "none"));
			if (tmp->vars) {
				struct tris_variable *v;
			...
			tris_cli(a->fd, "Destroy: %d\n", tmp->destroy);
			tris_cli(a->fd, "InAlarm: %d\n", tmp->inalarm);
			tris_cli(a->fd, "Signalling Type: %s\n", sig2str(tmp->sig));
			tris_cli(a->fd, "Radio: %d\n", tmp->radio);
			tris_cli(a->fd, "Owner: %s\n", tmp->owner ? tmp->owner->name : "<None>");
			tris_cli(a->fd, "Real: %s%s%s\n", tmp->subs[SUB_REAL].owner ? tmp->subs[SUB_REAL].owner->name : "<None>", tmp->subs[SUB_REAL].inthreeway ? " (Confed)" : "", tmp->subs[SUB_REAL].linear ? " (Linear)" : "");
			tris_cli(a->fd, "Callwait: %s%s%s\n", tmp->subs[SUB_CALLWAIT].owner ? tmp->subs[SUB_CALLWAIT].owner->name : "<None>", tmp->subs[SUB_CALLWAIT].inthreeway ? " (Confed)" : "", tmp->subs[SUB_CALLWAIT].linear ? " (Linear)" : "");
			tris_cli(a->fd, "Threeway: %s%s%s\n", tmp->subs[SUB_THREEWAY].owner ? tmp->subs[SUB_THREEWAY].owner->name : "<None>", tmp->subs[SUB_THREEWAY].inthreeway ? " (Confed)" : "", tmp->subs[SUB_THREEWAY].linear ? " (Linear)" : "");
			tris_cli(a->fd, "Confno: %d\n", tmp->confno);
			tris_cli(a->fd, "Propagated Conference: %d\n", tmp->propconfno);
			tris_cli(a->fd, "Real in conference: %d\n", tmp->inconference);
			tris_cli(a->fd, "DSP: %s\n", tmp->dsp ? "yes" : "no");
			tris_cli(a->fd, "Busy Detection: %s\n", tmp->busydetect ? "yes" : "no");
			if (tmp->busydetect) {
#if defined(BUSYDETECT_TONEONLY)
				tris_cli(a->fd, "    Busy Detector Helper: BUSYDETECT_TONEONLY\n");
#elif defined(BUSYDETECT_COMPARE_TONE_AND_SILENCE)
				tris_cli(a->fd, "    Busy Detector Helper: BUSYDETECT_COMPARE_TONE_AND_SILENCE\n");
#endif
#ifdef BUSYDETECT_DEBUG
				tris_cli(a->fd, "    Busy Detector Debug: Enabled\n");
#endif
				tris_cli(a->fd, "    Busy Count: %d\n", tmp->busycount);
				tris_cli(a->fd, "    Busy Pattern: %d,%d|%d,%d|%d,%d|%d,%d|%d,%d\n",
						tmp->busy_tonelength[0], tmp->busy_quietlength[0],
						tmp->busy_tonelength[1], tmp->busy_quietlength[1],
						tmp->busy_tonelength[2], tmp->busy_quietlength[2],
						tmp->busy_tonelength[3], tmp->busy_quietlength[3],
						tmp->busy_tonelength[4], tmp->busy_quietlength[4]);
			}

			tris_cli(a->fd, "TDD: %s\n", tmp->tdd ? "yes" : "no");
			tris_cli(a->fd, "Relax DTMF: %s\n", tmp->dtmfrelax ? "yes" : "no");
			tris_cli(a->fd, "Dialing/CallwaitCAS: %d/%d\n", tmp->dialing, tmp->callwaitcas);
			tris_cli(a->fd, "Default law: %s\n", tmp->law == DAHDI_LAW_MULAW ? "ulaw" : tmp->law == DAHDI_LAW_ALAW ? "alaw" : "unknown");
			tris_cli(a->fd, "Fax Handled: %s\n", tmp->faxhandled ? "yes" : "no");
			tris_cli(a->fd, "Pulse phone: %s\n", tmp->pulsedial ? "yes" : "no");
			tris_cli(a->fd, "DND: %s\n", tmp->dnd ? "yes" : "no");
			tris_cli(a->fd, "Echo Cancellation:\n");

			if (tmp->echocancel.head.tap_length) {
				tris_cli(a->fd, "\t%d taps\n", tmp->echocancel.head.tap_length);
				for (x = 0; x < tmp->echocancel.head.param_count; x++) {
					tris_cli(a->fd, "\t\t%s: %ud\n", tmp->echocancel.params[x].name, tmp->echocancel.params[x].value);
				}
				tris_cli(a->fd, "\t%scurrently %s\n", tmp->echocanbridged ? "" : "(unless TDM bridged) ", tmp->echocanon ? "ON" : "OFF");
			} else {
				tris_cli(a->fd, "\tnone\n");
			}
			tris_cli(a->fd, "Wait for dialtone: %dms\n", tmp->waitfordialtone);
			if (tmp->master)
				tris_cli(a->fd, "Master Channel: %d\n", tmp->master->channel);
			for (x = 0; x < MAX_SLAVES; x++) {
				if (tmp->slaves[x])
					tris_cli(a->fd, "Slave Channel: %d\n", tmp->slaves[x]->channel);
			}
#ifdef HAVE_OPENR2
			if (tmp->mfcr2) {
				...
				tris_cli(a->fd, "MFC/R2 MF State: %s\n", openr2_chan_get_mf_state_string(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 MF Group: %s\n", openr2_chan_get_mf_group_string(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 State: %s\n", openr2_chan_get_r2_state_string(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 Call State: %s\n", openr2_chan_get_call_state_string(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 Call Files Enabled: %s\n", openr2_chan_get_call_files_enabled(tmp->r2chan) ? "Yes" : "No");
				tris_cli(a->fd, "MFC/R2 Variant: %s\n", openr2_proto_get_variant_string(r2variant));
				tris_cli(a->fd, "MFC/R2 Max ANI: %d\n", openr2_context_get_max_ani(r2context));
				tris_cli(a->fd, "MFC/R2 Max DNIS: %d\n", openr2_context_get_max_dnis(r2context));
				tris_cli(a->fd, "MFC/R2 Get ANI First: %s\n", openr2_context_get_ani_first(r2context) ? "Yes" : "No");
#if defined(OR2_LIB_INTERFACE) && OR2_LIB_INTERFACE > 1
				tris_cli(a->fd, "MFC/R2 Skip Category Request: %s\n", openr2_context_get_skip_category_request(r2context) ? "Yes" : "No");
#endif
				tris_cli(a->fd, "MFC/R2 Immediate Accept: %s\n", openr2_context_get_immediate_accept(r2context) ? "Yes" : "No");
				tris_cli(a->fd, "MFC/R2 Accept on Offer: %s\n", tmp->mfcr2_accept_on_offer ? "Yes" : "No");
				tris_cli(a->fd, "MFC/R2 Charge Calls: %s\n", tmp->mfcr2_charge_calls ? "Yes" : "No");
				tris_cli(a->fd, "MFC/R2 Allow Collect Calls: %s\n", tmp->mfcr2_allow_collect_calls ? "Yes" : "No");
				tris_cli(a->fd, "MFC/R2 Forced Release: %s\n", tmp->mfcr2_forced_release ? "Yes" : "No");
				tris_cli(a->fd, "MFC/R2 MF Back Timeout: %dms\n", openr2_context_get_mf_back_timeout(r2context));
				tris_cli(a->fd, "MFC/R2 R2 Metering Pulse Timeout: %dms\n", openr2_context_get_metering_pulse_timeout(r2context));
				tris_cli(a->fd, "MFC/R2 Rx CAS: %s\n", openr2_chan_get_rx_cas_string(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 Tx CAS: %s\n", openr2_chan_get_tx_cas_string(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 MF Tx Signal: %d\n", openr2_chan_get_tx_mf_signal(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 MF Rx Signal: %d\n", openr2_chan_get_rx_mf_signal(tmp->r2chan));
				tris_cli(a->fd, "MFC/R2 Call Files Directory: %s\n", openr2_context_get_log_directory(r2context, calldir, sizeof(calldir)));
			}
#endif
#ifdef HAVE_SS7
			if (tmp->ss7) {
...
#ifdef HAVE_PRI
			if (tmp->pri) {
				tris_cli(a->fd, "PRI Flags: ");
				if (tmp->resetting)
					tris_cli(a->fd, "Resetting ");
				if (tmp->call)
					tris_cli(a->fd, "Call ");
				if (tmp->bearer)
					tris_cli(a->fd, "Bearer ");
				tris_cli(a->fd, "\n");
				if (tmp->logicalspan)
					tris_cli(a->fd, "PRI Logical Span: %d\n", tmp->logicalspan);
				else
					tris_cli(a->fd, "PRI Logical Span: Implicit\n");
			}
#endif
			memset(&ci, 0, sizeof(ci));
			...
			if (tmp->subs[SUB_REAL].dfd > -1) {
				...
				if (!ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_GETCONF, &ci)) {
					tris_cli(a->fd, "Actual Confinfo: Num/%d, Mode/0x%04x\n", ci.confno, ci.confmode);
				}
				if (!ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_GETCONFMUTE, &x)) {
					tris_cli(a->fd, "Actual Confmute: %s\n", x ? "Yes" : "No");
				}
				memset(&ps, 0, sizeof(ps));
				if (ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_GET_PARAMS, &ps) < 0) {
					tris_log(LOG_WARNING, "Failed to get parameters on channel %d: %s\n", tmp->channel, strerror(errno));
				} else {
					tris_cli(a->fd, "Hookstate (FXS only): %s\n", ps.rxisoffhook ? "Offhook" : "Onhook");
				}
			}
			tris_mutex_unlock(lock);
			return CLI_SUCCESS;
	...
	tris_cli(a->fd, "Unable to find given channel %d\n", channel);
	tris_mutex_unlock(lock);
	return CLI_FAILURE;
}

static char *handle_dahdi_show_cadences(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int i, j;
	switch (cmd) {
	case CLI_INIT:
...
/* Based on irqmiss.c */
static char *dahdi_show_status(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (ctl < 0) {
		tris_cli(a->fd, "No DAHDI found. Unable to open /dev/dahdi/ctl: %s\n", strerror(errno));
		return CLI_FAILURE;
	}
	tris_cli(a->fd, FORMAT2, "Description", "Alarms", "IRQ", "bpviol", "CRC4", "Framing", "Coding", "Options", "LBO");
	...
	for (span = 1; span < DAHDI_MAX_SPANS; ++span) {
		...
		tris_cli(a->fd, FORMAT, s.desc, alarmstr, s.irqmisses, s.bpvcount, s.crc4count,
...
static char *dahdi_show_version(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int pseudo_fd = -1;
	...
	if ((pseudo_fd = open("/dev/dahdi/ctl", O_RDONLY)) < 0) {
		tris_cli(a->fd, "Failed to open control file to get version.\n");
		return CLI_SUCCESS;
	}
	...
	if (ioctl(pseudo_fd, DAHDI_GETVERSION, &vi))
		tris_cli(a->fd, "Failed to get DAHDI version: %s\n", strerror(errno));
	else
		tris_cli(a->fd, "DAHDI Version: %s Echo Canceller: %s\n", vi.version, vi.echo_canceller);
...
static char *dahdi_set_hwgain(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int channel;
	int gain;
	int tx;
	...
	for (tmp = iflist; tmp; tmp = tmp->next) {
		...
		if (ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_SET_HWGAIN, &hwgain) < 0) {
			tris_cli(a->fd, "Unable to set the hardware gain for channel %d: %s\n", channel, strerror(errno));
			tris_mutex_unlock(&iflock);
			return CLI_FAILURE;
		}
		tris_cli(a->fd, "hardware %s gain set to %d (%.1f dB) on channel %d\n",
...
static char *dahdi_set_swgain(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int channel;
	float gain;
	int tx;
	...
	for (tmp = iflist; tmp; tmp = tmp->next) {
		...
		if (res) {
			tris_cli(a->fd, "Unable to set the software gain for channel %d\n", channel);
			tris_mutex_unlock(lock);
			return CLI_FAILURE;
		}

		tris_cli(a->fd, "software %s gain set to %.1f on channel %d\n",
...
static char *dahdi_set_dnd(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int channel;
	int on;
	...
	if ((channel = atoi(a->argv[3])) <= 0) {
		tris_cli(a->fd, "Expected channel number, got '%s'\n", a->argv[3]);
		return CLI_SHOWUSAGE;
	}

	if (tris_true(a->argv[4]))
		on = 1;
	else if (tris_false(a->argv[4]))
		on = 0;
	else {
		tris_cli(a->fd, "Expected 'on' or 'off', got '%s'\n", a->argv[4]);
		return CLI_SHOWUSAGE;
	}
...
static struct tris_cli_entry dahdi_cli[] = {
	TRIS_CLI_DEFINE(handle_dahdi_show_cadences, "List cadences"),
	TRIS_CLI_DEFINE(dahdi_show_channels, "Show active DAHDI channels"),
	TRIS_CLI_DEFINE(dahdi_show_channel, "Show information on a channel"),
	TRIS_CLI_DEFINE(dahdi_destroy_channel, "Destroy a channel"),
	TRIS_CLI_DEFINE(dahdi_restart_cmd, "Fully restart DAHDI channels"),
	TRIS_CLI_DEFINE(dahdi_show_status, "Show all DAHDI cards status"),
	TRIS_CLI_DEFINE(dahdi_show_version, "Show the DAHDI version in use"),
	TRIS_CLI_DEFINE(dahdi_set_hwgain, "Set hardware gain on a channel"),
	TRIS_CLI_DEFINE(dahdi_set_swgain, "Set software gain on a channel"),
	TRIS_CLI_DEFINE(dahdi_set_dnd, "Sets/resets DND (Do Not Disturb) mode on a channel"),
};
	...
	if (p) {
		switch (mode) {
			...
			case HANGUP:
				...
				break;
			default:
				tris_log(LOG_WARNING, "I don't know how to handle transfer event with this: %d on channel %s\n",mode, p->owner->name);
		}
	}
	return 0;
}
...
static int action_dahdidialoffhook(struct mansession *s, const struct message *m)
{
	...
	for (i = 0; i < strlen(number); i++) {
		struct tris_frame f = { TRIS_FRAME_DTMF, number[i] };
...
static int action_dahdishowchannels(struct mansession *s, const struct message *m)
{
	...
	if (!tris_strlen_zero(dahdichannel)) {
...
static int linkset_addsigchan(int sigchan)
{
	...
	if (!link) {
		tris_log(LOG_ERROR, "Invalid linkset number.  Must be between 1 and %d\n", NUM_SPANS + 1);
		return -1;
	}

	if (cur_ss7type < 0) {
		tris_log(LOG_ERROR, "Unspecified or invalid ss7type\n");
		return -1;
	}
	...
	if (!link->ss7) {
		tris_log(LOG_ERROR, "Can't create new SS7!\n");
		return -1;
	}
	...
	if (cur_pointcode < 0) {
		tris_log(LOG_ERROR, "Unspecified pointcode!\n");
		return -1;
	} else
		ss7_set_pc(link->ss7, cur_pointcode);

	if (sigchan < 0) {
		tris_log(LOG_ERROR, "Invalid sigchan!\n");
		return -1;
	} else {
		if (link->numsigchans >= NUM_DCHANS) {
			tris_log(LOG_ERROR, "Too many sigchans on linkset %d\n", cur_linkset);
			return -1;
		}
		...
		if ((link->fds[curfd] < 0) || (ioctl(link->fds[curfd],DAHDI_SPECIFY,&sigchan) == -1)) {
			tris_log(LOG_ERROR, "Unable to open SS7 sigchan %d (%s)\n", sigchan, strerror(errno));
			return -1;
		}
		memset(&p, 0, sizeof(p));
		...
		if (res) {
			dahdi_close_ss7_fd(link, curfd);
			tris_log(LOG_ERROR, "Unable to get span state for sigchan %d (%s)\n", sigchan, strerror(errno));
		}
	...
	if (cur_adjpointcode < 0) {
		tris_log(LOG_ERROR, "Unspecified adjpointcode!\n");
		return -1;
	} else {
	...
	if (cur_defaultdpc < 0) {
		tris_log(LOG_ERROR, "Unspecified defaultdpc!\n");
		return -1;
	}

	if (cur_networkindicator < 0) {
		tris_log(LOG_ERROR, "Invalid networkindicator!\n");
		return -1;
	} else
...
#if defined(HAVE_SS7)
static char *handle_ss7_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int span;
	switch (cmd) {
	case CLI_INIT:
	...
	span = atoi(a->argv[5]);
	if ((span < 1) || (span > NUM_SPANS)) {
		tris_cli(a->fd, "Invalid linkset %s.  Should be a number from %d to %d\n", a->argv[5], 1, NUM_SPANS);
		return CLI_SUCCESS;
	}
	if (!linksets[span-1].ss7) {
		tris_cli(a->fd, "No SS7 running on linkset %d\n", span);
		return CLI_SUCCESS;
	}
	if (linksets[span-1].ss7) {
		if (!strcasecmp(a->argv[3], "on")) {
			ss7_set_debug(linksets[span-1].ss7, SS7_DEBUG_MTP2 | SS7_DEBUG_MTP3 | SS7_DEBUG_ISUP);
			tris_cli(a->fd, "Enabled debugging on linkset %d\n", span);
		} else {
			ss7_set_debug(linksets[span-1].ss7, 0);
			tris_cli(a->fd, "Disabled debugging on linkset %d\n", span);
		}
	}
...
#if defined(HAVE_SS7)
static char *handle_ss7_block_cic(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int linkset, cic;
	int blocked = -1, i;
	switch (cmd) {
	...
	if ((linkset < 1) || (linkset > NUM_SPANS)) {
		tris_cli(a->fd, "Invalid linkset %s.  Should be a number %d to %d\n", a->argv[3], 1, NUM_SPANS);
		return CLI_SUCCESS;
	}
	...
	if (cic < 1) {
		tris_cli(a->fd, "Invalid CIC specified!\n");
		return CLI_SUCCESS;
	}
	...
	for (i = 0; i < linksets[linkset-1].numchans; i++) {
		if (linksets[linkset-1].pvts[i]->cic == cic) {
			...
			if (!blocked) {
				tris_mutex_lock(&linksets[linkset-1].lock);
				isup_blo(linksets[linkset-1].ss7, cic, linksets[linkset-1].pvts[i]->dpc);
				tris_mutex_unlock(&linksets[linkset-1].lock);
			}
		}
	}
	...
	if (!blocked)
		tris_cli(a->fd, "Sent blocking request for linkset %d on CIC %d\n", linkset, cic);
	else
		tris_cli(a->fd, "CIC %d already locally blocked\n", cic);
...
#if defined(HAVE_SS7)
static char *handle_ss7_block_linkset(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int linkset;
	int i;
	switch (cmd) {
	...
	for (i = 0; i < linksets[linkset-1].numchans; i++) {
		tris_cli(a->fd, "Sending remote blocking request on CIC %d\n", linksets[linkset-1].pvts[i]->cic);
		tris_mutex_lock(&linksets[linkset-1].lock);
...
#if defined(HAVE_SS7)
static char *handle_ss7_unblock_cic(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int linkset, cic;
	int i, blocked = -1;
	switch (cmd) {
	...
	if (blocked > 0)
		tris_cli(a->fd, "Sent unblocking request for linkset %d on CIC %d\n", linkset, cic);
...
#if defined(HAVE_SS7)
static char *handle_ss7_unblock_linkset(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int linkset;
	int i;
	switch (cmd) {
	...
	for (i = 0; i < linksets[linkset-1].numchans; i++) {
		tris_cli(a->fd, "Sending remote unblock request on CIC %d\n", linksets[linkset-1].pvts[i]->cic);
		tris_mutex_lock(&linksets[linkset-1].lock);
...
#if defined(HAVE_SS7)
static char *handle_ss7_show_linkset(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int linkset;
	struct dahdi_ss7 *ss7;
	switch (cmd) {
	...
	tris_cli(a->fd, "SS7 linkset %d status: %s\n", linkset, (ss7->state == LINKSET_STATE_UP) ? "Up" : "Down");
...
#if defined(HAVE_SS7)
static char *handle_ss7_version(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	tris_cli(a->fd, "libss7 version: %s\n", ss7_get_version());
...
#if defined(HAVE_SS7)
static struct tris_cli_entry dahdi_ss7_cli[] = {
	TRIS_CLI_DEFINE(handle_ss7_debug, "Enables SS7 debugging on a linkset"),
	TRIS_CLI_DEFINE(handle_ss7_block_cic, "Blocks the given CIC"),
	TRIS_CLI_DEFINE(handle_ss7_unblock_cic, "Unblocks the given CIC"),
	TRIS_CLI_DEFINE(handle_ss7_block_linkset, "Blocks all CICs on a linkset"),
	TRIS_CLI_DEFINE(handle_ss7_unblock_linkset, "Unblocks all CICs on a linkset"),
	TRIS_CLI_DEFINE(handle_ss7_show_linkset, "Shows the status of a linkset"),
	TRIS_CLI_DEFINE(handle_ss7_version, "Displays libss7 version"),
};
#endif	/* defined(HAVE_SS7) */
...
static char *handle_dpnss_show_span(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
#define DPNSSFORMAT "%7s %-10.10s\n"
	int span;
	char buf[8];
	int i;
	struct dahdi_dpnss *dpnss;
	switch (cmd) {
	case CLI_INIT:
		e->command = "dpnss show span";
		e->usage =
			"Usage: dpnss show span <span>\n"
			"       Shows the status of an DPNSS DLC.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	if (!dpnsslinks[span-1].dpnss) {
		tris_cli(a->fd, "No DPNSS running on span %d\n", span);
		return CLI_SUCCESS;
	}
	if (dpnsslinks[span-1].dpnss)
		dpnss = &dpnsslinks[span-1];

	tris_cli(a->fd, DPNSSFORMAT,"DLCNo","LinkStatus");

	for(i = 1; i < 32; i++){
		if (i == 16) continue;
		sprintf(buf,"%d",i);
		tris_cli(a->fd,DPNSSFORMAT,buf,dpnss_get_dlc_status(dpnss->dpnss,i));
	}
	return CLI_SUCCESS;
}

static char *handle_dpnss_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int span;
	int channo = 0;
	switch (cmd) {
	case CLI_INIT:
		e->command = "dpnss set debug {on|off} span ";
		e->usage =
			"Usage: dpnss set debug {on|off} span <span> <channo>\n"
			"       Enables debugging on a given DPNSS span\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	span = atoi(a->argv[5]);
	if ((span < 1) || (span > NUM_SPANS)) {
		tris_cli(a->fd, "Invalid Span Number %s.  Should be a number from %d to %d\n", a->argv[5], 1, NUM_SPANS);
		return CLI_SUCCESS;
	}
	if (!dpnsslinks[span-1].dpnss) {
		tris_cli(a->fd, "No DPNSS running on span %d\n", span);
	...
	if (a->argc > 6){
		channo = atoi(a->argv[6]);
		if ((channo < 1) || (channo > 31) || channo == 16) {
			tris_cli(a->fd, "Invalid Channel Number %s.  Should be a number from %d to %d\n", a->argv[6], 1, 31);
			return CLI_SUCCESS;
		}
	...
	if (dpnsslinks[span-1].dpnss) {
		if (!strcasecmp(a->argv[3], "on")) {
			dpnss_set_debug(dpnsslinks[span-1].dpnss, DPNSS_DEBUG_DLC| DPNSS_DEBUG_CHANNEL,channo);
			if (channo)
				tris_cli(a->fd, "Enabled debugging on span %d channel %d\n", span,channo);
			else
				tris_cli(a->fd, "Enabled debugging on span %d\n", span);
		} else {
			dpnss_set_debug(dpnsslinks[span-1].dpnss, 0,channo);
			if (channo)
				tris_cli(a->fd, "Disabled debugging on span %d channel %d\n", span,channo);
			else
				tris_cli(a->fd, "Disabled debugging on span %d\n", span);
		}
	}
...
static char *handle_dpnss_dlcreset(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int span;
	int channo = 0;
	switch (cmd) {
	case CLI_INIT:
		e->command = "dpnss dlcreset span ";
		e->usage =
			"Usage: dpnss dlcreset span <span> <channo>\n"
			"       Enables debugging on a given DPNSS span\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	...
	if (dpnsslinks[span-1].dpnss) {
		dpnss_dlcreset(dpnsslinks[span-1].dpnss,channo);
		if (channo)
			tris_cli(a->fd, "Resetting dlc %d on span %d\n", channo,span);
		else
			tris_cli(a->fd, "Resetting all dlc on span %d\n", span);
	}
...
#endif	/* defined(HAVE_DPNSS */
#if defined(HAVE_DPNSS)
static struct tris_cli_entry dahdi_dpnss_cli[] = {
	TRIS_CLI_DEFINE(handle_dpnss_debug,"Enables DPNSS debugging"),
	TRIS_CLI_DEFINE(handle_dpnss_show_span,"Show DPNSS DLC status"),	
	TRIS_CLI_DEFINE(handle_dpnss_dlcreset,"Reset DPNSS DLC")	
};
#endif
...
#ifdef HAVE_PRI
	for (i = 0; i < NUM_SPANS; i++) {
		if (pris[i].master != TRIS_PTHREADT_NULL)
			pthread_cancel(pris[i].master);
	}
	tris_cli_unregister_multiple(dahdi_pri_cli, ARRAY_LEN(dahdi_pri_cli));
	tris_unregister_application(dahdi_send_keypad_facility_app);
#ifdef HAVE_PRI_PROG_W_CAUSE
	tris_unregister_application(dahdi_send_callrerouting_facility_app);
#endif
#endif
#if defined(HAVE_SS7)
	for (i = 0; i < NUM_SPANS; i++) {
		if (linksets[i].master != TRIS_PTHREADT_NULL)
			pthread_cancel(linksets[i].master);
		}
	tris_cli_unregister_multiple(dahdi_ss7_cli, ARRAY_LEN(dahdi_ss7_cli));
#endif
#if defined(HAVE_DPNSS)
	for (i = 0; i < NUM_SPANS; i++) {
		if (dpnsslinks[i].master != TRIS_PTHREADT_NULL)
			pthread_cancel(dpnsslinks[i].master);
		dpnss_destory(dpnsslinks[i].dpnss);
	}
	tris_cli_unregister_multiple(dahdi_dpnss_cli, ARRAY_LEN(dahdi_dpnss_cli));
#endif
#if defined(HAVE_OPENR2)
	dahdi_r2_destroy_links();
	tris_cli_unregister_multiple(dahdi_mfcr2_cli, ARRAY_LEN(dahdi_mfcr2_cli));
	tris_unregister_application(dahdi_accept_r2_call_app);
#endif

	tris_cli_unregister_multiple(dahdi_cli, ARRAY_LEN(dahdi_cli));
	tris_manager_unregister( "DAHDIDialOffhook" );
	tris_manager_unregister( "DAHDIHangup" );
	tris_manager_unregister( "DAHDITransfer" );
	tris_manager_unregister( "DAHDIDNDoff" );
	tris_manager_unregister( "DAHDIDNDon" );
	tris_manager_unregister("DAHDIShowChannels");
	tris_manager_unregister("DAHDIRestart");
	tris_channel_unregister(&dahdi_tech);
	tris_mutex_lock(&iflock);
...
#endif
	tris_cond_destroy(&ss_thread_complete);
	return 0;
}
...
static int unload_module(void)
{
	...
	for (y = 0; y < NUM_SPANS; y++)
		tris_mutex_destroy(&pris[y].lock);
#endif
#ifdef HAVE_SS7
	for (y = 0; y < NUM_SPANS; y++)
		tris_mutex_destroy(&linksets[y].lock);
#endif /* HAVE_SS7 */

#ifdef HAVE_DPNSS
	for (y = 0; y < NUM_SPANS;y++)
		tris_mutex_destroy(&dpnsslinks[y].lock);
#endif
...
static int build_channels(struct dahdi_chan_conf *conf, int iscrv, const char *value, int reload, int lineno, int *found_pseudo)
{
	...
	if ((reload == 0) && (conf->chan.sig < 0) && !conf->is_sig_auto) {
		tris_log(LOG_ERROR, "Signalling must be specified before any channels are.\n");
		return -1;
	}

	c = tris_strdupa(value);

#ifdef HAVE_PRI
	pri = NULL;
	if (iscrv) {
		if (sscanf(c, "%30d:%n", &trunkgroup, &y) != 1) {
			tris_log(LOG_WARNING, "CRV must begin with trunkgroup followed by a colon at line %d.\n", lineno);
			return -1;
		}
		if (trunkgroup < 1) {
			tris_log(LOG_WARNING, "CRV trunk group must be a positive number at line %d.\n", lineno);
			return -1;
		}
		c += y;
		for (y = 0; y < NUM_SPANS; y++) {
		...
		if (!pri) {
			tris_log(LOG_WARNING, "No such trunk group %d at CRV declaration at line %d.\n", trunkgroup, lineno);
			return -1;
		}
	}
#endif
	...
	while ((chan = strsep(&c, ","))) {
		if (sscanf(chan, "%30d-%30d", &start, &finish) == 2) {
			...
			tris_log(LOG_ERROR, "Syntax error parsing '%s' at '%s'\n", value, chan);
			return -1;
		}
		if (finish < start) {
			tris_log(LOG_WARNING, "Sillyness: %d < %d\n", start, finish);
			x = finish;
			finish = start;
			start = x;
		}
...
#ifdef HAVE_PRI
				if (pri)
					tris_verb(3, "%s CRV %d:%d, %s signalling\n", reload ? "Reconfigured" : "Registered", trunkgroup, x, sig2str(tmp->sig));
				else
#endif
					tris_verb(3, "%s channel %d, %s signalling\n", reload ? "Reconfigured" : "Registered", x, sig2str(tmp->sig));
			} else {
				tris_log(LOG_ERROR, "Unable to %s channel '%s'\n",
...
static void process_echocancel(struct dahdi_chan_conf *confp, const char *data, unsigned int line)
{
	...
	if (!(param_count = tris_app_separate_args(parse, ',', params, ARRAY_LEN(params))))
		return;
	...
	x = tris_strlen_zero(params[0]) ? 0 : atoi(params[0]);

	if ((x == 32) || (x == 64) || (x == 128) || (x == 256) || (x == 512) || (x == 1024))
		confp->chan.echocancel.head.tap_length = x;
	else if ((confp->chan.echocancel.head.tap_length = tris_true(params[0])))
	...
	for (x = 1; x < param_count; x++) {
		...
		if (tris_app_separate_args(params[x], '=', (char **) &param, 2) < 1) {
			tris_log(LOG_WARNING, "Invalid echocancel parameter supplied at line %d: '%s'\n", line, params[x]);
			continue;
		}

		if (tris_strlen_zero(param.name) || (strlen(param.name) > sizeof(confp->chan.echocancel.params[0].name)-1)) {
			tris_log(LOG_WARNING, "Invalid echocancel parameter supplied at line %d: '%s'\n", line, param.name);
			continue;
		}
		...
		if (param.value) {
			if (sscanf(param.value, "%30d", &confp->chan.echocancel.params[confp->chan.echocancel.head.param_count].value) != 1) {
				tris_log(LOG_WARNING, "Invalid echocancel parameter value supplied at line %d: '%s'\n", line, param.value);
				continue;
			}
		}
...
static int process_dahdi(struct dahdi_chan_conf *confp, const char *cat, struct tris_variable *v, int reload, int options)
{
	struct dahdi_pvt *tmp;
	int y;
	int found_pseudo = 0;
	...
	for (; v; v = v->next) {
		...
		if (!strcasecmp(v->name, "parkinglot")) {
			tris_copy_string(confp->chan.parkinglot, v->value, sizeof(confp->chan.parkinglot));
		}
		...
		if (!strcasecmp(v->name, "channel")
			...
			if (res != 2) {
				tris_log(LOG_WARNING, "Parsing buffers option data failed, using defaults.\n");
			...
			if (!strcasecmp(policy, "full")) {
				...
				tris_log(LOG_WARNING, "Invalid policy name given (%s).\n", policy);
			}
			...
			if (res != 2) {
				tris_log(LOG_WARNING, "Parsing faxbuffers option data failed, using defaults.\n");
			...
			if (!strcasecmp(policy, "full")) {
 			...
 			tris_copy_string(dahdichan, v->value, sizeof(dahdichan));
		} else if (!strcasecmp(v->name, "usedistinctiveringdetection")) {
			usedistinctiveringdetection = tris_true(v->value);
		} else if (!strcasecmp(v->name, "distinctiveringaftercid")) {
			distinctiveringaftercid = tris_true(v->value);
		} else if (!strcasecmp(v->name, "dring1context")) {
			tris_copy_string(confp->chan.drings.ringContext[0].contextData,v->value,sizeof(confp->chan.drings.ringContext[0].contextData));
		} else if (!strcasecmp(v->name, "dring2context")) {
			tris_copy_string(confp->chan.drings.ringContext[1].contextData,v->value,sizeof(confp->chan.drings.ringContext[1].contextData));
		} else if (!strcasecmp(v->name, "dring3context")) {
			tris_copy_string(confp->chan.drings.ringContext[2].contextData,v->value,sizeof(confp->chan.drings.ringContext[2].contextData));
			...
			confp->chan.use_callerid = tris_true(v->value);
			...
			confp->chan.threewaycalling = tris_true(v->value);
		} else if (!strcasecmp(v->name, "cancallforward")) {
			confp->chan.cancallforward = tris_true(v->value);
			...
			tris_copy_string(confp->chan.mailbox, v->value, sizeof(confp->chan.mailbox));
		} else if (!strcasecmp(v->name, "hasvoicemail")) {
			if (tris_true(v->value) && tris_strlen_zero(confp->chan.mailbox)) {
				tris_copy_string(confp->chan.mailbox, cat, sizeof(confp->chan.mailbox));
			}
		} else if (!strcasecmp(v->name, "adsi")) {
			confp->chan.adsi = tris_true(v->value);
		} else if (!strcasecmp(v->name, "usesmdi")) {
			confp->chan.use_smdi = tris_true(v->value);
		} else if (!strcasecmp(v->name, "smdiport")) {
			tris_copy_string(confp->smdi_port, v->value, sizeof(confp->smdi_port));
		} else if (!strcasecmp(v->name, "transfer")) {
			confp->chan.transfer = tris_true(v->value);
		} else if (!strcasecmp(v->name, "canpark")) {
			confp->chan.canpark = tris_true(v->value);
		} else if (!strcasecmp(v->name, "echocancelwhenbridged")) {
			confp->chan.echocanbridged = tris_true(v->value);
		} else if (!strcasecmp(v->name, "busydetect")) {
			confp->chan.busydetect = tris_true(v->value);
		} else if (!strcasecmp(v->name, "busycount")) {
			confp->chan.busycount = atoi(v->value);
		} else if (!strcasecmp(v->name, "busypattern")) {
			if (sscanf(v->value, "%30d,%30d|%30d,%30d|%30d,%30d|%30d,%30d|%30d,%30d",
					&confp->chan.busy_tonelength[0], &confp->chan.busy_quietlength[0],
					&confp->chan.busy_tonelength[1], &confp->chan.busy_quietlength[1],
					&confp->chan.busy_tonelength[2], &confp->chan.busy_quietlength[2],
					&confp->chan.busy_tonelength[3], &confp->chan.busy_quietlength[3],
					&confp->chan.busy_tonelength[4], &confp->chan.busy_quietlength[4]) != 10) {
				tris_log(LOG_ERROR, "busypattern= expects busypattern=tonelength,quietlength at line %d.\n", v->lineno);
			}
			...
			} else if (!strcasecmp(v->value, "both") || tris_true(v->value))
			...
			if (sscanf(v->value, "%30d", &y) == 1) {
				if ((y < 10) || (y > 4000)) {
					tris_log(LOG_WARNING, "Echo training time must be within the range of 10 to 4000 ms at line %d.\n", v->lineno);
				} else {
			...
			confp->chan.hidecallerid = tris_true(v->value);
		} else if (!strcasecmp(v->name, "hidecalleridname")) {
			confp->chan.hidecalleridname = tris_true(v->value);
 		} else if (!strcasecmp(v->name, "pulsedial")) {
 			confp->chan.pulse = tris_true(v->value);
		} else if (!strcasecmp(v->name, "callreturn")) {
			confp->chan.callreturn = tris_true(v->value);
		} else if (!strcasecmp(v->name, "callwaiting")) {
			confp->chan.callwaiting = tris_true(v->value);
		} else if (!strcasecmp(v->name, "callwaitingcallerid")) {
			confp->chan.callwaitingcallerid = tris_true(v->value);
		} else if (!strcasecmp(v->name, "context")) {
			tris_copy_string(confp->chan.context, v->value, sizeof(confp->chan.context));
		} else if (!strcasecmp(v->name, "language")) {
			tris_copy_string(confp->chan.language, v->value, sizeof(confp->chan.language));
		} else if (!strcasecmp(v->name, "progzone")) {
			tris_copy_string(progzone, v->value, sizeof(progzone));
		} else if (!strcasecmp(v->name, "mohinterpret")
			||!strcasecmp(v->name, "musiconhold") || !strcasecmp(v->name, "musicclass")) {
			tris_copy_string(confp->chan.mohinterpret, v->value, sizeof(confp->chan.mohinterpret));
		} else if (!strcasecmp(v->name, "mohsuggest")) {
			tris_copy_string(confp->chan.mohsuggest, v->value, sizeof(confp->chan.mohsuggest));
		} else if (!strcasecmp(v->name, "parkinglot")) {
			tris_copy_string(parkinglot, v->value, sizeof(parkinglot));
		} else if (!strcasecmp(v->name, "stripmsd")) {
			tris_log(LOG_NOTICE, "Configuration option \"%s\" has been deprecated. Please use dialplan instead\n", v->name);
			confp->chan.stripmsd = atoi(v->value);
		} else if (!strcasecmp(v->name, "jitterbuffers")) {
			numbufs = atoi(v->value);
		} else if (!strcasecmp(v->name, "maxtxlen")) {
			maxtxlen = atoi(v->value);
		} else if (!strcasecmp(v->name, "group")) {
			confp->chan.group = tris_get_group(v->value);
		} else if (!strcasecmp(v->name, "callgroup")) {
			if (!strcasecmp(v->value, "none"))
				confp->chan.callgroup = 0;
			else
				confp->chan.callgroup = tris_get_group(v->value);
		} else if (!strcasecmp(v->name, "pickupgroup")) {
			if (!strcasecmp(v->value, "none"))
				confp->chan.pickupgroup = 0;
			else
				confp->chan.pickupgroup = tris_get_group(v->value);
		} else if (!strcasecmp(v->name, "setvar")) {
			char *varname = tris_strdupa(v->value), *varval = NULL;
			struct tris_variable *tmpvar;
			...
			confp->chan.immediate = tris_true(v->value);
		} else if (!strcasecmp(v->name, "transfertobusy")) {
			confp->chan.transfertobusy = tris_true(v->value);
			...
			if (sscanf(v->value, "%30f", &confp->chan.cid_rxgain) != 1) {
				tris_log(LOG_WARNING, "Invalid cid_rxgain: %s at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "rxgain")) {
			if (sscanf(v->value, "%30f", &confp->chan.rxgain) != 1) {
				tris_log(LOG_WARNING, "Invalid rxgain: %s at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "txgain")) {
			if (sscanf(v->value, "%30f", &confp->chan.txgain) != 1) {
				tris_log(LOG_WARNING, "Invalid txgain: %s at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "tonezone")) {
			if (sscanf(v->value, "%30d", &confp->chan.tonezone) != 1) {
				tris_log(LOG_WARNING, "Invalid tonezone: %s at line %d.\n", v->value, v->lineno);
			}
			...
			if (!strcasecmp(v->value, "asreceived")) {
				...
				tris_callerid_split(v->value, confp->chan.cid_name, sizeof(confp->chan.cid_name), confp->chan.cid_num, sizeof(confp->chan.cid_num));
			}
		} else if (!strcasecmp(v->name, "fullname")) {
			tris_copy_string(confp->chan.cid_name, v->value, sizeof(confp->chan.cid_name));
		} else if (!strcasecmp(v->name, "cid_number")) {
			tris_copy_string(confp->chan.cid_num, v->value, sizeof(confp->chan.cid_num));
		} else if (!strcasecmp(v->name, "useincomingcalleridondahditransfer")) {
			confp->chan.dahditrcallerid = tris_true(v->value);
		} else if (!strcasecmp(v->name, "restrictcid")) {
			confp->chan.restrictcid = tris_true(v->value);
		} else if (!strcasecmp(v->name, "usecallingpres")) {
			confp->chan.use_callingpres = tris_true(v->value);
		} else if (!strcasecmp(v->name, "accountcode")) {
			tris_copy_string(confp->chan.accountcode, v->value, sizeof(confp->chan.accountcode));
			...
			confp->chan.answeronpolarityswitch = tris_true(v->value);
		} else if (!strcasecmp(v->name, "hanguponpolarityswitch")) {
			confp->chan.hanguponpolarityswitch = tris_true(v->value);
		} else if (!strcasecmp(v->name, "sendcalleridafter")) {
			confp->chan.sendcalleridafter = atoi(v->value);
		} else if (!strcasecmp(v->name, "mwimonitornotify")) {
			tris_copy_string(mwimonitornotify, v->value, sizeof(mwimonitornotify));
			...
			if ( (strcasestr(v->value, "neon")) || (strcasestr(v->value, "hvac")) ){ 	/* 90V DC pulses */
			 ...
			 if (!strcasecmp(v->name, "signalling") || !strcasecmp(v->name, "signaling")) {
				...
				if (!strcasecmp(v->value, "em")) {
					...
					tris_log(LOG_WARNING, "How cool would it be if someone implemented this mode!  For now, sucks for you. (line %d)\n", v->lineno);
...
#endif
#ifdef HAVE_DPNSS
				} else if (!strcasecmp(v->value, "dpnss")) {
					confp->chan.sig = SIG_DPNSS;
#endif
#ifdef HAVE_OPENR2
					...
					tris_log(LOG_ERROR, "Unknown signalling method '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					tris_log(LOG_WARNING, "Unknown PRI dialplan '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					tris_log(LOG_WARNING, "Unknown PRI localdialplan '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national"))
					...
					tris_log(LOG_ERROR, "Unknown switchtype '%s' at line %d.\n", v->value, v->lineno);
					return -1;
				}
				...
				if (!strcasecmp(v->value, "sdn"))
					...
					tris_log(LOG_WARNING, "Unknown network-specific facility '%s' at line %d.\n", v->value, v->lineno);
					confp->pri.nsf = PRI_NSF_NONE;
				}
				...
				if (!strcasecmp(v->value, "outofband"))
					...
					tris_log(LOG_WARNING, "'%s' is not a valid pri indication value, should be 'inband' or 'outofband' at line %d.\n",
						v->value, v->lineno);
			} else if (!strcasecmp(v->name, "priexclusive")) {
				confp->chan.priexclusive = tris_true(v->value);
			} else if (!strcasecmp(v->name, "internationalprefix")) {
				tris_copy_string(confp->pri.internationalprefix, v->value, sizeof(confp->pri.internationalprefix));
			} else if (!strcasecmp(v->name, "nationalprefix")) {
				tris_copy_string(confp->pri.nationalprefix, v->value, sizeof(confp->pri.nationalprefix));
			} else if (!strcasecmp(v->name, "localprefix")) {
				tris_copy_string(confp->pri.localprefix, v->value, sizeof(confp->pri.localprefix));
			} else if (!strcasecmp(v->name, "privateprefix")) {
				tris_copy_string(confp->pri.privateprefix, v->value, sizeof(confp->pri.privateprefix));
			} else if (!strcasecmp(v->name, "unknownprefix")) {
				tris_copy_string(confp->pri.unknownprefix, v->value, sizeof(confp->pri.unknownprefix));
				...
				if (!strcasecmp(v->value, "never"))
					...
					tris_log(LOG_WARNING, "'%s' is not a valid reset interval, should be >= 60 seconds or 'never' at line %d.\n",
						v->value, v->lineno);
				...
				tris_copy_string(confp->pri.idleext, v->value, sizeof(confp->pri.idleext));
			} else if (!strcasecmp(v->name, "idledial")) {
				tris_copy_string(confp->pri.idledial, v->value, sizeof(confp->pri.idledial));
				...
				confp->pri.discardremoteholdretrieval = tris_true(v->value);
#ifdef HAVE_PRI_INBANDDISCONNECT
			} else if (!strcasecmp(v->name, "inbanddisconnect")) {
				confp->pri.inbanddisconnect = tris_true(v->value);
#endif
				...
				tris_copy_string(tmp, v->value, sizeof(tmp));
				c = tmp;
				timerc = strsep(&c, ",");
				if (!tris_strlen_zero(timerc) && !tris_strlen_zero(c)) {
				...
				confp->pri.facilityenable = tris_true(v->value);
				...
				if (!strcasecmp(v->value, "itu")) {
					...
					tris_log(LOG_WARNING, "'%s' is an unknown ss7 switch type at line %d.!\n", v->value, v->lineno);
				...
				tris_copy_string(confp->ss7.internationalprefix, v->value, sizeof(confp->ss7.internationalprefix));
			} else if (!strcasecmp(v->name, "ss7_nationalprefix")) {
				tris_copy_string(confp->ss7.nationalprefix, v->value, sizeof(confp->ss7.nationalprefix));
			} else if (!strcasecmp(v->name, "ss7_subscriberprefix")) {
				tris_copy_string(confp->ss7.subscriberprefix, v->value, sizeof(confp->ss7.subscriberprefix));
			} else if (!strcasecmp(v->name, "ss7_unknownprefix")) {
				tris_copy_string(confp->ss7.unknownprefix, v->value, sizeof(confp->ss7.unknownprefix));
				...
				if (!strcasecmp(v->value, "national")) {
					...
					tris_log(LOG_WARNING, "Unknown SS7 called_nai '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					tris_log(LOG_WARNING, "Unknown SS7 calling_nai '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				tris_copy_string(confp->mfcr2.r2proto_file, v->value, sizeof(confp->mfcr2.r2proto_file));
				tris_log(LOG_WARNING, "MFC/R2 Protocol file '%s' will be used, you only should use this if you *REALLY KNOW WHAT YOU ARE DOING*.\n", confp->mfcr2.r2proto_file);
			} else if (!strcasecmp(v->name, "mfcr2_logdir")) {
				tris_copy_string(confp->mfcr2.logdir, v->value, sizeof(confp->mfcr2.logdir));
				...
				if (OR2_VAR_UNKNOWN == confp->mfcr2.variant) {
					tris_log(LOG_WARNING, "Unknown MFC/R2 variant '%s' at line %d, defaulting to ITU.\n", v->value, v->lineno);
				...
				if (!confp->mfcr2.mfback_timeout) {
					tris_log(LOG_WARNING, "MF timeout of 0? hum, I will protect you from your ignorance. Setting default.\n");
					confp->mfcr2.mfback_timeout = -1;
				} else if (confp->mfcr2.mfback_timeout > 0 && confp->mfcr2.mfback_timeout < 500) {
					tris_log(LOG_WARNING, "MF timeout less than 500ms is not recommended, you have been warned!\n");
				}
				...
				if (confp->mfcr2.metering_pulse_timeout > 500) {
					tris_log(LOG_WARNING, "Metering pulse timeout greater than 500ms is not recommended, you have been warned!\n");
				}
			} else if (!strcasecmp(v->name, "mfcr2_get_ani_first")) {
				confp->mfcr2.get_ani_first = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_double_answer")) {
				confp->mfcr2.double_answer = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_charge_calls")) {
				confp->mfcr2.charge_calls = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_accept_on_offer")) {
				confp->mfcr2.accept_on_offer = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_allow_collect_calls")) {
				confp->mfcr2.allow_collect_calls = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_forced_release")) {
				confp->mfcr2.forced_release = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_immediate_accept")) {
				confp->mfcr2.immediate_accept = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_call_files")) {
				confp->mfcr2.call_files = tris_true(v->value) ? 1 : 0;
			} else if (!strcasecmp(v->name, "mfcr2_max_ani")) {
				confp->mfcr2.max_ani = atoi(v->value);
				if (confp->mfcr2.max_ani >= TRIS_MAX_EXTENSION){
					confp->mfcr2.max_ani = TRIS_MAX_EXTENSION - 1;
				}
			} else if (!strcasecmp(v->name, "mfcr2_max_dnis")) {
				confp->mfcr2.max_dnis = atoi(v->value);
				if (confp->mfcr2.max_dnis >= TRIS_MAX_EXTENSION){
					confp->mfcr2.max_dnis = TRIS_MAX_EXTENSION - 1;
				}
				...
				if (OR2_CALLING_PARTY_CATEGORY_UNKNOWN == confp->mfcr2.category) {
					...
					tris_log(LOG_WARNING, "Invalid MFC/R2 caller category '%s' at line %d. Using national subscriber as default.\n",
							v->value, v->lineno);
				}
			} else if (!strcasecmp(v->name, "mfcr2_logging")) {
				openr2_log_level_t tmplevel;
				char *clevel;
				char *logval = tris_strdupa(v->value);
				while (logval) {
 					clevel = strsep(&logval,",");
					if (-1 == (tmplevel = openr2_log_get_level(clevel))) {
						tris_log(LOG_WARNING, "Ignoring invalid logging level: '%s' at line %d.\n", clevel, v->lineno);
						continue;
					}
...
#endif /* HAVE_OPENR2 */
#ifdef HAVE_DPNSS
			} else if (!strcasecmp(v->name, "dpnss_crbit")) {
				confp->chan.dpnsscrbit = tris_true(v->value) ? 1 : 0;
#endif
				...
				tris_copy_string(original_args, v->value, sizeof(original_args));
				...
				if (element_count % 2 == 1) {
					tris_log(LOG_ERROR, "Must be a silence duration for each ring duration: %s at line %d.\n", original_args, v->lineno);
					cadence_is_ok = 0;
				}
				...
				for (i = 0; i < element_count; i++) {
					if (c[i] == 0) {
						tris_log(LOG_ERROR, "Ring or silence duration cannot be zero: %s at line %d.\n", original_args, v->lineno);
						cadence_is_ok = 0;
						break;
					} else if (c[i] < 0) {
						if (i % 2 == 1) {
							...
							if (cid_location == -1) {
								...
								tris_log(LOG_ERROR, "CID location specified twice: %s at line %d.\n", original_args, v->lineno);
								cadence_is_ok = 0;
								break;
							}
						} else {
							if (firstcadencepos == 0) {
								firstcadencepos = i; /* only recorded to avoid duplicate specification */
								 ...
								 tris_log(LOG_ERROR, "First cadence position specified twice: %s at line %d.\n", original_args, v->lineno);
								cadence_is_ok = 0;
								break;
							}
						}
				...
				if (cadence_is_ok) {
					...
					if (element_count < 2) {
						tris_log(LOG_ERROR, "Minimum cadence is ring,pause: %s at line %d.\n", original_args, v->lineno);
					} else {
						if (cid_location == -1) {
						...
						if ((num_cadence+1) >= NUM_CADENCE_MAX)
							tris_log(LOG_ERROR, "Already %d cadences; can't add another: %s at line %d.\n", NUM_CADENCE_MAX, original_args, v->lineno);
						else {
							cadences[num_cadence] = new_cadence;
							cidrings[num_cadence++] = cid_location;
							tris_verb(3, "cadence 'r%d' added: %s\n",num_cadence,original_args);
						}
					}
				}
				...
				if (ctlfd == -1) {
					tris_log(LOG_ERROR, "Unable to open /dev/dahdi/ctl to set toneduration at line %d.\n", v->lineno);
					return -1;
				}
				...
				if (toneduration > -1) {
					...
					if (res < 0) {
						tris_log(LOG_ERROR, "Invalid tone duration: %d ms at line %d: %s\n", toneduration, v->lineno, strerror(errno));
						return -1;
					}
				}
				close(ctlfd);
			} else if (!strcasecmp(v->name, "defaultcic")) {
				tris_copy_string(defaultcic, v->value, sizeof(defaultcic));
			} else if (!strcasecmp(v->name, "defaultozz")) {
				tris_copy_string(defaultozz, v->value, sizeof(defaultozz));
			} else if (!strcasecmp(v->name, "mwilevel")) {
				mwilevel = atoi(v->value);
			}
		} else if (!(options & PROC_DAHDI_OPT_NOWARN) )
			tris_log(LOG_WARNING, "Ignoring any changes to '%s' (on reload) at line %d.\n", v->name, v->lineno);
	}
	if (dahdichan[0]) {
	...
	if (!found_pseudo && reload != 1) {
		...
		*/
		...
		if (tmp) {
			tris_verb(3, "Automatically generated pseudo channel\n");
		} else {
			tris_log(LOG_WARNING, "Unable to register pseudo channel!\n");
		}
	}
	return 0;
}
...
static int setup_dahdi(int reload)
{
	...
	if (!cfg) {
		...
		if (ucfg == CONFIG_STATUS_FILEUNCHANGED) {
			...
			tris_log(LOG_ERROR, "File users.conf cannot be parsed.  Aborting.\n");
			return 0;
		}
	...
	if (reload != 1) {
		...
		v = tris_variable_browse(cfg, "trunkgroups");
		while (v) {
				...
				if (trunkgroup > 0) {
					if ((c = strchr(v->value, ','))) {
						...
						while (c && (i < NUM_DCHANS)) {
							...
							if (dchannels[i] < 0) {
								tris_log(LOG_WARNING, "D-channel for trunk group %d must be a postiive number at line %d of chan_dahdi.conf\n", trunkgroup, v->lineno);
							} else
								i++;
							c = strchr(c + 1, ',');
						}
						if (i) {
							if (pri_create_trunkgroup(trunkgroup, dchannels)) {
								tris_log(LOG_WARNING, "Unable to create trunk group %d with Primary D-channel %d at line %d of chan_dahdi.conf\n", trunkgroup, dchannels[0], v->lineno);
						} else
								tris_verb(2, "Created trunk group %d with Primary D-channel %d and %d backup%s\n", trunkgroup, dchannels[0], i - 1, (i == 1) ? "" : "s");
						} else
							tris_log(LOG_WARNING, "Trunk group %d lacks any valid D-channels at line %d of chan_dahdi.conf\n", trunkgroup, v->lineno);
					} else
						tris_log(LOG_WARNING, "Trunk group %d lacks a primary D-channel at line %d of chan_dahdi.conf\n", trunkgroup, v->lineno);
				} else
					tris_log(LOG_WARNING, "Trunk group identifier must be a positive integer at line %d of chan_dahdi.conf\n", v->lineno);
						...
						if (trunkgroup > 0) {
							...
							if (logicalspan >= 0) {
								if (pri_create_spanmap(spanno - 1, trunkgroup, logicalspan)) {
									tris_log(LOG_WARNING, "Failed to map span %d to trunk group %d (logical span %d)\n", spanno, trunkgroup, logicalspan);
							} else
									tris_verb(2, "Mapped span %d to trunk group %d (logical span %d)\n", spanno, trunkgroup, logicalspan);
							} else
								tris_log(LOG_WARNING, "Logical span must be a postive number, or '0' (for unspecified) at line %d of chan_dahdi.conf\n", v->lineno);
						} else
							tris_log(LOG_WARNING, "Trunk group must be a postive number at line %d of chan_dahdi.conf\n", v->lineno);
					} else
						tris_log(LOG_WARNING, "Missing trunk group for span map at line %d of chan_dahdi.conf\n", v->lineno);
				} else
					tris_log(LOG_WARNING, "Span number must be a postive integer at line %d of chan_dahdi.conf\n", v->lineno);
			} else {
				tris_log(LOG_NOTICE, "Ignoring unknown keyword '%s' in trunkgroups\n", v->name);
			}
			v = v->next;
		}
	}
	...
	v = tris_variable_browse(cfg, "channels");
	...
	for (cat = tris_category_browse(cfg, NULL); cat ; cat = tris_category_browse(cfg, cat)) {
		...
		if ((res = process_dahdi(&conf, cat, tris_variable_browse(cfg, cat), reload, PROC_DAHDI_OPT_NOCHAN))) {
			tris_mutex_unlock(&iflock);
			tris_config_destroy(cfg);
			if (ucfg) {
				tris_config_destroy(cfg);
	...
	if (ucfg) {
		...
		process_dahdi(&base_conf, "", tris_variable_browse(ucfg, "general"), 1, 0);
		...
		for (cat = tris_category_browse(ucfg, NULL); cat ; cat = tris_category_browse(ucfg, cat)) {
			...
			chans = tris_variable_retrieve(ucfg, cat, "dahdichan");

			if (tris_strlen_zero(chans)) {
				continue;
			}

			memcpy(&conf, &base_conf, sizeof(conf));

			if ((res = process_dahdi(&conf, cat, tris_variable_browse(ucfg, cat), reload, PROC_DAHDI_OPT_NOCHAN | PROC_DAHDI_OPT_NOWARN))) {
				tris_config_destroy(ucfg);
				tris_mutex_unlock(&iflock);
				return res;
			}
	...
	if (reload != 1) {
		...
		for (x = 0; x < NUM_SPANS; x++) {
			if (pris[x].pvts[0]) {
				if (start_pri(pris + x)) {
					tris_log(LOG_ERROR, "Unable to start D-channel on span %d\n", x + 1);
					return -1;
				} else
					tris_verb(2, "Starting D-Channel on span %d\n", x + 1);
			}
		}
	}
#endif
	...
	if (reload != 1) {
		...
		for (x = 0; x < NUM_SPANS; x++) {
			if (linksets[x].ss7) {
				if (tris_pthread_create(&linksets[x].master, NULL, ss7_linkset, &linksets[x])) {
					tris_log(LOG_ERROR, "Unable to start SS7 linkset on span %d\n", x + 1);
					return -1;
				} else
					tris_verb(2, "Starting SS7 linkset on span %d\n", x + 1);
			}
		}
	}
#endif
	...
	if (reload != 1) {
		int x;
		for (x = 0; x < NUM_SPANS; x++) {
			if (dpnsslinks[x].pvts[0]) {
				if (tris_pthread_create(&dpnsslinks[x].master, NULL, dpnss_linkset, &dpnsslinks[x])) {
					tris_log(LOG_ERROR, "Unable to start DPNSS link on span %d\n", x + 1);
					return -1;
				} else
					tris_verb(2, "Starting DPNSS linkset on span %d\n", x + 1);
			}
		}
	}
#endif
	...
	if (reload != 1) {
		...
		for (x = 0; x < r2links_count; x++) {
			if (tris_pthread_create(&r2links[x]->r2master, NULL, mfcr2_monitor, r2links[x])) {
				tris_log(LOG_ERROR, "Unable to start R2 monitor on channel group %d\n", x + 1);
				return -1;
			} else {
				tris_verb(2, "Starting R2 monitor on channel group %d\n", x + 1);
			}
		}
	}
#endif
...
static int load_module(void)
{
	...
	memset(pris, 0, sizeof(pris));
	for (y = 0; y < NUM_SPANS; y++) {
		tris_mutex_init(&pris[y].lock);
		pris[y].offset = -1;
		pris[y].master = TRIS_PTHREADT_NULL;
		for (i = 0; i < NUM_DCHANS; i++)
			pris[y].fds[i] = -1;
	}
	pri_set_error(dahdi_pri_error);
	pri_set_message(dahdi_pri_message);
	tris_register_application_xml(dahdi_send_keypad_facility_app, dahdi_send_keypad_facility_exec);
#ifdef HAVE_PRI_PROG_W_CAUSE
	tris_register_application_xml(dahdi_send_callrerouting_facility_app, dahdi_send_callrerouting_facility_exec);
#endif
#endif
#ifdef HAVE_SS7
	memset(linksets, 0, sizeof(linksets));
	for (y = 0; y < NUM_SPANS; y++) {
		tris_mutex_init(&linksets[y].lock);
		linksets[y].master = TRIS_PTHREADT_NULL;
		for (i = 0; i < NUM_DCHANS; i++)
			linksets[y].fds[i] = -1;
	}
	ss7_set_error(dahdi_ss7_error);
	...
	memset(dpnsslinks,0,sizeof(dpnsslinks));
	for(y = 0; y < NUM_SPANS;y++){
		tris_mutex_init(&dpnsslinks[y].lock);
		dpnsslinks[y].master = TRIS_PTHREADT_NULL;
		dpnsslinks[y].dfd = -1;
	}
	dpnss_set_message(dahdi_dpnss_message);
#endif 
	...
	if (tris_channel_register(&dahdi_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel class 'DAHDI'\n");
		__unload_module();
		return TRIS_MODULE_LOAD_FAILURE;
	}
#ifdef HAVE_PRI
	tris_string_field_init(&inuse, 16);
	tris_string_field_set(&inuse, name, "GR-303InUse");
	tris_cli_register_multiple(dahdi_pri_cli, ARRAY_LEN(dahdi_pri_cli));
#endif
#ifdef HAVE_SS7
	tris_cli_register_multiple(dahdi_ss7_cli, ARRAY_LEN(dahdi_ss7_cli));
#endif
#ifdef HAVE_DPNSS
	tris_cli_register_multiple(dahdi_dpnss_cli, ARRAY_LEN(dahdi_dpnss_cli));
#endif
#ifdef HAVE_OPENR2
	tris_cli_register_multiple(dahdi_mfcr2_cli, sizeof(dahdi_mfcr2_cli)/sizeof(dahdi_mfcr2_cli[0]));
	tris_register_application_xml(dahdi_accept_r2_call_app, dahdi_accept_r2_call_exec);
#endif

	tris_cli_register_multiple(dahdi_cli, ARRAY_LEN(dahdi_cli));

	memset(round_robin, 0, sizeof(round_robin));
	tris_manager_register( "DAHDITransfer", 0, action_transfer, "Transfer DAHDI Channel" );
	tris_manager_register( "DAHDIHangup", 0, action_transferhangup, "Hangup DAHDI Channel" );
	tris_manager_register( "DAHDIDialOffhook", 0, action_dahdidialoffhook, "Dial over DAHDI channel while offhook" );
	tris_manager_register( "DAHDIDNDon", 0, action_dahdidndon, "Toggle DAHDI channel Do Not Disturb status ON" );
	tris_manager_register( "DAHDIDNDoff", 0, action_dahdidndoff, "Toggle DAHDI channel Do Not Disturb status OFF" );
	tris_manager_register("DAHDIShowChannels", 0, action_dahdishowchannels, "Show status DAHDI channels");
	tris_manager_register("DAHDIRestart", 0, action_dahdirestart, "Fully Restart DAHDI channels (terminates calls)");

	tris_cond_init(&ss_thread_complete, NULL);
...
#define	ASCII_BYTES_PER_CHAR 80
	...
	if (idx < 0) {
		tris_log(LOG_WARNING, "Huh?  I don't exist?\n");
		return -1;
	}
	if (!text[0]) return(0); /* if nothing to send, dont */
	if ((!p->tdd) && (!p->mate)) return(0);  /* if not in TDD mode, just return */
	if (p->mate)
		buf = tris_malloc(((strlen(text) + 1) * ASCII_BYTES_PER_CHAR) + END_SILENCE_LEN + HEADER_LEN);
	else
		buf = tris_malloc(((strlen(text) + 1) * TDD_BYTES_PER_CHAR) + END_SILENCE_LEN);
	if (!buf)
		return -1;
	mybuf = buf;
	if (p->mate) {
		int codec = TRIS_LAW(p);
		...
		if (len < 1) {
			tris_log(LOG_ERROR, "TDD generate (len %d) failed!!\n", (int)strlen(text));
			tris_free(mybuf);
			return -1;
		}
	}
...
static int reload(void)
{
	...
	restarting = 0;
	if (res) {
		tris_log(LOG_WARNING, "Reload of chan_dahdi.so is unsuccessful!\n");
		return -1;
	}
	return 0;
}
 ...
 * TRIS_MODULE_INFO(, , "DAHDI Telephony"
 */
