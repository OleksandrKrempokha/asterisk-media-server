 ...
 * Configuration file \link Config_gateway gateway.conf \endlink
 *
 * ********** IMPORTANT *
 * \note TCP/TLS support is EXPERIMENTAL and WILL CHANGE. This applies to configuration
 *	settings, dialplan commands and dialplans apps/functions
 * See \ref gateway_tcp_tls
 * 
 *
 * ******** General TODO:s
 ...
 * - Support of GATEWAY domains for devices, so that we match on username@domain in the From: header
 * - Connect registrations with a specific device on the incoming call. It's not done
 *   automatically in Trismedia
 *
 * \ingroup channel_drivers
 *
 * \par Overview of the handling of GATEWAY sessions
 * The GATEWAY channel handles several types of GATEWAY sessions, or dialogs,
 * not all of them being "telephone calls".
 * - Incoming calls that will be sent to the PBX core
 * - Outgoing calls, generated by the PBX
 * - GATEWAY subscriptions and notifications of states and voicemail messages
 * - GATEWAY registrations, both inbound and outbound
 * - GATEWAY peer management (peerpoke, OPTIONS)
 * - GATEWAY text messages
 *
 * In the GATEWAY channel, there's a list of active GATEWAY dialogs, which includes
 * all of these when they are active. "gateway show channels" in the CLI will
 * show most of these, excluding subscriptions which are shown by
 * "gateway show subscriptions"
 *
 * \par incoming packets
 * Incoming packets are received in the monitoring thread, then handled by
 * gatewaysock_read() for udp only. In tcp, packets are read by the tcp_helper thread.
 * gatewaysock_read() function parses the packet and matches an existing
 * dialog or starts a new GATEWAY dialog.
 * 
 * gatewaysock_read sends the packet to handle_incoming(), that parses a bit more.
 ...
 * gatewaysock_read locks the tris_channel if it exists (an active call) and
 * unlocks it after we have processed the GATEWAY message.
 *
 ...
 * the gateway_answer() function is called.
 *
 * The actual media - Video or Audio - is mostly handled by the RTP subsystem
 * in rtp.c 
 * 
 * \par Outbound calls
 * Outbound calls are set up by the PBX through the gateway_request_call()
 * function. After that, they are activated by gateway_call().
 * 
 * \par Hanging up
 * The PBX issues a hangup on both incoming and outgoing calls through
 * the gateway_hangup() function
 */

/*!  
 * \page gateway_tcp_tls GATEWAY TCP and TLS support
 * 
 ...
 *	If someone registers a GATEWAYS uri, this forces us to set up a TLS connection back.
 * \todo Add TCP/TLS information to function GATEWAYPEER and GATEWAYCHANINFO
 ...
 *		
 * \todo re-evaluate the transport= setting in gateway.conf. This is right now not well
 * 	thought of. If a device in gateway.conf contacts us via TCP, we should not switch transport,
 ...
 * \todo We need to test TCP sessions with GATEWAY proxies and in regards
 *       to the GATEWAY outbound specs.
 ...
 *	DNS naptr support is crucial. A GATEWAY uri might lead to a TLS connection.
 *	Also note that due to outbound proxy settings, a GATEWAYS uri might have to be sent on UDP (not to recommend though)
 ...
 * parse_register_contact(struct gateway_pvt *pvt, struct gateway_peer *peer, struct gateway_request *req)
 ...
 * get_destination(struct gateway_pvt *p, struct gateway_request *oreq)
 *	- Doesn't store the information that we got an incoming GATEWAYS request in the channel, so that
 *	  we can require a secure signalling path OUT of Trismedia (on GATEWAY or IAX2). Possibly, the call should
 ...
 * get_refer_info(struct gateway_pvt *transferer, struct gateway_request *outgoing_req)
 *	- As above, if we have a GATEWAYS: uri in the refer-to header
...
/*!  \page gateway_session_timers GATEWAY Session Timers in Trismedia Chan_gateway

	The GATEWAY Session-Timers is an extension of the GATEWAY protocol that allows end-points and proxies to
	...
	the state in the proxies and the remote UA (this is done for the benefit of GATEWAY entities in the path
	...
	added to the gateway.conf file.
		session-timers=["accept", "originate", "refuse"]
		session-expires=[integer]
		session-minse=[integer]
		session-refresher=["uas", "uac"]

	The session-timers parameter in gateway.conf defines the mode of operation of GATEWAY session-timers feature in
	...
	2. Originate :: In the "originate" mode, the Trismedia server requests the remote 
		...
		against hanging GATEWAY channels due to network or end-point failures, Trismedia
...
/*** DOCUMENTATION
	<application name="GATEWAYDtmfMode" language="en_US">
		<synopsis>
			Change the dtmfmode for a GATEWAY call.
		</synopsis>
		<syntax>
		...
		<description>
			<para>Changes the dtmfmode for a GATEWAY call.</para>
		</description>
	</application>
	<application name="GATEWAYAddHeader" language="en_US">
		<synopsis>
			Add a GATEWAY header to the outbound call.
		</synopsis>
		<syntax argsep=":">
		...
		<description>
			<para>Adds a header to a GATEWAY call placed with DIAL.</para>
			<para>Remember to use the X-header if you are adding non-standard GATEWAY
			headers, like <literal>X-Trismedia-Accountcode:</literal>. Use this with care.
			Adding the wrong headers may jeopardize the GATEWAY dialog.</para>
			<para>Always returns <literal>0</literal>.</para>
		</description>
	</application>
	<application name="GATEWAYRemoveHeader" language="en_US">
		<synopsis>
			Remove GATEWAY headers previously added with GATEWAYAddHeader
		</synopsis>
		<syntax>
			<parameter name="Header" required="false" />
		</syntax>
		<description>
			<para>GATEWAYRemoveHeader() allows you to remove headers which were previously 
			added with GATEWAYAddHeader(). If no parameter is supplied, all previously added 
			...
			<para>GATEWAYAddHeader(P-Asserted-Identity: sip:foo@bar);</para>
			<para>GATEWAYAddHeader(P-Preferred-Identity: sip:bar@foo);</para>
			<para></para>
			<para>// remove all headers</para>
			<para>GATEWAYRemoveHeader();</para>
			<para>// remove all P- headers</para>
			<para>GATEWAYRemoveHeader(P-);</para>
			<para>// remove only the PAI header (note the : at the end)</para>
			<para>GATEWAYRemoveHeader(P-Asserted-Identity:);</para>
			<para></para>
			<para>Always returns <literal>0</literal>.</para>
		</description>
	</application>
	<function name="GATEWAY_HEADER" language="en_US">
		<synopsis>
			Gets the specified GATEWAY header.
		</synopsis>
		<syntax>
		...
		<description>
			...
			times, GATEWAY_HEADER takes an optional second argument to specify which header with
			that name to retrieve. Headers start at offset <literal>1</literal>.</para>
		</description>
	</function>
	<function name="GATEWAYPEER" language="en_US">
		<synopsis>
			Gets GATEWAY peer information.
		</synopsis>
		<syntax>
	...
	<function name="GATEWAYCHANINFO" language="en_US">
		<synopsis>
			Gets the specified GATEWAY parameter from the current channel.
		</synopsis>
		<syntax>
	...
	<function name="CHECKGATEWAYDOMAIN" language="en_US">
		<synopsis>
		...
		<description>
			...
			as a local GATEWAY domain that this Trismedia server is configured to handle.
			Returns the domain name if it is locally handled, otherwise an empty string.
			Check the <literal>domain=</literal> configuration in <filename>gateway.conf</filename>.</para>
		</description>
	</function>
 ***/
...
#define	GATEWAYBUFSIZE		512		/*!< Buffer size for many operations */

#define XMIT_ERROR		-2

#define GATEWAY_RESERVED ";/?:@&=+$,# "		/*!< Reserved characters in the username part of the URI */
...
#define DEFAULT_TIMER_T1                 500              /*!< GATEWAY timer T1 (according to RFC 3261) */
#define GATEWAY_TRANS_TIMEOUT            64 * DEFAULT_TIMER_T1 /*!< GATEWAY request timeout (rfc 3261) 64*T1 
                                                      \todo Use known T1 for timeout (peerpoke)
                                                      */
#define DEFAULT_TRANS_TIMEOUT        -1               /*!< Use default GATEWAY transaction timeout */
...
#define GATEWAY_MAX_HEADERS              64               /*!< Max amount of GATEWAY headers to read */
#define GATEWAY_MAX_LINES                256               /*!< Max amount of lines in GATEWAY attachment (like SDP) */
#define GATEWAY_MIN_PACKET               4096             /*!< Initialize size of memory to allocate for packets */
#define MAX_HISTORY_ENTRIES 	     50	              /*!< Max entires in the history list for a gateway_pvt */

#define INITIAL_CSEQ                 101              /*!< Our initial gateway sequence number */
...
static const char config[] = "gateway.conf";		/*!< Main configuration file */
static const char notify_config[] = "gateway_notify.conf";	/*!< Configuration file for sending Notify with CLI commands to reconfigure or reboot phones */
...
enum transfermodes {
	TRANSFER_OPENFORALL,            /*!< Allow all GATEWAY transfers */
	TRANSFER_CLOSED,                /*!< Allow no GATEWAY transfers */
};
...
/*! \brief When sending a GATEWAY message, we can send with a few options, depending on
	type of GATEWAY request. UNRELIABLE is moslty used for responses to repeated requests,
	where the original response would be sent RELIABLE in an INVITE transaction */
enum xmittype {
	XMIT_CRITICAL = 2,              /*!< Transmit critical GATEWAY message reliably, with re-transmits.
                                              If it fails, it's critical and will cause a teardown of the session */
	XMIT_RELIABLE = 1,              /*!< Transmit GATEWAY message reliably, with re-transmits */
	XMIT_UNRELIABLE = 0,            /*!< Transmit GATEWAY message without bothering with re-transmits */
};
...
static const struct cfsubscription_types {
	...
	{ NONE,		   "-",        "unknown",	             "unknown" },
 	/* RFC 4235: GATEWAY Dialog event package */
...
/*! \brief States for outbound registrations (with register= lines in gateway.conf */
enum gatewayregistrystate {
	REG_STATE_UNREGISTERED = 0,	/*!< We are not registered 
		 *  \note Initial state. We should have a timeout scheduled for the initial
		 * (or next) registration transmission, calling gateway_reregister
		 */
...
/*! \brief Modes in which Trismedia can be configured to run GATEWAY Session-Timers */
enum st_mode {
...
/*! \brief Define some implemented GATEWAY transports 
	\note Trismedia does not support SCTP or UDP/DTLS 
*/
enum gateway_transport {
	GATEWAY_TRANSPORT_UDP = 1,		/*!< Unreliable transport for GATEWAY, needs retransmissions */
	GATEWAY_TRANSPORT_TCP = 1 << 1,	/*!< Reliable, but unsecure */
	GATEWAY_TRANSPORT_TLS = 1 << 2,	/*!< TCP/TLS - reliable and secure transport for signalling */
};

/*! \brief definition of a gateway proxy server
 *
 * For outbound proxies, a gateway_peer will contain a reference to a 
 * dynamically allocated instance of a gateway_proxy. A gateway_pvt may also
 * contain a reference to a peer's outboundproxy, or it may contain
 * a reference to the gateway_cfg.outboundproxy.
 */
struct gateway_proxy {
	char name[MAXHOSTNAMELEN];      /*!< DNS name of domain/host or IP */
	struct sockaddr_in ip;          /*!< Currently used IP address and port */
	struct sockaddr_in recv;
	time_t last_dnsupdate;          /*!< When this was resolved */
	enum gateway_transport transport;	
...
/*! \brief States whether a GATEWAY message can create a dialog in Trismedia. */
...
/*! \brief GATEWAY Request methods known by Trismedia 
...
enum gatewaymethod {
	GATEWAY_UNKNOWN,		/*!< Unknown response */
	GATEWAY_RESPONSE,		/*!< Not request, response to outbound request */
	GATEWAY_REGISTER,		/*!< Registration to the mothership, tell us where you are located */
	GATEWAY_OPTIONS,		/*!< Check capabilities of a device, used for "ping" too */
	GATEWAY_NOTIFY,		/*!< Status update, Part of the event package standard, result of a SUBSCRIBE or a REFER */
	GATEWAY_INVITE,		/*!< Set up a session */
	GATEWAY_ACK,		/*!< End of a three-way handshake started with INVITE. */
	GATEWAY_PRACK,		/*!< Reliable pre-call signalling. Not supported in Trismedia. */
	GATEWAY_BYE,		/*!< End of a session */
	GATEWAY_REFER,		/*!< Refer to another URI (transfer) */
	GATEWAY_SUBSCRIBE,		/*!< Subscribe for updates (voicemail, session status, device status, presence) */
	GATEWAY_MESSAGE,		/*!< Text messaging */
	GATEWAY_UPDATE,		/*!< Update a dialog. We can send UPDATE; but not accept it */
	GATEWAY_INFO,		/*!< Information updates during a session */
	GATEWAY_CANCEL,		/*!< Cancel an INVITE */
	GATEWAY_PUBLISH,		/*!< Not supported in Trismedia */
	GATEWAY_PING,		/*!< Not supported at all, no standard but still implemented out there */
};

/*! \brief Settings for the 'notifycid' option, see gateway.conf.sample for details. */
...
/*! \brief The core structure to setup dialogs. We parse incoming messages by using
	structure and then route the messages according to the type.
      ...
      \note Note that gateway_methods[i].id == i must hold or the code breaks */
static const struct  cfgateway_methods { 
	enum gatewaymethod id;
	int need_rtp;		/*!< when this is the 'primary' use for a pvt structure, does it need RTP? */
	char * const text;
	enum can_create_dialog can_create;
} gateway_methods[] = {
	{ GATEWAY_UNKNOWN,	 RTP,    "-UNKNOWN-", 	CAN_CREATE_DIALOG },
	{ GATEWAY_RESPONSE,	 NO_RTP, "SIP/2.0",	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_REGISTER,	 NO_RTP, "REGISTER", 	CAN_CREATE_DIALOG },
 	{ GATEWAY_OPTIONS,	 NO_RTP, "OPTIONS", 	CAN_CREATE_DIALOG },
	{ GATEWAY_NOTIFY,	 NO_RTP, "NOTIFY", 	CAN_CREATE_DIALOG },
	{ GATEWAY_INVITE,	 RTP,    "INVITE", 	CAN_CREATE_DIALOG },
	{ GATEWAY_ACK,	 NO_RTP, "ACK", 	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_PRACK,	 NO_RTP, "PRACK", 	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_BYE,	 NO_RTP, "BYE", 	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_REFER,	 NO_RTP, "REFER", 	CAN_CREATE_DIALOG },
	{ GATEWAY_SUBSCRIBE, NO_RTP, "SUBSCRIBE", 	CAN_CREATE_DIALOG },
	{ GATEWAY_MESSAGE,	 NO_RTP, "MESSAGE", 	CAN_CREATE_DIALOG },
	{ GATEWAY_UPDATE,	 NO_RTP, "UPDATE", 	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_INFO,	 NO_RTP, "INFO", 	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_CANCEL,	 NO_RTP, "CANCEL", 	CAN_NOT_CREATE_DIALOG },
	{ GATEWAY_PUBLISH,	 NO_RTP, "PUBLISH", 	CAN_CREATE_DIALOG_UNSUPPORTED_METHOD },
	{ GATEWAY_PING,	 NO_RTP, "PING", 	CAN_CREATE_DIALOG_UNSUPPORTED_METHOD }
};

static unsigned int chan_idx;

/*!  Define GATEWAY option tags, used in Require: and Supported: headers 
...
/* GATEWAY options */
#define GATEWAY_OPT_REPLACES	(1 << 0)
#define GATEWAY_OPT_100REL		(1 << 1)
#define GATEWAY_OPT_TIMER		(1 << 2)
#define GATEWAY_OPT_EARLY_SESSION	(1 << 3)
#define GATEWAY_OPT_JOIN		(1 << 4)
#define GATEWAY_OPT_PATH		(1 << 5)
#define GATEWAY_OPT_PREF		(1 << 6)
#define GATEWAY_OPT_PRECONDITION	(1 << 7)
#define GATEWAY_OPT_PRIVACY		(1 << 8)
#define GATEWAY_OPT_SDP_ANAT	(1 << 9)
#define GATEWAY_OPT_SEC_AGREE	(1 << 10)
#define GATEWAY_OPT_EVENTLIST	(1 << 11)
#define GATEWAY_OPT_GRUU		(1 << 12)
#define GATEWAY_OPT_TARGET_DIALOG	(1 << 13)
#define GATEWAY_OPT_NOREFERSUB	(1 << 14)
#define GATEWAY_OPT_HISTINFO	(1 << 15)
#define GATEWAY_OPT_RESPRIORITY	(1 << 16)
#define GATEWAY_OPT_FROMCHANGE	(1 << 17)
#define GATEWAY_OPT_RECLISTINV	(1 << 18)
#define GATEWAY_OPT_RECLISTSUB	(1 << 19)
#define GATEWAY_OPT_OUTBOUND	(1 << 20)
#define GATEWAY_OPT_UNKNOWN		(1 << 21)


/*! \brief List of well-known GATEWAY options. If we get this in a require,
   we should check the list and answer accordingly. */
static const struct cfgateway_options {
	int id;			/*!< Bitmap ID */
	int supported;		/*!< Supported by Trismedia ? */
	char * const text;	/*!< Text id, as in standard */
} gateway_options[] = {	/* XXX used in 3 places */
	/* RFC3262: PRACK 100% reliability */
	{ GATEWAY_OPT_100REL,	NOT_SUPPORTED,	"100rel" },	
	/* RFC3959: GATEWAY Early session support */
	{ GATEWAY_OPT_EARLY_SESSION, NOT_SUPPORTED,	"early-session" },
	/* SIMPLE events:  RFC4662 */
	{ GATEWAY_OPT_EVENTLIST,	NOT_SUPPORTED,	"eventlist" },
	/* RFC 4916- Connected line ID updates */
	{ GATEWAY_OPT_FROMCHANGE,	NOT_SUPPORTED,	"from-change" },
	/* GRUU: Globally Routable User Agent URI's */
	{ GATEWAY_OPT_GRUU,		NOT_SUPPORTED,	"gruu" },
	/* RFC4244 History info */
	{ GATEWAY_OPT_HISTINFO,	NOT_SUPPORTED,	"histinfo" },
	/* RFC3911: GATEWAY Join header support */
	{ GATEWAY_OPT_JOIN,		NOT_SUPPORTED,	"join" },
	/* Disable the REFER subscription, RFC 4488 */
	{ GATEWAY_OPT_NOREFERSUB,	NOT_SUPPORTED,	"norefersub" },
	/* GATEWAY outbound - the final NAT battle - draft-gateway-outbound */
	{ GATEWAY_OPT_OUTBOUND,	NOT_SUPPORTED,	"outbound" },
	/* RFC3327: Path support */
	{ GATEWAY_OPT_PATH,		NOT_SUPPORTED,	"path" },
	/* RFC3840: Callee preferences */
	{ GATEWAY_OPT_PREF,		NOT_SUPPORTED,	"pref" },
	/* RFC3312: Precondition support */
	{ GATEWAY_OPT_PRECONDITION,	NOT_SUPPORTED,	"precondition" },
	/* RFC3323: Privacy with proxies*/
	{ GATEWAY_OPT_PRIVACY,	NOT_SUPPORTED,	"privacy" },
	/* RFC-ietf-gateway-uri-list-conferencing-02.txt conference invite lists */
	{ GATEWAY_OPT_RECLISTINV,	NOT_SUPPORTED,	"recipient-list-invite" },
	/* RFC-ietf-gateway-uri-list-subscribe-02.txt - subscription lists */
	{ GATEWAY_OPT_RECLISTSUB,	NOT_SUPPORTED,	"recipient-list-subscribe" },
	/* RFC3891: Replaces: header for transfer */
	{ GATEWAY_OPT_REPLACES,	SUPPORTED,	"replaces" },	
	/* One version of Polycom firmware has the wrong label */
	{ GATEWAY_OPT_REPLACES,	SUPPORTED,	"replace" },	
	/* RFC4412 Resource priorities */
	{ GATEWAY_OPT_RESPRIORITY,	NOT_SUPPORTED,	"resource-priority" },
	/* RFC3329: Security agreement mechanism */
	{ GATEWAY_OPT_SEC_AGREE,	NOT_SUPPORTED,	"sec_agree" },
	/* RFC4092: Usage of the SDP ANAT Semantics in the GATEWAY */
	{ GATEWAY_OPT_SDP_ANAT,	NOT_SUPPORTED,	"sdp-anat" },
	/* RFC4028: GATEWAY Session-Timers */
	{ GATEWAY_OPT_TIMER,	SUPPORTED,	"timer" },
	/* RFC4538: Target-dialog */
	{ GATEWAY_OPT_TARGET_DIALOG,NOT_SUPPORTED,	"tdialog" },
};


/*! \brief GATEWAY Methods we support 
	\todo This string should be set dynamically. We only support REFER and SUBSCRIBE if we have
	allowsubscribe and allowrefer on in gateway.conf.
*/
#define ALLOWED_METHODS "INVITE, ACK, CANCEL, OPTIONS, BYE, REFER, SUBSCRIBE, NOTIFY, INFO"

/*! \brief GATEWAY Extensions we support 
...
/*! \brief Standard GATEWAY unsecure port for UDP and TCP from RFC 3261. DO NOT CHANGE THIS */
#define STANDARD_GATEWAY_PORT	5060
/*! \brief Standard GATEWAY TLS port from RFC 3261. DO NOT CHANGE THIS */
#define STANDARD_TLS_PORT	5061

/*! \note in many GATEWAY headers, absence of a port number implies port 5060,
 ...
 * You can run Trismedia with GATEWAY on a different port with a configuration
...
/*! \name DefaultValues Default values, set and reset in reload_config before reading configuration 
   ...
   gateway.conf.sample for new installations. These may differ to keep backwards compatibility,
...
#define DEFAULT_COMPACTHEADERS	FALSE		/*!< Send compact (one-character) GATEWAY headers. Default off */
#define DEFAULT_TOS_GATEWAY         0               /*!< Call signalling packets should be marked as DSCP CS3, but the default is 0 to be compatible with previous versions. */
...
#define DEFAULT_COS_GATEWAY         4		/*!< Level 2 class of service for GATEWAY signalling */
...
#define DEFAULT_PEDANTIC	FALSE		/*!< Avoid following GATEWAY standards for dialog matching */
...
#define DEFAULT_CALLEVENTS	FALSE		/*!< Extra manager GATEWAY call events */
#define DEFAULT_ALWAYSAUTHREJECT	FALSE	/*!< Don't reject authentication requests always */
#define DEFAULT_REGEXTENONQUALIFY FALSE
#define DEFAULT_T1MIN		100		/*!< 100 MS for minimal roundtrip time */
#define DEFAULT_MAX_CALL_BITRATE (384)		/*!< Max bitrate for video */
#ifndef DEFAULT_USERAGENT
#define DEFAULT_USERAGENT "Trismedia"	/*!< Default Useragent: header unless re-defined in gateway.conf */
#define DEFAULT_SDPSESSION "Trismedia"	/*!< Default SDP session name, (s=) header unless re-defined in gateway.conf */
#define DEFAULT_SDPOWNER "root"			/*!< Default SDP username field in (o=) header unless re-defined in gateway.conf */
#endif
/*@}*/ 
...
static unsigned int default_transports;			/*!< Default Transports (enum gateway_transport) that are acceptable */
static unsigned int default_primary_transport;		/*!< Default primary Transport (enum gateway_transport) for outbound connections to devices */
...
*/
/*@{*/ 
/*! \brief a place to store all global settings for the gateway channel driver 
...
struct gateway_settings {
	...
	int pedanticgatewaychecking;	/*!< Extra checking ?  Default off */
	...
	int compactheaders;		/*!< send compact gateway headers */
	int allow_external_domains;	/*!< Accept calls to external GATEWAY domains? */
	...
	enum transfermodes allowtransfer;	/*!< GATEWAY Refer restriction scheme */
	...
	struct gateway_proxy outboundproxy;	/*!< Outbound proxy */
	char default_context[TRIS_MAX_CONTEXT];
	char default_subscribecontext[TRIS_MAX_CONTEXT];
};

static struct gateway_settings gateway_cfg;
...
static unsigned int global_tos_gateway;		/*!< IP type of service for GATEWAY packets */
static unsigned int global_tos_audio;		/*!< IP type of service for audio RTP packets */
static unsigned int global_tos_video;		/*!< IP type of service for video RTP packets */
static unsigned int global_tos_text;		/*!< IP type of service for text RTP packets */
static unsigned int global_cos_gateway;		/*!< 802.1p class of service for GATEWAY packets */
...
static int recordhistory;		/*!< Record GATEWAY history. Off by default */
static int dumphistory;			/*!< Dump history to verbose before destroying GATEWAY dialog */
static char global_regcontext[TRIS_MAX_CONTEXT];		/*!< Context for auto-extensions */
static char global_useragent[TRIS_MAX_EXTENSION];	/*!< Useragent for the GATEWAY channel */
static char global_sdpsession[TRIS_MAX_EXTENSION];	/*!< SDP session name for the GATEWAY channel */
static char global_sdpowner[TRIS_MAX_EXTENSION];	/*!< SDP owner name for the GATEWAY channel */
...
static struct tris_flags global_flags[2] = {{0}};        /*!< global GATEWAY_ flags */
...
TRIS_MUTEX_DEFINE_STATIC(gateway_reload_lock);

/*! \brief This is the thread for the monitor which checks for input on the channels
   which are not currently in use.  */
static pthread_t monitor_thread = TRIS_PTHREADT_NULL;

static int gateway_reloading = FALSE;                       /*!< Flag for avoiding multiple reloads at the same time */
static enum channelreloadreason gateway_reloadreason;       /*!< Reason for last reload/load of configuration */

static struct sched_context *sched;     /*!< The scheduling context */
static struct io_context *io;           /*!< The IO context */
static int *gatewaysock_read_id;            /*!< ID of IO entry for gatewaysock FD */
...
/*! \brief The GATEWAY socket definition */
struct gateway_socket {
	enum gateway_transport type;	/*!< UDP, TCP or TLS */
...
/*! \brief gateway_request: The data grabbed from the UDP socket
 *
 * \verbatim
 ...
 * Then call parse_request() and req.method = find_gateway_method();
 * to initialize the other fields. The \r\n at the end of each line is   
 * replaced by \0, so that data[] is not a conforming GATEWAY message anymore.
...
struct gateway_request {
	ptrdiff_t rlPart1; 	        /*!< Offset of the GATEWAY Method Name or "SIP/2.0" protocol version */
	...
	int headers;            /*!< # of GATEWAY Headers */
	...
	/* Array of offsets into the request string of each GATEWAY header*/
	ptrdiff_t header[GATEWAY_MAX_HEADERS];
	/* Array of offsets into the request string of each SDP line*/
	ptrdiff_t line[GATEWAY_MAX_LINES];
	struct tris_str *data;	
	/* XXX Do we need to unref socket.ser when the request goes away? */
	struct gateway_socket socket;	/*!< The socket used for this request */
	TRIS_LIST_ENTRY(gateway_request) next;
};

/* \brief given a gateway_request and an offset, return the char * that resides there
 *
 * It used to be that rlPart1, rlPart2, and the header and line arrays were character
 * pointers. They are now offsets into the tris_str portion of the gateway_request structure.
...
struct gateway_dual {
	...
	struct gateway_request req;		/*!< Request that caused the transfer (REFER) */
...
struct gateway_invite_param {
	int addgatewayheaders;		/*!< Add extra GATEWAY headers */
	const char *uri_options;	/*!< URI options to add to the URI */
	const char *vxml_url;		/*!< VXML url for Cisco phones */
	char *auth;			/*!< Authentication */
	char *authheader;		/*!< Auth header */
	enum gateway_auth_type auth_type;	/*!< Authentication type */
	const char *replaces;		/*!< Replaces header for call transfers */
	int transfer;			/*!< Flag - is this Invite part of a GATEWAY transfer? (invite/replaces) */
};

/*! \brief Structure to save routing information for a GATEWAY session */
struct gateway_route {
	struct gateway_route *next;
	char hop[0];
};

/*! \brief Modes for GATEWAY domain handling in the PBX */
enum domain_mode {
	GATEWAY_DOMAIN_AUTO,		/*!< This domain is auto-configured */
	GATEWAY_DOMAIN_CONFIG,		/*!< This domain is from configuration */
};
...
*/
struct domain {
	char domain[MAXHOSTNAMELEN];		/*!< GATEWAY domain we are responsible for */
...
static TRIS_LIST_HEAD_STATIC(domain_list, domain);	/*!< The GATEWAY domain list */


/*! \brief gateway_history: Structure for saving transactions within a GATEWAY dialog */
struct gateway_history {
	TRIS_LIST_ENTRY(gateway_history) list;
	char event[0];	/* actually more, depending on needs */
};

TRIS_LIST_HEAD_NOLOCK(gateway_history_head, gateway_history); /*!< history list, entry in gateway_pvt */

/*! \brief gateway_auth: Credentials for authentication to other GATEWAY services */
struct gateway_auth {
	char realm[TRIS_MAX_EXTENSION];  /*!< Realm in which these credentials are valid */
	char username[256];             /*!< Username */
	char secret[256];               /*!< Secret */
	char md5secret[256];            /*!< MD5Secret */
	struct gateway_auth *next;          /*!< Next auth structure in list */
};
...
*/
/*@{*/ 
#define GATEWAY_OUTGOING		(1 << 0)	/*!< D: Direction of the last transaction in this dialog */
#define GATEWAY_RINGING		(1 << 2)	/*!< D: Have sent 180 ringing */
#define GATEWAY_PROGRESS_SENT	(1 << 3)	/*!< D: Have sent 183 message progress */
#define GATEWAY_NEEDREINVITE	(1 << 4)	/*!< D: Do we need to send another reinvite? */
#define GATEWAY_PENDINGBYE		(1 << 5)	/*!< D: Need to send bye after we ack? */
#define GATEWAY_GOTREFER		(1 << 6)	/*!< D: Got a refer? */
#define GATEWAY_CALL_LIMIT		(1 << 7)	/*!< D: Call limit enforced for this call */
#define GATEWAY_INC_COUNT		(1 << 8)	/*!< D: Did this dialog increment the counter of in-use calls? */
#define GATEWAY_INC_RINGING		(1 << 9)	/*!< D: Did this connection increment the counter of in-use calls? */
#define GATEWAY_DEFER_BYE_ON_TRANSFER	(1 << 10)	/*!< D: Do not hangup at first tris_hangup */

#define GATEWAY_PROMISCREDIR	(1 << 11)	/*!< DP: Promiscuous redirection */
#define GATEWAY_TRUSTRPID		(1 << 12)	/*!< DP: Trust RPID headers? */
#define GATEWAY_USEREQPHONE		(1 << 13)	/*!< DP: Add user=phone to numeric URI. Default off */
#define GATEWAY_USECLIENTCODE	(1 << 14)	/*!< DP: Trust X-ClientCode info message */

/* DTMF flags - see str2dtmfmode() and dtmfmode2str() */
#define GATEWAY_DTMF		(7 << 15)	/*!< DP: DTMF Support: five settings, uses three bits */
#define GATEWAY_DTMF_RFC2833	(0 << 15)	/*!< DP: DTMF Support: RTP DTMF - "rfc2833" */
#define GATEWAY_DTMF_INBAND		(1 << 15)	/*!< DP: DTMF Support: Inband audio, only for ULAW/ALAW - "inband" */
#define GATEWAY_DTMF_INFO		(2 << 15)	/*!< DP: DTMF Support: GATEWAY Info messages - "info" */
#define GATEWAY_DTMF_AUTO		(3 << 15)	/*!< DP: DTMF Support: AUTO switch between rfc2833 and in-band DTMF */
#define GATEWAY_DTMF_SHORTINFO      (4 << 15)       /*!< DP: DTMF Support: GATEWAY Info messages - "info" - short variant */

/* NAT settings - see nat2str() */
#define GATEWAY_NAT			(3 << 18)	/*!< DP: four settings, uses two bits */
#define GATEWAY_NAT_NEVER		(0 << 18)	/*!< DP: No nat support */
#define GATEWAY_NAT_RFC3581		(1 << 18)	/*!< DP: NAT RFC3581 */
#define GATEWAY_NAT_ROUTE		(2 << 18)	/*!< DP: NAT Only ROUTE */
#define GATEWAY_NAT_ALWAYS		(3 << 18)	/*!< DP: NAT Both ROUTE and RFC3581 */

/* re-INVITE related settings */
#define GATEWAY_REINVITE		(7 << 20)	/*!< DP: four settings, uses three bits */
#define GATEWAY_REINVITE_NONE	(0 << 20)	/*!< DP: no reinvite allowed */
#define GATEWAY_DIRECT_MEDIA	(1 << 20)	/*!< DP: allow peers to be reinvited to send media directly p2p */
#define GATEWAY_DIRECT_MEDIA_NAT	(2 << 20)	/*!< DP: allow media reinvite when new peer is behind NAT */
#define GATEWAY_REINVITE_UPDATE	(4 << 20)	/*!< DP: use UPDATE (RFC3311) when reinviting this peer */

/* "insecure" settings - see insecure2str() */
#define GATEWAY_INSECURE		(3 << 23)	/*!< DP: three settings, uses two bits */
#define GATEWAY_INSECURE_NONE	(0 << 23)	/*!< DP: secure mode */
#define GATEWAY_INSECURE_PORT	(1 << 23)	/*!< DP: don't require matching port for incoming requests */
#define GATEWAY_INSECURE_INVITE	(1 << 24)	/*!< DP: don't require authentication for incoming INVITEs */

/* Sending PROGRESS in-band settings */
#define GATEWAY_PROG_INBAND		(3 << 25)	/*!< DP: three settings, uses two bits */
#define GATEWAY_PROG_INBAND_NEVER	(0 << 25)
#define GATEWAY_PROG_INBAND_NO	(1 << 25)
#define GATEWAY_PROG_INBAND_YES	(2 << 25)

#define GATEWAY_SENDRPID		(1 << 29)	/*!< DP: Remote Party-ID Support */
#define GATEWAY_G726_NONSTANDARD	(1 << 31)	/*!< DP: Use non-standard packing for G726-32 data */

/*! \brief Flags to copy from peer/user to dialog */
#define GATEWAY_FLAGS_TO_COPY \
	(GATEWAY_PROMISCREDIR | GATEWAY_TRUSTRPID | GATEWAY_SENDRPID | GATEWAY_DTMF | GATEWAY_REINVITE | \
	 GATEWAY_PROG_INBAND | GATEWAY_USECLIENTCODE | GATEWAY_NAT | GATEWAY_G726_NONSTANDARD | \
	 GATEWAY_USEREQPHONE | GATEWAY_INSECURE)
/*@}*/ 
...
/*@{*/ 
/* realtime flags */
#define GATEWAY_PAGE2_RTCACHEFRIENDS	(1 << 0)	/*!< GP: Should we keep RT objects in memory for extended time? */
#define GATEWAY_PAGE2_RTAUTOCLEAR		(1 << 2)	/*!< GP: Should we clean memory from peers after expiry? */
/* Space for addition of other realtime flags in the future */
#define GATEWAY_PAGE2_CONSTANT_SSRC     (1 << 8)	/*!< GDP: Don't change SSRC on reinvite */
#define GATEWAY_PAGE2_STATECHANGEQUEUE	(1 << 9)	/*!< D: Unsent state pending change exists */

#define GATEWAY_PAGE2_RPORT_PRESENT         (1 << 10)       /*!< Was rport received in the Via header? */
#define GATEWAY_PAGE2_VIDEOSUPPORT		(1 << 14)	/*!< DP: Video supported if offered? */
#define GATEWAY_PAGE2_TEXTSUPPORT		(1 << 15)	/*!< GDP: Global text enable */
#define GATEWAY_PAGE2_ALLOWSUBSCRIBE	(1 << 16)	/*!< GP: Allow subscriptions from this peer? */
#define GATEWAY_PAGE2_ALLOWOVERLAP		(1 << 17)	/*!< DP: Allow overlap dialing ? */
#define GATEWAY_PAGE2_SUBSCRIBEMWIONLY	(1 << 18)	/*!< GP: Only issue MWI notification if subscribed to */
#define GATEWAY_PAGE2_IGNORESDPVERSION	(1 << 19)	/*!< GDP: Ignore the SDP session version number we receive and treat all sessions as new */

#define GATEWAY_PAGE2_T38SUPPORT		        (3 << 20)	/*!< GDP: T.38 Fax Support */
#define GATEWAY_PAGE2_T38SUPPORT_UDPTL	        (1 << 20)	/*!< GDP: T.38 Fax Support (no error correction) */
#define GATEWAY_PAGE2_T38SUPPORT_UDPTL_FEC	        (2 << 20)	/*!< GDP: T.38 Fax Support (FEC error correction) */
#define GATEWAY_PAGE2_T38SUPPORT_UDPTL_REDUNDANCY	(3 << 20)	/*!< GDP: T.38 Fax Support (redundancy error correction) */

#define GATEWAY_PAGE2_CALL_ONHOLD		(3 << 23)	/*!< D: Call hold states: */
#define GATEWAY_PAGE2_CALL_ONHOLD_ACTIVE    (1 << 23)       /*!< D: Active hold */
#define GATEWAY_PAGE2_CALL_ONHOLD_ONEDIR	(2 << 23)	/*!< D: One directional hold */
#define GATEWAY_PAGE2_CALL_ONHOLD_INACTIVE	(3 << 23)	/*!< D: Inactive hold */

#define GATEWAY_PAGE2_RFC2833_COMPENSATE    (1 << 25)	/*!< DP: Compensate for buggy RFC2833 implementations */
#define GATEWAY_PAGE2_BUGGY_MWI		(1 << 26)	/*!< DP: Buggy CISCO MWI fix */
#define GATEWAY_PAGE2_DIALOG_ESTABLISHED    (1 << 27)       /*!< 29: Has a dialog been established? */
#define GATEWAY_PAGE2_FAX_DETECT		(1 << 28)		/*!< DP: Fax Detection support */
#define GATEWAY_PAGE2_REGISTERTRYING        (1 << 29)       /*!< DP: Send 100 Trying on REGISTER attempts */
#define GATEWAY_PAGE2_UDPTL_DESTINATION     (1 << 30)       /*!< DP: Use source IP of RTP as destination if NAT is enabled */
#define GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS	(1 << 31)       /*!< DP: Always set up video, even if endpoints don't support it */

#define GATEWAY_PAGE2_FLAGS_TO_COPY \
	(GATEWAY_PAGE2_ALLOWSUBSCRIBE | GATEWAY_PAGE2_ALLOWOVERLAP | GATEWAY_PAGE2_IGNORESDPVERSION | \
	GATEWAY_PAGE2_VIDEOSUPPORT | GATEWAY_PAGE2_T38SUPPORT | GATEWAY_PAGE2_RFC2833_COMPENSATE | \
	GATEWAY_PAGE2_BUGGY_MWI | GATEWAY_PAGE2_TEXTSUPPORT | GATEWAY_PAGE2_FAX_DETECT | \
	GATEWAY_PAGE2_UDPTL_DESTINATION | GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS | GATEWAY_PAGE2_CONSTANT_SSRC)
 ...
 * (which means that if gatewaydebug is set in the config file, we can
 * only turn it off by reloading the config).
 */
enum gateway_debug_e {
	gateway_debug_none = 0,
	gateway_debug_config = 1,
	gateway_debug_console = 2,
};

static enum gateway_debug_e gatewaydebug;

/*! \brief extra debugging for 'text' related events.
 * At the moment this is set together with gateway_debug_console.
 * \note It should either go away or be implemented properly.
 */
static int gatewaydebug_text;
...
/*! \brief Structure to handle GATEWAY transfers. Dynamically allocated when needed
	\note OEJ: Should be moved to string fields */
struct gateway_refer {
	...
	char replaces_callid[GATEWAYBUFSIZE];			/*!< Replace info: callid */
	char replaces_callid_totag[GATEWAYBUFSIZE/2];		/*!< Replace info: to-tag */
	char replaces_callid_fromtag[GATEWAYBUFSIZE/2];		/*!< Replace info: from-tag */
	struct gateway_pvt *refer_call;			/*!< Call we are referring. This is just a reference to a
							 * dialog owned by someone else, so we should not destroy
							 * it when the gateway_refer object goes.
							 */
 ...
 *   GATEWAY Session-Timers feature on a per dialog basis.
 */
struct gateway_st_dlg {
 ...
 *   of GATEWAY Session-Timers feature on a per user/peer basis.
 */
struct gateway_st_cfg {
...
/*! \brief Structure used for each GATEWAY dialog, ie. a call, a registration, a subscribe.
 * Created and initialized by gateway_alloc(), the descriptor goes into the list of
 * descriptors (dialoglist).
 */
struct gateway_pvt {
	struct gateway_pvt *next;			/*!< Next dialog in chain */
	enum invitestates invitestate;		/*!< Track state of GATEWAY_INVITEs */
	int method;				/*!< GATEWAY method that opened this dialog */
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(qop);		/*!< Quality of Protection, since GATEWAY wasn't complicated enough yet. */
		TRIS_STRING_FIELD(domain);	/*!< Authorization domain */
		TRIS_STRING_FIELD(from);		/*!< The From: header */
		TRIS_STRING_FIELD(useragent);	/*!< User agent in GATEWAY request */
	...
	struct gateway_socket socket;		/*!< The socket used for this dialog */
	unsigned int ocseq;			/*!< Current outgoing seqno */
	unsigned int icseq;			/*!< Current incoming seqno */
	tris_group_t callgroup;			/*!< Call group */
	tris_group_t pickupgroup;		/*!< Pickup group */
	int lastinvite;				/*!< Last Cseq of invite */
	struct tris_flags flags[2];		/*!< GATEWAY_ flags */
	...
	int timer_t1;				/*!< GATEWAY timer T1, ms rtt */
	int timer_b;                            /*!< GATEWAY timer B, ms */
	unsigned int gatewayoptions;		/*!< Supported GATEWAY options on the other end */
	unsigned int reqgatewayoptions;		/*!< Required GATEWAY options on the other end */
	...
	struct gateway_proxy *outboundproxy;	/*!< Outbound proxy for this dialog. Use ref_proxy to set this instead of setting it directly*/
	...
	struct gateway_route *route;		/*!< Head of linked list of routing steps (fm Record-Route) */
	int route_persistant;			/*!< Is this the "real" route? */
	struct tris_variable *notify_headers;    /*!< Custom notify type */
	struct gateway_auth *peerauth;		/*!< Realm authentication */
	...
	struct gateway_request initreq;		/*!< Latest request that opened a new transaction
							within this dialog.
	...
	struct gateway_refer *refer;		/*!< REFER: GATEWAY transfer data structure */
	...
	struct gateway_peer *relatedpeer;		/*!< If this dialog is related to a peer, which one 
							Used in peerpoke, mwi subscriptions */
	struct gateway_registry *registry;		/*!< If this is a REGISTER dialog, to which registry */
	struct tris_rtp *rtp;			/*!< RTP Session */
	struct tris_rtp *vrtp;			/*!< Video RTP session */
	struct tris_rtp *trtp;			/*!< Text RTP session */
	struct gateway_pkt *packets;		/*!< Packets scheduled for re-transmission */
	struct gateway_history_head *history;	/*!< History of this GATEWAY dialog */
	size_t history_entries;			/*!< Number of entires in the history */
	struct tris_variable *chanvars;		/*!< Channel variables to set for inbound call */
	TRIS_LIST_HEAD_NOLOCK(request_queue, gateway_request) request_queue; /*!< Requests that arrived but could not be processed immediately */
	...
	struct gateway_invite_param *options;	/*!< Options for INVITE */
	int autoframing;			/*!< The number of Asters we group in a Pyroflax
							...
							you know more) */
	struct gateway_st_dlg *stimer;		/*!< GATEWAY Session-Timers */              
  
	int red; 				/*!< T.140 RTP Redundancy */
	int hangupcause;			/*!< Storage of hangupcause copied from our owner before we disconnect from the AST channel (only used at hangup) */

	struct gateway_subscription_mwi *mwi;       /*!< If this is a subscription MWI dialog, to which subscription */
...
/*! \brief
 * Here we implement the container for dialogs (gateway_pvt), defining
...
#define gateway_pvt_lock(x) ao2_lock(x)
#define gateway_pvt_trylock(x) ao2_trylock(x)
#define gateway_pvt_unlock(x) ao2_unlock(x)
...
static struct gateway_pvt *dialog_ref_debug(struct gateway_pvt *p, char *tag, char *file, int line, const char *func)
{
	if (p)
...
static struct gateway_pvt *dialog_unref_debug(struct gateway_pvt *p, char *tag, char *file, int line, const char *func)
{
	if (p)
		_ao2_ref_debug(p, -1, tag, file, line, func);
	return NULL;
...
#else
static struct gateway_pvt *dialog_ref(struct gateway_pvt *p, char *tag)
{
	if (p)
		ao2_ref(p, 1);
	else
...
static struct gateway_pvt *dialog_unref(struct gateway_pvt *p, char *tag)
{
	if (p)
		ao2_ref(p, -1);
	return NULL;
...
/*! \brief gateway packet - raw format for outbound packets that are sent or scheduled for transmission
 * Packets are linked in a list, whose head is in the struct gateway_pvt they belong to.
 * Each packet holds a reference to the parent struct gateway_pvt.
 * This structure is allocated in __gateway_reliable_xmit() and only for packets that
 * require retransmissions.
 */
struct gateway_pkt {
	struct gateway_pkt *next;			/*!< Next packet in linked list */
	int retrans;				/*!< Retransmission number */
	int method;				/*!< GATEWAY method for this packet */
	int seqno;				/*!< Sequence number */
	...
	struct gateway_pvt *owner;			/*!< Owner AST call */
	int retransid;				/*!< Retransmission ID */
	int timer_a;				/*!< GATEWAY timer A, retransmission timer */
	int timer_t1;				/*!< GATEWAY Timer T1, estimated RTT or 500 ms */
 ...
 * too much effort ...
 */
struct gateway_mailbox {
	char *mailbox;
	char *context;
	/*! Associated MWI subscription */
	struct tris_event_sub *event_sub;
	TRIS_LIST_ENTRY(gateway_mailbox) entry;
};

enum gateway_peer_type {
	GATEWAY_TYPE_PEER = (1 << 0),
	GATEWAY_TYPE_USER = (1 << 1),
};

/*! \brief Structure for GATEWAY peer data, we place calls to peers if registered  or fixed IP address (host) 
*/
/* XXX field 'name' must be first otherwise gateway_addrcmp() will fail, as will astobj2 hashing of the structure */
struct gateway_peer {
	...
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(fullcontact);		/*!< Contact registered with us (not in gateway.conf) */
		...
		TRIS_STRING_FIELD(parkinglot);		/*!<  Parkinglot */
		TRIS_STRING_FIELD(useragent);		/*!<  User agent in GATEWAY request (saved from registration) */
		);
	struct gateway_socket socket;	/*!< Socket used for this peer */
	enum gateway_transport default_outbound_transport;    /*!< Peer Registration may change the default outbound transport.
												If register expires, default should be reset. to this value */
	unsigned int transports:3;      /*!< Transports (enum gateway_transport) that are acceptable for this peer */
	struct gateway_auth *auth;		/*!< Realm authentication list */
	...
	struct tris_codec_pref prefs;	/*!<  codec prefs */
	int lastmsgssent;
	unsigned int gatewayoptions;	/*!<  Supported GATEWAY options */
	struct tris_flags flags[2];	/*!<  GATEWAY_ flags */

	/*! Mailboxes that this peer cares about */
	TRIS_LIST_HEAD_NOLOCK(, gateway_mailbox) mailboxes;
	...
	tris_group_t callgroup;		/*!<  Call group */
	tris_group_t pickupgroup;	/*!<  Pickup group */
	struct gateway_proxy *outboundproxy;	/*!< Outbound proxy for this peer */
	...
	struct gateway_pvt *mwipvt;		/*!<  Subscription for MWI */
	int autoframing;
	struct gateway_st_cfg stimer;	/*!<  GATEWAY Session-Timers */
	...
	enum gateway_peer_type type; /*!< Distinguish between "user" and "peer" types. This is used solely for CLI and manager commands */
};


/*! 
 * \brief Registrations with other GATEWAY proxies
 *
 * Created by gateway_register(), the entry is linked in the 'regl' list,
 * and never deleted (other than at 'gateway reload' or module unload times).
 ...
 */
struct gateway_registry {
	ASTOBJ_COMPONENTS_FULL(struct gateway_registry,1,1);
	TRIS_DECLARE_STRING_FIELDS(
	...
	enum gateway_transport transport;	/*!< Transport for this registration UDP, TCP or TLS */
	...
	int timeout; 			/*!< sched id of gateway_reg_timeout */
	int refresh;			/*!< How often to refresh */
	struct gateway_pvt *call;		/*!< create a gateway_pvt structure for each outbound "registration dialog" in progress */
	enum gatewayregistrystate regstate;	/*!< Registration state (see above) */
...
struct gateway_threadinfo {
	...
	enum gateway_transport type;	/*!< We keep a copy of the type here so we can display it in the connection list */
	TRIS_LIST_HEAD_NOLOCK(, tcptls_packet) packet_q;
};

/*! \brief Definition of an MWI subscription to another server */
struct gateway_subscription_mwi {
	ASTOBJ_COMPONENTS_FULL(struct gateway_subscription_mwi,1,1);
	TRIS_DECLARE_STRING_FIELDS(
...
/*! \brief  The register list: Other GATEWAY proxies we register with and place calls to */
static struct tris_register_list {
	ASTOBJ_CONTAINER_COMPONENTS(struct gateway_registry);
	int recheck;
} regl;

/*! \brief  The MWI subscription list */
static struct tris_subscription_mwi_list {
	ASTOBJ_CONTAINER_COMPONENTS(struct gateway_subscription_mwi);
} submwil;
...
{
	const struct gateway_peer *peer = obj;
...
{
	struct gateway_peer *peer = obj, *peer2 = arg;
...
static int peer_ipcmp_cb(void *obj, void *arg, int flags)
{
	...
	if ((peer->transports & peer2->transports) & (GATEWAY_TRANSPORT_TLS | GATEWAY_TRANSPORT_TCP)) {
		/* peer matching on port is not possible with TCP/TLS */
		return CMP_MATCH | CMP_STOP;
	} else if (tris_test_flag(&peer2->flags[0], GATEWAY_INSECURE_PORT)) {
		/* We are allowing match without port for peers configured that
		 * way in this pass through the peers. */
		return tris_test_flag(&peer->flags[0], GATEWAY_INSECURE_PORT) ?
...
{
	const struct gateway_threadinfo *th = obj;
...
{
	struct gateway_threadinfo *th = obj, *th2 = arg;
...
{
	const struct gateway_pvt *pvt = obj;
...
{
	struct gateway_pvt *pvt = obj, *pvt2 = arg;
 ...
 * \todo Move the gateway_auth list to TRIS_LIST */
static struct gateway_auth *authl = NULL;


/* --- Sockets and networking --------------*/

/*! \brief Main socket for UDP GATEWAY communication.
 *
 * gatewaysock is shared between the GATEWAY manager thread (which handles reload
 * requests), the udp io handler (gatewaysock_read()) and the user routines that
 * issue udp writes (using __gateway_xmit()).
 ...
 * default address/port in GATEWAY messages, and as the default address
 * (but not port) in SDP messages.
 */
static struct sockaddr_in internip;

/*! \brief our external IP address/port for GATEWAY sessions.
...
/*---------------------------- Forward declarations of functions in chan_gateway.c */
/* Note: This is added to help splitting up chan_gateway.c into several files
	in coming releases. */

/*--- PBX interface functions */
static struct tris_channel *gateway_request_call(const char *type, int format, void *data, int *cause, struct tris_channel* src);
static int gateway_devicestate(void *data);
static int gateway_sendtext(struct tris_channel *ast, const char *text);
static int gateway_call(struct tris_channel *ast, char *dest, int timeout);
static int gateway_sendhtml(struct tris_channel *chan, int subclass, const char *data, int datalen);
static int gateway_hangup(struct tris_channel *ast);
static int gateway_answer(struct tris_channel *ast);
static struct tris_frame *gateway_read(struct tris_channel *ast);
static int gateway_write(struct tris_channel *ast, struct tris_frame *frame);
static int gateway_indicate(struct tris_channel *ast, int condition, const void *data, size_t datalen);
static int gateway_transfer(struct tris_channel *ast, const char *dest);
static int gateway_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
static int gateway_senddigit_begin(struct tris_channel *ast, char digit);
static int gateway_senddigit_end(struct tris_channel *ast, char digit, unsigned int duration);
static int gateway_queryoption(struct tris_channel *chan, int option, void *data, int *datalen);
static const char *gateway_get_callid(struct tris_channel *chan);

static int handle_request_do(struct gateway_request *req, struct sockaddr_in *sin);
static int gateway_standard_port(enum gateway_transport type, int port);
static int gateway_prepare_socket(struct gateway_pvt *p);
static int gateway_parse_host(char *line, int lineno, char **hostname, int *portnum, enum gateway_transport *transport);

/*--- Transmitting responses and requests */
static int gatewaysock_read(int *id, int fd, short events, void *ignore);
static int __gateway_xmit(struct gateway_pvt *p, struct tris_str *data, int len);
static int __gateway_reliable_xmit(struct gateway_pvt *p, int seqno, int resp, struct tris_str *data, int len, int fatal, int gatewaymethod);
static int __transmit_response(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, enum xmittype reliable);
static int retrans_pkt(const void *data);
static int transmit_response_using_temp(tris_string_field callid, struct sockaddr_in *sin, int useglobal_nat, const int intended_method, const struct gateway_request *req, const char *msg);
static int transmit_response(struct gateway_pvt *p, const char *msg, const struct gateway_request *req);
static int transmit_response_reliable(struct gateway_pvt *p, const char *msg, const struct gateway_request *req);
static int transmit_response_with_date(struct gateway_pvt *p, const char *msg, const struct gateway_request *req);
static int transmit_response_with_sdp(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, enum xmittype reliable, int oldsdp);
static int transmit_response_with_unsupported(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, const char *unsupported);
static int transmit_response_with_auth(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, const char *rand, enum xmittype reliable, const char *header, int stale);
static int transmit_provisional_response(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, int with_sdp);
static int transmit_response_with_allow(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, enum xmittype reliable);
static void transmit_fake_auth_response(struct gateway_pvt *p, int gatewaymethod, struct gateway_request *req, enum xmittype reliable);
static int transmit_request(struct gateway_pvt *p, int gatewaymethod, int inc, enum xmittype reliable, int newbranch);
static int transmit_request_with_auth(struct gateway_pvt *p, int gatewaymethod, int seqno, enum xmittype reliable, int newbranch);
static int transmit_invite(struct gateway_pvt *p, int gatewaymethod, int sdp, int init);
static int transmit_reinvite_with_sdp(struct gateway_pvt *p, int t38version, int oldsdp);
static int transmit_info_with_digit(struct gateway_pvt *p, const char digit, unsigned int duration);
static int transmit_info_with_vidupdate(struct gateway_pvt *p);
static int transmit_message_with_text(struct gateway_pvt *p, const char *text);
static int transmit_refer(struct gateway_pvt *p, const char *dest);
static int transmit_notify_with_mwi(struct gateway_pvt *p, int newmsgs, int oldmsgs, const char *vmexten);
static int transmit_notify_with_gatewayfrag(struct gateway_pvt *p, int cseq, char *message, int terminate);
static int transmit_notify_custom(struct gateway_pvt *p, struct tris_variable *vars);
static int transmit_register(struct gateway_registry *r, int gatewaymethod, const char *auth, const char *authheader);
static int send_response(struct gateway_pvt *p, struct gateway_request *req, enum xmittype reliable, int seqno);
static int send_request(struct gateway_pvt *p, struct gateway_request *req, enum xmittype reliable, int seqno);
static void copy_request(struct gateway_request *dst, const struct gateway_request *src);
static void receive_message(struct gateway_pvt *p, struct gateway_request *req);
static void parse_moved_contact(struct gateway_pvt *p, struct gateway_request *req);
static int gateway_send_mwi_to_peer(struct gateway_peer *peer, const struct tris_event *event, int cache_only);

/*--- Dialog management */
static struct gateway_pvt *gateway_alloc(tris_string_field callid, struct sockaddr_in *sin,
				 int useglobal_nat, const int intended_method, struct gateway_request *req);
static int __gateway_autodestruct(const void *data);
static void gateway_scheddestroy(struct gateway_pvt *p, int ms);
static int gateway_cancel_destroy(struct gateway_pvt *p);
static struct gateway_pvt *gateway_destroy(struct gateway_pvt *p);
static void *dialog_unlink_all(struct gateway_pvt *dialog, int lockowner, int lockdialoglist);
static void *registry_unref(struct gateway_registry *reg, char *tag);
static void __gateway_destroy(struct gateway_pvt *p, int lockowner, int lockdialoglist);
static int __gateway_ack(struct gateway_pvt *p, int seqno, int resp, int gatewaymethod);
static void __gateway_pretend_ack(struct gateway_pvt *p);
static int __gateway_semi_ack(struct gateway_pvt *p, int seqno, int resp, int gatewaymethod);
static int auto_congest(const void *arg);
static int update_call_counter(struct gateway_pvt *fup, int event);
static int hangup_gateway2cause(int cause);
static const char *hangup_cause2gateway(int cause);
static struct gateway_pvt *find_call(struct gateway_request *req, struct sockaddr_in *sin, const int intended_method);
static void free_old_route(struct gateway_route *route);
static void list_route(struct gateway_route *route);
static void build_route(struct gateway_pvt *p, struct gateway_request *req, int backwards);
static enum check_auth_result register_verify(struct gateway_pvt *p, struct sockaddr_in *sin,
					      struct gateway_request *req, char *uri);
static struct gateway_pvt *get_gateway_pvt_byid_locked(const char *callid, const char *totag, const char *fromtag);
static void check_pendings(struct gateway_pvt *p);
static void *gateway_park_thread(void *stuff);
static int gateway_park(struct tris_channel *chan1, struct tris_channel *chan2, struct gateway_request *req, int seqno);
static int gateway_gatewayredirect(struct gateway_pvt *p, const char *dest);

/*--- Codec handling / SDP */
static void try_suggested_gateway_codec(struct gateway_pvt *p);
static const char *get_sdp_iterate(int* start, struct gateway_request *req, const char *name);
static char get_sdp_line(int *start, int stop, struct gateway_request *req, const char **value);
static int find_sdp(struct gateway_request *req);
static int process_sdp(struct gateway_pvt *p, struct gateway_request *req, int t38action);
static int process_sdp_o(const char *o, struct gateway_pvt *p);
static int process_sdp_c(const char *c, struct tris_hostent *hp);
static int process_sdp_a_sendonly(const char *a, int *sendonly);
static int process_sdp_a_audio(const char *a, struct gateway_pvt *p, struct tris_rtp *newaudiortp, int *last_rtpmap_codec);
static int process_sdp_a_video(const char *a, struct gateway_pvt *p, struct tris_rtp *newvideortp, int *last_rtpmap_codec);
static int process_sdp_a_text(const char *a, struct gateway_pvt *p, struct tris_rtp *newtextrtp, char *red_fmtp, int *red_num_gen, int *red_data_pt, int *last_rtpmap_codec);
static int process_sdp_a_image(const char *a, struct gateway_pvt *p);
static void add_codec_to_sdp(const struct gateway_pvt *p, int codec,
			     struct tris_str **m_buf, struct tris_str **a_buf,
			     int debug, int *min_packet_size);
static void add_noncodec_to_sdp(const struct gateway_pvt *p, int format,
				struct tris_str **m_buf, struct tris_str **a_buf,
				int debug);
static enum gateway_result add_sdp(struct gateway_request *resp, struct gateway_pvt *p, int oldsdp, int add_audio, int add_t38);
static void do_setnat(struct gateway_pvt *p, int natflags);
static void stop_media_flows(struct gateway_pvt *p);

/*--- Authentication stuff */
static int reply_digest(struct gateway_pvt *p, struct gateway_request *req, char *header, int gatewaymethod, char *digest, int digest_len);
static int build_reply_digest(struct gateway_pvt *p, int method, char *digest, int digest_len);
static enum check_auth_result check_auth(struct gateway_pvt *p, struct gateway_request *req, const char *username,
					 const char *secret, const char *md5secret, int gatewaymethod,
					 char *uri, enum xmittype reliable, int ignore);
static enum check_auth_result check_user_full(struct gateway_pvt *p, struct gateway_request *req,
					      int gatewaymethod, char *uri, enum xmittype reliable,
					      struct sockaddr_in *sin, struct gateway_peer **authpeer);
static int check_user(struct gateway_pvt *p, struct gateway_request *req, int gatewaymethod, char *uri, enum xmittype reliable, struct sockaddr_in *sin);

/*--- Domain handling */
static int check_gateway_domain(const char *domain, char *context, size_t len); /* Check if domain is one of our local domains */
static int add_gateway_domain(const char *domain, const enum domain_mode mode, const char *context);
static void clear_gateway_domains(void);

/*--- GATEWAY realm authentication */
static struct gateway_auth *add_realm_authentication(struct gateway_auth *authlist, const char *configuration, int lineno);
static int clear_realm_authentication(struct gateway_auth *authlist);	/* Clear realm authentication list (at reload) */
static struct gateway_auth *find_realm_authentication(struct gateway_auth *authlist, const char *realm);

/*--- Misc functions */
static void check_rtp_timeout(struct gateway_pvt *dialog, time_t t);
static int gateway_do_reload(enum channelreloadreason reason);
static int reload_config(enum channelreloadreason reason);
static int expire_register(const void *data);
static void *do_monitor(void *data);
static int restart_monitor(void);
static void peer_mailboxes_to_str(struct tris_str **mailbox_str, struct gateway_peer *peer);
static struct tris_variable *copy_vars(struct tris_variable *src);
/* static int gateway_addrcmp(char *name, struct sockaddr_in *sin);	Support for peer matching */
static int gateway_refer_allocate(struct gateway_pvt *p);
static void tris_quiet_chan(struct tris_channel *chan);
static int attempt_transfer(struct gateway_dual *transferer, struct gateway_dual *target);
 ...
 * gateway_request or a gateway_peer 
 */
#define check_request_transport(peer, tmpl) ({ \
	...
	if (peer->socket.type == tmpl->socket.type) \
		...
		ret = 1; \
	} else if (peer->socket.type & GATEWAY_TRANSPORT_TLS) { \
		tris_log(LOG_WARNING, \
			"peer '%s' HAS NOT USED (OR SWITCHED TO) TLS in favor of '%s' (but this was allowed in gateway.conf)!\n", \
...
static int gateway_devicestate(void *data);
static int gateway_poke_noanswer(const void *data);
static int gateway_poke_peer(struct gateway_peer *peer, int force);
static void gateway_poke_all_peers(void);
static void gateway_peer_hold(struct gateway_pvt *p, int hold);
static void mwi_event_cb(const struct tris_event *, void *);

/*--- Applications, functions, CLI and manager command helpers */
static const char *gateway_nat_mode(const struct gateway_pvt *p);
static char *gateway_show_inuse(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *transfermode2str(enum transfermodes mode) attribute_const;
static const char *nat2str(int nat) attribute_const;
static int peer_status(struct gateway_peer *peer, char *status, int statuslen);
static char *gateway_show_sched(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char * _gateway_show_peers(int fd, int *total, struct mansession *s, const struct message *m, int argc, const char *argv[]);
static char *gateway_show_peers(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_show_objects(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
static char *gateway_show_domains(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *_gateway_show_peer(int type, int fd, struct mansession *s, const struct message *m, int argc, const char *argv[]);
static char *gateway_show_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *_gateway_qualify_peer(int type, int fd, struct mansession *s, const struct message *m, int argc, const char *argv[]);
static char *gateway_qualify_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_show_registry(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_unregister(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_show_settings(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_show_mwi(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static const char *subscription_type2str(enum subscriptiontype subtype) attribute_pure;
static const struct cfsubscription_types *find_subscription_type(enum subscriptiontype subtype);
static char *complete_gateway_peer(const char *word, int state, int flags2);
static char *complete_gateway_registered_peer(const char *word, int state, int flags2);
static char *complete_gateway_show_history(const char *line, const char *word, int pos, int state);
static char *complete_gateway_show_peer(const char *line, const char *word, int pos, int state);
static char *complete_gateway_unregister(const char *line, const char *word, int pos, int state);
static char *complete_gatewaynotify(const char *line, const char *word, int pos, int state);
static char *gateway_show_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_show_channelstats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_show_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_do_debug_ip(int fd, char *arg);
static char *gateway_do_debug_peer(int fd, char *arg);
static char *gateway_do_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_cli_notify(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *gateway_set_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static int gateway_dtmfmode(struct tris_channel *chan, void *data);
static int gateway_addheader(struct tris_channel *chan, void *data);
static int gateway_do_reload(enum channelreloadreason reason);
static char *gateway_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
/*--- Debugging 
	...
	a GATEWAY dialog
*/
static void gateway_dump_history(struct gateway_pvt *dialog);	/* Dump history to debuglog at end of dialog, before destroying data */
static inline int gateway_debug_test_addr(const struct sockaddr_in *addr);
static inline int gateway_debug_test_pvt(struct gateway_pvt *p);


/*! \brief Append to GATEWAY dialog history 
	\return Always returns 0 */
#define append_history(p, event, fmt , args... )	append_history_full(p, "%-15s " fmt, event, ## args)
static void append_history_full(struct gateway_pvt *p, const char *fmt, ...);
static void gateway_dump_history(struct gateway_pvt *dialog);

/*--- Device object handling */
static struct gateway_peer *temp_peer(const char *name);
static struct gateway_peer *build_peer(const char *name, struct tris_variable *v, struct tris_variable *alt, int realtime, int devstate_only);
static int update_call_counter(struct gateway_pvt *fup, int event);
static void gateway_destroy_peer(struct gateway_peer *peer);
static void gateway_destroy_peer_fn(void *peer);
static void set_peer_defaults(struct gateway_peer *peer);
static struct gateway_peer *temp_peer(const char *name);
static void register_peer_exten(struct gateway_peer *peer, int onoff);
static struct gateway_peer *find_peer(const char *peer, struct sockaddr_in *sin, int realtime, int forcenamematch, int devstate_only, int transport);
static int gateway_poke_peer_s(const void *data);
static enum parse_register_result parse_register_contact(struct gateway_pvt *pvt, struct gateway_peer *p, struct gateway_request *req);
static void reg_source_db(struct gateway_peer *peer);
static void destroy_association(struct gateway_peer *peer);
...
static void set_socket_transport(struct gateway_socket *socket, int transport);
...
static void update_peer(struct gateway_peer *p, int expire);
...
static struct gateway_peer *realtime_peer(const char *peername, struct sockaddr_in *sin, int devstate_only);
static char *gateway_prune_realtime(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);

/*--- Internal UA client handling (outbound registrations) */
static void tris_gateway_ouraddrfor(struct in_addr *them, struct sockaddr_in *us, struct gateway_pvt *p);
static void gateway_registry_destroy(struct gateway_registry *reg);
static int gateway_register(const char *value, int lineno);
static const char *regstate2str(enum gatewayregistrystate regstate) attribute_const;
static int gateway_reregister(const void *data);
static int __gateway_do_register(struct gateway_registry *r);
static int gateway_reg_timeout(const void *data);
static void gateway_send_all_registers(void);
static int gateway_reinvite_retry(const void *data);

/*--- Parsing GATEWAY requests and responses */
static void append_date(struct gateway_request *req);	/* Append date to GATEWAY packet */
static int determine_firstline_parts(struct gateway_request *req);
static const struct cfsubscription_types *find_subscription_type(enum subscriptiontype subtype);
static const char *gettag(const struct gateway_request *req, const char *header, char *tagbuf, int tagbufsize);
static int find_gateway_method(const char *msg);
static unsigned int parse_gateway_options(struct gateway_pvt *pvt, const char *supported);
static int parse_request(struct gateway_request *req);
static const char *get_header(const struct gateway_request *req, const char *name);
static const char *referstatus2str(enum referstatus rstatus) attribute_pure;
static int method_match(enum gatewaymethod id, const char *name);
static void parse_copy(struct gateway_request *dst, const struct gateway_request *src);
static char *get_in_brackets(char *tmp);
static const char *find_alias(const char *name, const char *_default);
static const char *__get_header(const struct gateway_request *req, const char *name, int *start);
static int lws2sws(char *msgbuf, int len);
static void extract_uri(struct gateway_pvt *p, struct gateway_request *req);
static char *remove_uri_parameters(char *uri);
static int get_refer_info(struct gateway_pvt *transferer, struct gateway_request *outgoing_req);
static int get_also_info(struct gateway_pvt *p, struct gateway_request *oreq);
static int parse_ok_contact(struct gateway_pvt *pvt, struct gateway_request *req);
static int set_address_from_contact(struct gateway_pvt *pvt);
static void check_via(struct gateway_pvt *p, struct gateway_request *req);
static char *get_calleridname(const char *input, char *output, size_t outputsize);
static int get_rpid_num(const char *input, char *output, int maxlen);
static int get_rdnis(struct gateway_pvt *p, struct gateway_request *oreq);
static int get_destination(struct gateway_pvt *p, struct gateway_request *oreq);
static int get_msg_text(char *buf, int len, struct gateway_request *req, int addnewline);
static int transmit_state_notify(struct gateway_pvt *p, int state, int full, int timeout);

/*-- TCP connection handling ---*/
static void *_gateway_tcp_helper_thread(struct gateway_pvt *pvt, struct tris_tcptls_session_instance *tcptls_session);
static void *gateway_tcp_worker_fn(void *);

/*--- Constructing requests and responses */
static void initialize_initreq(struct gateway_pvt *p, struct gateway_request *req);
static int init_req(struct gateway_request *req, int gatewaymethod, const char *recip);
static int reqprep(struct gateway_request *req, struct gateway_pvt *p, int gatewaymethod, int seqno, int newbranch);
static void initreqprep(struct gateway_request *req, struct gateway_pvt *p, int gatewaymethod);
static int init_resp(struct gateway_request *resp, const char *msg);
static inline int resp_needs_contact(const char *msg, enum gatewaymethod method);
static int respprep(struct gateway_request *resp, struct gateway_pvt *p, const char *msg, const struct gateway_request *req);
static const struct sockaddr_in *gateway_real_dst(const struct gateway_pvt *p);
static void build_via(struct gateway_pvt *p);
static int create_addr_from_peer(struct gateway_pvt *r, struct gateway_peer *peer);
static int create_addr(struct gateway_pvt *dialog, const char *opeer, struct sockaddr_in *sin, int newdialog);
static char *generate_random_string(char *buf, size_t size);
static void build_callid_pvt(struct gateway_pvt *pvt);
static void build_callid_registry(struct gateway_registry *reg, struct in_addr ourip, const char *fromdomain);
static void make_our_tag(char *tagbuf, size_t len);
static int add_header(struct gateway_request *req, const char *var, const char *value);
static int add_header_contentLength(struct gateway_request *req, int len);
static int add_line(struct gateway_request *req, const char *line);
static int add_text(struct gateway_request *req, const char *text);
static int add_digit(struct gateway_request *req, char digit, unsigned int duration, int mode);
static int add_vidupdate(struct gateway_request *req);
static void add_route(struct gateway_request *req, struct gateway_route *route);
static int copy_header(struct gateway_request *req, const struct gateway_request *orig, const char *field);
static int copy_all_header(struct gateway_request *req, const struct gateway_request *orig, const char *field);
static int copy_via_headers(struct gateway_pvt *p, struct gateway_request *req, const struct gateway_request *orig, const char *field);
static void set_destination(struct gateway_pvt *p, char *uri);
static void append_date(struct gateway_request *req);
static void build_contact(struct gateway_pvt *p);
static void build_rpid(struct gateway_pvt *p);

/*------Request handling functions */
static int handle_incoming(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, int *recount, int *nounlock);
static int handle_request_invite(struct gateway_pvt *p, struct gateway_request *req, int debug, int seqno, struct sockaddr_in *sin, int *recount, char *e, int *nounlock);
static int handle_request_refer(struct gateway_pvt *p, struct gateway_request *req, int debug, int seqno, int *nounlock);
static int handle_request_bye(struct gateway_pvt *p, struct gateway_request *req);
static int handle_request_register(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, char *e);
static int handle_request_cancel(struct gateway_pvt *p, struct gateway_request *req);
static int handle_request_message(struct gateway_pvt *p, struct gateway_request *req);
static int handle_request_subscribe(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, int seqno, char *e);
static void handle_request_info(struct gateway_pvt *p, struct gateway_request *req);
static int handle_request_options(struct gateway_pvt *p, struct gateway_request *req);
static int handle_invite_replaces(struct gateway_pvt *p, struct gateway_request *req, int debug, int seqno, struct sockaddr_in *sin, int *nounlock);
static int handle_request_notify(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, int seqno, char *e);
static int local_attended_transfer(struct gateway_pvt *transferer, struct gateway_dual *current, struct gateway_request *req, int seqno);

/*------Response handling functions */
static void handle_response_invite(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno);
static void handle_response_notify(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno);
static void handle_response_refer(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno);
static void handle_response_subscribe(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno);
static int handle_response_register(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno);
static void handle_response(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno);

/*----- RTP interface functions */
static int gateway_set_rtp_peer(struct tris_channel *chan, struct tris_rtp *rtp, struct tris_rtp *vrtp,  struct tris_rtp *trtp, int codecs, int nat_active);
static enum tris_rtp_get_result gateway_get_rtp_peer(struct tris_channel *chan, struct tris_rtp **rtp);
static enum tris_rtp_get_result gateway_get_vrtp_peer(struct tris_channel *chan, struct tris_rtp **rtp);
static enum tris_rtp_get_result gateway_get_trtp_peer(struct tris_channel *chan, struct tris_rtp **rtp);
static int gateway_get_codec(struct tris_channel *chan);
static struct tris_frame *gateway_rtp_read(struct tris_channel *ast, struct gateway_pvt *p, int *faxdetect);

/*------ T38 Support --------- */
static int transmit_response_with_t38_sdp(struct gateway_pvt *p, char *msg, struct gateway_request *req, int retrans);
static struct tris_udptl *gateway_get_udptl_peer(struct tris_channel *chan);
static int gateway_set_udptl_peer(struct tris_channel *chan, struct tris_udptl *udptl);
static void change_t38_state(struct gateway_pvt *p, int state);

/*------ Session-Timers functions --------- */
static void proc_422_rsp(struct gateway_pvt *p, struct gateway_request *rsp);
static int  proc_session_timer(const void *vp);
static void stop_session_timer(struct gateway_pvt *p);
static void start_session_timer(struct gateway_pvt *p);
static void restart_session_timer(struct gateway_pvt *p);
...
static int st_get_se(struct gateway_pvt *, int max);
static enum st_refresher st_get_refresher(struct gateway_pvt *);
static enum st_mode st_get_mode(struct gateway_pvt *);
static struct gateway_st_dlg* gateway_st_alloc(struct gateway_pvt *const p);

/*!--- GATEWAY MWI Subscription support */
static int gateway_subscribe_mwi(const char *value, int lineno);
static void gateway_subscribe_mwi_destroy(struct gateway_subscription_mwi *mwi);
static void gateway_send_all_mwi_subscriptions(void);
static int gateway_subscribe_mwi_do(const void *data);
static int __gateway_subscribe_mwi_do(struct gateway_subscription_mwi *mwi);

/*! \brief Definition of this channel for PBX channel registration */
static const struct tris_channel_tech gateway_tech = {
	.type = "GATEWAY",
	.description = "Session Initiation Protocol (GATEWAY)",
	.capabilities = TRIS_FORMAT_AUDIO_MASK,	/* all audio formats */
	.properties = TRIS_CHAN_TP_WANTSJITTER | TRIS_CHAN_TP_CREATESJITTER,
	.requester = gateway_request_call,			/* called with chan unlocked */
	.devicestate = gateway_devicestate,			/* called with chan unlocked (not chan-specific) */
	.call = gateway_call,			/* called with chan locked */
	.send_html = gateway_sendhtml,
	.hangup = gateway_hangup,			/* called with chan locked */
	.answer = gateway_answer,			/* called with chan locked */
	.read = gateway_read,			/* called with chan locked */
	.write = gateway_write,			/* called with chan locked */
	.write_video = gateway_write,		/* called with chan locked */
	.write_text = gateway_write,
	.indicate = gateway_indicate,		/* called with chan locked */
	.transfer = gateway_transfer,		/* called with chan locked */
	.fixup = gateway_fixup,			/* called with chan locked */
	.send_digit_begin = gateway_senddigit_begin,	/* called with chan unlocked */
	.send_digit_end = gateway_senddigit_end,
	.bridge = tris_rtp_bridge,			/* XXX chan unlocked ? */
	.early_bridge = tris_rtp_early_bridge,
	.send_text = gateway_sendtext,		/* called with chan locked */
	.func_channel_read = acf_channel_read,
	.queryoption = gateway_queryoption,
	.get_pvt_uniqueid = gateway_get_callid,
};

/*! \brief This version of the gateway channel tech has no send_digit_begin
 ...
 * and is for use with channels using GATEWAY INFO DTMF.
 */
static struct tris_channel_tech gateway_tech_info;


/*! \brief Working TLS connection configuration */
static struct tris_tls_config gateway_tls_cfg;

/*! \brief Default TLS connection configuration */
static struct tris_tls_config default_tls_cfg;

/*! \brief The TCP server definition */
static struct tris_tcptls_session_args gateway_tcp_desc = {
	.accept_fd = -1,
	.master = TRIS_PTHREADT_NULL,
	.tls_cfg = NULL,
	.poll_timeout = -1,
	.name = "GATEWAY TCP server",
	.accept_fn = tris_tcptls_server_root,
	.worker_fn = gateway_tcp_worker_fn,
};

/*! \brief The TCP/TLS server definition */
static struct tris_tcptls_session_args gateway_tls_desc = {
	.accept_fd = -1,
	.master = TRIS_PTHREADT_NULL,
	.tls_cfg = &gateway_tls_cfg,
	.poll_timeout = -1,
	.name = "GATEWAY TLS server",
	.accept_fn = tris_tcptls_server_root,
	.worker_fn = gateway_tcp_worker_fn,
};

/* wrapper macro to tell whether t points to one of the gateway_tech descriptors */
#define IS_GATEWAY_TECH(t)  ((t) == &gateway_tech || (t) == &gateway_tech_info)
...
static struct tris_rtp_protocol gateway_rtp = {
	.type = "GATEWAY",
	.get_rtp_info = gateway_get_rtp_peer,
	.get_vrtp_info = gateway_get_vrtp_peer,
	.get_trtp_info = gateway_get_trtp_peer,
	.set_rtp_peer = gateway_set_rtp_peer,
	.get_codec = gateway_get_codec,
};
...
static void gateway_tcptls_client_args_destructor(void *obj)
{
...
static void gateway_threadinfo_destructor(void *obj)
{
	struct gateway_threadinfo *th = obj;
	...
	if (th->tcptls_session) {
		ao2_t_ref(th->tcptls_session, -1, "remove tcptls_session for gateway_threadinfo object");
	}
}

/*! \brief creates a gateway_threadinfo object and links it into the threadt table. */
static struct gateway_threadinfo *gateway_threadinfo_create(struct tris_tcptls_session_instance *tcptls_session, int transport)
{
	struct gateway_threadinfo *th;

	if (!tcptls_session || !(th = ao2_alloc(sizeof(*th), gateway_threadinfo_destructor))) {
		return NULL;
	}
	...
	if (pipe(th->alert_pipe) == -1) {
		ao2_t_ref(th, -1, "Failed to open alert pipe on gateway_threadinfo");
		tris_log(LOG_ERROR, "Could not create gateway alert pipe in tcptls thread, error %s\n", strerror(errno));
		return NULL;
	}
	ao2_t_ref(tcptls_session, +1, "tcptls_session ref for gateway_threadinfo object");
	th->tcptls_session = tcptls_session;
	th->type = transport ? transport : (tcptls_session->ssl ? GATEWAY_TRANSPORT_TLS: GATEWAY_TRANSPORT_TCP);
...
static int gateway_tcptls_write(struct tris_tcptls_session_instance *tcptls_session, const void *buf, size_t len)
{
	int res = len;
	struct gateway_threadinfo *th = NULL;
	struct tcptls_packet *packet = NULL;
	struct gateway_threadinfo tmp = {
		.tcptls_session = tcptls_session,
	};
	enum gateway_tcptls_alert alert = TCPTLS_ALERT_DATA;
	...
	if ((tcptls_session->fd == -1) ||
		!(th = ao2_t_find(threadt, &tmp, OBJ_POINTER, "ao2_find, getting gateway_threadinfo in tcp helper thread")) ||
	...
	ao2_t_ref(th, -1, "In gateway_tcptls_write, unref threadinfo object after finding it");
	return res;

tcptls_write_setup_error:
	if (th) {
		ao2_t_ref(th, -1, "In gateway_tcptls_write, unref threadinfo obj, could not create packet");
	}
	if (packet) {
...
/*! \brief GATEWAY TCP connection handler */
static void *gateway_tcp_worker_fn(void *data)
{
	struct tris_tcptls_session_instance *tcptls_session = data;

	return _gateway_tcp_helper_thread(NULL, tcptls_session);
}

/*! \brief GATEWAY TCP thread management function 
	This function reads from the socket, parses the packet into a request
*/
static void *_gateway_tcp_helper_thread(struct gateway_pvt *pvt, struct tris_tcptls_session_instance *tcptls_session) 
{
	int res, cl;
	struct gateway_request req = { 0, } , reqcpy = { 0, };
	struct gateway_threadinfo *me = NULL;
	char buf[1024] = "";
	...
	if (!tcptls_session->client) {
		if (!(me = gateway_threadinfo_create(tcptls_session, tcptls_session->ssl ? GATEWAY_TRANSPORT_TLS : GATEWAY_TRANSPORT_TCP))) {
			goto cleanup;
		}
		...
		if ((!(ca = tcptls_session->parent)) ||
			(!(me = ao2_t_find(threadt, &tmp, OBJ_POINTER, "ao2_find, getting gateway_threadinfo in tcp helper thread"))) ||
	...
	if (!(req.data = tris_str_create(GATEWAY_MIN_PACKET)))
		goto cleanup;
	if (!(reqcpy.data = tris_str_create(GATEWAY_MIN_PACKET)))
		goto cleanup;
	...
	for (;;) {
		...
		if (res < 0) {
			tris_debug(2, "GATEWAY %s server :: tris_wait_for_input returned %d\n", tcptls_session->ssl ? "SSL": "TCP", res);
			goto cleanup;
		}
		...
		if (fds[0].revents) { /* there is data on the socket to be read */
			...
			if (tcptls_session->ssl) {
				set_socket_transport(&req.socket, GATEWAY_TRANSPORT_TLS);
				req.socket.port = htons(ourport_tls);
			} else {
				set_socket_transport(&req.socket, GATEWAY_TRANSPORT_TCP);
		...
		if (fds[1].revents) { /* alert_pipe indicates there is data in the send queue to be sent */
			enum gateway_tcptls_alert alert;
			struct tcptls_packet *packet;
 ...
 */
static void *unref_peer(struct gateway_peer *peer, char *tag)
{
	ao2_t_ref(peer, -1, tag);
	return NULL;
}

static struct gateway_peer *ref_peer(struct gateway_peer *peer, char *tag)
{
	ao2_t_ref(peer, 1, tag);
	return peer;
}

/*! \brief maintain proper refcounts for a gateway_pvt's outboundproxy
 *
 ...
 *
 * \param pvt The gateway_pvt for which we wish to set the outboundproxy
 * \param proxy The gateway_proxy which we will point pvt towards.
 * \return Returns void
 */
static void ref_proxy(struct gateway_pvt *pvt, struct gateway_proxy *proxy)
{
	struct gateway_proxy *old_obproxy = pvt->outboundproxy;
	/* The gateway_cfg.outboundproxy is statically allocated, and so
	 * we don't ever need to adjust refcounts for it
	 */
	if (proxy && proxy != &gateway_cfg.outboundproxy) {
		ao2_ref(proxy, +1);
	}
	pvt->outboundproxy = proxy;
	if (old_obproxy && old_obproxy != &gateway_cfg.outboundproxy) {
...
static void *dialog_unlink_all(struct gateway_pvt *dialog, int lockowner, int lockdialoglist)
{
	...
	while((cp = dialog->packets)) {
		...
		dialog_unref(cp->owner, "remove all current packets in this dialog, and the pointer to the dialog too as part of __gateway_destroy");
		if (cp->data) {
			tris_free(cp->data);
		}
		tris_free(cp);
...
static void *registry_unref(struct gateway_registry *reg, char *tag)
{
	tris_debug(3, "GATEWAY Registry %s: refcount now %d\n", reg->hostname, reg->refcount - 1);
	ASTOBJ_UNREF(reg, gateway_registry_destroy);
	return NULL;
}

/*! \brief Add object reference to GATEWAY registry */
static struct gateway_registry *registry_addref(struct gateway_registry *reg, char *tag)
{
	tris_debug(3, "GATEWAY Registry %s: refcount now %d\n", reg->hostname, reg->refcount + 1);
...
static struct tris_udptl_protocol gateway_udptl = {
	type: "GATEWAY",
	get_udptl_info: gateway_get_udptl_peer,
	set_udptl_peer: gateway_set_udptl_peer,
};
...
static inline void pvt_set_needdestroy(struct gateway_pvt *pvt, const char *reason)
{
...
static void initialize_initreq(struct gateway_pvt *p, struct gateway_request *req)
{
	if (p->initreq.headers)
		tris_debug(1, "Initializing already initialized GATEWAY dialog %s (presumably reinvite)\n", p->callid);
	else
		tris_debug(1, "Initializing initreq for method %s - callid %s\n", gateway_methods[req->method].text, p->callid);
...
/*! \brief Encapsulate setting of GATEWAY_ALREADYGONE to be able to trace it with debugging */
static void gateway_alreadygone(struct gateway_pvt *dialog)
{
	tris_debug(3, "Setting GATEWAY_ALREADYGONE on dialog %s\n", dialog->callid);
	dialog->alreadygone = 1;
}

/*! Resolve DNS srv name or host name in a gateway_proxy structure */
static int proxy_update(struct gateway_proxy *proxy)
{
	...
	if (!inet_aton(proxy->name, &proxy->ip.sin_addr)) {
		...
		if (tris_get_ip_or_srv(&proxy->ip, proxy->name, gateway_cfg.srvlookup ? "_gateway._udp" : NULL) < 0) {
...
/*! \brief Allocate and initialize gateway proxy */
static struct gateway_proxy *proxy_allocate(char *name, char *port, int force)
{
	struct gateway_proxy *proxy;
	...
	proxy->ip.sin_port = htons(port_str2int(port, STANDARD_GATEWAY_PORT));
	proxy_update(proxy);
	return proxy;
}

/*! \brief Get default outbound proxy or global proxy */
static struct gateway_proxy *obproxy_get(struct gateway_pvt *dialog, struct gateway_peer *peer)
{
	...
	if (gateway_cfg.outboundproxy.name[0]) {
		if (gatewaydebug)
			tris_debug(1, "OBPROXY: Applying global OBproxy to this call\n");
		append_history(dialog, "OBproxy", "Using global obproxy %s", gateway_cfg.outboundproxy.name);
		return &gateway_cfg.outboundproxy;
	}
	if (gatewaydebug)
 ...
 * matches the gateway method 'id'.
 * Strictly speaking, GATEWAY methods are case SENSITIVE, but we do
 * a case-insensitive comparison to be more tolerant.
 * following Jon Postel's rule: Be gentle in what you accept, strict with what you send
 */
static int method_match(enum gatewaymethod id, const char *name)
{
	int len = strlen(gateway_methods[id].text);
	int l_name = name ? strlen(name) : 0;
	/* true if the string is long enough, and ends with whitespace, and matches */
	return (l_name >= len && name[len] < 33 &&
		!strncasecmp(gateway_methods[id].text, name, len));
}

/*! \brief  find_gateway_method: Find GATEWAY method from header */
static int find_gateway_method(const char *msg)
{
	int i, res = 0;
	...
	if (tris_strlen_zero(msg))
		return 0;
	for (i = 1; i < ARRAY_LEN(gateway_methods) && !res; i++) {
		if (method_match(i, msg))
			res = gateway_methods[i].id;
	}
	return res;
}
...
static unsigned int parse_gateway_options(struct gateway_pvt *pvt, const char *supported)
{
	...
	if (gatewaydebug)
		tris_debug(3, "Begin: parsing GATEWAY \"Supported: %s\"\n", supported);
	...
	for (next = temp; next; next = sep) {
		...
		if (gatewaydebug)
			tris_debug(3, "Found GATEWAY option: -%s-\n", next);
		for (i = 0; i < ARRAY_LEN(gateway_options); i++) {
			if (!strcasecmp(next, gateway_options[i].text)) {
				profile |= gateway_options[i].id;
				found = TRUE;
				if (gatewaydebug)
					tris_debug(3, "Matched GATEWAY option: %s\n", next);
				break;
			}
		}
		...
		if (!found)
			profile |= GATEWAY_OPT_UNKNOWN;

		if (!found && gatewaydebug) {
			if (!strncasecmp(next, "x-", 2))
				tris_debug(3, "Found private GATEWAY option, not supported: %s\n", next);
			else
				tris_debug(3, "Found no match for GATEWAY option: %s (Please file bug report!)\n", next);
		}
	}

	if (pvt)
		pvt->gatewayoptions = profile;
	return profile;
}
...
static const struct sockaddr_in *gateway_real_dst(const struct gateway_pvt *p)
{
	...
	return tris_test_flag(&p->flags[0], GATEWAY_NAT) & GATEWAY_NAT_ROUTE ? &p->recv : &p->sa;
}

/*! \brief Display GATEWAY nat mode */
static const char *gateway_nat_mode(const struct gateway_pvt *p)
{
	return tris_test_flag(&p->flags[0], GATEWAY_NAT) & GATEWAY_NAT_ROUTE ? "NAT" : "no NAT";
}
...
static inline int gateway_debug_test_pvt(struct gateway_pvt *p) 
{
	if (!gatewaydebug)
		return 0;
	return gateway_debug_test_addr(gateway_real_dst(p));
}
...
static int get_transport_str2enum(const char *transport)
{
	...
	if (!strcasecmp(transport, "udp")) {
		res |= GATEWAY_TRANSPORT_UDP;
	}
	if (!strcasecmp(transport, "tcp")) {
		res |= GATEWAY_TRANSPORT_TCP;
	}
	if (!strcasecmp(transport, "tls")) {
		res |= GATEWAY_TRANSPORT_TLS;
	}
...
static inline const char *get_transport_list(unsigned int transports) {
	switch (transports) {
		case GATEWAY_TRANSPORT_UDP:
			return "UDP";
		case GATEWAY_TRANSPORT_TCP:
			return "TCP";
		case GATEWAY_TRANSPORT_TLS:
			return "TLS";
		case GATEWAY_TRANSPORT_UDP | GATEWAY_TRANSPORT_TCP:
			return "TCP,UDP";
		case GATEWAY_TRANSPORT_UDP | GATEWAY_TRANSPORT_TLS:
			return "TLS,UDP";
		case GATEWAY_TRANSPORT_TCP | GATEWAY_TRANSPORT_TLS:
			return "TLS,TCP";
		default:
			return transports ? 
				"TLS,TCP,UDP" : "UNKNOWN";	
...
static inline const char *get_transport(enum gateway_transport t)
{
	switch (t) {
	case GATEWAY_TRANSPORT_UDP:
		return "UDP";
...
*/
static inline const char *get_transport_pvt(struct gateway_pvt *p)
{
...
/*! \brief Transmit GATEWAY message 
	Sends a GATEWAY request or response on a given socket (in the pvt)
	Called by retrans_pkt, send_request, send_response and 
	__gateway_reliable_xmit
*/
static int __gateway_xmit(struct gateway_pvt *p, struct tris_str *data, int len)
{
	int res = 0;
	const struct sockaddr_in *dst = gateway_real_dst(p);
	...
	if (gateway_prepare_socket(p) < 0)
		return XMIT_ERROR;

	if (p->socket.type == GATEWAY_TRANSPORT_UDP) {
		res = sendto(p->socket.fd, data->str, len, 0, (const struct sockaddr *)dst, sizeof(struct sockaddr_in));
	} else if (p->socket.tcptls_session) {
		res = gateway_tcptls_write(p->socket.tcptls_session, data->str, len);
	} else {
...
static void build_via(struct gateway_pvt *p)
{
	...
	const char *rport = tris_test_flag(&p->flags[0], GATEWAY_NAT) & GATEWAY_NAT_RFC3581 ? ";rport" : "";
 ...
 *
 * Using the localaddr structure built up with localnet statements in gateway.conf
...
/*	if (want_remap &&
	    (!gateway_cfg.matchexterniplocally || !tris_apply_ha(localaddr, us)) ) {*/
	if (externip.sin_addr.s_addr) {
		/* if we used externhost or stun, see if it is time to refresh the info */
		if (externexpire && time(NULL) >= externexpire) {
			if (stunaddr.sin_addr.s_addr) {
				tris_stun_request(gatewaysock, &stunaddr, NULL, &externip);
			} else {
		...
		case GATEWAY_TRANSPORT_TCP:
			if (gateway_tcp_desc.local_address.sin_addr.s_addr) {
				*us = gateway_tcp_desc.local_address;
			} else {
				us->sin_port = gateway_tcp_desc.local_address.sin_port;
			}
			break;
		case GATEWAY_TRANSPORT_TLS:
			if (gateway_tls_desc.local_address.sin_addr.s_addr) {
				*us = gateway_tls_desc.local_address;
			} else {
				us->sin_port = gateway_tls_desc.local_address.sin_port;
			}
				break;
		case GATEWAY_TRANSPORT_UDP:
	...
	tris_debug(3, "Setting GATEWAY_TRANSPORT_%s with address %s:%d\n", get_transport(p->socket.type), tris_inet_ntoa(us->sin_addr), ntohs(us->sin_port));
}

/*! \brief Append to GATEWAY dialog history with arg list  */
static __attribute__((format(printf, 2, 0))) void append_history_va(struct gateway_pvt *p, const char *fmt, va_list ap)
{
	char buf[80], *c = buf; /* max history length */
	struct gateway_history *hist;
	int l;
...
/*! \brief Retransmit GATEWAY message if no answer (Called from scheduler) */
static int retrans_pkt(const void *data)
{
	struct gateway_pkt *pkt = (struct gateway_pkt *)data, *prev, *cur = NULL;
	int reschedule = DEFAULT_RETRANS;
	int xmitres = 0;
	
	/* Lock channel PVT */
	gateway_pvt_lock(pkt->owner);

	if (pkt->retrans < MAX_RETRANS) {
		pkt->retrans++;
 		if (!pkt->timer_t1) {	/* Re-schedule using timer_a and timer_t1 */
			if (gatewaydebug)
 				tris_debug(4, "GATEWAY TIMER: Not rescheduling id #%d:%s (Method %d) (No timer T1)\n", pkt->retransid, gateway_methods[pkt->method].text, pkt->method);
		} else {
 			int gatewaytimer_a;

 			if (gatewaydebug)
 				tris_debug(4, "GATEWAY TIMER: Rescheduling retransmission #%d (%d) %s - %d\n", pkt->retransid, pkt->retrans, gateway_methods[pkt->method].text, pkt->method);
 			if (!pkt->timer_a)
 				pkt->timer_a = 2 ;
 			else
 			...
 			gatewaytimer_a = pkt->timer_t1 * pkt->timer_a;	/* Double each time */
 			if (pkt->method != GATEWAY_INVITE && gatewaytimer_a > 4000)
 				gatewaytimer_a = 4000;
 		
 			/* Reschedule re-transmit */
			reschedule = gatewaytimer_a;
 			tris_debug(4, "** GATEWAY timers: Rescheduling retransmission %d to %d ms (t1 %d ms (Retrans id #%d)) \n", pkt->retrans +1, gatewaytimer_a, pkt->timer_t1, pkt->retransid);
 		} 

		if (gateway_debug_test_pvt(pkt->owner)) {
			const struct sockaddr_in *dst = gateway_real_dst(pkt->owner);
			tris_verbose("Retransmitting #%d (%s) to %s:%d:\n%s\n---\n",
				pkt->retrans, gateway_nat_mode(pkt->owner),
				tris_inet_ntoa(dst->sin_addr),
				ntohs(dst->sin_port), pkt->data->str);
		}

		append_history(pkt->owner, "ReTx", "%d %s", reschedule, pkt->data->str);
		xmitres = __gateway_xmit(pkt->owner, pkt->data, pkt->packetlen);
		if (xmitres == XMIT_ERROR) {
			tris_log(LOG_WARNING, "Network error on retransmit in dialog %s\n", pkt->owner->callid);
		} else {
			gateway_pvt_unlock(pkt->owner);
			return  reschedule;
		}
	} 
	/* Too many retries */
	if (pkt->owner && pkt->method != GATEWAY_OPTIONS && xmitres == 0) {
		if (pkt->is_fatal || gatewaydebug)	/* Tell us if it's critical or if we're debugging */
			tris_log(LOG_WARNING, "Maximum retries exceeded on transmission %s for seqno %d (%s %s) -- See doc/gateway-retransmit.txt.\n",
				pkt->owner->callid, pkt->seqno,
				pkt->is_fatal ? "Critical" : "Non-critical", pkt->is_resp ? "Response" : "Request");
	} else if (pkt->method == GATEWAY_OPTIONS && gatewaydebug) {
			tris_log(LOG_WARNING, "Cancelling retransmit of OPTIONs (call id %s)  -- See doc/gateway-retransmit.txt.\n", pkt->owner->callid);
	...
	if (pkt->is_fatal) {
		while(pkt->owner->owner && tris_channel_trylock(pkt->owner->owner)) {
			gateway_pvt_unlock(pkt->owner);	/* GATEWAY_PVT, not channel */
			usleep(1);
			gateway_pvt_lock(pkt->owner);
		}
		...
		if (pkt->owner->owner) {
			gateway_alreadygone(pkt->owner);
			tris_log(LOG_WARNING, "Hanging up call %s - no reply to our critical packet (see doc/gateway-retransmit.txt).\n", pkt->owner->callid);
			...
			if (pkt->method != GATEWAY_OPTIONS && pkt->method != GATEWAY_REGISTER) {
	...
	if (pkt->method == GATEWAY_BYE) {
		/* We're not getting answers on GATEWAY BYE's.  Tear down the call anyway. */
		if (pkt->owner->owner) 
			tris_channel_unlock(pkt->owner->owner);
...
*/
static enum gateway_result __gateway_reliable_xmit(struct gateway_pvt *p, int seqno, int resp, struct tris_str *data, int len, int fatal, int gatewaymethod)
{
	struct gateway_pkt *pkt = NULL;
	int gatewaytimer_a = DEFAULT_RETRANS;
	int xmitres = 0;
	int respid;

	if (gatewaymethod == GATEWAY_INVITE) {
	...
	if (!(p->socket.type & GATEWAY_TRANSPORT_UDP)) {
		xmitres = __gateway_xmit(p, data, len);	/* Send packet */
	...
	pkt->method = gatewaymethod;
	pkt->seqno = seqno;
	pkt->is_resp = resp;
	pkt->is_fatal = fatal;
	pkt->owner = dialog_ref(p, "__gateway_reliable_xmit: setting pkt->owner");
	pkt->next = p->packets;
	...
	pkt->timer_t1 = p->timer_t1;	/* Set GATEWAY timer T1 */
	pkt->retransid = -1;
	if (pkt->timer_t1)
		gatewaytimer_a = pkt->timer_t1 * 2;

	/* Schedule retransmission */
	TRIS_SCHED_REPLACE_VARIABLE(pkt->retransid, sched, gatewaytimer_a, retrans_pkt, pkt, 1);
	if (gatewaydebug)
		tris_debug(4, "*** GATEWAY TIMER: Initializing retransmit timer on packet: Id  #%d\n", pkt->retransid);

	xmitres = __gateway_xmit(pkt->owner, pkt->data, pkt->packetlen);	/* Send packet */

	if (xmitres == XMIT_ERROR) {	/* Serious network trouble, no need to try again */
		append_history(pkt->owner, "XmitErr", "%s", pkt->is_fatal ? "(Critical)" : "(Non-critical)");
		tris_log(LOG_ERROR, "Serious Network Trouble; __gateway_xmit returns error for pkt data\n");
...
/*! \brief Kill a GATEWAY dialog (called only by the scheduler)
...
static int __gateway_autodestruct(const void *data)
{
	struct gateway_pvt *p = (struct gateway_pvt *)data;
	...
	if (p->subscribed) {
		...
		tris_debug(3, "Re-scheduled destruction of GATEWAY subscription %s\n", p->callid ? p->callid : "<unknown>");
	...
	if (p->packets) {
		if (!p->needdestroy) {
			char method_str[31];
			tris_debug(3, "Re-scheduled destruction of GATEWAY call %s\n", p->callid ? p->callid : "<unknown>");
			append_history(p, "ReliableXmit", "timeout");
			if (sscanf(p->lastmsg, "Tx: %30s", method_str) == 1 || sscanf(p->lastmsg, "Rx: %30s", method_str) == 1) {
				if (method_match(GATEWAY_CANCEL, method_str) || method_match(GATEWAY_BYE, method_str)) {
	...
	if (p->owner) {
		...
		transmit_request_with_auth(p, GATEWAY_BYE, 0, XMIT_RELIABLE, 1);
		append_history(p, "ReferBYE", "Sending BYE on transferer call leg %s", p->callid);
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
	} else {
		append_history(p, "AutoDestroy", "%s", p->callid);
		tris_debug(3, "Auto destroying GATEWAY dialog '%s'\n", p->callid);
		...
		/* gateway_destroy(p); */		/* Go ahead and destroy dialog. All attempts to recover is done */
		/* gateway_destroy also absorbs the reference */
	}
	dialog_unref(p, "The ref to a dialog passed to this sched callback is going out of scope; unref it.");
	return 0;
}

/*! \brief Schedule destruction of GATEWAY dialog */
static void gateway_scheddestroy(struct gateway_pvt *p, int ms)
{
	if (ms < 0) {
		if (p->timer_t1 == 0) {
	...
	if (gateway_debug_test_pvt(p))
		tris_verbose("Scheduling destruction of GATEWAY dialog '%s' in %d ms (Method: %s)\n", p->callid, ms, gateway_methods[p->method].text);
	if (gateway_cancel_destroy(p))
		tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");

	if (p->do_history)
		append_history(p, "SchedDestroy", "%d ms", ms);
	p->autokillid = tris_sched_add(sched, ms, __gateway_autodestruct, dialog_ref(p, "setting ref as passing into tris_sched_add for __gateway_autodestruct"));

	if (p->stimer && p->stimer->st_active == TRUE && p->stimer->st_schedid > 0)
		stop_session_timer(p);
}

/*! \brief Cancel destruction of GATEWAY dialog.
...
static int __gateway_ack(struct gateway_pvt *p, int seqno, int resp, int gatewaymethod)
{
	struct gateway_pkt *cur, *prev = NULL;
	...
	/* If we have an outbound proxy for this dialog, then delete it now since
	  ...
	  dialog, regardless of what the GATEWAY rfc says
	*/
	...
	for (cur = p->packets; cur; prev = cur, cur = cur->next) {
		if (cur->seqno != seqno || cur->is_resp != resp)
			continue;
		if (cur->is_resp || cur->method == gatewaymethod) {
			res = TRUE;
			msg = "Found";
			...
			if (cur->retransid > -1) {
				if (gatewaydebug)
					tris_debug(4, "** GATEWAY TIMER: Cancelling retransmit of packet (reply received) Retransid #%d\n", cur->retransid);
			}
			...
			while (cur->retransid > -1 && tris_sched_del(sched, cur->retransid)) {
				gateway_pvt_unlock(p);
				usleep(1);
				gateway_pvt_lock(p);
			}
			UNLINK(cur, p->packets, prev);
			dialog_unref(cur->owner, "unref pkt cur->owner dialog from gateway ack before freeing pkt");
			if (cur->data)
				tris_free(cur->data);
			tris_free(cur);
			break;
...
static void __gateway_pretend_ack(struct gateway_pvt *p)
{
	struct gateway_pkt *cur = NULL;

	while (p->packets) {
		int method;
		if (cur == p->packets) {
			tris_log(LOG_WARNING, "Have a packet that doesn't want to give up! %s\n", gateway_methods[cur->method].text);
			return;
		}
		cur = p->packets;
		method = (cur->method) ? cur->method : find_gateway_method(cur->data->str);
		__gateway_ack(p, cur->seqno, cur->is_resp, method);
	}
}
...
static int __gateway_semi_ack(struct gateway_pvt *p, int seqno, int resp, int gatewaymethod)
{
	struct gateway_pkt *cur;
	int res = FALSE;

	for (cur = p->packets; cur; cur = cur->next) {
		if (cur->seqno == seqno && cur->is_resp == resp &&
			(cur->is_resp || method_match(gatewaymethod, cur->data->str))) {
			/* this is our baby */
			if (cur->retransid > -1) {
				if (gatewaydebug)
					tris_debug(4, "*** GATEWAY TIMER: Cancelling retransmission #%d - %s (got response)\n", cur->retransid, gateway_methods[gatewaymethod].text);
			}
...
/*! \brief Copy GATEWAY request, parse it */
static void parse_copy(struct gateway_request *dst, const struct gateway_request *src)
{
...
static void add_blank(struct gateway_request *req)
{
	if (!req->lines) {
...
static int send_provisional_keepalive_full(struct gateway_pvt *pvt, int with_sdp)
{
	const char *msg = NULL;
...
static int send_provisional_keepalive(const void *data) {
	struct gateway_pvt *pvt = (struct gateway_pvt *) data;

	return send_provisional_keepalive_full(pvt, 0);
}

static int send_provisional_keepalive_with_sdp(const void *data) {
	struct gateway_pvt *pvt = (void *)data;

	return send_provisional_keepalive_full(pvt, 1);
}

static void update_provisional_keepalive(struct gateway_pvt *pvt, int with_sdp)
{
...
/*! \brief Transmit response on GATEWAY request*/
static int send_response(struct gateway_pvt *p, struct gateway_request *req, enum xmittype reliable, int seqno)
{
	int res;
	...
	if (gateway_debug_test_pvt(p)) {
		...
		tris_verbose("\n<--- %sTransmitting (%s) to %s:%d --->\n%s\n<------------>\n",
			reliable ? "Reliably " : "", gateway_nat_mode(p),
			tris_inet_ntoa(dst->sin_addr),
			ntohs(dst->sin_port), req->data->str);
	}
	if (p->do_history) {
		struct gateway_request tmp = { .rlPart1 = 0, };
		parse_copy(&tmp, req);
		append_history(p, reliable ? "TxRespRel" : "TxResp", "%s / %s - %s", tmp.data->str, get_header(&tmp, "CSeq"), 
			(tmp.method == GATEWAY_RESPONSE || tmp.method == GATEWAY_UNKNOWN) ? REQ_OFFSET_TO_STR(&tmp, rlPart2) : gateway_methods[tmp.method].text);
		tris_free(tmp.data);
	}

	/* If we are sending a final response to an INVITE, stop retransmitting provisional responses */
	if (p->initreq.method == GATEWAY_INVITE && reliable == XMIT_CRITICAL) {
	...
	res = (reliable) ?
		 __gateway_reliable_xmit(p, seqno, 1, req->data, req->len, (reliable == XMIT_CRITICAL), req->method) :
		__gateway_xmit(p, req->data, req->len);
	tris_free(req->data);
	req->data = NULL;
	if (res > 0)
		return 0;
...
/*! \brief Send GATEWAY Request to the other part of the dialogue */
static int send_request(struct gateway_pvt *p, struct gateway_request *req, enum xmittype reliable, int seqno)
{
	int res;
	...
	*/
	...
	add_blank(req);
	if (gateway_debug_test_pvt(p)) {
		if (tris_test_flag(&p->flags[0], GATEWAY_NAT_ROUTE))
	...
	if (p->do_history) {
		struct gateway_request tmp = { .rlPart1 = 0, };
		parse_copy(&tmp, req);
		append_history(p, reliable ? "TxReqRel" : "TxReq", "%s / %s - %s", tmp.data->str, get_header(&tmp, "CSeq"), gateway_methods[tmp.method].text);
		tris_free(tmp.data);
	}
	res = (reliable) ?
		__gateway_reliable_xmit(p, seqno, 0, req->data, req->len, (reliable == XMIT_CRITICAL), req->method) :
		__gateway_xmit(p, req->data, req->len);
	if (req->data) {
		tris_free(req->data);
		req->data = NULL;
...
/*! \brief Query an option on a GATEWAY dialog */
static int gateway_queryoption(struct tris_channel *chan, int option, void *data, int *datalen)
{
	int res = -1;
	enum tris_t38_state state = T38_STATE_UNAVAILABLE;
	struct gateway_pvt *p = (struct gateway_pvt *) chan->tech_pvt;
	...
	case TRIS_OPTION_T38_STATE:
		...
		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT)) {
			switch (p->t38.state) {
 ...
 * \verbatim 
 * general form we are expecting is gateway[s]:username[:password][;parameter]@host[:port][;...] 
 * \endverbatim
 * 
 */
	...
	char **ret_name, char **pass, char **domain, char **port, char **options, char **transport)
{
	...
	if (!domain) {
		...
		if ((c = strchr(uri, '@')) == NULL) {
			/* domain-only URI, according to the GATEWAY RFC. */
			dom = uri;
			name = "";
		} else {
			*c++ = '\0';
...
static int gateway_sendhtml(struct tris_channel *chan, int subclass, const char *data, int datalen)
{
	struct gateway_pvt *p = chan->tech_pvt;
	...
	case TRIS_STATE_UP:
		...
		} else if (!tris_test_flag(&p->flags[0], GATEWAY_PENDINGBYE)) {
			tris_set_flag(&p->flags[0], GATEWAY_NEEDREINVITE);	
		}	
		break;
	default:
...
/*! \brief Deliver GATEWAY call ID for the call */
static const char *gateway_get_callid(struct tris_channel *chan)
{
	return chan->tech_pvt ? ((struct gateway_pvt *) chan->tech_pvt)->callid : "";
}

/*! \brief Send GATEWAY MESSAGE text within a call
	Called from PBX core sendtext() application */
static int gateway_sendtext(struct tris_channel *ast, const char *text)
{
	struct gateway_pvt *p = ast->tech_pvt;
	int debug = gateway_debug_test_pvt(p);
...
/*! \brief Update peer object in realtime storage 
	...
	that name and store that in the "regserver" field in the gatewaypeers
...
static void realtime_update_peer(const char *peername, struct sockaddr_in *sin, const char *defaultuser, const char *fullcontact, const char *useragent, int expirey, int deprecated_username, int lastms)
{
	...
	int realtimeregs = tris_check_realtime("gatewayregs");

	tablename = realtimeregs ? "gatewayregs" : "gatewaypeers";
	...
	if (tris_strlen_zero(sysname))	/* No system name, disable this */
		sysname = NULL;
	else if (gateway_cfg.rtsave_sysname)
		syslabel = "regserver";
		...
		if ((context = strchr(ext, '@'))) {
			*context++ = '\0';	/* split ext@context */
			if (!tris_context_find(context)) {
				tris_log(LOG_WARNING, "Context %s must exist in regcontext= in gateway.conf!\n", context);
				continue;
			}
		} else {
		...
		if (onoff) {
			if (!tris_exists_extension(NULL, context, ext, 1, NULL)) {
				tris_add_extension(context, 1, ext, 1, NULL, NULL, "Noop",
					 tris_strdup(peer->name), tris_free_ptr, "GATEWAY");
			}
...
static void destroy_mailbox(struct gateway_mailbox *mailbox)
{
	if (mailbox->mailbox)
...
static void clear_peer_mailboxes(struct gateway_peer *peer)
{
	struct gateway_mailbox *mailbox;
...
static void gateway_destroy_peer_fn(void *peer)
{
	gateway_destroy_peer(peer);
}

/*! \brief Destroy peer object from memory */
static void gateway_destroy_peer(struct gateway_peer *peer)
{
	tris_debug(3, "Destroying GATEWAY peer %s\n", peer->name);
...
static void update_peer(struct gateway_peer *p, int expire)
{
	int rtcachefriends = tris_test_flag(&p->flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS);
	if (gateway_cfg.peer_rtupdate &&
...
static struct tris_variable *get_insecure_variable_from_config(struct tris_config *cfg)
{
	...
	while ((cat = tris_category_browse(cfg, cat))) {
		...
		if (tris_test_flag(&flags, GATEWAY_INSECURE_PORT)) {
 ...
 * Checks the "gatewaypeers" realtime family from extconfig.conf 
 * Checks the "gatewayregs" realtime family from extconfig.conf if it's configured.
 * This returns a pointer to a peer and because we use build_peer, we can rest
 * assured that the refcount is bumped.
*/
static struct gateway_peer *realtime_peer(const char *newpeername, struct sockaddr_in *sin, int devstate_only)
{
	struct gateway_peer *peer;
	struct tris_variable *var = NULL;
	...
	if (newpeername) {
		if (realtimeregs)
			varregs = tris_load_realtime("gatewayregs", "name", newpeername, SENTINEL);

		var = tris_load_realtime("gatewaypeers", "name", newpeername, "host", "dynamic", SENTINEL);
		if (!var && sin)
			var = tris_load_realtime("gatewaypeers", "name", newpeername, "host", tris_inet_ntoa(sin->sin_addr), SENTINEL);
		if (!var) {
			var = tris_load_realtime("gatewaypeers", "name", newpeername, SENTINEL);
			/*!\note
	...
	if (!var && sin) {	/* Then check on IP address for dynamic peers */
		...
		var = tris_load_realtime("gatewaypeers", "host", ipaddr, "port", portstring, SENTINEL);	/* First check for fixed IP hosts */
		if (var) {
			if (realtimeregs) {
			...
			if (realtimeregs)
				varregs = tris_load_realtime("gatewayregs", "ipaddr", ipaddr, "port", portstring, SENTINEL); /* Then check for registered hosts */
			else
				var = tris_load_realtime("gatewaypeers", "ipaddr", ipaddr, "port", portstring, SENTINEL); /* Then check for registered hosts */
			if (varregs) {
		...
		if (!var) { /*We couldn't match on ipaddress and port, so we need to check if port is insecure*/
			peerlist = tris_load_realtime_multientry("gatewaypeers", "host", ipaddr, SENTINEL);
			if (peerlist) {
				...
				if(var) {
					...
					peerlist = NULL;
					cat = NULL;
					peerlist = tris_load_realtime_multientry("gatewaypeers", "ipaddr", ipaddr, SENTINEL);
					if(peerlist) {
						var = get_insecure_variable_from_config(peerlist);
						if(var) {
							if (realtimeregs) {
				...
				if (realtimeregs) {
					peerlist = tris_load_realtime_multientry("gatewayregs", "ipaddr", ipaddr, SENTINEL);
					if (peerlist) {
	...
	if (tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS) && !devstate_only) {
		/* Cache peer */
		tris_copy_flags(&peer->flags[1], &global_flags[1], GATEWAY_PAGE2_RTAUTOCLEAR|GATEWAY_PAGE2_RTCACHEFRIENDS);
		if (tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RTAUTOCLEAR)) {
			TRIS_SCHED_REPLACE_UNREF(peer->expire, sched, gateway_cfg.rtautoclear * 1000, expire_register, peer,
...
{
	struct gateway_peer *search = obj, *match = arg;
	int *which_objects = data;

	/* Usernames in GATEWAY uri's are case sensitive. Domains are not */
	...
	case FINDUSERS:
		if (!(search->type & GATEWAY_TYPE_USER)) {
			return 0;
		}
		break;
	case FINDPEERS:
		if (!(search->type & GATEWAY_TYPE_PEER)) {
			return 0;
		}
		break;
	case FINDALLDEVICES:
 ...
 */
static struct gateway_peer *find_peer(const char *peer, struct sockaddr_in *sin, int realtime, int which_objects, int devstate_only, int transport)
{
	struct gateway_peer *p = NULL;
	struct gateway_peer tmp_peer;
	...
	if (peer) {
		...
		p = ao2_t_find(peers_by_ip, &tmp_peer, OBJ_POINTER, "ao2_find in peers_by_ip table"); /* WAS:  p = ASTOBJ_CONTAINER_FIND_FULL(&peerl, sin, name, gateway_addr_hashfunc, 1, gateway_addrcmp); */
		if (!p) {
			tris_set_flag(&tmp_peer.flags[0], GATEWAY_INSECURE_PORT);
			p = ao2_t_find(peers_by_ip, &tmp_peer, OBJ_POINTER, "ao2_find in peers_by_ip table 2"); /* WAS:  p = ASTOBJ_CONTAINER_FIND_FULL(&peerl, sin, name, gateway_addr_hashfunc, 1, gateway_addrcmp); */
			if (p) {
				return p;
			}
		}
...
/*! \brief Change the T38 state on a GATEWAY dialog */
static void change_t38_state(struct gateway_pvt *p, int state)
{
	int old = p->t38.state;
...
/*! \brief Set the global T38 capabilities on a GATEWAY dialog structure */
static void set_t38_capabilities(struct gateway_pvt *p)
{
	if (p->udptl) {
		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT) == GATEWAY_PAGE2_T38SUPPORT_UDPTL_REDUNDANCY) {
                        tris_udptl_set_error_correction_scheme(p->udptl, UDPTL_ERROR_CORRECTION_REDUNDANCY);
		} else if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT) == GATEWAY_PAGE2_T38SUPPORT_UDPTL_FEC) {
			tris_udptl_set_error_correction_scheme(p->udptl, UDPTL_ERROR_CORRECTION_FEC);
		} else if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT) == GATEWAY_PAGE2_T38SUPPORT_UDPTL) {
			tris_udptl_set_error_correction_scheme(p->udptl, UDPTL_ERROR_CORRECTION_NONE);
		}
	}
}

static void copy_socket_data(struct gateway_socket *to_sock, const struct gateway_socket *from_sock)
{
 ...
 *
 */
static int create_addr_from_peer(struct gateway_pvt *dialog, struct gateway_peer *peer)
{
	...
	tris_copy_flags(&dialog->flags[0], &peer->flags[0], GATEWAY_FLAGS_TO_COPY);
	tris_copy_flags(&dialog->flags[1], &peer->flags[1], GATEWAY_PAGE2_FLAGS_TO_COPY);
	dialog->capability = peer->capability;
	if (!tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS) &&
			(!tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT) ||
				!(dialog->capability & TRIS_FORMAT_VIDEO_MASK)) &&
			dialog->vrtp) {
		tris_rtp_destroy(dialog->vrtp);
		dialog->vrtp = NULL;
	}
	if (!tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_TEXTSUPPORT) && dialog->trtp) {
		tris_rtp_destroy(dialog->trtp);
		dialog->trtp = NULL;
	}
	dialog->prefs = peer->prefs;
	if (tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_T38SUPPORT)) {
		if (!dialog->udptl) {
	...
	do_setnat(dialog, tris_test_flag(&dialog->flags[0], GATEWAY_NAT) & GATEWAY_NAT_ROUTE);

	if (dialog->rtp) { /* Audio */
		tris_rtp_setdtmf(dialog->rtp, tris_test_flag(&dialog->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_RFC2833);
		tris_rtp_setdtmfcompensate(dialog->rtp, tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_RFC2833_COMPENSATE));
		...
		if (tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_CONSTANT_SSRC)) {
	...
	if ((tris_test_flag(&dialog->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_RFC2833) ||
	    (tris_test_flag(&dialog->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_AUTO))
		dialog->noncodeccapability |= TRIS_RTP_DTMF;
	else
		dialog->noncodeccapability &= ~TRIS_RTP_DTMF;
	if (peer->call_limit)
		tris_set_flag(&dialog->flags[0], GATEWAY_CALL_LIMIT);
	if (!dialog->portinuri)
...
static int create_addr(struct gateway_pvt *dialog, const char *opeer, struct sockaddr_in *sin, int newdialog)
{
	...
	dialog->timer_t1 = global_t1; /* Default GATEWAY retransmission timer T1 (RFC 3261) */
	dialog->timer_b = global_timer_b; /* Default GATEWAY transaction timer B (RFC 3261) */
	...
	if (sin) {
		...
		if (!sin->sin_port) {
			portno = port_str2int(port, (dialog->socket.type == GATEWAY_TRANSPORT_TLS) ? STANDARD_TLS_PORT : STANDARD_GATEWAY_PORT);
		} else {
		 ...
		 */
		if (!port && gateway_cfg.srvlookup) {
			char service[MAXHOSTNAMELEN];
			int tportno;
	
			snprintf(service, sizeof(service), "_gateway._%s.%s", get_transport(dialog->socket.type), peername);
	...
	if (!dialog->socket.type)
		set_socket_transport(&dialog->socket, GATEWAY_TRANSPORT_UDP);
	...
	if (!recvhost) {
		...
		tris_set_flag(&dialog->flags[0], GATEWAY_NAT);
	}
	return 0;
}
...
static int auto_congest(const void *arg)
{
	struct gateway_pvt *p = (struct gateway_pvt *)arg;
 ...
 * Only called by the scheduler, must return the reference when done.
 */
static int gateway_expire_timer(const void *arg)
{
	struct gateway_pvt *p = (struct gateway_pvt *)arg;
...
/*! \brief Initiate GATEWAY call from PBX 
 *      used from the dial() application      */
static int gateway_call(struct tris_channel *ast, char *dest, int timeout)
{
	int res;
	struct gateway_pvt *p = ast->tech_pvt;	/* chan is locked, so the reference cannot go away */
	struct varshead *headp;
	struct tris_var_t *current;
	const char *referer = NULL;   /* GATEWAY referrer */

	if ((ast->_state != TRIS_STATE_DOWN) && (ast->_state != TRIS_STATE_RESERVED)) {
		tris_log(LOG_WARNING, "gateway_call called on %s, neither down nor reserved\n", ast->name);
		return -1;
	}
	...
	TRIS_LIST_TRAVERSE(headp, current, entries) {
		...
		} else if (!p->options->uri_options && !strcasecmp(tris_var_name(current), "GATEWAY_URI_OPTIONS")) {
			p->options->uri_options = tris_var_value(current);
		} else if (!p->options->addgatewayheaders && !strncasecmp(tris_var_name(current), "GATEWAYADDHEADER", strlen("GATEWAYADDHEADER"))) {
			/* Check whether there is a variable with a name starting with GATEWAYADDHEADER */
			p->options->addgatewayheaders = 1;
		} else if (!strcasecmp(tris_var_name(current), "GATEWAYFROMDOMAIN")) {
			tris_string_field_set(p, fromdomain, tris_var_value(current));
		} else if (!strcasecmp(tris_var_name(current), "GATEWAYTRANSFER")) {
			/* This is a transfered call */
			p->options->transfer = 1;
		} else if (!strcasecmp(tris_var_name(current), "GATEWAYTRANSFER_REFERER")) {
			/* This is the referrer */
			referer = tris_var_value(current);
		} else if (!strcasecmp(tris_var_name(current), "GATEWAYTRANSFER_REPLACES")) {
	...
	res = 0;
	tris_set_flag(&p->flags[0], GATEWAY_OUTGOING);

	if (p->options->transfer) {
		char buf[GATEWAYBUFSIZE/2];
	...
	if (!(p->jointcapability & TRIS_FORMAT_AUDIO_MASK)) {
		...
		xmitres = transmit_invite(p, GATEWAY_INVITE, 1, 2);
		if (xmitres == XMIT_ERROR)
			return -1;
...
static void gateway_registry_destroy(struct gateway_registry *reg)
{
	...
	if (reg->call) {
		...
		tris_debug(3, "Destroying active GATEWAY dialog for registry %s@%s\n", reg->username, reg->hostname);
		dialog_unlink_all(reg->call, TRUE, TRUE);
		reg->call = dialog_unref(reg->call, "unref reg->call");
		/* reg->call = gateway_destroy(reg->call); */
	}
...
/*! \brief Execute destruction of GATEWAY dialog structure, release memory */
static void __gateway_destroy(struct gateway_pvt *p, int lockowner, int lockdialoglist)
{
	struct gateway_request *req;
	...
	if (gateway_debug_test_pvt(p))
		tris_verbose("Really destroying GATEWAY dialog '%s' Method: %s\n", p->callid, gateway_methods[p->method].text);

	if (tris_test_flag(&p->flags[0], GATEWAY_INC_COUNT) || tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD)) {
	...
	if (p->relatedpeer)
		p->relatedpeer = unref_peer(p->relatedpeer,"unsetting a dialog relatedpeer field in gateway_destroy");
...
/*! \brief  update_call_counter: Handle call_limit for GATEWAY devices
...
static int update_call_counter(struct gateway_pvt *fup, int event)
{
	...
	if (!tris_test_flag(&fup->flags[0], GATEWAY_CALL_LIMIT) && !tris_test_flag(&fup->flags[1], GATEWAY_PAGE2_CALL_ONHOLD))
		return 0;
	...
	case DEC_CALL_LIMIT:
		...
		if (inuse) {
			gateway_pvt_lock(fup);
			ao2_lock(p);
			if (*inuse > 0) {
				if (tris_test_flag(&fup->flags[0], GATEWAY_INC_COUNT)) {
					(*inuse)--;
					tris_clear_flag(&fup->flags[0], GATEWAY_INC_COUNT);
				}
			} else {
				*inuse = 0;
			}
			ao2_unlock(p);
			gateway_pvt_unlock(fup);
		}
		...
		if (inringing) {
			...
			ao2_lock(p);
			if (*inringing > 0) {
				if (tris_test_flag(&fup->flags[0], GATEWAY_INC_RINGING)) {
					(*inringing)--;
					tris_clear_flag(&fup->flags[0], GATEWAY_INC_RINGING);
				}
			} else {
			   *inringing = 0;
			}
		...
		gateway_pvt_lock(fup);
		ao2_lock(p);
		if (tris_test_flag(&fup->flags[1], GATEWAY_PAGE2_CALL_ONHOLD) && gateway_cfg.notifyhold) {
			tris_clear_flag(&fup->flags[1], GATEWAY_PAGE2_CALL_ONHOLD);
			ao2_unlock(p);
			gateway_pvt_unlock(fup);
			gateway_peer_hold(fup, FALSE);
		} else {
			ao2_unlock(p);
			gateway_pvt_unlock(fup);
		}
	...
	case INC_CALL_LIMIT:
		...
		if (inringing && (event == INC_CALL_RINGING)) {
			...
			if (!tris_test_flag(&fup->flags[0], GATEWAY_INC_RINGING)) {
				(*inringing)++;
				tris_set_flag(&fup->flags[0], GATEWAY_INC_RINGING);
			}
			ao2_unlock(p);
			gateway_pvt_unlock(fup);
		}
		if (inuse) {
			...
			if (!tris_test_flag(&fup->flags[0], GATEWAY_INC_COUNT)) {
				(*inuse)++;
				tris_set_flag(&fup->flags[0], GATEWAY_INC_COUNT);
			}
			ao2_unlock(p);
			gateway_pvt_unlock(fup);
		}
...
static void gateway_destroy_fn(void *p)
{
	gateway_destroy(p);
}

/*! \brief Destroy GATEWAY call structure.
 * Make it return NULL so the caller can do things like
 *	foo = gateway_destroy(foo);
 * and reduce the chance of bugs due to dangling pointers.
 */
static struct gateway_pvt * gateway_destroy(struct gateway_pvt *p)
{
	tris_debug(3, "Destroying GATEWAY dialog %s\n", p->callid);
	__gateway_destroy(p, TRUE, TRUE);
	return NULL;
}

/*! \brief Convert GATEWAY hangup causes to Trismedia hangup causes */
static int hangup_gateway2cause(int cause)
{
	...
	switch(cause) {
		...
		case 505:	/* GATEWAY version not supported */
			return TRIS_CAUSE_INTERWORKING;
...
/*! \brief Convert Trismedia hangup causes to GATEWAY codes 
\verbatim
 Possible values from causes.h
   ...
   ISUP Cause value                        GATEWAY response
   ----------------                        ------------
...
static const char *hangup_cause2gateway(int cause)
{
	switch (cause) {
		...
		default:
			tris_debug(1, "AST hangup cause %d (no match found in GATEWAY)\n", cause);
			return NULL;
	}
...
/*! \brief  gateway_hangup: Hangup GATEWAY call
...
static int gateway_hangup(struct tris_channel *ast)
{
	...
	if (tris_test_flag(&p->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER)) {
		if (tris_test_flag(&p->flags[0], GATEWAY_INC_COUNT) || tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD)) {
			if (gatewaydebug)
				tris_debug(1, "update_call_counter(%s) - decrement call limit counter on hangup\n", p->username);
			update_call_counter(p, DEC_CALL_LIMIT);
		}
		tris_debug(4, "GATEWAY Transfer: Not hanging up right now... Rescheduling hangup for %s.\n", p->callid);
		if (p->autokillid > -1 && gateway_cancel_destroy(p))
			tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
		tris_clear_flag(&p->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER);	/* Really hang up next time */
		p->needdestroy = 0;
	...
	if (tris_test_flag(ast, TRIS_FLAG_ZOMBIE)) {
		if (p->refer)
			tris_debug(1, "GATEWAY Transfer: Hanging up Zombie channel %s after transfer ... Call-ID: %s\n", ast->name, p->callid);
		else
			tris_debug(1, "Hanging up zombie call. Be scared.\n");
	} else
		tris_debug(1, "Hangup call %s, GATEWAY callid %s\n", ast->name, p->callid);
	...
	*/
	...
	if (!p->alreadygone && p->initreq.data && !tris_strlen_zero(p->initreq.data->str)) {
		if (needcancel) {	/* Outgoing call, not up */
			if (tris_test_flag(&p->flags[0], GATEWAY_OUTGOING)) {
				/* stop retransmitting an INVITE that has not received a response */
				/* if we can't send right now, mark it pending */
				if (p->invitestate == INV_CALLING) {
					/* We can't send anything in CALLING state */
					tris_set_flag(&p->flags[0], GATEWAY_PENDINGBYE);
					...
					for (cur = p->packets; cur; cur = cur->next) {
						__gateway_semi_ack(p, cur->seqno, cur->is_resp, cur->method ? cur->method : find_gateway_method(cur->data->str));
					}
					p->invitestate = INV_CANCELLED;
					/* Send a new request: CANCEL */
					transmit_request(p, GATEWAY_CANCEL, p->lastinvite, XMIT_RELIABLE, FALSE);
				...
				if (p->provisional_keepalive_sched_id > -1)
					TRIS_SCHED_DEL_UNREF(sched, p->provisional_keepalive_sched_id, dialog_unref(p, "when you delete the provisional_keepalive_sched_id, you should dec the refcount for the stored dialog ptr"));
				if (p->hangupcause && (res = hangup_cause2gateway(p->hangupcause)))
			...
			if (!p->pendinginvite) {
				...
				if (bridge) {
					struct gateway_pvt *q = bridge->tech_pvt;

					if (IS_GATEWAY_TECH(bridge->tech) && q && q->rtp)
				...
				tris_set_flag(&p->flags[0], GATEWAY_PENDINGBYE);	
				tris_clear_flag(&p->flags[0], GATEWAY_NEEDREINVITE);	
...
/*! \brief Try setting codec suggested by the GATEWAY_CODEC channel variable */
static void try_suggested_gateway_codec(struct gateway_pvt *p)
{
	int fmt;
	const char *codec;

	codec = pbx_builtin_getvar_helper(p->owner, "GATEWAY_CODEC");
	if (!codec) 
		return;

	fmt = tris_getformatbyname(codec);
	if (fmt) {
		tris_log(LOG_NOTICE, "Changing codec to '%s' for this call because of ${GATEWAY_CODEC} variable\n", codec);
		if (p->jointcapability & fmt) {
			p->jointcapability &= fmt;
			p->capability &= fmt;
		} else
			tris_log(LOG_NOTICE, "Ignoring ${GATEWAY_CODEC} variable because it is not shared by both ends.\n");
	} else
		tris_log(LOG_NOTICE, "Ignoring ${GATEWAY_CODEC} variable because of unrecognized/not configured codec (check allow/disallow in gateway.conf): %s\n", codec);
	return;	
}

/*! \brief  gateway_answer: Answer GATEWAY call , send 200 OK on Invite 
...
static int gateway_answer(struct tris_channel *ast)
{
	...
	if (ast->_state != TRIS_STATE_UP) {
		try_suggested_gateway_codec(p);	

		tris_setstate(ast, TRIS_STATE_UP);
		tris_debug(1, "GATEWAY answering channel: %s\n", ast->name);
		tris_rtp_new_source(p->rtp);
		res = transmit_response_with_sdp(p, "200 OK", &p->initreq, XMIT_CRITICAL, FALSE);
		tris_set_flag(&p->flags[1], GATEWAY_PAGE2_DIALOG_ESTABLISHED);
	}
	gateway_pvt_unlock(p);
	return res;
}
...
static int gateway_write(struct tris_channel *ast, struct tris_frame *frame)
{
	...
	case TRIS_FRAME_VOICE:
		...
		if (p) {
			...
			if (p->rtp) {
				...
				if ((ast->_state != TRIS_STATE_UP) &&
				    !tris_test_flag(&p->flags[0], GATEWAY_PROGRESS_SENT) &&
				    !tris_test_flag(&p->flags[0], GATEWAY_OUTGOING)) {
					tris_rtp_new_source(p->rtp);
					if (!global_prematuremediafilter) {
						p->invitestate = INV_EARLY_MEDIA;
						transmit_provisional_response(p, "183 Session Progress", &p->initreq, TRUE);
						tris_set_flag(&p->flags[0], GATEWAY_PROGRESS_SENT);
					}
	...
	case TRIS_FRAME_MODEM:
		...
		break;
	default: 
		tris_log(LOG_WARNING, "Can't send %d type frames with GATEWAY write\n", frame->frametype);
		return 0;
	}
...
/*! \brief  gateway_fixup: Fix up a channel:  If a channel is consumed, this is called.
        Basically update any ->owner links */
static int gateway_fixup(struct tris_channel *oldchan, struct tris_channel *newchan)
{
	int ret = -1;
	struct gateway_pvt *p;
	...
	if (!newchan || !newchan->tech_pvt) {
		if (!newchan)
			...
			tris_log(LOG_WARNING, "No GATEWAY tech_pvt! Fixup of %s failed.\n", oldchan->name);
		return -1;
	}
	p = newchan->tech_pvt;
	...
	if (p->owner != oldchan)
		...
		/* Re-invite RTP back to Trismedia. Needed if channel is masqueraded out of a native
		   ...
		   should stay up. */
		gateway_set_rtp_peer(newchan, NULL, NULL, 0, 0, 0);
		ret = 0;
	}
	tris_debug(3, "GATEWAY Fixup: New owner for dialogue %s: %s (Old parent: %s)\n", p->callid, p->owner->name, oldchan->name);
...
static int gateway_senddigit_begin(struct tris_channel *ast, char digit)
{
	...
	gateway_pvt_lock(p);
	switch (tris_test_flag(&p->flags[0], GATEWAY_DTMF)) {
	case GATEWAY_DTMF_INBAND:
		res = -1; /* Tell Trismedia to generate inband indications */
		break;
	case GATEWAY_DTMF_RFC2833:
		if (p->rtp)
...
/*! \brief Send DTMF character on GATEWAY channel
...
static int gateway_senddigit_end(struct tris_channel *ast, char digit, unsigned int duration)
{
	...
	gateway_pvt_lock(p);
	switch (tris_test_flag(&p->flags[0], GATEWAY_DTMF)) {
	case GATEWAY_DTMF_INFO:
	case GATEWAY_DTMF_SHORTINFO:
...
static int gateway_transfer(struct tris_channel *ast, const char *dest)
{
	...
	if (ast->_state == TRIS_STATE_RING)
		res = gateway_gatewayredirect(p, dest);
	else
...
static void interpret_t38_parameters(struct gateway_pvt *p, const struct tris_control_t38_parameters *parameters)
{
	if (!tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT)) {
		return;
	}
 ...
 * With GATEWAY a lot of indications is sent as messages, letting the device play
   the indication - busy signal, congestion etc 
   \return -1 to force tris_indicate to send indication in audio, 0 if GATEWAY can handle the indication by sending a message
*/
static int gateway_indicate(struct tris_channel *ast, int condition, const void *data, size_t datalen)
{
	struct gateway_pvt *p = ast->tech_pvt;
	...
	case TRIS_CONTROL_RINGING:
		if (ast->_state == TRIS_STATE_RING) {
			p->invitestate = INV_EARLY_MEDIA;
			if (!tris_test_flag(&p->flags[0], GATEWAY_PROGRESS_SENT) ||
			    (tris_test_flag(&p->flags[0], GATEWAY_PROG_INBAND) == GATEWAY_PROG_INBAND_NEVER)) {				
				/* Send 180 ringing if out-of-band seems reasonable */
				transmit_provisional_response(p, "180 Ringing", &p->initreq, 0);
				tris_set_flag(&p->flags[0], GATEWAY_RINGING);
				if (tris_test_flag(&p->flags[0], GATEWAY_PROG_INBAND) != GATEWAY_PROG_INBAND_YES)
					break;
			} else {
	...
	case TRIS_CONTROL_NOTIFY_PROCEEDING:
		transmit_notify_with_gatewayfrag(p, seqno, "100 Trying", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_RINGING:
		transmit_notify_with_gatewayfrag(p, seqno, "180 Ringing", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_PROGRESS:
		transmit_notify_with_gatewayfrag(p, seqno, "183 Session Progress", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_ANSWER:
		transmit_notify_with_gatewayfrag(p, seqno, "200 OK", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_OFFHOOK:
		transmit_notify_with_gatewayfrag(p, seqno, "404 Not Found", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_TAKEOFFHOOK:
		transmit_notify_with_gatewayfrag(p, seqno, "480 Unregistered", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_BUSY:
		...
		transmit_notify_with_gatewayfrag(p, seqno, message, TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_TIMEOUT:
		transmit_notify_with_gatewayfrag(p, seqno, "408 Request Timeout", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_FORBIDDEN:
		transmit_notify_with_gatewayfrag(p, seqno, "403 Forbidden", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_NOMONEY:
		transmit_notify_with_gatewayfrag(p, seqno, "402 No Money", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_OUTLINE:
		transmit_notify_with_gatewayfrag(p, seqno, "410 Gone Away", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_CIRCUITS:
		transmit_notify_with_gatewayfrag(p, seqno, "502 Bad gateway", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_ACCEPT:
		transmit_notify_with_gatewayfrag(p, seqno, "Accepted", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_CANCEL:
		transmit_notify_with_gatewayfrag(p, seqno, "Canceled", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_BYE:
		transmit_notify_with_gatewayfrag(p, seqno, "Byed", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_CONNECT:
		transmit_notify_with_gatewayfrag(p, seqno, "Connected", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_ANNOUNCE:
		transmit_notify_with_gatewayfrag(p, seqno, "Announced", TRUE);
		break;
...
/*! \brief Initiate a call in the GATEWAY channel
	called from gateway_request_call (calls from the pbx ) for outbound channels
	and from handle_request_invite for inbound channels
	
*/
static struct tris_channel *gateway_new(struct gateway_pvt *i, int state, const char *title)
{
...

	{
		...
		gateway_pvt_unlock(i);
		/* Don't hold a gateway pvt lock while we allocate a channel */
	...
	if (!tmp) {
		tris_log(LOG_WARNING, "Unable to allocate AST channel structure for GATEWAY channel\n");
		gateway_pvt_lock(i);
		return NULL;
	}
	gateway_pvt_lock(i);

	tmp->tech = ( tris_test_flag(&i->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_INFO || tris_test_flag(&i->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_SHORTINFO) ?  &gateway_tech_info : &gateway_tech;
	...
	tris_debug(3, "*** Our native formats are %s \n", tris_getformatname_multiple(buf, GATEWAYBUFSIZE, tmp->nativeformats));
	tris_debug(3, "*** Joint capabilities are %s \n", tris_getformatname_multiple(buf, GATEWAYBUFSIZE, i->jointcapability));
	tris_debug(3, "*** Our capabilities are %s \n", tris_getformatname_multiple(buf, GATEWAYBUFSIZE, i->capability));
	tris_debug(3, "*** TRIS_CODEC_CHOOSE formats are %s \n", tris_getformatname_multiple(buf, GATEWAYBUFSIZE, tris_codec_choose(&i->prefs, what, 1)));
	if (i->prefcodec)
		tris_debug(3, "*** Our preferred formats from the incoming channel are %s \n", tris_getformatname_multiple(buf, GATEWAYBUFSIZE, i->prefcodec));
	 ...
	 */
	if (i->vrtp) {
		if (tris_test_flag(&i->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT))
	...
	if ((tris_test_flag(&i->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_INBAND) || (tris_test_flag(&i->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_AUTO) ||
	    (tris_test_flag(&i->flags[1], GATEWAY_PAGE2_FAX_DETECT))) {
		int features = 0;
		...
		if (tris_test_flag(&i->flags[1], GATEWAY_PAGE2_FAX_DETECT)) {
	...
	tmp->tech_pvt = dialog_ref(i, "gateway_new: set chan->tech_pvt to i");
	 ...
	 * we should decode the uri before storing it in the channel, but leave it encoded in the gateway_pvt
	...
	if (!tris_strlen_zero(i->uri))
		pbx_builtin_setvar_helper(tmp, "GATEWAYURI", i->uri);
	if (!tris_strlen_zero(i->domain))
		pbx_builtin_setvar_helper(tmp, "GATEWAYDOMAIN", i->domain);
	if (!tris_strlen_zero(i->callid))
		pbx_builtin_setvar_helper(tmp, "GATEWAYCALLID", i->callid);
	if (i->rtp)
	...
	/* Inform manager user about new channel and their GATEWAY call ID */
	if (gateway_cfg.callevents)
		manager_event(EVENT_FLAG_SYSTEM, "ChannelUpdate",
			"Channel: %s\r\nUniqueid: %s\r\nChanneltype: %s\r\nGATEWAYcallid: %s\r\nGATEWAYfullcontact: %s\r\n",
			tmp->name, tmp->uniqueid, "GATEWAY", i->callid, i->fullcontact);

	return tmp;
}

/*! \brief Reads one line of GATEWAY message body */
...
static char *get_body(struct gateway_request *req, char *name, char delimiter) 
{
	int x;
	int len = strlen(name);
	char *r;
...
/*! \brief Find compressed GATEWAY alias */
static const char *find_alias(const char *name, const char *_default)
{
	/*! \brief Structure for conversion between compressed GATEWAY and "normal" GATEWAY */
...
static const char *__get_header(const struct gateway_request *req, const char *name, int *start)
{
	 ...
	 * Anyways, pedanticgatewaychecking controls whether we allow spaces before ':',
	...
	for (pass = 0; name && pass < 2;pass++) {
		...
		for (x = *start; x < req->headers; x++) {
			...
			if (!strncasecmp(header, name, len)) {
				...
				if (gateway_cfg.pedanticgatewaychecking)
...
/*! \brief Get header from GATEWAY request 
...
static struct tris_frame *gateway_rtp_read(struct tris_channel *ast, struct gateway_pvt *p, int *faxdetect)
{
	...
	if (f && (f->frametype == TRIS_FRAME_DTMF_BEGIN || f->frametype == TRIS_FRAME_DTMF_END) &&
	    (tris_test_flag(&p->flags[0], GATEWAY_DTMF) != GATEWAY_DTMF_RFC2833)) {
	...
	if (f && ((tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_INBAND) || tris_test_flag(&p->flags[1], GATEWAY_PAGE2_FAX_DETECT)) && p->dsp) {
		f = tris_dsp_process(p->owner, p->dsp, f);
			...
			if (f->subclass == 'f') {
				...
				if (tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_INBAND) {
...
/*! \brief Read GATEWAY RTP from channel */
static struct tris_frame *gateway_read(struct tris_channel *ast)
{
	...
	gateway_pvt_lock(p);
	fr = gateway_rtp_read(ast, p, &faxdetected);
	p->lastrtprx = time(NULL);

	/* If we detect a CNG tone and fax detection is enabled then send us off to the fax extension */
	if (faxdetected && tris_test_flag(&p->flags[1], GATEWAY_PAGE2_FAX_DETECT)) {
		tris_channel_lock(ast);
...
/*! \brief Build GATEWAY Call-ID value for a non-REGISTER transaction */
static void build_callid_pvt(struct gateway_pvt *pvt)
{
	char buf[33];
...
/*! \brief Build GATEWAY Call-ID value for a REGISTER transaction */
static void build_callid_registry(struct gateway_registry *reg, struct in_addr ourip, const char *fromdomain)
{
	char buf[33];
...
/*! \brief Make our GATEWAY dialog tag */
static void make_our_tag(char *tagbuf, size_t len)
{
...
static struct gateway_st_dlg* gateway_st_alloc(struct gateway_pvt *const p)
{
	...
	if (!(stp = tris_calloc(1, sizeof(struct gateway_st_dlg))))
		return NULL;
...
/*! \brief Allocate gateway_pvt structure, set defaults and link in the container.
				 ...
				 int useglobal_nat, const int intended_method, struct gateway_request *req)
{
	...
	if (!(p = ao2_t_alloc(sizeof(*p), gateway_destroy_fn, "allocate a dialog(pvt) struct")))
		return NULL;
	...
	if (req) {
		set_socket_transport(&p->socket, req->socket.type); /* Later in tris_gateway_ouraddrfor we need this to choose the right ip and port for the specific transport */
	} else {
		set_socket_transport(&p->socket, GATEWAY_TRANSPORT_UDP);
	}
	...
	if (intended_method != GATEWAY_OPTIONS) {	/* Peerpoke has it's own system */
		p->timer_t1 = global_t1;	/* Default GATEWAY retransmission timer T1 (RFC 3261) */
		p->timer_b = global_timer_b;	/* Default GATEWAY transaction timer B (RFC 3261) */
	}
	...
	if (!sin)
		...
		p->sa = *sin;
		tris_gateway_ouraddrfor(&p->sa.sin_addr, &p->ourip, p);
	}

	/* Copy global flags to this PVT at setup. */
	tris_copy_flags(&p->flags[0], &global_flags[0], GATEWAY_FLAGS_TO_COPY);
	tris_copy_flags(&p->flags[1], &global_flags[1], GATEWAY_PAGE2_FLAGS_TO_COPY);
	...
	if (gateway_methods[intended_method].need_rtp) {
		p->rtp = tris_rtp_new_with_bindaddr(sched, io, 1, 0, internip.sin_addr);
		/* If the global videosupport flag is on, we always create a RTP interface for video */
		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT))
			p->vrtp = tris_rtp_new_with_bindaddr(sched, io, 1, 0, internip.sin_addr);
 		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_TEXTSUPPORT))
		...
		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT))
 		...
 		if (!p->rtp|| (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT) && !p->vrtp) 
				|| (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_TEXTSUPPORT) && !p->trtp)) {
 			tris_log(LOG_WARNING, "Unable to create RTP audio %s%ssession: %s\n",
 				tris_test_flag(&p->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT) ? "and video " : "",
 				tris_test_flag(&p->flags[1], GATEWAY_PAGE2_TEXTSUPPORT) ? "and text " : "", strerror(errno));
			if (p->chanvars) {
				tris_variables_destroy(p->chanvars);
				p->chanvars = NULL;
			}
		...
		tris_rtp_setqos(p->rtp, global_tos_audio, global_cos_audio, "GATEWAY RTP");
		tris_rtp_setdtmf(p->rtp, tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_RFC2833);
		tris_rtp_setdtmfcompensate(p->rtp, tris_test_flag(&p->flags[1], GATEWAY_PAGE2_RFC2833_COMPENSATE));
		...
		if (p->vrtp) {
			tris_rtp_setqos(p->vrtp, global_tos_video, global_cos_video, "GATEWAY VRTP");
		...
		if (p->trtp) {
			tris_rtp_setqos(p->trtp, global_tos_text, global_cos_text, "GATEWAY TRTP");
	...
	if (useglobal_nat && sin) {
		...
		tris_copy_flags(&p->flags[0], &global_flags[0], GATEWAY_NAT);
		p->recv = *sin;
		do_setnat(p, tris_test_flag(&p->flags[0], GATEWAY_NAT) & GATEWAY_NAT_ROUTE);
	}

	do_setnat(p, GATEWAY_NAT_ROUTE);

	if (p->method != GATEWAY_REGISTER)
	...
	p->allowtransfer = gateway_cfg.allowtransfer;
	if ((tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_RFC2833) ||
	    (tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_AUTO))
		p->noncodeccapability |= TRIS_RTP_DTMF;
	if (p->udptl) {
		p->t38_maxdatagram = global_t38_maxdatagram;
		set_t38_capabilities(p);
	}
	tris_string_field_set(p, context, gateway_cfg.default_context);
	...
	p->expireid = tris_sched_add(sched, DEFAULT_CALL_EXPIRE_TIMEOUT, gateway_expire_timer, dialog_ref(p, "dialog ptr inc when SCHED_REPLACE add succeeded"));

	tris_debug(1, "Allocating new GATEWAY dialog for %s - %s (%s)\n", callid ? callid : p->callid, gateway_methods[intended_method].text, p->rtp ? "With RTP" : "No RTP");
	return p;
}

/*! \brief argument to the helper function to identify a call */
struct find_call_cb_arg {
	enum gatewaymethod method;
	const char *callid;
	const char *fromtag;
	const char *totag;
	const char *tag;
...
static int find_call_cb(void *__pvt, void *__arg, int flags)
{
	...
	if (!tris_strlen_zero(p->callid)) { /* XXX double check, do we allow match on empty p->callid ? */
		if (arg->method == GATEWAY_REGISTER)
 	  	  	found = (!strcmp(p->callid, arg->callid));
		else {
 	  		found = !strcmp(p->callid, arg->callid);
			if (gateway_cfg.pedanticgatewaychecking && found) {
				found = tris_strlen_zero(arg->tag) || tris_strlen_zero(p->theirtag) || !tris_test_flag(&p->flags[1], GATEWAY_PAGE2_DIALOG_ESTABLISHED) || !strcmp(p->theirtag, arg->tag);
			} 
		}
		...
		if (gateway_cfg.pedanticgatewaychecking && found && arg->method != GATEWAY_RESPONSE) { /* GATEWAY Request */
			...
			if (!found)
				tris_debug(5, "= Being pedantic: This is not our match on request: Call ID: %s Ourtag <null> Totag %s Method %s\n", p->callid, arg->totag, gateway_methods[arg->method].text);
		}
	}
 	return found;
}

/*! \brief find or create a dialog structure for an incoming GATEWAY message.
 * Connect incoming GATEWAY message to current dialog or create new dialog structure
 * Returns a reference to the gateway_pvt object, remember to give it back once done.
 *     Called by handle_incoming(), gatewaysock_read
 */
static struct gateway_pvt *find_call(struct gateway_request *req, struct sockaddr_in *sin, const int intended_method)
{
	struct gateway_pvt *p = NULL;
...
//	if (gateway_cfg.pedanticgatewaychecking) {
		/* In principle Call-ID's uniquely identify a call, but with a forking GATEWAY proxy
		   we need more to identify a branch - so we have to check branch, from
		   and to tags to identify a call leg.
		   For Trismedia to behave correctly, you need to turn on pedanticgatewaychecking
		   in gateway.conf
		   */
		...
		tag = (req->method == GATEWAY_RESPONSE) ? totag : fromtag;

		tris_debug(5, "= Looking for  Call ID: %s (Checking %s) --From tag %s --To-tag %s  \n", callid, req->method==GATEWAY_RESPONSE ? "To" : "From", fromtag, totag);

		/* All messages must always have From: tag */
		if (tris_strlen_zero(fromtag)) {
			tris_debug(5, "%s request has no from tag, dropping callid: %s from: %s\n", gateway_methods[req->method].text , callid, from );
			return NULL;
		}
		/* reject requests that must always have a To: tag */
		if (tris_strlen_zero(totag) && (req->method == GATEWAY_ACK || req->method == GATEWAY_BYE || req->method == GATEWAY_INFO )) {
...
restartsearch:
/*	if (!gateway_cfg.pedanticgatewaychecking) {
		struct gateway_pvt tmp_dialog = {
			.callid = callid,
		};			
		gateway_pvt_ptr = ao2_t_find(dialogs, &tmp_dialog, OBJ_POINTER, "ao2_find in dialogs");
		if (gateway_pvt_ptr) {  // well, if we don't find it-- what IS in there? 
			// Found the call 
			gateway_pvt_lock(gateway_pvt_ptr);
			return gateway_pvt_ptr;
		}
...
//	}
	...
	if (gateway_methods[intended_method].can_create == CAN_CREATE_DIALOG) {
		if (intended_method == GATEWAY_REFER) {
		...
		} else if (intended_method == GATEWAY_NOTIFY) {
			...
			/* Ok, time to create a new GATEWAY dialog object, a pvt */
			if ((p = gateway_alloc(callid, sin, 1, intended_method, req)))  {
				/* Ok, we've created a dialog, let's go and process it */
				gateway_pvt_lock(p);
			} else {
				/* We have a memory or file/socket error (can't allocate RTP sockets or something) so we're not
					getting a dialog from gateway_alloc. 
				...
				*/
				...
				tris_debug(4, "Failed allocating GATEWAY dialog, sending 500 Server internal error and giving up\n");
			}
		}
		return p; /* can be NULL */
	} else if( gateway_methods[intended_method].can_create == CAN_CREATE_DIALOG_UNSUPPORTED_METHOD) {
		/* A method we do not support, let's take it on the volley */
		transmit_response_using_temp(callid, sin, 1, intended_method, req, "501 Method Not Implemented");
		tris_debug(2, "Got a request with unsupported GATEWAY method.\n");
	} else if (intended_method != GATEWAY_RESPONSE && intended_method != GATEWAY_ACK) {
	...
	/* We do not respond to responses for dialogs that we don't know about, we just drop
	   the session quickly */
	if (intended_method == GATEWAY_RESPONSE)
		tris_debug(2, "That's odd...  Got a response on a call we dont know about. Callid %s\n", callid ? callid : "<unknown>");

	return NULL;
}

/*! \brief Parse register=> line in gateway.conf and add to registry */
static int gateway_register(const char *value, int lineno)
{
	struct gateway_registry *reg;
	int portnum = 0;
	enum gateway_transport transport = GATEWAY_TRANSPORT_UDP;
	char buf[256] = "";
	...
	if (host3.port) {
		if (!(portnum = port_str2int(host3.port, 0))) {
			tris_log(LOG_NOTICE, "'%s' is not a valid port number on line %d of gateway.conf. using default.\n", host3.port, lineno);
		}
	}

	/* set transport type */
	if (!pre2.transport) {
		transport = GATEWAY_TRANSPORT_UDP;
	} else if (!strncasecmp(pre2.transport, "tcp", 3)) {
		transport = GATEWAY_TRANSPORT_TCP;
	} else if (!strncasecmp(pre2.transport, "tls", 3)) {
		transport = GATEWAY_TRANSPORT_TLS;
	} else if (!strncasecmp(pre2.transport, "udp", 3)) {
		transport = GATEWAY_TRANSPORT_UDP;
	} else {
		transport = GATEWAY_TRANSPORT_UDP;
		tris_log(LOG_NOTICE, "'%.3s' is not a valid transport type on line %d of gateway.conf. defaulting to udp.\n", pre2.transport, lineno);
	}

	/* if no portnum specified, set default for transport */
	if (!portnum) {
		if (transport == GATEWAY_TRANSPORT_TLS) {
			portnum = STANDARD_TLS_PORT;
		} else {
			portnum = STANDARD_GATEWAY_PORT;
		}
	}

	if (!(reg = tris_calloc(1, sizeof(*reg)))) {
		tris_log(LOG_ERROR, "Out of memory. Can't allocate GATEWAY registry entry\n");
		return -1;
	}

	if (tris_string_field_init(reg, 256)) {
		tris_log(LOG_ERROR, "Out of memory. Can't allocate GATEWAY registry strings\n");
		tris_free(reg);
		return -1;
	}
...
/*! \brief Parse mwi=> line in gateway.conf and add to list */
static int gateway_subscribe_mwi(const char *value, int lineno)
{
	struct gateway_subscription_mwi *mwi;
	int portnum = 0;
	enum gateway_transport transport = GATEWAY_TRANSPORT_UDP;
	char buf[256] = "";
	...
	gateway_parse_host(buf, lineno, &username, &portnum, &transport);
	...
	ASTOBJ_UNREF(mwi, gateway_subscribe_mwi_destroy);
	
	return 0;
}

/*! \brief  Parse multiline GATEWAY headers into one header
	This is enabled if pedanticgatewaychecking is enabled */
static int lws2sws(char *msgbuf, int len) 
{
	int h = 0, t = 0; 
	int lws = 0; 
...
static int parse_request(struct gateway_request *req)
{
	...
	int i = 0, lim = GATEWAY_MAX_HEADERS - 1;
...
/*!
  \brief Determine whether a GATEWAY message contains an SDP in its body
  \param req the GATEWAY request to process
...
static int get_ip_and_port_from_sdp(struct gateway_request *req, const enum media_type media, struct sockaddr_in *sin)
{
	const char *m;
	const char *c;
...
/*! \brief Process GATEWAY SDP offer, select formats and activate RTP channels
...
static int process_sdp(struct gateway_pvt *p, struct gateway_request *req, int t38action)
{
	...
	if (debug) {
		...
		char s1[GATEWAYBUFSIZE], s2[GATEWAYBUFSIZE], s3[GATEWAYBUFSIZE], s4[GATEWAYBUFSIZE], s5[GATEWAYBUFSIZE];

		tris_verbose("Capabilities: us - %s, peer - audio=%s/video=%s/text=%s, combined - %s\n",
			    tris_getformatname_multiple(s1, GATEWAYBUFSIZE, p->capability),
			    tris_getformatname_multiple(s2, GATEWAYBUFSIZE, peercapability),
			    tris_getformatname_multiple(s3, GATEWAYBUFSIZE, vpeercapability),
			    tris_getformatname_multiple(s4, GATEWAYBUFSIZE, tpeercapability),
			    tris_getformatname_multiple(s5, GATEWAYBUFSIZE, newjointcapability));

		tris_verbose("Non-codec capabilities (dtmf): us - %s, peer - %s, combined - %s\n",
			    tris_rtp_lookup_mime_multiple(s1, GATEWAYBUFSIZE, p->noncodeccapability, 0, 0),
			    tris_rtp_lookup_mime_multiple(s2, GATEWAYBUFSIZE, peernoncodeccapability, 0, 0),
			    tris_rtp_lookup_mime_multiple(s3, GATEWAYBUFSIZE, newnoncodeccapability, 0, 0));
	}
	...
	if (p->rtp) {
		if (portno > 0) {
			...
			/* We are now ready to change the gateway session and p->rtp and p->vrtp with the offered codecs, since                                                                                                    
			   they are acceptable */
			...
			if (tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_AUTO) {
				tris_clear_flag(&p->flags[0], GATEWAY_DTMF);
				if (newnoncodeccapability & TRIS_RTP_DTMF) {
					/* XXX Would it be reasonable to drop the DSP at this point? XXX */
					tris_set_flag(&p->flags[0], GATEWAY_DTMF_RFC2833);
					/* Since RFC2833 is now negotiated we need to change some properties of the RTP stream */
					tris_rtp_setdtmf(p->rtp, 1);
					tris_rtp_setdtmfcompensate(p->rtp, tris_test_flag(&p->flags[1], GATEWAY_PAGE2_RFC2833_COMPENSATE));
				} else {
					tris_set_flag(&p->flags[0], GATEWAY_DTMF_INBAND);
				}
			}
	...
	if (p->udptl) {
		if (udptlportno > 0) {
			...
			if (tris_test_flag(&p->flags[0], GATEWAY_NAT) && tris_test_flag(&p->flags[1], GATEWAY_PAGE2_UDPTL_DESTINATION)) {
	...
	tris_debug(2, "We're settling with these formats: %s\n", tris_getformatname_multiple(buf, GATEWAYBUFSIZE, p->jointcapability));
	...
	if (!(p->owner->nativeformats & p->jointcapability) && (p->jointcapability & TRIS_FORMAT_AUDIO_MASK)) {
		if (debug) {
			char s1[GATEWAYBUFSIZE], s2[GATEWAYBUFSIZE];
			tris_debug(1, "Oooh, we need to change our audio formats since our peer supports only %s and not %s\n", 
				tris_getformatname_multiple(s1, GATEWAYBUFSIZE, p->jointcapability),
				tris_getformatname_multiple(s2, GATEWAYBUFSIZE, p->owner->nativeformats));
		}
	...
	if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD) && sin.sin_addr.s_addr && (!sendonly || sendonly == -1)) {
		...
		if (gateway_cfg.callevents)
			manager_event(EVENT_FLAG_CALL, "Hold",
				      ...
				      p->owner->name,
				      p->owner->uniqueid);
		if (gateway_cfg.notifyhold)
			gateway_peer_hold(p, FALSE);
		tris_clear_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD); /* Clear both flags */
	} else if (!sin.sin_addr.s_addr || (sendonly && sendonly != -1)) {
		int already_on_hold = tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD);
		...
		if (gateway_cfg.callevents && !tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD)) {
			manager_event(EVENT_FLAG_CALL, "Hold",
				      "Status: On\r\n"
				      "Channel: %s\r\n"
				      "Uniqueid: %s\r\n",
		...
		if (sendonly == 1)	/* One directional hold (sendonly/recvonly) */
			tris_set_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD_ONEDIR);
		else if (sendonly == 2)	/* Inactive stream */
			tris_set_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD_INACTIVE);
		else
			tris_set_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD_ACTIVE);
		if (gateway_cfg.notifyhold && !already_on_hold)
			gateway_peer_hold(p, TRUE);
	}
...
static int process_sdp_o(const char *o, struct gateway_pvt *p)
{
	...
	if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_IGNORESDPVERSION) ||
	...
	if (!strncasecmp(a, "ptime", 5)) {
		...
		if (*last_rtpmap_codec < SDP_MAX_RTPMAP_CODECS) {
			if (tris_rtp_set_rtpmap_type_rate(newaudiortp, codec, "audio", mimeSubtype,
			    tris_test_flag(&p->flags[0], GATEWAY_G726_NONSTANDARD) ? TRIS_RTP_OPT_G726_NONSTANDARD : 0, sample_rate) != -1) {
				if (debug)
...
/*! \brief Add header to GATEWAY message */
static int add_header(struct gateway_request *req, const char *var, const char *value)
{
	if (req->headers == GATEWAY_MAX_HEADERS) {
		tris_log(LOG_WARNING, "Out of GATEWAY header space\n");
		return -1;
	}
...
/*! \brief Add 'Content-Length' header to GATEWAY message */
static int add_header_contentLength(struct gateway_request *req, int len)
{
	char clen[10];
...
/*! \brief Add content (not header) to GATEWAY message */
static int add_line(struct gateway_request *req, const char *line)
{
	if (req->lines == GATEWAY_MAX_LINES)  {
		tris_log(LOG_WARNING, "Out of GATEWAY line space\n");
		return -1;
	}
	if (!req->lines)
...
/*! \brief Copy GATEWAY VIA Headers from the request to the response
...
static int copy_via_headers(struct gateway_pvt *p, struct gateway_request *req, const struct gateway_request *orig, const char *field)
{
	...
	for (;;) {
		...
		if (!copied) {	/* Only check for empty rport in topmost via header */
			...
			if (rport && ((tris_test_flag(&p->flags[0], GATEWAY_NAT) == GATEWAY_NAT_ALWAYS) || (tris_test_flag(&p->flags[0], GATEWAY_NAT) == GATEWAY_NAT_RFC3581))) {
...
/*! \brief Set destination from GATEWAY URI 
 *
 * Parse uri to h (host) and port - uri is already just the part inside the <> 
 * general form we are expecting is gateway[s]:username[:password][;parameter]@host[:port][;...] 
 * If there's a port given, turn NAPTR/SRV off. NAPTR might indicate GATEWAYS preference even
 * for GATEWAY: uri's
 *
...
static void set_destination(struct gateway_pvt *p, char *uri)
{
	...
	int debug=gateway_debug_test_pvt(p);
	int tls_on = FALSE;
	int use_dns = gateway_cfg.srvlookup;
	...
	/* Is "port" present? if not default to STANDARD_GATEWAY_PORT */
	if (*h == ':') {
		/* Parse port */
		++h;
		port = strtol(h, &h, 10);
		...
		port = tls_on ? STANDARD_TLS_PORT : STANDARD_GATEWAY_PORT;
...
/*! \brief Initialize GATEWAY response, based on GATEWAY request */
static int init_resp(struct gateway_request *resp, const char *msg)
{
	/* Initialize a response */
	memset(resp, 0, sizeof(*resp));
	resp->method = GATEWAY_RESPONSE;
	if (!(resp->data = tris_str_create(GATEWAY_MIN_PACKET)))
		return -1;
	resp->header[0] = 0;
...
/*! \brief Initialize GATEWAY request */
static int init_req(struct gateway_request *req, int gatewaymethod, const char *recip)
{
	...
	if (!(req->data = tris_str_create(GATEWAY_MIN_PACKET)))
		return -1;
	req->method = gatewaymethod;
	req->header[0] = 0;
	tris_str_set(&req->data, 0, "%s %s SIP/2.0\r\n", gateway_methods[gatewaymethod].text, recip);
	req->len = tris_str_strlen(req->data);
	req->headers++;
	return 0;
}
...
/*! \brief Prepare GATEWAY response packet */
static int respprep(struct gateway_request *resp, struct gateway_pvt *p, const char *msg, const struct gateway_request *req)
{
	char newto[2048];
	const char *ot;
	...
	if (!strcasestr(ot, "tag=") && strncmp(msg, "100", 3)) {
		...
		if (!tris_strlen_zero(p->theirtag) && tris_test_flag(&p->flags[0], GATEWAY_OUTGOING))
			snprintf(newto, sizeof(newto), "%s;tag=%s", ot, p->theirtag);
		else if (p->tag && !tris_test_flag(&p->flags[0], GATEWAY_OUTGOING))
	...
	if (p->method == GATEWAY_INVITE && p->stimer && p->stimer->st_active == TRUE && p->stimer->st_active_peer_ua == TRUE) {
		char se_hdr[256];
	...
	if (msg[0] == '2' && (p->method == GATEWAY_SUBSCRIBE || p->method == GATEWAY_REGISTER)) {
		...
		if (p->expiry) {	/* Only add contact if we have an expiry time */
			char contact[GATEWAYBUFSIZE];
			const char *contact_uri = p->method == GATEWAY_SUBSCRIBE ? p->our_contact : p->fullcontact;
...
/*! \brief Initialize a GATEWAY request message (not the initial one in a dialog) */
static int reqprep(struct gateway_request *req, struct gateway_pvt *p, int gatewaymethod, int seqno, int newbranch)
{
	struct gateway_request *orig = &p->initreq;
	char stripped[80];
	char tmp[80];
	char newto[2048];
	const char *c;
	...
	int is_outbound = tris_test_flag(&p->flags[0], GATEWAY_OUTGOING);	/* Session direction */

	memset(req, 0, sizeof(struct gateway_request));
	
	snprintf(p->lastmsg, sizeof(p->lastmsg), "Tx: %s", gateway_methods[gatewaymethod].text);
	...
	if (gatewaymethod == GATEWAY_CANCEL) {
		p->branch = p->invite_branch;
		build_via(p);
	} else if (newbranch && (gatewaymethod == GATEWAY_INVITE)) {
		p->branch ^= tris_random();
	...
	else if (gatewaymethod == GATEWAY_ACK) {
	...
	init_req(req, gatewaymethod, c);

	snprintf(tmp, sizeof(tmp), "%d %s", seqno, gateway_methods[gatewaymethod].text);
	...
	if (!strcasestr(ot, "tag=") && gatewaymethod != GATEWAY_CANCEL) {
		/* Add the proper tag if we don't have it already.  If they have specified
		   their tag, use it.  Otherwise, use our own tag */
	...
	if (gatewaymethod != GATEWAY_BYE && gatewaymethod != GATEWAY_CANCEL && gatewaymethod != GATEWAY_MESSAGE)
...
static int __transmit_response(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, enum xmittype reliable)
{
	...
	if (p->method == GATEWAY_INVITE && msg[0] != '1' && p->owner && p->owner->hangupcause) {
		char buf[10];
...
static int transmit_response_with_minse(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, int minse_int)
{
	struct gateway_request resp;
	char minse_str[20];
...
/*! \brief Append date to GATEWAY message */
static void append_date(struct gateway_request *req)
{
	char tmpdat[256];
	struct tm tm;
...
static int transmit_response_with_auth(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, const char *randdata, enum xmittype reliable, const char *header, int stale)
{
	struct gateway_request resp;
	char tmp[512];
	int seqno = 0;
	...
	snprintf(tmp, sizeof(tmp), "Digest algorithm=MD5, realm=\"%s\", nonce=\"%s\"%s", gateway_cfg.realm, randdata, stale ? ", stale=true" : "");
	respprep(&resp, p, msg, req);
...
/*! \brief Add text body to GATEWAY message */
static int add_text(struct gateway_request *req, const char *text)
{
	/* XXX Convert \n's to \r\n's XXX */
...
/*! \brief Add DTMF INFO tone to gateway message 
			     ...
			     int debug, int *min_packet_size)
{
	...
	tris_str_append(a_buf, 0, "a=rtpmap:%d %s/%d\r\n", rtp_code,
		       tris_rtp_lookup_mime_subtype(1, codec,
						   tris_test_flag(&p->flags[0], GATEWAY_G726_NONSTANDARD) ? TRIS_RTP_OPT_G726_NONSTANDARD : 0),
...
static void add_vcodec_to_sdp(const struct gateway_pvt *p, int codec,
			     struct tris_str **m_buf, struct tris_str **a_buf,
			     int debug, int *min_packet_size)
{
	int rtp_code;
...
static void add_tcodec_to_sdp(const struct gateway_pvt *p, int codec,
			     struct tris_str **m_buf, struct tris_str **a_buf,
			     int debug, int *min_packet_size)
{
	int rtp_code;
...
*/
static void get_our_media_address(struct gateway_pvt *p, int needvideo,
...
/*! \brief Add Session Description Protocol message 
    ...
    is used in Session-Timers where RE-INVITEs are used for refreshing GATEWAY sessions 
...
static enum gateway_result add_sdp(struct gateway_request *resp, struct gateway_pvt *p, int oldsdp, int add_audio, int add_t38)
{
	...
	/* XXX We should not change properties in the GATEWAY dialog until 
	...
	if (add_audio) {
		...
		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD) == GATEWAY_PAGE2_CALL_ONHOLD_ONEDIR)
			hold = "a=recvonly\r\n";
		else if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CALL_ONHOLD) == GATEWAY_PAGE2_CALL_ONHOLD_INACTIVE)
			hold = "a=inactive\r\n";
		else
			hold = "a=sendrecv\r\n";
		...
		/* Now, start adding audio codecs. These are added in this order:
		   ...
		   - Then preferences in order from gateway.conf device config for this peer/user
		...
		*/
		...
		if (m_audio->len - m_audio->used < 2 || m_video->len - m_video->used < 2 ||
			...
			tris_log(LOG_WARNING, "GATEWAY SDP may be truncated due to undersized buffer!!\n");
	}
...
/*! \brief copy GATEWAY request (mostly used to save request for responses) */
static void copy_request(struct gateway_request *dst, const struct gateway_request *src)
{
...
static int transmit_response_with_sdp(struct gateway_pvt *p, const char *msg, const struct gateway_request *req, enum xmittype reliable, int oldsdp)
{
	...
	respprep(&resp, p, msg, req);
	if (p->rtp) {
		if (!p->autoframing && !tris_test_flag(&p->flags[0], GATEWAY_OUTGOING)) {
...
/*! \brief Parse first line of incoming GATEWAY request */
static int determine_firstline_parts(struct gateway_request *req) 
{
...
\note 	A re-invite is basically a new INVITE with the same CALL-ID and TAG as the
	INVITE that opened the GATEWAY dialogue 
	We reinvite so that the audio stream (RTP) go directly between
	the GATEWAY UAs. GATEWAY Signalling stays with * in the path.
    ...
    GATEWAY session without modifying the media session. 
*/
static int transmit_reinvite_with_sdp(struct gateway_pvt *p, int t38version, int oldsdp)
{
	struct gateway_request req;
	
	reqprep(&req, p, tris_test_flag(&p->flags[0], GATEWAY_REINVITE_UPDATE) ?  GATEWAY_UPDATE : GATEWAY_INVITE, 0, 1);

	add_header(&req, "Allow", ALLOWED_METHODS);
	add_header(&req, "Supported", SUPPORTED_EXTENSIONS);
	if (gatewaydebug) {
		if (oldsdp == TRUE)
			add_header(&req, "X-trismedia-Info", "GATEWAY re-invite (Session-Timers)");
		else
			add_header(&req, "X-trismedia-Info", "GATEWAY re-invite (External RTP bridge)");
	}
	...
	tris_set_flag(&p->flags[0], GATEWAY_OUTGOING);       /* Change direction of this dialog */
...
/*! \brief Check Contact: URI of GATEWAY message */
static void extract_uri(struct gateway_pvt *p, struct gateway_request *req)
{
	char *stripped;
	char *c;
...
static void build_contact(struct gateway_pvt *p)
{
	...
	if (tris_strlen_zero(p->exten) || strncmp(p->exten, "VOOLGW", 6)) {
		if (!gateway_standard_port(p->socket.type, ourport)) {
			if (p->socket.type == GATEWAY_TRANSPORT_UDP)
...
/*! \brief Initiate new GATEWAY request to peer/user */
static void initreqprep(struct gateway_request *req, struct gateway_pvt *p, int gatewaymethod)
{
	struct tris_str *invite = tris_str_alloca(256);
	char from[256];
	char to[256];
	char tmp_n[GATEWAYBUFSIZE/2];	/* build a local copy of 'n' if needed */
	char tmp_l[GATEWAYBUFSIZE/2];	/* build a local copy of 'l' if needed */
	...
	if (tris_test_flag(&p->flags[0], GATEWAY_USEREQPHONE)) {
	 	const char *s = p->username;	/* being a string field, cannot be NULL */

		/* Test p->username against allowed characters in TRIS_DIGIT_ANY
			If it matches the allowed characters list, then gatewayuser = ";user=phone"
			If not, then gatewayuser = ""
		*/
	...
	snprintf(p->lastmsg, sizeof(p->lastmsg), "Init: %s", gateway_methods[gatewaymethod].text);
	...
	if (!tris_test_flag(&p->flags[0], GATEWAY_SENDRPID) &&
	...
	if (!gateway_standard_port(p->socket.type, ourport) && tris_strlen_zero(p->fromdomain))
		snprintf(from, sizeof(from), "\"%s\" <sip:%s@%s:%d>;tag=%s", n, l, d, ourport, p->tag);
	else
		snprintf(from, sizeof(from), "\"%s\" <sip:%s@%s>;tag=%s", n, l, d, p->tag);

	/* If we're calling a registered GATEWAY peer, use the fullcontact to dial to the peer */
	...
	tris_string_field_set(p, uri, invite->str);
 	...
 	if (!tris_strlen_zero(p->todnid)) {
 		...
 		if (gatewaymethod == GATEWAY_NOTIFY && !tris_strlen_zero(p->theirtag)) { 
 			/* If this is a NOTIFY, use the From: tag in the subscribe (RFC 3265) */
			snprintf(to, sizeof(to), "<%s%s>;tag=%s", (!strncasecmp(p->uri, "sip:", 4) ? "sip:" : ""), p->uri, p->theirtag);
 		} else if (p->options && p->options->vxml_url) {
 			/* If there is a VXML URL append it to the GATEWAY URL */
 			snprintf(to, sizeof(to), "<%s>;%s", p->uri, p->options->vxml_url);
 		} else 
 			snprintf(to, sizeof(to), "<%s>", p->uri);
 	}

	init_req(req, gatewaymethod, p->uri);
	/* now tmp_n is available so reuse it to build the CSeq */
	snprintf(tmp_n, sizeof(tmp_n), "%d %s", ++p->ocseq, gateway_methods[gatewaymethod].text);
	...
	if (tris_test_flag(&p->flags[0], GATEWAY_SENDRPID) && (gatewaymethod == GATEWAY_INVITE)) {
		build_rpid(p);
...
/*! \brief Build REFER/INVITE/OPTIONS/SUBSCRIBE message and transmit it 
 ...
 \param p gateway_pvt structure
 \param sdp unknown 
 \param gatewaymethod unknown 
...
static int transmit_invite(struct gateway_pvt *p, int gatewaymethod, int sdp, int init)
{
	...
	if (init > 1)
		initreqprep(&req, p, gatewaymethod);
	else
		/* If init=1, we should not generate a new branch. If it's 0, we need a new branch. */
		reqprep(&req, p, gatewaymethod, 0, init ? 0 : 1);
		
	if (p->options && p->options->auth)
		add_header(&req, p->options->authheader, p->options->auth);
	append_date(&req);
	if (gatewaymethod == GATEWAY_REFER) {	/* Call transfer */
		if (p->refer) {
			char buf[GATEWAYBUFSIZE];
	...
	} else if (gatewaymethod == GATEWAY_SUBSCRIBE) { /* We only support sending MWI subscriptions right now */
		char buf[GATEWAYBUFSIZE];
	...
	if (p->options && p->options->addgatewayheaders && p->owner) {
		...
		if (!headp)
			...
			TRIS_LIST_TRAVERSE(headp, current, entries) {  
				/* GATEWAYADDHEADER: Add GATEWAY header to outgoing call */
				if (!strncasecmp(tris_var_name(current), "GATEWAYADDHEADER", strlen("GATEWAYADDHEADER"))) {
					char *content, *end;
					...
					if ((content = strchr(headdup, ':'))) {
						*content++ = '\0';
						...
						if (gatewaydebug)
							tris_debug(1, "Adding GATEWAY Header \"%s\" with content :%s: \n", headdup, content);
					}
				}
			}
		}
...
static int gateway_subscribe_mwi_do(const void *data)
{
	struct gateway_subscription_mwi *mwi = (struct gateway_subscription_mwi*)data;
	...
	mwi->resub = -1;
	__gateway_subscribe_mwi_do(mwi);
	ASTOBJ_UNREF(mwi, gateway_subscribe_mwi_destroy);
...
static int __gateway_subscribe_mwi_do(struct gateway_subscription_mwi *mwi)
{
	...
	if (!mwi->dnsmgr) {
		...
		snprintf(transport, sizeof(transport), "_gateway._%s", get_transport(mwi->transport));
		tris_dnsmgr_lookup(mwi->hostname, &mwi->us, &mwi->dnsmgr, gateway_cfg.srvlookup ? transport : NULL);
	}

	/* If we already have a subscription up simply send a resubscription */
	if (mwi->call) {
		transmit_invite(mwi->call, GATEWAY_SUBSCRIBE, 0, 0);
		return 0;
	}
	
	/* Create a dialog that we will use for the subscription */
	if (!(mwi->call = gateway_alloc(NULL, NULL, 0, GATEWAY_SUBSCRIBE, NULL))) {
		return -1;
	}
	...
	tris_gateway_ouraddrfor(&mwi->call->sa.sin_addr, &mwi->call->ourip, mwi->call);
	build_contact(mwi->call);
	build_via(mwi->call);
	build_callid_pvt(mwi->call);
	tris_set_flag(&mwi->call->flags[0], GATEWAY_OUTGOING);
	
	/* Associate the call with us */
	mwi->call->mwi = ASTOBJ_REF(mwi);
	
	/* Actually send the packet */
	transmit_invite(mwi->call, GATEWAY_SUBSCRIBE, 0, 2);
...
static int find_calling_channel(struct tris_channel *c, void *data) {
	...
	return (c->pbx &&
			...
			(gateway_cfg.notifycid == IGNORE_CONTEXT || !strcasecmp(c->context, p->context)));
}
...
static int transmit_state_notify(struct gateway_pvt *p, int state, int full, int timeout)
{
	...
	case (TRIS_EXTENSION_RINGING | TRIS_EXTENSION_INUSE):
		statestring = (gateway_cfg.notifyringing) ? "early" : "confirmed";
	...
	if (strncasecmp(c, "sip:", 4) && strncasecmp(c, "sips:", 5)) {
		tris_log(LOG_WARNING, "Huh?  Not a GATEWAY header (%s)?\n", c);
		return -1;
	}
	...
	reqprep(&req, p, GATEWAY_NOTIFY, 0, 1);
	...
	case DIALOG_INFO_XML: /* SNOM subscribes in this format */
		...
		if ((state & TRIS_EXTENSION_RINGING) && gateway_cfg.notifyringing) {
		...
		if (state == TRIS_EXTENSION_ONHOLD) {
			tris_str_append(&tmp, 0, "<local>\n<target uri=\"%s\">\n"
			                                "<param pname=\"+gateway.rendering\" pvalue=\"no\"/>\n"
			                                "</target>\n</local>\n", mto);
		}
...
\note	- Notification only works for registered peers with mailbox= definitions
	in gateway.conf
	- We use the GATEWAY Event package message-summary
...
static int transmit_notify_with_mwi(struct gateway_pvt *p, int newmsgs, int oldmsgs, const char *vmexten)
{
	...
	initreqprep(&req, p, GATEWAY_NOTIFY);
	add_header(&req, "Event", "message-summary");
	...
	} else if (!gateway_standard_port(p->socket.type, ourport)) {
		if (p->socket.type == GATEWAY_TRANSPORT_UDP) {
	...
	/* Cisco has a bug in the GATEWAY stack where it can't accept the
		(0/0) notification. This can temporarily be disabled in
		gateway.conf with the "buggymwi" option */
	tris_str_append(&out, 0, "Voice-Message: %d/%d%s\r\n",
		newmsgs, oldmsgs, (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_BUGGY_MWI) ? "" : " (0/0)"));
...
/*! \brief Notify device with custom headers from gateway_notify.conf */
static int transmit_notify_custom(struct gateway_pvt *p, struct tris_variable *vars) {
	struct gateway_request req;
...
static int manager_gatewaynotify(struct mansession *s, const struct message *m)
{
	...
	if (tris_strlen_zero(channame)) {
		astman_send_error(s, m, "GATEWAYNotify requires a channel name");
		return 0;
	}

	if (!strncasecmp(channame, "gateway/", 4)) {
		channame += 4;
	}

	if (!(p = gateway_alloc(NULL, NULL, 0, GATEWAY_NOTIFY, NULL))) {
		astman_send_error(s, m, "Unable to build gateway pvt data for notify (memory/socket error)");
		return 0;
	}
	...
	dialog_ref(p, "bump the count of p, which transmit_gateway_request will decrement.");
	gateway_scheddestroy(p, GATEWAY_TRANS_TIMEOUT);
...
static char mandescr_gatewaynotify[] =
"Description: Sends a GATEWAY Notify event\n"
...
static const char *regstate2str(enum gatewayregistrystate regstate)
{
	return map_x_s(regstatestrings, regstate, "Unknown");
}

/*! \brief Update registration with GATEWAY Proxy.
 * Called from the scheduler when the previous registration expires,
 * so we don't have to cancel the pending event.
 * We assume the reference so the gateway_registry is valid, since it
 * is stored in the scheduled event anyways.
 */
static int gateway_reregister(const void *data) 
{
	/* if we are here, we know that we need to reregister. */
	struct gateway_registry *r= (struct gateway_registry *) data;
	...
	__gateway_do_register(r);
	registry_unref(r, "unref the re-register scheduled event");
	return 0;
}

/*! \brief Register with GATEWAY proxy */
static int __gateway_do_register(struct gateway_registry *r)
{
	int res;

	res = transmit_register(r, GATEWAY_REGISTER, NULL, NULL);
	return res;
}
...
static int gateway_reg_timeout(const void *data)
{
	...
	struct gateway_registry *r = (struct gateway_registry *)data; /* the ref count should have been bumped when the sched item was added */
	struct gateway_pvt *p;
	int res;
	...
	if (r->call) {
		/* Unlink us, destroy old call.  Locking is not relevant here because all this happens
		   in the single GATEWAY manager thread. */
		p = r->call;
		gateway_pvt_lock(p);
	...
	manager_event(EVENT_FLAG_SYSTEM, "Registry", "ChannelType: GATEWAY\r\nUsername: %s\r\nDomain: %s\r\nStatus: %s\r\n", r->username, r->hostname, regstate2str(r->regstate));
	registry_unref(r, "unreffing registry_unref r");
	return 0;
}

/*! \brief Transmit register to GATEWAY proxy or UA
 * auth = NULL on the initial registration (from gateway_reregister())
 */
static int transmit_register(struct gateway_registry *r, int gatewaymethod, const char *auth, const char *authheader)
{
	struct gateway_request req;
	...
	if (r->dnsmgr == NULL) {
		...
		snprintf(transport, sizeof(transport), "_gateway._%s", get_transport(r->transport)); /* have to use static get_transport function */
		tris_dnsmgr_lookup(peer ? peer->tohost : r->hostname, &r->us, &r->dnsmgr, gateway_cfg.srvlookup ? transport : NULL);
		if (peer) {
	...
	if (r->call) {	/* We have a registration */
		...
		/* Allocate GATEWAY dialog for registration */
		if (!(p = gateway_alloc( r->callid, NULL, 0, GATEWAY_REGISTER, NULL))) {
		...
		if (create_addr(p, r->hostname, &r->us, 0)) {
			...
			if (r->timeout > -1) {
				TRIS_SCHED_REPLACE_UNREF(r->timeout, sched, global_reg_timeout * 1000, gateway_reg_timeout, r,
				...
				r->timeout = tris_sched_add(sched, global_reg_timeout * 1000, gateway_reg_timeout, registry_addref(r, "add for REPLACE registry ptr"));
		...
		tris_set_flag(&p->flags[0], GATEWAY_OUTGOING);	/* Registration is outgoing call */
		r->call = dialog_ref(p, "copying dialog into registry r->call");		/* Save pointer to GATEWAY dialog */
		...
		if (r->transport == GATEWAY_TRANSPORT_TLS || r->transport == GATEWAY_TRANSPORT_TCP) {
			p->socket.port = gateway_tcp_desc.local_address.sin_port;
		}
	...
	if (!tris_strlen_zero(p->fromdomain)) {
		...
		if (domainport) {			
			if (atoi(domainport) != STANDARD_GATEWAY_PORT)
				snprintf(addr, sizeof(addr), "sip:%s:%s", p->fromdomain, domainport);
			else
				snprintf(addr, sizeof(addr), "sip:%s", p->fromdomain);
		} else {
			if (r->portno && r->portno != STANDARD_GATEWAY_PORT)
	...
	init_req(&req, gatewaymethod, addr);

	/* Add to CSEQ */
	snprintf(tmp, sizeof(tmp), "%u %s", ++r->ocseq, gateway_methods[gatewaymethod].text);
	p->ocseq = r->ocseq;
	...
	if (auth) 	/* Add auth header */
		...
		if(!build_reply_digest(p, gatewaymethod, digest, sizeof(digest)))
...
/*! \brief Transmit text with GATEWAY MESSAGE method */
static int transmit_message_with_text(struct gateway_pvt *p, const char *text)
{
	struct gateway_request req;
	
	reqprep(&req, p, GATEWAY_MESSAGE, 0, 1);
	add_text(&req, text);
	return send_request(p, &req, XMIT_RELIABLE, p->ocseq);
}

/*! \brief Allocate GATEWAY refer structure */
static int gateway_refer_allocate(struct gateway_pvt *p)
{
	p->refer = tris_calloc(1, sizeof(struct gateway_refer)); 
	return p->refer ? 1 : 0;
}

/*! \brief Transmit GATEWAY REFER message (initiated by the transfer() dialplan application
...
static int transmit_refer(struct gateway_pvt *p, const char *dest)
{
	...
	if (gatewaydebug)
		tris_debug(1, "GATEWAY transfer of %s to %s\n", p->callid, dest);
	...
	if (!strncasecmp(of, "sip:", 4)) {
		...
		tris_log(LOG_NOTICE, "From address missing 'gateway(s):', assuming sip:\n");
	}
	...
	if ((c = strchr(dest, '@')))
		...
		*c++ = '\0';
	if (c) 
		snprintf(referto, sizeof(referto), "<gateway%s:%s@%s>", use_tls ? "s" : "", dest, c);
	else
		snprintf(referto, sizeof(referto), "<gateway%s:%s>", use_tls ? "s" : "", dest);

	/* save in case we get 407 challenge */
	gateway_refer_allocate(p);
	...
	reqprep(&req, p, GATEWAY_REFER, 0, 1);
...
/*! \brief Send GATEWAY INFO dtmf message, see Cisco documentation on cisco.com */
static int transmit_info_with_digit(struct gateway_pvt *p, const char digit, unsigned int duration)
{
	struct gateway_request req;
	
	reqprep(&req, p, GATEWAY_INFO, 0, 1);
	add_digit(&req, digit, duration, (tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_SHORTINFO));
	return send_request(p, &req, XMIT_RELIABLE, p->ocseq);
}

/*! \brief Send GATEWAY INFO with video update request */
static int transmit_info_with_vidupdate(struct gateway_pvt *p)
{
	struct gateway_request req;
...
/*! \brief Transmit generic GATEWAY request 
	returns XMIT_ERROR if transmit failed with a critical error (don't retry)
*/
static int transmit_request(struct gateway_pvt *p, int gatewaymethod, int seqno, enum xmittype reliable, int newbranch)
{
	struct gateway_request resp;
	
	if (gatewaymethod == GATEWAY_ACK)
		p->invitestate = INV_CONFIRMED;

	reqprep(&resp, p, gatewaymethod, seqno, newbranch);
	if (gatewaymethod == GATEWAY_CANCEL && p->answered_elsewhere) 
		add_header(&resp, "Reason", "GATEWAY;cause=200;text=\"Call completed elsewhere\"");
...
static void auth_headers(enum gateway_auth_type code, char **header, char **respheader)
{
...
/*! \brief Transmit GATEWAY request, auth added */
static int transmit_request_with_auth(struct gateway_pvt *p, int gatewaymethod, int seqno, enum xmittype reliable, int newbranch)
{
	struct gateway_request resp;
	...
	if (!tris_strlen_zero(p->realm)) {
		...
		if(!build_reply_digest(p, gatewaymethod, digest, sizeof(digest))) {
			char *dummy, *response;
			enum gateway_auth_type code = p->options ? p->options->auth_type : PROXY_AUTH; /* XXX force 407 if unknown */
...
{
	int realtimeregs = tris_check_realtime("gatewayregs");
	char *tablename = (realtimeregs) ? "gatewayregs" : "gatewaypeers";

	if (!gateway_cfg.ignore_regexpire) {
		if (peer->rt_fromcontact && gateway_cfg.peer_rtupdate) {
...
/*! \brief Expire registration of GATEWAY peer */
static int expire_register(const void *data)
{
	struct gateway_peer *peer = (struct gateway_peer *)data;
	...
	manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: GATEWAY\r\nPeer: SIP/%s\r\nPeerStatus: Unregistered\r\nCause: Expired\r\n", peer->name);
	...
	*/
	...
	if (peer->selfdestruct ||
	    tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_RTAUTOCLEAR)) {
...
static void reg_source_db(struct gateway_peer *peer)
{
	...
	tris_debug(2, "GATEWAY Seeding peer from astdb: '%s' at %s@%s:%d for %d\n",
	...
	if (gatewaysock < 0) {
		/* GATEWAY isn't up yet, so schedule a poke only, pretty soon */
		TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, tris_random() % 5000 + 1, gateway_poke_peer_s, peer,
...
static int __set_address_from_contact(const char *fullcontact, struct sockaddr_in *sin, int tcp)
{
	struct hostent *hp;
	struct tris_hostent ahp;
	int port = STANDARD_GATEWAY_PORT;
	...
	/* 
	 * We have only the part in <brackets> here so we just need to parse a GATEWAY URI.
	 *
	...
	/* set port */
	if (((get_transport_str2enum(transport) == GATEWAY_TRANSPORT_TLS)) || !(strncasecmp(fullcontact, "gateways", 4))) {
		port = port_str2int(pt, STANDARD_TLS_PORT);
	} else {
		port = port_str2int(pt, STANDARD_GATEWAY_PORT);
	}
...
static int set_address_from_contact(struct gateway_pvt *pvt)
{
	if (tris_test_flag(&pvt->flags[0], GATEWAY_NAT_ROUTE)) {
	...
	return __set_address_from_contact(pvt->fullcontact, &pvt->sa, pvt->socket.type == GATEWAY_TRANSPORT_TLS ? 1 : 0);
}

/*! \brief Parse contact header and save registration (peer registration) */
static enum parse_register_result parse_register_contact(struct gateway_pvt *pvt, struct gateway_peer *peer, struct gateway_request *req)
{
	char contact[GATEWAYBUFSIZE];
	char data[GATEWAYBUFSIZE];
	...
	*/
	if (tris_strlen_zero(curi) && tris_strlen_zero(expires)) {
		...
		register_peer_exten(peer, FALSE);	/* Remove extension from regexten= setting in gateway.conf */
		tris_string_field_set(peer, fullcontact, "");
		tris_string_field_set(peer, useragent, "");
		peer->gatewayoptions = 0;
		peer->lastms = 0;
		peer->portinuri = 0;
		pvt->expiry = 0;

		tris_verb(3, "Unregistered GATEWAY '%s'\n", peer->name);
	...
	/* Make sure it's a GATEWAY URL */
	if (parse_uri(curi, "sip:,sips:", &curi, NULL, &host, &pt, NULL, &transport)) {
		tris_log(LOG_NOTICE, "Not a valid GATEWAY contact (missing sip:) trying to use anyway\n");
	}
	...
	if ((transport_type = get_transport_str2enum(transport))) {
		...
		port = port_str2int(pt, (transport_type == GATEWAY_TRANSPORT_TLS) ? STANDARD_TLS_PORT : STANDARD_GATEWAY_PORT);
	} else {
		port = port_str2int(pt, STANDARD_GATEWAY_PORT);
	...
	if (!tris_test_flag(&peer->flags[0], GATEWAY_NAT_ROUTE)) {
		peer->addr.sin_family = AF_INET;
	...
	/* Save GATEWAY options profile */
	peer->gatewayoptions = pvt->gatewayoptions;
	...
	if (expire < min_expiry)
		expire = min_expiry;
	if (peer->is_realtime && !tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS)) {
		peer->expire = -1;
	} else {
	...
	*/
	if (!peer->rt_fromcontact && (peer->socket.type & GATEWAY_TRANSPORT_UDP))
	...
	if (VERBOSITY_ATLEAST(2) && inaddrcmp(&peer->addr, &oldsin)) {
		tris_verbose(VERBOSE_PREFIX_3 "Registered GATEWAY '%s' at %s port %d\n", peer->name, tris_inet_ntoa(peer->addr.sin_addr), ntohs(peer->addr.sin_port));
	}
	gateway_poke_peer(peer, 0);
...
static void build_route(struct gateway_pvt *p, struct gateway_request *req, int backwards)
{
	struct gateway_route *thishop, *head, *tail;
	int start = 0;
	int len;
...
/*! \brief builds the gateway_pvt's randdata field which is used for the nonce
...
static void set_nonce_randdata(struct gateway_pvt *p, int forceupdate)
{
					 ...
					 char *uri, enum xmittype reliable, int ignore)
{
	...
	/* Whoever came up with the authentication section of GATEWAY can suck my %&#$&* for not putting
	...
	if (!tris_strlen_zero(md5secret))
		...
		char a1[256];
		snprintf(a1, sizeof(a1), "%s:%s:%s", username, gateway_cfg.realm, secret);
	...
	/* compute the expected response to compare with what we received */
	{
		...
		snprintf(a2, sizeof(a2), "%s:%s", gateway_methods[gatewaymethod].text,
...
static void gateway_peer_hold(struct gateway_pvt *p, int hold)
{
	struct gateway_peer *peer = find_peer(p->peername, NULL, 1, FINDALLDEVICES, FALSE, 0);
	...
	tris_devstate_changed(TRIS_DEVICE_UNKNOWN, "SIP/%s", peer->name);
	unref_peer(peer, "gateway_peer_hold: from find_peer operation");
...
{
	struct gateway_peer *peer = userdata;

	ao2_lock(peer);
	gateway_send_mwi_to_peer(peer, event, 0);
	ao2_unlock(peer);
}
...
static int cb_extensionstate(char *context, char* exten, int state, void *data)
{
	...
	case TRIS_EXTENSION_REMOVED:	/* Extension is gone */
		if (p->autokillid > -1 && gateway_cancel_destroy(p))	/* Remove subscription expiry for renewals */
			tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);	/* Delete subscription in 32 secs */
...
static void transmit_fake_auth_response(struct gateway_pvt *p, int gatewaymethod, struct gateway_request *req, enum xmittype reliable)
{
	...
	if (gatewaymethod == GATEWAY_REGISTER || gatewaymethod == GATEWAY_SUBSCRIBE) {
		response = "401 Unauthorized";
					      ...
					      struct gateway_request *req, char *uri)
{
	...
	if ((c = strchr(name, '@'))) {
		*c++ = '\0';
		...
		if (!TRIS_LIST_EMPTY(&domain_list)) {
			if (!check_gateway_domain(domain, NULL, 0)) {
	...
	if (peer) {
		...
		if (!peer->host_dynamic) {
			...
			tris_copy_flags(&p->flags[0], &peer->flags[0], GATEWAY_NAT);
			if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_REGISTERTRYING))
				transmit_response(p, "100 Trying", req);
			if (!(res = check_auth(p, req, peer->name, peer->secret, peer->md5secret, GATEWAY_REGISTER, uri, XMIT_UNRELIABLE, req->ignore))) {
				if (gateway_cancel_destroy(p))
					tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
				...
				if (check_request_transport(peer, req)) {
					...
					case PARSE_REGISTER_UPDATE:
						...
						transmit_response_with_date(p, "200 OK", req);
						if (!tris_test_flag((&peer->flags[1]), GATEWAY_PAGE2_SUBSCRIBEMWIONLY))
							peer->lastmsgssent = -1;
						res = 0;
						break;
					}
	...
	if (!peer && gateway_cfg.autocreatepeer) {
	...
	if (!peer && gateway_cfg.alwaysauthreject) {
	...
	if (res < 0) {
		...
		case AUTH_ACL_FAILED:
			if (gateway_cfg.alwaysauthreject) {
				transmit_fake_auth_response(p, GATEWAY_REGISTER, &p->initreq, XMIT_UNRELIABLE);
				if (global_authfailureevents) {
					manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: GATEWAY\r\nPeer: SIP/%s\r\nPeerStatus: Rejected\r\nCause: %s\r\nAddress: %s\r\nPort: %d\r\n",
...
static void gateway_set_redirstr(struct gateway_pvt *p, char *reason) {
...
static int get_rdnis(struct gateway_pvt *p, struct gateway_request *oreq)
{
	...
	*/
	...
	if (!strncasecmp(exten, "sip:", 4)) {
		...
		tris_log(LOG_WARNING, "Huh?  Not an RDNIS GATEWAY header (%s)?\n", exten);
		return -1;
	}
		...
		*params = '\0';	/* Cut off parameters  */
		...
		if ((reason = strcasestr(params, "reason="))) {
			...
			if (!tris_strlen_zero(reason)) {
				gateway_set_redirstr(p, reason);
				if (p->owner) {
					pbx_builtin_setvar_helper(p->owner, "__PRIREDIRECTREASON", p->redircause);
					pbx_builtin_setvar_helper(p->owner, "__GATEWAYREDIRECTREASON", reason);
				}
			}
		}
	}
	...
	if (p->owner) 
		pbx_builtin_setvar_helper(p->owner, "__GATEWAYRDNISDOMAIN", rdomain);
...
/*! \brief Find out who the call is for.
  ...
  \note If the incoming uri is a GATEWAYS: uri, we are required to carry this across
...
static int get_destination(struct gateway_pvt *p, struct gateway_request *oreq)
{
	...
	if (!TRIS_LIST_EMPTY(&domain_list)) {
		...
		domain_context[0] = '\0';
		if (!check_gateway_domain(p->domain, domain_context, sizeof(domain_context))) {
			if (!gateway_cfg.allow_external_domains && (req->method == GATEWAY_INVITE || req->method == GATEWAY_REFER)) {
				tris_debug(1, "Got GATEWAY %s to non-local domain '%s'; refusing request.\n", gateway_methods[req->method].text, p->domain);
				return -2;
			}
		}
	...
	if (req->method == GATEWAY_SUBSCRIBE && !tris_strlen_zero(p->subscribecontext))
	...
	if (req->method == GATEWAY_SUBSCRIBE) {
		char hint[TRIS_MAX_EXTENSION];
		...
		/* Check the dialplan for the username part of the request URI,
		   the domain will be stored in the GATEWAYDOMAIN variable
	...
	if((tris_test_flag(&global_flags[1], GATEWAY_PAGE2_ALLOWOVERLAP) && 
...
static struct gateway_pvt *get_gateway_pvt_byid_locked(const char *callid, const char *totag, const char *fromtag) 
{
	struct gateway_pvt *gateway_pvt_ptr;
	struct gateway_pvt tmp_dialog = {
		.callid = callid,
	};
	...
	gateway_pvt_ptr = ao2_t_find(dialogs, &tmp_dialog, OBJ_POINTER, "ao2_find of dialog in dialogs table");
	if (gateway_pvt_ptr) {
		...
		if (gateway_cfg.pedanticgatewaychecking) {
			...
			if (tris_strlen_zero(fromtag)) {
				gateway_pvt_unlock(gateway_pvt_ptr);
				tris_debug(4, "Matched %s call for callid=%s - no from tag specified, pedantic check fails\n",
					  gateway_pvt_ptr->outgoing_call == TRUE ? "OUTGOING": "INCOMING", gateway_pvt_ptr->callid);
				return NULL;
			}
			 ...
			 * > GATEWAY dialog (call-id, to-tag, and from-tag).  Upon receiving an INVITE
			 ...
			 */
			frommismatch = !!strcmp(fromtag, gateway_pvt_ptr->theirtag);
			tomismatch = !!strcmp(totag, gateway_pvt_ptr->tag);
		...
		if (totag)
			tris_debug(4, "Matched %s call - their tag is %s Our tag is %s\n",
					  gateway_pvt_ptr->outgoing_call == TRUE ? "OUTGOING": "INCOMING",
					  gateway_pvt_ptr->theirtag, gateway_pvt_ptr->tag);

		/* deadlock avoidance... */
		while (gateway_pvt_ptr->owner && tris_channel_trylock(gateway_pvt_ptr->owner)) {
			gateway_pvt_unlock(gateway_pvt_ptr);
			usleep(1);
			gateway_pvt_lock(gateway_pvt_ptr);
		}
 ...
 *
 * \note If we get a GATEWAYS uri in the refer-to header, we're required to set up a secure signalling path
...
static int get_refer_info(struct gateway_pvt *transferer, struct gateway_request *outgoing_req)
{
	...
	char *ptr;
	struct gateway_request *req = NULL;
	const char *transfer_context = NULL;
	struct gateway_refer *referdata;
	...
	if (transferer->owner) {
		...
		if (peer) {
			pbx_builtin_setvar_helper(peer, "GATEWAYREFERRINGCONTEXT", transferer->context);
			pbx_builtin_setvar_helper(peer, "GATEWAYREFERREDBYHDR", p_referred_by);
		}
	}
	...
	if (tris_strlen_zero(transfer_context)) {
		transfer_context = S_OR(transferer->owner->macrocontext,
					S_OR(transferer->context, gateway_cfg.default_context));
	}
	...
	if (referdata->attendedtransfer || tris_exists_extension(NULL, transfer_context, refer_to, 1, NULL) ) {
		if (gateway_debug_test_pvt(transferer)) {
			tris_verbose("GATEWAY transfer to extension %s@%s by %s\n", refer_to, transfer_context, referred_by_uri);
		}
		/* We are ready to transfer to the extension */
		return 0;
	} 
	if (gateway_debug_test_pvt(transferer))
		tris_verbose("Failed GATEWAY Transfer to non-existing extension %s in context %s\n n", refer_to, transfer_context);
 ...
 *	\note does not account for GATEWAYS: uri requirements, nor check transport
 */
static int get_also_info(struct gateway_pvt *p, struct gateway_request *oreq)
{
	char tmp[256] = "", *c, *a;
	struct gateway_request *req = oreq ? oreq : &p->initreq;
	struct gateway_refer *referdata = NULL;
	const char *transfer_context = NULL;
	
	if (!p->refer && !gateway_refer_allocate(p))
		return -1;
	...
	if (!strncasecmp(c, "sip:", 4)) {
		...
		tris_log(LOG_WARNING, "Huh?  Not a GATEWAY header in Also: transfer (%s)?\n", c);
		return -1;
	}
	...
	if (tris_strlen_zero(transfer_context)) {
		transfer_context = S_OR(p->owner->macrocontext,
					S_OR(p->context, gateway_cfg.default_context));
	}
	if (tris_exists_extension(NULL, transfer_context, c, 1, NULL)) {
		/* This is a blind transfer */
		tris_debug(1, "GATEWAY Bye-also transfer to Extension %s@%s \n", c, transfer_context);
...
/*! \brief check received= and rport= in a GATEWAY response.
 ...
 * Using these two fields GATEWAY can produce the correct
 * address and port in the GATEWAY headers without the need for STUN.
 * The address part is also reused for the media sessions.
 * Note that tris_gateway_ouraddrfor() still rewrites p->ourip
 * if you specify externip/seternaddr/stunaddr.
 */
static attribute_unused void check_via_response(struct gateway_pvt *p, struct gateway_request *req)
{
	char via[256];
	char *cur, *opts;
...
static void check_via(struct gateway_pvt *p, struct gateway_request *req)
{
	...
	if (c && (c[6] != '='))	/* rport query, not answer */
		tris_set_flag(&p->flags[1], GATEWAY_PAGE2_RPORT_PRESENT);
	...
	if (c) {
		*c = '\0';
		...
		p->sa.sin_family = AF_INET;
		memcpy(&p->sa.sin_addr, hp->h_addr, sizeof(p->sa.sin_addr));
		p->sa.sin_port = htons(port_str2int(pt, STANDARD_GATEWAY_PORT));

		if (gateway_debug_test_pvt(p)) {
			const struct sockaddr_in *dst = gateway_real_dst(p);
			tris_verbose("Sending to %s : %d (%s)\n", tris_inet_ntoa(dst->sin_addr), ntohs(dst->sin_port), gateway_nat_mode(p));
		}
	}
}

/*! \brief  Get caller id name from GATEWAY headers */
static char *get_calleridname(const char *input, char *output, size_t outputsize)
{
...
static void replace_cid(struct gateway_pvt *p, const char *rpid_num, const char *calleridname)
{
	/* replace callerid if rpid found, and not restricted */
	if (!tris_strlen_zero(rpid_num) && tris_test_flag(&p->flags[0], GATEWAY_TRUSTRPID)) {
...
static enum check_auth_result check_peer_ok(struct gateway_pvt *p, char *of,
	struct gateway_request *req, int gatewaymethod, struct sockaddr_in *sin,
	struct gateway_peer **authpeer,
	enum xmittype reliable,
	char *rpid_num, char *calleridname, char *uri2)
{
//	enum check_auth_result res;
	int debug=gateway_debug_test_addr(sin);
	struct gateway_peer *peer;

	if (gatewaymethod == GATEWAY_SUBSCRIBE) {
	...
	/* Take the peer */
	tris_copy_flags(&p->flags[0], &peer->flags[0], GATEWAY_FLAGS_TO_COPY);
	tris_copy_flags(&p->flags[1], &peer->flags[1], GATEWAY_PAGE2_FLAGS_TO_COPY);

	if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT) && p->udptl) {
		p->t38_maxdatagram = peer->t38_maxdatagram;
		set_t38_capabilities(p);
	}

	/* Copy GATEWAY extensions profile to peer */
	/* XXX is this correct before a successful auth ? */
	if (p->gatewayoptions)
		peer->gatewayoptions = p->gatewayoptions;

	replace_cid(p, rpid_num, calleridname);
	do_setnat(p, tris_test_flag(&p->flags[0], GATEWAY_NAT_ROUTE));
	...
	if (tris_test_flag(&peer->flags[0], GATEWAY_INSECURE_INVITE)) {
		/* Pretend there is no required authentication */
		tris_string_field_set(p, peersecret, NULL);
		tris_string_field_set(p, peermd5secret, NULL);
	}
/*	if (!(res = check_auth(p, req, peer->name, p->peersecret, p->peermd5secret, gatewaymethod, uri2, reliable, req->ignore))) {*/
		tris_copy_flags(&p->flags[0], &peer->flags[0], GATEWAY_FLAGS_TO_COPY);
		tris_copy_flags(&p->flags[1], &peer->flags[1], GATEWAY_PAGE2_FLAGS_TO_COPY);
		/* If we have a call limit, set flag */
		if (peer->call_limit)
			tris_set_flag(&p->flags[0], GATEWAY_CALL_LIMIT);
		...
		if (!tris_test_flag(&p->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS) &&
				(!tris_test_flag(&p->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT) ||
					!(p->capability & TRIS_FORMAT_VIDEO_MASK)) &&
				p->vrtp) {
			tris_rtp_destroy(p->vrtp);
			p->vrtp = NULL;
		}
		if ((!tris_test_flag(&p->flags[1], GATEWAY_PAGE2_TEXTSUPPORT) || !(p->capability & TRIS_FORMAT_TEXT_MASK)) && p->trtp) {
			tris_rtp_destroy(p->trtp);
			p->trtp = NULL;
 		}
		if ((tris_test_flag(&p->flags[0], GATEWAY_DTMF) == GATEWAY_DTMF_RFC2833) ||
					      ...
					      struct sockaddr_in *sin, struct gateway_peer **authpeer)
{
	...
	if (tris_strlen_zero(of)) {
		 ...
		 * as a username-only URI. The GATEWAY RFC (19.1.1) says that
	...
	res = check_peer_ok(p, of, req, gatewaymethod, sin,
			authpeer, reliable, rpid_num, calleridname, uri2);
	if (res != AUTH_DONT_KNOW)
		return res;

	/* Finally, apply the guest policy */
	if (gateway_cfg.allowguest) {
		replace_cid(p, rpid_num, calleridname);
		res = AUTH_SUCCESSFUL;
	} else if (gateway_cfg.alwaysauthreject)
		res = AUTH_FAKE_AUTH; /* reject with fake authorization request */
	else
		res = AUTH_SECRET_FAILED; /* we don't want any guests, authentication will fail */


	if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_RPORT_PRESENT)) {
		tris_set_flag(&p->flags[0], GATEWAY_NAT_ROUTE);
	}
...
static int check_user(struct gateway_pvt *p, struct gateway_request *req, int gatewaymethod, char *uri, enum xmittype reliable, struct sockaddr_in *sin)
{
	return check_user_full(p, req, gatewaymethod, uri, reliable, sin, NULL);
}

/*! \brief  Get text out of a GATEWAY MESSAGE packet */
static int get_msg_text(char *buf, int len, struct gateway_request *req, int addnewline)
{
	int x;
	int y;
...
/*! \brief  Receive GATEWAY MESSAGE method messages
...
static void receive_message(struct gateway_pvt *p, struct gateway_request *req)
{
	...
	if (p->owner) {
		if (gateway_debug_test_pvt(p))
			tris_verbose("GATEWAY Text message received: '%s'\n", buf);
		memset(&f, 0, sizeof(f));
...
#define FORMAT2 "%-25.25s %-15.15s %-15.15s \n"
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show inuse";
		e->usage =
			"Usage: gateway show inuse [all]\n"
			"       List all GATEWAY devices usage counters and limits.\n"
...
static struct _map_x_s natmodes[] = {
	{ GATEWAY_NAT_NEVER,        "No"},
	{ GATEWAY_NAT_ROUTE,        "Route"},
	{ GATEWAY_NAT_ALWAYS,       "Always"},
	{ GATEWAY_NAT_RFC3581,      "RFC3581"},
	{ -1,                   NULL}, /* terminator */
};
...
static struct _map_x_s natcfgmodes[] = {
	{ GATEWAY_NAT_NEVER,        "never"},
	{ GATEWAY_NAT_ROUTE,        "route"},
	{ GATEWAY_NAT_ALWAYS,       "yes"},
	{ GATEWAY_NAT_RFC3581,      "no"},
	{ -1,                   NULL}, /* terminator */
};
...
static char *gateway_show_tcp(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct gateway_threadinfo *th;
	struct ao2_iterator i;
...
#define FORMAT  "%-30.30s %-6d %-9.9s %-6.6s\n"
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show tcp";
		e->usage =
			"Usage: gateway show tcp\n"
	...
	while ((th = ao2_t_iterator_next(&i, "iterate through tcp threads for 'gateway show tcp'"))) {
...
/*! \brief  CLI Command 'GATEWAY Show Users' */
static char *gateway_show_users(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	regex_t regexbuf;
	int havepattern = FALSE;
	struct ao2_iterator user_iter;
	struct gateway_peer *user;

#define FORMAT  "%-25.25s  %-15.15s  %-15.15s  %-15.15s  %-5.5s%-10.10s\n"

	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show users";
		e->usage =
			"Usage: gateway show users [like <pattern>]\n"
			"       Lists all known GATEWAY users.\n"
	...
	while ((user = ao2_iterator_next(&user_iter))) {
		ao2_lock(user);
		if (!(user->type & GATEWAY_TYPE_USER)) {
			ao2_unlock(user);
			unref_peer(user, "gateway show users");
			continue;
		}
		...
		tris_cli(a->fd, FORMAT, user->name, 
			...
			nat2str(tris_test_flag(&user->flags[0], GATEWAY_NAT)));
		ao2_unlock(user);
		unref_peer(user, "gateway show users");
	}
...
/*! \brief Manager Action GATEWAYShowRegistry description */
...
/*! \brief Show GATEWAY registrations in the manager API */
static int manager_show_registry(struct mansession *s, const struct message *m)
{
	...
	ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
		...
		astman_append(s,
			...
			"State: %s\r\n"
			"RegistrationTime: %ld\r\n"
			"\r\n", idtext, iterator->hostname, iterator->portno ? iterator->portno : STANDARD_GATEWAY_PORT,
...
"Description: Lists GATEWAY peers in text format with details on current status.\n"
"Peerlist will follow as separate events, followed by a final event called\n"
"PeerlistComplete.\n"
"Variables: \n"
"  ActionID: <id>	Action ID for this transaction. Will be returned.\n";

/*! \brief  Show GATEWAY peers in the manager API */
/*    Inspired from chan_iax2 */
static int manager_gateway_show_peers(struct mansession *s, const struct message *m)
{
	const char *id = astman_get_header(m, "ActionID");
	const char *a[] = {"gateway", "show", "peers"};
	char idtext[256] = "";
	int total = 0;
	...
	_gateway_show_peers(-1, &total, s, m, 3, a);
...
static char *gateway_show_peers(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show peers";
		e->usage =
			"Usage: gateway show peers [like <pattern>]\n"
			"       Lists all known GATEWAY peers.\n"
	...
	return _gateway_show_peers(a->fd, NULL, NULL, NULL, a->argc, (const char **) a->argv);
}
...
int peercomparefunc(const void *a, const void *b)
{
	struct gateway_peer **ap = (struct gateway_peer **)a;
	struct gateway_peer **bp = (struct gateway_peer **)b;
	return strcmp((*ap)->name, (*bp)->name);
}


/*! \brief Execute gateway show peers command */
static char *_gateway_show_peers(int fd, int *total, struct mansession *s, const struct message *m, int argc, const char *argv[])
{
	regex_t regexbuf;
	int havepattern = FALSE;
...
#define FORMAT  "%-25.25s  %-15.15s %-3.3s %-3.3s %-3.3s %-8d %-10s %s\n"
	...
	struct gateway_peer **peerarray;
	int k;
	
	
	realtimepeers = tris_check_realtime("gatewaypeers");
	peerarray = tris_calloc(sizeof(struct gateway_peer *), objcount);
	...
	while ((peer = ao2_t_iterator_next(&i, "iterate thru peers table"))) {	
		...
		if (!(peer->type & GATEWAY_TYPE_PEER)) {
			ao2_unlock(peer);
	...
	qsort(peerarray, total_peers, sizeof(struct gateway_peer *), peercomparefunc);
	...
	for(k=0; k < total_peers; k++) {
		...
		snprintf(srch, sizeof(srch), FORMAT, name,
			...
			tris_test_flag(&peer->flags[0], GATEWAY_NAT_ROUTE) ? " N " : "   ",	/* NAT=yes? */
		...
		if (!s)  {/* Normal CLI list */
			...
			tris_test_flag(&peer->flags[0], GATEWAY_NAT_ROUTE) ? "yes" : "no",	/* NAT=yes? */
			tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT) ? "yes" : "no",	/* VIDEOSUPPORT=yes? */
			tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_TEXTSUPPORT) ? "yes" : "no",	/* TEXTSUPPORT=yes? */
	...
	if (!s)
		tris_cli(fd, "%d gateway peers [Monitored: %d online, %d offline Unmonitored: %d online, %d offline]\n",
...
{
	struct gateway_peer *peer = userobj;
...
{
	struct gateway_pvt *pvt = userobj;
	int refc = ao2_t_ref(userobj, 0, "");
	int *fd = arg;
...
/*! \brief List all allocated GATEWAY Objects (realtime or static) */
static char *gateway_show_objects(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char tmp[256];
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show objects";
		e->usage =
			"Usage: gateway show objects\n"
			"       Lists status of known GATEWAY objects\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}	
...
static struct _map_x_s dtmfstr[] = {
	{ GATEWAY_DTMF_RFC2833,     "rfc2833" },
	{ GATEWAY_DTMF_INFO,        "info" },
	{ GATEWAY_DTMF_SHORTINFO,   "shortinfo" },
	{ GATEWAY_DTMF_INBAND,      "inband" },
	{ GATEWAY_DTMF_AUTO,        "auto" },
	{ -1,                   NULL }, /* terminator */
};
...
static struct _map_x_s insecurestr[] = {
	{ GATEWAY_INSECURE_PORT,    "port" },
	{ GATEWAY_INSECURE_INVITE,  "invite" },
	{ GATEWAY_INSECURE_PORT | GATEWAY_INSECURE_INVITE, "port,invite" },
	{ 0,                    "no" },
	{ -1,                   NULL }, /* terminator */
};
...
static void cleanup_stale_contexts(char *new, char *old)
{
	...
	while ((oldcontext = strsep(&old, "&"))) {
		...
		if (stalecontext)
			tris_context_destroy(tris_context_find(stalecontext), "GATEWAY");
	}
}
...
{
	struct gateway_pvt *dialog = dialogobj;
	time_t *t = arg;

	if (gateway_pvt_trylock(dialog)) {
	...
	if (dialog->needdestroy && !dialog->packets && !dialog->owner) {
		...
		if (dialog->rtp && tris_rtp_get_bridged(dialog->rtp)) {
			tris_debug(2, "Bridge still active.  Delaying destruction of GATEWAY dialog '%s' Method: %s\n", dialog->callid, gateway_methods[dialog->method].text);
			gateway_pvt_unlock(dialog);
			return 0;
		}
		
		if (dialog->vrtp && tris_rtp_get_bridged(dialog->vrtp)) {
			tris_debug(2, "Bridge still active.  Delaying destroy of GATEWAY dialog '%s' Method: %s\n", dialog->callid, gateway_methods[dialog->method].text);
			gateway_pvt_unlock(dialog);
			return 0;
		}
...
{
	struct gateway_peer *peer = peerobj;
...
static char *gateway_prune_realtime(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (cmd == CLI_INIT) {
		e->command = "gateway prune realtime [peer|all]";
		e->usage =
			"Usage: gateway prune realtime [peer [<name>|all|like <pattern>]|all]\n"
		...
		if (a->pos == 4 && !strcasecmp(a->argv[3], "peer")) {
			...
			if (!cmplt)
				cmplt = complete_gateway_peer(a->word, a->n - sizeof(choices), GATEWAY_PAGE2_RTCACHEFRIENDS);
			return cmplt;
		}
		if (a->pos == 5 && !strcasecmp(a->argv[4], "like"))
			return complete_gateway_peer(a->word, a->n, GATEWAY_PAGE2_RTCACHEFRIENDS);
		return NULL;
	}
	switch (a->argc) {
	case 4:
		...
		break;
	case 5:
		/* gateway prune realtime {peer|like} name */
		name = a->argv[4];
	...
	if (multi) {
		if (prunepeer) {
			...
			while ((pi = ao2_t_iterator_next(&i, "iterate thru peers table"))) {
				...
				if (tris_test_flag(&pi->flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS)) {
					pi->the_mark = 1;
					pruned++;
				}
				ao2_unlock(pi);
		...
		if (prunepeer) {
			...
			if ((peer = ao2_t_find(peers, &tmp, OBJ_POINTER | OBJ_UNLINK, "finding to unlink from peers"))) {
				...
				if (!tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS)) {
				...
				unref_peer(peer, "gateway_prune_realtime: unref_peer: tossing temp peer ptr");
			} else
...
{
	switch (mode) {
	case GATEWAY_DOMAIN_AUTO:
		return "[Automatic]";
	case GATEWAY_DOMAIN_CONFIG:
		return "[Configured]";
	}
...
#define FORMAT "%-40.40s %-20.20s %-16.16s\n"
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show domains";
		e->usage =
			"Usage: gateway show domains\n"
			"       Lists all configured GATEWAY local domains.\n"
			"       Trismedia only responds to GATEWAY messages to local domains.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	if (TRIS_LIST_EMPTY(&domain_list)) {
		tris_cli(a->fd, "GATEWAY Domain support not enabled.\n\n");
		return CLI_SUCCESS;
	} else {
		tris_cli(a->fd, FORMAT, "Our local GATEWAY domains:", "Context", "Set by");
...
"Description: Show one GATEWAY peer with details on current status.\n"
"Variables: \n"
"  Peer: <name>           The peer name you want to check.\n"
"  ActionID: <id>	  Optional action ID for this AMI transaction.\n";

/*! \brief Show GATEWAY peers in the manager API  */
static int manager_gateway_show_peer(struct mansession *s, const struct message *m)
{
	const char *a[4];
	const char *peer;
	...
	_gateway_show_peer(1, -1, s, m, 4, a);
	astman_append(s, "\r\n\r\n" );
	return 0;
}
...
static char *gateway_show_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show peer";
		e->usage =
			"Usage: gateway show peer <name> [load]\n"
			"       Shows all details on one GATEWAY peer and the current status.\n"
			"       Option \"load\" forces lookup of peer in realtime storage.\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gateway_show_peer(a->line, a->word, a->pos, a->n);
	}
	return _gateway_show_peer(0, a->fd, NULL, NULL, a->argc, (const char **) a->argv);
}
...
/*! \brief Qualify GATEWAY peers in the manager API  */
static int manager_gateway_qualify_peer(struct mansession *s, const struct message *m)
{
	const char *a[4];
	const char *peer;
	...
	_gateway_qualify_peer(1, -1, s, m, 4, a);
	astman_append(s, "\r\n\r\n" );
	return 0;
}

/*! \brief Send an OPTIONS packet to a GATEWAY peer */
static char *gateway_qualify_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway qualify peer";
		e->usage =
			"Usage: gateway qualify peer <name> [load]\n"
			"       Requests a response from one GATEWAY peer and the current status.\n"
			"       Option \"load\" forces lookup of peer in realtime storage.\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gateway_show_peer(a->line, a->word, a->pos, a->n);
	}
	return _gateway_qualify_peer(0, a->fd, NULL, NULL, a->argc, (const char **) a->argv);
}
...
static struct _map_x_s faxecmodes[] = {
	{ GATEWAY_PAGE2_T38SUPPORT_UDPTL,			"None"},
	{ GATEWAY_PAGE2_T38SUPPORT_UDPTL_FEC,		"FEC"},
	{ GATEWAY_PAGE2_T38SUPPORT_UDPTL_REDUNDANCY,	"Redundancy"},
	{ -1,						NULL},
};
...
static char *_gateway_show_peer(int type, int fd, struct mansession *s, const struct message *m, int argc, const char *argv[])
{
	...
	if (peer && type==0 ) { /* Normal listing */
		...
		tris_cli(fd, "  Insecure     : %s\n", insecure2str(tris_test_flag(&peer->flags[0], GATEWAY_INSECURE)));
		tris_cli(fd, "  Nat          : %s\n", nat2str(tris_test_flag(&peer->flags[0], GATEWAY_NAT)));
		tris_cli(fd, "  ACL          : %s\n", cli_yesno(peer->ha != NULL));
		tris_cli(fd, "  T.38 support : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_T38SUPPORT)));
		tris_cli(fd, "  T.38 EC mode : %s\n", faxec2str(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_T38SUPPORT)));
		tris_cli(fd, "  T.38 MaxDtgrm: %d\n", peer->t38_maxdatagram);
		tris_cli(fd, "  DirectMedia  : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], GATEWAY_DIRECT_MEDIA)));
		tris_cli(fd, "  PromiscRedir : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], GATEWAY_PROMISCREDIR)));
		tris_cli(fd, "  User=Phone   : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], GATEWAY_USEREQPHONE)));
		tris_cli(fd, "  Video Support: %s\n", cli_yesno(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT)));
		tris_cli(fd, "  Text Support : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_TEXTSUPPORT)));
		tris_cli(fd, "  Ign SDP ver  : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_IGNORESDPVERSION)));
		tris_cli(fd, "  Trust RPID   : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], GATEWAY_TRUSTRPID)));
		tris_cli(fd, "  Send RPID    : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], GATEWAY_SENDRPID)));
		tris_cli(fd, "  Subscriptions: %s\n", cli_yesno(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_ALLOWSUBSCRIBE)));
		tris_cli(fd, "  Overlap dial : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_ALLOWOVERLAP)));
		if (peer->outboundproxy)
		...
		/* - is enumerated */
		tris_cli(fd, "  DTMFmode     : %s\n", dtmfmode2str(tris_test_flag(&peer->flags[0], GATEWAY_DTMF)));
		...
		tris_cli(fd, "  GATEWAY Options  : ");
		if (peer->gatewayoptions) {
			int lastoption = -1;
			for (x = 0 ; x < ARRAY_LEN(gateway_options); x++) {
				if (gateway_options[x].id != lastoption) {
					if (peer->gatewayoptions & gateway_options[x].id)
						tris_cli(fd, "%s ", gateway_options[x].text);
					lastoption = x;
				}
			}
		} else
		...
		tris_cli(fd, "  100 on REG   : %s\n", tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_REGISTERTRYING) ? "Yes" : "No");
		tris_cli(fd, "  Status       : ");
		...
		tris_cli(fd, "\n");
		peer = unref_peer(peer, "gateway_show_peer: unref_peer: done with peer ptr");
	} else  if (peer && type == 1) { /* manager listing */
		char buffer[256];
		struct tris_str *mailbox_str = tris_str_alloca(512);
		astman_append(s, "Channeltype: GATEWAY\r\n");
		...
		if (!tris_strlen_zero(peer->fromuser))
			astman_append(s, "GATEWAY-FromUser: %s\r\n", peer->fromuser);
		if (!tris_strlen_zero(peer->fromdomain))
			astman_append(s, "GATEWAY-FromDomain: %s\r\n", peer->fromdomain);
		...
		astman_append(s, "GATEWAY-AuthInsecure: %s\r\n", insecure2str(tris_test_flag(&peer->flags[0], GATEWAY_INSECURE)));
		astman_append(s, "GATEWAY-NatSupport: %s\r\n", nat2str(tris_test_flag(&peer->flags[0], GATEWAY_NAT)));
		astman_append(s, "ACL: %s\r\n", (peer->ha?"Y":"N"));
		astman_append(s, "GATEWAY-CanReinvite: %s\r\n", (tris_test_flag(&peer->flags[0], GATEWAY_DIRECT_MEDIA)?"Y":"N"));
		astman_append(s, "GATEWAY-DirectMedia: %s\r\n", (tris_test_flag(&peer->flags[0], GATEWAY_DIRECT_MEDIA)?"Y":"N"));
		astman_append(s, "GATEWAY-PromiscRedir: %s\r\n", (tris_test_flag(&peer->flags[0], GATEWAY_PROMISCREDIR)?"Y":"N"));
		astman_append(s, "GATEWAY-UserPhone: %s\r\n", (tris_test_flag(&peer->flags[0], GATEWAY_USEREQPHONE)?"Y":"N"));
		astman_append(s, "GATEWAY-VideoSupport: %s\r\n", (tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_VIDEOSUPPORT)?"Y":"N"));
		astman_append(s, "GATEWAY-TextSupport: %s\r\n", (tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_TEXTSUPPORT)?"Y":"N"));
		astman_append(s, "GATEWAY-T.38Support: %s\r\n", (tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_T38SUPPORT)?"Y":"N"));
		astman_append(s, "GATEWAY-T.38EC: %s\r\n", faxec2str(tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_T38SUPPORT)));
		astman_append(s, "GATEWAY-T.38MaxDtgrm: %d\r\n", peer->t38_maxdatagram);
		astman_append(s, "GATEWAY-Sess-Timers: %s\r\n", stmode2str(peer->stimer.st_mode_oper));
		astman_append(s, "GATEWAY-Sess-Refresh: %s\r\n", strefresher2str(peer->stimer.st_ref));
		astman_append(s, "GATEWAY-Sess-Expires: %d\r\n", peer->stimer.st_max_se);
		astman_append(s, "GATEWAY-Sess-Min: %d\r\n", peer->stimer.st_min_se);

		/* - is enumerated */
		astman_append(s, "GATEWAY-DTMFmode: %s\r\n", dtmfmode2str(tris_test_flag(&peer->flags[0], GATEWAY_DTMF)));
		...
		peer_status(peer, status, sizeof(status));
		astman_append(s, "%s\r\n", status);
 		astman_append(s, "GATEWAY-Useragent: %s\r\n", peer->useragent);
		...
		peer = unref_peer(peer, "gateway_show_peer: unref_peer: done with peer");
...
static char *complete_gateway_user(const char *word, int state)
{
	char *result = NULL;
	...
	while ((user = ao2_iterator_next(&user_iter))) {
		...
		if (!(user->type & GATEWAY_TYPE_USER)) {
			ao2_unlock(user);
			unref_peer(user, "complete gateway user");
			continue;
		}
...
/*! \brief Support routine for 'gateway show user' CLI */
static char *complete_gateway_show_user(const char *line, const char *word, int pos, int state)
{
	if (pos == 3)
		return complete_gateway_user(word, state);

	return NULL;
}

/*! \brief Show one user in detail */
static char *gateway_show_user(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char cbuf[256];
	struct gateway_peer *user;
	struct tris_variable *v;
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show user";
		e->usage =
			"Usage: gateway show user <name> [load]\n"
			"       Shows all details on one GATEWAY user and the current status.\n"
			"       Option \"load\" forces lookup of peer in realtime storage.\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gateway_show_user(a->line, a->word, a->pos, a->n);
	}
	...
	if ((user = find_peer(a->argv[3], NULL, load_realtime, FINDUSERS, FALSE, 0))) {
		...
		ao2_unlock(user);
		unref_peer(user, "gateway show user");
	} else {
...
static char *gateway_show_sched(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	struct tris_cb_names cbnames = {9, { "retrans_pkt",
                                        "__gateway_autodestruct",
                                        "expire_register",
                                        "auto_congest",
                                        "gateway_reg_timeout",
                                        "gateway_poke_peer_s",
                                        "gateway_poke_noanswer",
                                        "gateway_reregister",
                                        "gateway_reinvite_retry"},
								   { retrans_pkt,
                                     __gateway_autodestruct,
                                     expire_register,
                                     auto_congest,
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show sched";
		e->usage =
			"Usage: gateway show sched\n"
...
/*! \brief  Show GATEWAY Registry (registrations with other GATEWAY proxies */
static char *gateway_show_registry(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT  "%-30.30s %-6.6s %-12.12s  %8d %-20.20s %-25.25s\n"
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show registry";
		e->usage =
			"Usage: gateway show registry\n"
	...
	ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
		ASTOBJ_RDLOCK(iterator);
		snprintf(host, sizeof(host), "%s:%d", iterator->hostname, iterator->portno ? iterator->portno : STANDARD_GATEWAY_PORT);
	...
	tris_cli(a->fd, "%d GATEWAY registrations.\n", counter);
	return CLI_SUCCESS;
#undef FORMAT
#undef FORMAT2
}

/*! \brief Unregister (force expiration) a GATEWAY peer in the registry via CLI 
	\note This function does not tell the GATEWAY device what's going on,
...
static char *gateway_unregister(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway unregister";
		e->usage =
			"Usage: gateway unregister <peer>\n"
			"       Unregister (force expiration) a GATEWAY peer from the registry\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gateway_unregister(a->line, a->word, a->pos, a->n);
	}
	...
	if ((peer = find_peer(a->argv[2], NULL, load_realtime, FINDPEERS, TRUE, 0))) {
		...
		unref_peer(peer, "gateway_unregister: unref_peer via gateway_unregister: done with peer from find_peer call");
	} else {
...
/*! \brief GATEWAY show channelstats CLI (main function) */
static char *gateway_show_channelstats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show channelstats";
		e->usage =
			"Usage: gateway show channelstats\n"
			"       Lists all currently active GATEWAY channel's RTCP statistics.\n"
	...
	ao2_t_callback(dialogs, OBJ_NODATA, show_chanstats_cb, &arg, "callback to gateway show chanstats");
	tris_cli(a->fd, "%d active GATEWAY channel%s\n", arg.numchans, (arg.numchans != 1) ? "s" : ""); 
	return CLI_SUCCESS;
}
#undef FORMAT
#undef FORMAT2

/*! \brief List global settings for the GATEWAY channel */
static char *gateway_show_settings(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int realtimepeers;
	int realtimeregs;
	char codec_buf[GATEWAYBUFSIZE];
	const char *msg;	/* temporary msg pointer */

	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show settings";
		e->usage =
			"Usage: gateway show settings\n"
			"       Provides detailed list of the configuration of the GATEWAY channel.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	realtimepeers = tris_check_realtime("gatewaypeers");
	realtimeregs = tris_check_realtime("gatewayregs");

	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	tris_cli(a->fd, "\n\nGlobal Settings:\n");
	tris_cli(a->fd, "----------------\n");
	tris_cli(a->fd, "  UDP GATEWAY Port:           %d\n", ntohs(bindaddr.sin_port));
	tris_cli(a->fd, "  UDP Bindaddress:        %s\n", tris_inet_ntoa(bindaddr.sin_addr));
	tris_cli(a->fd, "  TCP GATEWAY Port:           ");
	if (gateway_tcp_desc.local_address.sin_family == AF_INET) {
		tris_cli(a->fd, "%d\n", ntohs(gateway_tcp_desc.local_address.sin_port));
		tris_cli(a->fd, "  TCP Bindaddress:        %s\n", tris_inet_ntoa(gateway_tcp_desc.local_address.sin_addr));
	} else {
		tris_cli(a->fd, "Disabled\n");
	}
	tris_cli(a->fd, "  TLS GATEWAY Port:           ");
	if (default_tls_cfg.enabled != FALSE) {
		tris_cli(a->fd, "%d\n", ntohs(gateway_tls_desc.local_address.sin_port));
		tris_cli(a->fd, "  TLS Bindaddress:        %s\n", tris_inet_ntoa(gateway_tls_desc.local_address.sin_addr));
	} else {
		tris_cli(a->fd, "Disabled\n");
	}
	tris_cli(a->fd, "  Videosupport:           %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_VIDEOSUPPORT)));
	tris_cli(a->fd, "  Textsupport:            %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_TEXTSUPPORT)));
	tris_cli(a->fd, "  Ignore SDP sess. ver.:  %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_IGNORESDPVERSION)));
	tris_cli(a->fd, "  AutoCreate Peer:        %s\n", cli_yesno(gateway_cfg.autocreatepeer));
	tris_cli(a->fd, "  Match Auth Username:    %s\n", cli_yesno(global_match_auth_username));
	tris_cli(a->fd, "  Allow unknown access:   %s\n", cli_yesno(gateway_cfg.allowguest));
	tris_cli(a->fd, "  Allow subscriptions:    %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_ALLOWSUBSCRIBE)));
	tris_cli(a->fd, "  Allow overlap dialing:  %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_ALLOWOVERLAP)));
	tris_cli(a->fd, "  Allow promsic. redir:   %s\n", cli_yesno(tris_test_flag(&global_flags[0], GATEWAY_PROMISCREDIR)));
	tris_cli(a->fd, "  Enable call counters:   %s\n", cli_yesno(global_callcounter));
	tris_cli(a->fd, "  GATEWAY domain support:     %s\n", cli_yesno(!TRIS_LIST_EMPTY(&domain_list)));
	tris_cli(a->fd, "  Realm. auth:            %s\n", cli_yesno(authl != NULL));
	tris_cli(a->fd, "  Our auth realm          %s\n", gateway_cfg.realm);
	tris_cli(a->fd, "  Call to non-local dom.: %s\n", cli_yesno(gateway_cfg.allow_external_domains));
	tris_cli(a->fd, "  URI user is phone no:   %s\n", cli_yesno(tris_test_flag(&global_flags[0], GATEWAY_USEREQPHONE)));
 	tris_cli(a->fd, "  Always auth rejects:    %s\n", cli_yesno(gateway_cfg.alwaysauthreject));
	tris_cli(a->fd, "  Direct RTP setup:       %s\n", cli_yesno(gateway_cfg.directrtpsetup));
	...
	tris_cli(a->fd, "  Regexten on Qualify:    %s\n", cli_yesno(gateway_cfg.regextenonqualify));
	tris_cli(a->fd, "  Caller ID:              %s\n", default_callerid);
	tris_cli(a->fd, "  From: Domain:           %s\n", default_fromdomain);
	tris_cli(a->fd, "  Record GATEWAY history:     %s\n", recordhistory ? "On" : "Off");
	tris_cli(a->fd, "  Call Events:            %s\n", gateway_cfg.callevents ? "On" : "Off");
	tris_cli(a->fd, "  Auth. Failure Events:   %s\n", global_authfailureevents ? "On" : "Off");

	tris_cli(a->fd, "  T.38 support:           %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_T38SUPPORT)));
	tris_cli(a->fd, "  T.38 EC mode:           %s\n", faxec2str(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_T38SUPPORT)));
	tris_cli(a->fd, "  T.38 MaxDtgrm:          %d\n", global_t38_maxdatagram);

	if (!realtimepeers && !realtimeregs)
		tris_cli(a->fd, "  GATEWAY realtime:           Disabled\n" );
	else
		tris_cli(a->fd, "  GATEWAY realtime:           Enabled\n" );
	tris_cli(a->fd, "  Qualify Freq :          %d ms\n", global_qualifyfreq);
	tris_cli(a->fd, "\nNetwork QoS Settings:\n");
	tris_cli(a->fd, "---------------------------\n");
	tris_cli(a->fd, "  IP ToS GATEWAY:             %s\n", tris_tos2str(global_tos_gateway));
	...
	tris_cli(a->fd, "  802.1p CoS GATEWAY:         %d\n", global_cos_gateway);
	...
	tris_cli(a->fd, "\nNetwork Settings:\n");
	tris_cli(a->fd, "---------------------------\n");
	/* determine if/how GATEWAY address can be remapped */
	if (localaddr == NULL)
	...
	tris_cli(a->fd, "  GATEWAY address remapping:  %s\n", msg);
	...
	tris_cli(a->fd, "  RFC2833 Compensation:   %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RFC2833_COMPENSATE)));
	tris_cli(a->fd, "  Compact GATEWAY headers:    %s\n", cli_yesno(gateway_cfg.compactheaders));
	...
	tris_cli(a->fd, "  DNS SRV lookup:         %s\n", cli_yesno(gateway_cfg.srvlookup));
	tris_cli(a->fd, "  Pedantic GATEWAY support:   %s\n", cli_yesno(gateway_cfg.pedanticgatewaychecking));
	...
	tris_cli(a->fd, "  Notify ringing state:   %s\n", cli_yesno(gateway_cfg.notifyringing));
	if (gateway_cfg.notifyringing) {
		tris_cli(a->fd, "    Include CID:          %s%s\n",
				cli_yesno(gateway_cfg.notifycid),
				gateway_cfg.notifycid == IGNORE_CONTEXT ? " (Ignoring context)" : "");
	}
	tris_cli(a->fd, "  Notify hold state:      %s\n", cli_yesno(gateway_cfg.notifyhold));
	tris_cli(a->fd, "  GATEWAY Transfer mode:      %s\n", transfermode2str(gateway_cfg.allowtransfer));
	tris_cli(a->fd, "  Max Call Bitrate:       %d kbps\n", default_maxcallbitrate);
	tris_cli(a->fd, "  Auto-Framing:           %s\n", cli_yesno(global_autoframing));
	tris_cli(a->fd, "  Outb. proxy:            %s %s\n", tris_strlen_zero(gateway_cfg.outboundproxy.name) ? "<not set>" : gateway_cfg.outboundproxy.name,
							gateway_cfg.outboundproxy.force ? "(forced)" : "");
	...
	tris_cli(a->fd, "  Context:                %s\n", gateway_cfg.default_context);
	tris_cli(a->fd, "  Nat:                    %s\n", nat2str(tris_test_flag(&global_flags[0], GATEWAY_NAT)));
	tris_cli(a->fd, "  DTMF:                   %s\n", dtmfmode2str(tris_test_flag(&global_flags[0], GATEWAY_DTMF)));
	tris_cli(a->fd, "  Qualify:                %d\n", default_qualify);
	tris_cli(a->fd, "  Use ClientCode:         %s\n", cli_yesno(tris_test_flag(&global_flags[0], GATEWAY_USECLIENTCODE)));
	tris_cli(a->fd, "  Progress inband:        %s\n", (tris_test_flag(&global_flags[0], GATEWAY_PROG_INBAND) == GATEWAY_PROG_INBAND_NEVER) ? "Never" : (tris_test_flag(&global_flags[0], GATEWAY_PROG_INBAND) == GATEWAY_PROG_INBAND_NO) ? "No" : "Yes" );
	tris_cli(a->fd, "  Language:               %s\n", default_language);
	...
	if (realtimepeers || realtimeregs) {
		...
		tris_cli(a->fd, "  Cache Friends:          %s\n", cli_yesno(tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS)));
		tris_cli(a->fd, "  Update:                 %s\n", cli_yesno(gateway_cfg.peer_rtupdate));
		tris_cli(a->fd, "  Ignore Reg. Expire:     %s\n", cli_yesno(gateway_cfg.ignore_regexpire));
		tris_cli(a->fd, "  Save sys. name:         %s\n", cli_yesno(gateway_cfg.rtsave_sysname));
		tris_cli(a->fd, "  Auto Clear:             %d\n", gateway_cfg.rtautoclear);
	}
	tris_cli(a->fd, "\n----\n");
	return CLI_SUCCESS;
}
...
#define FORMAT  "%-30.30s  %-12.12s  %-10.10s  %-10.10s\n"
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show mwi";
		e->usage =
			"Usage: gateway show mwi\n"
...
static int show_channels_cb(void *__cur, void *__arg, int flags)
{
	...
	if (cur->subscribed == NONE && !arg->subscriptions) {
		...
		tris_cli(arg->fd, FORMAT, tris_inet_ntoa(dst->sin_addr), 
				...
				cli_yesno(tris_test_flag(&cur->flags[1], GATEWAY_PAGE2_CALL_ONHOLD)),
				cur->needdestroy ? "(d)" : "",
				cur->lastmsg ,
				referstatus
			);
 ...
 */
static char *gateway_show_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct __show_chan_arg arg = { .fd = a->fd, .numchans = 0 };


	if (cmd == CLI_INIT) {
		e->command = "gateway show {channels|subscriptions}";
		e->usage =
			"Usage: gateway show channels\n"
			"       Lists all currently active GATEWAY calls (dialogs).\n"
			"Usage: gateway show subscriptions\n"
			"       Lists active GATEWAY subscriptions.\n";
		return NULL;
	} else if (cmd == CLI_GENERATE)
		return NULL;
	...
	tris_cli(arg.fd, "%d active GATEWAY %s%s\n", arg.numchans,
...
/*! \brief Support routine for 'gateway show channel' and 'gateway show history' CLI
 ...
 */
static char *complete_gatewaych(const char *line, const char *word, int pos, int state)
{
	int which=0;
	struct gateway_pvt *cur;
	char *c = NULL;
	int wordlen = strlen(word);
	struct ao2_iterator i;
...
/*! \brief Support routine for 'gateway show history' CLI */
static char *complete_gateway_show_history(const char *line, const char *word, int pos, int state)
{
	if (pos == 3)
		return complete_gatewaych(line, word, pos, state);

	return NULL;
}

/*! \brief Support routine for 'gateway show peer' CLI */
static char *complete_gateway_show_peer(const char *line, const char *word, int pos, int state)
{
	if (pos == 3) {
		return complete_gateway_peer(word, state, 0);
	}

	return NULL;
}

/*! \brief Support routine for 'gateway unregister' CLI */
static char *complete_gateway_unregister(const char *line, const char *word, int pos, int state)
{
       if (pos == 2)
               return complete_gateway_registered_peer(word, state, 0);

       return NULL;
}

/*! \brief Support routine for 'gateway notify' CLI */
static char *complete_gatewaynotify(const char *line, const char *word, int pos, int state)
{
	char *c = NULL;
...
static char *gateway_show_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show channel";
		e->usage =
			"Usage: gateway show channel <call-id>\n"
			"       Provides detailed status on a given GATEWAY dialog (identified by GATEWAY call-id).\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gatewaych(a->line, a->word, a->pos, a->n);
	}
	...
	while ((cur = ao2_t_iterator_next(&i, "iterate thru dialogs"))) {
		...
		if (!strncasecmp(cur->callid, a->argv[3], len)) {
			...
			if (cur->subscribed != NONE)
				...
				tris_cli(a->fd, "  * GATEWAY Call\n");
			tris_cli(a->fd, "  Curr. trans. direction:  %s\n", tris_test_flag(&cur->flags[0], GATEWAY_OUTGOING) ? "Outgoing" : "Incoming");
			...
			tris_cli(a->fd, "  NAT Support:            %s\n", nat2str(tris_test_flag(&cur->flags[0], GATEWAY_NAT)));
			...
			tris_cli(a->fd, "  GATEWAY User agent:         %s\n", cur->useragent);
			...
			tris_cli(a->fd, "  Promiscuous Redir:      %s\n", cli_yesno(tris_test_flag(&cur->flags[0], GATEWAY_PROMISCREDIR)));
			tris_cli(a->fd, "  Route:                  %s\n", cur->route ? cur->route->hop : "N/A");
			tris_cli(a->fd, "  DTMF Mode:              %s\n", dtmfmode2str(tris_test_flag(&cur->flags[0], GATEWAY_DTMF)));
			tris_cli(a->fd, "  GATEWAY Options:            ");
			if (cur->gatewayoptions) {
				int x;
				for (x = 0 ; x < ARRAY_LEN(gateway_options); x++) {
					if (cur->gatewayoptions & gateway_options[x].id)
						tris_cli(a->fd, "%s ", gateway_options[x].text);
				}
				tris_cli(a->fd, "\n");
			} else
				tris_cli(a->fd, "(none)\n");
	...
	if (!found) 
		tris_cli(a->fd, "No such GATEWAY Call ID starting with '%s'\n", a->argv[3]);
...
static char *gateway_show_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway show history";
		e->usage =
			"Usage: gateway show history <call-id>\n"
			"       Provides detailed dialog history on a given GATEWAY call (specified by call-id).\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gateway_show_history(a->line, a->word, a->pos, a->n);
	}
	...
	if (!recordhistory)
		tris_cli(a->fd, "\n***Note: History recording is currently DISABLED.  Use 'gateway set history on' to ENABLE.\n");
...
/*! \brief Dump GATEWAY history to debug log file at end of lifespan for GATEWAY dialog */
static void gateway_dump_history(struct gateway_pvt *dialog)
{
	int x = 0;
	struct gateway_history *hist;
	...
	if (!option_debug && !gatewaydebug) {
		if (!errmsg) {
			tris_log(LOG_NOTICE, "You must have debugging enabled (GATEWAY or Trismedia) in order to dump GATEWAY history.\n");
			errmsg = 1;
		}
		return;
	}

	tris_debug(1, "\n---------- GATEWAY HISTORY for '%s' \n", dialog->callid);
	if (dialog->subscribed)
		tris_debug(1, "  * Subscription\n");
	else
		tris_debug(1, "  * GATEWAY Call\n");
	if (dialog->history)
		TRIS_LIST_TRAVERSE(dialog->history, hist, list)
			tris_debug(1, "  %-3.3d. %s\n", ++x, hist->event);
	if (!x)
		tris_debug(1, "Call '%s' has no history\n", dialog->callid);
	tris_debug(1, "\n---------- END GATEWAY HISTORY for '%s' \n", dialog->callid);
}


/*! \brief  Receive GATEWAY INFO Message */
static void handle_request_info(struct gateway_pvt *p, struct gateway_request *req)
{
	char buf[1024];
	unsigned int event;
	...
	if (!strcasecmp(c, "application/dtmf-relay") ||
		...
		if (tris_test_flag(&p->flags[0], GATEWAY_USECLIENTCODE)) {
...
/*! \brief Enable GATEWAY Debugging for a single IP */
static char *gateway_do_debug_ip(int fd, char *arg)
{
	struct hostent *hp;
	struct tris_hostent ahp;
	...
	if (port == 0)
		tris_cli(fd, "GATEWAY Debugging Enabled for IP: %s\n", tris_inet_ntoa(debugaddr.sin_addr));
	else
		tris_cli(fd, "GATEWAY Debugging Enabled for IP: %s:%d\n", tris_inet_ntoa(debugaddr.sin_addr), port);

	gatewaydebug |= gateway_debug_console;

	return CLI_SUCCESS;
}

/*! \brief  Turn on GATEWAY debugging for a given peer */
static char *gateway_do_debug_peer(int fd, char *arg)
{
	struct gateway_peer *peer = find_peer(arg, NULL, TRUE, FINDPEERS, FALSE, 0);
	if (!peer)
	...
	if (peer)
		unref_peer(peer, "gateway_do_debug_peer: unref_peer, from find_peer call");
	return CLI_SUCCESS;
}

/*! \brief Turn on GATEWAY debugging (CLI command) */
static char *gateway_do_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int oldgatewaydebug = gatewaydebug & gateway_debug_console;
	char *what;

	if (cmd == CLI_INIT) {
		e->command = "gateway set debug {on|off|ip|peer}";
		e->usage =
			"Usage: gateway set debug {off|on|ip addr[:port]|peer peername}\n"
			"       Globally disables dumping of GATEWAY packets,\n"
			"       or enables it either globally or for a (single)\n"
			"       IP address or registered peer.\n";
		return NULL;
	} else if (cmd == CLI_GENERATE) {
		if (a->pos == 4 && !strcasecmp(a->argv[3], "peer")) 
			return complete_gateway_peer(a->word, a->n, 0);
		return NULL;
        }
	...
	if (a->argc == e->args) {       /* on/off */
		if (!strcasecmp(what, "on")) {
			gatewaydebug |= gateway_debug_console;
			gatewaydebug_text = 1;	/*! \note this can be a special debug command - "gateway debug text" or something */
			memset(&debugaddr, 0, sizeof(debugaddr));
			tris_cli(a->fd, "GATEWAY Debugging %senabled\n", oldgatewaydebug ? "re-" : "");
			return CLI_SUCCESS;
		} else if (!strcasecmp(what, "off")) {
			gatewaydebug &= ~gateway_debug_console;
			gatewaydebug_text = 0;
			tris_cli(a->fd, "GATEWAY Debugging Disabled\n");
			return CLI_SUCCESS;
		}
	} else if (a->argc == e->args +1) {/* ip/peer */
		if (!strcasecmp(what, "ip"))
			return gateway_do_debug_ip(a->fd, a->argv[e->args]);
		else if (!strcasecmp(what, "peer"))
			return gateway_do_debug_peer(a->fd, a->argv[e->args]);
	}
	return CLI_SHOWUSAGE;   /* default, failure */
}

/*! \brief Cli command to send GATEWAY notify to peer */
static char *gateway_cli_notify(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway notify";
		e->usage =
			"Usage: gateway notify <type> <peer> [<peer>...]\n"
			"       Send a NOTIFY message to a GATEWAY peer or peers\n"
			"       Message types are defined in gateway_notify.conf\n";
		return NULL;
	case CLI_GENERATE:
		return complete_gatewaynotify(a->line, a->word, a->pos, a->n);
	}
	...
	for (i = 3; i < a->argc; i++) {
		...
		if (!(p = gateway_alloc(NULL, NULL, 0, GATEWAY_NOTIFY, NULL))) {
			tris_log(LOG_WARNING, "Unable to build gateway pvt data for notify (memory/socket error)\n");
			return CLI_FAILURE;
		}
...
/*! \brief Enable/Disable GATEWAY History logging (CLI) */
static char *gateway_set_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway set history {on|off}";
		e->usage =
			"Usage: gateway set history {on|off}\n"
			"       Enables/Disables recording of GATEWAY dialog history for debugging purposes.\n"
			"       Use 'gateway show history' to view the history of a call number.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	if (!strncasecmp(a->argv[e->args - 1], "on", 2)) {
		recordhistory = TRUE;
		tris_cli(a->fd, "GATEWAY History Recording Enabled (use 'gateway show history')\n");
	} else if (!strncasecmp(a->argv[e->args - 1], "off", 3)) {
		recordhistory = FALSE;
		tris_cli(a->fd, "GATEWAY History Recording Disabled\n");
	} else {
		return CLI_SHOWUSAGE;
	}
	return CLI_SUCCESS;
...
static int do_register_auth(struct gateway_pvt *p, struct gateway_request *req, enum gateway_auth_type code)
{
	...
	memset(digest, 0, sizeof(digest));
	if (reply_digest(p, req, header, GATEWAY_REGISTER, digest, sizeof(digest))) {
		/* There's nothing to use for authentication */
 		/* No digest challenge in request */
 		if (gateway_debug_test_pvt(p) && p->registry)
	...
	return transmit_register(p->registry, GATEWAY_REGISTER, digest, respheader); 
}

/*! \brief Add authentication on outbound GATEWAY packet */
static int do_proxy_auth(struct gateway_pvt *p, struct gateway_request *req, enum gateway_auth_type code, int gatewaymethod, int init)
{
	char *header, *respheader;
	char digest[1024];
	...
	p->authtries++;
	auth_headers(code, &header, &respheader);
	tris_debug(2, "Auth attempt %d on %s\n", p->authtries, gateway_methods[gatewaymethod].text);
	memset(digest, 0, sizeof(digest));
	if (reply_digest(p, req, header, gatewaymethod, digest, sizeof(digest) )) {
	...
	return transmit_invite(p, gatewaymethod, gatewaymethod == GATEWAY_INVITE, init); 
}

/*! \brief  reply to authentication for outbound registrations
\return	Returns -1 if we have no auth 
\note	This is used for register= servers in gateway.conf, GATEWAY proxies we register
...
static int reply_digest(struct gateway_pvt *p, struct gateway_request *req, char *header, int gatewaymethod,  char *digest, int digest_len)
{
	...
	if (p->registry) {
		struct gateway_registry *r = p->registry;
	...
	return build_reply_digest(p, gatewaymethod, digest, digest_len); 
}
...
static int build_reply_digest(struct gateway_pvt *p, int method, char* digest, int digest_len)
{
	...
	struct gateway_auth *auth = NULL;	/* Realm authentication */
	...
	if (tris_strlen_zero(username))	/* We have no authentication */
 	...
 	/* Calculate GATEWAY digest response */
 	snprintf(a1, sizeof(a1), "%s:%s:%s", username, p->realm, secret);
	snprintf(a2, sizeof(a2), "%s:%s", gateway_methods[method].text, uri);
	if (!tris_strlen_zero(md5secret))
		tris_copy_string(a1_hash, md5secret, sizeof(a1_hash));
	else
		tris_md5_hash(a1_hash, a1);
...
/*! \brief Read GATEWAY header (dialplan function) */
static int func_header_read(struct tris_channel *chan, const char *function, char *data, char *buf, size_t len) 
{
	...
	tris_channel_lock(chan);
	if (!IS_GATEWAY_TECH(chan->tech)) {
		tris_log(LOG_WARNING, "This function can only be used on GATEWAY channels.\n");
		tris_channel_unlock(chan);
		return -1;
	}
...
static struct tris_custom_function gateway_header_function = {
	.name = "GATEWAY_HEADER",
	.read = func_header_read,
};

/*! \brief  Dial plan function to check if domain is local */
static int func_check_gatewaydomain(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "CHECKGATEWAYDOMAIN requires an argument - A domain name\n");
		return -1;
	}
	if (check_gateway_domain(data, NULL, 0))
...
static struct tris_custom_function checkgatewaydomain_function = {
	.name = "CHECKGATEWAYDOMAIN",
	.read = func_check_gatewaydomain,
};

/*! \brief  ${GATEWAYPEER()} Dialplan function - reads peer data */
static int function_gatewaypeer(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct gateway_peer *peer;
	char *colname;
	...
	if ((colname = strchr(data, ':'))) {	/*! \todo Will be deprecated after 1.4 */
		...
		*colname++ = '\0';
		if (deprecation_warning++ % 10 == 0)
			tris_log(LOG_WARNING, "GATEWAYPEER(): usage of ':' to separate arguments is deprecated.  Please use ',' instead.\n");
	...
	unref_peer(peer, "unref_peer from function_gatewaypeer, just before return");

	return 0;
}

/*! \brief Structure to declare a dialplan function: GATEWAYPEER */
static struct tris_custom_function gatewaypeer_function = {
	.name = "GATEWAYPEER",
	.read = function_gatewaypeer,
};

/*! \brief ${GATEWAYCHANINFO()} Dialplan function - reads gateway channel data */
static int function_gatewaychaninfo_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct gateway_pvt *p;
	...
	*buf = 0;
	...
	if (deprecated++ % 20 == 0) {
		/* Deprecated in 1.6.1 */
		tris_log(LOG_WARNING, "GATEWAYCHANINFO() is deprecated.  Please transition to using CHANNEL().\n");
	}
...
/*! \brief Structure to declare a dialplan function: GATEWAYCHANINFO */
static struct tris_custom_function gatewaychaninfo_function = {
	.name = "GATEWAYCHANINFO",
	.read = function_gatewaychaninfo_read,
};

/*! \brief Parse 302 Moved temporalily response 
	\todo XXX Doesn't redirect over TLS on sips: uri's.
		If we get a redirect to a GATEWAYS: uri, this needs to be going back to the
...
static void parse_moved_contact(struct gateway_pvt *p, struct gateway_request *req)
{
	...
	if (tris_test_flag(&p->flags[0], GATEWAY_PROMISCREDIR)) {
		char *host = NULL;
		...
		if (p->owner) {
			pbx_builtin_setvar_helper(p->owner, "GATEWAYDOMAIN", domain);
			tris_string_field_set(p->owner, call_forward, s);
		}
	}
}

/*! \brief Check pending actions on GATEWAY call */
static void check_pendings(struct gateway_pvt *p)
{
	if (tris_test_flag(&p->flags[0], GATEWAY_PENDINGBYE)) {
		...
		if (p->invitestate == INV_PROCEEDING || p->invitestate == INV_EARLY_MEDIA) {
			...
			transmit_request_with_auth(p, GATEWAY_BYE, 0, XMIT_RELIABLE, TRUE);
		}
		tris_clear_flag(&p->flags[0], GATEWAY_PENDINGBYE);	
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
	} else if (tris_test_flag(&p->flags[0], GATEWAY_NEEDREINVITE)) {
...
static int gateway_reinvite_retry(const void *data)
{
	...
	gateway_pvt_lock(p); /* called from schedule thread which requires a lock */
	tris_set_flag(&p->flags[0], GATEWAY_NEEDREINVITE);
	p->waitid = -1;
	check_pendings(p);
	gateway_pvt_unlock(p);
	dialog_unref(p, "unref the dialog ptr from gateway_reinvite_retry, because it held a dialog ptr");
	return 0;
}


/*! \brief Handle GATEWAY response to INVITE dialogue */
static void handle_response_invite(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno)
{
	int outgoing = tris_test_flag(&p->flags[0], GATEWAY_OUTGOING);
	int res = 0;
	int xmitres = 0;
	...
	if (reinvite)
		tris_debug(4, "GATEWAY response %d to RE-invite on %s call %s\n", resp, outgoing ? "outgoing" : "incoming", p->callid);
	else
		tris_debug(4, "GATEWAY response %d to standard invite\n", resp);
	...
	/* Acknowledge sequence number - This only happens on INVITE from GATEWAY-call */
	...
	*/
	...
	case 101:	/* Dialog establishment */
		if (!req->ignore && p->invitestate != INV_CANCELLED && gateway_cancel_destroy(p))
			tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
		check_pendings(p);
		break;
	...
	case 200:	/* 200 OK on invite - someone's answering our call */
		...
		/* This is important when we have a GATEWAY proxy between us and the phone */
		if (outgoing) {
		...
		if (!req->ignore && p->owner) {
			if (!reinvite) {
				...
				if (gateway_cfg.callevents)
					manager_event(EVENT_FLAG_SYSTEM, "ChannelUpdate",
						"Channel: %s\r\nChanneltype: %s\r\nUniqueid: %s\r\nGATEWAYcallid: %s\r\nGATEWAYfullcontact: %s\r\nPeername: %s\r\n",
						p->owner->name, "GATEWAY", p->owner->uniqueid, p->callid, p->fullcontact, p->peername);
			} else {	/* RE-invite */
				tris_queue_frame(p->owner, &tris_null_frame);
			}
		} else {
		...
		p->invitestate = INV_TERMINATED;
		tris_set_flag(&p->flags[1], GATEWAY_PAGE2_DIALOG_ESTABLISHED);
		xmitres = transmit_request(p, GATEWAY_ACK, seqno, XMIT_UNRELIABLE, TRUE);
		check_pendings(p);
		break;
	...
	case 401: /* Www auth */
		/* First we ACK */
		xmitres = transmit_request(p, GATEWAY_ACK, seqno, XMIT_UNRELIABLE, FALSE);
		if (p->options)
		...
		if (!req->ignore) {
			if (p->authtries < MAX_AUTHTRIES)
				p->invitestate = INV_CALLING;
			if (p->authtries == MAX_AUTHTRIES || do_proxy_auth(p, req, resp, GATEWAY_INVITE, 1)) {
	...
	case 408: /* Request timeout */
		tris_log(LOG_WARNING, "Re-invite to non-existing call leg on other UA. GATEWAY dialog '%s'. Giving up.\n", p->callid);
		xmitres = transmit_request(p, GATEWAY_ACK, seqno, XMIT_UNRELIABLE, FALSE);
		if (p->owner)
	...
	case 428: /* Use identity header - rfc 4474 - not supported by Trismedia yet */
		xmitres = transmit_request(p, GATEWAY_ACK, seqno, XMIT_UNRELIABLE, FALSE);
		append_history(p, "Identity", "GATEWAY identity is required. Not supported by Trismedia.");
		tris_log(LOG_WARNING, "GATEWAY identity required by proxy. GATEWAY dialog '%s'. Giving up.\n", p->callid);
		if (p->owner)
			tris_queue_control(p->owner, TRIS_CONTROL_CONGESTION);
		break;
	...
	case 487: /* Cancelled transaction */
		/* We have sent CANCEL on an outbound INVITE 
			This transaction is already scheduled to be killed by gateway_hangup().
		*/
		xmitres = transmit_request(p, GATEWAY_ACK, seqno, XMIT_UNRELIABLE, FALSE);
		if (p->owner && !req->ignore) {
	...
	case 491: /* Pending */
		...
		if (p->owner && !req->ignore) {
			if (p->owner->_state != TRIS_STATE_UP) {
				...
				p->waitid = tris_sched_add(sched, wait, gateway_reinvite_retry, dialog_ref(p, "passing dialog ptr into sched structure based on waitid for gateway_reinvite_retry."));
				tris_log(LOG_WARNING, "just did sched_add waitid(%d) for gateway_reinvite_retry for dialog %s in handle_response_invite\n", p->waitid, p->callid);
...
/* \brief Handle GATEWAY response in NOTIFY transaction
...
static void handle_response_notify(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno)
{
	...
	case 200:   /* Notify accepted */
		...
		if (p->owner) {
			...
			if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_STATECHANGEQUEUE)) {
				/* Ready to send the next state we have on queue */
				tris_clear_flag(&p->flags[1], GATEWAY_PAGE2_STATECHANGEQUEUE);
				cb_extensionstate((char *)p->context, (char *)p->exten, p->laststate, (void *) p);
			}
		}
		break;
	case 401:   /* Not www-authorized on GATEWAY method */
	case 407:   /* Proxy auth */
		if (!p->notify_headers) {
		...
		if (p->authtries > 1 || do_proxy_auth(p, req, resp, GATEWAY_NOTIFY, 0)) {
...
/* \brief Handle GATEWAY response in SUBSCRIBE transaction */
static void handle_response_subscribe(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno)
{
	if (!p->mwi) {
		return;
	...
	case 200: /* Subscription accepted */
		...
		p->mwi->subscribed = 1;
		if ((p->mwi->resub = tris_sched_add(sched, mwi_expiry * 1000, gateway_subscribe_mwi_do, ASTOBJ_REF(p->mwi))) < 0) {
			ASTOBJ_UNREF(p->mwi, gateway_subscribe_mwi_destroy);
		}
		break;
	case 401:
	case 407:
		tris_string_field_set(p, theirtag, NULL);
		if (p->authtries > 1 || do_proxy_auth(p, req, resp, GATEWAY_SUBSCRIBE, 0)) {
...
/* \brief Handle GATEWAY response in REFER transaction
...
static void handle_response_refer(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno)
{
	...
	case 407:   /* Proxy auth */
		...
		if (p->authtries > 1 || do_proxy_auth(p, req, resp, GATEWAY_REFER, 0)) {
	...
	case 501:   /* Method not implemented */
		...
		tris_log(LOG_NOTICE, "GATEWAY transfer to %s failed, call miserably fails. \n", p->refer->refer_to);
		pvt_set_needdestroy(p, "received 500/501 response");
		p->refer->status = REFER_FAILED;
		break;
	case 603:   /* Transfer declined */
		tris_log(LOG_NOTICE, "GATEWAY transfer to %s declined, call miserably fails. \n", p->refer->refer_to);
		p->refer->status = REFER_FAILED;
...
static int handle_response_register(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno)
{
	...
	case 403:	/* Forbidden */
		...
		break;
	case 404:	/* Not found */
		tris_log(LOG_WARNING, "Got 404 Not found on GATEWAY register to service %s@%s, giving up\n", p->registry->username, p->registry->hostname);
		pvt_set_needdestroy(p, "received 404 response");
		if (r->call)
	...
	case 423:	/* Interval too brief */
		...
		if (r->expiry > max_expiry) {
			...
			r->regstate = REG_STATE_UNREGISTERED;
			transmit_register(r, GATEWAY_REGISTER, NULL, NULL);
		}
		manager_event(EVENT_FLAG_SYSTEM, "Registry", "ChannelType: GATEWAY\r\nUsername: %s\r\nDomain: %s\r\nStatus: %s\r\n", r->username, r->hostname, regstate2str(r->regstate));
		break;
	...
	case 200:	/* 200 OK */
		...
		TRIS_SCHED_REPLACE_UNREF(r->expire, sched, expires_ms, gateway_reregister, r, 
...
static void handle_response_peerpoke(struct gateway_pvt *p, int resp, struct gateway_request *req)
{
	struct gateway_peer *peer = /* ref_peer( */ p->relatedpeer /* , "bump refcount on p, as it is being used in this function(handle_response_peerpoke)")*/ ; /* hope this is already refcounted! */
	...
	if (statechanged) {
		...
		tris_devstate_changed(TRIS_DEVICE_UNKNOWN, "SIP/%s", peer->name);
		if (gateway_cfg.peer_rtupdate) {
			tris_update_realtime(tris_check_realtime("gatewayregs") ? "gatewayregs" : "gatewaypeers", "name", peer->name, "lastms", str_lastms, SENTINEL);
		}
		manager_event(EVENT_FLAG_SYSTEM, "PeerStatus",
			"ChannelType: GATEWAY\r\nPeer: SIP/%s\r\nPeerStatus: %s\r\nTime: %d\r\n",
			peer->name, s, pingtime);
		if (is_reachable && gateway_cfg.regextenonqualify)
			register_peer_exten(peer, TRUE);
	}
...
/*! \brief Handle GATEWAY response in dialogue
...
static void handle_response(struct gateway_pvt *p, int resp, char *rest, struct gateway_request *req, int seqno)
{
	...
	gatewaymethod = find_gateway_method(msg);

	owner = p->owner;
	if (owner) 
		owner->hangupcause = hangup_gateway2cause(resp);
	...
	if (p->socket.type == GATEWAY_TRANSPORT_UDP) {
		...
		if ((resp >= 100) && (resp <= 199)) {
			ack_res = __gateway_semi_ack(p, seqno, 0, gatewaymethod);
		} else {
			ack_res = __gateway_ack(p, seqno, 0, gatewaymethod);
		}
	...
	if (!p->owner && gatewaymethod == GATEWAY_NOTIFY && p->pendinginvite) 
		p->pendinginvite = 0;
	...
	*/
	 ...
	 */
	if ((resp == 404 || resp == 408 || resp == 481) && gatewaymethod == GATEWAY_BYE) {
		pvt_set_needdestroy(p, "received 4XX response to a BYE");
		return;
	}

	if (p->relatedpeer && p->method == GATEWAY_OPTIONS) {
		/* We don't really care what the response is, just that it replied back. 
		   Well, as long as it's not a 100 response...  since we might
		   need to hang around for something more "definitive" */
		if (resp != 100)
			handle_response_peerpoke(p, resp, req);
	} else if (tris_test_flag(&p->flags[0], GATEWAY_OUTGOING)) {
		switch(resp) {
		case 100:	/* 100 Trying */
		...
		case 200:	/* 200 OK */
			...
			if (gatewaymethod == GATEWAY_MESSAGE || gatewaymethod == GATEWAY_INFO) {
				/* We successfully transmitted a message 
					or a video update request in INFO */
				/* Nothing happens here - the message is inside a dialog */
			} else if (gatewaymethod == GATEWAY_INVITE) {
				handle_response_invite(p, resp, rest, req, seqno);
			} else if (gatewaymethod == GATEWAY_NOTIFY) {
				handle_response_notify(p, resp, rest, req, seqno);
			} else if (gatewaymethod == GATEWAY_REGISTER) {
				res = handle_response_register(p, resp, rest, req, seqno);
			} else if (gatewaymethod == GATEWAY_SUBSCRIBE) {
				tris_set_flag(&p->flags[1], GATEWAY_PAGE2_DIALOG_ESTABLISHED);
				handle_response_subscribe(p, resp, rest, req, seqno);
			} else if (gatewaymethod == GATEWAY_BYE) {		/* Ok, we're ready to go */
				pvt_set_needdestroy(p, "received 200 response");
				tris_clear_flag(&p->flags[1], GATEWAY_PAGE2_DIALOG_ESTABLISHED);
			}
			break;
		case 202:   /* Transfer accepted */
			if (gatewaymethod == GATEWAY_REFER) 
		...
		case 407: /* Proxy auth required */
			if (gatewaymethod == GATEWAY_INVITE)
				...
				handle_response_notify(p, resp, rest, req, seqno);
			else if (gatewaymethod == GATEWAY_REFER)
				handle_response_refer(p, resp, rest, req, seqno);
			else if (gatewaymethod == GATEWAY_SUBSCRIBE)
				handle_response_subscribe(p, resp, rest, req, seqno);
			else if (p->registry && gatewaymethod == GATEWAY_REGISTER)
				res = handle_response_register(p, resp, rest, req, seqno);
			else if (gatewaymethod == GATEWAY_BYE) {
				if (p->options)
					p->options->auth_type = resp;
				if (tris_strlen_zero(p->authname)) {
		...
		case 403: /* Forbidden - we failed authentication */
			...
			break;
		case 404: /* Not found */
			if (p->registry && gatewaymethod == GATEWAY_REGISTER)
				res = handle_response_register(p, resp, rest, req, seqno);
			else if (gatewaymethod == GATEWAY_INVITE)
				handle_response_invite(p, resp, rest, req, seqno);
			else if (gatewaymethod == GATEWAY_SUBSCRIBE)
			...
			break;
		case 423: /* Interval too brief */
			if (gatewaymethod == GATEWAY_REGISTER)
				res = handle_response_register(p, resp, rest, req, seqno);
			break;
		...
		case 481: /* Call leg does not exist */
			if (gatewaymethod == GATEWAY_INVITE) {
				...
				tris_log(LOG_WARNING, "Remote host can't match request %s to call '%s'. Giving up.\n", gateway_methods[gatewaymethod].text, p->callid);
			} else if (gatewaymethod == GATEWAY_CANCEL) {
		...
		default:
			if ((resp >= 300) && (resp < 700)) {
				/* Fatal response */
				if ((resp != 487))
					tris_verb(3, "Got GATEWAY response %d \"%s\" back from %s\n", resp, rest, tris_inet_ntoa(p->sa.sin_addr));
				...
				case 603: /* Decline */
					...
					break;
				case 482: /*!
					\note GATEWAY is incapable of performing a hairpin call, which
				...
				default:
					/* Send hangup */	
					if (owner && gatewaymethod != GATEWAY_MESSAGE && gatewaymethod != GATEWAY_INFO && gatewaymethod != GATEWAY_BYE)
						tris_queue_hangup_with_cause(p->owner, TRIS_CAUSE_PROTOCOL_ERROR);
					break;
				}
				/* ACK on invite */
				if (gatewaymethod == GATEWAY_INVITE) 
					transmit_request(p, GATEWAY_ACK, seqno, XMIT_UNRELIABLE, FALSE);
				if (gatewaymethod != GATEWAY_MESSAGE && gatewaymethod != GATEWAY_INFO) 
					gateway_alreadygone(p);
				if (!p->owner) {
				...
				if (gatewaymethod == GATEWAY_INVITE) {
					if (!req->ignore && gateway_cancel_destroy(p))
						tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
					if (find_sdp(req))
		...
		/* Responses to OUTGOING GATEWAY requests on INCOMING calls 
		   get handled here. As well as out-of-call message responses */
		if (req->debug)
			tris_verbose("GATEWAY Response message for INCOMING dialog %s arrived\n", msg);

		if (gatewaymethod == GATEWAY_INVITE && resp == 200) {
		...
		case 200:
			if (gatewaymethod == GATEWAY_INVITE) {
				...
				pvt_set_needdestroy(p, "transaction completed");
			} else if (gatewaymethod == GATEWAY_MESSAGE || gatewaymethod == GATEWAY_INFO) {
		...
		case 407:
			if (gatewaymethod == GATEWAY_REFER)
				...
				if (p->authtries == MAX_AUTHTRIES || do_proxy_auth(p, req, resp, gatewaymethod, 0)) {
					tris_log(LOG_NOTICE, "Failed to authenticate on %s to '%s'\n", msg, get_header(&p->initreq, "From"));
					pvt_set_needdestroy(p, "failed to authenticate BYE");
				}
			}
		...
		default:	/* Errors without handlers */
			if ((resp >= 100) && (resp < 200)) {
				if (gatewaymethod == GATEWAY_INVITE) { 	/* re-invite */
					if (!req->ignore && gateway_cancel_destroy(p))
						tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
				}
			}
			if ((resp >= 300) && (resp < 700)) {
				if ((resp != 487))
					tris_verb(3, "Incoming call: Got GATEWAY response %d \"%s\" back from %s\n", resp, rest, tris_inet_ntoa(p->sa.sin_addr));
				switch(resp) {
				case 415: /* Unsupported media type */
				case 488: /* Not acceptable here - codec error */
				case 603: /* Decline */
				...
				case 504: /* Server timeout */
					...
					/* re-invite failed */
					if (gatewaymethod == GATEWAY_INVITE && gateway_cancel_destroy(p))
						tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
					break;
				}
			}
...
/*! \brief Park GATEWAY call support function 
...
static void *gateway_park_thread(void *stuff)
{
	...
	struct gateway_dual *d;
	struct gateway_request req = {0,};
	int ext;
	int res;
	...
	tris_debug(4, "GATEWAY Park: Transferer channel %s, Transferee %s\n", transferer->name, transferee->name);
	...
	if (!res)	{
		/* Transfer succeeded */
		append_history(transferer->tech_pvt, "GATEWAYpark", "Parked call on %d", ext);
		transmit_notify_with_gatewayfrag(transferer->tech_pvt, d->seqno, "200 OK", TRUE);
		transferer->hangupcause = TRIS_CAUSE_NORMAL_CLEARING;
		tris_hangup(transferer); /* This will cause a BYE */
		tris_debug(1, "GATEWAY Call parked on extension '%d'\n", ext);
	} else {
		transmit_notify_with_gatewayfrag(transferer->tech_pvt, d->seqno, "503 Service Unavailable", TRUE);
		append_history(transferer->tech_pvt, "GATEWAYpark", "Parking failed\n");
		tris_debug(1, "GATEWAY Call parked failed \n");
		/* Do not hangup call */
	}
	if (d->req.data)
		tris_free(d->req.data);
...
static int gateway_park(struct tris_channel *chan1, struct tris_channel *chan2, struct gateway_request *req, int seqno)
{
	...
	transferer = tris_channel_alloc(0, TRIS_STATE_DOWN, 0, 0, chan2->accountcode, chan2->exten, chan2->context, chan2->amaflags, "GATEWAYPeer/%s", chan2->name);
	...
	while (tris_channel_trylock(chan2)) {
		struct gateway_pvt *pvt = chan2->tech_pvt;
		gateway_pvt_unlock(pvt);
		usleep(1);
		gateway_pvt_lock(pvt);
	}
	...
	if ((d = tris_calloc(1, sizeof(*d)))) {
		...
		d->seqno = seqno;
		if (tris_pthread_create_detached_background(&th, NULL, gateway_park_thread, d) < 0) {
			/* Could not start thread */
			if (d->req.data)
				tris_free(d->req.data);
			tris_free(d);	/* We don't need it anymore. If thread is created, d will be free'd
					   by gateway_park_thread() */
			return 0;
		}
	} 
	return -1;
...
/*! \brief Turn off generator data 
	XXX Does this function belong in the GATEWAY channel?
*/
static void tris_quiet_chan(struct tris_channel *chan) 
{
...
/*! \brief Attempt transfer of GATEWAY call 
...
static int attempt_transfer(struct gateway_dual *transferer, struct gateway_dual *target)
{
	...
	tris_debug(4, "-- END Gateway transfer:--------------------\n");
	if (transferer->chan2) { /* We have a bridge on the transferer's channel */
		...
		tris_debug(3, "GATEWAY transfer: Four channels to handle\n");
		...
		tris_debug(3, "GATEWAY transfer: Three channels to handle\n");
	}
	...
	if (peera && peerb && peerc && (peerb != peerc)) {
		...
		tris_debug(4, "GATEWAY transfer: trying to masquerade %s into %s\n", peerc->name, peerb->name);
		if (tris_channel_masquerade(peerb, peerc)) {
			tris_log(LOG_WARNING, "Failed to masquerade %s into %s\n", peerb->name, peerc->name);
			res = -1;
		} else
			tris_debug(4, "GATEWAY transfer: Succeeded to masquerade channels.\n");
		return res;
	} else {
		tris_log(LOG_NOTICE, "GATEWAY Transfer attempted with no appropriate bridged calls to transfer\n");
		if (transferer->chan1)
...
static int handle_request_notify(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, int seqno, char *e)
{
	...
	if (!strcmp(event, "refer")) {
		...
		/* EventID for each transfer... EventID is basically the REFER cseq 
		 ...
		 We should hangup when we are getting a 200 OK in a gatewayfrag
		...
		*/
		tris_debug(3, "* GATEWAY Transfer NOTIFY Attachment: \n---%s\n---\n", buf);
		...
		*sep++ = '\0';			/* Response string */
		...
		if (!tris_strlen_zero(c)) {
			...
			if ((event = tris_event_new(TRIS_EVENT_MWI,
						   ...
						   TRIS_EVENT_IE_CONTEXT, TRIS_EVENT_IE_PLTYPE_STR, "GATEWAY_Remote",
		...
		transmit_response(p, "200 OK", req);
		 ...
		 /* Used by Gatewayura/Linksys for NAT pinhole,
...
/*! \brief Handle the transfer part of INVITE with a replaces: header, 
	...
	only p's lock should remain at the end of this function.  p's lock is held by gatewaysock_read()
 */
static int handle_invite_replaces(struct gateway_pvt *p, struct gateway_request *req, int debug, int seqno, struct sockaddr_in *sin, int *nounlock)
{
	int earlyreplace = 0;
	...
	if (targetcall && targetcall->_state == TRIS_STATE_RINGING)
		tris_debug(4, "GATEWAY transfer: Target channel is in ringing state\n");

	if (targetcall) 
		tris_debug(4, "GATEWAY transfer: Invite Replace incoming channel should bridge to channel %s while hanging up channel %s\n", targetcall->name, replacecall->name); 
	else
		tris_debug(4, "GATEWAY transfer: Invite Replace incoming channel should replace and hang up channel %s (one call leg)\n", replacecall->name); 
	...
	if (req->ignore) {
		...
		gateway_pvt_unlock(p->refer->refer_call);
		return 1;
	} 
	if (!c) {
	...
	/* We have three channels to play with
		...
		p->refer->refer_call: GATEWAY pvt dialog from transferer to pbx.
	...
	*/
	...
	if (! earlyreplace && ! oneleggedreplace )
		tris_set_flag(&p->refer->refer_call->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER);	/* Delay hangup */
	...
	/* unlock gateway pvt and owner so hangup can do its thing */
	tris_channel_unlock(replacecall);
	tris_channel_unlock(c);
	gateway_pvt_unlock(p->refer->refer_call);
	gateway_pvt_unlock(p);
	*nounlock = 1;
	...
	tris_hangup(c);
	gateway_pvt_lock(p); /* lock PVT structure again after hangup */

	return 0;
}

/*! \brief helper routine for gateway_uri_cmp
 *
 * This takes the parameters from two GATEWAY URIs and determines
 ...
 */
static int gateway_uri_params_cmp(const char *input1, const char *input2) 
{
	char *params1 = NULL;
	char *params2 = NULL;
	char *pos1;
...
/*! \brief helper routine for gateway_uri_cmp
 *
 * This takes the "headers" from two GATEWAY URIs and determines
 ...
 */
static int gateway_uri_headers_cmp(const char *input1, const char *input2)
{
	char *headers1 = NULL;
	char *headers2 = NULL;
	int zerolength1 = 0;
...
static int gateway_uri_cmp(const char *input1, const char *input2)
{
	 ...
	 * only case-sensitive comparison of a GATEWAY URI
	 */
	if ((host1 && !host2) ||
			(host2 && !host1) ||
	 ...
	 * because the GATEWAY URI checking rules have some interesting exceptions that make
	 * this possible. I will note 2 in particular
	 * 1. hostnames which resolve to the same IP address as well as a hostname and its
	 *    IP address are not considered a match with GATEWAY URI's.
	...
	if (gateway_uri_headers_cmp(headers1, headers2)) {
		return 1;
	}

	/* And now the parameters. Ugh */
	return gateway_uri_params_cmp(params1, params2);
}
...
static int do_magic_pickup(struct tris_channel *channel, const char *extension, const char *context)
{
	...
	tris_str_set(&str, 0, "%s@%s", extension, gateway_cfg.notifycid == IGNORE_CONTEXT ? "PICKUPMARK" : context);
...
static int gateway_t38_abort(const void *data)
{
	struct gateway_pvt *p = (struct gateway_pvt *) data;
	...
	transmit_response_reliable(p, "488 Not acceptable here", &p->initreq);
	p->t38id = -1;
	dialog_unref(p, "unref the dialog ptr from gateway_t38_abort, because it held a dialog ptr");
...
static int handle_request_invite(struct gateway_pvt *p, struct gateway_request *req, int debug, int seqno, struct sockaddr_in *sin, int *recount, char *e, int *nounlock)
{
	...
	if (!p->gatewayoptions) {
		...
		if (!tris_strlen_zero(supported))
			parse_gateway_options(p, supported);
	}

	/* Find out what they require */
	required = get_header(req, "Require");
	if (!tris_strlen_zero(required)) {
		required_profile = parse_gateway_options(NULL, required);
		if (required_profile && !(required_profile & (GATEWAY_OPT_REPLACES | GATEWAY_OPT_TIMER))) {
			...
			tris_log(LOG_WARNING, "Received GATEWAY INVITE with unsupported required extension: %s\n", required);
			p->invitestate = INV_COMPLETED;
			if (!p->lastinvite)
				gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
			res = -1;
	...
	p->gatewayoptions |= required_profile;
	p->reqgatewayoptions = required_profile;

	/* Check if this is a loop */
	if (tris_test_flag(&p->flags[0], GATEWAY_OUTGOING) && p->owner && (p->invitestate != INV_TERMINATED && p->invitestate != INV_CONFIRMED)) {
		/* This is a call to ourself.  Send ourselves an error code and stop
	   	processing immediately, as GATEWAY really has no good mechanism for
	   	being able to call yourself */
		/* If pedantic is on, we need to check the tags. If they're different, this is
	   	in fact a forked call through a GATEWAY proxy somewhere. */
		int different;
		char *initial_rlPart2 = REQ_OFFSET_TO_STR(&p->initreq, rlPart2);
		char *this_rlPart2 = REQ_OFFSET_TO_STR(req, rlPart2);
		if (gateway_cfg.pedanticgatewaychecking)
			different = gateway_uri_cmp(initial_rlPart2, this_rlPart2);
		else
	...
	if (!req->ignore && p->pendinginvite) {
		if (!tris_test_flag(&p->flags[0], GATEWAY_OUTGOING) && (p->invitestate == INV_COMPLETED || p->invitestate == INV_TERMINATED)) {
			 ...
			 * transaction. Calling __gateway_ack will take care of this by clearing the p->pendinginvite and removing the response
			 * from the previous transaction from the list of outstanding packets.
			 */
			__gateway_ack(p, p->pendinginvite, 1, 0);
		} else {
	...
	if (!tris_strlen_zero(p_replaces)) {
		...
		*/
		...
		if (strncmp(replace_id, "pickup-", 7) == 0) {
			struct gateway_pvt *subscription = NULL;
			replace_id += 7; /* Worst case we are looking at \0 */

			if ((subscription = get_gateway_pvt_byid_locked(replace_id, NULL, NULL)) == NULL) {
				...
				gateway_pvt_unlock(subscription);
		...
		if (!error && tris_strlen_zero(pickup.exten) && (p->refer->refer_call = get_gateway_pvt_byid_locked(replace_id, totag, fromtag)) == NULL) {
	...
	if (!req->ignore) {
		...
		if (!p->owner) {	/* Not a re-invite */
			...
			tris_clear_flag(&p->flags[0], GATEWAY_OUTGOING);	/* This is now an inbound dialog */
	...
	if (!p->lastinvite && !req->ignore && !p->owner) {
		...
		res = check_user(p, req, GATEWAY_INVITE, e, XMIT_RELIABLE, sin);
		...
		if (res < 0) { /* Something failed in authentication */
			if (res == AUTH_FAKE_AUTH) {
				...
				transmit_fake_auth_response(p, GATEWAY_INVITE, req, XMIT_RELIABLE);
			} else {
		...
		if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_T38SUPPORT) && !p->udptl && (p->udptl = tris_udptl_new_with_bindaddr(sched, io, 0, bindaddr.sin_addr))) {
			p->t38_maxdatagram = global_t38_maxdatagram;
			set_t38_capabilities(p);
		}
		...
		if (find_sdp(req)) {
			if (process_sdp(p, req, SDP_T38_INITIATE)) {
				...
				p->invitestate = INV_COMPLETED;
				gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				tris_debug(1, "No compatible codecs for this GATEWAY call.\n");
				res = -1;
				goto request_invite_cleanup;
			}
			if (tris_test_flag(&p->flags[1], GATEWAY_PAGE2_CONSTANT_SSRC)) {
				if (p->rtp) {
		...
		tris_debug(1, "Checking GATEWAY call limits for device %s\n", p->username);
		...
		if (!replace_id && gotdest) {	/* No matching extension found */
			if (gotdest == 1 && tris_test_flag(&p->flags[1], GATEWAY_PAGE2_ALLOWOVERLAP))
			...
			c = gateway_new(p, TRIS_STATE_DOWN, S_OR(p->peername, NULL));
			*recount = 1;
		...
		if (gatewaydebug) {
			if (!req->ignore)
				tris_debug(2, "Got a GATEWAY re-invite for call %s\n", p->callid);
			else
				tris_debug(2, "Got a GATEWAY re-transmit of INVITE for call %s\n", p->callid);
		}
		if (!req->ignore)
			reinvite = 1;
		c = p->owner;
	...
	/* Session-Timers */
	if (p->gatewayoptions & GATEWAY_OPT_TIMER) {
		...
		case SESSION_TIMER_MODE_REFUSE:
			if (p->reqgatewayoptions & GATEWAY_OPT_TIMER) {
				transmit_response_with_unsupported(p, "420 Option Disabled", req, required);
				tris_log(LOG_WARNING, "Received GATEWAY INVITE with supported but disabled option: %s\n", required);
				p->invitestate = INV_COMPLETED;
				if (!p->lastinvite) {
					gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				}
		...
		(based on session-timer-mode in gateway.conf) whether to run session-timers for
		this session or not. */
		switch (st_get_mode(p)) {
		case SESSION_TIMER_MODE_ORIGINATE:
			st_active = TRUE;
	...
	if (c) {	/* We have a call  -either a new call or an old one (RE-INVITE) */
		...
		if (c_state != TRIS_STATE_UP && reinvite &&
			 ...
			 * on the GATEWAY socket prior to an application calling tris_read on this channel
		...
		case TRIS_STATE_UP:
			...
			if (p->t38.state == T38_PEER_REINVITE) {
				p->t38id = tris_sched_add(sched, 5000, gateway_t38_abort, dialog_ref(p, "passing dialog ptr into sched structure based on t38id for gateway_t38_abort."));
			} else if (p->t38.state == T38_ENABLED) {
				tris_set_flag(&p->flags[1], GATEWAY_PAGE2_DIALOG_ESTABLISHED);
		...
		if (p && (p->autokillid == -1)) {
			...
			if (!p->jointcapability)
				...
				tris_log(LOG_NOTICE, "Unable to create/find GATEWAY channel for this INVITE\n");
				msg = "503 Unavailable";
			}
...
static int local_attended_transfer(struct gateway_pvt *transferer, struct gateway_dual *current, struct gateway_request *req, int seqno)
{
	struct gateway_dual target;		/* Chan 1: Call from tranferer to Trismedia */
					/* Chan 2: Call from Trismedia to target */
	int res = 0;
	struct gateway_pvt *targetcall_pvt;

	/* Check if the call ID of the replaces header does exist locally */
	if (!(targetcall_pvt = get_gateway_pvt_byid_locked(transferer->refer->replaces_callid, transferer->refer->replaces_callid_totag, 
		...
		if (transferer->refer->localtransfer) {
			...
			transmit_notify_with_gatewayfrag(transferer, seqno, "481 Call leg/transaction does not exist", TRUE);
			append_history(transferer, "Xfer", "Refer failed");
			tris_clear_flag(&transferer->flags[0], GATEWAY_GOTREFER);	
			transferer->refer->status = REFER_FAILED;
			return -1;
		}
		/* Fall through for remote transfers that we did not find locally */
		tris_debug(3, "GATEWAY attended transfer: Not our call - generating INVITE with replaces\n");
		return 0;
	}
	...
	if (!targetcall_pvt->owner) {	/* No active channel */
		tris_debug(4, "GATEWAY attended transfer: Error: No owner of target call\n");
		/* Cancel transfer */
		transmit_notify_with_gatewayfrag(transferer, seqno, "503 Service Unavailable", TRUE);
		append_history(transferer, "Xfer", "Refer failed");
		tris_clear_flag(&transferer->flags[0], GATEWAY_GOTREFER);
		transferer->refer->status = REFER_FAILED;
		gateway_pvt_unlock(targetcall_pvt);
		if (targetcall_pvt)
	...
	if (!target.chan2 || !(target.chan2->_state == TRIS_STATE_UP || target.chan2->_state == TRIS_STATE_RINGING) ) {
		...
		if (target.chan2) 
			tris_debug(4, "GATEWAY attended transfer: Error: Wrong state of target call: %s\n", tris_state2str(target.chan2->_state));
		else if (target.chan1->_state != TRIS_STATE_RING)
			tris_debug(4, "GATEWAY attended transfer: Error: No target channel\n");
		else
			tris_debug(4, "GATEWAY attended transfer: Attempting transfer in ringing state\n");
	}

	/* Transfer */
	if (gatewaydebug) {
		if (current->chan2)	/* We have two bridges */
			tris_debug(4, "GATEWAY attended transfer: trying to bridge %s and %s\n", target.chan1->name, current->chan2->name);
		else			/* One bridge, propably transfer of IVR/voicemail etc */
			tris_debug(4, "GATEWAY attended transfer: trying to make %s take over (masq) %s\n", target.chan1->name, current->chan1->name);
	}

	tris_set_flag(&transferer->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER);	/* Delay hangup */

	/* Perform the transfer */
	manager_event(EVENT_FLAG_CALL, "Transfer", "TransferMethod: GATEWAY\r\nTransferType: Attended\r\nChannel: %s\r\nUniqueid: %s\r\nGATEWAY-Callid: %s\r\nTargetChannel: %s\r\nTargetUniqueid: %s\r\n",
	...
	if (res) {
		/* Failed transfer */
		transmit_notify_with_gatewayfrag(transferer, seqno, "486 Busy Here", TRUE);
		append_history(transferer, "Xfer", "Refer failed");
		if (targetcall_pvt->owner)
			tris_channel_unlock(targetcall_pvt->owner);
		tris_clear_flag(&transferer->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER);
	} else {
		/* Transfer succeeded! */
		...
		transmit_notify_with_gatewayfrag(transferer, seqno, "200 OK", TRUE);
		...
		if (targetcall_pvt->owner) {
			tris_debug(1, "GATEWAY attended transfer: Unlocking channel %s\n", targetcall_pvt->owner->name);
			tris_channel_unlock(targetcall_pvt->owner);
		}
	}
	if (targetcall_pvt)
...
/*! \page GATEWAY_REFER GATEWAY transfer Support (REFER)

	REFER is used for call transfer in GATEWAY. We get a REFER
	...
	in the GATEWAY CC Transfer draft. (Transfer without
	a GRUU)
	...
	This is implemented with GATEWAY hold and transfer.
	...
	be in the INVITE to avoid the wrong UA in a forked GATEWAY proxy
...
static int handle_request_refer(struct gateway_pvt *p, struct gateway_request *req, int debug, int seqno, int *nounlock)
{
	struct gateway_dual current;	/* Chan1: Call between trismedia and transferer */
	...
	if (req->debug)
		tris_verbose("Call %s got a GATEWAY call transfer from %s: (REFER)!\n", p->callid, tris_test_flag(&p->flags[0], GATEWAY_OUTGOING) ? "callee" : "caller");

	if (!p->owner) {
		/* This is a REFER outside of an existing GATEWAY dialog */
	...
	if (p->allowtransfer == TRANSFER_CLOSED ) {
		...
		/* Do not destroy GATEWAY session */
		return 0;
	}

	if (!req->ignore && tris_test_flag(&p->flags[0], GATEWAY_GOTREFER)) {
	...
	if (res != 0) {
		...
		case -2:	/* Syntax error */
			...
			if (req->debug)
				tris_debug(1, "GATEWAY transfer to black hole can't be handled (no refer-to: )\n");
			break;
		case -3:
			transmit_response(p, "603 Declined (Non sip: uri)", req);
			append_history(p, "Xfer", "Refer failed. Non GATEWAY uri");
			if (req->debug)
				tris_debug(1, "GATEWAY transfer to non-GATEWAY uri denied\n");
			break;
		default:
			...
			transmit_notify_with_gatewayfrag(p, seqno, "404 Not found", TRUE);
			tris_clear_flag(&p->flags[0], GATEWAY_GOTREFER);	
			if (req->debug)
				tris_debug(1, "GATEWAY transfer to bad extension: %s\n", p->refer->refer_to);
			break;
		} 
		return 0;
	}
	...
	/* If we do not support GATEWAY domains, all transfers are local */
	if (gateway_cfg.allow_external_domains && check_gateway_domain(p->refer->refer_to_domain, NULL, 0)) {
		p->refer->localtransfer = 1;
		if (gatewaydebug)
			tris_debug(3, "This GATEWAY transfer is local : %s\n", p->refer->refer_to_domain);
	} else if (TRIS_LIST_EMPTY(&domain_list) || check_gateway_domain(p->refer->refer_to_domain, NULL, 0)) {
		/* This PBX doesn't bother with GATEWAY domains or domain is local, so this transfer is local */
		p->refer->localtransfer = 1;
	} else if (gatewaydebug)
			tris_debug(3, "This GATEWAY transfer is to a remote GATEWAY extension (remote domain %s)\n", p->refer->refer_to_domain);
	...
	and let the gateway_call function catch that we need replaces
	...
	*/
	...
	if (gatewaydebug)
		tris_debug(3, "GATEWAY %s transfer: Transferer channel %s, transferee channel %s\n", p->refer->attendedtransfer ? "attended" : "blind", current.chan1->name, current.chan2 ? current.chan2->name : "<none>");
	...
	/*if (!current.chan2 && !p->refer->attendedtransfer) {*/
		...
		/*if (gatewaydebug)
			tris_debug(3, "Refused GATEWAY transfer on non-bridged channel.\n");
		p->refer->status = REFER_FAILED;
	...
	if (current.chan2) {
		if (gatewaydebug)
			tris_debug(4, "Got GATEWAY transfer, applying to bridged peer '%s'\n", current.chan2->name);

		tris_queue_control(current.chan1, TRIS_CONTROL_UNHOLD);
	}

	tris_set_flag(&p->flags[0], GATEWAY_GOTREFER);
	...
	if (p->refer->attendedtransfer) {
		...
		/* Fall through for remote transfers that we did not find locally */
		if (gatewaydebug)
			tris_debug(4, "GATEWAY attended transfer: Still not our call - generating INVITE with replaces\n");
	...
	if (p->refer->localtransfer && !strcmp(p->refer->refer_to, tris_parking_ext())) {
		...
		*nounlock = 1;
		...
		manager_event(EVENT_FLAG_CALL, "Transfer", "TransferMethod: GATEWAY\r\nTransferType: Blind\r\nChannel: %s\r\nUniqueid: %s\r\nGATEWAY-Callid: %s\r\nTargetChannel: %s\r\nTargetUniqueid: %s\r\nTransferExten: %s\r\nTransfer2Parking: Yes\r\n",
			...
			p->refer->refer_to);
		if (gatewaydebug)
			tris_debug(4, "GATEWAY transfer to parking: trying to park %s. Parked by %s\n", current.chan2->name, current.chan1->name);
		gateway_park(current.chan2, current.chan1, req, seqno);
		return res;
	} 
	...
	if (current.chan2) {
		...
		pbx_builtin_setvar_helper(current.chan2, "GATEWAYDOMAIN", p->refer->refer_to_domain);
		pbx_builtin_setvar_helper(current.chan2, "GATEWAYTRANSFER", "yes");
		/* One for the new channel */
		pbx_builtin_setvar_helper(current.chan2, "_GATEWAYTRANSFER", "yes");
		/* Attended transfer to remote host, prepare headers for the INVITE */
		if (p->refer->referred_by) 
			pbx_builtin_setvar_helper(current.chan2, "_GATEWAYTRANSFER_REFERER", p->refer->referred_by);
	}
	/* Generate a Replaces string to be used in the INVITE during attended transfer */
	if (!tris_strlen_zero(p->refer->replaces_callid)) {
		char tempheader[GATEWAYBUFSIZE];
		...
		if (current.chan2)
			pbx_builtin_setvar_helper(current.chan2, "_GATEWAYTRANSFER_REPLACES", tempheader);
	}
	...
	*nounlock = 1;
	...
	/*if (!p->refer->attendedtransfer)
		transmit_notify_with_gatewayfrag(p, seqno, "183 Ringing", FALSE); */

	/* For blind transfer, this will lead to a new call */
	/* For attended transfer to remote host, this will lead to
	   a new GATEWAY call with a replaces header, if the dial plan allows it 
	*/
	/*if (!current.chan2) {*/
		...
		*/
		/*p->refer->status = REFER_FAILED;
		transmit_notify_with_gatewayfrag(p, seqno, "503 Service Unavailable (can't handle one-legged xfers)", TRUE);
		tris_clear_flag(&p->flags[0], GATEWAY_GOTREFER);	
		append_history(p, "Xfer", "Refer failed (only bridged calls).");
		return -1;
	}*/
	/*tris_set_flag(&p->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER);*/	/* Delay hangup */
	...
	if (!res) {
		...
		tris_clear_flag(&p->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER);	/* Don't delay hangup */
		tris_debug(3, "%s transfer failed. Resuming original call.\n", p->refer->attendedtransfer? "Attended" : "Blind");
		append_history(p, "Xfer", "Refer failed.");
		/* Failure of some kind */
		p->refer->status = REFER_FAILED;
		transmit_notify_with_gatewayfrag(p, seqno, "503 Service Unavailable", TRUE);
		tris_clear_flag(&p->flags[0], GATEWAY_GOTREFER);	
		res = -1;
	}
	return res;
...
static int handle_request_cancel(struct gateway_pvt *p, struct gateway_request *req)
{
	...
	if (p->owner)
		...
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
	if (p->initreq.len > 0) {
		struct gateway_pkt *pkt, *prev_pkt;
...
static int acf_channel_read(struct tris_channel *chan, const char *funcname, char *preparse, char *buf, size_t buflen)
{
	...
	/* Sanity check */
	if (!IS_GATEWAY_TECH(chan->tech)) {
		tris_log(LOG_ERROR, "Cannot call %s on a non-GATEWAY channel\n", funcname);
		return 0;
	}
...
static int handle_request_bye(struct gateway_pvt *p, struct gateway_request *req)
{
	...
	if (p->pendinginvite && !tris_test_flag(&p->flags[0], GATEWAY_OUTGOING) && !req->ignore) {
		transmit_response_reliable(p, "487 Request Terminated", &p->initreq);
	}
...
static void add_peer_mwi_subs(struct gateway_peer *peer)
{
	struct gateway_mailbox *mailbox;
...
static int handle_request_subscribe(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, int seqno, char *e)
{
	...
	struct gateway_peer *authpeer = NULL;
	...
	if (p->initreq.headers) {	
		...
		if (p->initreq.method != GATEWAY_SUBSCRIBE) {
			/* This is a SUBSCRIBE within another GATEWAY dialog, which we do not support */
	...
	*/
	...
	if (tris_strlen_zero(eventheader)) {
		transmit_response(p, "489 Bad Event", req);
		tris_debug(2, "Received GATEWAY subscribe for unknown event package: <none>\n");
	...
	res = check_user_full(p, req, GATEWAY_SUBSCRIBE, e, 0, sin, &authpeer);
	...
	if (res < 0) {
		if (res == AUTH_FAKE_AUTH) {
			tris_log(LOG_NOTICE, "Sending fake auth rejection for device %s\n", get_header(req, "From"));
			transmit_fake_auth_response(p, GATEWAY_SUBSCRIBE, req, XMIT_UNRELIABLE);
		} else {
	...
	if (!tris_test_flag(&p->flags[1], GATEWAY_PAGE2_ALLOWSUBSCRIBE)) {
		transmit_response(p, "403 Forbidden (policy)", req);
	...
	if (!strcmp(event, "presence") || !strcmp(event, "dialog")) { /* Presence, RFC 3842 */
		...
		if (pidf_xml && strstr(p->useragent, "Polycom")) {
			...
			/* IETF draft: draft-ietf-gatewayping-dialog-package-05.txt */
		...
		if (!tris_strlen_zero(acceptheader) && strcmp(acceptheader, "application/simple-message-summary")) {
			...
			tris_debug(2, "Received GATEWAY mailbox subscription for unknown format: %s\n", acceptheader);
		...
		*/
		if (!authpeer || TRIS_LIST_EMPTY(&authpeer->mailboxes)) {
			...
			pvt_set_needdestroy(p, "received 404 response");
			tris_log(LOG_NOTICE, "Received GATEWAY subscribe for peer without mailbox: %s\n", authpeer->name);
			if (authpeer)
		...
		if (tris_test_flag(&authpeer->flags[1], GATEWAY_PAGE2_SUBSCRIBEMWIONLY)) {
		...
		if (authpeer->mwipvt && authpeer->mwipvt != p) {	/* Destroy old PVT if this is a new one */
			...
			/* gateway_destroy(authpeer->mwipvt); */
		}
		if (authpeer->mwipvt)
		...
		transmit_response(p, "489 Bad Event", req);
		tris_debug(2, "Received GATEWAY subscribe for unknown event package: %s\n", event);
	...
	if (p && !p->needdestroy) {
		...
		/* check if the requested expiry-time is within the approved limits from gateway.conf */
		...
		if (p->autokillid > -1 && gateway_cancel_destroy(p))	/* Remove subscription expiry for renewals */
			tris_log(LOG_WARNING, "Unable to cancel GATEWAY destruction.  Expect bad things.\n");
		if (p->expiry > 0)
			gateway_scheddestroy(p, (p->expiry + 10) * 1000);	/* Set timer for destruction of call at expiration */
		...
		if (p->subscribed == MWI_NOTIFICATION) {
			...
			if (p->relatedpeer) {	/* Send first notification */
				...
				gateway_send_mwi_to_peer(p->relatedpeer, NULL, 0);
				ao2_unlock(p->relatedpeer);
			}
		} else {
			struct gateway_pvt *p_old;
			...
			*/
			...
			while ((p_old = ao2_t_iterator_next(&i, "iterate thru dialogs"))) {
				...
				if (p_old->initreq.method != GATEWAY_SUBSCRIBE) {
					ao2_t_ref(p_old, -1, "toss dialog ptr from iterator_next before continue");
					continue;
				}
...
/*! \brief Handle incoming GATEWAY requests (methods) 
...
static int handle_incoming(struct gateway_pvt *p, struct gateway_request *req, struct sockaddr_in *sin, int *recount, int *nounlock)
{
	...
	if (tris_strlen_zero(cmd) || tris_strlen_zero(cseq)) {
		tris_log(LOG_ERROR, "Missing Cseq. Dropping this GATEWAY message, it's incomplete.\n");
		error = 1;
	}
	...
	/* Find out GATEWAY method for incoming request */
	if (req->method == GATEWAY_RESPONSE) {	/* Response to our request */
		...
		if (respid <= 0) {
			tris_log(LOG_WARNING, "Invalid GATEWAY response code: '%d'\n", respid);
			return 0;
		}
	...
	/* New GATEWAY request coming in 
	   (could be new request in existing GATEWAY dialog as well...) 
	 */			
	
	p->method = req->method;	/* Find out which GATEWAY method they are using */
	tris_debug(4, "**** Received %s (%d) - Command in GATEWAY %s\n", gateway_methods[p->method].text, gateway_methods[p->method].id, cmd); 

	if (p->icseq && (p->icseq > seqno) ) {
		if (p->pendinginvite && seqno == p->pendinginvite && (req->method == GATEWAY_ACK || req->method == GATEWAY_CANCEL)) {
			tris_debug(2, "Got CANCEL or ACK on INVITE with transactions in between.\n");
		}  else {
			tris_debug(1, "Ignoring too old GATEWAY packet packet %d (expecting >= %d)\n", seqno, p->icseq);
			/* We must respond according to RFC 3261 sec 12.2 */
			/*if (req->method != GATEWAY_ACK)
				transmit_response(p, "503 Server error", req);	
			return -1;*/
		}
	} else if (p->icseq &&
		   p->icseq == seqno &&
		   req->method != GATEWAY_ACK && req->method != GATEWAY_INFO && req->method != GATEWAY_REFER &&
		   (p->method != GATEWAY_CANCEL || p->alreadygone)) {
		/* ignore means "don't do anything with it" but still have to 
		   respond appropriately.  We do this if we receive a repeat of
		   the last sequence number  */
		req->ignore = 1;
		tris_debug(3, "Ignoring GATEWAY message because of retransmit (%s Seqno %d, ours %d)\n", gateway_methods[p->method].text, p->icseq, seqno);
	}
		
	if (seqno >= p->icseq)
		/* Next should follow monotonically (but not necessarily 
		   incrementally -- thanks again to the genius authors of GATEWAY --
		   increasing */
		p->icseq = seqno;
	...
	if (gateway_cfg.pedanticgatewaychecking) {
		...
		if (!p->initreq.headers && req->has_to_tag) {
			...
			if (!req->ignore && req->method == GATEWAY_INVITE) {
				transmit_response_reliable(p, "481 Call/Transaction Does Not Exist", req);
				/* Will cease to exist after ACK */
			} else if (req->method != GATEWAY_ACK) {
	...
	if (!e && (p->method == GATEWAY_INVITE || p->method == GATEWAY_SUBSCRIBE || p->method == GATEWAY_REGISTER || p->method == GATEWAY_NOTIFY)) {
		transmit_response(p, "400 Bad request", req);
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
		return -1;
	}

	if (memcmp(&sin->sin_addr, &internip.sin_addr, 4) || sin->sin_port != htons(STANDARD_GATEWAY_PORT)) {
		transmit_response(p, "403 Forbidden", req);
		gateway_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
		return -1;
	}

	/* Handle various incoming GATEWAY methods in requests */
	switch (p->method) {
	case GATEWAY_OPTIONS:
	...
	case GATEWAY_ACK:
		...
		if (seqno == p->pendinginvite) {
			p->invitestate = INV_TERMINATED;
			p->pendinginvite = 0;
			acked = __gateway_ack(p, seqno, 1 /* response */, 0);
			if (find_sdp(req)) {
			...
			p->glareinvite = 0;
			acked = __gateway_ack(p, seqno, 1, 0);
		}
		if (!acked) {
	...
	default:
		...
		tris_log(LOG_NOTICE, "Unknown GATEWAY command '%s' from '%s'\n", 
...
static void process_request_queue(struct gateway_pvt *p, int *recount, int *nounlock)
{
	struct gateway_request *req;
	...
	while ((req = TRIS_LIST_REMOVE_HEAD(&p->request_queue, next))) {
		if (handle_incoming(p, req, &p->recv, recount, nounlock) == -1) {
			/* Request failed */
			if (option_debug) {
				tris_log(LOG_DEBUG, "GATEWAY message could not be handled, bad request: %-70.70s\n", p->callid[0] ? p->callid : "<no callid>");
			}
		}
		tris_free(req);
	}
...
static int queue_request(struct gateway_pvt *p, const struct gateway_request *req)
{
	struct gateway_request *newreq;
...
/*! \brief Read data from GATEWAY UDP socket
\note gatewaysock_read locks the owner channel while we are processing the GATEWAY message
\return 1 on error, 0 on success
\note Successful messages is connected to GATEWAY call and forwarded to handle_incoming() 
*/
static int gatewaysock_read(int *id, int fd, short events, void *ignore)
{
	struct gateway_request req;
	...
	if (res < 0) {
		...
		if (errno == EAGAIN)
			tris_log(LOG_NOTICE, "GATEWAY: Received packet with bad UDP checksum\n");
		else 
#endif
	...
	req.len = res;
	req.socket.fd = gatewaysock;
	set_socket_transport(&req.socket, GATEWAY_TRANSPORT_UDP);
...
/*! \brief Handle incoming GATEWAY message - request or response 
...
static int handle_request_do(struct gateway_request *req, struct sockaddr_in *sin) 
{
	...
	if (gateway_debug_test_addr(sin))	/* Set the debug flag early on packet level */
		req->debug = 1;
	if (gateway_cfg.pedanticgatewaychecking)
		req->len = lws2sws(req->data->str, req->len);	/* Fix multiline headers */
	if (req->debug) {
		tris_verbose("\n<--- GATEWAY read from %s:%s:%d --->\n%s\n<------------->\n", 
	...
	req->method = find_gateway_method(REQ_OFFSET_TO_STR(req, rlPart1));
	...
	for (lockretry = 10; lockretry > 0; lockretry--) {
		...
		/* Find the active GATEWAY dialog or create a new one */
		p = find_call(req, sin, req->method);	/* returns p locked */
		if (p == NULL) {
			tris_debug(1, "Invalid GATEWAY message - rejected , no callid, len %d\n", req->len);
	...
	if (!lockretry) {
		...
		tris_log(LOG_ERROR, "GATEWAY transaction failed: %s \n", p->callid);
		if (req->method != GATEWAY_ACK)
	...
	/* if there are queued requests on this gateway_pvt, process them first, so that everything is
	   handled in order
	*/
	...
	if (handle_incoming(p, req, sin, &recount, &nounlock) == -1) {
		/* Request failed */
		tris_debug(1, "GATEWAY message could not be handled, bad request: %-70.70s\n", p->callid[0] ? p->callid : "<no callid>");
	}
...
static int gateway_standard_port(enum gateway_transport type, int port)
{
	if (type & GATEWAY_TRANSPORT_TLS)
		return port == STANDARD_TLS_PORT;
	else
		return port == STANDARD_GATEWAY_PORT;
}
 ...
 */
static struct tris_tcptls_session_instance *gateway_tcp_locate(struct sockaddr_in *s)
{
	struct gateway_threadinfo *th;
...
static int gateway_prepare_socket(struct gateway_pvt *p) 
{
	struct gateway_socket *s = &p->socket;
	static const char name[] = "GATEWAY socket";
	struct gateway_threadinfo *th = NULL;
	struct tris_tcptls_session_instance *tcptls_session;
	...
	if ((s->fd != -1) && (s->type == GATEWAY_TRANSPORT_UDP)) {
		return s->fd;
	}
	if ((s->type & (GATEWAY_TRANSPORT_TCP | GATEWAY_TRANSPORT_TLS)) &&
			(s->tcptls_session) &&
	...
	if (s->type == GATEWAY_TRANSPORT_UDP) {
		s->fd = gatewaysock;
		return s->fd;
	}
	...
	tmp_ca.remote_address = *(gateway_real_dst(p));
	if ((tcptls_session = gateway_tcp_locate(&tmp_ca.remote_address))) {
	...
	if (!(ca = ao2_alloc(sizeof(*ca), gateway_tcptls_client_args_destructor)) ||
		!(ca->name = tris_strdup(name))) {
		goto create_tcptls_session_fail;
	}
	ca->accept_fd = -1;
	ca->remote_address = *(gateway_real_dst(p));
	/* if type is TLS, we need to create a tls cfg for this session arg */
	if (s->type == GATEWAY_TRANSPORT_TLS) {
	...
	/* client connections need to have the gateway_threadinfo object created before
	 ...
	 * threadt container. */
	if (!(th = gateway_threadinfo_create(s->tcptls_session, s->type))) {
		goto create_tcptls_session_fail;
	...
	if (tris_pthread_create_background(&ca->master, NULL, gateway_tcp_worker_fn, s->tcptls_session)) {
 ...
 */
static int get_cached_mwi(struct gateway_peer *peer, int *new, int *old)
{
	struct gateway_mailbox *mailbox;
...
static int gateway_send_mwi_to_peer(struct gateway_peer *peer, const struct tris_event *event, int cache_only)
{
	...
	if (tris_test_flag((&peer->flags[1]), GATEWAY_PAGE2_SUBSCRIBEMWIONLY) && !peer->mwipvt)
		return 0;
	...
	if (peer->mwipvt) {
		...
		p = dialog_ref(peer->mwipvt, "gateway_send_mwi_to_peer: Setting dialog ptr p from peer->mwipvt-- should this be done?");
	} else {
		 ...
		 * the peer's socket information to the gateway_pvt we just allocated
		 */
		set_socket_transport(&p->socket, 0);
		if (create_addr_from_peer(p, peer)) {
			/* Maybe they're not registered, etc. */
			dialog_unlink_all(p, TRUE, TRUE);
			dialog_unref(p, "unref dialog p just created via gateway_alloc");
			/* gateway_destroy(p); */
			return 0;
		}
		/* Recalculate our side, and recalculate Call ID */
	...
	dialog_unref(p, "unref dialog ptr p just before it goes out of scope at the end of gateway_send_mwi_to_peer.");
	return 0;
}
...
static void check_rtp_timeout(struct gateway_pvt *dialog, time_t t)
{
	...
	*/
	...
	if (dialog->lastrtprx && (tris_rtp_get_rtptimeout(dialog->rtp) || tris_rtp_get_rtpholdtimeout(dialog->rtp)) &&
		...
		if (!tris_test_flag(&dialog->flags[1], GATEWAY_PAGE2_CALL_ONHOLD) || (tris_rtp_get_rtpholdtimeout(dialog->rtp) &&
...
/*! \brief The GATEWAY monitoring thread 
\note	This thread monitors all the GATEWAY sessions and peers that needs notification of mwi
...
static void *do_monitor(void *data)
{
	...
	/* Add an I/O event to our GATEWAY UDP socket */
	if (gatewaysock > -1) 
		gatewaysock_read_id = tris_io_add(io, gatewaysock, gatewaysock_read, TRIS_IO_IN, NULL);

	/* From here on out, we die whenever asked */
	for(;;) {
		/* Check for a reload request */
		tris_mutex_lock(&gateway_reload_lock);
		reloading = gateway_reloading;
		gateway_reloading = FALSE;
		tris_mutex_unlock(&gateway_reload_lock);
		if (reloading) {
			tris_verb(1, "Reloading GATEWAY\n");
			gateway_do_reload(gateway_reloadreason);

			/* Change the I/O fd of our UDP socket */
			if (gatewaysock > -1) {
				if (gatewaysock_read_id)
					gatewaysock_read_id = tris_io_change(io, gatewaysock_read_id, gatewaysock, NULL, 0, NULL);
				else
					gatewaysock_read_id = tris_io_add(io, gatewaysock, gatewaysock_read, TRIS_IO_IN, NULL);
			} else if (gatewaysock_read_id) {
				tris_io_remove(io, gatewaysock_read_id);
				gatewaysock_read_id = NULL;
			}
		}
...
static int proc_session_timer(const void *vp)
{
	struct gateway_pvt *p = (struct gateway_pvt *) vp;
	int sendreinv = FALSE;
	int res = 0;
...
static void proc_422_rsp(struct gateway_pvt *p, struct gateway_request *rsp)
{
	...
	transmit_invite(p, GATEWAY_INVITE, 1, 2); 
}


/*! \brief Get Max or Min SE (session timer expiry)
 * \param p pointer to the GATEWAY dialog 
 * \param max if true, get max se, otherwise min se
*/
int st_get_se(struct gateway_pvt *p, int max)
{
	if (max == TRUE) {
		if (p->stimer->st_cached_max_se) {
			return p->stimer->st_cached_max_se;
		} else if (p->peername) {
			struct gateway_peer *pp = find_peer(p->peername, NULL, TRUE, FINDPEERS, FALSE, 0);
			if (pp) {
 ...
 * \param p pointer to the GATEWAY dialog 
*/
enum st_refresher st_get_refresher(struct gateway_pvt *p)
{
 ...
 * \param p pointer to the GATEWAY dialog 
*/
enum st_mode st_get_mode(struct gateway_pvt *p)
{
	if (!p->stimer) 
		gateway_st_alloc(p);
...
static int gateway_poke_noanswer(const void *data)
{
	...
	if (peer->lastms > -1) {
		...
		if (gateway_cfg.peer_rtupdate) {
			tris_update_realtime(tris_check_realtime("gatewayregs") ? "gatewayregs" : "gatewaypeers", "name", peer->name, "lastms", "-1", SENTINEL);
		}
		manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: GATEWAY\r\nPeer: SIP/%s\r\nPeerStatus: Unreachable\r\nTime: %d\r\n", peer->name, -1);
		if (gateway_cfg.regextenonqualify) {
			register_peer_exten(peer, FALSE);
		}
	}
	...
	if (peer->call) {
		dialog_unlink_all(peer->call, TRUE, TRUE);
		peer->call = dialog_unref(peer->call, "unref dialog peer->call");
		/* peer->call = gateway_destroy(peer->call);*/
	}
	...
	/* Try again quickly */
	TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, 
			DEFAULT_FREQ_NOTOK, gateway_poke_peer_s, peer,
			unref_peer(_data, "removing poke peer ref"),
			unref_peer(peer, "removing poke peer ref"),
			ref_peer(peer, "adding poke peer ref"));
...
static int gateway_poke_peer(struct gateway_peer *peer, int force)
{
	...
	if (!(p = gateway_alloc(NULL, NULL, 0, GATEWAY_OPTIONS, NULL))) {
		return -1;
	}
	peer->call = dialog_ref(p, "copy gateway alloc from p to peer->call");
	...
	tris_copy_string(p->username, "__VOCAL_DATA_SHOULD_READ_THE_GATEWAY_SPEC__", sizeof(p->username));
	xmitres = transmit_invite(p, GATEWAY_INVITE, 0, 2); /* sinks the p refcount */
#else
	xmitres = transmit_invite(p, GATEWAY_OPTIONS, 0, 2); /* sinks the p refcount */
#endif
	peer->ps = tris_tvnow();
	if (xmitres == XMIT_ERROR) {
		gateway_poke_noanswer(peer);	/* Immediately unreachable, network problems */
	} else if (!force) {
		TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, peer->maxms * 2, gateway_poke_noanswer, peer,
				unref_peer(_data, "removing poke peer ref"),
				unref_peer(peer, "removing poke peer ref"),
				ref_peer(peer, "adding poke peer ref"));
	}
	dialog_unref(p, "unref dialog at end of gateway_poke_peer, obtained from gateway_alloc, just before it goes out of scope");
	return 0;
}
...
static int gateway_devicestate(void *data)
{
	...
	if ((p = find_peer(host, NULL, FALSE, FINDALLDEVICES, TRUE, 0))) {
		...
		unref_peer(p, "unref_peer, from gateway_devicestate, release ref from find_peer");
	} else {
...
/*! \brief PBX interface function -build GATEWAY pvt structure 
 *	GATEWAY calls initiated by the PBX arrive here. 
 *
 * \verbatim	
 * 	GATEWAY Dial string syntax
 *		SIP/exten@host!dnid
...
static struct tris_channel *gateway_request_call(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	...
	char *dnid;
 	...
 	char *authname = NULL;
	char *trans = NULL;
	enum gateway_transport transport = 0;
	 ...
	 * configured from gateway.conf, and gateway_tech.capabilities, which is
	...
	tris_debug(1, "Asked to create a GATEWAY channel with formats: %s\n", tris_getformatname_multiple(tmp, sizeof(tmp), oldformat));
	...
	if (!(p = gateway_alloc(NULL, NULL, 0, GATEWAY_INVITE, NULL))) {
		tris_log(LOG_ERROR, "Unable to build gateway pvt data for '%s' (Out of memory or socket error)\n", dest);
	...
	if (!(p->options = tris_calloc(1, sizeof(*p->options)))) {
		...
		/* gateway_destroy(p); */
		tris_log(LOG_ERROR, "Unable to build option GATEWAY data structure - Out of memory\n");
		*cause = TRIS_CAUSE_SWITCH_CONGESTION;
		return NULL;
	}

	/* Save the destination, the GATEWAY dial string */
		...
		*trans++ = '\0';
		if (!strcasecmp(trans, "tcp"))
			...
			if (strcasecmp(trans, "udp"))
				tris_log(LOG_WARNING, "'%s' is not a valid transport option to Dial() for GATEWAY calls, using udp by default.\n", trans);
			transport = GATEWAY_TRANSPORT_UDP;
		}
	} else { /* use default */
		transport = GATEWAY_TRANSPORT_UDP;
	...
	*/
	if (create_addr(p, host, NULL, 1)) {
		*cause = TRIS_CAUSE_UNREGISTERED;
		tris_debug(3, "Cant create GATEWAY call - target device not registered\n");
		dialog_unlink_all(p, TRUE, TRUE);
	...
	gateway_pvt_lock(p);
	tmpc = gateway_new(p, TRIS_STATE_DOWN, host);	/* Place the call */
	if (gateway_cfg.callevents)
		manager_event(EVENT_FLAG_SYSTEM, "ChannelUpdate",
			"Channel: %s\r\nChanneltype: %s\r\nGATEWAYcallid: %s\r\nGATEWAYfullcontact: %s\r\nPeername: %s\r\n",
			p->owner? p->owner->name : "", "GATEWAY", p->callid, p->fullcontact, p->peername);
	gateway_pvt_unlock(p);
	if (!tmpc) {
		dialog_unlink_all(p, TRUE, TRUE);
		/* gateway_destroy(p); */
	}
	dialog_unref(p, "toss pvt ptr at end of gateway_request_call");
	tris_update_use_count();
	restart_monitor();
	return tmpc;
}

/*! \brief Parse insecure= setting in gateway.conf and set flags according to setting */
static void set_insecure_flags (struct tris_flags *flags, const char *value, int lineno)
{
	if (tris_strlen_zero(value))
		return;
	...
	if (!tris_false(value)) {
		...
		while ((word = strsep(&next, ","))) {
			if (!strcasecmp(word, "port"))
				tris_set_flag(&flags[0], GATEWAY_INSECURE_PORT);
			else if (!strcasecmp(word, "invite"))
				tris_set_flag(&flags[0], GATEWAY_INSECURE_INVITE);
			else
			      ...
			      int *maxdatagram)
{
	...
	if (!strcasecmp(v->name, "t38pt_udptl")) {
		...
		tris_set_flag(&mask[1], GATEWAY_PAGE2_T38SUPPORT);

		while ((word = strsep(&next, ","))) {
			if (tris_true(word) || !strcasecmp(word, "fec")) {
				tris_clear_flag(&flags[1], GATEWAY_PAGE2_T38SUPPORT);
				tris_set_flag(&flags[1], GATEWAY_PAGE2_T38SUPPORT_UDPTL_FEC);
			} else if (!strcasecmp(word, "redundancy")) {
				tris_clear_flag(&flags[1], GATEWAY_PAGE2_T38SUPPORT);
				tris_set_flag(&flags[1], GATEWAY_PAGE2_T38SUPPORT_UDPTL_REDUNDANCY);
			} else if (!strcasecmp(word, "none")) {
				tris_clear_flag(&flags[1], GATEWAY_PAGE2_T38SUPPORT);
				tris_set_flag(&flags[1], GATEWAY_PAGE2_T38SUPPORT_UDPTL);
		...
		tris_set_flag(&mask[1], GATEWAY_PAGE2_UDPTL_DESTINATION);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_UDPTL_DESTINATION);
	} else {
		res = 0;
	}
...
static int handle_common_options(struct tris_flags *flags, struct tris_flags *mask, struct tris_variable *v)
{
	...
	if (!strcasecmp(v->name, "trustrpid")) {
		tris_set_flag(&mask[0], GATEWAY_TRUSTRPID);
		tris_set2_flag(&flags[0], tris_true(v->value), GATEWAY_TRUSTRPID);
	} else if (!strcasecmp(v->name, "sendrpid")) {
		tris_set_flag(&mask[0], GATEWAY_SENDRPID);
		tris_set2_flag(&flags[0], tris_true(v->value), GATEWAY_SENDRPID);
	} else if (!strcasecmp(v->name, "g726nonstandard")) {
		tris_set_flag(&mask[0], GATEWAY_G726_NONSTANDARD);
		tris_set2_flag(&flags[0], tris_true(v->value), GATEWAY_G726_NONSTANDARD);
	} else if (!strcasecmp(v->name, "useclientcode")) {
		tris_set_flag(&mask[0], GATEWAY_USECLIENTCODE);
		tris_set2_flag(&flags[0], tris_true(v->value), GATEWAY_USECLIENTCODE);
	} else if (!strcasecmp(v->name, "dtmfmode")) {
		tris_set_flag(&mask[0], GATEWAY_DTMF);
		tris_clear_flag(&flags[0], GATEWAY_DTMF);
		if (!strcasecmp(v->value, "inband"))
			tris_set_flag(&flags[0], GATEWAY_DTMF_INBAND);
		else if (!strcasecmp(v->value, "rfc2833"))
			tris_set_flag(&flags[0], GATEWAY_DTMF_RFC2833);
		else if (!strcasecmp(v->value, "info"))
			tris_set_flag(&flags[0], GATEWAY_DTMF_INFO);
		else if (!strcasecmp(v->value, "shortinfo"))
			tris_set_flag(&flags[0], GATEWAY_DTMF_SHORTINFO);
		else if (!strcasecmp(v->value, "auto"))
			tris_set_flag(&flags[0], GATEWAY_DTMF_AUTO);
		else {
			tris_log(LOG_WARNING, "Unknown dtmf mode '%s' on line %d, using rfc2833\n", v->value, v->lineno);
			tris_set_flag(&flags[0], GATEWAY_DTMF_RFC2833);
		}
	} else if (!strcasecmp(v->name, "nat")) {
		tris_set_flag(&mask[0], GATEWAY_NAT);
		tris_clear_flag(&flags[0], GATEWAY_NAT);
		if (!strcasecmp(v->value, "never"))
			tris_set_flag(&flags[0], GATEWAY_NAT_NEVER);
		else if (!strcasecmp(v->value, "route"))
			tris_set_flag(&flags[0], GATEWAY_NAT_ROUTE);
		else if (tris_true(v->value))
			tris_set_flag(&flags[0], GATEWAY_NAT_ALWAYS);
		else
			tris_set_flag(&flags[0], GATEWAY_NAT_RFC3581);
	} else if (!strcasecmp(v->name, "directmedia") || !strcasecmp(v->name, "canreinvite")) {
		tris_set_flag(&mask[0], GATEWAY_REINVITE);
		tris_clear_flag(&flags[0], GATEWAY_REINVITE);
		if (tris_true(v->value)) {
			tris_set_flag(&flags[0], GATEWAY_DIRECT_MEDIA | GATEWAY_DIRECT_MEDIA_NAT);
			...
			while ((word = strsep(&next, ","))) {
				if (!strcasecmp(word, "update")) {
					tris_set_flag(&flags[0], GATEWAY_REINVITE_UPDATE | GATEWAY_DIRECT_MEDIA);
				} else if (!strcasecmp(word, "nonat")) {
					tris_set_flag(&flags[0], GATEWAY_DIRECT_MEDIA);
					tris_clear_flag(&flags[0], GATEWAY_DIRECT_MEDIA_NAT);
				} else {
		...
		tris_set_flag(&mask[0], GATEWAY_INSECURE);
		tris_clear_flag(&flags[0], GATEWAY_INSECURE);
		set_insecure_flags(&flags[0], v->value, v->lineno);	
	} else if (!strcasecmp(v->name, "progressinband")) {
		tris_set_flag(&mask[0], GATEWAY_PROG_INBAND);
		tris_clear_flag(&flags[0], GATEWAY_PROG_INBAND);
		if (tris_true(v->value))
			tris_set_flag(&flags[0], GATEWAY_PROG_INBAND_YES);
		else if (strcasecmp(v->value, "never"))
			tris_set_flag(&flags[0], GATEWAY_PROG_INBAND_NO);
	} else if (!strcasecmp(v->name, "promiscredir")) {
		tris_set_flag(&mask[0], GATEWAY_PROMISCREDIR);
		tris_set2_flag(&flags[0], tris_true(v->value), GATEWAY_PROMISCREDIR);
	} else if (!strcasecmp(v->name, "videosupport")) {
		if (!strcasecmp(v->value, "always")) {
			tris_set_flag(&mask[1], GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS);
			tris_set_flag(&flags[1], GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS);
		} else {
			tris_set_flag(&mask[1], GATEWAY_PAGE2_VIDEOSUPPORT);
			tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_VIDEOSUPPORT);
		}
	} else if (!strcasecmp(v->name, "textsupport")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_TEXTSUPPORT);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_TEXTSUPPORT);
		res = 1;
	} else if (!strcasecmp(v->name, "allowoverlap")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_ALLOWOVERLAP);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_ALLOWOVERLAP);
	} else if (!strcasecmp(v->name, "allowsubscribe")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_ALLOWSUBSCRIBE);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_ALLOWSUBSCRIBE);
	} else if (!strcasecmp(v->name, "ignoresdpversion")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_IGNORESDPVERSION);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_IGNORESDPVERSION);
	} else if (!strcasecmp(v->name, "faxdetect")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_FAX_DETECT);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_FAX_DETECT);
	} else if (!strcasecmp(v->name, "rfc2833compensate")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_RFC2833_COMPENSATE);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_RFC2833_COMPENSATE);
	} else if (!strcasecmp(v->name, "buggymwi")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_BUGGY_MWI);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_BUGGY_MWI);
	} else if (!strcasecmp(v->name, "constantssrc")) {
		tris_set_flag(&mask[1], GATEWAY_PAGE2_CONSTANT_SSRC);
		tris_set2_flag(&flags[1], tris_true(v->value), GATEWAY_PAGE2_CONSTANT_SSRC);
	} else
		res = 0;
...
/*! \brief Add GATEWAY domain to list of domains we are responsible for */
static int add_gateway_domain(const char *domain, const enum domain_mode mode, const char *context)
{
	struct domain *d;
	...
	TRIS_LIST_UNLOCK(&domain_list);
 	...
 	if (gatewaydebug)	
		tris_debug(1, "Added local GATEWAY domain '%s'\n", domain);

	return 1;
}

/*! \brief  check_gateway_domain: Check if domain part of uri is local to our server */
static int check_gateway_domain(const char *domain, char *context, size_t len)
{
	struct domain *d;
	int result = 0;
...
static int clear_realm_authentication(struct gateway_auth *authlist)
{
	struct gateway_auth *a = authlist;
	struct gateway_auth *b;
...
static void set_peer_defaults(struct gateway_peer *peer)
{
	if (peer->expire == 0) {
		...
		*/
		peer->expire = -1;
		peer->pokeexpire = -1;
		peer->addr.sin_port = htons(STANDARD_GATEWAY_PORT);
		set_socket_transport(&peer->socket, GATEWAY_TRANSPORT_UDP);
	}
	peer->type = GATEWAY_TYPE_PEER;
	tris_copy_flags(&peer->flags[0], &global_flags[0], GATEWAY_FLAGS_TO_COPY);
	tris_copy_flags(&peer->flags[1], &global_flags[1], GATEWAY_PAGE2_FLAGS_TO_COPY);
	tris_string_field_set(peer, context, gateway_cfg.default_context);
	tris_string_field_set(peer, subscribecontext, gateway_cfg.default_subscribecontext);
	...
	peer->allowtransfer = gateway_cfg.allowtransfer;
...
static struct gateway_peer *temp_peer(const char *name)
{
	...
	if (!(peer = ao2_t_alloc(sizeof(*peer), gateway_destroy_peer_fn, "allocate a peer struct")))
		return NULL;
...
static void add_peer_mailboxes(struct gateway_peer *peer, const char *value)
{
...
static struct gateway_peer *build_peer(const char *name, struct tris_variable *v, struct tris_variable *alt, int realtime, int devstate_only)
{
	...
	if (!realtime || tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS)) {
	...
	for (; v || ((v = alt) && !(alt=NULL)); v = v->next) {
		if (!devstate_only) {
			...
			if (!strcasecmp(v->name, "transport") && !tris_strlen_zero(v->value)) {
				...
				while ((trans = strsep(&val, ","))) {
					...
					if (!strncasecmp(trans, "udp", 3)) {
						peer->transports |= GATEWAY_TRANSPORT_UDP;
					} else if (!strncasecmp(trans, "tcp", 3)) {
						peer->transports |= GATEWAY_TRANSPORT_TCP;
					} else if (!strncasecmp(trans, "tls", 3)) {
						peer->transports |= GATEWAY_TRANSPORT_TLS;
					} else {
				...
				if (!strcasecmp(v->value, "peer")) {
					...
					peer->type |= GATEWAY_TYPE_USER;
				} else if (!strcasecmp(v->value, "friend")) {
					peer->type = GATEWAY_TYPE_USER | GATEWAY_TYPE_PEER;
				}
				...
				tris_set2_flag(&peer->flags[0], tris_true(v->value), GATEWAY_USEREQPHONE);
				...
				if (!strcasecmp(v->name, "username")) {
					if (deprecation_warning) {
						tris_log(LOG_NOTICE, "The 'username' field for gateway peers has been deprecated in favor of the term 'defaultuser'\n");
				...
				tris_set2_flag(&peer->flags[1], tris_true(v->value), GATEWAY_PAGE2_SUBSCRIBEMWIONLY);
				...
				tris_set2_flag(&peer->flags[1], tris_true(v->value), GATEWAY_PAGE2_REGISTERTRYING);
		...
		if (realtime && !strcasecmp(v->name, "lastms")) {
			...
			if (!strcasecmp(v->value, "no")) {
				...
				tris_log(LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of gateway.conf\n", peer->name, v->lineno);
				peer->maxms = 0;
			}
			if (realtime && !tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS) && peer->maxms > 0) {
	...
	if (tris_str_strlen(fullcontact)) {
		...
		if (!tris_test_flag(&peer->flags[0], GATEWAY_NAT_ROUTE) || !peer->addr.sin_addr.s_addr) {
	...
	if (srvlookup && peer->dnsmgr == NULL) {
		...
		snprintf(transport, sizeof(transport), "_gateway._%s", get_transport(peer->socket.type));

		if (tris_dnsmgr_lookup(_srvlookup, &peer->addr, &peer->dnsmgr, gateway_cfg.srvlookup && !peer->portinuri ? transport : NULL)) {
	...
	if (!peer->addr.sin_port) {
		peer->addr.sin_port = htons(((peer->socket.type & GATEWAY_TRANSPORT_TLS) ? STANDARD_TLS_PORT : STANDARD_GATEWAY_PORT));
	}
	if (!peer->defaddr.sin_port) {
		peer->defaddr.sin_port = htons(((peer->socket.type & GATEWAY_TRANSPORT_TLS) ? STANDARD_TLS_PORT : STANDARD_GATEWAY_PORT));
	}
	if (!peer->socket.port) {
		peer->socket.port = htons(((peer->socket.type & GATEWAY_TRANSPORT_TLS) ? STANDARD_TLS_PORT : STANDARD_GATEWAY_PORT));
	}

	if (!gateway_cfg.ignore_regexpire && peer->host_dynamic && realtime) {
	...
	if (tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_ALLOWSUBSCRIBE)) {
		gateway_cfg.allowsubscribe = TRUE;	/* No global ban any more */
	}
	 ...
	 * subscribe to it now. */
	if (!devstate_only && !tris_test_flag(&peer->flags[1], GATEWAY_PAGE2_SUBSCRIBEMWIONLY) &&
		!TRIS_LIST_EMPTY(&peer->mailboxes)) {
		add_peer_mwi_subs(peer);
		/* Send MWI from the event cache only.  This is so we can send initial
		 * MWI if app_voicemail got loaded before chan_gateway.  If it is the other
		 * way, then we will get events when app_voicemail gets loaded. */
		gateway_send_mwi_to_peer(peer, NULL, 1);
	}
	...
	if (!tris_strlen_zero(callback)) { /* build string from peer info */
		...
		if (asprintf(&reg_string, "%s?%s:%s@%s/%s", peer->name, peer->username, !tris_strlen_zero(peer->remotesecret) ? peer->remotesecret : peer->secret, peer->tohost, callback) < 0) {
			...
			gateway_register(reg_string, 0); /* XXX TODO: count in registry_count */
...
{
	struct gateway_peer *peer = device;
	peer->the_mark = 1;
	return 0;
}

/*! \brief Re-read GATEWAY.conf config file
\note	This function reloads all config data, except for
	active peers (with registrations). They will only
	change configuration data at restart, not at reload.
	GATEWAY debug and recordhistory state will not change
 */
static int reload_config(enum channelreloadreason reason)
{
	...
	struct tris_flags config_flags = { reason == CHANNEL_MODULE_LOAD ? 0 : tris_test_flag(&global_flags[1], GATEWAY_PAGE2_RTCACHEFRIENDS) ? 0 : CONFIG_FLAG_FILEUNCHANGED };
	int auto_gateway_domains = FALSE;
	struct sockaddr_in old_bindaddr = bindaddr;
	int registry_count = 0, peer_count = 0;
	time_t run_start, run_end;
	
	run_start = time(0);
	tris_unload_realtime("gatewayregs");		
	tris_unload_realtime("gatewaypeers");
	...
	memset(&gateway_tcp_desc.local_address, 0, sizeof(gateway_tcp_desc.local_address));
	memset(&gateway_tls_desc.local_address, 0, sizeof(gateway_tls_desc.local_address));
	...
	gateway_tcp_desc.local_address.sin_port = htons(STANDARD_GATEWAY_PORT);
	gateway_tls_desc.local_address.sin_port = htons(STANDARD_TLS_PORT);

	if (reason != CHANNEL_MODULE_LOAD) {
		tris_debug(4, "--------------- GATEWAY reload started\n");

		clear_realm_authentication(authl);
		clear_gateway_domains();
		authl = NULL;
		...
		ASTOBJ_CONTAINER_DESTROYALL(&regl, gateway_registry_destroy);
	...
	memset(&gateway_cfg.outboundproxy, 0, sizeof(struct gateway_proxy));
	gateway_cfg.outboundproxy.ip.sin_port = htons(STANDARD_GATEWAY_PORT);
	gateway_cfg.outboundproxy.ip.sin_family = AF_INET;	/*!< Type of address: IPv4 */
	gateway_cfg.outboundproxy.force = FALSE;		/*!< Don't force proxy usage, use route: headers */
	...
	ourport_tcp = STANDARD_GATEWAY_PORT;
	ourport_tls = STANDARD_TLS_PORT;
	bindaddr.sin_port = htons(STANDARD_GATEWAY_PORT);
	gateway_cfg.srvlookup = DEFAULT_SRVLOOKUP;
	global_tos_gateway = DEFAULT_TOS_GATEWAY;
	global_tos_audio = DEFAULT_TOS_AUDIO;
	global_tos_video = DEFAULT_TOS_VIDEO;
	global_tos_text = DEFAULT_TOS_TEXT;
	global_cos_gateway = DEFAULT_COS_GATEWAY;
	...
	gateway_cfg.allow_external_domains = DEFAULT_ALLOW_EXT_DOM;				/* Allow external invites */
	global_regcontext[0] = '\0';
	gateway_cfg.regextenonqualify = DEFAULT_REGEXTENONQUALIFY;
	gateway_cfg.notifyringing = DEFAULT_NOTIFYRINGING;
	gateway_cfg.notifycid = DEFAULT_NOTIFYCID;
	gateway_cfg.notifyhold = FALSE;		/*!< Keep track of hold status for a peer */
	gateway_cfg.directrtpsetup = FALSE;		/* Experimental feature, disabled by default */
	gateway_cfg.alwaysauthreject = DEFAULT_ALWAYSAUTHREJECT;
	gateway_cfg.allowsubscribe = FALSE;
	...
	tris_copy_string(gateway_cfg.realm, S_OR(tris_config_TRIS_SYSTEM_NAME, DEFAULT_REALM), sizeof(gateway_cfg.realm));
	tris_copy_string(default_callerid, DEFAULT_CALLERID, sizeof(default_callerid));
	gateway_cfg.compactheaders = DEFAULT_COMPACTHEADERS;
	global_reg_timeout = DEFAULT_REGISTRATION_TIMEOUT;
	global_regattempts_max = 0;
	gateway_cfg.pedanticgatewaychecking = DEFAULT_PEDANTIC;
	gateway_cfg.autocreatepeer = DEFAULT_AUTOCREATEPEER;
	global_autoframing = 0;
	gateway_cfg.allowguest = DEFAULT_ALLOWGUEST;
	...
	gateway_cfg.allowtransfer = TRANSFER_OPENFORALL;	/* Merrily accept all transfers by default */
	gateway_cfg.rtautoclear = 120;
	tris_set_flag(&global_flags[1], GATEWAY_PAGE2_ALLOWSUBSCRIBE);	/* Default for all devices: TRUE */
	tris_set_flag(&global_flags[1], GATEWAY_PAGE2_ALLOWOVERLAP);		/* Default for all devices: TRUE */
	gateway_cfg.peer_rtupdate = TRUE;
	...
	/* Initialize some reasonable defaults at GATEWAY reload (used both for channel and as default for devices */
	tris_copy_string(gateway_cfg.default_context, DEFAULT_CONTEXT, sizeof(gateway_cfg.default_context));
	gateway_cfg.default_subscribecontext[0] = '\0';
	...
	tris_set_flag(&global_flags[0], GATEWAY_DTMF_RFC2833);			/*!< Default DTMF setting: RFC2833 */
	tris_set_flag(&global_flags[0], GATEWAY_NAT_RFC3581);			/*!< NAT support if requested by device with rport */
	tris_set_flag(&global_flags[0], GATEWAY_DIRECT_MEDIA);			/*!< Allow re-invites */

	/* Debugging settings, always default to off */
	dumphistory = FALSE;
	recordhistory = FALSE;
	gatewaydebug &= ~gateway_debug_config;

	/* Misc settings for the channel */
	global_relaxdtmf = FALSE;
	gateway_cfg.callevents = DEFAULT_CALLEVENTS;
	...
	gateway_cfg.matchexterniplocally = DEFAULT_MATCHEXTERNIPLOCALLY;

	/* Copy the default jb config over global_jbconf */
	memcpy(&global_jbconf, &default_jbconf, sizeof(struct tris_jb_conf));

	tris_clear_flag(&global_flags[1], GATEWAY_PAGE2_FAX_DETECT);
	tris_clear_flag(&global_flags[1], GATEWAY_PAGE2_VIDEOSUPPORT | GATEWAY_PAGE2_VIDEOSUPPORT_ALWAYS);
	tris_clear_flag(&global_flags[1], GATEWAY_PAGE2_TEXTSUPPORT);
	tris_clear_flag(&global_flags[1], GATEWAY_PAGE2_IGNORESDPVERSION);


	/* Read the [general] config section of gateway.conf (or from realtime config) */
	for (v = tris_variable_browse(cfg, "general"); v; v = v->next) {
		...
		if (!strcasecmp(v->name, "context")) {
			tris_copy_string(gateway_cfg.default_context, v->value, sizeof(gateway_cfg.default_context));
		} else if (!strcasecmp(v->name, "subscribecontext")) {
			tris_copy_string(gateway_cfg.default_subscribecontext, v->value, sizeof(gateway_cfg.default_subscribecontext));
  		} else if (!strcasecmp(v->name, "callcounter")) {
			global_callcounter = tris_true(v->value) ? 1 : 0;
  		} else if (!strcasecmp(v->name, "allowguest")) {
			gateway_cfg.allowguest = tris_true(v->value) ? 1 : 0;
		} else if (!strcasecmp(v->name, "realm")) {
			tris_copy_string(gateway_cfg.realm, v->value, sizeof(gateway_cfg.realm));
		} else if (!strcasecmp(v->name, "useragent")) {
			tris_copy_string(global_useragent, v->value, sizeof(global_useragent));
			tris_debug(1, "Setting GATEWAY channel User-Agent Name to %s\n", global_useragent);
			...
			gateway_cfg.allowtransfer = tris_true(v->value) ? TRANSFER_OPENFORALL : TRANSFER_CLOSED;
		} else if (!strcasecmp(v->name, "rtcachefriends")) {
			tris_set2_flag(&global_flags[1], tris_true(v->value), GATEWAY_PAGE2_RTCACHEFRIENDS);	
		} else if (!strcasecmp(v->name, "rtsavesysname")) {
			gateway_cfg.rtsave_sysname = tris_true(v->value);
		} else if (!strcasecmp(v->name, "rtupdate")) {
			gateway_cfg.peer_rtupdate = tris_true(v->value);
		} else if (!strcasecmp(v->name, "ignoreregexpire")) {
			gateway_cfg.ignore_regexpire = tris_true(v->value);
		} else if (!strcasecmp(v->name, "timert1")) {
			...
			while ((trans = strsep(&val, ","))) {
				...
				if (!strncasecmp(trans, "udp", 3)) 
					default_transports |= GATEWAY_TRANSPORT_UDP;
				else if (!strncasecmp(trans, "tcp", 3))
					default_transports |= GATEWAY_TRANSPORT_TCP;
				else if (!strncasecmp(trans, "tls", 3))
					default_transports |= GATEWAY_TRANSPORT_TLS;
				else
					tris_log(LOG_NOTICE, "'%s' is not a valid transport type. if no other is specified, udp will be used.\n", trans);
			...
			gateway_tcp_desc.local_address.sin_family = tris_false(v->value) ? 0 : AF_INET;
			tris_debug(2, "Enabling TCP socket for listening\n");
		} else if (!strcasecmp(v->name, "tcpbindaddr")) {
			int family = gateway_tcp_desc.local_address.sin_family;
			if (tris_parse_arg(v->value, PARSE_INADDR, &gateway_tcp_desc.local_address))
				tris_log(LOG_WARNING, "Invalid %s '%s' at line %d of %s\n", v->name, v->value, v->lineno, config);
			gateway_tcp_desc.local_address.sin_family = family;
			...
			gateway_tls_desc.local_address.sin_family = AF_INET;
			...
			if (tris_parse_arg(v->value, PARSE_INADDR, &gateway_tls_desc.local_address))
				tris_log(LOG_WARNING, "Invalid %s '%s' at line %d of %s\n", v->name, v->value, v->lineno, config);
			...
			int i = atoi(v->value);
			if (i > 0)
				gateway_cfg.rtautoclear = i;
			else
				i = 0;
			tris_set2_flag(&global_flags[1], i || tris_true(v->value), GATEWAY_PAGE2_RTAUTOCLEAR);
		} else if (!strcasecmp(v->name, "usereqphone")) {
			tris_set2_flag(&global_flags[0], tris_true(v->value), GATEWAY_USEREQPHONE);	
			...
			gateway_cfg.compactheaders = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifymimetype")) {
			tris_copy_string(default_notifymime, v->value, sizeof(default_notifymime));
		} else if (!strcasecmp(v->name, "directrtpsetup")) {
			gateway_cfg.directrtpsetup = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifyringing")) {
			gateway_cfg.notifyringing = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifyhold")) {
			gateway_cfg.notifyhold = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifycid")) {
			if (!strcasecmp(v->value, "ignore-context")) {
				gateway_cfg.notifycid = IGNORE_CONTEXT;
			} else {
				gateway_cfg.notifycid = tris_true(v->value);
			}
		} else if (!strcasecmp(v->name, "alwaysauthreject")) {
			gateway_cfg.alwaysauthreject = tris_true(v->value);
		} else if (!strcasecmp(v->name, "mohinterpret")) {
			...
			if (tris_strlen_zero(v->value)) {
				tris_log(LOG_WARNING, "no value given for outbound proxy on line %d of gateway.conf.", v->lineno);
				continue;
			}

			tok = tris_skip_blanks(strtok(tris_strdupa(v->value), ","));

			gateway_parse_host(tok, v->lineno, &proxyname, &portnum, &gateway_cfg.outboundproxy.transport);

			gateway_cfg.outboundproxy.ip.sin_port = htons(portnum);
	
			if ((tok = strtok(NULL, ","))) {
				gateway_cfg.outboundproxy.force = !strncasecmp(tris_skip_blanks(tok), "force", 5);
			} else {
				gateway_cfg.outboundproxy.force = FALSE;
			}

			if (tris_strlen_zero(proxyname)) {
				tris_log(LOG_WARNING, "you must specify a name for the outboundproxy on line %d of gateway.conf.", v->lineno);
				gateway_cfg.outboundproxy.name[0] = '\0';
				continue;
			}

			tris_copy_string(gateway_cfg.outboundproxy.name, proxyname, sizeof(gateway_cfg.outboundproxy.name));

			proxy_update(&gateway_cfg.outboundproxy);
		} else if (!strcasecmp(v->name, "autocreatepeer")) {
			gateway_cfg.autocreatepeer = tris_true(v->value);
		} else if (!strcasecmp(v->name, "match_auth_username")) {
			global_match_auth_username = tris_true(v->value);
		} else if (!strcasecmp(v->name, "srvlookup")) {
			gateway_cfg.srvlookup = tris_true(v->value);
		} else if (!strcasecmp(v->name, "pedantic")) {
			gateway_cfg.pedanticgatewaychecking = tris_true(v->value);
		...
		} else if (!strcasecmp(v->name, "gatewaydebug")) {
			if (tris_true(v->value))
				gatewaydebug |= gateway_debug_config;
			...
			gateway_cfg.allow_external_domains = tris_true(v->value);
		} else if (!strcasecmp(v->name, "autodomain")) {
			auto_gateway_domains = tris_true(v->value);
			...
			if (tris_strlen_zero(domain))
				...
				add_gateway_domain(tris_strip(domain), GATEWAY_DOMAIN_CONFIG, cntx ? tris_strip(cntx) : "");
		} else if (!strcasecmp(v->name, "register")) {
			if (gateway_register(v->value, v->lineno) == 0)
				registry_count++;
		} else if (!strcasecmp(v->name, "mwi")) {
			gateway_subscribe_mwi(v->value, v->lineno);
		} else if (!strcasecmp(v->name, "tos_gateway")) {
			if (tris_str2tos(v->value, &global_tos_gateway))
				tris_log(LOG_WARNING, "Invalid tos_gateway value at line %d, refer to QoS documentation\n", v->lineno);
		...
		} else if (!strcasecmp(v->name, "cos_gateway")) {
			if (tris_str2cos(v->value, &global_cos_gateway))
				tris_log(LOG_WARNING, "Invalid cos_gateway value at line %d, refer to QoS documentation\n", v->lineno);
			...
			if (!strcasecmp(v->value, "no")) {
				...
				tris_log(LOG_WARNING, "Qualification default should be 'yes', 'no', or a number of milliseconds at line %d of gateway.conf\n", v->lineno);
				default_qualify = 0;
			}
		} else if (!strcasecmp(v->name, "qualifyfreq")) {
			int i;
			...
			gateway_cfg.callevents = tris_true(v->value);
			...
			gateway_cfg.matchexterniplocally = tris_true(v->value);
		} else if (!strcasecmp(v->name, "constantssrc")) {
			tris_set2_flag(&global_flags[1], tris_true(v->value), GATEWAY_PAGE2_CONSTANT_SSRC);
		} else if (!strcasecmp(v->name, "session-timers")) {
			int i = (int) str2stmode(v->value); 
			if (i < 0) {
				tris_log(LOG_WARNING, "Invalid session-timers '%s' at line %d of %s\n", v->value, v->lineno, config);
	...
	if (!gateway_cfg.allow_external_domains && TRIS_LIST_EMPTY(&domain_list)) {
		tris_log(LOG_WARNING, "To disallow external domains, you need to configure local GATEWAY domains.\n");
		gateway_cfg.allow_external_domains = 1;
	}
	/* If not configured, set default transports */
	if (default_transports == 0) {
		default_transports = default_primary_transport = GATEWAY_TRANSPORT_UDP;
	}
	
	/* Build list of authentication to various GATEWAY realms, i.e. service providers */
	...
	if (ucfg) {
		...
		int genhasgateway, genregistergateway;
		const char *hasgateway, *registergateway;
		
		genhasgateway = tris_true(tris_variable_retrieve(ucfg, "general", "hasgateway"));
		genregistergateway = tris_true(tris_variable_retrieve(ucfg, "general", "registergateway"));
		gen = tris_variable_browse(ucfg, "general");
		cat = tris_category_browse(ucfg, NULL);
		while (cat) {
			if (strcasecmp(cat, "general")) {
				hasgateway = tris_variable_retrieve(ucfg, cat, "hasgateway");
				registergateway = tris_variable_retrieve(ucfg, cat, "registergateway");
				if (tris_true(hasgateway) || (!hasgateway && genhasgateway)) {
					peer = build_peer(cat, gen, tris_variable_browse(ucfg, cat), 0, 0);
					if (peer) {
						/* user.conf entries are always of type friend */
						peer->type = GATEWAY_TYPE_USER | GATEWAY_TYPE_PEER;
						ao2_t_link(peers, peer, "link peer into peer table");
						if ((peer->type & GATEWAY_TYPE_PEER) && peer->addr.sin_addr.s_addr) {
							ao2_t_link(peers_by_ip, peer, "link peer into peers_by_ip table");
						}
				...
				if (tris_true(registergateway) || (!registergateway && genregistergateway)) {
					char tmp[256];
	...
	if (tris_find_ourip(&internip.sin_addr, bindaddr)) {
		tris_log(LOG_WARNING, "Unable to get own IP address, GATEWAY disabled\n");
		tris_config_destroy(cfg);
		return 0;
	}
	tris_mutex_lock(&netlock);
	if ((gatewaysock > -1) && (memcmp(&old_bindaddr, &bindaddr, sizeof(struct sockaddr_in)))) {
		close(gatewaysock);
		gatewaysock = -1;
	}
	if (gatewaysock < 0) {
		gatewaysock = socket(AF_INET, SOCK_DGRAM, 0);
		if (gatewaysock < 0) {
			tris_log(LOG_WARNING, "Unable to create GATEWAY socket: %s\n", strerror(errno));
			tris_config_destroy(cfg);
			return -1;
		} else {
			/* Allow GATEWAY clients on the same host to access us: */
			const int reuseFlag = 1;

			setsockopt(gatewaysock, SOL_SOCKET, SO_REUSEADDR,
				   (const char*)&reuseFlag,
				   sizeof reuseFlag);

			tris_enable_packet_fragmentation(gatewaysock);

			if (bind(gatewaysock, (struct sockaddr *)&bindaddr, sizeof(bindaddr)) < 0) {
				...
				tris_verb(2, "GATEWAY Listening on %s:%d\n",
						tris_inet_ntoa(bindaddr.sin_addr), ntohs(bindaddr.sin_port));
				tris_netsock_set_qos(gatewaysock, global_tos_gateway, global_cos_gateway, "GATEWAY");
			}
		}
	}
	if (stunaddr.sin_addr.s_addr != 0) {
		tris_debug(1, "stun to %s:%d\n",
			tris_inet_ntoa(stunaddr.sin_addr) , ntohs(stunaddr.sin_port));
		tris_stun_request(gatewaysock, &stunaddr,
			NULL, &externip);
	...
	/* Start TCP server */
	tris_tcptls_server_start(&gateway_tcp_desc);
 	if (gateway_tcp_desc.accept_fd == -1 &&  gateway_tcp_desc.local_address.sin_family == AF_INET) {
		/* TCP server start failed. Tell the admin */
		tris_log(LOG_ERROR, "GATEWAY TCP Server start failed. Not listening on TCP socket.\n");
		gateway_tcp_desc.local_address.sin_family = 0;
	} else {
		tris_debug(2, "GATEWAY TCP server started\n");
	}

	/* Start TLS server if needed */
	memcpy(gateway_tls_desc.tls_cfg, &default_tls_cfg, sizeof(default_tls_cfg));

	if (tris_ssl_setup(gateway_tls_desc.tls_cfg)) {
		tris_tcptls_server_start(&gateway_tls_desc);
 		if (default_tls_cfg.enabled && gateway_tls_desc.accept_fd == -1) {
			tris_log(LOG_ERROR, "TLS Server start failed. Not listening on TLS socket.\n");
			gateway_tls_desc.tls_cfg = NULL;
		}
	} else if (gateway_tls_desc.tls_cfg->enabled) {
		gateway_tls_desc.tls_cfg = NULL;
		tris_log(LOG_WARNING, "GATEWAY TLS server did not load because of errors.\n");
	}


	/* Add default domains - host name, IP address and IP:port
	 * Only do this if user added any gateway domain with "localdomains" 
	 * In order to *not* break backwards compatibility 
	 * 	Some phones address us at IP only, some with additional port number 
	 */
	if (auto_gateway_domains) {
		char temp[MAXHOSTNAMELEN];

		/* First our default IP address */
		if (bindaddr.sin_addr.s_addr) {
			add_gateway_domain(tris_inet_ntoa(bindaddr.sin_addr), GATEWAY_DOMAIN_AUTO, NULL);
		} else if (internip.sin_addr.s_addr) {
		/* Our internal IP address, if configured */
			add_gateway_domain(tris_inet_ntoa(internip.sin_addr), GATEWAY_DOMAIN_AUTO, NULL);
		} else {
			tris_log(LOG_NOTICE, "Can't add wildcard IP address to domain list, please add IP address to domain manually.\n");
		}

		/* If TCP is running on a different IP than UDP, then add it too */
		if (gateway_tcp_desc.local_address.sin_addr.s_addr && !inaddrcmp(&bindaddr, &gateway_tcp_desc.local_address))
			add_gateway_domain(tris_inet_ntoa(gateway_tcp_desc.local_address.sin_addr), GATEWAY_DOMAIN_AUTO, NULL);

		/* If TLS is running on a differen IP than UDP and TCP, then add that too */
		if (gateway_tls_desc.local_address.sin_addr.s_addr && !inaddrcmp(&bindaddr, &gateway_tls_desc.local_address) && inaddrcmp(&gateway_tcp_desc.local_address, &gateway_tls_desc.local_address))
			add_gateway_domain(tris_inet_ntoa(gateway_tls_desc.local_address.sin_addr), GATEWAY_DOMAIN_AUTO, NULL);

		/* Our extern IP address, if configured */
		if (externip.sin_addr.s_addr)
			add_gateway_domain(tris_inet_ntoa(externip.sin_addr), GATEWAY_DOMAIN_AUTO, NULL);

		/* Extern host name (NAT traversal support) */
		if (!tris_strlen_zero(externhost))
			add_gateway_domain(externhost, GATEWAY_DOMAIN_AUTO, NULL);
		
		/* Our host name */
		if (!gethostname(temp, sizeof(temp)))
			add_gateway_domain(temp, GATEWAY_DOMAIN_AUTO, NULL);
	}
	...
	manager_event(EVENT_FLAG_SYSTEM, "ChannelReload", "ChannelType: GATEWAY\r\nReloadReason: %s\r\nRegistry_Count: %d\r\nPeer_Count: %d\r\n", channelreloadreason2txt(reason), registry_count, peer_count);
	run_end = time(0);
	tris_debug(4, "GATEWAY reload_config done...Runtime= %d sec\n", (int)(run_end-run_start));
...
static struct tris_udptl *gateway_get_udptl_peer(struct tris_channel *chan)
{
	...
	gateway_pvt_lock(p);
	if (p->udptl && tris_test_flag(&p->flags[0], GATEWAY_DIRECT_MEDIA))
		udptl = p->udptl;
	gateway_pvt_unlock(p);
	return udptl;
}
...
static int gateway_set_udptl_peer(struct tris_channel *chan, struct tris_udptl *udptl)
{
	...
	if (!tris_test_flag(&p->flags[0], GATEWAY_GOTREFER)) {
		if (!p->pendinginvite) {
			tris_debug(3, "Sending reinvite on GATEWAY '%s' - It's UDPTL soon redirected to IP %s:%d\n", p->callid, tris_inet_ntoa(udptl ? p->udptlredirip.sin_addr : p->ourip.sin_addr), udptl ? ntohs(p->udptlredirip.sin_port) : 0);
			transmit_reinvite_with_sdp(p, TRUE, FALSE);
		} else if (!tris_test_flag(&p->flags[0], GATEWAY_PENDINGBYE)) {
			tris_debug(3, "Deferring reinvite on GATEWAY '%s' - It's UDPTL will be redirected to IP %s:%d\n", p->callid, tris_inet_ntoa(udptl ? p->udptlredirip.sin_addr : p->ourip.sin_addr), udptl ? ntohs(p->udptlredirip.sin_port) : 0);
			tris_set_flag(&p->flags[0], GATEWAY_NEEDREINVITE);
		}
	}
...
static enum tris_rtp_get_result gateway_get_rtp_peer(struct tris_channel *chan, struct tris_rtp **rtp)
{
	...
	*rtp = p->rtp;
	...
	if (tris_rtp_getnat(*rtp) && !tris_test_flag(&p->flags[0], GATEWAY_DIRECT_MEDIA_NAT))
		res = TRIS_RTP_TRY_PARTIAL;
	else if (tris_test_flag(&p->flags[0], GATEWAY_DIRECT_MEDIA))
...
static enum tris_rtp_get_result gateway_get_vrtp_peer(struct tris_channel *chan, struct tris_rtp **rtp)
{
	...
	*rtp = p->vrtp;
	...
	if (tris_test_flag(&p->flags[0], GATEWAY_DIRECT_MEDIA))
		res = TRIS_RTP_TRY_NATIVE;
...
static int gateway_set_rtp_peer(struct tris_channel *chan, struct tris_rtp *rtp, struct tris_rtp *vrtp, struct tris_rtp *trtp, int codecs, int nat_active)
{
	...
	if (!tris_bridged_channel(chan) && !gateway_cfg.directrtpsetup) 	/* We are in early state */
		return 0;
	...
	*/
        if (nat_active && !tris_test_flag(&p->flags[0], GATEWAY_DIRECT_MEDIA_NAT)) {
                gateway_pvt_unlock(p);
                return 0;
        }
	...
	if (changed && !tris_test_flag(&p->flags[0], GATEWAY_GOTREFER) && !tris_test_flag(&p->flags[0], GATEWAY_DEFER_BYE_ON_TRANSFER)) {
		if (chan->_state != TRIS_STATE_UP) {	/* We are in early state */
			if (p->do_history)
				append_history(p, "ExtInv", "Initial invite sent with remote bridge proposal.");
			tris_debug(1, "Early remote bridge setting GATEWAY '%s' - Sending media to %s\n", p->callid, tris_inet_ntoa(rtp ? p->redirip.sin_addr : p->ourip.sin_addr));
		} else if (!p->pendinginvite) {		/* We are up, and have no outstanding invite */
			tris_debug(3, "Sending reinvite on GATEWAY '%s' - It's audio soon redirected to IP %s\n", p->callid, tris_inet_ntoa(rtp ? p->redirip.sin_addr : p->ourip.sin_addr));
			transmit_reinvite_with_sdp(p, FALSE, FALSE);
		} else if (!tris_test_flag(&p->flags[0], GATEWAY_PENDINGBYE)) {
			tris_debug(3, "Deferring reinvite on GATEWAY '%s' - It's audio will be redirected to IP %s\n", p->callid, tris_inet_ntoa(rtp ? p->redirip.sin_addr : p->ourip.sin_addr));
...
static char *app_dtmfmode = "GATEWAYDtmfMode";
static char *app_gatewayaddheader = "GATEWAYAddHeader";
static char *app_gatewayremoveheader = "GATEWAYRemoveHeader";

/*! \brief Set the DTMFmode for an outbound GATEWAY call (application) */
static int gateway_dtmfmode(struct tris_channel *chan, void *data)
{
	struct gateway_pvt *p;
	char *mode = data;
	...
	tris_channel_lock(chan);
	if (!IS_GATEWAY_TECH(chan->tech)) {
		tris_log(LOG_WARNING, "Call this application only on GATEWAY incoming calls\n");
		tris_channel_unlock(chan);
		return 0;
	}
	p = chan->tech_pvt;
	...
	if (!strcasecmp(mode, "info")) {
		tris_clear_flag(&p->flags[0], GATEWAY_DTMF);
		tris_set_flag(&p->flags[0], GATEWAY_DTMF_INFO);
		p->jointnoncodeccapability &= ~TRIS_RTP_DTMF;
	} else if (!strcasecmp(mode, "shortinfo")) {
		tris_clear_flag(&p->flags[0], GATEWAY_DTMF);
		tris_set_flag(&p->flags[0], GATEWAY_DTMF_SHORTINFO);
		p->jointnoncodeccapability &= ~TRIS_RTP_DTMF;
...
/*! \brief Add a GATEWAY header to an outbound INVITE */
static int gateway_addheader(struct tris_channel *chan, void *data)
{
	int no = 0;
	int ok = FALSE;
	...
	while (!ok && no <= 50) {
		no++;
		snprintf(varbuf, sizeof(varbuf), "__GATEWAYADDHEADER%.2d", no);
	...
	if (ok) {
		...
		if (gatewaydebug) {
			tris_debug(1, "GATEWAY Header added \"%s\" as %s\n", inbuf, varbuf);
		}
	} else {
		tris_log(LOG_WARNING, "Too many GATEWAY headers added, max 50\n");
	}
	tris_channel_unlock(chan);
	return 0;
}

/*! \brief Remove GATEWAY headers added previously with GatewayAddHeader application */
static int gateway_removeheader(struct tris_channel *chan, void *data)
{
	...
	TRIS_LIST_TRAVERSE_SAFE_BEGIN (headp, newvariable, entries) {
		if (strncasecmp(tris_var_name(newvariable), "GATEWAYADDHEADER", strlen("GATEWAYADDHEADER")) == 0) {
			if (removeall || (!strncasecmp(tris_var_value(newvariable),inbuf,strlen(inbuf)))) {
				if (gatewaydebug)
					tris_log(LOG_DEBUG,"removing GATEWAY Header \"%s\" as %s\n",
...
\note	Called by the transfer() dialplan application through the gateway_transfer()
...
static int gateway_gatewayredirect(struct gateway_pvt *p, const char *dest)
{
	...
	if (!host) {
		...
		if (tris_strlen_zero(tmp)) {
			tris_log(LOG_ERROR, "Cannot retrieve the 'To' header from the original GATEWAY request!\n");
			return 0;
		}
	...
	gateway_scheddestroy(p, GATEWAY_TRANS_TIMEOUT);	/* Make sure we stop send this reply. */
	gateway_alreadygone(p);
	/* hangup here */
	return 0;
}

/*! \brief Return GATEWAY UA's codec (part of the RTP interface) */
static int gateway_get_codec(struct tris_channel *chan)
{
	struct gateway_pvt *p = chan->tech_pvt;
...
static void gateway_poke_all_peers(void)
{
	...
	while ((peer = ao2_t_iterator_next(&i, "iterate thru peers table"))) {
		...
		TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, ms, gateway_poke_peer_s, peer,
				unref_peer(_data, "removing poke peer ref"),
				unref_peer(peer, "removing poke peer ref"),
				ref_peer(peer, "adding poke peer ref"));
		ao2_unlock(peer);
...
static void gateway_send_all_registers(void)
{
	...
	ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
		...
		ms += regspacing;
		TRIS_SCHED_REPLACE_UNREF(iterator->expire, sched, ms, gateway_reregister, iterator, 
...
static void gateway_send_all_mwi_subscriptions(void)
{
	ASTOBJ_CONTAINER_TRAVERSE(&submwil, 1, do {
		...
		if ((iterator->resub = tris_sched_add(sched, 1, gateway_subscribe_mwi_do, ASTOBJ_REF(iterator))) < 0) {
			ASTOBJ_UNREF(iterator, gateway_subscribe_mwi_destroy);
		}
		ASTOBJ_UNLOCK(iterator);
	} while (0));
}
...
static int gateway_do_reload(enum channelreloadreason reason)
{
	...
	gateway_poke_all_peers();

	/* Register with all services */
	gateway_send_all_registers();

	gateway_send_all_mwi_subscriptions();

	end_poke = time(0);
	
	tris_debug(4, "do_reload finished. peer poke/prune reg contact time = %d sec.\n", (int)(end_poke-start_poke));

	tris_debug(4, "--------------- GATEWAY reload done\n");
...
static char *gateway_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gateway reload";
		e->usage =
			"Usage: gateway reload\n"
			"       Reloads GATEWAY configuration from gateway.conf\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	tris_mutex_lock(&gateway_reload_lock);
	if (gateway_reloading) 
		tris_verbose("Previous GATEWAY reload not yet done\n");
	else {
		gateway_reloading = TRUE;
		gateway_reloadreason = (a && a->fd) ? CHANNEL_CLI_RELOAD : CHANNEL_MODULE_RELOAD;
	}
	tris_mutex_unlock(&gateway_reload_lock);
	restart_monitor();
...
{
	if (gateway_reload(0, 0, NULL))
		return 0;
	return 1;
}

/*! \brief GATEWAY Cli commands definition */
static struct tris_cli_entry cli_gateway[] = {
	TRIS_CLI_DEFINE(gateway_show_channels, "List active GATEWAY channels or subscriptions"),
	TRIS_CLI_DEFINE(gateway_show_channelstats, "List statistics for active GATEWAY channels"),
	TRIS_CLI_DEFINE(gateway_show_domains, "List our local GATEWAY domains"),
	TRIS_CLI_DEFINE(gateway_show_inuse, "List all inuse/limits"),
	TRIS_CLI_DEFINE(gateway_show_objects, "List all GATEWAY object allocations"),
	TRIS_CLI_DEFINE(gateway_show_peers, "List defined GATEWAY peers"),
	TRIS_CLI_DEFINE(gateway_show_registry, "List GATEWAY registration status"),
	TRIS_CLI_DEFINE(gateway_unregister, "Unregister (force expiration) a GATEWAY peer from the registry"),
	TRIS_CLI_DEFINE(gateway_show_settings, "Show GATEWAY global settings"),
	TRIS_CLI_DEFINE(gateway_show_mwi, "Show MWI subscriptions"),
	TRIS_CLI_DEFINE(gateway_cli_notify, "Send a notify packet to a GATEWAY peer"),
	TRIS_CLI_DEFINE(gateway_show_channel, "Show detailed GATEWAY channel info"),
	TRIS_CLI_DEFINE(gateway_show_history, "Show GATEWAY dialog history"),
	TRIS_CLI_DEFINE(gateway_show_peer, "Show details on specific GATEWAY peer"),
	TRIS_CLI_DEFINE(gateway_show_users, "List defined GATEWAY users"),
	TRIS_CLI_DEFINE(gateway_show_user, "Show details on specific GATEWAY user"),
	TRIS_CLI_DEFINE(gateway_qualify_peer, "Send an OPTIONS packet to a peer"),
	TRIS_CLI_DEFINE(gateway_show_sched, "Present a report on the status of the sched queue"),
	TRIS_CLI_DEFINE(gateway_prune_realtime, "Prune cached Realtime users/peers"),
	TRIS_CLI_DEFINE(gateway_do_debug, "Enable/Disable GATEWAY debugging"),
	TRIS_CLI_DEFINE(gateway_set_history, "Enable/Disable GATEWAY history"),
	TRIS_CLI_DEFINE(gateway_reload, "Reload GATEWAY configuration"),
	TRIS_CLI_DEFINE(gateway_show_tcp, "List TCP Connections")
};

/*! \brief PBX load module - initialization */
static int load_module(void)
{
	tris_verbose("GATEWAY channel loading...\n");
	...
	gateway_reloadreason = CHANNEL_MODULE_LOAD;

	if(reload_config(gateway_reloadreason))	/* Load the configuration from gateway.conf */
	 ...
	 * has const fields.
	 */
	memcpy(&gateway_tech_info, &gateway_tech, sizeof(gateway_tech));
	memset((void *) &gateway_tech_info.send_digit_begin, 0, sizeof(gateway_tech_info.send_digit_begin));

	/* Make sure we can register our gateway channel type */
	if (tris_channel_register(&gateway_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel type 'GATEWAY'\n");
		io_context_destroy(io);
		sched_context_destroy(sched);
		return TRIS_MODULE_LOAD_FAILURE;
	}

	/* Register all CLI functions for GATEWAY */
	tris_cli_register_multiple(cli_gateway, ARRAY_LEN(cli_gateway));

	/* Tell the RTP subdriver that we're here */
	tris_rtp_proto_register(&gateway_rtp);

	/* Tell the UDPTL subdriver that we're here */
	tris_udptl_proto_register(&gateway_udptl);

	/* Register dialplan applications */
	tris_register_application_xml(app_dtmfmode, gateway_dtmfmode);
	tris_register_application_xml(app_gatewayaddheader, gateway_addheader);
	tris_register_application_xml(app_gatewayremoveheader, gateway_removeheader);

	/* Register dialplan functions */
	tris_custom_function_register(&gateway_header_function);
	tris_custom_function_register(&gatewaypeer_function);
	tris_custom_function_register(&gatewaychaninfo_function);
	tris_custom_function_register(&checkgatewaydomain_function);

	/* Register manager commands */
	tris_manager_register2("GATEWAYpeers", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_gateway_show_peers,
			"List GATEWAY peers (text format)", mandescr_show_peers);
	tris_manager_register2("GATEWAYshowpeer", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_gateway_show_peer,
			"Show GATEWAY peer (text format)", mandescr_show_peer);
	tris_manager_register2("GATEWAYqualifypeer", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_gateway_qualify_peer,
			"Show GATEWAY peer (text format)", mandescr_show_peer);	/*! \todo Fix this XXX This must be all wrong XXXX */
	tris_manager_register2("GATEWAYshowregistry", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_show_registry,
			"Show GATEWAY registrations (text format)", mandescr_show_registry);
	tris_manager_register2("GATEWAYnotify", EVENT_FLAG_SYSTEM, manager_gatewaynotify,
			"Send a GATEWAY notify", mandescr_gatewaynotify);
	gateway_poke_all_peers();	
	gateway_send_all_registers();
	gateway_send_all_mwi_subscriptions();
	...
	tris_realtime_require_field(tris_check_realtime("gatewayregs") ? "gatewayregs" : "gatewaypeers",
		"name", RQ_CHAR, 10,
		"ipaddr", RQ_CHAR, 15,
		"port", RQ_UINTEGER2, 5,
...
static int unload_module(void)
{
	...
	tris_channel_unregister(&gateway_tech);

	/* Unregister dial plan functions */
	tris_custom_function_unregister(&gatewaychaninfo_function);
	tris_custom_function_unregister(&gatewaypeer_function);
	tris_custom_function_unregister(&gateway_header_function);
	tris_custom_function_unregister(&checkgatewaydomain_function);

	/* Unregister dial plan applications */
	tris_unregister_application(app_dtmfmode);
	tris_unregister_application(app_gatewayaddheader);
	tris_unregister_application(app_gatewayremoveheader);

	/* Unregister CLI commands */
	tris_cli_unregister_multiple(cli_gateway, ARRAY_LEN(cli_gateway));

	/* Disconnect from the RTP subsystem */
	tris_rtp_proto_unregister(&gateway_rtp);

	/* Disconnect from UDPTL */
	tris_udptl_proto_unregister(&gateway_udptl);

	/* Unregister AMI actions */
	tris_manager_unregister("GATEWAYpeers");
	tris_manager_unregister("GATEWAYshowpeer");
	tris_manager_unregister("GATEWAYqualifypeer");
	tris_manager_unregister("GATEWAYshowregistry");
	tris_manager_unregister("GATEWAYnotify");
	
	/* Kill TCP/TLS server threads */
	if (gateway_tcp_desc.master)
		tris_tcptls_server_stop(&gateway_tcp_desc);
	if (gateway_tls_desc.master)
		tris_tcptls_server_stop(&gateway_tls_desc);
	...
	ASTOBJ_CONTAINER_DESTROYALL(&submwil, gateway_subscribe_mwi_destroy);
	...
	if (con)
		tris_context_destroy(con, "GATEWAY");
	tris_unload_realtime("gatewayregs");
	tris_unload_realtime("gatewaypeers");
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Session Initiation Protocol (GATEWAY)",
		.load = load_module,
