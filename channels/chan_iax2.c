...
/*! \file
 *
 * \brief Implementation of Inter-Trismedia eXchange Version 2
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 250396 $")
...
#include "trismedia/paths.h"	/* need tris_config_TRIS_DATA_DIR for firmware */
...
static struct tris_codec_pref prefs;

static const char tdesc[] = "Inter Trismedia eXchange Driver (Ver 2)";
...
static char default_parkinglot[TRIS_MAX_CONTEXT];

static char language[MAX_LANGUAGE] = "";
static char regcontext[TRIS_MAX_CONTEXT] = "";
...
static struct tris_timer *timer;				/* Timer for trunking */

static struct tris_netsock_list *netsock;
static struct tris_netsock_list *outsock;		/*!< used if sourceaddress specified and bindaddr == INADDR_ANY */
...
#define IAX_CALLENCRYPTED(pvt) \
	(tris_test_flag(pvt, IAX_ENCRYPTED) && tris_test_flag(pvt, IAX_KEYPOPULATED))
...
#define IAX_DEBUGDIGEST(msg, key) do { \
		...
		\
		tris_log(LOG_NOTICE, msg " IAX_COMMAND_RTKEY to rotate key to '%s'\n", digest); \
	} while(0)

static	struct io_context *io;
static	struct tris_sched_thread *sched;
...
static pthread_t netthreadid = TRIS_PTHREADT_NULL;
...
struct iax2_user {
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(secret);
		TRIS_STRING_FIELD(dbsecret);
		TRIS_STRING_FIELD(accountcode);
		TRIS_STRING_FIELD(mohinterpret);
		TRIS_STRING_FIELD(mohsuggest);
		TRIS_STRING_FIELD(inkeys);               /*!< Key(s) this user can use to authenticate to us */
		TRIS_STRING_FIELD(language);
		TRIS_STRING_FIELD(cid_num);
		TRIS_STRING_FIELD(cid_name);
		TRIS_STRING_FIELD(parkinglot);           /*!< Default parkinglot for device */
	);
...
struct iax2_peer {
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(secret);
		TRIS_STRING_FIELD(dbsecret);
		TRIS_STRING_FIELD(outkey);	    /*!< What key we use to talk to this peer */

		TRIS_STRING_FIELD(regexten);     /*!< Extension to register (if regcontext is used) */
		TRIS_STRING_FIELD(context);      /*!< For transfers only */
		TRIS_STRING_FIELD(peercontext);  /*!< Context to pass to peer */
		TRIS_STRING_FIELD(mailbox);	    /*!< Mailbox */
		TRIS_STRING_FIELD(mohinterpret);
		TRIS_STRING_FIELD(mohsuggest);
		TRIS_STRING_FIELD(inkeys);		/*!< Key(s) this peer can use to authenticate to us */
		/* Suggested caller id if registering */
		TRIS_STRING_FIELD(cid_num);		/*!< Default context (for transfer really) */
		TRIS_STRING_FIELD(cid_name);		/*!< Default context (for transfer really) */
		TRIS_STRING_FIELD(zonetag);		/*!< Time Zone */
		TRIS_STRING_FIELD(parkinglot);   /*!< Default parkinglot for device */
	);
	struct tris_codec_pref prefs;
	struct tris_dnsmgr_entry *dnsmgr;		/*!< DNS refresh manager */
...
#define IAX2_TRUNK_PREFACE (sizeof(struct iax_frame) + sizeof(struct tris_iax2_meta_hdr) + sizeof(struct tris_iax2_meta_trunk_hdr))
...
struct iax2_trunk_peer {
	...
	int trunkerror;
	int calls;
	TRIS_LIST_ENTRY(iax2_trunk_peer) list;
};

static TRIS_LIST_HEAD_STATIC(tpeers, iax2_trunk_peer);

struct iax_firmware {
	TRIS_LIST_ENTRY(iax_firmware) list;
	int fd;
	int mmaplen;
	int dead;
	struct tris_iax2_firmware_header *fwh;
	unsigned char *buf;
};
...
struct iax2_registry {
	...
	struct tris_dnsmgr_entry *dnsmgr;	/*!< DNS refresh manager */
	TRIS_LIST_ENTRY(iax2_registry) entry;
};

static TRIS_LIST_HEAD_STATIC(registrations, iax2_registry);
...
struct chan_iax2_pvt {
	...
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(ani);
		/*! DNID */
		TRIS_STRING_FIELD(dnid);
		/*! RDNIS */
		TRIS_STRING_FIELD(rdnis);
		/*! Requested Extension */
		...
		TRIS_STRING_FIELD(secret);
		/*! MD5 challenge */
		TRIS_STRING_FIELD(challenge);
		/*! Public keys permitted keys for incoming authentication */
		TRIS_STRING_FIELD(inkeys);
		/*! Private key for outgoing authentication */
		TRIS_STRING_FIELD(outkey);
		/*! Preferred language */
		TRIS_STRING_FIELD(language);
		/*! Hostname/peername for naming purposes */
		TRIS_STRING_FIELD(host);

		TRIS_STRING_FIELD(dproot);
		TRIS_STRING_FIELD(accountcode);
		TRIS_STRING_FIELD(mohinterpret);
		TRIS_STRING_FIELD(mohsuggest);
		/*! received OSP token */
		TRIS_STRING_FIELD(osptoken);
		/*! Default parkinglot */
		TRIS_STRING_FIELD(parkinglot);
	);
	...
	tris_aes_encrypt_key ecx;
	/*! Decryption AES-128 Key corresponding to ecx */
	tris_aes_decrypt_key mydcx;
	...
	TRIS_LIST_HEAD_NOLOCK(signaling_queue, signaling_queue_entry) signaling_queue;
};

struct signaling_queue_entry {
	struct tris_frame f;
	TRIS_LIST_ENTRY(signaling_queue_entry) next;
};
 ...
 */
static TRIS_LIST_HEAD_STATIC(frame_queue, iax_frame);
 ...
 * However, to maintain old behavior for Trismedia 1.4, these are set to
...
static TRIS_LIST_HEAD_STATIC(firmwares, iax_firmware);
...
struct iax2_dpcache {
	char peercontext[TRIS_MAX_CONTEXT];
	...
	unsigned short callno;
	int waiters[256];
	TRIS_LIST_ENTRY(iax2_dpcache) cache_list;
	TRIS_LIST_ENTRY(iax2_dpcache) peer_list;
};

static TRIS_LIST_HEAD_STATIC(dpcache, iax2_dpcache);

static void reg_source_db(struct iax2_peer *p);
static struct iax2_peer *realtime_peer(const char *peername, struct sockaddr_in *sin);
static struct iax2_user *realtime_user(const char *username, struct sockaddr_in *sin);

static int tris_cli_netstats(struct mansession *s, int fd, int limit_fmt);
...
struct iax2_pkt_buf {
	TRIS_LIST_ENTRY(iax2_pkt_buf) entry;
	size_t len;
	unsigned char buf[1];
};

struct iax2_thread {
	TRIS_LIST_ENTRY(iax2_thread) list;
	...
	tris_mutex_t lock;
	tris_cond_t cond;
	tris_mutex_t init_lock;
	tris_cond_t init_cond;
	 ...
	 *  are queued up here. */
	TRIS_LIST_HEAD_NOLOCK(, iax2_pkt_buf) full_frames;
};

/* Thread lists */
static TRIS_LIST_HEAD_STATIC(idle_list, iax2_thread);
static TRIS_LIST_HEAD_STATIC(active_list, iax2_thread);
static TRIS_LIST_HEAD_STATIC(dynamic_list, iax2_thread);

static void *iax2_process_thread(void *data);
static void iax2_destroy(int callno);

static void signal_condition(tris_mutex_t *lock, tris_cond_t *cond)
{
	tris_mutex_lock(lock);
	tris_cond_signal(cond);
 ...
 */
static tris_mutex_t iaxsl[ARRAY_LEN(iaxs)];
...
static void iax_outputframe(struct iax_frame *f, struct tris_iax2_full_hdr *fhi, int rx, struct sockaddr_in *sin, int datalen)
{
	if (iaxdebug ||
...
static enum tris_bridge_result iax2_bridge(struct tris_channel *c0, struct tris_channel *c1, int flags, struct tris_frame **fo, struct tris_channel **rc, int timeoutms);
static int expire_registry(const void *data);
static int iax2_answer(struct tris_channel *c);
static int iax2_call(struct tris_channel *c, char *dest, int timeout);
static int iax2_devicestate(void *data);
static int iax2_digit_begin(struct tris_channel *c, char digit);
static int iax2_digit_end(struct tris_channel *c, char digit, unsigned int duration);
static int iax2_do_register(struct iax2_registry *reg);
static int iax2_fixup(struct tris_channel *oldchannel, struct tris_channel *newchan);
static int iax2_hangup(struct tris_channel *c);
static int iax2_indicate(struct tris_channel *c, int condition, const void *data, size_t datalen);
static int iax2_poke_peer(struct iax2_peer *peer, int heldcall);
static int iax2_provision(struct sockaddr_in *end, int sockfd, char *dest, const char *template, int force);
static int iax2_send(struct chan_iax2_pvt *pvt, struct tris_frame *f, unsigned int ts, int seqno, int now, int transfer, int final);
static int iax2_sendhtml(struct tris_channel *c, int subclass, const char *data, int datalen);
static int iax2_sendimage(struct tris_channel *c, struct tris_frame *img);
static int iax2_sendtext(struct tris_channel *c, const char *text);
static int iax2_setoption(struct tris_channel *c, int option, void *data, int datalen);
static int iax2_transfer(struct tris_channel *c, const char *dest);
static int iax2_write(struct tris_channel *c, struct tris_frame *f);
...
static struct tris_channel *iax2_request(const char *type, int format, void *data, int *cause, struct tris_channel* src);
static struct tris_frame *iax2_read(struct tris_channel *c);
static struct iax2_peer *build_peer(const char *name, struct tris_variable *v, struct tris_variable *alt, int temponly);
static struct iax2_user *build_user(const char *name, struct tris_variable *v, struct tris_variable *alt, int temponly);
...
static int decode_frame(tris_aes_decrypt_key *dcx, struct tris_iax2_full_hdr *fh, struct tris_frame *f, int *datalen);
static int encrypt_frame(tris_aes_encrypt_key *ecx, struct tris_iax2_full_hdr *fh, unsigned char *poo, int *datalen);
...
static const struct tris_channel_tech iax2_tech = {
	.type = "IAX2",
	.description = tdesc,
	.capabilities = IAX_CAPABILITY_FULLBANDWIDTH,
	.properties = TRIS_CHAN_TP_WANTSJITTER,
...
/*! \brief Send manager event at call setup to link between Trismedia channel name
...
static struct tris_datastore_info iax2_variable_datastore_info = {
	.type = "IAX2_VARIABLE",
...
{
	TRIS_LIST_HEAD(, tris_var_t) *oldlist = old, *newlist;
	struct tris_var_t *oldvar, *newvar;

	newlist = tris_calloc(sizeof(*newlist), 1);
	if (!newlist) {
		tris_log(LOG_ERROR, "Unable to duplicate iax2 variables\n");
		return NULL;
	}

	TRIS_LIST_HEAD_INIT(newlist);
	TRIS_LIST_LOCK(oldlist);
	TRIS_LIST_TRAVERSE(oldlist, oldvar, entries) {
		newvar = tris_var_assign(tris_var_name(oldvar), tris_var_value(oldvar));
		if (newvar)
			TRIS_LIST_INSERT_TAIL(newlist, newvar, entries);
		else
			tris_log(LOG_ERROR, "Unable to duplicate iax2 variable '%s'\n", tris_var_name(oldvar));
	}
...
{
	TRIS_LIST_HEAD(, tris_var_t) *oldlist = old;
	struct tris_var_t *oldvar;

	TRIS_LIST_LOCK(oldlist);
	while ((oldvar = TRIS_LIST_REMOVE_HEAD(oldlist, entries))) {
		tris_free(oldvar);
	}
	TRIS_LIST_UNLOCK(oldlist);
...
static void insert_idle_thread(struct iax2_thread *thread)
{
	if (thread->type == IAX_THREAD_TYPE_DYNAMIC) {
		TRIS_LIST_LOCK(&dynamic_list);
		TRIS_LIST_INSERT_TAIL(&dynamic_list, thread, list);
		TRIS_LIST_UNLOCK(&dynamic_list);
	} else {
		TRIS_LIST_LOCK(&idle_list);
		TRIS_LIST_INSERT_TAIL(&idle_list, thread, list);
		TRIS_LIST_UNLOCK(&idle_list);
	}
...
static struct iax2_thread *find_idle_thread(void)
{
	...
	TRIS_LIST_LOCK(&idle_list);
	thread = TRIS_LIST_REMOVE_HEAD(&idle_list, list);
	TRIS_LIST_UNLOCK(&idle_list);
	...
	TRIS_LIST_LOCK(&dynamic_list);
	thread = TRIS_LIST_REMOVE_HEAD(&dynamic_list, list);
	TRIS_LIST_UNLOCK(&dynamic_list);
	...
	if (iaxdynamicthreadcount >= iaxmaxthreadcount || !(thread = tris_calloc(1, sizeof(*thread))))
		return NULL;

	/* Set default values */
	tris_atomic_fetchadd_int(&iaxdynamicthreadcount, 1);
	thread->threadnum = tris_atomic_fetchadd_int(&iaxdynamicthreadnum, 1);
	thread->type = IAX_THREAD_TYPE_DYNAMIC;

	/* Initialize lock and condition */
	tris_mutex_init(&thread->lock);
	tris_cond_init(&thread->cond, NULL);
	tris_mutex_init(&thread->init_lock);
	tris_cond_init(&thread->init_cond, NULL);
	tris_mutex_lock(&thread->init_lock);

	/* Create thread and send it on it's way */
	if (tris_pthread_create_detached_background(&thread->threadid, NULL, iax2_process_thread, thread)) {
		tris_cond_destroy(&thread->cond);
		tris_mutex_destroy(&thread->lock);
		tris_free(thread);
		return NULL;
	}
	...
	tris_cond_wait(&thread->init_cond, &thread->init_lock);

	/* Done with init_lock */
	tris_mutex_unlock(&thread->init_lock);
...
static int __schedule_action(void (*func)(const void *data), const void *data, const char *funcname)
{
...
#ifdef DEBUG_SCHED_MULTITHREAD
		tris_copy_string(thread->curfunc, funcname, sizeof(thread->curfunc));
#endif
	...
	time(&t);
	if (t != lasterror) 
		tris_debug(1, "Out of idle IAX2 threads for scheduling!\n");
	lasterror = t;
...
static int iax2_sched_replace(int id, struct tris_sched_thread *st, int when, 
		tris_sched_cb callback, const void *data)
{
	tris_sched_thread_del(st, id);

	return tris_sched_thread_add(st, when, callback, data);
}

static int iax2_sched_add(struct tris_sched_thread *st, int when, 
		tris_sched_cb callback, const void *data)
{
	return tris_sched_thread_add(st, when, callback, data);
}
...
static void __send_ping(const void *data)
{
	...
	tris_mutex_lock(&iaxsl[callno]);

	if (iaxs[callno]) {
		if (iaxs[callno]->peercallno) {
			send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_PING, 0, NULL, 0, -1);
		...
		tris_debug(1, "I was supposed to send a PING with callno %d, but no such call exists.\n", callno);
	}

	tris_mutex_unlock(&iaxsl[callno]);
}
...
static void encmethods_to_str(int e, struct tris_str *buf)
{
	tris_str_set(&buf, 0, "(");
	if (e & IAX_ENCRYPT_AES128) {
		tris_str_append(&buf, 0, "aes128");
	}
	if (e & IAX_ENCRYPT_KEYROTATE) {
		tris_str_append(&buf, 0, ",keyrotate");
	}
	if (tris_str_strlen(buf) > 1) {
		tris_str_append(&buf, 0, ")");
	} else {
		tris_str_set(&buf, 0, "No");
	}
}
...
static void __send_lagrq(const void *data)
{
	...
	if (iaxs[callno]) {
		if (iaxs[callno]->peercallno) {
			send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_LAGRQ, 0, NULL, 0, -1);
		...
		tris_debug(1, "I was supposed to send a LAGRQ with callno %d, but no such call exists.\n", callno);
	}
...
static unsigned char compress_subclass(int subclass)
{
	...
	for (x = 0; x < IAX_MAX_SHIFT; x++) {
		if (subclass & (1 << x)) {
			if (power > -1) {
				tris_log(LOG_WARNING, "Can't compress subclass %d\n", subclass);
				return 0;
			} else
				power = x;
		}
...
static int peer_hash_cb(const void *obj, const int flags)
{
	...
	return tris_str_hash(peer->name);
}
...
static int user_hash_cb(const void *obj, const int flags)
{
	...
	return tris_str_hash(user->name);
}
...
static int iax2_getpeername(struct sockaddr_in sin, char *host, int len)
{
	...
	while ((peer = ao2_iterator_next(&i))) {
		if ((peer->addr.sin_addr.s_addr == sin.sin_addr.s_addr) &&
			...
			tris_copy_string(host, peer->name, len);
			peer_unref(peer);
			res = 1;
			break;
		}
...
static void iax2_destroy_helper(struct chan_iax2_pvt *pvt)
{
	...
	if (tris_test_flag(pvt, IAX_MAXAUTHREQ)) {
		...
		if (user) {
			tris_atomic_fetchadd_int(&user->curauthreq, -1);
			user_unref(user);	
		}

		tris_clear_flag(pvt, IAX_MAXAUTHREQ);
	}
	/* No more pings or lagrq's */
	TRIS_SCHED_DEL_SPINLOCK(tris_sched_thread_get_context(sched), pvt->pingid, &iaxsl[pvt->callno]);
	TRIS_SCHED_DEL_SPINLOCK(tris_sched_thread_get_context(sched), pvt->lagid, &iaxsl[pvt->callno]);
	tris_sched_thread_del(sched, pvt->autoid);
	tris_sched_thread_del(sched, pvt->authid);
	tris_sched_thread_del(sched, pvt->initid);
	tris_sched_thread_del(sched, pvt->jbid);
	tris_sched_thread_del(sched, pvt->keyrotateid);
}

static void iax2_frame_free(struct iax_frame *fr)
{
	tris_sched_thread_del(sched, fr->retrans);
	iax_frame_free(fr);
}
...
static int scheduled_destroy(const void *vid)
{
	...
	if (iaxs[callno]) {
		if (option_debug) {
			tris_log(LOG_DEBUG, "Really destroying %d now...\n", callno);
		}
		iax2_destroy(callno);
	}
	tris_mutex_unlock(&iaxsl[callno]);
...
static void send_signaling(struct chan_iax2_pvt *pvt)
{
	...
	while ((s = TRIS_LIST_REMOVE_HEAD(&pvt->signaling_queue, next))) {
...
/*! \brief All frames other than that of type TRIS_FRAME_IAX must be held until
 *  we have received a destination call number. */
static int queue_signalling(struct chan_iax2_pvt *pvt, struct tris_frame *f)
{
	struct signaling_queue_entry *new;

	if (f->frametype == TRIS_FRAME_IAX || !pvt->hold_signaling) {
		return 1; /* do not queue this frame */
	} else if (!(new = tris_calloc(1, sizeof(struct signaling_queue_entry)))) {
		return -1;  /* out of memory */
	}

	memcpy(&new->f, f, sizeof(new->f)); /* copy tris_frame into our queue entry */

	if (new->f.datalen) { /* if there is data in this frame copy it over as well */
		if (!(new->f.data.ptr = tris_calloc(1, new->f.datalen))) {
			free_signaling_queue_entry(new);
			return -1;
		}
		memcpy(new->f.data.ptr, f->data.ptr, sizeof(*new->f.data.ptr));
	}
	TRIS_LIST_INSERT_TAIL(&pvt->signaling_queue, new, next);
...
static void pvt_destructor(void *obj)
{
	...
	tris_mutex_lock(&iaxsl[pvt->callno]);
	iax2_destroy_helper(pvt);
	sched_delay_remove(&pvt->addr, pvt->callno_entry);
	pvt->callno_entry = NULL;
	tris_mutex_unlock(&iaxsl[pvt->callno]);

	/* Already gone */
	tris_set_flag(pvt, IAX_ALREADYGONE);	

	TRIS_LIST_LOCK(&frame_queue);
	TRIS_LIST_TRAVERSE(&frame_queue, cur, list) {
		/* Cancel any pending transmissions */
		if (cur->callno == pvt->callno) { 
			cur->retries = -1;
		}
	}
	TRIS_LIST_UNLOCK(&frame_queue);
	...
	if (!pvt->owner) {
		jb_frame frame;
		if (pvt->vars) {
		    tris_variables_destroy(pvt->vars);
		    pvt->vars = NULL;
		}
		...
		jb_destroy(pvt->jb);
		tris_string_field_free_memory(pvt);
	}
}
...
static struct chan_iax2_pvt *new_iax(struct sockaddr_in *sin, const char *host)
{
	...
	if (tris_string_field_init(tmp, 32)) {
		ao2_ref(tmp, -1);
		tmp = NULL;
		return NULL;
	}
	...
	tris_string_field_set(tmp,exten, "s");
	tris_string_field_set(tmp,host, host);
	...
	TRIS_LIST_HEAD_INIT_NOLOCK(&tmp->dpentries);

	tmp->hold_signaling = 1;
	TRIS_LIST_HEAD_INIT_NOLOCK(&tmp->signaling_queue);
...
static void update_max_trunk(void)
{
	...
	maxtrunkcall = max;
	if (iaxdebug)
		tris_debug(1, "New max trunk callno is %d\n", max);
}
...
static void update_max_nontrunk(void)
{
	...
	maxnontrunkcall = max;
	if (iaxdebug)
		tris_debug(1, "New max nontrunk callno is %d\n", max);
}
...
static int make_trunk(unsigned short callno, int locked)
{
	...
	if (iaxs[callno]->oseqno) {
		tris_log(LOG_WARNING, "Can't make trunk once a call has started!\n");
		return -1;
	}
	if (callno & TRUNK_CALL_START) {
		tris_log(LOG_WARNING, "Call %d is already a trunk\n", callno);
		return -1;
	}

	if (!(callno_entry = get_unused_callno(1, iaxs[callno]->callno_entry->validated))) {
		tris_log(LOG_WARNING, "Unable to trunk call: Insufficient space\n");
		return -1;
	}

	x = callno_entry->callno;
	tris_mutex_lock(&iaxsl[x]);

	/*!
	 * \note We delete these before switching the slot, because if
	 * they fire in the meantime, they will generate a warning.
	 */
	tris_sched_thread_del(sched, iaxs[callno]->pingid);
	tris_sched_thread_del(sched, iaxs[callno]->lagid);
	iaxs[x] = iaxs[callno];
	iaxs[x]->callno = x;
	...
	res = x;
	if (!locked)
		tris_mutex_unlock(&iaxsl[x]);

	tris_debug(1, "Made call %d into trunk call %d\n", callno, x);
...
static void store_by_transfercallno(struct chan_iax2_pvt *pvt)
{
	if (!pvt->transfercallno) {
		tris_log(LOG_ERROR, "This should not be called without a transfer call number.\n");
		return;
	}
...
{
	if (!pvt->peercallno) {
		tris_log(LOG_ERROR, "This should not be called without a peer call number.\n");
		return;
	}
...
static int calltoken_required(struct sockaddr_in *sin, const char *name, int subclass)
{
	...
	tris_debug(1, "Determining if address %s with username %s requires calltoken validation.  Optional = %d  calltoken_required = %d \n", tris_inet_ntoa(sin->sin_addr), name, optional, calltoken_required);
...
static void set_peercnt_limit(struct peercnt *peercnt)
{
	...
	if ((addr_range = ao2_callback(callno_limits, 0, addr_range_match_address_cb, &sin))) {
		...
		tris_debug(1, "custom addr_range %d found for %s\n", limit, tris_inet_ntoa(sin.sin_addr));
		ao2_ref(addr_range, -1);
	}
...
static int set_peercnt_limit_all_cb(void *obj, void *arg, int flags)
{
	...
	tris_debug(1, "Reset limits for peercnts table\n");
...
static void peercnt_modify(unsigned char reg, uint16_t limit, struct sockaddr_in *sin)
{
	...
	if ((peercnt = ao2_find(peercnts, &tmp, OBJ_POINTER))) {
		...
		tris_debug(1, "peercnt entry %s modified limit:%d registered:%d", tris_inet_ntoa(sin->sin_addr), peercnt->limit, peercnt->reg);
...
static int peercnt_add(struct sockaddr_in *sin)
{
	...
	if (peercnt->limit > peercnt->cur) {
		peercnt->cur++;
		tris_debug(1, "ip callno count incremented to %d for %s\n", peercnt->cur, tris_inet_ntoa(sin->sin_addr));
	} else { /* max num call numbers for this peer has been reached! */
		tris_log(LOG_ERROR, "maxcallnumber limit of %d for %s has been reached!\n", peercnt->limit, tris_inet_ntoa(sin->sin_addr));
		res = -1;
	}
...
static void peercnt_remove(struct peercnt *peercnt)
{
	...
	if (peercnt) {
		...
		ao2_lock(peercnts);
		peercnt->cur--;
		tris_debug(1, "ip callno count decremented to %d for %s\n", peercnt->cur, tris_inet_ntoa(sin.sin_addr));
 ...
 */
static void build_callno_limits(struct tris_variable *v)
{
	...
	for (; v; v = v->next) {
		...
		error = 0;
		found = 0;
		ha = tris_append_ha("permit", v->name, NULL, &error);

		/* check for valid config information */
		if (error) {
			tris_log(LOG_ERROR, "Call number limit for %s could not be added, Invalid address range\n.", v->name);
			continue;
		} else if ((sscanf(v->value, "%d", &limit) != 1) || (limit < 0)) {
			tris_log(LOG_ERROR, "Call number limit for %s could not be added. Invalid limit %s\n.", v->name, v->value);
			tris_free_ha(ha);
			continue;
		}

		tris_copy_ha(ha, &tmp.ha);
		...
		tris_copy_ha(ha, &addr_range->ha); /* this is safe because only one ha is possible for each limit */
		tris_free_ha(ha); /* cleanup the tmp ha */
...
static int add_calltoken_ignore(const char *addr)
{
	struct addr_range tmp;
	struct addr_range *addr_range = NULL;
	struct tris_ha *ha = NULL;
	int error = 0;

	if (tris_strlen_zero(addr)) {
		tris_log(LOG_WARNING, "invalid calltokenoptional %s\n", addr);
		return -1;
	}

	ha = tris_append_ha("permit", addr, NULL, &error);

	/* check for valid config information */
	if (error) {
		tris_log(LOG_WARNING, "Error %d creating calltokenoptional entry %s\n", error, addr);
		return -1;
	}
	...
	if ((addr_range = ao2_find(calltoken_ignores, &tmp, OBJ_POINTER))) {
		...
		/* copy over config data into addr_range object */
		tris_copy_ha(ha, &addr_range->ha); /* this is safe because only one ha is possible */
...
static char *handle_cli_iax2_show_callno_limits(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct ao2_iterator i;
	struct peercnt *peercnt;
	struct sockaddr_in sin;
	...
	case CLI_HANDLER:
		...
		tris_cli(a->fd, "%-15s %-12s %-12s\n", "Address", "Callno Usage", "Callno Limit");
		i = ao2_iterator_init(peercnts, 0);
		while ((peercnt = ao2_iterator_next(&i))) {
			sin.sin_addr.s_addr = peercnt->addr;
			if (a->argc == 5 && (!strcasecmp(a->argv[4], tris_inet_ntoa(sin.sin_addr)))) {
					tris_cli(a->fd, "%-15s %-12d %-12d\n", tris_inet_ntoa(sin.sin_addr), peercnt->cur, peercnt->limit);
					found = 1;
					break;
			} else {
				tris_cli(a->fd, "%-15s %-12d %-12d\n", tris_inet_ntoa(sin.sin_addr), peercnt->cur, peercnt->limit);
		...
		if (a->argc == 4) {
			...
			tris_cli(a->fd, "No callnumber table entries for %s found\n", a->argv[4] );
		}
...
static struct callno_entry *get_unused_callno(int trunk, int validated)
{
	...
	if ((!ao2_container_count(callno_pool) && !trunk) || (!ao2_container_count(callno_pool_trunk) && trunk)) {
		tris_log(LOG_WARNING, "Out of CallNumbers\n");
	...
	if (!validated && (total_nonval_callno_used >= global_maxcallno_nonval)) {
		tris_log(LOG_WARNING, "NON-CallToken callnumber limit is reached. Current:%d Max:%d\n", total_nonval_callno_used, global_maxcallno_nonval);
...
static int replace_callno(const void *obj)
{
	...
	if (!callno_entry->validated && (total_nonval_callno_used != 0)) {
		...
		tris_log(LOG_ERROR, "Attempted to decrement total non calltoken validated callnumbers below zero... Callno is:%d \n", callno_entry->callno);
	}
...
static void sched_delay_remove(struct sockaddr_in *sin, struct callno_entry *callno_entry)
{
	...
	if ((peercnt = ao2_find(peercnts, &tmp, OBJ_POINTER))) {
		...
		tris_debug(1, "schedule decrement of callno used for %s in %d seconds\n", tris_inet_ntoa(sin->sin_addr), MIN_REUSE_TIME);
...
static int __find_callno(unsigned short callno, unsigned short dcallno, struct sockaddr_in *sin, int new, int sockfd, int return_locked, int check_dcallno)
{
	...
	if (new <= NEW_ALLOW) {
		...
		if (callno && dcallno && iaxs[dcallno] && !iaxs[dcallno]->peercallno && match(sin, callno, dcallno, iaxs[dcallno], check_dcallno)) {
			...
			if (!res || !return_locked) {
				tris_mutex_unlock(&iaxsl[dcallno]);
			}
			return res;
		}
		if (dcallno) {
	...
	if (!res && (new >= NEW_ALLOW)) {
		...
		if (!iax2_getpeername(*sin, host, sizeof(host)))
			snprintf(host, sizeof(host), "%s:%d", tris_inet_ntoa(sin->sin_addr), ntohs(sin->sin_port));
		...
		if (!(callno_entry = get_unused_callno(0, validated))) {
			...
			tris_log(LOG_WARNING, "No more space\n");
			return 0;
		}
		x = callno_entry->callno;
		tris_mutex_lock(&iaxsl[x]);
		...
		if (iaxs[x]) {
			if (iaxdebug)
				tris_debug(1, "Creating new call structure %d\n", x);
			iaxs[x]->callno_entry = callno_entry;
			...
			tris_copy_flags(iaxs[x], &globalflags, IAX_NOTRANSFER | IAX_TRANSFERMEDIA | IAX_USEJITTERBUF | IAX_FORCEJITTERBUF | IAX_FORCE_ENCRYPT);	
			tris_string_field_set(iaxs[x], accountcode, accountcode);
			tris_string_field_set(iaxs[x], mohinterpret, mohinterpret);
			tris_string_field_set(iaxs[x], mohsuggest, mohsuggest);
			tris_string_field_set(iaxs[x], parkinglot, default_parkinglot);

			if (iaxs[x]->peercallno) {
				store_by_peercallno(iaxs[x]);
			}
		} else {
			tris_log(LOG_WARNING, "Out of resources\n");
			tris_mutex_unlock(&iaxsl[x]);
...
/*!
 * \brief Queue a frame to a call's owning trismedia channel
 *
 ...
 */
static int iax2_queue_frame(int callno, struct tris_frame *f)
{
	for (;;) {
		if (iaxs[callno] && iaxs[callno]->owner) {
			if (tris_channel_trylock(iaxs[callno]->owner)) {
				/* Avoid deadlock by pausing and trying again */
				DEADLOCK_AVOIDANCE(&iaxsl[callno]);
			} else {
				tris_queue_frame(iaxs[callno]->owner, f);
				tris_channel_unlock(iaxs[callno]->owner);
				break;
			}
		} else
			break;
...
/*!
 * \brief Queue a hangup frame on the tris_channel owner
 *
	...
	for (;;) {
		if (iaxs[callno] && iaxs[callno]->owner) {
			if (tris_channel_trylock(iaxs[callno]->owner)) {
				...
				tris_queue_hangup(iaxs[callno]->owner);
				tris_channel_unlock(iaxs[callno]->owner);
				break;
			}
		} else
...
/*!
 * \brief Queue a control frame on the tris_channel owner
 *
...
static int iax2_queue_control_data(int callno, 
	enum tris_control_frame_type control, const void *data, size_t datalen)
{
	for (;;) {
		if (iaxs[callno] && iaxs[callno]->owner) {
			if (tris_channel_trylock(iaxs[callno]->owner)) {
				...
				tris_queue_control_data(iaxs[callno]->owner, control, data, datalen);
				tris_channel_unlock(iaxs[callno]->owner);
				break;
			}
		} else
...
static int try_firmware(char *s)
{
	...
	struct tris_iax2_firmware_header *fwh, fwh2;
	struct MD5Context md5;
	unsigned char sum[16], buf[1024];
	char *s2, *last;

	if (!(s2 = alloca(strlen(s) + 100))) {
		tris_log(LOG_WARNING, "Alloca failed!\n");
		return -1;
	}
	...
	snprintf(s2, strlen(s) + 100, "/var/tmp/%s-%ld", last, (unsigned long)tris_random());

	if ((res = stat(s, &stbuf) < 0)) {
		tris_log(LOG_WARNING, "Failed to stat '%s': %s\n", s, strerror(errno));
		return -1;
	}
	...
	ifd = open(s, O_RDONLY);
	if (ifd < 0) {
		tris_log(LOG_WARNING, "Cannot open '%s': %s\n", s, strerror(errno));
		return -1;
	}
	fd = open(s2, O_RDWR | O_CREAT | O_EXCL, TRIS_FILE_MODE);
	if (fd < 0) {
		tris_log(LOG_WARNING, "Cannot open '%s' for writing: %s\n", s2, strerror(errno));
		close(ifd);
		return -1;
	}
	...
	while(len) {
		...
		if (res != chunk) {
			tris_log(LOG_WARNING, "Only read %d of %d bytes of data :(: %s\n", res, chunk, strerror(errno));
			close(ifd);
			close(fd);
			return -1;
		}
		...
		if (res != chunk) {
			tris_log(LOG_WARNING, "Only write %d of %d bytes of data :(: %s\n", res, chunk, strerror(errno));
			close(ifd);
			close(fd);
			return -1;
		}
	...
	if ((res = read(fd, &fwh2, sizeof(fwh2))) != sizeof(fwh2)) {
		tris_log(LOG_WARNING, "Unable to read firmware header in '%s'\n", s);
		close(fd);
		return -1;
	}
	if (ntohl(fwh2.magic) != IAX_FIRMWARE_MAGIC) {
		tris_log(LOG_WARNING, "'%s' is not a valid firmware file\n", s);
		close(fd);
		return -1;
	}
	if (ntohl(fwh2.datalen) != (stbuf.st_size - sizeof(fwh2))) {
		tris_log(LOG_WARNING, "Invalid data length in firmware '%s'\n", s);
		close(fd);
		return -1;
	}
	if (fwh2.devname[sizeof(fwh2.devname) - 1] || tris_strlen_zero((char *)fwh2.devname)) {
		tris_log(LOG_WARNING, "No or invalid device type specified for '%s'\n", s);
		close(fd);
		return -1;
	}
	fwh = (struct tris_iax2_firmware_header*)mmap(NULL, stbuf.st_size, PROT_READ, MAP_PRIVATE, fd, 0); 
	if (fwh == (void *) -1) {
		tris_log(LOG_WARNING, "mmap failed: %s\n", strerror(errno));
		close(fd);
		return -1;
	}
	MD5Init(&md5);
	...
	if (memcmp(sum, fwh->chksum, sizeof(sum))) {
		tris_log(LOG_WARNING, "Firmware file '%s' fails checksum\n", s);
		munmap((void*)fwh, stbuf.st_size);
		close(fd);
		return -1;
	}

	TRIS_LIST_TRAVERSE(&firmwares, cur, list) {
	...
	if (!cur && ((cur = tris_calloc(1, sizeof(*cur))))) {
		cur->fd = -1;
		TRIS_LIST_INSERT_TAIL(&firmwares, cur, list);
	}
...
static int iax_firmware_append(struct iax_ie_data *ied, const unsigned char *dev, unsigned int desc)
{
	...
	if (tris_strlen_zero((char *)dev) || !bs)
		return -1;
...
static void reload_firmware(int unload)
{
	...
	if (!unload) {
		snprintf(dir, sizeof(dir), "%s/firmware/iax", tris_config_TRIS_DATA_DIR);
		fwd = opendir(dir);
		if (fwd) {
			while((de = readdir(fwd))) {
				if (de->d_name[0] != '.') {
					...
					if (!try_firmware(fn)) {
						tris_verb(2, "Loaded firmware '%s'\n", de->d_name);
					}
				}
			}
			closedir(fwd);
		} else 
			tris_log(LOG_WARNING, "Error opening firmware directory '%s': %s\n", dir, strerror(errno));
	}

	/* Clean up leftovers */
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&firmwares, cur, list) {
		if (!cur->dead)
			continue;
...
static int __do_deliver(void *data)
{
	...
	fr->retrans = -1;
	tris_clear_flag(&fr->af, TRIS_FRFLAG_HAS_TIMING_INFO);
	if (iaxs[fr->callno] && !tris_test_flag(iaxs[fr->callno], IAX_ALREADYGONE))
...
static int handle_error(void)
{
	...
	if (res < 0)
		tris_log(LOG_WARNING, "Error detected, but unable to read error: %s\n", strerror(errno));
	else {
		if (m.msg_controllen) {
			sin = (struct sockaddr_in *)SO_EE_OFFENDER(&e);
			if (sin) 
				tris_log(LOG_WARNING, "Receive error from %s\n", tris_inet_ntoa(sin->sin_addr));
			else
				tris_log(LOG_WARNING, "No address detected??\n");
		} else {
			tris_log(LOG_WARNING, "Local error: %s\n", strerror(e.ee_errno));
		}
	}
#endif
	return 0;
...
static int send_packet(struct iax_frame *f)
{
	...
	if (iaxdebug)
		tris_debug(3, "Sending %d on %d/%d to %s:%d\n", f->ts, callno, iaxs[callno]->peercallno, tris_inet_ntoa(iaxs[callno]->addr.sin_addr), ntohs(iaxs[callno]->addr.sin_port));
	
	if (f->transfer) {
		if (iaxdebug)
			iax_showframe(f, NULL, 0, &iaxs[callno]->transfer, f->datalen - sizeof(struct tris_iax2_full_hdr));
		res = sendto(iaxs[callno]->sockfd, f->data, f->datalen, 0,(struct sockaddr *)&iaxs[callno]->transfer, sizeof(iaxs[callno]->transfer));
	} else {
		if (iaxdebug)
			iax_showframe(f, NULL, 0, &iaxs[callno]->addr, f->datalen - sizeof(struct tris_iax2_full_hdr));
...
static int iax2_predestroy(int callno)
{
	...
	if (!tris_test_flag(pvt, IAX_ALREADYGONE)) {
		iax2_destroy_helper(pvt);
		tris_set_flag(pvt, IAX_ALREADYGONE);	
	}
...
static void iax2_destroy(int callno)
{
	...
	struct tris_channel *owner = NULL;
...
retry:
	...
	if (owner) {
		if (tris_channel_trylock(owner)) {
			tris_debug(3, "Avoiding IAX destroy deadlock\n");
			DEADLOCK_AVOIDANCE(&iaxsl[callno]);
			goto retry;
		}
	}
	...
	if (pvt) {
		if (!owner) {
			...
			/* It is ok to use tris_queue_hangup() here instead of iax2_queue_hangup()
			 * because we already hold the owner channel lock. */
			tris_queue_hangup(owner);
		}
...
static int update_packet(struct iax_frame *f)
{
	...
	struct tris_iax2_full_hdr *fh = f->data;
	struct tris_frame af;
...
static void __attempt_transmit(const void *data)
{
		...
		if ((f->retries < 0) /* Already ACK'd */ ||
		    (f->retries >= max_retries) /* Too many attempts */) {
				...
				if (f->retries >= max_retries) {
					if (f->transfer) {
						/* Transfer timeout */
						send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_TXREJ, 0, NULL, 0, -1);
					} else if (f->final) {
						iax2_destroy(callno);
					} else {
						if (iaxs[callno]->owner)
							tris_log(LOG_WARNING, "Max retries exceeded to host %s on %s (type = %d, subclass = %d, ts=%d, seqno=%d)\n", tris_inet_ntoa(iaxs[f->callno]->addr.sin_addr),iaxs[f->callno]->owner->name , f->af.frametype, f->af.subclass, f->ts, f->oseqno);
						iaxs[callno]->error = ETIMEDOUT;
						if (iaxs[callno]->owner) {
							struct tris_frame fr = { TRIS_FRAME_CONTROL, TRIS_CONTROL_HANGUP, .data.uint32 = TRIS_CAUSE_DESTINATION_OUT_OF_ORDER };
							/* Hangup the fd */
							iax2_queue_frame(callno, &fr); /* XXX */
							/* Remember, owner could disappear */
							if (iaxs[callno] && iaxs[callno]->owner)
								iaxs[callno]->owner->hangupcause = TRIS_CAUSE_DESTINATION_OUT_OF_ORDER;
						} else {
							if (iaxs[callno]->reg) {
	...
	if (freeme) {
		...
		TRIS_LIST_LOCK(&frame_queue);
		TRIS_LIST_REMOVE(&frame_queue, f, list);
		TRIS_LIST_UNLOCK(&frame_queue);
...
static char *handle_cli_iax2_prune_realtime(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct iax2_peer *peer = NULL;
	struct iax2_user *user = NULL;
	...
	if (!strcmp(a->argv[3], "all")) {
		prune_users();
		prune_peers();
		tris_cli(a->fd, "Cache flushed successfully.\n");
		return CLI_SUCCESS;
	}
	...
	if (peer || user) {
		if (peer) {
			if (tris_test_flag(peer, IAX_RTCACHEFRIENDS)) {
				tris_set_flag(peer, IAX_RTAUTOCLEAR);
				expire_registry(peer_ref(peer));
				tris_cli(a->fd, "Peer %s was removed from the cache.\n", a->argv[3]);
			} else {
				tris_cli(a->fd, "Peer %s is not eligible for this operation.\n", a->argv[3]);
			}
			peer_unref(peer);
		}
		if (user) {
			if (tris_test_flag(user, IAX_RTCACHEFRIENDS)) {
				tris_set_flag(user, IAX_RTAUTOCLEAR);
				tris_cli(a->fd, "User %s was removed from the cache.\n", a->argv[3]);
			} else {
				tris_cli(a->fd, "User %s is not eligible for this operation.\n", a->argv[3]);
			}
			ao2_unlink(users,user);
			user_unref(user);
		}
	} else {
		tris_cli(a->fd, "%s was not found in the cache.\n", a->argv[3]);
	}

	return CLI_SUCCESS;
}

static char *handle_cli_iax2_test_losspct(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
#ifdef IAXTESTS
static char *handle_cli_iax2_test_late(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_iax2_test_resync(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_iax2_test_jitter(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_iax2_show_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char status[30];
	char cbuf[256];
	struct iax2_peer *peer;
	char codec_buf[512];
	struct tris_str *encmethods = tris_str_alloca(256);
	...
	if (peer) {
		...
		tris_cli(a->fd, "  Dynamic      : %s\n", tris_test_flag(peer, IAX_DYNAMIC) ? "Yes" : "No");
		tris_cli(a->fd, "  Callnum limit: %d\n", peer->maxcallno);
		tris_cli(a->fd, "  Calltoken req: %s\n", (peer->calltoken_required == CALLTOKEN_YES) ? "Yes" : ((peer->calltoken_required == CALLTOKEN_AUTO) ? "Auto" : "No"));
		tris_cli(a->fd, "  Trunk        : %s\n", tris_test_flag(peer, IAX_TRUNK) ? "Yes" : "No");
		tris_cli(a->fd, "  Encryption   : %s\n", peer->encmethods ? tris_str_buffer(encmethods) : "No");
		tris_cli(a->fd, "  Callerid     : %s\n", tris_callerid_merge(cbuf, sizeof(cbuf), peer->cid_name, peer->cid_num, "<unspecified>"));
		tris_cli(a->fd, "  Expire       : %d\n", peer->expire);
		tris_cli(a->fd, "  ACL          : %s\n", (peer->ha ? "Yes" : "No"));
		tris_cli(a->fd, "  Addr->IP     : %s Port %d\n",  peer->addr.sin_addr.s_addr ? tris_inet_ntoa(peer->addr.sin_addr) : "(Unspecified)", ntohs(peer->addr.sin_port));
		tris_cli(a->fd, "  Defaddr->IP  : %s Port %d\n", tris_inet_ntoa(peer->defaddr.sin_addr), ntohs(peer->defaddr.sin_port));
		...
		for(x = 0; x < 32 ; x++) {
			codec = tris_codec_pref_index(&peer->prefs,x);
			if(!codec)
				break;
			tris_cli(a->fd, "%s", tris_getformatname(codec));
			if(x < 31 && tris_codec_pref_index(&peer->prefs,x+1))
				tris_cli(a->fd, "|");
		}
		...
		tris_cli(a->fd, "  Status       : ");
		peer_status(peer, status, sizeof(status));	
		tris_cli(a->fd, "%s\n",status);
		tris_cli(a->fd, "  Qualify      : every %dms when OK, every %dms when UNREACHABLE (sample smoothing %s)\n", peer->pokefreqok, peer->pokefreqnotok, peer->smoothing ? "On" : "Off");
		tris_cli(a->fd, "\n");
		peer_unref(peer);
	} else {
...
static char *complete_iax2_peers(const char *line, const char *word, int pos, int state, int flags)
{
	...
	while ((peer = ao2_iterator_next(&i))) {
		if (!strncasecmp(peer->name, word, wordlen) && ++which > state
			&& (!flags || tris_test_flag(peer, flags))) {
			res = tris_strdup(peer->name);
			peer_unref(peer);
			break;
		}
		peer_unref(peer);
...
static char *handle_cli_iax2_show_stats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_cli(a->fd, "    IAX Statistics\n");
	tris_cli(a->fd, "---------------------\n");
	tris_cli(a->fd, "Outstanding frames: %d (%d ingress, %d egress)\n", iax_get_frames(), iax_get_iframes(), iax_get_oframes());
	tris_cli(a->fd, "%d timed and %d untimed transmits; MTU %d/%d/%d\n", trunk_timed, trunk_untimed,
		trunk_maxmtu, trunk_nmaxmtu, global_max_trunk_mtu);
	tris_cli(a->fd, "Packets in transmit queue: %d dead, %d final, %d total\n\n", dead, final, cnt);
...
static char *handle_cli_iax2_set_mtu(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int mtuv;
	...
	if (mtuv == 0) {
		tris_cli(a->fd, "Trunk MTU control disabled (mtu was %d)\n", global_max_trunk_mtu); 
		global_max_trunk_mtu = 0; 
		return CLI_SUCCESS; 
	}
	if (mtuv < 172 || mtuv > 4000) {
		tris_cli(a->fd, "Trunk MTU must be between 172 and 4000\n"); 
		return CLI_SHOWUSAGE; 
	}
	tris_cli(a->fd, "Trunk MTU changed from %d to %d\n", global_max_trunk_mtu, mtuv); 
	global_max_trunk_mtu = mtuv; 
	return CLI_SUCCESS;
}

static char *handle_cli_iax2_show_cache(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	TRIS_LIST_LOCK(&dpcache);

	tris_cli(a->fd, "%-20.20s %-12.12s %-9.9s %-8.8s %s\n", "Peer/Context", "Exten", "Exp.", "Wait.", "Flags");

	TRIS_LIST_TRAVERSE(&dpcache, dp, cache_list) {
		...
		if (!tris_strlen_zero(tmp)) {
			...
			tris_copy_string(tmp, "(none)", sizeof(tmp));
		}
		y = 0;
		...
		if (s > 0) {
			tris_cli(a->fd, "%-20.20s %-12.12s %-9d %-8d %s\n", pc, dp->exten, s, y, tmp);
		} else {
			tris_cli(a->fd, "%-20.20s %-12.12s %-9.9s %-8d %s\n", pc, dp->exten, "(expired)", y, tmp);
		}
	}

	TRIS_LIST_UNLOCK(&dpcache);
...
static void unwrap_timestamp(struct iax_frame *fr)
{
	...
	const int ts_shift = (fr->af.frametype == TRIS_FRAME_VIDEO) ? 15 : 16;
	...
	if ( (fr->ts & upper_mask) == last_upper ) {
		...
		if (x < -threshold) {
			...
			if (iaxdebug)
				tris_debug(1, "schedule_delivery: pushed forward timestamp\n");
		} else if (x > threshold) {
			...
			fr->ts = (last_upper - (1 << ts_shift)) | (fr->ts & lower_mask);
			if (iaxdebug)
				tris_debug(1, "schedule_delivery: pushed back timestamp\n");
		}
	}
}
...
static void update_jbsched(struct chan_iax2_pvt *pvt)
{
	...
	when = tris_tvdiff_ms(tris_tvnow(), pvt->rxcore);
...
static void __get_from_jb(const void *p) 
{
	...
	/* round up a millisecond since tris_sched_runq does; */
	/* prevents us from spinning while waiting for our now */
	/* to catch up with runq's now */
	now.tv_usec += 1000;
	
	ms = tris_tvdiff_ms(now, pvt->rxcore);
	
	if(ms >= (next = jb_next(pvt->jb))) {
		ret = jb_get(pvt->jb,&frame,ms,tris_codec_interp_len(pvt->voiceformat));
		switch(ret) {
		case JB_OK:
			fr = frame.data;
			__do_deliver(fr);
		...
		case JB_INTERP:
		{
			struct tris_frame af = { 0, };
			
			/* create an interpolation frame */
			af.frametype = TRIS_FRAME_VOICE;
			af.subclass = pvt->voiceformat;
			af.samples  = frame.ms * (tris_format_rate(pvt->voiceformat) / 1000);
			af.src  = "IAX2 JB interpolation";
			af.delivery = tris_tvadd(pvt->rxcore, tris_samp2tv(next, 1000));
			af.offset = TRIS_FRIENDLY_OFFSET;
			...
			if (!tris_test_flag(iaxs[callno], IAX_ALREADYGONE)) {
...
static int schedule_delivery(struct iax_frame *fr, int updatehistory, int fromtrunk, unsigned int *tsout)
{
	...
	int needfree = 0;
	struct tris_channel *owner = NULL;
	struct tris_channel *bridge = NULL;
	
	/* Attempt to recover wrapped timestamps */
	unwrap_timestamp(fr);

	/* delivery time is sender's sent timestamp converted back into absolute time according to our clock */
	if ( !fromtrunk && !tris_tvzero(iaxs[fr->callno]->rxcore))
		fr->af.delivery = tris_tvadd(iaxs[fr->callno]->rxcore, tris_samp2tv(fr->ts, 1000));
	else {
#if 0
		tris_debug(1, "schedule_delivery: set delivery to 0 as we don't have an rxcore yet, or frame is from trunk.\n");
#endif
		fr->af.delivery = tris_tv(0,0);
	}

	type = JB_TYPE_CONTROL;
	len = 0;

	if(fr->af.frametype == TRIS_FRAME_VOICE) {
		type = JB_TYPE_VOICE;
		len = tris_codec_get_samples(&fr->af) / (tris_format_rate(fr->af.subclass) / 1000);
	} else if(fr->af.frametype == TRIS_FRAME_CNG) {
		type = JB_TYPE_SILENCE;
	}

	if ( (!tris_test_flag(iaxs[fr->callno], IAX_USEJITTERBUF)) ) {
		if (tsout)
			*tsout = fr->ts;
		__do_deliver(fr);
		return -1;
	...
	if ((owner = iaxs[fr->callno]->owner))
		bridge = tris_bridged_channel(owner);
	...
	if ( (!tris_test_flag(iaxs[fr->callno], IAX_FORCEJITTERBUF)) && owner && bridge && (bridge->tech->properties & TRIS_CHAN_TP_WANTSJITTER) ) {
		jb_frame frame;
		...
		tris_sched_thread_del(sched, iaxs[fr->callno]->jbid);
...
static int iax2_transmit(struct iax_frame *fr)
{
	...
	fr->sentyet = 0;
	TRIS_LIST_LOCK(&frame_queue);
	TRIS_LIST_INSERT_TAIL(&frame_queue, fr, list);
	TRIS_LIST_UNLOCK(&frame_queue);
	/* Wake up the network and scheduler thread */
	if (netthreadid != TRIS_PTHREADT_NULL)
		pthread_kill(netthreadid, SIGURG);
	tris_sched_thread_poke(sched);
	return 0;
}
...
static int iax2_digit_begin(struct tris_channel *c, char digit)
{
	return send_command_locked(PTR_TO_CALLNO(c->tech_pvt), TRIS_FRAME_DTMF_BEGIN, digit, 0, NULL, 0, -1);
}

static int iax2_digit_end(struct tris_channel *c, char digit, unsigned int duration)
{
	return send_command_locked(PTR_TO_CALLNO(c->tech_pvt), TRIS_FRAME_DTMF_END, digit, 0, NULL, 0, -1);
}

static int iax2_sendtext(struct tris_channel *c, const char *text)
{
	
	return send_command_locked(PTR_TO_CALLNO(c->tech_pvt), TRIS_FRAME_TEXT,
		0, 0, (unsigned char *)text, strlen(text) + 1, -1);
}

static int iax2_sendimage(struct tris_channel *c, struct tris_frame *img)
{
	return send_command_locked(PTR_TO_CALLNO(c->tech_pvt), TRIS_FRAME_IMAGE, img->subclass, 0, img->data.ptr, img->datalen, -1);
}

static int iax2_sendhtml(struct tris_channel *c, int subclass, const char *data, int datalen)
{
	return send_command_locked(PTR_TO_CALLNO(c->tech_pvt), TRIS_FRAME_HTML, subclass, 0, (unsigned char *)data, datalen, -1);
}
...
static int iax2_fixup(struct tris_channel *oldchannel, struct tris_channel *newchan)
{
	...
	if (iaxs[callno])
		...
		tris_log(LOG_WARNING, "Uh, this isn't a good sign...\n");
	tris_mutex_unlock(&iaxsl[callno]);
	return 0;
}
...
static struct iax2_peer *realtime_peer(const char *peername, struct sockaddr_in *sin)
{
	...
	if (peername) {
		var = tris_load_realtime("iaxpeers", "name", peername, "host", "dynamic", SENTINEL);
		if (!var && sin)
			var = tris_load_realtime("iaxpeers", "name", peername, "host", tris_inet_ntoa(sin->sin_addr), SENTINEL);
	} else if (sin) {
		char porta[25];
		sprintf(porta, "%d", ntohs(sin->sin_port));
		var = tris_load_realtime("iaxpeers", "ipaddr", tris_inet_ntoa(sin->sin_addr), "port", porta, SENTINEL);
		if (var) {
	...
	if (!var && peername) { /* Last ditch effort */
		var = tris_load_realtime("iaxpeers", "name", peername, SENTINEL);
		/*!\note
	...
	peer = build_peer(peername, var, NULL, tris_test_flag((&globalflags), IAX_RTCACHEFRIENDS) ? 0 : 1);
	...
	for (tmp = var; tmp; tmp = tmp->next) {
		...
		if (!strcasecmp(tmp->name, "type")) {
			...
			tris_get_time_t(tmp->value, &regseconds, 0, NULL);
	...
	if (tris_test_flag((&globalflags), IAX_RTCACHEFRIENDS)) {
		tris_copy_flags(peer, &globalflags, IAX_RTAUTOCLEAR|IAX_RTCACHEFRIENDS);
		if (tris_test_flag(peer, IAX_RTAUTOCLEAR)) {
 			if (peer->expire > -1) {
 				if (!tris_sched_thread_del(sched, peer->expire)) {
 					peer->expire = -1;
 					peer_unref(peer);
 				}
 			}
		...
		ao2_link(peers, peer);
		if (tris_test_flag(peer, IAX_DYNAMIC))
			reg_source_db(peer);
	} else {
		tris_set_flag(peer, IAX_TEMPONLY);	
	}

	if (!tris_test_flag(&globalflags, IAX_RTIGNOREREGEXPIRE) && dynamic) {
		time(&nowtime);
		if ((nowtime - regseconds) > IAX_DEFAULT_REG_EXPIRE) {
			memset(&peer->addr, 0, sizeof(peer->addr));
			realtime_update_peer(peer->name, &peer->addr, 0);
			tris_debug(1, "realtime_peer: Bah, '%s' is expired (%d/%d/%d)!\n",
				peername, (int)(nowtime - regseconds), (int)regseconds, (int)nowtime);
		}
		else {
			tris_debug(1, "realtime_peer: Registration for '%s' still active (%d/%d/%d)!\n",
				peername, (int)(nowtime - regseconds), (int)regseconds, (int)nowtime);
		}
	}
...
static struct iax2_user *realtime_user(const char *username, struct sockaddr_in *sin)
{
	...
	var = tris_load_realtime("iaxusers", "name", username, "host", "dynamic", SENTINEL);
	if (!var)
		var = tris_load_realtime("iaxusers", "name", username, "host", tris_inet_ntoa(sin->sin_addr), SENTINEL);
	if (!var && sin) {
		char porta[6];
		snprintf(porta, sizeof(porta), "%d", ntohs(sin->sin_port));
		var = tris_load_realtime("iaxusers", "name", username, "ipaddr", tris_inet_ntoa(sin->sin_addr), "port", porta, SENTINEL);
		if (!var)
			var = tris_load_realtime("iaxusers", "ipaddr", tris_inet_ntoa(sin->sin_addr), "port", porta, SENTINEL);
	}
	if (!var) { /* Last ditch effort */
		var = tris_load_realtime("iaxusers", "name", username, SENTINEL);
		/*!\note
		 * If this one loaded something, then we need to ensure that the host
		 * field matched.  The only reason why we can't have this as a criteria
		 * is because we only have the IP address and the host field might be
	...
	user = build_user(username, var, NULL, !tris_test_flag((&globalflags), IAX_RTCACHEFRIENDS));
	...
	if (tris_test_flag((&globalflags), IAX_RTCACHEFRIENDS)) {
		tris_set_flag(user, IAX_RTCACHEFRIENDS);
		ao2_link(users, user);
	} else {
		tris_set_flag(user, IAX_TEMPONLY);	
	}
...
static void realtime_update_peer(const char *peername, struct sockaddr_in *sin, time_t regtime)
{
	...
	tris_update_realtime("iaxpeers", "name", peername, 
		"ipaddr", tris_inet_ntoa(sin->sin_addr), "port", port, 
...
static int create_addr(const char *peername, struct tris_channel *c, struct sockaddr_in *sin, struct create_addr_info *cai)
{
	struct iax2_peer *peer;
	int res = -1;
	struct tris_codec_pref ourprefs;

	tris_clear_flag(cai, IAX_SENDANI | IAX_TRUNK);
	...
	if (!(peer = find_peer(peername, 1))) {
		cai->found = 0;
		if (tris_get_ip_or_srv(sin, peername, srvlookup ? "_iax._udp" : NULL)) {
		...
		if (c)
			tris_codec_pref_prepend(&ourprefs, c->nativeformats, 1);
		tris_codec_pref_convert(&ourprefs, cai->prefs, sizeof(cai->prefs), 1);
		return 0;
	}
	...
	tris_copy_flags(cai, peer, IAX_SENDANI | IAX_TRUNK | IAX_NOTRANSFER | IAX_TRANSFERMEDIA | IAX_USEJITTERBUF | IAX_FORCEJITTERBUF | IAX_FORCE_ENCRYPT);
	...
	if (c) {
		tris_debug(1, "prepending %x to prefs\n", c->nativeformats);
		tris_codec_pref_prepend(&ourprefs, c->nativeformats, 1);
	}
	tris_codec_pref_convert(&ourprefs, cai->prefs, sizeof(cai->prefs), 1);
	tris_copy_string(cai->context, peer->context, sizeof(cai->context));
	tris_copy_string(cai->peercontext, peer->peercontext, sizeof(cai->peercontext));
	tris_copy_string(cai->username, peer->username, sizeof(cai->username));
	tris_copy_string(cai->timezone, peer->zonetag, sizeof(cai->timezone));
	tris_copy_string(cai->outkey, peer->outkey, sizeof(cai->outkey));
	tris_copy_string(cai->mohinterpret, peer->mohinterpret, sizeof(cai->mohinterpret));
	tris_copy_string(cai->mohsuggest, peer->mohsuggest, sizeof(cai->mohsuggest));
	if (tris_strlen_zero(peer->dbsecret)) {
		tris_copy_string(cai->secret, peer->secret, sizeof(cai->secret));
	} else {
		char *family;
		char *key = NULL;

		family = tris_strdupa(peer->dbsecret);
		key = strchr(family, '/');
		if (key)
			*key++ = '\0';
		if (!key || tris_db_get(family, key, cai->secret, sizeof(cai->secret))) {
			tris_log(LOG_WARNING, "Unable to retrieve database password for family/key '%s'!\n", peer->dbsecret);
			goto return_unref;
		}
	}
...
static void __auto_congest(const void *nothing)
{
	...
	struct tris_frame f = { TRIS_FRAME_CONTROL, TRIS_CONTROL_CONGESTION };
	tris_mutex_lock(&iaxsl[callno]);
	if (iaxs[callno]) {
		iaxs[callno]->initid = -1;
		iax2_queue_frame(callno, &f);
		tris_log(LOG_NOTICE, "Auto-congesting call due to slow response\n");
	}
	tris_mutex_unlock(&iaxsl[callno]);
}
...
static unsigned int iax2_datetime(const char *tz)
{
	...
	struct tris_tm tm;
	unsigned int tmp;
	tris_localtime(&t, &tm, tris_strlen_zero(tz) ? NULL : tz);
...
{
	struct {
		struct tris_iax2_full_hdr f;
		struct iax_ie_data ied;
	} data;
	size_t size = sizeof(struct tris_iax2_full_hdr);
...
static void resend_with_token(int callno, struct iax_frame *f, const char *newtoken)
{
	...
	int ie_data_pos = f->datalen - sizeof(struct tris_iax2_full_hdr);
	...
	/* ---6.--- */
	send_command(pvt, TRIS_FRAME_IAX, subclass, 0, data.ied.buf, data.ied.pos, -1);
}
 ...
 * 
 * \note frametype must be TRIS_FRAME_IAX.
 * 
 * \note
 ...
 */
static int handle_call_token(struct tris_iax2_full_hdr *fh, struct iax_ies *ies,
...
#define CALLTOKEN_IE_FORMAT   "%u?%s"     /* time + ? + (40 char hash) */
	struct tris_str *buf = tris_str_alloca(256);
	time_t t = time(NULL);
	...
	if (ies->calltoken && !ies->calltokendata) {  /* empty calltoken is provided, client supports calltokens */
		...
		tris_str_set(&buf, 0, CALLTOKEN_HASH_FORMAT, tris_inet_ntoa(sin->sin_addr), sin->sin_port, (unsigned int) t, randomcalltokendata);
		tris_sha1_hash(hash, tris_str_buffer(buf));

		tris_str_set(&buf, 0, CALLTOKEN_IE_FORMAT, (unsigned int) t, hash);
		iax_ie_append_str(&ied, IAX_IE_CALLTOKEN, tris_str_buffer(buf));
	...
	/* ----- Case 2 ----- */
		...
		tris_str_set(&buf, 0, CALLTOKEN_HASH_FORMAT, tris_inet_ntoa(sin->sin_addr), sin->sin_port, (unsigned int) rec_time, randomcalltokendata);
		tris_sha1_hash(hash, tris_str_buffer(buf));

		/* compare hashes and then check timestamp delay */
		if (strcmp(hash, rec_hash)) {
			tris_log(LOG_WARNING, "Address %s failed CallToken hash inspection\n", tris_inet_ntoa(sin->sin_addr));
			goto reject; /* received hash does not match ours, reject */
		} else if ((t < rec_time) || ((t - rec_time) >= MAX_CALLTOKEN_DELAY)) {
			tris_log(LOG_WARNING, "Too much delay in IAX2 calltoken timestamp from address %s\n", tris_inet_ntoa(sin->sin_addr));
	...
	/* ----- Case 3 ----- */
		...
		if (calltoken_required(sin, ies->username, subclass)) {
			tris_log(LOG_ERROR, "Call rejected, CallToken Support required. If unexpected, resolve by placing address %s in the calltokenoptional list or setting user %s requirecalltoken=no\n", tris_inet_ntoa(sin->sin_addr), S_OR(ies->username, "guest"));
			goto reject;
		}
...
static void parse_dial_string(char *data, struct parsed_dial_string *pds)
{
	...
	*/
	if (pds->password && (pds->password[0] == '[')) {
		pds->key = tris_strip_quoted(pds->password, "[", "]");
		pds->password = NULL;
	}
}
...
static int iax2_call(struct tris_channel *c, char *dest, int timeout)
{
	...
	struct tris_datastore *variablestore = tris_channel_datastore_find(c, &iax2_variable_datastore_info, NULL);
	...
	if ((c->_state != TRIS_STATE_DOWN) && (c->_state != TRIS_STATE_RESERVED)) {
		tris_log(LOG_WARNING, "Channel is already in use (%s)?\n", c->name);
		return -1;
	}
	...
	tmpstr = tris_strdupa(dest);
	parse_dial_string(tmpstr, &pds);

	if (tris_strlen_zero(pds.peer)) {
		tris_log(LOG_WARNING, "No peer provided in the IAX2 dial string '%s'\n", dest);
		return -1;
	}
	if (!pds.exten) {
	...
	if (create_addr(pds.peer, c, &sin, &cai)) {
		tris_log(LOG_WARNING, "No address associated with '%s'\n", pds.peer);
		return -1;
	}
	if (tris_strlen_zero(cai.secret) && tris_test_flag(iaxs[callno], IAX_FORCE_ENCRYPT)) {
		tris_log(LOG_WARNING, "Call terminated. No secret given and force encrypt enabled\n");
		return -1;
	}
	if (!pds.username && !tris_strlen_zero(cai.username))
		pds.username = cai.username;
	if (!pds.password && !tris_strlen_zero(cai.secret))
		pds.password = cai.secret;
	if (!pds.key && !tris_strlen_zero(cai.outkey))
		pds.key = cai.outkey;
	if (!pds.context && !tris_strlen_zero(cai.peercontext))
		pds.context = cai.peercontext;

	/* Keep track of the context for outgoing calls too */
	tris_copy_string(c->context, cai.context, sizeof(c->context));
	...
	if (l) {
		...
		if (n)
			...
			iax_ie_append_byte(&ied, IAX_IE_CALLINGPRES, TRIS_PRES_NUMBER_NOT_AVAILABLE);
	}
	...
	if (tris_test_flag(iaxs[callno], IAX_SENDANI) && c->cid.cid_ani)
		iax_ie_append_str(&ied, IAX_IE_CALLING_ANI, c->cid.cid_ani);

	if (!tris_strlen_zero(c->language))
		iax_ie_append_str(&ied, IAX_IE_LANGUAGE, c->language);
	if (!tris_strlen_zero(c->cid.cid_dnid))
		iax_ie_append_str(&ied, IAX_IE_DNID, c->cid.cid_dnid);
	if (!tris_strlen_zero(c->cid.cid_rdnis))
	...
	if (!tris_strlen_zero(c->context))
		tris_string_field_set(iaxs[callno], context, c->context);

	if (pds.username)
		tris_string_field_set(iaxs[callno], username, pds.username);

	iaxs[callno]->encmethods = cai.encmethods;

	iaxs[callno]->adsi = cai.adsi;
	
	tris_string_field_set(iaxs[callno], mohinterpret, cai.mohinterpret);
	tris_string_field_set(iaxs[callno], mohsuggest, cai.mohsuggest);

	if (pds.key)
		tris_string_field_set(iaxs[callno], outkey, pds.key);
	if (pds.password)
		tris_string_field_set(iaxs[callno], secret, pds.password);
	...
	if (!tris_strlen_zero(osp_token_ptr)) {
		if ((osp_token_length = strlen(osp_token_ptr)) <= IAX_MAX_OSPTOKEN_SIZE) {
			...
			tris_log(LOG_WARNING, "OSP token is too long\n");
	} else if (iaxdebug)
		tris_debug(1, "OSP token is undefined\n");
	...
	/* Add remote vars */
	if (variablestore) {
		TRIS_LIST_HEAD(, tris_var_t) *variablelist = variablestore->data;
		tris_debug(1, "Found an IAX variable store on this channel\n");
		TRIS_LIST_LOCK(variablelist);
		TRIS_LIST_TRAVERSE(variablelist, var, entries) {
			char tmp[256];
			int i;
			tris_debug(1, "Found IAXVAR '%s' with value '%s' (to transmit)\n", tris_var_name(var), tris_var_value(var));
			/* Automatically divide the value up into sized chunks */
			for (i = 0; i < strlen(tris_var_value(var)); i += 255 - (strlen(tris_var_name(var)) + 1)) {
				snprintf(tmp, sizeof(tmp), "%s=%s", tris_var_name(var), tris_var_value(var) + i);
				iax_ie_append_str(&ied, IAX_IE_VARIABLE, tmp);
			}
		}
		TRIS_LIST_UNLOCK(variablelist);
	}

	/* Transmit the string in a "NEW" request */
	add_empty_calltoken_ie(iaxs[callno], &ied); /* this _MUST_ be the last ie added */
	send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_NEW, 0, ied.buf, ied.pos, -1);

	tris_mutex_unlock(&iaxsl[callno]);
	tris_setstate(c, TRIS_STATE_RINGING);
...
static int iax2_hangup(struct tris_channel *c) 
{
	...
	tris_mutex_lock(&iaxsl[callno]);
	if (callno && iaxs[callno]) {
		tris_debug(1, "We're hanging up %s now...\n", c->name);
		alreadygone = tris_test_flag(iaxs[callno], IAX_ALREADYGONE);
		/* Send the hangup unless we have had a transmission error or are already gone */
 		iax_ie_append_byte(&ied, IAX_IE_CAUSECODE, (unsigned char)c->hangupcause);
		if (!iaxs[callno]->error && !alreadygone) {
 			if (send_command_final(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_HANGUP, 0, ied.buf, ied.pos, -1)) {
				tris_log(LOG_WARNING, "No final packet could be sent for callno %d\n", callno);
			}
			if (!iaxs[callno]) {
				tris_mutex_unlock(&iaxsl[callno]);
				return 0;
		...
		if (iaxs[callno] && alreadygone) {
			tris_debug(1, "Really destroying %s now...\n", c->name);
			iax2_destroy(callno);
		} else if (iaxs[callno]) {
			if (tris_sched_thread_add(sched, 10000, scheduled_destroy, CALLNO_TO_PTR(callno)) < 0) {
				tris_log(LOG_ERROR, "Unable to schedule iax2 callno %d destruction?!!  Destroying immediately.\n", callno);
				iax2_destroy(callno);
			}
		}
	} else if (c->tech_pvt) {
		 ...
		 * to avoid tris_channel_free() trying to free it.
		 */
		c->tech_pvt = NULL;
	}
	tris_mutex_unlock(&iaxsl[callno]);
...
static int iax2_setoption(struct tris_channel *c, int option, void *data, int datalen)
{
	...
	default:
	{
		...
		if (!(h = tris_malloc(datalen + sizeof(*h)))) {
			return -1;
		}

		h->flag = TRIS_OPTION_FLAG_REQUEST;
		h->option = htons(option);
		memcpy(h->data, data, datalen);
		res = send_command_locked(PTR_TO_CALLNO(c->tech_pvt), TRIS_FRAME_CONTROL,
					  TRIS_CONTROL_OPTION, 0, (unsigned char *) h,
					  datalen + sizeof(*h), -1);
		tris_free(h);
		return res;
	}
...
static struct tris_frame *iax2_read(struct tris_channel *c) 
{
	tris_log(LOG_NOTICE, "I should never be called!\n");
...
static int iax2_key_rotate(const void *vpvt)
{
	...
	tris_mutex_lock(&iaxsl[pvt->callno]);
	pvt->keyrotateid = 
		tris_sched_thread_add(sched, 120000 + (tris_random() % 180001), iax2_key_rotate, vpvt);

	snprintf(key, sizeof(key), "%lX", tris_random());
	...
	res = send_command(pvt, TRIS_FRAME_IAX, IAX_COMMAND_RTKEY, 0, ied.buf, ied.pos, -1);
...
static int iax2_start_transfer(unsigned short callno0, unsigned short callno1, int mediaonly)
{
	...
	unsigned int transferid = (unsigned int)tris_random();

	if (IAX_CALLENCRYPTED(iaxs[callno0]) || IAX_CALLENCRYPTED(iaxs[callno1])) {
		tris_debug(1, "transfers are not supported for encrypted calls at this time");
		tris_set_flag(iaxs[callno0], IAX_NOTRANSFER);
		tris_set_flag(iaxs[callno1], IAX_NOTRANSFER);
		return 0;
	}
	...
	res = send_command(iaxs[callno0], TRIS_FRAME_IAX, IAX_COMMAND_TXREQ, 0, ied0.buf, ied0.pos, -1);
	if (res)
		return -1;
	res = send_command(iaxs[callno1], TRIS_FRAME_IAX, IAX_COMMAND_TXREQ, 0, ied1.buf, ied1.pos, -1);
	if (res)
		return -1;
...
{
	tris_mutex_lock(&iaxsl[callno0]);
	while (tris_mutex_trylock(&iaxsl[callno1])) {
...
{
	tris_mutex_unlock(&iaxsl[callno1]);
	tris_mutex_unlock(&iaxsl[callno0]);
}
...
static enum tris_bridge_result iax2_bridge(struct tris_channel *c0, struct tris_channel *c1, int flags, struct tris_frame **fo, struct tris_channel **rc, int timeoutms)
{
	...
	for (/* ever */;;) {
		...
		if ((c0->tech != &iax2_tech) || (c1->tech != &iax2_tech)) {
			tris_verb(3, "Can't masquerade, we're different...\n");
		...
		if (c0->nativeformats != c1->nativeformats) {
				char buf0[255];
				char buf1[255];
				tris_getformatname_multiple(buf0, sizeof(buf0) -1, c0->nativeformats);
				tris_getformatname_multiple(buf1, sizeof(buf1) -1, c1->nativeformats);
			tris_verb(3, "Operating with different codecs %d[%s] %d[%s] , can't native bridge...\n", c0->nativeformats, buf0, c1->nativeformats, buf1);
			/* Remove from native mode */
			lock_both(callno0, callno1);
			if (iaxs[callno0])
				iaxs[callno0]->bridgecallno = 0;
		...
		if (!transferstarted && !tris_test_flag(iaxs[callno0], IAX_NOTRANSFER) && !tris_test_flag(iaxs[callno1], IAX_NOTRANSFER)) {
			/* Try the transfer */
			if (iax2_start_transfer(callno0, callno1, (flags & (TRIS_BRIDGE_DTMF_CHANNEL_0 | TRIS_BRIDGE_DTMF_CHANNEL_1)) ||
							tris_test_flag(iaxs[callno0], IAX_TRANSFERMEDIA) | tris_test_flag(iaxs[callno1], IAX_TRANSFERMEDIA)))
				tris_log(LOG_WARNING, "Unable to start the transfer\n");
			transferstarted = 1;
		}
		if ((iaxs[callno0]->transferring == TRANSFER_RELEASED) && (iaxs[callno1]->transferring == TRANSFER_RELEASED)) {
			/* Call has been transferred.  We're no longer involved */
			struct timeval now = tris_tvnow();
			if (tris_tvzero(waittimer)) {
				waittimer = now;
			} else if (now.tv_sec - waittimer.tv_sec > IAX_LINGER_TIMEOUT) {
				c0->_softhangup |= TRIS_SOFTHANGUP_DEV;
				c1->_softhangup |= TRIS_SOFTHANGUP_DEV;
				*fo = NULL;
				*rc = c0;
		...
		if ((f->frametype == TRIS_FRAME_CONTROL) && !(flags & TRIS_BRIDGE_IGNORE_SIGS) && (f->subclass != TRIS_CONTROL_SRCUPDATE)) {
			*fo = f;
			*rc = who;
			res =  TRIS_BRIDGE_COMPLETE;
			break;
		...
		if ((f->frametype == TRIS_FRAME_VOICE) ||
			 ...
			 */
			int monitored_source = (who == c0) ? TRIS_BRIDGE_DTMF_CHANNEL_0 : TRIS_BRIDGE_DTMF_CHANNEL_1;
			if (f->frametype == TRIS_FRAME_DTMF && (flags & monitored_source)) {
				*rc = who;
				*fo = f;
				res = TRIS_BRIDGE_COMPLETE;
				/* Remove from native mode */
...
{
	unsigned short callno = PTR_TO_CALLNO(c->tech_pvt);
	tris_debug(1, "Answering IAX2 call\n");
	tris_mutex_lock(&iaxsl[callno]);
	if (iaxs[callno])
		iax2_ami_channelupdate(iaxs[callno]);
	tris_mutex_unlock(&iaxsl[callno]);
	return send_command_locked(callno, TRIS_FRAME_CONTROL, TRIS_CONTROL_ANSWER, 0, NULL, 0, -1);
}
...
static int iax2_indicate(struct tris_channel *c, int condition, const void *data, size_t datalen)
{
	...
	if (iaxdebug)
		tris_debug(1, "Indicating condition %d\n", condition);
	...
	case TRIS_CONTROL_HOLD:
		if (strcasecmp(pvt->mohinterpret, "passthrough")) {
			tris_moh_start(c, data, pvt->mohinterpret);
			goto done;
		}
		break;
	...
	res = send_command(pvt, TRIS_FRAME_CONTROL, condition, 0, data, datalen, -1);
...
static int iax2_transfer(struct tris_channel *c, const char *dest)
{
	...
	tris_debug(1, "Transferring '%s' to '%s'\n", c->name, dest);
	return send_command_locked(callno, TRIS_FRAME_IAX, IAX_COMMAND_TRANSFER, 0, ied.buf, ied.pos, -1);
}
...
static int iax2_getpeertrunk(struct sockaddr_in sin)
{
	...
	while ((peer = ao2_iterator_next(&i))) {
		if ((peer->addr.sin_addr.s_addr == sin.sin_addr.s_addr) &&
		    (peer->addr.sin_port == sin.sin_port)) {
			res = tris_test_flag(peer, IAX_TRUNK);
			peer_unref(peer);
			break;
		}
		peer_unref(peer);
...
static struct tris_channel *tris_iax2_new(int callno, int state, int capability)
{
	...
	if (!(i = iaxs[callno])) {
		tris_log(LOG_WARNING, "No IAX2 pvt found for callno '%d' !\n", callno);
		return NULL;
	}

	/* Don't hold call lock */
	tris_mutex_unlock(&iaxsl[callno]);
	tmp = tris_channel_alloc(1, state, i->cid_num, i->cid_name, i->accountcode, i->exten, i->context, i->amaflags, "IAX2/%s-%d", i->host, i->callno);
	tris_mutex_lock(&iaxsl[callno]);
	if (i != iaxs[callno]) {
		if (tmp) {
	...
	tmp->readformat = tmp->rawreadformat = tris_best_codec(capability);
	tmp->writeformat = tmp->rawwriteformat = tris_best_codec(capability);
	...
	if (!tris_strlen_zero(i->ani))
		tmp->cid.cid_ani = tris_strdup(i->ani);
	else
	...
	if (i->iaxvars) {
		struct tris_datastore *variablestore;
		struct tris_variable *var, *prev = NULL;
		TRIS_LIST_HEAD(, tris_var_t) *varlist;
		tris_debug(1, "Loading up the channel with IAXVARs\n");
		varlist = tris_calloc(1, sizeof(*varlist));
		variablestore = tris_datastore_alloc(&iax2_variable_datastore_info, NULL);
		if (variablestore && varlist) {
			variablestore->data = varlist;
			variablestore->inheritance = DATASTORE_INHERIT_FOREVER;
			TRIS_LIST_HEAD_INIT(varlist);
			for (var = i->iaxvars; var; var = var->next) {
				struct tris_var_t *newvar = tris_var_assign(var->name, var->value);
				if (prev)
					tris_free(prev);
				prev = var;
				if (!newvar) {
					/* Don't abort list traversal, as this would leave i->iaxvars in an inconsistent state. */
					tris_log(LOG_ERROR, "Memory allocation error while processing IAX2 variables\n");
				} else {
					TRIS_LIST_INSERT_TAIL(varlist, newvar, entries);
				}
			}
			if (prev)
				tris_free(prev);
			i->iaxvars = NULL;
			tris_channel_datastore_add(i->owner, variablestore);
		} else {
			if (variablestore) {
				tris_datastore_free(variablestore);
			}
			if (varlist) {
				tris_free(varlist);
			}
...
static unsigned int calc_txpeerstamp(struct iax2_trunk_peer *tpeer, int sampms, struct timeval *now)
{
	...
	tpeer->trunkact = *now;
	mssincetx = tris_tvdiff_ms(*now, tpeer->lasttxtime);
	if (mssincetx > 5000 || tris_tvzero(tpeer->txtrunktime)) {
	...
	/* Calculate ms offset */
	ms = tris_tvdiff_ms(*now, tpeer->txtrunktime);
...
static unsigned int fix_peerts(struct timeval *rxtrunktime, int callno, unsigned int ts)
{
	...
	if (tris_tvzero(iaxs[callno]->rxcore)) {
		/* Initialize rxcore time if appropriate */
		iaxs[callno]->rxcore = tris_tvnow();
	...
	ms = tris_tvdiff_ms(*rxtrunktime, iaxs[callno]->rxcore);
	/* Return as the sum of trunk time and the difference between trunk and real time */
	return ms + ts;
}

static unsigned int calc_timestamp(struct chan_iax2_pvt *p, unsigned int ts, struct tris_frame *f)
{
	int ms;
	int voice = 0;
	int genuine = 0;
	int adjust;
	int rate = tris_format_rate(f->subclass) / 1000;
	...
	*/
	if (f) {
		if (f->frametype == TRIS_FRAME_VOICE) {
			voice = 1;
			delivery = &f->delivery;
		} else if (f->frametype == TRIS_FRAME_IAX) {
			genuine = 1;
		} else if (f->frametype == TRIS_FRAME_CNG) {
			p->notsilenttx = 0;	
		}
	}
	if (tris_tvzero(p->offset)) {
		p->offset = tris_tvnow();
	...
	if (delivery && !tris_tvzero(*delivery)) {
		ms = tris_tvdiff_ms(*delivery, p->offset);
		if (ms < 0) {
			ms = 0;
		}
		if (iaxdebug)
			tris_debug(3, "calc_timestamp: call %d/%d: Timestamp slaved to delivery time\n", p->callno, iaxs[p->callno]->peercallno);
	} else {
		ms = tris_tvdiff_ms(tris_tvnow(), p->offset);
		if (ms < 0)
			ms = 0;
		if (voice) {
			...
			if (p->notsilenttx && abs(ms - p->nextpred) <= MAX_TIMESTAMP_SKEW) {
				...
				*/
				...
				if (adjust < 0)
					p->offset = tris_tvsub(p->offset, tris_samp2tv(abs(adjust), 10000));
				else if (adjust > 0)
					p->offset = tris_tvadd(p->offset, tris_samp2tv(adjust, 10000));
				...
				* frame size too) */
				...
				if (iaxdebug && abs(ms - p->nextpred) > MAX_TIMESTAMP_SKEW )
					tris_debug(1, "predicted timestamp skew (%u) > max (%u), using real ts instead.\n",
...
static unsigned int calc_rxstamp(struct chan_iax2_pvt *p, unsigned int offset)
{
	...
	if (tris_tvzero(p->rxcore)) {
		p->rxcore = tris_tvnow();
		if (iaxdebug)
			tris_debug(1, "calc_rxstamp: call=%d: rxcore set to %d.%6.6d - %dms\n",
					p->callno, (int)(p->rxcore.tv_sec), (int)(p->rxcore.tv_usec), offset);
		p->rxcore = tris_tvsub(p->rxcore, tris_samp2tv(offset, 1000));
#if 1
		if (iaxdebug)
			tris_debug(1, "calc_rxstamp: call=%d: works out as %d.%6.6d\n",
					p->callno, (int)(p->rxcore.tv_sec),(int)( p->rxcore.tv_usec));
#endif
	}

	ms = tris_tvdiff_ms(tris_tvnow(), p->rxcore);
#ifdef IAXTESTS
	if (test_jit) {
		if (!test_jitpct || ((100.0 * tris_random() / (RAND_MAX + 1.0)) < test_jitpct)) {
			jit = (int)((float)test_jit * tris_random() / (RAND_MAX + 1.0));
			if ((int)(2.0 * tris_random() / (RAND_MAX + 1.0)))
				jit = -jit;
			ms += jit;
		}
	}
...
static struct iax2_trunk_peer *find_tpeer(struct sockaddr_in *sin, int fd)
{
	...
	TRIS_LIST_LOCK(&tpeers);

	TRIS_LIST_TRAVERSE(&tpeers, tpeer, list) {
		if (!inaddrcmp(&tpeer->addr, sin)) {
			tris_mutex_lock(&tpeer->lock);
			break;
		}
	}

	if (!tpeer) {
		if ((tpeer = tris_calloc(1, sizeof(*tpeer)))) {
			tris_mutex_init(&tpeer->lock);
			tpeer->lastsent = 9999;
			memcpy(&tpeer->addr, sin, sizeof(tpeer->addr));
			tpeer->trunkact = tris_tvnow();
			tris_mutex_lock(&tpeer->lock);
			tpeer->sockfd = fd;
#ifdef SO_NO_CHECK
			setsockopt(tpeer->sockfd, SOL_SOCKET, SO_NO_CHECK, &nochecksums, sizeof(nochecksums));
#endif
			tris_debug(1, "Created trunk peer for '%s:%d'\n", tris_inet_ntoa(tpeer->addr.sin_addr), ntohs(tpeer->addr.sin_port));
			TRIS_LIST_INSERT_TAIL(&tpeers, tpeer, list);
		}
	}

	TRIS_LIST_UNLOCK(&tpeers);
...
static int iax2_trunk_queue(struct chan_iax2_pvt *pvt, struct iax_frame *fr)
{
	...
	struct timeval now;
	int res; 
	struct tris_iax2_meta_trunk_entry *met;
	struct tris_iax2_meta_trunk_mini *mtm;
	...
	if (tpeer) {
		if (tpeer->trunkdatalen + f->datalen + 4 >= tpeer->trunkdataalloc) {
			...
			if (tpeer->trunkdataalloc < trunkmaxsize) {
				if (!(tmp = tris_realloc(tpeer->trunkdata, tpeer->trunkdataalloc + DEFAULT_TRUNKDATA + IAX2_TRUNK_PREFACE))) {
					tris_mutex_unlock(&tpeer->lock);
					return -1;
				}
				
				tpeer->trunkdataalloc += DEFAULT_TRUNKDATA;
				tpeer->trunkdata = tmp;
				tris_debug(1, "Expanded trunk '%s:%d' to %d bytes\n", tris_inet_ntoa(tpeer->addr.sin_addr), ntohs(tpeer->addr.sin_port), tpeer->trunkdataalloc);
			} else {
				tris_log(LOG_WARNING, "Maximum trunk data space exceeded to %s:%d\n", tris_inet_ntoa(tpeer->addr.sin_addr), ntohs(tpeer->addr.sin_port));
				tris_mutex_unlock(&tpeer->lock);
				return -1;
			}
		}
		...
		if (tris_test_flag(&globalflags, IAX_TRUNKTIMESTAMPS)) {
			mtm = (struct tris_iax2_meta_trunk_mini *)ptr;
			mtm->len = htons(f->datalen);
			mtm->mini.callno = htons(pvt->callno);
			mtm->mini.ts = htons(0xffff & fr->ts);
			ptr += sizeof(struct tris_iax2_meta_trunk_mini);
			tpeer->trunkdatalen += sizeof(struct tris_iax2_meta_trunk_mini);
		} else {
			met = (struct tris_iax2_meta_trunk_entry *)ptr;
			...
			ptr += sizeof(struct tris_iax2_meta_trunk_entry);
			tpeer->trunkdatalen += sizeof(struct tris_iax2_meta_trunk_entry);
		}
...
{
	long tmp;
	for (tmp = tris_random(); len > 0; tmp = tris_random()) {
...
static void build_encryption_keys(const unsigned char *digest, struct chan_iax2_pvt *pvt)
{
	...
	tris_aes_decrypt_key(digest, &pvt->dcx);
}
...
static void build_ecx_key(const unsigned char *digest, struct chan_iax2_pvt *pvt)
{
	...
	tris_aes_encrypt_key(digest, &pvt->ecx);
	tris_aes_decrypt_key(digest, &pvt->mydcx);
}

static void memcpy_decrypt(unsigned char *dst, const unsigned char *src, int len, tris_aes_decrypt_key *dcx)
{
#if 0
	/* Debug with "fake encryption" */
	int x;
	if (len % 16)
		tris_log(LOG_WARNING, "len should be multiple of 16, not %d!\n", len);
	for (x=0;x<len;x++)
		dst[x] = src[x] ^ 0xff;
#else	
...
static void memcpy_encrypt(unsigned char *dst, const unsigned char *src, int len, tris_aes_encrypt_key *ecx)
{
#if 0
	/* Debug with "fake encryption" */
	int x;
	...
	while(len > 0) {
		for (x=0;x<16;x++)
			curblock[x] ^= src[x];
		tris_aes_encrypt(curblock, dst, ecx);
...
static int decode_frame(tris_aes_decrypt_key *dcx, struct tris_iax2_full_hdr *fh, struct tris_frame *f, int *datalen)
{
	...
	if (ntohs(fh->scallno) & IAX_FLAG_FULL) {
		struct tris_iax2_full_enc_hdr *efh = (struct tris_iax2_full_enc_hdr *)fh;
		if (*datalen < 16 + sizeof(struct tris_iax2_full_hdr))
			return -1;
		/* Decrypt */
		memcpy_decrypt(workspace, efh->encdata, *datalen - sizeof(struct tris_iax2_full_enc_hdr), dcx);

		padding = 16 + (workspace[15] & 0x0f);
		if (iaxdebug)
			tris_debug(1, "Decoding full frame with length %d (padding = %d) (15=%02x)\n", *datalen, padding, workspace[15]);
		if (*datalen < padding + sizeof(struct tris_iax2_full_hdr))
			return -1;

		*datalen -= padding;
		memcpy(efh->encdata, workspace + padding, *datalen - sizeof(struct tris_iax2_full_enc_hdr));
		f->frametype = fh->type;
		...
		struct tris_iax2_mini_enc_hdr *efh = (struct tris_iax2_mini_enc_hdr *)fh;
		if (iaxdebug)
			tris_debug(1, "Decoding mini with length %d\n", *datalen);
		if (*datalen < 16 + sizeof(struct tris_iax2_mini_hdr))
			return -1;
		/* Decrypt */
		memcpy_decrypt(workspace, efh->encdata, *datalen - sizeof(struct tris_iax2_mini_enc_hdr), dcx);
		padding = 16 + (workspace[15] & 0x0f);
		if (*datalen < padding + sizeof(struct tris_iax2_mini_hdr))
			return -1;
		*datalen -= padding;
		memcpy(efh->encdata, workspace + padding, *datalen - sizeof(struct tris_iax2_mini_enc_hdr));
	}
	return 0;
}
...
static int encrypt_frame(tris_aes_encrypt_key *ecx, struct tris_iax2_full_hdr *fh, unsigned char *poo, int *datalen)
{
	...
	if (ntohs(fh->scallno) & IAX_FLAG_FULL) {
		struct tris_iax2_full_enc_hdr *efh = (struct tris_iax2_full_enc_hdr *)fh;
		if (iaxdebug)
			tris_debug(1, "Encoding full frame %d/%d with length %d\n", fh->type, fh->csub, *datalen);
		padding = 16 - ((*datalen - sizeof(struct tris_iax2_full_enc_hdr)) % 16);
		padding = 16 + (padding & 0xf);
		memcpy(workspace, poo, padding);
		memcpy(workspace + padding, efh->encdata, *datalen - sizeof(struct tris_iax2_full_enc_hdr));
		workspace[15] &= 0xf0;
		workspace[15] |= (padding & 0xf);
		if (iaxdebug)
			tris_debug(1, "Encoding full frame %d/%d with length %d + %d padding (15=%02x)\n", fh->type, fh->csub, *datalen, padding, workspace[15]);
		*datalen += padding;
		memcpy_encrypt(efh->encdata, workspace, *datalen - sizeof(struct tris_iax2_full_enc_hdr), ecx);
		if (*datalen >= 32 + sizeof(struct tris_iax2_full_enc_hdr))
			memcpy(poo, workspace + *datalen - 32, 32);
	} else {
		struct tris_iax2_mini_enc_hdr *efh = (struct tris_iax2_mini_enc_hdr *)fh;
		if (iaxdebug)
			tris_debug(1, "Encoding mini frame with length %d\n", *datalen);
		padding = 16 - ((*datalen - sizeof(struct tris_iax2_mini_enc_hdr)) % 16);
		padding = 16 + (padding & 0xf);
		memcpy(workspace, poo, padding);
		memcpy(workspace + padding, efh->encdata, *datalen - sizeof(struct tris_iax2_mini_enc_hdr));
		workspace[15] &= 0xf0;
		workspace[15] |= (padding & 0x0f);
		*datalen += padding;
		memcpy_encrypt(efh->encdata, workspace, *datalen - sizeof(struct tris_iax2_mini_enc_hdr), ecx);
		if (*datalen >= 32 + sizeof(struct tris_iax2_mini_enc_hdr))
			memcpy(poo, workspace + *datalen - 32, 32);
	}
	return 0;
}

static int decrypt_frame(int callno, struct tris_iax2_full_hdr *fh, struct tris_frame *f, int *datalen)
{
	int res=-1;
	if (!tris_test_flag(iaxs[callno], IAX_KEYPOPULATED)) {
		/* Search for possible keys, given secrets */
		struct MD5Context md5;
		unsigned char digest[16];
		char *tmppw, *stringp;
		
		tmppw = tris_strdupa(iaxs[callno]->secret);
		stringp = tmppw;
		while ((tmppw = strsep(&stringp, ";"))) {
			...
			if (!res) {
				tris_set_flag(iaxs[callno], IAX_KEYPOPULATED);
				break;
			}
		}
	} else 
...
static int iax2_send(struct chan_iax2_pvt *pvt, struct tris_frame *f, unsigned int ts, int seqno, int now, int transfer, int final)
{
	...
	struct tris_iax2_full_hdr *fh;
	struct tris_iax2_mini_hdr *mh;
	struct tris_iax2_video_hdr *vh;
	struct {
	...
	if (!pvt) {
		tris_log(LOG_WARNING, "No private structure for packet?\n");
		return -1;
	}
	...
	if(f->frametype == TRIS_FRAME_VOICE && f->datalen == 0)
		return 0;
#if 0
	tris_log(LOG_NOTICE, 
		"f->frametype %c= TRIS_FRAME_VOICE, %sencrypted, %srotation scheduled...\n",
		*("=!" + (f->frametype == TRIS_FRAME_VOICE)),
		IAX_CALLENCRYPTED(pvt) ? "" : "not ",
		pvt->keyrotateid != -1 ? "" : "no "
	);
#endif
	if (pvt->keyrotateid == -1 && f->frametype == TRIS_FRAME_VOICE && IAX_CALLENCRYPTED(pvt)) {
		iax2_key_rotate(pvt);
	}

	if ((tris_test_flag(pvt, IAX_TRUNK) || 
	...
	if (now) {
		...
		fr = iax_frame_new(DIRECTION_OUTGRESS, tris_test_flag(pvt, IAX_ENCRYPTED) ? f->datalen + 32 : f->datalen, (f->frametype == TRIS_FRAME_VOICE) || (f->frametype == TRIS_FRAME_VIDEO));
	if (!fr) {
	...
	if (!sendmini) {
		...
		fh = (struct tris_iax2_full_hdr *)(fr->af.data.ptr - sizeof(struct tris_iax2_full_hdr));
		...
		if (fr->af.frametype == TRIS_FRAME_VIDEO)
		...
		fr->datalen = fr->af.datalen + sizeof(struct tris_iax2_full_hdr);
		fr->data = fh;
		fr->retries = 0;
		...
		if ((f->frametype == TRIS_FRAME_IAX) && (f->subclass == IAX_COMMAND_ACK))
			fr->retries = -1;
		else if (f->frametype == TRIS_FRAME_VOICE)
			pvt->svoiceformat = f->subclass;
		else if (f->frametype == TRIS_FRAME_VIDEO)
			pvt->svideoformat = f->subclass & ~0x1;
		if (tris_test_flag(pvt, IAX_ENCRYPTED)) {
			if (tris_test_flag(pvt, IAX_KEYPOPULATED)) {
				if (fr->transfer)
					iax_outputframe(fr, NULL, 2, &pvt->transfer, fr->datalen - sizeof(struct tris_iax2_full_hdr));
				else
					iax_outputframe(fr, NULL, 2, &pvt->addr, fr->datalen - sizeof(struct tris_iax2_full_hdr));
				...
				tris_log(LOG_WARNING, "Supposed to send packet encrypted, but no key?\n");
		}
		...
		if (tris_test_flag(pvt, IAX_TRUNK)) {
			...
			res = 0;
		} else if (fr->af.frametype == TRIS_FRAME_VIDEO) {
			/* Video frame have no sequence number */
			fr->oseqno = -1;
			fr->iseqno = -1;
			vh = (struct tris_iax2_video_hdr *)(fr->af.data.ptr - sizeof(struct tris_iax2_video_hdr));
			vh->zeros = 0;
			vh->callno = htons(0x8000 | fr->callno);
			vh->ts = htons((fr->ts & 0x7FFF) | (fr->af.subclass & 0x1 ? 0x8000 : 0));
			fr->datalen = fr->af.datalen + sizeof(struct tris_iax2_video_hdr);
			fr->data = vh;
			fr->retries = -1;
			res = send_packet(fr);			
		} else {
			...
			fr->iseqno = -1;
			/* Mini frame will do */
			mh = (struct tris_iax2_mini_hdr *)(fr->af.data.ptr - sizeof(struct tris_iax2_mini_hdr));
			mh->callno = htons(fr->callno);
			mh->ts = htons(fr->ts & 0xFFFF);
			fr->datalen = fr->af.datalen + sizeof(struct tris_iax2_mini_hdr);
			fr->data = mh;
			fr->retries = -1;
			...
			if (tris_test_flag(pvt, IAX_ENCRYPTED)) {
				if (tris_test_flag(pvt, IAX_KEYPOPULATED)) {
					encrypt_frame(&pvt->ecx, (struct tris_iax2_full_hdr *)mh, pvt->semirand, &fr->datalen);
				} else
					tris_log(LOG_WARNING, "Supposed to send packet encrypted, but no key?\n");
			}
			res = send_packet(fr);
...
static char *handle_cli_iax2_show_users(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	regex_t regexbuf;
	int havepattern = 0;
...
#define FORMAT2 "%-15.15s  %-20.20s  %-15.15d  %-15.15s  %-5.5s  %-5.10s\n"
	...
	tris_cli(a->fd, FORMAT, "Username", "Secret", "Authen", "Def.Context", "A/C","Codec Pref");
	...
	for (user = ao2_iterator_next(&i); user; 
		...
		if (!tris_strlen_zero(user->secret)) {
  			tris_copy_string(auth,user->secret, sizeof(auth));
		} else if (!tris_strlen_zero(user->inkeys)) {
  			snprintf(auth, sizeof(auth), "Key: %-15.15s ", user->inkeys);
 		} else
			tris_copy_string(auth, "-no secret-", sizeof(auth));
		
		if(tris_test_flag(user,IAX_CODEC_NOCAP))
			pstr = "REQ Only";
		else if(tris_test_flag(user,IAX_CODEC_NOPREFS))
			pstr = "Disabled";
		else
			pstr = tris_test_flag(user,IAX_CODEC_USER_FIRST) ? "Caller" : "Host";
		
		tris_cli(a->fd, FORMAT2, user->name, auth, user->authmethods, 
...
#define FORMAT "%-15.15s  %-15.15s %s  %-15.15s  %-5d%s  %s %-10s%s"
	...
	if (!s)
		tris_cli(fd, FORMAT2, "Name/Username", "Host", "   ", "Mask", "Port", "   ", "Status", term);
	...
	for (peer = ao2_iterator_next(&i); peer; 
		...
		tris_copy_string(nm, tris_inet_ntoa(peer->mask), sizeof(nm));
		...
		if (s) {
			...
			tris_cli(fd, FORMAT, name,
				...
				tris_test_flag(peer, IAX_DYNAMIC) ? "(D)" : "(S)",
				nm,
				ntohs(peer->addr.sin_port),
				tris_test_flag(peer, IAX_TRUNK) ? "(T)" : "   ",
				peer->encmethods ? "(E)" : "   ",
				status,
				term);
		}
	...
	if (!s)
		tris_cli(fd,"%d iax2 peers [%d online, %d offline, %d unmonitored]%s", total_peers, online_peers, offline_peers, unmonitored_peers, term);
...
static char *handle_cli_iax2_show_threads(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct iax2_thread *thread = NULL;
	time_t t;
	...
	tris_cli(a->fd, "IAX2 Thread Information\n");
	time(&t);
	tris_cli(a->fd, "Idle Threads:\n");
	TRIS_LIST_LOCK(&idle_list);
	TRIS_LIST_TRAVERSE(&idle_list, thread, list) {
#ifdef DEBUG_SCHED_MULTITHREAD
		tris_cli(a->fd, "Thread %d: state=%d, update=%d, actions=%d, func='%s'\n", 
			thread->threadnum, thread->iostate, (int)(t - thread->checktime), thread->actions, thread->curfunc);
#else
		tris_cli(a->fd, "Thread %d: state=%d, update=%d, actions=%d\n", 
			thread->threadnum, thread->iostate, (int)(t - thread->checktime), thread->actions);
#endif
		threadcount++;
	}
	TRIS_LIST_UNLOCK(&idle_list);
	tris_cli(a->fd, "Active Threads:\n");
	TRIS_LIST_LOCK(&active_list);
	TRIS_LIST_TRAVERSE(&active_list, thread, list) {
		if (thread->type == IAX_THREAD_TYPE_DYNAMIC)
			type = 'D';
		else
			type = 'P';
#ifdef DEBUG_SCHED_MULTITHREAD
		tris_cli(a->fd, "Thread %c%d: state=%d, update=%d, actions=%d, func='%s'\n", 
			type, thread->threadnum, thread->iostate, (int)(t - thread->checktime), thread->actions, thread->curfunc);
#else
		tris_cli(a->fd, "Thread %c%d: state=%d, update=%d, actions=%d\n", 
			type, thread->threadnum, thread->iostate, (int)(t - thread->checktime), thread->actions);
#endif
		threadcount++;
	}
	TRIS_LIST_UNLOCK(&active_list);
	tris_cli(a->fd, "Dynamic Threads:\n");
	TRIS_LIST_LOCK(&dynamic_list);
	TRIS_LIST_TRAVERSE(&dynamic_list, thread, list) {
#ifdef DEBUG_SCHED_MULTITHREAD
		tris_cli(a->fd, "Thread %d: state=%d, update=%d, actions=%d, func='%s'\n",
			thread->threadnum, thread->iostate, (int)(t - thread->checktime), thread->actions, thread->curfunc);
#else
	...
	TRIS_LIST_UNLOCK(&dynamic_list);
	tris_cli(a->fd, "%d of %d threads accounted for with %d dynamic threads\n", threadcount, iaxthreadcount, dynamiccount);
	return CLI_SUCCESS;
}

static char *handle_cli_iax2_unregister(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct iax2_peer *p;
	...
	if (p) {
		if (p->expire > 0) {
			...
			if (peer) {
				...
				tris_cli(a->fd, "Peer %s unregistered\n", a->argv[2]);
			} else {
				tris_cli(a->fd, "Peer %s not found\n", a->argv[2]);
			}
		} else {
...
static char *handle_cli_iax2_show_peers(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
{
	tris_cli_netstats(s, -1, 0);
	astman_append(s, "\r\n");
	return RESULT_SUCCESS;
}

static char *handle_cli_iax2_show_firmware(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct iax_firmware *cur = NULL;
	...
	tris_cli(a->fd, "%-15.15s  %-15.15s %-15.15s\n", "Device", "Version", "Size");
	TRIS_LIST_LOCK(&firmwares);
	TRIS_LIST_TRAVERSE(&firmwares, cur, list) {
		if ((a->argc == 3) || (!strcasecmp(a->argv[3], (char *) cur->fwh->devname)))  {
			tris_cli(a->fd, "%-15.15s  %-15d %-15d\n", cur->fwh->devname, 
...
static char *handle_cli_iax2_show_registry(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT  "%-20.20s  %-6.6s  %-10.10s  %-20.20s %8d  %s\n"
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	tris_cli(a->fd, FORMAT2, "Host", "dnsmgr", "Username", "Perceived", "Refresh", "State");
	TRIS_LIST_LOCK(&registrations);
	TRIS_LIST_TRAVERSE(&registrations, reg, entry) {
		snprintf(host, sizeof(host), "%s:%d", tris_inet_ntoa(reg->addr.sin_addr), ntohs(reg->addr.sin_port));
		if (reg->us.sin_addr.s_addr) 
			snprintf(perceived, sizeof(perceived), "%s:%d", tris_inet_ntoa(reg->us.sin_addr), ntohs(reg->us.sin_port));
		else
			tris_copy_string(perceived, "<Unregistered>", sizeof(perceived));
		tris_cli(a->fd, FORMAT, host, 
					(reg->dnsmgr) ? "Y" : "N", 
					reg->username, perceived, reg->refresh, regstate2str(reg->regstate));
		counter++;
	}
	TRIS_LIST_UNLOCK(&registrations);
	tris_cli(a->fd, "%d IAX2 registrations.\n", counter);
	return CLI_SUCCESS;
#undef FORMAT
#undef FORMAT2
}
...
static char *handle_cli_iax2_show_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMATB "%-20.20s  %-15.15s  %-10.10s  %5.5d/%5.5d  %5.5d/%5.5d  [Native Bridged to ID=%5.5d]\n"
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	tris_cli(a->fd, FORMAT2, "Channel", "Peer", "Username", "ID (Lo/Rem)", "Seq (Tx/Rx)", "Lag", "Jitter", "JitBuf", "Format", "FirstMsg", "LastMsg");
	for (x = 0; x < ARRAY_LEN(iaxs); x++) {
		tris_mutex_lock(&iaxsl[x]);
		if (iaxs[x]) {
			int lag, jitter, localdelay;
			jb_info jbinfo;
			if (tris_test_flag(iaxs[x], IAX_USEJITTERBUF)) {
			...
			tris_cli(a->fd, FORMAT,
				...
				tris_inet_ntoa(iaxs[x]->addr.sin_addr),
				...
				jitter,
				localdelay,
				tris_getformatname(iaxs[x]->voiceformat),
	...
	tris_cli(a->fd, "%d active IAX channel%s\n", numchans, (numchans != 1) ? "s" : "");
	return CLI_SUCCESS;
#undef FORMAT
#undef FORMAT2
#undef FORMATB
	...
	for (x = 0; x < ARRAY_LEN(iaxs); x++) {
		...
		if (iaxs[x]) {
			...
			if (s)
				...
				tris_cli(fd, limit_fmt ? ACN_FORMAT1 : ACN_FORMAT2,
					iaxs[x]->owner ? iaxs[x]->owner->name : "(None)",
					iaxs[x]->pingtime,
					localjitter,
					localdelay,
...
static char *handle_cli_iax2_show_netstats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int numchans = 0;
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	tris_cli(a->fd, "                           -------- LOCAL ---------------------  -------- REMOTE --------------------\n");
	tris_cli(a->fd, "Channel               RTT  Jit  Del  Lost   %%  Drop  OOO  Kpkts  Jit  Del  Lost   %%  Drop  OOO  Kpkts FirstMsg    LastMsg\n");
	numchans = tris_cli_netstats(NULL, a->fd, 1);
	tris_cli(a->fd, "%d active IAX channel%s\n", numchans, (numchans != 1) ? "s" : "");
	return CLI_SUCCESS;
}

static char *handle_cli_iax2_set_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (!strcasecmp(a->argv[3], "peer")) {
		...
		if (!peer) {
			tris_cli(a->fd, "IAX2 peer '%s' does not exist\n", a->argv[e->args-1]);
			return CLI_FAILURE;
		}
		...
		tris_cli(a->fd, "IAX2 Debugging Enabled for IP: %s:%d\n",
			tris_inet_ntoa(debugaddr.sin_addr), ntohs(debugaddr.sin_port));

		ao2_ref(peer, -1);
	} else if (!strncasecmp(a->argv[3], "on", 2)) {
		iaxdebug = 1;
		tris_cli(a->fd, "IAX2 Debugging Enabled\n");
	} else {
		iaxdebug = 0;
		memset(&debugaddr, 0, sizeof(debugaddr));
		tris_cli(a->fd, "IAX2 Debugging Disabled\n");
	}
	return CLI_SUCCESS;
}

static char *handle_cli_iax2_set_debug_trunk(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (!strncasecmp(a->argv[e->args - 1], "on", 2)) {
		iaxtrunkdebug = 1;
		tris_cli(a->fd, "IAX2 Trunk Debugging Enabled\n");
	} else {
		iaxtrunkdebug = 0;
		tris_cli(a->fd, "IAX2 Trunk Debugging Disabled\n");
	}
	return CLI_SUCCESS;
}

static char *handle_cli_iax2_set_debug_jb(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (!strncasecmp(a->argv[e->args -1], "on", 2)) {
		...
		tris_cli(a->fd, "IAX2 Jitterbuffer Debugging Enabled\n");
	} else {
		jb_setoutput(jb_error_output, jb_warning_output, NULL);
		tris_cli(a->fd, "IAX2 Jitterbuffer Debugging Disabled\n");
	}
	return CLI_SUCCESS;
}
	...
	/* If there's an outstanding error, return failure now */
		if (!iaxs[callno]->error) {
			if (tris_test_flag(iaxs[callno], IAX_ALREADYGONE))
				...
				res = 0;
			else if ((f->frametype == TRIS_FRAME_VOICE) && tris_test_flag(iaxs[callno], IAX_QUELCH))
				res = 0;
			else if (!tris_test_flag(&iaxs[callno]->state, IAX_STATE_STARTED))
				res = 0;
			else
			/* Simple, just queue for transmission */
				res = iax2_send(iaxs[callno], f, 0, -1, 0, 0, 0);
		} else {
			tris_debug(1, "Write error: %s\n", strerror(errno));
		}
	}
...
static int check_access(int callno, struct sockaddr_in *sin, struct iax_ies *ies)
{
	...
	int bestscore = 0;
	int gotcapability = 0;
	struct tris_variable *v = NULL, *tmpvar = NULL;
	struct ao2_iterator i;

	if (!iaxs[callno])
		return res;
	if (ies->called_number)
		tris_string_field_set(iaxs[callno], exten, ies->called_number);
	if (ies->calling_number) {
		if (tris_test_flag(&globalflags, IAX_SHRINKCALLERID)) { 
			tris_shrink_phone_number(ies->calling_number);
		}
		tris_string_field_set(iaxs[callno], cid_num, ies->calling_number);
	}
	if (ies->calling_name)
		tris_string_field_set(iaxs[callno], cid_name, ies->calling_name);
	if (ies->calling_ani)
		tris_string_field_set(iaxs[callno], ani, ies->calling_ani);
	if (ies->dnid)
		tris_string_field_set(iaxs[callno], dnid, ies->dnid);
	if (ies->rdnis)
		tris_string_field_set(iaxs[callno], rdnis, ies->rdnis);
	if (ies->called_context)
		tris_string_field_set(iaxs[callno], context, ies->called_context);
	if (ies->language)
		tris_string_field_set(iaxs[callno], language, ies->language);
	if (ies->username)
		tris_string_field_set(iaxs[callno], username, ies->username);
	...
	if(ies->codec_prefs) {
		tris_codec_pref_convert(&iaxs[callno]->rprefs, ies->codec_prefs, 32, 0);
		tris_codec_pref_convert(&iaxs[callno]->prefs, ies->codec_prefs, 32, 0);
	}

	if (!gotcapability) 
		iaxs[callno]->peercapability = iaxs[callno]->peerformat;
	if (version > IAX_PROTO_VERSION) {
		tris_log(LOG_WARNING, "Peer '%s' has too new a protocol version (%d) for me\n", 
			tris_inet_ntoa(sin->sin_addr), version);
		return res;
	}
	/* Search the userlist for a compatible entry, and fill in the rest */
	i = ao2_iterator_init(users, 0);
	while ((user = ao2_iterator_next(&i))) {
		if ((tris_strlen_zero(iaxs[callno]->username) ||				/* No username specified */
			!strcmp(iaxs[callno]->username, user->name))	/* Or this username specified */
			&& tris_apply_ha(user->ha, sin) 	/* Access is permitted from this IP */
			&& (tris_strlen_zero(iaxs[callno]->context) ||			/* No context specified */
			     apply_context(user->contexts, iaxs[callno]->context))) {			/* Context is permitted */
			if (!tris_strlen_zero(iaxs[callno]->username)) {
				/* Exact match, stop right now. */
				if (best)
					user_unref(best);
				best = user;
				break;
			} else if (tris_strlen_zero(user->secret) && tris_strlen_zero(user->dbsecret) && tris_strlen_zero(user->inkeys)) {
	...
	ao2_iterator_destroy(&i);
	user = best;
	if (!user && !tris_strlen_zero(iaxs[callno]->username)) {
		user = realtime_user(iaxs[callno]->username, sin);
		if (user && !tris_strlen_zero(iaxs[callno]->context) &&			/* No context specified */
		    !apply_context(user->contexts, iaxs[callno]->context)) {		/* Context is permitted */
	...
	if (user) {
		...
		if (user->maxauthreq > 0)
			tris_set_flag(iaxs[callno], IAX_MAXAUTHREQ);
		iaxs[callno]->prefs = user->prefs;
		tris_copy_flags(iaxs[callno], user, IAX_CODEC_USER_FIRST | IAX_IMMEDIATE | IAX_CODEC_NOPREFS | IAX_CODEC_NOCAP | IAX_FORCE_ENCRYPT);
		iaxs[callno]->encmethods = user->encmethods;
		/* Store the requested username if not specified */
		if (tris_strlen_zero(iaxs[callno]->username))
			tris_string_field_set(iaxs[callno], username, user->name);
		/* Store whether this is a trunked call, too, of course, and move if appropriate */
		tris_copy_flags(iaxs[callno], user, IAX_TRUNK);
		iaxs[callno]->capability = user->capability;
		/* And use the default context */
		if (tris_strlen_zero(iaxs[callno]->context)) {
			if (user->contexts)
				tris_string_field_set(iaxs[callno], context, user->contexts->context);
			else
				tris_string_field_set(iaxs[callno], context, DEFAULT_CONTEXT);
		}
		/* And any input keys */
		tris_string_field_set(iaxs[callno], inkeys, user->inkeys);
		...
		if (tris_test_flag(user, IAX_HASCALLERID)) {
			iaxs[callno]->calling_tns = 0;
			iaxs[callno]->calling_ton = 0;
			tris_string_field_set(iaxs[callno], cid_num, user->cid_num);
			tris_string_field_set(iaxs[callno], cid_name, user->cid_name);
			tris_string_field_set(iaxs[callno], ani, user->cid_num);
			iaxs[callno]->calling_pres = TRIS_PRES_ALLOWED_USER_NUMBER_PASSED_SCREEN;
		} else if (tris_strlen_zero(iaxs[callno]->cid_num) && tris_strlen_zero(iaxs[callno]->cid_name)) {
			iaxs[callno]->calling_pres = TRIS_PRES_NUMBER_NOT_AVAILABLE;
		} /* else user is allowed to set their own CID settings */
		if (!tris_strlen_zero(user->accountcode))
			tris_string_field_set(iaxs[callno], accountcode, user->accountcode);
		if (!tris_strlen_zero(user->mohinterpret))
			tris_string_field_set(iaxs[callno], mohinterpret, user->mohinterpret);
		if (!tris_strlen_zero(user->mohsuggest))
			tris_string_field_set(iaxs[callno], mohsuggest, user->mohsuggest);
		if (!tris_strlen_zero(user->parkinglot))
			tris_string_field_set(iaxs[callno], parkinglot, user->parkinglot);
		if (user->amaflags)
			iaxs[callno]->amaflags = user->amaflags;
		if (!tris_strlen_zero(user->language))
			tris_string_field_set(iaxs[callno], language, user->language);
		tris_copy_flags(iaxs[callno], user, IAX_NOTRANSFER | IAX_TRANSFERMEDIA | IAX_USEJITTERBUF | IAX_FORCEJITTERBUF);	
		/* Keep this check last */
		if (!tris_strlen_zero(user->dbsecret)) {
			char *family, *key=NULL;
			char buf[80];
			family = tris_strdupa(user->dbsecret);
			key = strchr(family, '/');
			if (key) {
				*key = '\0';
				key++;
			}
			if (!key || tris_db_get(family, key, buf, sizeof(buf)))
				tris_log(LOG_WARNING, "Unable to retrieve database password for family/key '%s'!\n", user->dbsecret);
			else
				tris_string_field_set(iaxs[callno], secret, buf);
		} else
			tris_string_field_set(iaxs[callno], secret, user->secret);
		res = 0;
		user = user_unref(user);
	} else {
		...
		tris_string_field_set(iaxs[callno], secret, "badsecret");
	...
	tris_set2_flag(iaxs[callno], iax2_getpeertrunk(*sin), IAX_TRUNK);	
	return res;
}
...
static int raw_hangup(struct sockaddr_in *sin, unsigned short src, unsigned short dst, int sockfd)
{
	...
	fh.oseqno = 0;
	fh.iseqno = 0;
	fh.type = TRIS_FRAME_IAX;
	fh.csub = compress_subclass(IAX_COMMAND_INVAL);
	iax_outputframe(NULL, &fh, 0, sin, 0);
#if 0
	if (option_debug)
#endif	
		tris_debug(1, "Raw Hangup %s:%d, src=%d, dst=%d\n",
			tris_inet_ntoa(sin->sin_addr), ntohs(sin->sin_port), src, dst);
...
static int authenticate_request(int call_num)
{
	...
	if (tris_test_flag(p, IAX_MAXAUTHREQ)) {
	...
	if (authreq_restrict) {
		...
		iax_ie_append_byte(&ied, IAX_IE_CAUSECODE, TRIS_CAUSE_CALL_REJECTED);
		send_command_final(p, TRIS_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied.buf, ied.pos, -1);
		return 0;
	}

	iax_ie_append_short(&ied, IAX_IE_AUTHMETHODS, p->authmethods);
	if (p->authmethods & (IAX_AUTH_MD5 | IAX_AUTH_RSA)) {
		snprintf(challenge, sizeof(challenge), "%d", (int)tris_random());
		tris_string_field_set(p, challenge, challenge);
		/* snprintf(p->challenge, sizeof(p->challenge), "%d", (int)tris_random()); */
	...
	res = send_command(p, TRIS_FRAME_IAX, IAX_COMMAND_AUTHREQ, 0, ied.buf, ied.pos, -1);

	if (p->encmethods)
		tris_set_flag(p, IAX_ENCRYPTED);
...
static int authenticate_verify(struct chan_iax2_pvt *p, struct iax_ies *ies)
{
	...
	if (user) {
		if (tris_test_flag(p, IAX_MAXAUTHREQ)) {
			tris_atomic_fetchadd_int(&user->curauthreq, -1);
			tris_clear_flag(p, IAX_MAXAUTHREQ);
		}
		tris_string_field_set(p, host, user->name);
		user = user_unref(user);
	}
	if (tris_test_flag(p, IAX_FORCE_ENCRYPT) && !p->encmethods) { 
		tris_log(LOG_NOTICE, "Call Terminated, Incomming call is unencrypted while force encrypt is enabled.");
		return res;
	}
	if (!tris_test_flag(&p->state, IAX_STATE_AUTHENTICATED))
		return res;
	if (ies->password)
		tris_copy_string(secret, ies->password, sizeof(secret));
	if (ies->md5_result)
		tris_copy_string(md5secret, ies->md5_result, sizeof(md5secret));
	if (ies->rsa_result)
		tris_copy_string(rsasecret, ies->rsa_result, sizeof(rsasecret));
	if ((p->authmethods & IAX_AUTH_RSA) && !tris_strlen_zero(rsasecret) && !tris_strlen_zero(p->inkeys)) {
		struct tris_key *key;
		char *keyn;
		char tmpkey[256];
		char *stringp=NULL;
		tris_copy_string(tmpkey, p->inkeys, sizeof(tmpkey));
		stringp=tmpkey;
		keyn = strsep(&stringp, ":");
		while(keyn) {
			key = tris_key_get(keyn, TRIS_KEY_PUBLIC);
			if (key && !tris_check_signature(key, p->challenge, rsasecret)) {
				res = 0;
				break;
			} else if (!key)
				tris_log(LOG_WARNING, "requested inkey '%s' for RSA authentication does not exist\n", keyn);
			keyn = strsep(&stringp, ":");
		}
...
static int register_verify(int callno, struct sockaddr_in *sin, struct iax_ies *ies)
{
	...
	tris_clear_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED);
	/* iaxs[callno]->peer[0] = '\0'; not necc. any more-- stringfield is pre-inited to null string */
	if (ies->username)
		tris_copy_string(peer, ies->username, sizeof(peer));
	if (ies->password)
		tris_copy_string(secret, ies->password, sizeof(secret));
	if (ies->md5_result)
		tris_copy_string(md5secret, ies->md5_result, sizeof(md5secret));
	...
	if (tris_strlen_zero(peer)) {
		tris_log(LOG_NOTICE, "Empty registration from %s\n", tris_inet_ntoa(sin->sin_addr));
		return -1;
	}
	...
	if (!p || !iaxs[callno]) {
		if (iaxs[callno]) {
			...
			if (tris_strlen_zero(iaxs[callno]->challenge) &&
				!(!tris_strlen_zero(secret) && plaintext)) {
				/* by setting res to 0, an REGAUTH will be sent */
				res = 0;
			}
		}
		if (authdebug && !p)
			tris_log(LOG_NOTICE, "No registration for peer '%s' (from %s)\n", peer, tris_inet_ntoa(sin->sin_addr));
		goto return_unref;
	}

	if (!tris_test_flag(p, IAX_DYNAMIC)) {
		if (authdebug)
			tris_log(LOG_NOTICE, "Peer '%s' is not dynamic (from %s)\n", peer, tris_inet_ntoa(sin->sin_addr));
		goto return_unref;
	}

	if (!tris_apply_ha(p->ha, sin)) {
		if (authdebug)
			tris_log(LOG_NOTICE, "Host %s denied access to register peer '%s'\n", tris_inet_ntoa(sin->sin_addr), p->name);
		goto return_unref;
	}
	tris_string_field_set(iaxs[callno], secret, p->secret);
	tris_string_field_set(iaxs[callno], inkeys, p->inkeys);
	/* Check secret against what we have on file */
	if (!tris_strlen_zero(rsasecret) && (p->authmethods & IAX_AUTH_RSA) && !tris_strlen_zero(iaxs[callno]->challenge)) {
		if (!tris_strlen_zero(p->inkeys)) {
			char tmpkeys[256];
			char *stringp=NULL;
			tris_copy_string(tmpkeys, p->inkeys, sizeof(tmpkeys));
			stringp=tmpkeys;
			keyn = strsep(&stringp, ":");
			while(keyn) {
				key = tris_key_get(keyn, TRIS_KEY_PUBLIC);
				if (key && !tris_check_signature(key, iaxs[callno]->challenge, rsasecret)) {
					tris_set_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED);
					break;
				} else if (!key)
					tris_log(LOG_WARNING, "requested inkey '%s' does not exist\n", keyn);
				keyn = strsep(&stringp, ":");
			}
			if (!keyn) {
				if (authdebug)
					tris_log(LOG_NOTICE, "Host %s failed RSA authentication with inkeys '%s'\n", peer, p->inkeys);
				goto return_unref;
			}
		} else {
			if (authdebug)
				tris_log(LOG_NOTICE, "Host '%s' trying to do RSA authentication, but we have no inkeys\n", peer);
			goto return_unref;
		}
	} else if (!tris_strlen_zero(md5secret) && (p->authmethods & IAX_AUTH_MD5) && !tris_strlen_zero(iaxs[callno]->challenge)) {
		struct MD5Context md5;
		unsigned char digest[16];
		char *tmppw, *stringp;
		...
		if (tmppw) {
			...
			if (authdebug)
				tris_log(LOG_NOTICE, "Host %s failed MD5 authentication for '%s' (%s != %s)\n", tris_inet_ntoa(sin->sin_addr), p->name, requeststr, md5secret);
			goto return_unref;
		}
	} else if (!tris_strlen_zero(secret) && (p->authmethods & IAX_AUTH_PLAINTEXT)) {
		/* They've provided a plain text password and we support that */
		if (strcmp(secret, p->secret)) {
			if (authdebug)
				tris_log(LOG_NOTICE, "Host %s did not provide proper plaintext password for '%s'\n", tris_inet_ntoa(sin->sin_addr), p->name);
			goto return_unref;
		} else
			tris_set_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED);
	} else if (!tris_strlen_zero(iaxs[callno]->challenge) && tris_strlen_zero(md5secret) && tris_strlen_zero(rsasecret)) {
		/* if challenge has been sent, but no challenge response if given, reject. */
		goto return_unref;
	}
	tris_devstate_changed(TRIS_DEVICE_UNKNOWN, "IAX2/%s", p->name); /* Activate notification */
...
return_unref:
	if (iaxs[callno]) {
		tris_string_field_set(iaxs[callno], peer, peer);
...
static int authenticate(const char *challenge, const char *secret, const char *keyn, int authmethods, struct iax_ie_data *ied, struct sockaddr_in *sin, struct chan_iax2_pvt *pvt)
{
	int res = -1;
	int x;
	if (!tris_strlen_zero(keyn)) {
		if (!(authmethods & IAX_AUTH_RSA)) {
			if (tris_strlen_zero(secret)) 
				tris_log(LOG_NOTICE, "Asked to authenticate to %s with an RSA key, but they don't allow RSA authentication\n", tris_inet_ntoa(sin->sin_addr));
		} else if (tris_strlen_zero(challenge)) {
			tris_log(LOG_NOTICE, "No challenge provided for RSA authentication to %s\n", tris_inet_ntoa(sin->sin_addr));
		} else {
			char sig[256];
			struct tris_key *key;
			key = tris_key_get(keyn, TRIS_KEY_PRIVATE);
			if (!key) {
				tris_log(LOG_NOTICE, "Unable to find private key '%s'\n", keyn);
			} else {
				if (tris_sign(key, (char*)challenge, sig)) {
					tris_log(LOG_NOTICE, "Unable to sign challenge with key\n");
					res = -1;
				} else {
	...
	/* Fall back */
	if (res && !tris_strlen_zero(secret)) {
		if ((authmethods & IAX_AUTH_MD5) && !tris_strlen_zero(challenge)) {
			struct MD5Context md5;
			unsigned char digest[16];
			char digres[128];
			MD5Init(&md5);
			...
			tris_log(LOG_NOTICE, "No way to send secret to peer '%s' (their methods: %d)\n", tris_inet_ntoa(sin->sin_addr), authmethods);
	}
	return res;
}
...
static int authenticate_reply(struct chan_iax2_pvt *p, struct sockaddr_in *sin, struct iax_ies *ies, const char *override, const char *okey)
{
	...
	if (ies->username)
		tris_string_field_set(p, username, ies->username);
	if (ies->challenge)
		tris_string_field_set(p, challenge, ies->challenge);
	if (ies->authmethods)
	...
	if (!tris_strlen_zero(override) || !tris_strlen_zero(okey)) {
		/* Normal password authentication */
		res = authenticate(p->challenge, override, okey, authmethods, &ied, sin, p);
	} else {
		struct ao2_iterator i = ao2_iterator_init(peers, 0);
		while ((peer = ao2_iterator_next(&i))) {
			if ((tris_strlen_zero(p->peer) || !strcmp(p->peer, peer->name)) 
			    /* No peer specified at our end, or this is the peer */
			    && (tris_strlen_zero(peer->username) || (!strcmp(peer->username, p->username)))
		...
		if (!peer) {
			...
			const char *peer_name = tris_strdupa(p->peer);
			tris_mutex_unlock(&iaxsl[callno]);
	...
	if (ies->encmethods) {
		tris_set_flag(p, IAX_ENCRYPTED | IAX_KEYPOPULATED);
	} else if (tris_test_flag(iaxs[callno], IAX_FORCE_ENCRYPT)) {
		tris_log(LOG_NOTICE, "Call initiated without encryption while forceencryption=yes option is set");
	...
	if (!res) {
		...
		if (variablestore && varlist && p->owner) {
			...
			if (prev)
				tris_free(prev);
			ies->vars = NULL;
			tris_channel_datastore_add(p->owner, variablestore);
		} else {
			if (p->owner)
				tris_log(LOG_ERROR, "Memory allocation error while processing IAX2 variables\n");
			if (variablestore)
	...
	if (!res)
		res = send_command(p, TRIS_FRAME_IAX, IAX_COMMAND_AUTHREP, 0, ied.buf, ied.pos, -1);
	return res;
}
...
static int try_transfer(struct chan_iax2_pvt *pvt, struct iax_ies *ies)
{
	...
	if (!newcall || !new.sin_addr.s_addr || !new.sin_port) {
		tris_log(LOG_WARNING, "Invalid transfer request\n");
		return -1;
	}
	...
	send_command_transfer(pvt, TRIS_FRAME_IAX, IAX_COMMAND_TXCNT, 0, ied.buf, ied.pos);
	return 0;
}
...
static int complete_dpreply(struct chan_iax2_pvt *pvt, struct iax_ies *ies)
{
	...
	if (ies->called_number)
		tris_copy_string(exten, ies->called_number, sizeof(exten));
	...
	TRIS_LIST_LOCK(&dpcache);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&dpcache, dp, peer_list) {
		if (strcmp(dp->exten, exten))
			continue;
		TRIS_LIST_REMOVE_CURRENT(peer_list);
		dp->callno = 0;
...
static int iax2_ack_registry(struct iax_ies *ies, struct sockaddr_in *sin, int callno)
{
	...
	reg = iaxs[callno]->reg;
	if (!reg) {
		tris_log(LOG_WARNING, "Registry acknowledge on unknown registry '%s'\n", peer);
		return -1;
	}
	memcpy(&oldus, &reg->us, sizeof(oldus));
	oldmsgs = reg->messages;
	if (inaddrcmp(&reg->addr, sin)) {
		tris_log(LOG_WARNING, "Received unsolicited registry ack from '%s'\n", tris_inet_ntoa(sin->sin_addr));
		return -1;
	}
	...
	*/
	...
	if (inaddrcmp(&oldus, &reg->us) || (reg->messages != oldmsgs)) {
			if (reg->messages > 255)
				...
				tris_copy_string(msgstatus, " with 1 new message waiting\n", sizeof(msgstatus));
			else
				tris_copy_string(msgstatus, " with no messages waiting\n", sizeof(msgstatus));
			snprintf(ourip, sizeof(ourip), "%s:%d", tris_inet_ntoa(reg->us.sin_addr), ntohs(reg->us.sin_port));
		tris_verb(3, "Registered IAX2 to '%s', who sees us as %s%s\n", tris_inet_ntoa(sin->sin_addr), ourip, msgstatus);
	...
	const char *secret, const char *porta)
{
	...
	if (tris_dnsmgr_lookup(hostname, &reg->addr, &reg->dnsmgr, srvlookup ? "_iax._udp" : NULL) < 0) {
		tris_free(reg);
		return -1;
	}

	tris_copy_string(reg->username, username, sizeof(reg->username));

	if (secret)
		tris_copy_string(reg->secret, secret, sizeof(reg->secret));
	...
	TRIS_LIST_LOCK(&registrations);
	TRIS_LIST_INSERT_HEAD(&registrations, reg, entry);
	TRIS_LIST_UNLOCK(&registrations);
...
static int iax2_register(const char *value, int lineno)
{
	...
	tris_copy_string(copy, value, sizeof(copy));
	stringp = copy;
	username = strsep(&stringp, "@");
	hostname = strsep(&stringp, "@");

	if (!hostname) {
		tris_log(LOG_WARNING, "Format for registration is user[:secret]@host[:port] at line %d\n", lineno);
		return -1;
	}
...
static void register_peer_exten(struct iax2_peer *peer, int onoff)
{
	...
	if (!tris_strlen_zero(regcontext)) {
		...
		while((ext = strsep(&stringp, "&"))) {
			if (onoff) {
				if (!tris_exists_extension(NULL, regcontext, ext, 1, NULL))
					tris_add_extension(regcontext, 1, ext, 1, NULL, NULL,
							  "Noop", tris_strdup(peer->name), tris_free_ptr, "IAX2");
			} else
				tris_context_remove_extension(regcontext, ext, 1, NULL);
		}
	}
}
static void prune_peers(void);
...
static void unlink_peer(struct iax2_peer *peer)
{
	...
	if (peer->pokeexpire > -1) {
		if (!tris_sched_thread_del(sched, peer->pokeexpire)) {
			peer->pokeexpire = -1;
			peer_unref(peer);
		}
	}
...
static void __expire_registry(const void *data)
{
	...
	tris_debug(1, "Expiring registration for peer '%s'\n", peer->name);
	if (tris_test_flag((&globalflags), IAX_RTUPDATE) && (tris_test_flag(peer, IAX_TEMPONLY|IAX_RTCACHEFRIENDS)))
		realtime_update_peer(peer->name, &peer->addr, 0);
	...
	if (!tris_test_flag(peer, IAX_TEMPONLY))
		tris_db_del("IAX/Registry", peer->name);
	register_peer_exten(peer, 0);
	tris_devstate_changed(TRIS_DEVICE_UNAVAILABLE, "IAX2/%s", peer->name); /* Activate notification */
	if (iax2_regfunk)
...
static void reg_source_db(struct iax2_peer *p)
{
	...
	struct in_addr in;
	char *c, *d;
	if (!tris_test_flag(p, IAX_TEMPONLY) && (!tris_db_get("IAX/Registry", p->name, data, sizeof(data)))) {
		c = strchr(data, ':');
		if (c) {
			*c = '\0';
			c++;
			if (inet_aton(data, &in)) {
				...
				if (d) {
					*d = '\0';
					d++;
					tris_verb(3, "Seeding '%s' at %s:%d for %d\n", p->name,
						tris_inet_ntoa(in), atoi(c), atoi(d));
					iax2_poke_peer(p, 0);
					p->expiry = atoi(d);
					...
					p->addr.sin_port = htons(atoi(c));
 					if (p->expire > -1) {
 						if (!tris_sched_thread_del(sched, p->expire)) {
 							p->expire = -1;
 							peer_unref(p);
 						}
 					}
...
static int update_registry(struct sockaddr_in *sin, int callno, char *devtype, int fd, unsigned short refresh)
{
	...
	peer_name = tris_strdupa(iaxs[callno]->peer);
	...
	if (!(p = find_peer(peer_name, 1))) {
		tris_mutex_lock(&iaxsl[callno]);
		tris_log(LOG_WARNING, "No such peer '%s'\n", peer_name);
		return -1;
	}
	tris_mutex_lock(&iaxsl[callno]);
	if (!iaxs[callno])
	...
	if (tris_test_flag((&globalflags), IAX_RTUPDATE) && (tris_test_flag(p, IAX_TEMPONLY|IAX_RTCACHEFRIENDS))) {
	...
	if (inaddrcmp(&p->addr, sin)) {
		...
		snprintf(data, sizeof(data), "%s:%d:%d", tris_inet_ntoa(sin->sin_addr), ntohs(sin->sin_port), p->expiry);
		if (!tris_test_flag(p, IAX_TEMPONLY) && sin->sin_addr.s_addr) {
			tris_db_put("IAX/Registry", p->name, data);
			tris_verb(3, "Registered IAX2 '%s' (%s) at %s:%d\n", p->name,
					    tris_test_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED) ? "AUTHENTICATED" : "UNAUTHENTICATED", tris_inet_ntoa(sin->sin_addr), ntohs(sin->sin_port));
			manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: IAX2\r\nPeer: IAX2/%s\r\nPeerStatus: Registered\r\n", p->name);
			register_peer_exten(p, 1);
			tris_devstate_changed(TRIS_DEVICE_UNKNOWN, "IAX2/%s", p->name); /* Activate notification */
		} else if (!tris_test_flag(p, IAX_TEMPONLY)) {
			tris_verb(3, "Unregistered IAX2 '%s' (%s)\n", p->name,
					    tris_test_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED) ? "AUTHENTICATED" : "UNAUTHENTICATED");
			manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: IAX2\r\nPeer: IAX2/%s\r\nPeerStatus: Unregistered\r\n", p->name);
			register_peer_exten(p, 0);
			tris_db_del("IAX/Registry", p->name);
			tris_devstate_changed(TRIS_DEVICE_UNAVAILABLE, "IAX2/%s", p->name); /* Activate notification */
		}
		/* Update the host */
	...
	if (refresh > max_reg_expire) {
		tris_log(LOG_NOTICE, "Restricting registration for peer '%s' to %d seconds (requested %d)\n",
	...
	if (sin->sin_addr.s_addr) {
		...
		if (!tris_strlen_zero(p->mailbox)) {
			struct tris_event *event;
			int new, old;
			char *mailbox, *context;

			context = mailbox = tris_strdupa(p->mailbox);
			strsep(&context, "@");
			...
			if (event) {
				...
				tris_app_inboxcount(p->mailbox, &new, &old);
			}
		...
		if (tris_test_flag(p, IAX_HASCALLERID)) {
...
return_unref:
	...
	return res ? res : send_command_final(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_REGACK, 0, ied.buf, ied.pos, -1);
}
...
static int registry_authrequest(int callno)
{
	...
	if (sentauthmethod & (IAX_AUTH_RSA | IAX_AUTH_MD5)) {
		/* Build the challenge */
		snprintf(challenge, sizeof(challenge), "%d", (int)tris_random());
		tris_string_field_set(iaxs[callno], challenge, challenge);
...
return_unref:
	...
	return iaxs[callno] ? send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_REGAUTH, 0, ied.buf, ied.pos, -1) : -1;
}
...
static int registry_rerequest(struct iax_ies *ies, int callno, struct sockaddr_in *sin)
{
	...
	if (ies->username)
		tris_copy_string(peer, ies->username, sizeof(peer));
	if (ies->challenge)
		tris_copy_string(challenge, ies->challenge, sizeof(challenge));
	memset(&ied, 0, sizeof(ied));
	reg = iaxs[callno]->reg;
	if (reg) {
			if (inaddrcmp(&reg->addr, sin)) {
				tris_log(LOG_WARNING, "Received unsolicited registry authenticate request from '%s'\n", tris_inet_ntoa(sin->sin_addr));
				return -1;
			}
			if (tris_strlen_zero(reg->secret)) {
				tris_log(LOG_NOTICE, "No secret associated with peer '%s'\n", reg->username);
			...
			if (reg->secret[0] == '[') {
				char tmpkey[256];
				tris_copy_string(tmpkey, reg->secret + 1, sizeof(tmpkey));
			...
			if (!res) {
				...
				add_empty_calltoken_ie(iaxs[callno], &ied); /* this _MUST_ be the last ie added */
				return send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_REGREQ, 0, ied.buf, ied.pos, -1);
			} else
				return -1;
			tris_log(LOG_WARNING, "Registry acknowledge on unknown registery '%s'\n", peer);
	} else	
		tris_log(LOG_NOTICE, "Can't reregister without a reg\n");
	return -1;
}
...
static void __auth_reject(const void *nothing)
{
	...
	if (iaxs[callno]) {
		...
		if (iaxs[callno]->authfail == IAX_COMMAND_REGREJ) {
			...
			iax_ie_append_byte(&ied, IAX_IE_CAUSECODE, TRIS_CAUSE_FACILITY_REJECTED);
		} else if (iaxs[callno]->authfail == IAX_COMMAND_REJECT) {
			iax_ie_append_str(&ied, IAX_IE_CAUSE, "No authority found");
			iax_ie_append_byte(&ied, IAX_IE_CAUSECODE, TRIS_CAUSE_FACILITY_NOT_SUBSCRIBED);
		}
		send_command_final(iaxs[callno], TRIS_FRAME_IAX, iaxs[callno]->authfail, 0, ied.buf, ied.pos, -1);
	}
	tris_mutex_unlock(&iaxsl[callno]);
}
...
static void __auto_hangup(const void *nothing)
{
	...
	if (iaxs[callno]) {
		...
		iax_ie_append_byte(&ied, IAX_IE_CAUSECODE, TRIS_CAUSE_NO_USER_RESPONSE);
		send_command_final(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_HANGUP, 0, ied.buf, ied.pos, -1);
	}
	tris_mutex_unlock(&iaxsl[callno]);
}
...
static void iax2_dprequest(struct iax2_dpcache *dp, int callno)
{
	...
	send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_DPREQ, 0, ied.buf, ied.pos, -1);
	dp->flags |= CACHE_FLAG_TRANSMITTED;
}

static int iax2_vnak(int callno)
{
	return send_command_immediate(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_VNAK, 0, NULL, 0, iaxs[callno]->iseqno);
}
...
static void vnak_retransmit(int callno, int last)
{
	...
	TRIS_LIST_LOCK(&frame_queue);
	TRIS_LIST_TRAVERSE(&frame_queue, f, list) {
...
static int send_trunk(struct iax2_trunk_peer *tpeer, struct timeval *now)
{
	int res = 0;
	struct iax_frame *fr;
	struct tris_iax2_meta_hdr *meta;
	struct tris_iax2_meta_trunk_hdr *mth;
	int calls = 0;
	
	/* Point to frame */
	fr = (struct iax_frame *)tpeer->trunkdata;
	/* Point to meta data */
	meta = (struct tris_iax2_meta_hdr *)fr->afdata;
	mth = (struct tris_iax2_meta_trunk_hdr *)meta->data;
	if (tpeer->trunkdatalen) {
		...
		/* And the rest of the tris_iax2 header */
		fr->direction = DIRECTION_OUTGRESS;
		fr->retrans = -1;
		fr->transfer = 0;
		/* Any appropriate call will do */
		fr->data = fr->afdata;
		fr->datalen = tpeer->trunkdatalen + sizeof(struct tris_iax2_meta_hdr) + sizeof(struct tris_iax2_meta_trunk_hdr);
		res = transmit_trunk(fr, &tpeer->addr, tpeer->sockfd);
		calls = tpeer->calls;
#if 0
		tris_debug(1, "Trunking %d call chunks in %d bytes to %s:%d, ts=%d\n", calls, fr->datalen, tris_inet_ntoa(tpeer->addr.sin_addr), ntohs(tpeer->addr.sin_port), ntohl(mth->ts));
#endif		
...
static int timing_read(int *id, int fd, short events, void *cbdata)
{
	...
	if (iaxtrunkdebug)
		tris_verbose("Beginning trunk processing. Trunk queue ceiling is %d bytes per host\n", trunkmaxsize);
	...
	TRIS_LIST_LOCK(&tpeers);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&tpeers, tpeer, list) {
		processed++;
		res = 0;
		tris_mutex_lock(&tpeer->lock);
		...
		if (!drop && iax2_trunk_expired(tpeer, &now)) {
			...
			trunk_timed++; 
			if (iaxtrunkdebug)
				tris_verbose(" - Trunk peer (%s:%d) has %d call chunk%s in transit, %d bytes backloged and has hit a high water mark of %d bytes\n", tris_inet_ntoa(tpeer->addr.sin_addr), ntohs(tpeer->addr.sin_port), res, (res != 1) ? "s" : "", tpeer->trunkdatalen, tpeer->trunkdataalloc);
		}		
		totalcalls += res;	
		res = 0;
		tris_mutex_unlock(&tpeer->lock);
	...
	if (drop) {
		...
		tris_debug(1, "Dropping unused iax2 trunk peer '%s:%d'\n", tris_inet_ntoa(drop->addr.sin_addr), ntohs(drop->addr.sin_port));
		if (drop->trunkdata) {
			tris_free(drop->trunkdata);
			drop->trunkdata = NULL;
		}
		tris_mutex_unlock(&drop->lock);
		tris_mutex_destroy(&drop->lock);
		tris_free(drop);
		
	}

	if (iaxtrunkdebug)
		tris_verbose("Ending trunk processing with %d peers and %d call chunks processed\n", processed, totalcalls);
	iaxtrunkdebug = 0;
...
struct dpreq_data {
	...
	char callednum[TRIS_MAX_EXTENSION];
	char *callerid;
};
...
static void dp_lookup(int callno, const char *context, const char *callednum, const char *callerid, int skiplock)
{
	...
	memset(&ied1, 0, sizeof(ied1));
	mm = tris_matchmore_extension(NULL, context, callednum, 1, callerid);
	/* Must be started */
	if (!strcmp(callednum, tris_parking_ext()) || tris_exists_extension(NULL, context, callednum, 1, callerid)) {
		dpstatus = IAX_DPSTATUS_EXISTS;
	} else if (tris_canmatch_extension(NULL, context, callednum, 1, callerid)) {
		dpstatus = IAX_DPSTATUS_CANEXIST;
	} else {
		dpstatus = IAX_DPSTATUS_NONEXISTENT;
	}
	if (tris_ignore_pattern(context, callednum))
	...
	if (iaxs[callno]) {
		...
		send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_DPREP, 0, ied1.buf, ied1.pos, -1);
	}
	if (!skiplock)
		tris_mutex_unlock(&iaxsl[callno]);
}
...
static void spawn_dp_lookup(int callno, const char *context, const char *callednum, const char *callerid)
{
	...
	if (!(dpr = tris_calloc(1, sizeof(*dpr))))
		return;

	dpr->callno = callno;
	tris_copy_string(dpr->context, context, sizeof(dpr->context));
	tris_copy_string(dpr->callednum, callednum, sizeof(dpr->callednum));
	if (callerid)
		dpr->callerid = tris_strdup(callerid);
	if (tris_pthread_create_detached(&newthread, NULL, dp_lookup_thread, dpr)) {
		tris_log(LOG_WARNING, "Unable to start lookup thread!\n");
	}
}

struct iax_dual {
	struct tris_channel *chan1;
	struct tris_channel *chan2;
};

static void *iax_park_thread(void *stuff)
{
	struct tris_channel *chan1, *chan2;
	struct iax_dual *d;
	struct tris_frame *f;
	int ext;
	int res;
	...
	if (f)
		tris_frfree(f);
	res = tris_park_call(chan1, chan2, 0, &ext);
	tris_hangup(chan2);
	tris_log(LOG_NOTICE, "Parked on extension '%d'\n", ext);
	return NULL;
}

static int iax_park(struct tris_channel *chan1, struct tris_channel *chan2)
{
	struct iax_dual *d;
	struct tris_channel *chan1m, *chan2m;
	pthread_t th;
	chan1m = tris_channel_alloc(0, TRIS_STATE_DOWN, 0, 0, chan2->accountcode, chan1->exten, chan1->context, chan1->amaflags, "Parking/%s", chan1->name);
	chan2m = tris_channel_alloc(0, TRIS_STATE_DOWN, 0, 0, chan2->accountcode, chan2->exten, chan2->context, chan2->amaflags, "IAXPeer/%s",chan2->name);
	if (chan2m && chan1m) {
		/* Make formats okay */
		chan1m->readformat = chan1->readformat;
		chan1m->writeformat = chan1->writeformat;
		tris_channel_masquerade(chan1m, chan1);
		/* Setup the extensions and such */
		tris_copy_string(chan1m->context, chan1->context, sizeof(chan1m->context));
		tris_copy_string(chan1m->exten, chan1->exten, sizeof(chan1m->exten));
		...
		tris_channel_masquerade(chan2m, chan2);
		/* Setup the extensions and such */
		tris_copy_string(chan2m->context, chan2->context, sizeof(chan2m->context));
		tris_copy_string(chan2m->exten, chan2->exten, sizeof(chan2m->exten));
		chan2m->priority = chan2->priority;
		if (tris_do_masquerade(chan2m)) {
	...
	if ((d = tris_calloc(1, sizeof(*d)))) {
		d->chan1 = chan1m;
		d->chan2 = chan2m;
		if (!tris_pthread_create_detached_background(&th, NULL, iax_park_thread, d)) {
			return 0;
		}
		tris_free(d);
	}
...
static int check_provisioning(struct sockaddr_in *sin, int sockfd, char *si, unsigned int ver)
{
	...
	if (iax_provision_version(&ourver, rsi, 1))
		return 0;
	tris_debug(1, "Service identifier '%s', we think '%08x', they think '%08x'\n", si, ourver, ver);
	if (ourver != ver) 
...
static void save_osptoken(struct iax_frame *fr, struct iax_ies *ies) 
{
	...
	*(full_osptoken + offset) = '\0';
	...
	tris_string_field_set(iaxs[fr->callno], osptoken, full_osptoken);
}
...
static void log_jitterstats(unsigned short callno)
{
	...
	if (iaxs[callno] && iaxs[callno]->owner && iaxs[callno]->owner->name) {
		if(tris_test_flag(iaxs[callno], IAX_USEJITTERBUF)) {
		...
		tris_debug(3, "JB STATS:%s ping=%d ljitterms=%d ljbdelayms=%d ltotlost=%d lrecentlosspct=%d ldropped=%d looo=%d lrecvd=%d rjitterms=%d rjbdelayms=%d rtotlost=%d rrecentlosspct=%d rdropped=%d rooo=%d rrecvd=%d\n",
...
static void handle_deferred_full_frames(struct iax2_thread *thread)
{
	...
	tris_mutex_lock(&thread->lock);

	while ((pkt_buf = TRIS_LIST_REMOVE_HEAD(&thread->full_frames, entry))) {
		tris_mutex_unlock(&thread->lock);
...
static void defer_full_frame(struct iax2_thread *from_here, struct iax2_thread *to_here)
{
	...
	struct tris_iax2_full_hdr *fh, *cur_fh;

	if (!(pkt_buf = tris_calloc(1, sizeof(*pkt_buf) + from_here->buf_len)))
		return;

	pkt_buf->len = from_here->buf_len;
	memcpy(pkt_buf->buf, from_here->buf, pkt_buf->len);

	fh = (struct tris_iax2_full_hdr *) pkt_buf->buf;
	tris_mutex_lock(&to_here->lock);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&to_here->full_frames, cur_pkt_buf, entry) {
		cur_fh = (struct tris_iax2_full_hdr *) cur_pkt_buf->buf;
		if (fh->oseqno < cur_fh->oseqno) {
			TRIS_LIST_INSERT_BEFORE_CURRENT(pkt_buf, entry);
			break;
		}
	}
	TRIS_LIST_TRAVERSE_SAFE_END
	...
	if (!cur_pkt_buf)
		TRIS_LIST_INSERT_TAIL(&to_here->full_frames, pkt_buf, entry);
	
	tris_mutex_unlock(&to_here->lock);
}
...
static int socket_read(int *id, int fd, short events, void *cbdata)
{
	...
	if (!(thread = find_idle_thread())) {
		time(&t);
		if (t != last_errtime)
			tris_debug(1, "Out of idle IAX2 threads for I/O, pausing!\n");
		last_errtime = t;
		usleep(1);
		return 1;
	}
	...
	if (thread->buf_len < 0) {
		if (errno != ECONNREFUSED && errno != EAGAIN)
			tris_log(LOG_WARNING, "Error: %s\n", strerror(errno));
		handle_error();
		thread->iostate = IAX_IOSTATE_IDLE;
		signal_condition(&thread->lock, &thread->cond);
		return 1;
	}
	if (test_losspct && ((100.0 * tris_random() / (RAND_MAX + 1.0)) < test_losspct)) { /* simulate random loss condition */
		thread->iostate = IAX_IOSTATE_IDLE;
		signal_condition(&thread->lock, &thread->cond);
		return 1;
	}
	...
	fh = (struct tris_iax2_full_hdr *) thread->buf;
	if (ntohs(fh->scallno) & IAX_FLAG_FULL) {
		struct iax2_thread *cur = NULL;
		uint16_t callno = ntohs(fh->scallno) & ~IAX_FLAG_FULL;
		
		TRIS_LIST_LOCK(&active_list);
		TRIS_LIST_TRAVERSE(&active_list, cur, list) {
...
#ifdef DEBUG_SCHED_MULTITHREAD
	tris_copy_string(thread->curfunc, "socket_process", sizeof(thread->curfunc));
#endif
	signal_condition(&thread->lock, &thread->cond);
...
static int socket_process_meta(int packet_len, struct tris_iax2_meta_hdr *meta, struct sockaddr_in *sin, int sockfd,
	struct iax_frame *fr)
{
	unsigned char metatype;
	struct tris_iax2_meta_trunk_mini *mtm;
	struct tris_iax2_meta_trunk_hdr *mth;
	struct tris_iax2_meta_trunk_entry *mte;
	struct iax2_trunk_peer *tpeer;
	unsigned int ts;
	void *ptr;
	...
	if (packet_len < sizeof(*meta)) {
		tris_log(LOG_WARNING, "Rejecting packet from '%s.%d' that is flagged as a meta frame but is too short\n", 
	...
	if (packet_len < (sizeof(*meta) + sizeof(*mth))) {
		tris_log(LOG_WARNING, "midget meta trunk packet received (%d of %d min)\n", packet_len,
	...
	if (!tpeer) {
		tris_log(LOG_WARNING, "Unable to accept trunked packet from '%s:%d': No matching peer\n", 
			tris_inet_ntoa(sin->sin_addr), ntohs(sin->sin_port));
		return 1;
	}
	tpeer->trunkact = tris_tvnow();
	if (!ts || tris_tvzero(tpeer->rxtrunktime))
	...
	while (packet_len >= sizeof(*mte)) {
		...
		if (metatype == IAX_META_TRUNK_MINI) {
			...
			mte = (struct tris_iax2_meta_trunk_entry *)ptr;
			ptr += sizeof(*mte);
			...
			tris_log(LOG_WARNING, "Unknown meta trunk cmd from '%s:%d': dropping\n", tris_inet_ntoa(sin->sin_addr), ntohs(sin->sin_port));
			break;
		}
		...
		if (!iaxs[fr->callno]) {
			...
			tris_log(LOG_WARNING, "Received trunked frame before first full voice frame\n");
			iax2_vnak(fr->callno);
		} else {
			...
			if (f.datalen >= 0) {
				...
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED)) {
					...
					f.mallocd = 0;
					f.offset = 0;
					if (f.datalen && (f.frametype == TRIS_FRAME_VOICE)) 
						f.samples = tris_codec_get_samples(&f);
					else
						f.samples = 0;
					fr->outoforder = 0;
					iax_frame_wrap(fr, &f);
				...
				tris_log(LOG_WARNING, "Datalen < 0?\n");
			}
		}
		tris_mutex_unlock(&iaxsl[fr->callno]);
		ptr += len;
		packet_len -= len;
	}
...
static int acf_iaxvar_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct tris_datastore *variablestore = tris_channel_datastore_find(chan, &iax2_variable_datastore_info, NULL);
	TRIS_LIST_HEAD(, tris_var_t) *varlist;
	struct tris_var_t *var;
	...
	TRIS_LIST_LOCK(varlist);
	TRIS_LIST_TRAVERSE(varlist, var, entries) {
		if (strcmp(var->name, data) == 0) {
			tris_copy_string(buf, var->value, len);
			break;
		}
	}
	TRIS_LIST_UNLOCK(varlist);
	return 0;
}

static int acf_iaxvar_write(struct tris_channel *chan, const char *cmd, char *data, const char *value)
{
	struct tris_datastore *variablestore = tris_channel_datastore_find(chan, &iax2_variable_datastore_info, NULL);
	TRIS_LIST_HEAD(, tris_var_t) *varlist;
	struct tris_var_t *var;
	...
	if (!variablestore) {
		...
		varlist = tris_calloc(1, sizeof(*varlist));
		if (!varlist) {
			tris_log(LOG_ERROR, "Unable to assign new variable '%s'\n", data);
			return -1;
		}
		...
		variablestore->data = varlist;
		variablestore->inheritance = DATASTORE_INHERIT_FOREVER;
		tris_channel_datastore_add(chan, variablestore);
	} else
		varlist = variablestore->data;

	TRIS_LIST_LOCK(varlist);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(varlist, var, entries) {
		if (strcmp(var->name, data) == 0) {
	...
	TRIS_LIST_TRAVERSE_SAFE_END;
	var = tris_var_assign(data, value);
	if (var)
		TRIS_LIST_INSERT_TAIL(varlist, var, entries);
	else
		tris_log(LOG_ERROR, "Unable to assign new variable '%s'\n", data);
	TRIS_LIST_UNLOCK(varlist);
	return 0;
...
static struct tris_custom_function iaxvar_function = {
	.name = "IAXVAR",
...
static int socket_process(struct iax2_thread *thread)
{
	...
	int dcallno = 0;
	char decrypted = 0;
	struct tris_iax2_full_hdr *fh = (struct tris_iax2_full_hdr *)thread->buf;
	struct tris_iax2_mini_hdr *mh = (struct tris_iax2_mini_hdr *)thread->buf;
	struct tris_iax2_meta_hdr *meta = (struct tris_iax2_meta_hdr *)thread->buf;
	struct tris_iax2_video_hdr *vh = (struct tris_iax2_video_hdr *)thread->buf;
	struct iax_frame *fr;
	struct iax_frame *cur;
	struct tris_frame f = { 0, };
	struct tris_channel *c = NULL;
	...
	if (res < sizeof(*mh)) {
		tris_log(LOG_WARNING, "midget packet received (%d of %d min)\n", res, (int) sizeof(*mh));
		return 1;
	}
	if ((vh->zeros == 0) && (ntohs(vh->callno) & 0x8000)) {
		if (res < sizeof(*vh)) {
			tris_log(LOG_WARNING, "Rejecting packet from '%s.%d' that is flagged as a video frame but is too short\n", tris_inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
			return 1;
		}
	...
	if (ntohs(mh->callno) & IAX_FLAG_FULL) {
		if (res < sizeof(*fh)) {
			tris_log(LOG_WARNING, "Rejecting packet from '%s.%d' that is flagged as a full frame but is too short\n", tris_inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
			return 1;
		}
		...
		if ((dcallno != 1) && (fr->callno = find_callno(ntohs(mh->callno) & ~IAX_FLAG_FULL, dcallno, &sin, NEW_PREVENT, fd, 1))) {
			tris_mutex_lock(&iaxsl[fr->callno]);
			if (iaxs[fr->callno] && tris_test_flag(iaxs[fr->callno], IAX_ENCRYPTED)) {
				if (decrypt_frame(fr->callno, fh, &f, &res)) {
					tris_log(LOG_NOTICE, "Packet Decrypt Failed!\n");
					tris_mutex_unlock(&iaxsl[fr->callno]);
					return 1;
				}
				decrypted = 1;
		...
		if (f.frametype == TRIS_FRAME_IAX && f.subclass == IAX_COMMAND_POKE) {
			/* Reply back with a PONG, but don't care about the result. */
			send_apathetic_reply(1, ntohs(fh->scallno), &sin, IAX_COMMAND_PONG, ntohl(fh->ts), fh->iseqno + 1, fd, NULL);
			return 1;
		} else if (f.frametype == TRIS_FRAME_IAX && f.subclass == IAX_COMMAND_ACK && dcallno == 1) {
			/* Ignore */
			return 1;
		}
		...
		if (f.datalen) {
			if (f.frametype == TRIS_FRAME_IAX) {
				if (iax_parse_ies(&ies, thread->buf + sizeof(struct tris_iax2_full_hdr), f.datalen)) {
					tris_log(LOG_WARNING, "Undecodable frame received from '%s'\n", tris_inet_ntoa(sin.sin_addr));
					return 1;
				}
				f.data.ptr = NULL;
				f.datalen = 0;
			} else {
				f.data.ptr = thread->buf + sizeof(struct tris_iax2_full_hdr);
	...
	if (!fr->callno) {
		 ...
		 *    http://lists.digium.com/pipermail/trismedia-dev/2008-May/033217.html 
		 */

		if ((ntohs(mh->callno) & IAX_FLAG_FULL) && ((f.frametype == TRIS_FRAME_IAX) && (f.subclass == IAX_COMMAND_ACK))) {
			check_dcallno = 1;
		}

		if (!(fr->callno = find_callno(ntohs(mh->callno) & ~IAX_FLAG_FULL, dcallno, &sin, new, fd, check_dcallno))) {
			if (f.frametype == TRIS_FRAME_IAX && f.subclass == IAX_COMMAND_NEW) {
				send_apathetic_reply(1, ntohs(fh->scallno), &sin, IAX_COMMAND_REJECT, ntohl(fh->ts), fh->iseqno + 1, fd, NULL);
			} else if (f.frametype == TRIS_FRAME_IAX && (f.subclass == IAX_COMMAND_REGREQ || f.subclass == IAX_COMMAND_REGREL)) {
				send_apathetic_reply(1, ntohs(fh->scallno), &sin, IAX_COMMAND_REGREJ, ntohl(fh->ts), fh->iseqno + 1, fd, NULL);
			}
			return 1;
		}
	...
	if (tris_test_flag(iaxs[fr->callno], IAX_ENCRYPTED) && !decrypted) {
		if (decrypt_frame(fr->callno, fh, &f, &res)) {
			tris_log(LOG_NOTICE, "Packet Decrypt Failed!\n");
			tris_mutex_unlock(&iaxsl[fr->callno]);
			return 1;
	...
	if (ntohs(mh->callno) & IAX_FLAG_FULL) {
		if (iaxdebug)
			tris_debug(1, "Received packet %d, (%d, %d)\n", fh->oseqno, f.frametype, f.subclass);
...
#ifdef IAXTESTS
		if (test_resync) {
			tris_debug(1, "Simulating frame ts resync, was %u now %u\n", fr->ts, fr->ts + test_resync);
			fr->ts += test_resync;
		}
#endif /* IAXTESTS */
#if 0
		if ( (ntohs(fh->dcallno) & IAX_FLAG_RETRANS) ||
		     ( (f.frametype != TRIS_FRAME_VOICE) && ! (f.frametype == TRIS_FRAME_IAX &&
								(f.subclass == IAX_COMMAND_NEW ||
								 f.subclass == IAX_COMMAND_AUTHREQ ||
								 f.subclass == IAX_COMMAND_ACCEPT ||
								 f.subclass == IAX_COMMAND_REJECT))      ) )
#endif
		if ((ntohs(fh->dcallno) & IAX_FLAG_RETRANS) || (f.frametype != TRIS_FRAME_VOICE))
			updatehistory = 0;
			...
			if (
			 ((f.subclass != IAX_COMMAND_ACK) &&
			  ...
			  (f.frametype != TRIS_FRAME_IAX)) {
				...
				tris_debug(1, "Packet arrived out of order (expecting %d, got %d) (frametype = %d, subclass = %d)\n", 
				...
				if ((unsigned char) (iaxs[fr->callno]->iseqno - fr->oseqno) < 128) {
					...
					if ((f.frametype != TRIS_FRAME_IAX) || 
						...
						send_command_immediate(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_ACK, fr->ts, NULL, 0,fr->iseqno);
					}
				} else {
		...
		if (f.frametype == TRIS_FRAME_TEXT && thread->buf[res - 1] != '\0') {
		...
		if (!inaddrcmp(&sin, &iaxs[fr->callno]->addr) && 
			...
			if ((x != iaxs[fr->callno]->oseqno) || (iaxs[fr->callno]->oseqno == fr->iseqno)) {
				...
				for (x=iaxs[fr->callno]->rseqno; x != fr->iseqno; x++) {
					...
					if (iaxdebug)
						tris_debug(1, "Cancelling transmission of packet %d\n", x);
					call_to_destroy = 0;
					TRIS_LIST_LOCK(&frame_queue);
					TRIS_LIST_TRAVERSE(&frame_queue, cur, list) {
					...
					if (call_to_destroy) {
						if (iaxdebug)
							tris_debug(1, "Really destroying %d, having been acked on final message\n", call_to_destroy);
						tris_mutex_lock(&iaxsl[call_to_destroy]);
						iax2_destroy(call_to_destroy);
						tris_mutex_unlock(&iaxsl[call_to_destroy]);
					}
				}
				...
				tris_debug(1, "Received iseqno %d not within window %d->%d\n", fr->iseqno, iaxs[fr->callno]->rseqno, iaxs[fr->callno]->oseqno);
			}
		}
		if (inaddrcmp(&sin, &iaxs[fr->callno]->addr) && 
			((f.frametype != TRIS_FRAME_IAX) || 
		...
		if ((f.frametype == TRIS_FRAME_VOICE) ||
		    (f.frametype == TRIS_FRAME_VIDEO) ||
		    (f.frametype == TRIS_FRAME_IAX)) {
			if (tris_test_flag(iaxs[fr->callno], IAX_DELAYPBXSTART)) {
				tris_clear_flag(iaxs[fr->callno], IAX_DELAYPBXSTART);
				if (!tris_iax2_new(fr->callno, TRIS_STATE_RING, iaxs[fr->callno]->chosenformat)) {
					tris_mutex_unlock(&iaxsl[fr->callno]);
					return 1;
				}
			}
			...
			if (ies.vars) {
				struct tris_datastore *variablestore = NULL;
				struct tris_variable *var, *prev = NULL;
				TRIS_LIST_HEAD(, tris_var_t) *varlist;
				if ((c = iaxs[fr->callno]->owner)) {
					...
					if (variablestore && varlist) {
						...
						variablestore->inheritance = DATASTORE_INHERIT_FOREVER;
						TRIS_LIST_HEAD_INIT(varlist);
						tris_debug(1, "I can haz IAX vars?\n");
						for (var = ies.vars; var; var = var->next) {
							struct tris_var_t *newvar = tris_var_assign(var->name, var->value);
							if (prev) {
								tris_free(prev);
						...
						ies.vars = NULL;
						tris_channel_datastore_add(c, variablestore);
					} else {
						tris_log(LOG_ERROR, "Memory allocation error while processing IAX2 variables\n");
						if (variablestore) {
							tris_datastore_free(variablestore);
					...
					tris_debug(1, "No channel, so populating IAXVARs to the pvt, as an intermediate step.\n");
			...
			if (ies.vars) {
				tris_debug(1, "I have IAX variables, but they were not processed\n");
			}
		}
		...
		if ((f.frametype == TRIS_FRAME_IAX) && (f.subclass != IAX_COMMAND_CALLTOKEN) && iaxs[fr->callno]->hold_signaling) {
		...
		if (f.frametype == TRIS_FRAME_VOICE) {
			if (f.subclass != iaxs[fr->callno]->voiceformat) {
					...
					tris_debug(1, "Ooh, voice format changed to %d\n", f.subclass);
					if (iaxs[fr->callno]->owner) {
						int orignative;
retryowner:
						if (tris_channel_trylock(iaxs[fr->callno]->owner)) {
						...
						if (iaxs[fr->callno]) {
							if (iaxs[fr->callno]->owner) {
								...
								if (iaxs[fr->callno]->owner->readformat)
									tris_set_read_format(iaxs[fr->callno]->owner, iaxs[fr->callno]->owner->readformat);
								iaxs[fr->callno]->owner->nativeformats = orignative;
								tris_channel_unlock(iaxs[fr->callno]->owner);
							}
						} else {
							tris_debug(1, "Neat, somebody took away the channel at a magical time but i found it!\n");
							/* Free remote variables (if any) */
							if (ies.vars) {
								tris_variables_destroy(ies.vars);
								tris_debug(1, "I can haz iaxvars, but they is no good.  :-(\n");
								ies.vars = NULL;
							}
							tris_mutex_unlock(&iaxsl[fr->callno]);
							return 1;
		...
		if (f.frametype == TRIS_FRAME_VIDEO) {
			if (f.subclass != iaxs[fr->callno]->videoformat) {
				tris_debug(1, "Ooh, video format changed to %d\n", f.subclass & ~0x1);
				iaxs[fr->callno]->videoformat = f.subclass & ~0x1;
			}
		}
		if (f.frametype == TRIS_FRAME_CONTROL && iaxs[fr->callno]->owner) {
			if (f.subclass == TRIS_CONTROL_BUSY) {
				iaxs[fr->callno]->owner->hangupcause = TRIS_CAUSE_BUSY;
			} else if (f.subclass == TRIS_CONTROL_CONGESTION) {
				iaxs[fr->callno]->owner->hangupcause = TRIS_CAUSE_CONGESTION;
			}
		}
		if (f.frametype == TRIS_FRAME_IAX) {
			tris_sched_thread_del(sched, iaxs[fr->callno]->initid);
			/* Handle the IAX pseudo frame itself */
			if (iaxdebug)
				tris_debug(1, "IAX subclass %d received\n", f.subclass);
			...
			if (iaxs[fr->callno]->last < fr->ts &&
				...
				iaxs[fr->callno]->last = fr->ts;
				if (iaxdebug)
					tris_debug(1, "For call=%d, set last=%d\n", fr->callno, fr->ts);
			}
			...
			case IAX_COMMAND_QUELCH:
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED)) {
					...
					tris_set_flag(iaxs[fr->callno], IAX_QUELCH);
					if (ies.musiconhold) {
						if (iaxs[fr->callno]->owner && tris_bridged_channel(iaxs[fr->callno]->owner)) {
							const char *moh_suggest = iaxs[fr->callno]->mohsuggest;
							iax2_queue_control_data(fr->callno, TRIS_CONTROL_HOLD, 
								S_OR(moh_suggest, NULL),
								!tris_strlen_zero(moh_suggest) ? strlen(moh_suggest) + 1 : 0);
							if (!iaxs[fr->callno]) {
								tris_mutex_unlock(&iaxsl[fr->callno]);
								return 1;
							}
			...
			case IAX_COMMAND_UNQUELCH:
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED)) {
					...
					if (iaxs[fr->callno]->owner && tris_test_flag(iaxs[fr->callno], IAX_QUELCH)) {
						manager_event(EVENT_FLAG_CALL, "Hold",
							"Status: Off\r\n"
							"Channel: %s\r\n"
							"Uniqueid: %s\r\n",
					...
					tris_clear_flag(iaxs[fr->callno], IAX_QUELCH);
					if (iaxs[fr->callno]->owner && tris_bridged_channel(iaxs[fr->callno]->owner)) {
						iax2_queue_control_data(fr->callno, TRIS_CONTROL_UNHOLD, NULL, 0);
						if (!iaxs[fr->callno]) {
							tris_mutex_unlock(&iaxsl[fr->callno]);
							return 1;
						}
			...
			case IAX_COMMAND_TXACC:
				if (iaxs[fr->callno]->transferring == TRANSFER_BEGIN) {
					...
					send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_TXREADY, 0, ied1.buf, ied1.pos, -1);
					iaxs[fr->callno]->transferring = TRANSFER_READY;
				}
				break;
			case IAX_COMMAND_NEW:
				/* Ignore if it's already up */
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED | IAX_STATE_TBD))
					break;
				...
				if (tris_test_flag(iaxs[fr->callno], IAX_TRUNK)) {
					int new_callno;
				...
				if (check_access(fr->callno, &sin, &ies)) {
					...
					if (authdebug)
						tris_log(LOG_NOTICE, "Rejected connect attempt from %s, who was trying to reach '%s@%s'\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->exten, iaxs[fr->callno]->context);
					break;
				}
				if (tris_strlen_zero(iaxs[fr->callno]->secret) && tris_test_flag(iaxs[fr->callno], IAX_FORCE_ENCRYPT)) {
					auth_fail(fr->callno, IAX_COMMAND_REJECT);
					tris_log(LOG_WARNING, "Rejected connect attempt.  No secret present while force encrypt enabled.\n");
					break;
				}
				if (strcasecmp(iaxs[fr->callno]->exten, "TBD")) {
					const char *context, *exten, *cid_num;

					context = tris_strdupa(iaxs[fr->callno]->context);
					exten = tris_strdupa(iaxs[fr->callno]->exten);
					cid_num = tris_strdupa(iaxs[fr->callno]->cid_num);

					/* This might re-enter the IAX code and need the lock */
					tris_mutex_unlock(&iaxsl[fr->callno]);
					exists = tris_exists_extension(NULL, context, exten, 1, cid_num);
					tris_mutex_lock(&iaxsl[fr->callno]);
				...
				if (tris_strlen_zero(iaxs[fr->callno]->secret) && tris_strlen_zero(iaxs[fr->callno]->inkeys)) {
					if (strcmp(iaxs[fr->callno]->exten, "TBD") && !exists) {
						memset(&ied0, 0, sizeof(ied0));
						iax_ie_append_str(&ied0, IAX_IE_CAUSE, "No such context/extension");
						iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, TRIS_CAUSE_NO_ROUTE_DESTINATION);
						send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
						if (!iaxs[fr->callno]) {
							tris_mutex_unlock(&iaxsl[fr->callno]);
							return 1;
						}
						if (authdebug)
							tris_log(LOG_NOTICE, "Rejected connect attempt from %s, request '%s@%s' does not exist\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->exten, iaxs[fr->callno]->context);
					} else {
						/* Select an appropriate format */

						if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOPREFS)) {
							if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP)) {
								using_prefs = "reqonly";
							} else {
							...
							if (ies.codec_prefs)
								tris_codec_pref_convert(&iaxs[fr->callno]->rprefs, ies.codec_prefs, 32, 0);
							if (tris_codec_pref_index(&iaxs[fr->callno]->rprefs, 0)) {
								/* If we are codec_first_choice we let the caller have the 1st shot at picking the codec.*/
								if (tris_test_flag(iaxs[fr->callno], IAX_CODEC_USER_FIRST)) {
							...
							format = tris_codec_choose(&pref, iaxs[fr->callno]->capability & iaxs[fr->callno]->peercapability, 0);
							tris_codec_pref_string(&iaxs[fr->callno]->rprefs, caller_pref_buf, sizeof(caller_pref_buf) - 1);
							tris_codec_pref_string(&iaxs[fr->callno]->prefs, host_pref_buf, sizeof(host_pref_buf) - 1);
						}
						if (!format) {
							if(!tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP))
							...
							if (!format) {
								...
								iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, TRIS_CAUSE_BEARERCAPABILITY_NOTAVAIL);
								send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
								if (!iaxs[fr->callno]) {
									tris_mutex_unlock(&iaxsl[fr->callno]);
									return 1;
								...
								if (authdebug) {
									if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP))
										tris_log(LOG_NOTICE, "Rejected connect attempt from %s, requested 0x%x incompatible with our capability 0x%x.\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->peerformat, iaxs[fr->callno]->capability);
									else 
										tris_log(LOG_NOTICE, "Rejected connect attempt from %s, requested/capability 0x%x/0x%x incompatible with our capability 0x%x.\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->peerformat, iaxs[fr->callno]->peercapability, iaxs[fr->callno]->capability);
								}
							} else {
								/* Pick one... */
								if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP)) {
									...
									if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOPREFS)) {
										using_prefs = tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP) ? "reqonly" : "disabled";
										memset(&pref, 0, sizeof(pref));
										format = tris_best_codec(iaxs[fr->callno]->peercapability & iaxs[fr->callno]->capability);
										strcpy(caller_pref_buf,"disabled");
										strcpy(host_pref_buf,"disabled");
									} else {
										using_prefs = "mine";
										if (tris_codec_pref_index(&iaxs[fr->callno]->rprefs, 0)) {
											...
											format = tris_codec_choose(&pref, iaxs[fr->callno]->peercapability & iaxs[fr->callno]->capability, 1);
								...
								if (!format) {
									...
									iax_ie_append_str(&ied0, IAX_IE_CAUSE, "Unable to negotiate codec");
									iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, TRIS_CAUSE_BEARERCAPABILITY_NOTAVAIL);
									tris_log(LOG_ERROR, "No best format in 0x%x???\n", iaxs[fr->callno]->peercapability & iaxs[fr->callno]->capability);
									send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
									if (!iaxs[fr->callno]) {
										tris_mutex_unlock(&iaxsl[fr->callno]);
										return 1;
									...
									if (authdebug)
										tris_log(LOG_NOTICE, "Rejected connect attempt from %s, requested/capability 0x%x/0x%x incompatible with our capability 0x%x.\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->peerformat, iaxs[fr->callno]->peercapability, iaxs[fr->callno]->capability);
									tris_set_flag(iaxs[fr->callno], IAX_ALREADYGONE);	
									break;
								}
							}
						}
						if (format) {
							...
							send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_ACCEPT, 0, ied1.buf, ied1.pos, -1);
							if (strcmp(iaxs[fr->callno]->exten, "TBD")) {
								tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
								tris_verb(3, "Accepting UNAUTHENTICATED call from %s:\n"
												"%srequested format = %s,\n"
												"%srequested prefs = %s,\n"
												"%sactual format = %s,\n"
												"%shost prefs = %s,\n"
												...
												tris_inet_ntoa(sin.sin_addr), 
												VERBOSE_PREFIX_4,
												tris_getformatname(iaxs[fr->callno]->peerformat), 
												VERBOSE_PREFIX_4,
												caller_pref_buf,
												VERBOSE_PREFIX_4,
								...
								tris_set_flag(iaxs[fr->callno], IAX_DELAYPBXSTART);
							} else {
								tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD);
								/* If this is a TBD call, we're ready but now what...  */
								tris_verb(3, "Accepted unauthenticated TBD call from %s\n", tris_inet_ntoa(sin.sin_addr));
							}
						}
					}
					break;
				...
				if (!authenticate_request(fr->callno) && iaxs[fr->callno])
					tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_AUTHENTICATED);
				if (!iaxs[fr->callno]) {
					tris_mutex_unlock(&iaxsl[fr->callno]);
					return 1;
				}
			...
			case IAX_COMMAND_DPREQ:
				...
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD) &&
					!tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED) && ies.called_number) {
					if (iaxcompat) {
			...
			case IAX_COMMAND_HANGUP:
				tris_set_flag(iaxs[fr->callno], IAX_ALREADYGONE);
				tris_debug(1, "Immediately destroying %d, having received hangup\n", fr->callno);
			...
			case IAX_COMMAND_REJECT:
				...
				if (!tris_test_flag(iaxs[fr->callno], IAX_PROVISION)) {
					if (iaxs[fr->callno]->owner && authdebug)
						tris_log(LOG_WARNING, "Call rejected by %s: %s\n",
							tris_inet_ntoa(iaxs[fr->callno]->addr.sin_addr),
							ies.cause ? ies.cause : "<Unknown>");
					tris_debug(1, "Immediately destroying %d, having received reject\n",
						fr->callno);
				}
				/* Send ack immediately, before we destroy */
				send_command_immediate(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_ACK,
			...
			{
				struct tris_channel *bridged_chan;

				if (iaxs[fr->callno]->owner && (bridged_chan = tris_bridged_channel(iaxs[fr->callno]->owner)) && ies.called_number) {
					...
					if (!strcmp(ies.called_number, tris_parking_ext())) {
						struct tris_channel *saved_channel = iaxs[fr->callno]->owner;
						tris_mutex_unlock(&iaxsl[fr->callno]);
						if (iax_park(bridged_chan, saved_channel)) {
							tris_log(LOG_WARNING, "Failed to park call on '%s'\n", bridged_chan->name);
						} else {
							tris_debug(1, "Parked call on '%s'\n", tris_bridged_channel(iaxs[fr->callno]->owner)->name);
						}
						tris_mutex_lock(&iaxsl[fr->callno]);
					} else {
						if (tris_async_goto(bridged_chan, iaxs[fr->callno]->context, ies.called_number, 1))
							tris_log(LOG_WARNING, "Async goto of '%s' to '%s@%s' failed\n", bridged_chan->name, 
								ies.called_number, iaxs[fr->callno]->context);
						else {
							tris_debug(1, "Async goto of '%s' to '%s@%s' started\n", bridged_chan->name, 
								ies.called_number, iaxs[fr->callno]->context);
						}
					}
				} else {
					tris_debug(1, "Async goto not applicable on call %d\n", fr->callno);
				}
			...
			case IAX_COMMAND_ACCEPT:
				...
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED | IAX_STATE_TBD | IAX_STATE_AUTHENTICATED))
					break;
				if (tris_test_flag(iaxs[fr->callno], IAX_PROVISION)) {
					/* Send ack immediately, before we destroy */
					send_command_immediate(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_ACK, fr->ts, NULL, 0,fr->iseqno);
				...
				tris_verb(3, "Call accepted by %s (format %s)\n", tris_inet_ntoa(iaxs[fr->callno]->addr.sin_addr), tris_getformatname(iaxs[fr->callno]->peerformat));
				if (!(iaxs[fr->callno]->peerformat & iaxs[fr->callno]->capability)) {
					memset(&ied0, 0, sizeof(ied0));
					iax_ie_append_str(&ied0, IAX_IE_CAUSE, "Unable to negotiate codec");
					iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, TRIS_CAUSE_BEARERCAPABILITY_NOTAVAIL);
					...
					if (authdebug)
						tris_log(LOG_NOTICE, "Rejected call to %s, format 0x%x incompatible with our capability 0x%x.\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->peerformat, iaxs[fr->callno]->capability);
				} else {
					tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
					if (iaxs[fr->callno]->owner) {
						/* Switch us to use a compatible format */
						iaxs[fr->callno]->owner->nativeformats = iaxs[fr->callno]->peerformat;
						tris_verb(3, "Format for call is %s\n", tris_getformatname(iaxs[fr->callno]->owner->nativeformats));
retryowner2:
						if (tris_channel_trylock(iaxs[fr->callno]->owner)) {
							DEADLOCK_AVOIDANCE(&iaxsl[fr->callno]);
						...
						if (iaxs[fr->callno] && iaxs[fr->callno]->owner) {
							...
							if (iaxs[fr->callno]->owner->writeformat)
								tris_set_write_format(iaxs[fr->callno]->owner, iaxs[fr->callno]->owner->writeformat);	
							if (iaxs[fr->callno]->owner->readformat)
								tris_set_read_format(iaxs[fr->callno]->owner, iaxs[fr->callno]->owner->readformat);	
							tris_channel_unlock(iaxs[fr->callno]->owner);
						}
				...
				if (iaxs[fr->callno]) {
					TRIS_LIST_LOCK(&dpcache);
					TRIS_LIST_TRAVERSE(&iaxs[fr->callno]->dpentries, dp, peer_list)
						if (!(dp->flags & CACHE_FLAG_TRANSMITTED))
			...
			case IAX_COMMAND_POKE:
				...
				send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_PONG, fr->ts, NULL, 0, -1);
				if (!iaxs[fr->callno]) {
					tris_mutex_unlock(&iaxsl[fr->callno]);
					return 1;
				}
			...
			case IAX_COMMAND_PING:
			{
				...
				/* Send back a pong packet with the original timestamp */
				send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_PONG, fr->ts, pingied.buf, pingied.pos, -1);
			}
				break;
				...
				if (iaxs[fr->callno]->peerpoke) {
					...
					if ((peer->lastms < 0)  || (peer->historicms > peer->maxms)) {
						if (iaxs[fr->callno]->pingtime <= peer->maxms) {
							tris_log(LOG_NOTICE, "Peer '%s' is now REACHABLE! Time: %d\n", peer->name, iaxs[fr->callno]->pingtime);
							manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: IAX2\r\nPeer: IAX2/%s\r\nPeerStatus: Reachable\r\nTime: %d\r\n", peer->name, iaxs[fr->callno]->pingtime); 
							tris_devstate_changed(TRIS_DEVICE_NOT_INUSE, "IAX2/%s", peer->name); /* Activate notification */
						}
					} else if ((peer->historicms > 0) && (peer->historicms <= peer->maxms)) {
						if (iaxs[fr->callno]->pingtime > peer->maxms) {
							tris_log(LOG_NOTICE, "Peer '%s' is now TOO LAGGED (%d ms)!\n", peer->name, iaxs[fr->callno]->pingtime);
							manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "ChannelType: IAX2\r\nPeer: IAX2/%s\r\nPeerStatus: Lagged\r\nTime: %d\r\n", peer->name, iaxs[fr->callno]->pingtime); 
							tris_devstate_changed(TRIS_DEVICE_UNAVAILABLE, "IAX2/%s", peer->name); /* Activate notification */
						}
					}
					...
					iax2_destroy(fr->callno);
					peer->callno = 0;
					tris_debug(1, "Peer %s: got pong, lastms %d, historicms %d, maxms %d\n", peer->name, peer->lastms, peer->historicms, peer->maxms);
				}
				break;
			...
			case IAX_COMMAND_LAGRP:
				...
				if(f.subclass == IAX_COMMAND_LAGRQ) {
					...
					if (iaxdebug)
						tris_debug(1, "Peer %s lag measured as %dms\n",
							tris_inet_ntoa(iaxs[fr->callno]->addr.sin_addr), iaxs[fr->callno]->lag);
				}
				break;
			case IAX_COMMAND_AUTHREQ:
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED | IAX_STATE_TBD)) {
					tris_log(LOG_WARNING, "Call on %s is already up, can't start on it\n", iaxs[fr->callno]->owner ? iaxs[fr->callno]->owner->name : "<Unknown>");
					break;
				}
				if (authenticate_reply(iaxs[fr->callno], &iaxs[fr->callno]->addr, &ies, iaxs[fr->callno]->secret, iaxs[fr->callno]->outkey)) {
					struct tris_frame hangup_fr = { .frametype = TRIS_FRAME_CONTROL,
								.subclass = TRIS_CONTROL_HANGUP,
					};
					tris_log(LOG_WARNING, 
						"I don't know how to authenticate %s to %s\n", 
						ies.username ? ies.username : "<unknown>", tris_inet_ntoa(iaxs[fr->callno]->addr.sin_addr));
			...
			case IAX_COMMAND_AUTHREP:
				...
				if (authenticate_verify(iaxs[fr->callno], &ies)) {
					if (authdebug)
						tris_log(LOG_NOTICE, "Host %s failed to authenticate as %s\n", tris_inet_ntoa(iaxs[fr->callno]->addr.sin_addr), iaxs[fr->callno]->username);
					memset(&ied0, 0, sizeof(ied0));
					auth_fail(fr->callno, IAX_COMMAND_REJECT);
					break;
				}
				if (strcasecmp(iaxs[fr->callno]->exten, "TBD")) {
					/* This might re-enter the IAX code and need the lock */
					exists = tris_exists_extension(NULL, iaxs[fr->callno]->context, iaxs[fr->callno]->exten, 1, iaxs[fr->callno]->cid_num);
				} else
					exists = 0;
				if (strcmp(iaxs[fr->callno]->exten, "TBD") && !exists) {
					if (authdebug)
					...
					if (!format) {
						if(!tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP)) {
							tris_debug(1, "We don't do requested format %s, falling back to peer capability %d\n", tris_getformatname(iaxs[fr->callno]->peerformat), iaxs[fr->callno]->peercapability);
							format = iaxs[fr->callno]->peercapability & iaxs[fr->callno]->capability;
						}
						if (!format) {
							if (authdebug) {
							...
							if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP)) {
								...
								if(tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOPREFS)) {
									using_prefs = tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP) ? "reqonly" : "disabled";
									memset(&pref, 0, sizeof(pref));
									format = tris_test_flag(iaxs[fr->callno], IAX_CODEC_NOCAP) ?
										iaxs[fr->callno]->peerformat : tris_best_codec(iaxs[fr->callno]->peercapability & iaxs[fr->callno]->capability);
									strcpy(caller_pref_buf,"disabled");
									strcpy(host_pref_buf,"disabled");
								} else {
									using_prefs = "mine";
					...
					if (format) {
						...
						if (strcmp(iaxs[fr->callno]->exten, "TBD")) {
							tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
							tris_verb(3, "Accepting AUTHENTICATED call from %s:\n"
											"%srequested format = %s,\n"
											"%srequested prefs = %s,\n"
											"%sactual format = %s,\n"
											"%shost prefs = %s,\n"
							...
							tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
							if (!(c = tris_iax2_new(fr->callno, TRIS_STATE_RING, format)))
								iax2_destroy(fr->callno);
							else if (ies.vars) {
								struct tris_datastore *variablestore;
								struct tris_variable *var, *prev = NULL;
								...
								if (variablestore && varlist) {
									...
									variablestore->inheritance = DATASTORE_INHERIT_FOREVER;
									TRIS_LIST_HEAD_INIT(varlist);
									tris_debug(1, "I can haz IAX vars? w00t\n");
									for (var = ies.vars; var; var = var->next) {
										struct tris_var_t *newvar = tris_var_assign(var->name, var->value);
										if (prev)
											tris_free(prev);
							...
							tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD);
							/* If this is a TBD call, we're ready but now what...  */
							tris_verb(3, "Accepted AUTHENTICATED TBD call from %s\n", tris_inet_ntoa(sin.sin_addr));
							if (tris_test_flag(iaxs[fr->callno], IAX_IMMEDIATE)) {
								goto immediatedial;
							}
						}
					}
...
immediatedial:
				if (tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD)) {
					tris_clear_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD);
					tris_string_field_set(iaxs[fr->callno], exten, ies.called_number ? ies.called_number : "s");
					if (!tris_exists_extension(NULL, iaxs[fr->callno]->context, iaxs[fr->callno]->exten, 1, iaxs[fr->callno]->cid_num)) {
						if (authdebug)
							tris_log(LOG_NOTICE, "Rejected dial attempt from %s, request '%s@%s' does not exist\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->exten, iaxs[fr->callno]->context);
						memset(&ied0, 0, sizeof(ied0));
						iax_ie_append_str(&ied0, IAX_IE_CAUSE, "No such context/extension");
						iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, TRIS_CAUSE_NO_ROUTE_DESTINATION);
						send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
						...
						tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
						tris_verb(3, "Accepting DIAL from %s, formats = 0x%x\n", tris_inet_ntoa(sin.sin_addr), iaxs[fr->callno]->peerformat);
						tris_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
						send_command(iaxs[fr->callno], TRIS_FRAME_CONTROL, TRIS_CONTROL_PROGRESS, 0, NULL, 0, -1);
						if (!(c = tris_iax2_new(fr->callno, TRIS_STATE_RING, iaxs[fr->callno]->peerformat)))
							iax2_destroy(fr->callno);
						else if (ies.vars) {
							struct tris_datastore *variablestore;
							struct tris_variable *var, *prev = NULL;
			...
			case IAX_COMMAND_INVAL:
				...
				tris_debug(1, "Immediately destroying %d, having received INVAL\n", fr->callno);
				iax2_destroy(fr->callno);
				tris_debug(1, "Destroying call %d\n", fr->callno);
				break;
			case IAX_COMMAND_VNAK:
				tris_debug(1, "Received VNAK: resending outstanding frames\n");
			...
			case IAX_COMMAND_REGREL:
				...
				if ((tris_strlen_zero(iaxs[fr->callno]->secret) && tris_strlen_zero(iaxs[fr->callno]->inkeys)) ||
						tris_test_flag(&iaxs[fr->callno]->state, IAX_STATE_AUTHENTICATED)) {

					if (f.subclass == IAX_COMMAND_REGREL)
						memset(&sin, 0, sizeof(sin));
					if (update_registry(&sin, fr->callno, ies.devicetype, fd, ies.refresh))
						tris_log(LOG_WARNING, "Registry error\n");
					if (!iaxs[fr->callno]) {
						tris_mutex_unlock(&iaxsl[fr->callno]);
						return 1;
					}
			...
			case IAX_COMMAND_REGACK:
				if (iax2_ack_registry(&ies, &sin, fr->callno)) 
					tris_log(LOG_WARNING, "Registration failure\n");
				/* Send ack immediately, before we destroy */
				send_command_immediate(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_ACK, fr->ts, NULL, 0,fr->iseqno);
				iax2_destroy(fr->callno);
				break;
			case IAX_COMMAND_REGREJ:
				if (iaxs[fr->callno]->reg) {
					if (authdebug) {
						tris_log(LOG_NOTICE, "Registration of '%s' rejected: '%s' from: '%s'\n", iaxs[fr->callno]->reg->username, ies.cause ? ies.cause : "<unknown>", tris_inet_ntoa(sin.sin_addr));
			...
			case IAX_COMMAND_REGAUTH:
				...
				if (registry_rerequest(&ies, fr->callno, &sin)) {
					...
					iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, TRIS_CAUSE_FACILITY_NOT_SUBSCRIBED);
					send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
					if (!iaxs[fr->callno]) {
						tris_mutex_unlock(&iaxsl[fr->callno]);
						return 1;
			...
			case IAX_COMMAND_TXREJ:
				...
				tris_verb(3, "Channel '%s' unable to transfer\n", iaxs[fr->callno]->owner ? iaxs[fr->callno]->owner->name : "<Unknown>");
				memset(&iaxs[fr->callno]->transfer, 0, sizeof(iaxs[fr->callno]->transfer));
				if (iaxs[fr->callno]->bridgecallno) {
					if (iaxs[iaxs[fr->callno]->bridgecallno]->transferring) {
						iaxs[iaxs[fr->callno]->bridgecallno]->transferring = 0;
						send_command(iaxs[iaxs[fr->callno]->bridgecallno], TRIS_FRAME_IAX, IAX_COMMAND_TXREJ, 0, NULL, 0, -1);
					}
				}
				break;
			case IAX_COMMAND_TXREADY:
				if ((iaxs[fr->callno]->transferring == TRANSFER_BEGIN) ||
					...
					if (iaxs[fr->callno]->transferring == TRANSFER_MBEGIN)
						...
						iaxs[fr->callno]->transferring = TRANSFER_READY;
					tris_verb(3, "Channel '%s' ready to transfer\n", iaxs[fr->callno]->owner ? iaxs[fr->callno]->owner->name : "<Unknown>");
					if (iaxs[fr->callno]->bridgecallno) {
						if ((iaxs[iaxs[fr->callno]->bridgecallno]->transferring == TRANSFER_READY) ||
						    (iaxs[iaxs[fr->callno]->bridgecallno]->transferring == TRANSFER_MREADY)) {
							/* They're both ready, now release them. */
							if (iaxs[fr->callno]->transferring == TRANSFER_MREADY) {
								tris_verb(3, "Attempting media bridge of %s and %s\n", iaxs[fr->callno]->owner ? iaxs[fr->callno]->owner->name : "<Unknown>",
								...
								send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_TXMEDIA, 0, ied0.buf, ied0.pos, -1);
								send_command(iaxs[iaxs[fr->callno]->bridgecallno], TRIS_FRAME_IAX, IAX_COMMAND_TXMEDIA, 0, ied1.buf, ied1.pos, -1);
							} else {
								tris_verb(3, "Releasing %s and %s\n", iaxs[fr->callno]->owner ? iaxs[fr->callno]->owner->name : "<Unknown>",
										iaxs[iaxs[fr->callno]->bridgecallno]->owner ? iaxs[iaxs[fr->callno]->bridgecallno]->owner->name : "<Unknown>");

								iaxs[iaxs[fr->callno]->bridgecallno]->transferring = TRANSFER_RELEASED;
								iaxs[fr->callno]->transferring = TRANSFER_RELEASED;
								tris_set_flag(iaxs[iaxs[fr->callno]->bridgecallno], IAX_ALREADYGONE);
								tris_set_flag(iaxs[fr->callno], IAX_ALREADYGONE);
								...
								iax_ie_append_short(&ied0, IAX_IE_CALLNO, iaxs[iaxs[fr->callno]->bridgecallno]->peercallno);
								iax_ie_append_short(&ied1, IAX_IE_CALLNO, iaxs[fr->callno]->peercallno);
								send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_TXREL, 0, ied0.buf, ied0.pos, -1);
								send_command(iaxs[iaxs[fr->callno]->bridgecallno], TRIS_FRAME_IAX, IAX_COMMAND_TXREL, 0, ied1.buf, ied1.pos, -1);
							}
			...
			case IAX_COMMAND_TXCNT:
				if (iaxs[fr->callno]->transferring)
					send_command_transfer(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_TXACC, 0, NULL, 0);
				break;
			...
			case IAX_COMMAND_RTKEY:
				...
				tris_aes_decrypt_key((unsigned char *) ies.challenge, &iaxs[fr->callno]->dcx);
				break;
			case IAX_COMMAND_DPREP:
				complete_dpreply(iaxs[fr->callno], &ies);
				break;
			case IAX_COMMAND_UNSUPPORT:
				tris_log(LOG_NOTICE, "Peer did not understand our iax command '%d'\n", ies.iax_unknown);
				break;
			case IAX_COMMAND_FWDOWNL:
				/* Firmware download */
				if (!tris_test_flag(&globalflags, IAX_ALLOWFWDOWNLOAD)) {
					send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_UNSUPPORT, 0, NULL, 0, -1);
					break;
				}
				memset(&ied0, 0, sizeof(ied0));
				...
				if (res < 0)
					...
					send_command_final(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_FWDATA, 0, ied0.buf, ied0.pos, -1);
				else
					send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_FWDATA, 0, ied0.buf, ied0.pos, -1);
				if (!iaxs[fr->callno]) {
					tris_mutex_unlock(&iaxsl[fr->callno]);
					return 1;
				}
			...
			default:
				tris_debug(1, "Unknown IAX command %d on %d/%d\n", f.subclass, fr->callno, iaxs[fr->callno]->peercallno);
				memset(&ied0, 0, sizeof(ied0));
				iax_ie_append_byte(&ied0, IAX_IE_IAX_UNKNOWN, f.subclass);
				send_command(iaxs[fr->callno], TRIS_FRAME_IAX, IAX_COMMAND_UNSUPPORT, 0, ied0.buf, ied0.pos, -1);
			}
			/* Free remote variables (if any) */
			if (ies.vars) {
				tris_variables_destroy(ies.vars);
				tris_debug(1, "I can haz IAX vars, but they is no good :-(\n");
				ies.vars = NULL;
			}
		...
		if (iaxs[fr->callno]->videoformat > 0) 
			...
			tris_log(LOG_WARNING, "Received mini frame before first full video frame\n");
			iax2_vnak(fr->callno);
			tris_mutex_unlock(&iaxsl[fr->callno]);
			return 1;
		}
		...
		if (iaxs[fr->callno]->voiceformat > 0)
			...
			tris_debug(1, "Received mini frame before first full voice frame\n");
			iax2_vnak(fr->callno);
			tris_mutex_unlock(&iaxsl[fr->callno]);
			return 1;
		}
		f.datalen = res - sizeof(struct tris_iax2_mini_hdr);
		if (f.datalen < 0) {
			tris_log(LOG_WARNING, "Datalen < 0?\n");
			tris_mutex_unlock(&iaxsl[fr->callno]);
			return 1;
	...
	if (iaxs[fr->callno] && iaxs[fr->callno]->last < fr->ts) {
		...
		if (iaxdebug && iaxs[fr->callno])
			tris_debug(1, "Received out of order packet... (type=%d, subclass %d, ts = %d, last = %d)\n", f.frametype, f.subclass, fr->ts, iaxs[fr->callno]->last);
		fr->outoforder = -1;
	}
	fr->cacheable = ((f.frametype == TRIS_FRAME_VOICE) || (f.frametype == TRIS_FRAME_VIDEO));
	duped_fr = iaxfrdup2(fr);
	if (duped_fr) {
...
static void iax2_process_thread_cleanup(void *data)
{
	...
	tris_mutex_destroy(&thread->lock);
	tris_cond_destroy(&thread->cond);
	tris_mutex_destroy(&thread->init_lock);
	tris_cond_destroy(&thread->init_cond);
	tris_free(thread);
	tris_atomic_dec_and_test(&iaxactivethreadcount);
}
...
static void *iax2_process_thread(void *data)
{
	...
	tris_atomic_fetchadd_int(&iaxactivethreadcount,1);
	...
	for(;;) {
		...
		if (thread->type == IAX_THREAD_TYPE_DYNAMIC) {
			...
			wait = tris_tvadd(tris_tvnow(), tris_samp2tv(30000, 1000));
			ts.tv_sec = wait.tv_sec;
			ts.tv_nsec = wait.tv_usec * 1000;
			if (tris_cond_timedwait(&thread->cond, &thread->lock, &ts) == ETIMEDOUT) {
				...
				if ((t = TRIS_LIST_REMOVE(&dynamic_list, thread, list)))
					tris_atomic_fetchadd_int(&iaxdynamicthreadcount, -1);
				TRIS_LIST_UNLOCK(&dynamic_list);
				if (t) {
			...
			tris_cond_wait(&thread->cond, &thread->lock);
		}
		...
		TRIS_LIST_LOCK(&active_list);
		TRIS_LIST_INSERT_HEAD(&active_list, thread, list);
		TRIS_LIST_UNLOCK(&active_list);
		...
		TRIS_LIST_LOCK(&active_list);
		TRIS_LIST_REMOVE(&active_list, thread, list);
		TRIS_LIST_UNLOCK(&active_list);
	 ...
	 */
	TRIS_LIST_LOCK(&idle_list);
	TRIS_LIST_REMOVE(&idle_list, thread, list);
	TRIS_LIST_UNLOCK(&idle_list);

	TRIS_LIST_LOCK(&dynamic_list);
	TRIS_LIST_REMOVE(&dynamic_list, thread, list);
	TRIS_LIST_UNLOCK(&dynamic_list);
...
static int iax2_do_register(struct iax2_registry *reg)
{
	 ...
	 */
	if (reg->dnsmgr && tris_dnsmgr_changed(reg->dnsmgr) && (reg->callno > 0)) {
	...
	if (!reg->addr.sin_addr.s_addr) {
		if (iaxdebug)
			tris_debug(1, "Unable to send registration request for '%s' without IP address\n", reg->username);
	...
	if (!reg->callno) {
		tris_debug(3, "Allocate call number\n");
		reg->callno = find_callno_locked(0, 0, &reg->addr, NEW_FORCE, defaultsockfd, 0);
		if (reg->callno < 1) {
			tris_log(LOG_WARNING, "Unable to create call for registration\n");
			return -1;
		} else
			tris_debug(3, "Registration created on call %d\n", reg->callno);
		iaxs[reg->callno]->reg = reg;
		tris_mutex_unlock(&iaxsl[reg->callno]);
	}
	...
	send_command(iaxs[reg->callno],TRIS_FRAME_IAX, IAX_COMMAND_REGREQ, 0, ied.buf, ied.pos, -1);
...
static int iax2_provision(struct sockaddr_in *end, int sockfd, char *dest, const char *template, int force)
{
	...
	tris_debug(1, "Provisioning '%s' from template '%s'\n", dest, template);

	if (iax_provision_build(&provdata, &sig, template, force)) {
		tris_debug(1, "No provisioning found for template '%s'\n", template);
		return 0;
	}
	...
	if (iaxs[callno]) {
		...
		tris_set_flag(iaxs[callno], IAX_PROVISION);
		/* Got a call number now, so go ahead and send the provisioning information */
		send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_PROVISION, 0, ied.buf, ied.pos, -1);
	}
	tris_mutex_unlock(&iaxsl[callno]);
...
*/
static int iax2_prov_app(struct tris_channel *chan, void *data)
{
	int res;
	char *sdata;
	char *opts;
	...
	if (tris_strlen_zero(data))
		data = "default";
	sdata = tris_strdupa(data);
	opts = strchr(sdata, '|');
	if (opts)
		*opts='\0';

	if (chan->tech != &iax2_tech) {
		tris_log(LOG_NOTICE, "Can't provision a non-IAX device!\n");
		return -1;
	} 
	if (!callno || !iaxs[callno] || !iaxs[callno]->addr.sin_addr.s_addr) {
		tris_log(LOG_NOTICE, "Can't provision something with no IP?\n");
		return -1;
	}
	res = iax2_provision(&iaxs[callno]->addr, iaxs[callno]->sockfd, NULL, sdata, force);
	tris_verb(3, "Provisioned IAXY at '%s' with '%s'= %d\n",
		tris_inet_ntoa(iaxs[callno]->addr.sin_addr),
		sdata, res);
	return res;
}

static char *handle_cli_iax2_provision(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int force = 0;
	int res;
	...
	if (res < 0)
		tris_cli(a->fd, "Unable to find peer/address '%s'\n", a->argv[2]);
	else if (res < 1)
		tris_cli(a->fd, "No template (including wildcard) matching '%s'\n", a->argv[3]);
	else
		tris_cli(a->fd, "Provisioning '%s' with template '%s'%s\n", a->argv[2], a->argv[3], force ? ", forced" : "");
	return CLI_SUCCESS;
}
...
static void __iax2_poke_noanswer(const void *data)
{
	...
	if (peer->lastms > -1) {
		tris_log(LOG_NOTICE, "Peer '%s' is now UNREACHABLE! Time: %d\n", peer->name, peer->lastms);
...
static int iax2_poke_peer(struct iax2_peer *peer, int heldcall)
{
	...
	if ((callno = peer->callno) > 0) {
		tris_log(LOG_NOTICE, "Still have a callno...\n");
		tris_mutex_lock(&iaxsl[callno]);
		iax2_destroy(callno);
		tris_mutex_unlock(&iaxsl[callno]);
	}
	if (heldcall)
		tris_mutex_unlock(&iaxsl[heldcall]);
	callno = peer->callno = find_callno(0, 0, &peer->addr, NEW_FORCE, peer->sockfd, 0);
	if (heldcall)
		tris_mutex_lock(&iaxsl[heldcall]);
	if (peer->callno < 1) {
		tris_log(LOG_WARNING, "Unable to allocate call for poking peer '%s'\n", peer->name);
		return -1;
	}
	...
	if (iaxs[callno]) {
		...
		add_empty_calltoken_ie(iaxs[callno], &ied); /* this _MUST_ be the last ie added */
		send_command(iaxs[callno], TRIS_FRAME_IAX, IAX_COMMAND_POKE, 0, ied.buf, ied.pos, -1);
	}
	tris_mutex_unlock(&iaxsl[callno]);
...
static struct tris_channel *iax2_request(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	...
	tris_copy_flags(&cai, &globalflags, IAX_NOTRANSFER | IAX_TRANSFERMEDIA | IAX_USEJITTERBUF | IAX_FORCEJITTERBUF);
	...
	callno = find_callno_locked(0, 0, &sin, NEW_FORCE, cai.sockfd, 0);
	if (callno < 1) {
		tris_log(LOG_WARNING, "Unable to create call\n");
		*cause = TRIS_CAUSE_CONGESTION;
		return NULL;
	}

	/* If this is a trunk, update it now */
	tris_copy_flags(iaxs[callno], &cai, IAX_TRUNK | IAX_SENDANI | IAX_NOTRANSFER | IAX_TRANSFERMEDIA | IAX_USEJITTERBUF | IAX_FORCEJITTERBUF);
	if (tris_test_flag(&cai, IAX_TRUNK)) {
		int new_callno;
	...
	if (cai.found)
		tris_string_field_set(iaxs[callno], host, pds.peer);

	c = tris_iax2_new(callno, TRIS_STATE_DOWN, cai.capability);
	...
	if (c) {
		...
		if (c->nativeformats & format) 
			...
			if (res < 0) {
				tris_log(LOG_WARNING, "Unable to create translator path for %s to %s on %s\n",
					tris_getformatname(c->nativeformats), tris_getformatname(fmt), c->name);
				tris_hangup(c);
				return NULL;
			}
			c->nativeformats = native;
		}
		c->readformat = tris_best_codec(c->nativeformats);
...
static void *network_thread(void *ignore)
{
	...
	if (timer)
		tris_io_add(io, tris_timer_fd(timer), timing_read, TRIS_IO_IN | TRIS_IO_PRI, NULL);
	...
	for(;;) {
		...
		count = 0;
		wakeup = -1;
		TRIS_LIST_TRAVERSE_SAFE_BEGIN(&frame_queue, f, list) {
			if (f->sentyet)
				continue;
			
			/* Try to lock the pvt, if we can't... don't fret - defer it till later */
			if (tris_mutex_trylock(&iaxsl[f->callno])) {
				wakeup = 1;
				continue;
			}
			...
			tris_mutex_unlock(&iaxsl[f->callno]);
		...
		pthread_testcancel();
		if (count >= 20)
			tris_debug(1, "chan_iax2: Sent %d queued outbound frames all at once\n", count);

		/* Now do the IO, and run scheduled tasks */
		res = tris_io_wait(io, wakeup);
		if (res >= 0) {
			if (res >= 20)
				tris_debug(1, "chan_iax2: tris_io_wait ran %d I/Os all at once\n", res);
		}
	}
	return NULL;
}
...
static int start_network_thread(void)
{
	...
	for (x = 0; x < iaxthreadcount; x++) {
		thread = tris_calloc(1, sizeof(*thread));
		if (thread) {
			thread->type = IAX_THREAD_TYPE_POOL;
			thread->threadnum = ++threadcount;
			tris_mutex_init(&thread->lock);
			tris_cond_init(&thread->cond, NULL);
			if (tris_pthread_create_detached(&thread->threadid, NULL, iax2_process_thread, thread)) {
				tris_log(LOG_WARNING, "Failed to create new thread!\n");
				tris_free(thread);
				thread = NULL;
			}
			TRIS_LIST_LOCK(&idle_list);
	...
	tris_pthread_create_background(&netthreadid, NULL, network_thread, NULL);
	tris_verb(2, "%d helper threads started\n", threadcount);
	return 0;
}
...
static struct iax2_context *build_context(const char *context)
{
	...
	if ((con = tris_calloc(1, sizeof(*con))))
		tris_copy_string(con->context, context, sizeof(con->context));
...
static int check_srcaddr(struct sockaddr *sa, socklen_t salen)
{
	...
	if (sd < 0) {
		tris_log(LOG_ERROR, "Socket: %s\n", strerror(errno));
		return -1;
	}

	res = bind(sd, sa, salen);
	if (res < 0) {
		tris_debug(1, "Can't bind: %s\n", strerror(errno));
		close(sd);
		return 1;
	}
...
static int peer_set_srcaddr(struct iax2_peer *peer, const char *srcaddr)
{
	...
	if (!tris_get_ip(&sin, addr)) {
		struct tris_netsock *sock;
		int res;
		...
		if (res == 0) {
			...
			if (!(sock = tris_netsock_find(netsock, &sin)))
				sock = tris_netsock_find(outsock, &sin);
			if (sock) {
				sockfd = tris_netsock_sockfd(sock);
				nonlocal = 0;
			} else {
				unsigned int orig_saddr = sin.sin_addr.s_addr;
				/* INADDR_ANY matches anyway! */
				sin.sin_addr.s_addr = INADDR_ANY;
				if (tris_netsock_find(netsock, &sin)) {
					sin.sin_addr.s_addr = orig_saddr;
					sock = tris_netsock_bind(outsock, io, srcaddr, port, qos.tos, qos.cos, socket_read, NULL);
					if (sock) {
						sockfd = tris_netsock_sockfd(sock);
						tris_netsock_unref(sock);
						nonlocal = 0;
					} else {
						nonlocal = 2;
					}
	...
	if (nonlocal == 1) {
		tris_log(LOG_WARNING, "Non-local or unbound address specified (%s) in sourceaddress for '%s', reverting to default\n",
			srcaddr, peer->name);
		return -1;
        } else if (nonlocal == 2) {
		tris_log(LOG_WARNING, "Unable to bind to sourceaddress '%s' for '%s', reverting to default\n",
			srcaddr, peer->name);
			return -1;
	} else {
		tris_debug(1, "Using sourceaddress %s for '%s'\n", srcaddr, peer->name);
		return 0;
	}
}
...
static void peer_destructor(void *obj)
{
	...
	if (peer->mwi_event_sub)
		tris_event_unsubscribe(peer->mwi_event_sub);
...
static struct iax2_peer *build_peer(const char *name, struct tris_variable *v, struct tris_variable *alt, int temponly)
{
	...
	if (!temponly) {
		peer = ao2_find(peers, &tmp_peer, OBJ_POINTER);
		if (peer && !tris_test_flag(peer, IAX_DELME))
			firstpass = 0;
	}
	...
	if (peer) {
		...
		if (tris_string_field_init(peer, 32))
			peer = peer_unref(peer);
	}

	if (peer) {
		if (firstpass) {
			tris_copy_flags(peer, &globalflags, IAX_USEJITTERBUF | IAX_FORCEJITTERBUF | IAX_FORCE_ENCRYPT);
			peer->encmethods = iax2_encryption;
			peer->adsi = adsi;
			tris_string_field_set(peer,secret,"");
			if (!found) {
				tris_string_field_set(peer, name, name);
			...
			tris_string_field_set(peer,context,"");
			tris_string_field_set(peer,peercontext,"");
			tris_clear_flag(peer, IAX_HASCALLERID);
			tris_string_field_set(peer, cid_name, "");
			tris_string_field_set(peer, cid_num, "");
			tris_string_field_set(peer, mohinterpret, mohinterpret);
			tris_string_field_set(peer, mohsuggest, mohsuggest);
		}
		...
		while(v) {
			if (!strcasecmp(v->name, "secret")) {
				...
				tris_string_field_set(peer, mailbox, v->value);
			} else if (!strcasecmp(v->name, "hasvoicemail")) {
				if (tris_true(v->value) && tris_strlen_zero(peer->mailbox)) {
					tris_string_field_set(peer, mailbox, name);
				}
				...
				tris_string_field_set(peer, dbsecret, v->value);
			} else if (!strcasecmp(v->name, "trunk")) {
				tris_set2_flag(peer, tris_true(v->value), IAX_TRUNK);	
				if (tris_test_flag(peer, IAX_TRUNK) && !timer) {
					tris_log(LOG_WARNING, "Unable to support trunking on peer '%s' without a timing interface\n", peer->name);
					tris_clear_flag(peer, IAX_TRUNK);
				}
				...
				if (!peer->encmethods) {
					tris_clear_flag(peer, IAX_FORCE_ENCRYPT);
				}
				...
				if (tris_false(v->value)) {
					...
					peer->encmethods |= get_encrypt_methods(v->value);
					if (peer->encmethods) {
						tris_set_flag(peer, IAX_FORCE_ENCRYPT);
					}
				}
			} else if (!strcasecmp(v->name, "transfer")) {
				if (!strcasecmp(v->value, "mediaonly")) {
					tris_set_flags_to(peer, IAX_NOTRANSFER|IAX_TRANSFERMEDIA, IAX_TRANSFERMEDIA);	
				} else if (tris_true(v->value)) {
					tris_set_flags_to(peer, IAX_NOTRANSFER|IAX_TRANSFERMEDIA, 0);
				} else 
					tris_set_flags_to(peer, IAX_NOTRANSFER|IAX_TRANSFERMEDIA, IAX_NOTRANSFER);
			} else if (!strcasecmp(v->name, "jitterbuffer")) {
				tris_set2_flag(peer, tris_true(v->value), IAX_USEJITTERBUF);	
			} else if (!strcasecmp(v->name, "forcejitterbuffer")) {
				tris_set2_flag(peer, tris_true(v->value), IAX_FORCEJITTERBUF);	
			} else if (!strcasecmp(v->name, "host")) {
				if (!strcasecmp(v->value, "dynamic")) {
					/* They'll register with us */
					tris_set_flag(peer, IAX_DYNAMIC);	
					if (!found) {
					...
					tris_sched_thread_del(sched, peer->expire);
					tris_clear_flag(peer, IAX_DYNAMIC);
					if (tris_dnsmgr_lookup(v->value, &peer->addr, &peer->dnsmgr, srvlookup ? "_iax._udp" : NULL))
				...
				peer->ha = tris_append_ha(v->name, v->value, peer->ha, NULL);
				...
				tris_string_field_set(peer, peercontext, v->value);
				...
				tris_parse_allow_disallow(&peer->prefs, &peer->capability, v->value, 1);
			} else if (!strcasecmp(v->name, "disallow")) {
				tris_parse_allow_disallow(&peer->prefs, &peer->capability, v->value, 0);
			} else if (!strcasecmp(v->name, "callerid")) {
				if (!tris_strlen_zero(v->value)) {
					char name2[80];
					char num2[80];
					tris_callerid_split(v->value, name2, sizeof(name2), num2, sizeof(num2));
					tris_string_field_set(peer, cid_name, name2);
					tris_string_field_set(peer, cid_num, num2);
				} else {
					tris_string_field_set(peer, cid_name, "");
					tris_string_field_set(peer, cid_num, "");
				}
				tris_set_flag(peer, IAX_HASCALLERID);
			} else if (!strcasecmp(v->name, "fullname")) {
				tris_string_field_set(peer, cid_name, S_OR(v->value, ""));
				tris_set_flag(peer, IAX_HASCALLERID);
			} else if (!strcasecmp(v->name, "cid_number")) {
				tris_string_field_set(peer, cid_num, S_OR(v->value, ""));
				tris_set_flag(peer, IAX_HASCALLERID);
			} else if (!strcasecmp(v->name, "sendani")) {
				tris_set2_flag(peer, tris_true(v->value), IAX_SENDANI);	
			} else if (!strcasecmp(v->name, "inkeys")) {
				tris_string_field_set(peer, inkeys, v->value);
			} else if (!strcasecmp(v->name, "outkey")) {
				tris_string_field_set(peer, outkey, v->value);
				...
				if (!strcasecmp(v->value, "no")) {
					...
					tris_log(LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
					peer->maxms = 0;
				}
			} else if (!strcasecmp(v->name, "qualifysmoothing")) {
				peer->smoothing = tris_true(v->value);
			} else if (!strcasecmp(v->name, "qualifyfreqok")) {
				if (sscanf(v->value, "%30d", &peer->pokefreqok) != 1) {
					tris_log(LOG_WARNING, "Qualification testing frequency of peer '%s' when OK should a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
				}
			} else if (!strcasecmp(v->name, "qualifyfreqnotok")) {
				if (sscanf(v->value, "%30d", &peer->pokefreqnotok) != 1) {
					tris_log(LOG_WARNING, "Qualification testing frequency of peer '%s' when NOT OK should be a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
				} else tris_log(LOG_WARNING, "Set peer->pokefreqnotok to %d\n", peer->pokefreqnotok);
			} else if (!strcasecmp(v->name, "timezone")) {
				tris_string_field_set(peer, zonetag, v->value);
			} else if (!strcasecmp(v->name, "adsi")) {
				peer->adsi = tris_true(v->value);
			} else if (!strcasecmp(v->name, "maxcallnumbers")) {
				if (sscanf(v->value, "%10hu", &peer->maxcallno) != 1) {
					tris_log(LOG_WARNING, "maxcallnumbers must be set to a valid number. %s is not valid at line %d.\n", v->value, v->lineno);
				} else {
				...
				if (tris_false(v->value)) {
					...
					tris_log(LOG_WARNING, "requirecalltoken must be set to a valid value. at line %d\n", v->lineno);
				}
			} /* else if (strcasecmp(v->name,"type")) */
			/*	tris_log(LOG_WARNING, "Ignoring %s\n", v->name); */
			v = v->next;
			if (!v) {
				v = alt;
				alt = NULL;
		...
		tris_clear_flag(peer, IAX_DELME);	
	...
	if (!tris_strlen_zero(peer->mailbox)) {
		char *mailbox, *context;
		context = mailbox = tris_strdupa(peer->mailbox);
		strsep(&context, "@");
		if (tris_strlen_zero(context))
			context = "default";
		peer->mwi_event_sub = tris_event_subscribe(TRIS_EVENT_MWI, mwi_event_cb, NULL,
			TRIS_EVENT_IE_MAILBOX, TRIS_EVENT_IE_PLTYPE_STR, mailbox,
			TRIS_EVENT_IE_CONTEXT, TRIS_EVENT_IE_PLTYPE_STR, context,
			TRIS_EVENT_IE_END);
	}
...
static void user_destructor(void *obj)
{
	...
	if(user->vars) {
		tris_variables_destroy(user->vars);
		user->vars = NULL;
	}
...
static struct iax2_user *build_user(const char *name, struct tris_variable *v, struct tris_variable *alt, int temponly)
{
	...
	if (!temponly) {
		user = ao2_find(users, &tmp_user, OBJ_POINTER);
		if (user && !tris_test_flag(user, IAX_DELME))
			firstpass = 0;
	}
	...
	if (user) {
		if (firstpass) {
			...
			tris_string_field_set(user, name, name);
			tris_string_field_set(user, language, language);
			tris_copy_flags(user, &globalflags, IAX_USEJITTERBUF | IAX_FORCEJITTERBUF | IAX_CODEC_USER_FIRST | IAX_CODEC_NOPREFS | IAX_CODEC_NOCAP | IAX_FORCE_ENCRYPT);	
			tris_clear_flag(user, IAX_HASCALLERID);
			tris_string_field_set(user, cid_name, "");
			tris_string_field_set(user, cid_num, "");
			tris_string_field_set(user, accountcode, accountcode);
			tris_string_field_set(user, mohinterpret, mohinterpret);
			tris_string_field_set(user, mohsuggest, mohsuggest);
		}
		if (!v) {
			v = alt;
			alt = NULL;
			...
			if (!strcasecmp(v->name, "context")) {
				...
				if (con) {
					...
					conl = con;
					   ...
					   !strcasecmp(v->name, "deny")) {
				user->ha = tris_append_ha(v->name, v->value, user->ha, NULL);
			} else if (!strcasecmp(v->name, "setvar")) {
				varname = tris_strdupa(v->value);
				...
				tris_parse_allow_disallow(&user->prefs, &user->capability, v->value, 1);
			} else if (!strcasecmp(v->name, "disallow")) {
				tris_parse_allow_disallow(&user->prefs, &user->capability,v->value, 0);
			} else if (!strcasecmp(v->name, "trunk")) {
				tris_set2_flag(user, tris_true(v->value), IAX_TRUNK);	
				if (tris_test_flag(user, IAX_TRUNK) && !timer) {
					tris_log(LOG_WARNING, "Unable to support trunking on user '%s' without a timing interface\n", user->name);
					tris_clear_flag(user, IAX_TRUNK);
				}
			} else if (!strcasecmp(v->name, "auth")) {
				...
				if (!user->encmethods) {
					tris_clear_flag(user, IAX_FORCE_ENCRYPT);
				}
			} else if (!strcasecmp(v->name, "forceencryption")) {
				if (tris_false(v->value)) {
					tris_clear_flag(user, IAX_FORCE_ENCRYPT);
					...
					user->encmethods |= get_encrypt_methods(v->value);
					if (user->encmethods) {
						tris_set_flag(user, IAX_FORCE_ENCRYPT);
					}
				}
			} else if (!strcasecmp(v->name, "transfer")) {
				if (!strcasecmp(v->value, "mediaonly")) {
					tris_set_flags_to(user, IAX_NOTRANSFER|IAX_TRANSFERMEDIA, IAX_TRANSFERMEDIA);	
				} else if (tris_true(v->value)) {
					tris_set_flags_to(user, IAX_NOTRANSFER|IAX_TRANSFERMEDIA, 0);
				} else 
					tris_set_flags_to(user, IAX_NOTRANSFER|IAX_TRANSFERMEDIA, IAX_NOTRANSFER);
			} else if (!strcasecmp(v->name, "codecpriority")) {
				if(!strcasecmp(v->value, "caller"))
					tris_set_flag(user, IAX_CODEC_USER_FIRST);
				else if(!strcasecmp(v->value, "disabled"))
					tris_set_flag(user, IAX_CODEC_NOPREFS);
				else if(!strcasecmp(v->value, "reqonly")) {
					tris_set_flag(user, IAX_CODEC_NOCAP);
					tris_set_flag(user, IAX_CODEC_NOPREFS);
				}
			} else if (!strcasecmp(v->name, "immediate")) {
				tris_set2_flag(user, tris_true(v->value), IAX_IMMEDIATE);
			} else if (!strcasecmp(v->name, "jitterbuffer")) {
				tris_set2_flag(user, tris_true(v->value), IAX_USEJITTERBUF);
			} else if (!strcasecmp(v->name, "forcejitterbuffer")) {
				tris_set2_flag(user, tris_true(v->value), IAX_FORCEJITTERBUF);
			} else if (!strcasecmp(v->name, "dbsecret")) {
				tris_string_field_set(user, dbsecret, v->value);
			} else if (!strcasecmp(v->name, "secret")) {
				if (!tris_strlen_zero(user->secret)) {
					char *old = tris_strdupa(user->secret);

					tris_string_field_build(user, secret, "%s;%s", old, v->value);
				} else
					tris_string_field_set(user, secret, v->value);
			} else if (!strcasecmp(v->name, "callerid")) {
				if (!tris_strlen_zero(v->value) && strcasecmp(v->value, "asreceived")) {
					char name2[80];
					char num2[80];
					tris_callerid_split(v->value, name2, sizeof(name2), num2, sizeof(num2));
					tris_string_field_set(user, cid_name, name2);
					tris_string_field_set(user, cid_num, num2);
					tris_set_flag(user, IAX_HASCALLERID);
				} else {
					tris_clear_flag(user, IAX_HASCALLERID);
					tris_string_field_set(user, cid_name, "");
					tris_string_field_set(user, cid_num, "");
				...
				if (!tris_strlen_zero(v->value)) {
					tris_string_field_set(user, cid_name, v->value);
					tris_set_flag(user, IAX_HASCALLERID);
				} else {
					tris_string_field_set(user, cid_name, "");
					if (tris_strlen_zero(user->cid_num))
						tris_clear_flag(user, IAX_HASCALLERID);
				}
			} else if (!strcasecmp(v->name, "cid_number")) {
				if (!tris_strlen_zero(v->value)) {
					tris_string_field_set(user, cid_num, v->value);
					tris_set_flag(user, IAX_HASCALLERID);
				} else {
					tris_string_field_set(user, cid_num, "");
					if (tris_strlen_zero(user->cid_name))
						tris_clear_flag(user, IAX_HASCALLERID);
				}
			} else if (!strcasecmp(v->name, "accountcode")) {
				tris_string_field_set(user, accountcode, v->value);
			} else if (!strcasecmp(v->name, "mohinterpret")) {
				tris_string_field_set(user, mohinterpret, v->value);
			} else if (!strcasecmp(v->name, "mohsuggest")) {
				tris_string_field_set(user, mohsuggest, v->value);
			} else if (!strcasecmp(v->name, "parkinglot")) {
				tris_string_field_set(user, parkinglot, v->value);
			} else if (!strcasecmp(v->name, "language")) {
				tris_string_field_set(user, language, v->value);
				...
				tris_string_field_set(user, inkeys, v->value);
			} else if (!strcasecmp(v->name, "maxauthreq")) {
				user->maxauthreq = atoi(v->value);
				if (user->maxauthreq < 0)
					user->maxauthreq = 0;
			} else if (!strcasecmp(v->name, "adsi")) {
				user->adsi = tris_true(v->value);
			} else if (!strcasecmp(v->name, "requirecalltoken")) {
				/* default is required unless in optional ip list */
				if (tris_false(v->value)) {
		...
		if (!user->authmethods) {
			if (!tris_strlen_zero(user->secret)) {
				...
				if (!tris_strlen_zero(user->inkeys))
		...
		tris_clear_flag(user, IAX_DELME);
	}
cleanup:
	if (oldha)
		tris_free_ha(oldha);
...
static void delete_users(void)
{
	...
	TRIS_LIST_LOCK(&registrations);
	while ((reg = TRIS_LIST_REMOVE_HEAD(&registrations, entry))) {
		if (sched) {
			tris_sched_thread_del(sched, reg->expire);
		}
		if (reg->callno) {
			int callno = reg->callno;
			tris_mutex_lock(&iaxsl[callno]);
...
static void prune_users(void)
{
	...
	i = ao2_iterator_init(users, 0);
	while ((user = ao2_iterator_next(&i))) {
		if (tris_test_flag(user, IAX_DELME) || tris_test_flag(user, IAX_RTCACHEFRIENDS)) {
			ao2_unlink(users, user);
		}
		user_unref(user);
	}
...
static void prune_peers(void)
{
	...
	i = ao2_iterator_init(peers, 0);
	while ((peer = ao2_iterator_next(&i))) {
		if (tris_test_flag(peer, IAX_DELME) || tris_test_flag(peer, IAX_RTCACHEFRIENDS)) {
			unlink_peer(peer);
		}
		peer_unref(peer);
	}
...
static void set_config_destroy(void)
{
	...
	amaflags = 0;
	delayreject = 0;
	tris_clear_flag((&globalflags), IAX_NOTRANSFER);	
	tris_clear_flag((&globalflags), IAX_TRANSFERMEDIA);	
	tris_clear_flag((&globalflags), IAX_USEJITTERBUF);	
	tris_clear_flag((&globalflags), IAX_FORCEJITTERBUF);	
	delete_users();
...
static int set_config(const char *config_file, int reload)
{
	...
	if (!cfg) {
		...
		if ((cfg = tris_config_load(config_file, config_flags)) == CONFIG_STATUS_FILEINVALID) {
	...
	memset(&prefs, 0 , sizeof(struct tris_codec_pref));
	
	/* Reset Global Flags */
	memset(&globalflags, 0, sizeof(globalflags));
	tris_set_flag(&globalflags, IAX_RTUPDATE);
	tris_set_flag(&globalflags, IAX_SHRINKCALLERID);
	...
	tosval = tris_variable_retrieve(cfg, "general", "tos");
	if (tosval) {
		if (tris_str2tos(tosval, &qos.tos))
			tris_log(LOG_WARNING, "Invalid tos value, refer to QoS documentation\n");
	}
	/* Seed initial cos value */
	tosval = tris_variable_retrieve(cfg, "general", "cos");
	if (tosval) {
		if (tris_str2cos(tosval, &qos.cos))
			tris_log(LOG_WARNING, "Invalid cos value, refer to QoS documentation\n");
	}
	while(v) {
		if (!strcasecmp(v->name, "bindport")){ 
			if (reload)
				tris_log(LOG_NOTICE, "Ignoring bindport on reload\n");
			else
				portno = atoi(v->value);
			...
			if (reload) {
				if (atoi(v->value) != iaxthreadcount)
					tris_log(LOG_NOTICE, "Ignoring any changes to iaxthreadcount during reload\n");
			} else {
				iaxthreadcount = atoi(v->value);
				if (iaxthreadcount < 1) {
					tris_log(LOG_NOTICE, "iaxthreadcount must be at least 1.\n");
					iaxthreadcount = 1;
				} else if (iaxthreadcount > 256) {
					tris_log(LOG_NOTICE, "limiting iaxthreadcount to 256\n");
					iaxthreadcount = 256;
				}
			}
			...
			if (reload) {
				...
				if (iaxmaxthreadcount < 0) {
					tris_log(LOG_NOTICE, "iaxmaxthreadcount must be at least 0.\n");
					iaxmaxthreadcount = 0;
				} else if (iaxmaxthreadcount > 256) {
					tris_log(LOG_NOTICE, "Limiting iaxmaxthreadcount to 256\n");
...
#else
			if (tris_true(v->value))
				tris_log(LOG_WARNING, "Disabling RTP checksums is not supported on this operating system!\n");
#endif
		}
			...
			if (reload) {
				tris_log(LOG_NOTICE, "Ignoring bindaddr on reload\n");
			} else {
				if (!(ns = tris_netsock_bind(netsock, io, v->value, portno, qos.tos, qos.cos, socket_read, NULL))) {
					tris_log(LOG_WARNING, "Unable apply binding to '%s' at line %d\n", v->value, v->lineno);
				} else {
						if (strchr(v->value, ':'))
						tris_verb(2, "Binding IAX2 to '%s'\n", v->value);
						else
						tris_verb(2, "Binding IAX2 to '%s:%d'\n", v->value, portno);
					if (defaultsockfd < 0) 
						defaultsockfd = tris_netsock_sockfd(ns);
					tris_netsock_unref(ns);
				}
			}
		} else if (!strcasecmp(v->name, "authdebug")) {
			authdebug = tris_true(v->value);
		} else if (!strcasecmp(v->name, "encryption")) {
				iax2_encryption |= get_encrypt_methods(v->value);
				if (!iax2_encryption) {
					tris_clear_flag((&globalflags), IAX_FORCE_ENCRYPT);
				}
		} else if (!strcasecmp(v->name, "forceencryption")) {
			if (tris_false(v->value)) {
				tris_clear_flag((&globalflags), IAX_FORCE_ENCRYPT);
				...
				iax2_encryption |= get_encrypt_methods(v->value);
				if (iax2_encryption) {
					tris_set_flag((&globalflags), IAX_FORCE_ENCRYPT);
				}
			}
		} else if (!strcasecmp(v->name, "transfer")) {
			if (!strcasecmp(v->value, "mediaonly")) {
				tris_set_flags_to((&globalflags), IAX_NOTRANSFER|IAX_TRANSFERMEDIA, IAX_TRANSFERMEDIA);	
			} else if (tris_true(v->value)) {
				tris_set_flags_to((&globalflags), IAX_NOTRANSFER|IAX_TRANSFERMEDIA, 0);
			} else 
				tris_set_flags_to((&globalflags), IAX_NOTRANSFER|IAX_TRANSFERMEDIA, IAX_NOTRANSFER);
		} else if (!strcasecmp(v->name, "codecpriority")) {
			if(!strcasecmp(v->value, "caller"))
				tris_set_flag((&globalflags), IAX_CODEC_USER_FIRST);
			else if(!strcasecmp(v->value, "disabled"))
				tris_set_flag((&globalflags), IAX_CODEC_NOPREFS);
			else if(!strcasecmp(v->value, "reqonly")) {
				tris_set_flag((&globalflags), IAX_CODEC_NOCAP);
				tris_set_flag((&globalflags), IAX_CODEC_NOPREFS);
			}
		} else if (!strcasecmp(v->name, "jitterbuffer"))
			tris_set2_flag((&globalflags), tris_true(v->value), IAX_USEJITTERBUF);	
		else if (!strcasecmp(v->name, "forcejitterbuffer"))
			tris_set2_flag((&globalflags), tris_true(v->value), IAX_FORCEJITTERBUF);	
		else if (!strcasecmp(v->name, "delayreject"))
			delayreject = tris_true(v->value);
		else if (!strcasecmp(v->name, "allowfwdownload"))
			tris_set2_flag((&globalflags), tris_true(v->value), IAX_ALLOWFWDOWNLOAD);
		else if (!strcasecmp(v->name, "rtcachefriends"))
			tris_set2_flag((&globalflags), tris_true(v->value), IAX_RTCACHEFRIENDS);	
		else if (!strcasecmp(v->name, "rtignoreregexpire"))
			tris_set2_flag((&globalflags), tris_true(v->value), IAX_RTIGNOREREGEXPIRE);	
		else if (!strcasecmp(v->name, "rtupdate"))
			tris_set2_flag((&globalflags), tris_true(v->value), IAX_RTUPDATE);
		else if (!strcasecmp(v->name, "trunktimestamps"))
			tris_set2_flag(&globalflags, tris_true(v->value), IAX_TRUNKTIMESTAMPS);
			...
			if(i > 0)
				...
				i = 0;
			tris_set2_flag((&globalflags), i || tris_true(v->value), IAX_RTAUTOCLEAR);	
			...
			if (mtuv  == 0 )  
				...
				tris_log(LOG_NOTICE, "trunkmtu value out of bounds (%d) at line %d\n",
					mtuv, v->lineno);
			...
			if (sscanf(v->value, "%30d", &x) == 1) {
				if (x >= 0)
					...
					tris_log(LOG_NOTICE, "Nice try, but autokill has to be >0 or 'yes' or 'no' at line %d\n", v->lineno);
			} else if (tris_true(v->value)) {
			...
			if (!strcasecmp(v->value, "low")) {
				...
				tris_log(LOG_WARNING, "bandwidth must be either low, medium, or high\n");
		} else if (!strcasecmp(v->name, "allow")) {
			tris_parse_allow_disallow(&prefs, &capability, v->value, 1);
		} else if (!strcasecmp(v->name, "disallow")) {
			tris_parse_allow_disallow(&prefs, &capability, v->value, 0);
		} else if (!strcasecmp(v->name, "register")) {
			iax2_register(v->value, v->lineno);
		} else if (!strcasecmp(v->name, "iaxcompat")) {
			iaxcompat = tris_true(v->value);
		} else if (!strcasecmp(v->name, "regcontext")) {
			tris_copy_string(regcontext, v->value, sizeof(regcontext));
			/* Create context if it doesn't exist already */
			tris_context_find_or_create(NULL, NULL, regcontext, "IAX2");
			...
			tris_copy_string(default_parkinglot, v->value, sizeof(default_parkinglot));
		} else if (!strcasecmp(v->name, "accountcode")) {
			tris_copy_string(accountcode, v->value, sizeof(accountcode));
		} else if (!strcasecmp(v->name, "mohinterpret")) {
			tris_copy_string(mohinterpret, v->value, sizeof(mohinterpret));
		} else if (!strcasecmp(v->name, "mohsuggest")) {
			tris_copy_string(mohsuggest, v->value, sizeof(mohsuggest));
		} else if (!strcasecmp(v->name, "amaflags")) {
			format = tris_cdr_amaflags2int(v->value);
			if (format < 0) {
				tris_log(LOG_WARNING, "Invalid AMA Flags: %s at line %d\n", v->value, v->lineno);
			...
			if (sscanf(v->value, "%10hu", &global_maxcallno_nonval) != 1) {
				tris_log(LOG_WARNING, "maxcallnumbers_nonvalidated must be set to a valid number.  %s is not valid at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "calltokenoptional")) {
			if (add_calltoken_ignore(v->value)) {
				tris_log(LOG_WARNING, "Invalid calltokenoptional address range - '%s' line %d\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "shrinkcallerid")) {
			if (tris_true(v->value)) {
				tris_set_flag((&globalflags), IAX_SHRINKCALLERID);
			} else if (tris_false(v->value)) {
				tris_clear_flag((&globalflags), IAX_SHRINKCALLERID);
			} else {
	...
	if (defaultsockfd < 0) {
		if (!(ns = tris_netsock_bind(netsock, io, "0.0.0.0", portno, qos.tos, qos.cos, socket_read, NULL))) {
			tris_log(LOG_ERROR, "Unable to create network socket: %s\n", strerror(errno));
		} else {
			tris_verb(2, "Binding IAX2 to default address 0.0.0.0:%d\n", portno);
			defaultsockfd = tris_netsock_sockfd(ns);
			tris_netsock_unref(ns);
		}
	}
	if (reload) {
		tris_netsock_release(outsock);
		outsock = tris_netsock_list_alloc();
		if (!outsock) {
			tris_log(LOG_ERROR, "Could not allocate outsock list.\n");
			return -1;
		}
		tris_netsock_init(outsock);
	}

	if (min_reg_expire > max_reg_expire) {
		tris_log(LOG_WARNING, "Minimum registration interval of %d is more than maximum of %d, resetting minimum to %d\n",
	...
	if (ucfg) {
		...
		genhasiax = tris_true(tris_variable_retrieve(ucfg, "general", "hasiax"));
		genregisteriax = tris_true(tris_variable_retrieve(ucfg, "general", "registeriax"));
		gen = tris_variable_browse(ucfg, "general");
		cat = tris_category_browse(ucfg, NULL);
		while (cat) {
			if (strcasecmp(cat, "general")) {
				hasiax = tris_variable_retrieve(ucfg, cat, "hasiax");
				registeriax = tris_variable_retrieve(ucfg, cat, "registeriax");
				if (tris_true(hasiax) || (!hasiax && genhasiax)) {
				...
				if (tris_true(registeriax) || (!registeriax && genregisteriax)) {
					char tmp[256];
	...
	while(cat) {
		if (strcasecmp(cat, "general")) {
			...
			if (!strcasecmp(cat, "callnumberlimits")) {
				build_callno_limits(tris_variable_browse(cfg, cat));
			} else if (utype) {
...
static int reload_config(void)
{
	...
	tris_unload_realtime("iaxpeers");

	return 0;
}

static char *handle_cli_iax2_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static int cache_get_callno_locked(const char *data)
{
	...
	tris_debug(1, "peer: %s, username: %s, password: %s, context: %s\n",
	...
	tris_string_field_set(iaxs[callno], dproot, data);
...
static struct iax2_dpcache *find_cache(struct tris_channel *chan, const char *data, const char *context, const char *exten, int priority)
{
	struct iax2_dpcache *dp = NULL;
	struct timeval now = tris_tvnow();
	...
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&dpcache, dp, cache_list) {
		if (tris_tvcmp(now, dp->expiry) > 0) {
			TRIS_LIST_REMOVE_CURRENT(cache_list);
			if ((dp->flags & CACHE_FLAG_PENDING) || dp->callno)
				tris_log(LOG_WARNING, "DP still has peer field or pending or callno (flags = %d, peer = blah, callno = %d)\n", dp->flags, dp->callno);
			else
				tris_free(dp);
			continue;
		}
	...
	if (!dp) {
		...
		if ((callno = cache_get_callno_locked(data)) < 0) {
			tris_log(LOG_WARNING, "Unable to generate call for '%s'\n", data);
			return NULL;
		}
		if (!(dp = tris_calloc(1, sizeof(*dp)))) {
			tris_mutex_unlock(&iaxsl[callno]);
			return NULL;
		}
		tris_copy_string(dp->peercontext, data, sizeof(dp->peercontext));
		tris_copy_string(dp->exten, exten, sizeof(dp->exten));
		dp->expiry = tris_tvnow();
		dp->orig = dp->expiry;
		...
		TRIS_LIST_INSERT_TAIL(&dpcache, dp, cache_list);
		TRIS_LIST_INSERT_TAIL(&iaxs[callno]->dpentries, dp, peer_list);
		/* Send the request if we're already up */
		if (tris_test_flag(&iaxs[callno]->state, IAX_STATE_STARTED))
	...
	if (dp->flags & CACHE_FLAG_PENDING) {
		...
		if (x >= ARRAY_LEN(dp->waiters)) {
			tris_log(LOG_WARNING, "No more waiter positions available\n");
			return NULL;
		}
		if (pipe(com)) {
			tris_log(LOG_WARNING, "Unable to create pipe for comm\n");
			return NULL;
		}
		dp->waiters[x] = com[1];
		/* Okay, now we wait */
		...
		/* Defer any dtmf */
		if (chan)
			old = tris_channel_defer_dtmf(chan);
		doabort = 0;
		while(timeout) {
			c = tris_waitfor_nandfds(&chan, chan ? 1 : 0, &com[0], 1, NULL, &outfd, &timeout);
			if (outfd > -1)
				break;
			if (!c)
				continue;
		...
		if (!timeout) {
			tris_log(LOG_WARNING, "Timeout waiting for %s exten %s\n", data, exten);
		}
		TRIS_LIST_LOCK(&dpcache);
		dp->waiters[x] = -1;
		close(com[1]);
...
static int iax2_exists(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	int res = 0;
	struct iax2_dpcache *dp = NULL;
#if 0
	tris_log(LOG_NOTICE, "iax2_exists: con: %s, exten: %s, pri: %d, cid: %s, data: %s\n", context, exten, priority, callerid ? callerid : "<unknown>", data);
#endif
	...
	if ((dp = find_cache(chan, data, context, exten, priority))) {
		...
		tris_log(LOG_WARNING, "Unable to make DP cache\n");
	}
	TRIS_LIST_UNLOCK(&dpcache);
...
static int iax2_canmatch(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	int res = 0;
	struct iax2_dpcache *dp = NULL;
#if 0
	tris_log(LOG_NOTICE, "iax2_canmatch: con: %s, exten: %s, pri: %d, cid: %s, data: %s\n", context, exten, priority, callerid ? callerid : "<unknown>", data);
#endif
	if ((priority != 1) && (priority != 2))
		return 0;
...
/*! \brief Part of the IAX2 Switch interface */
static int iax2_matchmore(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	int res = 0;
	struct iax2_dpcache *dp = NULL;
#if 0
	tris_log(LOG_NOTICE, "iax2_matchmore: con: %s, exten: %s, pri: %d, cid: %s, data: %s\n", context, exten, priority, callerid ? callerid : "<unknown>", data);
#endif
	if ((priority != 1) && (priority != 2))
		return 0;
...
static int iax2_exec(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	char odata[256];
	char req[256];
	char *ncontext;
	struct iax2_dpcache *dp = NULL;
	struct tris_app *dial = NULL;
#if 0
	tris_log(LOG_NOTICE, "iax2_exec: con: %s, exten: %s, pri: %d, cid: %s, data: %s, newstack: %d\n", context, exten, priority, callerid ? callerid : "<unknown>", data, newstack);
#endif
	if (priority == 2) {
	...
	if ((dp = find_cache(chan, data, context, exten, priority))) {
		if (dp->flags & CACHE_FLAG_EXISTS) {
			tris_copy_string(odata, data, sizeof(odata));
			...
			tris_verb(3, "Executing Dial('%s')\n", req);
		} else {
			TRIS_LIST_UNLOCK(&dpcache);
			tris_log(LOG_WARNING, "Can't execute nonexistent extension '%s[@%s]' in data '%s'\n", exten, context, data);
			return -1;
		}
	}
	TRIS_LIST_UNLOCK(&dpcache);
	...
	if ((dial = pbx_findapp("Dial")))
		...
		tris_log(LOG_WARNING, "No dial application registered\n");

	return -1;
}

static int function_iaxpeer(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct iax2_peer *peer;
	char *peername, *colname;

	peername = tris_strdupa(data);
	...
	if (!strcmp(peername,"CURRENTCHANNEL")) {
		...
		tris_copy_string(buf, iaxs[callno]->addr.sin_addr.s_addr ? tris_inet_ntoa(iaxs[callno]->addr.sin_addr) : "", len);
		return 0;
	}
	...
	if (!strcasecmp(colname, "ip")) {
		...
		tris_copy_string(buf, peer->mailbox, len);
	} else  if (!strcasecmp(colname, "context")) {
		tris_copy_string(buf, peer->context, len);
	} else  if (!strcasecmp(colname, "expire")) {
		snprintf(buf, len, "%d", peer->expire);
	} else  if (!strcasecmp(colname, "dynamic")) {
		tris_copy_string(buf, (tris_test_flag(peer, IAX_DYNAMIC) ? "yes" : "no"), len);
...
struct tris_custom_function iaxpeer_function = {
	.name = "IAXPEER",
	.read = function_iaxpeer,
};

static int acf_channel_read(struct tris_channel *chan, const char *funcname, char *args, char *buf, size_t buflen)
{
	struct chan_iax2_pvt *pvt;
	unsigned int callno;
	int res = 0;
	...
	if (!chan || chan->tech != &iax2_tech) {
		tris_log(LOG_ERROR, "This function requires a valid IAX2 channel\n");
		return -1;
	}
	...
	if (!strcasecmp(args, "osptoken")) {
		tris_copy_string(buf, pvt->osptoken, buflen);
	} else if (!strcasecmp(args, "peerip")) {
		tris_copy_string(buf, pvt->addr.sin_addr.s_addr ? tris_inet_ntoa(pvt->addr.sin_addr) : "", buflen);
	} else if (!strcasecmp(args, "peername")) {
		tris_copy_string(buf, pvt->username, buflen);
	} else {
		res = -1;
	}
...
static int iax2_devicestate(void *data) 
{
	...
	tris_debug(3, "Checking device state for device %s\n", pds.peer);
	...
	tris_debug(3, "iax2_devicestate: Found peer. What's device state of %s? addr=%d, defaddr=%d maxms=%d, lastms=%d\n",
...
static struct tris_switch iax2_switch = 
{
	name:        "IAX2",
...
static struct tris_cli_entry cli_iax2[] = {
	TRIS_CLI_DEFINE(handle_cli_iax2_provision,           "Provision an IAX device"),
	TRIS_CLI_DEFINE(handle_cli_iax2_prune_realtime,      "Prune a cached realtime lookup"),
	TRIS_CLI_DEFINE(handle_cli_iax2_reload,              "Reload IAX configuration"),
	TRIS_CLI_DEFINE(handle_cli_iax2_set_mtu,             "Set the IAX systemwide trunking MTU"),
	TRIS_CLI_DEFINE(handle_cli_iax2_set_debug,           "Enable/Disable IAX debugging"),
	TRIS_CLI_DEFINE(handle_cli_iax2_set_debug_trunk,     "Enable/Disable IAX trunk debugging"),
	TRIS_CLI_DEFINE(handle_cli_iax2_set_debug_jb,        "Enable/Disable IAX jitterbuffer debugging"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_cache,          "Display IAX cached dialplan"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_channels,       "List active IAX channels"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_firmware,       "List available IAX firmware"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_netstats,       "List active IAX channel netstats"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_peer,           "Show details on specific IAX peer"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_peers,          "List defined IAX peers"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_registry,       "Display IAX registration status"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_stats,          "Display IAX statistics"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_threads,        "Display IAX helper thread info"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_users,          "List defined IAX users"),
	TRIS_CLI_DEFINE(handle_cli_iax2_test_losspct,        "Set IAX2 incoming frame loss percentage"),
	TRIS_CLI_DEFINE(handle_cli_iax2_unregister,          "Unregister (force expiration) an IAX2 peer from the registry"),
	TRIS_CLI_DEFINE(handle_cli_iax2_show_callno_limits,  "Show current entries in IP call number limit table"),
#ifdef IAXTESTS
	TRIS_CLI_DEFINE(handle_cli_iax2_test_jitter,         "Simulates jitter for testing"),
	TRIS_CLI_DEFINE(handle_cli_iax2_test_late,           "Test the receipt of a late frame"),
	TRIS_CLI_DEFINE(handle_cli_iax2_test_resync,         "Test a resync in received timestamps"),
#endif /* IAXTESTS */
};
...
static int __unload_module(void)
{
	...
	sched = tris_sched_thread_destroy(sched);
	
	/* Call for all threads to halt */
	TRIS_LIST_LOCK(&idle_list);
	while ((thread = TRIS_LIST_REMOVE_HEAD(&idle_list, list)))
		pthread_cancel(thread->threadid);
	TRIS_LIST_UNLOCK(&idle_list);

	TRIS_LIST_LOCK(&active_list);
	while ((thread = TRIS_LIST_REMOVE_HEAD(&active_list, list)))
		pthread_cancel(thread->threadid);
	TRIS_LIST_UNLOCK(&active_list);

	TRIS_LIST_LOCK(&dynamic_list);
	while ((thread = TRIS_LIST_REMOVE_HEAD(&dynamic_list, list)))
		pthread_cancel(thread->threadid);
	TRIS_LIST_UNLOCK(&dynamic_list);
	...
	tris_manager_unregister( "IAXpeers" );
	tris_manager_unregister( "IAXpeerlist" );
	tris_manager_unregister( "IAXnetstats" );
	tris_manager_unregister( "IAXregistry" );
	tris_unregister_application(papp);
	tris_cli_unregister_multiple(cli_iax2, ARRAY_LEN(cli_iax2));
	tris_unregister_switch(&iax2_switch);
	tris_channel_unregister(&iax2_tech);
	delete_users();
	iax_provision_unload();
	reload_firmware(1);

	for (x = 0; x < ARRAY_LEN(iaxsl); x++) {
		tris_mutex_destroy(&iaxsl[x]);
	}
	...
	con = tris_context_find(regcontext);
	if (con)
		tris_context_destroy(con, "IAX2");
	tris_unload_realtime("iaxpeers");
	return 0;
}
...
{
	tris_custom_function_unregister(&iaxpeer_function);
	tris_custom_function_unregister(&iaxvar_function);
...
static int load_module(void)
{
	...
	randomcalltokendata = tris_random();
	tris_custom_function_register(&iaxpeer_function);
	tris_custom_function_register(&iaxvar_function);
	...
	for (x = 0; x < ARRAY_LEN(iaxsl); x++) {
		tris_mutex_init(&iaxsl[x]);
	}

	if (!(sched = tris_sched_thread_create())) {
		tris_log(LOG_ERROR, "Failed to create scheduler thread\n");
		return TRIS_MODULE_LOAD_FAILURE;
	}

	if (!(io = io_context_create())) {
		tris_log(LOG_ERROR, "Failed to create I/O context\n");
		sched = tris_sched_thread_destroy(sched);
		return TRIS_MODULE_LOAD_FAILURE;
	}

	if (!(netsock = tris_netsock_list_alloc())) {
		tris_log(LOG_ERROR, "Failed to create netsock list\n");
		io_context_destroy(io);
		sched = tris_sched_thread_destroy(sched);
		return TRIS_MODULE_LOAD_FAILURE;
	}
	tris_netsock_init(netsock);
	...
	tris_cli_register_multiple(cli_iax2, ARRAY_LEN(cli_iax2));

	tris_register_application_xml(papp, iax2_prov_app);
	
	tris_manager_register( "IAXpeers", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_iax2_show_peers, "List IAX Peers" );
	tris_manager_register( "IAXpeerlist", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_iax2_show_peer_list, "List IAX Peers" );
	tris_manager_register( "IAXnetstats", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_iax2_show_netstats, "Show IAX Netstats" );
	tris_manager_register( "IAXregistry", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_iax2_show_registry, "Show IAX registrations");

	if ((timer = tris_timer_open())) {
		tris_timer_set_rate(timer, trunkfreq);
	}
	...
	if (set_config(config, 0) == -1) {
 	...
 	if (tris_channel_register(&iax2_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel class %s\n", "IAX2");
		__unload_module();
		return TRIS_MODULE_LOAD_FAILURE;
	}

	if (tris_register_switch(&iax2_switch)) 
		tris_log(LOG_ERROR, "Unable to register IAX switch\n");

	if (start_network_thread()) {
		tris_log(LOG_ERROR, "Unable to start network thread\n");
		__unload_module();
		return TRIS_MODULE_LOAD_FAILURE;
	} else
		tris_verb(2, "IAX Ready and Listening\n");
	...
	tris_realtime_require_field("iaxpeers", "name", RQ_CHAR, 10, "ipaddr", RQ_CHAR, 15, "port", RQ_UINTEGER2, 5, "regseconds", RQ_UINTEGER2, 6, SENTINEL);

	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Inter Trismedia eXchange (Ver 2)",
		.load = load_module,
