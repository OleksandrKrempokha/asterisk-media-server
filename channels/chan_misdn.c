 ...
 * \file
 *
 * \brief the chan_misdn channel driver for Trismedia
 *
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 248004 $")
...
static char *complete_ch(struct tris_cli_args *a);
static char *complete_debug_port(struct tris_cli_args *a);
static char *complete_show_config(struct tris_cli_args *a);

/* BEGIN: chan_misdn.h */

tris_mutex_t release_lock;

enum misdn_chan_state {
	MISDN_NOTHING = 0,         /*!< at beginning */
	MISDN_WAITING4DIGS,        /*!< when waiting for info */
	MISDN_EXTCANTMATCH,        /*!< when trismedia couldn't match our ext */
...
struct chan_list {
	/*! 
	 * \brief The "allowed_bearers" string read in from /etc/trismedia/misdn.conf
	 */
	...
	/*!
	 * \brief TRUE if a channel can be hung up by calling trismedia directly when done.
	 */
	int need_hangup;

	/*!
	 * \brief TRUE if we could send an TRIS_CONTROL_BUSY if needed.
	 */
	int need_busy;
	 ...
	 * \note The "noautorespond_on_setup" boolean read in from /etc/trismedia/misdn.conf
	 */
	 ...
	 * \note The "incoming_early_audio" boolean read in from /etc/trismedia/misdn.conf
	 */
	 ...
	 * \note The "faxdetect" option string read in from /etc/trismedia/misdn.conf
	 * \note It is settable by the misdn_set_opt() application.
	 */
	int faxdetect;
	 ...
	 * \note The "faxdetect_timeout" value read in from /etc/trismedia/misdn.conf
	 */
	...
	/*!
	 * \brief TRUE if we will use the Trismedia DSP to detect DTMF/Fax
	 * \note The "astdtmf" boolean read in from /etc/trismedia/misdn.conf
	 */
	int tris_dsp;

	/*!
	 * \brief Jitterbuffer length
	 * \note The "jitterbuffer" value read in from /etc/trismedia/misdn.conf
	 */
	int jb_len;

	/*!
	 * \brief Jitterbuffer upper threshold
	 * \note The "jitterbuffer_upper_threshold" value read in from /etc/trismedia/misdn.conf
	 */
	 ...
	 * \note tris_dsp_new() creates the DSP controller.
	 * \note Must use tris_dsp_free() to clean up. 
	 */
	struct tris_dsp *dsp;

	/*!
	 * \brief Associated Trismedia channel structure.
	 */
	 ...
	 * \note The "context" string read in from /etc/trismedia/misdn.conf
	 */
	char context[TRIS_MAX_CONTEXT];

	/*!
	 * \brief The configured music-on-hold class to use for this call.
	 * \note The "musicclass" string read in from /etc/trismedia/misdn.conf
	 */
	 ...
	 * \note The "far_alerting" boolean read in from /etc/trismedia/misdn.conf
	 */
	int far_alerting;

	/*!
	 * \brief TRUE if NT should disconnect an overlap dialing call when a timeout occurs.
	 * \note The "nttimeout" boolean read in from /etc/trismedia/misdn.conf
	 */
	int nttimeout;

	/*!
	 * \brief Other channel call record PID 
	 * \note Value imported from Trismedia environment variable MISDN_PID 
	 */
	int other_pid;

	/*!
	 * \brief Bridged other channel call record
	 * \note Pointer set when other_pid imported from Trismedia environment 
	 ...
	 * \note The "overlapdial" value read in from /etc/trismedia/misdn.conf
	 */
	int overlap_dial;
...
void export_ch(struct tris_channel *chan, struct misdn_bchannel *bc, struct chan_list *ch);
void import_ch(struct tris_channel *chan, struct misdn_bchannel *bc, struct chan_list *ch);
static struct tris_frame *process_tris_dsp(struct chan_list *tmp, struct tris_frame *frame);
...
static struct robin_list* get_robin_position(char *group) 
{
	...
	new = tris_calloc(1, sizeof(*new));
	if (!new) {
		return NULL;
	}
	new->group = tris_strdup(group);
	if (!new->group) {
		tris_free(new);
		return NULL;
	}
...
static struct tris_channel *misdn_new(struct chan_list *cl, int state,  char *exten, char *callerid, int format, int port, int c);
static void send_digit_to_chan(struct chan_list *cl, char digit );

static int pbx_start_chan(struct chan_list *ch);

#define MISDN_TRISMEDIA_TECH_PVT(ast) ast->tech_pvt
#define MISDN_TRISMEDIA_PVT(ast) 1
...
static int prefformat =  TRIS_FORMAT_ALAW ; /*  TRIS_FORMAT_SLINEAR ;  TRIS_FORMAT_ULAW | */
...
tris_mutex_t cl_te_lock;

static enum event_response_e
cb_events(enum event_e event, struct misdn_bchannel *bc, void *user_data);

static void send_cause2ast(struct tris_channel *ast, struct misdn_bchannel*bc, struct chan_list *ch);
...
static int misdn_check_l2l1(struct tris_channel *chan, void *data);
static int misdn_set_opt_exec(struct tris_channel *chan, void *data);
static int misdn_facility_exec(struct tris_channel *chan, void *data);
...
static struct chan_list * get_chan_by_ast(struct tris_channel *ast)
{
...
static struct chan_list * get_chan_by_tris_name(char *name)
{
	struct chan_list *tmp;
...
static void export_aoc_vars(int originator, struct tris_channel *ast, struct misdn_bchannel *bc)
{
	char buf[128];
	...
	if (originator == ORG_AST) {
		if (!(ast = tris_bridged_channel(ast))) {
			return;
		}
	}
...
static void *misdn_tasks_thread_func(void *data)
{
	...
	while (1) {
		wait = tris_sched_wait(misdn_tasks);
		if (wait < 0) {
			wait = 8000;
		}
...
static inline int _misdn_tasks_add_variable(int timeout, tris_sched_cb callback, const void *data, int variable)
{
	int task_id;
	...
	task_id = tris_sched_add_variable(misdn_tasks, timeout, callback, data, variable);
	misdn_tasks_wakeup();

	return task_id;
}

static int misdn_tasks_add(int timeout, tris_sched_cb callback, const void *data)
{
	return _misdn_tasks_add_variable(timeout, callback, data, 0);
}

static int misdn_tasks_add_variable(int timeout, tris_sched_cb callback, const void *data)
{
...
{
	TRIS_SCHED_DEL(misdn_tasks, task_id);
}
...
static int misdn_overlap_dial_task(const void *data)
{
	...
	tris_mutex_lock(&ch->overlap_tv_lock);
	tv_end = ch->overlap_tv;
	tris_mutex_unlock(&ch->overlap_tv_lock);

	tv_end.tv_sec += ch->overlap_dial;
	tv_now = tris_tvnow();

	if ((diff = tris_tvdiff_ms(tv_end, tv_now)) > 100) {
		return diff;
	}
	...
	if (tris_strlen_zero(ch->bc->dad)) {
		dad = "s";
		tris_copy_string(ch->ast->exten, "s", sizeof(ch->ast->exten));
	} else {
		dad = ch->bc->dad;
	}

	if (tris_exists_extension(ch->ast, ch->context, dad, 1, ch->bc->oad)) {
		ch->state = MISDN_DIALING;
		if (pbx_start_chan(ch) < 0) {
			chan_misdn_log(-1, ch->bc->port, "tris_pbx_start returned < 0 in misdn_overlap_dial_task\n");
			goto misdn_overlap_dial_task_disconnect;
		}
	} else {
misdn_overlap_dial_task_disconnect:
		hanguptone_indicate(ch);
		ch->bc->out_cause = TRIS_CAUSE_UNALLOCATED;
...
static void send_digit_to_chan(struct chan_list *cl, char digit)
{
	...
	struct tris_channel *chan = cl->ast; 
	...
	if (digit >= '0' && digit <='9') {
		...
		/* not handled */
		tris_debug(1, "Unable to handle DTMF tone '%c' for '%s'\n", digit, chan->name);
	}
}

/*** CLI HANDLING ***/
static char *handle_cli_misdn_set_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int level;
	...
	case 5:
		{
			...
			tris_cli(a->fd, "changing debug level for all ports to %d%s\n", misdn_debug[0], only ? " (only)" : "");
		}
		break;
	case 6:
	case 7:
		{
			...
			if (port <= 0 || port > max_ports) {
				switch (max_ports) {
				case 0:
					tris_cli(a->fd, "port number not valid! no ports available so you won't get lucky with any number here...\n");
					break;
				case 1:
					tris_cli(a->fd, "port number not valid! only port 1 is available.\n");
					break;
				default:
					tris_cli(a->fd, "port number not valid! only ports 1 to %d are available.\n", max_ports);
				}
				return 0;
			}
			if (a->argc == 7) {
			...
			tris_cli(a->fd, "changing debug level to %d%s for port %d\n", misdn_debug[port], misdn_debug_only[port] ? " (only)" : "", port);
		}
	}
...
static char *handle_cli_misdn_set_crypt_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_port_block(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_port_unblock(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_restart_port(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_restart_pid(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_port_up(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_port_down(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static inline void show_config_description(int fd, enum misdn_cfg_elements elem)
{
	...
	if (*def) {
		tris_cli(fd, "[%s] %s   (Default: %s)\n\t%s\n", section, name, def, desc);
	} else {
		tris_cli(fd, "[%s] %s\n\t%s\n", section, name, desc);
	}
}

static char *handle_cli_misdn_show_config(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
		...
		if (!strcmp(a->argv[3], "description")) {
			if (a->argc == 5) {
				...
				if (elem == MISDN_CFG_FIRST) {
					tris_cli(a->fd, "Unknown element: %s\n", a->argv[4]);
				} else {
			...
			tris_cli(a->fd, "Unknown option: %s\n", a->argv[3]);
			return CLI_SHOWUSAGE;
		}
	} else if (a->argc == 3 || onlyport == 0) {
		tris_cli(a->fd, "mISDN General-Config:\n");
		for (elem = MISDN_GEN_FIRST + 1, linebreak = 1; elem < MISDN_GEN_LAST; elem++, linebreak++) {
			misdn_cfg_get_config_string(0, elem, buffer, sizeof(buffer));
			tris_cli(a->fd, "%-36s%s", buffer, !(linebreak % 2) ? "\n" : "");
		}
		tris_cli(a->fd, "\n");
	}
	...
	if (onlyport > 0) {
		if (misdn_cfg_is_port_valid(onlyport)) {
			...
			tris_cli(a->fd, "Port %d is not active!\n", onlyport);
		}
	}
...
static struct state_struct state_array[] = {
	...
	{ MISDN_EXTCANTMATCH,        "EXTCANTMATCH" },        /*  when trismedia couldn't match our ext */
...
static void reload_config(void)
{
	...
	if (!g_config_initialized) {
		tris_log(LOG_WARNING, "chan_misdn is not initialized properly, still reloading ?\n");
		return ;
	}
...
static char *handle_cli_misdn_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	tris_cli(a->fd, "Reloading mISDN configuration\n");
	reload_config();
	return CLI_SUCCESS;
}
...
{
	struct tris_channel *ast = help->ast;
	tris_cli(fd,
...
static char *handle_cli_misdn_show_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	for (; help; help = help->next) {
		...
		if (!ast) {
			if (!bc) {
				tris_cli(a->fd, "chan_list obj. with l3id:%x has no bc and no ast Leg\n", help->l3id);
				continue;
			}
			tris_cli(a->fd, "bc with pid:%d has no Ast Leg\n", bc->pid);
			continue;
		}

		if (misdn_debug[0] > 2) {
			tris_cli(a->fd, "Bc:%p Ast:%p\n", bc, ast);
		}
		if (bc) {
			print_bc_info(a->fd, help, bc);
		} else {
			if (help->hold.state != MISDN_HOLD_IDLE) {
				tris_cli(a->fd, "ITS A HELD CALL BC:\n");
				tris_cli(a->fd, " --> l3_id: %x\n"
						" --> dad:%s oad:%s\n"
						" --> hold_port: %d\n"
						" --> hold_channel: %d\n",
						help->l3id,
				...
				tris_cli(a->fd, "* Channel in unknown STATE !!! Exten:%s, Callerid:%s\n", ast->exten, ast->cid.cid_num);
			}
		}
	}
...
static char *handle_cli_misdn_show_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct chan_list *help;
...
static char *handle_cli_misdn_set_tics(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static char *handle_cli_misdn_show_stacks(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int port;
	...
	tris_cli(a->fd, "BEGIN STACK_LIST:\n");
	for (port = misdn_cfg_get_next_port(0); port > 0;
	     port = misdn_cfg_get_next_port(port)) {
		char buf[128];
		get_show_stack_details(port, buf);
		tris_cli(a->fd, "  %s  Debug:%d%s\n", buf, misdn_debug[port], misdn_debug_only[port] ? "(only)" : "");
	}

	return CLI_SUCCESS;
}

static char *handle_cli_misdn_show_ports_stats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int port;
	...
	tris_cli(a->fd, "Port\tin_calls\tout_calls\n");
	for (port = misdn_cfg_get_next_port(0); port > 0;
	     port = misdn_cfg_get_next_port(port)) {
...
static char *handle_cli_misdn_show_port(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int port;
	char buf[128];
...
static char *handle_cli_misdn_send_facility(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *channame; 
	char *nr;
	struct chan_list *tmp;
	...
	if (strstr(a->argv[3], "calldeflect")) {
		if (a->argc < 6) {
			tris_verbose("calldeflect requires 1 arg: ToNumber\n\n");
			return 0;
		}
		channame = a->argv[4];
		nr = a->argv[5];

		tris_verbose("Sending Calldeflection (%s) to %s\n", nr, channame);
		tmp = get_chan_by_tris_name(channame);
		if (!tmp) {
			tris_verbose("Sending CD with nr %s to %s failed: Channel does not exist.\n", nr, channame);
			return 0; 
		}

		if (strlen(nr) >= 15) {
			tris_verbose("Sending CD with nr %s to %s failed: Number too long (up to 15 digits are allowed).\n", nr, channame);
			return 0; 
		}
		tmp->bc->fac_out.Function = Fac_CD;
		tris_copy_string((char *)tmp->bc->fac_out.u.CDeflection.DeflectedToNumber, nr, sizeof(tmp->bc->fac_out.u.CDeflection.DeflectedToNumber));
		misdn_lib_send_event(tmp->bc, EVENT_FACILITY);
	} else if (strstr(a->argv[3], "CFActivate")) {
		if (a->argc < 7) {
			tris_verbose("CFActivate requires 2 args: 1.FromNumber, 2.ToNumber\n\n");
			return 0;
		}
		port = atoi(a->argv[4]);
		served_nr = a->argv[5];
		...
		tris_verbose("Sending CFActivate  Port:(%d) FromNr. (%s) to Nr. (%s)\n", port, served_nr, nr);
		...
		tris_copy_string((char *)bc->fac_out.u.CFActivate.ServedUserNumber, served_nr, sizeof(bc->fac_out.u.CFActivate.ServedUserNumber));
		tris_copy_string((char *)bc->fac_out.u.CFActivate.ForwardedToNumber, nr, sizeof(bc->fac_out.u.CFActivate.ForwardedToNumber));

		misdn_lib_send_event(bc, EVENT_FACILITY);
	} else if (strstr(a->argv[3], "CFDeactivate")) {
		if (a->argc < 6) {
			tris_verbose("CFDeactivate requires 1 arg: FromNumber\n\n");
			return 0;
		}
		port = atoi(a->argv[4]);
		served_nr = a->argv[5];
		...
		misdn_make_dummy(bc, port, 0, misdn_lib_port_is_nt(port), 0);
		tris_verbose("Sending CFDeactivate  Port:(%d) FromNr. (%s)\n", port, served_nr);
...
static char *handle_cli_misdn_send_restart(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int port;
	int channel;
...
static char *handle_cli_misdn_send_digit(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *channame; 
	char *msg; 
	struct chan_list *tmp;
	...
	tris_cli(a->fd, "Sending %s to %s\n", msg, channame);

	tmp = get_chan_by_tris_name(channame);
	if (!tmp) {
		tris_cli(a->fd, "Sending %s to %s failed Channel does not exist\n", msg, channame);
		return CLI_SUCCESS; 
	}
#if 1
	for (i = 0; i < msglen; i++) {
		tris_cli(a->fd, "Sending: %c\n", msg[i]);
		send_digit_to_chan(tmp, msg[i]);
		/* res = tris_safe_sleep(tmp->ast, 250); */
		usleep(250000);
		/* res = tris_waitfor(tmp->ast,100); */
	}
#else
	tris_dtmf_stream(tmp->ast, NULL, msg, 250);
#endif

	return CLI_SUCCESS;
}

static char *handle_cli_misdn_toggle_echocancel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *channame;
	struct chan_list *tmp;
	...
	tris_cli(a->fd, "Toggling EchoCancel on %s\n", channame);
  
	tmp = get_chan_by_tris_name(channame);
	if (!tmp) {
		tris_cli(a->fd, "Toggling EchoCancel %s failed Channel does not exist\n", channame);
		return CLI_SUCCESS;
	}
...
static char *handle_cli_misdn_send_display(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char *channame;
	char *msg;
	struct chan_list *tmp;
	...
	if (tmp && tmp->bc) {
		tris_copy_string(tmp->bc->display, msg, sizeof(tmp->bc->display));
...
static char *complete_debug_port(struct tris_cli_args *a)
{
	...
	case 4:
		if (a->word[0] == 'p') {
			return tris_strdup("port");
		} else if (a->word[0] == 'o') {
			return tris_strdup("only");
		}
		break;
	case 6:
		if (a->word[0] == 'o') {
...
static char *complete_show_config(struct tris_cli_args *a)
{
	...
	case 3:
		if ((!strncmp(a->word, "description", wordlen)) && (++which > a->n)) {
			return tris_strdup("description");
		}
		if ((!strncmp(a->word, "descriptions", wordlen)) && (++which > a->n)) {
			return tris_strdup("descriptions");
		}
	...
	case 4:
		if (strstr(a->line, "description ")) {
			...
			if ((!wordlen || !strncmp(a->word, "general", wordlen)) && (++which > a->n)) {
				return tris_strdup("general");
			}
			if ((!wordlen || !strncmp(a->word, "ports", wordlen)) && (++which > a->n)) {
				return tris_strdup("ports");
			}
		}
		break;
	}
...
static struct tris_cli_entry chan_misdn_clis[] = {
	TRIS_CLI_DEFINE(handle_cli_misdn_port_block,        "Block the given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_port_down,         "Try to deactivate the L1 on the given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_port_unblock,      "Unblock the given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_port_up,           "Try to establish L1 on the given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_reload,            "Reload internal mISDN config, read from the config file"),
	TRIS_CLI_DEFINE(handle_cli_misdn_restart_pid,       "Restart the given pid"),
	TRIS_CLI_DEFINE(handle_cli_misdn_restart_port,      "Restart the given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_show_channel,      "Show an internal mISDN channel"),
	TRIS_CLI_DEFINE(handle_cli_misdn_show_channels,     "Show the internal mISDN channel list"),
	TRIS_CLI_DEFINE(handle_cli_misdn_show_config,       "Show internal mISDN config, read from the config file"),
	TRIS_CLI_DEFINE(handle_cli_misdn_show_port,         "Show detailed information for given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_show_ports_stats,  "Show mISDNs channel's call statistics per port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_show_stacks,       "Show internal mISDN stack_list"),
	TRIS_CLI_DEFINE(handle_cli_misdn_send_facility,     "Sends a Facility Message to the mISDN Channel"),
	TRIS_CLI_DEFINE(handle_cli_misdn_send_digit,        "Send DTMF digit to mISDN Channel"),
	TRIS_CLI_DEFINE(handle_cli_misdn_send_display,      "Send Text to mISDN Channel"),
	TRIS_CLI_DEFINE(handle_cli_misdn_send_restart,      "Send a restart for every bchannel on the given port"),
	TRIS_CLI_DEFINE(handle_cli_misdn_set_crypt_debug,   "Set CryptDebuglevel of chan_misdn, at the moment, level={1,2}"),
	TRIS_CLI_DEFINE(handle_cli_misdn_set_debug,         "Set Debuglevel of chan_misdn"),
	TRIS_CLI_DEFINE(handle_cli_misdn_set_tics,          "???"),
	TRIS_CLI_DEFINE(handle_cli_misdn_toggle_echocancel, "Toggle EchoCancel on mISDN Channel"),
};
...
static int update_config(struct chan_list *ch, int orig) 
{
	...
	if (!ch) {
		tris_log(LOG_WARNING, "Cannot configure without chanlist\n");
		return -1;
	}
	...
	bc = ch->bc;
	if (! ast || ! bc) {
		tris_log(LOG_WARNING, "Cannot configure without ast || bc\n");
		return -1;
	}
	...
	if (pres < 0 || screen < 0) {
		...
		case TRIS_PRES_RESTRICTED:
			bc->pres = 1;
			chan_misdn_log(2, port, " --> PRES: Restricted (1)\n");
			break;
		case TRIS_PRES_UNAVAILABLE:
			bc->pres = 2;
		...
		default:
		case TRIS_PRES_USER_NUMBER_UNSCREENED:
			bc->screen = 0;
			chan_misdn_log(2, port, " --> SCREEN: Unscreened (0)\n");
			break;
		case TRIS_PRES_USER_NUMBER_PASSED_SCREEN:
			bc->screen = 1;
			chan_misdn_log(2, port, " --> SCREEN: Passed Screen (1)\n");
			break;
		case TRIS_PRES_USER_NUMBER_FAILED_SCREEN:
			bc->screen = 2;
			chan_misdn_log(2, port, " --> SCREEN: Failed Screen (2)\n");
			break;
		case TRIS_PRES_NETWORK_NUMBER:
			bc->screen = 3;
...
static int update_pipeline_config(struct misdn_bchannel *bc)
{
	...
	if (ec == 1) {
		tris_copy_string(bc->pipeline, "mg2ec", sizeof(bc->pipeline));
	} else if (ec > 1) {
...
static int read_config(struct chan_list *ch, int orig)
{
	...
	tris_string_field_set(ast, language, lang);
	...
	misdn_cfg_get(port, MISDN_CFG_ASTDTMF, &ch->tris_dsp, sizeof(int));
	...
	tris_copy_string(ast->context, ch->context, sizeof(ast->context));
	...
	chan_misdn_log(5, port, " --> * CallGrp:%s PickupGrp:%s\n", tris_print_group(buf, sizeof(buf), cg), tris_print_group(buf2, sizeof(buf2), pg));
	ast->pickupgroup = pg;
	ast->callgroup = cg;
	...
	if (orig == ORG_AST) {
		...
		/* ORIGINATOR Trismedia (outgoing call) */
		...
		if (!tris_strlen_zero(callerid)) {
			...
			tris_copy_string(bc->oad, callerid, sizeof(bc->oad));
		}
		...
		tris_copy_string(buf, bc->oad, sizeof(buf));
		snprintf(bc->oad, sizeof(bc->oad), "%s%s", prefix, buf);

		if (!tris_strlen_zero(bc->dad)) {
			tris_copy_string(bc->orig_dad, bc->dad, sizeof(bc->orig_dad));
		}

		if (tris_strlen_zero(bc->dad) && !tris_strlen_zero(bc->keypad)) {
			tris_copy_string(bc->dad, bc->keypad, sizeof(bc->dad));
		}
		...
		tris_copy_string(buf, bc->dad, sizeof(buf));
		snprintf(bc->dad, sizeof(bc->dad), "%s%s", prefix, buf);

		if (strcmp(bc->dad, ast->exten)) {
			tris_copy_string(ast->exten, bc->dad, sizeof(ast->exten));
		}

		tris_set_callerid(ast, bc->oad, NULL, bc->oad);

		if ( !tris_strlen_zero(bc->rad) ) {
			if (ast->cid.cid_rdnis) {
				tris_free(ast->cid.cid_rdnis);
			}
			ast->cid.cid_rdnis = tris_strdup(bc->rad);
		}
	
		misdn_cfg_get(bc->port, MISDN_CFG_OVERLAP_DIAL, &ch->overlap_dial, sizeof(ch->overlap_dial));
		tris_mutex_init(&ch->overlap_tv_lock);
	} /* ORIG MISDN END */

	ch->overlap_dial_task = -1;
	
	if (ch->faxdetect  || ch->tris_dsp) {
		misdn_cfg_get(port, MISDN_CFG_FAXDETECT_TIMEOUT, &ch->faxdetect_timeout, sizeof(ch->faxdetect_timeout));
		if (!ch->dsp) {
			ch->dsp = tris_dsp_new();
		}
		if (ch->dsp) {
			tris_dsp_set_features(ch->dsp, DSP_FEATURE_DIGIT_DETECT | (ch->faxdetect ? DSP_FEATURE_FAX_DETECT : 0));
		}
	}
...
static int misdn_call(struct tris_channel *ast, char *dest, int timeout)
{
	int port = 0;
	int r;
	int exceed;
	...
	char *dest_cp = tris_strdupa(dest);
	 ...
	 */
	TRIS_NONSTANDARD_APP_ARGS(args, dest_cp, '/');

	if (tris_strlen_zero(args.ext)) {
		chan_misdn_log(0, 0, "misdn_call: No Extension given!\n");
		return -1;
	}

	if (!ast) {
		tris_log(LOG_WARNING, " --> ! misdn_call called on tris_channel *ast where ast == NULL\n");
		return -1;
	}

	if (((ast->_state != TRIS_STATE_DOWN) && (ast->_state != TRIS_STATE_RESERVED)) || !dest) {
		tris_log(LOG_WARNING, " --> ! misdn_call called on %s, neither down nor reserved (or dest==NULL)\n", ast->name);
		ast->hangupcause = TRIS_CAUSE_NORMAL_TEMPORARY_FAILURE;
		tris_setstate(ast, TRIS_STATE_DOWN);
		return -1;
	}

	ch = MISDN_TRISMEDIA_TECH_PVT(ast);
	if (!ch) {
		tris_log(LOG_WARNING, " --> ! misdn_call called on %s, neither down nor reserved (or dest==NULL)\n", ast->name);
		ast->hangupcause = TRIS_CAUSE_NORMAL_TEMPORARY_FAILURE;
		tris_setstate(ast, TRIS_STATE_DOWN);
	...
	if (ast->exten) {
		tris_copy_string(ast->exten, args.ext, sizeof(ast->exten));
		tris_copy_string(newbc->dad, args.ext, sizeof(newbc->dad));
	}

	tris_copy_string(newbc->rad, S_OR(ast->cid.cid_rdnis, ""), sizeof(newbc->rad));

	chan_misdn_log(3, port, " --> * adding2newbc callerid %s\n", ast->cid.cid_num);
	if (tris_strlen_zero(newbc->oad) && !tris_strlen_zero(ast->cid.cid_num)) {
		tris_copy_string(newbc->oad, ast->cid.cid_num, sizeof(newbc->oad));
	}

	newbc->capability = ast->transfercapability;
	pbx_builtin_setvar_helper(ast, "TRANSFERCAPABILITY", tris_transfercapability2str(newbc->capability));
	...
	if (r == -ENOCHAN ) {
		...
		ast->hangupcause = TRIS_CAUSE_NORMAL_CIRCUIT_CONGESTION;
		tris_setstate(ast, TRIS_STATE_DOWN);
		return -1;
	}
	...
	ast->hangupcause = TRIS_CAUSE_NORMAL_CLEARING;
...
static int misdn_answer(struct tris_channel *ast)
{
	struct chan_list *p;
	const char *tmp;

	if (!ast || !(p = MISDN_TRISMEDIA_TECH_PVT(ast))) {
		return -1;
	}
	...
	if (!p) {
		tris_log(LOG_WARNING, " --> Channel not connected ??\n");
		tris_queue_hangup_with_cause(ast, TRIS_CAUSE_NETWORK_OUT_OF_ORDER);
	}

	if (!p->bc) {
		chan_misdn_log(1, 0, " --> Got Answer, but there is no bc obj ??\n");

		tris_queue_hangup_with_cause(ast, TRIS_CAUSE_PROTOCOL_ERROR);
	}
	...
	if ( tris_strlen_zero(p->bc->cad) ) {
		chan_misdn_log(2, p->bc->port, " --> empty cad using dad\n");
		tris_copy_string(p->bc->cad, p->bc->dad, sizeof(p->bc->cad));
	}
...
static int misdn_digit_begin(struct tris_channel *chan, char digit)
{
	/* XXX Modify this callback to support Trismedia controlling the length of DTMF */
	return 0;
}

static int misdn_digit_end(struct tris_channel *ast, char digit, unsigned int duration)
{
	struct chan_list *p;
	struct misdn_bchannel *bc;
	char buf[2] = { digit, 0 };
	...
	if (!bc) {
		tris_log(LOG_WARNING, " --> !! Got Digit Event without having bchannel Object\n");
		return -1;
	}
	...
	case MISDN_CALLING_ACKNOWLEDGE:
		tris_copy_string(bc->info_dad, buf, sizeof(bc->info_dad));
		if (strlen(bc->dad) < sizeof(bc->dad) - 1) {
			strncat(bc->dad, buf, sizeof(bc->dad) - strlen(bc->dad) - 1);
		}
		tris_copy_string(p->ast->exten, bc->dad, sizeof(p->ast->exten));
...
static int misdn_fixup(struct tris_channel *oldast, struct tris_channel *ast)
{
	struct chan_list *p;
...
static int misdn_indication(struct tris_channel *ast, int cond, const void *data, size_t datalen)
{
	struct chan_list *p;

	if (!ast || ! (p=MISDN_TRISMEDIA_TECH_PVT(ast))) {
		tris_log(LOG_WARNING, "Returned -1 in misdn_indication\n");
		return -1;
	}
	...
	if (!p->bc) {
		if (p->hold.state == MISDN_HOLD_IDLE) {
			chan_misdn_log(1, 0, "* IND : Indication [%d] ignored on %s\n", cond,
				ast->name);
			tris_log(LOG_WARNING, "Private Pointer but no bc ?\n");
		} else {
	...
	case TRIS_CONTROL_BUSY:
		...
		tris_setstate(ast, TRIS_STATE_BUSY);

		p->bc->out_cause = TRIS_CAUSE_USER_BUSY;
	...
	case TRIS_CONTROL_CONGESTION:
		...
		p->bc->out_cause = TRIS_CAUSE_SWITCH_CONGESTION;
		start_bc_tones(p);
		misdn_lib_send_event( p->bc, EVENT_DISCONNECT);
...
static int misdn_hangup(struct tris_channel *ast)
{
	struct chan_list *p;
	struct misdn_bchannel *bc;
	const char *var;
	...
	MISDN_TRISMEDIA_TECH_PVT(ast) = NULL;

	tris_debug(1, "misdn_hangup(%s)\n", ast->name);
	...
	if (ast->_state == TRIS_STATE_RESERVED || p->state == MISDN_NOTHING) {
		/* between request and call */
		tris_debug(1, "State Reserved (or nothing) => chanIsAvail\n");
		release_chan_early(p);
		if (bc) {
	...
	if (!bc) {
		tris_log(LOG_WARNING, "Hangup with private but no bc ? state:%s l3id:%x\n",
			misdn_get_ch_state(p), p->l3id);
		release_chan_early(p);
		return 0;
	}
	...
	bc->out_cause = ast->hangupcause ? ast->hangupcause : TRIS_CAUSE_NORMAL_CLEARING;
	...
	if (var) {
		...
		tmpcause = atoi(var);
		bc->out_cause = tmpcause ? tmpcause : TRIS_CAUSE_NORMAL_CLEARING;
	}

	var = pbx_builtin_getvar_helper(ast, "MISDN_USERUSER");
	if (var) {
		tris_log(LOG_NOTICE, "MISDN_USERUSER: %s\n", var);
		tris_copy_string(bc->uu, var, sizeof(bc->uu));
	...
	case MISDN_INCOMING_SETUP:
		 ...
		 */
		tris_log(LOG_NOTICE, "release channel, in INCOMING_SETUP state.. no other events happened\n");
		release_chan(p, bc);
...
static struct tris_frame *process_tris_dsp(struct chan_list *tmp, struct tris_frame *frame)
{
	struct tris_frame *f;
 	...
 	if (tmp->dsp) {
 		f = tris_dsp_process(tmp->ast, tmp->dsp, frame);
 	} else {
	...
	tris_debug(1, "Detected inband DTMF digit: %c\n", f->subclass);
 
 	if (tmp->faxdetect && (f->subclass == 'f')) {
 		/* Fax tone -- Handle and return NULL */
 		if (!tmp->faxhandled) {
  			struct tris_channel *ast = tmp->ast;
 			tmp->faxhandled++;
			...
			*tmp->bc->pipeline = 0;
 			...
 			case 1:
  				if (strcmp(ast->exten, "fax")) {
 					...
 					context = tris_strlen_zero(context_tmp) ? (tris_strlen_zero(ast->macrocontext) ? ast->context : ast->macrocontext) : context_tmp;
 					if (tris_exists_extension(ast, context, "fax", 1, ast->cid.cid_num)) {
 						tris_verb(3, "Redirecting %s to fax extension (context:%s)\n", ast->name, context);
  						/* Save the DID/DNIS when we transfer the fax call to a "fax" extension */
  						pbx_builtin_setvar_helper(ast,"FAXEXTEN",ast->exten);
 						if (tris_async_goto(ast, context, "fax", 1)) {
 							tris_log(LOG_WARNING, "Failed to async goto '%s' into fax of '%s'\n", ast->name, context);
						}
  					} else {
 						tris_log(LOG_NOTICE, "Fax detected, but no fax extension ctx:%s exten:%s\n", context, ast->exten);
					}
 				} else {
 				...
 				break;
 			case 2:
 				tris_verb(3, "Not redirecting %s to fax extension, nojump is set.\n", ast->name);
 				break;
 			}
 		} else {
 	...
 	if (tmp->tris_dsp && (f->subclass != 'f')) {
 		chan_misdn_log(2, tmp->bc->port, " --> * SEND: DTMF (TRIS_DSP) :%c\n", f->subclass);
 	}
...
static struct tris_frame *misdn_read(struct tris_channel *ast)
{
	struct chan_list *tmp;
	fd_set rrfs;
	...
	if (!ast) {
 	...
 	if (!(tmp = MISDN_TRISMEDIA_TECH_PVT(ast))) {
	...
	if (FD_ISSET(tmp->pipe[0], &rrfs)) {
		len = read(tmp->pipe[0], tmp->tris_rd_buf, sizeof(tmp->tris_rd_buf));
	...
	tmp->frame.frametype = TRIS_FRAME_VOICE;
	tmp->frame.subclass = TRIS_FORMAT_ALAW;
	tmp->frame.datalen = len;
	tmp->frame.samples = len;
	tmp->frame.mallocd = 0;
	tmp->frame.offset = 0;
	tmp->frame.delivery = tris_tv(0, 0);
	tmp->frame.src = NULL;
	tmp->frame.data.ptr = tmp->tris_rd_buf;

	if (tmp->faxdetect && !tmp->faxhandled) {
		if (tmp->faxdetect_timeout) {
			if (tris_tvzero(tmp->faxdetect_tv)) {
				tmp->faxdetect_tv = tris_tvnow();
				chan_misdn_log(2, tmp->bc->port, "faxdetect: starting detection with timeout: %ds ...\n", tmp->faxdetect_timeout);
				return process_tris_dsp(tmp, &tmp->frame);
			} else {
				struct timeval tv_now = tris_tvnow();
				int diff = tris_tvdiff_ms(tv_now, tmp->faxdetect_tv);
...
static int misdn_write(struct tris_channel *ast, struct tris_frame *frame)
{
	struct chan_list *ch;
	int i  = 0;
	
	if (!ast || !(ch = MISDN_TRISMEDIA_TECH_PVT(ast))) {
		return -1;
	}
	...
	if (!ch->bc ) {
		tris_log(LOG_WARNING, "private but no bc\n");
		return -1;
	}
...
#ifdef MISDN_DEBUG
	{
		...
		tris_debug(1, "write2mISDN %p %d bytes: ", p, frame->samples);

		for (i = 0; i < max; i++) {
			tris_debug(1, "%2.2x ", ((char *) frame->data.ptr)[i]);
		}
	}
#endif
...
static enum tris_bridge_result  misdn_bridge (struct tris_channel *c0,
				      struct tris_channel *c1, int flags,
				      struct tris_frame **fo,
				      struct tris_channel **rc,
				      int timeoutms)

{
	struct chan_list *ch1, *ch2;
	struct tris_channel *carr[2], *who;
	int to = -1;
	struct tris_frame *f;
	int p1_b, p2_b;
	int bridging;
	...
	if (! p1_b || ! p2_b) {
		tris_log(LOG_NOTICE, "Falling back to Trismedia bridging\n");
	...
	if (! (flags & TRIS_BRIDGE_DTMF_CHANNEL_0) ) {
		ch1->ignore_dtmf = 1;
	}

	if (! (flags & TRIS_BRIDGE_DTMF_CHANNEL_1) ) {
		ch2->ignore_dtmf = 1;
	}

	for (;/*ever*/;) {
		to = -1;
		who = tris_waitfor_n(carr, 2, &to);

		if (!who) {
			tris_log(LOG_NOTICE, "misdn_bridge: empty read, breaking out\n");
			break;
		}
		f = tris_read(who);
...
static int dialtone_indicate(struct chan_list *cl)
{
	struct tris_channel *ast = cl->ast;
	int nd = 0;
	...
	cl->ts = tris_get_indication_tone(ast->zone, "dial");
	
	if (cl->ts) {
		cl->notxtone = 0;
		cl->norxtone = 0;
		/* This prods us in misdn_write */
		tris_playtones_start(ast, 0, cl->ts->data, 0);
	}
...
static int stop_indicate(struct chan_list *cl)
{
	...
	tris_playtones_stop(ast);

	if (cl->ts) {
		cl->ts = tris_tone_zone_sound_unref(cl->ts);
	}
...
static struct tris_channel *misdn_request(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	struct tris_channel *tmp = NULL;
	char group[BUFFERSIZE + 1] = "";
	char dial_str[128];
	char *buf2 = tris_strdupa(data);
	char *ext;
	char *port_str;
	char *p = NULL;
	int channel = 0;
	...
	if (!tris_strlen_zero(port_str)) {
		if (port_str[0] == 'g' && port_str[1] == ':' ) {
			/* We make a group call lets checkout which ports are in my group */
			port_str += 2;
			tris_copy_string(group, port_str, sizeof(group));
		...
		tris_log(LOG_WARNING, " --> ! IND : Dial(%s) WITHOUT Port or Group, check extensions.conf\n", dial_str);
		return NULL;
	}
	...
	if (!tris_strlen_zero(group)) {
			...
			do {
				...
				for (port = misdn_cfg_get_next_port_spin(rr->port); port > 0 && port != port_start;
					...
					if (!strcasecmp(cfg_group, group)) {
						...
						if (check && port_up < 0) {
							tris_log(LOG_WARNING, "This port (%d) is blocked\n", port);
						}
		...
		if (!newbc) {
			tris_log(LOG_WARNING, "Could not create channel on port:%d with extensions:%s\n", port, ext);
			return NULL;
		}
	}
	

	/* create tris_channel and link all the objects together */
	cl = init_chan_list(ORG_AST);
	if (!cl) {
		tris_log(LOG_ERROR, "Could not create call record for Dial(%s)\n", dial_str);
		return NULL;
	}
	cl->bc = newbc;
	
	tmp = misdn_new(cl, TRIS_STATE_RESERVED, ext, NULL, format, port, channel);
	if (!tmp) {
		tris_free(cl);
		tris_log(LOG_ERROR, "Could not create Trismedia object\n");
		return NULL;
	}
...
static int misdn_send_text(struct tris_channel *chan, const char *text)
{
	struct chan_list *tmp = chan->tech_pvt;
	
	if (tmp && tmp->bc) {
		tris_copy_string(tmp->bc->display, text, sizeof(tmp->bc->display));
		misdn_lib_send_event(tmp->bc, EVENT_INFORMATION);
	} else {
		tris_log(LOG_WARNING, "No chan_list but send_text request?\n");
		return -1;
	}
...
static struct tris_channel_tech misdn_tech = {
	.type = "mISDN",
	.description = "Channel driver for mISDN Support (Bri/Pri)",
	.capabilities = TRIS_FORMAT_ALAW ,
...
static struct tris_channel_tech misdn_tech_wo_bridge = {
	.type = "mISDN",
	.description = "Channel driver for mISDN Support (Bri/Pri)",
	.capabilities = TRIS_FORMAT_ALAW ,
	.requester = misdn_request,
...
static void update_name(struct tris_channel *tmp, int port, int c) 
{
	int chan_offset = 0;
	...
	if (strncmp(tmp->name, newname, strlen(newname))) {
		...
		tris_change_name(tmp, newname);
		chan_misdn_log(3, port, " --> updating channel name to [%s]\n", tmp->name);
	}
}

static struct tris_channel *misdn_new(struct chan_list *chlist, int state,  char *exten, char *callerid, int format, int port, int c)
{
	...
	if (callerid) {
		tris_callerid_parse(callerid, &cid_name, &cid_num);
	}

	tmp = tris_channel_alloc(1, state, cid_num, cid_name, "", exten, "", 0, "%s/%s%d-u%d", misdn_type, c ? "" : "tmp", chan_offset + c, glob_channel++);
	if (tmp) {
		...
		if (callerid) {
			...
			tmp->cid.cid_ani = tris_strdup(cid_num);
		}

		if (pipe(chlist->pipe) < 0) {
			tris_log(LOG_ERROR, "Pipe failed\n");
		}
		tris_channel_set_fd(tmp, 0, chlist->pipe[0]);

		tmp->rings = (state == TRIS_STATE_RING) ? 1 : 0;

		tris_jb_configure(tmp, misdn_get_global_jbconf());
	} else {
...
static void hangup_chan(struct chan_list *ch, struct misdn_bchannel *bc)
{
	...
	if (ch->ast) {
		send_cause2ast(ch->ast, bc, ch);
		tris_queue_hangup_with_cause(ch->ast, bc->cause);
...
static void release_chan(struct chan_list *ch, struct misdn_bchannel *bc)
{
	...
	if (ch->overlap_dial) {
		...
		tris_mutex_destroy(&ch->overlap_tv_lock);
	}
	...
	tris_mutex_unlock(&release_lock);
}
...
static int misdn_attempt_transfer(struct chan_list *active_ch, struct chan_list *held_ch)
{
	...
	bridged = tris_bridged_channel(held_ch->ast);
	if (bridged) {
		tris_queue_control(held_ch->ast, TRIS_CONTROL_UNHOLD);
		...
		retval = tris_channel_masquerade(active_ch->ast, bridged);
	} else {
		/*
...
static void do_immediate_setup(struct misdn_bchannel *bc, struct chan_list *ch, struct tris_channel *ast)
{
	char *predial;
	struct tris_frame fr;

	predial = tris_strdupa(ast->exten);
	...
	if (!tris_canmatch_extension(ast, ast->context, ast->exten, 1, bc->oad) || pbx_start_chan(ch) < 0) {
		ast = NULL;
		bc->out_cause = TRIS_CAUSE_UNALLOCATED;
		hangup_chan(ch, bc);
		hanguptone_indicate(ch);
	...
	while (!tris_strlen_zero(predial) ) {
		fr.frametype = TRIS_FRAME_DTMF;
		fr.subclass = *predial;
		fr.src = NULL;
		fr.data.ptr = NULL;
		fr.datalen = 0;
		...
		fr.mallocd = 0;
		fr.offset = 0;
		fr.delivery = tris_tv(0,0);

		if (ch->ast && MISDN_TRISMEDIA_PVT(ch->ast) && MISDN_TRISMEDIA_TECH_PVT(ch->ast)) {
			tris_queue_frame(ch->ast, &fr);
		}
		predial++;
	}
}
...
void import_ch(struct tris_channel *chan, struct misdn_bchannel *bc, struct chan_list *ch)
{
	...
	if (tmp) {
		tris_log(LOG_NOTICE, "MISDN_USERUSER: %s\n", tmp);
		tris_copy_string(bc->uu, tmp, sizeof(bc->uu));
		bc->uulen = strlen(bc->uu);
	}

	tmp = pbx_builtin_getvar_helper(chan, "MISDN_KEYPAD");
	if (tmp) {
		tris_copy_string(bc->keypad, tmp, sizeof(bc->keypad));
	}
...
int add_in_calls(int port)
{
	...
	if (max_in_calls >= 0 && max_in_calls < misdn_in_calls[port]) {
		tris_log(LOG_NOTICE, "Marking Incoming Call on port[%d]\n", port);
...
int add_out_calls(int port)
{
	...
	if (max_out_calls >= 0 && max_out_calls <= misdn_out_calls[port]) {
		tris_log(LOG_NOTICE, "Rejecting Outgoing Call on port[%d]\n", port);
...
static void start_pbx(struct chan_list *ch, struct misdn_bchannel *bc, struct tris_channel *chan) {
	if (pbx_start_chan(ch) < 0) {
		hangup_chan(ch, bc);
		chan_misdn_log(-1, bc->port, "tris_pbx_start returned <0 in SETUP\n");
		if (bc->nt) {
			hanguptone_indicate(ch);
			misdn_lib_send_event(bc, EVENT_RELEASE_COMPLETE);
		} else {
...
static void wait_for_digits(struct chan_list *ch, struct misdn_bchannel *bc, struct tris_channel *chan) {
...
cb_events(enum event_e event, struct misdn_bchannel *bc, void *user_data)
{
	...
	if (ch) {
		...
		case EVENT_TIMEOUT:
			if (!ch->ast) {
				chan_misdn_log(3, bc->port, "tris_hangup already called, so we have no ast ptr anymore in event(%s)\n", manager_isdn_get_info(event));
			}
			break;
		default:
			if (!ch->ast  || !MISDN_TRISMEDIA_PVT(ch->ast) || !MISDN_TRISMEDIA_TECH_PVT(ch->ast)) {
				if (event != EVENT_BCHAN_DATA) {
					tris_log(LOG_NOTICE, "No Ast or No private Pointer in Event (%d:%s)\n", event, manager_isdn_get_info(event));
				}
				return -1;
			}
		}
	...
	case EVENT_NEW_BC:
		...
		if (!ch) {
			tris_log(LOG_WARNING, "NEW_BC without chan_list?\n");
			break;
		}
	...
	case EVENT_INFORMATION:
		...
		if (ch->state == MISDN_WAITING4DIGS ) {
			...
			if (tris_strlen_zero(bc->info_dad) && ! tris_strlen_zero(bc->keypad)) {
				chan_misdn_log(1, bc->port, " --> using keypad as info\n");
				tris_copy_string(bc->info_dad, bc->keypad, sizeof(bc->info_dad));
			}

			strncat(bc->dad, bc->info_dad, sizeof(bc->dad) - strlen(bc->dad) - 1);
			tris_copy_string(ch->ast->exten, bc->dad, sizeof(ch->ast->exten));

			/* Check for Pickup Request first */
			if (!strcmp(ch->ast->exten, tris_pickup_ext())) {
				if (tris_pickup_call(ch->ast)) {
					hangup_chan(ch, bc);
				} else {
					struct tris_channel *chan = ch->ast;
					ch->state = MISDN_CALLING_ACKNOWLEDGE;
					tris_setstate(chan, TRIS_STATE_DOWN);
					hangup_chan(ch, bc);
					ch->ast = NULL;
					break;
				}
			...
			if (!tris_canmatch_extension(ch->ast, ch->context, bc->dad, 1, bc->oad)) {
				if (tris_exists_extension(ch->ast, ch->context, "i", 1, bc->oad)) {
					tris_log(LOG_WARNING,
			...
			if (tris_exists_extension(ch->ast, ch->context, bc->dad, 1, bc->oad))  {
				ch->state = MISDN_DIALING;
				start_pbx(ch, bc, ch->ast);
			}
		} else {
	...
	case EVENT_SETUP:
	{
		...
		if (bc->cw) {
			...
			bc->out_cause = cause ? cause : TRIS_CAUSE_NORMAL_CLEARING;
		...
		chan = misdn_new(ch, TRIS_STATE_RESERVED, bc->dad, bc->oad, TRIS_FORMAT_ALAW, bc->port, bc->channel);
		if (!chan) {
			tris_free(ch);
			misdn_lib_send_event(bc,EVENT_RELEASE_COMPLETE);
			tris_log(LOG_ERROR, "cb_events: misdn_new failed !\n"); 
			return 0;
		}
		...
		ch->ast->rings = 1;
		tris_setstate(ch->ast, TRIS_STATE_RINGING);

		switch (bc->pres) {
		case 1:
			pres = TRIS_PRES_RESTRICTED;
			chan_misdn_log(2, bc->port, " --> PRES: Restricted (1)\n");
			break;
		case 2:
			pres = TRIS_PRES_UNAVAILABLE;
			chan_misdn_log(2, bc->port, " --> PRES: Unavailable (2)\n");
			break;
		default:
			pres = TRIS_PRES_ALLOWED;
		...
		default:
		case 0:
			screen = TRIS_PRES_USER_NUMBER_UNSCREENED;
			chan_misdn_log(2, bc->port, " --> SCREEN: Unscreened (%d)\n", bc->screen);
			break;
		case 1:
			screen = TRIS_PRES_USER_NUMBER_PASSED_SCREEN;
			chan_misdn_log(2, bc->port, " --> SCREEN: Passed screen (1)\n");
			break;
		case 2:
			screen = TRIS_PRES_USER_NUMBER_FAILED_SCREEN;
			chan_misdn_log(2, bc->port, " --> SCREEN: failed screen (2)\n");
			break;
		case 3:
			screen = TRIS_PRES_NETWORK_NUMBER;
		...
		pbx_builtin_setvar_helper(chan, "TRANSFERCAPABILITY", tris_transfercapability2str(bc->capability));
		...
		if (!strstr(ch->allowed_bearers, "all")) {
			...
			if (i == ARRAY_LEN(allowed_bearers_array)) {
				...
				bc->out_cause = TRIS_CAUSE_INCOMPATIBLE_DESTINATION;
		...
		/* Check for Pickup Request first */
		if (!strcmp(chan->exten, tris_pickup_ext())) {
			if (!ch->noautorespond_on_setup) {
		 ...
		 * will be used by Trismedia automatically. */
	...
	case EVENT_SETUP_ACKNOWLEDGE:
		...
		if (!tris_strlen_zero(bc->infos_pending)) {
			/* TX Pending Infos */
			...
			tris_copy_string(ch->ast->exten, bc->dad, sizeof(ch->ast->exten));
			tris_copy_string(bc->info_dad, bc->infos_pending, sizeof(bc->info_dad));
			tris_copy_string(bc->infos_pending, "", sizeof(bc->infos_pending));
	...
	case EVENT_PROCEEDING:
		...
		tris_queue_control(ch->ast, TRIS_CONTROL_PROCEEDING);
		break;
	case EVENT_PROGRESS:
		if (bc->channel) {
			update_name(ch->ast, bc->port, bc->channel);
		...
		if (!bc->nt ) {
			...
			tris_queue_control(ch->ast, TRIS_CONTROL_PROGRESS);
		}
		break;
	case EVENT_ALERTING:
		...
		tris_queue_control(ch->ast, TRIS_CONTROL_RINGING);
		tris_setstate(ch->ast, TRIS_STATE_RINGING);
	...
	case EVENT_CONNECT:
		{
			...
			bridged = tris_bridged_channel(ch->ast);
			stop_indicate(ch);

			if (bridged && !strcasecmp(bridged->tech->type, "mISDN")) {
				struct chan_list *bridged_ch = MISDN_TRISMEDIA_TECH_PVT(bridged);

				chan_misdn_log(1, bc->port, " --> copying cpndialplan:%d and cad:%s to the A-Channel\n", bc->cpnnumplan, bc->cad);
				if (bridged_ch) {
					bridged_ch->bc->cpnnumplan = bc->cpnnumplan;
					tris_copy_string(bridged_ch->bc->cad, bc->cad, sizeof(bridged_ch->bc->cad));
				}
			}
		}
		ch->l3id = bc->l3_id;
		...
		tris_queue_control(ch->ast, TRIS_CONTROL_ANSWER);
		break;
		...
		if (ch) {
			...
			if (ch->originator == ORG_AST && !bc->nt && misdn_inband_avail(bc) && ch->state != MISDN_CONNECTED) {
				...
				if (ch->ast) {
					...
					if (bc->cause == TRIS_CAUSE_USER_BUSY) {
						tris_queue_control(ch->ast, TRIS_CONTROL_BUSY);
					}
				}
				ch->need_busy = 0;
				break;
	...
	case EVENT_CLEANUP:
		...
		switch (ch->state) {
		case MISDN_CALLING:
			bc->cause = TRIS_CAUSE_DESTINATION_OUT_OF_ORDER;
			break;
		default:
			break;
		}
	...
	case EVENT_TONE_GENERATE:
	{
		...
		struct tris_channel *ast = ch->ast;
		void *tmp;
		int res;
		int (*generate)(struct tris_channel *chan, void *tmp, int datalen, int samples);
		...
		if (tone_len < 0 || tone_len > 512 ) {
			tris_log(LOG_NOTICE, "TONE_GEN: len was %d, set to 128\n", tone_len);
			tone_len = 128;
		}
		...
		if (res) {
			tris_log(LOG_WARNING, "Auto-deactivating generator\n");
			tris_deactivate_generator(ast);
		} else {
			bc->tone_cnt = 0;
		}
	}
	...
	case EVENT_BCHAN_DATA:
		...
		if (!misdn_cap_is_speech(ch->bc->capability)) {
			...
			frame.frametype = TRIS_FRAME_VOICE; /* we have no data frames yet */
			frame.subclass = TRIS_FORMAT_ALAW;
			frame.datalen = bc->bframe_len;
			frame.samples = bc->bframe_len;
			frame.mallocd = 0;
			frame.offset = 0;
			frame.delivery = tris_tv(0, 0);
			frame.src = NULL;
			frame.data.ptr = bc->bframe;

			if (ch->ast) 
				tris_queue_frame(ch->ast, &frame);
		} else {
			fd_set wrfs;
			...
			if (FD_ISSET(ch->pipe[1], &wrfs)) {
				chan_misdn_log(9, bc->port, "writing %d bytes to trismedia\n", bc->bframe_len);
	...
	case EVENT_TIMEOUT:
		...
		case MISDN_WAITING4DIGS:
			if (bc->nt) {
				...
				bc->out_cause = TRIS_CAUSE_NORMAL_CLEARING;
				misdn_lib_send_event(bc, EVENT_RELEASE);
			}
			break;
		case MISDN_CLEANING: 
	...
	case EVENT_RETRIEVE:
		if (!ch) {
			...
			if (!ch || ch->hold.state != MISDN_HOLD_ACTIVE) {
				tris_log(LOG_WARNING, "No held call found, cannot Retrieve\n");
		...
		tris_queue_control(ch->ast, TRIS_CONTROL_UNHOLD);
	...
	case EVENT_HOLD:
	{
		...
		if (bridged) {
			...
			tris_queue_control(ch->ast, TRIS_CONTROL_HOLD);
	...
	case EVENT_FACILITY:
		...
		case Fac_CD:
			if (ch) {
				struct tris_channel *bridged = tris_bridged_channel(ch->ast);
				struct chan_list *ch_br;
				if (bridged && MISDN_TRISMEDIA_TECH_PVT(bridged)) {
					ch_br = MISDN_TRISMEDIA_TECH_PVT(bridged);
					/*ch->state = MISDN_FACILITY_DEFLECTED;*/
					if (ch_br->bc) {
						if (tris_exists_extension(bridged, ch->context, (char *)bc->fac_in.u.CDeflection.DeflectedToNumber, 1, bc->oad)) {
							ch_br->state = MISDN_DIALING;
							if (pbx_start_chan(ch_br) < 0) {
								chan_misdn_log(-1, ch_br->bc->port, "tris_pbx_start returned < 0 in misdn_overlap_dial_task\n");
							}
						}
					}
				}
...
/******************************************
 *
 *   Trismedia Channel Endpoint END
 *
 *
 *******************************************/
...
static int unload_module(void)
{
	...
	tris_log(LOG_VERBOSE, "-- Unregistering mISDN Channel Driver --\n");
	...
	tris_cli_unregister_multiple(chan_misdn_clis, sizeof(chan_misdn_clis) / sizeof(struct tris_cli_entry));

	/* tris_unregister_application("misdn_crypt"); */
	tris_unregister_application("misdn_set_opt");
	tris_unregister_application("misdn_facility");
	tris_unregister_application("misdn_check_l2l1");

	tris_channel_unregister(&misdn_tech);

	free_robin_list();
	misdn_cfg_destroy();
	misdn_lib_destroy();
  
	tris_free(misdn_out_calls);
	tris_free(misdn_in_calls);
	tris_free(misdn_debug_only);
	tris_free(misdn_ports);
	tris_free(misdn_debug);
...
static int load_module(void)
{
	...
	if (max_ports <= 0) {
		tris_log(LOG_ERROR, "Unable to initialize mISDN\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
	
	if (misdn_cfg_init(max_ports, 0)) {
		tris_log(LOG_ERROR, "Unable to initialize misdn_config.\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
	g_config_initialized = 1;
	
	misdn_debug = tris_malloc(sizeof(int) * (max_ports + 1));
	if (!misdn_debug) {
		tris_log(LOG_ERROR, "Out of memory for misdn_debug\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
	misdn_ports = tris_malloc(sizeof(int) * (max_ports + 1));
	if (!misdn_ports) {
		tris_free(misdn_debug);
		tris_log(LOG_ERROR, "Out of memory for misdn_ports\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
	...
	*misdn_ports = 0;
	misdn_debug_only = tris_calloc(max_ports + 1, sizeof(int));
	if (!misdn_debug_only) {
		tris_free(misdn_ports);
		tris_free(misdn_debug);
		tris_log(LOG_ERROR, "Out of memory for misdn_debug_only\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}

	misdn_cfg_get(0, MISDN_GEN_TRACEFILE, tempbuf, sizeof(tempbuf));
	if (!tris_strlen_zero(tempbuf)) {
		tracing = 1;
	}

	misdn_in_calls = tris_malloc(sizeof(int) * (max_ports + 1));
	if (!misdn_in_calls) {
		tris_free(misdn_debug_only);
		tris_free(misdn_ports);
		tris_free(misdn_debug);
		tris_log(LOG_ERROR, "Out of memory for misdn_in_calls\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
	misdn_out_calls = tris_malloc(sizeof(int) * (max_ports + 1));
	if (!misdn_out_calls) {
		tris_free(misdn_in_calls);
		tris_free(misdn_debug_only);
		tris_free(misdn_ports);
		tris_free(misdn_debug);
		tris_log(LOG_ERROR, "Out of memory for misdn_out_calls\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
	...
	tris_mutex_init(&cl_te_lock);
	tris_mutex_init(&release_lock);

	misdn_cfg_update_ptp();
	misdn_cfg_get_ports_string(ports);

	if (!tris_strlen_zero(ports)) {
	...
	if (tris_channel_register(&misdn_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel class %s\n", misdn_type);
		unload_module();
		return TRIS_MODULE_LOAD_DECLINE;
	}
  
	tris_cli_register_multiple(chan_misdn_clis, sizeof(chan_misdn_clis) / sizeof(struct tris_cli_entry));

	tris_register_application("misdn_set_opt", misdn_set_opt_exec, "misdn_set_opt",
		"misdn_set_opt(:<opt><optarg>:<opt><optarg>...):\n"
		"Sets mISDN opts. and optargs\n"
		"\n"
		"The available options are:\n"
		"    a - Have Trismedia detect DTMF tones on called channel\n"
		...
		"    i - Ignore detected DTMF tones, don't signal them to Trismedia,\n"
	...
	tris_register_application("misdn_facility", misdn_facility_exec, "misdn_facility",
	...
	tris_register_application("misdn_check_l2l1", misdn_check_l2l1, "misdn_check_l2l1",
...
static int misdn_facility_exec(struct tris_channel *chan, void *data)
{
	struct chan_list *ch = MISDN_TRISMEDIA_TECH_PVT(chan);
	char *parse;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(facility_type);
		TRIS_APP_ARG(arg)[99];
	);
	...
	if (strcasecmp(chan->tech->type, "mISDN")) {
		tris_log(LOG_WARNING, "misdn_facility makes only sense with chan_misdn channels!\n");
		return -1;
	}
	
	if (tris_strlen_zero((char *)data)) {
		tris_log(LOG_WARNING, "misdn_facility requires arguments: facility_type[,<args>]\n");
		return -1;
	}
	...
	if (tris_strlen_zero(args.facility_type)) {
		tris_log(LOG_WARNING, "misdn_facility requires arguments: facility_type[,<args>]\n");
		return -1;
	}

	if (!strcasecmp(args.facility_type, "calldeflect")) {
		if (tris_strlen_zero(args.arg[0])) {
			tris_log(LOG_WARNING, "Facility: Call Deflection requires an argument: Number\n");
		}

		if (strlen(args.arg[0]) >= sizeof(ch->bc->fac_out.u.CDeflection.DeflectedToNumber)) {
			tris_log(LOG_WARNING, "Facility: Number argument too long (up to %d digits are allowed). Ignoring.\n", (int)sizeof(ch->bc->fac_out.u.CDeflection.DeflectedToNumber));
			return 0;
		}
		ch->bc->fac_out.Function = Fac_CD;
		tris_copy_string((char *)ch->bc->fac_out.u.CDeflection.DeflectedToNumber, args.arg[0], sizeof(ch->bc->fac_out.u.CDeflection.DeflectedToNumber));
...
static int misdn_check_l2l1(struct tris_channel *chan, void *data)
{
	...
	TRIS_DECLARE_APP_ARGS(args,
			TRIS_APP_ARG(grouppar);
			TRIS_APP_ARG(timeout);
	);

	if (tris_strlen_zero((char *)data)) {
		tris_log(LOG_WARNING, "misdn_check_l2l1 Requires arguments\n");
		return -1;
	}
	...
	if (args.argc != 2) {
		tris_log(LOG_WARNING, "Wrong argument count\n");
		return 0;
	}

	/*tris_log(LOG_NOTICE, "Arguments: group/port '%s' timeout '%s'\n", args.grouppar, args.timeout);*/
	...
	if (dowait) {
		...
		tris_safe_sleep(chan, timeout * 1000);
	}
...
static int misdn_set_opt_exec(struct tris_channel *chan, void *data)
{
	...
	if (strcasecmp(chan->tech->type, "mISDN")) {
		tris_log(LOG_WARNING, "misdn_set_opt makes only sense with chan_misdn channels!\n");
		return -1;
	}
	
	if (tris_strlen_zero((char *)data)) {
		tris_log(LOG_WARNING, "misdn_set_opt Requires arguments\n");
		return -1;
	}
	...
	for (tok = strtok_r(parse, ":", &tokb);
		...
		case 'd' :
			tris_copy_string(ch->bc->display, ++tok, sizeof(ch->bc->display));
		...
		case 'c':
			keyidx = atoi(++tok);
			{
				...
				if (key) {
					tris_copy_string(ch->bc->crypt_key, key, sizeof(ch->bc->crypt_key));
				}
		...
		case 'a':
			chan_misdn_log(1, ch->bc->port, "SETOPT: TRIS_DSP (for DTMF)\n");
			ch->tris_dsp = 1;
			break;
	...
	if (ch->faxdetect || ch->tris_dsp) {
		...
		if (ch->dsp) {
			tris_dsp_set_features(ch->dsp, DSP_FEATURE_DIGIT_DETECT | DSP_FEATURE_FAX_DETECT);
		}
	}

	if (ch->tris_dsp) {
		chan_misdn_log(1, ch->bc->port, "SETOPT: with TRIS_DSP we deactivate mISDN_dsp\n");
		ch->bc->nodsp = 1;
	}
...
struct misdn_jb *misdn_jb_init(int size, int upper_threshold)
{
	...
	jb->state_empty = 0;
	jb->bytes_wrote = 0;
	jb->samples = tris_malloc(size * sizeof(char));
	if (!jb->samples) {
		tris_free(jb);
	...
	jb->ok = tris_malloc(size * sizeof(char));
	if (!jb->ok) {
		tris_free(jb->samples);
		tris_free(jb);
...
static void chan_misdn_log(int level, int port, char *tmpl, ...)
{
	...
	if (! ((0 <= port) && (port <= max_ports))) {
		tris_log(LOG_WARNING, "cb_log called with out-of-range port number! (%d)\n", port);
		port = 0;
		level = -1;
	}
	...
	if (level == -1) {
		...
		tris_console_puts(port_buf);
		tris_console_puts(buf);
	}
	
	if ((level <= misdn_debug[0]) && !tris_strlen_zero(global_tracefile) ) {
		char ctimebuf[30];
		time_t tm = time(NULL);
		...
		if (!fp) {
			tris_console_puts("Error opening Tracefile: [ ");
			tris_console_puts(global_tracefile);
			tris_console_puts(" ] ");
			
			tris_console_puts(strerror(errno));
			tris_console_puts("\n");
			return ;
		}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Channel driver for mISDN Support (BRI/PRI)",
	.load = load_module,
