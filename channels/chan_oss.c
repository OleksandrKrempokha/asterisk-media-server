...
#define	BOOST_MAX	40			/*!< slightly less than 7 bits */
	...
	int oss_write_dst;
	/*! buffers used in oss_read - TRIS_FRIENDLY_OFFSET space for headers
	 *  plus enough room for a full frame
	 */
	char oss_read_buf[FRAME_SIZE * 2 + TRIS_FRIENDLY_OFFSET];
	int readpos;				/*!< read position above */
	struct tris_frame read_f;	/*!< returned by oss_read */
};
...
struct video_desc *get_video_desc(struct tris_channel *c)
{
...
static struct tris_channel *oss_request(const char *type, int format, void *data, int *cause, struct tris_channel* src);
static int oss_digit_begin(struct tris_channel *c, char digit);
static int oss_digit_end(struct tris_channel *c, char digit, unsigned int duration);
static int oss_text(struct tris_channel *c, const char *text);
static int oss_hangup(struct tris_channel *c);
static int oss_answer(struct tris_channel *c);
static struct tris_frame *oss_read(struct tris_channel *chan);
static int oss_call(struct tris_channel *c, char *dest, int timeout);
static int oss_write(struct tris_channel *chan, struct tris_frame *f);
static int oss_indicate(struct tris_channel *chan, int cond, const void *data, size_t datalen);
static int oss_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
static char tdesc[] = "OSS Console Channel Driver";

/* cannot do const because need to update some fields at runtime */
static struct tris_channel_tech oss_tech = {
	.type = "Console",
	.description = tdesc,
	.capabilities = TRIS_FORMAT_SLINEAR, /* overwritten later */
...
static int setformat(struct chan_oss_pvt *o, int mode)
{
	...
	if (mode == O_CLOSE)		/* we are done */
		return 0;
	if (tris_tvdiff_ms(tris_tvnow(), o->lastopen) < 1000)
		return -1;				/* don't open too often */
	o->lastopen = tris_tvnow();
	fd = o->sounddev = open(o->device, mode | O_NONBLOCK);
	if (fd < 0) {
		tris_log(LOG_WARNING, "Unable to re-open DSP device %s: %s\n", o->device, strerror(errno));
		return -1;
	}
	if (o->owner)
		tris_channel_set_fd(o->owner, 0, fd);
	...
	case O_RDWR:
		...
		if (res == 0 && (fmt & DSP_CAP_DUPLEX)) {
			tris_verb(2, "Console is full duplex\n");
			o->duplex = M_FULL;
		};
		break;
...
static int oss_call(struct tris_channel *c, char *dest, int timeout)
{
	...
	char *parse = tris_strdupa(dest);

	TRIS_NONSTANDARD_APP_ARGS(args, parse, '/');

	tris_verbose(" << Call to device '%s' dnid '%s' rdnis '%s' on console from '%s' <%s> >>\n", dest, c->cid.cid_dnid, c->cid.cid_rdnis, c->cid.cid_name, c->cid.cid_num);
	if (!tris_strlen_zero(args.flags) && strcasecmp(args.flags, "answer") == 0) {
		f.frametype = TRIS_FRAME_CONTROL;
		f.subclass = TRIS_CONTROL_ANSWER;
		tris_queue_frame(c, &f);
	} else if (!tris_strlen_zero(args.flags) && strcasecmp(args.flags, "noanswer") == 0) {
		f.frametype = TRIS_FRAME_CONTROL;
...
static int oss_write(struct tris_channel *c, struct tris_frame *f)
{
	int src;
	struct chan_oss_pvt *o = c->tech_pvt;
...
static struct tris_frame *oss_read(struct tris_channel *c)
{
	int res;
	struct chan_oss_pvt *o = c->tech_pvt;
	struct tris_frame *f = &o->read_f;
	...
	f->data.ptr = o->oss_read_buf + TRIS_FRIENDLY_OFFSET;
...
static int oss_indicate(struct tris_channel *c, int cond, const void *data, size_t datalen)
{
	struct chan_oss_pvt *o = c->tech_pvt;
	int res = 0;
 ...
 */
static struct tris_channel *oss_new(struct chan_oss_pvt *o, char *ext, char *ctx, int state)
{
	struct tris_channel *c;

	c = tris_channel_alloc(1, state, o->cid_num, o->cid_name, "", ext, ctx, 0, "Console/%s", o->device + 5);
	if (c == NULL)
		return NULL;
	c->tech = &oss_tech;
	if (o->sounddev < 0)
		setformat(o, O_RDWR);
	tris_channel_set_fd(c, 0, o->sounddev); /* -1 if device closed, override later */
	c->nativeformats = TRIS_FORMAT_SLINEAR;
	/* if the console makes the call, add video to the offer */
	if (state == TRIS_STATE_RINGING)
...
static struct tris_channel *oss_request(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	...
	tris_log(LOG_WARNING, "oss_request ty <%s> data 0x%p <%s>\n", type, data, (char *) data);
	if (o == NULL) {
	...
	if (o->owner) {
		tris_log(LOG_NOTICE, "Already have a call (chan %p) on the OSS channel\n", o->owner);
		*cause = TRIS_CAUSE_BUSY;
		return NULL;
	}
	c = oss_new(o, NULL, NULL, TRIS_STATE_DOWN);
	if (c == NULL) {
		tris_log(LOG_WARNING, "Unable to create new OSS channel\n");
		return NULL;
	}
	return c;
}
 ...
 */
static char *console_cmd(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct chan_oss_pvt *o = find_desc(oss_active);
	...
	if (o == NULL) {
		tris_log(LOG_WARNING, "Cannot find device %s (should not happen!)\n",
			oss_active);
		return CLI_FAILURE;
	}
	var = a->argv[e->args-1];
...
static char *console_do_answer(int fd)
{
	...
	if (!o->owner) {
		if (fd > -1)
			tris_cli(fd, "No one is calling us\n");
		return CLI_FAILURE;
	}
	o->hookstate = 1;
	tris_queue_frame(o->owner, &f);
	return CLI_SUCCESS;
}
...
static char *console_hangup(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	o->hookstate = 0;
	if (o->owner)
		tris_queue_hangup_with_cause(o->owner, TRIS_CAUSE_NORMAL_CLEARING);
	setformat(o, O_CLOSE);
	return CLI_SUCCESS;
}

static char *console_flash(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
static char *console_dial(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (a->argc == e->args + 1)
		s = tris_ext_ctx(a->argv[e->args], &mye, &myc);
	...
	if (tris_exists_extension(NULL, myc, mye, 1, NULL)) {
		o->hookstate = 1;
		oss_new(o, mye, myc, TRIS_STATE_RINGING);
	} else
...
static char *console_mute(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct chan_oss_pvt *o = find_desc(oss_active);
	char *s;
	int toggle = 0;
	...
	if (!strcasecmp(s, "mute"))
		...
		return CLI_SHOWUSAGE;
	tris_cli(a->fd, "Console mic is %s\n", o->mute ? "off" : "on");
	return CLI_SUCCESS;
}

static char *console_transfer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct chan_oss_pvt *o = find_desc(oss_active);
	struct tris_channel *b = NULL;
	char *tmp, *ext, *ctx;
	...
	if (o == NULL)
		return CLI_FAILURE;
	if (o->owner == NULL || (b = tris_bridged_channel(o->owner)) == NULL) {
		tris_cli(a->fd, "There is no call to transfer\n");
		return CLI_SUCCESS;
	}

	tmp = tris_ext_ctx(a->argv[2], &ext, &ctx);
	if (ctx == NULL)			/* supply default context if needed */
		ctx = o->owner->context;
	if (!tris_exists_extension(b, ctx, ext, 1, b->cid.cid_num))
		tris_cli(a->fd, "No such extension exists\n");
	else {
		tris_cli(a->fd, "Whee, transferring %s to %s@%s.\n", b->name, ext, ctx);
		if (tris_async_goto(b, ctx, ext, 1))
			tris_cli(a->fd, "Failed to transfer :(\n");
	}
	if (tmp)
		tris_free(tmp);
	return CLI_SUCCESS;
...
static char *console_active(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (a->argc == 3)
		tris_cli(a->fd, "active console is [%s]\n", oss_active);
	else if (a->argc != 4)
		return CLI_SHOWUSAGE;
	else {
		struct chan_oss_pvt *o;
		if (strcmp(a->argv[3], "show") == 0) {
			for (o = oss_default.next; o; o = o->next)
				tris_cli(a->fd, "device [%s] exists\n", o->name);
			return CLI_SUCCESS;
		}
		o = find_desc(a->argv[3]);
		if (o == NULL)
			tris_cli(a->fd, "No device [%s] exists\n", a->argv[3]);
		else
			oss_active = o->name;
	}
	return CLI_SUCCESS;
...
static void store_boost(struct chan_oss_pvt *o, const char *s)
{
	...
	if (sscanf(s, "%30lf", &boost) != 1) {
		tris_log(LOG_WARNING, "invalid boost <%s>\n", s);
		return;
	}
	if (boost < -BOOST_MAX) {
		tris_log(LOG_WARNING, "boost %s too small, using %d\n", s, -BOOST_MAX);
		boost = -BOOST_MAX;
	} else if (boost > BOOST_MAX) {
		tris_log(LOG_WARNING, "boost %s too large, using %d\n", s, BOOST_MAX);
		boost = BOOST_MAX;
	}
	boost = exp(log(10) * boost / 20) * BOOST_SCALE;
	o->boost = boost;
	tris_log(LOG_WARNING, "setting boost %s to %d\n", s, o->boost);
}

static char *console_boost(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct chan_oss_pvt *o = find_desc(oss_active);
	...
	if (a->argc == 2)
		tris_cli(a->fd, "boost currently %5.1f\n", 20 * log10(((double) o->boost / (double) BOOST_SCALE)));
	else if (a->argc == 3)
		store_boost(o, a->argv[2]);
	return CLI_SUCCESS;
}

static struct tris_cli_entry cli_oss[] = {
	...
	TRIS_CLI_DEFINE(console_flash, "Flash a call on the console"),
	TRIS_CLI_DEFINE(console_dial, "Dial an extension on the console"),
	TRIS_CLI_DEFINE(console_mute, "Disable/Enable mic input"),
	TRIS_CLI_DEFINE(console_transfer, "Transfer a call to a different extension"),	
	TRIS_CLI_DEFINE(console_cmd, "Generic console command"),
	...
	TRIS_CLI_DEFINE(console_boost, "Sets/displays mic boost in dB"),
	TRIS_CLI_DEFINE(console_active, "Sets/displays active console"),
};
...
static void store_mixer(struct chan_oss_pvt *o, const char *s)
{
	...
	for (i = 0; i < strlen(s); i++) {
		if (!isalnum(s[i]) && strchr(" \t-/", s[i]) == NULL) {
			tris_log(LOG_WARNING, "Suspect char %c in mixer cmd, ignoring:\n\t%s\n", s[i], s);
			return;
		}
	}
	if (o->mixer_cmd)
		tris_free(o->mixer_cmd);
	o->mixer_cmd = tris_strdup(s);
	tris_log(LOG_WARNING, "setting mixer %s\n", s);
}
...
static void store_config_core(struct chan_oss_pvt *o, const char *var, const char *value)
{
	...
	/* handle jb conf */
	if (!tris_jb_read_conf(&global_jbconf, var, value))
		return;
 ...
 */
static struct chan_oss_pvt *store_config(struct tris_config *cfg, char *ctg)
{
	...
	if (tris_strlen_zero(o->device))
		tris_copy_string(o->device, DEV_DSP, sizeof(o->device));
	if (o->mixer_cmd) {
		char *cmd;
		...
		if (asprintf(&cmd, "mixer %s", o->mixer_cmd) < 0) {
			...
			tris_log(LOG_WARNING, "running [%s]\n", cmd);
			if (system(cmd) < 0) {
				tris_log(LOG_WARNING, "system() failed: %s\n", strerror(errno));
			}
			tris_free(cmd);
		}
	}
...
openit:
	...
	if (setformat(o, O_RDWR) < 0) {	/* open device */
		tris_verb(1, "Device %s not detected\n", ctg);
		tris_verb(1, "Turn off OSS support by adding " "'noload=chan_oss.so' in /etc/trismedia/modules.conf\n");
		goto error;
	}
	if (o->duplex != M_FULL)
		tris_log(LOG_WARNING, "XXX I don't work right with non " "full-duplex sound cards XXX\n");
#endif /* TRYOPEN */
...
static int load_module(void)
{
	...
	if (tris_channel_register(&oss_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel type 'OSS'\n");
		return TRIS_MODULE_LOAD_FAILURE;
	}

	tris_cli_register_multiple(cli_oss, ARRAY_LEN(cli_oss));
...
static int unload_module(void)
{
	...
	tris_channel_unregister(&oss_tech);
	tris_cli_unregister_multiple(cli_oss, ARRAY_LEN(cli_oss));
...
TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "OSS Console Channel Driver");
