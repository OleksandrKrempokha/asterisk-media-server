...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 218219 $")
...
static int prefformat = TRIS_FORMAT_G729A | TRIS_FORMAT_G723_1 | TRIS_FORMAT_SLINEAR | TRIS_FORMAT_ULAW;
...
static struct tris_channel *phone_request(const char *type, int format, void *data, int *cause, struct tris_channel* src);
static int phone_digit_begin(struct tris_channel *ast, char digit);
static int phone_digit_end(struct tris_channel *ast, char digit, unsigned int duration);
static int phone_call(struct tris_channel *ast, char *dest, int timeout);
static int phone_hangup(struct tris_channel *ast);
static int phone_answer(struct tris_channel *ast);
static struct tris_frame *phone_read(struct tris_channel *ast);
static int phone_write(struct tris_channel *ast, struct tris_frame *frame);
static struct tris_frame *phone_exception(struct tris_channel *ast);
static int phone_send_text(struct tris_channel *ast, const char *text);
static int phone_fixup(struct tris_channel *old, struct tris_channel *new);
static int phone_indicate(struct tris_channel *chan, int condition, const void *data, size_t datalen);

static const struct tris_channel_tech phone_tech = {
	.type = "Phone",
	.description = tdesc,
	.capabilities = TRIS_FORMAT_G723_1 | TRIS_FORMAT_SLINEAR | TRIS_FORMAT_ULAW | TRIS_FORMAT_G729A,
...
static struct tris_channel_tech phone_tech_fxs = {
	.type = "Phone",
	.description = tdesc,
	.requester = phone_request,
	.send_digit_begin = phone_digit_begin,
...
static struct tris_channel_tech *cur_tech;
...
static int phone_indicate(struct tris_channel *chan, int condition, const void *data, size_t datalen)
{
	...
	case TRIS_CONTROL_SRCUPDATE:
		...
		break;
	default:
		tris_log(LOG_WARNING, "Condition %d is not supported on channel %s\n", condition, chan->name);
	}
	return res;
}
...
static int phone_digit_begin(struct tris_channel *chan, char digit)
{
	/* XXX Modify this callback to let Trismedia support controlling the length of DTMF */
	return 0;
}
...
static int phone_digit_end(struct tris_channel *ast, char digit, unsigned int duration)
{
	...
	case 'F':
		...
		return 0;
	default:
		tris_log(LOG_WARNING, "Unknown digit '%c'\n", digit);
		return -1;
	}
	tris_debug(1, "Dialed %d\n", outdigit);
...
static int phone_call(struct tris_channel *ast, char *dest, int timeout)
{
	...
	PHONE_CID cid;
	struct timeval UtcTime = tris_tvnow();
	struct tris_tm tm;
	int start;

	tris_localtime(&UtcTime, &tm, NULL);
	...
	if (tris_strlen_zero(ast->cid.cid_name))
		strcpy(cid.name, DEFAULT_CALLER_ID);
	else
		tris_copy_string(cid.name, ast->cid.cid_name, sizeof(cid.name));

	if (ast->cid.cid_num) 
		tris_copy_string(cid.number, ast->cid.cid_num, sizeof(cid.number));

	p = ast->tech_pvt;

	if ((ast->_state != TRIS_STATE_DOWN) && (ast->_state != TRIS_STATE_RESERVED)) {
		tris_log(LOG_WARNING, "phone_call called on %s, neither down nor reserved\n", ast->name);
		return -1;
	}
	tris_debug(1, "Ringing %s on %s (%d)\n", dest, ast->name, ast->fds[0]);
...
static int phone_hangup(struct tris_channel *ast)
{
	...
	if (ioctl(p->fd, PHONE_REC_STOP))
		tris_log(LOG_WARNING, "Failed to stop recording\n");
	if (ioctl(p->fd, PHONE_PLAY_STOP))
		tris_log(LOG_WARNING, "Failed to stop playing\n");
	if (ioctl(p->fd, PHONE_RING_STOP))
		tris_log(LOG_WARNING, "Failed to stop ringing\n");
	if (ioctl(p->fd, PHONE_CPT_STOP))
		tris_log(LOG_WARNING, "Failed to stop sounds\n");

	/* If it's an FXO, hang them up */
	if (p->mode == MODE_FXO) {
		if (ioctl(p->fd, PHONE_PSTN_SET_STATE, PSTN_ON_HOOK))
			tris_debug(1, "ioctl(PHONE_PSTN_SET_STATE) failed on %s (%s)\n",ast->name, strerror(errno));
	}

	/* If they're off hook, give a busy signal */
	if (ioctl(p->fd, PHONE_HOOKSTATE)) {
		tris_debug(1, "Got hunghup, giving busy signal\n");
...
static int phone_setup(struct tris_channel *ast)
{
	struct phone_pvt *p;
	p = ast->tech_pvt;
	ioctl(p->fd, PHONE_CPT_STOP);
	/* Nothing to answering really, just start recording */
	if (ast->rawreadformat == TRIS_FORMAT_G729A) {
		/* Prefer g729 */
		ioctl(p->fd, PHONE_REC_STOP);
		if (p->lastinput != TRIS_FORMAT_G729A) {
			p->lastinput = TRIS_FORMAT_G729A;
			if (ioctl(p->fd, PHONE_REC_CODEC, G729)) {
				tris_log(LOG_WARNING, "Failed to set codec to g729\n");
				return -1;
			}
		}
        } else if (ast->rawreadformat == TRIS_FORMAT_G723_1) {
		ioctl(p->fd, PHONE_REC_STOP);
		if (p->lastinput != TRIS_FORMAT_G723_1) {
			p->lastinput = TRIS_FORMAT_G723_1;
			if (ioctl(p->fd, PHONE_REC_CODEC, G723_63)) {
				tris_log(LOG_WARNING, "Failed to set codec to g723.1\n");
				return -1;
			}
		}
	} else if (ast->rawreadformat == TRIS_FORMAT_SLINEAR) {
		ioctl(p->fd, PHONE_REC_STOP);
		if (p->lastinput != TRIS_FORMAT_SLINEAR) {
			p->lastinput = TRIS_FORMAT_SLINEAR;
			if (ioctl(p->fd, PHONE_REC_CODEC, LINEAR16)) {
				tris_log(LOG_WARNING, "Failed to set codec to signed linear 16\n");
				return -1;
			}
		}
	} else if (ast->rawreadformat == TRIS_FORMAT_ULAW) {
		ioctl(p->fd, PHONE_REC_STOP);
		if (p->lastinput != TRIS_FORMAT_ULAW) {
			p->lastinput = TRIS_FORMAT_ULAW;
			if (ioctl(p->fd, PHONE_REC_CODEC, ULAW)) {
				tris_log(LOG_WARNING, "Failed to set codec to uLaw\n");
				return -1;
			}
		}
		...
		if (p->lastinput != ast->rawreadformat) {
			...
			if (ioctl(p->fd, PHONE_REC_CODEC, ast->rawreadformat)) {
				tris_log(LOG_WARNING, "Failed to set codec to %d\n", 
					ast->rawreadformat);
				return -1;
			}
		}
	} else {
		tris_log(LOG_WARNING, "Can't do format %s\n", tris_getformatname(ast->rawreadformat));
		return -1;
	}
	if (ioctl(p->fd, PHONE_REC_START)) {
		tris_log(LOG_WARNING, "Failed to start recording\n");
		return -1;
	}
...
static int phone_answer(struct tris_channel *ast)
{
	...
	if (p->mode == MODE_FXO) {
		if (ioctl(p->fd, PHONE_PSTN_SET_STATE, PSTN_OFF_HOOK))
			...
			tris_debug(1, "Took linejack off hook\n");
	}
	phone_setup(ast);
	tris_debug(1, "phone_answer(%s)\n", ast->name);
	ast->rings = 0;
...
static struct tris_frame  *phone_exception(struct tris_channel *ast)
{
	...
	p->fr.offset = 0;
	p->fr.mallocd=0;
	p->fr.delivery = tris_tv(0,0);
	
	phonee.bytes = ioctl(p->fd, PHONE_EXCEPTION);
	if (phonee.bits.dtmf_ready)  {
		tris_debug(1, "phone_exception(): DTMF\n");
	
		/* We've got a digit -- Just handle this nicely and easily */
		digit =  ioctl(p->fd, PHONE_GET_DTMF_ASCII);
		p->fr.subclass = digit;
		p->fr.frametype = TRIS_FRAME_DTMF;
		return &p->fr;
	}
	if (phonee.bits.hookstate) {
		tris_debug(1, "Hookstate changed\n");
		res = ioctl(p->fd, PHONE_HOOKSTATE);
		/* See if we've gone on hook, if so, notify by returning NULL */
		tris_debug(1, "New hookstate: %d\n", res);
		if (!res && (p->mode != MODE_FXO))
			return NULL;
		else {
			if (ast->_state == TRIS_STATE_RINGING) {
				/* They've picked up the phone */
				p->fr.frametype = TRIS_FRAME_CONTROL;
				p->fr.subclass = TRIS_CONTROL_ANSWER;
				phone_setup(ast);
				tris_setstate(ast, TRIS_STATE_UP);
				return &p->fr;
			}  else 
				tris_log(LOG_WARNING, "Got off hook in weird state %d\n", ast->_state);
		}
	}
#if 1
	if (phonee.bits.pstn_ring)
		tris_verbose("Unit is ringing\n");
	if (phonee.bits.caller_id) {
		tris_verbose("We have caller ID\n");
	}
	if (phonee.bits.pstn_wink)
		tris_verbose("Detected Wink\n");
#endif
	/* Strange -- nothing there.. */
	p->fr.frametype = TRIS_FRAME_NULL;
	p->fr.subclass = 0;
	return &p->fr;
}
...
static struct tris_frame  *phone_read(struct tris_channel *ast)
{
...
#if 0
		if (errno == EAGAIN) {
			tris_log(LOG_WARNING, "Null frame received\n");
			p->fr.frametype = TRIS_FRAME_NULL;
			p->fr.subclass = 0;
			return &p->fr;
		}
#endif
		tris_log(LOG_WARNING, "Error reading: %s\n", strerror(errno));
		return NULL;
	}
	p->fr.data.ptr = p->buf;
	if (p->mode != MODE_FXS)
	...
	p->fr.samples = 240;
	p->fr.datalen = res;
	p->fr.frametype = p->lastinput <= TRIS_FORMAT_AUDIO_MASK ?
                          TRIS_FRAME_VOICE : 
			  p->lastinput <= TRIS_FORMAT_PNG ? TRIS_FRAME_IMAGE 
			  : TRIS_FRAME_VIDEO;
	p->fr.subclass = p->lastinput;
	p->fr.offset = TRIS_FRIENDLY_OFFSET;
	/* Byteswap from little-endian to native-endian */
	if (p->fr.subclass == TRIS_FORMAT_SLINEAR)
		tris_frame_byteswap_le(&p->fr);
	return &p->fr;
}
...
static int phone_write_buf(struct phone_pvt *p, const char *buf, int len, int frlen, int swap)
{
	...
	if (space < len)
		len = space;
	if (swap)
		tris_swapcopy_samples(p->obuf+p->obuflen, buf, len/2);
	else
 ...
 */
				...
				tris_log(LOG_WARNING, "Only wrote %d of %d bytes\n", res, frlen);
			}
		}
		p->obuflen -= frlen;
...
static int phone_write(struct tris_channel *ast, struct tris_frame *frame)
{
	...
	if (frame->frametype != TRIS_FRAME_VOICE && p->mode != MODE_FXS) {
		if (frame->frametype != TRIS_FRAME_IMAGE)
			tris_log(LOG_WARNING, "Don't know what to do with  frame type '%d'\n", frame->frametype);
		return 0;
	}
	if (!(frame->subclass &
		(TRIS_FORMAT_G723_1 | TRIS_FORMAT_SLINEAR | TRIS_FORMAT_ULAW | TRIS_FORMAT_G729A)) && 
	    p->mode != MODE_FXS) {
...
#endif	
	if (frame->subclass == TRIS_FORMAT_G729A) {
		if (p->lastformat != TRIS_FORMAT_G729A) {
			ioctl(p->fd, PHONE_PLAY_STOP);
			ioctl(p->fd, PHONE_REC_STOP);
			if (ioctl(p->fd, PHONE_PLAY_CODEC, G729)) {
				tris_log(LOG_WARNING, "Unable to set G729 mode\n");
				return -1;
			}
			if (ioctl(p->fd, PHONE_REC_CODEC, G729)) {
				tris_log(LOG_WARNING, "Unable to set G729 mode\n");
			...
			p->lastformat = TRIS_FORMAT_G729A;
			p->lastinput = TRIS_FORMAT_G729A;
			/* Reset output buffer */
			p->obuflen = 0;
			codecset = 1;
		...
		if (frame->datalen > 80) {
			tris_log(LOG_WARNING, "Frame size too large for G.729 (%d bytes)\n", frame->datalen);
			return -1;
		}
		maxfr = 80;
        } else if (frame->subclass == TRIS_FORMAT_G723_1) {
		if (p->lastformat != TRIS_FORMAT_G723_1) {
			ioctl(p->fd, PHONE_PLAY_STOP);
			ioctl(p->fd, PHONE_REC_STOP);
			if (ioctl(p->fd, PHONE_PLAY_CODEC, G723_63)) {
				tris_log(LOG_WARNING, "Unable to set G723.1 mode\n");
				return -1;
			}
			if (ioctl(p->fd, PHONE_REC_CODEC, G723_63)) {
				tris_log(LOG_WARNING, "Unable to set G723.1 mode\n");
			...
			p->lastformat = TRIS_FORMAT_G723_1;
			p->lastinput = TRIS_FORMAT_G723_1;
			/* Reset output buffer */
			p->obuflen = 0;
			codecset = 1;
		...
		if (frame->datalen > 24) {
			tris_log(LOG_WARNING, "Frame size too large for G.723.1 (%d bytes)\n", frame->datalen);
			return -1;
		}
		maxfr = 24;
	} else if (frame->subclass == TRIS_FORMAT_SLINEAR) {
		if (p->lastformat != TRIS_FORMAT_SLINEAR) {
			ioctl(p->fd, PHONE_PLAY_STOP);
			ioctl(p->fd, PHONE_REC_STOP);
			if (ioctl(p->fd, PHONE_PLAY_CODEC, LINEAR16)) {
				tris_log(LOG_WARNING, "Unable to set 16-bit linear mode\n");
				return -1;
			}
			if (ioctl(p->fd, PHONE_REC_CODEC, LINEAR16)) {
				tris_log(LOG_WARNING, "Unable to set 16-bit linear mode\n");
			...
			p->lastformat = TRIS_FORMAT_SLINEAR;
			p->lastinput = TRIS_FORMAT_SLINEAR;
			codecset = 1;
			/* Reset output buffer */
			p->obuflen = 0;
		...
		maxfr = 480;
	} else if (frame->subclass == TRIS_FORMAT_ULAW) {
		if (p->lastformat != TRIS_FORMAT_ULAW) {
			ioctl(p->fd, PHONE_PLAY_STOP);
			ioctl(p->fd, PHONE_REC_STOP);
			if (ioctl(p->fd, PHONE_PLAY_CODEC, ULAW)) {
				tris_log(LOG_WARNING, "Unable to set uLaw mode\n");
				return -1;
			}
			if (ioctl(p->fd, PHONE_REC_CODEC, ULAW)) {
				tris_log(LOG_WARNING, "Unable to set uLaw mode\n");
		...
		if (p->lastformat != frame->subclass) {
			...
			if (ioctl(p->fd, PHONE_PLAY_CODEC, frame->subclass)) {
				tris_log(LOG_WARNING, "Unable to set %d mode\n",
					frame->subclass);
				return -1;
			}
 	...
 	if (codecset) {
		...
		if (ioctl(p->fd, PHONE_PLAY_START)) {
			tris_log(LOG_WARNING, "Failed to start playback\n");
			return -1;
		}
		if (ioctl(p->fd, PHONE_REC_START)) {
			tris_log(LOG_WARNING, "Failed to start recording\n");
	...
	while(sofar < frame->datalen) {
		...
		if (res != expected) {
			if ((errno != EAGAIN) && (errno != EINTR)) {
				if (res < 0) 
					tris_log(LOG_WARNING, "Write returned error (%s)\n", strerror(errno));
	/*
	 * Card is in non-blocking mode now and it works well now, but there are
	 * lot of messages like this. So, this message is temporarily disabled.
	 */
...
static struct tris_channel *phone_new(struct phone_pvt *i, int state, char *cntx)
{
	struct tris_channel *tmp;
	struct phone_codec_data queried_codec;
	tmp = tris_channel_alloc(1, state, i->cid_num, i->cid_name, "", i->ext, i->context, 0, "Phone/%s", i->dev + 5);
	if (tmp) {
		tmp->tech = cur_tech;
		tris_channel_set_fd(tmp, 0, i->fd);
		...
		if (i->mode == MODE_FXS &&
			...
			if (queried_codec.type == LINEAR16)
				...
				tmp->rawreadformat =
				tmp->rawwriteformat =
				prefformat & ~TRIS_FORMAT_SLINEAR;
			}
		}
		else {
		...
		/* no need to call tris_setstate: the channel_alloc already did its job */
		if (state == TRIS_STATE_RING)
			tmp->rings = 1;
		tmp->tech_pvt = i;
		tris_copy_string(tmp->context, cntx, sizeof(tmp->context));
		if (!tris_strlen_zero(i->ext))
			tris_copy_string(tmp->exten, i->ext, sizeof(tmp->exten));
		else
			strcpy(tmp->exten, "s");
...
static void phone_mini_packet(struct phone_pvt *i)
{
	...
	if (res < 1) {
		tris_log(LOG_WARNING, "Read returned %d: %s\n", res, strerror(errno));
		return;
	}
}
...
static void phone_check_exception(struct phone_pvt *i)
{
	...
	if (phonee.bits.dtmf_ready)  {
		...
		if (i->mode == MODE_DIALTONE || i->mode == MODE_FXS || i->mode == MODE_SIGMA) {
			...
			i->dialtone = 0;
			if (strlen(i->ext) < TRIS_MAX_EXTENSION - 1)
				strncat(i->ext, digit, sizeof(i->ext) - strlen(i->ext) - 1);
			if ((i->mode != MODE_FXS ||
			     !(phonee.bytes = ioctl(i->fd, PHONE_EXCEPTION)) ||
			     !phonee.bits.dtmf_ready) &&
			    tris_exists_extension(NULL, i->context, i->ext, 1, i->cid_num)) {
				/* It's a valid extension in its context, get moving! */
				phone_new(i, TRIS_STATE_RING, i->context);
				/* No need to restart monitor, we are the monitor */
			} else if (!tris_canmatch_extension(NULL, i->context, i->ext, 1, i->cid_num)) {
				/* There is nothing in the specified extension that can match anymore.
				   Try the default */
				if (tris_exists_extension(NULL, "default", i->ext, 1, i->cid_num)) {
					/* Check the default, too... */
					phone_new(i, TRIS_STATE_RING, "default");
					/* XXX This should probably be justified better XXX */
				}  else if (!tris_canmatch_extension(NULL, "default", i->ext, 1, i->cid_num)) {
					/* It's not a valid extension, give a busy signal */
					tris_debug(1, "%s can't match anything in %s or default\n", i->ext, i->context);
					ioctl(i->fd, PHONE_BUSY);
					i->cpt = 1;
				}
			}
#if 0
			tris_verbose("Extension is %s\n", i->ext);
#endif
		}
	}
	if (phonee.bits.hookstate) {
...
static void *do_monitor(void *data)
{
	...
	while (monitor) {
		...
		if (tris_mutex_lock(&iflock)) {
			tris_log(LOG_ERROR, "Unable to grab interface lock\n");
			return NULL;
		}
		...
		while (i) {
			if (FD_ISSET(i->fd, &rfds)) 
				tris_log(LOG_WARNING, "Descriptor %d appears twice (%s)?\n", i->fd, i->dev);
			if (!i->owner) {
				...
				if (i->dialtone && i->mode != MODE_SIGMA) {
					...
					if (tris_tvzero(wait)) {
						...
						if (write(i->fd, DialTone + tonepos, 240) != 240)
							tris_log(LOG_WARNING, "Dial tone write error\n");
					}
					dotone++;
				}
			}
		...
		if (dotone && i && i->mode != MODE_SIGMA) {
			...
			if (tonepos >= sizeof(DialTone))
					tonepos = 0;
			if (tris_tvzero(wait)) {
				wait = tris_tv(30000, 0);
			}
			res = tris_select(n + 1, &rfds, NULL, &efds, &wait);
		} else {
			res = tris_select(n + 1, &rfds, NULL, &efds, NULL);
			wait = tris_tv(0,0);
			tonepos = 0;
		}
		/* Okay, select has finished.  Let's see what happened.  */
		if (res < 0) {
			tris_debug(1, "select return %d: %s\n", res, strerror(errno));
			continue;
		}
	...
	if (tmp) {
		...
		if (mode == MODE_FXO) {
			if (ioctl(tmp->fd, IXJCTL_PORT, PORT_PSTN)) {
				tris_debug(1, "Unable to set port to PSTN\n");
			}
		} else {
			if (ioctl(tmp->fd, IXJCTL_PORT, PORT_POTS)) 
				 if (mode != MODE_FXS)
				      tris_debug(1, "Unable to set port to POTS\n");
		}
		...
		tris_copy_string(tmp->dev, iface, sizeof(tmp->dev));
		tris_copy_string(tmp->context, context, sizeof(tmp->context));
		tmp->next = NULL;
		tmp->obuflen = 0;
		tmp->dialtone = 0;
		tmp->cpt = 0;
		tris_copy_string(tmp->cid_num, cid_num, sizeof(tmp->cid_num));
		tris_copy_string(tmp->cid_name, cid_name, sizeof(tmp->cid_name));
		tmp->txgain = txgain;
...
static struct tris_channel *phone_request(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	...
	if (tris_mutex_lock(&iflock)) {
		tris_log(LOG_ERROR, "Unable to lock interface list???\n");
		return NULL;
	}
	p = iflist;
	while(p) {
		if (p->mode == MODE_FXS ||
		    format & (TRIS_FORMAT_G729A | TRIS_FORMAT_G723_1 | TRIS_FORMAT_SLINEAR | TRIS_FORMAT_ULAW)) {
		    size_t length = strlen(p->dev + 5);
    		if (strncmp(name, p->dev + 5, length) == 0 &&
    		    !isalnum(name[length])) {
    		    if (!p->owner) {
                     tmp = phone_new(p, TRIS_STATE_DOWN, p->context);
                     break;
                } else
...
static int __unload_module(void)
{
	...
	if (cur_tech)
		tris_channel_unregister(cur_tech);
	if (!tris_mutex_lock(&iflock)) {
...
static int load_module(void)
{
	...
	while(v) {
		...
		if (!strcasecmp(v->name, "device")) {
			...
			silencesupression = tris_true(v->value);
			...
			if (!strcasecmp(v->value, "g729")) {
				prefformat = TRIS_FORMAT_G729A;
                        } else if (!strcasecmp(v->value, "g723.1")) {
				prefformat = TRIS_FORMAT_G723_1;
			} else if (!strcasecmp(v->value, "slinear")) {
				if (mode == MODE_FXS)
				    prefformat |= TRIS_FORMAT_SLINEAR;
				else prefformat = TRIS_FORMAT_SLINEAR;
			} else if (!strcasecmp(v->value, "ulaw")) {
				prefformat = TRIS_FORMAT_ULAW;
			} else
				tris_log(LOG_WARNING, "Unknown format '%s'\n", v->value);
			...
			if (!strcasecmp(v->value, "off")) {
				...
				tris_log(LOG_WARNING, "Unknown echo cancellation '%s'\n", v->value);
	...
	if (mode == MODE_FXS) {
		...
		cur_tech = (struct tris_channel_tech *) &phone_tech;

	/* Make sure we can register our Adtranphone channel type */

	if (tris_channel_register(cur_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel class 'Phone'\n");
		tris_config_destroy(cfg);
		__unload_module();
		return TRIS_MODULE_LOAD_FAILURE;
	}
...
TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Linux Telephony API Support");
