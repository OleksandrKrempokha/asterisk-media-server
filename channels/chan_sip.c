 ...
 * sipsock_read locks the tris_channel if it exists (an active call) and
 ...
 *	  we can require a secure signalling path OUT of Trismedia (on SIP or IAX2). Possibly, the call should
...
/*!  \page sip_session_timers SIP Session Timers in Trismedia Chan_sip
	...
	2. Originate :: In the "originate" mode, the Trismedia server requests the remote 
		...
		against hanging SIP channels due to network or end-point failures, Trismedia
...
/*** DOCUMENTATION
	...
	<function name="CHECKSIPDOMAIN" language="en_US">
		...
		<description>
			...
			as a local SIP domain that this Trismedia server is configured to handle.
...
/*! \brief Modes in which Trismedia can be configured to run SIP Session-Timers */
enum st_mode {
...
/*! \brief States whether a SIP message can create a dialog in Trismedia. */
...
/*! \brief SIP Request methods known by Trismedia 
...
enum sipmethod {
	...
	SIP_PRACK,		/*!< Reliable pre-call signalling. Not supported in Trismedia. */
	...
	SIP_PUBLISH,		/*!< Not supported in Trismedia */
 ...
 * You can run Trismedia with SIP on a different port with a configuration
...
#define DEFAULT_USERAGENT "Trismedia"	/*!< Default Useragent: header unless re-defined in sip.conf */
#define DEFAULT_SDPSESSION "Trismedia"	/*!< Default SDP session name, (s=) header unless re-defined in sip.conf */
...
static unsigned int global_tos_desktop;		/*!< IP type of service for video RTP packets */
...
static unsigned int global_cos_desktop;		/*!< 802.1p class of service for video RTP packets */
...
static char global_useragent[TRIS_MAX_EXTENSION];	/*!< Useragent for the SIP channel */
static char global_sdpsession[TRIS_MAX_EXTENSION];	/*!< SDP session name for the SIP channel */
static char global_sdpowner[TRIS_MAX_EXTENSION];	/*!< SDP owner name for the SIP channel */
...
/* spc settings */
static int enable_spc;
static char spc_host[256];
static int spc_port;
static char spc_file[256];
static char spc_codec[256];
...
static struct tris_flags global_flags[2] = {{0}};        /*!< global SIP_ flags */
...
TRIS_MUTEX_DEFINE_STATIC(spclock);
TRIS_MUTEX_DEFINE_STATIC(sip_reload_lock);

/*! \brief This is the thread for the monitor which checks for input on the channels
   which are not currently in use.  */
static pthread_t monitor_thread = TRIS_PTHREADT_NULL;
static pthread_t spc_thread = TRIS_PTHREADT_NULL;
...
struct sip_request {
	...
	TRIS_LIST_ENTRY(sip_request) next;
};
 ...
 * pointers. They are now offsets into the tris_str portion of the sip_request structure.
...
static TRIS_LIST_HEAD_STATIC(domain_list, domain);	/*!< The SIP domain list */


/*! \brief sip_history: Structure for saving transactions within a SIP dialog */
struct sip_history {
	TRIS_LIST_ENTRY(sip_history) list;
	char event[0];	/* actually more, depending on needs */
};

TRIS_LIST_HEAD_NOLOCK(sip_history_head, sip_history); /*!< history list, entry in sip_pvt */
...
#define SIP_DEFER_BYE_ON_TRANSFER	(1 << 10)	/*!< D: Do not hangup at first tris_hangup */
...
#define SIP_PAGE2_PLAYMOH			(1 << 7) /* play moh */
...
#define SIP_PAGE2_DESKTOPSUPPORT		(1 << 13)	/*!< DP: Video supported if offered? */
...
struct sip_pvt {
	...
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(qop);		/*!< Quality of Protection, since SIP wasn't complicated enough yet. */
	...
	struct tris_flags flags[2];		/*!< SIP_ flags */
	...
	char nodesktop;				/*!< Didn't get desktop in invite, don't offer */
	...
	struct sockaddr_in dredirip;		/*!< Where our Desktop Sharing RTP should be going if not to us */
	...
	struct tris_rtp *drtp;			/*!< Desktop sharing RTP session */
	...
	TRIS_LIST_HEAD_NOLOCK(request_queue, sip_request) request_queue; /*!< Requests that arrived but could not be processed immediately */
	 ...
	 * image, audio, text, and video and desktop_sharing. Therefore we need to keep track of which types of media were offered.
	 *
	 ...
	 */
	struct offered_media offered_media[5];
}; 
...
struct sip_mailbox {
	...
	TRIS_LIST_ENTRY(sip_mailbox) entry;
};
...
struct sip_peer {
	...
	TRIS_DECLARE_STRING_FIELDS(
		...
		TRIS_STRING_FIELD(fullcontact);		/*!< Contact registered with us (not in sip.conf) */
		...
		TRIS_STRING_FIELD(parkinglot);		/*!<  Parkinglot */
		TRIS_STRING_FIELD(useragent);		/*!<  User agent in SIP request (saved from registration) */
		);
	...
	TRIS_LIST_HEAD_NOLOCK(, sip_mailbox) mailboxes;
...
static int peer_ipcmp_cb(void *obj, void *arg, int flags)
{
	...
	} else if (tris_test_flag(&peer2->flags[0], SIP_INSECURE_PORT)) {
		/* We are allowing match without port for peers configured that
		 * way in this pass through the peers. */
		return tris_test_flag(&peer->flags[0], SIP_INSECURE_PORT) ?
...
static struct tris_channel *sip_request_call(const char *type, int format, void *data, int *cause, struct tris_channel* src);
static int sip_devicestate(void *data);
static int sip_sendtext(struct tris_channel *ast, const char *text);
static int sip_call(struct tris_channel *ast, char *dest, int timeout);
static int sip_sendhtml(struct tris_channel *chan, int subclass, const char *data, int datalen);
static int sip_hangup(struct tris_channel *ast);
static int sip_answer(struct tris_channel *ast);
static struct tris_frame *sip_read(struct tris_channel *ast);
static int sip_write(struct tris_channel *ast, struct tris_frame *frame);
static int sip_indicate(struct tris_channel *ast, int condition, const void *data, size_t datalen);
static int sip_transfer(struct tris_channel *ast, const char *dest);
static int sip_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
static int sip_senddigit_begin(struct tris_channel *ast, char digit);
static int sip_senddigit_end(struct tris_channel *ast, char digit, unsigned int duration);
static int sip_queryoption(struct tris_channel *chan, int option, void *data, int *datalen);
static const char *sip_get_callid(struct tris_channel *chan);
...
static int __sip_xmit(struct sip_pvt *p, struct tris_str *data, int len);
static int __sip_reliable_xmit(struct sip_pvt *p, int seqno, int resp, struct tris_str *data, int len, int fatal, int sipmethod);
static int __transmit_response(struct sip_pvt *p, const char *msg, const struct sip_request *req, enum xmittype reliable);
static int retrans_pkt(const void *data);
static int transmit_response_using_temp(tris_string_field callid, struct sockaddr_in *sin, int useglobal_nat, const int intended_method, const struct sip_request *req, const char *msg);
...
static int transmit_notify_custom(struct sip_pvt *p, struct tris_variable *vars);
...
static int sip_send_mwi_to_peer(struct sip_peer *peer, const struct tris_event *event, int cache_only);

/*--- Dialog management */
static struct sip_pvt *sip_alloc(tris_string_field callid, struct sockaddr_in *sin,
...
static int sip_park(struct tris_channel *chan1, struct tris_channel *chan2, struct sip_request *req, int seqno);
...
static int process_sdp_a_audio(const char *a, struct sip_pvt *p, struct tris_rtp *newaudiortp, int *last_rtpmap_codec);
static int process_sdp_a_video(const char *a, struct sip_pvt *p, struct tris_rtp *newvideortp, int *last_rtpmap_codec);
static int process_sdp_a_desktop(const char *a, struct sip_pvt *p, struct tris_rtp *newdesktoprtp, int *last_rtpmap_codec);
static int process_sdp_a_text(const char *a, struct sip_pvt *p, struct tris_rtp *newtextrtp, char *red_fmtp, int *red_num_gen, int *red_data_pt, int *last_rtpmap_codec);
...
static void *spc_monitor(void *data);
static int restart_spc(void);

static void peer_mailboxes_to_str(struct tris_str **mailbox_str, struct sip_peer *peer);
...
static char *sip_show_inuse(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
static char *sip_show_sched(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char * _sip_show_peers(int fd, int *total, struct mansession *s, const struct message *m, int argc, const char *argv[]);
static char *sip_show_peers(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_show_objects(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
static char *sip_show_domains(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *_sip_show_peer(int type, int fd, struct mansession *s, const struct message *m, int argc, const char *argv[]);
static char *sip_show_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *_sip_qualify_peer(int type, int fd, struct mansession *s, const struct message *m, int argc, const char *argv[]);
static char *sip_qualify_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_show_registry(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_unregister(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_show_settings(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_show_mwi(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
static char *sip_show_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_show_channelstats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_show_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_do_debug_ip(int fd, char *arg);
static char *sip_do_debug_peer(int fd, char *arg);
static char *sip_do_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_cli_notify(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *sip_set_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static int sip_dtmfmode(struct tris_channel *chan, void *data);
static int sip_addheader(struct tris_channel *chan, void *data);
static int sip_do_reload(enum channelreloadreason reason);
static char *sip_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
static struct sip_peer *build_peer(const char *name, struct tris_variable *v, struct tris_variable *alt, int realtime, int devstate_only);
static int update_call_counter(struct sip_pvt *fup, int event);
...
static char *sip_prune_realtime(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);

/*--- Internal UA client handling (outbound registrations) */
static void tris_sip_ouraddrfor(struct in_addr *them, struct sockaddr_in *us, struct sip_pvt *p);
...
static void *_sip_tcp_helper_thread(struct sip_pvt *pvt, struct tris_tcptls_session_instance *tcptls_session);
...
static int sip_set_rtp_peer(struct tris_channel *chan, struct tris_rtp *rtp, struct tris_rtp *vrtp,  struct tris_rtp *trtp, int codecs, int nat_active);
static enum tris_rtp_get_result sip_get_rtp_peer(struct tris_channel *chan, struct tris_rtp **rtp);
static enum tris_rtp_get_result sip_get_vrtp_peer(struct tris_channel *chan, struct tris_rtp **rtp);
static enum tris_rtp_get_result sip_get_trtp_peer(struct tris_channel *chan, struct tris_rtp **rtp);
static int sip_get_codec(struct tris_channel *chan);
static struct tris_frame *sip_rtp_read(struct tris_channel *ast, struct sip_pvt *p, int *faxdetect);

/*------ T38 Support --------- */
static int transmit_response_with_t38_sdp(struct sip_pvt *p, char *msg, struct sip_request *req, int retrans);
static struct tris_udptl *sip_get_udptl_peer(struct tris_channel *chan);
static int sip_set_udptl_peer(struct tris_channel *chan, struct tris_udptl *udptl);
...
/* addtional functions */
static struct tris_rtp * sip_get_rtp(char * rtp_name, struct tris_channel * src_chan);
static int sip_set_rtp(char * rtp_name, struct tris_channel * dst_chan, struct tris_channel * src_chan);
static int sip_get_rtpneed(char * rtp_name, struct tris_channel * src_chan);

/*! \brief Definition of this channel for PBX channel registration */
static const struct tris_channel_tech sip_tech = {
	.type = "SIP",
	...
	.get_pvt_rtp = sip_get_rtp,
	.set_pvt_rtp = sip_set_rtp,
	.get_pvt_rtpneed = sip_get_rtpneed,
};
...
static struct tris_tcptls_session_args sip_tls_desc = {
	.accept_fd = -1,
	.master = TRIS_PTHREADT_NULL,
	.tls_cfg = &sip_tls_cfg,
	.poll_timeout = -1,
...
static struct tris_rtp_protocol sip_rtp = {
	.type = "SIP",
...
static struct sip_threadinfo *sip_threadinfo_create(struct tris_tcptls_session_instance *tcptls_session, int transport)
{
	...
	if (pipe(th->alert_pipe) == -1) {
		...
		tris_log(LOG_ERROR, "Could not create sip alert pipe in tcptls thread, error %s\n", strerror(errno));
		return NULL;
	}
...
static int sip_tcptls_write(struct tris_tcptls_session_instance *tcptls_session, const void *buf, size_t len)
{
	int res = len;
...
static void *_sip_tcp_helper_thread(struct sip_pvt *pvt, struct tris_tcptls_session_instance *tcptls_session) 
{
	...
	if (!(req.data = tris_str_create(SIP_MIN_PACKET)))
		goto cleanup;
	if (!(reqcpy.data = tris_str_create(SIP_MIN_PACKET)))
		goto cleanup;
	...
	for (;;) {
		...
		if (res < 0) {
			tris_debug(2, "SIP %s server :: tris_wait_for_input returned %d\n", tcptls_session->ssl ? "SSL": "TCP", res);
			goto cleanup;
		}
...
static void *registry_unref(struct sip_registry *reg, char *tag)
{
	tris_debug(3, "SIP Registry %s: refcount now %d\n", reg->hostname, reg->refcount - 1);
...
static struct tris_udptl_protocol sip_udptl = {
	type: "SIP",
...
static void initialize_initreq(struct sip_pvt *p, struct sip_request *req)
{
	if (p->initreq.headers)
		tris_debug(1, "Initializing already initialized SIP dialog %s (presumably reinvite)\n", p->callid);
	else
...
{
	tris_debug(3, "Setting SIP_ALREADYGONE on dialog %s\n", dialog->callid);
...
static int proxy_update(struct sip_proxy *proxy)
{
	...
	if (!inet_aton(proxy->name, &proxy->ip.sin_addr)) {
		...
		if (tris_get_ip_or_srv(&proxy->ip, proxy->name, sip_cfg.srvlookup ? "_sip._udp" : NULL) < 0) {
...
static unsigned int parse_sip_options(struct sip_pvt *pvt, const char *supported)
{
	...
	if (sipdebug)
		tris_debug(3, "Begin: parsing SIP \"Supported: %s\"\n", supported);
	...
	for (next = temp; next; next = sep) {
		...
		if (sipdebug)
			tris_debug(3, "Found SIP option: -%s-\n", next);
		for (i = 0; i < ARRAY_LEN(sip_options); i++) {
			if (!strcasecmp(next, sip_options[i].text)) {
				profile |= sip_options[i].id;
				found = TRUE;
				if (sipdebug)
					tris_debug(3, "Matched SIP option: %s\n", next);
				break;
			}
		}
		...
		if (!found && sipdebug) {
			if (!strncasecmp(next, "x-", 2))
				tris_debug(3, "Found private SIP option, not supported: %s\n", next);
			else
				tris_debug(3, "Found no match for SIP option: %s (Please file bug report!)\n", next);
		}
	}
...
static const struct sockaddr_in *sip_real_dst(const struct sip_pvt *p)
{
	...
	return tris_test_flag(&p->flags[0], SIP_NAT) & SIP_NAT_ROUTE ? &p->recv : &p->sa;
}

/*! \brief Display SIP nat mode */
static const char *sip_nat_mode(const struct sip_pvt *p)
{
	return tris_test_flag(&p->flags[0], SIP_NAT) & SIP_NAT_ROUTE ? "NAT" : "no NAT";
}
...
static void build_via(struct sip_pvt *p)
{
	...
	const char *rport = tris_test_flag(&p->flags[0], SIP_NAT) & SIP_NAT_RFC3581 ? ";rport" : "";
...
/*	if (want_remap &&
	    (!sip_cfg.matchexterniplocally || !tris_apply_ha(localaddr, us)) ) {*/
	if (externip.sin_addr.s_addr) {
		/* if we used externhost or stun, see if it is time to refresh the info */
		if (externexpire && time(NULL) >= externexpire) {
			if (stunaddr.sin_addr.s_addr) {
				tris_stun_request(sipsock, &stunaddr, NULL, &externip);
			} else {
...
static int retrans_pkt(const void *data)
{
	...
	if (pkt->retrans < MAX_RETRANS) {
		pkt->retrans++;
 		if (!pkt->timer_t1) {	/* Re-schedule using timer_a and timer_t1 */
			if (sipdebug)
 				tris_debug(4, "SIP TIMER: Not rescheduling id #%d:%s (Method %d) (No timer T1)\n", pkt->retransid, sip_methods[pkt->method].text, pkt->method);
		} else {
 			int siptimer_a;

 			if (sipdebug)
 				tris_debug(4, "SIP TIMER: Rescheduling retransmission #%d (%d) %s - %d\n", pkt->retransid, pkt->retrans, sip_methods[pkt->method].text, pkt->method);
 			if (!pkt->timer_a)
 				pkt->timer_a = 2 ;
 			else
			...
			reschedule = siptimer_a;
 			tris_debug(4, "** SIP timers: Rescheduling retransmission %d to %d ms (t1 %d ms (Retrans id #%d)) \n", pkt->retrans +1, siptimer_a, pkt->timer_t1, pkt->retransid);
 		} 
	...
	if (pkt->owner && pkt->method != SIP_OPTIONS && xmitres == 0) {
		if (pkt->is_fatal || sipdebug)	/* Tell us if it's critical or if we're debugging */
			tris_log(LOG_WARNING, "Maximum retries exceeded on transmission %s for seqno %d (%s %s) -- See doc/sip-retransmit.txt.\n",
				pkt->owner->callid, pkt->seqno,
				pkt->is_fatal ? "Critical" : "Non-critical", pkt->is_resp ? "Response" : "Request");
	} else if (pkt->method == SIP_OPTIONS && sipdebug) {
			tris_log(LOG_WARNING, "Cancelling retransmit of OPTIONs (call id %s)  -- See doc/sip-retransmit.txt.\n", pkt->owner->callid);
	...
	if (pkt->is_fatal) {
		...
		if (pkt->owner->owner) {
			...
			tris_log(LOG_WARNING, "Hanging up call %s - no reply to our critical packet (see doc/sip-retransmit.txt).\n", pkt->owner->callid);
...
*/
static enum sip_result __sip_reliable_xmit(struct sip_pvt *p, int seqno, int resp, struct tris_str *data, int len, int fatal, int sipmethod)
{
	...
	TRIS_SCHED_REPLACE_VARIABLE(pkt->retransid, sched, siptimer_a, retrans_pkt, pkt, 1);
	if (sipdebug)
		tris_debug(4, "*** SIP TIMER: Initializing retransmit timer on packet: Id  #%d\n", pkt->retransid);

	xmitres = __sip_xmit(pkt->owner, pkt->data, pkt->packetlen);	/* Send packet */

	if (xmitres == XMIT_ERROR) {	/* Serious network trouble, no need to try again */
		append_history(pkt->owner, "XmitErr", "%s", pkt->is_fatal ? "(Critical)" : "(Non-critical)");
		tris_log(LOG_ERROR, "Serious Network Trouble; __sip_xmit returns error for pkt data\n");
...
static int __sip_autodestruct(const void *data)
{
	...
	if (p->subscribed) {
		...
		tris_debug(3, "Re-scheduled destruction of SIP subscription %s\n", p->callid ? p->callid : "<unknown>");
	...
	if (p->packets) {
		if (!p->needdestroy) {
			char method_str[31];
			tris_debug(3, "Re-scheduled destruction of SIP call %s\n", p->callid ? p->callid : "<unknown>");
	...
	if (p->owner) {
		...
		tris_debug(3, "Auto destroying SIP dialog '%s'\n", p->callid);
...
static void sip_scheddestroy(struct sip_pvt *p, int ms)
{
	...
	if (sip_debug_test_pvt(p))
		tris_verbose("Scheduling destruction of SIP dialog '%s' in %d ms (Method: %s)\n", p->callid, ms, sip_methods[p->method].text);
	if (sip_cancel_destroy(p))
		tris_log(LOG_WARNING, "Unable to cancel SIP destruction.  Expect bad things.\n");

	if (p->do_history)
		append_history(p, "SchedDestroy", "%d ms", ms);
	p->autokillid = tris_sched_add(sched, ms, __sip_autodestruct, dialog_ref(p, "setting ref as passing into tris_sched_add for __sip_autodestruct"));
	...
	*/
	...
	for (cur = p->packets; cur; prev = cur, cur = cur->next) {
		...
		if (cur->is_resp || cur->method == sipmethod) {
			...
			if (cur->retransid > -1) {
				if (sipdebug)
					tris_debug(4, "** SIP TIMER: Cancelling retransmit of packet (reply received) Retransid #%d\n", cur->retransid);
			}
...
static int __sip_semi_ack(struct sip_pvt *p, int seqno, int resp, int sipmethod)
{
	...
	for (cur = p->packets; cur; cur = cur->next) {
		if (cur->seqno == seqno && cur->is_resp == resp &&
			...
			if (cur->retransid > -1) {
				if (sipdebug)
					tris_debug(4, "*** SIP TIMER: Cancelling retransmission #%d - %s (got response)\n", cur->retransid, sip_methods[sipmethod].text);
			}
...
static int send_request(struct sip_pvt *p, struct sip_request *req, enum xmittype reliable, int seqno)
{
	...
	*/
	...
	add_blank(req);
	if (sip_debug_test_pvt(p)) {
		if (tris_test_flag(&p->flags[0], SIP_NAT_ROUTE))
...
static int sip_queryoption(struct tris_channel *chan, int option, void *data, int *datalen)
{
	...
	case TRIS_OPTION_T38_STATE:
		...
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT)) {
			switch (p->t38.state) {
...
static int sip_sendhtml(struct tris_channel *chan, int subclass, const char *data, int datalen)
{
	...
	case TRIS_STATE_UP:
		...
		} else if (!tris_test_flag(&p->flags[0], SIP_PENDINGBYE)) {
			tris_set_flag(&p->flags[0], SIP_NEEDREINVITE);	
		}	
		break;
	default:
...
static void realtime_update_peer(const char *peername, struct sockaddr_in *sin, const char *defaultuser, const char *fullcontact, const char *useragent, int expirey, int deprecated_username, int lastms)
{
	...
	int realtimeregs = tris_check_realtime("sipregs");
		...
		if ((context = strchr(ext, '@'))) {
			*context++ = '\0';	/* split ext@context */
			if (!tris_context_find(context)) {
				tris_log(LOG_WARNING, "Context %s must exist in regcontext= in sip.conf!\n", context);
				continue;
			}
		} else {
		...
		if (onoff) {
			if (!tris_exists_extension(NULL, context, ext, 1, NULL)) {
				tris_add_extension(context, 1, ext, 1, NULL, NULL, "Noop",
					 tris_strdup(peer->name), tris_free_ptr, "SIP");
			}
...
static void sip_destroy_peer(struct sip_peer *peer)
{
	tris_debug(3, "Destroying SIP peer %s\n", peer->name);
...
static void update_peer(struct sip_peer *p, int expire)
{
	int rtcachefriends = tris_test_flag(&p->flags[1], SIP_PAGE2_RTCACHEFRIENDS);
...
static struct tris_variable *get_insecure_variable_from_config(struct tris_config *cfg)
{
	...
	while ((cat = tris_category_browse(cfg, cat))) {
		...
		if (tris_test_flag(&flags, SIP_INSECURE_PORT)) {
...
static struct sip_peer *realtime_peer(const char *newpeername, struct sockaddr_in *sin, int devstate_only)
{
	...
	if (newpeername) {
		if (realtimeregs)
			varregs = tris_load_realtime("sipregs", "name", newpeername, SENTINEL);

		var = tris_load_realtime("sippeers", "name", newpeername, "host", "dynamic", SENTINEL);
		if (!var && sin)
			var = tris_load_realtime("sippeers", "name", newpeername, "host", tris_inet_ntoa(sin->sin_addr), SENTINEL);
		if (!var) {
			var = tris_load_realtime("sippeers", "name", newpeername, SENTINEL);
			/*!\note
	...
	if (!var && sin) {	/* Then check on IP address for dynamic peers */
		...
		var = tris_load_realtime("sippeers", "host", ipaddr, "port", portstring, SENTINEL);	/* First check for fixed IP hosts */
		if (var) {
			if (realtimeregs) {
			...
			if (realtimeregs)
				varregs = tris_load_realtime("sipregs", "ipaddr", ipaddr, "port", portstring, SENTINEL); /* Then check for registered hosts */
			else
				var = tris_load_realtime("sippeers", "ipaddr", ipaddr, "port", portstring, SENTINEL); /* Then check for registered hosts */
			if (varregs) {
		...
		if (!var) { /*We couldn't match on ipaddress and port, so we need to check if port is insecure*/
			peerlist = tris_load_realtime_multientry("sippeers", "host", ipaddr, SENTINEL);
			if (peerlist) {
				...
				if(var) {
					...
					peerlist = NULL;
					cat = NULL;
					peerlist = tris_load_realtime_multientry("sippeers", "ipaddr", ipaddr, SENTINEL);
					if(peerlist) {
						var = get_insecure_variable_from_config(peerlist);
						if(var) {
							if (realtimeregs) {
				...
				if (realtimeregs) {
					peerlist = tris_load_realtime_multientry("sipregs", "ipaddr", ipaddr, SENTINEL);
					if (peerlist) {
	...
	if (tris_test_flag(&global_flags[1], SIP_PAGE2_RTCACHEFRIENDS) && !devstate_only) {
		/* Cache peer */
		tris_copy_flags(&peer->flags[1], &global_flags[1], SIP_PAGE2_RTAUTOCLEAR|SIP_PAGE2_RTCACHEFRIENDS);
		if (tris_test_flag(&global_flags[1], SIP_PAGE2_RTAUTOCLEAR)) {
			TRIS_SCHED_REPLACE_UNREF(peer->expire, sched, sip_cfg.rtautoclear * 1000, expire_register, peer,
...
static struct sip_peer *find_peer(const char *peer, struct sockaddr_in *sin, int realtime, int which_objects, int devstate_only, int transport)
{
	...
	if (peer) {
		...
		if (!p) {
			tris_set_flag(&tmp_peer.flags[0], SIP_INSECURE_PORT);
...
{
	if (p->udptl) {
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT) == SIP_PAGE2_T38SUPPORT_UDPTL_REDUNDANCY) {
                        tris_udptl_set_error_correction_scheme(p->udptl, UDPTL_ERROR_CORRECTION_REDUNDANCY);
		} else if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT) == SIP_PAGE2_T38SUPPORT_UDPTL_FEC) {
			tris_udptl_set_error_correction_scheme(p->udptl, UDPTL_ERROR_CORRECTION_FEC);
		} else if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT) == SIP_PAGE2_T38SUPPORT_UDPTL) {
...
static int create_addr_from_peer(struct sip_pvt *dialog, struct sip_peer *peer)
{
	...
	tris_copy_flags(&dialog->flags[0], &peer->flags[0], SIP_FLAGS_TO_COPY);
	tris_copy_flags(&dialog->flags[1], &peer->flags[1], SIP_PAGE2_FLAGS_TO_COPY);
	dialog->capability = peer->capability;
	if (!tris_test_flag(&dialog->flags[1], SIP_PAGE2_VIDEOSUPPORT_ALWAYS) &&
			(!tris_test_flag(&dialog->flags[1], SIP_PAGE2_VIDEOSUPPORT) ||
				!(dialog->capability & TRIS_FORMAT_VIDEO_MASK)) &&
			dialog->vrtp) {
		tris_rtp_destroy(dialog->vrtp);
		dialog->vrtp = NULL;
	}
	if (!tris_test_flag(&dialog->flags[1], SIP_PAGE2_TEXTSUPPORT) && dialog->trtp) {
		tris_rtp_destroy(dialog->trtp);
		dialog->trtp = NULL;
	}
	dialog->prefs = peer->prefs;
	if (tris_test_flag(&dialog->flags[1], SIP_PAGE2_T38SUPPORT)) {
		if (!dialog->udptl) {
	...
	do_setnat(dialog, tris_test_flag(&dialog->flags[0], SIP_NAT) & SIP_NAT_ROUTE);

	if (dialog->rtp) { /* Audio */
		tris_rtp_setdtmf(dialog->rtp, tris_test_flag(&dialog->flags[0], SIP_DTMF) == SIP_DTMF_RFC2833);
		tris_rtp_setdtmfcompensate(dialog->rtp, tris_test_flag(&dialog->flags[1], SIP_PAGE2_RFC2833_COMPENSATE));
		...
		if (tris_test_flag(&dialog->flags[1], SIP_PAGE2_CONSTANT_SSRC)) {
	...
	if ((tris_test_flag(&dialog->flags[0], SIP_DTMF) == SIP_DTMF_RFC2833) ||
	    (tris_test_flag(&dialog->flags[0], SIP_DTMF) == SIP_DTMF_AUTO))
		dialog->noncodeccapability |= TRIS_RTP_DTMF;
	else
		dialog->noncodeccapability &= ~TRIS_RTP_DTMF;
	if (peer->call_limit)
		tris_set_flag(&dialog->flags[0], SIP_CALL_LIMIT);
	if (!dialog->portinuri)
...
static int create_addr(struct sip_pvt *dialog, const char *opeer, struct sockaddr_in *sin, int newdialog)
{
	...
	if (!recvhost) {
		...
		tris_set_flag(&dialog->flags[0], SIP_NAT);
	}
	return 0;
}
 ...
 * Only called by the scheduler, must return the reference when done.
 */
static int sip_expire_timer(const void *arg)
{
	struct sip_pvt *p = (struct sip_pvt *)arg;
	const char* val;
	int tmp = 0;
	...
	if (p->owner) {
		val = pbx_builtin_getvar_helper(p->owner, "is3broadcast");
		if (!tris_strlen_zero(val) && (!strncmp(val, "recorder", 8) || !strncmp(val, "listener", 8))) {
			sip_pvt_unlock(p);
			dialog_unref(p, "unreffing arg passed into auto_congest callback (p->initid) broadcast");
			return 0;
		} else if (!tris_strlen_zero(val) && (!strncmp(val, "meetme", 6))) {
			p->expireid = tmp;
			pbx_builtin_setvar_helper(p->owner, "is3broadcast", "");
			sip_pvt_unlock(p);
			return DEFAULT_CALL_EXPIRE_TIMEOUT;
		}
		/* XXX fails on possible deadlock */
		if (!tris_channel_trylock(p->owner)) {
			append_history(p, "Cong", "Auto-congesting (timer)");
...
static int sip_call(struct tris_channel *ast, char *dest, int timeout)
{
	...
	if ((ast->_state != TRIS_STATE_DOWN) && (ast->_state != TRIS_STATE_RESERVED)) {
		tris_log(LOG_WARNING, "sip_call called on %s, neither down nor reserved\n", ast->name);
		return -1;
	}
	...
	TRIS_LIST_TRAVERSE(headp, current, entries) {
		...
		} else if (!p->options->uri_options && !strcasecmp(tris_var_name(current), "SIP_URI_OPTIONS")) {
			p->options->uri_options = tris_var_value(current);
		} else if (!p->options->addsipheaders && !strncasecmp(tris_var_name(current), "SIPADDHEADER", strlen("SIPADDHEADER"))) {
			/* Check whether there is a variable with a name starting with SIPADDHEADER */
			p->options->addsipheaders = 1;
		} else if (!strcasecmp(tris_var_name(current), "SIPFROMDOMAIN")) {
			tris_string_field_set(p, fromdomain, tris_var_value(current));
		} else if (!strcasecmp(tris_var_name(current), "SIPTRANSFER")) {
			/* This is a transfered call */
			p->options->transfer = 1;
		} else if (!strcasecmp(tris_var_name(current), "SIPTRANSFER_REFERER")) {
			/* This is the referrer */
			referer = tris_var_value(current);
		} else if (!strcasecmp(tris_var_name(current), "SIPTRANSFER_REPLACES")) {
	...
	res = 0;
	tris_set_flag(&p->flags[0], SIP_OUTGOING);
...
static void sip_registry_destroy(struct sip_registry *reg)
{
	...
	if (reg->call) {
		...
		tris_debug(3, "Destroying active SIP dialog for registry %s@%s\n", reg->username, reg->hostname);
...
static void __sip_destroy(struct sip_pvt *p, int lockowner, int lockdialoglist)
{
	...
	if (sip_debug_test_pvt(p))
		tris_verbose("Really destroying SIP dialog '%s' Method: %s\n", p->callid, sip_methods[p->method].text);

	if (tris_test_flag(&p->flags[0], SIP_INC_COUNT) || tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD)) {
...
static int update_call_counter(struct sip_pvt *fup, int event)
{
	...
	if (!tris_test_flag(&fup->flags[0], SIP_CALL_LIMIT) && !tris_test_flag(&fup->flags[1], SIP_PAGE2_CALL_ONHOLD))
		return 0;
	...
	case DEC_CALL_LIMIT:
		...
		if (inuse) {
			...
			ao2_lock(p);
			if (*inuse > 0) {
				if (tris_test_flag(&fup->flags[0], SIP_INC_COUNT)) {
					(*inuse)--;
					tris_clear_flag(&fup->flags[0], SIP_INC_COUNT);
				}
			} else {
				*inuse = 0;
			}
		...
		if (inringing) {
			...
			ao2_lock(p);
			if (*inringing > 0) {
				if (tris_test_flag(&fup->flags[0], SIP_INC_RINGING)) {
					(*inringing)--;
					tris_clear_flag(&fup->flags[0], SIP_INC_RINGING);
				}
			} else {
			   *inringing = 0;
			}
		...
		sip_pvt_lock(fup);
		ao2_lock(p);
		if (tris_test_flag(&fup->flags[1], SIP_PAGE2_CALL_ONHOLD) && sip_cfg.notifyhold) {
			tris_clear_flag(&fup->flags[1], SIP_PAGE2_CALL_ONHOLD);
			ao2_unlock(p);
			sip_pvt_unlock(fup);
	...
	case INC_CALL_LIMIT:
		...
		if (inringing && (event == INC_CALL_RINGING)) {
			...
			if (!tris_test_flag(&fup->flags[0], SIP_INC_RINGING)) {
				(*inringing)++;
				tris_set_flag(&fup->flags[0], SIP_INC_RINGING);
			}
			ao2_unlock(p);
			sip_pvt_unlock(fup);
		}
		if (inuse) {
			...
			if (!tris_test_flag(&fup->flags[0], SIP_INC_COUNT)) {
				(*inuse)++;
				tris_set_flag(&fup->flags[0], SIP_INC_COUNT);
			}
			ao2_unlock(p);
			sip_pvt_unlock(fup);
		}
...
static struct sip_pvt * sip_destroy(struct sip_pvt *p)
{
	tris_debug(3, "Destroying SIP dialog %s\n", p->callid);
	__sip_destroy(p, TRUE, TRUE);
	return NULL;
}

/*! \brief Convert SIP hangup causes to Trismedia hangup causes */
static int hangup_sip2cause(int cause)
{
...
/*! \brief Convert Trismedia hangup causes to SIP codes 
\verbatim
...
static const char *hangup_cause2sip(int cause)
{
	switch (cause) {
		...
		default:
			tris_debug(1, "AST hangup cause %d (no match found in SIP)\n", cause);
			return NULL;
	}
...
static int sip_hangup(struct tris_channel *ast)
{
	...
	if (tris_test_flag(&p->flags[0], SIP_DEFER_BYE_ON_TRANSFER)) {
		if (tris_test_flag(&p->flags[0], SIP_INC_COUNT) || tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD)) {
			if (sipdebug)
				tris_debug(1, "update_call_counter(%s) - decrement call limit counter on hangup\n", p->username);
			update_call_counter(p, DEC_CALL_LIMIT);
		}
		tris_debug(4, "SIP Transfer: Not hanging up right now... Rescheduling hangup for %s.\n", p->callid);
		if (p->autokillid > -1 && sip_cancel_destroy(p))
			tris_log(LOG_WARNING, "Unable to cancel SIP destruction.  Expect bad things.\n");
		sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
		tris_clear_flag(&p->flags[0], SIP_DEFER_BYE_ON_TRANSFER);	/* Really hang up next time */
		p->needdestroy = 0;
	...
	if (tris_test_flag(ast, TRIS_FLAG_ZOMBIE)) {
		if (p->refer)
			tris_debug(1, "SIP Transfer: Hanging up Zombie channel %s after transfer ... Call-ID: %s\n", ast->name, p->callid);
		else
			tris_debug(1, "Hanging up zombie call. Be scared.\n");
	} else
		tris_debug(1, "Hangup call %s, SIP callid %s\n", ast->name, p->callid);
	...
	*/
	...
	if (!p->alreadygone && p->initreq.data && !tris_strlen_zero(p->initreq.data->str)) {
		if (needcancel) {	/* Outgoing call, not up */
			if (tris_test_flag(&p->flags[0], SIP_OUTGOING)) {
				/* stop retransmitting an INVITE that has not received a response */
				/* if we can't send right now, mark it pending */
				if (p->invitestate == INV_CALLING) {
					/* We can't send anything in CALLING state */
					tris_set_flag(&p->flags[0], SIP_PENDINGBYE);
			...
			if (!p->pendinginvite) {
				...
				if (p->drtp)
					desktopqos = tris_rtp_get_quality(p->drtp, NULL, RTPQOS_SUMMARY);
				if (p->trtp)
				...
				if (p->do_history) {
					...
					if (p->drtp)
						append_history(p, "RTCPdesktop", "Quality:%s", desktopqos);
					if (p->trtp)
				...
				if (p->drtp && oldowner)
					pbx_builtin_setvar_helper(oldowner, "RTPDESKTOPQOS", desktopqos);
				if (p->trtp && oldowner)
				...
				tris_set_flag(&p->flags[0], SIP_PENDINGBYE);	
				tris_clear_flag(&p->flags[0], SIP_NEEDREINVITE);	
...
static void try_suggested_sip_codec(struct sip_pvt *p)
{
	...
	if (fmt) {
		tris_log(LOG_NOTICE, "Changing codec to '%s' for this call because of ${SIP_CODEC} variable\n", codec);
		if (p->jointcapability & fmt) {
			p->jointcapability &= fmt;
			p->capability &= fmt;
		} else
			tris_log(LOG_NOTICE, "Ignoring ${SIP_CODEC} variable because it is not shared by both ends.\n");
	} else
		tris_log(LOG_NOTICE, "Ignoring ${SIP_CODEC} variable because of unrecognized/not configured codec (check allow/disallow in sip.conf): %s\n", codec);
	return;	
}
...
static int sip_write(struct tris_channel *ast, struct tris_frame *frame)
{
	...
	case TRIS_FRAME_VOICE:
		...
		if (p) {
			...
			if (p->rtp) {
				...
				if ((ast->_state != TRIS_STATE_UP) &&
				    !tris_test_flag(&p->flags[0], SIP_PROGRESS_SENT) &&
				    !tris_test_flag(&p->flags[0], SIP_OUTGOING)) {
					tris_rtp_new_source(p->rtp);
					if (!global_prematuremediafilter) {
						p->invitestate = INV_EARLY_MEDIA;
						transmit_provisional_response(p, "183 Session Progress", &p->initreq, TRUE);
						tris_set_flag(&p->flags[0], SIP_PROGRESS_SENT);
					}
	...
	case TRIS_FRAME_DESKTOP:
		if (p) {
			...
			if (p->drtp) {
				...
				p->lastrtptx = time(NULL);
				res = tris_rtp_write(p->drtp, frame);
			}
			sip_pvt_unlock(p);
		}
		break;
	...
	case TRIS_FRAME_MODEM:
		...
		break;
	default: 
		tris_log(LOG_WARNING, "Can't send %d type frames with SIP write\n", frame->frametype);
		return 0;
	}
...
static int sip_fixup(struct tris_channel *oldchan, struct tris_channel *newchan)
{
	...
	if (!newchan || !newchan->tech_pvt) {
		if (!newchan)
			...
			tris_log(LOG_WARNING, "No SIP tech_pvt! Fixup of %s failed.\n", oldchan->name);
		return -1;
	}
	p = newchan->tech_pvt;
	...
	tris_debug(3, "SIP Fixup: New owner for dialogue %s: %s (Old parent: %s)\n", p->callid, p->owner->name, oldchan->name);
...
static int sip_senddigit_begin(struct tris_channel *ast, char digit)
{
	...
	sip_pvt_lock(p);
	switch (tris_test_flag(&p->flags[0], SIP_DTMF)) {
...
static void interpret_t38_parameters(struct sip_pvt *p, const struct tris_control_t38_parameters *parameters)
{
	if (!tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT)) {
		return;
	}
 ...
 * With SIP a lot of indications is sent as messages, letting the device play
   ...
   \return -1 to force tris_indicate to send indication in audio, 0 if SIP can handle the indication by sending a message
*/
static int sip_indicate(struct tris_channel *ast, int condition, const void *data, size_t datalen)
{
	struct sip_pvt *p = ast->tech_pvt;
	...
	char message[256], *buf;
	int seqno = 0;
	int confrefer = 0;

	seqno = ast->seqno;
	confrefer = ast->seqtype;
	sip_pvt_lock(p);
	switch(condition) {
	case TRIS_CONTROL_RINGING:
		if (ast->_state == TRIS_STATE_RING) {
			p->invitestate = INV_EARLY_MEDIA;
			if (!tris_test_flag(&p->flags[0], SIP_PROGRESS_SENT) ||
			    (tris_test_flag(&p->flags[0], SIP_PROG_INBAND) == SIP_PROG_INBAND_NEVER)) {				
				/* Send 180 ringing if out-of-band seems reasonable */
				transmit_provisional_response(p, "180 Ringing", &p->initreq, 0);
				tris_set_flag(&p->flags[0], SIP_RINGING);
				if (tris_test_flag(&p->flags[0], SIP_PROG_INBAND) != SIP_PROG_INBAND_YES)
					break;
			} else {
	...
	case TRIS_CONTROL_HOLD:
		...
		if(tris_test_flag(&p->flags[1], SIP_PAGE2_PLAYMOH))
	...
	case TRIS_CONTROL_SRCUPDATE:
		tris_rtp_new_source(p->rtp);
		//if the channel belongs to App conference, we send re-invite request to peer.
		if(p->owner->appl && !strcasecmp(p->owner->appl, "Conference")){
			tris_log(LOG_DEBUG, "yz: channel SRCUPDATEd. sending re-INVITE to %s\n", p->owner->cid.cid_num);
			transmit_reinvite_with_sdp(p, 0, 0);
		}
		break;
	case TRIS_CONTROL_SRCCHANGE:
		tris_rtp_change_source(p->rtp);
		break;
	case TRIS_CONTROL_NOTIFY_NEWMEMBER:
		//if the channel belongs to App conference, we send notify request to peer.
		if(p->owner->appl && !strcasecmp(p->owner->appl, "Conference")){
			char * newmember = data ? (char*)data : "";
			if(data)
				tris_log(LOG_DEBUG, "yz: new member entered. sending NOTIFY %s to %s\n", newmember, p->owner->cid.cid_num);
			else
				tris_log(LOG_DEBUG, "yz: new member entered. sending NOTIFY to %s\n", p->owner->cid.cid_num);
			transmit_notify_with_sipfrag(p, -1, newmember, TRUE);
		}
		break;
	case TRIS_CONTROL_NOTIFY_PROCEEDING:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "100 Trying", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_RINGING:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "180 Ringing", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_PROGRESS:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "183 Session Progress", TRUE);
		break;
	...
	case TRIS_CONTROL_NOTIFY_TAKEOFFHOOK:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "480 Unregistered", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_BUSY:
		...
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, message, TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_TIMEOUT:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "408 Request Timeout", TRUE);
		break;
	...
	case TRIS_CONTROL_NOTIFY_NOMONEY:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "402 No Money", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_OUTLINE:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "410 Gone Away", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_CIRCUITS:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "502 Bad gateway", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_ACCEPT:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "Accepted", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_CANCEL:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "Canceled", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_BYE:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "Byed", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_CONNECT:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "Connected", TRUE);
		break;
	case TRIS_CONTROL_NOTIFY_ANNOUNCE:
		if (confrefer)
			transmit_notify_with_sipfrag(p, seqno, "Announced", TRUE);
		break;
...
*/
static struct tris_channel *sip_new(struct sip_pvt *i, int state, const char *title)
{
	...
	if (!tmp) {
		tris_log(LOG_WARNING, "Unable to allocate AST channel structure for SIP channel\n");
		sip_pvt_lock(i);
		return NULL;
	}
	...
	tmp->tech = ( tris_test_flag(&i->flags[0], SIP_DTMF) == SIP_DTMF_INFO || tris_test_flag(&i->flags[0], SIP_DTMF) == SIP_DTMF_SHORTINFO) ?  &sip_tech_info : &sip_tech;
	...
	tris_debug(3, "*** Our native formats are %s \n", tris_getformatname_multiple(buf, SIPBUFSIZE, tmp->nativeformats));
	tris_debug(3, "*** Joint capabilities are %s \n", tris_getformatname_multiple(buf, SIPBUFSIZE, i->jointcapability));
	tris_debug(3, "*** Our capabilities are %s \n", tris_getformatname_multiple(buf, SIPBUFSIZE, i->capability));
	tris_debug(3, "*** TRIS_CODEC_CHOOSE formats are %s \n", tris_getformatname_multiple(buf, SIPBUFSIZE, tris_codec_choose(&i->prefs, what, 1)));
	if (i->prefcodec)
		tris_debug(3, "*** Our preferred formats from the incoming channel are %s \n", tris_getformatname_multiple(buf, SIPBUFSIZE, i->prefcodec));
	 ...
	 */
	if (i->vrtp) {
		if (tris_test_flag(&i->flags[1], SIP_PAGE2_VIDEOSUPPORT))
	...
	if (i->drtp) {
		needdesktop = TRIS_FORMAT_DESKTOP;
	}
	...
	if ((tris_test_flag(&i->flags[0], SIP_DTMF) == SIP_DTMF_INBAND) || (tris_test_flag(&i->flags[0], SIP_DTMF) == SIP_DTMF_AUTO) ||
	    (tris_test_flag(&i->flags[1], SIP_PAGE2_FAX_DETECT))) {
		int features = 0;
		...
		if (tris_test_flag(&i->flags[1], SIP_PAGE2_FAX_DETECT)) {
	...
	if (needdesktop && i->drtp) {
		tris_channel_set_fd(tmp, 6, tris_rtp_fd(i->drtp));
		tris_channel_set_fd(tmp, 7, tris_rtcp_fd(i->drtp));
	}
	if (needtext && i->trtp) 
...
static struct tris_frame *sip_rtp_read(struct tris_channel *ast, struct sip_pvt *p, int *faxdetect)
{
	...
	case 5:
		...
		break;
	case 6:
		f = tris_desktop_read(p->drtp); /* RTP Desktop */
		break;
	case 7:
		f = tris_rtcp_read(p->drtp);	/* RTCP Control Channel for desktop */
		break;
	default:
		f = &tris_null_frame;
	}
	...
	if (f && (f->frametype == TRIS_FRAME_DTMF_BEGIN || f->frametype == TRIS_FRAME_DTMF_END) &&
	    (tris_test_flag(&p->flags[0], SIP_DTMF) != SIP_DTMF_RFC2833)) {
	...
	if (f && ((tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_INBAND) || tris_test_flag(&p->flags[1], SIP_PAGE2_FAX_DETECT)) && p->dsp) {
		f = tris_dsp_process(p->owner, p->dsp, f);
			...
			if (f->subclass == 'f') {
				...
				if (tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_INBAND) {
...
static struct tris_frame *sip_read(struct tris_channel *ast)
{
	...
	if (faxdetected && tris_test_flag(&p->flags[1], SIP_PAGE2_FAX_DETECT)) {
		tris_channel_lock(ast);
...
static struct sip_st_dlg* sip_st_alloc(struct sip_pvt *const p)
{
	...
	if (!(stp = tris_calloc(1, sizeof(struct sip_st_dlg))))
		return NULL;
				 ...
				 int useglobal_nat, const int intended_method, struct sip_request *req)
{
	...
	if (req) {
		set_socket_transport(&p->socket, req->socket.type); /* Later in tris_sip_ouraddrfor we need this to choose the right ip and port for the specific transport */
	} else {
	...
	if (!sin)
		...
		p->sa = *sin;
		tris_sip_ouraddrfor(&p->sa.sin_addr, &p->ourip, p);
	}

	/* Copy global flags to this PVT at setup. */
	tris_copy_flags(&p->flags[0], &global_flags[0], SIP_FLAGS_TO_COPY);
	tris_copy_flags(&p->flags[1], &global_flags[1], SIP_PAGE2_FLAGS_TO_COPY);
	...
	if (sip_methods[intended_method].need_rtp) {
		...
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_VIDEOSUPPORT)) {
			p->vrtp = tris_rtp_new_with_bindaddr(sched, io, 1, 0, internip.sin_addr);
		}
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_DESKTOPSUPPORT)) {
			p->drtp = tris_rtp_new_with_bindaddr(sched, io, 1, 0, internip.sin_addr);
		}
 		if (tris_test_flag(&p->flags[1], SIP_PAGE2_TEXTSUPPORT))
		...
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT))
 		...
 		if (!p->rtp|| (tris_test_flag(&p->flags[1], SIP_PAGE2_VIDEOSUPPORT) && !p->vrtp) ||
				(tris_test_flag(&p->flags[1], SIP_PAGE2_DESKTOPSUPPORT) && !p->drtp)
				|| (tris_test_flag(&p->flags[1], SIP_PAGE2_TEXTSUPPORT) && !p->trtp)) {
 			tris_log(LOG_WARNING, "Unable to create RTP audio %s%ssession: %s\n",
 				tris_test_flag(&p->flags[1], SIP_PAGE2_VIDEOSUPPORT) ? "and video " : "",
 				tris_test_flag(&p->flags[1], SIP_PAGE2_TEXTSUPPORT) ? "and text " : "", strerror(errno));
			if (p->chanvars) {
				tris_variables_destroy(p->chanvars);
				p->chanvars = NULL;
			}
		...
		tris_rtp_setqos(p->rtp, global_tos_audio, global_cos_audio, "SIP RTP");
		tris_rtp_setdtmf(p->rtp, tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_RFC2833);
		tris_rtp_setdtmfcompensate(p->rtp, tris_test_flag(&p->flags[1], SIP_PAGE2_RFC2833_COMPENSATE));
		...
		if (p->vrtp) {
			tris_rtp_setqos(p->vrtp, global_tos_video, global_cos_video, "SIP VRTP");
		...
		if (p->drtp) {
			tris_rtp_setqos(p->drtp, global_tos_desktop, global_cos_desktop, "SIP DRTP");
			tris_rtp_setdtmf(p->drtp, 0);
			tris_rtp_setdtmfcompensate(p->drtp, 0);
			tris_rtp_set_rtptimeout(p->drtp, global_rtptimeout);
			tris_rtp_set_rtpholdtimeout(p->drtp, global_rtpholdtimeout);
			tris_rtp_set_rtpkeepalive(p->drtp, global_rtpkeepalive);
		}
		if (p->trtp) {
			tris_rtp_setqos(p->trtp, global_tos_text, global_cos_text, "SIP TRTP");
	...
	if (useglobal_nat && sin) {
		...
		tris_copy_flags(&p->flags[0], &global_flags[0], SIP_NAT);
		p->recv = *sin;
		do_setnat(p, tris_test_flag(&p->flags[0], SIP_NAT) & SIP_NAT_ROUTE);
	}

	do_setnat(p, SIP_NAT_ROUTE);
	...
	if ((tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_RFC2833) ||
	    (tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_AUTO))
		p->noncodeccapability |= TRIS_RTP_DTMF;
	if (p->udptl) {
		p->t38_maxdatagram = global_t38_maxdatagram;
		set_t38_capabilities(p);
	}
	tris_string_field_set(p, context, sip_cfg.default_context);
	...
	p->expireid = tris_sched_add(sched, DEFAULT_CALL_EXPIRE_TIMEOUT, sip_expire_timer, dialog_ref(p, "dialog ptr inc when SCHED_REPLACE add succeeded"));

	tris_debug(1, "Allocating new SIP dialog for %s - %s (%s)\n", callid ? callid : p->callid, sip_methods[intended_method].text, p->rtp ? "With RTP" : "No RTP");
	return p;
}
	...
	if (!tris_strlen_zero(p->callid)) { /* XXX double check, do we allow match on empty p->callid ? */
		if (arg->method == SIP_REGISTER)
			...
			if (sip_cfg.pedanticsipchecking && found) {
				found = tris_strlen_zero(arg->tag) || tris_strlen_zero(p->theirtag) || !tris_test_flag(&p->flags[1], SIP_PAGE2_DIALOG_ESTABLISHED) || !strcmp(p->theirtag, arg->tag);
			} 
		}
...
//	if (sip_cfg.pedanticsipchecking) {
		/* In principle Call-ID's uniquely identify a call, but with a forking SIP proxy
		   ...
		   For Trismedia to behave correctly, you need to turn on pedanticsipchecking
		   in sip.conf
		   */
		...
		if (tris_strlen_zero(totag) && (req->method == SIP_ACK || req->method == SIP_BYE || req->method == SIP_INFO )) {
...
//	}
	...
	if (sip_methods[intended_method].can_create == CAN_CREATE_DIALOG) {
		if (intended_method == SIP_REFER) {
			...
			if ((p = sip_alloc(callid, sin, 1, intended_method, req)))  {
				...
				*/
				...
				tris_debug(4, "Failed allocating SIP dialog, sending 500 Server internal error and giving up\n");
			}
		}
		...
		tris_debug(2, "Got a request with unsupported SIP method.\n");
...
static int sip_register(const char *value, int lineno)
{
	...
	if (host3.port) {
		if (!(portnum = port_str2int(host3.port, 0))) {
			tris_log(LOG_NOTICE, "'%s' is not a valid port number on line %d of sip.conf. using default.\n", host3.port, lineno);
		}
	}
	...
	if (!pre2.transport) {
		...
		transport = SIP_TRANSPORT_UDP;
		tris_log(LOG_NOTICE, "'%.3s' is not a valid transport type on line %d of sip.conf. defaulting to udp.\n", pre2.transport, lineno);
	}
	...
	if (!(reg = tris_calloc(1, sizeof(*reg)))) {
		tris_log(LOG_ERROR, "Out of memory. Can't allocate SIP registry entry\n");
		return -1;
	}

	if (tris_string_field_init(reg, 256)) {
		tris_log(LOG_ERROR, "Out of memory. Can't allocate SIP registry strings\n");
		tris_free(reg);
		return -1;
	}
...
static int process_sdp(struct sip_pvt *p, struct sip_request *req, int t38action)
{
	...
	int dpeercapability = 0, dpeernoncodeccapability = 0;
	int tpeercapability = 0, tpeernoncodeccapability = 0;

	struct tris_rtp *newaudiortp, *newvideortp, *newdesktoprtp, *newtextrtp;
...
#ifdef LOW_MEMORY
		newdesktoprtp = tris_threadstorage_get(&ts_desktop_rtp, tris_rtp_alloc_size());
#else
		newdesktoprtp = alloca(tris_rtp_alloc_size());
#endif
		memset(newdesktoprtp, 0, tris_rtp_alloc_size());
		tris_rtp_new_init(newdesktoprtp);
		tris_rtp_pt_clear(newdesktoprtp);
	...
	/* default: novideo, nodesktop and notext set */
	p->novideo = TRUE;
	p->nodesktop = TRUE;
	p->notext = TRUE;
	...
	if (p->drtp)
		tris_rtp_pt_clear(newdesktoprtp);  /* Must be cleared in case no m=desktop line exists */
	...
	while ((type = get_sdp_line(&iterator, next - 1, req, &value)) != '\0') {
		...
		case 'a':
			if (process_sdp_a_sendonly(value, &sendonly)) {
				...
				processed = TRUE;
			else if (process_sdp_a_desktop(value, p, newdesktoprtp, &last_rtpmap_codec))
				processed = TRUE;
	...
	while (!tris_strlen_zero(nextm)) {
		...
		/* Search for desktop media definition */
		} else if ((sscanf(m, "desktop %30u/%30u RTP/AVP %n", &x, &numberofports, &len) == 2 && len > 0) ||
			   (sscanf(m, "desktop %30u RTP/AVP %n", &x, &len) == 1 && len >= 0)) {
			desktop = TRUE;
			p->nodesktop = FALSE;
			p->offered_media[SDP_DESKTOP].offered = TRUE;
			numberofmediastreams++;
			dportno = x;
		
			/* Scan through the RTP payload types specified in a "m=" line: */
			codecs = m + len;
			tris_copy_string(p->offered_media[SDP_DESKTOP].text, codecs, sizeof(p->offered_media[SDP_DESKTOP].text));
			/*for (; !tris_strlen_zero(codecs); codecs = tris_skip_blanks(codecs + len)) {
				if (sscanf(codecs, "%30u%n", &codec, &len) != 1) {
					tris_log(LOG_WARNING, "Error in codec string '%s'\n", codecs);
					return -1;
				...
				if (debug)
					tris_verbose("Found RTP desktop format %d\n", codec);
				tris_rtp_set_m_type(newdesktoprtp, codec);
			}*/
		...
		while ((type = get_sdp_line(&iterator, next - 1, req, &value)) != '\0') {
			...
			case 'c':
				if (audio) {
					...
					if (process_sdp_c(value, &desktophp)) {
						processed = TRUE;
						dhp = &desktophp.hp;
					}
				} else if (text) {
			...
			case 'a':
				...
				/* Desktop specific scanning */
				else if (desktop) {
					if (process_sdp_a_sendonly(value, &dsendonly))
						processed = TRUE;
					else if (process_sdp_a_desktop(value, p, newdesktoprtp, &last_rtpmap_codec))
						processed = TRUE;
				}
	...
	/* Sanity checks */
	if (!hp && !vhp && !dhp && !thp && !ihp) {
		tris_log(LOG_WARNING, "Insufficient information in SDP (c=)...\n");
		return -1;
	}

	if (portno == -1 && vportno == -1 && dportno == -1 && udptlportno == -1  && tportno == -1)
	...
	tris_rtp_get_current_formats(newdesktoprtp, &dpeercapability, &dpeernoncodeccapability);
	tris_rtp_get_current_formats(newtextrtp, &tpeercapability, &tpeernoncodeccapability);

	newjointcapability = p->capability & (peercapability | vpeercapability | dpeercapability | tpeercapability);
	newpeercapability = (peercapability | vpeercapability | dpeercapability | tpeercapability);
	...
	if (debug) {
		...
		tris_verbose("Capabilities: us - %s, peer - audio=%s/video=%s/desktop=%s/text=%s, combined - %s\n",
			    tris_getformatname_multiple(s1, SIPBUFSIZE, p->capability),
			    tris_getformatname_multiple(s2, SIPBUFSIZE, peercapability),
			    tris_getformatname_multiple(s3, SIPBUFSIZE, vpeercapability),
			    tris_getformatname_multiple(s5, SIPBUFSIZE, dpeercapability),
			    tris_getformatname_multiple(s4, SIPBUFSIZE, tpeercapability),
			    tris_getformatname_multiple(s5, SIPBUFSIZE, newjointcapability));

		tris_verbose("Non-codec capabilities (dtmf): us - %s, peer - %s, combined - %s\n",
			    tris_rtp_lookup_mime_multiple(s1, SIPBUFSIZE, p->noncodeccapability, 0, 0),
			    tris_rtp_lookup_mime_multiple(s2, SIPBUFSIZE, peernoncodeccapability, 0, 0),
			    tris_rtp_lookup_mime_multiple(s3, SIPBUFSIZE, newnoncodeccapability, 0, 0));
	}
	...
	if (p->rtp) {
		if (portno > 0) {
			...
			if (tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_AUTO) {
				tris_clear_flag(&p->flags[0], SIP_DTMF);
				if (newnoncodeccapability & TRIS_RTP_DTMF) {
					/* XXX Would it be reasonable to drop the DSP at this point? XXX */
					tris_set_flag(&p->flags[0], SIP_DTMF_RFC2833);
					/* Since RFC2833 is now negotiated we need to change some properties of the RTP stream */
					tris_rtp_setdtmf(p->rtp, 1);
					tris_rtp_setdtmfcompensate(p->rtp, tris_test_flag(&p->flags[1], SIP_PAGE2_RFC2833_COMPENSATE));
				} else {
					tris_set_flag(&p->flags[0], SIP_DTMF_INBAND);
				}
			}
	...
	/* Setup desktop address and port */
	if (p->drtp) {
		if (dportno > 0) {
			dsin.sin_family = AF_INET;
			dsin.sin_port = htons(dportno);
			memcpy(&dsin.sin_addr, dhp->h_addr, sizeof(dsin.sin_addr));
			tris_rtp_set_peer(p->drtp, &dsin);
			if (debug) 
				tris_verbose("Peer desktop RTP is at port %s:%d\n", tris_inet_ntoa(dsin.sin_addr), ntohs(dsin.sin_port));
			tris_rtp_pt_copy(p->drtp, newdesktoprtp);
		} else {
			tris_rtp_stop(p->drtp);
			if (debug)
				tris_verbose("Peer doesn't provide desktop\n");
		}
	}
	...
	if (p->udptl) {
		if (udptlportno > 0) {
			...
			if (tris_test_flag(&p->flags[0], SIP_NAT) && tris_test_flag(&p->flags[1], SIP_PAGE2_UDPTL_DESTINATION)) {
	...
	tris_debug(2, "We're settling with these formats: %s\n", tris_getformatname_multiple(buf, SIPBUFSIZE, p->jointcapability));

	if (sendonly && sendonly != -1) {
		if (sendonly == 1)	/* One directional hold (sendonly/recvonly) */
			tris_set_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD_ONEDIR);
		else if (sendonly == 2)	/* Inactive stream */
			tris_set_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD_INACTIVE);
		else
			tris_set_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD_ACTIVE);
	}
	...
	if (!(p->owner->nativeformats & p->jointcapability) && (p->jointcapability & TRIS_FORMAT_AUDIO_MASK)) {
		if (debug) {
			...
			tris_debug(1, "Oooh, we need to change our audio formats since our peer supports only %s and not %s\n", 
				tris_getformatname_multiple(s1, SIPBUFSIZE, p->jointcapability),
				tris_getformatname_multiple(s2, SIPBUFSIZE, p->owner->nativeformats));
		}
	...
	if (tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD) && sin.sin_addr.s_addr && (!sendonly || sendonly == -1)) {
		...
		tris_clear_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD); /* Clear both flags */
	} else if (!sin.sin_addr.s_addr || (sendonly && sendonly != -1)) {
		int already_on_hold = tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD);
		...
		if (sip_cfg.callevents && !tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD)) {
			manager_event(EVENT_FLAG_CALL, "Hold",
				      "Status: On\r\n"
				      "Channel: %s\r\n"
				      "Uniqueid: %s\r\n",
...
static int process_sdp_o(const char *o, struct sip_pvt *p)
{
	...
	if (tris_test_flag(&p->flags[1], SIP_PAGE2_IGNORESDPVERSION) ||
	...
	if (!strncasecmp(a, "ptime", 5)) {
		...
		if (*last_rtpmap_codec < SDP_MAX_RTPMAP_CODECS) {
			if (tris_rtp_set_rtpmap_type_rate(newaudiortp, codec, "audio", mimeSubtype,
			    tris_test_flag(&p->flags[0], SIP_G726_NONSTANDARD) ? TRIS_RTP_OPT_G726_NONSTANDARD : 0, sample_rate) != -1) {
				if (debug)
	...
	if (sscanf(a, "rtpmap: %30u %127[^/]/%30u", &codec, mimeSubtype, &sample_rate) == 3) {
		...
		if (*last_rtpmap_codec < SDP_MAX_RTPMAP_CODECS) {
			...
			if (!strncasecmp(mimeSubtype, "H26", 3) || !strncasecmp(mimeSubtype, "MP4", 3)) {
				if (!strncasecmp(mimeSubtype, "H264", 4) && codec != 125) {
...
static int process_sdp_a_desktop(const char *a, struct sip_pvt *p, struct tris_rtp *newdesktoprtp, int *last_rtpmap_codec)
{
	...
	if (sscanf(a, "rtpmap: %30u %127[^/]/%30u", &codec, mimeSubtype, &sample_rate) == 3) {
		...
		if (*last_rtpmap_codec < SDP_MAX_RTPMAP_CODECS) {
			...
			if (!strncasecmp(mimeSubtype, "H26", 3) || !strncasecmp(mimeSubtype, "MP4", 3)) {
				if (!strncasecmp(mimeSubtype, "H264", 4) && codec != 125) {
					if (tris_rtp_set_rtpmap_type_rate(newdesktoprtp, codec, "desktop", mimeSubtype, 0, sample_rate) != -1) {
						if (debug)
							tris_verbose("Found desktop description format %s for ID %d\n", mimeSubtype, codec);
						//found_rtpmap_codecs[last_rtpmap_codec] = codec;
						(*last_rtpmap_codec)++;
						found = TRUE;
					} else {
						tris_rtp_unset_m_type(newdesktoprtp, codec);
						if (debug) 
							tris_verbose("Found unknown media description format %s for ID %d\n", mimeSubtype, codec);
					}
				}
...
static int add_header(struct sip_request *req, const char *var, const char *value)
{
	if (req->headers == SIP_MAX_HEADERS) {
		tris_log(LOG_WARNING, "Out of SIP header space\n");
		return -1;
	}
...
static int add_line(struct sip_request *req, const char *line)
{
	if (req->lines == SIP_MAX_LINES)  {
		tris_log(LOG_WARNING, "Out of SIP line space\n");
		return -1;
	}
	if (!req->lines)
...
static int copy_via_headers(struct sip_pvt *p, struct sip_request *req, const struct sip_request *orig, const char *field)
{
	...
	for (;;) {
		...
		if (!copied) {	/* Only check for empty rport in topmost via header */
			...
			if (rport && ((tris_test_flag(&p->flags[0], SIP_NAT) == SIP_NAT_ALWAYS) || (tris_test_flag(&p->flags[0], SIP_NAT) == SIP_NAT_RFC3581))) {
...
static int init_resp(struct sip_request *resp, const char *msg)
{
	...
	if (!(resp->data = tris_str_create(SIP_MIN_PACKET)))
		return -1;
	resp->header[0] = 0;
...
static int respprep(struct sip_request *resp, struct sip_pvt *p, const char *msg, const struct sip_request *req)
{
	...
	if (!strcasestr(ot, "tag=") && strncmp(msg, "100", 3)) {
		...
		if (!tris_strlen_zero(p->theirtag) && tris_test_flag(&p->flags[0], SIP_OUTGOING))
			snprintf(newto, sizeof(newto), "%s;tag=%s", ot, p->theirtag);
		else if (p->tag && !tris_test_flag(&p->flags[0], SIP_OUTGOING))
...
static int reqprep(struct sip_request *req, struct sip_pvt *p, int sipmethod, int seqno, int newbranch)
{
	...
	int is_outbound = tris_test_flag(&p->flags[0], SIP_OUTGOING);	/* Session direction */
			     ...
			     int debug, int *min_packet_size)
{
	...
	tris_str_append(a_buf, 0, "a=rtpmap:%d %s/%d\r\n", rtp_code,
		       tris_rtp_lookup_mime_subtype(1, codec,
						   tris_test_flag(&p->flags[0], SIP_G726_NONSTANDARD) ? TRIS_RTP_OPT_G726_NONSTANDARD : 0),
			     ...
			     int debug, int *min_packet_size)
{
	...
	tris_str_append(a_buf, 0, "a=rtpmap:%d %s/%d\r\n", rtp_code,
		       ...
		       tris_rtp_lookup_sample_rate(1, codec));
	if (rtp_code == 126) {
		tris_str_append(a_buf, 0, "a=fmtp:126 profile-level-id=42e015; packetization-mode=1;\r\n");
	}
	/* Add fmtp code here */
}

/*! \brief Add desktop codec offer to SDP offer/answer body in INVITE or 200 OK */
/* This is different to the audio one now so we can add more caps later */
static void add_dcodec_to_sdp(const struct sip_pvt *p, int codec,
			     struct tris_str **m_buf, struct tris_str **a_buf,
			     int debug, int *min_packet_size)
{
	int rtp_code;
	...
	if (debug)
		tris_verbose("Adding desktop codec 0x%x (%s) to SDP\n", codec, tris_getformatname(codec));

	if ((rtp_code = tris_rtp_lookup_code(p->drtp, 1, codec)) == -1)
		return;
...
*/
static void get_our_media_address(struct sip_pvt *p, int needvideo, int needdesktop,
	struct sockaddr_in *sin, struct sockaddr_in *vsin, struct sockaddr_in *dsin, struct sockaddr_in *tsin,
	struct sockaddr_in *dest, struct sockaddr_in *vdest, struct sockaddr_in *ddest)
{
	...
	if (needdesktop) {
		/* Determine desktop sharing destination */
		if (p->dredirip.sin_addr.s_addr) {
			ddest->sin_addr = p->dredirip.sin_addr;
			ddest->sin_port = p->dredirip.sin_port;
		} else {
			ddest->sin_addr = p->ourip.sin_addr;
			ddest->sin_port = dsin->sin_port;
		}
	}
...
static enum sip_result add_sdp(struct sip_request *resp, struct sip_pvt *p, int oldsdp, int add_audio, int add_t38)
{
	...
	struct tris_str *m_desktop = tris_str_alloca(256);  /* Media declaration line for desktop sharing */
	...
	struct tris_str *a_desktop = tris_str_alloca(1024); /* Attributes for desktop */
	...
	int needvideo = FALSE;
	int needtext = FALSE;
	int needdesktop = FALSE;
	int debug = sip_debug_test_pvt(p);
	int min_audio_packet_size = 0;
	int min_video_packet_size = 0;
	int min_desktop_packet_size = 0;
	...
	if (add_audio) {
		...
		/* Check if we need desktop in this call */
		if (!p->nodesktop) {
			if (p->drtp) {
				needdesktop = TRUE;
				tris_debug(2, "This call needs desktop offers!\n");
			} else
				tris_debug(2, "This call needs desktop offers, but there's no desktop support enabled!\n");
		}
	...
	get_our_media_address(p, needvideo, needdesktop, &sin, &vsin, &dsin, &tsin, &dest, &vdest, &ddest);
	...
	if (add_audio) {
		...
		/* Ok, we need desktop. Let's add what we need for desktop and set codecs.
		   Video is handled differently than audio since we can not transcode. */
		if (needdesktop) {
			tris_str_append(&m_desktop, 0, "m=desktop %d RTP/AVP %s", ntohs(ddest.sin_port), p->offered_media[SDP_DESKTOP].text);

			/* Build max bitrate string */
			if (debug) 
				tris_verbose("Desktop sharing is at %s port %d\n", tris_inet_ntoa(p->ourip.sin_addr), ntohs(ddest.sin_port));	
		...
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD) == SIP_PAGE2_CALL_ONHOLD_ONEDIR)
			hold = "a=recvonly\r\n";
		else if (tris_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD) == SIP_PAGE2_CALL_ONHOLD_INACTIVE)
			hold = "a=inactive\r\n";
		else
			hold = "a=sendrecv\r\n";
		...
		*/
		...
		for (x = 1; x <= (needtext ? TRIS_FORMAT_TEXT_MASK : (needvideo ? TRIS_FORMAT_VIDEO_MASK : TRIS_FORMAT_AUDIO_MASK)); x <<= 1) {
			...
			else if (x & TRIS_FORMAT_DESKTOP)
				add_dcodec_to_sdp(p, x, &m_desktop, &a_desktop, debug, &min_desktop_packet_size);
		...
		/* XXX don't think you can have ptime for desktop sharing */
		min_video_packet_size = min_video_packet_size;
		if (min_desktop_packet_size)
			tris_str_append(&a_desktop, 0, "a=ptime:%d\r\n", min_desktop_packet_size);
		...
		if (m_audio->len - m_audio->used < 2 || m_video->len - m_video->used < 2 ||
		    ...
		    m_desktop->len - m_desktop->used < 2 || a_desktop->len - a_desktop->used < 2)
			tris_log(LOG_WARNING, "SIP SDP may be truncated due to undersized buffer!!\n");
	}
	...
	if (needaudio)
 	...
 	if (needdesktop) /* only if desktop response is appropriate */
 		len += m_desktop->used + a_desktop->used + strlen(bandwidth) + strlen(hold);
	...
	if (needdesktop) { /* only if desktop response is appropriate */
		add_line(resp, m_desktop->str);
		add_line(resp, a_desktop->str);
		add_line(resp, hold);	/* Repeat hold for the video stream */
	} else if (p->offered_media[SDP_DESKTOP].offered) {
		snprintf(dummy_answer, sizeof(dummy_answer), "m=desktop 0 RTP/AVP %s\r\n", p->offered_media[SDP_DESKTOP].text);
		add_line(resp, dummy_answer);
	}
	if (needtext) { /* only if text response is appropriate */
...
static int transmit_response_with_sdp(struct sip_pvt *p, const char *msg, const struct sip_request *req, enum xmittype reliable, int oldsdp)
{
	...
	respprep(&resp, p, msg, req);
	if (p->rtp) {
		if (!p->autoframing && !tris_test_flag(&p->flags[0], SIP_OUTGOING)) {
...
static int transmit_reinvite_with_sdp(struct sip_pvt *p, int t38version, int oldsdp)
{
	...
	reqprep(&req, p, tris_test_flag(&p->flags[0], SIP_REINVITE_UPDATE) ?  SIP_UPDATE : SIP_INVITE, 0, 1);

	add_header(&req, "Allow", ALLOWED_METHODS);
	add_header(&req, "Supported", SUPPORTED_EXTENSIONS);
	if (sipdebug) {
		if (oldsdp == TRUE)
			add_header(&req, "X-trismedia-Info", "SIP re-invite (Session-Timers)");
		else
			add_header(&req, "X-trismedia-Info", "SIP re-invite (External RTP bridge)");
	}

	if (p->do_history)
		append_history(p, "ReInv", "Re-invite sent");

	// I need changed sip_pvt's offered media for appconference, so comment below line and add condition.
	//memset(p->offered_media, 0, sizeof(p->offered_media));
	if(p->owner->appl && strcasecmp(p->owner->appl, "Conference"))
		memset(p->offered_media, 0, sizeof(p->offered_media));
	...
	tris_set_flag(&p->flags[0], SIP_OUTGOING);       /* Change direction of this dialog */
...
static void initreqprep(struct sip_request *req, struct sip_pvt *p, int sipmethod)
{
	...
	if (tris_test_flag(&p->flags[0], SIP_USEREQPHONE)) {
	...
	if (!tris_test_flag(&p->flags[0], SIP_SENDRPID) &&
	...
	if (!sip_standard_port(p->socket.type, ourport) && tris_strlen_zero(p->fromdomain))
	...
	tris_string_field_set(p, uri, invite->str);
 	...
 	if (!tris_strlen_zero(p->todnid)) {
 		...
 		if (sipmethod == SIP_NOTIFY && !tris_strlen_zero(p->theirtag)) { 
	...
	if (tris_test_flag(&p->flags[0], SIP_SENDRPID) && (sipmethod == SIP_INVITE)) {
		build_rpid(p);
	...
	if (p->options && p->options->addsipheaders && p->owner) {
		...
		if (!headp)
			...
			TRIS_LIST_TRAVERSE(headp, current, entries) {  
				...
				if (!strncasecmp(tris_var_name(current), "SIPADDHEADER", strlen("SIPADDHEADER"))) {
					char *content, *end;
					...
					if ((content = strchr(headdup, ':'))) {
						*content++ = '\0';
						...
						if (sipdebug)
							tris_debug(1, "Adding SIP Header \"%s\" with content :%s: \n", headdup, content);
					}
				}
			}
		}
...
static int __sip_subscribe_mwi_do(struct sip_subscription_mwi *mwi)
{
	...
	if (!mwi->dnsmgr) {
		...
		tris_dnsmgr_lookup(mwi->hostname, &mwi->us, &mwi->dnsmgr, sip_cfg.srvlookup ? transport : NULL);
	}
	...
	tris_sip_ouraddrfor(&mwi->call->sa.sin_addr, &mwi->call->ourip, mwi->call);
	build_contact(mwi->call);
	build_via(mwi->call);
	build_callid_pvt(mwi->call);
	tris_set_flag(&mwi->call->flags[0], SIP_OUTGOING);
...
static int transmit_state_notify(struct sip_pvt *p, int state, int full, int timeout)
{
	...
	if (strncasecmp(c, "sip:", 4) && strncasecmp(c, "sips:", 5)) {
		tris_log(LOG_WARNING, "Huh?  Not a SIP header (%s)?\n", c);
		return -1;
	}
	...
	case DIALOG_INFO_XML: /* SNOM subscribes in this format */
		...
		if ((state & TRIS_EXTENSION_RINGING) && sip_cfg.notifyringing) {
...
static int transmit_notify_with_mwi(struct sip_pvt *p, int newmsgs, int oldmsgs, const char *vmexten)
{
	...
	tris_str_append(&out, 0, "Voice-Message: %d/%d%s\r\n",
		newmsgs, oldmsgs, (tris_test_flag(&p->flags[1], SIP_PAGE2_BUGGY_MWI) ? "" : " (0/0)"));
...
static int transmit_notify_with_sipfrag(struct sip_pvt *p, int cseq, char *message, int terminate)
{
	...
	reqprep(&req, p, SIP_NOTIFY, 0, 1);
	if (cseq == -1) {
		snprintf(tmp, sizeof(tmp), "videoconf");
		cseq = 0;
	} else {
		const char* buf = NULL;
		if (p->owner)
			buf = p->owner->refer_phonenum;
		if (!tris_strlen_zero(buf))
			snprintf(tmp, sizeof(tmp), "conf;id=%d;value=%s", cseq, buf);
		else
			snprintf(tmp, sizeof(tmp), "conf;id=%d", cseq);
	}
...
static int transmit_register(struct sip_registry *r, int sipmethod, const char *auth, const char *authheader)
{
	...
	if (r->dnsmgr == NULL) {
		...
		tris_dnsmgr_lookup(peer ? peer->tohost : r->hostname, &r->us, &r->dnsmgr, sip_cfg.srvlookup ? transport : NULL);
		if (peer) {
	...
	if (r->call) {	/* We have a registration */
		...
		if (create_addr(p, r->hostname, &r->us, 0)) {
			...
			if (r->timeout > -1) {
				TRIS_SCHED_REPLACE_UNREF(r->timeout, sched, global_reg_timeout * 1000, sip_reg_timeout, r,
				...
				r->timeout = tris_sched_add(sched, global_reg_timeout * 1000, sip_reg_timeout, registry_addref(r, "add for REPLACE registry ptr"));
		...
		tris_set_flag(&p->flags[0], SIP_OUTGOING);	/* Registration is outgoing call */
...
static int sip_refer_allocate(struct sip_pvt *p)
{
	p->refer = tris_calloc(1, sizeof(struct sip_refer)); 
	return p->refer ? 1 : 0;
}
...
static int transmit_refer(struct sip_pvt *p, const char *dest)
{
	...
	if (sipdebug)
		tris_debug(1, "SIP transfer of %s to %s\n", p->callid, dest);
	...
	if (!strncasecmp(of, "sip:", 4)) {
		...
		tris_log(LOG_NOTICE, "From address missing 'sip(s):', assuming sip:\n");
	}
...
static int transmit_info_with_digit(struct sip_pvt *p, const char digit, unsigned int duration)
{
	...
	add_digit(&req, digit, duration, (tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_SHORTINFO));
	return send_request(p, &req, XMIT_RELIABLE, p->ocseq);
}
...
static int expire_register(const void *data)
{
	...
	*/
	...
	if (peer->selfdestruct ||
	    tris_test_flag(&peer->flags[1], SIP_PAGE2_RTAUTOCLEAR)) {
...
static void reg_source_db(struct sip_peer *peer)
{
	...
	tris_debug(2, "SIP Seeding peer from astdb: '%s' at %s@%s:%d for %d\n",
	...
	if (sipsock < 0) {
		...
		TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, tris_random() % 5000 + 1, sip_poke_peer_s, peer,
...
static int set_address_from_contact(struct sip_pvt *pvt)
{
	if (tris_test_flag(&pvt->flags[0], SIP_NAT_ROUTE)) {
...
static enum parse_register_result parse_register_contact(struct sip_pvt *pvt, struct sip_peer *peer, struct sip_request *req)
{
	...
	*/
	if (tris_strlen_zero(curi) && tris_strlen_zero(expires)) {
		...
		tris_verb(3, "Unregistered SIP '%s'\n", peer->name);
	...
	if (parse_uri(curi, "sip:,sips:", &curi, NULL, &host, &pt, NULL, &transport)) {
		tris_log(LOG_NOTICE, "Not a valid SIP contact (missing sip:) trying to use anyway\n");
	}
	...
	if (!tris_test_flag(&peer->flags[0], SIP_NAT_ROUTE)) {
		peer->addr.sin_family = AF_INET;
	...
	if (expire < min_expiry)
		expire = min_expiry;
	if (peer->is_realtime && !tris_test_flag(&peer->flags[1], SIP_PAGE2_RTCACHEFRIENDS)) {
		peer->expire = -1;
	} else {
	...
	*/
	...
	if (VERBOSITY_ATLEAST(2) && inaddrcmp(&peer->addr, &oldsin)) {
		tris_verbose(VERBOSE_PREFIX_3 "Registered SIP '%s' at %s port %d\n", peer->name, tris_inet_ntoa(peer->addr.sin_addr), ntohs(peer->addr.sin_port));
	}
	sip_poke_peer(peer, 0);
					      ...
					      struct sip_request *req, char *uri)
{
	...
	if (peer) {
		...
		if (!peer->host_dynamic) {
			...
			tris_copy_flags(&p->flags[0], &peer->flags[0], SIP_NAT);
			if (tris_test_flag(&p->flags[1], SIP_PAGE2_REGISTERTRYING))
			...
			if (!(res = check_auth(p, req, peer->name, peer->secret, peer->md5secret, SIP_REGISTER, uri, XMIT_UNRELIABLE, req->ignore))) {
				...
				if (check_request_transport(peer, req)) {
					...
					case PARSE_REGISTER_UPDATE:
						...
						transmit_response_with_date(p, "200 OK", req);
						if (!tris_test_flag((&peer->flags[1]), SIP_PAGE2_SUBSCRIBEMWIONLY))
							peer->lastmsgssent = -1;
						res = 0;
						break;
					}
...
static int get_rdnis(struct sip_pvt *p, struct sip_request *oreq)
{
	...
	*/
	...
	if (!strncasecmp(exten, "sip:", 4)) {
		...
		tris_log(LOG_WARNING, "Huh?  Not an RDNIS SIP header (%s)?\n", exten);
		return -1;
	}
...
static int get_destination(struct sip_pvt *p, struct sip_request *oreq)
{
	...
	if (!TRIS_LIST_EMPTY(&domain_list)) {
		...
		if (!check_sip_domain(p->domain, domain_context, sizeof(domain_context))) {
			if (!sip_cfg.allow_external_domains && (req->method == SIP_INVITE || req->method == SIP_REFER)) {
				tris_debug(1, "Got SIP %s to non-local domain '%s'; refusing request.\n", sip_methods[req->method].text, p->domain);
				return -2;
			}
		}
	...
	if (req->method == SIP_SUBSCRIBE && !tris_strlen_zero(p->subscribecontext))
	...
	if((tris_test_flag(&global_flags[1], SIP_PAGE2_ALLOWOVERLAP) && 
...
static struct sip_pvt *get_sip_pvt_byid_locked(const char *callid, const char *totag, const char *fromtag) 
{
	...
	if (sip_pvt_ptr) {
		...
		/* deadlock avoidance... */
		while (sip_pvt_ptr->owner && tris_channel_trylock(sip_pvt_ptr->owner)) {
			sip_pvt_unlock(sip_pvt_ptr);
			usleep(1);
			sip_pvt_lock(sip_pvt_ptr);
		}
...
static int get_refer_info(struct sip_pvt *transferer, struct sip_request *outgoing_req)
{
	...
	if (referdata->attendedtransfer || tris_exists_extension(NULL, transfer_context, refer_to, 1, NULL) ) {
		if (sip_debug_test_pvt(transferer)) {
			tris_verbose("SIP transfer to extension %s@%s by %s\n", refer_to, transfer_context, referred_by_uri);
		}
		/* We are ready to transfer to the extension */
		return 0;
	} 
	if (sip_debug_test_pvt(transferer))
		tris_verbose("Failed SIP Transfer to non-existing extension %s in context %s\n n", refer_to, transfer_context);
...
static int get_also_info(struct sip_pvt *p, struct sip_request *oreq)
{
	...
	if (!strncasecmp(c, "sip:", 4)) {
		...
		tris_log(LOG_WARNING, "Huh?  Not a SIP header in Also: transfer (%s)?\n", c);
		return -1;
	}
	...
	if (tris_exists_extension(NULL, transfer_context, c, 1, NULL)) {
		...
		tris_debug(1, "SIP Bye-also transfer to Extension %s@%s \n", c, transfer_context);
 ...
 * Note that tris_sip_ouraddrfor() still rewrites p->ourip
...
static void check_via(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (c && (c[6] != '='))	/* rport query, not answer */
		tris_set_flag(&p->flags[1], SIP_PAGE2_RPORT_PRESENT);
	...
	if (c) {
		*c = '\0';
		...
		if (sip_debug_test_pvt(p)) {
			const struct sockaddr_in *dst = sip_real_dst(p);
			tris_verbose("Sending to %s : %d (%s)\n", tris_inet_ntoa(dst->sin_addr), ntohs(dst->sin_port), sip_nat_mode(p));
		}
	}
}
...
static void replace_cid(struct sip_pvt *p, const char *rpid_num, const char *calleridname)
{
	...
	if (!tris_strlen_zero(rpid_num) && tris_test_flag(&p->flags[0], SIP_TRUSTRPID)) {
...
//	enum check_auth_result res;
	...
	/* Take the peer */
	tris_copy_flags(&p->flags[0], &peer->flags[0], SIP_FLAGS_TO_COPY);
	tris_copy_flags(&p->flags[1], &peer->flags[1], SIP_PAGE2_FLAGS_TO_COPY);

	if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT) && p->udptl) {
	...
	do_setnat(p, tris_test_flag(&p->flags[0], SIP_NAT_ROUTE));
	...
	if (tris_test_flag(&peer->flags[0], SIP_INSECURE_INVITE)) {
...
/*	if (!(res = check_auth(p, req, peer->name, p->peersecret, p->peermd5secret, sipmethod, uri2, reliable, req->ignore))) {*/
		...
		if (peer->call_limit)
			tris_set_flag(&p->flags[0], SIP_CALL_LIMIT);
		...
		if (!tris_test_flag(&p->flags[1], SIP_PAGE2_VIDEOSUPPORT_ALWAYS) &&
				(!tris_test_flag(&p->flags[1], SIP_PAGE2_VIDEOSUPPORT) ||
					!(p->capability & TRIS_FORMAT_VIDEO_MASK)) &&
				p->vrtp) {
			tris_rtp_destroy(p->vrtp);
			p->vrtp = NULL;
		}
		if ((!tris_test_flag(&p->flags[1], SIP_PAGE2_TEXTSUPPORT) || !(p->capability & TRIS_FORMAT_TEXT_MASK)) && p->trtp) {
			tris_rtp_destroy(p->trtp);
			p->trtp = NULL;
 		}
		if ((tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_RFC2833) ||
					      ...
					      struct sockaddr_in *sin, struct sip_peer **authpeer)
{
	...
	if (tris_test_flag(&p->flags[1], SIP_PAGE2_RPORT_PRESENT)) {
		tris_set_flag(&p->flags[0], SIP_NAT_ROUTE);
	}
...
static void receive_message(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (p->owner) {
		if (sip_debug_test_pvt(p))
			tris_verbose("SIP Text message received: '%s'\n", buf);
		memset(&f, 0, sizeof(f));
...
static char *sip_show_tcp(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct sip_threadinfo *th;
	struct ao2_iterator i;
...
static char *sip_show_users(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	regex_t regexbuf;
...
#define FORMAT  "%-25.25s  %-15.15s  %-15.15s  %-15.15s  %-5.5s%-10.10s\n"
	...
	while ((user = ao2_iterator_next(&user_iter))) {
		...
		tris_cli(a->fd, FORMAT, user->name, 
			...
			nat2str(tris_test_flag(&user->flags[0], SIP_NAT)));
		ao2_unlock(user);
		unref_peer(user, "sip show users");
	}
...
#define FORMAT  "%-25.25s  %-15.15s %-3.3s %-3.3s %-3.3s %-8d %-10s %s\n"
	...
	realtimepeers = tris_check_realtime("sippeers");
	peerarray = tris_calloc(sizeof(struct sip_peer *), objcount);
	...
	for(k=0; k < total_peers; k++) {
		...
		snprintf(srch, sizeof(srch), FORMAT, name,
			...
			tris_test_flag(&peer->flags[0], SIP_NAT_ROUTE) ? " N " : "   ",	/* NAT=yes? */
		...
		if (!s)  {/* Normal CLI list */
			...
			tris_test_flag(&peer->flags[0], SIP_NAT_ROUTE) ? "yes" : "no",	/* NAT=yes? */
			tris_test_flag(&peer->flags[1], SIP_PAGE2_VIDEOSUPPORT) ? "yes" : "no",	/* VIDEOSUPPORT=yes? */
			tris_test_flag(&peer->flags[1], SIP_PAGE2_TEXTSUPPORT) ? "yes" : "no",	/* TEXTSUPPORT=yes? */
	...
	if (!s)
		tris_cli(fd, "%d sip peers [Monitored: %d online, %d offline Unmonitored: %d online, %d offline]\n",
...
static void cleanup_stale_contexts(char *new, char *old)
{
	...
	while ((oldcontext = strsep(&old, "&"))) {
		...
		if (stalecontext)
			tris_context_destroy(tris_context_find(stalecontext), "SIP");
	}
}
...
static int dialog_needdestroy(void *dialogobj, void *arg, int flags) 
{
	...
	if (dialog->needdestroy && !dialog->packets && !dialog->owner) {
		...
		if (dialog->rtp && tris_rtp_get_bridged(dialog->rtp)) {
			tris_debug(2, "Bridge still active.  Delaying destruction of SIP dialog '%s' Method: %s\n", dialog->callid, sip_methods[dialog->method].text);
			sip_pvt_unlock(dialog);
			return 0;
		}
		
		if (dialog->vrtp && tris_rtp_get_bridged(dialog->vrtp)) {
			tris_debug(2, "Bridge still active.  Delaying destroy of SIP dialog '%s' Method: %s\n", dialog->callid, sip_methods[dialog->method].text);
			sip_pvt_unlock(dialog);
			return 0;
		}
	...
	if (multi) {
		if (prunepeer) {
			...
			while ((pi = ao2_t_iterator_next(&i, "iterate thru peers table"))) {
				...
				if (tris_test_flag(&pi->flags[1], SIP_PAGE2_RTCACHEFRIENDS)) {
					pi->the_mark = 1;
					pruned++;
				}
				ao2_unlock(pi);
		...
		if (prunepeer) {
			...
			if ((peer = ao2_t_find(peers, &tmp, OBJ_POINTER | OBJ_UNLINK, "finding to unlink from peers"))) {
				...
				if (!tris_test_flag(&peer->flags[1], SIP_PAGE2_RTCACHEFRIENDS)) {
...
#define FORMAT "%-40.40s %-20.20s %-16.16s\n"
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Trismedia only responds to SIP messages to local domains.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	if (TRIS_LIST_EMPTY(&domain_list)) {
		tris_cli(a->fd, "SIP Domain support not enabled.\n\n");
		return CLI_SUCCESS;
	} else {
		tris_cli(a->fd, FORMAT, "Our local SIP domains:", "Context", "Set by");
...
static char *_sip_show_peer(int type, int fd, struct mansession *s, const struct message *m, int argc, const char *argv[])
{
	...
	if (peer && type==0 ) { /* Normal listing */
		...
		tris_cli(fd, "  Insecure     : %s\n", insecure2str(tris_test_flag(&peer->flags[0], SIP_INSECURE)));
		tris_cli(fd, "  Nat          : %s\n", nat2str(tris_test_flag(&peer->flags[0], SIP_NAT)));
		tris_cli(fd, "  ACL          : %s\n", cli_yesno(peer->ha != NULL));
		tris_cli(fd, "  T.38 support : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], SIP_PAGE2_T38SUPPORT)));
		tris_cli(fd, "  T.38 EC mode : %s\n", faxec2str(tris_test_flag(&peer->flags[1], SIP_PAGE2_T38SUPPORT)));
		tris_cli(fd, "  T.38 MaxDtgrm: %d\n", peer->t38_maxdatagram);
		tris_cli(fd, "  DirectMedia  : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], SIP_DIRECT_MEDIA)));
		tris_cli(fd, "  PromiscRedir : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], SIP_PROMISCREDIR)));
		tris_cli(fd, "  User=Phone   : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], SIP_USEREQPHONE)));
		tris_cli(fd, "  Video Support: %s\n", cli_yesno(tris_test_flag(&peer->flags[1], SIP_PAGE2_VIDEOSUPPORT)));
		tris_cli(fd, "  Text Support : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], SIP_PAGE2_TEXTSUPPORT)));
		tris_cli(fd, "  Ign SDP ver  : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], SIP_PAGE2_IGNORESDPVERSION)));
		tris_cli(fd, "  Trust RPID   : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], SIP_TRUSTRPID)));
		tris_cli(fd, "  Send RPID    : %s\n", cli_yesno(tris_test_flag(&peer->flags[0], SIP_SENDRPID)));
		tris_cli(fd, "  Subscriptions: %s\n", cli_yesno(tris_test_flag(&peer->flags[1], SIP_PAGE2_ALLOWSUBSCRIBE)));
		tris_cli(fd, "  Overlap dial : %s\n", cli_yesno(tris_test_flag(&peer->flags[1], SIP_PAGE2_ALLOWOVERLAP)));
		if (peer->outboundproxy)
		...
		/* - is enumerated */
		tris_cli(fd, "  DTMFmode     : %s\n", dtmfmode2str(tris_test_flag(&peer->flags[0], SIP_DTMF)));
		...
		tris_cli(fd, "  100 on REG   : %s\n", tris_test_flag(&peer->flags[1], SIP_PAGE2_REGISTERTRYING) ? "Yes" : "No");
		tris_cli(fd, "  Status       : ");
		...
		astman_append(s, "SIP-CanReinvite: %s\r\n", (tris_test_flag(&peer->flags[0], SIP_DIRECT_MEDIA)?"Y":"N"));
		astman_append(s, "SIP-DirectMedia: %s\r\n", (tris_test_flag(&peer->flags[0], SIP_DIRECT_MEDIA)?"Y":"N"));
		astman_append(s, "SIP-PromiscRedir: %s\r\n", (tris_test_flag(&peer->flags[0], SIP_PROMISCREDIR)?"Y":"N"));
		astman_append(s, "SIP-UserPhone: %s\r\n", (tris_test_flag(&peer->flags[0], SIP_USEREQPHONE)?"Y":"N"));
		astman_append(s, "SIP-VideoSupport: %s\r\n", (tris_test_flag(&peer->flags[1], SIP_PAGE2_VIDEOSUPPORT)?"Y":"N"));
		astman_append(s, "SIP-TextSupport: %s\r\n", (tris_test_flag(&peer->flags[1], SIP_PAGE2_TEXTSUPPORT)?"Y":"N"));
		astman_append(s, "SIP-T.38Support: %s\r\n", (tris_test_flag(&peer->flags[1], SIP_PAGE2_T38SUPPORT)?"Y":"N"));
...
static char *sip_show_user(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char cbuf[256];
	struct sip_peer *user;
	struct tris_variable *v;
...
#define FORMAT  "%-30.30s %-6.6s %-12.12s  %8d %-20.20s %-25.25s\n"
	...
	tris_cli(a->fd, "%d SIP registrations.\n", counter);
	return CLI_SUCCESS;
#undef FORMAT
#undef FORMAT2
}
...
static char *sip_show_channelstats(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_cli(a->fd, "%d active SIP channel%s\n", arg.numchans, (arg.numchans != 1) ? "s" : ""); 
	return CLI_SUCCESS;
}
#undef FORMAT
#undef FORMAT2
...
static char *sip_show_settings(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	realtimepeers = tris_check_realtime("sippeers");
	realtimeregs = tris_check_realtime("sipregs");

	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	tris_cli(a->fd, "\n\nGlobal Settings:\n");
	tris_cli(a->fd, "----------------\n");
	tris_cli(a->fd, "  UDP SIP Port:           %d\n", ntohs(bindaddr.sin_port));
	tris_cli(a->fd, "  UDP Bindaddress:        %s\n", tris_inet_ntoa(bindaddr.sin_addr));
	tris_cli(a->fd, "  TCP SIP Port:           ");
	if (sip_tcp_desc.local_address.sin_family == AF_INET) {
		tris_cli(a->fd, "%d\n", ntohs(sip_tcp_desc.local_address.sin_port));
		tris_cli(a->fd, "  TCP Bindaddress:        %s\n", tris_inet_ntoa(sip_tcp_desc.local_address.sin_addr));
	} else {
		tris_cli(a->fd, "Disabled\n");
	}
	tris_cli(a->fd, "  TLS SIP Port:           ");
	if (default_tls_cfg.enabled != FALSE) {
		tris_cli(a->fd, "%d\n", ntohs(sip_tls_desc.local_address.sin_port));
		tris_cli(a->fd, "  TLS Bindaddress:        %s\n", tris_inet_ntoa(sip_tls_desc.local_address.sin_addr));
	} else {
		tris_cli(a->fd, "Disabled\n");
	}
	tris_cli(a->fd, "  Videosupport:           %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_VIDEOSUPPORT)));
	tris_cli(a->fd, "  Textsupport:            %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_TEXTSUPPORT)));
	tris_cli(a->fd, "  Ignore SDP sess. ver.:  %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_IGNORESDPVERSION)));
	...
	tris_cli(a->fd, "  Allow subscriptions:    %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_ALLOWSUBSCRIBE)));
	tris_cli(a->fd, "  Allow overlap dialing:  %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_ALLOWOVERLAP)));
	tris_cli(a->fd, "  Allow promsic. redir:   %s\n", cli_yesno(tris_test_flag(&global_flags[0], SIP_PROMISCREDIR)));
	tris_cli(a->fd, "  Enable call counters:   %s\n", cli_yesno(global_callcounter));
	tris_cli(a->fd, "  SIP domain support:     %s\n", cli_yesno(!TRIS_LIST_EMPTY(&domain_list)));
	...
	tris_cli(a->fd, "  URI user is phone no:   %s\n", cli_yesno(tris_test_flag(&global_flags[0], SIP_USEREQPHONE)));
	...
	tris_cli(a->fd, "  802.1p CoS SIP:         %d\n", global_cos_sip);
	...
	tris_cli(a->fd, "  SIP address remapping:  %s\n", msg);
	...
	tris_cli(a->fd, "  RFC2833 Compensation:   %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_RFC2833_COMPENSATE)));
	tris_cli(a->fd, "  Compact SIP headers:    %s\n", cli_yesno(sip_cfg.compactheaders));
	...
	tris_cli(a->fd, "  Nat:                    %s\n", nat2str(tris_test_flag(&global_flags[0], SIP_NAT)));
	tris_cli(a->fd, "  DTMF:                   %s\n", dtmfmode2str(tris_test_flag(&global_flags[0], SIP_DTMF)));
	tris_cli(a->fd, "  Qualify:                %d\n", default_qualify);
	tris_cli(a->fd, "  Use ClientCode:         %s\n", cli_yesno(tris_test_flag(&global_flags[0], SIP_USECLIENTCODE)));
	tris_cli(a->fd, "  Progress inband:        %s\n", (tris_test_flag(&global_flags[0], SIP_PROG_INBAND) == SIP_PROG_INBAND_NEVER) ? "Never" : (tris_test_flag(&global_flags[0], SIP_PROG_INBAND) == SIP_PROG_INBAND_NO) ? "No" : "Yes" );
	tris_cli(a->fd, "  Language:               %s\n", default_language);
	...
	if (realtimepeers || realtimeregs) {
		...
		tris_cli(a->fd, "  Cache Friends:          %s\n", cli_yesno(tris_test_flag(&global_flags[1], SIP_PAGE2_RTCACHEFRIENDS)));
...
static int show_channels_cb(void *__cur, void *__arg, int flags)
{
	...
	if (cur->subscribed == NONE && !arg->subscriptions) {
		...
		tris_cli(arg->fd, FORMAT, tris_inet_ntoa(dst->sin_addr), 
				...
				cli_yesno(tris_test_flag(&cur->flags[1], SIP_PAGE2_CALL_ONHOLD)),
				cur->needdestroy ? "(d)" : "",
				cur->lastmsg ,
				referstatus
			);
 ...
 */
static char *sip_show_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct __show_chan_arg arg = { .fd = a->fd, .numchans = 0 };
	...
	tris_cli(arg.fd, "%d active SIP %s%s\n", arg.numchans,
...
static char *sip_show_channel(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	while ((cur = ao2_t_iterator_next(&i, "iterate thru dialogs"))) {
		...
		if (!strncasecmp(cur->callid, a->argv[3], len)) {
			...
			if (cur->subscribed != NONE)
				...
				tris_cli(a->fd, "  * SIP Call\n");
			tris_cli(a->fd, "  Curr. trans. direction:  %s\n", tris_test_flag(&cur->flags[0], SIP_OUTGOING) ? "Outgoing" : "Incoming");
			...
			tris_cli(a->fd, "  NAT Support:            %s\n", nat2str(tris_test_flag(&cur->flags[0], SIP_NAT)));
			...
			tris_cli(a->fd, "  SIP User agent:         %s\n", cur->useragent);
			...
			tris_cli(a->fd, "  Promiscuous Redir:      %s\n", cli_yesno(tris_test_flag(&cur->flags[0], SIP_PROMISCREDIR)));
			tris_cli(a->fd, "  Route:                  %s\n", cur->route ? cur->route->hop : "N/A");
			tris_cli(a->fd, "  DTMF Mode:              %s\n", dtmfmode2str(tris_test_flag(&cur->flags[0], SIP_DTMF)));
			tris_cli(a->fd, "  SIP Options:            ");
			if (cur->sipoptions) {
				int x;
				for (x = 0 ; x < ARRAY_LEN(sip_options); x++) {
					if (cur->sipoptions & sip_options[x].id)
						tris_cli(a->fd, "%s ", sip_options[x].text);
				}
				tris_cli(a->fd, "\n");
			} else
				tris_cli(a->fd, "(none)\n");
	...
	if (!found) 
		tris_cli(a->fd, "No such SIP Call ID starting with '%s'\n", a->argv[3]);
...
static void sip_dump_history(struct sip_pvt *dialog)
{
	...
	if (!option_debug && !sipdebug) {
		if (!errmsg) {
			tris_log(LOG_NOTICE, "You must have debugging enabled (SIP or Trismedia) in order to dump SIP history.\n");
			errmsg = 1;
		}
		return;
	}
...
static void handle_request_info(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (!strcasecmp(c, "application/dtmf-relay") ||
		...
		if (tris_test_flag(&p->flags[0], SIP_USECLIENTCODE)) {
...
static char *sip_do_debug_ip(int fd, char *arg)
{
	...
	if (port == 0)
		tris_cli(fd, "SIP Debugging Enabled for IP: %s\n", tris_inet_ntoa(debugaddr.sin_addr));
	else
		tris_cli(fd, "SIP Debugging Enabled for IP: %s:%d\n", tris_inet_ntoa(debugaddr.sin_addr), port);
...
static char *sip_do_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (a->argc == e->args) {       /* on/off */
		if (!strcasecmp(what, "on")) {
			...
			tris_cli(a->fd, "SIP Debugging %senabled\n", oldsipdebug ? "re-" : "");
			return CLI_SUCCESS;
		} else if (!strcasecmp(what, "off")) {
			sipdebug &= ~sip_debug_console;
			sipdebug_text = 0;
			tris_cli(a->fd, "SIP Debugging Disabled\n");
			return CLI_SUCCESS;
		}
...
static char *sip_cli_notify(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	for (i = 3; i < a->argc; i++) {
		...
		if (!(p = sip_alloc(NULL, NULL, 0, SIP_NOTIFY, NULL))) {
			tris_log(LOG_WARNING, "Unable to build sip pvt data for notify (memory/socket error)\n");
			return CLI_FAILURE;
		}
...
static char *sip_set_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (!strncasecmp(a->argv[e->args - 1], "on", 2)) {
		recordhistory = TRUE;
		tris_cli(a->fd, "SIP History Recording Enabled (use 'sip show history')\n");
	} else if (!strncasecmp(a->argv[e->args - 1], "off", 3)) {
		recordhistory = FALSE;
		tris_cli(a->fd, "SIP History Recording Disabled\n");
	} else {
		return CLI_SHOWUSAGE;
	}
	return CLI_SUCCESS;
...
static int func_header_read(struct tris_channel *chan, const char *function, char *data, char *buf, size_t len) 
{
	...
	if (!IS_SIP_TECH(chan->tech)) {
		tris_log(LOG_WARNING, "This function can only be used on SIP channels.\n");
		tris_channel_unlock(chan);
		return -1;
	}
...
static struct tris_custom_function sip_header_function = {
	.name = "SIP_HEADER",
	.read = func_header_read,
};

/*! \brief  Dial plan function to check if domain is local */
static int func_check_sipdomain(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "CHECKSIPDOMAIN requires an argument - A domain name\n");
		return -1;
	}
...
static struct tris_custom_function checksipdomain_function = {
	.name = "CHECKSIPDOMAIN",
	.read = func_check_sipdomain,
};

/*! \brief  ${SIPPEER()} Dialplan function - reads peer data */
static int function_sippeer(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct sip_peer *peer;
	char *colname;
	...
	if ((colname = strchr(data, ':'))) {	/*! \todo Will be deprecated after 1.4 */
		...
		*colname++ = '\0';
		if (deprecation_warning++ % 10 == 0)
			tris_log(LOG_WARNING, "SIPPEER(): usage of ':' to separate arguments is deprecated.  Please use ',' instead.\n");
...
static struct tris_custom_function sippeer_function = {
	.name = "SIPPEER",
	.read = function_sippeer,
};

/*! \brief ${SIPCHANINFO()} Dialplan function - reads sip channel data */
static int function_sipchaninfo_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct sip_pvt *p;
	...
	*buf = 0;
	...
	if (deprecated++ % 20 == 0) {
		/* Deprecated in 1.6.1 */
		tris_log(LOG_WARNING, "SIPCHANINFO() is deprecated.  Please transition to using CHANNEL().\n");
	}
...
static struct tris_custom_function sipchaninfo_function = {
	.name = "SIPCHANINFO",
...
static void parse_moved_contact(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (tris_test_flag(&p->flags[0], SIP_PROMISCREDIR)) {
		char *host = NULL;
...
static void check_pendings(struct sip_pvt *p)
{
	if (tris_test_flag(&p->flags[0], SIP_PENDINGBYE)) {
		...
		tris_clear_flag(&p->flags[0], SIP_PENDINGBYE);	
		sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
	} else if (tris_test_flag(&p->flags[0], SIP_NEEDREINVITE)) {
...
static void handle_response_invite(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
	int outgoing = tris_test_flag(&p->flags[0], SIP_OUTGOING);
	int res = 0;
	int xmitres = 0;
	...
	if (reinvite)
		tris_debug(4, "SIP response %d to RE-invite on %s call %s\n", resp, outgoing ? "outgoing" : "incoming", p->callid);
	else
		tris_debug(4, "SIP response %d to standard invite\n", resp);
	...
	*/
	...
	case 408: /* Request timeout */
		tris_log(LOG_WARNING, "Re-invite to non-existing call leg on other UA. SIP dialog '%s'. Giving up.\n", p->callid);
		xmitres = transmit_request(p, SIP_ACK, seqno, XMIT_UNRELIABLE, FALSE);
		if (p->owner)
	...
	case 428: /* Use identity header - rfc 4474 - not supported by Trismedia yet */
		xmitres = transmit_request(p, SIP_ACK, seqno, XMIT_UNRELIABLE, FALSE);
		append_history(p, "Identity", "SIP identity is required. Not supported by Trismedia.");
		tris_log(LOG_WARNING, "SIP identity required by proxy. SIP dialog '%s'. Giving up.\n", p->callid);
		if (p->owner)
			tris_queue_control(p->owner, TRIS_CONTROL_CONGESTION);
		break;
	...
	case 491: /* Pending */
		...
		if (p->owner && !req->ignore) {
			if (p->owner->_state != TRIS_STATE_UP) {
				...
				p->waitid = tris_sched_add(sched, wait, sip_reinvite_retry, dialog_ref(p, "passing dialog ptr into sched structure based on waitid for sip_reinvite_retry."));
				tris_log(LOG_WARNING, "just did sched_add waitid(%d) for sip_reinvite_retry for dialog %s in handle_response_invite\n", p->waitid, p->callid);
...
static void handle_response_notify(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
	...
	case 200:   /* Notify accepted */
		...
		if (p->owner) {
			...
			if (tris_test_flag(&p->flags[1], SIP_PAGE2_STATECHANGEQUEUE)) {
				/* Ready to send the next state we have on queue */
				tris_clear_flag(&p->flags[1], SIP_PAGE2_STATECHANGEQUEUE);
...
static void handle_response_subscribe(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
	...
	case 200: /* Subscription accepted */
		...
		p->mwi->subscribed = 1;
		if ((p->mwi->resub = tris_sched_add(sched, mwi_expiry * 1000, sip_subscribe_mwi_do, ASTOBJ_REF(p->mwi))) < 0) {
...
static void handle_response_refer(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
	...
	case 501:   /* Method not implemented */
		...
		tris_log(LOG_NOTICE, "SIP transfer to %s failed, call miserably fails. \n", p->refer->refer_to);
		pvt_set_needdestroy(p, "received 500/501 response");
		p->refer->status = REFER_FAILED;
		break;
	case 603:   /* Transfer declined */
		tris_log(LOG_NOTICE, "SIP transfer to %s declined, call miserably fails. \n", p->refer->refer_to);
		p->refer->status = REFER_FAILED;
...
static int handle_response_register(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
	...
	case 403:	/* Forbidden */
		...
		break;
	case 404:	/* Not found */
		tris_log(LOG_WARNING, "Got 404 Not found on SIP register to service %s@%s, giving up\n", p->registry->username, p->registry->hostname);
		pvt_set_needdestroy(p, "received 404 response");
		if (r->call)
	...
	case 200:	/* 200 OK */
		...
		TRIS_SCHED_REPLACE_UNREF(r->expire, sched, expires_ms, sip_reregister, r, 
...
static void handle_response_peerpoke(struct sip_pvt *p, int resp, struct sip_request *req)
{
	...
	if (statechanged) {
		...
		tris_devstate_changed(TRIS_DEVICE_UNKNOWN, "SIP/%s", peer->name);
		if (sip_cfg.peer_rtupdate) {
			tris_update_realtime(tris_check_realtime("sipregs") ? "sipregs" : "sippeers", "name", peer->name, "lastms", str_lastms, SENTINEL);
		}
...
static void handle_response(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
	...
	*/
	...
	if (p->relatedpeer && p->method == SIP_OPTIONS) {
	...
	} else if (tris_test_flag(&p->flags[0], SIP_OUTGOING)) {
		switch(resp) {
		case 100:	/* 100 Trying */
		...
		case 200:	/* 200 OK */
			...
			if (sipmethod == SIP_MESSAGE || sipmethod == SIP_INFO) {
				...
				pvt_set_needdestroy(p, "received 200 response");
				tris_clear_flag(&p->flags[1], SIP_PAGE2_DIALOG_ESTABLISHED);
			}
			break;
		case 202:   /* Transfer accepted */
		...
		default:
			if ((resp >= 300) && (resp < 700)) {
				/* Fatal response */
				if ((resp != 487))
					tris_verb(3, "Got SIP response %d \"%s\" back from %s\n", resp, rest, tris_inet_ntoa(p->sa.sin_addr));
		...
		if (req->debug)
			tris_verbose("SIP Response message for INCOMING dialog %s arrived\n", msg);
		...
		default:	/* Errors without handlers */
			...
			if ((resp >= 300) && (resp < 700)) {
				if ((resp != 487))
					tris_verb(3, "Incoming call: Got SIP response %d \"%s\" back from %s\n", resp, rest, tris_inet_ntoa(p->sa.sin_addr));
				switch(resp) {
				case 415: /* Unsupported media type */
				case 488: /* Not acceptable here - codec error */
				case 603: /* Decline */
...
static void *sip_park_thread(void *stuff)
{
	...
	tris_debug(4, "SIP Park: Transferer channel %s, Transferee %s\n", transferer->name, transferee->name);
	...
	if (!res)	{
		...
		tris_debug(1, "SIP Call parked on extension '%d'\n", ext);
	} else {
		...
		tris_debug(1, "SIP Call parked failed \n");
		/* Do not hangup call */
	}
	if (d->req.data)
		tris_free(d->req.data);
...
static int sip_park(struct tris_channel *chan1, struct tris_channel *chan2, struct sip_request *req, int seqno)
{
	...
	transferer = tris_channel_alloc(0, TRIS_STATE_DOWN, 0, 0, chan2->accountcode, chan2->exten, chan2->context, chan2->amaflags, "SIPPeer/%s", chan2->name);
	...
	if ((d = tris_calloc(1, sizeof(*d)))) {
		...
		d->seqno = seqno;
		if (tris_pthread_create_detached_background(&th, NULL, sip_park_thread, d) < 0) {
...
static int attempt_transfer(struct sip_dual *transferer, struct sip_dual *target)
{
	...
	if (transferer->chan2) { /* We have a bridge on the transferer's channel */
		...
		tris_debug(3, "SIP transfer: Four channels to handle\n");
		...
		tris_debug(3, "SIP transfer: Three channels to handle\n");
	}
	...
	if (peera && peerb && peerc && (peerb != peerc)) {
		...
		tris_debug(4, "SIP transfer: trying to masquerade %s into %s\n", peerc->name, peerb->name);
		if (tris_channel_masquerade(peerb, peerc)) {
			tris_log(LOG_WARNING, "Failed to masquerade %s into %s\n", peerb->name, peerc->name);
			res = -1;
		} else
			tris_debug(4, "SIP transfer: Succeeded to masquerade channels.\n");
		return res;
	} else {
		tris_log(LOG_NOTICE, "SIP Transfer attempted with no appropriate bridged calls to transfer\n");
		if (transferer->chan1)
...
static int handle_request_notify(struct sip_pvt *p, struct sip_request *req, struct sockaddr_in *sin, int seqno, char *e)
{
	...
	if (!strcmp(event, "refer")) {
		...
		*/
		tris_debug(3, "* SIP Transfer NOTIFY Attachment: \n---%s\n---\n", buf);
		...
		*sep++ = '\0';			/* Response string */
		...
		if (!tris_strlen_zero(c)) {
			...
			if ((event = tris_event_new(TRIS_EVENT_MWI,
						   ...
						   TRIS_EVENT_IE_CONTEXT, TRIS_EVENT_IE_PLTYPE_STR, "SIP_Remote",
...
static int handle_invite_replaces(struct sip_pvt *p, struct sip_request *req, int debug, int seqno, struct sockaddr_in *sin, int *nounlock)
{
	...
	if (targetcall && targetcall->_state == TRIS_STATE_RINGING)
		tris_debug(4, "SIP transfer: Target channel is in ringing state\n");

	if (targetcall) 
		tris_debug(4, "SIP transfer: Invite Replace incoming channel should bridge to channel %s while hanging up channel %s\n", targetcall->name, replacecall->name); 
	else
		tris_debug(4, "SIP transfer: Invite Replace incoming channel should replace and hang up channel %s (one call leg)\n", replacecall->name); 
	...
	*/
	...
	if (! earlyreplace && ! oneleggedreplace )
		tris_set_flag(&p->refer->refer_call->flags[0], SIP_DEFER_BYE_ON_TRANSFER);	/* Delay hangup */
...
static int do_magic_pickup(struct tris_channel *channel, const char *extension, const char *context)
{
	...
	tris_str_set(&str, 0, "%s@%s", extension, sip_cfg.notifycid == IGNORE_CONTEXT ? "PICKUPMARK" : context);
...
static int handle_request_invite(struct sip_pvt *p, struct sip_request *req, int debug, int seqno, struct sockaddr_in *sin, int *recount, char *e, int *nounlock)
{
	...
	if (!tris_strlen_zero(required)) {
		...
		if (required_profile && !(required_profile & (SIP_OPT_REPLACES | SIP_OPT_TIMER))) {
			...
			tris_log(LOG_WARNING, "Received SIP INVITE with unsupported required extension: %s\n", required);
			p->invitestate = INV_COMPLETED;
			if (!p->lastinvite)
				sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
			res = -1;
	...
	if (tris_test_flag(&p->flags[0], SIP_OUTGOING) && p->owner && (p->invitestate != INV_TERMINATED && p->invitestate != INV_CONFIRMED)) {
	...
	if (!req->ignore && p->pendinginvite) {
		if (!tris_test_flag(&p->flags[0], SIP_OUTGOING) && (p->invitestate == INV_COMPLETED || p->invitestate == INV_TERMINATED)) {
	...
	if (!tris_strlen_zero(p_replaces)) {
		...
		*/
		...
		if (!error && tris_strlen_zero(pickup.exten) && (p->refer->refer_call = get_sip_pvt_byid_locked(replace_id, totag, fromtag)) == NULL) {
	...
	if (!req->ignore) {
		...
		if (!p->owner) {	/* Not a re-invite */
			...
			tris_clear_flag(&p->flags[0], SIP_OUTGOING);	/* This is now an inbound dialog */
	...
	if (!p->lastinvite && !req->ignore && !p->owner) {
		...
		if (tris_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT) && !p->udptl && (p->udptl = tris_udptl_new_with_bindaddr(sched, io, 0, bindaddr.sin_addr))) {
			p->t38_maxdatagram = global_t38_maxdatagram;
			set_t38_capabilities(p);
		}
		...
		if (find_sdp(req)) {
			if (process_sdp(p, req, SDP_T38_INITIATE)) {
				...
				p->invitestate = INV_COMPLETED;
				sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				tris_debug(1, "No compatible codecs for this SIP call.\n");
				res = -1;
				goto request_invite_cleanup;
			}
			if (tris_test_flag(&p->flags[1], SIP_PAGE2_CONSTANT_SSRC)) {
				if (p->rtp) {
		...
		tris_debug(1, "Checking SIP call limits for device %s\n", p->username);
		...
		if (!replace_id && gotdest) {	/* No matching extension found */
			if (gotdest == 1 && tris_test_flag(&p->flags[1], SIP_PAGE2_ALLOWOVERLAP))
			...
			c = sip_new(p, TRIS_STATE_DOWN, S_OR(p->peername, NULL));
			*recount = 1;
		...
		if (sipdebug) {
			if (!req->ignore)
				tris_debug(2, "Got a SIP re-invite for call %s\n", p->callid);
			else
				tris_debug(2, "Got a SIP re-transmit of INVITE for call %s\n", p->callid);
		}
		if (!req->ignore)
			reinvite = 1;
		c = p->owner;
	...
	if (p->sipoptions & SIP_OPT_TIMER) {
		...
		case SESSION_TIMER_MODE_REFUSE:
			if (p->reqsipoptions & SIP_OPT_TIMER) {
				...
				tris_log(LOG_WARNING, "Received SIP INVITE with supported but disabled option: %s\n", required);
				p->invitestate = INV_COMPLETED;
				if (!p->lastinvite) {
					sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				}
	...
	if (c) {	/* We have a call  -either a new call or an old one (RE-INVITE) */
		...
		errorinfo = get_header(req, "Conf-Name");
		pbx_builtin_setvar_helper(c, "Conf-Name", errorinfo);
		...
		if (c_state != TRIS_STATE_UP && reinvite &&
			 ...
			 * on the SIP socket prior to an application calling tris_read on this channel
		...
		case TRIS_STATE_UP:
			...
			if (p->t38.state == T38_PEER_REINVITE) {
				p->t38id = tris_sched_add(sched, 5000, sip_t38_abort, dialog_ref(p, "passing dialog ptr into sched structure based on t38id for sip_t38_abort."));
			} else if (p->t38.state == T38_ENABLED) {
				tris_set_flag(&p->flags[1], SIP_PAGE2_DIALOG_ESTABLISHED);
		...
		if (p && (p->autokillid == -1)) {
			...
			if (!p->jointcapability)
				...
				tris_log(LOG_NOTICE, "Unable to create/find SIP channel for this INVITE\n");
				msg = "503 Unavailable";
			}
...
static int local_attended_transfer(struct sip_pvt *transferer, struct sip_dual *current, struct sip_request *req, int seqno)
{
	struct sip_dual target;		/* Chan 1: Call from tranferer to Trismedia */
	...
	if (!(targetcall_pvt = get_sip_pvt_byid_locked(transferer->refer->replaces_callid, transferer->refer->replaces_callid_totag, 
		...
		if (transferer->refer->localtransfer) {
			...
			tris_clear_flag(&transferer->flags[0], SIP_GOTREFER);	
			transferer->refer->status = REFER_FAILED;
			return -1;
		}
		/* Fall through for remote transfers that we did not find locally */
		tris_debug(3, "SIP attended transfer: Not our call - generating INVITE with replaces\n");
		return 0;
	}
	...
	if (!targetcall_pvt->owner) {	/* No active channel */
		tris_debug(4, "SIP attended transfer: Error: No owner of target call\n");
		/* Cancel transfer */
	...
	if (!target.chan2 || !(target.chan2->_state == TRIS_STATE_UP || target.chan2->_state == TRIS_STATE_RINGING) ) {
		...
		if (target.chan2) 
			tris_debug(4, "SIP attended transfer: Error: Wrong state of target call: %s\n", tris_state2str(target.chan2->_state));
		else if (target.chan1->_state != TRIS_STATE_RING)
			tris_debug(4, "SIP attended transfer: Error: No target channel\n");
		else
			tris_debug(4, "SIP attended transfer: Attempting transfer in ringing state\n");
	}

	/* Transfer */
	if (sipdebug) {
		if (current->chan2)	/* We have two bridges */
			tris_debug(4, "SIP attended transfer: trying to bridge %s and %s\n", target.chan1->name, current->chan2->name);
		else			/* One bridge, propably transfer of IVR/voicemail etc */
			tris_debug(4, "SIP attended transfer: trying to make %s take over (masq) %s\n", target.chan1->name, current->chan1->name);
	}

	tris_set_flag(&transferer->flags[0], SIP_DEFER_BYE_ON_TRANSFER);	/* Delay hangup */
	...
	if (res) {
		...
		tris_clear_flag(&transferer->flags[0], SIP_DEFER_BYE_ON_TRANSFER);
	} else {
		/* Transfer succeeded! */
		...
		if (targetcall_pvt->owner) {
			tris_debug(1, "SIP attended transfer: Unlocking channel %s\n", targetcall_pvt->owner->name);
			tris_channel_unlock(targetcall_pvt->owner);
		}
	}
	if (targetcall_pvt)
...
static int handle_request_refer(struct sip_pvt *p, struct sip_request *req, int debug, int seqno, int *nounlock)
{
	struct sip_dual current;	/* Chan1: Call between trismedia and transferer */
	...
	if (req->debug)
		tris_verbose("Call %s got a SIP call transfer from %s: (REFER)!\n", p->callid, tris_test_flag(&p->flags[0], SIP_OUTGOING) ? "callee" : "caller");
	...
	if (!req->ignore && tris_test_flag(&p->flags[0], SIP_GOTREFER)) {
	...
	if (res != 0) {
		...
		case -2:	/* Syntax error */
			...
			if (req->debug)
				tris_debug(1, "SIP transfer to black hole can't be handled (no refer-to: )\n");
			break;
		case -3:
			transmit_response(p, "603 Declined (Non sip: uri)", req);
			append_history(p, "Xfer", "Refer failed. Non SIP uri");
			if (req->debug)
				tris_debug(1, "SIP transfer to non-SIP uri denied\n");
			break;
		default:
			...
			transmit_notify_with_sipfrag(p, seqno, "404 Not found", TRUE);
			tris_clear_flag(&p->flags[0], SIP_GOTREFER);	
			if (req->debug)
				tris_debug(1, "SIP transfer to bad extension: %s\n", p->refer->refer_to);
			break;
		} 
		return 0;
	}
	...
	if (sip_cfg.allow_external_domains && check_sip_domain(p->refer->refer_to_domain, NULL, 0)) {
		...
		if (sipdebug)
			tris_debug(3, "This SIP transfer is local : %s\n", p->refer->refer_to_domain);
	} else if (TRIS_LIST_EMPTY(&domain_list) || check_sip_domain(p->refer->refer_to_domain, NULL, 0)) {
		/* This PBX doesn't bother with SIP domains or domain is local, so this transfer is local */
		p->refer->localtransfer = 1;
	} else if (sipdebug)
			tris_debug(3, "This SIP transfer is to a remote SIP extension (remote domain %s)\n", p->refer->refer_to_domain);
	...
	*/
	...
	if (sipdebug)
		tris_debug(3, "SIP %s transfer: Transferer channel %s, transferee channel %s\n", p->refer->attendedtransfer ? "attended" : "blind", current.chan1->name, current.chan2 ? current.chan2->name : "<none>");
	...
	/*if (!current.chan2 && !p->refer->attendedtransfer) {*/
		...
		/*if (sipdebug)
			tris_debug(3, "Refused SIP transfer on non-bridged channel.\n");
		p->refer->status = REFER_FAILED;
	...
	if (current.chan2) {
		if (sipdebug)
			tris_debug(4, "Got SIP transfer, applying to bridged peer '%s'\n", current.chan2->name);

		tris_queue_control(current.chan1, TRIS_CONTROL_UNHOLD);
	}

	tris_set_flag(&p->flags[0], SIP_GOTREFER);
	...
	if (p->refer->attendedtransfer) {
		...
		/* Fall through for remote transfers that we did not find locally */
		if (sipdebug)
			tris_debug(4, "SIP attended transfer: Still not our call - generating INVITE with replaces\n");
	...
	if (p->refer->localtransfer && !strcmp(p->refer->refer_to, tris_parking_ext())) {
		...
		*nounlock = 1;
		...
		manager_event(EVENT_FLAG_CALL, "Transfer", "TransferMethod: SIP\r\nTransferType: Blind\r\nChannel: %s\r\nUniqueid: %s\r\nSIP-Callid: %s\r\nTargetChannel: %s\r\nTargetUniqueid: %s\r\nTransferExten: %s\r\nTransfer2Parking: Yes\r\n",
			...
			p->refer->refer_to);
		if (sipdebug)
			tris_debug(4, "SIP transfer to parking: trying to park %s. Parked by %s\n", current.chan2->name, current.chan1->name);
	...
	*/
	...
	/*tris_set_flag(&p->flags[0], SIP_DEFER_BYE_ON_TRANSFER);*/	/* Delay hangup */
	...
	if (!res) {
		...
		tris_clear_flag(&p->flags[0], SIP_DEFER_BYE_ON_TRANSFER);	/* Don't delay hangup */
...
static int acf_channel_read(struct tris_channel *chan, const char *funcname, char *preparse, char *buf, size_t buflen)
{
	...
	/* Sanity check */
	if (!IS_SIP_TECH(chan->tech)) {
		tris_log(LOG_ERROR, "Cannot call %s on a non-SIP channel\n", funcname);
		return 0;
	}
...
static int handle_request_bye(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (p->pendinginvite && !tris_test_flag(&p->flags[0], SIP_OUTGOING) && !req->ignore) {
		transmit_response_reliable(p, "487 Request Terminated", &p->initreq);
	}
	...
	if (p->do_history || p->owner) {
		...
		if (p->drtp) {
			videoqos = tris_rtp_get_quality(p->drtp, NULL, RTPQOS_SUMMARY);
			if (p->do_history)
				append_history(p, "RTCPdesktop", "Quality:%s", videoqos);
			if (p->owner)
				pbx_builtin_setvar_helper(p->owner, "RTPDESKTOPQOS", videoqos);
		}
...
static int handle_request_subscribe(struct sip_pvt *p, struct sip_request *req, struct sockaddr_in *sin, int seqno, char *e)
{
	...
	*/
	...
	if (tris_strlen_zero(eventheader)) {
		transmit_response(p, "489 Bad Event", req);
		tris_debug(2, "Received SIP subscribe for unknown event package: <none>\n");
	...
	if (!tris_test_flag(&p->flags[1], SIP_PAGE2_ALLOWSUBSCRIBE)) {
		transmit_response(p, "403 Forbidden (policy)", req);
	...
	if (!strcmp(event, "presence") || !strcmp(event, "dialog")) { /* Presence, RFC 3842 */
		...
		if (!tris_strlen_zero(acceptheader) && strcmp(acceptheader, "application/simple-message-summary")) {
			...
			tris_debug(2, "Received SIP mailbox subscription for unknown format: %s\n", acceptheader);
		...
		*/
		if (!authpeer || TRIS_LIST_EMPTY(&authpeer->mailboxes)) {
			...
			pvt_set_needdestroy(p, "received 404 response");
			tris_log(LOG_NOTICE, "Received SIP subscribe for peer without mailbox: %s\n", authpeer->name);
			if (authpeer)
		...
		if (tris_test_flag(&authpeer->flags[1], SIP_PAGE2_SUBSCRIBEMWIONLY)) {
		...
		transmit_response(p, "489 Bad Event", req);
		tris_debug(2, "Received SIP subscribe for unknown event package: %s\n", event);
...
static int handle_incoming(struct sip_pvt *p, struct sip_request *req, struct sockaddr_in *sin, int *recount, int *nounlock)
{
	...
	if (tris_strlen_zero(cmd) || tris_strlen_zero(cseq)) {
		tris_log(LOG_ERROR, "Missing Cseq. Dropping this SIP message, it's incomplete.\n");
		error = 1;
	}
	...
	if (req->method == SIP_RESPONSE) {	/* Response to our request */
		...
		if (respid <= 0) {
			tris_log(LOG_WARNING, "Invalid SIP response code: '%d'\n", respid);
			return 0;
		}
	...
	tris_debug(4, "**** Received %s (%d) - Command in SIP %s\n", sip_methods[p->method].text, sip_methods[p->method].id, cmd); 
	...
	if (p->icseq && (p->icseq > seqno) ) {
		if (p->pendinginvite && seqno == p->pendinginvite && (req->method == SIP_ACK || req->method == SIP_CANCEL)) {
			...
			tris_debug(1, "Ignoring too old SIP packet packet %d (expecting >= %d)\n", seqno, p->icseq);
		...
		req->ignore = 1;
		tris_debug(3, "Ignoring SIP message because of retransmit (%s Seqno %d, ours %d)\n", sip_methods[p->method].text, p->icseq, seqno);
	}
	...
	if (memcmp(&sin->sin_addr, &internip.sin_addr, 4) || sin->sin_port != htons(STANDARD_SIP_PORT)) {
		transmit_response(p, "403 Forbidden", req);
		sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
		return -1;
	}
	...
	default:
		...
		tris_log(LOG_NOTICE, "Unknown SIP command '%s' from '%s'\n", 
...
static void process_request_queue(struct sip_pvt *p, int *recount, int *nounlock)
{
	...
	while ((req = TRIS_LIST_REMOVE_HEAD(&p->request_queue, next))) {
		if (handle_incoming(p, req, &p->recv, recount, nounlock) == -1) {
			/* Request failed */
			if (option_debug) {
				tris_log(LOG_DEBUG, "SIP message could not be handled, bad request: %-70.70s\n", p->callid[0] ? p->callid : "<no callid>");
			}
		}
		tris_free(req);
	}
...
static int sipsock_read(int *id, int fd, short events, void *ignore)
{
	...
	if (res < 0) {
		...
		if (errno == EAGAIN)
			tris_log(LOG_NOTICE, "SIP: Received packet with bad UDP checksum\n");
		else 
#endif
...
static int handle_request_do(struct sip_request *req, struct sockaddr_in *sin) 
{
	...
	for (lockretry = 10; lockretry > 0; lockretry--) {
		...
		if (p == NULL) {
			tris_debug(1, "Invalid SIP message - rejected , no callid, len %d\n", req->len);
	...
	if (!lockretry) {
		...
		tris_log(LOG_ERROR, "SIP transaction failed: %s \n", p->callid);
		if (req->method != SIP_ACK)
			transmit_response(p, "503 Server error", req);	/* We must respond according to RFC 3261 sec 12.2 */
	...
	*/
	...
	if (handle_incoming(p, req, sin, &recount, &nounlock) == -1) {
		/* Request failed */
		tris_debug(1, "SIP message could not be handled, bad request: %-70.70s\n", p->callid[0] ? p->callid : "<no callid>");
	}
 ...
 */
static struct tris_tcptls_session_instance *sip_tcp_locate(struct sockaddr_in *s)
{
	struct sip_threadinfo *th;
...
static int sip_prepare_socket(struct sip_pvt *p) 
{
	...
	if (tris_pthread_create_background(&ca->master, NULL, sip_tcp_worker_fn, s->tcptls_session)) {
...
static int sip_send_mwi_to_peer(struct sip_peer *peer, const struct tris_event *event, int cache_only)
{
	...
	if (tris_test_flag((&peer->flags[1]), SIP_PAGE2_SUBSCRIBEMWIONLY) && !peer->mwipvt)
		return 0;
...
static void check_rtp_timeout(struct sip_pvt *dialog, time_t t)
{
	...
	*/
	...
	if (dialog->lastrtprx && (tris_rtp_get_rtptimeout(dialog->rtp) || tris_rtp_get_rtpholdtimeout(dialog->rtp)) &&
		...
		if (!tris_test_flag(&dialog->flags[1], SIP_PAGE2_CALL_ONHOLD) || (tris_rtp_get_rtpholdtimeout(dialog->rtp) &&
...
static void *do_monitor(void *data)
{
	...
	if (sipsock > -1) 
		sipsock_read_id = tris_io_add(io, sipsock, sipsock_read, TRIS_IO_IN, NULL);

	/* From here on out, we die whenever asked */
	for(;;) {
		/* Check for a reload request */
		tris_mutex_lock(&sip_reload_lock);
		reloading = sip_reloading;
		sip_reloading = FALSE;
		tris_mutex_unlock(&sip_reload_lock);
		if (reloading) {
			tris_verb(1, "Reloading SIP\n");
			sip_do_reload(sip_reloadreason);

			/* Change the I/O fd of our UDP socket */
			if (sipsock > -1) {
				if (sipsock_read_id)
					sipsock_read_id = tris_io_change(io, sipsock_read_id, sipsock, NULL, 0, NULL);
				else
					sipsock_read_id = tris_io_add(io, sipsock, sipsock_read, TRIS_IO_IN, NULL);
			} else if (sipsock_read_id) {
				tris_io_remove(io, sipsock_read_id);
...
/*! \brief The SPC monitoring thread 
\note	This thread monitors all the SIP sessions and peers that needs notification of mwi
	(and thus do not have a separate thread) indefinitely 
*/
static void *spc_monitor(void *data)
{
	struct tris_channel* spc;
	struct sip_pvt* p;
	int portno = spc_port;
	struct sockaddr_in sin;
	char host[256];
	tris_copy_string(host, spc_host, sizeof(host));
	struct tris_hostent audiohp;
	struct hostent* hp;

	hp = tris_gethostbyname(host, &audiohp);
	if (!hp) {
		tris_verbose("Can't get hp\n");
		return NULL;
	}

	p = sip_alloc("abcd", 0, 0, 0, 0);
	...
	if (p->rtp) {
		if (portno > 0) {
			...
			/* We are now ready to change the sip session and p->rtp and p->vrtp with the offered codecs, since 																								   
			   they are acceptable */
			tris_rtp_pt_clear(p->rtp);
			if (!strncmp(spc_codec, "ulaw", 4)) {
				p->jointcapability = TRIS_FORMAT_ULAW;				/* Our joint codec profile for this call */
				p->peercapability = TRIS_FORMAT_ULAW;					/* The other sides capability in latest offer */
				tris_rtp_set_m_type(p->rtp, 0);
			} else if (!strncmp(spc_codec, "alaw", 4)) {
				p->jointcapability = TRIS_FORMAT_ALAW;				/* Our joint codec profile for this call */
				p->peercapability = TRIS_FORMAT_ALAW;					/* The other sides capability in latest offer */
				tris_rtp_set_m_type(p->rtp, 8);
			}

			p->jointnoncodeccapability = TRIS_RTP_DTMF; 	/* DTMF capabilities */
			if (tris_test_flag(&p->flags[0], SIP_DTMF) == SIP_DTMF_AUTO) {
				tris_clear_flag(&p->flags[0], SIP_DTMF);
				if (p->jointnoncodeccapability & TRIS_RTP_DTMF) {
					/* XXX Would it be reasonable to drop the DSP at this point? XXX */
					tris_set_flag(&p->flags[0], SIP_DTMF_RFC2833);
					/* Since RFC2833 is now negotiated we need to change some properties of the RTP stream */
					tris_rtp_setdtmf(p->rtp, 1);
	...
	spc = sip_new(p, TRIS_STATE_DOWN, "spc");
	
	/* From here on out, we die whenever asked */
	for(;;) {
		if (!tris_streamfile(spc, spc_file, spc->language))
			tris_waitstream(spc, "");
		pthread_testcancel();
	}
...
{
	/* If we're supposed to be stopped -- stay stopped */
	if (spc_thread == TRIS_PTHREADT_STOP)
		return 0;
	tris_mutex_lock(&spclock);
	if (spc_thread == pthread_self()) {
		tris_mutex_unlock(&spclock);
		tris_log(LOG_WARNING, "Cannot kill myself\n");
		return -1;
	}
	if (spc_thread != TRIS_PTHREADT_NULL) {
		/* Wake up the thread */
		pthread_kill(spc_thread, SIGURG);
	} else {
		/* Start a new monitor */
		if (tris_pthread_create_background(&spc_thread, NULL, spc_monitor, NULL) < 0) {
			tris_mutex_unlock(&spclock);
			tris_log(LOG_ERROR, "Unable to start monitor thread.\n");
			return -1;
		}
...
static int sip_poke_noanswer(const void *data)
{
	...
	if (peer->lastms > -1) {
		...
		if (sip_cfg.peer_rtupdate) {
			tris_update_realtime(tris_check_realtime("sipregs") ? "sipregs" : "sippeers", "name", peer->name, "lastms", "-1", SENTINEL);
		}
...
static int sip_poke_peer(struct sip_peer *peer, int force)
{
	...
	tris_copy_string(p->username, "__VOCAL_DATA_SHOULD_READ_THE_SIP_SPEC__", sizeof(p->username));
	...
	if (xmitres == XMIT_ERROR) {
		...
		TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, peer->maxms * 2, sip_poke_noanswer, peer,
				unref_peer(_data, "removing poke peer ref"),
				unref_peer(peer, "removing poke peer ref"),
				ref_peer(peer, "adding poke peer ref"));
	}
...
static struct tris_channel *sip_request_call(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	...
	tris_debug(1, "Asked to create a SIP channel with formats: %s\n", tris_getformatname_multiple(tmp, sizeof(tmp), oldformat));
	...
	if (!(p = sip_alloc(NULL, NULL, 0, SIP_INVITE, NULL))) {
		tris_log(LOG_ERROR, "Unable to build sip pvt data for '%s' (Out of memory or socket error)\n", dest);
	...
	if (!(p->options = tris_calloc(1, sizeof(*p->options)))) {
		...
		/* sip_destroy(p); */
		tris_log(LOG_ERROR, "Unable to build option SIP data structure - Out of memory\n");
		*cause = TRIS_CAUSE_SWITCH_CONGESTION;
		return NULL;
	}
		...
		*trans++ = '\0';
		if (!strcasecmp(trans, "tcp"))
			...
			if (strcasecmp(trans, "udp"))
				tris_log(LOG_WARNING, "'%s' is not a valid transport option to Dial() for SIP calls, using udp by default.\n", trans);
			transport = SIP_TRANSPORT_UDP;
		}
	} else { /* use default */
		transport = SIP_TRANSPORT_UDP;
	...
	*/
	if (create_addr(p, host, NULL, 1)) {
		*cause = TRIS_CAUSE_UNREGISTERED;
		tris_debug(3, "Cant create SIP call - target device not registered\n");
		dialog_unlink_all(p, TRUE, TRUE);
	...
	sip_pvt_lock(p);
	tmpc = sip_new(p, TRIS_STATE_DOWN, host);	/* Place the call */
	if (sip_cfg.callevents)
		manager_event(EVENT_FLAG_SYSTEM, "ChannelUpdate",
			"Channel: %s\r\nChanneltype: %s\r\nSIPcallid: %s\r\nSIPfullcontact: %s\r\nPeername: %s\r\n",
	...
	if (!tris_false(value)) {
		...
		while ((word = strsep(&next, ","))) {
			if (!strcasecmp(word, "port"))
				tris_set_flag(&flags[0], SIP_INSECURE_PORT);
			else if (!strcasecmp(word, "invite"))
				tris_set_flag(&flags[0], SIP_INSECURE_INVITE);
			else
			      ...
			      int *maxdatagram)
{
	...
	if (!strcasecmp(v->name, "t38pt_udptl")) {
		...
		tris_set_flag(&mask[1], SIP_PAGE2_T38SUPPORT);

		while ((word = strsep(&next, ","))) {
			if (tris_true(word) || !strcasecmp(word, "fec")) {
				tris_clear_flag(&flags[1], SIP_PAGE2_T38SUPPORT);
				tris_set_flag(&flags[1], SIP_PAGE2_T38SUPPORT_UDPTL_FEC);
			} else if (!strcasecmp(word, "redundancy")) {
				tris_clear_flag(&flags[1], SIP_PAGE2_T38SUPPORT);
				tris_set_flag(&flags[1], SIP_PAGE2_T38SUPPORT_UDPTL_REDUNDANCY);
			} else if (!strcasecmp(word, "none")) {
				tris_clear_flag(&flags[1], SIP_PAGE2_T38SUPPORT);
				tris_set_flag(&flags[1], SIP_PAGE2_T38SUPPORT_UDPTL);
		...
		tris_set_flag(&mask[1], SIP_PAGE2_UDPTL_DESTINATION);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_UDPTL_DESTINATION);
	} else {
		res = 0;
	}
...
static int handle_common_options(struct tris_flags *flags, struct tris_flags *mask, struct tris_variable *v)
{
	...
	if (!strcasecmp(v->name, "trustrpid")) {
		tris_set_flag(&mask[0], SIP_TRUSTRPID);
		tris_set2_flag(&flags[0], tris_true(v->value), SIP_TRUSTRPID);
	} else if (!strcasecmp(v->name, "sendrpid")) {
		tris_set_flag(&mask[0], SIP_SENDRPID);
		tris_set2_flag(&flags[0], tris_true(v->value), SIP_SENDRPID);
	} else if (!strcasecmp(v->name, "g726nonstandard")) {
		tris_set_flag(&mask[0], SIP_G726_NONSTANDARD);
		tris_set2_flag(&flags[0], tris_true(v->value), SIP_G726_NONSTANDARD);
	} else if (!strcasecmp(v->name, "useclientcode")) {
		tris_set_flag(&mask[0], SIP_USECLIENTCODE);
		tris_set2_flag(&flags[0], tris_true(v->value), SIP_USECLIENTCODE);
	} else if (!strcasecmp(v->name, "dtmfmode")) {
		tris_set_flag(&mask[0], SIP_DTMF);
		tris_clear_flag(&flags[0], SIP_DTMF);
		if (!strcasecmp(v->value, "inband"))
			tris_set_flag(&flags[0], SIP_DTMF_INBAND);
		else if (!strcasecmp(v->value, "rfc2833"))
			tris_set_flag(&flags[0], SIP_DTMF_RFC2833);
		else if (!strcasecmp(v->value, "info"))
			tris_set_flag(&flags[0], SIP_DTMF_INFO);
		else if (!strcasecmp(v->value, "shortinfo"))
			tris_set_flag(&flags[0], SIP_DTMF_SHORTINFO);
		else if (!strcasecmp(v->value, "auto"))
			tris_set_flag(&flags[0], SIP_DTMF_AUTO);
		else {
			tris_log(LOG_WARNING, "Unknown dtmf mode '%s' on line %d, using rfc2833\n", v->value, v->lineno);
			tris_set_flag(&flags[0], SIP_DTMF_RFC2833);
		}
	} else if (!strcasecmp(v->name, "nat")) {
		tris_set_flag(&mask[0], SIP_NAT);
		tris_clear_flag(&flags[0], SIP_NAT);
		if (!strcasecmp(v->value, "never"))
			tris_set_flag(&flags[0], SIP_NAT_NEVER);
		else if (!strcasecmp(v->value, "route"))
			tris_set_flag(&flags[0], SIP_NAT_ROUTE);
		else if (tris_true(v->value))
			tris_set_flag(&flags[0], SIP_NAT_ALWAYS);
		else
			tris_set_flag(&flags[0], SIP_NAT_RFC3581);
	} else if (!strcasecmp(v->name, "directmedia") || !strcasecmp(v->name, "canreinvite")) {
		tris_set_flag(&mask[0], SIP_REINVITE);
		tris_clear_flag(&flags[0], SIP_REINVITE);
		if (tris_true(v->value)) {
			tris_set_flag(&flags[0], SIP_DIRECT_MEDIA | SIP_DIRECT_MEDIA_NAT);
			...
			while ((word = strsep(&next, ","))) {
				if (!strcasecmp(word, "update")) {
					tris_set_flag(&flags[0], SIP_REINVITE_UPDATE | SIP_DIRECT_MEDIA);
				} else if (!strcasecmp(word, "nonat")) {
					tris_set_flag(&flags[0], SIP_DIRECT_MEDIA);
					tris_clear_flag(&flags[0], SIP_DIRECT_MEDIA_NAT);
				} else {
		...
		tris_set_flag(&mask[0], SIP_INSECURE);
		tris_clear_flag(&flags[0], SIP_INSECURE);
		set_insecure_flags(&flags[0], v->value, v->lineno);	
	} else if (!strcasecmp(v->name, "progressinband")) {
		tris_set_flag(&mask[0], SIP_PROG_INBAND);
		tris_clear_flag(&flags[0], SIP_PROG_INBAND);
		if (tris_true(v->value))
			tris_set_flag(&flags[0], SIP_PROG_INBAND_YES);
		else if (strcasecmp(v->value, "never"))
			tris_set_flag(&flags[0], SIP_PROG_INBAND_NO);
	} else if (!strcasecmp(v->name, "promiscredir")) {
		tris_set_flag(&mask[0], SIP_PROMISCREDIR);
		tris_set2_flag(&flags[0], tris_true(v->value), SIP_PROMISCREDIR);
	} else if (!strcasecmp(v->name, "videosupport")) {
		if (!strcasecmp(v->value, "always")) {
			tris_set_flag(&mask[1], SIP_PAGE2_VIDEOSUPPORT_ALWAYS);
			tris_set_flag(&flags[1], SIP_PAGE2_VIDEOSUPPORT_ALWAYS);
		} else {
			tris_set_flag(&mask[1], SIP_PAGE2_VIDEOSUPPORT);
			tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_VIDEOSUPPORT);
		}
	} else if (!strcasecmp(v->name, "desktopsupport")) {
		tris_set_flag(&mask[1], SIP_PAGE2_DESKTOPSUPPORT);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_DESKTOPSUPPORT);
	} else if (!strcasecmp(v->name, "textsupport")) {
		tris_set_flag(&mask[1], SIP_PAGE2_TEXTSUPPORT);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_TEXTSUPPORT);
		res = 1;
	} else if (!strcasecmp(v->name, "allowoverlap")) {
		tris_set_flag(&mask[1], SIP_PAGE2_ALLOWOVERLAP);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_ALLOWOVERLAP);
	} else if (!strcasecmp(v->name, "allowsubscribe")) {
		tris_set_flag(&mask[1], SIP_PAGE2_ALLOWSUBSCRIBE);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_ALLOWSUBSCRIBE);
	} else if (!strcasecmp(v->name, "ignoresdpversion")) {
		tris_set_flag(&mask[1], SIP_PAGE2_IGNORESDPVERSION);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_IGNORESDPVERSION);
	} else if (!strcasecmp(v->name, "faxdetect")) {
		tris_set_flag(&mask[1], SIP_PAGE2_FAX_DETECT);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_FAX_DETECT);
	} else if (!strcasecmp(v->name, "rfc2833compensate")) {
		tris_set_flag(&mask[1], SIP_PAGE2_RFC2833_COMPENSATE);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_RFC2833_COMPENSATE);
	} else if (!strcasecmp(v->name, "buggymwi")) {
		tris_set_flag(&mask[1], SIP_PAGE2_BUGGY_MWI);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_BUGGY_MWI);
	} else if (!strcasecmp(v->name, "constantssrc")) {
		tris_set_flag(&mask[1], SIP_PAGE2_CONSTANT_SSRC);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_CONSTANT_SSRC);
	} else if (!strcasecmp(v->name, "playmoh")) {
		tris_set_flag(&mask[1], SIP_PAGE2_PLAYMOH);
		tris_set2_flag(&flags[1], tris_true(v->value), SIP_PAGE2_PLAYMOH);
	} else
		res = 0;
...
static int add_sip_domain(const char *domain, const enum domain_mode mode, const char *context)
{
	...
	TRIS_LIST_UNLOCK(&domain_list);
 	...
 	if (sipdebug)	
		tris_debug(1, "Added local SIP domain '%s'\n", domain);
...
static void set_peer_defaults(struct sip_peer *peer)
{
	...
	tris_copy_flags(&peer->flags[0], &global_flags[0], SIP_FLAGS_TO_COPY);
	tris_copy_flags(&peer->flags[1], &global_flags[1], SIP_PAGE2_FLAGS_TO_COPY);
	tris_string_field_set(peer, context, sip_cfg.default_context);
	tris_string_field_set(peer, subscribecontext, sip_cfg.default_subscribecontext);
...
static struct sip_peer *build_peer(const char *name, struct tris_variable *v, struct tris_variable *alt, int realtime, int devstate_only)
{
	...
	if (!realtime || tris_test_flag(&global_flags[1], SIP_PAGE2_RTCACHEFRIENDS)) {
	...
	for (; v || ((v = alt) && !(alt=NULL)); v = v->next) {
		if (!devstate_only) {
			...
			if (!strcasecmp(v->name, "transport") && !tris_strlen_zero(v->value)) {
				...
				tris_set2_flag(&peer->flags[0], tris_true(v->value), SIP_USEREQPHONE);
				...
				if (!strcasecmp(v->name, "username")) {
					if (deprecation_warning) {
						tris_log(LOG_NOTICE, "The 'username' field for sip peers has been deprecated in favor of the term 'defaultuser'\n");
				...
				tris_set2_flag(&peer->flags[1], tris_true(v->value), SIP_PAGE2_SUBSCRIBEMWIONLY);
				...
				tris_set2_flag(&peer->flags[1], tris_true(v->value), SIP_PAGE2_REGISTERTRYING);
		...
		if (realtime && !strcasecmp(v->name, "lastms")) {
			...
			if (!strcasecmp(v->value, "no")) {
				...
				tris_log(LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of sip.conf\n", peer->name, v->lineno);
				peer->maxms = 0;
			}
			if (realtime && !tris_test_flag(&global_flags[1], SIP_PAGE2_RTCACHEFRIENDS) && peer->maxms > 0) {
	...
	if (tris_str_strlen(fullcontact)) {
		...
		if (!tris_test_flag(&peer->flags[0], SIP_NAT_ROUTE) || !peer->addr.sin_addr.s_addr) {
	...
	if (srvlookup && peer->dnsmgr == NULL) {
		...
		if (tris_dnsmgr_lookup(_srvlookup, &peer->addr, &peer->dnsmgr, sip_cfg.srvlookup && !peer->portinuri ? transport : NULL)) {
	...
	if (tris_test_flag(&peer->flags[1], SIP_PAGE2_ALLOWSUBSCRIBE)) {
	 ...
	 * subscribe to it now. */
	if (!devstate_only && !tris_test_flag(&peer->flags[1], SIP_PAGE2_SUBSCRIBEMWIONLY) &&
...
static int reload_config(enum channelreloadreason reason)
{
	...
	struct tris_flags config_flags = { reason == CHANNEL_MODULE_LOAD ? 0 : tris_test_flag(&global_flags[1], SIP_PAGE2_RTCACHEFRIENDS) ? 0 : CONFIG_FLAG_FILEUNCHANGED };
	...
	run_start = time(0);
	tris_unload_realtime("sipregs");		
	tris_unload_realtime("sippeers");
	...
	if (reason != CHANNEL_MODULE_LOAD) {
		tris_debug(4, "--------------- SIP reload started\n");
	...
	tris_copy_string(sip_cfg.realm, S_OR(tris_config_TRIS_SYSTEM_NAME, DEFAULT_REALM), sizeof(sip_cfg.realm));
	...
	tris_set_flag(&global_flags[1], SIP_PAGE2_ALLOWSUBSCRIBE);	/* Default for all devices: TRUE */
	tris_set_flag(&global_flags[1], SIP_PAGE2_ALLOWOVERLAP);		/* Default for all devices: TRUE */
	...
	tris_copy_string(sip_cfg.default_context, DEFAULT_CONTEXT, sizeof(sip_cfg.default_context));
	...
	tris_set_flag(&global_flags[0], SIP_DTMF_RFC2833);			/*!< Default DTMF setting: RFC2833 */
	tris_set_flag(&global_flags[0], SIP_NAT_RFC3581);			/*!< NAT support if requested by device with rport */
	tris_set_flag(&global_flags[0], SIP_DIRECT_MEDIA);			/*!< Allow re-invites */
	...
	/* spc settings */
	enable_spc = 0;
	tris_copy_string(spc_host, "170.102.126.40", sizeof(spc_host));
	spc_port = 3456;
	tris_copy_string(spc_file, "voicemail/first_menu", sizeof(spc_file));
	tris_copy_string(spc_codec, "ulaw", sizeof(spc_codec));
	...
	tris_clear_flag(&global_flags[1], SIP_PAGE2_FAX_DETECT);
	tris_clear_flag(&global_flags[1], SIP_PAGE2_VIDEOSUPPORT | SIP_PAGE2_VIDEOSUPPORT_ALWAYS);
	tris_clear_flag(&global_flags[1], SIP_PAGE2_TEXTSUPPORT);
	tris_clear_flag(&global_flags[1], SIP_PAGE2_IGNORESDPVERSION);
	...
	for (v = tris_variable_browse(cfg, "general"); v; v = v->next) {
		...
		if (!strcasecmp(v->name, "context")) {
			tris_copy_string(sip_cfg.default_context, v->value, sizeof(sip_cfg.default_context));
		} else if (!strcasecmp(v->name, "enable_spc")) {
			enable_spc = tris_true(v->value) ? 1 : 0;
		} else if (!strcasecmp(v->name, "spc_host")) {
			tris_copy_string(spc_host, v->value, sizeof(spc_host));
		} else if (!strcasecmp(v->name, "spc_port")) {
			spc_port = atoi(v->value);
		} else if (!strcasecmp(v->name, "spc_file")) {
			tris_copy_string(spc_file, v->value, sizeof(spc_file));
		} else if (!strcasecmp(v->name, "spc_codec")) {
			tris_copy_string(spc_codec, v->value, sizeof(spc_codec));
		} else if (!strcasecmp(v->name, "subscribecontext")) {
			tris_copy_string(sip_cfg.default_subscribecontext, v->value, sizeof(sip_cfg.default_subscribecontext));
  		} else if (!strcasecmp(v->name, "callcounter")) {
			global_callcounter = tris_true(v->value) ? 1 : 0;
  		} else if (!strcasecmp(v->name, "allowguest")) {
			sip_cfg.allowguest = tris_true(v->value) ? 1 : 0;
		} else if (!strcasecmp(v->name, "realm")) {
			tris_copy_string(sip_cfg.realm, v->value, sizeof(sip_cfg.realm));
		} else if (!strcasecmp(v->name, "useragent")) {
			tris_copy_string(global_useragent, v->value, sizeof(global_useragent));
			tris_debug(1, "Setting SIP channel User-Agent Name to %s\n", global_useragent);
			...
			sip_cfg.allowtransfer = tris_true(v->value) ? TRANSFER_OPENFORALL : TRANSFER_CLOSED;
		} else if (!strcasecmp(v->name, "rtcachefriends")) {
			tris_set2_flag(&global_flags[1], tris_true(v->value), SIP_PAGE2_RTCACHEFRIENDS);	
		} else if (!strcasecmp(v->name, "rtsavesysname")) {
			sip_cfg.rtsave_sysname = tris_true(v->value);
		} else if (!strcasecmp(v->name, "rtupdate")) {
			sip_cfg.peer_rtupdate = tris_true(v->value);
		} else if (!strcasecmp(v->name, "ignoreregexpire")) {
			sip_cfg.ignore_regexpire = tris_true(v->value);
		} else if (!strcasecmp(v->name, "timert1")) {
			...
			sip_tcp_desc.local_address.sin_family = tris_false(v->value) ? 0 : AF_INET;
			...
			if (tris_parse_arg(v->value, PARSE_INADDR, &sip_tcp_desc.local_address))
			...
			if (tris_parse_arg(v->value, PARSE_INADDR, &sip_tls_desc.local_address))
				tris_log(LOG_WARNING, "Invalid %s '%s' at line %d of %s\n", v->name, v->value, v->lineno, config);
			...
			if (i > 0)
				...
				i = 0;
			tris_set2_flag(&global_flags[1], i || tris_true(v->value), SIP_PAGE2_RTAUTOCLEAR);
		} else if (!strcasecmp(v->name, "usereqphone")) {
			tris_set2_flag(&global_flags[0], tris_true(v->value), SIP_USEREQPHONE);	
			...
			sip_cfg.compactheaders = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifymimetype")) {
			tris_copy_string(default_notifymime, v->value, sizeof(default_notifymime));
		} else if (!strcasecmp(v->name, "directrtpsetup")) {
			sip_cfg.directrtpsetup = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifyringing")) {
			sip_cfg.notifyringing = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifyhold")) {
			sip_cfg.notifyhold = tris_true(v->value);
		} else if (!strcasecmp(v->name, "notifycid")) {
			if (!strcasecmp(v->value, "ignore-context")) {
				sip_cfg.notifycid = IGNORE_CONTEXT;
			} else {
				sip_cfg.notifycid = tris_true(v->value);
			}
		} else if (!strcasecmp(v->name, "alwaysauthreject")) {
			sip_cfg.alwaysauthreject = tris_true(v->value);
		} else if (!strcasecmp(v->name, "mohinterpret")) {
			...
			if (tris_strlen_zero(v->value)) {
				tris_log(LOG_WARNING, "no value given for outbound proxy on line %d of sip.conf.", v->lineno);
				continue;
			}
			...
			if ((tok = strtok(NULL, ","))) {
				sip_cfg.outboundproxy.force = !strncasecmp(tris_skip_blanks(tok), "force", 5);
			} else {
				sip_cfg.outboundproxy.force = FALSE;
			}

			if (tris_strlen_zero(proxyname)) {
				tris_log(LOG_WARNING, "you must specify a name for the outboundproxy on line %d of sip.conf.", v->lineno);
				sip_cfg.outboundproxy.name[0] = '\0';
				continue;
			}

			tris_copy_string(sip_cfg.outboundproxy.name, proxyname, sizeof(sip_cfg.outboundproxy.name));

			proxy_update(&sip_cfg.outboundproxy);
		} else if (!strcasecmp(v->name, "autocreatepeer")) {
			sip_cfg.autocreatepeer = tris_true(v->value);
		} else if (!strcasecmp(v->name, "match_auth_username")) {
			global_match_auth_username = tris_true(v->value);
		} else if (!strcasecmp(v->name, "srvlookup")) {
			sip_cfg.srvlookup = tris_true(v->value);
		} else if (!strcasecmp(v->name, "pedantic")) {
			sip_cfg.pedanticsipchecking = tris_true(v->value);
			...
			sip_cfg.allow_external_domains = tris_true(v->value);
		} else if (!strcasecmp(v->name, "autodomain")) {
			auto_sip_domains = tris_true(v->value);
			...
			if (tris_strlen_zero(domain))
				...
				add_sip_domain(tris_strip(domain), SIP_DOMAIN_CONFIG, cntx ? tris_strip(cntx) : "");
			...
			if (tris_str2tos(v->value, &global_tos_sip))
				tris_log(LOG_WARNING, "Invalid tos_sip value at line %d, refer to QoS documentation\n", v->lineno);
			...
			if (tris_str2cos(v->value, &global_cos_sip))
				tris_log(LOG_WARNING, "Invalid cos_sip value at line %d, refer to QoS documentation\n", v->lineno);
			...
			if (!strcasecmp(v->value, "no")) {
				...
				tris_log(LOG_WARNING, "Qualification default should be 'yes', 'no', or a number of milliseconds at line %d of sip.conf\n", v->lineno);
				default_qualify = 0;
			}
		} else if (!strcasecmp(v->name, "qualifyfreq")) {
			int i;
			...
			sip_cfg.callevents = tris_true(v->value);
			...
			sip_cfg.matchexterniplocally = tris_true(v->value);
		} else if (!strcasecmp(v->name, "constantssrc")) {
			tris_set2_flag(&global_flags[1], tris_true(v->value), SIP_PAGE2_CONSTANT_SSRC);
		} else if (!strcasecmp(v->name, "session-timers")) {
			int i = (int) str2stmode(v->value); 
			if (i < 0) {
				tris_log(LOG_WARNING, "Invalid session-timers '%s' at line %d of %s\n", v->value, v->lineno, config);
	...
	if (!sip_cfg.allow_external_domains && TRIS_LIST_EMPTY(&domain_list)) {
		tris_log(LOG_WARNING, "To disallow external domains, you need to configure local SIP domains.\n");
	...
	if (ucfg) {
		...
		genhassip = tris_true(tris_variable_retrieve(ucfg, "general", "hassip"));
		genregistersip = tris_true(tris_variable_retrieve(ucfg, "general", "registersip"));
		gen = tris_variable_browse(ucfg, "general");
		cat = tris_category_browse(ucfg, NULL);
		while (cat) {
			if (strcasecmp(cat, "general")) {
				hassip = tris_variable_retrieve(ucfg, cat, "hassip");
				registersip = tris_variable_retrieve(ucfg, cat, "registersip");
				if (tris_true(hassip) || (!hassip && genhassip)) {
				...
				if (tris_true(registersip) || (!registersip && genregistersip)) {
					char tmp[256];
	...
	if (tris_find_ourip(&internip.sin_addr, bindaddr)) {
		tris_log(LOG_WARNING, "Unable to get own IP address, SIP disabled\n");
		tris_config_destroy(cfg);
		return 0;
	}
	tris_mutex_lock(&netlock);
	...
	if (sipsock < 0) {
		...
		if (sipsock < 0) {
			tris_log(LOG_WARNING, "Unable to create SIP socket: %s\n", strerror(errno));
			tris_config_destroy(cfg);
			return -1;
		} else {
			...
			tris_enable_packet_fragmentation(sipsock);
			...
			if (bind(sipsock, (struct sockaddr *)&bindaddr, sizeof(bindaddr)) < 0) {
				...
				tris_verb(2, "SIP Listening on %s:%d\n",
						tris_inet_ntoa(bindaddr.sin_addr), ntohs(bindaddr.sin_port));
				tris_netsock_set_qos(sipsock, global_tos_sip, global_cos_sip, "SIP");
			}
		}
	}
	if (stunaddr.sin_addr.s_addr != 0) {
		tris_debug(1, "stun to %s:%d\n",
			tris_inet_ntoa(stunaddr.sin_addr) , ntohs(stunaddr.sin_port));
		tris_stun_request(sipsock, &stunaddr,
			NULL, &externip);
	...
	if (tris_ssl_setup(sip_tls_desc.tls_cfg)) {
		...
		sip_tls_desc.tls_cfg = NULL;
		tris_log(LOG_WARNING, "SIP TLS server did not load because of errors.\n");
	}
	...
	if (auto_sip_domains) {
		...
		if (bindaddr.sin_addr.s_addr) {
			add_sip_domain(tris_inet_ntoa(bindaddr.sin_addr), SIP_DOMAIN_AUTO, NULL);
		} else if (internip.sin_addr.s_addr) {
		/* Our internal IP address, if configured */
			add_sip_domain(tris_inet_ntoa(internip.sin_addr), SIP_DOMAIN_AUTO, NULL);
		} else {
		...
		if (sip_tcp_desc.local_address.sin_addr.s_addr && !inaddrcmp(&bindaddr, &sip_tcp_desc.local_address))
			add_sip_domain(tris_inet_ntoa(sip_tcp_desc.local_address.sin_addr), SIP_DOMAIN_AUTO, NULL);
		...
		if (sip_tls_desc.local_address.sin_addr.s_addr && !inaddrcmp(&bindaddr, &sip_tls_desc.local_address) && inaddrcmp(&sip_tcp_desc.local_address, &sip_tls_desc.local_address))
			add_sip_domain(tris_inet_ntoa(sip_tls_desc.local_address.sin_addr), SIP_DOMAIN_AUTO, NULL);

		/* Our extern IP address, if configured */
		if (externip.sin_addr.s_addr)
			add_sip_domain(tris_inet_ntoa(externip.sin_addr), SIP_DOMAIN_AUTO, NULL);
	...
	run_end = time(0);
	tris_debug(4, "SIP reload_config done...Runtime= %d sec\n", (int)(run_end-run_start));
...
static struct tris_udptl *sip_get_udptl_peer(struct tris_channel *chan)
{
	...
	sip_pvt_lock(p);
	if (p->udptl && tris_test_flag(&p->flags[0], SIP_DIRECT_MEDIA))
		udptl = p->udptl;
	sip_pvt_unlock(p);
	return udptl;
}
...
static int sip_set_udptl_peer(struct tris_channel *chan, struct tris_udptl *udptl)
{
	...
	if (!tris_test_flag(&p->flags[0], SIP_GOTREFER)) {
		if (!p->pendinginvite) {
			tris_debug(3, "Sending reinvite on SIP '%s' - It's UDPTL soon redirected to IP %s:%d\n", p->callid, tris_inet_ntoa(udptl ? p->udptlredirip.sin_addr : p->ourip.sin_addr), udptl ? ntohs(p->udptlredirip.sin_port) : 0);
			transmit_reinvite_with_sdp(p, TRUE, FALSE);
		} else if (!tris_test_flag(&p->flags[0], SIP_PENDINGBYE)) {
			tris_debug(3, "Deferring reinvite on SIP '%s' - It's UDPTL will be redirected to IP %s:%d\n", p->callid, tris_inet_ntoa(udptl ? p->udptlredirip.sin_addr : p->ourip.sin_addr), udptl ? ntohs(p->udptlredirip.sin_port) : 0);
			tris_set_flag(&p->flags[0], SIP_NEEDREINVITE);
		}
	}
...
static enum tris_rtp_get_result sip_get_rtp_peer(struct tris_channel *chan, struct tris_rtp **rtp)
{
	...
	*rtp = p->rtp;
	...
	if (tris_rtp_getnat(*rtp) && !tris_test_flag(&p->flags[0], SIP_DIRECT_MEDIA_NAT))
		res = TRIS_RTP_TRY_PARTIAL;
	else if (tris_test_flag(&p->flags[0], SIP_DIRECT_MEDIA))
...
static enum tris_rtp_get_result sip_get_vrtp_peer(struct tris_channel *chan, struct tris_rtp **rtp)
{
	...
	*rtp = p->vrtp;
	...
	if (tris_test_flag(&p->flags[0], SIP_DIRECT_MEDIA))
		res = TRIS_RTP_TRY_NATIVE;
...
static int sip_set_rtp_peer(struct tris_channel *chan, struct tris_rtp *rtp, struct tris_rtp *vrtp, struct tris_rtp *trtp, int codecs, int nat_active)
{
	...
	if (!tris_bridged_channel(chan) && !sip_cfg.directrtpsetup) 	/* We are in early state */
		return 0;
	...
	*/
        if (nat_active && !tris_test_flag(&p->flags[0], SIP_DIRECT_MEDIA_NAT)) {
                sip_pvt_unlock(p);
                return 0;
        }
	...
	if (changed && !tris_test_flag(&p->flags[0], SIP_GOTREFER) && !tris_test_flag(&p->flags[0], SIP_DEFER_BYE_ON_TRANSFER)) {
		if (chan->_state != TRIS_STATE_UP) {	/* We are in early state */
			if (p->do_history)
				append_history(p, "ExtInv", "Initial invite sent with remote bridge proposal.");
			tris_debug(1, "Early remote bridge setting SIP '%s' - Sending media to %s\n", p->callid, tris_inet_ntoa(rtp ? p->redirip.sin_addr : p->ourip.sin_addr));
		} else if (!p->pendinginvite) {		/* We are up, and have no outstanding invite */
			tris_debug(3, "Sending reinvite on SIP '%s' - It's audio soon redirected to IP %s\n", p->callid, tris_inet_ntoa(rtp ? p->redirip.sin_addr : p->ourip.sin_addr));
			transmit_reinvite_with_sdp(p, FALSE, FALSE);
		} else if (!tris_test_flag(&p->flags[0], SIP_PENDINGBYE)) {
			tris_debug(3, "Deferring reinvite on SIP '%s' - It's audio will be redirected to IP %s\n", p->callid, tris_inet_ntoa(rtp ? p->redirip.sin_addr : p->ourip.sin_addr));
...
static int sip_dtmfmode(struct tris_channel *chan, void *data)
{
	...
	tris_channel_lock(chan);
	if (!IS_SIP_TECH(chan->tech)) {
		tris_log(LOG_WARNING, "Call this application only on SIP incoming calls\n");
		tris_channel_unlock(chan);
		return 0;
	}
	p = chan->tech_pvt;
	...
	if (!strcasecmp(mode, "info")) {
		tris_clear_flag(&p->flags[0], SIP_DTMF);
		tris_set_flag(&p->flags[0], SIP_DTMF_INFO);
		p->jointnoncodeccapability &= ~TRIS_RTP_DTMF;
	} else if (!strcasecmp(mode, "shortinfo")) {
		tris_clear_flag(&p->flags[0], SIP_DTMF);
		tris_set_flag(&p->flags[0], SIP_DTMF_SHORTINFO);
		p->jointnoncodeccapability &= ~TRIS_RTP_DTMF;
...
static int sip_addheader(struct tris_channel *chan, void *data)
{
	...
	if (ok) {
		...
		if (sipdebug) {
			tris_debug(1, "SIP Header added \"%s\" as %s\n", inbuf, varbuf);
		}
	} else {
		tris_log(LOG_WARNING, "Too many SIP headers added, max 50\n");
	}
	tris_channel_unlock(chan);
	return 0;
}
...
static int sip_removeheader(struct tris_channel *chan, void *data)
{
	...
	TRIS_LIST_TRAVERSE_SAFE_BEGIN (headp, newvariable, entries) {
		if (strncasecmp(tris_var_name(newvariable), "SIPADDHEADER", strlen("SIPADDHEADER")) == 0) {
			if (removeall || (!strncasecmp(tris_var_value(newvariable),inbuf,strlen(inbuf)))) {
				if (sipdebug)
					tris_log(LOG_DEBUG,"removing SIP Header \"%s\" as %s\n",
...
static int sip_sipredirect(struct sip_pvt *p, const char *dest)
{
	...
	if (!host) {
		...
		if (tris_strlen_zero(tmp)) {
			tris_log(LOG_ERROR, "Cannot retrieve the 'To' header from the original SIP request!\n");
			return 0;
		}
...
static void sip_poke_all_peers(void)
{
	...
	while ((peer = ao2_t_iterator_next(&i, "iterate thru peers table"))) {
		...
		TRIS_SCHED_REPLACE_UNREF(peer->pokeexpire, sched, ms, sip_poke_peer_s, peer,
				unref_peer(_data, "removing poke peer ref"),
				unref_peer(peer, "removing poke peer ref"),
				ref_peer(peer, "adding poke peer ref"));
		ao2_unlock(peer);
...
static void sip_send_all_registers(void)
{
	...
	ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
		...
		ms += regspacing;
		TRIS_SCHED_REPLACE_UNREF(iterator->expire, sched, ms, sip_reregister, iterator, 
...
static void sip_send_all_mwi_subscriptions(void)
{
	ASTOBJ_CONTAINER_TRAVERSE(&submwil, 1, do {
		...
		if ((iterator->resub = tris_sched_add(sched, 1, sip_subscribe_mwi_do, ASTOBJ_REF(iterator))) < 0) {
...
static char *sip_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_mutex_lock(&sip_reload_lock);
	if (sip_reloading) 
		tris_verbose("Previous SIP reload not yet done\n");
	else {
		sip_reloading = TRUE;
...
static struct tris_cli_entry cli_sip[] = {
	TRIS_CLI_DEFINE(sip_show_channels, "List active SIP channels or subscriptions"),
	TRIS_CLI_DEFINE(sip_show_channelstats, "List statistics for active SIP channels"),
	TRIS_CLI_DEFINE(sip_show_domains, "List our local SIP domains"),
	TRIS_CLI_DEFINE(sip_show_inuse, "List all inuse/limits"),
	TRIS_CLI_DEFINE(sip_show_objects, "List all SIP object allocations"),
	TRIS_CLI_DEFINE(sip_show_peers, "List defined SIP peers"),
	TRIS_CLI_DEFINE(sip_show_registry, "List SIP registration status"),
	TRIS_CLI_DEFINE(sip_unregister, "Unregister (force expiration) a SIP peer from the registry"),
	TRIS_CLI_DEFINE(sip_show_settings, "Show SIP global settings"),
	TRIS_CLI_DEFINE(sip_show_mwi, "Show MWI subscriptions"),
	TRIS_CLI_DEFINE(sip_cli_notify, "Send a notify packet to a SIP peer"),
	TRIS_CLI_DEFINE(sip_show_channel, "Show detailed SIP channel info"),
	TRIS_CLI_DEFINE(sip_show_history, "Show SIP dialog history"),
	TRIS_CLI_DEFINE(sip_show_peer, "Show details on specific SIP peer"),
	TRIS_CLI_DEFINE(sip_show_users, "List defined SIP users"),
	TRIS_CLI_DEFINE(sip_show_user, "Show details on specific SIP user"),
	TRIS_CLI_DEFINE(sip_qualify_peer, "Send an OPTIONS packet to a peer"),
	TRIS_CLI_DEFINE(sip_show_sched, "Present a report on the status of the sched queue"),
	TRIS_CLI_DEFINE(sip_prune_realtime, "Prune cached Realtime users/peers"),
	TRIS_CLI_DEFINE(sip_do_debug, "Enable/Disable SIP debugging"),
	TRIS_CLI_DEFINE(sip_set_history, "Enable/Disable SIP history"),
	TRIS_CLI_DEFINE(sip_reload, "Reload SIP configuration"),
	TRIS_CLI_DEFINE(sip_show_tcp, "List TCP Connections")
};

/*! \brief PBX load module - initialization */
static int load_module(void)
{
	tris_verbose("SIP channel loading...\n");
	...
	if (tris_channel_register(&sip_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel type 'SIP'\n");
	...
	tris_cli_register_multiple(cli_sip, ARRAY_LEN(cli_sip));

	/* Tell the RTP subdriver that we're here */
	tris_rtp_proto_register(&sip_rtp);

	/* Tell the UDPTL subdriver that we're here */
	tris_udptl_proto_register(&sip_udptl);

	/* Register dialplan applications */
	tris_register_application_xml(app_dtmfmode, sip_dtmfmode);
	tris_register_application_xml(app_sipaddheader, sip_addheader);
	tris_register_application_xml(app_sipremoveheader, sip_removeheader);

	/* Register dialplan functions */
	tris_custom_function_register(&sip_header_function);
	tris_custom_function_register(&sippeer_function);
	tris_custom_function_register(&sipchaninfo_function);
	tris_custom_function_register(&checksipdomain_function);

	/* Register manager commands */
	tris_manager_register2("SIPpeers", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_sip_show_peers,
			"List SIP peers (text format)", mandescr_show_peers);
	tris_manager_register2("SIPshowpeer", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_sip_show_peer,
			"Show SIP peer (text format)", mandescr_show_peer);
	tris_manager_register2("SIPqualifypeer", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_sip_qualify_peer,
			"Show SIP peer (text format)", mandescr_show_peer);	/*! \todo Fix this XXX This must be all wrong XXXX */
	tris_manager_register2("SIPshowregistry", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_show_registry,
			"Show SIP registrations (text format)", mandescr_show_registry);
	tris_manager_register2("SIPnotify", EVENT_FLAG_SYSTEM, manager_sipnotify,
	...
	tris_realtime_require_field(tris_check_realtime("sipregs") ? "sipregs" : "sippeers",
		"name", RQ_CHAR, 10,
		"ipaddr", RQ_CHAR, 15,
		"port", RQ_UINTEGER2, 5,
...
static int unload_module(void)
{
	...
	tris_channel_unregister(&sip_tech);

	/* Unregister dial plan functions */
	tris_custom_function_unregister(&sipchaninfo_function);
	tris_custom_function_unregister(&sippeer_function);
	tris_custom_function_unregister(&sip_header_function);
	tris_custom_function_unregister(&checksipdomain_function);

	/* Unregister dial plan applications */
	tris_unregister_application(app_dtmfmode);
	tris_unregister_application(app_sipaddheader);
	tris_unregister_application(app_sipremoveheader);

	/* Unregister CLI commands */
	tris_cli_unregister_multiple(cli_sip, ARRAY_LEN(cli_sip));

	/* Disconnect from the RTP subsystem */
	tris_rtp_proto_unregister(&sip_rtp);

	/* Disconnect from UDPTL */
	tris_udptl_proto_unregister(&sip_udptl);

	/* Unregister AMI actions */
	tris_manager_unregister("SIPpeers");
	tris_manager_unregister("SIPshowpeer");
	tris_manager_unregister("SIPqualifypeer");
	tris_manager_unregister("SIPshowregistry");
	tris_manager_unregister("SIPnotify");
	
	/* Kill TCP/TLS server threads */
	if (sip_tcp_desc.master)
		tris_tcptls_server_stop(&sip_tcp_desc);
	if (sip_tls_desc.master)
		tris_tcptls_server_stop(&sip_tls_desc);
	...
	tris_mutex_lock(&spclock);
	if (spc_thread && (spc_thread != TRIS_PTHREADT_STOP) && (spc_thread != TRIS_PTHREADT_NULL)) {
		pthread_cancel(spc_thread);
		pthread_kill(spc_thread, SIGURG);
		pthread_join(spc_thread, NULL);
	}
	spc_thread = TRIS_PTHREADT_STOP;
	tris_mutex_unlock(&spclock);
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Session Initiation Protocol (SIP)",
		.load = load_module,
...
static struct tris_rtp * sip_get_rtp(char * rtp_name, struct tris_channel * src_chan)
{
	struct sip_pvt * src_pvt = src_chan->tech_pvt;
	if(!strcasecmp(rtp_name, "desktop"))
		return src_pvt->drtp;
	else
		return NULL;
}
...
static int sip_set_rtp(char * rtp_name, struct tris_channel * dst_chan, struct tris_channel * src_chan)
{
	struct tris_rtp * dst_rtp = NULL, * src_rtp = NULL;
	struct sip_pvt * dst_pvt = dst_chan->tech_pvt;
	struct sip_pvt * src_pvt = src_chan->tech_pvt;
	int needrtp = 0;

	tris_log(LOG_DEBUG, "yz: set sip channel rtp. rtp name=%s\n", rtp_name);
	
	if(!strcasecmp(rtp_name, "desktop")){
		if(dst_pvt->drtp)	dst_rtp = dst_pvt->drtp;
		if(src_pvt->drtp)	src_rtp = src_pvt->drtp;
		if(src_rtp && !src_pvt->nodesktop){
			needrtp = 1;
			dst_pvt->nodesktop = 0;
			dst_pvt->offered_media[SDP_DESKTOP].offered = 1;
			tris_copy_string(dst_pvt->offered_media[SDP_DESKTOP].text, src_pvt->offered_media[SDP_DESKTOP].text, sizeof(dst_pvt->offered_media[SDP_DESKTOP].text));
		}else{
			dst_pvt->nodesktop = 1;
			dst_pvt->offered_media[SDP_DESKTOP].offered = 0;
			dst_pvt->offered_media[SDP_DESKTOP].text[0] = '\0';
		}
	}
	
	tris_log(LOG_DEBUG, "yz: dst_rtp=%p, src_rtp=%p, needrtp=%d\n", dst_rtp, src_rtp, needrtp);

	if(needrtp == 1){ /* desktop sharing beginup */
		/* if desktop sharing rpt is not allocated, we create the rtp for desktop sharing. */
		if(!dst_rtp){
			dst_pvt->drtp = tris_rtp_new_with_bindaddr(sched, io, 1, 0, internip.sin_addr);
			if(!dst_pvt->drtp){
				tris_log(LOG_WARNING, "Unable to create RTP desktop session: %s\n", strerror(errno));
				return -1;
			}
			tris_rtp_setqos(dst_rtp, 0, 0, "SIP DRTP");
			tris_rtp_setdtmf(dst_rtp, 0);
			tris_rtp_setdtmfcompensate(dst_rtp, 0);
			tris_rtp_set_rtptimeout(dst_rtp, 0);
			tris_rtp_set_rtpholdtimeout(dst_rtp, 0);
			tris_rtp_set_rtpkeepalive(dst_rtp, 0);
			
			tris_log(LOG_DEBUG, "yz: dst rtp made. dst_rtp=%p\n", dst_rtp);
		}
	
		tris_rtp_pt_clear(dst_rtp);  /* Must be cleared */ 
	
		tris_rtp_pt_copy(dst_rtp, src_rtp);
	}else if(dst_rtp){		/* desktop sharing endup */
		tris_rtp_pt_clear(dst_rtp);
		tris_rtp_stop(dst_rtp);
	}
