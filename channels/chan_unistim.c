 ...
 *
 * UNISTIM channel driver for trismedia
 *
 * Copyright (C) 2005 - 2007, Cedric Hans
 * 
 * Cedric Hans <cedric.hans@mlkj.net>
 *
 * Trismedia 1.4 patch by Peter Be
 *
 ...
 * \file
 *
 * \brief chan_unistim channel driver for Trismedia
...
#include "trismedia/paths.h"	/* tris_config_TRIS_LOG_DIR used in (too ?) many places */
...
/*! Beware, G729 and G723 are not supported by trismedia, except with the proper licence */
#define CAPABILITY TRIS_FORMAT_ALAW | TRIS_FORMAT_ULAW    /* | TRIS_FORMAT_G729A | TRIS_FORMAT_G723_1 */
...
#define TRIS_CONFIG_MAX_PATH     255
...
/* #define DEBUG_TIMER tris_verbose */
...
TRIS_MUTEX_DEFINE_STATIC(unistim_reload_lock);
TRIS_MUTEX_DEFINE_STATIC(usecnt_lock);
static int usecnt = 0;
/* extern char tris_config_TRIS_LOG_DIR[TRIS_CONFIG_MAX_PATH]; */
...
TRIS_MUTEX_DEFINE_STATIC(sessionlock);
/*! Protect the device list */
TRIS_MUTEX_DEFINE_STATIC(devicelock);
...
struct unistim_subchannel {
	...
	/*! Trismedia channel used by the subchannel */
...
static struct unistim_device {
	...
	struct tris_tone_zone *tz;	       /*!< Tone zone for res_indications (ring, busy, congestion) */
	...
	char call_forward[TRIS_MAX_EXTENSION];   /*!< Forward number */
	...
	int nat;					/*!< Used by the obscure tris_rtp_setnat */
	...
	struct tris_silence_generator *silence_generator;
...
/*! Protos */
static struct tris_channel *unistim_new(struct unistim_subchannel *sub, int state);
static int load_module(void);
static int reload(void);
static int unload_module(void);
static int reload_config(void);
static void show_main_page(struct unistimsession *pte);
static struct tris_channel *unistim_request(const char *type, int format, 
	void *data, int *cause, struct tris_channel* src);
static int unistim_call(struct tris_channel *ast, char *dest, int timeout);
static int unistim_hangup(struct tris_channel *ast);
static int unistim_answer(struct tris_channel *ast);
static struct tris_frame *unistim_read(struct tris_channel *ast);
static int unistim_write(struct tris_channel *ast, struct tris_frame *frame);
static int unistim_indicate(struct tris_channel *ast, int ind, const void *data,
	size_t datalen);
static int unistim_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
static int unistim_senddigit_begin(struct tris_channel *ast, char digit);
static int unistim_senddigit_end(struct tris_channel *ast, char digit, 
	unsigned int duration);
static int unistim_sendtext(struct tris_channel *ast, const char *text);
...
static const struct tris_channel_tech unistim_tech = {
	.type = channel_type,
	.description = tdesc,
	...
	const struct sockaddr_in *addr_ourip)
{
	...
	if (unistimdebug) {
		...
		tris_verb(0, "\n**> From %s sending %d bytes to %s ***\n",
					tris_inet_ntoa(addr_ourip->sin_addr), (int) size,
					tris_inet_ntoa(addr_to->sin_addr));
		for (tmp = 0; tmp < size; tmp++)
			tris_verb(0, "%.2x ", (unsigned char) data[tmp]);
		tris_verb(0, "\n******************************************\n");
...
static void send_client(int size, const unsigned char *data, struct unistimsession *pte)
{
	...
	tris_mutex_lock(&pte->lock);
	buf_pos = pte->last_buf_available;

	if (buf_pos >= MAX_BUF_NUMBER) {
		tris_log(LOG_WARNING, "Error : send queue overflow\n");
		tris_mutex_unlock(&pte->lock);
		return;
	}
...
/*#ifdef DUMP_PACKET */
	if (unistimdebug)
		tris_verb(6, "Sending datas with seq #0x%.4x Using slot #%d :\n", pte->seq_server, buf_pos);
/*#endif */
...
static int get_to_address(int fd, struct sockaddr_in *toAddr)
{
	...
	if (err == -1)
		tris_log(LOG_WARNING, "recvmsg returned an error: %s\n", strerror(errno));
...
static struct unistimsession *create_client(const struct sockaddr_in *addr_from)
{
	...
	if (!(s = tris_calloc(1, sizeof(*s))))
		return NULL;

	memcpy(&s->sin, addr_from, sizeof(struct sockaddr_in));
	get_to_address(unistimsock, &s->sout);
	if (unistimdebug) {
		tris_verb(0, "Creating a new entry for the phone from %s received via server ip %s\n",
			 tris_inet_ntoa(addr_from->sin_addr), tris_inet_ntoa(s->sout.sin_addr));
	}
	tris_mutex_init(&s->lock);
	tris_mutex_lock(&sessionlock);
	s->next = sessions;
	sessions = s;
...
static void check_send_queue(struct unistimsession *pte)
{
	...
	if (pte->last_buf_available == 1) {
		if (unistimdebug)
			tris_verb(6, "Our single packet was ACKed.\n");
		pte->last_buf_available--;
		set_ping_timer(pte);
		return;
	}
	/* Check if this ACK catch up our latest packet */
		...
		if (unistimdebug)
			tris_verb(6, "Our send queue is completely ACKed.\n");
		pte->last_buf_available = 0;    /* Purge the send queue */
		set_ping_timer(pte);
		return;
	}
	if (unistimdebug)
		tris_verb(6, "We still have packets in our send queue\n");
	return;
}
...
static void send_start_timer(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending start timer\n");
...
static void send_stop_timer(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending stop timer\n");
...
static void Sendicon(unsigned char pos, unsigned char status, struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending icon pos %d with status 0x%.2x\n", pos, status);
...
static void send_tone(struct unistimsession *pte, uint16_t tone1, uint16_t tone2)
{
	...
	if (!tone1) {
		if (unistimdebug)
			tris_verb(0, "Sending Stream Based Tone Off\n");
	...
	/* Since most of the world use a continuous tone, it's useless
	   if (unistimdebug)
	   tris_verb(0, "Sending Stream Based Tone Cadence Download\n");
	...
	if (unistimdebug)
		tris_verb(0, "Sending Stream Based Tone Frequency Component List Download %d %d\n", tone1, tone2);
	tone1 *= 8;
	if (!tone2) {
	...
	if (unistimdebug)
		tris_verb(0, "Sending Stream Based Tone On\n");
			 ...
			 const char *text)
{
	...
	if (unistimdebug)
		tris_verb(0, "Sending favorite pos %d with status 0x%.2x\n", pos, status);
...
static void refresh_all_favorite(struct unistimsession *pte)
{
	...
	if (unistimdebug)
		tris_verb(0, "Refreshing all favorite\n");
	for (i = 0; i < 6; i++) {
...
{
	if (unistimdebug)
		tris_verb(0, "Trying to register extension '%s' into context '%s' to %s\n",
					pte->device->extension_number, pte->device->lines->context,
					pte->device->lines->fullname);
	return tris_add_extension(pte->device->lines->context, 0,
...
{
	if (unistimdebug)
		tris_verb(0, "Trying to unregister extension '%s' context '%s'\n",
					pte->device->extension_number, pte->device->lines->context);
	return tris_context_remove_extension(pte->device->lines->context,
...
static void close_client(struct unistimsession *s)
{
	...
	if (cur) {				      /* Session found ? */
		if (cur->device) {	      /* This session was registered ? */
			...
			if (unistimdebug)
				tris_verb(0, "close_client session %p device %p lines %p sub %p\n",
			...
			if (sub) {
				if (sub->owner) {       /* Call in progress ? */
					if (unistimdebug)
						tris_verb(0, "Aborting call\n");
					tris_queue_hangup_with_cause(sub->owner, TRIS_CAUSE_NETWORK_OUT_OF_ORDER);
				}
			} else
				tris_log(LOG_WARNING, "Freeing a client with no subchannel !\n");
			if (!tris_strlen_zero(s->device->extension_number))
				UnregisterExtension(s);
			cur->device->session = NULL;
		} else {
			if (unistimdebug)
				tris_verb(0, "Freeing an unregistered client\n");
		}
		if (prev)
			prev->next = cur->next;
		else
		...
		tris_log(LOG_WARNING, "Trying to delete non-existent session %p?\n", s);
	tris_mutex_unlock(&sessionlock);
	return;
}
...
static int send_retransmit(struct unistimsession *pte)
{
	...
	if (++pte->nb_retransmit >= NB_MAX_RETRANSMIT) {
		if (unistimdebug)
			tris_verb(0, "Too many retransmit - freeing client\n");
		tris_mutex_unlock(&pte->lock);
		close_client(pte);
		return 1;
	}
	...
	for (i = pte->last_buf_available - (pte->seq_server - pte->last_seq_ack);
		...
		if (unistimdebug) {
			...
			seq = ntohs(sbuf[1]);
			tris_verb(0, "Retransmit slot #%d (seq=#0x%.4x), last ack was #0x%.4x\n", i,
		 ...
		 const char *text)
{
	...
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending text at pos %d, inverse flag %d\n", pos, inverse);
...
static void send_text_status(struct unistimsession *pte, const char *text)
{
	...
	int i;
	if (unistimdebug)
		tris_verb(0, "Sending status text\n");
	if (pte->device) {
...
static void send_led_update(struct unistimsession *pte, unsigned char led)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending led_update (%x)\n", led);
				 ...
				 unsigned char mute)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending select output packet output=%x volume=%x mute=%x\n", output,
					volume, mute);
	...
	if (output == OUTPUT_HANDSET) {
		...
		tris_log(LOG_WARNING, "Invalid ouput (%d)\n", output);
...
static void send_ring(struct unistimsession *pte, char volume, char style)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending ring packet\n");
...
static void send_no_ring(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending no ring packet\n");
...
static void send_texttitle(struct unistimsession *pte, const char *text)
{
	...
	int i;
	if (unistimdebug)
		tris_verb(0, "Sending title text\n");
...
static void send_date_time2(struct unistimsession *pte)
{
	...
	if (unistimdebug)
		tris_verb(0, "Sending Time & Date #2\n");
...
static void send_date_time3(struct unistimsession *pte)
{
	...
	if (unistimdebug)
		tris_verb(0, "Sending Time & Date #3\n");
...
static void send_cursor_pos(struct unistimsession *pte, unsigned char pos)
{
	BUFFSEND;
	if (unistimdebug)
		tris_verb(0, "Sending set cursor position\n");
...
static void rcv_resume_connection_with_server(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		tris_verb(0, "ResumeConnectionWithServer received\n");
		tris_verb(0, "Sending packet_send_query_mac_address\n");
	}
...
static int alloc_sub(struct unistim_line *l, int x)
{
	...
	if (unistimdebug)
		tris_verb(3, "Allocating UNISTIM subchannel #%d on %s@%s ptr=%p\n", x, l->name, l->parent->name, sub);
	sub->parent = l;
	sub->subtype = x;
	l->subs[x] = sub;
...
static void rcv_mac_addr(struct unistimsession *pte, const unsigned char *buf)
{
	...
	int res = 0;
	if (unistimdebug)
		tris_verb(0, "Mac Address received : ");
	...
	if (!res) {
		...
		case AUTOPROVISIONING_NO:
			tris_log(LOG_WARNING, "No entry found for this phone : %s\n", addrmac);
		...
		case AUTOPROVISIONING_YES:
			{
				...
				if (unistimdebug)
					tris_verb(0, "New phone, autoprovisioning on\n");
				...
				while (d) {
					if (!strcasecmp(d->name, "template")) {
						...
						if (!(newd = tris_malloc(sizeof(*newd)))) {
							tris_mutex_unlock(&devicelock);
							return;
						}

						memcpy(newd, d, sizeof(*newd));
						if (!(newl = tris_malloc(sizeof(*newl)))) {
							tris_free(newd);
							tris_mutex_unlock(&devicelock);
							return;
						}
						...
						tris_copy_string(newd->id, addrmac, sizeof(newd->id));
						tris_copy_string(newd->name, addrmac, sizeof(newd->name));
				...
				tris_mutex_unlock(&devicelock);
				if (!d) {
					tris_log(LOG_WARNING, "No entry [template] found in unistim.conf\n");
					pte->state = STATE_AUTHDENY;
				}
			}
			break;
		...
		case AUTOPROVISIONING_DB:
			...
			break;
		default:
			tris_log(LOG_WARNING, "Internal error : unknown autoprovisioning value = %d\n",
					autoprovisioning);
		}
	}
	if (pte->state != STATE_AUTHDENY) {
		tris_verb(3, "Device '%s' successfuly registered\n", pte->device->name);
		...
		case EXTENSION_ASK:
			...
			if (tris_strlen_zero(pte->device->extension_number))
			...
			break;
		case EXTENSION_LINE:
			tris_copy_string(pte->device->extension_number, pte->device->lines->name,
		...
		case EXTENSION_TN:
			...
			break;
		default:
			tris_log(LOG_WARNING, "Internal error, extension value unknown : %d\n",
	...
	if (unistimdebug)
		tris_verb(0, "Sending query_basic_manager_04\n");
	...
	if (unistimdebug)
		tris_verb(0, "Sending query_basic_manager_10\n");
...
{
	char tmp[TRIS_CONFIG_MAX_PATH], tmp2[TRIS_CONFIG_MAX_PATH];
	...
	snprintf(tmp, sizeof(tmp), "%s/%s", tris_config_TRIS_LOG_DIR, USTM_LOG_DIR);
	if (tris_mkdir(tmp, 0770)) {
		if (errno != EEXIST) {
	...
	snprintf(tmp, sizeof(tmp), "%s/%s/%s-%c.csv", tris_config_TRIS_LOG_DIR,
	...
	if (count > MAX_ENTRY_LOG) {
		tris_log(LOG_WARNING, "Invalid count in history header of %s (%d max %d)\n", tmp,
				count, MAX_ENTRY_LOG);
		fclose(f);
		return -1;
	}
	snprintf(tmp2, sizeof(tmp2), "%s/%s/%s-%c.csv.tmp", tris_config_TRIS_LOG_DIR,
			 USTM_LOG_DIR, pte->device->name, way);
	if (!(f2 = fopen(tmp2, "w"))) {
...
static int attempt_transfer(struct unistim_subchannel *p1, struct unistim_subchannel *p2)
{
	...
	if (!p1->owner || !p2->owner) {
		tris_log(LOG_WARNING, "Transfer attempted without dual ownership?\n");
		return -1;
	}
	chana = p1->owner;
	chanb = p2->owner;
	bridgea = tris_bridged_channel(chana);
	bridgeb = tris_bridged_channel(chanb);
	...
	if (peera && peerb && peerc && (peerb != peerc)) {
		...
		if (peerb->cdr && peerc->cdr) {
			peerb->cdr = tris_cdr_append(peerb->cdr, peerc->cdr);
		} else if (peerc->cdr) {
			peerb->cdr = peerc->cdr;
		}
		peerc->cdr = NULL;
		...
		if (chana)
			tris_softhangup_nolock(chana, TRIS_SOFTHANGUP_DEV);
		if (chanb)
			tris_softhangup_nolock(chanb, TRIS_SOFTHANGUP_DEV);
		return -1;
	}
	return 0;
}
	...
	if (sub->owner) {
		...
		if (l->subs[SUB_THREEWAY]) {
			...
			if (attempt_transfer(sub, l->subs[SUB_THREEWAY]) < 0)
				tris_verb(0, "attempt_transfer failed.\n");
		} else
			tris_queue_hangup(sub->owner);
	} else {
		if (l->subs[SUB_THREEWAY]) {
			if (l->subs[SUB_THREEWAY]->owner)
				tris_queue_hangup_with_cause(l->subs[SUB_THREEWAY]->owner, TRIS_CAUSE_NORMAL_CLEARING);
			else
				tris_log(LOG_WARNING, "threeway sub without owner\n");
		} else
			tris_verb(0, "USTM(%s@%s-%d) channel already destroyed\n", sub->parent->name,
...
static void *unistim_ss(void *data)
{
	...
	tris_verb(3, "Starting switch on '%s@%s-%d' to %s\n", l->name, l->parent->name, sub->subtype, s->device->phone_number);
	tris_copy_string(chan->exten, s->device->phone_number, sizeof(chan->exten));
	tris_copy_string(s->device->redial_number, s->device->phone_number,
...
static void start_rtp(struct unistim_subchannel *sub)
{
	...
	/* Sanity checks */
	if (!sub) {
		tris_log(LOG_WARNING, "start_rtp with a null subchannel !\n");
		return;
	}
	if (!sub->parent) {
		tris_log(LOG_WARNING, "start_rtp with a null line !\n");
		return;
	}
	if (!sub->parent->parent) {
		tris_log(LOG_WARNING, "start_rtp with a null device !\n");
		return;
	}
	if (!sub->parent->parent->session) {
		tris_log(LOG_WARNING, "start_rtp with a null session !\n");
		return;
	}
	...
	/* Allocate the RTP */
	if (unistimdebug)
		tris_verb(0, "Starting RTP. Bind on %s\n", tris_inet_ntoa(sout.sin_addr));
	sub->rtp = tris_rtp_new_with_bindaddr(sched, io, 1, 0, sout.sin_addr);
	if (!sub->rtp) {
		tris_log(LOG_WARNING, "Unable to create RTP session: %s binaddr=%s\n",
				strerror(errno), tris_inet_ntoa(sout.sin_addr));
		tris_mutex_unlock(&sub->lock);
		return;
	}
	if (sub->rtp && sub->owner) {
		sub->owner->fds[0] = tris_rtp_fd(sub->rtp);
		sub->owner->fds[1] = tris_rtcp_fd(sub->rtp);
	}
	if (sub->rtp) {
		tris_rtp_setqos(sub->rtp, qos.tos_audio, qos.cos_audio, "UNISTIM RTP");
		tris_rtp_setnat(sub->rtp, sub->parent->parent->nat);
	}
	...
	if (!(sub->owner->nativeformats & sub->owner->readformat)) {
		int fmt;
		fmt = tris_best_codec(sub->owner->nativeformats);
		tris_log(LOG_WARNING,
				"Our read/writeformat has been changed to something incompatible : %s (%d), using %s (%d) best codec from %d\n",
				tris_getformatname(sub->owner->readformat),
				sub->owner->readformat, tris_getformatname(fmt), fmt,
				sub->owner->nativeformats);
		sub->owner->readformat = fmt;
		sub->owner->writeformat = fmt;
	}
	codec = tris_rtp_lookup_code(sub->rtp, 1, sub->owner->readformat);
	...
	if (unistimdebug) {
		tris_verb(0, "RTP started : Our IP/port is : %s:%hd with codec %s (%d)\n",
			 tris_inet_ntoa(us.sin_addr),
			 htons(us.sin_port), tris_getformatname(sub->owner->readformat),
			 sub->owner->readformat);
		tris_verb(0, "Starting phone RTP stack. Our public IP is %s\n",
					tris_inet_ntoa(public.sin_addr));
	}
	if ((sub->owner->readformat == TRIS_FORMAT_ULAW) ||
		(sub->owner->readformat == TRIS_FORMAT_ALAW)) {
		if (unistimdebug)
			tris_verb(0, "Sending packet_send_rtp_packet_size for codec %d\n", codec);
	...
	if (unistimdebug)
		tris_verb(0, "Sending Jitter Buffer Parameters Configuration\n");
	...
	if (sub->parent->parent->rtp_method != 0) {
		...
		if (unistimdebug)
			tris_verb(0, "Sending OpenAudioStreamTX using method #%d\n",
		...
		if (unistimdebug)
			tris_verb(0, "Sending OpenAudioStreamRX\n");
		if (sub->parent->parent->rtp_method == 3)
		...
		if (unistimdebug)
			tris_verb(0, "Sending packet_send_call default method\n");
		...
		else if (sub->owner->readformat == TRIS_FORMAT_ALAW)
			buffsend[42] = 1;       /* 1 = 20ms (160 bytes), 2 = 40ms (320 bytes) */
		else if (sub->owner->readformat == TRIS_FORMAT_G723_1)
			buffsend[42] = 2;       /* 1 = 30ms (24 bytes), 2 = 60 ms (48 bytes) */
		else if (sub->owner->readformat == TRIS_FORMAT_G729A)
			buffsend[42] = 2;       /* 1 = 10ms (10 bytes), 2 = 20ms (20 bytes) */
		else
			tris_log(LOG_WARNING, "Unsupported codec %s (%d) !\n",
					tris_getformatname(sub->owner->readformat), sub->owner->readformat);
...
static void SendDialTone(struct unistimsession *pte)
{
	...
	if (tris_strlen_zero(pte->device->country)) {
		if (unistimdebug)
			tris_verb(0, "No country defined, using US tone\n");
		send_tone(pte, 350, 440);
		return;
	}
	if (strlen(pte->device->country) != 2) {
		if (unistimdebug)
			tris_verb(0, "Country code != 2 char, using US tone\n");
		send_tone(pte, 350, 440);
		return;
	}
	i = 0;
	while (frequency[i].freq1) {
		if ((frequency[i].country[0] == pte->device->country[0]) &&
			...
			if (unistimdebug)
				tris_verb(0, "Country code found (%s), freq1=%d freq2=%d\n",
...
static void handle_dial_page(struct unistimsession *pte)
{
	...
	if (pte->device->call_forward[0] == -1) {
		...
		if (pte->device->call_forward[1] != 0) {
			...
			tris_copy_string(pte->device->phone_number, pte->device->call_forward + 1,
...
static void TransferCallStep1(struct unistimsession *pte)
{
	...
	if (!sub->owner) {
		tris_log(LOG_WARNING, "Unable to find subchannel for music on hold\n");
		return;
	}
	if (p->subs[SUB_THREEWAY]) {
		if (unistimdebug)
			tris_verb(0, "Transfer canceled, hangup our threeway channel\n");
		if (p->subs[SUB_THREEWAY]->owner)
			tris_queue_hangup_with_cause(p->subs[SUB_THREEWAY]->owner, TRIS_CAUSE_NORMAL_CLEARING);
		else
			tris_log(LOG_WARNING, "Canceling a threeway channel without owner\n");
		return;
	}
	/* Start music on hold if appropriate */
	if (pte->device->moh)
		tris_log(LOG_WARNING, "Transfer with peer already listening music on hold\n");
	else {
		if (tris_bridged_channel(p->subs[SUB_REAL]->owner)) {
			tris_moh_start(tris_bridged_channel(p->subs[SUB_REAL]->owner),
						  pte->device->lines->musicclass, NULL);
			pte->device->moh = 1;
		} else {
			tris_log(LOG_WARNING, "Unable to find peer subchannel for music on hold\n");
			return;
		}
	}
	/* Silence our channel */
	if (!pte->device->silence_generator) {
		pte->device->silence_generator =
			tris_channel_start_silence_generator(p->subs[SUB_REAL]->owner);
		if (pte->device->silence_generator == NULL)
			tris_log(LOG_WARNING, "Unable to start a silence generator.\n");
		else if (unistimdebug)
			tris_verb(0, "Starting silence generator\n");
	}
...
static void HandleCallOutgoing(struct unistimsession *s)
{
	...
	if (!sub->owner) {		      /* A call is already in progress ? */
		c = unistim_new(sub, TRIS_STATE_DOWN);   /* No, starting a new one */
		if (c) {
			/* Need to start RTP before calling tris_pbx_run */
			if (!sub->rtp)
				start_rtp(sub);
			...
			/* start switch */
			if (tris_pthread_create(&t, NULL, unistim_ss, c)) {
				display_last_error("Unable to create switch thread");
				tris_queue_hangup_with_cause(c, TRIS_CAUSE_SWITCH_CONGESTION);
			}
		} else
		...
		if (s->device->moh) {
			...
			if (s->device->silence_generator) {
				if (unistimdebug)
					tris_verb(0, "Stopping silence generator\n");
				tris_channel_stop_silence_generator(subchannel->owner,
												   s->device->silence_generator);
				s->device->silence_generator = NULL;
			}
			send_tone(s, 0, 0);
			/* Make new channel */
			c = unistim_new(p->subs[SUB_THREEWAY], TRIS_STATE_DOWN);
			if (!c) {
				tris_log(LOG_WARNING, "Cannot allocate new structure on channel %p\n", p);
				return;
			}
			...
			if (tris_pthread_create(&t, NULL, unistim_ss, p->subs[SUB_THREEWAY]->owner)) {
				tris_log(LOG_WARNING, "Unable to start simple switch on channel %p\n", p);
				tris_hangup(c);
				return;
			}
			if (unistimdebug)
				tris_verb(0, "Started three way call on channel %p (%s) subchan %d\n",
...
static void HandleCallIncoming(struct unistimsession *s)
{
	...
	if (!sub) {
		...
		tris_verb(0, "Handle Call Incoming for %s@%s\n", sub->parent->name,
					s->device->name);
	start_rtp(sub);
	if (!sub->rtp)
		tris_log(LOG_WARNING, "Unable to create channel for %s@%s\n", sub->parent->name,
...
{
	struct tris_frame f = { .frametype = TRIS_FRAME_DTMF, .subclass = digit, .src = "unistim" };
	struct unistim_subchannel *sub;
	sub = pte->device->lines->subs[SUB_REAL];
	if (!sub->owner || sub->alreadygone) {
		tris_log(LOG_WARNING, "Unable to find subchannel in dtmf senddigit\n");
		return -1;
	}
...
static void key_call(struct unistimsession *pte, char keycode)
{
	...
	case KEY_ONHOLD:
		{
			struct unistim_subchannel *sub;
			struct tris_channel *bridgepeer = NULL;
			sub = pte->device->lines->subs[SUB_REAL];
			if (!sub->owner) {
				tris_log(LOG_WARNING, "Unable to find subchannel for music on hold\n");
				return;
			}
			if ((bridgepeer = tris_bridged_channel(sub->owner))) {
				if (pte->device->moh) {
					tris_moh_stop(bridgepeer);
					pte->device->moh = 0;
					send_select_output(pte, pte->device->output, pte->device->volume,
									 MUTE_OFF);
				} else {
					tris_moh_start(bridgepeer, pte->device->lines->musicclass, NULL);
					pte->device->moh = 1;
					send_select_output(pte, pte->device->output, pte->device->volume,
									 MUTE_ON);
				}
...
static void Keyfavorite(struct unistimsession *pte, char keycode)
{
	...
	if ((keycode < KEY_FAV1) && (keycode > KEY_FAV5)) {
		tris_log(LOG_WARNING, "It's not a favorite key\n");
		return;
	}
	if (keycode == KEY_FAV0)
		return;
	...
	if (pte->device->softkeyicon[fav] == 0)
		return;
	tris_copy_string(pte->device->phone_number, pte->device->softkeynumber[fav],
					sizeof(pte->device->phone_number));
...
static void key_dial_page(struct unistimsession *pte, char keycode)
{
	...
	if (pte->device->call_forward[0] == -1) {
		if (keycode == KEY_FUNC1) {
			tris_copy_string(pte->device->call_forward, pte->device->phone_number,
	...
	switch (keycode) {
	case KEY_FUNC2:
		if (tris_strlen_zero(pte->device->redial_number))
			break;
		tris_copy_string(pte->device->phone_number, pte->device->redial_number,
						sizeof(pte->device->phone_number));
	case KEY_FUNC1:
		HandleCallOutgoing(pte);
		break;
	case KEY_HANGUP:
		if (pte->device->lines->subs[SUB_REAL]->owner) {
			...
			if (pte->device->silence_generator) {
				if (unistimdebug)
					tris_verb(0, "Stopping silence generator\n");
				tris_channel_stop_silence_generator(pte->device->lines->subs[SUB_REAL]->
												   owner, pte->device->silence_generator);
				pte->device->silence_generator = NULL;
			}
			send_tone(pte, 0, 0);
			tris_moh_stop(tris_bridged_channel(pte->device->lines->subs[SUB_REAL]->owner));
			pte->device->moh = 0;
...
static void key_select_extension(struct unistimsession *pte, char keycode)
{
	...
	case KEY_FUNC1:
		...
		if (autoprovisioning == AUTOPROVISIONING_TN) {
			...
			while (d) {
				if (d->id[0] == 'T') {  /* It's a TN device ? */
					...
					if (!strcmp((d->id) + 1, pte->buff_entry)) {
						...
						tris_copy_string((pte->device->extension_number) + 1,
			...
			tris_copy_string(pte->device->extension_number, pte->buff_entry,
...
static void show_entry_history(struct unistimsession *pte, FILE ** f)
{
	...
	line[sizeof(line) - 1] = '\0';
	tris_copy_string(pte->device->lst_cid, line, sizeof(pte->device->lst_cid));
...
static void show_main_page(struct unistimsession *pte)
{
	...
	if ((pte->device->extension == EXTENSION_ASK) &&
		(tris_strlen_zero(pte->device->extension_number))) {
	...
	send_favorite(pte->device->softkeylinepos, FAV_ICON_ONHOOK_BLACK, pte,
				 pte->device->softkeylabel[pte->device->softkeylinepos]);
	if (!tris_strlen_zero(pte->device->call_forward)) {
	...
	if (tris_strlen_zero(pte->device->maintext2)) {
		strcpy(tmpbuf, "IP : ");
		strcat(tmpbuf, tris_inet_ntoa(pte->sin.sin_addr));
		send_text(TEXT_LINE2, TEXT_NORMAL, pte, tmpbuf);
	} else
...
static void key_history(struct unistimsession *pte, char keycode)
{
	...
	case KEY_FUNC3:
		if (!ReformatNumber(pte->device->lst_cid))
			break;
		tris_copy_string(pte->device->redial_number, pte->device->lst_cid,
...
static void process_request(int size, unsigned char *buf, struct unistimsession *pte)
{
	...
	if (memcmp(buf + SIZE_HEADER, packet_recv_firm_version, sizeof(packet_recv_firm_version)) ==
		...
		buf[size] = 0;
		if (unistimdebug)
			tris_verb(0, "Got the firmware version : '%s'\n", buf + 13);
	...
	if (memcmp(buf + SIZE_HEADER, packet_recv_r2, sizeof(packet_recv_r2)) == 0) {
		if (unistimdebug)
			tris_verb(0, "R2 received\n");
		return;
	}

	if (pte->state < STATE_MAINPAGE) {
		if (unistimdebug)
			tris_verb(0, "Request not authorized in this state\n");
		return;
	}
	if (!memcmp(buf + SIZE_HEADER, packet_recv_pressed_key, sizeof(packet_recv_pressed_key))) {
		...
		if (unistimdebug)
			tris_verb(0, "Key pressed : keycode = 0x%.2x - current state : %d\n", keycode,
						pte->state);

		switch (pte->state) {
		case STATE_INIT:
			if (unistimdebug)
				tris_verb(0, "No keys allowed in the init state\n");
			break;
		case STATE_AUTHDENY:
			if (unistimdebug)
				tris_verb(0, "No keys allowed in authdeny state\n");
			break;
		case STATE_MAINPAGE:
	...
	strcpy(tmpbuf, tris_inet_ntoa(pte->sin.sin_addr));
	strcat(tmpbuf, " Unknown request packet\n");
	if (unistimdebug)
		tris_debug(1, "%s", tmpbuf);
	return;
}
	...
	struct sockaddr_in *addr_from)
{
	...
	strcpy(tmpbuf, tris_inet_ntoa(addr_from->sin_addr));

	if (size < 10) {
		if (size == 0) {
			tris_log(LOG_WARNING, "%s Read error\n", tmpbuf);
		} else {
			tris_log(LOG_NOTICE, "%s Packet too short - ignoring\n", tmpbuf);
		}
		return;
	}
	if (sbuf[0] == 0xffff) {	/* Starting with 0xffff ? *//* Yes, discovery packet ? */
		if (size != sizeof(packet_rcv_discovery)) {
			tris_log(LOG_NOTICE, "%s Invalid size of a discovery packet\n", tmpbuf);
		} else {
			if (memcmp(buf, packet_rcv_discovery, sizeof(packet_rcv_discovery)) == 0) {
				if (unistimdebug)
					tris_verb(0, "Discovery packet received - Sending Discovery ACK\n");
				if (pte) {	      /* A session was already active for this IP ? */
					if (pte->state == STATE_INIT) { /* Yes, but it's a dupe */
						if (unistimdebug)
							tris_verb(1, "Duplicated Discovery packet\n");
			...
			tris_log(LOG_NOTICE, "%s Invalid discovery packet\n", tmpbuf);
		}
		return;
	}
	if (!pte) {
		if (unistimdebug)
			tris_verb(0, "%s Not a discovery packet from an unknown source : ignoring\n",
						tmpbuf);
		return;
	}

	if (sbuf[0] != 0) {		     /* Starting with something else than 0x0000 ? */
		tris_log(LOG_NOTICE, "Unknown packet received - ignoring\n");
		return;
	}
	if (buf[5] != 2) {
		tris_log(LOG_NOTICE, "%s Wrong direction : got 0x%.2x expected 0x02\n", tmpbuf,
				buf[5]);
		return;
	}
	seq = ntohs(sbuf[1]);
	if (buf[4] == 1) {
		tris_mutex_lock(&pte->lock);
		if (unistimdebug)
			tris_verb(6, "ACK received for packet #0x%.4x\n", seq);
		pte->nb_retransmit = 0;
		...
		if (pte->last_seq_ack > seq) {
			if (pte->last_seq_ack == 0xffff) {
				tris_verb(0, "ACK at 0xffff, restarting counter.\n");
				pte->last_seq_ack = 0;
			} else
				tris_log(LOG_NOTICE,
		...
		if (unistimdebug)
			tris_verb(0, "%s ACK gap : Received ACK #0x%.4x, previous was #0x%.4x\n",
						tmpbuf, seq, pte->last_seq_ack);
	...
	if (buf[4] == 0) {
		tris_log(LOG_NOTICE, "%s Retransmit request for packet #0x%.4x\n", tmpbuf, seq);
		if (pte->last_seq_ack > seq) {
			tris_log(LOG_NOTICE,
	...
	tris_log(LOG_NOTICE, "%s Unknown request : got 0x%.2x expected 0x00,0x01 or 0x02\n",
			tmpbuf, buf[4]);
	return;
}

static struct unistimsession *channel_to_session(struct tris_channel *ast)
{
	struct unistim_subchannel *sub;
	if (!ast) {
		tris_log(LOG_WARNING, "Unistim callback function called with a null channel\n");
		return NULL;
	}
	if (!ast->tech_pvt) {
		tris_log(LOG_WARNING, "Unistim callback function called without a tech_pvt\n");
		return NULL;
	}
	sub = ast->tech_pvt;

	if (!sub->parent) {
		tris_log(LOG_WARNING, "Unistim callback function called without a line\n");
		return NULL;
	}
	if (!sub->parent->parent) {
		tris_log(LOG_WARNING, "Unistim callback function called without a device\n");
		return NULL;
	}
	if (!sub->parent->parent->session) {
		tris_log(LOG_WARNING, "Unistim callback function called without a session\n");
		return NULL;
	}
...
static int unistim_call(struct tris_channel *ast, char *dest, int timeout)
{
	...
	if ((ast->_state != TRIS_STATE_DOWN) && (ast->_state != TRIS_STATE_RESERVED)) {
		tris_log(LOG_WARNING, "unistim_call called on %s, neither down nor reserved\n",
				ast->name);
		return -1;
	}

	if (unistimdebug)
		tris_verb(3, "unistim_call(%s)\n", ast->name);
...
static int unistim_hangup(struct tris_channel *ast)
{
	...
	if (!s) {
		...
		if (sub->rtp) {
			if (unistimdebug)
				tris_verb(0, "Destroying RTP session\n");
	...
	l = sub->parent;
	if (unistimdebug)
		tris_verb(0, "unistim_hangup(%s) on %s@%s\n", ast->name, l->name, l->parent->name);

	if ((l->subs[SUB_THREEWAY]) && (sub->subtype == SUB_REAL)) {
		if (unistimdebug)
			tris_verb(0, "Real call disconnected while talking to threeway\n");
		sub->owner = NULL;
		ast->tech_pvt = NULL;
		return 0;
	}
	if ((l->subs[SUB_REAL]->owner) && (sub->subtype == SUB_THREEWAY) &&
		(sub->alreadygone == 0)) {
		if (unistimdebug)
			tris_verb(0, "threeway call disconnected, switching to real call\n");
		...
		send_text_status(s, "Hangup Transf");
		tris_moh_stop(tris_bridged_channel(l->subs[SUB_REAL]->owner));
	...
	if (!s) {
		if (unistimdebug)
			tris_verb(0, "Asked to hangup channel not connected (no session)\n");
		if (sub->rtp) {
			if (unistimdebug)
				tris_verb(0, "Destroying RTP session\n");
			tris_rtp_destroy(sub->rtp);
	...
	if (sub->subtype == SUB_REAL) {
		...
		if (s->device->silence_generator) {
			if (unistimdebug)
				tris_verb(0, "Stopping silence generator\n");
			if (sub->owner)
				tris_channel_stop_silence_generator(sub->owner,
												   s->device->silence_generator);
			else
				tris_log(LOG_WARNING,
	...
	if (sub->rtp) {
		...
		tris_verb(0, "No RTP session to destroy\n");
	if (l->subs[SUB_THREEWAY]) {
		if (unistimdebug)
			tris_verb(0, "Cleaning other subchannels\n");
		unalloc_sub(l, SUB_THREEWAY);
	}
...
static int unistim_answer(struct tris_channel *ast)
{
	...
	s = channel_to_session(ast);
	if (!s) {
		tris_log(LOG_WARNING, "unistim_answer on a disconnected device ?\n");
		return -1;
	}
	sub = ast->tech_pvt;
	l = sub->parent;
	...
	if ((!sub->rtp) && (!l->subs[SUB_THREEWAY]))
		start_rtp(sub);
	if (unistimdebug)
		tris_verb(0, "unistim_answer(%s) on %s@%s-%d\n", ast->name, l->name,
...
static int unistimsock_read(int *id, int fd, short events, void *ignore)
{
	...
	if (dw_num_bytes_rcvd == -1) {
		if (errno == EAGAIN)
			tris_log(LOG_NOTICE, "UNISTIM: Received packet with bad UDP checksum\n");
		else if (errno != ECONNREFUSED)
			tris_log(LOG_WARNING, "Recv error %d (%s)\n", errno, strerror(errno));
		return 1;
	}
	...
	if (unistimdebug)
		tris_verb(0, "\n*** Dump %d bytes from %s - phone_table[%d] ***\n",
					dw_num_bytes_rcvd, tris_inet_ntoa(addr_from.sin_addr), tmp);
	for (dw_num_bytes_rcvdd = 0; dw_num_bytes_rcvdd < dw_num_bytes_rcvd;
		 dw_num_bytes_rcvdd++)
		tris_verb(0, "%.2x ", (unsigned char) buff[dw_num_bytes_rcvdd]);
	tris_verb(0, "\n******************************************\n");
#endif

	if (!found) {
		if (unistimdebug)
			tris_verb(0, "Received a packet from an unknown source\n");
...
static struct tris_frame *unistim_rtp_read(const struct tris_channel *ast,
	const struct unistim_subchannel *sub)
{
	...
	if (!ast) {
		tris_log(LOG_WARNING, "Channel NULL while reading\n");
		return &tris_null_frame;
	}

	if (!sub->rtp) {
		tris_log(LOG_WARNING, "RTP handle NULL while reading on subchannel %d\n",
				sub->subtype);
		return &tris_null_frame;
	}
		...
		if (f->frametype == TRIS_FRAME_VOICE) {
			if (f->subclass != sub->owner->nativeformats) {
				tris_debug(1,
						...
						tris_getformatname(sub->owner->nativeformats),
						sub->owner->nativeformats, tris_getformatname(f->subclass),
						f->subclass);
...
static int unistim_write(struct tris_channel *ast, struct tris_frame *frame)
{
	...
	if (frame->frametype != TRIS_FRAME_VOICE) {
		if (frame->frametype == TRIS_FRAME_IMAGE)
			...
			tris_log(LOG_WARNING, "Can't send %d type frames with unistim_write\n",
					frame->frametype);
			return 0;
		}
	} else {
		if (!(frame->subclass & ast->nativeformats)) {
			tris_log(LOG_WARNING,
					...
					tris_getformatname(frame->subclass), frame->subclass,
					tris_getformatname(ast->nativeformats), ast->nativeformats,
					tris_getformatname(ast->readformat), ast->readformat,
					ast->writeformat);
			return -1;
		}
	}
...
static int unistim_fixup(struct tris_channel *oldchan, struct tris_channel *newchan)
{
	...
	tris_debug(1, "New owner for channel USTM/%s@%s-%d is %s\n", l->name,
			l->parent->name, p->subtype, newchan->name);

	if (p->owner != oldchan) {
		tris_log(LOG_WARNING, "old channel wasn't %s (%p) but was %s (%p)\n",
...
static void in_band_indication(struct tris_channel *ast, const struct tris_tone_zone *tz,
	const char *indication)
{
	struct tris_tone_zone_sound *ts = NULL;

	if ((ts = tris_get_indication_tone(tz, indication))) {
		tris_playtones_start(ast, 0, ts->data, 1);
		ts = tris_tone_zone_sound_unref(ts);
	} else {
		tris_log(LOG_WARNING, "Unable to get indication tone for %s\n", indication);
	}
}
...
static struct unistim_subchannel *find_subchannel_by_name(const char *dest)
{
	...
	at = strchr(line, '@');
	if (!at) {
		tris_log(LOG_NOTICE, "Device '%s' has no @ (at) sign!\n", dest);
		return NULL;
	}
	*at = '\0';
	at++;
					...
					if (at) {       /* Other options ? */
						...
						if (*at == 'r') {       /* distinctive ring */
							...
							if ((*at < '0') || (*at > '7')) /* ring style */
								tris_log(LOG_WARNING, "Invalid ring selection (%s)", at);
							else {
								char ring_volume = -1;
								...
								if ((*at >= '0') && (*at <= '3'))       /* ring volume */
									ring_volume = *at - '0';
								if (unistimdebug)
									tris_verb(0, "Distinctive ring : style #%d volume %d\n",
...
static int unistim_senddigit_end(struct tris_channel *ast, char digit, unsigned int duration)
{
	struct unistimsession *pte = channel_to_session(ast);
	...
	if (!sub->owner || sub->alreadygone) {
		tris_log(LOG_WARNING, "Unable to find subchannel in dtmf senddigit_end\n");
		return -1;
	}

	if (unistimdebug)
		tris_verb(0, "Send Digit off %c\n", digit);
...
static int unistim_sendtext(struct tris_channel *ast, const char *text)
{
	...
	if (unistimdebug)
		tris_verb(0, "unistim_sendtext called\n");

	if (!text) {
		tris_log(LOG_WARNING, "unistim_sendtext called with a null text\n");
		return 1;
	}
	...
	if (text[0] == '@') {
		...
		while (text[i]) {
			...
			case 1:
				if (cur != '@') {
					tris_log(LOG_WARNING, "sendtext failed : invalid position\n");
					return 1;
				}
				tok = 2;
				continue;
		...
		if (tok != 6) {
			tris_log(LOG_WARNING, "sendtext failed : incomplet command\n");
			return 1;
		}
		if (!pte->device) {
			tris_log(LOG_WARNING, "sendtext failed : no device ?\n");
			return 1;
		}
...
static int unistim_send_mwi_to_peer(struct unistimsession *s, unsigned int tick)
{
	...
	if (event) {
		...
		new = tris_app_has_voicemail(peer->mailbox, "INBOX");
	}
...
static struct tris_channel *unistim_new(struct unistim_subchannel *sub, int state)
{
	...
	if (!sub) {
		tris_log(LOG_WARNING, "subchannel null in unistim_new\n");
		return NULL;
	}
	if (!sub->parent) {
		tris_log(LOG_WARNING, "no line for subchannel %p\n", sub);
		return NULL;
	}
	l = sub->parent;
	tmp = tris_channel_alloc(1, state, l->cid_num, NULL, l->accountcode, l->exten, 
		l->context, l->amaflags, "%s-%08x", l->fullname, (int) (long) sub);
	if (unistimdebug)
		tris_verb(0, "unistim_new sub=%d (%p) chan=%p\n", sub->subtype, sub, tmp);
	if (!tmp) {
	...
	if (unistimdebug)
		tris_verb(0, "Best codec = %d from nativeformats %d (line cap=%d global=%d)\n", fmt,
			 tmp->nativeformats, l->capability, CAPABILITY);
	tris_string_field_build(tmp, name, "USTM/%s@%s-%d", l->name, l->parent->name,
						   sub->subtype);
	if ((sub->rtp) && (sub->subtype == 0)) {
		if (unistimdebug)
			tris_verb(0, "New unistim channel with a previous rtp handle ?\n");
		tmp->fds[0] = tris_rtp_fd(sub->rtp);
		tmp->fds[1] = tris_rtcp_fd(sub->rtp);
	}
	if (sub->rtp)
		tris_jb_configure(tmp, &global_jbconf);
		
/*      tmp->type = type; */
	tris_setstate(tmp, state);
	...
	sub->owner = tmp;
	tris_mutex_lock(&usecnt_lock);
	usecnt++;
	tris_mutex_unlock(&usecnt_lock);
	tris_update_use_count();
	tmp->callgroup = l->callgroup;
	tmp->pickupgroup = l->pickupgroup;
	tris_string_field_set(tmp, call_forward, l->parent->call_forward);
	if (!tris_strlen_zero(l->cid_num)) {
		char *name, *loc, *instr;
		instr = tris_strdup(l->cid_num);
		if (instr) {
			tris_callerid_parse(instr, &name, &loc);
			tmp->cid.cid_num = tris_strdup(loc);
			tmp->cid.cid_name = tris_strdup(name);
			tris_free(instr);
		}
	}
	tmp->priority = 1;
	if (state != TRIS_STATE_DOWN) {
		if (unistimdebug)
			tris_verb(0, "Starting pbx in unistim_new\n");
...
static void *do_monitor(void *data)
{
	...
	if (unistimsock > -1)
		tris_io_add(io, unistimsock, unistimsock_read, TRIS_IO_IN, NULL);
	...
	for (;;) {
		...
		while (cur) {
			...
			if (cur->device) {
				if ((!tris_strlen_zero(cur->device->lines->mailbox)) &&
		...
		res = tris_io_wait(io, res);     /* This function will call unistimsock_read if a packet is received */
		/* Check for a reload request */
		tris_mutex_lock(&unistim_reload_lock);
		reloading = unistim_reloading;
		unistim_reloading = 0;
		tris_mutex_unlock(&unistim_reload_lock);
		if (reloading) {
			tris_verb(1, "Reloading unistim.conf...\n");
			reload_config();
		}
		pthread_testcancel();
	}
...
static int restart_monitor(void)
{
	...
	if (monitor_thread != TRIS_PTHREADT_NULL) {
		...
		/* Start a new monitor */
		if (tris_pthread_create(&monitor_thread, &attr, do_monitor, NULL) < 0) {
			tris_mutex_unlock(&monlock);
...
static struct tris_channel *unistim_request(const char *type, int format, void *data,
										   int *cause, struct tris_channel* src)
{
	int oldformat;
	struct unistim_subchannel *sub;
	...
	tris_log(LOG_NOTICE,
			...
			tris_getformatname(oldformat), CAPABILITY, tris_getformatname(format), format);
	if (!format) {
		tris_log(LOG_NOTICE,
				"Asked to get a channel of unsupported format %s while capability is %s\n",
				tris_getformatname(oldformat), tris_getformatname(CAPABILITY));
		return NULL;
	}

	tris_copy_string(tmp, dest, sizeof(tmp));
	if (tris_strlen_zero(tmp)) {
		tris_log(LOG_NOTICE, "Unistim channels require a device\n");
		return NULL;
	}
	...
	tris_verb(3, "unistim_request(%s)\n", tmp);
	/* Busy ? */
	if (sub->owner) {
		if (unistimdebug)
			tris_verb(0, "Can't create channel : Busy !\n");
		*cause = TRIS_CAUSE_BUSY;
		return NULL;
	}
	sub->parent->capability = format;
	tmpc = unistim_new(sub, TRIS_STATE_DOWN);
	if (!tmpc)
		tris_log(LOG_WARNING, "Unable to make channel for '%s'\n", tmp);
	if (unistimdebug)
		tris_verb(0, "unistim_request owner = %p\n", sub->owner);
	restart_monitor();
...
static char *unistim_info(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_cli(a->fd, "Dumping internal structures :\ndevice\n->line\n-->sub\n");
	while (device) {
		...
		while (line) {
			...
			for (i = 0; i < MAX_SUBS; i++) {
				...
				if (sub->subtype != i)
					tris_cli(a->fd, "Warning ! subchannel->subs[%d] have a subtype=%d\n", i,
							sub->subtype);
				tris_cli(a->fd,
	...
	while (s) {
		tris_cli(a->fd,
				...
				tris_inet_ntoa(s->sin.sin_addr), s->timeout, s->state, s->macaddr,
				s->device, s);
		s = s->next;
	}
	tris_mutex_unlock(&sessionlock);
...
static char *unistim_sp(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	BUFFSEND;
	struct unistim_subchannel *sub;
	int i, j = 0, len;
	...
	tris_copy_string(tmp, a->argv[3] + 5, sizeof(tmp));
	sub = find_subchannel_by_name(tmp);
	if (!sub) {
		tris_cli(a->fd, "Can't find '%s'\n", tmp);
		return CLI_SUCCESS;
	}
	if (!sub->parent->parent->session) {
		tris_cli(a->fd, "'%s' is not connected\n", tmp);
		return CLI_SUCCESS;
	}
	tris_cli(a->fd, "Sending '%s' to %s (%p)\n", a->argv[4], tmp, sub->parent->parent->session);
	for (i = 0; i < len; i++) {
		c = a->argv[4][i];
		if (c >= 'a')
			c -= 'a' - 10;
...
static char *unistim_do_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (!strcasecmp(a->argv[3], "on")) {
		unistimdebug = 1;
		tris_cli(a->fd, "UNISTIM Debugging Enabled\n");
	} else if (!strcasecmp(a->argv[3], "off")) {
		unistimdebug = 0;
		tris_cli(a->fd, "UNISTIM Debugging Disabled\n");
	} else
		return CLI_SHOWUSAGE;
 ...
 * Runs in the trismedia main thread, so don't do anything useful
 * but setting a flag and waiting for do_monitor to do the job
 * in our thread */
static char *unistim_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
...
static struct tris_cli_entry unistim_cli[] = {
	TRIS_CLI_DEFINE(unistim_reload, "Reload UNISTIM configuration"),
	TRIS_CLI_DEFINE(unistim_info, "Show UNISTIM info"),
	TRIS_CLI_DEFINE(unistim_sp, "Send packet (for reverse engineering)"),
	TRIS_CLI_DEFINE(unistim_do_debug, "Toggle UNITSTIM debugging"),
};
...
static int ParseBookmark(const char *text, struct unistim_device *d)
{
	...
	tris_copy_string(line, text, sizeof(line));
	/* Position specified ? */
	if ((len > 2) && (line[1] == '@')) {
		p = line[0];
		if ((p >= '0') && (p <= '5'))
		...
		if (d->softkeyicon[p] != 0) {
			tris_log(LOG_WARNING, "Invalid position %d for bookmark : already used\n:", p);
			return 0;
		}
		...
		if (p > 5) {
			tris_log(LOG_WARNING, "No more free bookmark position\n");
			return 0;
		}
	}
	at = strchr(line, '@');
	if (!at) {
		tris_log(LOG_NOTICE, "Bookmark entry '%s' has no @ (at) sign!\n", text);
		return 0;
	}
	*at = '\0';
	at++;
	...
	if (tris_strlen_zero(number)) {
		tris_log(LOG_NOTICE, "Bookmark entry '%s' has no number\n", text);
		return 0;
	}
	if (tris_strlen_zero(line)) {
		tris_log(LOG_NOTICE, "Bookmark entry '%s' has no description\n", text);
		return 0;
	}
	...
	if (!at)
		...
		*at = '\0';
		at++;
		icon = at;
		if (tris_strlen_zero(icon)) {
			tris_log(LOG_NOTICE, "Bookmark entry '%s' has no icon value\n", text);
			return 0;
		}
		if (strncmp(icon, "USTM/", 5))
			...
			d->softkeyicon[p] = 1;
			tris_copy_string(d->softkeydevice[p], icon + 5, sizeof(d->softkeydevice[p]));
		}
	}
	tris_copy_string(d->softkeylabel[p], line, sizeof(d->softkeylabel[p]));
	tris_copy_string(d->softkeynumber[p], number, sizeof(d->softkeynumber[p]));
	if (unistimdebug)
		tris_verb(0, "New bookmark at pos %d label='%s' number='%s' icon=%x\n",
	...
	while (d) {
		for (i = 0; i < 6; i++) {
			if (d->softkeyicon[i] == 1) {   /* Something for us */
				...
				if (d->sp[i] == NULL)
					tris_log(LOG_NOTICE, "Bookmark entry with device %s not found\n",
							d->softkeydevice[i]);
			}
		}
		d = d->next;
...
static struct unistim_device *build_device(const char *cat, const struct tris_variable *v)
{
	struct unistim_device *d;
	struct unistim_line *l = NULL;
	int create = 1;
	int nbsoftkey, dateformat, timeformat, callhistory;
	char linelabel[TRIS_MAX_EXTENSION];
	char context[TRIS_MAX_EXTENSION];
	...
	while (d) {
		if (!strcmp(d->name, cat)) {
			...
			if (unistimsock < 0) {
				/* It's a dupe */
				tris_log(LOG_WARNING, "Duplicate entry found (%s), ignoring.\n", cat);
				tris_mutex_unlock(&devicelock);
				return NULL;
			}
	...
	if (create) {
		...
		tris_copy_string(d->name, cat, sizeof(d->name));
	}
	tris_copy_string(context, DEFAULTCONTEXT, sizeof(context));
	d->contrast = -1;
	...
	while (v) {
		if (!strcasecmp(v->name, "rtp_port"))
			...
			tris_copy_string(d->id, v->value, sizeof(d->id));
		else if (!strcasecmp(v->name, "tn"))
			tris_copy_string(d->extension_number, v->value, sizeof(d->extension_number));
		else if (!strcasecmp(v->name, "permit") || !strcasecmp(v->name, "deny"))
			d->ha = tris_append_ha(v->name, v->value, d->ha, NULL);
			...
			if ((d->contrast < 0) || (d->contrast > 15)) {
				tris_log(LOG_WARNING, "constrast must be beetween 0 and 15");
				d->contrast = 8;
			}
		} else if (!strcasecmp(v->name, "nat"))
			d->nat = tris_true(v->value);
			...
			if (!strcasecmp(v->value, "asreceived"))
				...
				tris_copy_string(l->cid_num, v->value, sizeof(l->cid_num));
		} else if (!strcasecmp(v->name, "language"))
			tris_copy_string(l->language, v->value, sizeof(l->language));
		else if (!strcasecmp(v->name, "country"))
			tris_copy_string(d->country, v->value, sizeof(d->country));
		else if (!strcasecmp(v->name, "accountcode"))
			tris_copy_string(l->accountcode, v->value, sizeof(l->accountcode));
			...
			tris_copy_string(l->musicclass, v->value, sizeof(l->musicclass));
		else if (!strcasecmp(v->name, "callgroup"))
			l->callgroup = tris_get_group(v->value);
		else if (!strcasecmp(v->name, "pickupgroup"))
			l->pickupgroup = tris_get_group(v->value);
		else if (!strcasecmp(v->name, "mailbox"))
			tris_copy_string(l->mailbox, v->value, sizeof(l->mailbox));
		else if (!strcasecmp(v->name, "parkinglot"))
			tris_copy_string(l->parkinglot, v->value, sizeof(l->parkinglot));
			...
			if (!strcasecmp(v->value, "none"))
				...
				tris_log(LOG_WARNING, "Unknown extension option.\n");
			...
			tris_copy_string(l->name, v->value, sizeof(l->name));
			...
			if (!len)		       /* label is undefined ? */
				tris_copy_string(d->softkeylabel[0], v->value, sizeof(d->softkeylabel[0]));
			else {
				if ((len > 2) && (linelabel[1] == '@')) {
					...
					tris_copy_string(d->softkeylabel[d->softkeylinepos], linelabel + 2,
									sizeof(d->softkeylabel[d->softkeylinepos]));
					d->softkeyicon[d->softkeylinepos] = FAV_ICON_ONHOOK_BLACK;
				} else
					tris_copy_string(d->softkeylabel[0], linelabel,
									sizeof(d->softkeylabel[0]));
			}
			nbsoftkey++;
			tris_copy_string(l->context, context, sizeof(l->context));
	...
	d->tz = tris_get_indication_zone(d->country);
	if ((d->tz == NULL) && !tris_strlen_zero(d->country))
		tris_log(LOG_WARNING, "Country '%s' was not found in indications.conf\n",
				d->country);
	d->datetimeformat = 56 + (dateformat * 4);
	d->datetimeformat += timeformat;
	if (!d->lines) {
		tris_log(LOG_ERROR, "An Unistim device must have at least one line!\n");
		tris_mutex_destroy(&l->lock);
		tris_free(l);
		if (d->tz) {
			d->tz = tris_tone_zone_unref(d->tz);
		}
		tris_free(d);
		return NULL;
	}
	if ((autoprovisioning == AUTOPROVISIONING_TN) &&
		(!tris_strlen_zero(d->extension_number))) {
		d->extension = EXTENSION_TN;
		if (!tris_strlen_zero(d->id))
			tris_log(LOG_WARNING,
					"tn= and device= can't be used together. Ignoring device= entry\n");
		d->id[0] = 'T';		 /* magic : this is a tn entry */
		tris_copy_string((d->id) + 1, d->extension_number, sizeof(d->id) - 1);
		d->extension_number[0] = '\0';
	} else if (tris_strlen_zero(d->id)) {
		if (strcmp(d->name, "template")) {
			tris_log(LOG_ERROR, "You must specify the mac address with device=\n");
			tris_mutex_destroy(&l->lock);
			tris_free(l);
			if (d->tz) {
				d->tz = tris_tone_zone_unref(d->tz);
	...
	if (d->contrast == -1)
		d->contrast = 8;
	if (tris_strlen_zero(d->maintext0))
		strcpy(d->maintext0, "Welcome");
	if (tris_strlen_zero(d->maintext1))
		strcpy(d->maintext1, d->name);
	if (tris_strlen_zero(d->titledefault)) {
		struct tris_tm tm = { 0, };
		struct timeval cur_time = tris_tvnow();

		if ((tris_localtime(&cur_time, &tm, 0)) == 0 || tris_strlen_zero(tm.tm_zone)) {
			display_last_error("Error in tris_localtime()");
			tris_copy_string(d->titledefault, "UNISTIM for*", 12);
		} else {
			if (strlen(tm.tm_zone) < 4) {
				...
				tris_copy_string(d->titledefault, tm.tm_zone, 12);
		}
	}
	...
	if (create) {
		...
		devices = d;
		tris_mutex_unlock(&devicelock);
		tris_verb(3, "Added device '%s'\n", d->name);
	} else {
		tris_verb(3, "Device '%s' reloaded\n", d->name);
	}
	return d;
}
...
static int reload_config(void)
{
	...
	while (v) {
		...
		if (!strcasecmp(v->name, "keepalive"))
			...
			if (!strcasecmp(v->value, "no"))
				...
				tris_log(LOG_WARNING, "Unknown autoprovisioning option.\n");
	...
	if ((unistim_keepalive < 10) ||
		...
		tris_log(LOG_ERROR, "keepalive is invalid in %s\n", config);
	...
	if ((unistim_port < 1) || (unistim_port > 65535)) {
		tris_log(LOG_ERROR, "port is not set or invalid in %s\n", config);
		tris_config_destroy(cfg);
		return -1;
	}
	...
	while (cat) {
		if (strcasecmp(cat, "general")) {
			d = build_device(cat, tris_variable_browse(cfg, cat));
		}
	...
	while (d) {
		if (d->to_delete) {
			...
			if (unistimdebug)
				tris_verb(0, "Removing device '%s'\n", d->name);
			if (!d->lines) {
				tris_log(LOG_ERROR, "Device '%s' without a line !, aborting\n", d->name);
				tris_config_destroy(cfg);
				return 0;
			}
			if (!d->lines->subs[0]) {
				tris_log(LOG_ERROR, "Device '%s' without a subchannel !, aborting\n",
						d->name);
				tris_config_destroy(cfg);
				return 0;
			}
			...
			tris_mutex_destroy(&d->lines->subs[0]->lock);
			tris_free(d->lines->subs[0]);
			...
			tris_mutex_destroy(&d->lines->lock);
			tris_free(d->lines);
			if (d->session) {
				...
				tris_mutex_destroy(&d->session->lock);
				tris_free(d->session);
			}
			if (devices == d)
				devices = d->next;
	...
	if (unistimsock < 0) {
		tris_log(LOG_WARNING, "Unable to create UNISTIM socket: %s\n", strerror(errno));
		return -1;
	}
#ifdef HAVE_PKTINFO
	{
	...
	if (bind(unistimsock, (struct sockaddr *) &bindaddr, sizeof(bindaddr)) < 0) {
		tris_log(LOG_WARNING, "Failed to bind to %s:%d: %s\n",
				tris_inet_ntoa(bindaddr.sin_addr), htons(bindaddr.sin_port),
				strerror(errno));
		close(unistimsock);
		unistimsock = -1;
	} else {
		tris_verb(2, "UNISTIM Listening on %s:%d\n", tris_inet_ntoa(bindaddr.sin_addr), htons(bindaddr.sin_port));
		tris_netsock_set_qos(unistimsock, qos.tos, qos.cos, "UNISTIM");
	}
	return 0;
}

static enum tris_rtp_get_result unistim_get_vrtp_peer(struct tris_channel *chan, 
	struct tris_rtp **rtp)
{
	return TRIS_RTP_TRY_NATIVE;
}

static enum tris_rtp_get_result unistim_get_rtp_peer(struct tris_channel *chan, 
	struct tris_rtp **rtp)
{
	struct unistim_subchannel *sub;
	...
	if (unistimdebug)
		tris_verb(0, "unistim_get_rtp_peer called\n");
...
static int unistim_set_rtp_peer(struct tris_channel *chan, struct tris_rtp *rtp, 
	struct tris_rtp *vrtp, struct tris_rtp *trtp, int codecs, int nat_active)
{
	struct unistim_subchannel *sub;

	if (unistimdebug)
		tris_verb(0, "unistim_set_rtp_peer called\n");
...
static struct tris_rtp_protocol unistim_rtp = {
	.type = channel_type,
...
int load_module(void)
{
	...
	if (!(buff = tris_malloc(SIZE_PAGE)))
		goto buff_failed;

	io = io_context_create();
	if (!io) {
		tris_log(LOG_ERROR, "Failed to allocate IO context\n");
		goto io_failed;
	}

	sched = sched_context_create();
	if (!sched) {
		tris_log(LOG_ERROR, "Failed to allocate scheduler context\n");
		goto sched_failed;
	}
	...
	if (tris_channel_register(&unistim_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel type '%s'\n", channel_type);
		goto chanreg_failed;
	} 

	tris_rtp_proto_register(&unistim_rtp);

	tris_cli_register_multiple(unistim_cli, ARRAY_LEN(unistim_cli));
...
static int unload_module(void)
{
	...
	tris_cli_unregister_multiple(unistim_cli, ARRAY_LEN(unistim_cli));

	tris_channel_unregister(&unistim_tech);
	tris_rtp_proto_unregister(&unistim_rtp);
...
/*! reload: Part of Trismedia module interface ---*/
int reload(void)
{
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "UNISTIM Protocol (USTM)",
    .load = load_module,
