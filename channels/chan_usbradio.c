 ...
 *	}
 *
 * XXX NOTE these macros should NOT be replicated in other parts of trismedia. 
...
#define M_BOOL(tag, dst)	M_F(tag, (dst) = tris_true(__val) )
#define M_UINT(tag, dst)	M_F(tag, (dst) = strtoul(__val, NULL, 0) )
#define M_STR(tag, dst)		M_F(tag, tris_copy_string(dst, __val, sizeof(dst)))
...
static struct sound sounds[] = {
	{ TRIS_CONTROL_RINGING, "RINGING", ringtone, sizeof(ringtone)/2, 16000, 32000, 1 },
	{ TRIS_CONTROL_BUSY, "BUSY", busy, sizeof(busy)/2, 4000, 4000, 1 },
	{ TRIS_CONTROL_CONGESTION, "CONGESTION", busy, sizeof(busy)/2, 2000, 2000, 1 },
	{ TRIS_CONTROL_RING, "RING10", ring10, sizeof(ring10)/2, 16000, 32000, 1 },
	{ TRIS_CONTROL_ANSWER, "ANSWER", answer, sizeof(answer)/2, 2200, 0, 0 },
...
#define	BOOST_MAX	40			/* slightly less than 7 bits */
	...
	char ext[TRIS_MAX_EXTENSION];
	char ctx[TRIS_MAX_CONTEXT];
	...
	/* buffers used in usbradio_read - TRIS_FRIENDLY_OFFSET space for headers
	 * plus enough room for a full frame
	 */
	char usbradio_read_buf[FRAME_SIZE * (2 * 12) + TRIS_FRIENDLY_OFFSET];
	char usbradio_read_buf_8k[FRAME_SIZE * 2 + TRIS_FRIENDLY_OFFSET];
	int readpos;				/* read position above */
	struct tris_frame read_f;	/* returned by usbradio_read */
...
static struct chan_usbradio_pvt usbradio_default = {
	...
	.ext = "s",
	.ctx = "default",
	.readpos = TRIS_FRIENDLY_OFFSET,	/* start here on reads */
	.lastopen = { 0, 0 },
	.boost = BOOST_SCALE,
	.wanteeprom = 1,
	.area = 0,
...
static struct tris_channel *usbradio_request(const char *type, int format, void *data
, int *cause, struct tris_channel* src);
static int usbradio_digit_begin(struct tris_channel *c, char digit);
static int usbradio_digit_end(struct tris_channel *c, char digit, unsigned int duration);
static int usbradio_text(struct tris_channel *c, const char *text);
static int usbradio_hangup(struct tris_channel *c);
static int usbradio_answer(struct tris_channel *c);
static struct tris_frame *usbradio_read(struct tris_channel *chan);
static int usbradio_call(struct tris_channel *c, char *dest, int timeout);
static int usbradio_write(struct tris_channel *chan, struct tris_frame *f);
static int usbradio_indicate(struct tris_channel *chan, int cond, const void *data, size_t datalen);
static int usbradio_fixup(struct tris_channel *oldchan, struct tris_channel *newchan);
...
static const struct tris_channel_tech usbradio_tech = {
	.type = "Radio",
	.description = tdesc,
...
static int hid_device_mklist(void)
{
    ...
    for (usb_bus = usb_busses;
                  ...
                  == C108_PRODUCT_ID))
		{
			...
			if (i >= 32) return -1;
			usb_device_list = tris_realloc(usb_device_list,
...
static void *hidthread(void *arg)
{
	...
	if (usb_dev == NULL) {
		tris_log(LOG_ERROR,"USB HID device not found\n");
		pthread_exit(NULL);
	}
	usb_handle = usb_open(usb_dev);
	if (usb_handle == NULL) {
	        tris_log(LOG_ERROR,"Not able to open USB device\n");
		pthread_exit(NULL);
	}
	if (usb_claim_interface(usb_handle,C108_HID_INTERFACE) < 0)
	{
	    if (usb_detach_kernel_driver_np(usb_handle,C108_HID_INTERFACE) < 0) {
		        tris_log(LOG_ERROR,"Not able to detach the USB device\n");
			pthread_exit(NULL);
		}
		if (usb_claim_interface(usb_handle,C108_HID_INTERFACE) < 0) {
		        tris_log(LOG_ERROR,"Not able to claim the USB device\n");
			pthread_exit(NULL);
		}
	}
	memset(buf,0,sizeof(buf));
	...
	if (pipe(o->pttkick) == -1)
	{
	    tris_log(LOG_ERROR,"Not able to create pipe\n");
		pthread_exit(NULL);
	}
	...
	while(!o->stophid)
	{
		...
		/* tris_select emulates linux behaviour in terms of timeout handling */
		res = tris_select(o->pttkick[0] + 1, &rfds, NULL, NULL, &to);
		if (res < 0) {
		...
		if(o->wanteeprom)
		{
			...
			if (o->eepromctl == 1)  /* to read */
			{
				...
				if (!get_eeprom(usb_handle,o->eeprom))
				{
					...
					{
						tris_log(LOG_NOTICE,"UNSUCCESSFUL: EEPROM MAGIC NUMBER BAD on channel %s\n",o->name);
					}
					else
					{
						...
						tris_log(LOG_NOTICE,"EEPROM Loaded on channel %s\n",o->name);
					}
				}
				else
				{
					tris_log(LOG_NOTICE,"USB Adapter has no EEPROM installed or Checksum BAD on channel %s\n",o->name);
				}
			...
			if (o->eepromctl == 2) /* to write */
			{
				...
				hid_set_outputs(usb_handle,bufsave);
				tris_log(LOG_NOTICE,"USB Parameters written to EEPROM on %s\n",o->name);
			}
			o->eepromctl = 0;
			tris_mutex_unlock(&o->eepromlock);
		}
 ...
 */
#if	0
static char *tris_ext_ctx(const char *src, char **ext, char **ctx)
{
...
static int used_blocks(struct chan_usbradio_pvt *o)
{
	...
	if (ioctl(o->sounddev, SNDCTL_DSP_GETOSPACE, &info)) {
		if (!(o->warned & WARN_used_blocks)) {
			tris_log(LOG_WARNING, "Error reading output space\n");
	...
	if (o->total_blocks == 0) {
		if (0)					/* debugging */
			tris_log(LOG_WARNING, "fragtotal %d size %d avail %d\n", info.fragstotal, info.fragsize, info.fragments);
...
static int soundcard_writeframe(struct chan_usbradio_pvt *o, short *data)
{
	...
	//  filling the buffer when trismedia clock > usb sound clock
	...
	if (res > o->queuesize) {	/* no room to write a block */
	    // tris_log(LOG_WARNING, "sound device write buffer overflow\n");
		if (o->w_errors++ == 0 && (usbradio_debug & 0x4))
			tris_log(LOG_WARNING, "write: used %d blocks (%d)\n", res, o->w_errors);
		return 0;
	}
	o->w_errors = 0;
...
static void send_sound(struct chan_usbradio_pvt *o)
{
	...
	for (ofs = 0; ofs < FRAME_SIZE; ofs += l) {
		...
		if (l > 0) {
			...
			if (0)
				tris_log(LOG_WARNING, "send_sound sound %d/%d of %d into %d\n", l_sampsent, l, s->samplen, ofs);
			l_sampsent += l;
...
static void *sound_thread(void *arg)
{
	...
	for (;;) {
		...
		/* tris_select emulates linux behaviour in terms of timeout handling */
		res = tris_select(maxfd + 1, &rfds, &wfds, NULL, NULL);
		if (res < 1) {
			tris_log(LOG_WARNING, "select failed: %s\n", strerror(errno));
			sleep(1);
			continue;
		...
		if (FD_ISSET(o->sndcmd[0], &rfds)) {
			...
			if (sounds[i].ind == -1)
				tris_log(LOG_WARNING, "invalid sound index: %d\n", what);
		}
		if (o->sounddev > -1) {
...
static int setformat(struct chan_usbradio_pvt *o, int mode)
{
	...
	if (res < 0) {
		tris_log(LOG_WARNING, "Unable to set format to 16-bit signed\n");
		return -1;
	}
	switch (mode) {
		case O_RDWR:
			...
			if (res == 0 && (fmt & DSP_CAP_DUPLEX)) {
				if (option_verbose > 1)
					tris_verbose(VERBOSE_PREFIX_2 "Console is full duplex\n");
				o->duplex = M_FULL;
			};
			break;
		case O_WRONLY:
	...
	if (res < 0) {
		tris_log(LOG_WARNING, "Failed to set audio device to mono\n");
		return -1;
	}
...
static int usbradio_text(struct tris_channel *c, const char *text)
{
	...
	if(o->debuglevel)tris_verbose(" << Console Received usbradio text %s >> \n", text);
    ...
    { 
		...
		ppbinout(chan);
        if(o->debuglevel)tris_log(LOG_NOTICE,"parse usbradio SETCHAN cmd: %s chan: %i\n",text,chan);
        return 0;
    }
	
    if (cnt < 6)
    {
	    tris_log(LOG_ERROR,"Cannot parse usbradio text: %s\n",text);
	    return 0;
    }
	else
	{
		if(o->debuglevel)tris_verbose(" << %s %s %s %s %s %c >> \n", cmd,rxs,txs,rxpl,txpl,pwr);	
	}
    
    if (strcmp(cmd,"SETFREQ")==0)
    {
        if(o->debuglevel)tris_log(LOG_NOTICE,"parse usbradio SETFREQ cmd: %s\n",text);
		tx=strtod(txs,NULL);
		rx=strtod(rxs,NULL);
	...
	tris_log(LOG_ERROR,"Cannot parse usbradio cmd: %s\n",text);
	return 0;
}
...
static int usbradio_call(struct tris_channel *c, char *dest, int timeout)
{
	...
	o->stophid = 0;
	time(&o->lasthidtime);
	tris_pthread_create_background(&o->hidthread, NULL, hidthread, o);
...
static int usbradio_hangup(struct tris_channel *c)
{
	...
	//tris_log(LOG_NOTICE, "usbradio_hangup()\n");
#ifndef	NEW_TRISMEDIA
	o->cursound = -1;
	o->nosound = 0;
#endif
	...
	if (o->hookstate) {
		if (o->autoanswer || o->autohangup) {
			...
			ring(o, TRIS_CONTROL_CONGESTION);
		}
	}
	o->stophid = 1;
...
static int usbradio_write(struct tris_channel *c, struct tris_frame *f)
{
	struct chan_usbradio_pvt *o = c->tech_pvt;
...
static struct tris_frame *usbradio_read(struct tris_channel *c)
{
	int res, src, datalen, oldpttout;
	int cd,sd;
	struct chan_usbradio_pvt *o = c->tech_pvt;
	struct tris_frame *f = &o->read_f,*f1;
	struct tris_frame wf = { TRIS_FRAME_CONTROL };
	time_t now;
	...
	if (o->lasthidtime)
	{
		...
		{
			tris_log(LOG_ERROR,"HID process has died or something!!\n");
			return NULL;
		}
	}
	/* XXX can be simplified returning &tris_null_frame */
	/* prepare a NULL frame in case we don't have enough data to return */
	memset(f, '\0', sizeof(struct tris_frame));
	...
	if (res < 0)				/* audio data not ready, return a NULL frame */
	{
		...
		{
			tris_log(LOG_ERROR,"Stuck USB read channel [%s], un-sticking it!\n",o->name);
			o->readerrs = 0;
			return NULL;
		}
		if (o->readerrs == 1) 
			tris_log(LOG_WARNING,"Possibly stuck USB read channel. [%s]\n",o->name);
		return f;
	}
	if (o->readerrs) tris_log(LOG_WARNING,"Nope, USB read channel [%s] wasn't stuck after all.\n",o->name);
	o->readerrs = 0;
	o->readpos += res;
	...
	#if DEBUG_CAPTURES == 1
	if ((o->b.rxcapraw && frxcapraw) && (fwrite((o->usbradio_read_buf + TRIS_FRIENDLY_OFFSET),1,FRAME_SIZE * 2 * 2 * 6,frxcapraw) != FRAME_SIZE * 2 * 2 * 6)) {
		tris_log(LOG_ERROR, "fwrite() failed: %s\n", strerror(errno));
	}
	#endif
	...
	if(o->txkeyed||o->txtestkey)
	{
		...
		{
			o->pmrChan->txPttIn=1;
			if(o->debuglevel) tris_log(LOG_NOTICE,"txPttIn = %i, chan %s\n",o->pmrChan->txPttIn,o->owner->name);
		}
	}
	...
	PmrRx(         o->pmrChan, 
		   (i16 *)(o->usbradio_read_buf + TRIS_FRIENDLY_OFFSET),
		   (i16 *)(o->usbradio_read_buf_8k + TRIS_FRIENDLY_OFFSET),
		   (i16 *)(o->usbradio_write_buf_1));

	if (oldpttout != o->pmrChan->txPttOut)
	{
		if(o->debuglevel) tris_log(LOG_NOTICE,"txPttOut = %i, chan %s\n",o->pmrChan->txPttOut,o->owner->name);
		kickptt(o);
	}
    ...
    if (frxoutraw) fwrite((o->usbradio_read_buf_8k + TRIS_FRIENDLY_OFFSET),1,FRAME_SIZE * 2,frxoutraw);
	#endif
	...
	{													
		if(o->debuglevel)tris_log(LOG_NOTICE,"rxdcsdecode = %s, chan %s\n",o->pmrChan->rxctcssfreq,o->owner->name);
		// printf("rxctcssdecode = %i, chan %s\n",o->pmrChan->rxCtcss->decode,o->owner->name);
	...
	{								
		if(o->debuglevel)tris_log(LOG_NOTICE,"rxLSDecode = %s, chan %s\n",o->pmrChan->rxctcssfreq,o->owner->name);
	...
	if ( cd && sd )
	{
		...
		if(!o->rxkeyed && o->debuglevel)tris_log(LOG_NOTICE,"o->rxkeyed = 1, chan %s\n", o->owner->name);
		o->rxkeyed = 1;
	}
	else 
	{
		//if(o->rxkeyed)o->pmrChan->dd.b.doitnow=1;
		if(o->rxkeyed && o->debuglevel)tris_log(LOG_NOTICE,"o->rxkeyed = 0, chan %s\n",o->owner->name);
		o->rxkeyed = 0;
	}
	...
	{
		o->lastrx = 0;
		//printf("TRIS_CONTROL_RADIO_UNKEY\n");
		wf.subclass = TRIS_CONTROL_RADIO_UNKEY;
		tris_queue_frame(o->owner, &wf);
	}
	else if ((!o->lastrx) && (o->rxkeyed))
	{
		o->lastrx = 1;
		//printf("TRIS_CONTROL_RADIO_KEY\n");
		wf.subclass = TRIS_CONTROL_RADIO_KEY;
		if(o->rxctcssdecode)  	
        {
	        wf.data.ptr = o->rxctcssfreq;
	        wf.datalen = strlen(o->rxctcssfreq) + 1;
			TRACEO(1,("TRIS_CONTROL_RADIO_KEY text=%s\n",o->rxctcssfreq));
        }
		tris_queue_frame(o->owner, &wf);
	}

	o->readpos = TRIS_FRIENDLY_OFFSET;	/* reset read pointer for next frame */
	if (c->_state != TRIS_STATE_UP)	/* drop data if frame is not up */
		return f;
	...
	f->data.ptr = o->usbradio_read_buf_8k + TRIS_FRIENDLY_OFFSET;
	...
	if (o->dsp)
	{
	    f1 = tris_dsp_process(c,o->dsp,f);
	    if ((f1->frametype == TRIS_FRAME_DTMF_END) ||
	      (f1->frametype == TRIS_FRAME_DTMF_BEGIN))
	    {
		if ((f1->subclass == 'm') || (f1->subclass == 'u'))
		{
			f1->frametype = TRIS_FRAME_NULL;
			f1->subclass = 0;
			return(f1);
		}
		if (f1->frametype == TRIS_FRAME_DTMF_END)
			tris_log(LOG_NOTICE,"Got DTMF char %c\n",f1->subclass);
		return(f1);
	    }
	}
	return f;
...
static int usbradio_fixup(struct tris_channel *oldchan, struct tris_channel *newchan)
{
	...
	tris_log(LOG_WARNING,"usbradio_fixup()\n");
	o->owner = newchan;
	return 0;
}

static int usbradio_indicate(struct tris_channel *c, int cond, const void *data, size_t datalen)
{
	struct chan_usbradio_pvt *o = c->tech_pvt;
	int res = -1;
	...
	switch (cond) {
		...
		case TRIS_CONTROL_HOLD:
			...
			tris_moh_start(c, data, o->mohinterpret);
			break;
		...
		case TRIS_CONTROL_PROCEEDING:
			tris_verbose(" << Call Proceeding... >> \n");
			tris_moh_stop(c);
			break;
		case TRIS_CONTROL_PROGRESS:
			tris_verbose(" << Call Progress... >> \n");
			tris_moh_stop(c);
			break;
		case TRIS_CONTROL_RADIO_KEY:
			o->txkeyed = 1;
			if(o->debuglevel)tris_verbose(" << TRIS_CONTROL_RADIO_KEY Radio Transmit On. >> \n");
			break;
		case TRIS_CONTROL_RADIO_UNKEY:
			o->txkeyed = 0;
			if(o->debuglevel)tris_verbose(" << TRIS_CONTROL_RADIO_UNKEY Radio Transmit Off. >> \n");
			break;
		default:
 ...
 */
static struct tris_channel *usbradio_new(struct chan_usbradio_pvt *o, char *ext, char *ctx, int state)
{
	struct tris_channel *c;

	c = tris_channel_alloc(1, state, o->cid_num, o->cid_name, "", ext, ctx, 0, "Radio/%s", o->name);
	if (c == NULL)
		return NULL;
	...
	c->nativeformats = TRIS_FORMAT_SLINEAR;
	c->readformat = TRIS_FORMAT_SLINEAR;
	c->writeformat = TRIS_FORMAT_SLINEAR;
	c->tech_pvt = o;

	if (!tris_strlen_zero(o->language))
		tris_string_field_set(c, language, o->language);
	/* Don't use tris_set_callerid() here because it will
	 * generate a needless NewCallerID event */
	c->cid.cid_num = tris_strdup(o->cid_num);
	c->cid.cid_ani = tris_strdup(o->cid_num);
	c->cid.cid_name = tris_strdup(o->cid_name);
	if (!tris_strlen_zero(ext))
		c->cid.cid_dnid = tris_strdup(ext);
...
/*
*/
static struct tris_channel *usbradio_request(const char *type, int format, void *data, int *cause, struct tris_channel* src)
{
	struct tris_channel *c;
	...
	if (0)
	{
		tris_log(LOG_WARNING, "usbradio_request type <%s> data 0x%p <%s>\n", type, data, (char *) data);
	}
	if (o == NULL) {
		tris_log(LOG_NOTICE, "Device %s not found\n", (char *) data);
		/* XXX we could default to 'dsp' perhaps ? */
		return NULL;
	}
	if ((format & TRIS_FORMAT_SLINEAR) == 0) {
		tris_log(LOG_NOTICE, "Format 0x%x unsupported\n", format);
		return NULL;
	}
	if (o->owner) {
		tris_log(LOG_NOTICE, "Already have a call (chan %p) on the usb channel\n", o->owner);
		*cause = TRIS_CAUSE_BUSY;
		return NULL;
	}
	c = usbradio_new(o, NULL, NULL, TRIS_STATE_DOWN);
	if (c == NULL) {
		tris_log(LOG_WARNING, "Unable to create new usb channel\n");
		return NULL;
	}
...
static int radio_tune(int fd, int argc, char *argv[])
{
	...
	{
		tris_cli(fd,"Active radio interface is [%s]\n",usbradio_active);
		tris_cli(fd,"Output A is currently set to ");
		if(o->txmixa==TX_OUT_COMPOSITE)tris_cli(fd,"composite.\n");
		else if (o->txmixa==TX_OUT_VOICE)tris_cli(fd,"voice.\n");
		else if (o->txmixa==TX_OUT_LSD)tris_cli(fd,"tone.\n");
		else if (o->txmixa==TX_OUT_AUX)tris_cli(fd,"auxvoice.\n");
		else tris_cli(fd,"off.\n");

		tris_cli(fd,"Output B is currently set to ");
		if(o->txmixb==TX_OUT_COMPOSITE)tris_cli(fd,"composite.\n");
		else if (o->txmixb==TX_OUT_VOICE)tris_cli(fd,"voice.\n");
		else if (o->txmixb==TX_OUT_LSD)tris_cli(fd,"tone.\n");
		else if (o->txmixb==TX_OUT_AUX)tris_cli(fd,"auxvoice.\n");
		else tris_cli(fd,"off.\n");

		tris_cli(fd,"Tx Voice Level currently set to %d\n",o->txmixaset);
		tris_cli(fd,"Tx Tone Level currently set to %d\n",o->txctcssadj);
		tris_cli(fd,"Rx Squelch currently set to %d\n",o->rxsquelchadj);
		tris_cli(fd,"Device String is %s\n",o->devstr);
		return RESULT_SHOWUSAGE;
	}
	...
	else if (!strcasecmp(argv[2],"rxsquelch"))
	{
		if (argc == 3)
		{
		    tris_cli(fd,"Current Signal Strength is %d\n",((32767-o->pmrChan->rxRssi)*1000/32767));
		    tris_cli(fd,"Current Squelch setting is %d\n",o->rxsquelchadj);
			//tris_cli(fd,"Current Raw RSSI        is %d\n",o->pmrChan->rxRssi);
		    //tris_cli(fd,"Current (real) Squelch setting is %d\n",*(o->pmrChan->prxSquelchAdjust));
		} else {
			i = atoi(argv[3]);
			if ((i < 0) || (i > 999)) return RESULT_SHOWUSAGE;
			tris_cli(fd,"Changed Squelch setting to %d\n",i);
			o->rxsquelchadj = i;
		...
		{
			tris_log(LOG_ERROR,"No txvoice output configured.\n");
		}
		else if (argc == 3)
		{
			if((o->txmixa==TX_OUT_VOICE)||(o->txmixa==TX_OUT_COMPOSITE))
				tris_cli(fd,"Current txvoice setting on Channel A is %d\n",o->txmixaset);
			else
				tris_cli(fd,"Current txvoice setting on Channel B is %d\n",o->txmixbset);
		}
		else
		{
			i = atoi(argv[3]);
			...
			{
			 	o->txmixaset=i;
				tris_cli(fd,"Changed txvoice setting on Channel A to %d\n",o->txmixaset);
			}
			else
			{
			 	o->txmixbset=i;   
				tris_cli(fd,"Changed txvoice setting on Channel B to %d\n",o->txmixbset);
			}
			mixer_write(o);
			mult_set(o);
			tris_cli(fd,"Changed Tx Voice Output setting to %d\n",i);
		}
		...
		{
			tris_log(LOG_WARNING,"No auxvoice output configured.\n");
		}
		else if (argc == 3)
		{
			if(o->txmixa==TX_OUT_AUX)
				tris_cli(fd,"Current auxvoice setting on Channel A is %d\n",o->txmixaset);
			else
				tris_cli(fd,"Current auxvoice setting on Channel B is %d\n",o->txmixbset);
		}
		else
		{
			i = atoi(argv[3]);
			...
			{
				o->txmixbset=i;
				tris_cli(fd,"Changed auxvoice setting on Channel A to %d\n",o->txmixaset);
			}
			else
			{
				o->txmixbset=i;
				tris_cli(fd,"Changed auxvoice setting on Channel B to %d\n",o->txmixbset);
			}
			mixer_write(o);
			mult_set(o);
		}
	...
	{
		if (argc == 3)
			tris_cli(fd,"Current Tx CTCSS modulation setting = %d\n",o->txctcssadj);
		else
		{
			i = atoi(argv[3]);
			if ((i < 0) || (i > 999)) return RESULT_SHOWUSAGE;
			o->txctcssadj = i;
			set_txctcss_level(o);
			tris_cli(fd,"Changed Tx CTCSS modulation setting to %i\n",i);
		}
		o->txtestkey=1;
		usleep(5000000);
		o->txtestkey=0;
	...
	{
		tris_cli(fd,"File capture (trace) was rx=%d tx=%d and now off.\n",o->b.rxcap2,o->b.txcap2);
		tris_cli(fd,"File capture (raw)   was rx=%d tx=%d and now off.\n",o->b.rxcapraw,o->b.txcapraw);
	...
	else if (!strcasecmp(argv[2],"load"))
	{
		...
		tris_cli(fd,"Requesting loading of tuning settings from EEPROM for channel %s\n",o->name);
	}
	else
	{
...
static int radio_set_debug_off(int fd, int argc, char *argv[])
{
	...
	o->debuglevel=0;
	tris_cli(fd,"usbradio debug off.\n");
	return RESULT_SUCCESS;
}
...
{
        if (argc == 2)
                tris_cli(fd, "active (command) USB Radio device is [%s]\n", usbradio_active);
        else if (argc != 3)
                return RESULT_SHOWUSAGE;
        else {
                ...
                if (strcmp(argv[2], "show") == 0) {
                        for (o = usbradio_default.next; o; o = o->next)
                                tris_cli(fd, "device [%s] exists\n", o->name);
                        return RESULT_SUCCESS;
                }
                o = find_desc(argv[2]);
                if (o == NULL)
                        tris_cli(fd, "No device [%s] exists\n", argv[2]);
                else
				{
...
static int radio_set_xpmr_debug(int fd, int argc, char *argv[])
{
	...
	tris_cli(fd,"usbradio xdebug on tracelevel %i\n",o->pmrChan->tracelevel);
...
static struct tris_cli_entry cli_usbradio[] = {
	{ { "radio", "key", NULL },
...
{
	tris_callerid_split(s, o->cid_name, sizeof(o->cid_name), o->cid_num, sizeof(o->cid_num));
}
#endif
...
static void store_rxdemod(struct chan_usbradio_pvt *o, char *s)
{
	if (!strcasecmp(s,"no")){
		...
		tris_log(LOG_WARNING,"Unrecognized rxdemod parameter: %s\n",s);
	}

	//tris_log(LOG_WARNING, "set rxdemod = %s\n", s);
}
...
static void store_txmixa(struct chan_usbradio_pvt *o, char *s)
{
	if (!strcasecmp(s,"no")){
		...
		tris_log(LOG_WARNING,"Unrecognized txmixa parameter: %s\n",s);
	}

	//tris_log(LOG_WARNING, "set txmixa = %s\n", s);
}
...
static void store_txmixb(struct chan_usbradio_pvt *o, char *s)
{
	if (!strcasecmp(s,"no")){
		...
		tris_log(LOG_WARNING,"Unrecognized txmixb parameter: %s\n",s);
	}

	//tris_log(LOG_WARNING, "set txmixb = %s\n", s);
}
/*
*/
static void store_rxcdtype(struct chan_usbradio_pvt *o, char *s)
{
	if (!strcasecmp(s,"no")){
		...
		tris_log(LOG_WARNING,"Unrecognized rxcdtype parameter: %s\n",s);
	}

	//tris_log(LOG_WARNING, "set rxcdtype = %s\n", s);
}
/*
*/
static void store_rxsdtype(struct chan_usbradio_pvt *o, char *s)
{
	if (!strcasecmp(s,"no") || !strcasecmp(s,"SD_IGNORE")){
		...
		tris_log(LOG_WARNING,"Unrecognized rxsdtype parameter: %s\n",s);
	}

	//tris_log(LOG_WARNING, "set rxsdtype = %s\n", s);
}
/*
*/
static void store_rxgain(struct chan_usbradio_pvt *o, char *s)
{
	...
	sscanf(s, "%30f", &f); 
	o->rxgain = f;
	//tris_log(LOG_WARNING, "set rxgain = %f\n", f);
}
/*
*/
static void store_rxvoiceadj(struct chan_usbradio_pvt *o, char *s)
{
	...
	sscanf(s, "%30f", &f);
	o->rxvoiceadj = f;
	//tris_log(LOG_WARNING, "set rxvoiceadj = %f\n", f);
}
/*
*/
static void store_rxctcssadj(struct chan_usbradio_pvt *o, char *s)
{
	...
	sscanf(s, "%30f", &f);
	o->rxctcssadj = f;
	//tris_log(LOG_WARNING, "set rxctcssadj = %f\n", f);
}
/*
*/
static void store_txtoctype(struct chan_usbradio_pvt *o, char *s)
{
	if (!strcasecmp(s,"no") || !strcasecmp(s,"TOC_NONE")){
		...
		tris_log(LOG_WARNING,"Unrecognized txtoctype parameter: %s\n",s);
	}
}
/*
*/
static void tune_txoutput(struct chan_usbradio_pvt *o, int value, int fd)
{
	...
	if (fd > 0) tris_cli(fd,"Tone output starting on channel %s...\n",o->name);
	usleep(5000000);
	TxTestTone(o->pmrChan, 0);
	if (fd > 0) tris_cli(fd,"Tone output ending on channel %s...\n",o->name);
	o->pmrChan->txPttIn=0;
	o->txtestkey=0;
}
/*
...
static void tune_rxinput(int fd, struct chan_usbradio_pvt *o)
{
    ...
    tris_cli(fd,"tune rxnoise maxtries=%i, target=%i, tolerance=%i\n",maxtries,target,tolerance);
	...
	while(tries<maxtries)
	{
        ...
        if(!meas)meas++;
		tris_cli(fd,"tries=%i, setting=%i, meas=%i\n",tries,setting,meas);
	...
	tris_cli(fd,"DONE tries=%i, setting=%i, meas=%i\n",tries,
		(setting * 1000) / o->micmax,meas);
	if( meas<(target-tolerance) || meas>(target+tolerance) ){
		tris_cli(fd,"ERROR: RX INPUT ADJUST FAILED.\n");
	}else{
		tris_cli(fd,"INFO: RX INPUT ADJUST SUCCESS.\n");	
...
static void tune_rxvoice(int fd, struct chan_usbradio_pvt *o)
{
	...
	tris_cli(fd,"INFO: RX VOICE ADJUST START.\n");	
	tris_cli(fd,"target=%i tolerance=%i \n",target,tolerance);

	o->pmrChan->b.tuning=1;
	if(!o->pmrChan->spsMeasure)
		tris_cli(fd,"ERROR: NO MEASURE BLOCK.\n");

	if(!o->pmrChan->spsMeasure->source || !o->pmrChan->prxVoiceAdjust )
		tris_cli(fd,"ERROR: NO SOURCE OR MEASURE SETTING.\n");
	...
	while(tries<maxtries)
	{
		*(o->pmrChan->prxVoiceAdjust)=setting*M_Q8;
		...
		usleep(1000000);
		meas = o->pmrChan->spsMeasure->apeak;
		tris_cli(fd,"tries=%i, setting=%f, meas=%i\n",tries,setting,meas);
	...
	tris_cli(fd,"DONE tries=%i, setting=%f, meas=%f\n",tries,setting,(float)meas);
	if( meas<(target-tolerance) || meas>(target+tolerance) ){
		tris_cli(fd,"ERROR: RX VOICE GAIN ADJUST FAILED.\n");
	}else{
		tris_cli(fd,"INFO: RX VOICE GAIN ADJUST SUCCESS.\n");
...
static void tune_rxctcss(int fd, struct chan_usbradio_pvt *o)
{
	...
	tris_cli(fd,"INFO: RX CTCSS ADJUST START.\n");	
	tris_cli(fd,"target=%i tolerance=%i \n",target,tolerance);
	...
	tris_cli(fd,"DONE tries=%i, setting=%f, meas=%f\n",tries,setting,(float)meas);
	if( meas<(target-tolerance) || meas>(target+tolerance) ){
		tris_cli(fd,"ERROR: RX CTCSS GAIN ADJUST FAILED.\n");
	}else{
		tris_cli(fd,"INFO: RX CTCSS GAIN ADJUST SUCCESS.\n");
...
static void tune_write(struct chan_usbradio_pvt *o)
{
	...
	char fname[200];
 	...
 	snprintf(fname,sizeof(fname) - 1,"/etc/trismedia/usbradio_tune_%s.conf",o->name);
	fp = fopen(fname,"w");
...
static int xpmr_config(struct chan_usbradio_pvt *o)
{
	//tris_log(LOG_NOTICE,"xpmr_config()\n");

	TRACEO(1,("xpmr_config()\n"));

	if(o->pmrChan==NULL)
	{
		tris_log(LOG_ERROR,"pmr channel structure NULL\n");
		return 1;
	}
 ...
 */
static struct chan_usbradio_pvt *store_config(struct tris_config *cfg, char *ctg)
{
	struct tris_variable *v;
	struct chan_usbradio_pvt *o;
	struct tris_config *cfg1;
	int i;
	char fname[200];
#ifdef	NEW_TRISMEDIA
	struct tris_flags zeroflag = {0};
#endif
	if (ctg == NULL) {
		...
		if (strcmp(ctg, "general") == 0) {
		    ...
		    // tris_log(LOG_NOTICE,"tris_calloc for chan_usbradio_pvt of %s\n",ctg);
			if (!(o = tris_calloc(1, sizeof(*o))))
				return NULL;
			*o = usbradio_default;
			o->name = tris_strdup(ctg);
			if (!usbradio_active) 
				usbradio_active = o->name;
		}
	}
	tris_mutex_init(&o->eepromlock);
	strcpy(o->mohinterpret, "default");
	/* fill other fields from configuration */
	for (v = tris_variable_browse(cfg, ctg); v; v = v->next) {
...
#ifdef	NEW_TRISMEDIA
	cfg1 = tris_config_load(fname,zeroflag);
#else
	cfg1 = tris_config_load(fname);
#endif
	o->rxmixerset = 500;
	o->txmixaset = 500;
	o->txmixbset = 500;
	...
	if (cfg1 && cfg1 != CONFIG_STATUS_FILEINVALID) {
		for (v = tris_variable_browse(cfg1, o->name); v; v = v->next) {
		...
		tris_config_destroy(cfg1);
	} else tris_log(LOG_WARNING,"File %s not found, using default parameters.\n",fname);
	...
	if ((!usb_list_check(o->devstr)) || find_desc_usb(o->devstr))
	{
		...
		if (!*s)
		{
			tris_log(LOG_WARNING,"Unable to assign USB device for channel %s\n",o->name);
			goto error;
		}
		tris_log(LOG_NOTICE,"Assigned USB device %s to usbradio channel %s\n",s,o->name);
		strcpy(o->devstr,s);
	}
	...
	if (i < 0)
	{
	        tris_log(LOG_ERROR,"Not able to find alsa USB device\n");
		goto error;
	}
	o->devicenum = i;
	...
	o->lastopen = tris_tvnow();	/* don't leave it 0 or tvdiff may wrap */
	o->dsp = tris_dsp_new();
	if (o->dsp)
	{
#ifdef	NEW_TRISMEDIA
	  tris_dsp_set_features(o->dsp,DSP_FEATURE_DIGIT_DETECT);
	  tris_dsp_set_digitmode(o->dsp,DSP_DIGITMODE_DTMF | DSP_DIGITMODE_MUTECONF | DSP_DIGITMODE_RELAXDTMF);
#else
	  tris_dsp_set_features(o->dsp,DSP_FEATURE_DTMF_DETECT);
	  tris_dsp_digitmode(o->dsp,DSP_DIGITMODE_DTMF | DSP_DIGITMODE_MUTECONF | DSP_DIGITMODE_RELAXDTMF);
#endif
	}
	...
	if(o->pmrChan==NULL)
	{
		...
		// tris_log(LOG_NOTICE,"createPmrChannel() %s\n",o->name);
		...
		*(o->pmrChan->prxCtcssAdjust)=o->rxctcssadj*M_Q8;
		...
		{
			tris_log(LOG_ERROR,"No txtone output configured.\n");
		}
		...
		if(o->b.radioactive)
		{
			...
			tris_log(LOG_NOTICE,"radio active set to [%s]\n",o->name);
		}
	}
...
#ifndef	NEW_TRISMEDIA
	if (pipe(o->sndcmd) != 0) {
		tris_log(LOG_ERROR, "Unable to create pipe\n");
		goto error;
	}

	tris_pthread_create_background(&o->sthread, NULL, sound_thread, o);
#endif
...
static char *handle_console_key(struct tris_cli_entry *e,
...
static char *handle_console_unkey(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
...
static char *handle_radio_tune(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
...
static char *handle_radio_debug(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_radio_debug_off(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
...
static char *handle_radio_active(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
...
static char *handle_set_xdebug(struct tris_cli_entry *e,
	int cmd, struct tris_cli_args *a)
{
...
static struct tris_cli_entry cli_usbradio[] = {
	TRIS_CLI_DEFINE(handle_console_key,"Simulate Rx Signal Present"),
	TRIS_CLI_DEFINE(handle_console_unkey,"Simulate Rx Signal Loss"),
	TRIS_CLI_DEFINE(handle_radio_tune,"Radio Tune"),
	TRIS_CLI_DEFINE(handle_radio_debug,"Radio Debug On"),
	TRIS_CLI_DEFINE(handle_radio_debug_off,"Radio Debug Off"),
	TRIS_CLI_DEFINE(handle_radio_active,"Change commanded device"),
	TRIS_CLI_DEFINE(handle_set_xdebug,"Radio set xpmr debug level")
};
...
static int load_module(void)
{
	...
	if (hid_device_mklist()) {
		tris_log(LOG_NOTICE, "Unable to make hid list\n");
	...
	/* load config file */
#ifdef	NEW_TRISMEDIA
	if (!(cfg = tris_config_load(config,zeroflag)) || cfg == CONFIG_STATUS_FILEINVALID) {
#else
	if (!(cfg = tris_config_load(config))) || cfg == CONFIG_STATUS_FILEINVALID {
#endif
	...
	} while ( (ctg = tris_category_browse(cfg, ctg)) != NULL);

	tris_config_destroy(cfg);

	if (find_desc(usbradio_active) == NULL) {
		tris_log(LOG_NOTICE, "radio active device %s not found\n", usbradio_active);
		/* XXX we could default to 'dsp' perhaps ? */
		/* XXX should cleanup allocated memory etc. */
		return TRIS_MODULE_LOAD_FAILURE;
	}

	if (tris_channel_register(&usbradio_tech)) {
		tris_log(LOG_ERROR, "Unable to register channel type 'usb'\n");
		return TRIS_MODULE_LOAD_FAILURE;
	}

	tris_cli_register_multiple(cli_usbradio, ARRAY_LEN(cli_usbradio));
...
static int unload_module(void)
{
	...
	tris_log(LOG_WARNING, "unload_module() called\n");

	tris_channel_unregister(&usbradio_tech);
	tris_cli_unregister_multiple(cli_usbradio, ARRAY_LEN(cli_usbradio));

	for (o = usbradio_default.next; o; o = o->next) {

		tris_log(LOG_WARNING, "destroyPmrChannel() called\n");
...
#endif
		if (o->dsp) tris_dsp_free(o->dsp);
		if (o->owner)
			tris_softhangup(o->owner, TRIS_SOFTHANGUP_APPUNLOAD);
		if (o->owner)			/* XXX how ??? */
			return -1;
...
TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "usb Console Channel Driver");
