...
#include "trismedia.h"	/* tris_strdupa */
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 126480 $")
#include "trismedia/utils.h"	/* tris_strdupa */
#include "console_video.h"	/* tris_strdupa */
...
struct board *board_setup(SDL_Surface *screen, SDL_Rect *dest,
	SDL_Surface *font, SDL_Rect *font_rects)
{
	struct board *b = tris_calloc(1, sizeof (*b));
	SDL_Rect br;
	...
	b->text = tris_calloc(b->v_w*b->v_h + 1, 1);
	if (b->text == NULL) {
		tris_log(LOG_WARNING, "Unable to allocate board history memory.\n");
		tris_free(b);
		return NULL;
	}
	...
	if (b->blank == NULL) { 
		tris_log(LOG_WARNING, "Unable to allocate board virtual screen: %s\n",
				SDL_GetError());
		tris_free(b->text);
		tris_free(b);
		return NULL;
	...
	if (0) tris_log(LOG_WARNING, "Message board %dx%d@%d,%d successfully initialized\n",
