...
#include "trismedia/utils.h"	/* tris_calloc and tris_realloc */
#include <math.h>		/* sqrt */
...
{
	tris_log(LOG_WARNING, "keypad_toggle(%i) called\n", index);
	...
	case KEY_MUTE: /* send or do not send audio */
		tris_cli_command(env->gui->outfd, "console mute toggle");
		break;
...
static void keypad_pick_up(struct video_desc *env)
{
	...
	tris_log(LOG_WARNING, "keypad_pick_up called\n");

	if (env->owner) { /* someone is calling us, just answer */
		tris_cli_command(gui->outfd, "console answer");
	} else { /* we have someone to call */
		char buf[160];
		const char *who = tris_skip_blanks(read_message(gui->bd_msg));
		buf[sizeof(buf) - 1] = '\0';
		snprintf(buf, sizeof(buf), "console dial %s", who);
		tris_log(LOG_WARNING, "doing <%s>\n", buf);
		print_message(gui->bd_dialed, "\n");
		print_message(gui->bd_dialed, who);
		reset_board(gui->bd_msg);
		tris_cli_command(gui->outfd, buf);
	}
}
...
static int switch_video_out(struct video_desc *env, int index, Uint8 button)
{
	...
	if (index >= env->out.device_num) {
		tris_log(LOG_WARNING, "no devices\n");
		return 1;
	}
	...
	if (index == *p) {
		tris_log(LOG_WARNING, "device %s already selected\n", env->out.devices[index].name);
		return 0;
	}
	tris_log(LOG_WARNING, "switching to %s...\n", env->out.devices[index].name);
	/* already open */
	...
	tris_log(LOG_WARNING, "device is down\n");
	return 1;
}
...
static void handle_mousedown(struct video_desc *env, SDL_MouseButtonEvent button)
{
...
#if 0
	tris_log(LOG_WARNING, "event %d %d have %d/%d regions at %p\n",
	...
	if (index < 128) {	/* surely clicked on the keypad, don't care which key */
		...
		/* turn on or off the devices selectively with other mouse buttons */
			...
			if (!ret)
				tris_log(LOG_WARNING, "unable to turn on device %s\n",
					env->out.devices[index].name);
			else if (ret == 1)
				tris_log(LOG_WARNING, "device %s changed state to on\n",
					env->out.devices[index].name);
			else if (ret == 2)
				tris_log(LOG_WARNING, "device %s changed state to off\n",
					env->out.devices[index].name);
			return;
		}
	}
	...
	*/
	...
	case KEY_PICK_UP:
		...
		break;
	case KEY_HANG_UP:
		tris_cli_command(gui->outfd, "console hangup");
		break;
	...
	case KEY_OUT_OF_KEYPAD:
		tris_log(LOG_WARNING, "nothing clicked, coordinates: %d, %d\n", button.x, button.y);
		break;
	...
	default:
		tris_log(LOG_WARNING, "function not yet defined %i\n", index);
	}
}
...
#define MY_EV (SDL_MOUSEBUTTONDOWN|SDL_KEYDOWN)
	while ( (n = SDL_PeepEvents(ev, N_EVENTS, SDL_GETEVENT, SDL_ALLEVENTS)) > 0) {
		for (i = 0; i < n; i++) {
#if 0
			tris_log(LOG_WARNING, "------ event %d at %d %d\n",
			...
			case SDL_ACTIVEEVENT:
				...
				if (ev[i].active.gain == 0 && ev[i].active.state & SDL_APPACTIVE) {
					tris_log(LOG_WARNING, "/* somebody has killed us ? */");
					tris_cli_command(gui->outfd, "stop now");
				}
#endif
				break;
			...
			case SDL_MOUSEBUTTONUP:
				if (drag->drag_window == DRAG_LOCAL && env->out.device_num) {
					...
					/* tris_log(LOG_WARNING, "moving: %d, %d\n", dx, dy); */
					pip_move(env, dx, dy);
	...
	if (1) {
		struct timeval b, a = tris_tvnow();
		int i;
		//SDL_Lock_EventThread();
		SDL_PumpEvents();
		b = tris_tvnow();
		i = tris_tvdiff_ms(b, a);
		if (i > 3)
...
{
	struct gui_info *gui = tris_calloc(1, sizeof(*gui));
	...
	if (gui->keypad == NULL)	/* no keypad, we are done */
		return gui;
	/* XXX load image */
	if (!tris_strlen_zero(font)) {
		int i;
		SDL_Rect *r;

		gui->font = load_image(font);
		if (!gui->font) {
			tris_log(LOG_WARNING, "Unable to load font %s, no output available\n", font);
			goto error;
		}
		tris_log(LOG_WARNING, "Loaded font %s\n", font);
...
#define FONT_W 9
	...
	if (gui->outfd < 0) {
		tris_log(LOG_WARNING, "Unable output fd\n");
		goto error;
	}
	return gui;
...
static void keypad_setup(struct gui_info *gui, const char *kp_file)
{
	...
	fd = fopen(kp_file, "r");
	if (fd == NULL) {
		tris_log(LOG_WARNING, "fail to open %s\n", kp_file);
		return;
	}
	/*
	...
	while (fgets(buf, sizeof(buf), fd)) {
		...
		s = tris_skip_blanks(buf);
		tris_trim_blanks(s);
		if (memcmp(s, region, reg_len))
			break;	/* keyword not found */
		s = tris_skip_blanks(s + reg_len); /* space between token and '=' */
		if (*s++ != '=')	/* missing separator */
			break;
		if (*s == '>')	/* skip '>' if present */
			s++;
		keypad_cfg_read(gui, tris_skip_blanks(s));
	}
	fclose(fd);
}
...
{
	tris_log(LOG_WARNING, "%s error_code %d\n", __FUNCTION__, e->error_code);
	return 0;
}
#endif /* HAVE_X11 */
...
static void sdl_setup(struct video_desc *env)
{
	...
	if (!tris_strlen_zero(e)) {
		...
		if (!success) {
			tris_log(LOG_WARNING, "%s error in window\n", __FUNCTION__);
			return;
		}
	}	
#endif
	...
	if (gui == NULL && SDL_Init(SDL_INIT_VIDEO)) {
 		tris_log(LOG_WARNING, "Could not initialize SDL - %s\n",
                        SDL_GetError());
	 ...
	 */
	if (!info || !info->vfmt) {
 		tris_log(LOG_WARNING, "Bad SDL_GetVideoInfo - %s\n",
                        SDL_GetError());
		return;
	}
	...
	if (!gui->screen) {
		tris_log(LOG_ERROR, "SDL: could not set video mode - exiting\n");
		goto no_sdl;
	}
	...
	const char *e = getenv("SDL_WINDOWID");
	if (tris_strlen_zero(e))	 /* no external window, don't bother doing this */
		break;
	...
	SDL_WM_SetCaption("Trismedia console Video Output", NULL);
...
#if 0
	tris_log(LOG_WARNING, "result %d [%d] for match %d,%d in type %d p0 %d,%d p1 %d,%d h %d\n",
...
static int keypad_cfg_read(struct gui_info *gui, const char *val)
{
	...
	case 7:
		if (e.c == KEY_FONT) {	/* font - x0 y0 w h rows cols */
			tris_log(LOG_WARNING, "font not supported yet\n");
			break;
		}
		/* token circle|rect x0 y0 x1 y1 h */
		if (e.x1 < e.x0 || e.h <= 0) {
			tris_log(LOG_WARNING, "error in coordinates\n");
			e.type = 0;
			break;
		}
	...
	// tris_log(LOG_WARNING, "reading [%s] returns %d %d\n", val, i, ret);
	if (ret == 0)
		return 0;
	if (gui->kp_size == 0) {
		gui->kp = tris_calloc(10, sizeof(e));
		if (gui->kp == NULL) {
			tris_log(LOG_WARNING, "cannot allocate kp");
			return 0;
		}
		gui->kp_size = 10;
	}
	if (gui->kp_size == gui->kp_used) { /* must allocate */
		struct keypad_entry *a = tris_realloc(gui->kp, sizeof(e)*(gui->kp_size+10));
		if (a == NULL) {
			tris_log(LOG_WARNING, "cannot reallocate kp");
			return 0;
		}
		gui->kp = a;
		gui->kp_size += 10;
	...
	// tris_log(LOG_WARNING, "now %d regions\n", gui->kp_used);
	return 1;
}
#endif	/* HAVE_SDL */
