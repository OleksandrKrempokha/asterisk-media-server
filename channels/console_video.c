 ...
 * thus not compiling in TRIS_DEVMODE, or don't have swscale, in which case
...
#include "trismedia.h"
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 147807 $")
#include <sys/ioctl.h>
...
{
	tris_log(LOG_NOTICE, "voice only, console video support not present\n");
}
...
int console_video_formats = 
	TRIS_FORMAT_H263_PLUS | TRIS_FORMAT_H263 |
	TRIS_FORMAT_MP4_VIDEO | TRIS_FORMAT_H264 | TRIS_FORMAT_H261 ;
...
struct video_desc {
	...
	tris_mutex_t		dec_lock;	/* sync decoder and video thread */
	int			shutdown;	/* set to shutdown vthread */
	struct tris_channel	*owner;		/* owner channel */
...
used_mem(const char *msg)
{
	...
	tris_log(LOG_WARNING, "used mem (vsize, rss) %s ", msg);
	system(in);
	return 0;
}
#endif
...
static struct fbuf_t *grabber_read(struct video_device *dev, int fps)
{
	...
	if (tris_tvdiff_ms(now, dev->last_frame) < 1000/fps)
...
static struct video_codec_desc *map_config_video_format(char *name)
{
	...
	if (supported_codecs[i] == NULL) {
		tris_log(LOG_WARNING, "Cannot find codec for '%s'\n", name);
		i = 0;
		strcpy(name, supported_codecs[i]->name);
	}
	tris_log(LOG_WARNING, "Using codec '%s'\n", name);
...
static int video_out_init(struct video_desc *env)
{
	...
	if (!v->codec) {
		tris_log(LOG_WARNING, "Cannot find the encoder for format %d\n",
			codec);
	...
	enc_in->data = tris_calloc(1, enc_in->size);
	if (!enc_in->data) {
		tris_log(LOG_WARNING, "Cannot allocate encoder input buffer\n");
		return video_out_uninit(env);
	}
	/* construct an AVFrame that points into buf_in */
	v->enc_in_frame = avcodec_alloc_frame();
	if (!v->enc_in_frame) {
		tris_log(LOG_WARNING, "Unable to allocate the encoding video frame\n");
		return video_out_uninit(env);
	}
	...
	if (avcodec_open(enc_ctx, v->codec) < 0) {
		tris_log(LOG_WARNING, "Unable to initialize the encoder %d\n",
			codec);
		av_free(enc_ctx);
		v->enc_ctx = NULL;
		return video_out_uninit(env);
	 ...
	 */
	v->enc_out.data = tris_calloc(1, enc_in->size);
...
void console_video_uninit(struct video_desc *env)
{
	...
	if (env->stayopen == 0) { /* gui opened by a call, do the shutdown */
		...
		for (i=0; env->shutdown && i < 10; i++) {
			if (env->owner)
				tris_channel_unlock(env->owner);
			usleep(t);
			t = 1000000;
			if (env->owner)
				tris_channel_lock(env->owner);
		}
		env->vthread = NULL;
	}
	...
	struct fbuf_t *out, AVPicture *p_out)
{
	...
	if (convert_ctx == NULL) {
		tris_log(LOG_ERROR, "FFMPEG::convert_cmodel : swscale context initialization failed");
		return;
	}
	if (0)
		tris_log(LOG_WARNING, "in %d %dx%d out %d %dx%d\n",
...
/*
 * This function is called (by trismedia) for each video packet
...
int console_write_video(struct tris_channel *chan, struct tris_frame *f)
{
	...
	if (v == NULL) {
		...
		tris_log(LOG_WARNING, "Cannot initialize input decoder\n");
		return 0;
	}
	...
	if ((random() % 10000) <= 100*DROP_PACKETS) {
		tris_log(LOG_NOTICE, "Packet lost [%d]\n", f->seqno);
		return 0;
	}
#endif
	if (v->discard) {
		...
		if (f->subclass & 0x01) {
			...
			v->discard = 0;
			tris_log(LOG_WARNING, "out of discard mode, frame %d\n", f->seqno);
		}
		return 0;
	}
	...
	if (v->next_seq != f->seqno) {
		tris_log(LOG_WARNING, "discarding frame out of order, %d %d\n",
			v->next_seq, f->seqno);
		v->discard = 1;
		return 0;
	}
	...
	if (f->data.ptr == NULL || f->datalen < 2) {
		tris_log(LOG_WARNING, "empty video frame, discard\n");
		return 0;
	}
	if (v->d_callbacks->dec_decap(v->dec_in_cur, f->data.ptr, f->datalen)) {
		tris_log(LOG_WARNING, "error in dec_decap, enter discard\n");
		v->discard = 1;
	}
	if (f->subclass & 0x01) {	// RTP Marker
		...
		tris_mutex_unlock(&env->dec_lock);
	}
	return 0;
}
 ...
 * Return a list of tris_frame representing the video fragments.
 ...
 */
static struct tris_frame *get_video_frames(struct video_desc *env, struct tris_frame **tail)
{
	struct video_out_desc *v = &env->out;
	struct tris_frame *dummy;
	...
	*tail = NULL;
	...
	if (v->enc_out.data == NULL) {
		...
		if (a++ < 2)
			tris_log(LOG_WARNING, "fail, no encoder output buffer\n");
		return NULL;
	}
	v->enc->enc_run(v);
...
static void *video_thread(void *arg)
{
	 ...
	 */
	if (!tris_strlen_zero(env->sdl_videodriver)) { /* override */
		const char *s = getenv("DISPLAY");
		setenv("SDL_VIDEODRIVER", env->sdl_videodriver, 1);
		if (s && !strcasecmp(env->sdl_videodriver, "aalib-console")) {
			tris_copy_string(save_display, s, sizeof(save_display));
			unsetenv("DISPLAY");
		}
	}
	sdl_setup(env);
	if (!tris_strlen_zero(save_display))
		setenv("DISPLAY", save_display, 1);

	tris_mutex_init(&env->dec_lock);	/* used to sync decoder and renderer */

	if (grabber_open(&env->out)) {
		tris_log(LOG_WARNING, "cannot open local video source\n");
	} 
	...
	for (;;) {
		...
		* otherwise the drag will not work */ 
		...
		/* sleep for a while */
		tris_select(0, NULL, NULL, NULL, &t);
		...
		if (chan == NULL) {
			...
			while (f) {
				struct tris_frame *g = TRIS_LIST_NEXT(f, frame_list);
				tris_frfree(f);
				f = g;
			}
			continue;
		...
		/* TRIS_LIST_INSERT_TAIL is only good for one frame, cannot use here */
		...
		chan->readq.last = p;
		/*
		 * more or less same as tris_queue_frame, but extra
		 * write on the alertpipe to signal frames.
		 */
		if (fd > -1) {
			int blah = 1, l = sizeof(blah);
			for (p = f; p; p = TRIS_LIST_NEXT(p, frame_list)) {
				if (write(fd, &blah, l) != l)
					tris_log(LOG_WARNING, "Unable to write to alert pipe on %s, frametype/subclass %d/%d: %s!\n",
	...
	tris_mutex_destroy(&env->dec_lock);
	env->shutdown = 0;
	return NULL;
}
...
void console_video_start(struct video_desc *env, struct tris_channel *owner)
{
	tris_log(LOG_WARNING, "env %p chan %p\n", env, owner);
	...
	tris_log(LOG_WARNING, "start video out %s %dx%d\n",
	...
	if (env->out.fps == 0) {
		env->out.fps = 15;
		tris_log(LOG_WARNING, "fps unset, forcing to %d\n", env->out.fps);
	}
	if (env->out.bitrate == 0) {
		env->out.bitrate = 65000;
		tris_log(LOG_WARNING, "bitrate unset, forcing to %d\n", env->out.bitrate);
	}
	/* create the thread as detached so memory is freed on termination */
	tris_pthread_create_detached_background(&env->vthread,
...
static int video_geom(struct fbuf_t *b, const char *s)
{
	...
	if (fp->s) {
		...
		tris_log(LOG_WARNING, "Invalid video_size %s, using 352x288\n", s);
		b->w = 352;
		b->h = 288;
	}
	return 0;
...
static int device_table_fill(struct video_device *devices, int *device_num_p, const char *s)
{
	...
	p->name = tris_strdup(s);		/* copy the name */
	/* other fields initially NULL */
	p->grabber = NULL;
	p->grabber_data = NULL;
	p->dev_buf = NULL;
	p->last_frame = tris_tvnow();
	p->status_index = 0;
	(*device_num_p)++;			/* one device added */
	return 0;
}

/* extend tris_cli with video commands. Called by console_video_config */
int console_video_cli(struct video_desc *env, const char *var, int fd)
{
	if (env == NULL)
		...
		tris_cli(fd, "sizes: video %dx%d camera %dx%d local %dx%d remote %dx%d in %dx%d\n",
			env->enc_in.w, env->enc_in.h,
		...
		tris_cli(fd, "bitrate is [%d]\n", env->out.bitrate);
        } else if (!strcasecmp(var, "qmin")) {
		tris_cli(fd, "qmin is [%d]\n", env->out.qmin);
        } else if (!strcasecmp(var, "fps")) {
		tris_cli(fd, "fps is [%d]\n", env->out.fps);
		...
		if (env->gui && env->owner)
			tris_cli_command(-1, "console hangup");
		else /* not in a call */
	...
	const char *var, const char *val)
{
	...
	if (penv == NULL) {
		tris_log(LOG_WARNING, "bad argument penv=NULL\n");
		return 1;	/* error */
	}
	/* allocate the video descriptor first time we get here */
	env = *penv;
	if (env == NULL) {
		env = *penv = tris_calloc(1, sizeof(struct video_desc));
		if (env == NULL) {
			tris_log(LOG_WARNING, "fail to allocate video_desc\n");
			return 1;	/* error */
