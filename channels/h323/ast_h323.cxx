 ...
 *
 * This file is part of the chan_h323 driver for Trismedia
 *
 ...
 *
 * Version Info: $Id: tris_h323.cxx 201679 2009-06-18 16:44:37Z dvossel $
 */
...
const char *h323product  = "H.323 Channel Driver for Trismedia";
...
class PTrismediaLog : public PObject, public iostream {
	PCLASSINFO(PTrismediaLog, PObject);

	public:
	PTrismediaLog() : iostream(cout.rdbuf()) { init(&buffer); }
	~PTrismediaLog() { flush(); }

	private:
	PTrismediaLog(const PTrismediaLog &) : iostream(cout.rdbuf()) { }
	PTrismediaLog & operator=(const PTrismediaLog &) { return *this; }
...
static PTrismediaLog *logstream = NULL;

int PTrismediaLog::Buffer::overflow(int c)
{
	if (pptr() >= epptr()) {
		int ppos = pptr() - pbase();
...
int PTrismediaLog::Buffer::underflow()
{
	return EOF;
}

int PTrismediaLog::Buffer::sync()
{
	char *str = tris_strdup(string);
	char *s, *s1;
	char c;

	/* Pass each line with different tris_verbose() call */
	for (s = str; s && *s; s = s1) {
		s1 = strchr(s, '\n');
		if (!s1)
			s1 = s + strlen(s);
...
PBoolean MyH323Connection::OnReceivedSignalSetup(const H323SignalPDU & setupPDU)
{
	...
	/* Notify Trismedia of the request */
...
{
	struct __codec__ {
		unsigned int trismedia_codec;
		unsigned int h245_cap;
		const char *oid;
		const char *formatName;
	};
	static const struct __codec__ codecs[] = {
		{ TRIS_FORMAT_G723_1, H245_AudioCapability::e_g7231 },
		{ TRIS_FORMAT_GSM, H245_AudioCapability::e_gsmFullRate },
		{ TRIS_FORMAT_ULAW, H245_AudioCapability::e_g711Ulaw64k },
		{ TRIS_FORMAT_ALAW, H245_AudioCapability::e_g711Alaw64k },
		{ TRIS_FORMAT_G729A, H245_AudioCapability::e_g729AnnexA },
		{ TRIS_FORMAT_G729A, H245_AudioCapability::e_g729 },
		{ TRIS_FORMAT_G726_AAL2, H245_AudioCapability::e_nonStandard, NULL, CISCO_G726r32 },
#ifdef TRIS_FORMAT_MODEM
		{ TRIS_FORMAT_MODEM, H245_DataApplicationCapability_application::e_t38fax },
#endif
		{ 0 }
	};
...
#ifdef HAVE_H261
		{ TRIS_FORMAT_H261, H245_VideoCapability::e_h261VideoCapability },
#endif
#ifdef HAVE_H263
		{ TRIS_FORMAT_H263, H245_VideoCapability::e_h263VideoCapability },
#endif
#ifdef HAVE_H264
		{ TRIS_FORMAT_H264, H245_VideoCapability::e_genericVideoCapability, "0.0.8.241.0.0.1" },
#endif
		{ 0 }
	};
#endif
	...
	for (int i = 0; i < remoteCapabilities.GetSize(); ++i) {
		...
		case H323Capability::e_Audio:
			for (int x = 0; codecs[x].trismedia_codec > 0; ++x) {
				if ((subType == codecs[x].h245_cap) && (!codecs[x].formatName || (!strcmp(codecs[x].formatName, (const char *)remoteCapabilities[i].GetFormatName())))) {
					int tris_codec = codecs[x].trismedia_codec;
					int ms = 0;
					if (!(peer_capabilities & tris_codec)) {
						struct tris_format_list format;
						tris_codec_pref_append(&prefs, tris_codec);
						format = tris_codec_pref_getsize(&prefs, tris_codec);
						if ((tris_codec == TRIS_FORMAT_ALAW) || (tris_codec == TRIS_FORMAT_ULAW)) {
							ms = remoteCapabilities[i].GetTxFramesInPacket();
						} else
							ms = remoteCapabilities[i].GetTxFramesInPacket() * format.inc_ms;
						tris_codec_pref_setsize(&prefs, tris_codec, ms);
					}
					if (h323debug) {
						cout << "Found peer capability " << remoteCapabilities[i] << ", Trismedia code is " << tris_codec << ", frame size (in ms) is " << ms << endl;
					}
					peer_capabilities |= tris_codec;
				}
			}
			break;
...
//						sendUserInputMode = SendUserInputAsInlineRFC2833;
		...
		case H323Capability::e_Video:
			for (int x = 0; vcodecs[x].trismedia_codec > 0; ++x) {
				if (subType == vcodecs[x].h245_cap) {
					...
					if ((subType != H245_VideoCapability::e_genericVideoCapability) ||
						...
						if (h323debug) {
							cout << "Found peer video capability " << remoteCapabilities[i] << ", Trismedia code is " << vcodecs[x].trismedia_codec << endl;
						}
						peer_capabilities |= vcodecs[x].trismedia_codec;
					}
					if (cap)
						delete(cap);
				}
	...
	if (h323debug) {
		...
		tris_codec_pref_string(&prefs, caps2_str, sizeof(caps2_str));
		cout << "Peer capabilities = " << tris_getformatname_multiple(caps_str, sizeof(caps_str), peer_capabilities)
...
void MyH323Connection::SetCapabilities(int caps, int dtmf_mode, void *_prefs, int pref_codec)
{
	...
	int x, y;
	char caps_str[1024];
	struct tris_codec_pref *prefs = (struct tris_codec_pref *)_prefs;
	struct tris_format_list format;
	...
	if (h323debug) {
		cout << "Setting capabilities to " << tris_getformatname_multiple(caps_str, sizeof(caps_str), caps) << endl;
		tris_codec_pref_string(prefs, caps_str, sizeof(caps_str));
	...
	for (y = 0, x = -1; x < 32 + 32; ++x) {
		if (x < 0)
			codec = pref_codec;
		else if (y || (!(codec = tris_codec_pref_index(prefs, x)))) {
			if (!y)
				y = 1;
			else
				y <<= 1;
		...
		case TRIS_FORMAT_G729A:
			TRIS_G729ACapability *g729aCap;
			TRIS_G729Capability *g729Cap;
			lastcap = localCapabilities.SetCapability(0, 0, g729aCap = new TRIS_G729ACapability(frames_per_packet));
			lastcap = localCapabilities.SetCapability(0, 0, g729Cap = new TRIS_G729Capability(frames_per_packet));
			g729aCap->SetTxFramesInPacket(format.cur_ms);
			g729Cap->SetTxFramesInPacket(format.cur_ms);
			break;
		case TRIS_FORMAT_G723_1:
			TRIS_G7231Capability *g7231Cap;
			lastcap = localCapabilities.SetCapability(0, 0, g7231Cap = new TRIS_G7231Capability(frames_per_packet, TRUE));
			g7231Cap->SetTxFramesInPacket(format.cur_ms);
			lastcap = localCapabilities.SetCapability(0, 0, g7231Cap = new TRIS_G7231Capability(frames_per_packet, FALSE));
			g7231Cap->SetTxFramesInPacket(format.cur_ms);
			break;
		case TRIS_FORMAT_GSM:
			TRIS_GSM0610Capability *gsmCap;
			lastcap = localCapabilities.SetCapability(0, 0, gsmCap = new TRIS_GSM0610Capability(frames_per_packet));
			gsmCap->SetTxFramesInPacket(format.cur_ms);
			break;
		case TRIS_FORMAT_ULAW:
			TRIS_G711Capability *g711uCap;
			lastcap = localCapabilities.SetCapability(0, 0, g711uCap = new TRIS_G711Capability(format.cur_ms, H323_G711Capability::muLaw));
			g711uCap->SetTxFramesInPacket(format.cur_ms);
			break;
		case TRIS_FORMAT_ALAW:
			TRIS_G711Capability *g711aCap;
			lastcap = localCapabilities.SetCapability(0, 0, g711aCap = new TRIS_G711Capability(format.cur_ms, H323_G711Capability::ALaw));
			g711aCap->SetTxFramesInPacket(format.cur_ms);
			break;
		case TRIS_FORMAT_G726_AAL2:
			TRIS_CiscoG726Capability *g726Cap;
			lastcap = localCapabilities.SetCapability(0, 0, g726Cap = new TRIS_CiscoG726Capability(frames_per_packet));
	...
	if (dtmfMode) {
		...
		if (dtmfMode == H323_DTMF_INBAND) {
			...
			if ((dtmfMode & H323_DTMF_CISCO) != 0) {
				...
				cap = new TRIS_CiscoDtmfCapability();
				if (cap && cap->IsUsable(*this)) {
					lastcap = localCapabilities.SetCapability(0, lastcap, cap);
			...
			sendUserInputMode = SendUserInputAsTone;	/* RFC2833 transmission handled at Trismedia level */
		}
	}
...
PBoolean MyH323_ExternalRTPChannel::Start(void)
{
	...
	/* Notify Trismedia of remote RTP information */
 ...
 */
int h323_set_capabilities(const char *token, int cap, int dtmf_mode, struct tris_codec_pref *prefs, int pref_codec)
{
	MyH323Connection *conn;
...
int h323_set_gk(int gatekeeper_discover, char *gatekeeper, char *secret)
{
	...
	if (gatekeeper_discover) {
		...
		if (endPoint->SetGatekeeper(gkName, rasChannel)) {
			...
			/* XXX Maybe we should fire a new thread to attempt to re-register later and not kill trismedia here? */
			return 1;
		}
	}
	return 0;
