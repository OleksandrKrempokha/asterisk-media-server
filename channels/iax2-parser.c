...
/*! \file
 *
 * \brief Implementation of Inter-Trismedia eXchange Protocol, v 2
 *
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 243945 $")
...
TRIS_LIST_HEAD_NOLOCK(iax_frame_list, iax_frame);
...
static void dump_addr(char *output, int maxlen, void *value, int len)
{
	...
	if (len == (int)sizeof(sin)) {
		...
		snprintf(output, maxlen, "IPV4 %s:%d", tris_inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
	} else {
		tris_copy_string(output, "Invalid Address", maxlen);
	}
}
...
static void dump_prefs(char *output, int maxlen, void *value, int len)
{
	...
	tris_codec_pref_convert(&pref, output, total_len, 0);
	memset(output,0,total_len);
	tris_codec_pref_string(&pref, output, total_len);
}
...
static void dump_int(char *output, int maxlen, void *value, int len)
{
	if (len == (int)sizeof(unsigned int))
		...
		tris_copy_string(output, "Invalid INT", maxlen);	
}
...
static void dump_short(char *output, int maxlen, void *value, int len)
{
	if (len == (int)sizeof(unsigned short))
		...
		tris_copy_string(output, "Invalid SHORT", maxlen);
}
...
static void dump_byte(char *output, int maxlen, void *value, int len)
{
	if (len == (int)sizeof(unsigned char))
		...
		tris_copy_string(output, "Invalid BYTE", maxlen);
}
...
static void dump_datetime(char *output, int maxlen, void *value, int len)
{
	...
	if (len == (int)sizeof(unsigned int)) {
		...
		tris_strftime(output, maxlen, "%Y-%m-%d  %T", &tm); 
	} else
		tris_copy_string(output, "Invalid DATETIME format!", maxlen);
}
...
static void dump_ipaddr(char *output, int maxlen, void *value, int len)
{
	...
	if (len == (int)sizeof(unsigned int)) {
		...
		snprintf(output, maxlen, "%s", tris_inet_ntoa(sin.sin_addr));
	} else
		tris_copy_string(output, "Invalid IPADDR", maxlen);
}
...
static void dump_samprate(char *output, int maxlen, void *value, int len)
{
	...
	if (len == (int)sizeof(unsigned short)) {
		...
		if (sr & IAX_RATE_48KHZ)
			strcat(tmp, ",48khz");
		if (strlen(tmp))
			tris_copy_string(output, &tmp[1], maxlen);
		else
			tris_copy_string(output, "None Specified!\n", maxlen);
	} else
		tris_copy_string(output, "Invalid SHORT", maxlen);
...
static void dump_prov_ies(char *output, int maxlen, unsigned char *iedata, int len)
{
	...
	while(len > 2) {
		...
		if (ielen + 2> len) {
			...
			tris_copy_string(output, tmp, maxlen);
...
void iax_frame_subclass2str(enum iax_frame_subclass subclass, char *str, size_t len)
{
	...
	tris_copy_string(str, cmd, len);
}

void iax_showframe(struct iax_frame *f, struct tris_iax2_full_hdr *fhi, int rx, struct sockaddr_in *sin, int datalen)
{
	...
	if (fh->type == TRIS_FRAME_DTMF_BEGIN || fh->type == TRIS_FRAME_DTMF_END) {
		sprintf(subclass2, "%c", fh->csub);
		subclass = subclass2;
	} else if (fh->type == TRIS_FRAME_IAX) {
			iax_frame_subclass2str((int)fh->csub, subclass2, sizeof(subclass2));
			subclass = subclass2;
	} else if (fh->type == TRIS_FRAME_CONTROL) {
...
int iax_parse_ies(struct iax_ies *ies, unsigned char *data, int datalen)
{
	...
	int ie;
	char tmp[256], *tmp2;
	struct tris_variable *var, *var2, *prev;
	unsigned int count;
	...
	while(datalen >= 2) {
		...
		case IAX_IE_VARIABLE:
			tris_copy_string(tmp, (char *)data + 2, len + 1);
			tmp2 = strchr(tmp, '=');
			if (tmp2)
				*tmp2++ = '\0';
			else
				tmp2 = "";
			{
				...
				for (var2 = ies->vars, prev = NULL; var2; prev = var2, var2 = var2->next) {
					if (strcmp(tmp, var2->name) == 0) {
						tris_str_set(&str, 0, "%s%s", var2->value, tmp2);
						var = tris_variable_new(tmp, tris_str_buffer(str), var2->file);
						var->next = var2->next;
						if (prev) {
							prev->next = var;
						} else {
			...
			if (!var2) {
				var = tris_variable_new(tmp, tmp2, "");
				snprintf(tmp, sizeof(tmp), "Assigned (%p)%s to (%p)%s\n", var->name, var->name, var->value, var->value);
				outputf(tmp);
				var->next = ies->vars;
				ies->vars = var;
...
void iax_frame_wrap(struct iax_frame *fr, struct tris_frame *f)
{
	...
	fr->af.offset = TRIS_FRIENDLY_OFFSET;
	fr->af.src = f->src;
	...
	if (fr->af.datalen) {
		...
		if (copy_len > fr->afdatalen) {
			tris_log(LOG_ERROR, "Losing frame data because destination buffer size '%d' bytes not big enough for '%d' bytes in the frame\n",
				(int) fr->afdatalen, (int) fr->af.datalen);
			copy_len = fr->afdatalen;
		}
#if __BYTE_ORDER == __LITTLE_ENDIAN
		/* We need to byte-swap slinear samples from network byte order */
		if ((fr->af.frametype == TRIS_FRAME_VOICE) && (fr->af.subclass == TRIS_FORMAT_SLINEAR)) {
			/* 2 bytes / sample for SLINEAR */
			tris_swapcopy_samples(fr->af.data.ptr, f->data.ptr, copy_len / 2);
		} else
#endif
...
struct iax_frame *iax_frame_new(int direction, int datalen, unsigned int cacheable)
{
	...
	if ((iax_frames = tris_threadstorage_get(&frame_cache, sizeof(*iax_frames)))) {
		smallest = TRIS_LIST_FIRST(&iax_frames->list);
		TRIS_LIST_TRAVERSE_SAFE_BEGIN(&iax_frames->list, fr, list) {
	...
	if (!fr) {
		if (iax_frames->size >= FRAME_CACHE_MAX_SIZE && smallest) {
			...
			TRIS_LIST_REMOVE(&iax_frames->list, smallest, list);
			if (!(fr = tris_realloc(smallest, sizeof(*fr) + datalen))) {
				TRIS_LIST_INSERT_TAIL(&iax_frames->list, smallest, list);
				return NULL;
			}
		} else if (!(fr = tris_calloc_cache(1, sizeof(*fr) + datalen)))
			return NULL;
		fr->afdatalen = datalen;
	}
#else
	if (!(fr = tris_calloc(1, sizeof(*fr) + datalen)))
		return NULL;
	fr->afdatalen = datalen;
#endif
	...
	if (fr->direction == DIRECTION_INGRESS)
		tris_atomic_fetchadd_int(&iframes, 1);
	else
		tris_atomic_fetchadd_int(&oframes, 1);
	
	tris_atomic_fetchadd_int(&frames, 1);
...
void iax_frame_free(struct iax_frame *fr)
{
...
#if !defined(LOW_MEMORY)
	if (!fr->cacheable || !(iax_frames = tris_threadstorage_get(&frame_cache, sizeof(*iax_frames)))) {
		tris_free(fr);
		return;
	}
	...
	if (iax_frames->size < FRAME_CACHE_MAX_SIZE) {
		...
		if (TRIS_LIST_FIRST(&iax_frames->list) && TRIS_LIST_FIRST(&iax_frames->list)->afdatalen < fr->afdatalen) {
			TRIS_LIST_INSERT_TAIL(&iax_frames->list, fr, list);
		} else {
			TRIS_LIST_INSERT_HEAD(&iax_frames->list, fr, list);
		}
		iax_frames->size++;
		return;
	}
...
static void frame_cache_cleanup(void *data)
{
	...
	while ((current = TRIS_LIST_REMOVE_HEAD(&framelist->list, list)))
		tris_free(current);
