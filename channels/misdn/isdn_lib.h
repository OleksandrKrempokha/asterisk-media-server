/*
 * Chan_Misdn -- Channel Driver for Trismedia
 *
 * Interface to mISDN
 *
...
struct misdn_bchannel {
	 ...
	 * \note This value is user configurable in /etc/trismedia/misdn.conf
	 */
	 ...
	 * \note This value is set to "dialplan" in /etc/trismedia/misdn.conf for outgoing calls
	 */
	 ...
	 * \note This value is set to "localdialplan" in /etc/trismedia/misdn.conf for outgoing calls
	 */
	enum mISDN_NUMBER_PLAN onumplan;

	/*! \brief Type-of-number in ISDN terms for the connected party number
	 * \note This value is set to "cpndialplan" in /etc/trismedia/misdn.conf for outgoing calls
	 */
	...
	/*! \brief TRUE if AOCDtype and AOCD data are ready to export to Trismedia */
	 ...
	 * \note Collected from the incoming SETUP message and exported to Trismedia variable MISDN_URATE.
	 */
	int urate;
	 ...
	 * \note This value can be set to "callerid" in /etc/trismedia/misdn.conf for outgoing calls
	 */
	char oad[32];
	 ...
	 * \note Contents exported and imported to Trismedia variable MISDN_KEYPAD
	 */
	char keypad[32];
...
/* 	unsigned char cldsub[24]; */
	 ...
	 * \note Contents exported and imported to Trismedia variable MISDN_USERUSER
	 ...
	 * \note Need to use the TRIS_CAUSE_xxx code definitions in causes.h
	 */
	int cause;
...
#ifdef MISDN_1_2
	/*! \brief The configuration string for the mISDN dsp pipeline in /etc/trismedia/misdn.conf. */
	char pipeline[128];
#else
	 ...
	 * \note This value is user configurable in /etc/trismedia/misdn.conf (echocancel)
	 */
	int ec_deftaps;
#endif
