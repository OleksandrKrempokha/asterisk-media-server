...
static const struct misdn_cfg_spec port_spec[] = {
	...
	{ "presentation", MISDN_CFG_PRES, MISDN_CTYPE_INT, "-1", NONE,
		...
		"\tTrismedia's CALLERPRES function.\n"
		"\n"
	...
	{ "astdtmf", MISDN_CFG_ASTDTMF, MISDN_CTYPE_BOOL, "no", NONE,
		"Enable this if you want to use the Trismedia dtmf detector\n"
	...
	{ "l1watcher_timeout", MISDN_CFG_L1_TIMEOUT, MISDN_CTYPE_BOOLINT, "0", 4,
		...
		"\tan Trismedia restart should do the trick." },
	...
	{ "msns", MISDN_CFG_MSNS, MISDN_CTYPE_MSNLIST, "*", NONE,
		...
		"\tindicate the incoming calls to Trismedia.\n"
...
#endif
	{ "tracefile", MISDN_GEN_TRACEFILE, MISDN_CTYPE_STR, "/var/log/trismedia/misdn.log", NONE,
...
static tris_mutex_t config_mutex; 

#define CLI_ERROR(name, value, section) ({ \
	tris_log(LOG_WARNING, "misdn.conf: \"%s=%s\" (section: %s) invalid or out of range. " \
...
static int _enum_array_map (void)
{
	...
	for (i = MISDN_CFG_FIRST + 1; i < MISDN_CFG_LAST; ++i) {
		...
		if (!ok) {
			tris_log(LOG_WARNING, "Enum element %d in misdn_cfg_elements (port section) has no corresponding element in the config struct!\n", i);
			return -1;
		}
	}
	for (i = MISDN_GEN_FIRST + 1; i < MISDN_GEN_LAST; ++i) {
		...
		if (!ok) {
			tris_log(LOG_WARNING, "Enum element %d in misdn_cfg_elements (general section) has no corresponding element in the config struct!\n", i);
			return -1;
		}
	}
	return 0;
...
{
	tris_mutex_lock(&config_mutex);
}

static inline void misdn_cfg_unlock (void)
{
	tris_mutex_unlock(&config_mutex);
}
...
void misdn_cfg_get(int port, enum misdn_cfg_elements elem, void *buf, int bufsize)
{
	...
	if ((elem < MISDN_CFG_LAST) && !misdn_cfg_is_port_valid(port)) {
		...
		tris_log(LOG_WARNING, "Invalid call to misdn_cfg_get! Port number %d is not valid.\n", port);
		return;
	}
	...
	if (elem == MISDN_CFG_PTP) {
		...
		if ((place = map[elem]) < 0) {
			memset(buf, 0, bufsize);
			tris_log(LOG_WARNING, "Invalid call to misdn_cfg_get! Invalid element (%d) requested.\n", elem);
		} else {
			if (elem < MISDN_CFG_LAST) {
				switch (port_spec[place].type) {
				case MISDN_CTYPE_STR:
					if (port_cfg[port][place].str) {
						tris_copy_string(buf, port_cfg[port][place].str, bufsize);
					} else if (port_cfg[0][place].str) {
						tris_copy_string(buf, port_cfg[0][place].str, bufsize);
					} else
						memset(buf, 0, bufsize);
					break;
				default:
				...
				case MISDN_CTYPE_STR:
					tris_copy_string(buf, S_OR(general_cfg[place].str, ""), bufsize);
					break;
				default:
...
void misdn_cfg_get_name(enum misdn_cfg_elements elem, void *buf, int bufsize)
{
	...
	tris_copy_string(buf, spec ? spec[place].name : "", bufsize);
}
...
void misdn_cfg_get_desc (enum misdn_cfg_elements elem, void *buf, int bufsize, void *buf_default, int bufsize_default)
{
	...
	/* here comes a hack to replace the (not existing) "name" element with the "ports" element */
	if (elem == MISDN_CFG_GROUPNAME) {
		tris_copy_string(buf, ports_description, bufsize);
	...
	if (!spec || !spec[place].desc)
		...
		tris_copy_string(buf, spec[place].desc, bufsize);
		if (buf_default && bufsize) {
			if (!strcmp(spec[place].def, NO_DEFAULT))
				memset(buf_default, 0, 1);
			else
				tris_copy_string(buf_default, spec[place].def, bufsize_default);
		}
	}
}
...
int misdn_cfg_is_msn_valid (int port, char* msn)
{
	...
	if (!misdn_cfg_is_port_valid(port)) {
		tris_log(LOG_WARNING, "Invalid call to misdn_cfg_is_msn_valid! Port number %d is not valid.\n", port);
		return 0;
	}
	...
	for (; iter; iter = iter->next) 
		if (*(iter->msn) == '*' || tris_extension_match(iter->msn, msn)) {
			re = 1;
			break;
		}
	misdn_cfg_unlock();
...
void misdn_cfg_get_config_string (int port, enum misdn_cfg_elements elem, char* buf, int bufsize)
{
	...
	if ((elem < MISDN_CFG_LAST) && !misdn_cfg_is_port_valid(port)) {
		*buf = 0;
		tris_log(LOG_WARNING, "Invalid call to misdn_cfg_get_config_string! Port number %d is not valid.\n", port);
		return;
	}
	...
	if (elem == MISDN_CFG_PTP) {
		...
		case MISDN_CTYPE_ASTGROUP:
			if (port_cfg[port][place].grp)
				snprintf(buf, bufsize, " -> %s: %s", port_spec[place].name, 
						 tris_print_group(tempbuf, sizeof(tempbuf), *port_cfg[port][place].grp));
			else if (port_cfg[0][place].grp)
				snprintf(buf, bufsize, " -> %s: %s", port_spec[place].name, 
						 tris_print_group(tempbuf, sizeof(tempbuf), *port_cfg[0][place].grp));
			else
				snprintf(buf, bufsize, " -> %s:", port_spec[place].name);
			break;
		...
		*buf = 0;
		tris_log(LOG_WARNING, "Invalid call to misdn_cfg_get_config_string! Invalid config element (%d) requested.\n", elem);
	}
	misdn_cfg_unlock();
}
...
static int _parse (union misdn_cfg_pt *dest, const char *value, enum misdn_cfg_type type, int boolint_def)
{
	...
	case MISDN_CTYPE_STR:
		...
		if ((len = strlen(value))) {
			dest->str = tris_malloc((len + 1) * sizeof(char));
			strncpy(dest->str, value, len);
			dest->str[len] = 0;
		} else {
			dest->str = tris_malloc(sizeof(char));
			dest->str[0] = 0;
		}
		break;
	case MISDN_CTYPE_INT:
	{
		...
		if (res) {
			if (!dest->num) {
				dest->num = tris_malloc(sizeof(int));
			}
	...
	case MISDN_CTYPE_BOOLINT:
		...
		if (sscanf(value, "%30d", &tmp)) {
			...
			*(dest->num) = (tris_true(value) ? boolint_def : 0);
		}
		break;
	case MISDN_CTYPE_MSNLIST:
		for (valtmp = strsep(&tmp2, ","); valtmp; valtmp = strsep(&tmp2, ",")) {
			if ((len = strlen(valtmp))) {
				struct msn_list *ml = tris_malloc(sizeof(*ml));
				ml->msn = tris_calloc(len+1, sizeof(char));
	...
	case MISDN_CTYPE_ASTGROUP:
		if (!dest->grp) {
			dest->grp = tris_malloc(sizeof(tris_group_t));
		}
		*(dest->grp) = tris_get_group(value);
		break;
	}
...
static void _build_general_config (struct tris_variable *v)
{
	int pos;
...
static void _build_port_config (struct tris_variable *v, char *cat)
{
	int pos, i;
	...
	for (; v; v = v->next) {
		if (!strcasecmp(v->name, "ports")) {
			char *token, *tmp = tris_strdupa(v->value);
...
void misdn_cfg_update_ptp (void)
{
	...
	if (!tris_strlen_zero(misdn_init)) {
		...
		if (fp) {
			...
			tris_log(LOG_WARNING,"Couldn't open %s: %s\n", misdn_init, strerror(errno));
		}
	}
#else
	int i;
	...
	for (i = 1; i <= max_ports; ++i) {
		...
		fp = fopen(filename, "r");
		if (!fp) {
			tris_log(LOG_WARNING, "Could not open %s: %s\n", filename, strerror(errno));
			continue;
		}
		if (fscanf(fp, "0x%08x", &proto) != 1)
			tris_log(LOG_WARNING, "Could not parse contents of %s!\n", filename);
		else
			ptp[i] = proto & 1<<5 ? 1 : 0;
		fclose(fp);
	}
...
void misdn_cfg_destroy (void)
{
	...
	tris_free(port_cfg);
	tris_free(general_cfg);
	tris_free(ptp);
	tris_free(map);

	misdn_cfg_unlock();
	tris_mutex_destroy(&config_mutex);
}
...
int misdn_cfg_init(int this_max_ports, int reload)
{
	...
	if (!(cfg = tris_config_load2(config, "chan_misdn", config_flags)) || cfg == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_WARNING, "missing or invalid file: misdn.conf\n");
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED)
		return 0;

	tris_mutex_init(&config_mutex);
	...
	if (this_max_ports) {
		...
		map = tris_calloc(MISDN_GEN_LAST + 1, sizeof(int));
		if (_enum_array_map())
			return -1;
		p = tris_calloc(1, (max_ports + 1) * sizeof(union misdn_cfg_pt *)
		...
		general_cfg = tris_calloc(1, sizeof(union misdn_cfg_pt *) * NUM_GEN_ELEMENTS);
		ptp = tris_calloc(max_ports + 1, sizeof(int));
	}
	else {
		/* misdn reload */
		_free_port_cfg();
...
struct tris_jb_conf *misdn_get_global_jbconf() {
