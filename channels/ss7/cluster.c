...
static pthread_t cluster_thread = TRIS_PTHREADT_NULL;
...
static void set_socket_opt(int s, int l, int o, int v)
{
  ...
  if ((err = setsockopt(s, l, o, &v, len)) < 0) {
    tris_log(LOG_WARNING, "Cannot set socket option, %s\n", strerror(errno));
  }
}
...
static void declare_host_state(struct host* host, alivestate state)
{
  if (host->state != state) {
	...
	for (i = 0; i < host->n_spans; i++) {
      ...
      tris_log(LOG_WARNING, "No alive signal from %s, assumed down.\n", host->name);
    }
    else if (state == STATE_ALIVE) {
      tris_log(LOG_WARNING, "Alive signal from %s, now up.\n", host->name);
    }
  }
}
...
static void add_sender(struct host* host, struct in_addr addr, int hostix)
{
  if (find_sender(host, addr) != -1) {
    tris_log(LOG_NOTICE, "Cluster has multiple identical entries: host %s %s\n", host->name, inaddr2s(addr));
    return;
  }
...
static void check_senders(void)
{
  ...
  for (i = 0; i < n_senders; i++) {
    ...
    if ((tdiff > CLUSTER_ALIVE_TIMEOUT) && (senders[i].state == STATE_ALIVE)) {
      tris_log(LOG_WARNING, "No alive signal from %s %s, for %d msec\n", senders[i].host->name, inaddr2s(senders[i].addr), tdiff);
...
static void cluster_put(int linkix, unsigned char* buf, int len)
{
  ...
  if(res) {
    tris_log(LOG_ERROR, "Cluster receive buffer full, packet lost.\n");
    /* Todo FIFO full ... */
  } else {
    res = write(receivepipe[1], &linkix, sizeof(linkix));
    if (res < 0) {
      tris_log(LOG_NOTICE, "Could not write cluster event pipe: %s.\n", strerror(errno));
    }
  }
}
...
void cluster_mtp_received(struct link* link, struct mtp_event* event)
{
  if (!cluster_running || !this_host->n_receivers)
    return;
  tris_log(LOG_DEBUG, "cluster mtp received on link '%s', typ=%d\n", link ? link->name : "?", event->typ);
...
void cluster_mtp_sent(struct link* link, struct mtp_req* req)
{
  if (!cluster_running || !this_host->n_receivers)
    return;
  tris_log(LOG_DEBUG, "cluster mtp sent on link '%s', typ=%d\n", link ? link->name : "?", req->typ);
...
void cluster_mtp_forward(struct mtp_req* req)
{
  ...
  if (!cluster_running)
    return;
  tris_log(LOG_DEBUG, "cluster mtp forward, link %s, typ=%d, len=%d\n", link ? link->name : "?", req->typ, req->len);
...
static int setup_receiver_socket(void)
{
  ...
  if (receiver_socket < 0) {
    tris_log(LOG_ERROR, "Cannot create receiver socket, errno=%d: %s\n", errno, strerror(errno));
    return -1;
  }
  ...
  if (listen(receiver_socket, MAX_HOSTS) < 0) {
    tris_log(LOG_ERROR, "Cannot listen on receiver socket, errno=%d: %s\n", errno, strerror(errno));
    close(receiver_socket);
    receiver_socket = -1;
    return -1;
  }
...
static void connect_receiver(int receiverix, int targetix)
{
  ...
  res = fcntl(s, F_GETFL);
  if(res < 0) {
    tris_log(LOG_WARNING, "SS7: Could not obtain flags for socket fd: %s.\n", strerror(errno));
    return;
  }
  flags = res | O_NONBLOCK;
  res = fcntl(s, F_SETFL, flags);
  if(res < 0) {
    tris_log(LOG_WARNING, "SS7: Could not set socket fd non-blocking: %s.\n", strerror(errno));
    return;
  }
  tris_log(LOG_DEBUG, "Trying to connect to %s %s\n", host_name, inaddr2s(sock.sin_addr));

  if (connect(s, &sock, sizeof(sock)) < 0) {
    if (errno != EINPROGRESS) {
      tris_log(LOG_ERROR, "Cannot connect receiver socket %s, %s\n", inaddr2s(sock.sin_addr), strerror(errno));
      close(s);
      return;
    }
...
static void cluster_send_packets(struct receiver* receiver, unsigned char* buf, int len)
{
  ...
  for (targetix = 0; targetix < receiver->n_targets; targetix++) {
    tris_log(LOG_DEBUG, "send packets %s, targetix %d, connected %d\n", receiver->targets[targetix].host->name, targetix, receiver_stat->target[targetix].connected);
...
static int cluster_receive_packet(int senderix, int fd)
{
  ...
  res = read(fd, buf, sz);
  if (res <= 0) {
    tris_log(LOG_NOTICE, "Could not read received packet: %s.\n", strerror(errno));
    return -1;
  }
  else if (res == 0) {
    tris_log(LOG_NOTICE, "Received 0 bytes, closing socket: %s.\n", strerror(errno));
  ...
  if (event->typ < MTP_EVENT_ALIVE) {
    ...
    //    tris_log(LOG_DEBUG, "Received mtp req %d, buff len %d, res %d\n", req->typ, req->len, res);
  }
  else {
  ...
  if (res > 0) {
    tris_log(LOG_DEBUG, "Received event, senderix=%d, hostix=%d, lastseq=%ld, seqno=%ld, typ=%d\n", senderix, hostix, host_last_seq_no[hostix], event->seq_no, event->typ);
...
static void *cluster_thread_main(void *data)
{
  ...
  tris_verbose_ss7(VERBOSE_PREFIX_3 "Starting cluster thread, pid=%d.\n", getpid());
  ...
  while (cluster_running) {
    ...
    timeout = tris_sched_wait(cluster_sched);
	...
	case FD_PIPE: {
	  if(fds[i].revents & POLLIN) {
	    ...
	    if (res < 0) {
	      tris_log(LOG_NOTICE, "Could not read cluster event pipe: %s.\n", strerror(errno));
	      continue;
	    }
	    if ((res = lffifo_get(receivebuf, fifobuf, sizeof(fifobuf))) != 0) {
	      if(res < 0) {
		tris_log(LOG_ERROR, "Got oversize packet in cluster receive buffer.\n");
		continue;
	      }
	    }
	    tris_log(LOG_DEBUG, "fifo get res %d, typ %d, linkix %d, link %s\n", res, req->typ, linkix, links[linkix].name);
	    if (res > 0) {
		...
		if (links[linkix].receiver) {
		  ...
		  tris_log(LOG_WARNING, "No way to send packet to cluster, link='%s', reqtype=%d\n", links[linkix].name, req->typ);
		}
	      }
	    }
	...
	case FD_LISTEN: {
	  if(fds[i].revents & POLLIN) {
		...
		if (senderix >= 0) {
		  ...
		  tris_log(LOG_NOTICE, "Got socket connection from unexpected sender %s %s\n", host->name, inaddr2s(from_addr.sin_addr));
		}
	      }
	      tris_log(LOG_NOTICE, "Accepted socket connection from %s, fd %d\n", host?host->name : "unknown", afd);
	    }
	    else {
	      tris_log(LOG_WARNING, "Accept of receiver connection failed: %s.\n", strerror(errno));
	    }
	    break;
	  }
	}
	case FD_ACCEPTED: {
	  ...
	  if((err == -1) || (fds[i].revents & (POLLERR|POLLNVAL))) {
	    ...
	    tris_log(LOG_NOTICE, "Got error on accepted socket: %d %s\n", i, strerror(error));
	    close(fds[i].fd);
	...
	case FD_INPROGRESS: {
	  ...
	  if(fds[i].revents & (POLLERR|POLLNVAL)) {
	    if (receiver_stat->target[targetix].reported++ % 100 == 0) {
	      ...
	      unsigned int len = sizeof(error);
	      getsockopt(fds[i].fd, SOL_SOCKET, SO_ERROR, &error, &len);
	      tris_log(LOG_NOTICE, "Socket connection failed to host: %s, inf %s, addr %s, error: %s\n", host_name, if_name, inaddr2s(receiver->targets[targetix].inf->addr), strerror(error));
	    }
	    disconnect_receiver(receiver, targetix);
	  }
	  else if(fds[i].revents & (POLLHUP)) {
	    tris_log(LOG_NOTICE, "Lost connection to receiver host: %s, inf %s, addr %s\n", host_name, if_name, inaddr2s(receiver->targets[targetix].inf->addr));
	    disconnect_receiver(receiver, targetix);
	  }
	  else if(fds[i].revents & (POLLIN|POLLOUT)) {
	    tris_log(LOG_NOTICE, "Connected to receiver host: %s, inf %s, addr %s \n", host_name, if_name, inaddr2s(receiver->targets[targetix].inf->addr));
...
static void build_sender_list(void)
{
  ...
  if (!n_senders) {
    tris_log(LOG_DEBUG, "Found no senders to supervise\n");
  }
}
...
static void wait_for_connections(void)
{
  ...
  for (cnt = 0; cnt < 800; cnt++) {
    ...
    if (cnt % 100 == 0)
      tris_log(LOG_DEBUG, "wait %d %d %d %d\n", n, c, n_accepted, n_senders);
		 ...
		 void (*isup_block_handler_callback)(struct link*))
{
  ...
  if(res < 0) {
    tris_log(LOG_ERROR, "Unable to allocate cluster event pipe: %s.\n",
            strerror(errno));
    goto fail;
  }
  res = fcntl(receivepipe[0], F_GETFL);
  if(res < 0) {
    tris_log(LOG_ERROR, "Could not obtain flags for read end of "
  ...
  if(res < 0) {
    tris_log(LOG_ERROR, "Could not set read end of cluster event pipe "
            "non-blocking: %s.\n", strerror(errno));
    goto fail;
  }
  res = fcntl(receivepipe[1], F_GETFL);
  if(res < 0) {
    tris_log(LOG_ERROR, "Could not obtain flags for write end of "
            "cluster event pipe: %s.\n", strerror(errno));
    goto fail;
  }
  flags = res | O_NONBLOCK;
  ...
  if(res < 0) {
    tris_log(LOG_ERROR, "Could not set write end of cluster event pipe "
            "non-blocking: %s.\n", strerror(errno));
    goto fail;
  }
  cluster_sched = sched_context_create();
  if(cluster_sched == NULL) {
    tris_log(LOG_ERROR, "Unable to create cluster scheduling context.\n");
    goto fail;
  }

  cluster_running = 1;          /* Otherwise there is a race, and
                                   cluster may exit immediately */
  if(tris_pthread_create(&cluster_thread, NULL, cluster_thread_main, NULL) < 0) {
    tris_log(LOG_ERROR, "Unable to start cluster thread.\n");
    cluster_running = 0;
    goto fail;
  }
  ...
  if(res != 0) {
    tris_log(LOG_WARNING, "Failed to set cluster thread to realtime priority: %s.\n",
            strerror(res));
  }
...
int cmd_cluster_status(int fd, int argc, char *argv[])
{
  ...
  for (linkix = 0; linkix < this_host->n_receivers; linkix++) {
    for (targetix = 0; targetix < this_host->receivers[linkix].n_targets; targetix++) {
      ...
      tris_cli(fd, "receiver %s if %s, addr %s, c:%s, p:%s, last try %d msec, %d fails, %lu forwards\n",
