...
static int load_config_jitter(struct tris_config *cfg)
{
  ...
  v = tris_variable_browse(cfg, "jitter");
  while(v != NULL) {
    tris_jb_read_conf_ss7(&global_jbconf, v->name, v->value);
    v = v->next;
  }
...
struct tris_jb_conf *ss7_get_global_jbconf() {
...
static int make_host_schannels(void)
{
  ...
  if (this_host->n_spans == 0) {
    tris_log(LOG_ERROR, "No links defined in configuration for host '%s'.\n", this_host->name);
    return -1;
  }
  ...
  if (!llink) {
    tris_log(LOG_ERROR, "No links enabled on host '%s'.\n", this_host->name);
    return -1;
  }
  return 0;
}
...
static struct host* find_my_host(void)
{
  ...
  if ((host = lookup_host(buf)) == NULL) {
    tris_log(LOG_ERROR, "Unable to find host configuration for host '%s'.\n", buf);
  }
  return host;
}
...
static void show_config(void)
{
  ...
  for (i = 0; i < n_hosts; i++) {
    tris_log(LOG_DEBUG, "Host %s, links %d, ifs %d\n", hosts[i].name, hosts[i].n_spans, hosts[i].n_ifs);
  }
}

static int load_config_linkset(struct tris_config *cfg, char* cat)
{
  struct tris_variable *v;
  ...
  if (n_linksets == MAX_LINKSETS) {
    tris_log(LOG_ERROR, "Too many linksets defined. Max %d\n", MAX_LINKSETS);
    return -1;
  }
  ...
  while(v != NULL) {
	...
	tris_log(LOG_ERROR, "Error invalid SS7 variant '%s'.\n", v->value);
	return -1;
      }
	...
	tris_log(LOG_ERROR, "Error invalid hunting policy '%s'.\n", v->value);
	return -1;
      }
      has_hunt_policy = 1;
    } else if(0 == strcasecmp(v->name, "enabled")) {
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for enabled entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
      ...
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for use_connect entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
      ...
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for enable_st entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
      linkset->enable_st = strcasecmp(v->value, "yes") == 0;
      has_enable_st = 1;
	} else if(0 == strcasecmp(v->name, "is_st")) {
		if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for enable_st entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
		}
		linkset->is_st = strcasecmp(v->value, "yes") == 0;
	} else if(0 == strcasecmp(v->name, "is_md")) {
		if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	  tris_log(LOG_ERROR, "Invalid value '%s' for enable_st entry for linkset '%s'.\n", v->value, linkset_name);
	  return -1;
		}
		linkset->is_md = strcasecmp(v->value, "yes") == 0;
	...
	tris_log(LOG_ERROR, "Invalid value '%s' for subservice entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
      has_subservice = 1;
	...
	tris_log(LOG_ERROR, "Invalid value '%s' for loadshare entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
    } else if(0 == strcasecmp(v->name, "t35")) {
      char action_buf[100];
      if(sscanf(v->value, "%d,%s", &linkset->t35_value, action_buf) != 2) {
	tris_log(LOG_ERROR, "Invalid synax in '%s' for t35 entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
	...
	tris_log(LOG_ERROR, "Invalid t35 action '%s'.\n", action_buf);
	return -1;
      }
    } else if(0 == strcasecmp(v->name, "dni_chunk_limit")) {
      if(sscanf(v->value, "%d", &linkset->dni_chunk_limit) != 1) {
	tris_log(LOG_ERROR, "Invalid synax in '%s' for dni_chunk_limit entry for linkset '%s'.\n", v->value, linkset_name);
	return -1;
      }
      if (linkset->dni_chunk_limit < 0 || linkset->dni_chunk_limit > 99) {
        tris_log(LOG_ERROR, "Invalid value '%s' for config option '%s', aborting.\n", v->value, v->name);
        return -1;
      }
	...
	{
	  tris_log(LOG_ERROR, "Invalid value '%s' for grs entry for linkset '%s'.\n", v->value, linkset_name);
	  return -1;
	}
	...
	linkset->grs = 0;
      ...
      tris_log(LOG_ERROR, "Unknown config option '%s', aborting.\n", v->name);
      return -1;
    }
  ...
  if (!has_hunt_policy) {
    tris_log(LOG_ERROR, "Missing hunt_policy entry for linkset '%s'\n", linkset_name);
    return -1;
  }
  if (!has_enabled) {
    tris_log(LOG_ERROR, "Missing enabled entry for linkset '%s'\n", linkset_name);
    return -1;
  }
  if (!has_use_connect) {
    tris_log(LOG_ERROR, "Missing use_connect entry for linkset '%s'\n", linkset_name);
    return -1;
  }
  if (!has_enable_st) {
    tris_log(LOG_ERROR, "Missing enable_st entry for linkset '%s'\n", linkset_name);
    return -1;
  }
  if (!has_subservice) {
    tris_log(LOG_ERROR, "Missing subservice entry for linkset '%s'\n", linkset_name);
    return -1;
  }
  linkset->context = strdup(context);
  linkset->language = strdup(language);
  if (!has_context)
    tris_log(LOG_NOTICE, "Using default context '%s' for linkset '%s'\n", linkset->context, linkset_name);
  if (!has_language)
    tris_log(LOG_NOTICE, "Using default language '%s' for linkset '%s'\n", linkset->language, linkset_name);
...
static int load_config_link(struct tris_config *cfg, char* cat)
{
  struct tris_variable *v;
  ...
  if (lookup_link(link_name)) {
    tris_log(LOG_ERROR, "Links '%s' defined twice.\n", link_name);
    return -1;
  }
  if (n_links == MAX_LINKS) {
    tris_log(LOG_ERROR, "Too many links defined while parsing config for link '%s' (max %d).\n", link_name, MAX_LINKS);
    return -1;
  }
  ...
  *link->mtp3server_port = 0;
  ...
  while(v != NULL) {
    if(0 == strcasecmp(v->name, "linkset")) {
      ...
      if (!linkset) {
	tris_log(LOG_ERROR, "Linkset '%s' not found while parsing link '%s'.\n", v->value, link_name);
	return -1;
      }
      has_linkset = 1;
    } else if(0 == strcasecmp(v->name, "enabled")) {
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for enabled entry for link '%s'.\n", v->value, link_name);
	return -1;
      }
      ...
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for sltm entry for link '%s'.\n", v->value, link_name);
	return -1;
      }
      link->send_sltm = strcasecmp(v->value, "yes") == 0;
    } else if(0 == strcasecmp(v->name, "auto_block")) {
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for auto_block entry for link '%s'.\n", v->value, link_name);
	return -1;
      }
      link->auto_block = strcasecmp(v->value, "yes") == 0;
    } else if(0 == strcasecmp(v->name, "firstcic")) {
      if(sscanf(v->value, "%d", &link->first_cic) != 1) {
	tris_log(LOG_ERROR, "Invalid firstcic entry '%s' for link '%s'.\n", v->value, link_name);
	return -1;
      }
      has_firstcic = 1;
    } else if(0 == strcasecmp(v->name, "channels")) {
      link->channelmask = 0;
      tris_copy_string(chan_spec_buf, v->value, sizeof(chan_spec_buf));
	...
	if(sscanf(p, "%d-%d", &first, &last) != 2 ||
	  ...
	  tris_log(LOG_DEBUG, "Channel range '%s' is %d %d \n", p, first,last);
	  tris_log(LOG_ERROR, "Illegal channel range '%s' for "
	...
	if(sscanf(v->value, "%d,%[^:]:%s", &link->schannel, host, port) == 3) {
	  ...
	  tris_log(LOG_ERROR, "Invalid schannel entry '%s' for link '%s'.\n", v->value, link_name);
	  return -1;
	}
      }
      has_schannel = 1;
      ...
      if (strcasecmp(v->value, "no") == 0) {
        ...
        tris_log(LOG_ERROR, "Invalid value '%s' for echocancel entry for link '%s'.\n", v->value, link_name);
      ...
      {
        tris_log(LOG_ERROR, "Invalid value '%s' for echocan_train entry for "
      ...
      {
        tris_log(LOG_ERROR, "Invalid value '%s' for echocan_taps entry for "
        ...
        if (sscanf(v->value, "%f", &link->rxgain) != 1) {
            tris_log(LOG_WARNING, "Invalid rxgain: %s\n", v->value);
            link->rxgain = 0.0;
        }

    } else if(0 == strcasecmp(v->name, "txgain")) {
        if (sscanf(v->value, "%f", &link->txgain) != 1) {
            tris_log(LOG_WARNING, "Invalid txgain: %s\n", v->value);
            link->txgain = 0.0;
        }
    } else if(0 == strcasecmp(v->name, "initial_alignment")) {
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for initial_alignment entry for link '%s'.\n", v->value, link_name);
	return -1;
      }
  ...
  if (!has_linkset) {
    tris_log(LOG_ERROR, "Missing linkset entry for link '%s'.\n", link_name);
    return -1;
  }
  if (!has_enabled) {
    tris_log(LOG_ERROR, "Missing enabled entry for link '%s'.\n", link_name);
    return -1;
  }
  if (!has_firstcic) {
    tris_log(LOG_ERROR, "Missing firstcic entry for link '%s'.\n", link_name);
    return -1;
  }
  if (!has_channels) {
    tris_log(LOG_ERROR, "Missing channels entry for link '%s'.\n", link_name);
    return -1;
  }
  if (!has_schannel) {
    tris_log(LOG_ERROR, "Missing schannel entry for link '%s'.\n", link_name);
    return -1;
  }

  if (linkset->n_links == MAX_LINKS_PER_LINKSET) {
    tris_log(LOG_ERROR, "Too many links defined for linkset '%s' for link '%s' (max %d).\n", linkset->name, link_name, MAX_LINKS_PER_LINKSET);
    return -1;
  }
  ...
  if (link->enabled) {
	...
	linkset->schannels[linkset->n_schannels++] = link;
      ...
      tris_log(LOG_NOTICE, "Disabling link '%s' on disabled linkset '%s'\n", link->name, linkset->name);
      link->enabled = 0;
    }
  }
  tris_log(LOG_NOTICE, "%s link '%s' on linkset '%s', firstcic=%d\n", link->enabled ? "Configured" : "Ignoring disabled", link->name, linkset->name, link->first_cic);
...
static int load_config_host(struct tris_config *cfg, char* cat)
{
  struct tris_variable *v;
  ...
  if (n_hosts == MAX_HOSTS) {
    tris_log(LOG_ERROR, "Too many hosts defined while parsing config for host '%s' (max %d).\n", host_name, MAX_HOSTS);
    return -1;
  }
  ...
  while(v != NULL) {
	...
	tris_log(LOG_ERROR, "Error: Invalid number '%s' for "
	...
	return -1;
      ...
      tris_copy_string(dpc_spec_buf, v->value, sizeof(dpc_spec_buf));
      spec = &dpc_spec_buf[0];
      p = strsep(&spec, ",");
      while(p && *p) {
	int dpc;
	...
	if(sscanf(p, "%[^:]:%s", linkset_name_buf, dpc_buf) != 2) {
	  tris_log(LOG_ERROR, "Invalid DPC specification '%s' for host '%s'", p, host_name);
	  return -1;
	}
	...
	if (!dpc) {
	  tris_log(LOG_ERROR, "Invalid DPC value '%s' for linkset '%s' for host '%s'", dpc_buf, linkset_name_buf, host_name);
	  return -1;
	}
	...
	if (!linkset) {
	  ...
	  if (!link) {
	    tris_log(LOG_ERROR, "Unknown link/linkset '%s' for host '%s'", linkset_name_buf, host_name);
	    return -1;
	  }
	  link->dpc = dpc;
	}
	...
	p = strsep(&spec, ",");
      ...
      tris_copy_string(peer_spec_buf, v->value, sizeof(peer_spec_buf));
      tris_log(LOG_DEBUG, "peers '%s' \n", peer_spec_buf);
	...
	if (host->n_peers == MAX_LINKS_PER_HOST) {
	  tris_log(LOG_ERROR, "Too many routes defined for host '%s' (max %d).\n", host->name, MAX_LINKS_PER_HOST);
	  return -1;
	}
	while ((*p == ' ') || (*p == '\t'))
	  p++;
	if(sscanf(p, "%[^:]:%s", link_buf, host_buf) != 2) {
	  tris_log(LOG_ERROR, "Invalid peer specification '%s' for host '%s'.\n", p, host_name);
	  return -1;
	}
	link = lookup_link(link_buf);
	if (!link) {
	  tris_log(LOG_ERROR, "Unknown link '%s' for peers '%s' for host '%s'", link_buf, p, host_name);
	  return -1;
	}
	...
	p = strsep(&spec, ",");
      ...
      if ((strcasecmp(v->value, "yes") != 0) && (strcasecmp(v->value, "no") != 0)) {
	tris_log(LOG_ERROR, "Invalid value '%s' for enabled entry for host '%s'.\n", v->value, host_name);
	return -1;
      }
	...
	tris_log(LOG_ERROR, "Unknown default_linkset '%s' for host '%s'.\n", v->value, host_name);
	return -1;
      }
    } else if(0 == strcasecmp(v->name, "links")) {
      tris_copy_string(links_spec_buf, v->value, sizeof(links_spec_buf));
      tris_log(LOG_DEBUG, "links '%s' \n", links_spec_buf);
	...
	tris_log(LOG_ERROR, "Too many interfaces defined for host '%s' (max %d).\n", host->name, MAX_IFS_PER_HOST);
	return -1;
      }
      if ((addr = inet_addr(v->value)) == INADDR_NONE) {
	tris_log(LOG_ERROR, "Invalid IP address '%s' for interface '%s' for host '%s'.\n", v->value, v->value, host_name);
	return -1;
      }
	  ...
	  tris_log(LOG_ERROR, "Invalid ssn value '%s' for host '%s'.\n", v->value, host_name);
	  return -1;
	}
	  ...
	  tris_log(LOG_ERROR, "Invalid globaltitle value '%s' for host '%s'.\n", v->value, host_name);
	  return -1;
      }
    } else if(0 == strcasecmp(v->name, "route")) {
      char route_spec_buf[500] = {0,};
      tris_copy_string(route_spec_buf, v->value, sizeof(route_spec_buf));
      tris_log(LOG_DEBUG, "route '%s' \n", route_spec_buf);
	...
	if(sscanf(p, "%[0-9]:%s", dest_buf, linkset_buf) != 2) {
	  if(sscanf(p, ":%s", linkset_buf) != 1) {
	    tris_log(LOG_ERROR, "Invalid route specification '%s' for host '%s'.\n", p, host_name);
	    return -1;
	  }
	  *dest_buf = '\0';
	}
	...
	if (!linkset) {
	  tris_log(LOG_ERROR, "Unknown linkset '%s' for route '%s' for host '%s'\n", linkset_buf, p, host_name);
	  return -1;
	}
  ...
  if (!has_opc) {
    tris_log(LOG_ERROR, "Missing opc entry for host '%s'.\n", host_name);
    return -1;
  }
  if (!has_dpc) {
    tris_log(LOG_ERROR, "Missing dpc entry for host '%s'.\n", host_name);
    return -1;
  }
  if (!has_links) {
    tris_log(LOG_ERROR, "Missing links entry for host '%s'.\n", host_name);
    return -1;
  }
  if (!has_enabled) {
    tris_log(LOG_ERROR, "Missing enabled entry for host '%s'.\n", host_name);
    return -1;
  }
  if (!has_if) {
    tris_log(LOG_WARNING, "Missing interface entries for host '%s'.\n", host_name);
  }
  host->n_receivers = 0;
  host->state = STATE_UNKNOWN;
  host->has_signalling_receivers = 0;
  tris_log(LOG_DEBUG, "host %s, n_spans %d \n", host->name, host->n_spans);
  n_hosts++;
  return 0;
}

static int load_config_cluster(struct tris_config *cfg)
{
  struct tris_variable *v;
  struct link* link;
  struct host* host;
  ...
  v = tris_variable_browse(cfg, "cluster");
  while(v != NULL) {
    if (strcasecmp(v->name, "port") == 0) {
      if(sscanf(v->value, "%d", &clusterlistenport) != 1) {
	tris_log(LOG_ERROR, "The port entry '%s' in cluster section is not valid.\n", v->name);
	return -1;
      }
      has_port = 1;
      v = v->next;
    ...
    if ((link = lookup_link(v->name)) == NULL) {
      tris_log(LOG_ERROR, "The link '%s' is not defined while parsing cluster category.\n", v->name);
      return -1;
    }
    host = NULL;
    for (i = 0; i < n_hosts; i++) {
	...
	if (host->n_receivers == MAX_LINKS_PER_HOST) {
	  tris_log(LOG_ERROR, "Too many receivers defined for host '%s' (max %d).\n", host->name, MAX_LINKS_PER_HOST);
	  return -1;
	}
	tris_log(LOG_DEBUG, "found link '%s'  on %s\n", v->name, host->name);
	receiver = &host->receivers[host->n_receivers];
	receiver->receiverix = host->n_receivers++;
	receiver->n_targets = 0;
	link->receiver = receiver;
	tris_copy_string(dup_spec_buf, v->value, sizeof(dup_spec_buf));
	spec = &dup_spec_buf[0];
	p = strsep(&spec, ",");
	while(p) {
	  ...
	  if(sscanf(p, "%[^#]#%s", host_name_buf, if_name_buf) != 2) {
	    tris_log(LOG_ERROR, "Invalid host#if specification '%s'.\n", p);
	    return -1;
	  }
	  if ((target_host = lookup_host(host_name_buf)) == NULL) {
	    tris_log(LOG_ERROR, "Host '%s' not found in dup spec '%s'.\n", host_name_buf, p);
	    return -1;
	  }
	  if (strncasecmp(if_name_buf, "if-", 3)) {
	    tris_log(LOG_ERROR, "Invalid interface name: '%s' in dup spec '%s'.\n", host_name_buf, p);
	    return -1;
	  }
	  for (i = 0; i < n_hosts; i++) {
	  ...
	  if (!target_if) {
	    tris_log(LOG_ERROR, "Interface '%s' not found for host '%s'.\n", if_name_buf, host_name_buf);
	    return -1;
	  }
	  if (receiver->n_targets == 2*MAX_HOSTS) {
	    tris_log(LOG_ERROR, "Too many targets defined for link '%s' (max %d).\n", link->name, 2*MAX_HOSTS);
	    return -1;
	  }
	  receiver->targets[receiver->n_targets].host = target_host;
	  receiver->targets[receiver->n_targets].inf = target_if;
	  receiver->n_targets++;
	  tris_log(LOG_DEBUG, "Added target %s#%s for link %s on host %s \n", target_host->name, target_if->name, link->name, host->name);
	  p = strsep(&spec, ",");
	}
      }
      else {
	tris_log(LOG_WARNING, "The link '%s' is not used by any host.\n", v->name);
      }
    }
    v = v->next;
  }
  if (!has_port) {
    tris_log(LOG_WARNING, "Missing port entry in cluster section");
    return -1;
  }
  return 0;
}
...
int load_config(int reload)
{
  ...
  cfg = tris_ss7config_load(conffile_name);
  if(cfg == NULL) {
  ...
  while ((prevcat = tris_category_browse(cfg, prevcat)) != NULL) {
	...
	goto fail;
      ...
      tris_log(LOG_ERROR, "Error invalid config category '%s'.\n", prevcat);
      goto fail;
    }
  }
  ...
  for (i = 0; i < n_linksets; i++) {
    ...
    tris_log(LOG_NOTICE, "Configuring DPC %d for linkset '%s'.\n", linksets[i].dpc, linksets[i].name);
  }
  for (i = 0; i < n_linksets; i++) {
    int any = 0;
    if (!linksets[i].enabled)
	...
	if (this_host->spans[k].link == linksets[i].links[j]) {
	  if (!linksets[i].dpc) {
	    tris_log(LOG_ERROR, "No DPC specified for linkset '%s'.\n", linksets[i].name);
	    goto fail;
	  }
	  any = any || linksets[i].links[j]->enabled;
	}
    linksets[i].enabled = any;
    tris_log(LOG_DEBUG, "Setting linkset %d '%s' enabled %d\n", i, linksets[i].name, any);
  }
  if (!this_host->enabled) {
    tris_log(LOG_ERROR, "Host '%s' not enabled, quitting!\n", this_host->name);
    goto fail;
  }
  if (this_host->default_linkset) {
    ...
    if (!haslinkset) {
      tris_log(LOG_ERROR, "Default linkset '%s' for host '%s' is not configured for this host!\n", this_host->default_linkset->name, this_host->name);
      goto fail;
    }
  }
  else {
	...
	if (linkset && (linkset != this_host->spans[k].link->linkset)) {
	  tris_log(LOG_ERROR, "Host '%s' has multiple linksets, need to specify a default_linkset!\n", this_host->name);
	  goto fail;
	}
