...
int param_decode(unsigned char *buf, int buflen, ...) {
  ...
  while((type = va_arg(args, typeof(type))) != 0) {
    ...
    if(i + param_len > buflen) {
      tris_log(LOG_NOTICE, "Short ISUP message for parameter type %d, "
  ...
  for(j = 0; (type = va_arg(args, typeof(type))) != 0; j++) {
    if(j >= sizeof(opt_decoders)/sizeof(opt_decoders[0])) {
      tris_log(LOG_ERROR, "Fatal: too many decoders.\n");
      return 0;
    }
  ...
  if(i >= buflen) {
    tris_log(LOG_NOTICE, "Short ISUP message for optional part, len %d < %d.\n",
            buflen, i + 1);
    return 0;
  }
  if(buf[i] == 0) {
  ...
  for(;;) {
    ...
    if(i + 2 > buflen) {
      tris_log(LOG_NOTICE, "Short ISUP message for optional parameter type %d, "
              "len %d < %d.\n", type, buflen, i + 2);
      return 0;
    }
    param_len = buf[i + 1];
...
static int decode_optional_backward_call_indicators(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'optional backward call indicator', len %d < 1.\n", len);
    return 0;
  }
...
static int decode_event_info(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'event information', len %d < 1.\n", len);
    return 0;
  }
...
static int decode_rel_cause(unsigned char *p, int len, void *data) {
  ...
  if(len < 2) {
    tris_log(LOG_NOTICE, "Short parameter 'cause indicators', len %d < 2.\n", len);
    return 0;
  }
  *cause_ptr = p[1] & 0x7f;
  return 1;
...
static int decode_suspend_resume(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'suspend/resume indicators', len %d < 1.\n", len);
    return 0;
  }
  *indicator = p[0];
  return 1;
...
static int decode_range_and_status(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'range and status', len %d < 1.\n", len);
    return 0;
  }
  parm->range = p[0];

  if(parm->range == 0) {
    tris_log(LOG_NOTICE, "Invalid range 0 (must be >= 1) in range and status.\n");
    return 0;
  }

  status_len = ((parm->range + 1) + 7)/8;
  if(len < 1 + status_len) {
    tris_log(LOG_NOTICE, "Short parameter 'range and status', len %d < %d.\n",
            len, 1 + status_len);
    return 0;
  }
...
static int decode_range_no_status(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'range and no status', len %d < 1.\n", len);
    return 0;
  }
  *range_ptr = p[0];
  return 1;
...
static int decode_cgsmti(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'circuit group supervision message "
            "type indicator', len %d < 1.\n", len);
    return 0;
  }
  cgsmti = p[0] & 0x3;
  if(cgsmti != 0 && cgsmti != 1) {
    tris_log(LOG_NOTICE, "Unimplemented 'circuit group supervision message "
...
static int decode_noci_contcheck(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'nature of connection indicators', "
            "len %d < 1.\n", len);
    return 0;
  }
...
static int decode_transmission_medium(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'Transmission medium requirement', "
            "len %d < 1.\n", len);
    return 0;
  }
...
static int decode_redir_inf(unsigned char *p, int len, void *data) {
  ...
  if(len < 1) {
    tris_log(LOG_NOTICE, "Short parameter 'redirection information', "
            "len %d < 1.\n", len);
    return 0;
  }
...
static int decode_isup_sni(unsigned char *p, int len, void *data) {
  ...
  if(len < 2) {
    tris_log(LOG_NOTICE, "Short parameter for ISUP phone number, len %d < 2.\n",
            len);
    return 0;
  }
  ...
  if(num_dig > PHONENUM_MAX) {
    tris_log(LOG_NOTICE, "Too many digits in phone number %d > %d, truncated.\n",
...
int decode_isup_phonenum(int with_presentation_restrict, unsigned char *p, int len, void *data) {
  ...
  if(with_presentation_restrict) {
    switch((p[1] >> 2) & 0x3) {
      ...
      case 3:                   /* Reserved */
        tris_log(LOG_NOTICE, "Found presentation restrict type 0x3, assuming "
      ...
      default:
        tris_log(LOG_ERROR, "This cannot happen!?!.\n");
    }
  } else {
    n->present = 1;
    n->restricted = 0;
  ...
  if(len == 2) {
    tris_log(LOG_DEBUG, "No digits in phone number.\n");
    return 1;
  }
  ...
  switch(nature_of_adr_ind) {
	...
	break;			// KimPH entered break 
    case 0x02:                  // Unknown; again experience suggests
                                // this should be international.
      tris_log(LOG_NOTICE,"National (significant) or unknown nature of address "
    ...
    case 0x04:                  // International -> add '00'.
      ...
      break;
    default:
      tris_log(LOG_NOTICE, "unknown nature of address indicator 0x%0x.\n",
              nature_of_adr_ind);
      return 0;
  }
...
int decode_isup_msg(struct isup_msg *msg, ss7_variant variant, unsigned char *buf, int len) {
  ...
  if(len < i) {
    tris_log(LOG_NOTICE, "Got short ISUP message (len=%d < %d).\n", len, i);
    return 0;
  }
  ...
  switch(msg->typ) {
    ...
    default:
      tris_log(LOG_DEBUG, "Got unknown ISUP message type %d.\n", msg->typ);
      return 0;
  }
}
...
void isup_msg_init(unsigned char *buf, int buflen, ss7_variant variant, int opc, int dpc, int cic,
  ...
  if(buflen < 7) {
    tris_log(LOG_ERROR, "Buffer too small, size %d < 7.\n", buflen);
    return;
  }
...
void isup_msg_add_fixed(unsigned char *buf, int buflen, int *current,
  ...
  if(param_len < 0 || param_len > 255) {
    tris_log(LOG_ERROR, "Unreasonable size of parameter %d.\n", param_len);
    return;
  }
  if(*current + param_len > buflen) {
    tris_log(LOG_ERROR, "Buffer too small for fixed parameter, size "
...
void isup_msg_start_variable_part(unsigned char *buf, int buflen,
  ...
  if(*current + needed_size > buflen) {
    tris_log(LOG_ERROR, "Buffer too small for variable part of ISUP message, "
...
void isup_msg_add_variable(unsigned char *buf, int buflen, int *variable_ptr,
  ...
  if(param_len < 0 || param_len > 255) {
    tris_log(LOG_ERROR, "Unreasonable size of parameter length %d.\n", param_len);
    return;
  }
  if(*variable_ptr >= *current) {
    tris_log(LOG_ERROR, "Internal: variable_ptr=%d >= current=%d.\n",
            *variable_ptr, *current);
    return;
  }
  if(*current + 1 + param_len > buflen) {
    tris_log(LOG_ERROR, "Buffer too small for variable parameter, size "
            "%d < %d.\n", buflen, *current + 1 + param_len);
    return;
  }
  if(*current - *variable_ptr > 255) {
    tris_log(LOG_ERROR, "Too much data in variable part, %d > 255.\n",
...
void isup_msg_start_optional_part(unsigned char *buf, int buflen, int *variable_ptr,
  ...
  if(*current + 1 > buflen) {   /* The "+1" is room for the end marker */
    tris_log(LOG_ERROR, "Buffer too small for optional parameter, size "
            "%d < %d.\n", buflen, *current + 1);
    return;
  }
  if(*current - *variable_ptr > 255) {
...
void isup_msg_add_optional(unsigned char *buf, int buflen, int *current,
  ...
  if(*current + 2 + param_len > buflen) {
    tris_log(LOG_ERROR, "Buffer too small for optional parameter, "
...
/* Only call this if at letris one optional parameter was added. */
void isup_msg_end_optional_part(unsigned char *buf, int buflen, int *current) {
  if(*current + 1 > buflen) {
    tris_log(LOG_ERROR, "Buffer too small for optional parameter end marker, "
            "size %d < %d.\n", buflen, *current + 1);
    return;
  }
