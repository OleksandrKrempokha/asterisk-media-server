...
char tris_config_TRIS_CONFIG_DIR[128];
...
static void process_event(struct mtp_event* event)
{
  ...
  case MTP_EVENT_STATUS:
	...
	tris_log(LOG_WARNING, "MTP is now INSERVICE for linkset '%s'.\n", link->linkset->name);
...
void l4sccp_event(struct mtp_event* event)
{
  ...
  if (n == n_registry) {
    tris_log(LOG_ERROR, "Unhandled SCCP event, OPC=%d, DPC=%d, typ=%d\n", opc, dpc, typ);
  }
}
...
void l4isup_event(struct mtp_event* event)
{
  ...
  if(!res) {
    ...
    tris_log(LOG_ERROR, "Unhandled ISUP event, OPC=%d, DPC=%d, CIC=%d, typ=%s\n", opc, dpc,  cic, isupmsg(isup_msg.typ));
  }
}
...
static void mtp_mainloop(void)
{
  ...
  tris_verbose_ss7(VERBOSE_PREFIX_3 "Starting mtp mainloop, pid=%d.\n", getpid());
  ...
  for (i = 0; i < this_host->n_schannels; i++) {
    ...
    if (strcmp(link->mtp3server_host, this_host->name) == 0) {
      ...
      if (link->mtp3fd == -1) {
        tris_log(LOG_ERROR, "Could not setup mtp3 listen port %d, %d:%s\n", port, errno, strerror(errno));
	return;
      }
  ...
  loop:
	...
	if(res == 0) {
	  ...
	  if(errno == EAGAIN || errno == EWOULDBLOCK) {
	    ...
	    tris_log(LOG_WARNING, "Error reading mtp3 socket %d, errno=%d: %s.\n", servsock, errno, strerror(errno));
	    close_socket(servsock, asockets, &n_asockets, i-n_listen);
	    rebuild_fds = 1;
	    break;
	  }
...
#undef inet_ntoa
	  ...
	  if ((req->typ == MTP_REQ_ISUP) || (req->typ == MTP_REQ_SCCP)) {
	    ...
	    do {
	      ...
	      if (res == 0) {
		tris_log(LOG_WARNING, "Unexpectec EOF on mtp3 socket %d\n", servsock);
		close_socket(servsock, asockets, &n_asockets, i-n_listen);
		rebuild_fds = 1;
		goto loop;
	      }
	  ...
	  case MTP_REQ_REGISTER_L4:
		...
		tris_log(LOG_ERROR, "Too many client connections\n");
		break;
	      }
	  ...
	  case MTP_REQ_ISUP:
		...
		if ((memcmp((void*) &registry[i].client, (void*) &from, sizeof(from)) == 0) &&
		  ...
		  if (!mtp_send_fifo) {
		    tris_log(LOG_ERROR, "Send fifo not ready for link '%s'\n", link->name);
		    break;
		  }
		  if (!mtp_send_fifo[link->linkset->lsi]) {
		    tris_log(LOG_ERROR, "Send fifo missing for link '%s', lsi %d\n", link->name, link->linkset->lsi);
		    break;
		  }
		  req->isup.link = NULL;
	  ...
	  case MTP_REQ_SCCP:
	    ...
	    break;
	  default:
	    tris_log(LOG_NOTICE, "Unknown req type %d.\n", req->typ);
	    break;
	  }
	}
      }
...
int main(int argc, char* argv[])
{
  ...
  strcpy(tris_config_TRIS_CONFIG_DIR, "/etc/trismedia");
  
  while ((c = getopt(argc, argv, "c:m:dp")) != -1) {
    switch (c) {
    case 'c':
      strcpy(tris_config_TRIS_CONFIG_DIR, optarg);
      break;
    case 'm':
