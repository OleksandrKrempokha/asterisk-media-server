...
#include "trismedia/utils.h"	/* tris_calloc() */
...
typedef struct tris_frame *(*encoder_encap_f)(struct fbuf_t *, int mtu,
		struct tris_frame **tail);
...
static uint32_t getbits(struct bitbuf *b, int n)
{
	...
	uint32_t retval = 0;
	if (n> 31) {
		tris_log(LOG_WARNING, "too many bits %d, max 32\n", n);
		return 0;
	}
	if (n + b->ofs > b->bitsize) {
		tris_log(LOG_WARNING, "bitbuf overflow %d of %d\n", n + b->ofs, b->bitsize);
		n = b->bitsize - b->ofs;
	}
...
static void check_h261(struct fbuf_t *b)
{
	...
	if (x != 0x10) {
		tris_log(LOG_WARNING, "bad PSC 0x%x\n", x);
		return;
	}
	x = getbits(&a, 5);	/* temporal reference */
	y = getbits(&a, 6);	/* ptype */
	if (0)
	tris_log(LOG_WARNING, "size %d TR %d PTY spl %d doc %d freeze %d %sCIF hi %d\n",
		b->used,
		x,
		(y & 0x20) ? 1 : 0,
		(y & 0x10) ? 1 : 0,
	...
	while ( (x = getbits(&a, 1)) == 1)
		tris_log(LOG_WARNING, "PSPARE 0x%x\n", getbits(&a, 8));
	// tris_log(LOG_WARNING, "PSPARE 0 - start GOB LAYER\n");
	while ( (x = bitbuf_left(&a)) > 0) {
		// tris_log(LOG_WARNING, "GBSC %d bits left\n", x);
		x = getbits(&a, 16); /* GBSC 0000 0000 0000 0001 */
		if (x != 0x1) {
			tris_log(LOG_WARNING, "bad GBSC 0x%x\n", x);
			break;
		}
		x = getbits(&a, 4);	/* group number */
		y = getbits(&a, 5);	/* gquant */
		if (x == 0) {
			tris_log(LOG_WARNING, "  bad GN %d\n", x);
			break;
		}
		while ( (x = getbits(&a, 1)) == 1)
			tris_log(LOG_WARNING, "GSPARE 0x%x\n", getbits(&a, 8));
...
void dump_buf(struct fbuf_t *b)
{
	...
	tris_log(LOG_WARNING, "buf size %d of %d\n", b->used, b->size);
	for (i = 0; i < b->used; i++) {
		x = i & 0xf;
		if ( x == 0) {	/* new line */
			if (i != 0)
...
/*!
 * Build an tris_frame for a given chunk of data, and link it into
 * the queue, with possibly 'head' bytes at the beginning to
 * fill in some fields later.
 */
static struct tris_frame *create_video_frame(uint8_t *start, uint8_t *end,
	               int format, int head, struct tris_frame *prev)
{
	int len = end-start;
	uint8_t *data;
	struct tris_frame *f;

	data = tris_calloc(1, len+head);
	f = tris_calloc(1, sizeof(*f));
	if (f == NULL || data == NULL) {
		tris_log(LOG_WARNING, "--- frame error f %p data %p len %d format %d\n",
				f, data, len, format);
		if (f)
			tris_free(f);
		if (data)
	...
	f->data.ptr = data;
	f->mallocd = TRIS_MALLOCD_DATA | TRIS_MALLOCD_HDR;
	//f->has_timing_info = 1;
	//f->ts = tris_tvdiff_ms(tris_tvnow(), out->ts);
	f->datalen = len+head;
	...
	if (prev)
	        TRIS_LIST_NEXT(prev, frame_list) = f;
	...
	int sbit, int ebit)
{
	...
	if (b->data == NULL) {
		...
		b->used = 0;
		b->ebit = 0;
		b->data = tris_calloc(1, b->size);
	} else if (b->used + need > b->size) {
		b->size = b->used + need;
		b->data = tris_realloc(b->data, b->size);
	}
	if (b->data == NULL) {
		tris_log(LOG_WARNING, "alloc failure for %d, discard\n",
			b->size);
		return 1;
	}
	if (b->used == 0 && b->ebit != 0) {
		tris_log(LOG_WARNING, "ebit not reset at start\n");
		b->ebit = 0;
	}
	dst = b->data + b->used;
	...
	if (i == 0) {	/* easy case, just append */
		...
		tris_log(LOG_WARNING, "must handle shift %d %d at %d\n",
			b->ebit, sbit, b->used);
		return 1;
	}
	memcpy(dst, src, len);
 ...
 * calling the encoder, encapsulating the bitstream in tris_frames,
 * extracting payload from tris_frames, and calling the decoder.
 */
 ...
 */
static struct tris_frame *h263p_encap(struct fbuf_t *b, int mtu,
	struct tris_frame **tail)
{
	struct tris_frame *cur = NULL, *first = NULL;
	uint8_t *d = b->data;
	int len = b->used;
	...
	for (;len > 0; len -= l, d += l) {
		...
		if (l < 1 || l > mtu) {
			tris_log(LOG_WARNING, "--- frame error l %d\n", l);
			break;
		}
		...
		f = create_video_frame(d, d+l, TRIS_FORMAT_H263_PLUS, h, cur);
		if (!f)
			break;
...
static int h263p_decap(struct fbuf_t *b, uint8_t *data, int len)
{
	...
	if (len < 2) {
		tris_log(LOG_WARNING, "invalid framesize %d\n", len);
		return 1;
	}
...
static int ffmpeg_encode(struct video_out_desc *v)
{
	...
	if (i > 0) {
		tris_log(LOG_WARNING, "have %d more bytes\n", i);
		b->used += i;
	}
	return 0;
}
...
static int ffmpeg_decode(struct video_dec_desc *v, struct fbuf_t *b)
{
	...
	while (srclen) {
		...
		if (full_frame == 1)	/* full frame */
			break;
		if (ret < 0) {
			tris_log(LOG_NOTICE, "Error decoding\n");
			break;
		}
	}
 ...
 */
static struct tris_frame *h263_encap(struct fbuf_t *b, int mtu,
		struct tris_frame **tail)
{
	uint8_t *d = b->data;
	int start = 0, i, len = b->used;
	struct tris_frame *f, *cur = NULL, *first = NULL;
...
#define H263_MIN_LEN	6
	...
	for (i = H263_MIN_LEN, start = 0; start < len; start = i, i += 3) {
		//tris_log(LOG_WARNING, "search at %d of %d/%d\n", i, start, len);
		for (; i < len ; i++) {
			uint8_t x, rpos, lpos;
			...
			if (rpos == 0x80) {	/* lucky case */
				...
				tris_log(LOG_WARNING, "unaligned GBSC 0x%x %d\n",
					rpos, rpos_i);
			}
			break;
		}
		 ...
		 */
		f = create_video_frame(d + start, d+i, TRIS_FORMAT_H263,
				pheader_len, cur);
...
static int h263_decap(struct fbuf_t *b, uint8_t *data, int len)
{
	...
	if ( (data[0] & 0x80) == 0) {
		...
		tris_log(LOG_WARNING, "unsupported mode 0x%x\n",
			data[0]);
		return 1;
	}
 ...
 */
static struct tris_frame *h261_encap(struct fbuf_t *b, int mtu,
		struct tris_frame **tail)
{
	uint8_t *d = b->data;
	int start = 0, i, len = b->used;
...
#define H261_MIN_LEN 10
	...
	for (i = H261_MIN_LEN, start = 0; start < len - 1; start = i, i += 4) {
		 ...
		 * We do not split it yet even if larger than MTU.
		 */
		f = create_video_frame(d + start, d+i, TRIS_FORMAT_H261,
				pheader_len, cur);
...
static struct tris_frame *mpeg4_encap(struct fbuf_t *b, int mtu,
	struct tris_frame **tail)
{
	struct tris_frame *f, *cur = NULL, *first = NULL;
	uint8_t *d = b->data;
	...
	for (;d < end; d += len, cur = f) {
		len = MIN(mtu, end - d);
		f = create_video_frame(d, d + len, TRIS_FORMAT_MP4_VIDEO, 0, cur);
		if (!f)
			break;
		if (!first)
			first = f;
 ...
 */
static struct tris_frame *h264_encap(struct fbuf_t *b, int mtu,
	struct tris_frame **tail)
{
	struct tris_frame *f = NULL, *cur = NULL, *first = NULL;
...
#define HAVE_NAL(x) (x[-4] == 0 && x[-3] == 0 && x[-2] == 0 && x[-1] == 1)
   ...
   for (;start < end - 4; start = d) {
	...
	if (d >= end) {	/* NAL not found */
		...
		tris_log(LOG_WARNING, "skip invalid nal type %d at %d of %d\n",
	...
	if (size < mtu) {	// test - don't fragment
		// Single NAL Unit
		f = create_video_frame(start, d - 4, TRIS_FORMAT_H264, 0, cur);
		if (!f)
			break;
		if (!first)
			first = f;
	...
	while (size) {
		...
		f = create_video_frame(start, start+frag_size, TRIS_FORMAT_H264, 2, cur);
		if (!f)
			break;
...
static int h264_decap(struct fbuf_t *b, uint8_t *data, int len)
{
	...
	if (len < 2) {
		tris_log(LOG_WARNING, "--- invalid len %d\n", len);
		return 1;
	}
	/* first of all, check if the packet has F == 0 */
	if (data[0] & 0x80) {
		tris_log(LOG_WARNING, "--- forbidden packet; nal: %02x\n",
			data[0]);
		return 1;
	}
	...
	case 0:
	case 31:
		tris_log(LOG_WARNING, "--- invalid type: %d\n", type);
		return 1;
	case 24:
	case 25:
	case 26:
	case 27:
	case 29:
		tris_log(LOG_WARNING, "--- encapsulation not supported : %d\n", type);
		return 1;
	case 28:	/* FU-A Unit */
...
/*
 * Table of translation between trismedia and ffmpeg formats.
 ...
 */
struct _cm {    /* map ffmpeg codec types to trismedia formats */
	uint32_t	tris_format;	/* 0 is a terminator */
	enum CodecID	codec;
...
static struct _cm video_formats[] = {
        { TRIS_FORMAT_H263_PLUS, CODEC_ID_H263,  CM_RD }, /* incoming H263P ? */
        { TRIS_FORMAT_H263_PLUS, CODEC_ID_H263P, CM_WR },
        { TRIS_FORMAT_H263,      CODEC_ID_H263,  CM_RD },
        { TRIS_FORMAT_H263,      CODEC_ID_H263,  CM_WR },
        { TRIS_FORMAT_H261,      CODEC_ID_H261,  CM_RDWR },
        { TRIS_FORMAT_H264,      CODEC_ID_H264,  CM_RDWR },
        { TRIS_FORMAT_MP4_VIDEO, CODEC_ID_MPEG4, CM_RDWR },
        { 0,                    0, 0 },
};
                

/*! \brief map an trismedia format into an ffmpeg one */
static enum CodecID map_video_format(uint32_t tris_format, int rw)
{
	struct _cm *i;

	for (i = video_formats; i->tris_format != 0; i++)
		if (tris_format & i->tris_format && rw & i->rw && rw & i->rw)
			return i->codec;
...
/*
 * Map the TRIS_FORMAT to the library. If not recognised, fail.
...
static struct video_codec_desc *map_video_codec(int fmt)
{
	...
	for (i = 0; supported_codecs[i]; i++)
		if (fmt == supported_codecs[i]->format) {
			tris_log(LOG_WARNING, "using %s for format 0x%x\n",
 ...
 */
static struct video_dec_desc *dec_init(uint32_t the_tris_format)
{
	enum CodecID codec;
	struct video_dec_desc *v = tris_calloc(1, sizeof(*v));
	if (v == NULL)
		return NULL;

	v->discard = 1;

	v->d_callbacks = map_video_codec(the_tris_format);
	if (v->d_callbacks == NULL) {
		tris_log(LOG_WARNING, "cannot find video codec, drop input 0x%x\n", the_tris_format);
		return dec_uninit(v);
	}
	...
	if (!v->codec) {
		tris_log(LOG_WARNING, "Unable to find the decoder for format %d\n", codec);
		return dec_uninit(v);
	}
	/*
	...
	if (!v->dec_ctx) {
		tris_log(LOG_WARNING, "Cannot allocate the decoder context\n");
		return dec_uninit(v);
	}
	/* XXX call dec_init() ? */
	if (avcodec_open(v->dec_ctx, v->codec) < 0) {
		tris_log(LOG_WARNING, "Cannot open the decoder context\n");
		av_free(v->dec_ctx);
		v->dec_ctx = NULL;
		return dec_uninit(v);
	}
	...
	if (!v->parser) {
		tris_log(LOG_WARNING, "Cannot initialize the decoder parser\n");
		return dec_uninit(v);
	}

	v->d_frame = avcodec_alloc_frame();
	if (!v->d_frame) {
		tris_log(LOG_WARNING, "Cannot allocate decoding video frame\n");
		return dec_uninit(v);
	}
