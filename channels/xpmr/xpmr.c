...
void	pptp_init (void)
{
    ...
    if (ppdrvdev < 0)
    {
        tris_log(LOG_ERROR, "open /dev/ppdrv_ppdrvdev returned %i\n",ppdrvdev);
		exit(0);
	}
...
void	ppbinout	(u8 chan)
{
    ...
    // ioctl(ppdrvdev, PPDRV_IOC_PINSET, BIN_PROG_3|BIN_PROG_2|BIN_PROG_1|BIN_PROG_0 ); 
    tris_log(LOG_NOTICE, "mask=%i 0x%x\n",i,i); 
#endif
}
/*
...
void	ppspiout	(u32 spidata)
{
    ...
    if (ppdrvdev < 0)
    {
        tris_log(LOG_ERROR, "no parallel port permission ppdrvdev %i\n",ppdrvdev);
		exit(0);
	}
