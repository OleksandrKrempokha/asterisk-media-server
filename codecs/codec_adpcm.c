...
static int adpcmtolin_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
...
static int lintoadpcm_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
...
static struct tris_frame *lintoadpcm_frameout(struct tris_trans_pvt *pvt)
{
	struct adpcm_encoder_pvt *tmp = pvt->pvt;
	struct tris_frame *f;
	int i;
	...
	f = tris_trans_frameout(pvt, pvt->samples/2, 0);
...
static struct tris_translator adpcmtolin = {
	.name = "adpcmtolin",
	.srcfmt = TRIS_FORMAT_ADPCM,
...
static struct tris_translator lintoadpcm = {
	.name = "lintoadpcm",
	.srcfmt = TRIS_FORMAT_SLINEAR,
	.dstfmt = TRIS_FORMAT_ADPCM,
...
static int parse_config(int reload)
{
	...
	for (var = tris_variable_browse(cfg, "plc"); var ; var = var->next) {
		if (!strcasecmp(var->name, "genericplc")) {
			adpcmtolin.useplc = tris_true(var->value) ? 1 : 0;
			tris_verb(3, "codec_adpcm: %susing generic PLC\n", adpcmtolin.useplc ? "" : "not ");
		}
	}
...
static int unload_module(void)
{
	...
	res = tris_unregister_translator(&lintoadpcm);
	res |= tris_unregister_translator(&adpcmtolin);
...
static int load_module(void)
{
	...
	if (parse_config(0))
		return TRIS_MODULE_LOAD_DECLINE;
	res = tris_register_translator(&adpcmtolin);
	if (!res)
		res = tris_register_translator(&lintoadpcm);
	else
		tris_unregister_translator(&adpcmtolin);
	if (res)
		return TRIS_MODULE_LOAD_FAILURE;
	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Adaptive Differential PCM Coder/Decoder",
		.load = load_module,
