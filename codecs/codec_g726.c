...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 229285 $")
...
static int lintog726_new(struct tris_trans_pvt *pvt)
{
...
static int g726aal2tolin_framein (struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct g726_coder_pvt *tmp = pvt->pvt;
...
static int lintog726aal2_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct g726_coder_pvt *tmp = pvt->pvt;
...
static int g726tolin_framein (struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct g726_coder_pvt *tmp = pvt->pvt;
	unsigned char *src = f->data.ptr;
	int16_t *dst = pvt->outbuf.i16 + pvt->samples;
...
static int lintog726_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct g726_coder_pvt *tmp = pvt->pvt;
	int16_t *src = f->data.ptr;
	unsigned int i;
...
static struct tris_translator g726tolin = {
	.name = "g726tolin",
	.srcfmt = TRIS_FORMAT_G726,
...
static struct tris_translator lintog726 = {
	.name = "lintog726",
	.srcfmt = TRIS_FORMAT_SLINEAR,
	.dstfmt = TRIS_FORMAT_G726,
	.newpvt = lintog726_new,	/* same for both directions */
...
static struct tris_translator g726aal2tolin = {
	.name = "g726aal2tolin",
	.srcfmt = TRIS_FORMAT_G726_AAL2,
	.dstfmt = TRIS_FORMAT_SLINEAR,
	.newpvt = lintog726_new,	/* same for both directions */
...
static struct tris_translator lintog726aal2 = {
	.name = "lintog726aal2",
	.srcfmt = TRIS_FORMAT_SLINEAR,
	.dstfmt = TRIS_FORMAT_G726_AAL2,
	.newpvt = lintog726_new,	/* same for both directions */
...
static int parse_config(int reload)
{
	...
	for (var = tris_variable_browse(cfg, "plc"); var; var = var->next) {
		if (!strcasecmp(var->name, "genericplc")) {
			g726tolin.useplc = tris_true(var->value) ? 1 : 0;
			tris_verb(3, "codec_g726: %susing generic PLC\n",
...
static int unload_module(void)
{
	...
	res |= tris_unregister_translator(&g726tolin);
	res |= tris_unregister_translator(&lintog726);

	res |= tris_unregister_translator(&g726aal2tolin);
	res |= tris_unregister_translator(&lintog726aal2);
...
static int load_module(void)
{
	...
	res |= tris_register_translator(&g726tolin);
	res |= tris_register_translator(&lintog726);

	res |= tris_register_translator(&g726aal2tolin);
	res |= tris_register_translator(&lintog726aal2);
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "ITU G.726-32kbps G726 Transcoder",
		.load = load_module,
