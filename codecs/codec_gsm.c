...
static int gsm_new(struct tris_trans_pvt *pvt)
{
...
static int gsmtolin_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct gsm_translator_pvt *tmp = pvt->pvt;
	int x;
	...
	for (x=0; x < f->datalen; x += flen) {
		...
		if (gsm_decode(tmp->gsm, src, dst + pvt->samples)) {
			tris_log(LOG_WARNING, "Invalid GSM data (1)\n");
			return -1;
		}
		...
		if (flen == MSGSM_FRAME_LEN) {
			if (gsm_decode(tmp->gsm, data + GSM_FRAME_LEN, dst + pvt->samples)) {
				tris_log(LOG_WARNING, "Invalid GSM data (2)\n");
				return -1;
			}
			pvt->samples += GSM_SAMPLES;
			pvt->datalen += 2 * GSM_SAMPLES;
...
static int lintogsm_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct gsm_translator_pvt *tmp = pvt->pvt;
...
static struct tris_frame *lintogsm_frameout(struct tris_trans_pvt *pvt)
{
	struct gsm_translator_pvt *tmp = pvt->pvt;
	int datalen = 0;
	int samples = 0;
...
static void gsm_destroy_stuff(struct tris_trans_pvt *pvt)
{
	struct gsm_translator_pvt *tmp = pvt->pvt;
	if (tmp->gsm)
		gsm_destroy(tmp->gsm);
...
static struct tris_translator gsmtolin = {
	.name = "gsmtolin", 
	.srcfmt = TRIS_FORMAT_GSM,
...
static struct tris_translator lintogsm = {
	.name = "lintogsm", 
	.srcfmt = TRIS_FORMAT_SLINEAR,
	.dstfmt = TRIS_FORMAT_GSM,
	.newpvt = gsm_new,
...
static int parse_config(int reload)
{
	...
	for (var = tris_variable_browse(cfg, "plc"); var; var = var->next) {
	       if (!strcasecmp(var->name, "genericplc")) {
		       gsmtolin.useplc = tris_true(var->value) ? 1 : 0;
			   tris_verb(3, "codec_gsm: %susing generic PLC\n", gsmtolin.useplc ? "" : "not ");
	       }
	}
...
static int unload_module(void)
{
	...
	res = tris_unregister_translator(&lintogsm);
	if (!res)
		res = tris_unregister_translator(&gsmtolin);
...
static int load_module(void)
{
	...
	if (parse_config(0))
		return TRIS_MODULE_LOAD_DECLINE;
	res = tris_register_translator(&gsmtolin);
	if (!res) 
		res=tris_register_translator(&lintogsm);
	else
		tris_unregister_translator(&gsmtolin);
	if (res) 
		return TRIS_MODULE_LOAD_FAILURE;
	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "GSM Coder/Decoder",
		.load = load_module,
