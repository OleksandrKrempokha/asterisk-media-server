...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 228447 $")
...
static int lintoilbc_new(struct tris_trans_pvt *pvt)
{
...
static int ilbctolin_new(struct tris_trans_pvt *pvt)
{
	struct ilbc_coder_pvt *tmp = pvt->pvt;
...
static int ilbctolin_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct ilbc_coder_pvt *tmp = pvt->pvt;
	...
	if (!f->data.ptr && f->datalen) {
		tris_log(LOG_DEBUG, "issue 16070, ILIB ERROR. data = NULL datalen = %d src = %s\n", f->datalen, f->src ? f->src : "no src set");
		f->datalen = 0;
	}
	...
	if (f->datalen % ILBC_FRAME_LEN) {
		tris_log(LOG_WARNING, "Huh?  An ilbc frame that isn't a multiple of 50 bytes long from %s (%d)?\n", f->src, f->datalen);
		return -1;
	}
...
static int lintoilbc_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct ilbc_coder_pvt *tmp = pvt->pvt;
...
static struct tris_frame *lintoilbc_frameout(struct tris_trans_pvt *pvt)
{
	struct ilbc_coder_pvt *tmp = pvt->pvt;
	int datalen = 0;
	int samples = 0;
...
static struct tris_translator ilbctolin = {
	.name = "ilbctolin", 
	.srcfmt = TRIS_FORMAT_ILBC,
...
static struct tris_translator lintoilbc = {
	.name = "lintoilbc", 
	.srcfmt = TRIS_FORMAT_SLINEAR,
	.dstfmt = TRIS_FORMAT_ILBC,
...
static int unload_module(void)
{
	...
	res = tris_unregister_translator(&lintoilbc);
	res |= tris_unregister_translator(&ilbctolin);
...
static int load_module(void)
{
	...
	res = tris_register_translator(&ilbctolin);
	if (!res) 
		res=tris_register_translator(&lintoilbc);
	else
		tris_unregister_translator(&ilbctolin);
	if (res)
		return TRIS_MODULE_LOAD_FAILURE;
	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "iLBC Coder/Decoder");
