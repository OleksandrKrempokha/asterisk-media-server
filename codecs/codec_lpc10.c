...
static int lpc10_enc_new(struct tris_trans_pvt *pvt)
{
...
static int lpc10_dec_new(struct tris_trans_pvt *pvt)
{
	struct lpc10_coder_pvt *tmp = pvt->pvt;
...
static int lpc10tolin_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct lpc10_coder_pvt *tmp = pvt->pvt;
	...
	while (len + LPC10_BYTES_IN_COMPRESSED_FRAME <= f->datalen) {
		...
		if (lpc10_decode(bits, tmpbuf, tmp->lpc10.dec)) {
			tris_log(LOG_WARNING, "Invalid lpc10 data\n");
			return -1;
		}
...
static int lintolpc10_framein(struct tris_trans_pvt *pvt, struct tris_frame *f)
{
	struct lpc10_coder_pvt *tmp = pvt->pvt;
...
static struct tris_frame *lintolpc10_frameout(struct tris_trans_pvt *pvt)
{
	struct lpc10_coder_pvt *tmp = pvt->pvt;
	int x;
...
static void lpc10_destroy(struct tris_trans_pvt *arg)
{
	struct lpc10_coder_pvt *pvt = arg->pvt;
	/* Enc and DEC are both just allocated, so they can be freed */
	tris_free(pvt->lpc10.enc);
}

static struct tris_translator lpc10tolin = {
	.name = "lpc10tolin", 
	.srcfmt = TRIS_FORMAT_LPC10,
...
static struct tris_translator lintolpc10 = {
	.name = "lintolpc10", 
	.srcfmt = TRIS_FORMAT_SLINEAR,
	.dstfmt = TRIS_FORMAT_LPC10,
...
static int parse_config(int reload)
{
	...
	for (var = tris_variable_browse(cfg, "plc"); var; var = var->next) {
		if (!strcasecmp(var->name, "genericplc")) {
			lpc10tolin.useplc = tris_true(var->value) ? 1 : 0;
			tris_verb(3, "codec_lpc10: %susing generic PLC\n",
...
static int unload_module(void)
{
	...
	res = tris_unregister_translator(&lintolpc10);
	res |= tris_unregister_translator(&lpc10tolin);
...
static int load_module(void)
{
	...
	if (parse_config(0))
		return TRIS_MODULE_LOAD_DECLINE;
	res = tris_register_translator(&lpc10tolin);
	if (!res) 
		res = tris_register_translator(&lintolpc10);
	else
		tris_unregister_translator(&lpc10tolin);
	if (res)
		return TRIS_MODULE_LOAD_FAILURE;
	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "LPC10 2.4kbps Coder/Decoder",
		.load = load_module,
