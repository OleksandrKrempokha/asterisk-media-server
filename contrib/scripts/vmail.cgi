#!/usr/bin/perl
#
# Web based Voicemail for Trismedia
#
...
$footer = "<hr><font size=-1><a href=\"http://www.trismedia.org\">The Trismedia Open Source PBX</a> Copyright 2004-2008, <a href=\"http://www.digium.com\">Digium, Inc.</a></a>";
...
<TITLE>Trismedia Web-Voicemail</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
...
sub check_login($$)
{
	...
	if (!$filename) {
		$filename = "/etc/trismedia/voicemail.conf";
	}
#	print header;
#	print "Including <h2>$filename</h2> while in <h2>$category</h2>...\n";
	...
	while(<VMAIL>) {
		chomp;
		if (/include\s\"([^\"]+)\"$/) {
			($tmp, $category) = &check_login("/etc/trismedia/$1", $category);
			if (length($tmp)) {
#				print "Got '$tmp'\n";
				return ($tmp, $category);
			}
...
sub check_login_users {
	...
	my ($found, $fullname) = (0, "");
	open VMAIL, "</etc/trismedia/users.conf";
	while (<VMAIL>) {
		chomp;
		if (m/\[(.*)\]/) {
			if ($1 eq $mbox) {
...
sub validmailbox($$$$)
{
	...
	while (<VMAIL>) {
		chomp;
		if (/include\s\"([^\"]+)\"$/) {
			($tmp, $category) = &validmailbox($mbox, $context, "/etc/trismedia/$1");
			if ($tmp) {
				return ($tmp, $category);
			}
		} elsif (/\[(.*)\]/) {
...
#	print "Including <h2>$filename</h2> while in <h2>$category</h2>...\n";
	...
	while(<VMAIL>) {
		...
		s/\;.*$//;
		if (/include\s\"([^\"]+)\"$/) {
			($tmp2, $category) = &mailbox_options($context, $current, "/etc/trismedia/$1", $category);
#			print "Got '$tmp2'...\n";
			$tmp .= $tmp2;
		} elsif (/\[(.*)\]/) {
			$category = $1;
...
sub msgcount() 
{
	...
	my $path = "/var/spool/trismedia/voicemail/$context/$mailbox/$folder";
	if (opendir(DIR, $path)) {
...
sub getfields()
{
	...
	my $fields;
	if (open(MSG, "</var/spool/trismedia/voicemail/$context/$mailbox/$folder/msg${msg}.txt")) {
		while(<MSG>) {
			s/\#.*$//g;
			if (/^(\w+)\s*\=\s*(.*)$/) {
				$fields->{$1} = $2;
...
<TITLE>Trismedia Web-Voicemail: Preferences</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
...
<TITLE>Trismedia Web-Voicemail: $folder Message $msgid</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
<table width=100% align=center>
...
sub message_audio()
{
	...
	my $path = "/var/spool/trismedia/voicemail/$context/$mailbox/$folder/msg${msgid}.$format";
...
<TITLE>Trismedia Web-Voicemail: $mbox\@$context $folder</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
<table width=100% align=center>
...
sub message_rename()
{
	...
	my $path = "/var/spool/trismedia/voicemail/$context/$mbox/$newfolder";
	$path =~ /^(.*)$/;
	$path = $1;
	mkdir $path, 0770;
	$path = "/var/spool/trismedia/voicemail/$context/$mbox/$oldfolder";
	...
	foreach $oldfile (@files) {
		...
		if ($tmp =~ /^(msg${old}.\w+)$/) {
			...
			$newfile = "/var/spool/trismedia/voicemail/$context/$mbox/$newfolder/$tmp";
...
sub message_copy()
{
	...
	my $path = "/var/spool/trismedia/voicemail/$context/$newmbox";
	$path =~ /^(.*)$/;
	$path = $1;
	mkdir $path, 0770;
	$path = "/var/spool/trismedia/voicemail/$context/$newmbox/INBOX";
	$path =~ /^(.*)$/;
	$path = $1;
	mkdir $path, 0770;
	$path = "/var/spool/trismedia/voicemail/$context/$mbox/$oldfolder";
	...
	foreach $oldfile (@files) {
		...
		if ($tmp =~ /^(msg${old}.\w+)$/) {
			...
			$oldfile = $path . "/$tmp";
			$tmp =~ s/msg${old}/msg${new}/;
			$newfile = "/var/spool/trismedia/voicemail/$context/$newmbox/INBOX/$tmp";
...
sub message_delete()
{
	...
	my $path = "/var/spool/trismedia/voicemail/$context/$mbox/$folder";
	opendir(DIR, $path) || die("Unable to open directory\n");
...
sub message_forward()
{
	...
	my $path = "/var/spool/trismedia/voicemail/$context/$newmbox/INBOX";
	if ($msgs[0]) {
		if (&lock_path($path) == 0) {
...
sub message_delete_or_move()
{
	...
	$path = "/var/spool/trismedia/voicemail/$context/$mbox/$folder";
	if ($msgs[0]) {
		if (&lock_path($path) == 0) {
