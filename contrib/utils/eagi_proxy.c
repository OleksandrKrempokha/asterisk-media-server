/*
 * Trismedia EAGI -> TCP/IP proxy
 * 	by Danijel Korzinek (devil_slayer _at_ hotmail.com)
 *
 * This simple C application allows you to control trismedia thru one TCP/IP
 ...
 * 	any AGI command there (http://www.voip-info.org/wiki-Trismedia+AGI),
 * 	e.g.:
 ...
 * 1) Main thread - reads commands from the socket and sends them to trismedia
 * 2) stdin_thread - reads trismedia output and sends it to the command socket
 * 3) signal_thread - reads the sound from trismedia and sends it to the signal socket
 */
