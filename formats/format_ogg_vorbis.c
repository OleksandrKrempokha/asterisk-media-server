 ...
 */
static int ogg_vorbis_open(struct tris_filestream *s)
{
	int i;
	int bytes;
	int result;
	...
	if (result != 1) {
		if(bytes < BLOCK_SIZE) {
			tris_log(LOG_ERROR, "Run out of data...\n");
		} else {
			tris_log(LOG_ERROR, "Input does not appear to be an Ogg bitstream.\n");
		}
	...
	if (ogg_stream_pagein(&tmp->os, &tmp->og) < 0) { 
		tris_log(LOG_ERROR, "Error reading first page of Ogg bitstream data.\n");
error:
	...
	if (ogg_stream_packetout(&tmp->os, &tmp->op) != 1) { 
		tris_log(LOG_ERROR, "Error reading initial header packet.\n");
		goto error;
	}
	
	if (vorbis_synthesis_headerin(&tmp->vi, &tmp->vc, &tmp->op) < 0) { 
		tris_log(LOG_ERROR, "This Ogg bitstream does not contain Vorbis audio data.\n");
		goto error;
	}
	...
	for (i = 0; i < 2 ; ) {
			...
			if (result == 1) {
				...
				while(i < 2) {
					...
					if(result == 0)
						break;
					if(result < 0) {
						tris_log(LOG_ERROR, "Corrupt secondary header.  Exiting.\n");
						goto error;
					}
		...
		bytes = fread(buffer, 1, BLOCK_SIZE, s->f);
		if (bytes == 0 && i < 2) {
			tris_log(LOG_ERROR, "End of file before finding all Vorbis headers!\n");
			goto error;
		}
		ogg_sync_wrote(&tmp->oy, bytes);
	}
	...
	for (ptr = tmp->vc.user_comments; *ptr; ptr++)
		tris_debug(1, "OGG/Vorbis comment: %s\n", *ptr);
		tris_debug(1, "OGG/Vorbis bitstream is %d channel, %ldHz\n", tmp->vi.channels, tmp->vi.rate);
		tris_debug(1, "OGG/Vorbis file encoded by: %s\n", tmp->vc.vendor);

	if (tmp->vi.channels != 1) {
		tris_log(LOG_ERROR, "Only monophonic OGG/Vorbis files are currently supported!\n");
		goto error;
	}
	
	if (tmp->vi.rate != DEFAULT_SAMPLE_RATE) {
		tris_log(LOG_ERROR, "Only 8000Hz OGG/Vorbis files are currently supported!\n");
 ...
 */
static int ogg_vorbis_rewrite(struct tris_filestream *s,
						 const char *comment)
{
	ogg_packet header;
	...
	if (vorbis_encode_init_vbr(&tmp->vi, 1, DEFAULT_SAMPLE_RATE, 0.4)) {
		tris_log(LOG_ERROR, "Unable to initialize Vorbis encoder!\n");
		return -1;
	}

	vorbis_comment_init(&tmp->vc);
	vorbis_comment_add_tag(&tmp->vc, "ENCODER", "Trismedia");
	if (comment)
	...
	ogg_stream_init(&tmp->os, tris_random());
 ...
 */
static int ogg_vorbis_write(struct tris_filestream *fs, struct tris_frame *f)
{
	int i;
	float **buffer;
	short *data;
	...
	if (!s->writing) {
		tris_log(LOG_ERROR, "This stream is not set up for writing!\n");
		return -1;
	}
 ...
 */
static void ogg_vorbis_close(struct tris_filestream *fs)
{
	struct vorbis_desc *s = (struct vorbis_desc *)fs->_private;
...
static int read_samples(struct tris_filestream *fs, float ***pcm)
{
	int samples_in;
	int result;
	char *buffer;
 ...
 */
static struct tris_frame *ogg_vorbis_read(struct tris_filestream *fs,
					 int *whennext)
{
	int clipflag = 0;
	int i;
	...
	fs->fr.frametype = TRIS_FRAME_VOICE;
	fs->fr.subclass = TRIS_FORMAT_SLINEAR;
	fs->fr.mallocd = 0;
	TRIS_FRAME_SET_BUFFER(&fs->fr, fs->buf, TRIS_FRIENDLY_OFFSET, BUF_SIZE);
	...
	while (samples_out != SAMPLES_MAX) {
		...
		if (clipflag)
			tris_log(LOG_WARNING, "Clipping in frame %ld\n", (long) (s->vd.sequence));
...
static int ogg_vorbis_trunc(struct tris_filestream *s)
{
	tris_log(LOG_WARNING, "Truncation is not supported on OGG/Vorbis streams!\n");
	return -1;
}
 ...
 * \return 0 on success, -1 on failure.
 */
static int ogg_vorbis_seek(struct tris_filestream *s, off_t sample_offset, int whence)
{
	tris_log(LOG_WARNING, "Seeking is not supported on OGG/Vorbis streams!\n");
	return -1;
}

static off_t ogg_vorbis_tell(struct tris_filestream *s)
{
	tris_log(LOG_WARNING, "Telling is not supported on OGG/Vorbis streams!\n");
	return -1;
}

static const struct tris_format vorbis_f = {
	.name = "ogg_vorbis",
	.exts = "ogg",
...
{
	if (tris_format_register(&vorbis_f))
...
{
	return tris_format_unregister(vorbis_f.name);
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_LOAD_ORDER, "OGG/Vorbis audio",
	.load = load_module,
