...
static unsigned long get_time(void)
{
	...
	cur = times( &buf );
	if( cur < 0 ) {
		tris_log( LOG_WARNING, "Cannot get current time\n" );
		return 0;
	}
	return cur * 1000 / sysconf( _SC_CLK_TCK );
}

static int pcma_open(struct tris_filestream *s)
{
	if (s->fmt->format == TRIS_FORMAT_ALAW)
		pd->starttime = get_time();
	return 0;
}

static int pcma_rewrite(struct tris_filestream *s, const char *comment)
{
	return pcma_open(s);
}
#endif

static struct tris_frame *pcm_read(struct tris_filestream *s, int *whennext)
{
	int res;
	...
	s->fr.datalen = res;
	if (s->fmt->format == TRIS_FORMAT_G722)
...
static int pcm_seek(struct tris_filestream *fs, off_t sample_offset, int whence)
{
	...
	case SEEK_FORCECUR:
		...
		break;
	default:
		tris_log(LOG_WARNING, "invalid whence %d, assuming SEEK_SET\n", whence);
		offset = sample_offset;
	}
	if (offset < 0) {
		tris_log(LOG_WARNING, "negative offset %ld, resetting to 0\n", (long) offset);
		offset = 0;
	}
	if (whence == SEEK_FORCECUR && offset > max) { /* extend the file */
		size_t left = offset - max;
		const char *src = (fs->fmt->format == TRIS_FORMAT_ALAW) ? alaw_silence : ulaw_silence;
		...
		if (offset > max) {
			tris_log(LOG_WARNING, "offset too large %ld, truncating to %ld\n", (long) offset, (long) max);
			offset = max;
		}
...
static int pcm_trunc(struct tris_filestream *fs)
{
	return ftruncate(fileno(fs->f), ftello(fs->f));
}

static off_t pcm_tell(struct tris_filestream *fs)
{
	return ftello(fs->f);
}

static int pcm_write(struct tris_filestream *fs, struct tris_frame *f)
{
	int res;
	...
	if (f->subclass != fs->fmt->format) {
		tris_log(LOG_WARNING, "Asked to write incompatible format frame (%d)!\n", f->subclass);
		return -1;
	}
		...
		*/
		...
		if (stat_buf.st_size < fpos) {
			...
			if (fseek(fs->f, cur, SEEK_SET) < 0) {
				tris_log( LOG_WARNING, "Cannot seek in file: %s\n", strerror(errno) );
				return -1;
			}
			memset(buf, 0x55, 512);
			while (cur < fpos) {
...
static int check_header(FILE *f)
{
	...
	if (fread(header, 1, AU_HEADER_SIZE, f) != AU_HEADER_SIZE) {
		tris_log(LOG_WARNING, "Read failed (header)\n");
		return -1;
	}
	magic = ltohl(header[AU_HDR_MAGIC_OFF]);
	if (magic != (uint32_t) AU_MAGIC) {
		tris_log(LOG_WARNING, "Bad magic: 0x%x\n", magic);
	}
...
/*	data_size = ltohl(header[AU_HDR_DATA_SIZE_OFF]); */
	...
	if (encoding != AU_ENC_8BIT_ULAW) {
		tris_log(LOG_WARNING, "Unexpected format: %d. Only 8bit ULAW allowed (%d)\n", encoding, AU_ENC_8BIT_ULAW);
		return -1;
	}
	sample_rate = ltohl(header[AU_HDR_SAMPLE_RATE_OFF]);
	if (sample_rate != DEFAULT_SAMPLE_RATE) {
		tris_log(LOG_WARNING, "Sample rate can only be 8000 not %d\n", sample_rate);
		return -1;
	}
	channels = ltohl(header[AU_HDR_CHANNELS_OFF]);
	if (channels != 1) {
		tris_log(LOG_WARNING, "Not in mono: channels=%d\n", channels);
		return -1;
	}
	/* Skip to data */
	fseek(f, 0, SEEK_END);
	...
	if (fseek(f, hdr_size, SEEK_SET) == -1 ) {
		tris_log(LOG_WARNING, "Failed to skip to data: %d\n", hdr_size);
		return -1;
	}
	return data_size;
}
...
static int update_header(FILE *f)
{
	...
	if (cur < 0) {
		tris_log(LOG_WARNING, "Unable to find our position\n");
		return -1;
	}
	if (fseek(f, AU_HDR_DATA_SIZE_OFF * sizeof(uint32_t), SEEK_SET)) {
		tris_log(LOG_WARNING, "Unable to set our position\n");
		return -1;
	}
	if (fwrite(&datalen, 1, sizeof(datalen), f) != sizeof(datalen)) {
		tris_log(LOG_WARNING, "Unable to set write file size\n");
		return -1;
	}
	if (fseek(f, cur, SEEK_SET)) {
		tris_log(LOG_WARNING, "Unable to return to position\n");
		return -1;
	}
	return 0;
}
...
static int write_header(FILE *f)
{
	...
	if (fwrite(header, 1, AU_HEADER_SIZE, f) != AU_HEADER_SIZE) {
		tris_log(LOG_WARNING, "Unable to write header\n");
		return -1;
	}
	return 0;
}

static int au_open(struct tris_filestream *s)
{
...
static int au_rewrite(struct tris_filestream *s, const char *comment)
{
	if (write_header(s->f))
		return -1;
	return 0;
...
static int au_seek(struct tris_filestream *fs, off_t sample_offset, int whence)
{
	off_t min, max, cur;
	long offset = 0, bytes;

	if (fs->fmt->format == TRIS_FORMAT_G722)
...
static int au_trunc(struct tris_filestream *fs)
{
...
static off_t au_tell(struct tris_filestream *fs)
{
	off_t offset = ftello(fs->f);
	return offset - AU_HEADER_SIZE;
}

static const struct tris_format alaw_f = {
	.name = "alaw",
	.exts = "alaw|al|alw",
	.format = TRIS_FORMAT_ALAW,
	.write = pcm_write,
	.seek = pcm_seek,
	.trunc = pcm_trunc,
	.tell = pcm_tell,
...
static const struct tris_format pcm_f = {
	.name = "pcm",
	.exts = "pcm|ulaw|ul|mu|ulw",
	.format = TRIS_FORMAT_ULAW,
	.write = pcm_write,
	.seek = pcm_seek,
	.trunc = pcm_trunc,
	.tell = pcm_tell,
...
static const struct tris_format g722_f = {
	.name = "g722",
	.exts = "g722",
	.format = TRIS_FORMAT_G722,
	.write = pcm_write,
	.seek = pcm_seek,
	.trunc = pcm_trunc,
	.tell = pcm_tell,
	.read = pcm_read,
	.buf_size = (BUF_SIZE * 2) + TRIS_FRIENDLY_OFFSET,
};

static const struct tris_format au_f = {
	.name = "au",
	.exts = "au",
	.format = TRIS_FORMAT_ULAW,
	.open = au_open,
	...
	.tell = au_tell,
	.read = pcm_read,
	.buf_size = BUF_SIZE + TRIS_FRIENDLY_OFFSET,	/* this many shorts */
};
...
static int load_module(void)
{
	...
	for (i = 0; i < ARRAY_LEN(ulaw_silence); i++)
		ulaw_silence[i] = TRIS_LIN2MU(0);
	for (i = 0; i < ARRAY_LEN(alaw_silence); i++)
		alaw_silence[i] = TRIS_LIN2A(0);

	if ( tris_format_register(&pcm_f)
		|| tris_format_register(&alaw_f)
		|| tris_format_register(&au_f)
		|| tris_format_register(&g722_f) )
...
{
	return tris_format_unregister(pcm_f.name)
		|| tris_format_unregister(alaw_f.name)
		|| tris_format_unregister(au_f.name)
		|| tris_format_unregister(g722_f.name);
}	

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_LOAD_ORDER, "Raw/Sun uLaw/ALaw 8KHz (PCM,PCMA,AU), G.722 16Khz",
	.load = load_module,
