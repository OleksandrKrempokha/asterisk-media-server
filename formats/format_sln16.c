...
static struct tris_frame *slinear_read(struct tris_filestream *s, int *whennext)
{
	...
	s->fr.subclass = TRIS_FORMAT_SLINEAR16;
	s->fr.mallocd = 0;
...
static int slinear_write(struct tris_filestream *fs, struct tris_frame *f)
{
	...
	if (f->subclass != TRIS_FORMAT_SLINEAR16) {
		tris_log(LOG_WARNING, "Asked to write non-slinear16 frame (%d)!\n", f->subclass);
		return -1;
	}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_LOAD_ORDER, "Raw Signed Linear 16KHz Audio support (SLN16)",
	.load = load_module,
