...
static int check_header(FILE *f)
{
	...
	if (fread(&type, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (type)\n");
		return -1;
	}
	if (fread(&size, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (size)\n");
		return -1;
	}
	size = ltohl(size);
	if (fread(&formtype, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (formtype)\n");
		return -1;
	}
	if (memcmp(&type, "RIFF", 4)) {
		tris_log(LOG_WARNING, "Does not begin with RIFF\n");
		return -1;
	}
	if (memcmp(&formtype, "WAVE", 4)) {
		tris_log(LOG_WARNING, "Does not contain WAVE\n");
		return -1;
	}
	if (fread(&fmt, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (fmt)\n");
		return -1;
	}
	if (memcmp(&fmt, "fmt ", 4)) {
		tris_log(LOG_WARNING, "Does not say fmt\n");
		return -1;
	}
	...
	if (ltohl(hsize) < 16) {
		tris_log(LOG_WARNING, "Unexpected header size %d\n", ltohl(hsize));
		return -1;
	}
	if (fread(&format, 1, 2, f) != 2) {
		tris_log(LOG_WARNING, "Read failed (format)\n");
		return -1;
	}
	if (ltohs(format) != 1) {
		tris_log(LOG_WARNING, "Not a wav file %d\n", ltohs(format));
		return -1;
	}
	...
	if (ltohs(chans) != 1) {
		tris_log(LOG_WARNING, "Not in mono %d\n", ltohs(chans));
		return -1;
	}
	if (fread(&freq, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (freq)\n");
		return -1;
	}
	if (ltohl(freq) != DEFAULT_SAMPLE_RATE) {
		tris_log(LOG_WARNING, "Unexpected frequency %d\n", ltohl(freq));
		return -1;
	}
	/* Ignore the byte frequency */
	if (fread(&bysec, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (BYTES_PER_SECOND)\n");
		return -1;
	}
	/* Check bytes per sample */
	if (fread(&bysam, 1, 2, f) != 2) {
		tris_log(LOG_WARNING, "Read failed (BYTES_PER_SAMPLE)\n");
		return -1;
	}
	if (ltohs(bysam) != 2) {
		tris_log(LOG_WARNING, "Can only handle 16bits per sample: %d\n", ltohs(bysam));
		return -1;
	}
	if (fread(&bisam, 1, 2, f) != 2) {
		tris_log(LOG_WARNING, "Read failed (Bits Per Sample): %d\n", ltohs(bisam));
		return -1;
	}
	/* Skip any additional header */
	if (fseek(f,ltohl(hsize)-16,SEEK_CUR) == -1 ) {
		tris_log(LOG_WARNING, "Failed to skip remaining header bytes: %d\n", ltohl(hsize)-16 );
		return -1;
	}
	...
	{ 
	    ...
	    if (fread(&buf, 1, 4, f) != 4) {
			tris_log(LOG_WARNING, "Read failed (data)\n");
			return -1;
	    }
	    ...
	    if(memcmp(buf, "fact", 4) != 0 ) {
			tris_log(LOG_WARNING, "Unknown block - not fact or data\n");
			return -1;
	    }
	    if (fseek(f,data,SEEK_CUR) == -1 ) {
			tris_log(LOG_WARNING, "Failed to skip fact block: %d\n", data );
			return -1;
	    }
	}
#if 0
...
static int update_header(FILE *f)
{
	...
	if (fwrite(&datalen, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Unable to set write datalen\n");
		return -1;
	}
...
static int wav_open(struct tris_filestream *s)
{
...
static int wav_rewrite(struct tris_filestream *s, const char *comment)
{
	/* We don't have any header to read or anything really, but
	   if we did, it would go here.  We also might want to check
	   and be sure it's a valid file.  */
...
static void wav_close(struct tris_filestream *s)
{
	char zero = 0;
...
static struct tris_frame *wav_read(struct tris_filestream *s, int *whennext)
{
	int res;
	...
	if (bytes < 0)
		bytes = 0;
/* 	tris_debug(1, "here: %d, maxlen: %d, bytes: %d\n", here, s->maxlen, bytes); */
	s->fr.frametype = TRIS_FRAME_VOICE;
	s->fr.subclass = TRIS_FORMAT_SLINEAR;
	s->fr.mallocd = 0;
	TRIS_FRAME_SET_BUFFER(&s->fr, s->buf, TRIS_FRIENDLY_OFFSET, bytes);
...
static int wav_write(struct tris_filestream *fs, struct tris_frame *f)
{
#if __BYTE_ORDER == __BIG_ENDIAN
	int x;
	short tmp[8000], *tmpi;
	...
	if (f->datalen > sizeof(tmp)) {
		tris_log(LOG_WARNING, "Data length is too long\n");
		return -1;
	}
	tmpi = f->data.ptr;
...
#endif
		tris_log(LOG_WARNING, "Bad write (%d): %s\n", res, strerror(errno));
		return -1;
	}
...
static int wav_seek(struct tris_filestream *fs, off_t sample_offset, int whence)
{
...
static int wav_trunc(struct tris_filestream *fs)
{
...
static off_t wav_tell(struct tris_filestream *fs)
{
	off_t offset;
	offset = ftello(fs->f);
...
static const struct tris_format wav_f = {
	.name = "wav",
	.exts = "wav",
	...
	.read = wav_read,
	.close = wav_close,
	.buf_size = WAV_BUF_SIZE + TRIS_FRIENDLY_OFFSET,
	.desc_size = sizeof(struct wav_desc),
};

static int load_module(void)
{
	if (tris_format_register(&wav_f))
...
{
	return tris_format_unregister(wav_f.name);
}	

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_LOAD_ORDER, "Microsoft WAV format (8000Hz Signed Linear)",
	.load = load_module,
