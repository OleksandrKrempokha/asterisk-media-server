...
static int check_header(FILE *f)
{
	...
	if (ltohs(format) != 49) {
		tris_log(LOG_WARNING, "Not a GSM file %d\n", ltohs(format));
		return -1;
	}
	...
	if (fread(&freq, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (X_1)\n");
		return -1;
	}
	/* Ignore the two weird fields */
	if (fread(&freq, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (X_2/X_3)\n");
		return -1;
	}
	/* Ignore the byte frequency */
	if (fread(&freq, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (Y_1)\n");
		return -1;
	}
	/* Check for the word fact */
	if (fread(&fact, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (fact)\n");
		return -1;
	}
	if (memcmp(&fact, "fact", 4)) {
		tris_log(LOG_WARNING, "Does not say fact\n");
		return -1;
	}
	/* Ignore the "fact value" */
	if (fread(&fact, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (fact header)\n");
		return -1;
	}
	if (fread(&fact, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Read failed (fact value)\n");
		return -1;
	}
	...
	if (memcmp(&data, "data", 4)) {
		tris_log(LOG_WARNING, "Does not say data\n");
		return -1;
	}
...
static int update_header(FILE *f)
{
	...
	if (fwrite(&filelen, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Unable to write file size\n");
		return -1;
	}
	...
	if (fwrite(&samples, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Unable to write samples\n");
		return -1;
	}
	...
	if (fwrite(&datalen, 1, 4, f) != 4) {
		tris_log(LOG_WARNING, "Unable to write datalen\n");
		return -1;
	}
...
static struct tris_frame *wav_read(struct tris_filestream *s, int *whennext)
{
	...
	s->fr.offset = TRIS_FRIENDLY_OFFSET;
...
static int wav_write(struct tris_filestream *s, struct tris_frame *f)
{
	int len;
	int size;
	struct wavg_desc *fs = (struct wavg_desc *)s->_private;
	...
	for (len = 0; len < f->datalen ; len += size) {
		...
		if (src && (res = fwrite(src, 1, MSGSM_FRAME_SIZE, s->f)) != MSGSM_FRAME_SIZE) {
			tris_log(LOG_WARNING, "Bad write (%d/65): %s\n", res, strerror(errno));
			return -1;
		}
...
static const struct tris_format wav49_f = {
	.name = "wav49",
	.exts = "WAV|wav49",
	...
	.tell = wav_tell,
	.read = wav_read,
	.buf_size = 2*GSM_FRAME_SIZE + TRIS_FRIENDLY_OFFSET,
	.desc_size = sizeof(struct wavg_desc),
};

static int load_module(void)
{
	if (tris_format_register(&wav49_f))
...
{
	return tris_format_unregister(wav49_f.name);
}	

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_LOAD_ORDER, "Microsoft WAV format (Proprietary GSM)",
	.load = load_module,
