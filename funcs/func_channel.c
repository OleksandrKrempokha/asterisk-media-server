...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 243485 $")
...
#define locked_copy_string(chan, dest, source, len) \
	do { \
		...
		tris_copy_string(dest, source, len); \
		tris_channel_unlock(chan); \
	} while (0)
#define locked_string_field_set(chan, field, source) \
	do { \
		tris_channel_lock(chan); \
		tris_string_field_set(chan, field, source); \
		tris_channel_unlock(chan); \
	} while (0)
...
static int func_channel_read(struct tris_channel *chan, const char *function,
			     char *data, char *buf, size_t len)
{
	...
	if (!strcasecmp(data, "audionativeformat"))
		...
		/* tris_getformatname_multiple(buf, len, chan->nativeformats & TRIS_FORMAT_AUDIO_MASK); */
		tris_copy_string(buf, tris_getformatname(chan->nativeformats & TRIS_FORMAT_AUDIO_MASK), len);
	else if (!strcasecmp(data, "videonativeformat"))
		/* use the _multiple version when chan->nativeformats holds multiple formats */
		/* tris_getformatname_multiple(buf, len, chan->nativeformats & TRIS_FORMAT_VIDEO_MASK); */
		tris_copy_string(buf, tris_getformatname(chan->nativeformats & TRIS_FORMAT_VIDEO_MASK), len);
	else if (!strcasecmp(data, "audioreadformat"))
		tris_copy_string(buf, tris_getformatname(chan->readformat), len);
	else if (!strcasecmp(data, "audiowriteformat"))
		tris_copy_string(buf, tris_getformatname(chan->writeformat), len);
#ifdef CHANNEL_TRACE
	else if (!strcasecmp(data, "trace")) {
		tris_channel_lock(chan);
		tris_copy_string(buf, tris_channel_trace_is_enabled(chan) ? "1" : "0", len);
		tris_channel_unlock(chan);
	}
#endif
		...
		locked_copy_string(chan, buf, tris_state2str(chan->_state), len);
		...
		char groupbuf[256];
		locked_copy_string(chan, buf,  tris_print_group(groupbuf, sizeof(groupbuf), chan->callgroup), len);
	} else if (!chan->tech->func_channel_read
		 || chan->tech->func_channel_read(chan, function, data, buf, len)) {
		tris_log(LOG_WARNING, "Unknown or unavailable item requested: '%s'\n", data);
		ret = -1;
	}
...
static int func_channel_write(struct tris_channel *chan, const char *function,
			      char *data, const char *value)
{
	...
	if (!strcasecmp(data, "language"))
		...
		tris_channel_lock(chan);
		if (tris_true(value)) 
			ret = tris_channel_trace_enable(chan);
		else if (tris_false(value)) 
			ret = tris_channel_trace_disable(chan);
		else {
			ret = -1;
			tris_log(LOG_WARNING, "Invalid value for CHANNEL(trace).");
		}
		tris_channel_unlock(chan);
	}
#endif
	else if (!strcasecmp(data, "tonezone")) {
		struct tris_tone_zone *new_zone;
		if (!(new_zone = tris_get_indication_zone(value))) {
			tris_log(LOG_ERROR, "Unknown country code '%s' for tonezone. Check indications.conf for available country codes.\n", value);
			ret = -1;	
		} else {
			tris_channel_lock(chan);
			if (chan->zone) {
			...
			chan->zone = tris_tone_zone_ref(new_zone);
			tris_channel_unlock(chan);
			new_zone = tris_tone_zone_unref(new_zone);
		}
	} else if (!strcasecmp(data, "callgroup"))
		chan->callgroup = tris_get_group(value);
	else if (!strcasecmp(data, "txgain")) {
		sscanf(value, "%4hhd", &gainset);
		tris_channel_setoption(chan, TRIS_OPTION_TXGAIN, &gainset, sizeof(gainset), 0);
	} else if (!strcasecmp(data, "rxgain")) {
		sscanf(value, "%4hhd", &gainset);
		tris_channel_setoption(chan, TRIS_OPTION_RXGAIN, &gainset, sizeof(gainset), 0);
	} else if (!strcasecmp(data, "transfercapability")) {
		unsigned short i;
		for (i = 0; i < 0x20; i++) {
...
static struct tris_custom_function channel_function = {
	.name = "CHANNEL",
	.read = func_channel_read,
	.write = func_channel_write,
};

static int func_channels_read(struct tris_channel *chan, const char *function, char *data, char *buf, size_t maxlen)
{
	...
	if (!tris_strlen_zero(data)) {
		if ((res = regcomp(&re, data, REG_EXTENDED | REG_ICASE | REG_NOSUB))) {
			...
			tris_log(LOG_WARNING, "Error compiling regular expression for %s(%s): %s\n", function, data, buf);
			return -1;
		}
	}

	for (c = tris_channel_walk_locked(NULL); c; tris_channel_unlock(c), c = tris_channel_walk_locked(c)) {
		if (tris_strlen_zero(data) || regexec(&re, c->name, 0, NULL, 0) == 0) {
			size_t namelen = strlen(c->name);
			if (buflen + namelen + (tris_strlen_zero(buf) ? 0 : 1) + 1 < maxlen) {
				...
				tris_log(LOG_WARNING, "Number of channels exceeds the available buffer space.  Output will be truncated!\n");
			}
		}
	}
...
static struct tris_custom_function channels_function = {
	.name = "CHANNELS",
...
static int unload_module(void)
{
	...
	res |= tris_custom_function_unregister(&channel_function);
	res |= tris_custom_function_unregister(&channels_function);
...
static int load_module(void)
{
	...
	res |= tris_custom_function_register(&channel_function);
	res |= tris_custom_function_register(&channels_function);
	
	return res;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Channel information dialplan functions");
