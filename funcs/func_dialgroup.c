...
struct group_entry {
	char name[TRIS_CHANNEL_NAME];
};

struct group {
	char name[TRIS_MAX_EXTENSION];
...
static int dialgroup_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!grhead) {
		if (!tris_strlen_zero(cmd)) {
			tris_log(LOG_WARNING, "No such dialgroup '%s'\n", data);
		}
		return -1;
	}
	...
	while ((entry = ao2_iterator_next(&i))) {
		...
		if (len - bufused > tmp + 2) {
			if (bufused != 0)
				buf[bufused++] = '&';
			tris_copy_string(buf + bufused, entry->name, len - bufused);
			bufused += tmp;
		} else if (trunc_warning++ == 0) {
			if (!tris_strlen_zero(cmd)) {
				tris_log(LOG_WARNING, "Dialgroup '%s' is too large.  Truncating list.\n", data);
			} else {
				res = 1;
				ao2_ref(entry, -1);
				break;
...
static int dialgroup_refreshdb(struct tris_channel *chan, const char *cdialgroup)
{
	int len = 500, res = 0;
	char *buf = NULL;
	char *dialgroup = tris_strdupa(cdialgroup);

	do {
		len *= 2;
		buf = tris_realloc(buf, len);
	...
	if (tris_strlen_zero(buf)) {
		tris_db_del("dialgroup", cdialgroup);
	} else {
		tris_db_put("dialgroup", cdialgroup, buf);
	}
	tris_free(buf);
	return 0;
}

static int dialgroup_write(struct tris_channel *chan, const char *cmd, char *data, const char *cvalue)
{
	struct group *grhead;
	struct group_entry *entry;
	int j, needrefresh = 1;
	...
	TRIS_DECLARE_APP_ARGS(inter,
		TRIS_APP_ARG(faces)[100];
	);
	char *value = tris_strdupa(cvalue);

	TRIS_STANDARD_APP_ARGS(args, data);
	TRIS_NONSTANDARD_APP_ARGS(inter, value, '&');
	...
	if (!(grhead = ao2_find(group_container, args.group, 0))) {
		...
		tris_copy_string(grhead->name, args.group, sizeof(grhead->name));
		ao2_link(group_container, grhead);
	}

	if (tris_strlen_zero(args.op)) {
	...
	if (strcasecmp(args.op, "add") == 0) {
		for (j = 0; j < inter.argc; j++) {
			if ((entry = ao2_alloc(sizeof(*entry), NULL))) {
				tris_copy_string(entry->name, inter.faces[j], sizeof(entry->name));
				ao2_link(grhead->entries, entry);
				ao2_ref(entry, -1);
			} else {
				tris_log(LOG_WARNING, "Unable to add '%s' to dialgroup '%s'\n", inter.faces[j], grhead->name);
			}
		}
		...
		for (j = 0; j < inter.argc; j++) {
			if ((entry = ao2_find(grhead->entries, inter.faces[j], OBJ_UNLINK))) {
				...
				tris_log(LOG_WARNING, "Interface '%s' not found in dialgroup '%s'\n", inter.faces[j], grhead->name);
			}
		}
	} else {
		tris_log(LOG_ERROR, "Unrecognized operation: %s\n", args.op);
		needrefresh = 0;
	}
	ao2_ref(grhead, -1);
...
static struct tris_custom_function dialgroup_function = {
	.name = "DIALGROUP",
	.read = dialgroup_read,
...
{
	int res = tris_custom_function_unregister(&dialgroup_function);
...
{
	struct tris_db_entry *dbtree, *tmp;
	char groupname[TRIS_MAX_EXTENSION], *ptr;

	if ((group_container = ao2_container_alloc(37, group_hash_fn, group_cmp_fn))) {
		/* Refresh groups from astdb */
		if ((dbtree = tris_db_gettree("dialgroup", NULL))) {
			for (tmp = dbtree; tmp; tmp = tmp->next) {
				tris_copy_string(groupname, tmp->key, sizeof(groupname));
		...
		return tris_custom_function_register(&dialgroup_function);
	} else {
		return TRIS_MODULE_LOAD_DECLINE;
	}
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Dialgroup dialplan function");
