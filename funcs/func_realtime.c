...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 206811 $")
...
TRIS_THREADSTORAGE(buf1);
TRIS_THREADSTORAGE(buf2);
TRIS_THREADSTORAGE(buf3);

static int function_realtime_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len) 
{
	struct tris_variable *var, *head;
	struct tris_str *out;
	size_t resultslen;
	int n;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(family);
		TRIS_APP_ARG(fieldmatch);
		TRIS_APP_ARG(value);
		TRIS_APP_ARG(delim1);
		TRIS_APP_ARG(delim2);
	);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Syntax: REALTIME(family,fieldmatch[,value[,delim1[,delim2]]]) - missing argument!\n");
		return -1;
	}
	...
	head = tris_load_realtime_all(args.family, args.fieldmatch, args.value, SENTINEL);
	...
	out = tris_str_alloca(resultslen);
	for (var = head; var; var = var->next)
		tris_str_append(&out, 0, "%s%s%s%s", var->name, args.delim2, var->value, args.delim1);
	tris_copy_string(buf, tris_str_buffer(out), len);

	tris_variables_destroy(head);
...
static int function_realtime_write(struct tris_channel *chan, const char *cmd, char *data, const char *value)
{
	int res = 0;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(family);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Syntax: %s(family,fieldmatch,value,newcol) - missing argument!\n", cmd);
		return -1;
	}
	...
	res = tris_update_realtime(args.family, args.fieldmatch, args.value, args.field, (char *)value, SENTINEL);

	if (res < 0) {
		tris_log(LOG_WARNING, "Failed to update. Check the debug log for possible data repository related entries.\n");
	}
...
static int realtimefield_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len) 
{
	struct tris_variable *var, *head;
	struct tris_str *escapebuf = tris_str_thread_get(&buf1, 16);
	struct tris_str *fields = tris_str_thread_get(&buf2, 16);
	struct tris_str *values = tris_str_thread_get(&buf3, 16);
	int first = 0;
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Syntax: %s(family,fieldmatch,value%s) - missing argument!\n", cmd, which == rtfield ? ",fieldname" : "");
		return -1;
	}
	...
	if (!(head = tris_load_realtime_all(args.family, args.fieldmatch, args.value, SENTINEL))) {
		if (chan) {
			tris_autoservice_stop(chan);
		}
		return -1;
	...
	tris_str_reset(fields);
	tris_str_reset(values);

	for (var = head; var; var = var->next) {
		if (which == rtfield) {
			tris_debug(1, "Comparing %s to %s\n", var->name, args.fieldname);
			if (!strcasecmp(var->name, args.fieldname)) {
				tris_debug(1, "Match! Value is %s\n", var->value);
				tris_copy_string(buf, var->value, len);
				break;
			}
		} else if (which == rthash) {
			tris_debug(1, "Setting hash key %s to value %s\n", var->name, var->value);
			tris_str_append(&fields, 0, "%s%s", first ? "" : ",", tris_str_set_escapecommas(&escapebuf, 0, var->name, INT_MAX));
			tris_str_append(&values, 0, "%s%s", first ? "" : ",", tris_str_set_escapecommas(&escapebuf, 0, var->value, INT_MAX));
			first = 0;
		}
	}
	tris_variables_destroy(head);
...
static int function_realtime_store(struct tris_channel *chan, const char *cmd, char *data, const char *value)
{
	int res = 0;
	char storeid[32];
	char *valcopy;
	TRIS_DECLARE_APP_ARGS(a,
		TRIS_APP_ARG(family);
		TRIS_APP_ARG(f)[30]; /* fields */
	);

	TRIS_DECLARE_APP_ARGS(v,
		TRIS_APP_ARG(v)[30]; /* values */
	);

	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Syntax: REALTIME_STORE(family,field1,field2,...,field30) - missing argument!\n");
		return -1;
	}
	...
	valcopy = tris_strdupa(value);
	TRIS_STANDARD_APP_ARGS(a, data);
	TRIS_STANDARD_APP_ARGS(v, valcopy);

	res = tris_store_realtime(a.family, 
	...
	if (res < 0) {
		tris_log(LOG_WARNING, "Failed to store. Check the debug log for possible data repository related entries.\n");
	} else {
...
static int function_realtime_readdestroy(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len) 
{
	struct tris_variable *var, *head;
	struct tris_str *out;
	size_t resultslen;
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Syntax: REALTIME_DESTROY(family,fieldmatch[,value[,delim1[,delim2]]]) - missing argument!\n");
		return -1;
	}
	...
	tris_destroy_realtime(args.family, args.fieldmatch, args.value, SENTINEL);
	tris_variables_destroy(head);
...
struct tris_custom_function realtime_function = {
	.name = "REALTIME",
	.read = function_realtime_read,
	.write = function_realtime_write,
};

struct tris_custom_function realtimefield_function = {
	.name = "REALTIME_FIELD",
	.read = realtimefield_read,
	.write = function_realtime_write,
};

struct tris_custom_function realtimehash_function = {
	.name = "REALTIME_HASH",
	.read = realtimefield_read,
};

struct tris_custom_function realtime_store_function = {
	.name = "REALTIME_STORE",
	.write = function_realtime_store,
};

struct tris_custom_function realtime_destroy_function = {
...
{
	int res = 0;
	res |= tris_custom_function_unregister(&realtime_function);
	res |= tris_custom_function_unregister(&realtime_store_function);
	res |= tris_custom_function_unregister(&realtime_destroy_function);
	res |= tris_custom_function_unregister(&realtimefield_function);
	res |= tris_custom_function_unregister(&realtimehash_function);
	return res;
}
...
{
	int res = 0;
	res |= tris_custom_function_register(&realtime_function);
	res |= tris_custom_function_register(&realtime_store_function);
	res |= tris_custom_function_register(&realtime_destroy_function);
	res |= tris_custom_function_register(&realtimefield_function);
	res |= tris_custom_function_register(&realtimehash_function);
	return res;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Read/Write/Store/Destroy values from a RealTime repository");
