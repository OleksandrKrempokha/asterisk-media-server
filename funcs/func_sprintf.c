 ...
 ***/
static int acf_sprintf(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	for (i = 0; arg.format[i]; i++) {
		...
		case SPRINTF_CONVERSION:
			if (strchr("diouxXc", arg.format[i])) {
				...
				tris_copy_string(formatbuf, formatstart, sizeof(formatbuf));
				formatbuf[&arg.format[i] - formatstart + 1] = '\0';

				/* Convert the argument into the required type */
				if (arg.var[argcount]) {
					if (sscanf(arg.var[argcount++], "%30d", &tmpi) != 1) {
						tris_log(LOG_ERROR, "Argument '%s' is not an integer number for format '%s'\n", arg.var[argcount - 1], formatbuf);
						goto sprintf_fail;
					}
				} else {
					tris_log(LOG_ERROR, "SPRINTF() has more format specifiers than arguments!\n");
					goto sprintf_fail;
				}
				...
				if (arg.var[argcount]) {
					if (sscanf(arg.var[argcount++], "%30lf", &tmpd) != 1) {
						tris_log(LOG_ERROR, "Argument '%s' is not a floating point number for format '%s'\n", arg.var[argcount - 1], formatbuf);
						goto sprintf_fail;
					}
				} else {
					tris_log(LOG_ERROR, "SPRINTF() has more format specifiers than arguments!\n");
				...
				*bufptr++ = arg.format[i];
				...
				tris_log(LOG_ERROR, "Format type not supported: '%s' with argument '%s'\n", formatbuf, arg.var[argcount++]);
				goto sprintf_fail;
			}
			state = -1;
			break;
...
static struct tris_custom_function sprintf_function = {
	.name = "SPRINTF",
	.read = acf_sprintf,
};
...
static int unload_module(void)
{
	...
	res |= tris_custom_function_unregister(&sprintf_function);
...
static int load_module(void)
{
	...
	res |= tris_custom_function_register(&sprintf_function);

	return res;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "SPRINTF dialplan function");
