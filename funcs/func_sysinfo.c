...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 87233 $")
...
static int sysinfo_helper(struct tris_channel *chan, const char *cmd, char *data,
		                         char *buf, size_t len)
{
#if defined(HAVE_SYSINFO)
	struct sysinfo sys_info;
	if (sysinfo(&sys_info)) {
		tris_log(LOG_ERROR, "FAILED to retrieve system information\n");
		return -1;
	}
#endif
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Syntax: ${SYSINFO(<parameter>)} - missing argument!)\n");
		return -1;
		...
		snprintf(buf, len, "%d", tris_active_calls());
	}
#if defined(HAVE_SYSINFO)
		...
		tris_log(LOG_ERROR, "Unknown sysinfo parameter type '%s'.\n", data);
		return -1;
	}
...
static struct tris_custom_function sysinfo_function = {
	.name = "SYSINFO",
...
{
	return tris_custom_function_unregister(&sysinfo_function);
}

static int load_module(void)
{
	return tris_custom_function_register(&sysinfo_function);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "System information related functions");
