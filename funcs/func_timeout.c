...
static int timeout_read(struct tris_channel *chan, const char *cmd, char *data,
			char *buf, size_t len)
{
	struct timeval myt;
	...
	if (!data) {
		tris_log(LOG_ERROR, "Must specify type of timeout to get.\n");
		return -1;
	}
	...
	case 'A':
		if (tris_tvzero(chan->whentohangup)) {
			...
			myt = tris_tvnow();
			snprintf(buf, len, "%.3f", tris_tvdiff_ms(chan->whentohangup, myt) / 1000.0);
		}
		break;
	...
	default:
		tris_log(LOG_ERROR, "Unknown timeout type specified.\n");
		return -1;
	}
...
static int timeout_write(struct tris_channel *chan, const char *cmd, char *data,
			 const char *value)
{
	double x = 0.0;
	long sec = 0L;
	...
	if (!data) {
		tris_log(LOG_ERROR, "Must specify type of timeout to set.\n");
		return -1;
	}
	...
	case 'A':
		...
		if (VERBOSITY_ATLEAST(3)) {
			if (!tris_tvzero(chan->whentohangup)) {
				when = tris_tvadd(when, tris_tvnow());
				tris_strftime(timestr, sizeof(timestr), "%Y-%m-%d %H:%M:%S.%3q %Z",
					tris_localtime(&when, &myt, NULL));
				tris_verbose("Channel will hangup at %s.\n", timestr);
			} else {
				tris_verbose("Channel hangup cancelled.\n");
			}
		}
		break;
	...
	case 'R':
		if (chan->pbx) {
			...
			tris_verb(3, "Response timeout set to %.3f\n", chan->pbx->rtimeoutms / 1000.0);
		}
		break;
	...
	case 'D':
		if (chan->pbx) {
			...
			tris_verb(3, "Digit timeout set to %.3f\n", chan->pbx->dtimeoutms / 1000.0);
		}
		break;
...
static struct tris_custom_function timeout_function = {
	.name = "TIMEOUT",
	.read = timeout_read,
	.write = timeout_write,
};
...
{
	return tris_custom_function_unregister(&timeout_function);
}

static int load_module(void)
{
	return tris_custom_function_register(&timeout_function);
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Channel timeout dialplan functions");
