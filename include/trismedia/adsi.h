...
#ifndef _TRISMEDIA_ADSI_H
#define _TRISMEDIA_ADSI_H
...
/*! Perform Trismedia ADSI initialization (for channel drivers that want 
 ...
 * \retval -1 on hangup.
 */
extern int (*tris_adsi_channel_init)(struct tris_channel *chan);

extern int (*tris_adsi_begin_download)(struct tris_channel *chan, char *service, unsigned char *fdn, unsigned char *sec, int version);

extern int (*tris_adsi_end_download)(struct tris_channel *chan);
 ...
 *   have to use the tris_adsi_channel_init again instead. 
 * \param chan Channel to restore
 *
 * \retval 0 on success (or adsi unavailable)
 * \retval -1 on hangup
 */
extern int (*tris_adsi_channel_restore)(struct tris_channel *chan);
 ...
 * \retval -1 on hangup
 */
extern int (*tris_adsi_print)(struct tris_channel *chan, char **lines, int *align, int voice);
 ...
 */
extern int (*tris_adsi_load_session)(struct tris_channel *chan, unsigned char *app, int ver, int data);
extern int (*tris_adsi_unload_session)(struct tris_channel *chan);

/* ADSI Layer 2 transmission functions */
extern int (*tris_adsi_transmit_messages)(struct tris_channel *chan, unsigned char **msg, int *msglen, int *msgtype);
extern int (*tris_adsi_transmit_message)(struct tris_channel *chan, unsigned char *msg, int msglen, int msgtype);
extern int (*tris_adsi_transmit_message_full)(struct tris_channel *chan, unsigned char *msg, int msglen, int msgtype, int dowait);
/*! Read some encoded DTMF data. 
 * Returns number of bytes received
 */
extern int (*tris_adsi_read_encoded_dtmf)(struct tris_channel *chan, unsigned char *buf, int maxlen);
...
extern int (*tris_adsi_connect_session)(unsigned char *buf, unsigned char *fdn, int ver);

/*! Build Query CPE ID of equipment. 
 *  Returns number of bytes added to message
 */
extern int (*tris_adsi_query_cpeid)(unsigned char *buf);
extern int (*tris_adsi_query_cpeinfo)(unsigned char *buf);
 ...
 */
extern int (*tris_adsi_get_cpeid)(struct tris_channel *chan, unsigned char *cpeid, int voice);

extern int (*tris_adsi_get_cpeinfo)(struct tris_channel *chan, int *width, int *height, int *buttons, int voice);
...
extern int (*tris_adsi_download_connect)(unsigned char *buf, char *service, unsigned char *fdn, unsigned char *sec, int ver);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_disconnect_session)(unsigned char *buf);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_download_disconnect)(unsigned char *buf);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_data_mode)(unsigned char *buf);
extern int (*tris_adsi_clear_soft_keys)(unsigned char *buf);
extern int (*tris_adsi_clear_screen)(unsigned char *buf);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_voice_mode)(unsigned char *buf, int when);

/*! 
 * \brief Returns non-zero if Channel does or might support ADSI 
 * \param chan Channel to check
 */
extern int (*tris_adsi_available)(struct tris_channel *chan);
...
extern int (*tris_adsi_display)(unsigned char *buf, int page, int line, int just, int wrap, char *col1, char *col2);
...
extern int (*tris_adsi_set_line)(unsigned char *buf, int page, int line);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_load_soft_key)(unsigned char *buf, int key, const char *llabel, const char *slabel, char *ret, int data);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_set_keys)(unsigned char *buf, unsigned char *keys);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_input_control)(unsigned char *buf, int page, int line, int display, int format, int just);
 ...
 * \retval -1 on error.
 */
extern int (*tris_adsi_input_format)(unsigned char *buf, int num, int dir, int wrap, char *format1, char *format2);

#endif /* _TRISMEDIA_ADSI_H */
