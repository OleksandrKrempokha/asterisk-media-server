...
/*
 * Object Model for Trismedia
 */

#ifndef _TRISMEDIA_ASTOBJ_H
#define _TRISMEDIA_ASTOBJ_H
 ...
 *    ASTOBJ_INIT(obj1);
 *    ASTOBJ_WRLOCK(obj1);
 *    tris_copy_string(obj1->name, "obj1", sizeof(obj1->name));
 *    ASTOBJ_UNLOCK(obj1);
 *
...
#define ASTOBJ_DEFAULT_HASH		tris_strhash
 ...
 */
#define ASTOBJ_RDLOCK(object) tris_mutex_lock(&(object)->_lock)

/*! \brief Lock an ASTOBJ for writing.
 */
#define ASTOBJ_WRLOCK(object) tris_mutex_lock(&(object)->_lock)

#define ASTOBJ_TRYWRLOCK(object) tris_mutex_trylock(&(object)->_lock)

/*! \brief Unlock a locked object. */
#define ASTOBJ_UNLOCK(object) tris_mutex_unlock(&(object)->_lock)
...
#define ASTOBJ_UNREF(object,destructor) \
	do { \
		...
		if (__builtin_expect((object)->refcount > 0, 1)) \
			...
			tris_log(TRIS_LOG_WARNING, "Unreferencing unreferenced (object)!\n"); \
		ASTOBJ_UNLOCK(object); \
		if (newcount == 0) { \
			tris_mutex_destroy(&(object)->_lock); \
			destructor((object)); \
		} \
		(object) = NULL; \
	} while(0)
 ...
 */
#define ASTOBJ_CONTAINER_RDLOCK(container) tris_mutex_lock(&(container)->_lock)

/*! \brief Lock an ASTOBJ_CONTAINER for writing. 
 */
#define ASTOBJ_CONTAINER_WRLOCK(container) tris_mutex_lock(&(container)->_lock)

/*! \brief Unlock an ASTOBJ_CONTAINER. */
#define ASTOBJ_CONTAINER_UNLOCK(container) tris_mutex_unlock(&(container)->_lock)
...
#define ASTOBJ_CONTAINER_INIT_FULL(container,hashes,buckets) \
	do { \
		tris_mutex_init(&(container)->_lock); \
	} while(0)
...
#define ASTOBJ_CONTAINER_DESTROY_FULL(container,hashes,buckets) \
	do { \
		tris_mutex_destroy(&(container)->_lock); \
	} while(0)
...
#define ASTOBJ_CONTAINER_DUMP(fd,s,slen,container) \
	ASTOBJ_CONTAINER_TRAVERSE(container, 1, do { ASTOBJ_DUMP(s,slen,iterator); tris_cli(fd, "%s", s); } while(0))

#if defined(__cplusplus) || defined(c_plusplus)
}
#endif

#endif /* _TRISMEDIA_ASTOBJ_H */
