...
struct tris_bridge_channel {
	...
	/*! Trismedia channel participating in the bridge */
	struct tris_channel *chan;
	/*! Trismedia channel we are swapping with (if swapping) */
	struct tris_channel *swap;
	/*! Bridge this channel is participating in */
	struct tris_bridge *bridge;
	...
	TRIS_LIST_ENTRY(tris_bridge_channel) entry;
};
...
struct tris_bridge {
	...
	struct tris_bridge_technology *technology;
	/*! Private information unique to the bridge technology */
	void *bridge_pvt;
 ...
 * \code
 * struct tris_bridge *bridge;
 * bridge = tris_bridge_new(TRIS_BRIDGE_CAPABILITY_1TO1MIX, TRIS_BRIDGE_FLAG_DISSOLVE);
 * \endcode
 *
 ...
 *
 * \code
 * int possible = tris_bridge_check(TRIS_BRIDGE_CAPABILITY_1TO1MIX);
 * \endcode
 *
 ...
 *
 * \code
 * tris_bridge_destroy(bridge);
 * \endcode
 *
 * This destroys a bridge that was previously created using tris_bridge_new.
 */
 ...
 *
 * \code
 * tris_bridge_join(bridge, chan, NULL, NULL);
 * \endcode
 *
 ...
 *
 * \code
 * tris_bridge_depart(bridge, chan);
 * \endcode
 *
 ...
 *       using the tris_bridge_impart API call.
 */
 ...
 *
 * \code
 * tris_bridge_remove(bridge, chan);
 * \endcode
 *
 * This removes the channel pointed to by the chan pointer from the bridge
 ...
 *       using both tris_bridge_join and tris_bridge_impart.
 */
 ...
 *
 * \code
 * tris_bridge_merge(bridge0, bridge1);
 * \endcode
 *
 ...
 *
 * \code
 * tris_bridge_suspend(bridge, chan);
 * \endcode
 *
 * This suspends the channel pointed to by chan from the bridge pointed to by bridge temporarily.
 * Control of the channel is given to the calling thread. This differs from tris_bridge_depart as
 ...
 *
 * \code
 * tris_bridge_unsuspend(bridge, chan);
 * \endcode
 *
 ...
 * TRIS_BRIDGE_CHANNEL_STATE_WAIT.
 *
