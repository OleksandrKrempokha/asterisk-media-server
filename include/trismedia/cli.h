...
#ifndef _TRISMEDIA_CLI_H
#define _TRISMEDIA_CLI_H
...
#define TRIS_MAX_CMD_LEN 	16

#define TRIS_MAX_ARGS 64

#define TRIS_CLI_COMPLETE_EOF	"_EOF_"
...
/*! \page CLI_command_API CLI command API
   ...
   CLI commands are described by a struct tris_cli_entry that contains
   ...
   The prototype is the following:
	...
	char *new_setdebug(const struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);

	...
	// this is how we create the entry to register 
	TRIS_CLI_DEFINE(new_setdebug, "short description")
	...

   To help the transition, we make the pointer to the struct tris_cli_entry
   available to old-style handlers via argv[-1].

   An example of new-style handler is the following

\code
static char *test_new_cli(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	static char *choices = { "one", "two", "three", NULL };
        ...
        case CLI_GENERATE:
                if (a->pos > e->args)
                        return NULL;
        	return tris_cli_complete(a->word, choices, a->n);

        default:        
                // we are guaranteed to be called with argc >= e->args;
                if (a->argc > e->args + 1) // we accept one extra argument
                        return CLI_SHOWUSAGE;
                tris_cli(a->fd, "done this well for %s\n", e->args[argc-1]);
                return CLI_SUCCESS;
        }
}
...
enum tris_cli_fn {
	...
	CLI_GENERATE = -3,	/* behave as 'generator', remap argv to struct tris_cli_args */
	CLI_HANDLER = -4,	/* run the normal handler */
};

/* argument for new-style CLI handler */
struct tris_cli_args {
	int fd;
	int argc;
	char **argv;
...
typedef char *(*cli_fn)(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);

/*! \brief descriptor for a cli entry. 
 * \arg \ref CLI_command_API
 */
struct tris_cli_entry {
	char * const cmda[TRIS_MAX_CMD_LEN];	/*!< words making up the command.
 ...
 * (in this example we complete the word in position 2):
  \code
    char *my_generate(const char *line, const char *word, int pos, int n)
    {
        static char *choices = { "one", "two", "three", NULL };
	if (pos == 2)
        	return tris_cli_complete(word, choices, n);
	else
		return NULL;
    }
  \endcode
 */
char *tris_cli_complete(const char *word, char *const choices[], int pos);
...
#define tris_cli_command(fd,s) tris_cli_command_full(CLI_NO_PERMS, CLI_NO_PERMS, fd, s) 
...
#define tris_cli_command_multiple(fd,size,s) tris_cli_command_multiple_full(CLI_NO_PERMS, CLI_NO_PERMS, fd, size, s)
 ...
 * Unregister your own command.  You must pass a completed tris_cli_entry structure
 * \return 0
 */
 ...
 * \retval -1 on failure
 */
char *tris_cli_generator(const char *, const char *, int);

int tris_cli_generatornummatches(const char *, const char *);
 ...
 * by the caller.
 */
char **tris_cli_completion_matches(const char *, const char *);
