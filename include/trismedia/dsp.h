/*
 * Trismedia -- An open source telephony toolkit.
 *
 * Copyright (C) 1999 - 2005, Digium, Inc.
 *
 * Mark Spencer <markster@digium.com>
 *
 * See http://www.trismedia.org for more information about
 * the Trismedia project. Please do not directly contact
 * any of the maintainers of this project for assistance;
 * the project provides a web site, mailing lists and IRC
 * channels for your use.
 *
 * This program is free software, distributed under the terms of
 * the GNU General Public License Version 2. See the LICENSE file
 * at the top of the source tree.
 */

/*! \file
 * \brief Convenient Signal Processing routines
 */

#ifndef _TRISMEDIA_DSP_H
#define _TRISMEDIA_DSP_H

#define DSP_FEATURE_SILENCE_SUPPRESS	(1 << 0)
#define DSP_FEATURE_BUSY_DETECT		(1 << 1)
#define DSP_FEATURE_DIGIT_DETECT	(1 << 3)
#define DSP_FEATURE_FAX_DETECT		(1 << 4)

#define	DSP_DIGITMODE_DTMF			0				/*!< Detect DTMF digits */
#define DSP_DIGITMODE_MF			1				/*!< Detect MF digits */

#define DSP_DIGITMODE_NOQUELCH		(1 << 8)		/*!< Do not quelch DTMF from in-band */
#define DSP_DIGITMODE_MUTECONF		(1 << 9)		/*!< Mute conference */
#define DSP_DIGITMODE_MUTEMAX		(1 << 10)		/*!< Delay audio by a frame to try to extra quelch */
#define DSP_DIGITMODE_RELAXDTMF		(1 << 11)		/*!< "Radio" mode (relaxed DTMF) */

#define DSP_PROGRESS_TALK		(1 << 16)		/*!< Enable talk detection */
#define DSP_PROGRESS_RINGING		(1 << 17)		/*!< Enable calling tone detection */
#define DSP_PROGRESS_BUSY		(1 << 18)		/*!< Enable busy tone detection */
#define DSP_PROGRESS_CONGESTION		(1 << 19)		/*!< Enable congestion tone detection */
#define DSP_FEATURE_CALL_PROGRESS	(DSP_PROGRESS_TALK | DSP_PROGRESS_RINGING | DSP_PROGRESS_BUSY | DSP_PROGRESS_CONGESTION)
#define DSP_FEATURE_WAITDIALTONE	(1 << 20)		/*!< Enable dial tone detection */

#define DSP_FAXMODE_DETECT_CNG	(1 << 0)
#define DSP_FAXMODE_DETECT_CED	(1 << 1)
#define DSP_FAXMODE_DETECT_ALL	(DSP_FAXMODE_DETECT_CNG | DSP_FAXMODE_DETECT_CED)

#define DSP_TONE_STATE_SILENCE  0
#define DSP_TONE_STATE_RINGING  1
#define DSP_TONE_STATE_DIALTONE 2
#define DSP_TONE_STATE_TALKING  3
#define DSP_TONE_STATE_BUSY     4
#define DSP_TONE_STATE_SPECIAL1	5
#define DSP_TONE_STATE_SPECIAL2 6
#define DSP_TONE_STATE_SPECIAL3 7
#define DSP_TONE_STATE_HUNGUP 	8

struct tris_dsp;

enum threshold {
	/* Array offsets */
	THRESHOLD_SILENCE = 0,
	/* Always the last */
	THRESHOLD_MAX = 1,
};

struct tris_dsp *tris_dsp_new(void);
void tris_dsp_free(struct tris_dsp *dsp);

/*! \brief Set threshold value for silence */
void tris_dsp_set_threshold(struct tris_dsp *dsp, int threshold);

/*! \brief Set number of required cadences for busy */
void tris_dsp_set_busy_count(struct tris_dsp *dsp, int cadences);

/*! \brief Set expected lengths of the busy tone */
void tris_dsp_set_busy_pattern(struct tris_dsp *dsp, int *tonelength, int *quietlength);

/*! \brief Scans for progress indication in audio */
int tris_dsp_call_progress(struct tris_dsp *dsp, struct tris_frame *inf);

/*! \brief Set zone for doing progress detection */
int tris_dsp_set_call_progress_zone(struct tris_dsp *dsp, char *zone);

/*! \brief Return TRIS_FRAME_NULL frames when there is silence, TRIS_FRAME_BUSY on
   busies, and call progress, all dependent upon which features are enabled */
struct tris_frame *tris_dsp_process(struct tris_channel *chan, struct tris_dsp *dsp, struct tris_frame *inf);

/*! \brief Return non-zero if this is silence.  Updates "totalsilence" with the total
   number of seconds of silence  */
int tris_dsp_silence(struct tris_dsp *dsp, struct tris_frame *f, int *totalsilence);

/*!
 * \brief Return non-zero if this is noise.  Updates "totalnoise" with the total
 * number of seconds of noise
 * \since 1.6.1
 */
int tris_dsp_noise(struct tris_dsp *dsp, struct tris_frame *f, int *totalnoise);

/*! \brief Return non-zero if historically this should be a busy, request that
  tris_dsp_silence has already been called */
int tris_dsp_busydetect(struct tris_dsp *dsp);

/*! \brief Return non-zero if DTMF hit was found */
int tris_dsp_digitdetect(struct tris_dsp *dsp, struct tris_frame *f);

/*! \brief Reset total silence count */
void tris_dsp_reset(struct tris_dsp *dsp);

/*! \brief Reset DTMF detector */
void tris_dsp_digitreset(struct tris_dsp *dsp);

/*! \brief Select feature set */
void tris_dsp_set_features(struct tris_dsp *dsp, int features);

/*! \brief Get pending DTMF/MF digits */
int tris_dsp_getdigits(struct tris_dsp *dsp, char *buf, int max);

/*! \brief Set digit mode
 * \version 1.6.1 renamed from tris_dsp_digitmode to tris_dsp_set_digitmode
 */
int tris_dsp_set_digitmode(struct tris_dsp *dsp, int digitmode);

/*! \brief Set fax mode */
int tris_dsp_set_faxmode(struct tris_dsp *dsp, int faxmode);

/*!
 * \brief Returns true if DSP code was muting any fragment of the last processed frame.
 * Muting (squelching) happens when DSP code removes DTMF/MF/generic tones from the audio
 * \since 1.6.1
 */
int tris_dsp_was_muted(struct tris_dsp *dsp);

/*! \brief Get tstate (Tone State) */
int tris_dsp_get_tstate(struct tris_dsp *dsp);

/*! \brief Get tcount (Threshold counter) */
int tris_dsp_get_tcount(struct tris_dsp *dsp);

/*!
 * \brief Get silence threshold from dsp.conf
 * \since 1.6.1
 */
int tris_dsp_get_threshold_from_settings(enum threshold which);

/*!
 * \brief Reloads dsp settings from dsp.conf
 * \since 1.6.1
 */
int tris_dsp_reload(void);

/*!
 * \brief Load dsp settings from dsp.conf
 * \since 1.6.1
 */
int tris_dsp_init(void);

/*!
 * \brief Hint that a frame from a dsp was freed
 *
 * This is called from tris_frame_free if TRIS_FRFLAG_FROM_DSP is set. This occurs
 * because it is possible for the dsp to be freed while someone still holds a reference
 * to the frame that is in that dsp. This has been known to happen when the dsp on a DAHDI
 * channel detects a busy signal. The channel is hung up, and the application that read the
 * frame to begin with still has a reference to the frame.
 *
 * \return nothing
 */
void tris_dsp_frame_freed(struct tris_frame *fr);
#endif /* _TRISMEDIA_DSP_H */
