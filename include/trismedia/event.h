 ...
 * Trismedia modules.  Code can generate events, and other code can subscribe to
 * them.
 *
 ...
 * to internal Trismedia MWI events with the MAILBOX information element with
 * a value of "1234".
 *
...
#include "trismedia/event_defs.h"

/*!
 * \brief Subscriber event callback type
 *
 * \param event the event being passed to the subscriber
 * \param userdata the data provider in the call to tris_event_subscribe()
 *
 * \return The event callbacks do not return anything.
 */
typedef void (*tris_event_cb_t)(const struct tris_event *event, void *userdata);
 ...
 * \code
 *    <enum tris_event_ie_type>, [enum tris_event_ie_pltype, [payload] ]
 * \endcode
 * and must end with TRIS_EVENT_IE_END.
 *
 * If the ie_type specified is *not* TRIS_EVENT_IE_END, then it must be followed
 * by a valid IE payload type.  If the payload type specified is
 * TRIS_EVENT_IE_PLTYPE_EXISTS, then the 3rd argument should not be provided.
 ...
 *
 * \code
 * peer->mwi_event_sub = tris_event_subscribe(TRIS_EVENT_MWI, mwi_event_cb, peer,
 *     TRIS_EVENT_IE_MAILBOX, TRIS_EVENT_IE_PLTYPE_STR, peer->mailbox,
 *     TRIS_EVENT_IE_END);
 * \endcode
 *
 * This creates a subscription to TRIS_EVENT_MWI events that contain an
 * information element, TRIS_EVENT_IE_MAILBOX, with the same string value
 * contained in peer->mailbox.  Also, the event callback will be passed a
 * pointer to the peer.
 */
struct tris_event_sub *tris_event_subscribe(enum tris_event_type event_type,
 ...
 * tris_event_subscribe_new(), but for some reason, you want to destroy it
 ...
 *
 * \param sub the dynamic subscription allocated with tris_event_subscribe_new()
...
int tris_event_sub_append_ie_uint(struct tris_event_sub *sub,
	enum tris_event_ie_type ie_type, uint32_t uint);
 ...
 */
struct tris_event_sub *tris_event_unsubscribe(struct tris_event_sub *event_sub);
 ...
 * Otherwise, a payload must also be specified.
 *
 * \return This returns one of the values defined in the tris_event_subscriber_res
 ...
 *
 * \code
 * if (tris_event_check_subscriber(TRIS_EVENT_MWI,
 ...
 */
enum tris_event_subscriber_res tris_event_check_subscriber(enum tris_event_type event_type, ...);
 ...
 */
void tris_event_report_subs(const struct tris_event_sub *sub);
 ...
 */
struct tris_event *tris_event_new(enum tris_event_type event_type, ...);
 ...
 *
 * \param tris_event_type The type of event to retrieve from the cache
 *
 * The rest of the arguments to this function specify information elements to
 ...
 *
 * \note The caller of this function *must* call tris_event_destroy() on the
 ...
 */
struct tris_event *tris_event_get_cached(enum tris_event_type, ...);
 ...
 *
 * \return This function returns the hash value as calculated by tris_str_hash()
