...
#ifndef _TRISMEDIA_FILE_H
#define _TRISMEDIA_FILE_H
...
#define TRIS_DIGIT_ANY "0123456789#*ABCD"
#define TRIS_DIGIT_ANYNUM "0123456789"
 ...
 * Prepares a channel for the streaming of a file.  To start the stream, afterward do a tris_waitstream() on the channel
 * Also, it will stop any existing streams on the channel.
 * \retval 0 on success.
 * \retval -1 on failure.
 */
int tris_streamfile(struct tris_channel *c, const char *filename, const char *preflang);
 ...
 */
int tris_stopstream(struct tris_channel *c);
 ...
 * \return -1 on failure
 */
int tris_filerename(const char *oldname, const char *newname, const char *fmt);
 ...
 */
int tris_filecopy(const char *oldname, const char *newname, const char *fmt);
 ...
 * \retval -1 on error.
 */
int tris_waitstream_fr(struct tris_channel *c, const char *breakon, const char *forward, const char *rewind, int ms);
 ...
 */
int tris_waitstream_full(struct tris_channel *c, const char *breakon, int audiofd, int monfd);
 ...
 * Please note, this is a blocking function.  Program execution will not return until tris_waitstream completes it's execution.
 * \retval a struct tris_filestream on success.
 ...
 * Send a frame to a filestream -- note: does NOT free the frame, call tris_frfree manually
 * \retval 0 on success.
 * \retval -1 on failure.
 */
 ...
 * \retval a tris_filestream pointer if it opens the file.
 ...
 * \param s tris_filestream to apply
 * \retval 0 on success.
 * \retval -1 on failure.
 */
 ...
 * \param fs tris_filestream to perform seek on
 ...
 * \param s tris_filestream to act on
 * \return a frame.
...
#define TRIS_RESERVED_POINTERS 20
