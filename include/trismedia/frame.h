/*
 * Trismedia -- An open source telephony toolkit.
 *
 * Copyright (C) 1999 - 2005, Digium, Inc.
 *
 * Mark Spencer <markster@digium.com>
 *
 * See http://www.trismedia.org for more information about
 * the Trismedia project. Please do not directly contact
 * any of the maintainers of this project for assistance;
 * the project provides a web site, mailing lists and IRC
 * channels for your use.
 *
 * This program is free software, distributed under the terms of
 * the GNU General Public License Version 2. See the LICENSE file
 * at the top of the source tree.
 */

/*! \file
 * \brief Trismedia internal frame definitions.
 * \arg For an explanation of frames, see \ref Def_Frame
 * \arg Frames are send of Trismedia channels, see \ref Def_Channel
 */

#ifndef _TRISMEDIA_FRAME_H
#define _TRISMEDIA_FRAME_H

#if defined(__cplusplus) || defined(c_plusplus)
extern "C" {
#endif

#include <sys/time.h>

#include "trismedia/endian.h"
#include "trismedia/linkedlists.h"

struct tris_codec_pref {
	char order[32];
	char framing[32];
};

/*! \page Def_Frame AST Multimedia and signalling frames
	\section Def_AstFrame What is an tris_frame ?
 	A frame of data read used to communicate between 
 	between channels and applications.
	Frames are divided into frame types and subclasses.

	\par Frame types 
	\arg \b VOICE:	Voice data, subclass is codec (TRIS_FORMAT_*)
	\arg \b VIDEO:	Video data, subclass is codec (TRIS_FORMAT_*)
	\arg \b DTMF:	A DTMF digit, subclass is the digit
	\arg \b IMAGE:	Image transport, mostly used in IAX
	\arg \b TEXT:	Text messages and character by character (real time text)
	\arg \b HTML:	URL's and web pages
	\arg \b MODEM:	Modulated data encodings, such as T.38 and V.150
	\arg \b IAX:	Private frame type for the IAX protocol
	\arg \b CNG:	Comfort noice frames
	\arg \b CONTROL:	A control frame, subclass defined as TRIS_CONTROL_
	\arg \b NULL:	Empty, useless frame

	\par Files
	\arg frame.h	Definitions
	\arg frame.c	Function library
	\arg \ref Def_Channel Trismedia channels
	\section Def_ControlFrame Control Frames
	Control frames send signalling information between channels
	and devices. They are prefixed with TRIS_CONTROL_, like TRIS_CONTROL_FRAME_HANGUP
	\arg \b HANGUP	The other end has hungup
	\arg \b RING	Local ring
	\arg \b RINGING	The other end is ringing
	\arg \b ANSWER	The other end has answered
	\arg \b BUSY	Remote end is busy
	\arg \b TAKEOFFHOOK	Make it go off hook (what's "it" ? )
	\arg \b OFFHOOK	Line is off hook
	\arg \b CONGESTION	Congestion (circuit is busy, not available)
	\arg \b FLASH	Other end sends flash hook
	\arg \b WINK	Other end sends wink
	\arg \b OPTION	Send low-level option
	\arg \b RADIO_KEY	Key radio (see app_rpt.c)
	\arg \b RADIO_UNKEY	Un-key radio (see app_rpt.c)
	\arg \b PROGRESS	Other end indicates call progress
	\arg \b PROCEEDING	Indicates proceeding
	\arg \b HOLD	Call is placed on hold
	\arg \b UNHOLD	Call is back from hold
	\arg \b VIDUPDATE	Video update requested
	\arg \b SRCUPDATE       The source of media has changed

*/

/*!
 * \brief Frame types 
 *
 * \note It is important that the values of each frame type are never changed,
 *       because it will break backwards compatability with older versions.
 *       This is because these constants are transmitted directly over IAX2.
 */
enum tris_frame_type {
	/*! DTMF end event, subclass is the digit */
	TRIS_FRAME_DTMF_END = 1,
	/*! Voice data, subclass is TRIS_FORMAT_* */
	TRIS_FRAME_VOICE,
	/*! Video frame, maybe?? :) */
	TRIS_FRAME_VIDEO,
	/*! A control frame, subclass is TRIS_CONTROL_* */
	TRIS_FRAME_CONTROL,
	/*! An empty, useless frame */
	TRIS_FRAME_NULL,
	/*! Inter Trismedia Exchange private frame type */
	TRIS_FRAME_IAX,
	/*! Text messages */
	TRIS_FRAME_TEXT,
	/*! File frame, maybe?? :) */
	TRIS_FRAME_FILE,
	/*! Desktop frame, maybe?? :) */
	TRIS_FRAME_DESKTOP,
	/*! Chat frame, maybe?? :) */
	TRIS_FRAME_CHAT,
	/*! Image Frames */
	TRIS_FRAME_IMAGE,
	/*! HTML Frame */
	TRIS_FRAME_HTML,
	/*! Comfort Noise frame (subclass is level of CNG in -dBov), 
	    body may include zero or more 8-bit quantization coefficients */
	TRIS_FRAME_CNG,
	/*! Modem-over-IP data streams */
	TRIS_FRAME_MODEM,	
	/*! DTMF begin event, subclass is the digit */
	TRIS_FRAME_DTMF_BEGIN,
};
#define TRIS_FRAME_DTMF TRIS_FRAME_DTMF_END

enum {
	/*! This frame contains valid timing information */
	TRIS_FRFLAG_HAS_TIMING_INFO = (1 << 0),
	/*! This frame came from a translator and is still the original frame.
	 *  The translator can not be free'd if the frame inside of it still has
	 *  this flag set. */
	TRIS_FRFLAG_FROM_TRANSLATOR = (1 << 1),
	/*! This frame came from a dsp and is still the original frame.
	 *  The dsp cannot be free'd if the frame inside of it still has
	 *  this flag set. */
	TRIS_FRFLAG_FROM_DSP = (1 << 2),
};

/*! \brief Data structure associated with a single frame of data
 */
struct tris_frame {
	/*! Kind of frame */
	enum tris_frame_type frametype;				
	/*! Subclass, frame dependent */
	int subclass;				
	/*! Length of data */
	int datalen;				
	/*! Number of samples in this frame */
	int samples;				
	/*! Was the data malloc'd?  i.e. should we free it when we discard the frame? */
	int mallocd;				
	/*! The number of bytes allocated for a malloc'd frame header */
	size_t mallocd_hdr_len;
	/*! How many bytes exist _before_ "data" that can be used if needed */
	int offset;				
	/*! Optional source of frame for debugging */
	const char *src;				
	/*! Pointer to actual data */
	union { void *ptr; uint32_t uint32; char pad[8]; } data;
	/*! Global delivery time */		
	struct timeval delivery;
	/*! For placing in a linked list */
	TRIS_LIST_ENTRY(tris_frame) frame_list;
	/*! Misc. frame flags */
	unsigned int flags;
	/*! Timestamp in milliseconds */
	long ts;
	/*! Length in milliseconds */
	long len;
	/*! Sequence number */
	int seqno;
	int promoter;
};

/*!
 * Set the various field of a frame to point to a buffer.
 * Typically you set the base address of the buffer, the offset as
 * TRIS_FRIENDLY_OFFSET, and the datalen as the amount of bytes queued.
 * The remaining things (to be done manually) is set the number of
 * samples, which cannot be derived from the datalen unless you know
 * the number of bits per sample.
 */
#define	TRIS_FRAME_SET_BUFFER(fr, _base, _ofs, _datalen)	\
	{					\
	(fr)->data.ptr = (char *)_base + (_ofs);	\
	(fr)->offset = (_ofs);			\
	(fr)->datalen = (_datalen);		\
	}

/*! Queueing a null frame is fairly common, so we declare a global null frame object
    for this purpose instead of having to declare one on the stack */
extern struct tris_frame tris_null_frame;

/*! \brief Offset into a frame's data buffer.
 *
 * By providing some "empty" space prior to the actual data of an tris_frame,
 * this gives any consumer of the frame ample space to prepend other necessary
 * information without having to create a new buffer.
 *
 * As an example, RTP can use the data from an tris_frame and simply prepend the
 * RTP header information into the space provided by TRIS_FRIENDLY_OFFSET instead
 * of having to create a new buffer with the necessary space allocated.
 */
#define TRIS_FRIENDLY_OFFSET 	64	
#define TRIS_MIN_OFFSET 		32	/*! Make sure we keep at least this much handy */

/*! Need the header be free'd? */
#define TRIS_MALLOCD_HDR		(1 << 0)
/*! Need the data be free'd? */
#define TRIS_MALLOCD_DATA	(1 << 1)
/*! Need the source be free'd? (haha!) */
#define TRIS_MALLOCD_SRC		(1 << 2)

/* MODEM subclasses */
/*! T.38 Fax-over-IP */
#define TRIS_MODEM_T38		1
/*! V.150 Modem-over-IP */
#define TRIS_MODEM_V150		2

/* HTML subclasses */
/*! Sending a URL */
#define TRIS_HTML_URL		1
/*! Data frame */
#define TRIS_HTML_DATA		2
/*! Beginning frame */
#define TRIS_HTML_BEGIN		4
/*! End frame */
#define TRIS_HTML_END		8
/*! Load is complete */
#define TRIS_HTML_LDCOMPLETE	16
/*! Peer is unable to support HTML */
#define TRIS_HTML_NOSUPPORT	17
/*! Send URL, and track */
#define TRIS_HTML_LINKURL	18
/*! No more HTML linkage */
#define TRIS_HTML_UNLINK		19
/*! Reject link request */
#define TRIS_HTML_LINKREJECT	20

/* Data formats for capabilities and frames alike */
/*! G.723.1 compression */
#define TRIS_FORMAT_G723_1	(1 << 0)
/*! GSM compression */
#define TRIS_FORMAT_GSM		(1 << 1)
/*! Raw mu-law data (G.711) */
#define TRIS_FORMAT_ULAW		(1 << 2)
/*! Raw A-law data (G.711) */
#define TRIS_FORMAT_ALAW		(1 << 3)
/*! ADPCM (G.726, 32kbps, AAL2 codeword packing) */
#define TRIS_FORMAT_G726_AAL2	(1 << 4)
/*! ADPCM (IMA) */
#define TRIS_FORMAT_ADPCM	(1 << 5)
/*! Raw 16-bit Signed Linear (8000 Hz) PCM */
#define TRIS_FORMAT_SLINEAR	(1 << 6)
/*! LPC10, 180 samples/frame */
#define TRIS_FORMAT_LPC10	(1 << 7)
/*! G.729A audio */
#define TRIS_FORMAT_G729A	(1 << 8)
/*! SpeeX Free Compression */
#define TRIS_FORMAT_SPEEX	(1 << 9)
/*! iLBC Free Compression */
#define TRIS_FORMAT_ILBC		(1 << 10)
/*! ADPCM (G.726, 32kbps, RFC3551 codeword packing) */
#define TRIS_FORMAT_G726		(1 << 11)
/*! G.722 */
#define TRIS_FORMAT_G722		(1 << 12)
/*! G.722.1 (also known as Siren7, 32kbps assumed) */
#define TRIS_FORMAT_SPEEX16	(1 << 13)
/*! G.722.1 Annex C (also known as Siren14, 48kbps assumed) */
#define TRIS_FORMAT_SIREN14	(1 << 14)
/*! Raw 16-bit Signed Linear (16000 Hz) PCM */
#define TRIS_FORMAT_SLINEAR16	(1 << 15)
/*! Maximum audio mask */
#define TRIS_FORMAT_AUDIO_MASK   ((1 << 16)-1)
/*! JPEG Images */
#define TRIS_FORMAT_JPEG		(1 << 16)
/*! PNG Images */
#define TRIS_FORMAT_PNG		(1 << 17)
/*! H.261 Video */
#define TRIS_FORMAT_H261		(1 << 18)
/*! H.263 Video */
#define TRIS_FORMAT_H263		(1 << 19)
/*! H.263+ Video */
#define TRIS_FORMAT_H263_PLUS	(1 << 20)
/*! H.264 Video */
#define TRIS_FORMAT_H264		(1 << 21)
/*! MPEG4 Video */
#define TRIS_FORMAT_MP4_VIDEO	(1 << 22)
#define TRIS_FORMAT_VIDEO_MASK   (((1 << 24)-1) & ~(TRIS_FORMAT_AUDIO_MASK))
/*! T.140 RED Text format RFC 4103 */
#define TRIS_FORMAT_T140RED      (1 << 25)
/*! T.140 Text format - ITU T.140, RFC 4103 */
#define TRIS_FORMAT_T140		(1 << 26)
/*! Maximum text mask */
#define TRIS_FORMAT_MAX_TEXT	(1 << 27)
#define TRIS_FORMAT_TEXT_MASK   (((1 << 28)-1) & ~(TRIS_FORMAT_AUDIO_MASK) & ~(TRIS_FORMAT_VIDEO_MASK))
#define TRIS_FORMAT_FILE	(1 << 28)
#define TRIS_FORMAT_DESKTOP (1 << 29)
#define TRIS_FORMAT_CHAT (1 << 30)
/*! SpeeX Wideband (16kHz) Free Compression */
#define TRIS_FORMAT_SIREN7    (1 << 31)

enum tris_control_frame_type {
	TRIS_CONTROL_HANGUP = 1,		/*!< Other end has hungup */
	TRIS_CONTROL_RING = 2,		/*!< Local ring */
	TRIS_CONTROL_RINGING = 3,	/*!< Remote end is ringing */
	TRIS_CONTROL_ANSWER = 4,		/*!< Remote end has answered */
	TRIS_CONTROL_BUSY = 5,		/*!< Remote end is busy */
	TRIS_CONTROL_TAKEOFFHOOK = 6,	/*!< Make it go off hook */
	TRIS_CONTROL_OFFHOOK = 7,	/*!< Line is off hook */
	TRIS_CONTROL_CONGESTION = 8,	/*!< Congestion (circuits busy) */
	TRIS_CONTROL_FLASH = 9,		/*!< Flash hook */
	TRIS_CONTROL_WINK = 10,		/*!< Wink */
	TRIS_CONTROL_OPTION = 11,	/*!< Set a low-level option */
	TRIS_CONTROL_RADIO_KEY = 12,	/*!< Key Radio */
	TRIS_CONTROL_RADIO_UNKEY = 13,	/*!< Un-Key Radio */
	TRIS_CONTROL_PROGRESS = 14,	/*!< Indicate PROGRESS */
	TRIS_CONTROL_PROCEEDING = 15,	/*!< Indicate CALL PROCEEDING */
	TRIS_CONTROL_HOLD = 16,		/*!< Indicate call is placed on hold */
	TRIS_CONTROL_UNHOLD = 17,	/*!< Indicate call is left from hold */
	TRIS_CONTROL_VIDUPDATE = 18,	/*!< Indicate video frame update */
	_XXX_TRIS_CONTROL_T38 = 19,	/*!< T38 state change request/notification \deprecated This is no longer supported. Use TRIS_CONTROL_T38_PARAMETERS instead. */
	TRIS_CONTROL_SRCUPDATE = 20,     /*!< Indicate source of media has changed */
	TRIS_CONTROL_T38_PARAMETERS = 24, /*!< T38 state change request/notification with parameters */
	TRIS_CONTROL_TIMEOUT = 21,
	TRIS_CONTROL_FORBIDDEN = 22,
	TRIS_CONTROL_ROUTEFAIL = 23,
	TRIS_CONTROL_REJECTED = 25,
	TRIS_CONTROL_UNAVAILABLE = 26,
	TRIS_CONTROL_NOTIFY_PROCEEDING = 27, /* 100 */
	TRIS_CONTROL_NOTIFY_RINGING = 28, /* 180 */
	TRIS_CONTROL_NOTIFY_PROGRESS = 29, /* 183 */
	TRIS_CONTROL_NOTIFY_ANSWER = 30, /* 200 */
	TRIS_CONTROL_NOTIFY_OFFHOOK = 31, /* 404 */
	TRIS_CONTROL_NOTIFY_TAKEOFFHOOK = 32, /* 480 */
	TRIS_CONTROL_NOTIFY_BUSY = 33, /* 486 */
	TRIS_CONTROL_NOTIFY_TIMEOUT = 34, /* 408 */
	TRIS_CONTROL_NOTIFY_FORBIDDEN = 35, /* 403 */
	TRIS_CONTROL_NOTIFY_NOMONEY = 36, /* 402 */
	TRIS_CONTROL_NOTIFY_OUTLINE = 37, /* 410 */
	TRIS_CONTROL_NOTIFY_CIRCUITS = 38, /* 502 */
	TRIS_CONTROL_NOTIFY_ACCEPT = 39,
	TRIS_CONTROL_NOTIFY_CANCEL = 40,
	TRIS_CONTROL_NOTIFY_CONNECT = 41,
	TRIS_CONTROL_NOTIFY_BYE = 42,
	TRIS_CONTROL_NOTIFY_ANNOUNCE = 43,
	TRIS_CONTROL_NOTIFY_CALLERBYE = 44, /* CallerBye */
	TRIS_CONTROL_NOTIFY_CALLEEBYE = 45, /* CalleeBye */
	TRIS_CONTROL_REFER = 46,	/* Received Refer */
	TRIS_CONTROL_REFER_INFO = 47,	/* Received Info derived from Refer*/
	TRIS_CONTROL_NOTIFY_NEWMEMBER = 50, /* Notify entrance of new member */
	TRIS_CONTROL_SRCCHANGE = 51  /*!< Media source has changed and requires a new RTP SSRC */
};

enum tris_control_t38 {
	TRIS_T38_REQUEST_NEGOTIATE = 1,	/*!< Request T38 on a channel (voice to fax) */
	TRIS_T38_REQUEST_TERMINATE,	/*!< Terminate T38 on a channel (fax to voice) */
	TRIS_T38_NEGOTIATED,		/*!< T38 negotiated (fax mode) */
	TRIS_T38_TERMINATED,		/*!< T38 terminated (back to voice) */
	TRIS_T38_REFUSED			/*!< T38 refused for some reason (usually rejected by remote end) */
};

enum tris_control_t38_rate {
	TRIS_T38_RATE_2400 = 0,
	TRIS_T38_RATE_4800,
	TRIS_T38_RATE_7200,
	TRIS_T38_RATE_9600,
	TRIS_T38_RATE_12000,
	TRIS_T38_RATE_14400,
};

enum tris_control_t38_rate_management {
	TRIS_T38_RATE_MANAGEMENT_TRANSFERRED_TCF = 0,
	TRIS_T38_RATE_MANAGEMENT_LOCAL_TCF,
};

struct tris_control_t38_parameters {
	enum tris_control_t38 request_response;			/*!< Request or response of the T38 control frame */
	unsigned int version;					/*!< Supported T.38 version */
	unsigned int max_ifp; 					/*!< Maximum IFP size supported */
	enum tris_control_t38_rate rate;				/*!< Maximum fax rate supported */
	enum tris_control_t38_rate_management rate_management;	/*!< Rate management setting */
	unsigned int fill_bit_removal:1;			/*!< Set if fill bit removal can be used */
	unsigned int transcoding_mmr:1;				/*!< Set if MMR transcoding can be used */
	unsigned int transcoding_jbig:1;			/*!< Set if JBIG transcoding can be used */
};

#define TRIS_SMOOTHER_FLAG_G729		(1 << 0)
#define TRIS_SMOOTHER_FLAG_BE		(1 << 1)

/* Option identifiers and flags */
#define TRIS_OPTION_FLAG_REQUEST		0
#define TRIS_OPTION_FLAG_ACCEPT		1
#define TRIS_OPTION_FLAG_REJECT		2
#define TRIS_OPTION_FLAG_QUERY		4
#define TRIS_OPTION_FLAG_ANSWER		5
#define TRIS_OPTION_FLAG_WTF		6

/*! Verify touchtones by muting audio transmission 
	(and reception) and verify the tone is still present */
#define TRIS_OPTION_TONE_VERIFY		1		

/*! Put a compatible channel into TDD (TTY for the hearing-impared) mode */
#define	TRIS_OPTION_TDD			2

/*! Relax the parameters for DTMF reception (mainly for radio use) */
#define	TRIS_OPTION_RELAXDTMF		3

/*! Set (or clear) Audio (Not-Clear) Mode */
#define	TRIS_OPTION_AUDIO_MODE		4

/*! Set channel transmit gain 
 * Option data is a single signed char
   representing number of decibels (dB)
   to set gain to (on top of any gain
   specified in channel driver)
*/
#define TRIS_OPTION_TXGAIN		5

/*! Set channel receive gain
 * Option data is a single signed char
   representing number of decibels (dB)
   to set gain to (on top of any gain
   specified in channel driver)
*/
#define TRIS_OPTION_RXGAIN		6

/* set channel into "Operator Services" mode */
#define	TRIS_OPTION_OPRMODE		7

/*! Explicitly enable or disable echo cancelation for the given channel */
#define	TRIS_OPTION_ECHOCAN		8

/* !
 * Read-only. Allows query current status of T38 on the channel.
 * data: tris_t38state
 */
#define TRIS_OPTION_T38_STATE		10

struct oprmode {
	struct tris_channel *peer;
	int mode;
} ;

struct tris_option_header {
	/* Always keep in network byte order */
#if __BYTE_ORDER == __BIG_ENDIAN
        uint16_t flag:3;
        uint16_t option:13;
#else
#if __BYTE_ORDER == __LITTLE_ENDIAN
        uint16_t option:13;
        uint16_t flag:3;
#else
#error Byte order not defined
#endif
#endif
		uint8_t data[0];
};


/*! \brief Definition of supported media formats (codecs) */
struct tris_format_list {
	int bits;	/*!< bitmask value */
	char *name;	/*!< short name */
	int samplespersecond; /*!< Number of samples per second (8000/16000) */
	char *desc;	/*!< Description */
	int fr_len;	/*!< Single frame length in bytes */
	int min_ms;	/*!< Min value */
	int max_ms;	/*!< Max value */
	int inc_ms;	/*!< Increment */
	int def_ms;	/*!< Default value */
	unsigned int flags;	/*!< Smoother flags */
	int cur_ms;	/*!< Current value */
};


/*! \brief  Requests a frame to be allocated 
 * 
 * \param source 
 * Request a frame be allocated.  source is an optional source of the frame, 
 * len is the requested length, or "0" if the caller will supply the buffer 
 */
#if 0 /* Unimplemented */
struct tris_frame *tris_fralloc(char *source, int len);
#endif

/*!  
 * \brief Frees a frame or list of frames
 * 
 * \param fr Frame to free, or head of list to free
 * \param cache Whether to consider this frame for frame caching
 */
void tris_frame_free(struct tris_frame *fr, int cache);

#define tris_frfree(fr) tris_frame_free(fr, 1)

/*! \brief Makes a frame independent of any static storage
 * \param fr frame to act upon
 * Take a frame, and if it's not been malloc'd, make a malloc'd copy
 * and if the data hasn't been malloced then make the
 * data malloc'd.  If you need to store frames, say for queueing, then
 * you should call this function.
 * \return Returns a frame on success, NULL on error
 * \note This function may modify the frame passed to it, so you must
 * not assume the frame will be intact after the isolated frame has
 * been produced. In other words, calling this function on a frame
 * should be the last operation you do with that frame before freeing
 * it (or exiting the block, if the frame is on the stack.)
 */
struct tris_frame *tris_frisolate(struct tris_frame *fr);

/*! \brief Copies a frame 
 * \param fr frame to copy
 * Duplicates a frame -- should only rarely be used, typically frisolate is good enough
 * \return Returns a frame on success, NULL on error
 */
struct tris_frame *tris_frdup(const struct tris_frame *fr);

void tris_swapcopy_samples(void *dst, const void *src, int samples);

/* Helpers for byteswapping native samples to/from 
   little-endian and big-endian. */
#if __BYTE_ORDER == __LITTLE_ENDIAN
#define tris_frame_byteswap_le(fr) do { ; } while(0)
#define tris_frame_byteswap_be(fr) do { struct tris_frame *__f = (fr); tris_swapcopy_samples(__f->data.ptr, __f->data.ptr, __f->samples); } while(0)
#else
#define tris_frame_byteswap_le(fr) do { struct tris_frame *__f = (fr); tris_swapcopy_samples(__f->data.ptr, __f->data.ptr, __f->samples); } while(0)
#define tris_frame_byteswap_be(fr) do { ; } while(0)
#endif


/*! \brief Get the name of a format
 * \param format id of format
 * \return A static string containing the name of the format or "unknown" if unknown.
 */
char* tris_getformatname(int format);

/*! \brief Get the names of a set of formats
 * \param buf a buffer for the output string
 * \param size size of buf (bytes)
 * \param format the format (combined IDs of codecs)
 * Prints a list of readable codec names corresponding to "format".
 * ex: for format=TRIS_FORMAT_GSM|TRIS_FORMAT_SPEEX|TRIS_FORMAT_ILBC it will return "0x602 (GSM|SPEEX|ILBC)"
 * \return The return value is buf.
 */
char* tris_getformatname_multiple(char *buf, size_t size, int format);

/*!
 * \brief Gets a format from a name.
 * \param name string of format
 * \return This returns the form of the format in binary on success, 0 on error.
 */
int tris_getformatbyname(const char *name);

/*! \brief Get a name from a format 
 * Gets a name from a format
 * \param codec codec number (1,2,4,8,16,etc.)
 * \return This returns a static string identifying the format on success, 0 on error.
 */
char *tris_codec2str(int codec);

/*! \name TRIS_Smoother 
*/
/*@{ */
/*! \page tris_smooth The AST Frame Smoother
The tris_smoother interface was designed specifically
to take frames of variant sizes and produce frames of a single expected
size, precisely what you want to do.

The basic interface is:

- Initialize with tris_smoother_new()
- Queue input frames with tris_smoother_feed()
- Get output frames with tris_smoother_read()
- when you're done, free the structure with tris_smoother_free()
- Also see tris_smoother_test_flag(), tris_smoother_set_flags(), tris_smoother_get_flags(), tris_smoother_reset()
*/
struct tris_smoother;

struct tris_smoother *tris_smoother_new(int bytes);
void tris_smoother_set_flags(struct tris_smoother *smoother, int flags);
int tris_smoother_get_flags(struct tris_smoother *smoother);
int tris_smoother_test_flag(struct tris_smoother *s, int flag);
void tris_smoother_free(struct tris_smoother *s);
void tris_smoother_reset(struct tris_smoother *s, int bytes);

/*!
 * \brief Reconfigure an existing smoother to output a different number of bytes per frame
 * \param s the smoother to reconfigure
 * \param bytes the desired number of bytes per output frame
 * \return nothing
 *
 */
void tris_smoother_reconfigure(struct tris_smoother *s, int bytes);

int __tris_smoother_feed(struct tris_smoother *s, struct tris_frame *f, int swap);
struct tris_frame *tris_smoother_read(struct tris_smoother *s);
#define tris_smoother_feed(s,f) __tris_smoother_feed(s, f, 0)
#if __BYTE_ORDER == __LITTLE_ENDIAN
#define tris_smoother_feed_be(s,f) __tris_smoother_feed(s, f, 1)
#define tris_smoother_feed_le(s,f) __tris_smoother_feed(s, f, 0)
#else
#define tris_smoother_feed_be(s,f) __tris_smoother_feed(s, f, 0)
#define tris_smoother_feed_le(s,f) __tris_smoother_feed(s, f, 1)
#endif
/*@} Doxygen marker */

const struct tris_format_list *tris_get_format_list_index(int index);
const struct tris_format_list *tris_get_format_list(size_t *size);
void tris_frame_dump(const char *name, struct tris_frame *f, char *prefix);

/*! \page AudioCodecPref Audio Codec Preferences

	In order to negotiate audio codecs in the order they are configured
	in \<channel\>.conf for a device, we set up codec preference lists
	in addition to the codec capabilities setting. The capabilities
	setting is a bitmask of audio and video codecs with no internal
	order. This will reflect the offer given to the other side, where
	the prefered codecs will be added to the top of the list in the
	order indicated by the "allow" lines in the device configuration.
	
	Video codecs are not included in the preference lists since they
	can't be transcoded and we just have to pick whatever is supported
*/

/*! 
 *\brief Initialize an audio codec preference to "no preference".
 * \arg \ref AudioCodecPref 
*/
void tris_codec_pref_init(struct tris_codec_pref *pref);

/*! 
 * \brief Codec located at a particular place in the preference index.
 * \arg \ref AudioCodecPref 
*/
int tris_codec_pref_index(struct tris_codec_pref *pref, int index);

/*! \brief Remove audio a codec from a preference list */
void tris_codec_pref_remove(struct tris_codec_pref *pref, int format);

/*! \brief Append a audio codec to a preference list, removing it first if it was already there 
*/
int tris_codec_pref_append(struct tris_codec_pref *pref, int format);

/*! \brief Prepend an audio codec to a preference list, removing it first if it was already there 
*/
void tris_codec_pref_prepend(struct tris_codec_pref *pref, int format, int only_if_existing);

/*! \brief Select the best audio format according to preference list from supplied options. 
   If "find_best" is non-zero then if nothing is found, the "Best" format of 
   the format list is selected, otherwise 0 is returned. */
int tris_codec_choose(struct tris_codec_pref *pref, int formats, int find_best);

/*! \brief Set packet size for codec
*/
int tris_codec_pref_setsize(struct tris_codec_pref *pref, int format, int framems);

/*! \brief Get packet size for codec
*/
struct tris_format_list tris_codec_pref_getsize(struct tris_codec_pref *pref, int format);

/*! \brief Parse an "allow" or "deny" line in a channel or device configuration 
        and update the capabilities mask and pref if provided.
	Video codecs are not added to codec preference lists, since we can not transcode
	\return Returns number of errors encountered during parsing
 */
int tris_parse_allow_disallow(struct tris_codec_pref *pref, int *mask, const char *list, int allowing);

/*! \brief Dump audio codec preference list into a string */
int tris_codec_pref_string(struct tris_codec_pref *pref, char *buf, size_t size);

/*! \brief Shift an audio codec preference list up or down 65 bytes so that it becomes an ASCII string */
void tris_codec_pref_convert(struct tris_codec_pref *pref, char *buf, size_t size, int right);

/*! \brief Returns the number of samples contained in the frame */
int tris_codec_get_samples(struct tris_frame *f);

/*! \brief Returns the number of bytes for the number of samples of the given format */
int tris_codec_get_len(int format, int samples);

/*! \brief Appends a frame to the end of a list of frames, truncating the maximum length of the list */
struct tris_frame *tris_frame_enqueue(struct tris_frame *head, struct tris_frame *f, int maxlen, int dupe);


/*! \brief Gets duration in ms of interpolation frame for a format */
static inline int tris_codec_interp_len(int format) 
{ 
	return (format == TRIS_FORMAT_ILBC) ? 30 : 20;
}

/*!
  \brief Adjusts the volume of the audio samples contained in a frame.
  \param f The frame containing the samples (must be TRIS_FRAME_VOICE and TRIS_FORMAT_SLINEAR)
  \param adjustment The number of dB to adjust up or down.
  \return 0 for success, non-zero for an error
 */
int tris_frame_adjust_volume(struct tris_frame *f, int adjustment);

/*!
  \brief Sums two frames of audio samples.
  \param f1 The first frame (which will contain the result)
  \param f2 The second frame
  \return 0 for success, non-zero for an error

  The frames must be TRIS_FRAME_VOICE and must contain TRIS_FORMAT_SLINEAR samples,
  and must contain the same number of samples.
 */
int tris_frame_slinear_sum(struct tris_frame *f1, struct tris_frame *f2);

/*!
 * \brief Get the sample rate for a given format.
 */
static force_inline int tris_format_rate(int format)
{
	switch (format) {
	case TRIS_FORMAT_G722:
	case TRIS_FORMAT_SLINEAR16:
	case TRIS_FORMAT_SIREN7:
	case TRIS_FORMAT_SPEEX16:
		return 16000;
	case TRIS_FORMAT_SIREN14:
		return 32000;
	default:
		return 8000;
	}
}

#if defined(__cplusplus) || defined(c_plusplus)
}
#endif

#endif /* _TRISMEDIA_FRAME_H */
