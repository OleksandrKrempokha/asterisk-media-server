 ...
 */
#ifndef _TRISMEDIA_HASHTAB_H_
#define _TRISMEDIA_HASHTAB_H_
 ...
 * \ref AstHash
 */
/*! \page AstHash Hash Table support in Trismedia
...
struct tris_hashtab_bucket
{
	const void *object;                    /*!< whatever it is we are storing in this table */
	struct tris_hashtab_bucket *next;       /*!< a DLL of buckets in hash collision */
	struct tris_hashtab_bucket *prev;       /*!< a DLL of buckets in hash collision */
	struct tris_hashtab_bucket *tnext;      /*!< a DLL of all the hash buckets for traversal */
	struct tris_hashtab_bucket *tprev;      /*!< a DLL of all the hash buckets for traversal */
};

struct tris_hashtab
{
	struct tris_hashtab_bucket **array;
	struct tris_hashtab_bucket *tlist;		/*!< the head of a DLList of all the hashbuckets in the table (for traversal). */
	...
	int (*newsize) (struct tris_hashtab *tab);	/*!< a ptr to func that returns int, a new size for hash tab, based on curr_size */
	int (*resize) (struct tris_hashtab *tab);	/*!< a function to decide whether this hashtable should be resized now */
	...
	tris_rwlock_t lock;                                /* is this as good as it sounds? */
};

/*! \brief an iterator for traversing the buckets */
struct tris_hashtab_iter
{
	struct tris_hashtab *tab;
	struct tris_hashtab_bucket *next;
};
 ...
 */
int tris_hashtab_resize_none(struct tris_hashtab *tab);
...
int tris_hashtab_newsize_none(struct tris_hashtab *tab);
 ...
 * \sa tris_hashtable_hash_string_nocase
 * \sa tris_hashtab_hash_string_sax
 ...
 * \sa tris_hastable_hash_string
 * \sa tris_hastable_hash_string_nocase
 */
unsigned int tris_hashtab_hash_string_sax(const void *obj);


unsigned int tris_hashtab_hash_int(const int num);  /* right now, both these funcs are just result = num%modulus; */


unsigned int tris_hashtab_hash_short(const short num);
...
struct tris_hashtab * _tris_hashtab_create(int initial_buckets,
...
#define tris_hashtab_create(a,b,c,d,e,f)	_tris_hashtab_create(a,b,c,d,e,f,__FILE__,__LINE__,__PRETTY_FUNCTION__)
#else
struct tris_hashtab * tris_hashtab_create(int initial_buckets,
					int (*compare)(const void *a, const void *b),
					int (*resize)(struct tris_hashtab *),
					int (*newsize)(struct tris_hashtab *tab),
					unsigned int (*hash)(const void *obj),
...
#define	tris_hashtab_insert_immediate(a,b)	_tris_hashtab_insert_immediate(a, b, __FILE__, __LINE__, __PRETTY_FUNCTION__)
#else
...
#define	tris_hashtab_insert_immediate_bucket(a,b,c)	_tris_hashtab_insert_immediate_bucket(a, b, c, __FILE__, __LINE__, __PRETTY_FUNCTION__)
#else
...
#define	tris_hashtab_insert_safe(a,b)	_tris_hashtab_insert_safe(a,b,__FILE__, __LINE__, __PRETTY_FUNCTION__)
#else
...
/*!
 * \brief Similar to tris_hashtab_lookup but sets h to the key hash value if the lookup fails.
 * \note This has the modulus applied, and will not be useful for long term storage if the table is resizable.
*/
void * tris_hashtab_lookup_bucket(struct tris_hashtab *tab, const void *obj, unsigned int *h);
...
#define	tris_hashtab_dup(a,b)	_tris_hashtab_dup(a,b,__FILE__,__LINE__,__PRETTY_FUNCTION__)
#else
...
#define	tris_hashtab_start_traversal(a)	_tris_hashtab_start_traversal(a,__FILE__,__LINE__,__PRETTY_FUNCTION__)
#else
...
#define	tris_hashtab_start_write_traversal(a)	_tris_hashtab_start_write_traversal(a,__FILE__,__LINE__,__PRETTY_FUNCTION__)
#else
