...
#ifndef _TRISMEDIA_LOCALTIME_H
#define _TRISMEDIA_LOCALTIME_H
 ...
 */
struct tris_tm *tris_localtime(const struct timeval *timep, struct tris_tm *p_tm, const char *zone);
 ...
 */
struct timeval tris_mktime(struct tris_tm * const tmp, const char *zone);
 ...
 */
int tris_strftime(char *buf, size_t len, const char *format, const struct tris_tm *tm);

/*!\brief Special version of strptime(3) which places the answer in the common
 * structure tris_tm.  Also, unlike strptime(3), tris_strptime() initializes its
 * memory prior to use.
