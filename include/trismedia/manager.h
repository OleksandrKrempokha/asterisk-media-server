...
#ifndef _TRISMEDIA_MANAGER_H
#define _TRISMEDIA_MANAGER_H
...
/*!
 \file
 \brief The AMI - Trismedia Manager Interface - is a TCP protocol created to
 manage Trismedia with third-party software.
...
#define DEFAULT_MANAGER_PORT 5038	/* Default port for Trismedia management via TCP */
...
#define TRIS_MAX_MANHEADERS 128
...
struct manager_custom_hook {
	...
	TRIS_RWLIST_ENTRY(manager_custom_hook) list;
};
...
struct message {
	...
	const char *headers[TRIS_MAX_MANHEADERS];
};
...
struct manager_action {
	...
	/*! For easy linking */
	TRIS_RWLIST_ENTRY(manager_action) list;
};

/*! \brief External routines may register/unregister manager callbacks this way 
 * \note  Use tris_manager_register2() to register with help text for new manager commands */
#define tris_manager_register(a, b, c, d) tris_manager_register2(a, b, c, d, NULL)
...
/*! \brief External routines may send trismedia manager events this way 
...
/*! \brief Called by Trismedia initialization */
int init_manager(void);

/*! \brief Called by Trismedia module functions and the CLI command */
