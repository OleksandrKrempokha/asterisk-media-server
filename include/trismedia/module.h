...
/*! \file
 * \brief Trismedia module definitions.
 *
 * This file contains the definitons for functions Trismedia modules should
 * provide and some other module related functions.
 */

#ifndef _TRISMEDIA_MODULE_H
#define _TRISMEDIA_MODULE_H
...
logos (including \"Trismedia\" or \"Digium\") without \
express written permission of Digium, Inc. is prohibited.\n"

#define TRIS_MODULE_CONFIG "modules.conf" /*!< \brief Module configuration file */

enum tris_module_unload_mode {
	TRIS_FORCE_SOFT = 0, /*!< Softly unload a module, only if not in use */
	TRIS_FORCE_FIRM = 1, /*!< Firmly unload a module, even if in use */
	TRIS_FORCE_HARD = 2, /*!< as FIRM, plus dlclose() on the module. Not recommended
				as it may cause crashes */
};

enum tris_module_load_result {
	TRIS_MODULE_LOAD_SUCCESS = 0,    /*!< Module loaded and configured */
	TRIS_MODULE_LOAD_DECLINE = 1,    /*!< Module is not configured */
	TRIS_MODULE_LOAD_SKIP = 2,       /*!< Module was skipped for some reason */
	TRIS_MODULE_LOAD_PRIORITY = 3,   /*!< Module is not loaded yet, but is added to prioity heap */
	TRIS_MODULE_LOAD_FAILURE = -1,   /*!< Module could not be loaded properly */
};
 ...
 *
 * \return See possible enum values for tris_module_load_result.
 */
enum tris_module_load_result tris_load_resource(const char *resource_name);

/*! 
 * \brief Unload a module.
 * \param resource_name The name of the module to unload.
 * \param tris_module_unload_mode The force flag. This should be set using one of the TRIS_FORCE flags.
 *
 * This function unloads a module.  It will only unload modules that are not in
 * use (usecount not zero), unless #TRIS_FORCE_FIRM or #TRIS_FORCE_HARD is 
 * specified.  Setting #TRIS_FORCE_FIRM or #TRIS_FORCE_HARD will unload the
 * module regardless of consequences (NOT RECOMMENDED).
 *
 * \retval 0 on success.
 * \retval -1 on error.
 */
int tris_unload_resource(const char *resource_name, enum tris_module_unload_mode);
 ...
 *
 * \note The tris_module_user_* functions take care of calling this function for you.
 */
 ...
 * \retval -1 on failure.
 */
int tris_loader_register(int (*updater)(void));
 ...
 * \retval -1 on failure.
 */
int tris_loader_unregister(int (*updater)(void));

/*!
 * \brief Run the unload() callback for all loaded modules
 *
 * This function should be called when Trismedia is shutting down gracefully.
 */
void tris_module_shutdown(void);

/*! 
 * \brief Match modules names for the Trismedia cli.
...
struct tris_module_user_list;

/*! \page ModMngmnt The Trismedia Module management interface
 *
 * All modules must implement the module API (load, unload...)
 */

enum tris_module_flags {
	TRIS_MODFLAG_DEFAULT = 0,
	TRIS_MODFLAG_GLOBAL_SYMBOLS = (1 << 0),
	TRIS_MODFLAG_LOAD_ORDER = (1 << 1),
};

struct tris_module_info {
	...
	struct tris_module *self;
	enum tris_module_load_result (*load)(void);	/*!< register stuff etc. Optional. */
	...
	/*! 
	 * This holds the TRISMEDIA_GPL_KEY, signifiying that you agree to the terms of
	 * the Trismedia license as stated in the TRISMEDIA_GPL_KEY.  Your module will not
	...
	/*! The value of TRIS_BUILDOPT_SUM when this module was compiled */
	 ...
	 *  TRIS_MODFLAG_LOAD_ORDER flag is set.  If the TRIS_MODFLAG_LOAD_ORDER flag is not set,
	 *  this value will never be read and the module will be given the lowest possible priority
	 *  on load. */
	unsigned char load_pri;
};

void tris_module_register(const struct tris_module_info *);
void tris_module_unregister(const struct tris_module_info *);

struct tris_module_user *__tris_module_user_add(struct tris_module *, struct tris_channel *);
void __tris_module_user_remove(struct tris_module *, struct tris_module_user *);
void __tris_module_user_hangup_all(struct tris_module *);

#define tris_module_user_add(chan) __tris_module_user_add(tris_module_info->self, chan)
#define tris_module_user_remove(user) __tris_module_user_remove(tris_module_info->self, user)
#define tris_module_user_hangup_all() __tris_module_user_hangup_all(tris_module_info->self)

struct tris_module *tris_module_ref(struct tris_module *);
void tris_module_unref(struct tris_module *);

#if defined(__cplusplus) || defined(c_plusplus)
#define TRIS_MODULE_INFO(keystr, flags_to_set, desc, load_func, unload_func, reload_func)	\
	static struct tris_module_info __mod_info = {	\
		NULL,					\
		load_func,				\
		reload_func,				\
		unload_func,				\
	...
	{ \
		tris_module_register(&__mod_info); \
	} \
	static void  __attribute__((destructor)) __unreg_module(void) \
	{ \
		tris_module_unregister(&__mod_info); \
	} \
	static const __attribute__((unused)) struct tris_module_info *tris_module_info = &__mod_info

#define TRIS_MODULE_INFO_STANDARD(keystr, desc)		\
	TRIS_MODULE_INFO(keystr, TRIS_MODFLAG_DEFAULT, desc,	\
			load_module,			\
			unload_module,		\
			NULL			\
		       )
...
static const __attribute__((unused)) struct tris_module_info *tris_module_info;
...
static int __backup_globals(void)
{
	...
	if (!(__global_backup = tris_malloc(data_size)))
		return -1;
...
#define TRIS_MODULE_INFO(keystr, flags_to_set, desc, fields...)	\
	static struct tris_module_info 				\
		...
		.description = desc,				\
		.key = keystr,					\
		.buildopt_sum = TRIS_BUILDOPT_SUM,		\
		fields						\
	};							\
	static void  __attribute__((constructor)) __reg_module(void) \
	{ \
	...
	static const struct tris_module_info *tris_module_info = &__mod_info
 ...
 *                    the application
 * 
 * This registers an application with Trismedia's internal application list. 
 ...
 * \retval -1 failure.
 */
#define tris_register_application(app, execute, synopsis, description) tris_register_application2(app, execute, synopsis, description, tris_module_info->self)
 ...
 * \retval -1 failure.
 */
#define tris_register_application_xml(app, execute) tris_register_application(app, execute, NULL, NULL)
 ...
 * \retval -1 failure.
 */
int tris_register_application2(const char *app, int (*execute)(struct tris_channel *, void *),
 ...
 * 
 * This unregisters an application from Trismedia's internal application list.
 * 
 * \retval 0 success 
 * \retval -1 failure
 */
