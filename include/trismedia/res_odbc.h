...
#ifndef _TRISMEDIA_RES_ODBC_H
#define _TRISMEDIA_RES_ODBC_H
...
/*! \brief Flags for use with \see tris_odbc_request_obj2 */
enum {
...
struct odbc_cache_columns {
	...
	SQLINTEGER octetlen;
	TRIS_RWLIST_ENTRY(odbc_cache_columns) list;
};

struct odbc_cache_tables {
	char *connection;
	char *table;
	TRIS_RWLIST_HEAD(_columns, odbc_cache_columns) columns;
	TRIS_RWLIST_ENTRY(odbc_cache_tables) list;
};
 ...
 */
int tris_odbc_smart_execute(struct odbc_obj *obj, SQLHSTMT stmt) __attribute__((deprecated));
...
#define tris_odbc_request_obj2(a, b)	_tris_odbc_request_obj2(a, b, __FILE__, __PRETTY_FUNCTION__, __LINE__)
#else
...
#define tris_odbc_request_obj(a, b)	_tris_odbc_request_obj(a, b, __FILE__, __PRETTY_FUNCTION__, __LINE__)
#else
 ...
 * to \see tris_odbc_request_obj2 (or formerly, to tris_odbc_request_obj).  Note that the
 ...
 * \param table Cached table structure, as returned from tris_odbc_find_table()
...
/*!
 * \brief Release a table returned from tris_odbc_find_table
 */
#define tris_odbc_release_table(ptr) if (ptr) { TRIS_RWLIST_UNLOCK(&(ptr)->columns); }

/*!\brief Wrapper for SQLGetData to use with dynamic strings
 * \param buf Address of the pointer to the tris_str structure.
