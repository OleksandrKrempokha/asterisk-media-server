...
#ifndef _TRISMEDIA_SCHED_H
#define _TRISMEDIA_SCHED_H
 ...
 */
#define TRIS_SCHED_DEL(sched, id) \
	({ \
		int _count = 0; \
		int _sched_res = -1; \
		while (id > -1 && (_sched_res = tris_sched_del(sched, id)) && ++_count < 10) \
			usleep(1); \
		if (_count == 10 && option_debug > 2) { \
			tris_log(LOG_DEBUG, "Unable to cancel schedule ID %d.\n", id); \
		} \
		id = -1; \
		(_sched_res); \
	})
 ...
 * \since 1.6.1
 */
#define TRIS_SCHED_DEL_UNREF(sched, id, refcall)			\
	do { \
		int _count = 0; \
		while (id > -1 && tris_sched_del(sched, id) && ++_count < 10) { \
			usleep(1); \
		} \
		if (_count == 10) \
			tris_log(LOG_WARNING, "Unable to cancel schedule ID %d.  This is probably a bug (%s: %s, line %d).\n", id, __FILE__, __PRETTY_FUNCTION__, __LINE__); \
		if (id > -1) \
			refcall; \
		id = -1; \
	} while (0);
 ...
 * \since 1.6.1
 */
#define TRIS_SCHED_DEL_SPINLOCK(sched, id, lock) \
	({ \
		int _count = 0; \
		int _sched_res = -1; \
		while (id > -1 && (_sched_res = tris_sched_del(sched, id)) && ++_count < 10) { \
...
#define TRIS_SCHED_REPLACE_VARIABLE(id, sched, when, callback, data, variable) \
	do { \
		int _count = 0; \
		while (id > -1 && tris_sched_del(sched, id) && ++_count < 10) { \
			usleep(1); \
		...
		if (_count == 10) \
			tris_log(LOG_WARNING, "Unable to cancel schedule ID %d.  This is probably a bug (%s: %s, line %d).\n", id, __FILE__, __PRETTY_FUNCTION__, __LINE__); \
		id = tris_sched_add_variable(sched, when, callback, data, variable); \
	} while (0);

#define TRIS_SCHED_REPLACE(id, sched, when, callback, data) \
		TRIS_SCHED_REPLACE_VARIABLE(id, sched, when, callback, data, 0)

/*!
 * \note Not currently used in the source?
 * \since 1.6.1
 */
#define TRIS_SCHED_REPLACE_VARIABLE_UNREF(id, sched, when, callback, data, variable, unrefcall, addfailcall, refcall) \
	do { \
		int _count = 0, _res=1;											 \
		void *_data = (void *)tris_sched_find_data(sched, id);			\
		while (id > -1 && (_res = tris_sched_del(sched, id) && _count++ < 10)) { \
			usleep(1); \
		} \
		if (!_res && _data)							\
...
#define TRIS_SCHED_REPLACE_UNREF(id, sched, when, callback, data, unrefcall, addfailcall, refcall) \
	TRIS_SCHED_REPLACE_VARIABLE_UNREF(id, sched, when, callback, data, 0, unrefcall, addfailcall, refcall)
 ...
 */
typedef int (*tris_sched_cb)(const void *data);
#define TRIS_SCHED_CB(a) ((tris_sched_cb)(a))

struct tris_cb_names {
	int numassocs;
	char *list[10];
	tris_sched_cb cblist[10];
};
 ...
 */
int tris_sched_add(struct sched_context *con, int when, tris_sched_cb callback, const void *data) attribute_warn_unused_result;

/*!
 * \brief replace a scheduler entry
 * \deprecated You should use the TRIS_SCHED_REPLACE() macro instead.
 *
 * This deletes the scheduler entry for old_id if it exists, and then
 * calls tris_sched_add to create a new entry.  A negative old_id will
 * be ignored.
 *
 * \retval -1 failure
 * \retval otherwise, returns scheduled item ID
 */
int tris_sched_replace(int old_id, struct sched_context *con, int when, tris_sched_cb callback, const void *data) attribute_warn_unused_result;
 ...
 * \return Returns a schedule item ID on success, -1 on failure
 */
int tris_sched_add_variable(struct sched_context *con, int when, tris_sched_cb callback, const void *data, int variable) attribute_warn_unused_result;

/*!
 * \brief replace a scheduler entry
 * \deprecated You should use the TRIS_SCHED_REPLACE_VARIABLE() macro instead.
 *
 * This deletes the scheduler entry for old_id if it exists, and then
 * calls tris_sched_add to create a new entry.  A negative old_id will
 * be ignored.
 ...
 * \retval otherwise, returns scheduled item ID
 */
int tris_sched_replace_variable(int old_id, struct sched_context *con, int when, tris_sched_cb callback, const void *data, int variable) attribute_warn_unused_result;
 ...
 * routine directly, but use the TRIS_SCHED_DEL() macro instead (especially if
 ...
 */
#ifndef TRIS_DEVMODE
int tris_sched_del(struct sched_context *con, int id) attribute_warn_unused_result;
#else
int _tris_sched_del(struct sched_context *con, int id, const char *file, int line, const char *function) attribute_warn_unused_result;
#define	tris_sched_del(a, b)	_tris_sched_del(a, b, __FILE__, __LINE__, __PRETTY_FUNCTION__)
#endif
 ...
 */
int tris_sched_wait(struct sched_context *con) attribute_warn_unused_result;
 ...
 *
 */
#define tris_sched_add_object(obj,con,when,callback) tris_sched_add((con),(when),(callback), ASTOBJ_REF((obj)))

/*!
 * \brief Convenience macro for objects and reference (del)
 *
 */
#define tris_sched_del_object(obj,destructor,con,id) do { \
	if ((id) > -1) { \
		tris_sched_del((con),(id)); \
		(id) = -1; \
 ...
 *
 * The purpose of the tris_sched_thread API is to provide a common implementation
...
/*!
 * \brief Get the scheduler context for a given tris_sched_thread
 *
 ...
 * \param st the handle to the scheduler and thread
 *
 * \return the sched_context associated with an tris_sched_thread
 */
 ...
 *
 * This uses the TRIS_SCHED_DEL macro internally.
 *
 * \param st the handle to the scheduler and thread
 ...
 */
#define tris_sched_thread_del(st, id) ({ \
	struct sched_context *__tmp_context = tris_sched_thread_get_context(st); \
	TRIS_SCHED_DEL(__tmp_context, id); \
})
