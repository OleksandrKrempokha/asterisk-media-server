 ...
 * A taskprocessor is created the first time its name is requested via the tris_taskprocessor_get()
 ...
 * tris_taskprocessor_push(taskprocessor, taskhandler, taskdata) function and freed by the
 * taskprocessor after the task handling function returns.  A module releases its reference to a
 * taskprocessor using the tris_taskprocessor_unreference() function which may result in the
...
/*! \brief tris_tps_options for specification of taskprocessor options
 *
 * Specify whether a taskprocessor should be created via tris_taskprocessor_get() if the taskprocessor 
 ...
 * and only if it exists, use the TPS_REF_IF_EXISTS option in tris_taskprocessor_get(). */
enum tris_tps_options {
 ...
 * disabled by specifying the TPS_REF_IF_EXISTS tris_tps_options as the second argument to tris_taskprocessor_get().
