 ...
 * \file tcptls.h
 *
 * \brief Generic support for tcp/tls servers in Trismedia.
...
#ifndef _TRISMEDIA_TCPTLS_H
#define _TRISMEDIA_TCPTLS_H
...
/*! SSL support */
#define TRIS_CERTFILE "trismedia.pem"

enum tris_ssl_flags {
	/*! Verify certificate when acting as server */
	TRIS_SSL_VERIFY_CLIENT = (1 << 0),
	/*! Don't verify certificate when connecting to a server */
	TRIS_SSL_DONT_VERIFY_SERVER = (1 << 1),
	/*! Don't compare "Common Name" against IP or hostname */
	TRIS_SSL_IGNORE_COMMON_NAME = (1 << 2)
};

struct tris_tls_config {
	int enabled;
	char *certfile;
	char *cipher;
	char *cafile;
 ...
 * worker_fn() is a struct tris_tcptls_session_instance, which contains the address
 ...
 */
struct tris_tcptls_session_args {
	...
	struct tris_tls_config *tls_cfg; /*!< points to the SSL configuration if any */
	int accept_fd;
	int poll_timeout;
	pthread_t master;
 ...
 */
struct tris_tcptls_session_instance {
...
/*	iint (*ssl_setup)(SSL *); */
	int client;
	struct sockaddr_in remote_address;
	struct tris_tcptls_session_args *parent;
	tris_mutex_t lock;
};
...
/* \brief Creates a client connection's tris_tcptls_session_instance. */
struct tris_tcptls_session_instance *tris_tcptls_client_create(struct tris_tcptls_session_args *desc);

void *tris_tcptls_server_root(void *);

/*!
 * \brief This is a generic (re)start routine for a TCP server,
 * which does the socket/bind/listen and starts a thread for handling
 * accept().
 * \version 1.6.1 changed desc parameter to be of tris_tcptls_session_args type
 */
...
HOOK_T tris_tcptls_server_read(struct tris_tcptls_session_instance *ser, void *buf, size_t count);
HOOK_T tris_tcptls_server_write(struct tris_tcptls_session_instance *ser, const void *buf, size_t count);

#endif /* _TRISMEDIA_TCPTLS_H */
