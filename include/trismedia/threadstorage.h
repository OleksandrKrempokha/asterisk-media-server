...
/*!
 * \page AstThreadStorage The Trismedia Thread Storage API
 *
 *
 ...
 * storage.
 *
 * The motivation for using this code in Trismedia is for situations where
 * storing data on a thread-specific basis can provide some amount of
 * performance benefit.  For example, there are some call types in Trismedia
 * where tris_frame structures must be allocated very rapidly (easily 50, 100,
...
#ifndef TRISMEDIA_THREADSTORAGE_H
#define TRISMEDIA_THREADSTORAGE_H
 ...
 * Example usage:
 * \code
 * TRIS_THREADSTORAGE(my_buf);
 * \endcode
 */
#define TRIS_THREADSTORAGE(name) \
	TRIS_THREADSTORAGE_CUSTOM_SCOPE(name, NULL, tris_free_ptr, static) 
#define TRIS_THREADSTORAGE_PUBLIC(name) \
	TRIS_THREADSTORAGE_CUSTOM_SCOPE(name, NULL, tris_free_ptr,) 
#define TRIS_THREADSTORAGE_EXTERNAL(name) \
	extern struct tris_threadstorage name
 ...
 *           as the argument.
 * \param cleanup This is a custom function that will be called instead of tris_free
 *              when the thread goes away.  Note that if this is used, it *MUST*
 *              call free on the allocated memory.
 *
 * Example usage:
 * \code
 * TRIS_THREADSTORAGE_CUSTOM(my_buf, my_init, my_cleanup);
 * \endcode
 */
#define TRIS_THREADSTORAGE_CUSTOM(a,b,c)	TRIS_THREADSTORAGE_CUSTOM_SCOPE(a,b,c,static)

#if defined(PTHREAD_ONCE_INIT_NEEDS_BRACES)
# define TRIS_PTHREAD_ONCE_INIT { PTHREAD_ONCE_INIT }
#else
# define TRIS_PTHREAD_ONCE_INIT PTHREAD_ONCE_INIT
#endif

#if !defined(DEBUG_THREADLOCALS)
#define TRIS_THREADSTORAGE_CUSTOM_SCOPE(name, c_init, c_cleanup, scope)	\
static void __init_##name(void);                \
scope struct tris_threadstorage name = {         \
	.once = TRIS_PTHREAD_ONCE_INIT,              \
...
{                                               \
	__tris_threadstorage_object_remove(data);    \
	c_cleanup(data);                            \
}                                               \
static void __init_##name(void)                 \
{                                               \
 ...
 *      TRIS_THREADSTORAGE(my_buf), then this argument would be (&my_buf).
 ...
 *      void *buf;
 *
 *      if (!(buf = tris_threadstorage_get(&my_buf, MY_BUF_SIZE)))
 *           return;
 *      ...
 * }
 * \endcode
...
#if !defined(DEBUG_THREADLOCALS)
TRIS_INLINE_API(
void *tris_threadstorage_get(struct tris_threadstorage *ts, size_t init_size),
{
	void *buf;

	pthread_once(&ts->once, ts->key_init);
	if (!(buf = pthread_getspecific(ts->key))) {
		if (!(buf = tris_calloc(1, init_size)))
			return NULL;
...
#else /* defined(DEBUG_THREADLOCALS) */
TRIS_INLINE_API(
void *__tris_threadstorage_get(struct tris_threadstorage *ts, size_t init_size, const char *file, const char *function, unsigned int line),
{
	void *buf;
	...
	if (!(buf = pthread_getspecific(ts->key))) {
		...
		pthread_setspecific(ts->key, buf);
		__tris_threadstorage_object_add(buf, init_size, file, function, line);
	}
...
#define tris_threadstorage_get(ts, init_size) __tris_threadstorage_get(ts, init_size, __FILE__, __PRETTY_FUNCTION__, __LINE__)
#endif /* defined(DEBUG_THREADLOCALS) */

#endif /* TRISMEDIA_THREADSTORAGE_H */
