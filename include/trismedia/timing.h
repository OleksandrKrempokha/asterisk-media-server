...
/*!
  ...
  Portions of Trismedia require a timing source, a periodic trigger
  for media handling activities. The functions in this file allow
  a loadable module to provide a timing source for Trismedia and its
  ...
  The timing source used by Trismedia must provide the following
  features:
...
#ifndef _TRISMEDIA_TIMING_H
#define _TRISMEDIA_TIMING_H

#if defined(__cplusplus) || defined(c_plusplus)
extern "C" {
#endif

enum tris_timer_event {
	TRIS_TIMING_EVENT_EXPIRED = 1,
	TRIS_TIMING_EVENT_CONTINUOUS = 2,
};
...
struct tris_timing_interface {
	...
	enum tris_timer_event (*timer_get_event)(int handle);
 ...
 *
 * \param funcs An instance of the \c tris_timing_interfaces structure with pointers
 *        to the functions provided by the timing implementation.
 *
 * \retval NULL failure 
 * \retval non-Null handle to be passed to tris_unregister_timing_interface() on success
 * \since 1.6.1
 */
#define tris_register_timing_interface(i) _tris_register_timing_interface(i, tris_module_info->self)
 ...
 *        tris_register_timing_interface().
 *
 * \retval 0 success
 ...
 *       returned TRIS_TIMING_EVENT_EXPIRED.
 *
 * \return nothing
 * \since 1.6.1
 */
