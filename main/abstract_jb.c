...
/*! \brief Create */
typedef void * (*jb_create_impl)(struct tris_jb_conf *general_config, long resynch_threshold);
/*! \brief Destroy */
typedef void (*jb_destroy_impl)(void *jb);
/*! \brief Put first frame */
typedef int (*jb_put_first_impl)(void *jb, struct tris_frame *fin, long now);
/*! \brief Put frame */
typedef int (*jb_put_impl)(void *jb, struct tris_frame *fin, long now);
/*! \brief Get frame for now */
typedef int (*jb_get_impl)(void *jb, struct tris_frame **fout, long now, long interpl);
/*! \brief Get next */
typedef long (*jb_next_impl)(void *jb);
/*! \brief Remove first frame */
typedef int (*jb_remove_impl)(void *jb, struct tris_frame **fout);
/*! \brief Force resynch */
...
struct tris_jb_impl
{
	char name[TRIS_JB_IMPL_NAME_SIZE];
...
/* fixed */
static void *jb_create_fixed(struct tris_jb_conf *general_config, long resynch_threshold);
static void jb_destroy_fixed(void *jb);
static int jb_put_first_fixed(void *jb, struct tris_frame *fin, long now);
static int jb_put_fixed(void *jb, struct tris_frame *fin, long now);
static int jb_get_fixed(void *jb, struct tris_frame **fout, long now, long interpl);
static long jb_next_fixed(void *jb);
static int jb_remove_fixed(void *jb, struct tris_frame **fout);
static void jb_force_resynch_fixed(void *jb);
static void jb_empty_and_reset_fixed(void *jb);
/* adaptive */
static void * jb_create_adaptive(struct tris_jb_conf *general_config, long resynch_threshold);
static void jb_destroy_adaptive(void *jb);
static int jb_put_first_adaptive(void *jb, struct tris_frame *fin, long now);
static int jb_put_adaptive(void *jb, struct tris_frame *fin, long now);
static int jb_get_adaptive(void *jb, struct tris_frame **fout, long now, long interpl);
static long jb_next_adaptive(void *jb);
static int jb_remove_adaptive(void *jb, struct tris_frame **fout);
static void jb_force_resynch_adaptive(void *jb);
static void jb_empty_and_reset_adaptive(void *jb);

/* Available jb implementations */
static struct tris_jb_impl avail_impl[] =
{
	{
		.name = "fixed",
...
static void jb_choose_impl(struct tris_channel *chan);
static void jb_get_and_deliver(struct tris_channel *chan);
static int create_jb(struct tris_channel *chan, struct tris_frame *first_frame);
static long get_now(struct tris_jb *jb, struct timeval *tv);
...
static void jb_choose_impl(struct tris_channel *chan)
{
	struct tris_jb *jb = &chan->jb;
	struct tris_jb_conf *jbconf = &jb->conf;
	struct tris_jb_impl *test_impl;
	int i, avail_impl_count = ARRAY_LEN(avail_impl);

	jb->impl = &avail_impl[default_impl];

	if (tris_strlen_zero(jbconf->impl)) {
		return;
	}
...
{
	struct tris_jb *jb0 = &c0->jb;
	struct tris_jb *jb1 = &c1->jb;
	struct tris_jb_conf *conf0 = &jb0->conf;
	struct tris_jb_conf *conf1 = &jb1->conf;
	int c0_wants_jitter = c0->tech->properties & TRIS_CHAN_TP_WANTSJITTER;
	int c0_creates_jitter = c0->tech->properties & TRIS_CHAN_TP_CREATESJITTER;
	int c0_jb_enabled = tris_test_flag(conf0, TRIS_JB_ENABLED);
	int c0_force_jb = tris_test_flag(conf0, TRIS_JB_FORCED);
	int c0_jb_timebase_initialized = tris_test_flag(jb0, JB_TIMEBASE_INITIALIZED);
	int c0_jb_created = tris_test_flag(jb0, JB_CREATED);
	int c1_wants_jitter = c1->tech->properties & TRIS_CHAN_TP_WANTSJITTER;
	int c1_creates_jitter = c1->tech->properties & TRIS_CHAN_TP_CREATESJITTER;
	int c1_jb_enabled = tris_test_flag(conf1, TRIS_JB_ENABLED);
	int c1_force_jb = tris_test_flag(conf1, TRIS_JB_FORCED);
	int c1_jb_timebase_initialized = tris_test_flag(jb1, JB_TIMEBASE_INITIALIZED);
	int c1_jb_created = tris_test_flag(jb1, JB_CREATED);
	int inuse = 0;
	...
	if (((!c0_wants_jitter && c1_creates_jitter) || (c0_force_jb && c1_creates_jitter)) && c0_jb_enabled) {
		...
		if (!c0_jb_timebase_initialized) {
			...
			tris_set_flag(jb0, JB_TIMEBASE_INITIALIZED);
		}
	...
	if (((!c1_wants_jitter && c0_creates_jitter) || (c1_force_jb && c0_creates_jitter)) && c1_jb_enabled) {
		...
		if (!c1_jb_timebase_initialized) {
			...
			tris_set_flag(jb1, JB_TIMEBASE_INITIALIZED);
		}
...
int tris_jb_get_when_to_wakeup(struct tris_channel *c0, struct tris_channel *c1, int time_left)
{
	struct tris_jb *jb0 = &c0->jb;
	struct tris_jb *jb1 = &c1->jb;
	int c0_use_jb = tris_test_flag(jb0, JB_USE);
	int c0_jb_is_created = tris_test_flag(jb0, JB_CREATED);
	int c1_use_jb = tris_test_flag(jb1, JB_USE);
	int c1_jb_is_created = tris_test_flag(jb1, JB_CREATED);
...
{
	struct tris_jb *jb = &chan->jb;
	struct tris_jb_impl *jbimpl = jb->impl;
	void *jbobj = jb->jbobj;
	struct tris_frame *frr;
	long now = 0;

	if (!tris_test_flag(jb, JB_USE))
		return -1;

	if (f->frametype != TRIS_FRAME_VOICE) {
		if (f->frametype == TRIS_FRAME_DTMF && tris_test_flag(jb, JB_CREATED)) {
	...
	if (!tris_test_flag(f, TRIS_FRFLAG_HAS_TIMING_INFO) || f->len < 2 || f->ts < 0) {
		tris_log(LOG_WARNING, "%s received frame with invalid timing info: "
			"has_timing_info=%d, len=%ld, ts=%ld, src=%s\n",
			chan->name, tris_test_flag(f, TRIS_FRFLAG_HAS_TIMING_INFO), f->len, f->ts, f->src);
		return -1;
	}
	...
	if (!frr) {
		tris_log(LOG_ERROR, "Failed to isolate frame for the jitterbuffer on channel '%s'\n", chan->name);
		return -1;
	}

	if (!tris_test_flag(jb, JB_CREATED)) {
		if (create_jb(chan, frr)) {
			tris_frfree(frr);
			/* Disable the jitterbuffer */
			tris_clear_flag(jb, JB_USE);
			return -1;
		}

		tris_set_flag(jb, JB_CREATED);
		return 0;
	} else {
		now = get_now(jb, NULL);
...
static void jb_get_and_deliver(struct tris_channel *chan)
{
	...
	struct tris_jb_impl *jbimpl = jb->impl;
	void *jbobj = jb->jbobj;
	struct tris_frame *f, finterp = { .frametype = TRIS_FRAME_VOICE, };
	long now;
	...
	while (now >= jb->next) {
		interpolation_len = tris_codec_interp_len(jb->last_format);
		...
		case JB_IMPL_INTERP:
			...
			f->delivery = tris_tvadd(jb->timebase, tris_samp2tv(jb->next, 1000));
		...
		case JB_IMPL_NOFRAME:
			...
			return;
		default:
			tris_log(LOG_ERROR, "This should never happen!\n");
			tris_assert("JB type unknown" == NULL);
			break;
		}
...
static int create_jb(struct tris_channel *chan, struct tris_frame *frr)
{
	struct tris_jb *jb = &chan->jb;
	struct tris_jb_conf *jbconf = &jb->conf;
	struct tris_jb_impl *jbimpl = jb->impl;
	...
	long now;
	char logfile_pathname[20 + TRIS_JB_IMPL_NAME_SIZE + 2*TRIS_CHANNEL_NAME + 1];
	char name1[TRIS_CHANNEL_NAME], name2[TRIS_CHANNEL_NAME], *tmp;
	int res;

	jbobj = jb->jbobj = jbimpl->create(jbconf, jbconf->resync_threshold);
	if (!jbobj) {
		tris_log(LOG_WARNING, "Failed to create jitterbuffer on channel '%s'\n", chan->name);
		return -1;
	}
	...
	if (res != JB_IMPL_OK) {
		tris_log(LOG_WARNING, "Failed to put first frame in the jitterbuffer on channel '%s'\n", chan->name);
		/*
	...
	if (tris_test_flag(jbconf, TRIS_JB_LOG)) {
		...
		bridged = tris_bridged_channel(chan);
		/* We should always have bridged chan if a jitterbuffer is in use */
		tris_assert(bridged != NULL);
		...
		snprintf(logfile_pathname, sizeof(logfile_pathname),
			"/tmp/tris_%s_jb_%s--%s.log", jbimpl->name, name1, name2);
		if (!(safe_fd = mkstemp(safe_logfile)) > -1 || unlink(logfile_pathname) || link(safe_logfile, logfile_pathname) || unlink(safe_logfile) || !(jb->logfile = fdopen(safe_fd, "w+b"))) {
			...
			tris_log(LOG_ERROR, "Failed to create frame log file with pathname '%s': %s\n", logfile_pathname, strerror(errno));
		}
	...
	tris_verb(3, "%s jitterbuffer created on channel %s\n", jbimpl->name, chan->name);
...
void tris_jb_destroy(struct tris_channel *chan)
{
	...
	if (tris_test_flag(jb, JB_CREATED)) {
		...
		tris_clear_flag(jb, JB_CREATED);

		tris_verb(3, "%s jitterbuffer destroyed on channel %s\n", jbimpl->name, chan->name);
	}
}


static long get_now(struct tris_jb *jb, struct timeval *when)
{
	struct timeval now;
	...
	return tris_tvdiff_ms(*when, jb->timebase);
}


int tris_jb_read_conf(struct tris_jb_conf *conf, const char *varname, const char *value)
{
	int prefixlen = sizeof(TRIS_JB_CONF_PREFIX) - 1;
	const char *name;
	int tmp;

	if (strncasecmp(TRIS_JB_CONF_PREFIX, varname, prefixlen)) {
		return -1;
	}

	name = varname + prefixlen;

	if (!strcasecmp(name, TRIS_JB_CONF_ENABLE)) {
		tris_set2_flag(conf, tris_true(value), TRIS_JB_ENABLED);
	} else if (!strcasecmp(name, TRIS_JB_CONF_FORCE)) {
		tris_set2_flag(conf, tris_true(value), TRIS_JB_FORCED);
	} else if (!strcasecmp(name, TRIS_JB_CONF_MAX_SIZE)) {
		if ((tmp = atoi(value)) > 0)
			conf->max_size = tmp;
	} else if (!strcasecmp(name, TRIS_JB_CONF_RESYNCH_THRESHOLD)) {
		if ((tmp = atoi(value)) > 0)
			conf->resync_threshold = tmp;
	} else if (!strcasecmp(name, TRIS_JB_CONF_IMPL)) {
		if (!tris_strlen_zero(value))
			snprintf(conf->impl, sizeof(conf->impl), "%s", value);
	} else if (!strcasecmp(name, TRIS_JB_CONF_TARGET_EXTRA)) {
		if (sscanf(value, "%30d", &tmp) == 1) {
			conf->target_extra = tmp;
		}
	} else if (!strcasecmp(name, TRIS_JB_CONF_LOG)) {
		tris_set2_flag(conf, tris_true(value), TRIS_JB_LOG);
	} else {
		return -1;
	}
