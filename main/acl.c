...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 248949 $")
...
static void score_address(const struct sockaddr_in *sin, struct in_addr *best_addr, int *best_score)
{
	...
	address = tris_inet_ntoa(sin->sin_addr);
...
static struct tris_ha *tris_duplicate_ha(struct tris_ha *original)
{
	struct tris_ha *new_ha;

	if ((new_ha = tris_malloc(sizeof(*new_ha)))) {
		/* Copy from original to new object */
		tris_copy_ha(original, new_ha);
	}
...
{
	struct tris_ha *start = original;
	struct tris_ha *ret = NULL;
	struct tris_ha *current, *prev = NULL;

	while (start) {
		current = tris_duplicate_ha(start);  /* Create copy of this object */
		if (prev) {
...
struct tris_ha *tris_append_ha(const char *sense, const char *stuff, struct tris_ha *path, int *error)
{
	struct tris_ha *ha;
	char *nm;
	struct tris_ha *prev = NULL;
	struct tris_ha *ret;
	int x;
	char *tmp = tris_strdupa(stuff);
		...
		*nm = '\0';
		...
		if (!strchr(nm, '.')) {
			if ((sscanf(nm, "%30d", &x) == 1) && (x >= 0) && (x <= 32)) {
				...
				tris_log(LOG_WARNING, "Invalid CIDR in %s\n", stuff);
				tris_free(ha);
				if (error) {
					*error = 1;
				}
			...
			tris_log(LOG_WARNING, "Invalid mask in %s\n", stuff);
			tris_free(ha);
			if (error) {
				*error = 1;
			}
	...
	if (!inet_aton(tmp, &ha->netaddr)) {
		tris_log(LOG_WARNING, "Invalid IP address in %s\n", stuff);
		tris_free(ha);
		if (error) {
			*error = 1;
		}
	...
	ha->sense = strncasecmp(sense, "p", 1) ? TRIS_SENSE_DENY : TRIS_SENSE_ALLOW;
	...
	tris_debug(1, "%s/%s sense %d appended to acl for peer\n", tris_strdupa(tris_inet_ntoa(ha->netaddr)), tris_strdupa(tris_inet_ntoa(ha->netmask)), ha->sense);
...
{
	/* Start optimistic */
	int res = TRIS_SENSE_ALLOW;
	while (ha) {
#if 0	/* debugging code */
		char iabuf[INET_ADDRSTRLEN];
		char iabuf2[INET_ADDRSTRLEN];
		/* DEBUG */
		tris_copy_string(iabuf, tris_inet_ntoa(sin->sin_addr), sizeof(iabuf));
		tris_copy_string(iabuf2, tris_inet_ntoa(ha->netaddr), sizeof(iabuf2));
		tris_debug(1, "##### Testing %s with %s\n", iabuf, iabuf2);
#endif
		/* For each rule, if this address and the netmask = the net address
		   apply the current rule */
		if ((sin->sin_addr.s_addr & ha->netmask.s_addr) == ha->netaddr.s_addr) {
			res = ha->sense;
			return TRIS_SENSE_DENY;
		}
		ha = ha->next;
	}
	return res;
...
int tris_get_ip_or_srv(struct sockaddr_in *sin, const char *value, const char *service)
{
	...
	if (service) {
		...
		if (tris_get_srv(NULL, host, sizeof(host), &tportno, srv) > 0) {
			sin->sin_port = htons(tportno);
			value = host;
		}
	}
	if ((hp = tris_gethostbyname(value, &ahp))) {
		memcpy(&sin->sin_addr, hp->h_addr, sizeof(sin->sin_addr));
	} else {
		tris_log(LOG_WARNING, "Unable to lookup '%s'\n", value);
		return -1;
	}
	return 0;
}
...
{
	return tris_get_ip_or_srv(sin, value, NULL);
}
...
int tris_ouraddrfor(struct in_addr *them, struct in_addr *us)
{
	...
	if ((s = socket(PF_INET, SOCK_DGRAM, 0)) < 0) {
		tris_log(LOG_ERROR, "Cannot create socket\n");
		return -1;
	}
	...
	if (connect(s, (struct sockaddr *)&sin, sizeof(sin))) {
		tris_log(LOG_WARNING, "Cannot connect\n");
		close(s);
		return -1;
	}
	slen = sizeof(sin);
	...
	close(s);
	tris_debug(3, "Found IP address for this socket\n");
	*us = sin.sin_addr;
	return 0;
}
...
int tris_find_ourip(struct in_addr *ourip, struct sockaddr_in bindaddr)
{
	...
	if (ntohl(bindaddr.sin_addr.s_addr)) {
		...
		tris_debug(3, "Attached to given IP address\n");
		return 0;
	}
	/* try to use our hostname */
	if (gethostname(ourhost, sizeof(ourhost) - 1)) {
		tris_log(LOG_WARNING, "Unable to get hostname\n");
	} else {
		if ((hp = tris_gethostbyname(ourhost, &ahp))) {
			memcpy(ourip, hp->h_addr, sizeof(*ourip));
			tris_debug(3, "Found one IP address based on local hostname %s.\n", ourhost);
			return 0;
		}
	}
	tris_debug(3, "Trying to check A.ROOT-SERVERS.NET and get our IP address for that connection\n");
	/* A.ROOT-SERVERS.NET. */
	if (inet_aton("198.41.0.4", &saddr) && !tris_ouraddrfor(&saddr, ourip)) {
		return 0;
	}
