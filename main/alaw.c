...
#ifndef G711_NEW_ALGORITHM
unsigned char __tris_lin2a[8192];
#else
unsigned char __tris_lin2a[TRIS_ALAW_TAB_SIZE];
#endif
short __tris_alaw[256];
...
void tris_alaw_init(void)
{
...
#ifndef G711_NEW_ALGORITHM
	for (i = 0; i < 256; i++) {
		__tris_alaw[i] = alaw2linear(i);
	}
	/* set up the reverse (mu-law) conversion table */
	for (i = -32768; i < 32768; i++) {
		__tris_lin2a[((unsigned short)i) >> 3] = linear2alaw(i);
	}
#else
	for (i = 0; i < 256; i++) {
		__tris_alaw[i] = alaw2linear(i);
	...
	for (i = 0; i <= 32768; i += TRIS_ALAW_STEP) {
		TRIS_LIN2A_LOOKUP(i) = linear2alaw(i, 0 /* half-cooked */);
	}
#endif
	...
	for (i = -32768; i < 32768; ++i) {
		...
		unsigned char e2 = TRIS_LIN2A(i);
		short d2 = alaw2linear(e2);
		short d3 = TRIS_ALAW(e1);

		if (e1 != e2 || d1 != d3 || d2 != d3) {
			tris_log(LOG_WARNING, "a-Law coding tables test failed on %d: e1=%u, e2=%u, d1=%d, d2=%d\n",
					i, (unsigned)e1, (unsigned)e2, (int)d1, (int)d2);
		}
	}
	tris_log(LOG_NOTICE, "a-Law coding tables test complete.\n");
#endif /* TEST_CODING_TABLES */

#ifdef TEST_TANDEM_TRANSCODING
	/* tandem transcoding test */
	for (i = -32768; i < 32768; ++i) {
		unsigned char e1 = TRIS_LIN2A(i);
		short d1 = TRIS_ALAW(e1);
		unsigned char e2 = TRIS_LIN2A(d1);
		short d2 = TRIS_ALAW(e2);
		unsigned char e3 = TRIS_LIN2A(d2);
		short d3 = TRIS_ALAW(e3);

		if (e1 != e2 || e2 != e3 || d1 != d2 || d2 != d3) {
			tris_log(LOG_WARNING, "a-Law tandem transcoding test failed on %d: e1=%u, e2=%u, d1=%d, d2=%d, d3=%d\n",
					i, (unsigned)e1, (unsigned)e2, (int)d1, (int)d2, (int)d3);
		}
	}
	tris_log(LOG_NOTICE, "a-Law tandem transcoding test complete.\n");
