...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 228340 $")
...
#define	COPY(dst, src)                                             \
	do {                                                           \
		...
		if (slen + 1 > dlen) {                                     \
			...
			if (slash) {                                           \
				tris_copy_string(dst, slash + 1, dlen);             \
			} else {                                               \
				tris_copy_string(dst, src + slen - dlen + 1, dlen); \
			}                                                      \
		} else {                                                   \
			tris_copy_string(dst, src, dlen);                       \
		}                                                          \
	} while (0)
...
int __tris_fdleak_socket(int domain, int type, int protocol, const char *file, int line, const char *func)
{
	...
	if (domain == PF_UNIX) {
		tris_copy_string(sdomain, "PF_UNIX", sizeof(sdomain));
	} else if (domain == PF_INET) {
		tris_copy_string(sdomain, "PF_INET", sizeof(sdomain));
	} else {
		snprintf(sdomain, sizeof(sdomain), "%d", domain);
	}

	if (type == SOCK_DGRAM) {
		tris_copy_string(stype, "SOCK_DGRAM", sizeof(stype));
		if (protocol == 0) {
			sproto = "udp";
		}
	} else if (type == SOCK_STREAM) {
		tris_copy_string(stype, "SOCK_STREAM", sizeof(stype));
		if (protocol == 0) {
			sproto = "tcp";
		}
	} else {
...
static char *handle_show_fd(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int i;
	char line[24];
	struct rlimit rl;
	...
	if (rl.rlim_cur == RLIM_INFINITY || rl.rlim_max == RLIM_INFINITY) {
		tris_copy_string(line, "unlimited", sizeof(line));
	} else {
		snprintf(line, sizeof(line), "%d/%d", (int) rl.rlim_cur, (int) rl.rlim_max);
	}
	tris_cli(a->fd, "Current maxfiles: %s\n", line);
	for (i = 0; i < 1024; i++) {
		if (fdleaks[i].isopen) {
			snprintf(line, sizeof(line), "%d", fdleaks[i].line);
			tris_cli(a->fd, "%5d %15s:%-7.7s (%-25s): %s(%s)\n", i, fdleaks[i].file, line, fdleaks[i].function, fdleaks[i].callname, fdleaks[i].callargs);
		}
	}
	return CLI_SUCCESS;
}

static struct tris_cli_entry cli_show_fd = TRIS_CLI_DEFINE(handle_show_fd, "Show open file descriptors");

int tris_fd_init(void)
{
	return tris_cli_register(&cli_show_fd);
}
