...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 206869 $")
...
#define TRIS_CALLERID_UNKNOWN	"<unknown>"
...
static inline void gen_tones(unsigned char *buf, int len, int codec, float ddr1, float ddi1, float ddr2, float ddi2, float *cr1, float *ci1, float *cr2, float *ci2)
{
	...
	for (x = 0; x < len; x++) {
		...
		*cr2 *= t;
		*ci2 *= t; 	
		buf[x] = TRIS_LIN2X((*cr1 + *cr2) * 2048.0);
	}
}
...
struct callerid_state *callerid_new(int cid_signalling)
{
	...
	if ((cid = tris_calloc(1, sizeof(*cid)))) {
	...
	if (cidstring[0] == 'B') {
		...
		if (code == 0)
			...
			*flags = CID_PRIVATE_NUMBER;
			...
			tris_debug(1, "Unknown DTMF code %d\n", code);
		...
		for (i = 1; i < strlen(cidstring); i++) {
			...
			if (isdigit(cidstring[i]))
				...
				tris_debug(1, "Unknown CID digit '%c'\n",
					cidstring[i]);
		}
		number[i-1] = 0;
		 ...
		 * for the best */
		tris_log(LOG_WARNING, "Couldn't detect start-character. CID "
		...
		tris_debug(1, "Unknown CID protocol, start digit '%c'\n", cidstring[0]);
		*flags = CID_UNKNOWN_NUMBER;
	}
}

int tris_gen_cas(unsigned char *outbuf, int sendsas, int len, int codec)
{
	int pos = 0;
	int saslen = 2400;
	float cr1 = 1.0;
...
int callerid_feed_jp(struct callerid_state *cid, unsigned char *ubuf, int len, int codec)
{
	...
	for (x = 0; x < len; x++) 
		buf[x+cid->oldlen/2] = TRIS_XLAW(ubuf[x]);
	...
	while (mylen >= 160) {
		...
		if (mylen < 0) {
			tris_log(LOG_ERROR, "No start bit found in fsk data.\n");
			return -1;
		}
		...
		if (res < 0) {
			tris_log(LOG_NOTICE, "fsk_serial failed\n");
			return -1;
		}
		...
		if (res == 1) {
			...
			case 7:	/* NUMBER LENGTH */
				...
				if ((cid->len+2) >= sizeof(cid->rawdata)) {
					tris_log(LOG_WARNING, "too long caller id string\n") ;
					return -1;
				}
				cid->rawdata[cid->pos++] = b;
				break;
			...
			case 11: /* CRC Checksum 2 */
				cid->sawflag = 12;
				if (cid->crc != 0) {
					tris_log(LOG_WARNING, "crc checksum error\n") ;
					return -1;
				} 
				...
				for (x = 0; x < cid->pos;) {
					...
					case 0x02: /* caller id  number */
						...
						cid->flags = 0;
						res = cid->rawdata[x++];
						tris_copy_string(cid->number, &cid->rawdata[x], res+1);
						x += res;
						break;
					...
					case 0x22: /* dialed number additional information */
						...
						default:   /* reserved */
							if (option_debug > 1)
								tris_log(LOG_NOTICE, "did info:#1=%X\n", cid->rawdata[x]);
							break ;
						}
						x++;
						/* numbering plan octed 4 */
				...
				break;
			default:
				tris_log(LOG_ERROR, "invalid value in sawflag %d\n", cid->sawflag);
			}
		}
	}
	if (mylen) {
	...
	while (mylen >= 160) {
		...
		if (res == 1) {
			...
			case 4: /* Retrieve message */
				if (cid->pos >= 128) {
					tris_log(LOG_WARNING, "Caller ID too long???\n");
					return -1;
				}
				cid->rawdata[cid->pos++] = b;
				cid->len--;
			...
			case 5: /* Check checksum */
				if (b != (256 - (cid->cksum & 0xff))) {
					tris_log(LOG_NOTICE, "Caller*ID failed checksum\n");
					/* Try again */
					cid->sawflag = 0;
					break;
				}
				...
				if ((cid->type == 0x80) || (cid->type == 0x82)) {
					...
					for (x = 0; x < cid->pos;) {
						...
						case 4: /* Number */
							res = cid->rawdata[x];
							if (res > 32) {
								tris_log(LOG_NOTICE, "Truncating long caller ID number from %d bytes to 32\n", cid->rawdata[x]);
								res = 32; 
							}
							if (tris_strlen_zero(cid->number)) {
						...
						case 8: /* Name */
							res = cid->rawdata[x];
							if (res > 32) {
								tris_log(LOG_NOTICE, "Truncating long caller ID name from %d bytes to 32\n", cid->rawdata[x]);
								res = 32; 
							}
						...
						case 22: /* Something French */
							break;
						default:
							tris_log(LOG_NOTICE, "Unknown IE %d\n", cid->rawdata[x - 1]);
						}
						res = cid->rawdata[x];
						if (0 > res){	/* Negative offset in the CID Spill */
							tris_log(LOG_NOTICE, "IE %d has bad field length of %d at offset %d\n", cid->rawdata[x-1], cid->rawdata[x], x);
							/* Try again */
							cid->sawflag = 0;
							break; 	/* Exit the loop */
						}
					...
					/* SDMF */
					tris_copy_string(cid->number, cid->rawdata + 8, sizeof(cid->number));
				}
				if (!strcmp(cid->number, "P")) {
					strcpy(cid->number, "");
					cid->flags |= CID_PRIVATE_NUMBER;
				} else if (!strcmp(cid->number, "O") || tris_strlen_zero(cid->number)) {
					strcpy(cid->number, "");
					cid->flags |= CID_UNKNOWN_NUMBER;
				}
				...
				} else if (!strcmp(cid->name, "O") || tris_strlen_zero(cid->name)) {
					strcpy(cid->name, "");
				...
				break;
			default:
				tris_log(LOG_ERROR, "Dunno what to do with a digit in sawflag %d\n", cid->sawflag);
			}
		}
	}
	if (mylen) {
...
static int callerid_genmsg(char *msg, int size, const char *number, const char *name, int flags)
{
	...
	size -= res;
	ptr += res;
	if (tris_strlen_zero(number) || (flags & CID_UNKNOWN_NUMBER)) {
	...
	if (tris_strlen_zero(name) || (flags & CID_UNKNOWN_NAME)) {
...
*/
static int tris_is_valid_string(const char *exten, const char *valid)
{
	int x;
...
{
	return tris_is_valid_string(n, "0123456789*#+");
}

/*! \brief checks if string consists only of digits and ( ) - * \# and + 
	Pre-qualifies the string for tris_shrink_phone_number()
	\return 1 if string is valid AST shrinkable phone number
	\return 0 if not
*/
int tris_is_shrinkable_phonenumber(const char *exten)
{
	return tris_is_valid_string(exten, "0123456789*#+()-.");
}
	...
	if ((ls = strrchr(instr, '<')) && (le = strrchr(ls, '>'))) {
		...
		*location = ls + 1;	/* and this is the result */
		if ((ns = strchr(instr, '"')) && (ne = strchr(ns + 1, '"'))) {
			...
			*name = tris_skip_blanks(instr);
			tris_trim_blanks(*name);
		}
	} else {	/* no valid brackets */
		char tmp[256];

		tris_copy_string(tmp, instr, sizeof(tmp));
		tris_shrink_phone_number(tmp);
		if (tris_isphonenumber(tmp)) {	/* Assume it's just a location */
			*name = NULL;
...
static int __tris_callerid_generate(unsigned char *buf, const char *name, const char *number, int callwaiting, int codec)
{
...
{
	return __tris_callerid_generate(buf, name, number, 0, codec);
}

int tris_callerid_callwaiting_generate(unsigned char *buf, const char *name, const char *number, int codec)
{
	return __tris_callerid_generate(buf, name, number, 1, codec);
}
...
char *tris_callerid_merge(char *buf, int bufsiz, const char *name, const char *num, const char *unknown)
{
	...
	if (name && num)
		...
		tris_copy_string(buf, name, bufsiz);
	else if (num)
		tris_copy_string(buf, num, bufsiz);
	else
		tris_copy_string(buf, unknown, bufsiz);
	return buf;
}

int tris_callerid_split(const char *buf, char *name, int namelen, char *num, int numlen)
{
	char *tmp;
	char *l = NULL, *n = NULL;

	tmp = tris_strdupa(buf);
	tris_callerid_parse(tmp, &n, &l);
	if (n)
		tris_copy_string(name, n, namelen);
	else
		name[0] = '\0';
	if (l) {
		tris_shrink_phone_number(l);
		tris_copy_string(num, l, numlen);
	} else
		num[0] = '\0';
	return 0;
}
...
static struct {
	...
	{  TRIS_PRES_ALLOWED_USER_NUMBER_NOT_SCREENED, "allowed_not_screened", "Presentation Allowed, Not Screened"},
	{  TRIS_PRES_ALLOWED_USER_NUMBER_PASSED_SCREEN, "allowed_passed_screen", "Presentation Allowed, Passed Screen"},
	{  TRIS_PRES_ALLOWED_USER_NUMBER_FAILED_SCREEN, "allowed_failed_screen", "Presentation Allowed, Failed Screen"},
	{  TRIS_PRES_ALLOWED_NETWORK_NUMBER, "allowed", "Presentation Allowed, Network Number"},
	{  TRIS_PRES_PROHIB_USER_NUMBER_NOT_SCREENED, "prohib_not_screened", "Presentation Prohibited, Not Screened"},
	{  TRIS_PRES_PROHIB_USER_NUMBER_PASSED_SCREEN, "prohib_passed_screen", "Presentation Prohibited, Passed Screen"},
	{  TRIS_PRES_PROHIB_USER_NUMBER_FAILED_SCREEN, "prohib_failed_screen", "Presentation Prohibited, Failed Screen"},
	{  TRIS_PRES_PROHIB_NETWORK_NUMBER, "prohib", "Presentation Prohibited, Network Number"},
	{  TRIS_PRES_NUMBER_NOT_AVAILABLE, "unavailable", "Number Unavailable"},
};

/*! \brief Convert caller ID text code to value 
	used in config file parsing
	\param data text string
	\return value TRIS_PRES_ from callerid.h 
*/
...
/*! \brief Convert caller ID pres value to explanatory string 
	\param data value (see callerid.h TRIS_PRES_ ) 
