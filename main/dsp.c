...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 235775 $")
 ...
 * can be marked and tris_dsp_process later will mute all of them.
 *
...
static void mute_fragment(struct tris_dsp *dsp, fragment_t *fragment)
{
	if (dsp->mute_fragments >= ARRAY_LEN(dsp->mute_data)) {
		tris_log(LOG_ERROR, "Too many fragments to mute. Ignoring\n");
		return;
	}
...
static void tris_tone_detect_init(tone_detect_state_t *s, int freq, int duration, int amp)
{
	...
	*/
	...
	tris_debug(1, "Setup tone %d Hz, %d ms, block_size=%d, hits_required=%d\n", freq, duration, s->block_size, s->hits_required);
}

static void tris_fax_detect_init(struct tris_dsp *s)
{
	tris_tone_detect_init(&s->cng_tone_state, FAX_TONE_CNG_FREQ, FAX_TONE_CNG_DURATION, FAX_TONE_CNG_DB);
	tris_tone_detect_init(&s->ced_tone_state, FAX_TONE_CED_FREQ, FAX_TONE_CED_DURATION, FAX_TONE_CED_DB);
}

static void tris_dtmf_detect_init (dtmf_detect_state_t *s)
{
	int i;
...
static void tris_mf_detect_init (mf_detect_state_t *s)
{
	int i;
...
static void tris_digit_detect_init(digit_detect_state_t *s, int mf)
{
	s->current_digits = 0;
	s->detected_digits = 0;
	s->lost_digits = 0;
	...
	if (mf) {
		tris_mf_detect_init(&s->td.mf);
	} else {
		tris_dtmf_detect_init(&s->td.dtmf);
	}
}

static int tone_detect(struct tris_dsp *dsp, tone_detect_state_t *s, int16_t *amp, int samples)
{
	float tone_energy;
	int i;
	int hit = 0;
	...
	for (start = 0;  start < samples;  start = end) {
		...
		tris_debug(10, "tone %d, Ew=%.2E, Et=%.2E, s/n=%10.2f\n", s->freq, tone_energy, s->energy, tone_energy / (s->energy - tone_energy));
		hit = 0;
		if (tone_energy > s->energy * s->threshold) {
			tris_debug(10, "Hit! count=%d\n", s->hit_count);
			hit = 1;
		}
		...
		if (s->hit_count == s->hits_required) {
			tris_debug(1, "%d Hz done detected\n", s->freq);
			res = 1;
		}
...
static void store_digit(digit_detect_state_t *s, char digit)
{
	...
	if (s->current_digits < MAX_DTMF_DIGITS) {
		...
		tris_log(LOG_WARNING, "Digit lost due to full buffer\n");
		s->lost_digits++;
	}
}

static int dtmf_detect(struct tris_dsp *dsp, digit_detect_state_t *s, int16_t amp[], int samples, int squelch, int relax)
{
	float row_energy[4];
	float col_energy[4];
	float famp;
...
static int mf_detect(struct tris_dsp *dsp, digit_detect_state_t *s, int16_t amp[],
...
static int __tris_dsp_call_progress(struct tris_dsp *dsp, short *s, int len)
{
	int x;
	int y;
	int pass;
	...
	while (len) {
		...
		if (dsp->gsamps == dsp->gsamp_size) {
			...
			case PROG_MODE_UK:
				...
				break;
			default:
				tris_log(LOG_WARNING, "Can't process in unknown prog mode '%d'\n", dsp->progmode);
			}
				...
				switch (dsp->tstate) {
					...
					case DSP_TONE_STATE_HUNGUP:
						if ((dsp->features & DSP_FEATURE_CALL_PROGRESS) &&
							...
							res = TRIS_CONTROL_HANGUP;
							dsp->features &= ~DSP_FEATURE_CALL_PROGRESS;
						}
						break;
				}
				if (dsp->ringtimeout==THRESH_RING2ANSWER) {
					tris_debug(1, "Consider call as answered because of timeout after last ring\n");
					res = TRIS_CONTROL_ANSWER;
					dsp->features &= ~DSP_FEATURE_CALL_PROGRESS;
				}
			} else {
				tris_debug(5, "Stop state %d with duration %d\n", dsp->tstate, dsp->tcount);
				tris_debug(5, "Start state %d\n", newstate);
				dsp->tstate = newstate;
				dsp->tcount = 1;
			}
...
int tris_dsp_call_progress(struct tris_dsp *dsp, struct tris_frame *inf)
{
	if (inf->frametype != TRIS_FRAME_VOICE) {
		tris_log(LOG_WARNING, "Can't check call progress of non-voice frames\n");
		return 0;
	}
	if (inf->subclass != TRIS_FORMAT_SLINEAR) {
		tris_log(LOG_WARNING, "Can only check call progress in signed-linear frames\n");
		return 0;
	}
	return __tris_dsp_call_progress(dsp, inf->data.ptr, inf->datalen / 2);
}

static int __tris_dsp_silence_noise(struct tris_dsp *dsp, short *s, int len, int *totalsilence, int *totalnoise)
{
	int accum;
	int x;
	int res = 0;
...
int tris_dsp_busydetect(struct tris_dsp *dsp)
{
	int res = 0, x;
	...
	if (!res)
		return 0;
	for(i=0; i<5; i++) {
		if (dsp->busy_tonelength[i] > 0) {
			if (abs(avgtone - dsp->busy_tonelength[i]) > (dsp->busy_tonelength[i]*BUSY_PAT_PERCENT/100)) {
#ifdef BUSYDETECT_DEBUG
				tris_debug(5, "busy detector: avgtone of %d not close enough to desired %d\n",
					avgtone, dsp->busy_tonelength[i]);
#endif
				res = 0;
			} else {				
				res = 1;
	...
	/* If we know the expected busy tone silent-period length, check we are in the range */
		if (dsp->busy_quietlength[i] > 0) {
			if (abs(avgsilence - dsp->busy_quietlength[i]) > (dsp->busy_quietlength[i]*BUSY_PAT_PERCENT/100)) {
#ifdef BUSYDETECT_DEBUG
				tris_debug(5, "busy detector: avgsilence of %d not close enough to desired %d\n",
					avgsilence, dsp->busy_quietlength[i]);
#endif
				res = 0;
			} else {
				if (res)
	...
	if (res) {
		tris_debug(5, "tris_dsp_busydetect detected busy, avgtone: %d, avgsilence %d\n", avgtone, avgsilence);
	} else {
		tris_debug(5, "busy detector: FAILED with avgtone: %d, avgsilence %d\n", avgtone, avgsilence);
	}
#endif
	return res;
}

int tris_dsp_silence(struct tris_dsp *dsp, struct tris_frame *f, int *totalsilence)
{
	short *s;
	int len;
	
	if (f->frametype != TRIS_FRAME_VOICE) {
		tris_log(LOG_WARNING, "Can't calculate silence on a non-voice frame\n");
		return 0;
	}
	if (f->subclass != TRIS_FORMAT_SLINEAR) {
		tris_log(LOG_WARNING, "Can only calculate silence on signed-linear frames :(\n");
		return 0;
	}
	s = f->data.ptr;
	len = f->datalen/2;
	return __tris_dsp_silence_noise(dsp, s, len, totalsilence, NULL);
}

int tris_dsp_noise(struct tris_dsp *dsp, struct tris_frame *f, int *totalnoise)
{
       short *s;
       int len;

       if (f->frametype != TRIS_FRAME_VOICE) {
               tris_log(LOG_WARNING, "Can't calculate noise on a non-voice frame\n");
               return 0;
       }
       if (f->subclass != TRIS_FORMAT_SLINEAR) {
               tris_log(LOG_WARNING, "Can only calculate noise on signed-linear frames :(\n");
               return 0;
       }
       s = f->data.ptr;
       len = f->datalen/2;
       return __tris_dsp_silence_noise(dsp, s, len, NULL, totalnoise);
}


struct tris_frame *tris_dsp_process(struct tris_channel *chan, struct tris_dsp *dsp, struct tris_frame *af)
{
	int silence;
	int res;
	...
	unsigned char *odata;
	int len;
	struct tris_frame *outf = NULL;

	if (!af) {
		return NULL;
	}
	if (af->frametype != TRIS_FRAME_VOICE) {
		return af;
	}
	...
	case TRIS_FORMAT_ULAW:
		...
		for (x = 0;x < len; x++) {
			shortdata[x] = TRIS_MULAW(odata[x]);
		}
		break;
	case TRIS_FORMAT_ALAW:
		shortdata = alloca(af->datalen * 2);
		for (x = 0; x < len; x++) {
			shortdata[x] = TRIS_ALAW(odata[x]);
		}
		break;
	default:
		/*Display warning only once. Otherwise you would get hundreds of warnings every second */
		if (dsp->display_inband_dtmf_warning)
			tris_log(LOG_WARNING, "Inband DTMF is not supported on codec %s. Use RFC2833\n", tris_getformatname(af->subclass));
	...
	if ((dsp->features & DSP_FEATURE_SILENCE_SUPPRESS) || (dsp->features & DSP_FEATURE_BUSY_DETECT)) {
		res = __tris_dsp_silence_noise(dsp, shortdata, len, &silence, NULL);
	}

	if ((dsp->features & DSP_FEATURE_SILENCE_SUPPRESS) && silence) {
		memset(&dsp->f, 0, sizeof(dsp->f));
		dsp->f.frametype = TRIS_FRAME_NULL;
		tris_frfree(af);
		tris_set_flag(&dsp->f, TRIS_FRFLAG_FROM_DSP);
		return &dsp->f;
	}
	if ((dsp->features & DSP_FEATURE_BUSY_DETECT) && tris_dsp_busydetect(dsp)) {
		chan->_softhangup |= TRIS_SOFTHANGUP_DEV;
		memset(&dsp->f, 0, sizeof(dsp->f));
		dsp->f.frametype = TRIS_FRAME_CONTROL;
		dsp->f.subclass = TRIS_CONTROL_BUSY;
		tris_frfree(af);
		tris_debug(1, "Requesting Hangup because the busy tone was detected on channel %s\n", chan->name);
		tris_set_flag(&dsp->f, TRIS_FRFLAG_FROM_DSP);
		return &dsp->f;
	}
		...
		if (dsp->digit_state.current_digits) {
			...
			if (!dsp->dtmf_began) {
				...
				if (dsp->features & DSP_FEATURE_BUSY_DETECT) {
					...
					tris_debug(1, "DTMF Detected - Reset busydetector\n");
				}
			}
	...
	if (fax_digit) {
		...
		memset(&dsp->f, 0, sizeof(dsp->f));
		dsp->f.frametype = TRIS_FRAME_DTMF;
	...
	if ((dsp->features & DSP_FEATURE_CALL_PROGRESS)) {
		res = __tris_dsp_call_progress(dsp, shortdata, len);
		if (res) {
			switch (res) {
			...
			case TRIS_CONTROL_HANGUP:
				...
				if (chan) 
					tris_queue_frame(chan, &dsp->f);
				break;
			default:
				tris_log(LOG_WARNING, "Don't know how to represent call progress message %d\n", res);
			}
		}
	...
	case TRIS_FORMAT_ULAW:
		for (x = 0; x < len; x++) {
			odata[x] = TRIS_LIN2MU((unsigned short) shortdata[x]);
		}
		break;
	case TRIS_FORMAT_ALAW:
		for (x = 0; x < len; x++) {
			odata[x] = TRIS_LIN2A((unsigned short) shortdata[x]);
		}
		break;
	}
	...
	if (outf) {
		if (chan) {
			tris_queue_frame(chan, af);
		}
		tris_frfree(af);
		tris_set_flag(outf, TRIS_FRFLAG_FROM_DSP);
		return outf;
	} else {
		return af;
	}
...
static void tris_dsp_prog_reset(struct tris_dsp *dsp)
{
	int max = 0;
	int x;
...
struct tris_dsp *tris_dsp_new(void)
{
	struct tris_dsp *dsp;
	
	if ((dsp = tris_calloc(1, sizeof(*dsp)))) {		
		...
		tris_digit_detect_init(&dsp->digit_state, dsp->digitmode & DSP_DIGITMODE_MF);
		dsp->display_inband_dtmf_warning = 1;
		/* Initialize initial DSP progress detect parameters */
		tris_dsp_prog_reset(dsp);
		/* Initialize fax detector */
		tris_fax_detect_init(dsp);
	}
	return dsp;
}

void tris_dsp_set_features(struct tris_dsp *dsp, int features)
{
	dsp->features = features;
}

void tris_dsp_free(struct tris_dsp *dsp)
{
	if (tris_test_flag(&dsp->f, TRIS_FRFLAG_FROM_DSP)) {
...
void tris_dsp_set_threshold(struct tris_dsp *dsp, int threshold)
{
	dsp->threshold = threshold;
}

void tris_dsp_set_busy_count(struct tris_dsp *dsp, int cadences)
{
	if (cadences < 4) {
		cadences = 4;
	}
...
void tris_dsp_set_busy_pattern(struct tris_dsp *dsp, int *tonelength, int *quietlength)
{
	int i;
	for (i=0;i<5;i++) {
		dsp->busy_tonelength[i] = tonelength[i];
		dsp->busy_quietlength[i] = quietlength[i];
		tris_debug(1, "dsp busy pattern set to %d,%d\n", tonelength[i], quietlength[i]);
	}
}

void tris_dsp_digitreset(struct tris_dsp *dsp)
{
	int i;
...
void tris_dsp_reset(struct tris_dsp *dsp)
{
	int x;
...
int tris_dsp_set_digitmode(struct tris_dsp *dsp, int digitmode)
{
	int new;
	int old;
	...
	if (old != new) {
		...
		tris_digit_detect_init(&dsp->digit_state, new & DSP_DIGITMODE_MF);
	}
	dsp->digitmode = digitmode;
	return 0;
}

int tris_dsp_set_faxmode(struct tris_dsp *dsp, int faxmode)
{
...
int tris_dsp_set_call_progress_zone(struct tris_dsp *dsp, char *zone)
{
	int x;
...
int tris_dsp_was_muted(struct tris_dsp *dsp)
{
	return (dsp->mute_fragments > 0);
}

int tris_dsp_get_tstate(struct tris_dsp *dsp) 
{
	return dsp->tstate;
}

int tris_dsp_get_tcount(struct tris_dsp *dsp) 
{
	return dsp->tcount;
}
...
static int _dsp_init(int reload)
{
	...
	cfg = tris_config_load2(CONFIG_FILE_NAME, "dsp", config_flags);
	...
	if (cfg && cfg != CONFIG_STATUS_FILEUNCHANGED) {
		...
		value = tris_variable_retrieve(cfg, "default", "silencethreshold");
		if (value && sscanf(value, "%30d", &thresholds[THRESHOLD_SILENCE]) != 1) {
			tris_log(LOG_WARNING, "%s: '%s' is not a valid silencethreshold value\n", CONFIG_FILE_NAME, value);
...
int tris_dsp_get_threshold_from_settings(enum threshold which)
{
	return thresholds[which];
}

int tris_dsp_init(void)
{
	return _dsp_init(0);
}

int tris_dsp_reload(void)
{
	return _dsp_init(1);
}

void tris_dsp_frame_freed(struct tris_frame *fr)
{
	struct tris_dsp *dsp;

	tris_clear_flag(fr, TRIS_FRFLAG_FROM_DSP);

	dsp = (struct tris_dsp *) (((char *) fr) - offsetof(struct tris_dsp, f));
