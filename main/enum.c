...
/*! \file
 *
 * \brief ENUM Support for Trismedia
 *
...
TRIS_MUTEX_DEFINE_STATIC(enumlock);
...
static int txt_callback(void *context, unsigned char *answer, int len, unsigned char *fullanswer)
{
	...
	if (i > len) {	/* illegal packet */
		tris_log(LOG_WARNING, "txt_callback: malformed TXT record.\n");
		return 0;
	}

	if (i >= sizeof(c->txt)) {	/* too long? */
		tris_log(LOG_WARNING, "txt_callback: TXT record too long.\n");
		i = sizeof(c->txt) - 1;
	}

	tris_copy_string(c->txt, (char *)answer, i + 1);  /* this handles the \0 termination */
	c->txtlen = i;
...
static int blr_txt(const char *cc, const char *suffix)
{
	...
	tris_mutex_lock(&enumlock);

	tris_verb(4, "blr_txt()  cc='%s', suffix='%s', c_bl='%s'\n", cc, suffix, ienum_branchlabel);

	if (sizeof(domain) < (strlen(cc) * 2 + strlen(ienum_branchlabel) + strlen(suffix) + 2)) {
		tris_mutex_unlock(&enumlock);
		tris_log(LOG_WARNING, "ERROR: string sizing in blr_txt.\n");
		return -1;
	}
	...
	tris_verb(4, "blr_txt() FQDN for TXT record: %s, cc was %s\n", domain, cc);

	ret = tris_search_dns(&context, domain, C_IN, T_TXT, txt_callback);

	if (ret > 0) {
		ret = atoi(context.txt);

		if ((ret >= 0) && (ret < 20)) {
			tris_verb(3, "blr_txt() BLR TXT record for %s is %d (apex: %s)\n", cc, ret, suffix);
			return ret;
		}
	}
	
	tris_verb(3, "blr_txt() BLR TXT record for %s not found (apex: %s)\n", cc, suffix);
...
static int ebl_callback(void *context, unsigned char *answer, int len, unsigned char *fullanswer)
{
	...
	if ((c->pos > 15) || len < 2) {	/* illegal packet */
		tris_log(LOG_WARNING, "ebl_callback: malformed EBL record.\n");
		return 0;
	}
	...
	tris_copy_string(c->separator, (char *)answer, i + 1);
	c->sep_len = i;
...
static int blr_ebl(const char *cc, const char *suffix, char *separator, int sep_len, char* apex, int apex_len)
{
	...
	tris_verb(4, "blr_ebl()  cc='%s', suffix='%s', c_bl='%s'\n", cc, suffix, ienum_branchlabel);

	if (sizeof(domain) < (strlen(cc) * 2 + strlen(ienum_branchlabel) + strlen(suffix) + 2)) {
		tris_mutex_unlock(&enumlock);
		tris_log(LOG_WARNING, "ERROR: string sizing in blr_EBL.\n");
		return -1;
	}
	...
	tris_verb(4, "blr_ebl() FQDN for EBL record: %s, cc was %s\n", domain, cc);

	ret = tris_search_dns(&context, domain, C_IN, T_EBL, ebl_callback);
	if (ret > 0) {
		ret = context.pos;

		if ((ret >= 0) && (ret < 20)) {
			tris_verb(3, "blr_txt() BLR EBL record for %s is %d/%s/%s)\n", cc, ret, context.separator, context.apex);
			tris_copy_string(separator, context.separator, sep_len);
			tris_copy_string(apex, context.apex, apex_len);
			return ret;
		}
	}
	tris_verb(3, "blr_txt() BLR EBL record for %s not found (apex: %s)\n", cc, suffix);
	return -1;
}
...
static unsigned int parse_ie(char *data, unsigned int maxdatalen, unsigned char *src, unsigned int srclen)
{
	...
	if (len > srclen) {
		tris_log(LOG_WARNING, "ENUM parsing failed: Wanted %d characters, got %d\n", len, srclen);
		return -1;
	}
...
static int parse_naptr(unsigned char *dst, int dstsize, char *tech, int techsize, unsigned char *answer, int len, unsigned char *naptrinput)
{
	...
	if (len < sizeof(struct naptr)) {
		tris_log(LOG_WARNING, "NAPTR record length too short\n");
		return -1;
	}
	answer += sizeof(struct naptr);
	len -= sizeof(struct naptr);
	if ((res = parse_ie(flags, sizeof(flags) - 1, answer, len)) < 0) {
		tris_log(LOG_WARNING, "Failed to get flags from NAPTR record\n");
		return -1;
	} else {
		answer += res;
		len -= res;
	...
	if ((res = parse_ie(services, sizeof(services) - 1, answer, len)) < 0) {
		tris_log(LOG_WARNING, "Failed to get services from NAPTR record\n");
		return -1;
	} else {
		answer += res;
		len -= res;
	...
	if ((res = parse_ie(regexp, sizeof(regexp) - 1, answer, len)) < 0) {
		tris_log(LOG_WARNING, "Failed to get regexp from NAPTR record\n");
		return -1;
	} else {
		answer += res;
		len -= res;
	...
	tris_debug(3, "NAPTR input='%s', flags='%s', services='%s', regexp='%s', repl='%s'\n",
		naptrinput, flags, services, regexp, repl);


	if (tolower(flags[0]) != 'u') {
		tris_log(LOG_WARNING, "NAPTR Flag must be 'U' or 'u'.\n");
		return -1;
	}
	...
	if (p){
		...
		tris_copy_string(tech_return, p, sizeof(tech_return));
	} else {
		...
		if (p) {
			*p = 0;
			...
			if (p)
				*p = 0;
			tris_copy_string(tech_return, services, sizeof(tech_return));
		}
	}

	regexp_len = strlen(regexp);
	if (regexp_len < 7) {
		tris_log(LOG_WARNING, "Regex too short to be meaningful.\n");
		return -1;
	}
	...
	if ((delim2 == NULL) || (regexp[regexp_len - 1] != delim)) {  /* is the second delimiter found, and is the end of the regexp a delimiter */
		tris_log(LOG_WARNING, "Regex delimiter error (on \"%s\").\n", regexp);
		return -1;
 ...
 */
	...
	if (regcomp(&preg, pattern, REG_EXTENDED | REG_NEWLINE)) {
		tris_log(LOG_WARNING, "NAPTR Regex compilation error (regex = \"%s\").\n", regexp);
		return -1;
	}

	if (preg.re_nsub > ARRAY_LEN(pmatch)) {
		tris_log(LOG_WARNING, "NAPTR Regex compilation error: too many subs.\n");
		regfree(&preg);
		return -1;
	}
	...
	if ((rc = regexec(&preg, (char *) naptrinput, max_bt, pmatch, 0))) {
		...
		tris_log(LOG_WARNING, "NAPTR Regex match failed. Reason: %s\n", errbuff);
		regfree(&preg);
		return -1;
	}
	regfree(&preg);
	...
	while (*subst && (d_len > 0)) {
		if ((subst[0] == '\\') && isdigit(subst[1])) { /* is this character the beginning of a backreference */
			...
			if (matchindex >= ARRAY_LEN(pmatch)) {
				tris_log(LOG_WARNING, "Error during regex substitution. Invalid pmatch index.\n");
				return -1;
			}
			/* pmatch len is 10. we are garanteed a single char 0-9 is a valid index */
			size = pmatch[matchindex].rm_eo - pmatch[matchindex].rm_so;
			if (size > d_len) {
				tris_log(LOG_WARNING, "Not enough space during NAPTR regex substitution.\n");
				return -1;
			}
			...
			if ((strlen((char *) naptrinput) >= pmatch[matchindex].rm_eo) && (pmatch[matchindex].rm_so <= pmatch[matchindex].rm_eo)) {
				...
				tris_log(LOG_WARNING, "Error during regex substitution. Invalid backreference index.\n");
				return -1;
			}
			...
			*d++ = *subst++;
			...
			tris_log(LOG_WARNING, "Error during regex substitution.\n");
			return -1;
		}
	}
	*d = 0;
	tris_copy_string((char *) dst, tempdst, dstsize);
	dst[dstsize - 1] = '\0';
	...
	if (*tech != '\0'){ /* check if it is requested NAPTR */
		...
		if (!strncasecmp(tech_return, tech, sizeof(tech_return) < techsize ? sizeof(tech_return): techsize)){
			tris_copy_string(tech, tech_return, techsize);
...
static int enum_callback(void *context, unsigned char *answer, int len, unsigned char *fullanswer)
{
	...
	if (res < 0) {
		tris_log(LOG_WARNING, "Failed to parse naptr\n");
		return -1;
	} else if ((res == 0) && !tris_strlen_zero(c->dst)) { /* ok, we got needed NAPTR */
		if (c->options & ENUMLOOKUP_OPTIONS_COUNT) { /* counting RRs */
			c->count++;
			snprintf(c->dst, c->dstlen, "%d", c->count);
		} else  {
			if ((p = tris_realloc(c->naptr_rrs, sizeof(*c->naptr_rrs) * (c->naptr_rrs_count + 1)))) {
				c->naptr_rrs = p;
				memcpy(&c->naptr_rrs[c->naptr_rrs_count].naptr, answer, sizeof(c->naptr_rrs->naptr));
				c->naptr_rrs[c->naptr_rrs_count].result = tris_strdup(c->dst);
				c->naptr_rrs[c->naptr_rrs_count].tech = tris_strdup(c->tech);
...
/* ENUM lookup */
int tris_get_enum(struct tris_channel *chan, const char *number, char *dst, int dstlen, char *tech, int techlen, char* suffix, char* options, unsigned int record, struct enum_context **argcontext)
{
	...
	if (tris_strlen_zero(suffix)) {
		tris_log(LOG_WARNING, "tris_get_enum need a suffix parameter now.\n");
		return -1;
	}

	tris_verb(2, "tris_get_enum(num='%s', tech='%s', suffix='%s', options='%s', record=%d\n", number, tech, suffix, options, record);

/*
  We don't need that any more, that "n" preceding the number has been replaced by a flag
  in the options paramter.
	tris_copy_string(naptrinput, number, sizeof(naptrinput));
*/
/*
...
*/
	tris_copy_string(naptrinput, number[0] == 'n' ? number + 1 : number, sizeof(naptrinput));
	if (number[0] == '+') {
		number++;
	}

	if (!(context = tris_calloc(1, sizeof(*context))))
		return -1;
	...
	tris_verb(2, "ENUM options(%s): pos=%d, options='%d'\n", options, context->position, context->options);
	tris_debug(1, "tris_get_enum(): n='%s', tech='%s', suffix='%s', options='%d', record='%d'\n",
	...
	tris_copy_string(left, number, sizeof(left));
	middle[0] = '\0';
	/*
	 * I-ENUM can change the apex, thus we copy it 
	 */
	tris_copy_string(apex, suffix, sizeof(apex));
	/* ISN rewrite */
	if ((context->options & ENUMLOOKUP_OPTIONS_ISN) && (p1 = strchr(number, '*'))) {
		*p1++ = '\0';
		tris_copy_string(left, number, sizeof(left));
		tris_copy_string(middle, p1, sizeof(middle) - 1);
		strcat(middle, ".");

		tris_verb(2, "ISN ENUM: left=%s, middle='%s'\n", left, middle);
	/* Direct DNS lookup rewrite */
	} else if (context->options & ENUMLOOKUP_OPTIONS_DIRECT) {
		left[0] = 0; /* nothing to flip around */
		tris_copy_string(middle, number, sizeof(middle) - 1);
		strcat(middle, ".");
 
		tris_verb(2, "DIRECT ENUM:  middle='%s'\n", middle);
	/* Infrastructure ENUM rewrite */
		...
		sdl = cc_len;
		tris_mutex_lock(&enumlock);
		tris_copy_string(sep, ienum_branchlabel, sizeof(sep)); /* default */
		tris_mutex_unlock(&enumlock);

		switch (ebl_alg) {
		case ENUMLOOKUP_BLR_EBL:
			tris_copy_string(cc, number, cc_len); /* cclen() never returns more than 3 */
			sdl = blr_ebl(cc, suffix, sep, sizeof(sep) - 1, n_apex, sizeof(n_apex) - 1);

			if (sdl >= 0) {
				tris_copy_string(apex, n_apex, sizeof(apex));
				tris_verb(2, "EBL ENUM: sep=%s, apex='%s'\n", sep, n_apex);
			} else {
				sdl = cc_len;
			}
			break;
		...
		if (sdl > strlen(number)) {	/* Number too short for this sdl? */
			tris_log(LOG_WARNING, "I-ENUM: subdomain location %d behind number %s\n", sdl, number);
			return 0;
		}
		tris_copy_string(left, number + sdl, sizeof(left));

		tris_mutex_lock(&enumlock);
		tris_copy_string(middle, sep, sizeof(middle) - 1);
		strcat(middle, ".");
		tris_mutex_unlock(&enumlock);

		/* check the space we need for middle */
		if ((sdl * 2 + strlen(middle) + 2) > sizeof(middle)) {
			tris_log(LOG_WARNING, "tris_get_enum: not enough space for I-ENUM rewrite.\n");
			return -1;
		}
		...
		*p1 = '\0';
		...
		tris_verb(2, "I-ENUM: cclen=%d, left=%s, middle='%s', apex='%s'\n", cc_len, left, middle, apex);
	}

	if (strlen(left) * 2 + 2 > sizeof(domain)) {
		tris_log(LOG_WARNING, "string to long in tris_get_enum\n");
		return -1;
	}
	...
	*p1 = '\0';
	...
	tris_copy_string(tmp, domain, spaceleft);
	...
	time_start = tris_tvnow();
	ret = tris_search_dns(context, tmp, C_IN, T_NAPTR, enum_callback);
	time_end = tris_tvnow();

	tris_verb(2, "tris_get_enum() profiling: %s, %s, %d ms\n", 
			(ret == 0) ? "OK" : "FAIL", tmp, tris_tvdiff_ms(time_end, time_start));

	if (ret < 0) {
		tris_debug(1, "No such number found: %s (%s)\n", tmp, strerror(errno));
		strcpy(dst, "0");
		ret = 0;
	}
	...
	if (context->naptr_rrs_count >= context->position && ! (context->options & ENUMLOOKUP_OPTIONS_COUNT)) {
		...
		for (k = 0; k < context->naptr_rrs_count; k++) {
			if (context->naptr_rrs[k].sort_pos == context->position - 1) {
				tris_copy_string(context->dst, context->naptr_rrs[k].result, dstlen);
				tris_copy_string(context->tech, context->naptr_rrs[k].tech, techlen);
				break;
			}
		}
	...
	if (!argcontext) {
		for (k = 0; k < context->naptr_rrs_count; k++) {
			tris_free(context->naptr_rrs[k].result);
			tris_free(context->naptr_rrs[k].tech);
		}
		tris_free(context->naptr_rrs);
		tris_free(context);
	} else
		*argcontext = context;
...
int tris_get_txt(struct tris_channel *chan, const char *number, char *txt, int txtlen, char *suffix)
{
	struct txt_context context;
	char tmp[259 + 512];
	...
	tris_debug(4, "tris_get_txt: Number = '%s', suffix = '%s'\n", number, suffix);
	...
	tris_copy_string(&tmp[newpos], suffix, sizeof(tmp) - newpos);

	if (ret < 0) {
		tris_debug(2, "No such number found in ENUM: %s (%s)\n", tmp, strerror(errno));
		ret = 0;
	} else {
		tris_copy_string(txt, context.txt, txtlen);
	}
	if (chan) {
		ret |= tris_autoservice_stop(chan);
	}
...
static int private_enum_init(int reload)
{
	...
	if ((cfg = tris_config_load2("enum.conf", "enum", config_flags)) == CONFIG_STATUS_FILEUNCHANGED)
		return 0;
	...
	tris_mutex_lock(&enumlock);
	if (cfg) {
		if ((string = tris_variable_retrieve(cfg, "ienum", "branchlabel"))) {
			tris_copy_string(ienum_branchlabel, string, sizeof(ienum_branchlabel));
		}

		if ((string = tris_variable_retrieve(cfg, "ienum", "ebl_alg"))) {
			...
			if (!strcasecmp(string, "txt"))
				...
				tris_log(LOG_WARNING, "No valid parameter for ienum/ebl_alg.\n");
		}
...
int tris_enum_init(void)
{
	return private_enum_init(0);
}

int tris_enum_reload(void)
{
