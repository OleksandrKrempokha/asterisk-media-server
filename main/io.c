 ...
 */
struct io_rec {
	tris_io_cb callback;		/*!< What is to be called */
...
struct io_context *io_context_create(void)
{
	...
	if (!(tmp->fds = tris_calloc(1, (GROW_SHRINK_SIZE / 2) * sizeof(*tmp->fds)))) {
		tris_free(tmp);
		tmp = NULL;
	} else {
		if (!(tmp->ior = tris_calloc(1, (GROW_SHRINK_SIZE / 2) * sizeof(*tmp->ior)))) {
			tris_free(tmp->fds);
			tris_free(tmp);
			tmp = NULL;
		}
...
static int io_grow(struct io_context *ioc)
{
	...
	DEBUG(tris_debug(1, "io_grow()\n"));

	ioc->maxfdcnt += GROW_SHRINK_SIZE;

	if ((tmp = tris_realloc(ioc->ior, (ioc->maxfdcnt + 1) * sizeof(*ioc->ior)))) {
		ioc->ior = tmp;
		if ((tmp = tris_realloc(ioc->fds, (ioc->maxfdcnt + 1) * sizeof(*ioc->fds)))) {
			ioc->fds = tmp;
		} else {
			/*
...
int *tris_io_add(struct io_context *ioc, int fd, tris_io_cb callback, short events, void *data)
{
	...
	DEBUG(tris_debug(1, "tris_io_add()\n"));
	...
	if (!(ioc->ior[ioc->fdcnt].id = tris_malloc(sizeof(*ioc->ior[ioc->fdcnt].id)))) {
...
int tris_io_remove(struct io_context *ioc, int *_id)
{
	int x;

	if (!_id) {
		tris_log(LOG_WARNING, "Asked to remove NULL?\n");
		return -1;
	}
	...
	tris_log(LOG_NOTICE, "Unable to remove unknown id %p\n", _id);
...
int tris_io_wait(struct io_context *ioc, int howlong)
{
	...
	DEBUG(tris_debug(1, "tris_io_wait()\n"));

	if ((res = tris_poll(ioc->fds, ioc->fdcnt, howlong)) <= 0) {
		return res;
	}
		...
		if (ioc->fds[x].revents && ioc->ior[x].id) {
			...
			if (ioc->ior[x].callback) {
				if (!ioc->ior[x].callback(ioc->ior[x].id, ioc->fds[x].fd, ioc->fds[x].revents, ioc->ior[x].data)) {
					...
					tris_io_remove(ioc, ioc->ior[x].id);
				}
			}
			ioc->current_ioc = -1;
		}
...
void tris_io_dump(struct io_context *ioc)
{
	...
	tris_debug(1, "Trismedia IO Dump: %d entries, %d max entries\n", ioc->fdcnt, ioc->maxfdcnt);
	tris_debug(1, "================================================\n");
	tris_debug(1, "| ID    FD     Callback    Data        Events  |\n");
	tris_debug(1, "+------+------+-----------+-----------+--------+\n");
	for (x = 0; x < ioc->fdcnt; x++) {
		tris_debug(1, "| %.4d | %.4d | %p | %p | %.6x |\n", 
				*ioc->ior[x].id,
				ioc->fds[x].fd,
				ioc->ior[x].callback,
				ioc->ior[x].data,
...
int tris_restore_tty(int fd, int oldstate)
{
	int res;
	struct termios tios;
	if (oldstate < 0)
