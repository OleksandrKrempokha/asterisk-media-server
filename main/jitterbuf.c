...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 174584 $")
...
static int queue_put(jitterbuf *jb, void *data, const enum jb_frame_type type, long ms, long ts) 
{
	...
	if ((frame = jb->free)) {
		jb->free = frame->next;
	} else if (!(frame = tris_malloc(sizeof(*frame)))) {
...
enum jb_return_code jb_put(jitterbuf *jb, void *data, const enum jb_frame_type type, long ms, long ts, long now) 
{
	...
	if (numts >= jb->info.conf.max_jitterbuf) {
		if (!jb->dropem) {
			tris_debug(1, "Attempting to exceed Jitterbuf max %ld timeslots\n",
...
static enum jb_return_code _jb_get(jitterbuf *jb, jb_frame *frameout, long now, long interpl) 
{
	...
	if (!jb->info.silence_begin_ts) { 
		...
		if (!frame) {
			 ...
			 * iaxclient talking to app_echo on trismedia.  In this case,
			 * my outgoing packets go through trismedia's (old)
