...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 189540 $")
...
static struct tris_tm *	gmtsub P((const struct timeval * timep, long offset,
				struct tris_tm * tmp));
static struct tris_tm *	localsub P((const struct timeval * timep, long offset,
				struct tris_tm * tmp, const struct state *sp));
...
static int		normalize_overflow P((int * tensptr, int * unitsptr,
				const int base));
static struct timeval	time1 P((struct tris_tm * tmp,
				struct tris_tm * (*funcp) P((const struct timeval *,
				long, struct tris_tm *, const struct state *sp)),
				long offset, const struct state *sp));
static struct timeval	time2 P((struct tris_tm *tmp,
				struct tris_tm * (*funcp) P((const struct timeval *,
				long, struct tris_tm*, const struct state *sp)),
				long offset, int * okayp, const struct state *sp));
static struct timeval	time2sub P((struct tris_tm *tmp,
				struct tris_tm * (*funcp) (const struct timeval *,
				long, struct tris_tm*, const struct state *sp),
				long offset, int * okayp, int do_norm_secs, const struct state *sp));
static struct tris_tm *	timesub P((const struct timeval * timep, long offset,
				const struct state * sp, struct tris_tm * tmp));
static int		tmcomp P((const struct tris_tm * atmp,
				const struct tris_tm * btmp));
...
static TRIS_LIST_HEAD_STATIC(zonelist, state);

#ifndef TZ_STRLEN_MAX
#define TZ_STRLEN_MAX 255
#endif /* !defined TZ_STRLEN_MAX */

static pthread_t inotify_thread = TRIS_PTHREADT_NULL;
static tris_cond_t initialization;
static tris_mutex_t initialization_lock;
#ifdef HAVE_INOTIFY
...
static void *inotify_daemon(void *data)
{
	...
	tris_mutex_lock(&initialization_lock);
	tris_cond_signal(&initialization);
	tris_mutex_unlock(&initialization_lock);

	if (inotify_fd < 0) {
		tris_log(LOG_ERROR, "Cannot initialize file notification service: %s (%d)\n", strerror(errno), errno);
		inotify_thread = TRIS_PTHREADT_NULL;
		return NULL;
	}
	...
	for (;/*ever*/;) {
		...
		if ((res = read(inotify_fd, &buf, sizeof(buf))) < sizeof(buf.iev) && res > 0) {
			...
			tris_log(LOG_ERROR, "Inotify read less than a full event (%zd < %zd)?!!\n", res, sizeof(buf.iev));
			break;
		} else if (res < 0) {
			...
			tris_log(LOG_ERROR, "Inotify failed: %s\n", strerror(errno));
			break;
		}
		TRIS_LIST_LOCK(&zonelist);
		TRIS_LIST_TRAVERSE_SAFE_BEGIN(&zonelist, cur, list) {
...
{
	if (inotify_thread == TRIS_PTHREADT_NULL) {
		tris_cond_init(&initialization, NULL);
		tris_mutex_init(&initialization_lock);
		tris_mutex_lock(&initialization_lock);
		if (!(tris_pthread_create_background(&inotify_thread, NULL, inotify_daemon, NULL))) {
			/* Give the thread a chance to initialize */
			tris_cond_wait(&initialization, &initialization_lock);
		} else {
			tris_log(LOG_ERROR, "Unable to start notification thread\n");
			tris_mutex_unlock(&initialization_lock);
			return;
		}
		tris_mutex_unlock(&initialization_lock);
...
static void add_notify(struct state *sp, const char *path)
{
	...
	if (inotify_thread == TRIS_PTHREADT_NULL) {
		...
		tris_mutex_init(&initialization_lock);
		tris_mutex_lock(&initialization_lock);
		if (!(tris_pthread_create_background(&inotify_thread, NULL, notify_daemon, NULL))) {
			/* Give the thread a chance to initialize */
			tris_cond_wait(&initialization, &initialization_lock);
		}
		tris_mutex_unlock(&initialization_lock);
...
static const struct state *tris_tzset(const char *zone)
{
	struct state *sp;

	if (tris_strlen_zero(zone))
		zone = "/etc/localtime";

	TRIS_LIST_LOCK(&zonelist);
	TRIS_LIST_TRAVERSE(&zonelist, sp, list) {
	...
	tris_copy_string(sp->name, zone, sizeof(sp->name));
	TRIS_LIST_LOCK(&zonelist);
	TRIS_LIST_INSERT_TAIL(&zonelist, sp, list);
	TRIS_LIST_UNLOCK(&zonelist);
	return sp;
}
...
static struct tris_tm *localsub(const struct timeval *timep, const long offset, struct tris_tm *tmp, const struct state *sp)
{
	const struct ttinfo *	ttisp;
	int			i;
	struct tris_tm *		result;
	struct timeval	t;
...
struct tris_tm *tris_localtime(const struct timeval *timep, struct tris_tm *tmp, const char *zone)
{
	const struct state *sp = tris_tzset(zone);
...
void tris_get_dst_info(const time_t * const timep, int *dst_enabled, time_t *dst_start, time_t *dst_end, int *gmt_off, const char * const zone)
{
	int i;	
	int transition1 = -1;
	int transition2 = -1;
...
static struct tris_tm *gmtsub(const struct timeval *timep, const long offset, struct tris_tm *tmp)
{
	struct tris_tm *	result;
	struct state *sp;
	...
	if (!sp) {
		if (!(sp = (struct state *) tris_calloc(1, sizeof *sp)))
			return NULL;
		gmtload(sp);
		TRIS_LIST_INSERT_TAIL(&zonelist, sp, list);
	}
...
static struct tris_tm *timesub(const struct timeval *timep, const long offset, const struct state *sp, struct tris_tm *tmp)
{
...
static int tmcomp(const struct tris_tm *atmp, const struct tris_tm *btmp)
{
	int	result;
...
static struct timeval time2sub(struct tris_tm *tmp, struct tris_tm * (* const funcp) (const struct timeval *, long, struct tris_tm *, const struct state *), const long offset, int *okayp, const int do_norm_secs, const struct state *sp)
{
	int			dir;
	int			i, j;
	int			saved_seconds;
	...
	*/
	...
	for ( ; ; ) {
		...
		if ((*funcp)(&t, offset, &mytm, sp) == NULL) {
			...
			** a struct tris_tm; arrange things so that it is less
...
static struct timeval time2(struct tris_tm *tmp, struct tris_tm * (* const funcp) (const struct timeval *, long, struct tris_tm*, const struct state *sp), const long offset, int *okayp, const struct state *sp)
{
	struct timeval	t;
...
static struct timeval time1(struct tris_tm *tmp, struct tris_tm * (* const funcp) (const struct timeval *, long, struct tris_tm *, const struct state *), const long offset, const struct state *sp)
{
	struct timeval			t;
	int			samei, otheri;
	...
	** and did some math on it that yielded a "struct tris_tm" that's bad.
...
struct timeval tris_mktime(struct tris_tm *tmp, const char *zone)
{
	const struct state *sp;
	if (!(sp = tris_tzset(zone)))
		return WRONG;
	return time1(tmp, localsub, 0L, sp);
}

int tris_strftime(char *buf, size_t len, const char *tmp, const struct tris_tm *tm)
{
	size_t fmtlen = strlen(tmp) + 1;
	char *format = tris_calloc(1, fmtlen), *fptr = format, *newfmt;
	...
	for (; *tmp; tmp++) {
		if (*tmp == '%') {
			...
			case 'q': /* Milliseconds */
				...
				newfmt = tris_realloc(format, fmtlen + decimals);
				if (!newfmt) {
					tris_free(format);
					return -1;
				}
...
char *tris_strptime(const char *s, const char *format, struct tris_tm *tm)
{
	struct tm tm2 = { 0, };
	char *res = strptime(s, format, &tm2);
	memcpy(tm, &tm2, sizeof(*tm));
	tm->tm_usec = 0;
	/* strptime(3) doesn't set .tm_isdst correctly, so to force tris_mktime(3)
