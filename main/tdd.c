...
int tris_tdd_gen_ecdisa(unsigned char *outbuf, int len)
{
	int pos = 0;
	int cnt;
	while (len) {
...
int tdd_feed(struct tdd_state *tdd, unsigned char *ubuf, int len)
{
	...
	mylen += tdd->oldlen / 2;
	for (x = 0; x < len; x++) 
		buf[x + tdd->oldlen / 2] = TRIS_MULAW(ubuf[x]);
	c = res = 0;
	while (mylen >= 1320) { /* has to have enough to work on */
		olen = mylen;
		res = fsk_serial(&tdd->fskd, buf, &mylen, &b);
		if (mylen < 0) {
			tris_log(LOG_ERROR, "fsk_serial made mylen < 0 (%d) (olen was %d)\n", mylen, olen);
			free(obuf);
			return -1;
		}
		buf += (olen - mylen);
