...
int tris_term_init(void)
{
	...
	if (!term)
		return 0;
	if (!tris_opt_console || tris_opt_no_color || !tris_opt_no_fork)
		return 0;
	...
	if (termfd > -1) {
		...
		if (sz_names + sz_bools + n_nums < actsize) {
			 ...
			 * include it here, as it conflicts with include/trismedia/term.h */
	...
	if (vt100compat) {
		...
		} else if (tris_opt_force_black_background) {
...
char *term_color(char *outbuf, const char *inbuf, int fgcolor, int bgcolor, int maxout)
{
	...
	if (!vt100compat) {
		tris_copy_string(outbuf, inbuf, maxout);
		return outbuf;
	}
	if (!fgcolor) {
		tris_copy_string(outbuf, inbuf, maxout);
	...
	if (fgcolor & 128) {
		attr = tris_opt_light_background ? 0 : ATTR_BRIGHT;
		fgcolor &= ~128;
	}
	...
	if (tris_opt_force_black_background) {
...
int tris_term_color_code(struct tris_str **str, int fgcolor, int bgcolor)
{
	int attr = 0;
	...
	if (tris_opt_force_black_background) {
		tris_str_append(str, 0, "%c[%d;%d;%dm", ESC, attr, fgcolor, COLOR_BLACK + 10);
	} else if (bgcolor) {
		tris_str_append(str, 0, "%c[%d;%d;%dm", ESC, attr, fgcolor, bgcolor + 10);
	} else {
		tris_str_append(str, 0, "%c[%d;%dm", ESC, attr, fgcolor);
	}
...
char *term_prompt(char *outbuf, const char *inbuf, int maxout)
{
	...
	if (tris_opt_force_black_background) {
		snprintf(outbuf, maxout, "%c[%d;%dm%c%c[%d;%dm%s",
			...
			inbuf + 1);
	} else if (tris_opt_light_background) {
