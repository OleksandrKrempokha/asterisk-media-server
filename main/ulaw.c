...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 213182 $")
 ...
 * ZEROTRAP is not implemented in TRIS_LIN2MU and so the coding table
...
unsigned char __tris_lin2mu[16384];
short __tris_mulaw[256];
...
unsigned char __tris_lin2mu[TRIS_ULAW_TAB_SIZE];
short __tris_mulaw[256];
...
void tris_ulaw_init(void)
{
	...
	for (i = -32768; i < 32768; i++) {
		__tris_lin2mu[((unsigned short)i) >> 2] = linear2ulaw(i);
	}
#else
	
	for (i = 0; i < 256; i++) {
		__tris_mulaw[i] = ulaw2linear(i);
	}
	/* set up the reverse (mu-law) conversion table */
	for (i = 0; i <= 32768; i += TRIS_ULAW_STEP) {
		TRIS_LIN2MU_LOOKUP(i) = linear2ulaw(i, 0 /* half-cooked */);
	}
#endif
	...
	for (i = -32768; i < 32768; ++i) {
		...
		unsigned char e2 = TRIS_LIN2MU(i);
		short d2 = ulaw2linear(e2);
		short d3 = TRIS_MULAW(e1);
		
		if (e1 != e2 || d1 != d3 || d2 != d3) {
			tris_log(LOG_WARNING, "u-Law coding tables test failed on %d: e1=%u, e2=%u, d1=%d, d2=%d\n",
					i, (unsigned)e1, (unsigned)e2, (int)d1, (int)d2);
		}
	}
	tris_log(LOG_NOTICE, "u-Law coding table test complete.\n");
#endif /* TEST_CODING_TABLES */
	
#ifdef TEST_TANDEM_TRANSCODING
	/* tandem transcoding test */
	for (i = -32768; i < 32768; ++i) {
		unsigned char e1 = TRIS_LIN2MU(i);
		short d1 = TRIS_MULAW(e1);
		unsigned char e2 = TRIS_LIN2MU(d1);
		short d2 = TRIS_MULAW(e2);
		unsigned char e3 = TRIS_LIN2MU(d2);
		short d3 = TRIS_MULAW(e3);
		
		if (i < 0 && e1 == 0x7f && e2 == 0xff && e3 == 0xff)
			continue; /* known and normal negative 0 case */
		
		if (e1 != e2 || e2 != e3 || d1 != d2 || d2 != d3) {
			tris_log(LOG_WARNING, "u-Law tandem transcoding test failed on %d: e1=%u, e2=%u, d1=%d, d2=%d, d3=%d\n",
					i, (unsigned)e1, (unsigned)e2, (int)d1, (int)d2, (int)d3);
		}
	}
	tris_log(LOG_NOTICE, "u-Law tandem transcoding test complete.\n");
