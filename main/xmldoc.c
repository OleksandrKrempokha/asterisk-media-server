...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 196951 $")
...
struct documentation_tree {
	...
	struct tris_xml_doc *doc;			/*!< Open document pointer. */
	TRIS_RWLIST_ENTRY(documentation_tree) entry;
};

static char *xmldoc_get_syntax_cmd(struct tris_xml_node *fixnode, const char *name, int printname);
static int xmldoc_parse_enumlist(struct tris_xml_node *fixnode, const char *tabs, struct tris_str **buffer);
 ...
 */
static TRIS_RWLIST_HEAD_STATIC(xmldoc_tree, documentation_tree);
...
static char *xmldoc_string_wrap(const char *text, int columns, int maxdiff)
{
	...
	if (!text || columns <= 0 || maxdiff < 0) {
		tris_log(LOG_WARNING, "Passing wrong arguments while trying to wrap the text\n");
		return NULL;
	}

	tmp = tris_str_create(strlen(text) * 3);
		...
		if (needtobreak || !(count % colmax)) {
			if (text[i] == ' ') {
				...
				if (xmldoc_wait_nextspace(text, i, maxdiff)) {
					...
					tris_str_append(&tmp, 0, "%c", text[i]);
					continue;
				}
				...
				if (backspace) {
					needtobreak = 1;
					tris_str_truncate(tmp, -backspace);
					i -= backspace + 1;
					continue;
				}
				tris_str_append(&tmp, 0, "\n%s", postbr);
	...
	ret = tris_strdup(tris_str_buffer(tmp));
	tris_free(tmp);
...
char *tris_xmldoc_printable(const char *bwinput, int withcolors)
{
	...
	if (!(colorized = tris_str_create(256))) {
		return NULL;
	}

	if (withcolors) {
		tris_term_color_code(&colorized, base_fg, 0);
		if (!colorized) {
			return NULL;
		}
	}
	...
	for (i = 0; i < bwinputlen; i++) {
		...
		for (c = 0; c < ARRAY_LEN(colorized_tags); c++) {
			...
			/* Setup color */
			if (withcolors) {
				tris_term_color_code(&colorized, colorized_tags[c].colorfg, 0);
				if (!colorized) {
					return NULL;
				}
			}
			...
			tris_str_append(&colorized, 0, "%s", colorized_tags[c].init);
			if (!colorized) {
				return NULL;
			}
			{
				char buf[len + 1];
				tris_copy_string(buf, bwinput + i + strlen(colorized_tags[c].inittag), sizeof(buf));
				tris_str_append(&colorized, 0, "%s", buf);
			}
			if (!colorized) {
				return NULL;
			}
			...
			tris_str_append(&colorized, 0, "%s", colorized_tags[c].end);
			if (!colorized) {
				return NULL;
			}
		...
		if (!colorsection) {
			tris_str_append(&colorized, 0, "%c", bwinput[i]);
			if (!colorized) {
				return NULL;
			}
		}
	...
	if (withcolors) {
		tris_str_append(&colorized, 0, "%s", term_end());
		if (!colorized) {
			return NULL;
		}
	}
	...
	wrapped = xmldoc_string_wrap(tris_str_buffer(colorized), xmldoc_text_columns, xmldoc_max_diff);
 ...
 */
static void xmldoc_string_cleanup(const char *text, struct tris_str **output, int lastspaces)
{
	int i;
	size_t textlen;
	...
	*output = tris_str_create(textlen);
	if (!(*output)) {
		tris_log(LOG_ERROR, "Problem allocating output buffer\n");
		return;
	}
	...
	for (i = 0; i < textlen; i++) {
		if (text[i] == '\n' || text[i] == '\r') {
			...
			tris_str_append(output, 0, " ");
			continue;
		} else {
			tris_str_append(output, 0, "%c", text[i]);
		}
	}

	/* remove last spaces (we dont want always to remove the trailing spaces). */
	if (lastspaces) {
		tris_str_trim_blanks(*output);
	}
}
 ...
 *  \retval NULL on error.
 *  \retval A node of type tris_xml_node.
 */
static struct tris_xml_node *xmldoc_get_node(const char *type, const char *name, const char *language)
{
	struct tris_xml_node *node = NULL;
	struct documentation_tree *doctree;
	const char *lang;

	TRIS_RWLIST_RDLOCK(&xmldoc_tree);
	TRIS_LIST_TRAVERSE(&xmldoc_tree, doctree, entry) {
		/* the core xml documents have priority over thirdparty document. */
		node = tris_xml_get_root(doctree->doc);
		while ((node = tris_xml_find_element(node, type, "name", name))) {
			/* Check language */
			lang = tris_xml_get_attribute(node, "language");
			if (lang && !strcmp(lang, language)) {
				tris_xml_free_attr(lang);
				break;
			} else if (lang) {
				tris_xml_free_attr(lang);
			}
		...
		if (node && tris_xml_node_get_children(node)) {
			break;
		}
		...
		node = tris_xml_get_root(doctree->doc);
		if (tris_xml_node_get_children(node)) {
			if ((node = tris_xml_find_element(tris_xml_node_get_children(node), type, "name", name))) {
				break;
			}
		}
	}
	TRIS_RWLIST_UNLOCK(&xmldoc_tree);
...
static void __attribute__((format(printf, 4, 5))) xmldoc_reverse_helper(int reverse, int *len, char **syntax, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	if (tris_vasprintf(&tmpfmt, fmt, ap) < 0) {
		va_end(ap);
		return;
	}
	va_end(ap);
	...
	*syntax = tris_realloc(*syntax, totlen);
 ...
 */
static int xmldoc_has_inside(struct tris_xml_node *fixnode, const char *what)
{
	struct tris_xml_node *node = fixnode;

	for (node = tris_xml_node_get_children(fixnode); node; node = tris_xml_node_get_next(node)) {
		if (!strcasecmp(tris_xml_node_get_name(node), what)) {
			return 1;
		}
	}
	return 0;
 ...
 */
static int xmldoc_has_nodes(struct tris_xml_node *fixnode)
{
	struct tris_xml_node *node = fixnode;

	for (node = tris_xml_node_get_children(fixnode); node; node = tris_xml_node_get_next(node)) {
		if (strcasecmp(tris_xml_node_get_name(node), "text")) {
			return 1;
		}
	}
	return 0;
 ...
 */
static int xmldoc_has_specialtags(struct tris_xml_node *fixnode)
{
	struct tris_xml_node *node = fixnode;
	int i;
	...
	for (node = tris_xml_node_get_children(fixnode); node; node = tris_xml_node_get_next(node)) {
		for (i = 0; i < ARRAY_LEN(special_tags); i++) {
			if (!strcasecmp(tris_xml_node_get_name(node), special_tags[i].tagname)) {
				return 1;
			}
		}
	}
 ...
 *  \param rootnode A pointer to the tris_xml root node.
 ...
 *  \retval NULL on error.
 *  \retval An tris_malloc'ed string with the syntax generated.
 */
static char *xmldoc_get_syntax_fun(struct tris_xml_node *rootnode, const char *rootname, const char *childname, int printparenthesis, int printrootname)
{
#define GOTONEXT(__rev, __a) (__rev ? tris_xml_node_get_prev(__a) : tris_xml_node_get_next(__a))
#define ISLAST(__rev, __a)  (__rev == 1 ? (tris_xml_node_get_prev(__a) ? 0 : 1) : (tris_xml_node_get_next(__a) ? 0 : 1))
#define MP(__a) ((multiple ? __a : ""))
	struct tris_xml_node *node = NULL, *firstparam = NULL, *lastparam = NULL;
	...
	if (tris_strlen_zero(rootname) || tris_strlen_zero(childname)) {
		tris_log(LOG_WARNING, "Tried to look in XML tree with faulty rootname or childname while creating a syntax.\n");
		return NULL;
	}

	if (!rootnode || !tris_xml_node_get_children(rootnode)) {
		/* If the rootnode field is not found, at least print name. */
		tris_asprintf(&syntax, "%s%s", (printrootname ? rootname : ""), (printparenthesis ? "()" : ""));
		return syntax;
	}

	/* Get the argument separator from the root node attribute name 'argsep', if not found
	defaults to ','. */
	attrargsep = tris_xml_get_attribute(rootnode, "argsep");
	if (attrargsep) {
		argsep = tris_strdupa(attrargsep);
		tris_xml_free_attr(attrargsep);
	} else {
		argsep = tris_strdupa(",");
	}

	/* Get order of evaluation. */
	for (node = tris_xml_node_get_children(rootnode); node; node = tris_xml_node_get_next(node)) {
		if (strcasecmp(tris_xml_node_get_name(node), childname)) {
			continue;
		}
		required = 0;
		hasparams = 1;
		if ((paramtype = tris_xml_get_attribute(node, "required"))) {
			if (tris_true(paramtype)) {
				required = 1;
			}
			tris_xml_free_attr(paramtype);
		}
	...
	for (; node; node = GOTONEXT(reverse, node)) {
		...
		if (xmldoc_has_inside(node, "argument")) {
			parenthesis = tris_xml_get_attribute(node, "hasparams");
			prnparenthesis = 0;
			if (parenthesis) {
				prnparenthesis = tris_true(parenthesis);
				if (!strcasecmp(parenthesis, "optional")) {
					prnparenthesis = 2;
				}
				tris_xml_free_attr(parenthesis);
			}
			argname = tris_xml_get_attribute(node, "name");
			if (argname) {
				paramname = xmldoc_get_syntax_fun(node, argname, "argument", prnparenthesis, prnparenthesis);
				tris_xml_free_attr(argname);
			} else {
				/* Malformed XML, print **UNKOWN** */
				paramname = tris_strdup("**unknown**");
			}
		} else {
			paramnameattr = tris_xml_get_attribute(node, "name");
			if (!paramnameattr) {
				tris_log(LOG_WARNING, "Malformed XML %s: no %s name\n", rootname, childname);
				if (syntax) {
			...
			paramname = tris_strdup(paramnameattr);
			tris_xml_free_attr(paramnameattr);
		}

		/* Defaults to 'false'. */
		multiple = 0;
		if ((multipletype = tris_xml_get_attribute(node, "multiple"))) {
			if (tris_true(multipletype)) {
				multiple = 1;
			}
			tris_xml_free_attr(multipletype);
		}
 ...
 */
static char *xmldoc_parse_cmd_enumlist(struct tris_xml_node *fixnode)
{
	struct tris_xml_node *node = fixnode;
	struct tris_str *paramname;
	char *enumname, *ret;
	int first = 1;

	paramname = tris_str_create(128);
	if (!paramname) {
		return tris_strdup("{<unkown>}");
	}

	tris_str_append(&paramname, 0, "{");

	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		if (strcasecmp(tris_xml_node_get_name(node), "enum")) {
			continue;
		}
		...
		tris_str_append(&paramname, 0, "%s", enumname);
		first = 0;
		tris_free(enumname);
	}
	...
	ret = tris_strdup(tris_str_buffer(paramname));
	tris_free(paramname);
...
static char *xmldoc_get_syntax_cmd(struct tris_xml_node *fixnode, const char *name, int printname)
{
	struct tris_str *syntax;
	struct tris_xml_node *tmpnode, *node = fixnode;
	char *ret, *paramname;
	const char *paramtype, *attrname, *literal;
	int required, isenum, first = 1, isliteral;

	syntax = tris_str_create(128);
	if (!syntax) {
		/* at least try to return something... */
		return tris_strdup(name);
	}

	/* append name to output string. */
	if (printname) {
		tris_str_append(&syntax, 0, "%s", name);
		first = 0;
	}

	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		if (strcasecmp(tris_xml_node_get_name(node), "parameter")) {
			continue;
		}
		...
		if (xmldoc_has_inside(node, "parameter")) {
			...
			attrname = tris_xml_get_attribute(node, "name");
			if (!attrname) {
				/* ignore this bogus parameter and continue. */
				continue;
			}
			paramname = tris_strdup(attrname);
			tris_xml_free_attr(attrname);
			isenum = 0;
		} else {
			/* parse enumlist (note that this is a special enumlist
			that is used to describe a syntax like {<param1>|<param2>|...} */
			for (tmpnode = tris_xml_node_get_children(node); tmpnode; tmpnode = tris_xml_node_get_next(tmpnode)) {
				if (!strcasecmp(tris_xml_node_get_name(tmpnode), "enumlist")) {
					break;
				}
			}
		...
		required = 0;
		paramtype = tris_xml_get_attribute(node, "required");
		if (paramtype) {
			required = tris_true(paramtype);
			tris_xml_free_attr(paramtype);
		}

		/* Is this a replaceable value or a fixed parameter value? */
		isliteral = 0;
		literal = tris_xml_get_attribute(node, "literal");
		if (literal) {
			isliteral = tris_true(literal);
			tris_xml_free_attr(literal);
		}
		 ...
		 */
		tris_str_append(&syntax, 0, "%s%s%s%s%s%s",
				(first ? "" : " "),
				(required ? "" : "["),
				(isenum || isliteral ? "" : "<"),
				paramname,
	...
	ret = tris_strdup(tris_str_buffer(syntax));
	tris_free(syntax);
...
char *tris_xmldoc_build_syntax(const char *type, const char *name)
{
	...
	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		if (!strcasecmp(tris_xml_node_get_name(node), "syntax")) {
			break;
		}
	}
 ...
 *  \param buffer This must be an already allocated tris_str. It will be used
 ...
 *  \retval 0 on error.
 */
static int xmldoc_parse_para(struct tris_xml_node *node, const char *tabs, const char *posttabs, struct tris_str **buffer)
{
	const char *tmptext;
	struct tris_xml_node *tmp;
	int ret = 0;
	struct tris_str *tmpstr;

	if (!node || !tris_xml_node_get_children(node)) {
		return ret;
	}

	if (strcasecmp(tris_xml_node_get_name(node), "para")) {
		return ret;
	}

	tris_str_append(buffer, 0, "%s", tabs);

	ret = 1;

	for (tmp = tris_xml_node_get_children(node); tmp; tmp = tris_xml_node_get_next(tmp)) {
		/* Get the text inside the <para> element and append it to buffer. */
		tmptext = tris_xml_get_text(tmp);
		if (tmptext) {
			/* Strip \n etc. */
			xmldoc_string_cleanup(tmptext, &tmpstr, 0);
			tris_xml_free_text(tmptext);
			if (tmpstr) {
				if (strcasecmp(tris_xml_node_get_name(tmp), "text")) {
					tris_str_append(buffer, 0, "<%s>%s</%s>", tris_xml_node_get_name(tmp),
							tris_str_buffer(tmpstr), tris_xml_node_get_name(tmp));
				} else {
					tris_str_append(buffer, 0, "%s", tris_str_buffer(tmpstr));
				}
				tris_free(tmpstr);
				ret = 2;
			}
	...
	tris_str_append(buffer, 0, "%s", posttabs);
 ...
 */
static int xmldoc_parse_specialtags(struct tris_xml_node *fixnode, const char *tabs, const char *posttabs, struct tris_str **buffer)
{
	struct tris_xml_node *node = fixnode;
	int ret = 0, i, count = 0;
	...
	for (i = 0; i < ARRAY_LEN(special_tags); i++) {
		...
		/* concat data */
		if (!tris_strlen_zero(special_tags[i].init)) {
			tris_str_append(buffer, 0, "%s%s", tabs, special_tags[i].init);
		}
		...
		if (!tris_strlen_zero(special_tags[i].end)) {
			tris_str_append(buffer, 0, "%s%s", special_tags[i].end, posttabs);
		}
 ...
 */
static int xmldoc_parse_argument(struct tris_xml_node *fixnode, int insideparameter, const char *paramtabs, const char *tabs, struct tris_str **buffer)
{
	struct tris_xml_node *node = fixnode;
	const char *argname;
	int count = 0, ret = 0;
	...
	if (xmldoc_has_inside(node, "para") || xmldoc_has_specialtags(node)) {
		tris_str_append(buffer, 0, "%s%s%s", tabs, argname, (insideparameter ? "\n" : ""));
		tris_xml_free_attr(argname);
	} else {
		tris_xml_free_attr(argname);
		return 0;
 ...
 *         in buffer.
 *  \param buffer This must be an already created tris_str. It will be used
 *         to store the result (if already has something it will be appended to the current
 *         string).
 *  \retval 0 if no data is appended.
 *  \retval 1 if data is appended.
 */
static int xmldoc_parse_variable(struct tris_xml_node *node, const char *tabs, struct tris_str **buffer)
{
	struct tris_xml_node *tmp;
	const char *valname;
	const char *tmptext;
	struct tris_str *cleanstr;
	...
	for (tmp = tris_xml_node_get_children(node); tmp; tmp = tris_xml_node_get_next(tmp)) {
		...
		if (strcasecmp(tris_xml_node_get_name(tmp), "value")) {
			continue;
		}

		/* Parse a <value> tag only. */
		if (!printedpara) {
			tris_str_append(buffer, 0, "\n");
			printedpara = 1;
		}
		/* Parse each <value name='valuename'>desciption</value> */
		valname = tris_xml_get_attribute(tmp, "name");
		if (valname) {
			ret = 1;
			tris_str_append(buffer, 0, "%s<value>%s</value>", tabs, valname);
			tris_xml_free_attr(valname);
		}
		tmptext = tris_xml_get_text(tmp);
		...
		if (tmptext) {
			...
			tris_xml_free_text(tmptext);
			if (cleanstr && tris_str_strlen(cleanstr) > 0) {
				tris_str_append(buffer, 0, ":%s", tris_str_buffer(cleanstr));
			}
			tris_free(cleanstr);
		}
		tris_str_append(buffer, 0, "\n");
 ...
 *  \retval 0 On error.
 */
static int xmldoc_parse_variablelist(struct tris_xml_node *node, const char *tabs, struct tris_str **buffer)
{
	struct tris_xml_node *tmp;
	const char *varname;
	char *vartabs;
	...
	if (strcasecmp(tris_xml_node_get_name(node), "variablelist")) {
		return ret;
	}

	/* use this spacing (add 4 spaces) inside a variablelist node. */
	tris_asprintf(&vartabs, "%s    ", tabs);
	if (!vartabs) {
		return ret;
	}
	for (tmp = tris_xml_node_get_children(node); tmp; tmp = tris_xml_node_get_next(tmp)) {
		...
		if (!strcasecmp(tris_xml_node_get_name(tmp), "variable")) {
			/* Store the variable name in buffer. */
			varname = tris_xml_get_attribute(tmp, "name");
			if (varname) {
				tris_str_append(buffer, 0, "%s<variable>%s</variable>: ", tabs, varname);
				tris_xml_free_attr(varname);
...
char *tris_xmldoc_build_seealso(const char *type, const char *name)
{
	...
	if (tris_strlen_zero(type) || tris_strlen_zero(name)) {
		return NULL;
	}
	...
	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		if (!strcasecmp(tris_xml_node_get_name(node), "see-also")) {
			break;
		}
	}
	...
	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		if (strcasecmp(tris_xml_node_get_name(node), "ref")) {
			continue;
		}

		/* parse the <ref> node. 'type' attribute is required. */
		typename = tris_xml_get_attribute(node, "type");
		if (!typename) {
			continue;
		}
		content = tris_xml_get_text(node);
		if (!content) {
			tris_xml_free_attr(typename);
			continue;
		}
		if (!strcasecmp(typename, "application")) {
			tris_str_append(&outputstr, 0, "%s%s()",	(first ? "" : ", "), content);
		} else if (!strcasecmp(typename, "function")) {
			tris_str_append(&outputstr, 0, "%s%s", (first ? "" : ", "), content);
		} else if (!strcasecmp(typename, "astcli")) {
			tris_str_append(&outputstr, 0, "%s<astcli>%s</astcli>", (first ? "" : ", "), content);
		} else {
			tris_str_append(&outputstr, 0, "%s%s", (first ? "" : ", "), content);
		}
		first = 0;
		tris_xml_free_text(content);
		tris_xml_free_attr(typename);
	}

	output = tris_strdup(tris_str_buffer(outputstr));
	tris_free(outputstr);
 ...
 *  \brief fixnode An tris_xml_node pointer to the <enum> node.
 ...
 */
static int xmldoc_parse_enum(struct tris_xml_node *fixnode, const char *tabs, struct tris_str **buffer)
{
	struct tris_xml_node *node = fixnode;
	int ret = 0;
	char *optiontabs;

	tris_asprintf(&optiontabs, "%s    ", tabs);
 ...
 *  \param fixnode As tris_xml pointer to the <enumlist> node.
 *  \param buffer The tris_str output buffer.
...
static int xmldoc_parse_enumlist(struct tris_xml_node *fixnode, const char *tabs, struct tris_str **buffer)
{
	...
	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		...
		enumname = tris_xml_get_attribute(node, "name");
		if (enumname) {
			tris_str_append(buffer, 0, "%s<enum>%s</enum>", tabs, enumname);
			tris_xml_free_attr(enumname);
 ...
 *  \param fixnode An tris_xml pointer to the <option> node.
 ...
 */
static int xmldoc_parse_option(struct tris_xml_node *fixnode, const char *tabs, struct tris_str **buffer)
{
	struct tris_xml_node *node;
	int ret = 0;
	char *optiontabs;
	...
	for (node = tris_xml_node_get_children(fixnode); node; node = tris_xml_node_get_next(node)) {
		if (!strcasecmp(tris_xml_node_get_name(node), "argument")) {
			/* if this is the first data appended to buffer, print a \n*/
			if (!ret && tris_xml_node_get_children(node)) {
				/* print \n */
				tris_str_append(buffer, 0, "\n");
			}
 ...
 */
static void xmldoc_parse_optionlist(struct tris_xml_node *fixnode, const char *tabs, struct tris_str **buffer)
{
	struct tris_xml_node *node;
	...
	for (node = tris_xml_node_get_children(fixnode); node; node = tris_xml_node_get_next(node)) {
		...
		if (strcasecmp(tris_xml_node_get_name(node), "option")) {
			continue;
		}

		/* Get the option name. */
		optname = tris_xml_get_attribute(node, "name");
		if (!optname) {
			continue;
		}

		optparams = 1;
		hasparams = tris_xml_get_attribute(node, "hasparams");
		...
		if (!optionsyntax) {
			tris_xml_free_attr(optname);
			tris_xml_free_attr(hasparams);
			continue;
		}

		tris_str_append(buffer, 0, "%s%s: ", tabs, optionsyntax);
 ...
 */
static void xmldoc_parse_parameter(struct tris_xml_node *fixnode, const char *tabs, struct tris_str **buffer)
{
	...
	if (!(paramname = tris_xml_get_attribute(node, "name"))) {
		/* parameter MUST have an attribute name. */
		return;
	}

	tris_asprintf(&internaltabs, "%s    ", tabs);
	if (!internaltabs) {
		return;
	}

	if (!hasarguments && xmldoc_has_nodes(node)) {
		tris_str_append(buffer, 0, "%s\n", paramname);
		tris_xml_free_attr(paramname);
		printed = 1;
	}

	for (node = tris_xml_node_get_children(node); node; node = tris_xml_node_get_next(node)) {
		if (!strcasecmp(tris_xml_node_get_name(node), "optionlist")) {
			xmldoc_parse_optionlist(node, internaltabs, buffer);
		} else if (!strcasecmp(tris_xml_node_get_name(node), "enumlist")) {
			xmldoc_parse_enumlist(node, internaltabs, buffer);
		} else if (!strcasecmp(tris_xml_node_get_name(node), "argument")) {
			xmldoc_parse_argument(node, 1, internaltabs, (!hasarguments ? "        " : ""), buffer);
		} else if (!strcasecmp(tris_xml_node_get_name(node), "para")) {
			if (!printed) {
				tris_str_append(buffer, 0, "%s\n", paramname);
				tris_xml_free_attr(paramname);
				printed = 1;
...
char *tris_xmldoc_build_arguments(const char *type, const char *name)
{
	...
	if (tris_str_strlen(ret) > 0) {
		/* remove last '\n' */
		char *buf = tris_str_buffer(ret);
		if (buf[tris_str_strlen(ret) - 1] == '\n') {
			tris_str_truncate(ret, -1);
		}
		retstr = tris_strdup(tris_str_buffer(ret));
	}
	tris_free(ret);
 ...
 */
static struct tris_str *xmldoc_get_formatted(struct tris_xml_node *node, int raw_output, int raw_wrap)
{
	struct tris_xml_node *tmp;
	...
	if (raw_output) {
		notcleanret = tris_xml_get_text(node);
		tmpstr = notcleanret;
		xmldoc_string_cleanup(tris_skip_blanks(notcleanret), &ret, 0);
		tris_xml_free_text(tmpstr);
	} else {
		for (tmp = tris_xml_node_get_children(node); tmp; tmp = tris_xml_node_get_next(tmp)) {
		...
		/* remove last '\n' */
		/* XXX Don't modify tris_str internals manually */
		tmpstr = tris_str_buffer(ret);
		if (tmpstr[tris_str_strlen(ret) - 1] == '\n') {
			tris_str_truncate(ret, -1);
		}
	}
	return ret;
...
/*!
 *  \brief Get the content of a field (synopsis, description, etc) from an trismedia document tree
...
static char *xmldoc_build_field(const char *type, const char *name, const char *var, int raw)
{
	struct tris_xml_node *node;
	char *ret = NULL;
	struct tris_str *formatted;

	if (tris_strlen_zero(type) || tris_strlen_zero(name)) {
		tris_log(LOG_ERROR, "Tried to look in XML tree with faulty values.\n");
		return ret;
	}
	...
	if (!node) {
		tris_log(LOG_WARNING, "Counldn't find %s %s in XML documentation\n", type, name);
		return ret;
	}

	node = tris_xml_find_element(tris_xml_node_get_children(node), var, NULL, NULL);

	if (!node || !tris_xml_node_get_children(node)) {
		tris_log(LOG_DEBUG, "Cannot find variable '%s' in tree '%s'\n", name, var);
		return ret;
	}

	formatted = xmldoc_get_formatted(node, raw, raw);
	if (tris_str_strlen(formatted) > 0) {
		ret = tris_strdup(tris_str_buffer(formatted));
	}
	tris_free(formatted);
...
static void xmldoc_unload_documentation(void)
{
	...
	TRIS_RWLIST_WRLOCK(&xmldoc_tree);
	while ((doctree = TRIS_RWLIST_REMOVE_HEAD(&xmldoc_tree, entry))) {
		tris_free(doctree->filename);
		tris_xml_close(doctree->doc);
	}
	TRIS_RWLIST_UNLOCK(&xmldoc_tree);
...
int tris_xmldoc_load_documentation(void)
{
	...
	if ((cfg = tris_config_load2("trismedia.conf", "" /* core can't reload */, cnfflags)) && cfg != CONFIG_STATUS_FILEINVALID) {
		for (var = tris_variable_browse(cfg, "options"); var; var = var->next) {
	...
	/* register function to be run when trismedia finish. */
	tris_register_atexit(xmldoc_unload_documentation);

	/* Get every *-LANG.xml file inside $(ASTDATADIR)/documentation */
	tris_asprintf(&xmlpattern, "%s/documentation{/thirdparty/,/}*-{%s,%.2s_??,%s}.xml", tris_config_TRIS_DATA_DIR,
	...
	if (globret == GLOB_NOSPACE) {
		tris_log(LOG_WARNING, "Glob Expansion of pattern '%s' failed: Not enough memory\n", xmlpattern);
		tris_free(xmlpattern);
		return 1;
	} else if (globret  == GLOB_ABORTED) {
		tris_log(LOG_WARNING, "Glob Expansion of pattern '%s' failed: Read error\n", xmlpattern);
		tris_free(xmlpattern);
		return 1;
	}
	tris_free(xmlpattern);
	...
	for (i = 0; i < globbuf.gl_pathc; i++) {
		...
		tmpdoc = NULL;
		tmpdoc = tris_xml_open(globbuf.gl_pathv[i]);
		if (!tmpdoc) {
			tris_log(LOG_ERROR, "Could not open XML documentation at '%s'\n", globbuf.gl_pathv[i]);
			continue;
		}
		/* Get doc root node and check if it starts with '<docs>' */
		root_node = tris_xml_get_root(tmpdoc);
		if (!root_node) {
			tris_log(LOG_ERROR, "Error getting documentation root node");
			tris_xml_close(tmpdoc);
			continue;
		}
		/* Check root node name for malformed xmls. */
		if (strcmp(tris_xml_node_get_name(root_node), "docs")) {
			tris_log(LOG_ERROR, "Documentation file is not well formed!\n");
			tris_xml_close(tmpdoc);
			continue;
		}
		doc_tree = tris_calloc(1, sizeof(*doc_tree));
		if (!doc_tree) {
			tris_log(LOG_ERROR, "Unable to allocate documentation_tree structure!\n");
			tris_xml_close(tmpdoc);
			continue;
		}
		doc_tree->doc = tmpdoc;
		doc_tree->filename = tris_strdup(globbuf.gl_pathv[i]);
		TRIS_RWLIST_INSERT_TAIL(&xmldoc_tree, doc_tree, entry);
	}
	TRIS_RWLIST_UNLOCK(&xmldoc_tree);
