# TRIS_GCC_ATTRIBUTE([attribute name])

AC_DEFUN([TRIS_GCC_ATTRIBUTE],
[
...
# TRIS_EXT_LIB_SETUP([package symbol name], [package friendly name], [package option name], [additional help text])

AC_DEFUN([TRIS_EXT_LIB_SETUP],
[
$1_DESCRIP="$2"
$1_OPTION="$3"
...
# TRIS_EXT_LIB_CHECK([package symbol name], [package library name], [function to check], [package header], [additional LIB data])

AC_DEFUN([TRIS_EXT_LIB_CHECK],
[
if test "${USE_$1}" != "no"; then
   ...
   AC_CHECK_LIB([$2], [$3], [TRIS_$1_FOUND=yes], [TRIS_$1_FOUND=no], ${pbxlibdir} $5)

   if test "${TRIS_$1_FOUND}" = "yes"; then
      $1_LIB="-l$2 $5"
      $1_HEADER_FOUND="1"
      if test "x${$1_DIR}" != "x"; then
...
AC_DEFUN(
[TRIS_CHECK_GNU_MAKE], [AC_CACHE_CHECK([for GNU make], [ac_cv_GNU_MAKE],
...
if test  "x$ac_cv_GNU_MAKE" = "xNot Found"  ; then
   AC_MSG_ERROR( *** Please install GNU make.  It is required to build Trismedia!)
   exit 1
fi
...
# TRIS_FUNC_FORK
# -------------
AN_FUNCTION([fork],  [TRIS_FUNC_FORK])
AN_FUNCTION([vfork], [TRIS_FUNC_FORK])
AC_DEFUN([TRIS_FUNC_FORK],
