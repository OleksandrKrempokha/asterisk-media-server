 ...
 * 
 * \brief A menu-driven system for Trismedia module selection
 */
...
struct categories categories = TRIS_LIST_HEAD_NOLOCK_INIT_VALUE;
...
struct tree {
	...
	mxml_node_t *root;
	/*! for linking */
	TRIS_LIST_ENTRY(tree) list;
};

/*! The list of trees from menuselect-tree files */
static TRIS_LIST_HEAD_NOLOCK_STATIC(trees, tree);
...
struct dep_file {
	...
	TRIS_LIST_ENTRY(dep_file) list;
};
TRIS_LIST_HEAD_NOLOCK_STATIC(deps_file, dep_file);
...
static struct category *add_category(struct category *cat)
{
	...
	TRIS_LIST_TRAVERSE(&categories, tmp, list) {
		if (!strcmp(tmp->name, cat->name)) {
			return tmp;
		}
	}
	TRIS_LIST_INSERT_TAIL(&categories, cat, list);
...
static void free_member(struct member *mem)
{
	...
	while ((dep = TRIS_LIST_REMOVE_HEAD(&mem->deps, list)))
		free(dep);
	while ((cnf = TRIS_LIST_REMOVE_HEAD(&mem->conflicts, list)))
		free(cnf);
	while ((use = TRIS_LIST_REMOVE_HEAD(&mem->uses, list)))
		free(use);
	free(mem);
}
...
static int parse_tree(const char *tree_file)
{
	...
	TRIS_LIST_INSERT_HEAD(&trees, tree, list);
		     ...
		     cur2 = mxmlFindElement(cur2, cur, "member", NULL, NULL, MXML_NO_DESCEND))
		{
			     ...
			     cur3 = mxmlFindElement(cur3, cur2, "depend", NULL, NULL, MXML_NO_DESCEND))
			{
				...
				if (!strlen_zero(cur3->child->value.opaque)) {
					...
					TRIS_LIST_INSERT_TAIL(&mem->deps, dep, list);
				} else
					free(dep);
			}
			     ...
			     cur3 = mxmlFindElement(cur3, cur2, "conflict", NULL, NULL, MXML_NO_DESCEND))
			{
				...
				if (!strlen_zero(cur3->child->value.opaque)) {
					...
					TRIS_LIST_INSERT_TAIL(&mem->conflicts, cnf, list);
				} else
					free(cnf);
			}
			     ...
			     cur3 = mxmlFindElement(cur3, cur2, "use", NULL, NULL, MXML_NO_DESCEND))
			{
				...
				if (!strlen_zero(cur3->child->value.opaque)) {
					...
					TRIS_LIST_INSERT_TAIL(&mem->uses, use, list);
				} else
					free(use);
			}
...
static unsigned int calc_dep_failures(int interactive, int pre_confload)
{
	...
	TRIS_LIST_TRAVERSE(&categories, cat, list) {
		TRIS_LIST_TRAVERSE(&cat->members, mem, list) {
			old_failure = mem->depsfailed;
			TRIS_LIST_TRAVERSE(&mem->deps, dep, list) {
				if (dep->member)
					continue;

				mem->depsfailed = HARD_FAILURE;
				TRIS_LIST_TRAVERSE(&deps_file, dep_file, list) {
...
static unsigned int calc_conflict_failures(int interactive, int pre_confload)
{
	...
	TRIS_LIST_TRAVERSE(&categories, cat, list) {
		TRIS_LIST_TRAVERSE(&cat->members, mem, list) {
			...
			TRIS_LIST_TRAVERSE(&mem->conflicts, cnf, list) {
				if (cnf->member)
					continue;
...
static int process_deps(void)
{
	...
	while (memset(buf, 0, sizeof(buf)), fgets(buf, sizeof(buf), f)) {
		...
		TRIS_LIST_INSERT_TAIL(&deps_file, dep_file, list);
	}
...
static void free_deps_file(void)
{
	...
	while ((dep_file = TRIS_LIST_REMOVE_HEAD(&deps_file, list)))
		free(dep_file);
}
...
static int match_member_relations(void)
{
	...
	TRIS_LIST_TRAVERSE(&categories, cat, list) {
		TRIS_LIST_TRAVERSE(&cat->members, mem, list) {
			TRIS_LIST_TRAVERSE(&mem->deps, dep, list) {
				TRIS_LIST_TRAVERSE(&cat->members, mem2, list) {
				...
				TRIS_LIST_TRAVERSE(&categories, cat2, list) {
					TRIS_LIST_TRAVERSE(&cat2->members, mem2, list) {
						if (strcasecmp(mem2->name, dep->name))
							continue;
	...
	TRIS_LIST_TRAVERSE(&categories, cat, list) {
		TRIS_LIST_TRAVERSE(&cat->members, mem, list) {
			TRIS_LIST_TRAVERSE(&mem->uses, use, list) {
				TRIS_LIST_TRAVERSE(&cat->members, mem2, list) {
...
static int generate_makedeps_file(void)
{
	...
	TRIS_LIST_TRAVERSE(&categories, cat, list) {
		TRIS_LIST_TRAVERSE(&cat->members, mem, list) {
			...
			if (TRIS_LIST_EMPTY(&mem->deps) && TRIS_LIST_EMPTY(&mem->uses))
				continue;
	...
	TRIS_LIST_TRAVERSE(&categories, cat, list) {
		...
		TRIS_LIST_TRAVERSE(&cat->members, mem, list) {
			...
			if (mem->touch_on_change) {
				for (buf = tris_strdupa(mem->touch_on_change), file = strsep(&buf, " ");
				     file;
				     file = strsep(&buf, " ")) {
			...
			if (mem->remove_on_change) {
				for (buf = tris_strdupa(mem->remove_on_change), file = strsep(&buf, " ");
				     file;
				     file = strsep(&buf, " ")) {
		...
		if (cat->touch_on_change && had_changes) {
			for (buf = tris_strdupa(cat->touch_on_change), file = strsep(&buf, " ");
			     file;
			     file = strsep(&buf, " ")) {
				strcpy(&touchcommand[9], file);
				system(touchcommand);
		...
		if (cat->remove_on_change && had_changes) {
			for (buf = tris_strdupa(cat->remove_on_change), file = strsep(&buf, " ");
			     file;
			     file = strsep(&buf, " ")) {
				strcpy(&rmcommand[7], file);
				system(rmcommand);
...
static void free_member_list(void)
{
	...
	while ((cat = TRIS_LIST_REMOVE_HEAD(&categories, list))) {
		while ((mem = TRIS_LIST_REMOVE_HEAD(&cat->members, list))) {
			while ((dep = TRIS_LIST_REMOVE_HEAD(&mem->deps, list)))
				free(dep);
			while ((cnf = TRIS_LIST_REMOVE_HEAD(&mem->conflicts, list)))
				free(cnf);
...
static void free_trees(void)
{
	...
	while ((tree = TRIS_LIST_REMOVE_HEAD(&trees, list))) {
