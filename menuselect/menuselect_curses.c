...
static void display_mem_info(WINDOW *menu, struct member *mem, int start, int end)
{
	...
	if (!TRIS_LIST_EMPTY(&mem->deps)) {
		...
		TRIS_LIST_TRAVERSE(&mem->deps, dep, list) {
			...
			if (TRIS_LIST_NEXT(dep, list))
				strncat(buf, ", ", sizeof(buf) - strlen(buf) - 1);
		}
		waddstr(menu, buf);
	}
	if (!TRIS_LIST_EMPTY(&mem->uses)) {
		wmove(menu, end - start + 4, max_x / 2 - 16);
		strcpy(buf, "Can use: ");
...
struct blip {
	...
	int oy;
	int goingleft;
	TRIS_LIST_ENTRY(blip) entry;
};

static TRIS_LIST_HEAD_NOLOCK(, blip) blips;
...
static int init_blips(void)
{
	...
	cur->x = max_x / 2;
	cur->y = max_y - 1;
	TRIS_LIST_INSERT_HEAD(&blips, cur, entry);
	tank = cur;
...
static int repaint_screen(void)
{
	...
	TRIS_LIST_TRAVERSE(&blips, cur, entry) {
...
static int tank_shoot(void)
{
	...
	shot->x = tank->x;
	shot->y = max_y - 2;
	TRIS_LIST_INSERT_HEAD(&blips, shot, entry);
...
static int move_aliens(void)
{
	...
	TRIS_LIST_TRAVERSE(&blips, cur, entry) {
		...
		if (random() % 100 < BOMB_PROB && cur->y != max_y) {
			...
			bomb->x = cur->x;
			bomb->y = cur->y + 1;
			TRIS_LIST_INSERT_HEAD(&blips, bomb, entry);
		}
	}
...
static int remove_blip(struct blip *blip)
{
	...
	TRIS_LIST_REMOVE(&blips, blip, entry);
...
static int check_placement(void)
{
	...
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&blips, cur, entry) {
		if (cur->y <= 0 || cur->y >= max_y) {
			TRIS_LIST_REMOVE_CURRENT(&blips, entry);
			remove_blip(cur);
...
static void play_space(void)
{
	...
	while ((blip = TRIS_LIST_REMOVE_HEAD(&blips, entry)))
		free(blip);
