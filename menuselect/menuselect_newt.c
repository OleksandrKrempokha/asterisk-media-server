...
static void display_member_info(struct member *mem)
{
	...
	if (TRIS_LIST_EMPTY(&mem->uses)) {
		...
		strcpy(buffer, "");
		TRIS_LIST_TRAVERSE(&mem->uses, uses, list) {
			strncat(buffer, uses->displayname, sizeof(buffer) - strlen(buffer) - 1);
			if (TRIS_LIST_NEXT(uses, list))
				strncat(buffer, ", ", sizeof(buffer) - strlen(buffer) - 1);
		}
...
static void build_members_menu(int overlay)
{
	...
	if (!overlay) {
		display_member_info(TRIS_LIST_FIRST(&cat->members));
	}
...
int run_menu(void)
{
	...
	root_menu_callback(rootOptions, TRIS_LIST_FIRST(&categories));
