...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 136298 $")
...
{
	if (len == 6)
		tris_eid_to_str(output, maxlen, (dundi_eid *)value);
	else
...
static void dump_answer(char *output, int maxlen, void *value, int len)
{
	...
	tris_eid_to_str(eid_str, sizeof(eid_str), &answer->eid);
