...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 249916 $")
...
static pthread_t precachethreadid = TRIS_PTHREADT_NULL;
static pthread_t clearcachethreadid = TRIS_PTHREADT_NULL;
...
struct permission {
	TRIS_LIST_ENTRY(permission) list;
	int allow;
	char name[0];
};

struct dundi_packet {
	TRIS_LIST_ENTRY(dundi_packet) list;
	struct dundi_hdr *h;
	int datalen;
...
struct dundi_precache_queue {
	TRIS_LIST_ENTRY(dundi_precache_queue) list;
	char *context;
	time_t expiration;
	char number[0];
};
...
struct dundi_transaction {
	...
	tris_aes_decrypt_key dcx;                       /*!< AES 128 Decryption context */
	...
	TRIS_LIST_HEAD_NOLOCK(packetlist, dundi_packet) packets;  /*!< Packets to be retransmitted */
	struct packetlist lasttrans;                   /*!< Last transmitted / ACK'd packet */
	struct dundi_request *parent;                  /*!< Parent request (if there is one) */
	TRIS_LIST_ENTRY(dundi_transaction) parentlist;  /*!< Next with respect to the parent */
	TRIS_LIST_ENTRY(dundi_transaction) all;         /*!< Next with respect to all DUNDi transactions */
};

struct dundi_request {
	char dcontext[TRIS_MAX_EXTENSION];
	char number[TRIS_MAX_EXTENSION];
	dundi_eid query_eid;
	dundi_eid root_eid;
	...
	TRIS_LIST_HEAD_NOLOCK(, dundi_transaction) trans;  /*!< Transactions */
	TRIS_LIST_ENTRY(dundi_request) list;
};

struct dundi_mapping {
	char dcontext[TRIS_MAX_EXTENSION];
	char lcontext[TRIS_MAX_EXTENSION];
	int _weight;
	char *weightstr;
	int options;
	int tech;
	int dead;
	char dest[TRIS_MAX_EXTENSION];
	TRIS_LIST_ENTRY(dundi_mapping) list;
};

struct dundi_peer {
	dundi_eid eid;
	struct sockaddr_in addr;               /*!< Address of DUNDi peer */
	TRIS_LIST_HEAD_NOLOCK(permissionlist, permission) permit;
	...
	tris_aes_encrypt_key us_ecx;            /*!< Cached AES 128 Encryption context */
	tris_aes_decrypt_key us_dcx;            /*!< Cached AES 128 Decryption context */
	uint32_t them_keycrc32;                /*!< CRC-32 of our key */
	tris_aes_encrypt_key them_ecx;          /*!< Cached AES 128 Encryption context */
	tris_aes_decrypt_key them_dcx;          /*!< Cached AES 128 Decryption context */
	...
	TRIS_LIST_ENTRY(dundi_peer) list;
};

static TRIS_LIST_HEAD_STATIC(peers, dundi_peer);
static TRIS_LIST_HEAD_STATIC(pcq, dundi_precache_queue);
static TRIS_LIST_HEAD_NOLOCK_STATIC(mappings, dundi_mapping);
static TRIS_LIST_HEAD_NOLOCK_STATIC(requests, dundi_request);
static TRIS_LIST_HEAD_NOLOCK_STATIC(alltrans, dundi_transaction);
...
static int has_permission(struct permissionlist *permlist, char *cont)
{
	...
	TRIS_LIST_TRAVERSE(permlist, perm, list) {
...
static int dundi_lookup_internal(struct dundi_result *result, int maxret, struct tris_channel *chan, const char *dcontext, const char *number, int ttl, int blockempty, struct dundi_hint_metadata *md, int *expiration, int cybpass, int modeselect, dundi_eid *skip, dundi_eid *avoid[], int direct[]);
...
static struct dundi_transaction *find_transaction(struct dundi_hdr *hdr, struct sockaddr_in *sin)
{
	...
	TRIS_LIST_TRAVERSE(&alltrans, trans, all) {
...
static int get_trans_id(void)
{
	...
	int stid = (tris_random() % 32766) + 1;
	int tid = stid;

	do {
		TRIS_LIST_TRAVERSE(&alltrans, t, all) {
			if (t->strans == tid)
				break;
		}
		if (!t)
...
static int reset_transaction(struct dundi_transaction *trans)
{
	...
	trans->oseqno = 0;
	trans->aseqno = 0;
	tris_clear_flag(trans, FLAG_FINAL);
	return 0;
}
...
static struct dundi_peer *find_peer(dundi_eid *eid)
{
	...
	TRIS_LIST_TRAVERSE(&peers, cur, list) {
		if (!tris_eid_cmp(&cur->eid,eid))
			break;
	}
...
struct dundi_query_state {
	...
	dundi_eid reqeid;
	char called_context[TRIS_MAX_EXTENSION];
	char called_number[TRIS_MAX_EXTENSION];
...
static int dundi_lookup_local(struct dundi_result *dr, struct dundi_mapping *map, char *called_number, dundi_eid *us_eid, int anscnt, struct dundi_hint_metadata *hmd)
{
	...
	int x;
	if (!tris_strlen_zero(map->lcontext)) {
		if (tris_exists_extension(NULL, map->lcontext, called_number, 1, NULL))
			tris_set_flag(&flags, DUNDI_FLAG_EXISTS);
		if (tris_canmatch_extension(NULL, map->lcontext, called_number, 1, NULL))
			tris_set_flag(&flags, DUNDI_FLAG_CANMATCH);
		if (tris_matchmore_extension(NULL, map->lcontext, called_number, 1, NULL))
			tris_set_flag(&flags, DUNDI_FLAG_MATCHMORE);
		if (tris_ignore_pattern(map->lcontext, called_number))
			tris_set_flag(&flags, DUNDI_FLAG_IGNOREPAT);

		/* Clearly we can't say 'don't ask' anymore if we found anything... */
		if (tris_test_flag(&flags, TRIS_FLAGS_ALL))
			tris_clear_flag_nonstd(hmd, DUNDI_HINT_DONT_ASK);

		if (map->options & DUNDI_FLAG_INTERNAL_NOPARTIAL) {
			/* Skip partial answers */
			tris_clear_flag(&flags, DUNDI_FLAG_MATCHMORE|DUNDI_FLAG_CANMATCH);
		}
		if (tris_test_flag(&flags, TRIS_FLAGS_ALL)) {
			struct varshead headp;
			struct tris_var_t *newvariable;
			tris_set_flag(&flags, map->options & 0xffff);
			tris_copy_flags(dr + anscnt, &flags, TRIS_FLAGS_ALL);
			dr[anscnt].techint = map->tech;
			dr[anscnt].weight = get_mapping_weight(map);
			dr[anscnt].expiration = dundi_cache_time;
			tris_copy_string(dr[anscnt].tech, tech2str(map->tech), sizeof(dr[anscnt].tech));
			dr[anscnt].eid = *us_eid;
			tris_eid_to_str(dr[anscnt].eid_str, sizeof(dr[anscnt].eid_str), &dr[anscnt].eid);
			if (tris_test_flag(&flags, DUNDI_FLAG_EXISTS)) {
				TRIS_LIST_HEAD_INIT_NOLOCK(&headp);
				newvariable = tris_var_assign("NUMBER", called_number);
				TRIS_LIST_INSERT_HEAD(&headp, newvariable, entries);
				newvariable = tris_var_assign("EID", dr[anscnt].eid_str);
				TRIS_LIST_INSERT_HEAD(&headp, newvariable, entries);
				newvariable = tris_var_assign("SECRET", cursecret);
				TRIS_LIST_INSERT_HEAD(&headp, newvariable, entries);
				newvariable = tris_var_assign("IPADDR", ipaddr);
				TRIS_LIST_INSERT_HEAD(&headp, newvariable, entries);
			...
			char tmp[TRIS_MAX_EXTENSION + 1] = "";
			for (x = 0; x < (sizeof(tmp) - 1); x++) {
				tmp[x] = called_number[x];
				if (!tmp[x])
					break;
				if (!tris_canmatch_extension(NULL, map->lcontext, tmp, 1, NULL)) {
					...
					if (strlen(tmp) > strlen(hmd->exten)) {
						tris_copy_string(hmd->exten, tmp, sizeof(hmd->exten));
					}
					break;
				}
			}
...
static void *dundi_lookup_thread(void *data)
{
	...
	tris_debug(1, "Whee, looking up '%s@%s' for '%s'\n", st->called_number, st->called_context,
			st->eids[0] ? tris_eid_to_str(eid_str, sizeof(eid_str), st->eids[0]) :  "ourselves");
	...
	TRIS_LIST_LOCK(&peers);
	/* Truncate if "don't ask" isn't present */
	if (!tris_test_flag_nonstd(&hmd, DUNDI_HINT_DONT_ASK))
		hmd.exten[0] = '\0';
	if (tris_test_flag(st->trans, FLAG_DEAD)) {
		tris_debug(1, "Our transaction went away!\n");
		st->trans->thread = 0;
...
static void *dundi_precache_thread(void *data)
{
	...
	tris_debug(1, "Whee, precaching '%s@%s' for '%s'\n", st->called_number, st->called_context,
		st->eids[0] ? tris_eid_to_str(eid_str, sizeof(eid_str), st->eids[0]) :  "ourselves");
	memset(&ied, 0, sizeof(ied));
...
static void *dundi_query_thread(void *data)
{
	...
	memset(&hmd, 0, sizeof(hmd));
	if (!tris_eid_cmp(&st->trans->us_eid, &st->reqeid)) {
		/* Ooh, it's us! */
		tris_debug(1, "Neat, someone look for us!\n");
		tris_copy_string(dei.orgunit, dept, sizeof(dei.orgunit));
		tris_copy_string(dei.org, org, sizeof(dei.org));
		tris_copy_string(dei.locality, locality, sizeof(dei.locality));
		tris_copy_string(dei.stateprov, stateprov, sizeof(dei.stateprov));
		tris_copy_string(dei.country, country, sizeof(dei.country));
		tris_copy_string(dei.email, email, sizeof(dei.email));
		tris_copy_string(dei.phone, phone, sizeof(dei.phone));
		res = 1;
	} else {
		/* If we do not have a canonical result, keep looking */
	...
	if (tris_test_flag(st->trans, FLAG_DEAD)) {
		...
		if (res) {
			...
			if (!tris_strlen_zero(dei.ipaddr))
...
static int dundi_answer_entity(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	if (ies->eidcount > 1) {
		...
		if (!tris_eid_cmp(ies->eids[0], ies->eids[ies->eidcount - 1]))
			skipfirst = 1;
	}
	totallen = sizeof(struct dundi_query_state);
	totallen += (ies->eidcount - skipfirst) * sizeof(dundi_eid);
	st = tris_calloc(1, totallen);
	if (st) {
		tris_copy_string(st->called_context, ies->called_context, sizeof(st->called_context));
		...
		tris_debug(1, "Answering EID query for '%s@%s'!\n", tris_eid_to_str(eid_str, sizeof(eid_str), ies->reqeid), ies->called_context);

		trans->thread = 1;
		if (tris_pthread_create_detached(&lookupthread, NULL, dundi_query_thread, st)) {
			struct dundi_ie_data ied = { 0, };
			trans->thread = 0;
			tris_log(LOG_WARNING, "Unable to create thread!\n");
			tris_free(st);
...
static int cache_save_hint(dundi_eid *eidpeer, struct dundi_request *req, struct dundi_hint *hint, int expiration)
{
	...
	if (!tris_test_flag_nonstd(hint, htons(DUNDI_HINT_DONT_ASK)))
		return 0;

	unaffected = tris_test_flag_nonstd(hint, htons(DUNDI_HINT_UNAFFECTED));
	...
	tris_db_put("dundi/cache", key1, data);
	tris_debug(1, "Caching hint at '%s'\n", key1);
	tris_db_put("dundi/cache", key2, data);
	tris_debug(1, "Caching hint at '%s'\n", key2);
	return 0;
}
...
static int dundi_prop_precache(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	trans->parent = &dr;
	tris_copy_string(dr.dcontext, ies->called_context ? ies->called_context : "e164", sizeof(dr.dcontext));
	tris_copy_string(dr.number, ies->called_number, sizeof(dr.number));
	...
	for (x=0;x<ies->anscount;x++) {
		if (trans->parent->respcount < trans->parent->maxcount) {
			...
			if (z == trans->parent->respcount) {
				...
				tris_eid_to_str(trans->parent->dr[trans->parent->respcount].eid_str,
					sizeof(trans->parent->dr[trans->parent->respcount].eid_str),
					&ies->answers[x]->eid);
				tris_copy_string(trans->parent->dr[trans->parent->respcount].dest, (char *)ies->answers[x]->data,
					sizeof(trans->parent->dr[trans->parent->respcount].dest));
					tris_copy_string(trans->parent->dr[trans->parent->respcount].tech, tech2str(ies->answers[x]->protocol),
					sizeof(trans->parent->dr[trans->parent->respcount].tech));
				trans->parent->respcount++;
				tris_clear_flag_nonstd(trans->parent->hmd, DUNDI_HINT_DONT_ASK);
			...
			tris_log(LOG_NOTICE, "Dropping excessive answers in precache for %s@%s\n",
	...
	mapcount = 0;
	TRIS_LIST_TRAVERSE(&mappings, cur, list) {
	...
	if (st) {
		tris_copy_string(st->called_context, ies->called_context, sizeof(st->called_context));
		tris_copy_string(st->called_number, ies->called_number, sizeof(st->called_number));
		st->trans = trans;
		st->ttl = ies->ttl - 1;
		...
		st->nummaps = mapcount;
		tris_debug(1, "Forwarding precache for '%s@%s'!\n", ies->called_number, ies->called_context);
		trans->thread = 1;
		if (tris_pthread_create_detached(&lookupthread, NULL, dundi_precache_thread, st)) {
			trans->thread = 0;
			tris_log(LOG_WARNING, "Unable to create thread!\n");
			tris_free(st);
			memset(&ied, 0, sizeof(ied));
...
static int dundi_answer_query(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	if (st) {
		...
		st->nummaps = mapcount;
		tris_debug(1, "Answering query for '%s@%s'!\n", ies->called_number, ies->called_context);
		trans->thread = 1;
		if (tris_pthread_create_detached(&lookupthread, NULL, dundi_lookup_thread, st)) {
			trans->thread = 0;
			tris_log(LOG_WARNING, "Unable to create thread!\n");
			tris_free(st);
			memset(&ied, 0, sizeof(ied));
...
static int cache_lookup_internal(time_t now, struct dundi_request *req, char *key, char *eid_str_full, int *lowexpiration)
{
	...
	/* Build request string */
	if (!tris_db_get("dundi/cache", key, data, sizeof(data))) {
		time_t timeout;
		ptr = data;
		if (!tris_get_time_t(ptr, &timeout, 0, &length)) {
			int expiration = timeout - now;
			if (expiration > 0) {
				tris_debug(1, "Found cache expiring in %d seconds!\n", expiration);
				ptr += length + 1;
						...
						*term = '\0';
						...
						if (src) {
							*src = '\0';
							...
							src = "";
						tris_debug(1, "Found cached answer '%s/%s' originally from '%s' with flags '%s' on behalf of '%s'\n",
						...
						if (z == req->respcount) {
							/* Copy into parent responses */
							tris_copy_flags(&(req->dr[req->respcount]), &flags, TRIS_FLAGS_ALL);
							...
							tris_eid_to_str(req->dr[req->respcount].eid_str,
								sizeof(req->dr[req->respcount].eid_str), &req->dr[req->respcount].eid);
							tris_copy_string(req->dr[req->respcount].dest, ptr,
								sizeof(req->dr[req->respcount].dest));
							tris_copy_string(req->dr[req->respcount].tech, tech2str(tech),
								sizeof(req->dr[req->respcount].tech));
							req->respcount++;
							tris_clear_flag_nonstd(req->hmd, DUNDI_HINT_DONT_ASK);
...
static int cache_lookup(struct dundi_request *req, dundi_eid *peer_eid, uint32_t crc32, int *lowexpiration)
{
	...
	dundi_eid_to_str_short(eidroot_str, sizeof(eidroot_str), &req->root_eid);
	tris_eid_to_str(eid_str_full, sizeof(eid_str_full), peer_eid);
		...
		while(!res2) {
			...
			if (res2) {
				if (strlen(tmp) > strlen(req->hmd->exten)) {
					...
					tris_copy_string(req->hmd->exten, tmp, sizeof(req->hmd->exten));
				}
			}
		}
		res |= res2;
...
static void apply_peer(struct dundi_transaction *trans, struct dundi_peer *p)
{
	...
	if (!tris_strlen_zero(p->inkey))
		tris_set_flag(trans, FLAG_ENCRYPT);
	if (p->maxms) {
...
static int do_register_expire(const void *data)
{
	...
	char eid_str[20];
	tris_debug(1, "Register expired for '%s'\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
...
static int update_key(struct dundi_peer *peer)
{
	...
	struct tris_key *ekey, *skey;
	char eid_str[20];
	int res;
	if (!peer->keyexpire || (peer->keyexpire < time(NULL))) {
		build_iv(key);
		tris_aes_encrypt_key(key, &peer->us_ecx);
		tris_aes_decrypt_key(key, &peer->us_dcx);
		ekey = tris_key_get(peer->inkey, TRIS_KEY_PUBLIC);
		if (!ekey) {
			tris_log(LOG_NOTICE, "No such key '%s' for creating RSA encrypted shared key for '%s'!\n",
				peer->inkey, tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
			return -1;
		}
		skey = tris_key_get(peer->outkey, TRIS_KEY_PRIVATE);
		if (!skey) {
			tris_log(LOG_NOTICE, "No such key '%s' for signing RSA encrypted shared key for '%s'!\n",
				peer->outkey, tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
			return -1;
		}
		if ((res = tris_encrypt_bin(peer->txenckey, key, sizeof(key), ekey)) != 128) {
			tris_log(LOG_NOTICE, "Whoa, got a weird encrypt size (%d != %d)!\n", res, 128);
			return -1;
		}
		if ((res = tris_sign_bin(skey, (char *)peer->txenckey, 128, peer->txenckey + 128))) {
			tris_log(LOG_NOTICE, "Failed to sign key (%d)!\n", res);
			return -1;
		}
...
static int encrypt_memcpy(unsigned char *dst, unsigned char *src, int len, unsigned char *iv, tris_aes_encrypt_key *ecx)
{
...
static int decrypt_memcpy(unsigned char *dst, unsigned char *src, int len, unsigned char *iv, tris_aes_decrypt_key *dcx)
{
...
static struct dundi_hdr *dundi_decrypt(struct dundi_transaction *trans, unsigned char *dst, int *dstlen, struct dundi_hdr *ohdr, struct dundi_encblock *src, int srclen)
{
	...
	*h = *ohdr;
	...
	if (uncompress(dst + 6, &bytes, decrypt_space, srclen) != Z_OK) {
		tris_debug(1, "Ouch, uncompress failed :(\n");
		return NULL;
	}
	/* Update length */
	*dstlen = bytes + 6;
...
static int dundi_encrypt(struct dundi_transaction *trans, struct dundi_packet *pack)
{
	...
	if (compress_space) {
		...
		if (res != Z_OK) {
			tris_debug(1, "Ouch, compression failed!\n");
			return -1;
		}
		memset(&ied, 0, sizeof(ied));
		/* Say who we are */
		if (!pack->h->iseqno && !pack->h->oseqno) {
			...
			if (update_key(peer))
				return -1;
			if (!peer->sentfullkey)
				tris_set_flag(trans, FLAG_SENDFULLKEY);
			/* Append key data */
			dundi_ie_append_eid(&ied, DUNDI_IE_EID, &trans->us_eid);
			if (tris_test_flag(trans, FLAG_SENDFULLKEY)) {
		...
		if ((ied.pos + bytes) >= sizeof(ied.buf)) {
			tris_log(LOG_NOTICE, "Final packet too large!\n");
			return -1;
		}
...
static int check_key(struct dundi_peer *peer, unsigned char *newkey, unsigned char *newsig, uint32_t keycrc32)
{
	...
	int res;
	struct tris_key *key, *skey;
	char eid_str[20];
	tris_debug(1, "Expected '%08x' got '%08x'\n", peer->them_keycrc32, keycrc32);
	...
	/* Decrypt key */
	key = tris_key_get(peer->outkey, TRIS_KEY_PRIVATE);
	if (!key) {
		tris_log(LOG_NOTICE, "Unable to find key '%s' to decode shared key from '%s'\n",
			peer->outkey, tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		return -1;
	}

	skey = tris_key_get(peer->inkey, TRIS_KEY_PUBLIC);
	if (!skey) {
		tris_log(LOG_NOTICE, "Unable to find key '%s' to verify shared key from '%s'\n",
			peer->inkey, tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		return -1;
	}

	/* First check signature */
	res = tris_check_signature_bin(skey, (char *)newkey, 128, newsig);
	if (res)
		return 0;

	res = tris_decrypt_bin(dst, newkey, sizeof(dst), key);
	if (res != 16) {
		if (res >= 0)
			tris_log(LOG_NOTICE, "Weird, key decoded to the wrong size (%d)\n", res);
		return 0;
	}
	/* Decrypted, passes signature */
	tris_debug(1, "Wow, new key combo passed signature and decrypt!\n");
	memcpy(peer->rxenckey, newkey, 128);
	memcpy(peer->rxenckey + 128, newsig, 128);
	peer->them_keycrc32 = crc32(0L, peer->rxenckey, 128);
	tris_aes_decrypt_key(dst, &peer->them_dcx);
	tris_aes_encrypt_key(dst, &peer->them_ecx);
	return 1;
}
...
static void deep_copy_peer(struct dundi_peer *peer_dst, const struct dundi_peer *peer_src)
{
	...
	TRIS_LIST_TRAVERSE(&peer_src->permit, cur, list) {
		if (!(perm = tris_calloc(1, sizeof(*perm) + strlen(cur->name) + 1)))
			continue;

		perm->allow = cur->allow;
		strcpy(perm->name, cur->name);

		TRIS_LIST_INSERT_HEAD(&peer_dst->permit, perm, list);
	}

	TRIS_LIST_TRAVERSE(&peer_src->include, cur, list) {
		if (!(perm = tris_calloc(1, sizeof(*perm) + strlen(cur->name) + 1)))
			continue;
		...
		TRIS_LIST_INSERT_HEAD(&peer_dst->include, perm, list);
	}
}
...
static int handle_command_response(struct dundi_transaction *trans, struct dundi_hdr *hdr, int datalen, int encrypted)
{
...
#ifdef LOW_MEMORY
	struct dundi_ie_data *ied = tris_calloc(1, sizeof(*ied));
#else
	...
	if (datalen) {
		...
		tris_debug(1, "Got canonical message %d (%d), %d bytes data%s\n", cmd, hdr->oseqno, datalen, final ? " (Final)" : "");
		if (dundi_parse_ies(&ies, bufcpy, datalen) < 0) {
			tris_log(LOG_WARNING, "Failed to parse DUNDI information elements!\n");
			goto return_cleanup;
		}
	}
	switch(cmd) {
		...
		if (!peer) {
			...
			if (hasauth) {
				...
				if (cmd == DUNDI_COMMAND_EIDQUERY) {
					...
					if (tris_strlen_zero(ies.called_number)) {
	...
	case DUNDI_COMMAND_REGREQ:
		...
		if (any_peer && peer == any_peer) {
			...
			peer = tris_calloc(1, sizeof(*peer));
			if (peer) {
				...
				TRIS_LIST_LOCK(&peers);
				TRIS_LIST_INSERT_HEAD(&peers, peer, list);
				TRIS_LIST_UNLOCK(&peers);
			}
		}
		...
		if (!peer || !peer->dynamic) {
			...
			int hasauth = 0;
			trans->us_eid = peer->us_eid;
			if (!tris_strlen_zero(peer->inkey)) {
				hasauth = encrypted;
			} else
				hasauth = 1;
			if (hasauth) {
				...
				char data[256];
				int needqual = 0;
				TRIS_SCHED_DEL(sched, peer->registerexpire);
				peer->registerexpire = tris_sched_add(sched, (expire + 10) * 1000, do_register_expire, peer);
				snprintf(data, sizeof(data), "%s:%d:%d", tris_inet_ntoa(trans->addr.sin_addr),
					ntohs(trans->addr.sin_port), expire);
				tris_db_put("dundi/dpeers", dundi_eid_to_str_short(eid_str, sizeof(eid_str), &peer->eid), data);
				if (inaddrcmp(&peer->addr, &trans->addr)) {
					tris_verb(3, "Registered DUNDi peer '%s' at '%s:%d'\n",
							tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid),
							tris_inet_ntoa(trans->addr.sin_addr), ntohs(trans->addr.sin_port));
					needqual = 1;
				}
	...
	case DUNDI_COMMAND_DPRESPONSE:
		...
		if (ies.cause < 1) {
			...
			tris_debug(1, "Looks like success of some sort (%d), %d answers\n", ies.cause, ies.anscount);
			if (tris_test_flag(trans, FLAG_ENCRYPT)) {
				authpass = encrypted;
			} else
				authpass = 1;
			if (authpass) {
				...
				if (trans->parent && trans->parent->dr) {
					...
					for (x=0;x<ies.anscount;x++) {
						if (trans->parent->respcount < trans->parent->maxcount) {
							...
							tris_log(LOG_NOTICE, "Dropping excessive answers to request for %s@%s\n",
								trans->parent->number, trans->parent->dcontext);
					}
					...
					cache_save(&trans->them_eid, trans->parent, y,
							ies.hint ? tris_test_flag_nonstd(ies.hint, htons(DUNDI_HINT_UNAFFECTED)) : 0, ies.expiration, 0);
					if (ies.hint) {
						cache_save_hint(&trans->them_eid, trans->parent, ies.hint, ies.expiration);
						if (tris_test_flag_nonstd(ies.hint, htons(DUNDI_HINT_TTL_EXPIRED)))
							tris_set_flag_nonstd(trans->parent->hmd, DUNDI_HINT_TTL_EXPIRED);
						if (tris_test_flag_nonstd(ies.hint, htons(DUNDI_HINT_DONT_ASK))) {
							if (strlen((char *)ies.hint->data) > strlen(trans->parent->hmd->exten)) {
								tris_copy_string(trans->parent->hmd->exten, (char *)ies.hint->data,
	...
	case DUNDI_COMMAND_EIDRESPONSE:
		...
		if (ies.cause < 1) {
			/* Success of some sort */
			tris_debug(1, "Looks like success of some sort (%d)\n", ies.cause);
			if (tris_test_flag(trans, FLAG_ENCRYPT)) {
				authpass = encrypted;
			} else
				authpass = 1;
			if (authpass) {
				...
				if (trans->parent && trans->parent->dei && ies.q_org) {
					if (!trans->parent->respcount) {
						trans->parent->respcount++;
						if (ies.q_dept)
							tris_copy_string(trans->parent->dei->orgunit, ies.q_dept, sizeof(trans->parent->dei->orgunit));
						if (ies.q_org)
							tris_copy_string(trans->parent->dei->org, ies.q_org, sizeof(trans->parent->dei->org));
						if (ies.q_locality)
							tris_copy_string(trans->parent->dei->locality, ies.q_locality, sizeof(trans->parent->dei->locality));
						if (ies.q_stateprov)
							tris_copy_string(trans->parent->dei->stateprov, ies.q_stateprov, sizeof(trans->parent->dei->stateprov));
						if (ies.q_country)
							tris_copy_string(trans->parent->dei->country, ies.q_country, sizeof(trans->parent->dei->country));
						if (ies.q_email)
							tris_copy_string(trans->parent->dei->email, ies.q_email, sizeof(trans->parent->dei->email));
						if (ies.q_phone)
							tris_copy_string(trans->parent->dei->phone, ies.q_phone, sizeof(trans->parent->dei->phone));
						if (ies.q_ipaddr)
							tris_copy_string(trans->parent->dei->ipaddr, ies.q_ipaddr, sizeof(trans->parent->dei->ipaddr));
						if (!tris_eid_cmp(&trans->them_eid, &trans->parent->query_eid)) {
							/* If it's them, update our address */
							tris_copy_string(trans->parent->dei->ipaddr, tris_inet_ntoa(trans->addr.sin_addr), sizeof(trans->parent->dei->ipaddr));
						}
					}
					if (ies.hint) {
						if (tris_test_flag_nonstd(ies.hint, htons(DUNDI_HINT_TTL_EXPIRED)))
	...
	case DUNDI_COMMAND_REGRESPONSE:
		...
		if (ies.cause < 1) {
			...
			if (!hasauth) {
				tris_log(LOG_NOTICE, "Reponse to register not authorized!\n");
				if (!final) {
				...
				tris_debug(1, "Yay, we've registered as '%s' to '%s'\n", tris_eid_to_str(eid_str, sizeof(eid_str), &trans->us_eid),
						tris_eid_to_str(eid_str2, sizeof(eid_str2), &trans->them_eid));
				/* Close connection if not final */
				if (!final)
					dundi_send(trans, DUNDI_COMMAND_CANCEL, 0, 1, NULL);
			}
	...
	case DUNDI_COMMAND_ENCREJ:
		if ((tris_test_flag(trans, FLAG_SENDFULLKEY)) || TRIS_LIST_EMPTY(&trans->lasttrans) || !(peer = find_peer(&trans->them_eid))) {
			...
			if (final) {
				...
				if (!reset_transaction(trans)) {
					...
					dundi_parse_ies(&ies, (TRIS_LIST_FIRST(&trans->lasttrans))->h->ies, (TRIS_LIST_FIRST(&trans->lasttrans))->datalen - sizeof(struct dundi_hdr));
					...
					dundi_send(trans, DUNDI_COMMAND_ENCRYPT, 0, (TRIS_LIST_FIRST(&trans->lasttrans))->h->cmdresp & 0x80, ied);
					peer->sentfullkey = 1;
				}
			}
		}
	...
	case DUNDI_COMMAND_ENCRYPT:
		if (!encrypted) {
			...
			if (tris_test_flag(trans, FLAG_ENCRYPT) && ies.encblock && ies.enclen) {
				...
				if (dhdr) {
					...
					tris_debug(1, "Ouch, decrypt failed :(\n");
				}
			}
		}
		if (!final) {
			/* Turn off encryption */
			tris_clear_flag(trans, FLAG_ENCRYPT);
			dundi_send(trans, DUNDI_COMMAND_ENCREJ, 0, 1, NULL);
		}
		break;
	default:
...
static void destroy_packets(struct packetlist *p)
{
	...
	while ((pack = TRIS_LIST_REMOVE_HEAD(p, list))) {
		TRIS_SCHED_DEL(sched, pack->retransid);
		tris_free(pack);
	}
}
...
static int ack_trans(struct dundi_transaction *trans, int iseqno)
{
	...
	TRIS_LIST_TRAVERSE(&trans->packets, pack, list) {
		if ((pack->h->oseqno + 1) % 255 == iseqno) {
			destroy_packet(pack, 0);
			if (!TRIS_LIST_EMPTY(&trans->lasttrans)) {
				tris_log(LOG_WARNING, "Whoa, there was still a last trans?\n");
				destroy_packets(&trans->lasttrans);
			}
			TRIS_LIST_INSERT_HEAD(&trans->lasttrans, pack, list);
			TRIS_SCHED_DEL(sched, trans->autokillid);
			return 1;
		}
	}
...
static int handle_frame(struct dundi_hdr *h, struct sockaddr_in *sin, int datalen)
{
	...
	if (h->oseqno == trans->iseqno) {
		...
		if (ack_trans(trans, h->iseqno) && tris_test_flag(trans, FLAG_FINAL)) {
			/* If final, we're done */
		...
		tris_debug(1, "Dropping packet out of window!\n");
	}
	return 0;
}
...
static void build_secret(char *secret, int seclen)
{
	...
	build_iv(tmp);
	secret[0] = '\0';
	tris_base64encode(secret, tmp, sizeof(tmp), seclen);
...
static void save_secret(const char *newkey, const char *oldkey)
{
	...
	tris_db_put(secretpath, "secret", tmp);
	snprintf(tmp, sizeof(tmp), "%d", (int)rotatetime);
	tris_db_put(secretpath, "secretexpiry", tmp);
}
...
static void load_password(void)
{
	...
	tris_db_get(secretpath, "secretexpiry", tmp, sizeof(tmp));
	if (!tris_get_time_t(tmp, &expired, 0, NULL)) {
		tris_db_get(secretpath, "secret", tmp, sizeof(tmp));
		current = strchr(tmp, ';');
		if (!current)
			current = tmp;
		else {
	...
	if (current) {
		...
		tris_copy_string(cursecret, current, sizeof(cursecret));
		rotatetime = expired;
	} else {
...
static void check_password(void)
{
	...
	if ((now - rotatetime) >= 0) {
		/* Time to rotate keys */
		tris_copy_string(oldsecret, cursecret, sizeof(oldsecret));
		build_secret(cursecret, sizeof(cursecret));
...
static void *network_thread(void *ignore)
{
	...
	tris_io_add(io, netsocket, socket_read, TRIS_IO_IN, NULL);
	...
	netthreadid = TRIS_PTHREADT_NULL;
...
static void *process_clearcache(void *ignore)
{
	...
	while (!dundi_shutdown) {
		...
		db_entry = db_tree = tris_db_gettree("dundi/cache", NULL);
		for (; db_entry; db_entry = db_entry->next) {
			time_t expiry;

			if (!tris_get_time_t(db_entry->data, &expiry, 0, NULL)) {
				if (expiry < now) {
					tris_debug(1, "clearing expired DUNDI cache entry: %s\n", db_entry->key);
					tris_db_del("dundi/cache", db_entry->key + striplen);
				}
			}
		}
	...
	clearcachethreadid = TRIS_PTHREADT_NULL;
	return NULL;
}
...
static void *process_precache(void *ign)
{
	...
	while (!dundi_shutdown) {
		...
		run = 0;
		TRIS_LIST_LOCK(&pcq);
		if ((qe = TRIS_LIST_FIRST(&pcq))) {
			if (!qe->expiration) {
				/* Gone...  Remove... */
				TRIS_LIST_REMOVE_HEAD(&pcq, list);
				tris_free(qe);
			} else if (qe->expiration < now) {
				/* Process this entry */
				qe->expiration = 0;
				tris_copy_string(context, qe->context, sizeof(context));
				tris_copy_string(number, qe->number, sizeof(number));
				run = 1;
			}
		}
		TRIS_LIST_UNLOCK(&pcq);
		if (run) {
	...
	precachethreadid = TRIS_PTHREADT_NULL;
...
static int start_network_thread(void)
{
	...
	tris_pthread_create_background(&precachethreadid, NULL, process_precache, NULL);
	tris_pthread_create_background(&clearcachethreadid, NULL, process_clearcache, NULL);
	return 0;
}

static char *dundi_set_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (!strncasecmp(a->argv[e->args -1], "on", 2)) {
		dundidebug = 1;
		tris_cli(a->fd, "DUNDi Debugging Enabled\n");
	} else {
		dundidebug = 0;
		tris_cli(a->fd, "DUNDi Debugging Disabled\n");
	}
	return CLI_SUCCESS;
}

static char *dundi_store_history(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (!strncasecmp(a->argv[e->args -1], "on", 2)) {
		...
		tris_cli(a->fd, "DUNDi History Storage Enabled\n");
	} else {
		global_storehistory = 0;
		tris_cli(a->fd, "DUNDi History Storage Disabled\n");
	}
	return CLI_SUCCESS;
}

static char *dundi_flush(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int stats = 0;
	switch (cmd) {
	case CLI_INIT:
	...
	if (stats) {
		...
		int x;
		TRIS_LIST_LOCK(&peers);
		TRIS_LIST_TRAVERSE(&peers, p, list) {
			for (x = 0;x < DUNDI_TIMING_HISTORY; x++) {
				if (p->lookups[x])
					tris_free(p->lookups[x]);
				p->lookups[x] = NULL;
				p->lookuptimes[x] = 0;
			}
			p->avgms = 0;
		...
		tris_db_deltree("dundi/cache", NULL);
		tris_cli(a->fd, "DUNDi Cache Flushed\n");
	}
	return CLI_SUCCESS;
}
...
static char *complete_peer_helper(const char *line, const char *word, int pos, int state, int rpos)
{
	...
	len = strlen(word);
	TRIS_LIST_TRAVERSE(&peers, p, list) {
		const char *s = tris_eid_to_str(eid_str, sizeof(eid_str), &p->eid);
...
static char *dundi_do_lookup(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int res;
	char tmp[256];
	char fs[80] = "";
	...
	if (res < 0)
		tris_cli(a->fd, "DUNDi lookup returned error.\n");
	else if (!res)
		tris_cli(a->fd, "DUNDi lookup returned no results.\n");
	else
		sort_results(dr, res);
	for (x=0;x<res;x++) {
		tris_cli(a->fd, "%3d. %5d %s/%s (%s)\n", x + 1, dr[x].weight, dr[x].tech, dr[x].dest, dundi_flags2str(fs, sizeof(fs), dr[x].flags));
		tris_cli(a->fd, "     from %s, expires in %d s\n", dr[x].eid_str, dr[x].expiration);
	}
	tris_cli(a->fd, "DUNDi lookup completed in %d ms\n", tris_tvdiff_ms(tris_tvnow(), start));
	return CLI_SUCCESS;
}

static char *dundi_do_precache(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int res;
	char tmp[256];
	char *context;
	...
	if (res < 0)
		tris_cli(a->fd, "DUNDi precache returned error.\n");
	else if (!res)
		tris_cli(a->fd, "DUNDi precache returned no error.\n");
	tris_cli(a->fd, "DUNDi lookup completed in %d ms\n", tris_tvdiff_ms(tris_tvnow(), start));
	return CLI_SUCCESS;
}

static char *dundi_do_query(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int res;
	char tmp[256];
	char *context;
	...
	if ((a->argc < 3) || (a->argc > 3))
		return CLI_SHOWUSAGE;
	if (tris_str_to_eid(&eid, a->argv[2])) {
		tris_cli(a->fd, "'%s' is not a valid EID!\n", a->argv[2]);
		return CLI_SHOWUSAGE;
	}
	tris_copy_string(tmp, a->argv[2], sizeof(tmp));
	context = strchr(tmp, '@');
	...
	if (res < 0)
		tris_cli(a->fd, "DUNDi Query EID returned error.\n");
	else if (!res)
		tris_cli(a->fd, "DUNDi Query EID returned no results.\n");
	else {
		tris_cli(a->fd, "DUNDi Query EID succeeded:\n");
		tris_cli(a->fd, "Department:      %s\n", dei.orgunit);
		tris_cli(a->fd, "Organization:    %s\n", dei.org);
		tris_cli(a->fd, "City/Locality:   %s\n", dei.locality);
		tris_cli(a->fd, "State/Province:  %s\n", dei.stateprov);
		tris_cli(a->fd, "Country:         %s\n", dei.country);
		tris_cli(a->fd, "E-mail:          %s\n", dei.email);
		tris_cli(a->fd, "Phone:           %s\n", dei.phone);
		tris_cli(a->fd, "IP Address:      %s\n", dei.ipaddr);
	}
	return CLI_SUCCESS;
}

static char *dundi_show_peer(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	struct dundi_peer *peer;
	struct permission *p;
	char *order;
	...
	if (a->argc != 4)
		return CLI_SHOWUSAGE;
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE(&peers, peer, list) {
		if (!strcasecmp(tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid), a->argv[3]))
			break;
	}
	if (peer) {
		switch(peer->order) {
		...
		tris_cli(a->fd, "Peer:    %s\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		tris_cli(a->fd, "Model:   %s\n", model2str(peer->model));
		tris_cli(a->fd, "Host:    %s\n", peer->addr.sin_addr.s_addr ? tris_inet_ntoa(peer->addr.sin_addr) : "<Unspecified>");
		tris_cli(a->fd, "Dynamic: %s\n", peer->dynamic ? "yes" : "no");
		tris_cli(a->fd, "Reg:     %s\n", peer->registerid < 0 ? "No" : "Yes");
		tris_cli(a->fd, "In Key:  %s\n", tris_strlen_zero(peer->inkey) ? "<None>" : peer->inkey);
		tris_cli(a->fd, "Out Key: %s\n", tris_strlen_zero(peer->outkey) ? "<None>" : peer->outkey);
		if (!TRIS_LIST_EMPTY(&peer->include))
			tris_cli(a->fd, "Include logic%s:\n", peer->model & DUNDI_MODEL_OUTBOUND ? "" : " (IGNORED)");
		TRIS_LIST_TRAVERSE(&peer->include, p, list)
			tris_cli(a->fd, "-- %s %s\n", p->allow ? "include" : "do not include", p->name);
		if (!TRIS_LIST_EMPTY(&peer->permit))
			tris_cli(a->fd, "Query logic%s:\n", peer->model & DUNDI_MODEL_INBOUND ? "" : " (IGNORED)");
		TRIS_LIST_TRAVERSE(&peer->permit, p, list)
			tris_cli(a->fd, "-- %s %s\n", p->allow ? "permit" : "deny", p->name);
		cnt = 0;
		for (x = 0;x < DUNDI_TIMING_HISTORY; x++) {
			if (peer->lookups[x]) {
				if (!cnt)
					tris_cli(a->fd, "Last few query times:\n");
				tris_cli(a->fd, "-- %d. %s (%d ms)\n", x + 1, peer->lookups[x], peer->lookuptimes[x]);
				cnt++;
			}
		}
		if (cnt)
			tris_cli(a->fd, "Average query time: %d ms\n", peer->avgms);
	} else
		tris_cli(a->fd, "No such peer '%s'\n", a->argv[3]);
	TRIS_LIST_UNLOCK(&peers);
	return CLI_SUCCESS;
}

static char *dundi_show_peers(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT "%-20.20s %-15.15s %s %-10.10s %-8.8s %-15.15s\n"
	...
	TRIS_LIST_LOCK(&peers);
	tris_cli(a->fd, FORMAT2, "EID", "Host", "Model", "AvgTime", "Status");
	TRIS_LIST_TRAVERSE(&peers, peer, list) {
		char status[20];
		int print_line = -1;
		char srch[2000];
		...
		snprintf(srch, sizeof(srch), FORMAT, tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid),
					peer->addr.sin_addr.s_addr ? tris_inet_ntoa(peer->addr.sin_addr) : "(Unspecified)",
        ...
        if (print_line) {
			tris_cli(a->fd, FORMAT, tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid),
					peer->addr.sin_addr.s_addr ? tris_inet_ntoa(peer->addr.sin_addr) : "(Unspecified)",
					peer->dynamic ? "(D)" : "(S)", model2str(peer->model), avgms, status);
		}
	}
	tris_cli(a->fd, "%d dundi peers [%d online, %d offline, %d unmonitored]\n", total_peers, online_peers, offline_peers, unmonitored_peers);
	TRIS_LIST_UNLOCK(&peers);
	return CLI_SUCCESS;
#undef FORMAT
#undef FORMAT2
...
static char *dundi_show_trans(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT "%-16.16s:%5d %-5.5d %-5.5d %-3.3d %-3.3d %-3.3d\n"
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	TRIS_LIST_LOCK(&peers);
	tris_cli(a->fd, FORMAT2, "Remote", "Src", "Dst", "Tx", "Rx", "Ack");
	TRIS_LIST_TRAVERSE(&alltrans, trans, all) {
		tris_cli(a->fd, FORMAT, tris_inet_ntoa(trans->addr.sin_addr),
...
static char *dundi_show_entityid(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	char eid_str[20];
	switch (cmd) {
	case CLI_INIT:
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	TRIS_LIST_LOCK(&peers);
	tris_eid_to_str(eid_str, sizeof(eid_str), &global_eid);
	TRIS_LIST_UNLOCK(&peers);
	tris_cli(a->fd, "Global EID for this system is '%s'\n", eid_str);
	return CLI_SUCCESS;
}

static char *dundi_show_requests(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT "%-15s %-15s %-15s %-3.3d %-3.3d\n"
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	TRIS_LIST_LOCK(&peers);
	tris_cli(a->fd, FORMAT2, "Number", "Context", "Root", "Max", "Rsp");
	TRIS_LIST_TRAVERSE(&requests, req, list) {
		tris_cli(a->fd, FORMAT, req->number, req->dcontext,
			dundi_eid_zero(&req->root_eid) ? "<unspecified>" : tris_eid_to_str(eidstr, sizeof(eidstr), &req->root_eid), req->maxcount, req->respcount);
	}
	TRIS_LIST_UNLOCK(&peers);
	return CLI_SUCCESS;
#undef FORMAT
...
static char *dundi_show_mappings(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT "%-12.12s %-7s %-12.12s %-10.10s %-5.5s %-25.25s\n"
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	TRIS_LIST_LOCK(&peers);
	tris_cli(a->fd, FORMAT2, "DUNDi Cntxt", "Weight", "Local Cntxt", "Options", "Tech", "Destination");
	TRIS_LIST_TRAVERSE(&mappings, map, list) {
		snprintf(weight, sizeof(weight), "%d", get_mapping_weight(map));
		tris_cli(a->fd, FORMAT, map->dcontext, weight,
			tris_strlen_zero(map->lcontext) ? "<none>" : map->lcontext,
...
static char *dundi_show_precache(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
...
#define FORMAT "%-12.12s %-12.12s %02d:%02d:%02d\n"
	...
	if (a->argc != 3)
		return CLI_SHOWUSAGE;
	time(&now);
	tris_cli(a->fd, FORMAT2, "Number", "Context", "Expiration");
	TRIS_LIST_LOCK(&pcq);
	TRIS_LIST_TRAVERSE(&pcq, qe, list) {
		s = qe->expiration - now;
		h = s / 3600;
		s = s % 3600;
		m = s / 60;
		s = s % 60;
		tris_cli(a->fd, FORMAT, qe->number, qe->context, h,m,s);
	}
	TRIS_LIST_UNLOCK(&pcq);
...
static struct tris_cli_entry cli_dundi[] = {
	TRIS_CLI_DEFINE(dundi_set_debug, "Enable/Disable DUNDi debugging"),
	TRIS_CLI_DEFINE(dundi_store_history, "Enable/Disable DUNDi historic records"),
	TRIS_CLI_DEFINE(dundi_flush, "Flush DUNDi cache"),
	TRIS_CLI_DEFINE(dundi_show_peers, "Show defined DUNDi peers"),
	TRIS_CLI_DEFINE(dundi_show_trans, "Show active DUNDi transactions"),
	TRIS_CLI_DEFINE(dundi_show_entityid, "Display Global Entity ID"),
	TRIS_CLI_DEFINE(dundi_show_mappings, "Show DUNDi mappings"),
	TRIS_CLI_DEFINE(dundi_show_precache, "Show DUNDi precache"),
	TRIS_CLI_DEFINE(dundi_show_requests, "Show DUNDi requests"),
	TRIS_CLI_DEFINE(dundi_show_peer, "Show info on a specific DUNDi peer"),
	TRIS_CLI_DEFINE(dundi_do_precache, "Precache a number in DUNDi"),
	TRIS_CLI_DEFINE(dundi_do_lookup, "Lookup a number in DUNDi"),
	TRIS_CLI_DEFINE(dundi_do_query, "Query a DUNDi EID"),
};
...
static struct dundi_transaction *create_transaction(struct dundi_peer *p)
{
	...
	if (tid < 1)
		return NULL;
	if (!(trans = tris_calloc(1, sizeof(*trans))))
		return NULL;

	if (global_storehistory) {
		trans->start = tris_tvnow();
		tris_set_flag(trans, FLAG_STOREHIST);
	}
	trans->retranstimer = DUNDI_DEFAULT_RETRANS_TIMER;
	trans->autokillid = -1;
	if (p) {
	...
	trans->strans = tid;
	TRIS_LIST_INSERT_HEAD(&alltrans, trans, all);
...
static int dundi_xmit(struct dundi_packet *pack)
{
	...
	if (res < 0) {
		tris_log(LOG_WARNING, "Failed to transmit to '%s:%d': %s\n",
			tris_inet_ntoa(pack->parent->addr.sin_addr),
...
{
	if (pack->parent)
		TRIS_LIST_REMOVE(&pack->parent->packets, pack, list);
	TRIS_SCHED_DEL(sched, pack->retransid);
	if (needfree)
		tris_free(pack);
}
...
static void destroy_trans(struct dundi_transaction *trans, int fromtimeout)
{
	...
	int cnt;
	char eid_str[20];
	if (tris_test_flag(trans, FLAG_ISREG | FLAG_ISQUAL | FLAG_STOREHIST)) {
		TRIS_LIST_TRAVERSE(&peers, peer, list) {
			...
			if (peer->qualtrans == trans) {
				if (fromtimeout) {
					if (peer->lastms > -1)
						tris_log(LOG_NOTICE, "Peer '%s' has become UNREACHABLE!\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
					peer->lastms = -1;
				} else {
					ms = tris_tvdiff_ms(tris_tvnow(), peer->qualtx);
					if (ms < 1)
						ms = 1;
					if (ms < peer->maxms) {
						if ((peer->lastms >= peer->maxms) || (peer->lastms < 0))
							tris_log(LOG_NOTICE, "Peer '%s' has become REACHABLE!\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
					} else if (peer->lastms < peer->maxms) {
						tris_log(LOG_NOTICE, "Peer '%s' has become TOO LAGGED (%d ms)\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid), ms);
					}
					peer->lastms = ms;
				}
				peer->qualtrans = NULL;
			}
			if (tris_test_flag(trans, FLAG_STOREHIST)) {
				if (trans->parent && !tris_strlen_zero(trans->parent->number)) {
					if (!tris_eid_cmp(&trans->them_eid, &peer->eid)) {
						peer->avgms = 0;
						cnt = 0;
						if (peer->lookups[DUNDI_TIMING_HISTORY-1])
							tris_free(peer->lookups[DUNDI_TIMING_HISTORY-1]);
						...
						peer->lookuptimes[0] = tris_tvdiff_ms(tris_tvnow(), trans->start);
						peer->lookups[0] = tris_malloc(strlen(trans->parent->number) + strlen(trans->parent->dcontext) + 2);
						if (peer->lookups[0]) {
	...
	if (trans->parent) {
		...
		TRIS_LIST_REMOVE(&trans->parent->trans, trans, parentlist);
		if (TRIS_LIST_EMPTY(&trans->parent->trans)) {
			/* Wake up sleeper */
...
static int dundi_rexmit(const void *data)
{
	...
	if (pack->retrans < 1) {
		pack->retransid = -1;
		if (!tris_test_flag(pack->parent, FLAG_ISQUAL))
			tris_log(LOG_NOTICE, "Max retries exceeded to host '%s:%d' msg %d on call %d\n",
				tris_inet_ntoa(pack->parent->addr.sin_addr),
...
static int dundi_send(struct dundi_transaction *trans, int cmdresp, int flags, int final, struct dundi_ie_data *ied)
{
	...
	if (tris_test_flag(trans, FLAG_ENCRYPT))
		len += 384;
	pack = tris_calloc(1, len);
	if (pack) {
		pack->h = (struct dundi_hdr *)(pack->data);
		if (cmdresp != DUNDI_COMMAND_ACK) {
			pack->retransid = tris_sched_add(sched, trans->retranstimer, dundi_rexmit, pack);
			pack->retrans = DUNDI_DEFAULT_RETRANS - 1;
			TRIS_LIST_INSERT_HEAD(&trans->packets, pack, list);
		}
		pack->parent = trans;
		...
		if (final) {
			...
			tris_set_flag(trans, FLAG_FINAL);
		}
		...
		if (!res)
			res = dundi_xmit(pack);
		if (res)
			tris_log(LOG_NOTICE, "Failed to send packet to '%s'\n", tris_eid_to_str(eid_str, sizeof(eid_str), &trans->them_eid));
...
static int do_autokill(const void *data)
{
	...
	char eid_str[20];
	tris_log(LOG_NOTICE, "Transaction to '%s' took too long to ACK, destroying\n",
		tris_eid_to_str(eid_str, sizeof(eid_str), &trans->them_eid));
	trans->autokillid = -1;
...
{
	struct dundi_peer *p;
	if (!tris_eid_cmp(eid, us)) {
		dundi_ie_append_eid(ied, DUNDI_IE_EID_DIRECT, eid);
		return;
	}
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE(&peers, p, list) {
		if (!tris_eid_cmp(&p->eid, eid)) {
...
static int dundi_discover(struct dundi_transaction *trans)
{
	...
	int x;
	if (!trans->parent) {
		tris_log(LOG_WARNING, "Tried to discover a transaction with no parent?!?\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
	...
	if (trans->autokilltimeout)
		trans->autokillid = tris_sched_add(sched, trans->autokilltimeout, do_autokill, trans);
...
static int dundi_query(struct dundi_transaction *trans)
{
	...
	int x;
	if (!trans->parent) {
		tris_log(LOG_WARNING, "Tried to query a transaction with no parent?!?\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
	dundi_ie_append_short(&ied, DUNDI_IE_VERSION, DUNDI_DEFAULT_VERSION);
...
static int discover_transactions(struct dundi_request *dr)
{
	struct dundi_transaction *trans;
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE(&dr->trans, trans, parentlist) {
...
static int precache_transactions(struct dundi_request *dr, struct dundi_mapping *maps, int mapcount, int *expiration, int *foundanswers)
{
	...
	TRIS_LIST_TRAVERSE(&dr->trans, trans, parentlist) {
		if (trans->thread)
			tris_log(LOG_WARNING, "This shouldn't happen, really...\n");
		trans->thread = 1;
	}
	TRIS_LIST_UNLOCK(&peers);

	TRIS_LIST_TRAVERSE(&dr->trans, trans, parentlist) {
		if (!tris_test_flag(trans, FLAG_DEAD))
			precache_trans(trans, maps, mapcount, expiration, foundanswers);
	}

	/* Cleanup any that got destroyed in the mean time */
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&dr->trans, trans, parentlist) {
		trans->thread = 0;
		if (tris_test_flag(trans, FLAG_DEAD)) {
			tris_debug(1, "Our transaction went away!\n");
	...
	TRIS_LIST_TRAVERSE(&dr->trans, trans, parentlist) {
		...
		TRIS_LIST_TRAVERSE(&peers, peer, list) {
			if (has_permission(&peer->include, dr->dcontext) &&
			    tris_eid_cmp(&peer->eid, &trans->them_eid) &&
				(peer->order <= order)) {
				/* For each other transaction, make sure we don't
				   ask this EID about the others if they're not
				   already in the list */
				if (!tris_eid_cmp(&tmp, &peer->eid))
					x = -1;
				else {
					for (x=0;x<trans->eidcount;x++) {
						if (!tris_eid_cmp(&trans->eids[x], &peer->eid))
							break;
					}
				}
				if (x == trans->eidcount) {
...
static int append_transaction(struct dundi_request *dr, struct dundi_peer *p, int ttl, dundi_eid *avoid[])
{
	...
	if (tris_strlen_zero(dr->number))
		tris_debug(1, "Will query peer '%s' for '%s' (context '%s')\n", tris_eid_to_str(eid_str, sizeof(eid_str), &p->eid), tris_eid_to_str(eid_str2, sizeof(eid_str2), &dr->query_eid), dr->dcontext);
	else
		tris_debug(1, "Will query peer '%s' for '%s@%s'\n", tris_eid_to_str(eid_str, sizeof(eid_str), &p->eid), dr->number, dr->dcontext);
	...
	trans->eidcount = x;
	TRIS_LIST_INSERT_HEAD(&dr->trans, trans, parentlist);
...
static void cancel_request(struct dundi_request *dr)
{
	...
	TRIS_LIST_LOCK(&peers);
	while ((trans = TRIS_LIST_REMOVE_HEAD(&dr->trans, parentlist))) {
...
static void abort_request(struct dundi_request *dr)
{
	...
	TRIS_LIST_LOCK(&peers);
	while ((trans = TRIS_LIST_FIRST(&dr->trans))) {
			...
			if (p->order <= order) {
				...
				if((nocache || !(res = cache_lookup(dr, &p->eid, dr->crc32, &dr->expiration)))) {
					...
					/* Make sure we haven't already seen it and that it won't
					   affect our answer */
					for (x=0;avoid[x];x++) {
						if (!tris_eid_cmp(avoid[x], &p->eid) || !tris_eid_cmp(avoid[x], &p->us_eid)) {
							/* If not a direct connection, it affects our answer */
							if (directs && !directs[x])
								tris_clear_flag_nonstd(dr->hmd, DUNDI_HINT_UNAFFECTED);
							break;
						}
					}
					...
					if (allowconnect) {
						if (!avoid[x] && (!blockempty || !dundi_eid_zero(&p->us_eid))) {
							...
							tris_debug(1, "Avoiding '%s' in transaction\n", tris_eid_to_str(eid_str, sizeof(eid_str), avoid[x]));
						}
					}
				}
				*foundcache |= res;
...
static int register_request(struct dundi_request *dr, struct dundi_request **pending)
{
	...
	char eid_str[20];
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE(&requests, cur, list) {
		tris_debug(1, "Checking '%s@%s' vs '%s@%s'\n", cur->dcontext, cur->number,
			dr->dcontext, dr->number);
		if (!strcasecmp(cur->dcontext, dr->dcontext) &&
		    !strcasecmp(cur->number, dr->number) &&
		    (!tris_eid_cmp(&cur->root_eid, &dr->root_eid) || (cur->crc32 == dr->crc32))) {
			tris_debug(1, "Found existing query for '%s@%s' for '%s' crc '%08x'\n",
				cur->dcontext, cur->number, tris_eid_to_str(eid_str, sizeof(eid_str), &cur->root_eid), cur->crc32);
			*pending = cur;
			res = 1;
			break;
		}
	...
	if (!res) {
		tris_debug(1, "Registering request for '%s@%s' on behalf of '%s' crc '%08x'\n",
				dr->number, dr->dcontext, tris_eid_to_str(eid_str, sizeof(eid_str), &dr->root_eid), dr->crc32);
		/* Go ahead and link us in since nobody else is searching for this */
		TRIS_LIST_INSERT_HEAD(&requests, dr, list);
		*pending = NULL;
	}
	TRIS_LIST_UNLOCK(&peers);
	return res;
...
{
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_REMOVE(&requests, dr, list);
	TRIS_LIST_UNLOCK(&peers);
}
...
static int dundi_lookup_internal(struct dundi_result *result, int maxret, struct tris_channel *chan, const char *dcontext, const char *number, int ttl, int blockempty, struct dundi_hint_metadata *hmd, int *expiration, int cbypass, int modeselect, dundi_eid *skip, dundi_eid *avoid[], int direct[])
{
	int res;
	...
	if (chan && tris_check_hangup(chan))
		return 0;
	...
	memset(&dr, 0, sizeof(dr));
	if (pipe(dr.pfds)) {
		tris_log(LOG_WARNING, "pipe failed: %s\n" , strerror(errno));
		return -1;
	}
	dr.dr = result;
	dr.hmd = hmd;
	...
	tris_copy_string(dr.dcontext, dcontext ? dcontext : "e164", sizeof(dr.dcontext));
	tris_copy_string(dr.number, number, sizeof(dr.number));
	if (rooteid)
		dr.root_eid = *rooteid;
	res = register_request(&dr, &pending);
	if (res) {
		/* Already a request */
		if (rooteid && !tris_eid_cmp(&dr.root_eid, &pending->root_eid)) {
			/* This is on behalf of someone else.  Go ahead and close this out since
			   they'll get their answer anyway. */
			tris_debug(1, "Oooh, duplicate request for '%s@%s' for '%s'\n",
				dr.number,dr.dcontext,tris_eid_to_str(eid_str, sizeof(eid_str), &dr.root_eid));
			close(dr.pfds[0]);
			close(dr.pfds[1]);
			return -2;
		} else {
			/* Wait for the cache to populate */
			tris_debug(1, "Waiting for similar request for '%s@%s' for '%s'\n",
				dr.number,dr.dcontext,tris_eid_to_str(eid_str, sizeof(eid_str), &pending->root_eid));
			start = tris_tvnow();
			while(check_request(pending) && (tris_tvdiff_ms(tris_tvnow(), start) < ttlms) && (!chan || !tris_check_hangup(chan))) {
	...
	} while (skipped && !foundcache && TRIS_LIST_EMPTY(&dr.trans));
	/* If no TTL, abort and return 0 now after setting TTL expired hint.  Couldn't
	   ...
	   or not. */
	if (!ttl) {
		tris_set_flag_nonstd(hmd, DUNDI_HINT_TTL_EXPIRED);
		abort_request(&dr);
	...
	start = tris_tvnow();
	while (!TRIS_LIST_EMPTY(&dr.trans) && (tris_tvdiff_ms(tris_tvnow(), start) < ttlms) && (!chan || !tris_check_hangup(chan))) {
		ms = 100;
		tris_waitfor_n_fd(dr.pfds, 1, &ms, NULL);
	}
	if (chan && tris_check_hangup(chan))
		tris_debug(1, "Hrm, '%s' hungup before their query for %s@%s finished\n", chan->name, dr.number, dr.dcontext);
	cancel_request(&dr);
	unregister_request(&dr);
	res = dr.respcount;
...
int dundi_lookup(struct dundi_result *result, int maxret, struct tris_channel *chan, const char *dcontext, const char *number, int cbypass)
{
	struct dundi_hint_metadata hmd;
	dundi_eid *avoid[1] = { NULL, };
	int direct[1] = { 0, };
...
static void reschedule_precache(const char *number, const char *context, int expiration)
{
	...
	TRIS_LIST_LOCK(&pcq);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&pcq, qe, list) {
	...
	if ((prev = TRIS_LIST_FIRST(&pcq))) {
		while (TRIS_LIST_NEXT(prev, list) && ((TRIS_LIST_NEXT(prev, list))->expiration <= qe->expiration))
			prev = TRIS_LIST_NEXT(prev, list);
		TRIS_LIST_INSERT_AFTER(&pcq, prev, qe, list);
	} else
		TRIS_LIST_INSERT_HEAD(&pcq, qe, list);
	TRIS_LIST_UNLOCK(&pcq);
}
...
static void dundi_precache_full(void)
{
	...
	TRIS_LIST_TRAVERSE(&mappings, cur, list) {
		tris_log(LOG_NOTICE, "Should precache context '%s'\n", cur->dcontext);
		tris_rdlock_contexts();
		con = NULL;
		while ((con = tris_walk_contexts(con))) {
			if (strcasecmp(cur->lcontext, tris_get_context_name(con)))
				continue;
			/* Found the match, now queue them all up */
			tris_rdlock_context(con);
			e = NULL;
			while ((e = tris_walk_context_extensions(con, e)))
				reschedule_precache(tris_get_extension_name(e), cur->dcontext, 0);
...
static int dundi_precache_internal(const char *context, const char *number, int ttl, dundi_eid *avoids[])
{
	...
	if (!context)
		context = "e164";
	tris_debug(1, "Precache internal (%s@%s)!\n", number, context);
	...
	dr.dr = dr2;
	tris_copy_string(dr.number, number, sizeof(dr.number));
	tris_copy_string(dr.dcontext, context ? context : "e164", sizeof(dr.dcontext));
	dr.maxcount = MAX_RESULTS;
	dr.expiration = dundi_cache_time;
	dr.hmd = &hmd;
	dr.pfds[0] = dr.pfds[1] = -1;
	...
	if (foundanswers) {
		if (dr.expiration > 0)
			...
			tris_log(LOG_NOTICE, "Weird, expiration = %d, but need to precache for %s@%s?!\n", dr.expiration, dr.number, dr.dcontext);
	}
	start = tris_tvnow();
	while (!TRIS_LIST_EMPTY(&dr.trans) && (tris_tvdiff_ms(tris_tvnow(), start) < ttlms)) {
		if (dr.pfds[0] > -1) {
			ms = 100;
			tris_waitfor_n_fd(dr.pfds, 1, &ms, NULL);
		} else
...
TRIS_APP_OPTIONS(dundi_query_opts, BEGIN_OPTIONS
	TRIS_APP_OPTION('b', OPT_BYPASS_CACHE),
END_OPTIONS );

static int dundifunc_read(struct tris_channel *chan, const char *cmd, char *num, char *buf, size_t len)
{
	int results;
	int x;
	...
	if (tris_strlen_zero(num)) {
		tris_log(LOG_WARNING, "DUNDILOOKUP requires an argument (number)\n");
		return -1;
	}

	u = tris_module_user_add(chan);

	parse = tris_strdupa(num);

	TRIS_STANDARD_APP_ARGS(args, parse);

	if (!tris_strlen_zero(args.options)) {
		tris_app_parse_options(dundi_query_opts, &opts, NULL, args.options);
	}
	if (tris_strlen_zero(args.context)) {
		args.context = "e164";
	}

	results = dundi_lookup(dr, MAX_RESULTS, NULL, args.context, args.number, tris_test_flag(&opts, OPT_BYPASS_CACHE));
	if (results > 0) {
		sort_results(dr, results);
		for (x = 0; x < results; x++) {
			if (tris_test_flag(dr + x, DUNDI_FLAG_EXISTS)) {
...
static struct tris_custom_function dundi_function = {
	.name = "DUNDILOOKUP",
	.read = dundifunc_read,
};
...
static const struct tris_datastore_info dundi_result_datastore_info = {
	.type = "DUNDIQUERY",
	.destroy = drds_destroy_cb,
};

static int dundi_query_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct tris_module_user *u;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(number);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "DUNDIQUERY requires an argument (number)\n");
		tris_module_user_remove(u);
		return -1;
	}

	if (!chan) {
		tris_log(LOG_ERROR, "DUNDIQUERY can not be used without a channel!\n");
		tris_module_user_remove(u);
		return -1;
	}
	...
	if (!(drds = tris_calloc(1, sizeof(*drds)))) {
		tris_module_user_remove(u);
		return -1;
	}

	drds->id = tris_atomic_fetchadd_int((int *) &dundi_result_id, 1);
	snprintf(buf, len, "%u", drds->id);

	if (!(datastore = tris_datastore_alloc(&dundi_result_datastore_info, buf))) {
		drds_destroy(drds);
		tris_module_user_remove(u);
		return -1;
	}
	...
	drds->num_results = dundi_lookup(drds->results, ARRAY_LEN(drds->results), NULL, args.context,
		args.number, tris_test_flag(&opts, OPT_BYPASS_CACHE));
...
static struct tris_custom_function dundi_query_function = {
	.name = "DUNDIQUERY",
	.read = dundi_query_read,
};

static int dundi_result_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	struct tris_module_user *u;
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(id);
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "DUNDIRESULT requires an argument (id and resultnum)\n");
		goto finish;
	}

	if (!chan) {
		tris_log(LOG_ERROR, "DUNDRESULT can not be used without a channel!\n");
		goto finish;
	}
	...
	if (tris_strlen_zero(args.id)) {
		tris_log(LOG_ERROR, "A result ID must be provided to DUNDIRESULT\n");
		goto finish;
	}

	if (tris_strlen_zero(args.resultnum)) {
		tris_log(LOG_ERROR, "A result number must be given to DUNDIRESULT!\n");
		goto finish;
	}

	tris_channel_lock(chan);
	datastore = tris_channel_datastore_find(chan, &dundi_result_datastore_info, args.id);
	tris_channel_unlock(chan);

	if (!datastore) {
		tris_log(LOG_WARNING, "No DUNDi results found for query ID '%s'\n", args.id);
		goto finish;
	}
	...
	if (sscanf(args.resultnum, "%30u", &num) != 1) {
		tris_log(LOG_ERROR, "Invalid value '%s' for resultnum to DUNDIRESULT!\n",
			args.resultnum);
		goto finish;
	}
	...
	if (num && num <= drds->num_results) {
		...
		tris_log(LOG_WARNING, "Result number %u is not valid for DUNDi query results for ID %s!\n", num, args.id);
...
static struct tris_custom_function dundi_result_function = {
	.name = "DUNDIRESULT",
...
static void destroy_permissions(struct permissionlist *permlist)
{
	...
	while ((perm = TRIS_LIST_REMOVE_HEAD(permlist, list)))
		tris_free(perm);
}

static void destroy_peer(struct dundi_peer *peer)
{
	TRIS_SCHED_DEL(sched, peer->registerid);
	if (peer->regtrans)
		destroy_trans(peer->regtrans, 0);
	TRIS_SCHED_DEL(sched, peer->qualifyid);
...
static void prune_peers(void)
{
	...
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&peers, peer, list) {
		if (peer->dead) {
			TRIS_LIST_REMOVE_CURRENT(list);
			destroy_peer(peer);
		}
...
static void prune_mappings(void)
{
	...
	TRIS_LIST_LOCK(&peers);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&mappings, map, list) {
		if (map->dead) {
			TRIS_LIST_REMOVE_CURRENT(list);
			destroy_map(map);
		}
...
static void append_permission(struct permissionlist *permlist, const char *s, int allow)
{
	...
	if (!(perm = tris_calloc(1, sizeof(*perm) + strlen(s) + 1)))
		return;

	strcpy(perm->name, s);
	perm->allow = allow;

	TRIS_LIST_INSERT_TAIL(permlist, perm, list);
}
...
static void build_mapping(const char *name, const char *value)
{
	...
	if (!map) {
		if (!(map = tris_calloc(1, sizeof(*map))))
			return;
		TRIS_LIST_INSERT_HEAD(&mappings, map, list);
		map->dead = 1;
	}
	map->options = 0;
	...
	if ((x == 1) && tris_strlen_zero(fields[0])) {
		/* Placeholder mapping */
		tris_copy_string(map->dcontext, name, sizeof(map->dcontext));
		map->dead = 0;
	} else if (x >= 4) {
		tris_copy_string(map->dcontext, name, sizeof(map->dcontext));
		tris_copy_string(map->lcontext, fields[0], sizeof(map->lcontext));
		if ((sscanf(fields[1], "%30d", &map->_weight) == 1) && (map->_weight >= 0) && (map->_weight <= MAX_WEIGHT)) {
			tris_copy_string(map->dest, fields[3], sizeof(map->dest));
			if ((map->tech = str2tech(fields[2])))
				map->dead = 0;
		} else if (!strncmp(fields[1], "${", 2) && fields[1][strlen(fields[1]) - 1] == '}') {
			map->weightstr = tris_strdup(fields[1]);
			tris_copy_string(map->dest, fields[3], sizeof(map->dest));
			if ((map->tech = str2tech(fields[2])))
				map->dead = 0;
		} else {
			tris_log(LOG_WARNING, "Invalid weight '%s' specified, deleting entry '%s/%s'\n", fields[1], map->dcontext, map->lcontext);
		}
		for (y = 4;y < x; y++) {
			if (!strcasecmp(fields[y], "nounsolicited"))
				...
				tris_log(LOG_WARNING, "Don't know anything about option '%s'\n", fields[y]);
		}
	} else
		tris_log(LOG_WARNING, "Expected at least %d arguments in map, but got only %d\n", 4, x);
}
...
static int do_register(const void *data)
{
	...
	char eid_str[20];
	char eid_str2[20];
	tris_debug(1, "Register us as '%s' to '%s'\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->us_eid), tris_eid_to_str(eid_str2, sizeof(eid_str2), &peer->eid));
	peer->registerid = tris_sched_add(sched, default_expiration * 1000, do_register, data);
	/* Destroy old transaction if there is one */
	if (peer->regtrans)
		destroy_trans(peer->regtrans, 0);
	peer->regtrans = create_transaction(peer);
	if (peer->regtrans) {
		tris_set_flag(peer->regtrans, FLAG_ISREG);
		memset(&ied, 0, sizeof(ied));
		dundi_ie_append_short(&ied, DUNDI_IE_VERSION, DUNDI_DEFAULT_VERSION);
		...
		tris_log(LOG_NOTICE, "Unable to create new transaction for registering to '%s'!\n", tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
...
static void qualify_peer(struct dundi_peer *peer, int schedonly)
{
	...
	if (peer->maxms > 0) {
		...
		if (schedonly)
			when = 5000;
		peer->qualifyid = tris_sched_add(sched, when, do_qualify, peer);
		if (!schedonly)
			peer->qualtrans = create_transaction(peer);
		if (peer->qualtrans) {
			peer->qualtx = tris_tvnow();
			tris_set_flag(peer->qualtrans, FLAG_ISQUAL);
...
static void populate_addr(struct dundi_peer *peer, dundi_eid *eid)
{
	...
	int port, expire;
	char eid_str[20];
	tris_eid_to_str(eid_str, sizeof(eid_str), eid);
	if (!tris_db_get("dundi/dpeers", eid_str, data, sizeof(data))) {
		c = strchr(data, ':');
		if (c) {
			*c = '\0';
			c++;
...
static void build_peer(dundi_eid *eid, struct tris_variable *v, int *globalpcmode)
{
	struct dundi_peer *peer;
	struct tris_hostent he;
	struct hostent *hp;
	dundi_eid testeid;
	int needregister=0;
	char eid_str[20];
	...
	for (; v; v = v->next) {
		if (!strcasecmp(v->name, "inkey")) {
			tris_copy_string(peer->inkey, v->value, sizeof(peer->inkey));
		} else if (!strcasecmp(v->name, "outkey")) {
			tris_copy_string(peer->outkey, v->value, sizeof(peer->outkey));
		} else if (!strcasecmp(v->name, "host")) {
			if (!strcasecmp(v->value, "dynamic")) {
				peer->dynamic = 1;
			} else {
				hp = tris_gethostbyname(v->value, &he);
				if (hp) {
					memcpy(&peer->addr.sin_addr, hp->h_addr, sizeof(peer->addr.sin_addr));
					peer->dynamic = 0;
				} else {
					tris_log(LOG_WARNING, "Unable to find host '%s' at line %d\n", v->value, v->lineno);
					peer->dead = 1;
				}
			}
		} else if (!strcasecmp(v->name, "ustothem")) {
			if (!tris_str_to_eid(&testeid, v->value))
				peer->us_eid = testeid;
			else
				tris_log(LOG_WARNING, "'%s' is not a valid DUNDi Entity Identifier at line %d\n", v->value, v->lineno);
			...
			needregister = tris_true(v->value);
			...
			if (!strcasecmp(v->value, "primary"))
				...
				tris_log(LOG_WARNING, "'%s' is not a valid order, should be primary, secondary, tertiary or quartiary at line %d\n", v->value, v->lineno);
			}
			...
			if (!strcasecmp(v->value, "no")) {
				...
				tris_log(LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of dundi.conf\n",
					tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid), v->lineno);
				peer->maxms = 0;
			}
			...
			if (!strcasecmp(v->value, "inbound"))
				...
				tris_log(LOG_WARNING, "Unknown model '%s', should be 'none', 'outbound', 'inbound', or 'symmetric' at line %d\n",
					v->value, v->lineno);
			}
			...
			if (!strcasecmp(v->value, "inbound"))
				...
				tris_log(LOG_WARNING, "Unknown pcmodel '%s', should be 'none', 'outbound', 'inbound', or 'symmetric' at line %d\n",
					v->value, v->lineno);
			}
		}
	}
	...
	if (!peer->model && !peer->pcmodel) {
		tris_log(LOG_WARNING, "Peer '%s' lacks a model or pcmodel, discarding!\n",
			tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		peer->dead = 1;
	} else if ((peer->model & DUNDI_MODEL_INBOUND) && (peer->pcmodel & DUNDI_MODEL_OUTBOUND)) {
		tris_log(LOG_WARNING, "Peer '%s' may not be both inbound/symmetric model and outbound/symmetric precache model, discarding!\n",
			tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		peer->dead = 1;
	} else if ((peer->model & DUNDI_MODEL_OUTBOUND) && (peer->pcmodel & DUNDI_MODEL_INBOUND)) {
		tris_log(LOG_WARNING, "Peer '%s' may not be both outbound/symmetric model and inbound/symmetric precache model, discarding!\n",
			tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		peer->dead = 1;
	} else if (!TRIS_LIST_EMPTY(&peer->include) && !(peer->model & DUNDI_MODEL_OUTBOUND) && !(peer->pcmodel & DUNDI_MODEL_INBOUND)) {
		tris_log(LOG_WARNING, "Peer '%s' is supposed to be included in outbound searches but isn't an outbound peer or inbound precache!\n",
			tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
	} else if (!TRIS_LIST_EMPTY(&peer->permit) && !(peer->model & DUNDI_MODEL_INBOUND) && !(peer->pcmodel & DUNDI_MODEL_OUTBOUND)) {
		tris_log(LOG_WARNING, "Peer '%s' is supposed to have permission for some inbound searches but isn't an inbound peer or outbound precache!\n",
			tris_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
	} else {
		if (needregister) {
			peer->registerid = tris_sched_add(sched, 2000, do_register, peer);
		}
		qualify_peer(peer, 1);
	}
	TRIS_LIST_UNLOCK(&peers);
...
static int dundi_helper(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *data, int flag)
{
	struct dundi_result results[MAX_RESULTS];
	int res;
	int x;
	...
	if (!strncasecmp(context, "macro-", 6)) {
		if (!chan) {
			tris_log(LOG_NOTICE, "Can't use macro mode without a channel!\n");
			return -1;
		}
		...
		if (!strcasecmp(exten, "s")) {
			...
			if (tris_strlen_zero(exten))
				exten = chan->exten;
			if (tris_strlen_zero(exten)) {
				tris_log(LOG_WARNING, "Called in Macro mode with no ARG1 or MACRO_EXTEN?\n");
				return -1;
			}
		}
		if (tris_strlen_zero(data))
	...
	for (x=0;x<res;x++) {
		if (tris_test_flag(results + x, flag))
			found++;
	}
	if (found >= priority)
		return 1;
...
static int dundi_exists(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	return dundi_helper(chan, context, exten, priority, data, DUNDI_FLAG_EXISTS);
}

static int dundi_canmatch(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	return dundi_helper(chan, context, exten, priority, data, DUNDI_FLAG_CANMATCH);
}

static int dundi_exec(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	struct dundi_result results[MAX_RESULTS];
	int res;
	int x=0;
	...
	if (res > 0) {
		...
		for (x=0;x<res;x++) {
			if (tris_test_flag(results + x, DUNDI_FLAG_EXISTS)) {
				if (!--priority)
					break;
			}
		}
...
static int dundi_matchmore(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	return dundi_helper(chan, context, exten, priority, data, DUNDI_FLAG_MATCHMORE);
}

static struct tris_switch dundi_switch = {
	.name        = "DUNDi",
...
static int set_config(char *config_file, struct sockaddr_in* sin, int reload)
{
	...
	if (!(cfg = tris_config_load(config_file, config_flags)) || cfg == CONFIG_STATUS_FILEINVALID) {
	...
	if (!gethostname(hn, sizeof(hn)-1)) {
		hp = tris_gethostbyname(hn, &he);
		if (hp) {
			memcpy(&sin2.sin_addr, hp->h_addr, sizeof(sin2.sin_addr));
			tris_copy_string(ipaddr, tris_inet_ntoa(sin2.sin_addr), sizeof(ipaddr));
		} else
			tris_log(LOG_WARNING, "Unable to look up host '%s'\n", hn);
	} else
		tris_log(LOG_WARNING, "Unable to get host name!\n");
	TRIS_LIST_LOCK(&peers);

	memcpy(&global_eid, &tris_eid_default, sizeof(global_eid));

	global_storehistory = 0;
	tris_copy_string(secretpath, "dundi", sizeof(secretpath));
	...
	while(v) {
		if (!strcasecmp(v->name, "port")){
			...
			if(last_port==0){
				...
				tris_log(LOG_WARNING, "change to port ignored until next trismedia re-start\n");
		} else if (!strcasecmp(v->name, "bindaddr")) {
			struct hostent *hep;
			struct tris_hostent hent;
			hep = tris_gethostbyname(v->value, &hent);
			if (hep) {
				memcpy(&sin->sin_addr, hep->h_addr, sizeof(sin->sin_addr));
			} else
				tris_log(LOG_WARNING, "Invalid host/IP '%s'\n", v->value);
			...
			if ((sscanf(v->value, "%30d", &x) == 1) && (x > 0) && (x < DUNDI_DEFAULT_TTL)) {
				...
				tris_log(LOG_WARNING, "'%s' is not a valid TTL at line %d, must be number from 1 to %d\n",
			...
			if (!tris_str_to_eid(&testeid, v->value))
				...
				tris_log(LOG_WARNING, "Invalid global endpoint identifier '%s' at line %d\n", v->value, v->lineno);
			...
			tris_copy_string(dept, v->value, sizeof(dept));
		} else if (!strcasecmp(v->name, "organization")) {
			tris_copy_string(org, v->value, sizeof(org));
		} else if (!strcasecmp(v->name, "locality")) {
			tris_copy_string(locality, v->value, sizeof(locality));
		} else if (!strcasecmp(v->name, "stateprov")) {
			tris_copy_string(stateprov, v->value, sizeof(stateprov));
		} else if (!strcasecmp(v->name, "country")) {
			tris_copy_string(country, v->value, sizeof(country));
		} else if (!strcasecmp(v->name, "email")) {
			tris_copy_string(email, v->value, sizeof(email));
		} else if (!strcasecmp(v->name, "phone")) {
			tris_copy_string(phone, v->value, sizeof(phone));
		} else if (!strcasecmp(v->name, "storehistory")) {
			global_storehistory = tris_true(v->value);
		} else if (!strcasecmp(v->name, "cachetime")) {
			if ((sscanf(v->value, "%30d", &x) == 1)) {
				dundi_cache_time = x;
			} else {
				tris_log(LOG_WARNING, "'%s' is not a valid cache time at line %d. Using default value '%d'.\n",
	...
	TRIS_LIST_UNLOCK(&peers);
	mark_mappings();
	v = tris_variable_browse(cfg, "mappings");
	while(v) {
	...
	while(cat) {
		if (strcasecmp(cat, "general") && strcasecmp(cat, "mappings")) {
			/* Entries */
			if (!tris_str_to_eid(&testeid, cat))
				build_peer(&testeid, tris_variable_browse(cfg, cat), &globalpcmodel);
			else if (!strcasecmp(cat, "*")) {
				build_peer(&empty_eid, tris_variable_browse(cfg, cat), &globalpcmodel);
				any_peer = find_peer(NULL);
			} else
				tris_log(LOG_NOTICE, "Ignoring invalid EID entry '%s'\n", cat);
		}
...
static int unload_module(void)
{
	...
	dundi_shutdown = 1;
	if (previous_netthreadid != TRIS_PTHREADT_NULL) {
		pthread_kill(previous_netthreadid, SIGURG);
		pthread_join(previous_netthreadid, NULL);
	}
	if (previous_precachethreadid != TRIS_PTHREADT_NULL) {
		pthread_kill(previous_precachethreadid, SIGURG);
		pthread_join(previous_precachethreadid, NULL);
	}
 	if (previous_clearcachethreadid != TRIS_PTHREADT_NULL) {
 		pthread_cancel(previous_clearcachethreadid);
 		pthread_join(previous_clearcachethreadid, NULL);
 	}

	tris_cli_unregister_multiple(cli_dundi, ARRAY_LEN(cli_dundi));
	tris_unregister_switch(&dundi_switch);
	tris_custom_function_unregister(&dundi_function);
	tris_custom_function_unregister(&dundi_query_function);
	tris_custom_function_unregister(&dundi_result_function);
	close(netsocket);
...
static int load_module(void)
{
	...
	if (bind(netsocket, (struct sockaddr *) &sin, sizeof(sin))) {
		tris_log(LOG_ERROR, "Unable to bind to %s port %d: %s\n",
			tris_inet_ntoa(sin.sin_addr), ntohs(sin.sin_port), strerror(errno));
		return TRIS_MODULE_LOAD_FAILURE;
	}

	tris_netsock_set_qos(netsocket, tos, 0, "DUNDi");
	...
	tris_cli_register_multiple(cli_dundi, ARRAY_LEN(cli_dundi));
	if (tris_register_switch(&dundi_switch))
		tris_log(LOG_ERROR, "Unable to register DUNDi switch\n");
	tris_custom_function_register(&dundi_function);
	tris_custom_function_register(&dundi_query_function);
	tris_custom_function_register(&dundi_result_function);

	tris_verb(2, "DUNDi Ready and Listening on %s port %d\n", tris_inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));

	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "Distributed Universal Number Discovery (DUNDi)",
		.load = load_module,
