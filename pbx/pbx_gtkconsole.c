...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 126356 $")
...
TRIS_MUTEX_DEFINE_STATIC(verb_lock);
...
static void remove_module(void)
{
	...
	if (GTK_CLIST(modules)->selection) {
		...
		gdk_threads_leave();
		res = tris_unload_resource(module, 0);
		gdk_threads_enter();
		if (res) {
...
static void file_ok_sel(GtkWidget *w, GtkFileSelection *fs)
{
	...
	char buf[256];
	snprintf(tmp, sizeof(tmp), "%s/", tris_config_TRIS_MODULE_DIR);
	if (!strncmp(module, (char *)tmp, strlen(tmp))) 
		module += strlen(tmp);
	gdk_threads_leave();
	if (tris_load_resource(module)) {
...
static void add_module(void)
{
	char tmp[PATH_MAX];
	GtkWidget *filew;
	snprintf(tmp, sizeof(tmp), "%s/*.so", tris_config_TRIS_MODULE_DIR);
...
static int mod_update(void)
{
	...
	tris_update_module_list(add_mod, NULL);
	if (module)
...
static void exit_now(GtkWidget *widget, gpointer data)
{
	...
	tris_loader_unregister(mod_update);
	gtk_main_quit();
	inuse--;
	tris_update_use_count();
	res = tris_unregister_verbose(verboser);
	tris_unload_resource("pbx_gtkconsole", 0);
	tris_verb(2, "GTK Console Monitor Exiting\n");
	/* XXX Trying to quit after calling this makes trismedia segfault XXX */
}
...
#if 0
	/* Clever... */
	tris_cli_command(clipipe[1], "quit");
#else
	kill(getpid(), SIGTERM);
#endif
}
...
static int cli_activate(void)
{
	...
	if (strlen(buf)) {
		tris_cli_command(clipipe[1], buf);
	}
	return TRUE;
}
...
static int show_console(void)
{
	...
	quit = gtk_button_new_with_label("Quit Trismedia");
	...
	gtk_window_set_title(GTK_WINDOW(window), "Trismedia Console");
	gtk_widget_grab_focus(cli);
	tris_pthread_create(&console_thread, NULL, consolethread, NULL);
	/* XXX Okay, seriously fix me! XXX */
	usleep(100000);
	res = tris_register_verbose(verboser);
	gtk_clist_freeze(GTK_CLIST(verb));
	tris_loader_register(mod_update);
	gtk_clist_thaw(GTK_CLIST(verb));
	gdk_input_add(clipipe[0], GDK_INPUT_READ, cliinput, NULL);
	mod_update();
	update_statusbar("Trismedia Console Ready");
	return 0;
}
...
{
	if (pipe(clipipe)) {
		tris_log(LOG_WARNING, "Unable to create CLI pipe\n");
	...
	if (gtk_init_check(NULL, NULL))  {
		if (!show_console()) {
			inuse++;
			tris_update_use_count();
			tris_verb(2, "Launched GTK Console monitor\n");
		} else
			tris_log(LOG_WARNING, "Unable to start GTK console\n");
	} else {
		if (option_debug)
			tris_log(LOG_DEBUG, "Unable to start GTK console monitor -- ignoring\n");
		else
			tris_verb(2, "GTK is not available -- skipping monitor\n");
	}
	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "GTK Console",
		.load = load_module,
