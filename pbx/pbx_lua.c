...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 231191 $")
...
static int lua_load_extensions(lua_State *L, struct tris_channel *chan);
...
static int lua_find_extension(lua_State *L, const char *context, const char *exten, int priority, tris_switch_f *func, int push_func);
...
static lua_State *lua_get_state(struct tris_channel *chan);

static int exists(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data);
static int canmatch(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data);
static int matchmore(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data);
static int exec(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data);

TRIS_MUTEX_DEFINE_STATIC(config_file_lock);
...
static const struct tris_datastore_info lua_datastore = {
	.type = "lua",
...
static int lua_pbx_exec(lua_State *L)
{
	...
	app_name = tris_strdupa(lua_tostring(L, -1));
	lua_pop(L, 1);
	...
	context = tris_strdupa(lua_tostring(L, -1));
	lua_pop(L, 1);
	
	lua_getfield(L, LUA_REGISTRYINDEX, "exten");
	exten = tris_strdupa(lua_tostring(L, -1));
	lua_pop(L, 1);
	...
	if (nargs > 1) {
		...
		if (!lua_isnil(L, 2))
			tris_build_string(&data_next, &data_left, "%s", luaL_checkstring(L, 2));

		for (i = 3; i <= nargs; i++) {
			if (lua_isnil(L, i))
				tris_build_string(&data_next, &data_left, ",");
			else
				tris_build_string(&data_next, &data_left, ",%s", luaL_checkstring(L, i));
		}
	}
	
	tris_verb(3, "Executing [%s@%s:%d] %s(\"%s\", \"%s\")\n",
...
static int lua_get_variable_value(lua_State *L)
{
	...
	lua_getfield(L, 1, "name");
	name = tris_strdupa(lua_tostring(L, -1));
	lua_pop(L, 1);
	...
	/* if this is a dialplan function then use tris_func_read(), otherwise
	 * use pbx_retrieve_variable() */
	if (!tris_strlen_zero(name) && name[strlen(name) - 1] == ')') {
		value = tris_func_read(chan, name, workspace, LUA_BUF_SIZE) ? NULL : workspace;
	} else {
...
/*!
 * \brief Create the 'application' metatable, used to execute trismedia
...
{
	struct tris_channel *chan;
	char *name = tris_strdupa(luaL_checkstring(L, 2));
	char *value = NULL;
	char *workspace = alloca(LUA_BUF_SIZE);
	workspace[0] = '\0';
...
static int lua_func_read(lua_State *L)
{
	...
	tris_build_string(&fullname_next, &fullname_left, "%s(", name);
	
	if (nargs > 1) {
		int i;

		if (!lua_isnil(L, 2))
			tris_build_string(&fullname_next, &fullname_left, "%s", luaL_checkstring(L, 2));

		for (i = 3; i <= nargs; i++) {
			if (lua_isnil(L, i))
				tris_build_string(&fullname_next, &fullname_left, ",");
			else
				tris_build_string(&fullname_next, &fullname_left, ",%s", luaL_checkstring(L, i));
		}
	}
 ...
 *
 * \return This function returns the result of the tris_autoservice_start()
 ...
 *
 * \return This function returns the result of the tris_autoservice_stop()
 * function as a boolean to its lua caller.
 */
static int lua_autoservice_stop(lua_State *L)
{
	...
	res = tris_autoservice_stop(chan);
...
static int lua_check_hangup(lua_State *L)
{
	...
	lua_pushboolean(L, tris_check_hangup(chan));
	return 1;
}
...
static int lua_error_function(lua_State *L)
{
	...
	/* pass number arguments right through back to trismedia*/
	if (lua_isnumber(L, -1)) {
		return 1;
	}
...
static int lua_register_switches(lua_State *L)
{
	...
	for (lua_pushnil(L); lua_next(L, extensions); lua_pop(L, 1)) {
		...
		con = tris_context_find_or_create(&local_contexts, local_table, context_str, registrar);
		if (!con) {
		...
		/* register the switch */
		if (tris_context_add_switch2(con, "Lua", "", 0, registrar)) {
			/* remove extensions table and context key and value */
			lua_pop(L, 3);
...
static char *lua_read_extensions_file(lua_State *L, long *size)
{
	FILE *f;
	char *data;
	char *path = alloca(strlen(config) + strlen(tris_config_TRIS_CONFIG_DIR) + 2);
	sprintf(path, "%s/%s", tris_config_TRIS_CONFIG_DIR, config);
...
static lua_State *lua_get_state(struct tris_channel *chan)
{
	...
	if (!chan) {
		...
		if (!L) {
			tris_log(LOG_ERROR, "Error allocating lua_State, no memory\n");
			return NULL;
		}

		if (lua_load_extensions(L, NULL)) {
			const char *error = lua_tostring(L, -1);
			tris_log(LOG_ERROR, "Error loading extensions.lua: %s\n", error);
			lua_close(L);
			return NULL;
		}
		return L;
		...
		datastore = tris_channel_datastore_find(chan, &lua_datastore, NULL);
		tris_channel_unlock(chan);

		if (!datastore) {
			/* nothing found, allocate a new lua state */
			datastore = tris_datastore_alloc(&lua_datastore, NULL);
			if (!datastore) {
				tris_log(LOG_ERROR, "Error allocation channel datastore for lua_State\n");
				return NULL;
			}
			...
			if (lua_load_extensions(L, chan)) {
				const char *error = lua_tostring(L, -1);
				tris_log(LOG_ERROR, "Error loading extensions.lua for %s: %s\n", chan->name, error);
...
static int exists(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	int res;
	lua_State *L;
	struct tris_module_user *u = tris_module_user_add(chan);
	if (!u) {
		tris_log(LOG_ERROR, "Error adjusting use count, probably could not allocate memory\n");
		return 0;
	}
...
static int exec(struct tris_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	if (!lua_find_extension(L, context, exten, priority, &exists, 1)) {
		...
		tris_log(LOG_ERROR, "Could not find extension %s in context %s\n", exten, context);
		if (!chan) lua_close(L);
		tris_module_user_remove(u);
		return -1;
	}
	...
	if (res) {
		if (res == LUA_ERRRUN) {
			...
			if (lua_isnumber(L, -1)) {
				...
				const char *error = lua_tostring(L, -1);
				tris_log(LOG_ERROR, "Error executing lua extension: %s\n", error);
			}
		} else if (res == LUA_ERRERR) {
			res = -1;
			tris_log(LOG_ERROR, "Error in the lua error handler (this is probably a bug in pbx_lua)\n");
...
static int lua_find_extension(lua_State *L, const char *context, const char *exten, int priority, tris_switch_f *func, int push_func)
{
	...
	lua_getglobal(L, "extensions");
	if (lua_isnil(L, -1)) {
		tris_log(LOG_ERROR, "Unable to find 'extensions' table in extensions.lua\n");
		lua_pop(L, 1);
		return 0;
	}
	...
	for (i = 1; i < lua_objlen(L, context_order_table) + 1; i++) {
		...
		if (func == &matchmore)
			match = tris_extension_close(e, exten, E_MATCHMORE);
		else if (func == &canmatch)
			match = tris_extension_close(e, exten, E_CANMATCH);
		else
			match = tris_extension_match(e, exten);
...
static struct tris_switch lua_switch = {
        .name		= "Lua",
...
{
	int res = TRIS_MODULE_LOAD_SUCCESS;
...
static int unload_module(void)
{
	...
	tris_unregister_switch(&lua_switch);
	lua_free_extensions();
	return 0;
}
...
static int load_module(void)
{
	...
	if (tris_register_switch(&lua_switch)) {
		tris_log(LOG_ERROR, "Unable to register LUA PBX switch\n");
		return TRIS_MODULE_LOAD_DECLINE;
	}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_GLOBAL_SYMBOLS, "Lua PBX Switch",
		.load = load_module,
