...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 184632 $");
...
static const struct {
	...
	{ "mwi", TRIS_EVENT_MWI },
	{ "device_state", TRIS_EVENT_DEVICE_STATE_CHANGE },
};
...
struct subscribe_event {
	TRIS_LIST_ENTRY(subscribe_event) entry;
...
struct publish_event {
	TRIS_LIST_ENTRY(publish_event) entry;
...
struct event_channel {
	TRIS_RWLIST_ENTRY(event_channel) entry;
	TRIS_LIST_HEAD_NOLOCK(, subscribe_event) subscribe_events;
	TRIS_LIST_HEAD_NOLOCK(, publish_event) publish_events;
	SaEvtChannelHandleT handle;
	char name[1];
};

static TRIS_RWLIST_HEAD_STATIC(event_channels, event_channel);
...
static void queue_event(struct tris_event *tris_event)
{
	tris_event_queue_and_cache(tris_event);
}
	...
	const SaEvtEventHandleT event_handle, const SaSizeT event_datalen)
{
	...
	struct tris_event *event_dup, *event = (void *) buf;
	SaAisErrorT ais_res;
	SaSizeT len = sizeof(buf);

	if (event_datalen > len) {
		tris_log(LOG_ERROR, "Event received with size %u, which is too big\n"
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error retrieving event payload: %s\n", 
			ais_err2str(ais_res));
		return;
	}

	if (!tris_eid_cmp(&tris_eid_default, tris_event_get_ie_raw(event, TRIS_EVENT_IE_EID))) {
		/* Don't feed events back in that originated locally. */
		return;
	}

	if (!(event_dup = tris_malloc(len)))
		return;
...
static const char *type_to_filter_str(enum tris_event_type type)
{
...
static void tris_event_cb(const struct tris_event *tris_event, void *data)
{
	...
	tris_log(LOG_DEBUG, "Got an event to forward\n");

	if (tris_eid_cmp(&tris_eid_default, tris_event_get_ie_raw(tris_event, TRIS_EVENT_IE_EID))) {
		/* If the event didn't originate from this server, don't send it back out. */
		tris_log(LOG_DEBUG, "Returning here\n");
		return;
	}

	ais_res = saEvtEventAllocate(event_channel->handle, &event_handle);
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error allocating event: %s\n", ais_err2str(ais_res));
		tris_log(LOG_DEBUG, "Returning here\n");
		return;
	}
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error getting local node name: %s\n", ais_err2str(ais_res));
		goto return_event_free;
	}

	filter_str = type_to_filter_str(tris_event_get_type(tris_event));
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error setting event attributes: %s\n", ais_err2str(ais_res));
		goto return_event_free;
	}

	ais_res = saEvtEventPublish(event_handle, 
		tris_event, tris_event_get_size(tris_event), &event_id);
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error publishing event: %s\n", ais_err2str(ais_res));
		goto return_event_free;
	}
...
return_event_free:
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error freeing allocated event: %s\n", ais_err2str(ais_res));
	}
	tris_log(LOG_DEBUG, "Returning here (event_free)\n");
}

static char *ais_evt_show_event_channels(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	TRIS_RWLIST_RDLOCK(&event_channels);
	TRIS_RWLIST_TRAVERSE(&event_channels, event_channel, entry) {
		...
		TRIS_LIST_TRAVERSE(&event_channel->publish_events, publish_event, entry) {
			tris_cli(a->fd, "=== ==> Publishing Event Type: %s\n", 
				type_to_filter_str(publish_event->type));
		}
		
		TRIS_LIST_TRAVERSE(&event_channel->subscribe_events, subscribe_event, entry) {
			tris_cli(a->fd, "=== ==> Subscribing to Event Type: %s\n", 
	...
	TRIS_RWLIST_UNLOCK(&event_channels);
...
static struct tris_cli_entry ais_cli[] = {
	TRIS_CLI_DEFINE(ais_evt_show_event_channels, "Show configured event channels"),
};

static void add_publish_event(struct event_channel *event_channel, const char *event_type)
{
	int i;
	enum tris_event_type type = -1;
	struct publish_event *publish_event;
	...
	if (type == -1) {
		tris_log(LOG_WARNING, "publish_event option given with invalid value '%s'\n", event_type);
		return;
	}

	if (type == TRIS_EVENT_DEVICE_STATE_CHANGE && tris_enable_distributed_devstate()) {
		return;
	}

	if (!(publish_event = tris_calloc(1, sizeof(*publish_event)))) {
		return;
	}

	publish_event->type = type;
	tris_log(LOG_DEBUG, "Subscribing to event type %d\n", type);
	publish_event->sub = tris_event_subscribe(type, tris_event_cb, event_channel,
		TRIS_EVENT_IE_END);
	tris_event_dump_cache(publish_event->sub);

	TRIS_LIST_INSERT_TAIL(&event_channel->publish_events, publish_event, entry);
}
...
static void add_subscribe_event(struct event_channel *event_channel, const char *event_type)
{
	...
	if (type == -1) {
		tris_log(LOG_WARNING, "subscribe_event option given with invalid value '%s'\n", event_type);
		return;
	}
	...
	if (!(subscribe_event = tris_calloc(1, sizeof(*subscribe_event)))) {
		return;
	}

	subscribe_event->type = type;
	subscribe_event->id = tris_atomic_fetchadd_int(&unique_id, +1);

	ais_res = set_egress_subscription(event_channel, subscribe_event);
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error setting up egress subscription: %s\n",
			ais_err2str(ais_res));
		free(subscribe_event);
		return;
	}

	TRIS_LIST_INSERT_TAIL(&event_channel->subscribe_events, subscribe_event, entry);
}

static void build_event_channel(struct tris_config *cfg, const char *cat)
{
	...
	TRIS_RWLIST_WRLOCK(&event_channels);
	TRIS_RWLIST_TRAVERSE(&event_channels, event_channel, entry) {
	...
	TRIS_RWLIST_UNLOCK(&event_channels);
	if (event_channel) {
		tris_log(LOG_WARNING, "Event channel '%s' was specified twice in "
			"configuration.  Second instance ignored.\n", cat);
		return;
	}

	if (!(event_channel = tris_calloc(1, sizeof(*event_channel) + strlen(cat))))
		return;

	strcpy(event_channel->name, cat);
	tris_copy_string((char *) sa_name.value, cat, sizeof(sa_name.value));
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error opening event channel: %s\n", ais_err2str(ais_res));
		free(event_channel);
		return;
	}
	...
	for (var = tris_variable_browse(cfg, cat); var; var = var->next) {
		if (!strcasecmp(var->name, "type")) {
			...
			tris_log(LOG_WARNING, "Event channel '%s' contains invalid option '%s'\n",
				event_channel->name, var->name);
		}
	}

	TRIS_RWLIST_WRLOCK(&event_channels);
	TRIS_RWLIST_INSERT_TAIL(&event_channels, event_channel, entry);
	TRIS_RWLIST_UNLOCK(&event_channels);
}
...
static void load_config(void)
{
	...
	if (!(cfg = tris_config_load(filename, config_flags)) || cfg == CONFIG_STATUS_FILEINVALID)
		return;
...
{
	tris_event_unsubscribe(publish_event->sub);
	...
	struct subscribe_event *subscribe_event)
{
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error unsubscribing: %s\n", ais_err2str(ais_res));
	}
...
static void event_channel_destroy(struct event_channel *event_channel)
{
	...
	while ((publish_event = TRIS_LIST_REMOVE_HEAD(&event_channel->publish_events, entry)))
		publish_event_destroy(publish_event);
	while ((subscribe_event = TRIS_LIST_REMOVE_HEAD(&event_channel->subscribe_events, entry)))
		subscribe_event_destroy(event_channel, subscribe_event);

	ais_res = saEvtChannelClose(event_channel->handle);
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Error closing event channel '%s': %s\n",
...
static void destroy_event_channels(void)
{
	...
	TRIS_RWLIST_WRLOCK(&event_channels);
	while ((event_channel = TRIS_RWLIST_REMOVE_HEAD(&event_channels, entry)))
		event_channel_destroy(event_channel);
	TRIS_RWLIST_UNLOCK(&event_channels);
}

int tris_ais_evt_load_module(void)
{
	SaAisErrorT ais_res;

	ais_res = saEvtInitialize(&evt_handle, &evt_callbacks, &ais_version);
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Could not initialize eventing service: %s\n",
			ais_err2str(ais_res));
		return -1;
	}
...
int tris_ais_evt_unload_module(void)
{
	SaAisErrorT ais_res;
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Problem stopping eventing service: %s\n", 
			ais_err2str(ais_res));
		return -1;
	}
