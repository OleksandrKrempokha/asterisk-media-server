 ...
 * internal binary format of events between Trismedia servers over a network.
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 179164 $");
...
static void *dispatch_thread_handler(void *data)
{
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Failed to retrieve select fd for CLM service.  "
	...
	if (ais_res != SA_AIS_OK) {
		tris_log(LOG_ERROR, "Failed to retrieve select fd for EVT service.  "
			"This module will not operate.\n");
		return NULL;
	}
	...
	while (!dispatch_thread.stop) {
		...
		res = tris_select(max_fd + 1, &read_fds, NULL, NULL, NULL);
		if (res == -1 && errno != EINTR && errno != EAGAIN) {
			tris_log(LOG_ERROR, "Select error (%s) dispatch thread going away now, "
...
{
	if (tris_ais_clm_load_module())
		goto return_error;

	if (tris_ais_evt_load_module())
		goto evt_failed;

	if (tris_pthread_create_background(&dispatch_thread.id, NULL, 
		dispatch_thread_handler, NULL)) {
		tris_log(LOG_ERROR, "Error starting AIS dispatch thread.\n");
...
dispatch_failed:
	tris_ais_evt_unload_module();
evt_failed:
	tris_ais_clm_unload_module();
return_error:
...
static int unload_module(void)
{
	...
	if (dispatch_thread.id != TRIS_PTHREADT_NULL) {
...
TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "SAForum AIS");
