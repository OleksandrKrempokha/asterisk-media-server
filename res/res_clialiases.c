...
struct cli_alias {
	struct tris_cli_entry cli_entry; /*!< Actual CLI structure used for this alias */
...
static int alias_hash_cb(const void *obj, const int flags)
{
	...
	return tris_str_hash(alias->cli_entry.command);
}
...
static void alias_destroy(void *obj)
{
	...
	tris_cli_unregister(&alias->cli_entry);

	return;
}

/*! \brief Function which passes through an aliased CLI command to the real one */
static char *cli_alias_passthrough(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	case CLI_GENERATE:
		...
		if (!strncasecmp(alias->alias, alias->real_cmd, strlen(alias->alias))) {
			generator = NULL;
		} else if (!tris_strlen_zero(a->word)) {
			struct tris_str *real_cmd = tris_str_alloca(strlen(alias->real_cmd) + strlen(line) + 1);
			tris_str_append(&real_cmd, 0, "%s%s", alias->real_cmd, line);
			generator = tris_cli_generator(tris_str_buffer(real_cmd), a->word, a->n);
		} else {
			generator = tris_cli_generator(alias->real_cmd, a->word, a->n);
		}
		ao2_ref(alias, -1);
		return generator;
	}
	...
	if (a->argc != e->args) {
		struct tris_str *real_cmd = tris_str_alloca(2048);
		int i;

		tris_str_append(&real_cmd, 0, "%s", alias->real_cmd);

		/* Add the additional arguments that have been passed in */
		for (i = e->args + 1; i <= a->argc; i++) {
			tris_str_append(&real_cmd, 0, " %s", a->argv[i - 1]);
		}

		tris_cli_command(a->fd, tris_str_buffer(real_cmd));
	} else {
		tris_cli_command(a->fd, alias->real_cmd);
	}
...
static char *alias_show(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
#define FORMAT "%-50.50s %-50.50s\n"
	...
	tris_cli(a->fd, FORMAT, "Alias Command", "Real Command");

	i = ao2_iterator_init(cli_aliases, 0);
	for (; (alias = ao2_iterator_next(&i)); ao2_ref(alias, -1)) {
		tris_cli(a->fd, FORMAT, alias->alias, alias->real_cmd);
	}
...
static struct tris_cli_entry cli_alias[] = {
	TRIS_CLI_DEFINE(alias_show, "Show CLI command aliases"),
};
...
static void load_config(int reload)
{
	...
	struct cli_alias *alias;
	struct tris_variable *v, *v1;

	if (!(cfg = tris_config_load(config_file, config_flags)) || cfg == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_ERROR, "res_clialiases configuration file '%s' not found\n", config_file);
		return;
	...
	for (v = tris_variable_browse(cfg, "general"); v; v = v->next) {
		if (strcmp(v->name, "template")) {
			tris_log(LOG_WARNING, "%s is not a correct option in [%s]\n", v->name, "general");
			continue;
		}
		/* Read in those there CLI aliases */
		for (v1 = tris_variable_browse(cfg, v->value); v1; v1 = v1->next) {
			...
			tris_cli_register(&alias->cli_entry);
			ao2_link(cli_aliases, alias);
			tris_verbose(VERBOSE_PREFIX_2 "Aliased CLI command '%s' to '%s'\n", v1->name, v1->value);
			ao2_ref(alias, -1);
		}
	}
...
static int unload_module(void)
{
	...
	tris_cli_unregister_multiple(cli_alias, ARRAY_LEN(cli_alias));
...
static int load_module(void)
{
	...
	tris_cli_register_multiple(cli_alias, ARRAY_LEN(cli_alias));

	return TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_DEFAULT, "CLI Aliases",
		.load = load_module,
