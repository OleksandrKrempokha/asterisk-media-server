...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 240280 $")
...
#define RES_CONFIG_LDAP_DEFAULT_BASEDN "trismedia"

TRIS_MUTEX_DEFINE_STATIC(ldap_lock);
...
static char *realtime_ldap_status(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
struct ldap_table_config {
	...
	struct tris_variable *attributes;  /*!< attribute names conversion */
	struct tris_variable *delimiters;  /*!< the current delimiter is semicolon, so we are not using this variable */
	TRIS_LIST_ENTRY(ldap_table_config) entry;
	/* TODO: Make proxies work */
};

/*! \brief Should be locked before using it */
static TRIS_LIST_HEAD_NOLOCK_STATIC(table_configs, ldap_table_config);
static struct ldap_table_config *base_table_config;
static struct ldap_table_config *static_table_config;

static struct tris_cli_entry ldap_cli[] = {
	TRIS_CLI_DEFINE(realtime_ldap_status, "Shows connection information for the LDAP RealTime driver"),
};
...
static struct ldap_table_config *table_config_new(const char *table_name)
{
	...
	if (table_name) {
		if (!(p->table_name = tris_strdup(table_name))) {
			free(p);
			return NULL;
		}
	}
...
static struct ldap_table_config *table_config_for_table_name(const char *table_name)
{
	...
	TRIS_LIST_TRAVERSE(&table_configs, c, entry) {
...
static struct tris_variable *variable_named(struct tris_variable *var, const char *name)
{
...
/* takes a linked list of \a tris_variable variables, finds the one with the name variable_value
 * and returns the number of semicolons in the value for that \a tris_variable
 */
static int semicolon_count_var(struct tris_variable *var)
{
	struct tris_variable *var_value = variable_named(var, "variable_value");

	if (!var_value)
		return 0;

	tris_debug(1, "LINE(%d) semicolon_count_var: %s\n", __LINE__, var_value->value);
	...
	const char *attribute_name, const char *attribute_value)
{
	...
	if (tris_strlen_zero(attribute_name) || tris_strlen_zero(attribute_value))
		return;

	if (!(var = tris_variable_new(attribute_name, attribute_value, table_config->table_name)))
		return;
...
static void table_configs_free(void)
{
	...
	while ((c = TRIS_LIST_REMOVE_HEAD(&table_configs, entry))) {
		if (c->table_name)
			free(c->table_name);
		if (c->additional_filter)
			free(c->additional_filter);
		if (c->attributes)
			tris_variables_destroy(c->attributes);
		free(c);
	}
	...
	const char *attribute_name)
{
	...
	for (i = 0; i < ARRAY_LEN(configs); i++) {
		struct tris_variable *attribute;
 ...
 * \return a linked list of tris_variable variables.
 **/
static struct tris_variable *realtime_ldap_entry_to_var(struct ldap_table_config *table_config,
	LDAPMessage *ldap_entry)
{
	BerElement *ber = NULL;
	struct tris_variable *var = NULL;
	struct tris_variable *prev = NULL;
	int is_delimited = 0;
	int i = 0;
	...
	while (ldap_attribute_name) {
		...
		if (values) {
			...
			for (v = values; *v; v++) {
				value = *v;
				valptr = value->bv_val;
				tris_debug(2, "LINE(%d) attribute_name: %s LDAP value: %s\n", __LINE__, attribute_name, valptr);
				...
				if (valptr) {
					...
					if (is_delimited) {
						i = 0;
						pos = 0;
						while (!tris_strlen_zero(valptr + i)) {
							if (valptr[i] == ';'){
								valptr[i] = '\0';
								if (prev) {
									prev->next = tris_variable_new(attribute_name, &valptr[pos], table_config->table_name);
									if (prev->next) {
										prev = prev->next;
									}
								} else {
									prev = var = tris_variable_new(attribute_name, &valptr[pos], table_config->table_name);
								}
								pos = i + 1;
							}
							i++;
 ...
 *	
 * \return \a vars - an array of tris_variable variables terminated with a null.
 **/
static struct tris_variable **realtime_ldap_result_to_vars(struct ldap_table_config *table_config,
	LDAPMessage *ldap_result_msg, unsigned int *entries_count_ptr)
{
	...
	for (tot_count = 0; ldap_entry; tot_count++){ 
		struct tris_variable *tmp = realtime_ldap_entry_to_var(table_config, ldap_entry);
		tot_count += semicolon_count_var(tmp);
		ldap_entry = ldap_next_entry(ldapConn, ldap_entry);
		tris_variables_destroy(tmp);
	}
	...
	vars = tris_calloc(sizeof(struct tris_variable *), tot_count + 1);
	...
	for (entry_index = 0; ldap_entry; ) { 
		...
		do { /* while delim_count */
					...
					for (v = values; *v; v++) {
						...
						if (valptr) {
							if (delim_value == NULL 
								...
								delim_value = tris_strdup(valptr);

								if ((delim_tot_count = semicolon_count_str(delim_value)) > 0) {
									tris_debug(4, "LINE(%d) is delimited %d times: %s\n", __LINE__, delim_tot_count, delim_value);
									is_delimited = 1;
								}
							}
							...
							if (is_delimited != 0 
								...
								for (i = pos; !tris_strlen_zero(valptr + i); i++) {
									tris_debug(4, "LINE(%d) DELIM pos: %d i: %d\n", __LINE__, pos, i);
									if (delim_value[i] == ';') {
										delim_value[i] = '\0';

										tris_debug(2, "LINE(%d) DELIM - attribute_name: %s value: %s pos: %d\n", __LINE__, attribute_name, &delim_value[pos], pos);
							
										if (prev) {
											prev->next = tris_variable_new(attribute_name, &delim_value[pos], table_config->table_name);
											if (prev->next) {
												prev = prev->next;
											}
										} else {
											prev = var = tris_variable_new(attribute_name, &delim_value[pos], table_config->table_name);
										}
										pos = i + 1;
								...
								if (tris_strlen_zero(valptr + i)) {
									tris_debug(4, "LINE(%d) DELIM pos: %d i: %d delim_count: %d\n", __LINE__, pos, i, delim_count);
									/* Last delimited value */
									tris_debug(4, "LINE(%d) DELIM - attribute_name: %s value: %s pos: %d\n", __LINE__, attribute_name, &delim_value[pos], pos);
									if (prev) {
										prev->next = tris_variable_new(attribute_name, &delim_value[pos], table_config->table_name);
										if (prev->next) {
											prev = prev->next;
								...
								tris_debug(2, "LINE(%d) attribute_name: %s value: %s\n", __LINE__, attribute_name, valptr);

								if (prev) {
									prev->next = tris_variable_new(attribute_name, valptr, table_config->table_name);
									if (prev->next) {
										prev = prev->next;
									}
								} else {
									prev = var = tris_variable_new(attribute_name, valptr, table_config->table_name);
								}
							}
						}
					} /*!< for (v = values; *v; v++) */
			...
			if (static_table_config == table_config) {
				if (option_debug > 2) {
					const struct tris_variable *tmpdebug = variable_named(var, "variable_name");
					const struct tris_variable *tmpdebug2 = variable_named(var, "variable_value");
					if (tmpdebug && tmpdebug2) {
						tris_debug(3, "LINE(%d) Added to vars - %s = %s\n", __LINE__, tmpdebug->value, tmpdebug2->value);
					}
				}
		...
		if (static_table_config != table_config) {
			tris_debug(3, "LINE(%d) Added to vars - non static\n", __LINE__);
...
*/
static struct tris_variable *ldap_loadentry(struct ldap_table_config *table_config,
					   const char *dn)
{
	if (!table_config) {
		tris_log(LOG_ERROR, "No table config\n");
		return NULL;
	} else {
		struct tris_variable **vars = NULL;
		struct tris_variable *var = NULL;
		int result = -1;
		LDAPMessage *ldap_result_msg = NULL;
		int tries = 0;

		tris_debug(2, "ldap_loadentry dn=%s\n", dn);
		...
		if (result != LDAP_SUCCESS) {
			...
			tris_debug(2, "dn=%s\n", dn);
			tris_debug(2, "Query Failed because: %s\n",
				ldap_err2string(result));
			tris_mutex_unlock(&ldap_lock);
			return NULL;
		} else {
			int num_entry = 0;
			unsigned int *entries_count_ptr = NULL; /*!< not using this */
			if ((num_entry = ldap_count_entries(ldapConn, ldap_result_msg)) > 0) {
				tris_debug(3, "num_entry: %d\n", num_entry);

				vars = realtime_ldap_result_to_vars(table_config, ldap_result_msg, entries_count_ptr);
				if (num_entry > 1)
					tris_log(LOG_NOTICE, "More than one entry for dn=%s. Take only 1st one\n", dn);
			} else {
				tris_debug(2, "Could not find any entry dn=%s.\n", dn);
			}
		}
		...
		if (vars != NULL) {
			...
			vars = tris_realloc(vars, sizeof(struct tris_variable *));
		}
...
static char *substituted(struct tris_channel *channel, const char *string)
{
#define MAXRESULT	2048
	char *ret_string = NULL;

	if (!tris_strlen_zero(string)) {
		ret_string = tris_calloc(1, MAXRESULT);
		pbx_substitute_variables_helper(channel, string, ret_string, MAXRESULT - 1);
	}
	tris_debug(2, "substituted: string: '%s' => '%s' \n",
		string, ret_string);
	return ret_string;
}

/*! \brief caller should free returned pointer */
static char *cleaned_basedn(struct tris_channel *channel, const char *basedn)
{
	char *cbasedn = NULL;
	if (basedn) {
		char *p = NULL;
...
static void append_var_and_value_to_filter(struct tris_str **filter,
	...
	const char *name, const char *value)
{
	...
	tris_debug(2, "name='%s' value='%s'\n", name, value);

	if (like_pos) {
		int len = like_pos - name;
		name = new_name = tris_strdupa(name);
		new_name[len] = '\0';
		value = new_value = tris_strdupa(value);
	...
	tris_str_append(filter, 0, "(%s=%s)", name, value);
}

/*! \brief LDAP base function 
 * \return a null terminated array of tris_variable (one per entry) or NULL if no entry is found or if an error occured
 * caller should free the returned array and tris_variables
...
*/
static struct tris_variable **realtime_ldap_base_ap(unsigned int *entries_count_ptr,
	const char *basedn, const char *table_name, va_list ap)
{
	...
	struct tris_str *filter = NULL;
	int tries = 0;
	int result = 0;
	LDAPMessage *ldap_result_msg = NULL;

	if (!table_name) {
		tris_log(LOG_WARNING, "No table_name specified.\n");
		tris_free(clean_basedn);
		return NULL;
	} 

	if (!(filter = tris_str_create(80))) {
		tris_free(clean_basedn);
		return NULL;
	}
	...
	if (!newparam || !newval) {
		tris_log(LOG_WARNING, "Realtime retrieval requires at least 1 parameter"
	...
	if (!table_config) {
		tris_log(LOG_WARNING, "No table named '%s'.\n", table_name);
		tris_mutex_unlock(&ldap_lock);
		tris_free(filter);
		tris_free(clean_basedn);
		return NULL;
	...
	tris_str_append(&filter, 0, "(&");

	if (table_config && table_config->additional_filter)
		tris_str_append(&filter, 0, "%s", table_config->additional_filter);
	if (table_config != base_table_config && base_table_config && 
		base_table_config->additional_filter) {
		tris_str_append(&filter, 0, "%s", base_table_config->additional_filter);
	}
	...
	do {
		...
		result = ldap_search_ext_s(ldapConn, clean_basedn,
				  LDAP_SCOPE_SUBTREE, tris_str_buffer(filter), NULL, 0, NULL, NULL, NULL, LDAP_NO_LIMIT,
				  &ldap_result_msg);
		if (result != LDAP_SUCCESS && is_ldap_connect_error(result)) {
			tris_log(LOG_DEBUG, "Failed to query database. Try %d/10\n",
				tries + 1);
			if (++tries < 10) {
				usleep(1);
				if (ldapConn) {
	...
	if (result != LDAP_SUCCESS) {
		tris_log(LOG_WARNING, "Failed to query database. Check debug for more info.\n");
		tris_log(LOG_WARNING, "Query: %s\n", tris_str_buffer(filter));
		tris_log(LOG_WARNING, "Query Failed because: %s\n", ldap_err2string(result));
	} else {
		...
		if (ldap_count_entries(ldapConn, ldap_result_msg) > 0) {
			...
			tris_debug(1, "Could not find any entry matching %s in base dn %s.\n",
				tris_str_buffer(filter), clean_basedn);
		}
		...
		if (vars) {
			struct tris_variable **p = vars;
			while (*p) {
				struct tris_variable *append_var = NULL;
				struct tris_variable *tmp = *p;
				while (tmp) {
					if (strcasecmp(tmp->name, "accountBaseDN") == 0) {
						/* Get the variable to compare with for the defaults */
						struct tris_variable *base_var = ldap_loadentry(table_config, tmp->value);
						
						while (base_var) {
							struct tris_variable *next = base_var->next;
							struct tris_variable *test_var = *p;
							...
							if (base_var_found) {
								base_var->next = NULL;
								tris_variables_destroy(base_var);
								base_var = next;
							} else {
								if (append_var)
...
static struct tris_variable **realtime_ldap_base(unsigned int *entries_count_ptr,
...
/*! \brief See Trismedia doc
*
* For Realtime Dynamic(i.e., switch, queues, and directory) -- I think
*/
static struct tris_variable *realtime_ldap(const char *basedn,
					  const char *table_name, va_list ap)
{
	struct tris_variable **vars = realtime_ldap_base_ap(NULL, basedn, table_name, ap);
	struct tris_variable *var = NULL;

	if (vars) {
		struct tris_variable *last_var = NULL;
		struct tris_variable **p = vars;
		while (*p) {
			if (last_var) {
				while (last_var->next)
...
* however, the tris_load_realtime wil match on wildcharacters also depending on what the mode is set to
* this is an area of trismedia that could do with a lot of modification
* I think this function returns Realtime dynamic objects
*/
static struct tris_config *realtime_multi_ldap(const char *basedn,
      const char *table_name, va_list ap)
{
	struct tris_variable **vars =
	...
	if (vars) {
		cfg = tris_config_new();
		if (!cfg) {
			tris_log(LOG_ERROR, "Unable to create a config!\n");
		} else {
			struct tris_variable **p = vars;

			while (*p) {
				struct tris_category *cat = NULL;
				cat = tris_category_new("", table_name, -1);
				if (!cat) {
					tris_log(LOG_ERROR, "Unable to create a new category!\n");
					break;
				} else {
					struct tris_variable *var = *p;
					while (var) {
...
*	called on a reload
*/
static struct tris_config *config_ldap(const char *basedn, const char *table_name,
	const char *file, struct tris_config *cfg, struct tris_flags config_flags, const char *sugg_incl, const char *who_asked)
{
	unsigned int vars_count = 0;
	struct tris_variable **vars;
	int i = 0;
	struct tris_variable *new_v = NULL;
	struct tris_category *cur_cat = NULL;
	const char *last_category = NULL;
	int last_category_metric = 0;
	struct category_and_metric *categories;
	struct tris_variable **p;

	if (tris_strlen_zero(file) || !strcasecmp(file, RES_CONFIG_LDAP_CONF)) {
		tris_log(LOG_ERROR, "Cannot configure myself.\n");
		return NULL;
	}
	...
	if (!vars) {
		tris_log(LOG_WARNING, "Could not find config '%s' in database.\n", file);
		return NULL;
	}
	 ...
	 */
	if (!(categories = tris_calloc(sizeof(*categories), vars_count)))
		return NULL;

	for (vars_count = 0, p = vars; *p; p++) {
		struct tris_variable *category = variable_named(*p, "category");
		struct tris_variable *cat_metric = variable_named(*p, "cat_metric");
		struct tris_variable *var_name = variable_named(*p, "variable_name");
		struct tris_variable *var_val = variable_named(*p, "variable_value");
		struct tris_variable *var_metric = variable_named(*p, "var_metric");
		struct tris_variable *dn = variable_named(*p, "dn");
			
		tris_debug(1, "category: %s\n", category->value);
		tris_debug(1, "var_name: %s\n", var_name->value);
		tris_debug(1, "var_val: %s\n", var_val->value);
		tris_debug(1, "cat_metric: %s\n", cat_metric->value);
	...
	for (i = 0; i < vars_count; i++) {
		if (!strcmp(categories[i].variable_name, "#include")) {
			...
			if (!tris_config_internal_load(categories[i].variable_value, cfg, flags, "", who_asked))
				break;
			continue;
		}
		...
		if (!last_category || strcmp(last_category, categories[i].name) ||
			...
			cur_cat = tris_category_new(categories[i].name, table_name, -1);
			if (!cur_cat)
				break;
		...
		if (!(new_v = tris_variable_new(categories[i].variable_name, categories[i].variable_value, table_name)))
			break;
	...
	const char *lookup, va_list ap)
{
	...
	ldap_mods = tris_calloc(sizeof(LDAPMod *), mods_size);
	ldap_mods[0] = tris_calloc(1, sizeof(LDAPMod));

	ldap_mods[0]->mod_op = LDAP_MOD_REPLACE;
	ldap_mods[0]->mod_type = tris_strdup(newparam);

	ldap_mods[0]->mod_values = tris_calloc(sizeof(char *), 2);
	ldap_mods[0]->mod_values[0] = tris_strdup(newval);
	...
	while ((newparam = va_arg(ap, const char *))) {
		...
		for (i = 0; i < mods_size - 1; i++) {
			if (ldap_mods[i]&& !strcmp(ldap_mods[i]->mod_type, newparam)) {
				...
				ldap_mods[i]->mod_values[0] = tris_realloc(ldap_mods[i]->mod_values[0], sizeof(char) * (strlen(ldap_mods[i]->mod_values[0]) + strlen(newval) + 2));
		...
		if (!mod_exists) {
			mods_size++;
			ldap_mods = tris_realloc(ldap_mods, sizeof(LDAPMod *) * mods_size);
			ldap_mods[mods_size - 1] = NULL;
			
			ldap_mods[mods_size - 2] = tris_calloc(1, sizeof(LDAPMod));

			ldap_mods[mods_size - 2]->mod_type = tris_calloc(sizeof(char), strlen(newparam) + 1);
			...
			if (strlen(newval) == 0) {
				...
				ldap_mods[mods_size - 2]->mod_values = tris_calloc(sizeof(char *), 2);
				ldap_mods[mods_size - 2]->mod_values[0] = tris_calloc(sizeof(char), strlen(newval) + 1);
	...
	do {
		...
		result = ldap_search_ext_s(ldapConn, clean_basedn,
				  ...
				  &ldap_result_msg);
		if (result != LDAP_SUCCESS && is_ldap_connect_error(result)) {
			tris_log(LOG_WARNING, "Failed to query database. Try %d/3\n",
				tries + 1);
			tries++;
			if (tries < 3) {
				usleep(500000L * tries);
	...
	if (result != LDAP_SUCCESS) {
		tris_log(LOG_WARNING, "Failed to query directory. Check debug for more info.\n");
		tris_log(LOG_WARNING, "Query: %s\n", tris_str_buffer(filter));
		tris_log(LOG_WARNING, "Query Failed because: %s\n",
			ldap_err2string(result));
	...
	if ((num_entries = ldap_count_entries(ldapConn, ldap_result_msg)) > 0) {
		...
		for (i = 0; ldap_entry; i++) { 
			...
			if ((error = ldap_modify_ext_s(ldapConn, dn, ldap_mods, NULL, NULL)) != LDAP_SUCCESS) 
				tris_log(LOG_ERROR, "Couldn't modify dn:%s because %s", dn, ldap_err2string(error));
...
static int update2_ldap(const char *basedn, const char *table_name, va_list ap)
{
	...
	ldap_mods[0]->mod_op = LDAP_MOD_REPLACE;
	ldap_mods[0]->mod_type = tris_calloc(sizeof(char), strlen(newparam) + 1);
	strcpy(ldap_mods[0]->mod_type, newparam);

	ldap_mods[0]->mod_values = tris_calloc(sizeof(char), 2);
	ldap_mods[0]->mod_values[0] = tris_calloc(sizeof(char), strlen(newval) + 1);
...
static struct tris_config_engine ldap_engine = {
	.name = "ldap",
...
{
	if (parse_config() < 0) {
		tris_log(LOG_NOTICE, "Cannot load LDAP RealTime driver.\n");
		return 0;
	}
	...
	if (!ldap_reconnect()) 
		tris_log(LOG_WARNING, "Couldn't establish connection. Check debug.\n");

	tris_config_engine_register(&ldap_engine);
	tris_verb(1, "LDAP RealTime driver loaded.\n");
	tris_cli_register_multiple(ldap_cli, ARRAY_LEN(ldap_cli));
...
static int unload_module(void)
{
	...
	tris_cli_unregister_multiple(ldap_cli, ARRAY_LEN(ldap_cli));
	tris_config_engine_deregister(&ldap_engine);
	tris_verb(1, "LDAP RealTime unloaded.\n");
...
static int reload(void)
{
	...
	if (parse_config() < 0) {
		tris_log(LOG_NOTICE, "Cannot reload LDAP RealTime driver.\n");
		tris_mutex_unlock(&ldap_lock);
		return 0;
	}		
	...
	tris_verb(2, "LDAP RealTime reloaded.\n");
...
int parse_config(void)
{
	...
	config = tris_config_load(RES_CONFIG_LDAP_CONF, config_flags);
	if (config == CONFIG_STATUS_FILEMISSING || config == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_WARNING, "Cannot load configuration %s\n", RES_CONFIG_LDAP_CONF);
		return -1;
	}

	if (!(s = tris_variable_retrieve(config, "_general", "user"))) {
		tris_log(LOG_WARNING, "No directory user found, anonymous binding as default.\n");
		user[0] = '\0';
	} else 
		tris_copy_string(user, s, sizeof(user));

	if (!tris_strlen_zero(user)) {
		if (!(s = tris_variable_retrieve(config, "_general", "pass"))) {
			tris_log(LOG_WARNING, "No directory password found, using 'trismedia' as default.\n");
			tris_copy_string(pass, "trismedia", sizeof(pass));
		} else {
			tris_copy_string(pass, s, sizeof(pass));
		}
	}

	/* URL is preferred, use host and port if not found */
	if ((s = tris_variable_retrieve(config, "_general", "url"))) {
		tris_copy_string(url, s, sizeof(url));
	} else if ((host = tris_variable_retrieve(config, "_general", "host"))) {
		if (!(s = tris_variable_retrieve(config, "_general", "port")) || sscanf(s, "%5d", &port) != 1 || port > 65535) {
			tris_log(LOG_NOTICE, "No directory port found, using 389 as default.\n");
			port = 389;
		}

		snprintf(url, sizeof(url), "ldap://%s:%d", host, port);
	} else {
		tris_log(LOG_ERROR, "No directory URL or host found.\n");
		tris_config_destroy(config);
		return -1;
	}

	if (!(s = tris_variable_retrieve(config, "_general", "basedn"))) {
		tris_log(LOG_ERROR, "No LDAP base dn found, using '%s' as default.\n", RES_CONFIG_LDAP_DEFAULT_BASEDN);
		tris_copy_string(base_distinguished_name, RES_CONFIG_LDAP_DEFAULT_BASEDN, sizeof(base_distinguished_name));
	} else 
		tris_copy_string(base_distinguished_name, s, sizeof(base_distinguished_name));

	if (!(s = tris_variable_retrieve(config, "_general", "version")) && !(s = tris_variable_retrieve(config, "_general", "protocol"))) {
		tris_log(LOG_NOTICE, "No explicit LDAP version found, using 3 as default.\n");
		version = 3;
	} else if (sscanf(s, "%30d", &version) != 1 || version < 1 || version > 6) {
		tris_log(LOG_WARNING, "Invalid LDAP version '%s', using 3 as default.\n", s);
		version = 3;
	}

	table_configs_free();

	while ((category_name = tris_category_browse(config, category_name))) {
		...
		struct tris_variable *var = tris_variable_browse(config, category_name);
		...
		if (var) {
			...
			if (!table_config) {
				...
				TRIS_LIST_INSERT_HEAD(&table_configs, table_config, entry);
				if (is_general)
			...
			for (; var; var = var->next) {
				if (!strcasecmp(var->name, "additionalFilter")) {
					table_config->additional_filter = tris_strdup(var->value);
				} else {
...
static int ldap_reconnect(void)
{
	...
	if (ldapConn) {
		tris_debug(2, "Everything seems fine.\n");
		return 1;
	}

	if (tris_strlen_zero(url)) {
		tris_log(LOG_ERROR, "Not enough parameters to connect to ldap database\n");
		return 0;
	}

	if (LDAP_SUCCESS != ldap_initialize(&ldapConn, url)) {
		tris_log(LOG_ERROR, "Failed to init ldap connection to '%s'. Check debug for more info.\n", url);
		return 0;
	}

	if (LDAP_OPT_SUCCESS != ldap_set_option(ldapConn, LDAP_OPT_PROTOCOL_VERSION, &version)) {
		tris_log(LOG_WARNING, "Unable to set LDAP protocol version to %d, falling back to default.\n", version);
	}

	if (!tris_strlen_zero(user)) {
		tris_debug(2, "bind to '%s' as user '%s'\n", url, user);
		...
		tris_debug(2, "bind %s anonymously\n", url);
		cred.bv_val = NULL;
		cred.bv_len = 0;
		bind_result = ldap_sasl_bind_s(ldapConn, NULL, LDAP_SASL_SIMPLE, &cred, NULL, NULL, NULL);
	}
	if (bind_result == LDAP_SUCCESS) {
		tris_debug(2, "Successfully connected to database.\n");
		connect_time = time(NULL);
		return 1;
	} else {
		tris_log(LOG_WARNING, "bind failed: %s\n", ldap_err2string(bind_result));
		ldap_unbind_ext_s(ldapConn, NULL, NULL);
		ldapConn = NULL;
		return 0;
	}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_GLOBAL_SYMBOLS, "LDAP realtime interface",
	.load = load_module,
