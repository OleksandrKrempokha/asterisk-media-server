...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 217647 $")
 ...
 * as a tris_variable list.
 *
 * \retval var on success
 * \retval NULL on failure
*/
static struct tris_variable *realtime_odbc(const char *database, const char *table, va_list ap)
{
	struct odbc_obj *obj;
	SQLHSTMT stmt;
	char sql[1024];
	...
	if (!obj) {
		tris_log(LOG_ERROR, "No database handle available with the name of '%s' (check res_odbc.conf)\n", database);
		return NULL;
	}
	...
	snprintf(sql, sizeof(sql), "SELECT * FROM %s WHERE %s%s ?%s", table, newparam, op,
		strcasestr(newparam, "LIKE") && !tris_odbc_backslash_is_escape(obj) ? " ESCAPE '\\'" : "");
	...
	stmt = tris_odbc_prepare_and_execute(obj, custom_prepare, &cps);
	...
	for (x = 0; x < colcount; x++) {
		...
		if (indicator == SQL_NULL_DATA)
			rowdata[0] = '\0';
		else if (tris_strlen_zero(rowdata)) {
			...
			tris_copy_string(rowdata, " ", sizeof(rowdata));
		}
		...
		while(stringp) {
			...
			if (!tris_strlen_zero(tris_strip(chunk))) {
				if (prev) {
					prev->next = tris_variable_new(coltitle, chunk, "");
					if (prev->next)
						prev = prev->next;
				} else 
					prev = var = tris_variable_new(coltitle, chunk, "");
			}
		}
	}
 ...
 * Return results as an tris_config variable.
 *
 * \retval var on success
 * \retval NULL on failure
*/
static struct tris_config *realtime_multi_odbc(const char *database, const char *table, va_list ap)
{
	struct odbc_obj *obj;
	SQLHSTMT stmt;
	char sql[1024];
		...
		for (x=0;x<colcount;x++) {
			...
			while(stringp) {
				...
				if (!tris_strlen_zero(tris_strip(chunk))) {
					...
					var = tris_variable_new(coltitle, chunk, "");
...
static int update_odbc(const char *database, const char *table, const char *keyfield, const char *lookup, va_list ap)
{
	...
	struct odbc_cache_tables *tableptr = tris_odbc_find_table(database, table);
	...
	if (!table) {
		tris_odbc_release_table(tableptr);
		return -1;
	}
	...
	if (tableptr && !(column = tris_odbc_find_column(tableptr, newparam))) {
		tris_log(LOG_WARNING, "Key field '%s' does not exist in table '%s@%s'.  Update will fail\n", newparam, table, database);
	}

	snprintf(sql, sizeof(sql), "UPDATE %s SET %s=?", table, newparam);
	while((newparam = va_arg(aq, const char *))) {
		newval = va_arg(aq, const char *);
		if ((tableptr && (column = tris_odbc_find_column(tableptr, newparam))) || count > 63) {
	...
	if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
		tris_log(LOG_WARNING, "SQL Row Count error!\n[%s]\n\n", sql);
		return -1;
	}
...
static SQLHSTMT update2_prepare(struct odbc_obj *obj, void *data)
{
	...
	SQLHSTMT stmt;
	va_list ap;
	struct odbc_cache_tables *tableptr = tris_odbc_find_table(ups->database, ups->table);
	struct odbc_cache_columns *column;
	...
	if (!tableptr) {
		tris_log(LOG_ERROR, "Could not retrieve metadata for table '%s@%s'.  Update will fail!\n", ups->table, ups->database);
		return NULL;
	}
	...
	tris_str_set(&sql, 0, "UPDATE %s SET ", ups->table);
	...
	while ((newparam = va_arg(ap, const char *))) {
		newval = va_arg(ap, const char *);
		if ((column = tris_odbc_find_column(tableptr, newparam))) {
			tris_str_append(&sql, 0, "%s%s=? ", first ? "" : ", ", newparam);
			SQLBindParameter(stmt, x++, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, strlen(newval), 0, (void *)newval, 0, NULL);
			first = 0;
		} else {
			tris_log(LOG_NOTICE, "Not updating column '%s' in '%s@%s' because that column does not exist!\n", newparam, ups->table, ups->database);
		}
	}
	va_end(ap);
	...
	va_copy(ap, ups->ap);
	tris_str_append(&sql, 0, "WHERE");
	first = 1;

	while ((newparam = va_arg(ap, const char *))) {
		newval = va_arg(ap, const char *);
		if (!(column = tris_odbc_find_column(tableptr, newparam))) {
			tris_log(LOG_ERROR, "One or more of the criteria columns '%s' on '%s@%s' for this update does not exist!\n", newparam, ups->table, ups->database);
			tris_odbc_release_table(tableptr);
			SQLFreeHandle(SQL_HANDLE_STMT, stmt);
			return NULL;
		}
		tris_str_append(&sql, 0, "%s %s=?", first ? "" : " AND", newparam);
		SQLBindParameter(stmt, x++, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, strlen(newval), 0, (void *)newval, 0, NULL);
		first = 0;
	}
	va_end(ap);
	...
	res = SQLPrepare(stmt, (unsigned char *)tris_str_buffer(sql), SQL_NTS);
	if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
		tris_log(LOG_WARNING, "SQL Prepare failed![%s]\n", tris_str_buffer(sql));
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);
		return NULL;
...
static int update2_odbc(const char *database, const char *table, va_list ap)
{
	...
	if (!(obj = tris_odbc_request_obj(database, 0))) {
		return -1;
	}

	if (!(stmt = tris_odbc_prepare_and_execute(obj, update2_prepare, &ups))) {
		tris_odbc_release_obj(obj);
		return -1;
	}
...
static int store_odbc(const char *database, const char *table, va_list ap)
{
	...
	tris_copy_string(vals, "?", sizeof(vals));
	while ((newparam = va_arg(aq, const char *))) {
...
static struct tris_config *config_odbc(const char *database, const char *table, const char *file, struct tris_config *cfg, struct tris_flags flags, const char *sugg_incl, const char *who_asked)
{
	struct tris_variable *new_v;
	struct tris_category *cur_cat;
	int res = 0;
	struct odbc_obj *obj;
	char sqlbuf[1024] = "";
	char *sql = sqlbuf;
	...
	tris_build_string(&sql, &sqlleft, "SELECT cat_metric, category, var_name, var_val FROM %s ", table);
	tris_build_string(&sql, &sqlleft, "WHERE filename='%s' AND commented=0 ", file);
	tris_build_string(&sql, &sqlleft, "ORDER BY cat_metric DESC, var_metric ASC, category, var_name ");
	q.sql = sqlbuf;

	stmt = tris_odbc_prepare_and_execute(obj, config_odbc_prepare, &q);
	...
	cur_cat = tris_config_get_current_category(cfg);

	while ((res = SQLFetch(stmt)) != SQL_NO_DATA) {
		if (!strcmp (q.var_name, "#include")) {
			if (!tris_config_internal_load(q.var_val, cfg, loader_flags, "", who_asked)) {
				SQLFreeHandle(SQL_HANDLE_STMT, stmt);
				tris_odbc_release_obj(obj);
				return NULL;
			}
		...
		if (strcmp(last, q.category) || last_cat_metric != q.cat_metric) {
			cur_cat = tris_category_new(q.category, "", 99999);
			if (!cur_cat) {
				tris_log(LOG_WARNING, "Out of memory!\n");
				break;
			}
		...
		new_v = tris_variable_new(q.var_name, q.var_val, "");
...
#define warn_length(col, size)	tris_log(LOG_WARNING, "Realtime table %s@%s: column '%s' is not long enough to contain realtime data (needs %d)\n", table, database, col->name, size)
#define warn_type(col, type)	tris_log(LOG_WARNING, "Realtime table %s@%s: column '%s' is of the incorrect type (%d) to contain the required realtime data\n", table, database, col->name, col->type)
...
static int require_odbc(const char *database, const char *table, va_list ap)
{
	...
	while ((elm = va_arg(ap, char *))) {
		...
		TRIS_RWLIST_TRAVERSE(&tableptr->columns, col, list) {
...
#undef WARN_TYPE_OR_LENGTH
				...
				case SQL_REAL:
				case SQL_DOUBLE:
					if (!tris_rq_is_int(type) && type != RQ_FLOAT) {
						warn_type(col, type);
					}
					break;
				default:
					tris_log(LOG_WARNING, "Realtime table %s@%s: column type (%d) unrecognized for column '%s'\n", table, database, col->type, elm);
				}
				break;
			}
		}
		if (!col) {
			tris_log(LOG_WARNING, "Realtime table %s@%s requires column '%s', but that column does not exist!\n", table, database, elm);
		}
	}
	va_end(ap);
	TRIS_RWLIST_UNLOCK(&tableptr->columns);
	return 0;
}
#undef warn_length
#undef warn_type

static struct tris_config_engine odbc_engine = {
	.name = "odbc",
	...
	.unload_func = tris_odbc_clear_cache,
};

static int unload_module (void)
{
	tris_config_engine_deregister(&odbc_engine);

	tris_verb(1, "res_config_odbc unloaded.\n");
	return 0;
}

static int load_module (void)
{
	tris_config_engine_register(&odbc_engine);
	tris_verb(1, "res_config_odbc loaded.\n");
	return 0;
}
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_GLOBAL_SYMBOLS, "Realtime ODBC configuration",
		.load = load_module,
