 ...
 *
 * res_config_sqlite is a module for the Trismedia Open Source PBX to
 * support SQLite 2 databases. It can be used to fetch configuration
 * from a database (static configuration files and/or using the Trismedia
 * RealTime Architecture - ARA).  It can also be used to log CDR entries. 
 * Note that Trismedia already comes with a module named cdr_sqlite.
 ...
 * simple module for Trismedia, the other is to have the same database open for
 ...
 * Trismedia documentation. The file tables.sql can be used to create the
 * needed tables.
 *
...
/*!
 * \brief Trismedia callback function for CDR support.
 * \param cdr the CDR entry Trismedia sends us.
 *
 * Trismedia will call this function each time a CDR entry must be logged if
 * CDR support is enabled.
 *
 * \retval 0 on success
 * \retval 1 if an error occurred
 */
static int cdr_handler(struct tris_cdr *cdr);
 ...
 * parse a row and store it in a struct tris_config object. It relies on
...
/*!
 * \brief Trismedia callback function for static configuration.
 *
 * Trismedia will call this function when it loads its static configuration,
 ...
 * \param cfg the struct tris_config object to use when storing variables
 ...
 * \see add_cfg_entry()
 */
static struct tris_config * config_handler(const char *database, const char *table, const char *file,
	struct tris_config *cfg, struct tris_flags flags, const char *suggested_incl, const char *who_asked);
 ...
 *
 * This function is passed to the SQLite engine as a callback function to
 * parse a row and store it in a linked list of struct tris_variable objects.
 *
...
/*!
 * \brief Trismedia callback function for RealTime configuration.
 *
 * Trismedia will call this function each time it requires a variable
 ...
 *
 * \retval a linked list of struct tris_variable objects
 * \retval NULL if an error occurred
 * \see add_rt_cfg_entry()
 */
static struct tris_variable * realtime_handler(const char *database,
 ...
 * \param table the table to use
 * \param ap list of parameters and values to match
 * \retval a struct tris_config object storing categories and variables.
 * \retval NULL if an error occurred.
 *
 * \see add_rt_multi_cfg_entry()
 */
static struct tris_config * realtime_multi_handler(const char *database,
	const char *table, va_list ap);

/*!
 * \brief Trismedia callback function for RealTime configuration (variable
 * update).
 *
 * Trismedia will call this function each time a variable has been modified
 ...
 * create/store).
 *
 * Trismedia will call this function each time a variable has been created
...
/*!
 * \brief Trismedia callback function for RealTime configuration (destroys
 * variable).
 *
 * Trismedia will call this function each time a variable has been destroyed
...
/*!
 * \brief Trismedia callback function for the CLI status command.
 *
 * \param e CLI command
 * \param cmd 
 ...
 */
static char *handle_cli_show_sqlite_status(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *handle_cli_sqlite_show_tables(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
...
/*!
 * The structure specifying all callback functions used by Trismedia for static
 * and RealTime configuration.
 */
static struct tris_config_engine sqlite_engine =
{
 ...
 */
TRIS_MUTEX_DEFINE_STATIC(mutex);

/*!
 * Structure containing details and callback functions for the CLI status
 * command.
 */
static struct tris_cli_entry cli_status[] = {
	TRIS_CLI_DEFINE(handle_cli_show_sqlite_status, "Show status information about the SQLite 2 driver"),
	TRIS_CLI_DEFINE(handle_cli_sqlite_show_tables, "Cached table information about the SQLite 2 driver"),
};
...
struct sqlite_cache_columns {
	...
	TRIS_RWLIST_ENTRY(sqlite_cache_columns) list;
};

struct sqlite_cache_tables {
	char *name;
	TRIS_RWLIST_HEAD(_columns, sqlite_cache_columns) columns;
	TRIS_RWLIST_ENTRY(sqlite_cache_tables) list;
};

static TRIS_RWLIST_HEAD_STATIC(sqlite_tables, sqlite_cache_tables);

/*
 * Taken from Trismedia 1.2 cdr_sqlite.so.
 */
...
static void free_table(struct sqlite_cache_tables *tblptr)
{
	...
	TRIS_RWLIST_WRLOCK(&(tblptr->columns));
	while ((col = TRIS_RWLIST_REMOVE_HEAD(&(tblptr->columns), list))) {
		tris_free(col);
	}
	TRIS_RWLIST_UNLOCK(&(tblptr->columns));
	TRIS_RWLIST_HEAD_DESTROY(&(tblptr->columns));
	tris_free(tblptr);
}
...
static int find_table_cb(void *vtblptr, int argc, char **argv, char **columnNames)
{
	...
	char *sql = tris_strdupa(argv[0]), *start, *end, *type, *remainder;
	int i;
	TRIS_DECLARE_APP_ARGS(fie,
		TRIS_APP_ARG(ld)[100]; /* This means we support up to 100 columns per table */
	);
	struct sqlite_cache_columns *col;
	...
	TRIS_STANDARD_APP_ARGS(fie, start);
	for (i = 0; i < fie.argc; i++) {
		fie.ld[i] = tris_skip_blanks(fie.ld[i]);
		tris_debug(5, "Found field: %s\n", fie.ld[i]);
		if (strncasecmp(fie.ld[i], "PRIMARY KEY", 11) == 0 && (start = strchr(fie.ld[i], '(')) && (end = strchr(fie.ld[i], ')'))) {
			*end = '\0';
			TRIS_RWLIST_TRAVERSE(&(tblptr->columns), col, list) {
		...
		*type++ = '\0';
		type = tris_skip_blanks(type);
		for (remainder = type; *remainder > 32; remainder++);
		*remainder = '\0';
		if (!(col = tris_calloc(1, sizeof(*col) + strlen(fie.ld[i]) + strlen(type) + 2))) {
			return -1;
		}
		...
		TRIS_LIST_INSERT_TAIL(&(tblptr->columns), col, list);
	}
	return 0;
}
...
static struct sqlite_cache_tables *find_table(const char *tablename)
{
	...
	TRIS_RWLIST_RDLOCK(&sqlite_tables);

	for (i = 0; i < 2; i++) {
		TRIS_RWLIST_TRAVERSE(&sqlite_tables, tblptr, list) {
			if (strcmp(tblptr->name, tablename) == 0) {
				break;
			}
		}
		if (tblptr) {
			TRIS_RWLIST_RDLOCK(&(tblptr->columns));
			TRIS_RWLIST_UNLOCK(&sqlite_tables);
			return tblptr;
		}

		if (i == 0) {
			TRIS_RWLIST_UNLOCK(&sqlite_tables);
			TRIS_RWLIST_WRLOCK(&sqlite_tables);
		}
	}
	...
	if (!(tblptr = tris_calloc(1, sizeof(*tblptr) + strlen(tablename) + 1))) {
		TRIS_RWLIST_UNLOCK(&sqlite_tables);
		tris_log(LOG_ERROR, "Memory error.  Cannot cache table '%s'\n", tablename);
		return NULL;
	}
	tblptr->name = (char *)tblptr + sizeof(*tblptr);
	strcpy(tblptr->name, tablename); /* SAFE */
	TRIS_RWLIST_HEAD_INIT(&(tblptr->columns));

	tris_debug(1, "About to query table structure: %s\n", sql);

	tris_mutex_lock(&mutex);
	if ((err = sqlite_exec(db, sql, find_table_cb, tblptr, &errstr))) {
		tris_mutex_unlock(&mutex);
		tris_log(LOG_WARNING, "SQLite error %d: %s\n", err, errstr);
		tris_free(errstr);
		free_table(tblptr);
		return NULL;
	}
	...
	if (TRIS_LIST_EMPTY(&(tblptr->columns))) {
		free_table(tblptr);
		return NULL;
	}

	TRIS_RWLIST_INSERT_TAIL(&sqlite_tables, tblptr, list);
	TRIS_RWLIST_RDLOCK(&(tblptr->columns));
	TRIS_RWLIST_UNLOCK(&sqlite_tables);
	return tblptr;
}

#define release_table(a)	TRIS_RWLIST_UNLOCK(&((a)->columns))
...
static int set_var(char **var, const char *name, const char *value)
{
	...
	*var = tris_strdup(value);

	if (!*var) {
		tris_log(LOG_WARNING, "Unable to allocate variable %s\n", name);
		return 1;
	}
...
{
	if (!dbfile) {
		tris_log(LOG_ERROR, "Required parameter undefined: dbfile\n");
		return 1;
	}
...
static int load_config(void)
{
	...
	config = tris_config_load(RES_CONFIG_SQLITE_CONF_FILE, config_flags);

	if (config == CONFIG_STATUS_FILEMISSING || config == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_ERROR, "Unable to load " RES_CONFIG_SQLITE_CONF_FILE "\n");
		return 1;
	}

	for (var = tris_variable_browse(config, "general"); var; var = var->next) {
		if (!strcasecmp(var->name, "dbfile"))
			...
			tris_log(LOG_WARNING, "Unknown parameter : %s\n", var->name);
	}
...
static void unload_config(void)
{
	...
	tris_free(dbfile);
	dbfile = NULL;
	tris_free(config_table);
	config_table = NULL;
	tris_free(cdr_table);
	cdr_table = NULL;
	TRIS_RWLIST_WRLOCK(&sqlite_tables);
	while ((tbl = TRIS_RWLIST_REMOVE_HEAD(&sqlite_tables, list))) {
		free_table(tbl);
	}
	TRIS_RWLIST_UNLOCK(&sqlite_tables);
}
...
static int cdr_handler(struct tris_cdr *cdr)
{
	...
	struct sqlite_cache_columns *col;
	struct tris_str *sql1 = tris_str_create(160), *sql2 = tris_str_create(16);
	int first = 1;

	if (!tbl) {
		tris_log(LOG_WARNING, "No such table: %s\n", cdr_table);
		return -1;
	}

	tris_str_set(&sql1, 0, "INSERT INTO %s (", cdr_table);
	tris_str_set(&sql2, 0, ") VALUES (");

	TRIS_RWLIST_TRAVERSE(&(tbl->columns), col, list) {
		if (col->isint) {
			tris_cdr_getvar(cdr, col->name, &tmp, workspace, sizeof(workspace), 0, 1);
			if (!tmp) {
				continue;
			}
			if (sscanf(tmp, "%30d", &scannum) == 1) {
				tris_str_append(&sql1, 0, "%s%s", first ? "" : ",", col->name);
				tris_str_append(&sql2, 0, "%s%d", first ? "" : ",", scannum);
			}
		} else {
			tris_cdr_getvar(cdr, col->name, &tmp, workspace, sizeof(workspace), 0, 0);
			if (!tmp) {
				continue;
			}
			tris_str_append(&sql1, 0, "%s%s", first ? "" : ",", col->name);
			tmp = sqlite_mprintf("%Q", tmp);
			tris_str_append(&sql2, 0, "%s%s", first ? "" : ",", tmp);
			sqlite_freemem(tmp);
		}
		first = 0;
	}
	...
	tris_debug(1, "SQL query: %s\n", tris_str_buffer(sql1));

	tris_mutex_lock(&mutex);

	RES_CONFIG_SQLITE_BEGIN
		error = sqlite_exec(db, tris_str_buffer(sql1), NULL, NULL, &errormsg);
	RES_CONFIG_SQLITE_END(error)
...
static int add_cfg_entry(void *arg, int argc, char **argv, char **columnNames)
{
	...
	if (argc != RES_CONFIG_SQLITE_CONFIG_COLUMNS) {
		tris_log(LOG_WARNING, "Corrupt table\n");
		return 1;
	}
	...
	if (!strcmp(argv[RES_CONFIG_SQLITE_CONFIG_VAR_NAME], "#include")) {
		...
		cfg = tris_config_internal_load(val, args->cfg, args->flags, "", args->who_asked);

		if (!cfg) {
			tris_log(LOG_WARNING, "Unable to include %s\n", val);
			return 1;
		} else {
			args->cfg = cfg;
			return 0;
	...
	if (!args->cat_name || strcmp(args->cat_name, argv[RES_CONFIG_SQLITE_CONFIG_CATEGORY])) {
		args->cat = tris_category_new(argv[RES_CONFIG_SQLITE_CONFIG_CATEGORY], "", 99999);

		if (!args->cat) {
			tris_log(LOG_WARNING, "Unable to allocate category\n");
			return 1;
		}

		tris_free(args->cat_name);
		args->cat_name = tris_strdup(argv[RES_CONFIG_SQLITE_CONFIG_CATEGORY]);

		if (!args->cat_name) {
			tris_category_destroy(args->cat);
			return 1;
		}

		tris_category_append(args->cfg, args->cat);
	}

	var = tris_variable_new(argv[RES_CONFIG_SQLITE_CONFIG_VAR_NAME], argv[RES_CONFIG_SQLITE_CONFIG_VAR_VAL], "");

	if (!var) {
		tris_log(LOG_WARNING, "Unable to allocate variable");
		return 1;
	}

	tris_variable_append(args->cat, var);
	...
	struct tris_config *cfg, struct tris_flags flags, const char *suggested_incl, const char *who_asked)
{
	...
	if (!config_table) {
		if (!table) {
			tris_log(LOG_ERROR, "Table name unspecified\n");
			return NULL;
		}
	} else
		table = config_table;
	...
	if (!query) {
		tris_log(LOG_WARNING, "Unable to allocate SQL query\n");
		return NULL;
	}
...
static size_t get_params(va_list ap, const char ***params_ptr, const char ***vals_ptr, int warn)
{
	...
	while ((param = va_arg(ap, const char *)) && (val = va_arg(ap, const char *))) {
		if (!(tmp = tris_realloc(params, (params_count + 1) * sizeof(char *)))) {
			tris_free(params);
			tris_free(vals);
			return 0;
		}
		...
		if (!(tmp = tris_realloc(vals, (params_count + 1) * sizeof(char *)))) {
			tris_free(params);
			tris_free(vals);
			return 0;
		}
	...
	if (params_count > 0) {
		...
		*vals_ptr = vals;
		...
		tris_log(LOG_WARNING, "1 parameter and 1 value at least required\n");
	}
...
static int add_rt_cfg_entry(void *arg, int argc, char **argv, char **columnNames)
{
	...
	for (i = 0; i < argc; i++) {
		...
		if (!(var = tris_variable_new(columnNames[i], argv[i], "")))
			return 1;
...
static struct tris_variable * realtime_handler(const char *database, const char *table, va_list ap)
{
	...
	if (!table) {
		tris_log(LOG_WARNING, "Table name unspecified\n");
		return NULL;
	}
...
/* \endcond */
	...
	if (params_count > 1) {
		...
		for (i = 1; i < params_count; i++) {
			...
			if (!tmp_str) {
				tris_log(LOG_WARNING, "Unable to reallocate SQL query\n");
				tris_free(params);
				tris_free(vals);
				return NULL;
			}
	...
	if (error) {
		...
		sqlite_freemem(errormsg);
		tris_variables_destroy(args.var);
		return NULL;
	}
	sqlite_freemem(errormsg);
...
static int add_rt_multi_cfg_entry(void *arg, int argc, char **argv, char **columnNames)
{
	...
	if (!cat_name) {
		tris_log(LOG_ERROR, "Bogus SQL results, cat_name is NULL !\n");
		return 1;
	}

	if (!(cat = tris_category_new(cat_name, "", 99999))) {
		tris_log(LOG_WARNING, "Unable to allocate category\n");
		return 1;
	}

	tris_category_append(args->cfg, cat);
	...
	const char *table, va_list ap)
{
	...
	if (!(cfg = tris_config_new())) {
		tris_log(LOG_WARNING, "Unable to allocate configuration structure\n");
		return NULL;
	}
	...
	if (!(initfield = tris_strdup(params[0]))) {
		tris_config_destroy(cfg);
		tris_free(params);
		tris_free(vals);
		return NULL;
	...
	/*
	 * Trismedia sends us an already escaped string when searching for
	...
	va_list ap)
{
	...
	tris_str_set(&sql, 0, "UPDATE %s SET", table);
	tris_str_set(&where, 0, " WHERE");

	while ((param = va_arg(ap, const char *))) {
		value = va_arg(ap, const char *);
		tris_str_append(&where, 0, "%s %s = %s",
			first ? "" : " AND",
	...
	if (first) {
		tris_log(LOG_ERROR, "No criteria specified on update to '%s@%s'!\n", table, database);
		return -1;
	}
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite_exec(db, tris_str_buffer(sql), NULL, NULL, &errormsg);
	RES_CONFIG_SQLITE_END(error)
	...
	while ((elm = va_arg(ap, char *))) {
		...
		TRIS_RWLIST_TRAVERSE(&tbl->columns, col, list) {
			if (strcmp(col->name, elm) == 0) {
				 ...
				 * being a string).
				 */
				if (col->isint && !tris_rq_is_int(type)) {
					tris_log(LOG_WARNING, "Realtime table %s: column '%s' is an integer field, but Trismedia requires that it not be!\n", tablename, col->name);
					res = -1;
				}
				break;
			}
		...
		if (!col) {
			tris_log(LOG_WARNING, "Realtime table %s requires column '%s', but that column does not exist!\n", tablename, elm);
		}
	}
	TRIS_RWLIST_UNLOCK(&(tbl->columns));
	return res;
}
...
static int realtime_unload_handler(const char *unused, const char *tablename)
{
	struct sqlite_cache_tables *tbl;
	TRIS_RWLIST_WRLOCK(&sqlite_tables);
	TRIS_RWLIST_TRAVERSE_SAFE_BEGIN(&sqlite_tables, tbl, list) {
...
static char *handle_cli_show_sqlite_status(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_cli(a->fd, "SQLite database path: %s\n", dbfile);
	tris_cli(a->fd, "config_table: ");

	if (!config_table)
		tris_cli(a->fd, "unspecified, must be present in extconfig.conf\n");
	else
		tris_cli(a->fd, "%s\n", config_table);

	tris_cli(a->fd, "cdr_table: ");

	if (!cdr_table)
		tris_cli(a->fd, "unspecified, CDR support disabled\n");
	else
		tris_cli(a->fd, "%s\n", cdr_table);
...
static char *handle_cli_sqlite_show_tables(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	TRIS_RWLIST_RDLOCK(&sqlite_tables);
	TRIS_RWLIST_TRAVERSE(&sqlite_tables, tbl, list) {
		found++;
		tris_cli(a->fd, "Table %s:\n", tbl->name);
		TRIS_RWLIST_TRAVERSE(&(tbl->columns), col, list) {
			fprintf(stderr, "%s\n", col->name);
			tris_cli(a->fd, "  %20.20s  %-30.30s\n", col->name, col->type);
		}
	}
	TRIS_RWLIST_UNLOCK(&sqlite_tables);

	if (!found) {
		tris_cli(a->fd, "No tables currently in cache\n");
	}
...
static int unload_module(void)
{
	if (cli_status_registered)
		tris_cli_unregister_multiple(cli_status, ARRAY_LEN(cli_status));

	if (cdr_registered)
		tris_cdr_unregister(RES_CONFIG_SQLITE_NAME);

	tris_config_engine_deregister(&sqlite_engine);
...
static int load_module(void)
{
	...
	sqlite_freemem(errormsg);
	errormsg = NULL;
	tris_config_engine_register(&sqlite_engine);
...
/* \endcond */
		...
		if (!query) {
			tris_log(LOG_ERROR, "Unable to allocate SQL query\n");
			unload_module();
			return 1;
		}
		...
		error = tris_cdr_register(RES_CONFIG_SQLITE_NAME, RES_CONFIG_SQLITE_DESCRIPTION, cdr_handler);
	...
	error = tris_cli_register_multiple(cli_status, ARRAY_LEN(cli_status));
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_GLOBAL_SYMBOLS, "Realtime SQLite configuration",
		.load = load_module,
