...
/*! \file
 * \brief A resource for interfacing Trismedia directly as a client
...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 209234 $")
...
/*** DOCUMENTATION
	<application name="JabberSend" language="en_US">
		...
		<syntax>
			<parameter name="Jabber" required="true">
				<para>Client or transport Trismedia uses to connect to Jabber.</para>
			</parameter>
	...
	<application name="JabberStatus" language="en_US">
		...
		<syntax>
			<parameter name="Jabber" required="true">
				<para>Client or transport Trismedia users to connect to Jabber.</para>
			</parameter>
			<parameter name="JID" required="true">
				<para>XMPP/Jabber JID (Name) of recipient.</para>
			</parameter>
...
static int aji_send_exec(struct tris_channel *chan, void *data);
static int aji_status_exec(struct tris_channel *chan, void *data);
...
static char *aji_do_set_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *aji_do_reload(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *aji_show_clients(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *aji_show_buddies(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static char *aji_test(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a);
static int aji_create_client(char *label, struct tris_variable *var, int debug);
...
static struct tris_cli_entry aji_cli[] = {
	TRIS_CLI_DEFINE(aji_do_set_debug, "Enable/Disable Jabber debug"),
	TRIS_CLI_DEFINE(aji_do_reload, "Reload Jabber configuration"),
	TRIS_CLI_DEFINE(aji_show_clients, "Show state of clients and components"),
	TRIS_CLI_DEFINE(aji_show_buddies, "Show buddy lists of our clients"),
	TRIS_CLI_DEFINE(aji_test, "Shows roster, but is generally used for mog's debugging."),
};
...
static struct tris_flags globalflags = { AJI_AUTOREGISTER };
...
static void aji_client_destroy(struct aji_client *obj)
{
	...
	TRIS_LIST_LOCK(&obj->messages);
	while ((tmp = TRIS_LIST_REMOVE_HEAD(&obj->messages, list))) {
		if (tmp->from)
			tris_free(tmp->from);
		if (tmp->message)
			tris_free(tmp->message);
	}
	TRIS_LIST_HEAD_DESTROY(&obj->messages);
	tris_free(obj);
}
...
static struct aji_version *aji_find_version(char *node, char *version, ikspak *pak)
{
	...
	while(list) {
		if(!strcasecmp(list->node, node)) {
			...
			if(!res) {
				...
				res->jingle = 0;
				res->parent = list;
				tris_copy_string(res->version, version, sizeof(res->version));
	...
	if(!list) {
		...
		tris_copy_string(list->node, node, sizeof(list->node));
		tris_copy_string(res->version, version, sizeof(res->version));
		res->jingle = 0;
		res->parent = list;
		res->next = NULL;
...
static int aji_status_exec(struct tris_channel *chan, void *data)
{
	...
	TRIS_DECLARE_APP_ARGS(jid,
		TRIS_APP_ARG(screenname);
		TRIS_APP_ARG(resource);
	);

	if (deprecation_warning++ % 10 == 0)
		tris_log(LOG_WARNING, "JabberStatus is deprecated.  Please use the JABBER_STATUS dialplan function in the future.\n");

	if (!data) {
		tris_log(LOG_ERROR, "Usage: JabberStatus(<sender>,<jid>[/<resource>],<varname>\n");
		return 0;
	}
	s = tris_strdupa(data);
	...
	if (args.argc != 3) {
		tris_log(LOG_ERROR, "JabberStatus() requires 3 arguments.\n");
		return -1;
	}

	TRIS_NONSTANDARD_APP_ARGS(jid, args.jid, '/');

	if (!(client = tris_aji_get_client(args.sender))) {
		tris_log(LOG_WARNING, "Could not find sender connection: '%s'\n", args.sender);
		return -1;
	}
	buddy = ASTOBJ_CONTAINER_FIND(&client->buddies, jid.screenname);
	if (!buddy) {
		tris_log(LOG_WARNING, "Could not find buddy in list: '%s'\n", jid.screenname);
		return -1;
	}
	...
	if (!r && buddy->resources) 
		r = buddy->resources;
	if (!r)
		tris_log(LOG_NOTICE, "Resource '%s' of buddy '%s' was not found\n", jid.resource, jid.screenname);
	else
		stat = r->status;
...
static int acf_jabberstatus_read(struct tris_channel *chan, const char *name, char *data, char *buf, size_t buflen)
{
	struct aji_client *client = NULL;
	struct aji_buddy *buddy = NULL;
	struct aji_resource *r = NULL;
	...
	if (!data) {
		tris_log(LOG_ERROR, "Usage: JABBER_STATUS(<sender>,<jid>[/<resource>])\n");
		return 0;
	}
	TRIS_STANDARD_APP_ARGS(args, data);

	if (args.argc != 2) {
		tris_log(LOG_ERROR, "JABBER_STATUS requires 2 arguments: sender and jid.\n");
		return -1;
	}
...
static struct tris_custom_function jabberstatus_function = {
	.name = "JABBER_STATUS",
...
static int aji_send_exec(struct tris_channel *chan, void *data)
{
	...
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(sender);
		TRIS_APP_ARG(recipient);
		TRIS_APP_ARG(message);
	);

	if (!data) {
		tris_log(LOG_ERROR, "Usage:  JabberSend(<sender>,<recipient>,<message>)\n");
		return 0;
	}
	s = tris_strdupa(data);
	...
	TRIS_STANDARD_APP_ARGS(args, s);
	if (args.argc < 3) {
		tris_log(LOG_ERROR, "JabberSend requires 3 arguments: '%s'\n", (char *) data);
		return -1;
	}
	...
	if (strchr(args.recipient, '@') && !tris_strlen_zero(args.message))
		tris_aji_send_chat(client, args.recipient, args.message);
	return 0;
}
...
static int aji_tls_handshake(struct aji_client *client)
{
	...
	tris_debug(1, "Starting TLS handshake\n"); 
	...
	tris_debug(1, "TLS started with server\n"); 
...
static int aji_io_recv(struct aji_client *client, char *buffer, size_t buf_len, int timeout)
{
	...
	/* NULL value for tvptr makes tris_select wait indefinitely */
	tvptr = (timeout != -1) ? &tv : NULL;

	/* tris_select emulates linux behaviour in terms of timeout handling */
	res = tris_select(sock + 1, &fds, NULL, NULL, tvptr);
	if (res > 0) {
#ifdef HAVE_OPENSSL
		if (aji_is_secure(client)) {
...
static int aji_recv (struct aji_client *client, int timeout)
{
	...
	while (1) {
		...
		switch (ret) {
		case IKS_NOMEM:
			tris_log(LOG_WARNING, "Parsing failure: Out of memory.\n");
			break;
		case IKS_BADXML:
			tris_log(LOG_WARNING, "Parsing failure: Invalid XML.\n");
			break;
		case IKS_HOOK:
			tris_log(LOG_WARNING, "Parsing failure: Hook returned an error.\n");
			break;
		}
		if (ret != IKS_OK) {
			return ret;
		}
		tris_debug(3, "XML parsing successful\n");	
	}
	return IKS_OK;
}
...
static int aji_start_sasl(struct aji_client *client, enum ikssasltype type, char *username, char *pass)
{
	...
	if (!(type & IKS_STREAM_SASL_PLAIN)) {
		tris_log(LOG_ERROR, "Server does not support SASL PLAIN authentication\n");
	...
	tris_base64encode(base64, (const unsigned char *) s, len - 1, (len + 2) * 4 / 3);
	iks_insert_cdata(x, base64, 0);
	tris_aji_send(client, x);
	iks_delete(x);
...
static int aji_act_hook(void *data, int type, iks *node)
{
	...
	if(!node) {
		tris_log(LOG_ERROR, "aji_act_hook was called with out a packet\n"); /* most likely cause type is IKS_NODE_ERROR lost connection */
		ASTOBJ_UNREF(client, aji_client_destroy);
		return IKS_HOOK;
	}
	...
	if (!client->component) { /*client */
...
#ifndef HAVE_OPENSSL
				tris_log(LOG_ERROR, "OpenSSL not installed. You need to install OpenSSL on this system, or disable the TLS option in your configuration file\n");
				ASTOBJ_UNREF(client, aji_client_destroy);
				return IKS_HOOK;
#else
				if (aji_start_tls(client) == IKS_NET_TLSFAIL) {
					tris_log(LOG_ERROR, "Could not start TLS\n");
					ASTOBJ_UNREF(client, aji_client_destroy);
					return IKS_HOOK;		
				}
#endif
		...
		case IKS_NODE_NORMAL:
			...
			if (!strcmp("stream:features", iks_name(node))) {
				...
				if (client->usesasl) {
					...
					if (client->authorized) {
						...
						int ret;
						if (!client->jid->user) {
							tris_log(LOG_ERROR, "Malformed Jabber ID : %s (domain missing?)\n", client->jid->full);
							break;
						}
				...
				tris_log(LOG_ERROR, "JABBER: encryption failure. possible bad password.\n");
			...
			break;
		case IKS_NODE_ERROR: 
				tris_log(LOG_ERROR, "JABBER: Node Error\n");
				ASTOBJ_UNREF(client, aji_client_destroy);
				return IKS_HOOK;
				break;
		case IKS_NODE_STOP: 
				tris_log(LOG_WARNING, "JABBER: Disconnected\n");
				ASTOBJ_UNREF(client, aji_client_destroy);
				return IKS_HOOK;
				break;
		}
		...
		case IKS_NODE_START:
			if (client->state == AJI_DISCONNECTED) {
				...
				if(aji_recv(client, 1) == 2) /*XXX proper result for iksemel library on iks_recv of <handshake/> XXX*/
					...
					tris_log(LOG_WARNING, "Jabber didn't seem to handshake, failed to authenticate.\n");
				break;
			}
			break;
	...
	switch (pak->type) {
	case IKS_PAK_NONE:
		tris_debug(1, "JABBER: I don't know what to do with paktype NONE.\n");
		break;
	case IKS_PAK_MESSAGE:
		aji_handle_message(client, pak);
		tris_debug(1, "JABBER: Handling paktype MESSAGE.\n");
		break;
	case IKS_PAK_PRESENCE:
		aji_handle_presence(client, pak);
		tris_debug(1, "JABBER: Handling paktype PRESENCE\n");
		break;
	case IKS_PAK_S10N:
		aji_handle_subscribe(client, pak);
		tris_debug(1, "JABBER: Handling paktype S10N\n");
		break;
	case IKS_PAK_IQ:
		tris_debug(1, "JABBER: Handling paktype IQ\n");
		aji_handle_iq(client, node);
		break;
	default:
		tris_debug(1, "JABBER: I don't know anything about paktype '%d'\n", pak->type);
		break;
	}
...
static int aji_register_query_handler(void *data, ikspak *pak)
{
	...
	if (!buddy) {
		...
		tris_log(LOG_ERROR, "Someone.... %s tried to register but they aren't allowed\n", pak->from->partial);
		iq = iks_new("iq");
		query = iks_new("query");
		error = iks_new("error");
		...
		char *explain = "Welcome to Trismedia - the Open Source PBX.\n";
		iq = iks_new("iq");
		query = iks_new("query");
...
static int aji_ditems_handler(void *data, ikspak *pak)
{
	...
	if (!(node = iks_find_attrib(pak->query, "node"))) {
		...
		if (iq && query && item) {
			...
			iks_insert_attrib(item, "name", "Million Dollar Trismedia Commands");
...
static int aji_client_info_handler(void *data, ikspak *pak)
{
	...
	if (pak->subtype == IKS_TYPE_RESULT) {
		if (!resource) {
			tris_log(LOG_NOTICE, "JABBER: Received client info from %s when not requested.\n", pak->from->full);
			ASTOBJ_UNREF(client, aji_client_destroy);
			return IKS_FILTER_EAT;
		}
		...
		if (iq && ident && disco && google) {
			...
			iks_insert_attrib(ident, "name", "trismedia");
		...
		tris_log(LOG_NOTICE, "User %s does not support discovery.\n", pak->from->full);
	}
	ASTOBJ_UNREF(client, aji_client_destroy);
	return IKS_FILTER_EAT;
}
...
static int aji_dinfo_handler(void *data, ikspak *pak)
{
	...
	if (pak->subtype == IKS_TYPE_ERROR) {
		tris_log(LOG_WARNING, "Received error from a client, turn on jabber debug!\n");
		return IKS_FILTER_EAT;
	}
	if (pak->subtype == IKS_TYPE_RESULT) {
		if (!resource) {
		...
		if (iq && query && identity && disco && reg && commands && gateway && version && vcard && search && client) {
			...
			iks_insert_attrib(identity, "name", "Trismedia The Open Source PBX");
...
static void aji_handle_message(struct aji_client *client, ikspak *pak)
{
	...
	if (!(insert = tris_calloc(1, sizeof(*insert))))
		return;
	time(&insert->arrived);
	if (iks_find_cdata(pak->x, "body"))
		insert->message = tris_strdup(iks_find_cdata(pak->x, "body"));
	if (pak->id)
		tris_copy_string(insert->id, pak->id, sizeof(insert->message));
	if (pak->from)
		insert->from = tris_strdup(pak->from->full);
	TRIS_LIST_LOCK(&client->messages);
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&client->messages, tmp, list) {
		if (flag) {
	...
	TRIS_LIST_INSERT_HEAD(&client->messages, insert, list);
	TRIS_LIST_UNLOCK(&client->messages);
}
/*!
...
static void aji_handle_presence(struct aji_client *client, ikspak *pak)
{
	...
	if (!buddy && pak->from->partial) {
		...
		if (!strcmp((const char *)pak->from->partial, (const char *)client->jid->partial))
			...
			tris_log(LOG_NOTICE, "Got presence packet from %s, someone not in our roster!!!!\n", pak->from->partial);
		return;
	}
	...
	if (!found && status != 6 && pak->from->resource) {
		found = tris_calloc(1, sizeof(*found));

		if (!found) {
			tris_log(LOG_ERROR, "Out of memory!\n");
			return;
		}
		tris_copy_string(found->resource, pak->from->resource, sizeof(found->resource));
		found->status = status;
	...
	if(status !=6 && found && !found->cap) {
		...
		if(found->cap->jingle && option_debug > 4) {
			tris_debug(1,"Special case for google till they support discover.\n");
		}
		else {
			iks *iq, *query;
			iq = iks_new("iq");
	...
	case IKS_TYPE_AVAILABLE:
		tris_debug(3, "JABBER: I am available ^_* %i\n", pak->subtype);
		break;
	case IKS_TYPE_UNAVAILABLE:
		tris_debug(3, "JABBER: I am unavailable ^_* %i\n", pak->subtype);
		break;
	default:
		tris_debug(3, "JABBER: Ohh sexy and the wrong type: %i\n", pak->subtype);
	}
	switch (pak->show) {
	case IKS_SHOW_UNAVAILABLE:
		tris_debug(3, "JABBER: type: %i subtype %i\n", pak->subtype, pak->show);
		break;
	case IKS_SHOW_AVAILABLE:
		tris_debug(3, "JABBER: type is available\n");
		break;
	case IKS_SHOW_CHAT:
		tris_debug(3, "JABBER: type: %i subtype %i\n", pak->subtype, pak->show);
		break;
	case IKS_SHOW_AWAY:
		tris_debug(3, "JABBER: type is away\n");
		break;
	case IKS_SHOW_XA:
		tris_debug(3, "JABBER: type: %i subtype %i\n", pak->subtype, pak->show);
		break;
	case IKS_SHOW_DND:
		...
		break;
	default:
		tris_debug(3, "JABBER: Kinky! how did that happen %i\n", pak->show);
	}
}
...
static void aji_handle_subscribe(struct aji_client *client, ikspak *pak)
{
	...
	case IKS_TYPE_SUBSCRIBE:
		...
		if (presence && status) {
			...
			iks_insert_cdata(status, "Trismedia has approved subscription", 0);
			iks_insert_node(presence, status);
			tris_aji_send(client, presence);
		} else
			tris_log(LOG_ERROR, "Unable to allocate nodes\n");
	...
	default:
		if (option_verbose > 4) {
			tris_verbose(VERBOSE_PREFIX_3 "JABBER: This is a subcription of type %i\n", pak->subtype);
		}
	}
}
...
int tris_aji_send_chat(struct aji_client *client, const char *address, const char *message)
{
	...
	if (client->state == AJI_CONNECTED) {
		...
		if (message_packet) {
			...
			res = tris_aji_send(client, message_packet);
		} else {
			tris_log(LOG_ERROR, "Out of memory.\n");
		}
		...
		tris_log(LOG_WARNING, "JABBER: Not connected can't send\n");
	return 1;
}
...
int tris_aji_join_chat(struct aji_client *client, char *room)
{
	...
	if (presence && priority && client) {
		...
		res = tris_aji_send(client, presence);
...
int tris_aji_invite_chat(struct aji_client *client, char *user, char *room, char *message)
{
	...
	if (client && invite && body && namespace) {
		...
		res = tris_aji_send(client, invite);
	} else 
		tris_log(LOG_ERROR, "Out of memory.\n");
...
static void *aji_recv_loop(void *data)
{
	...
	while(res != IKS_OK) {
		tris_debug(3, "JABBER: Connecting.\n");
	...
	do {
		if (res == IKS_NET_RWERR || client->timeout == 0) {
			while(res != IKS_OK) {
				tris_debug(3, "JABBER: reconnecting.\n");
				res = aji_reconnect(client);
				sleep(4);
			}
		}
		...
		if (client->state == AJI_DISCONNECTING) {
			tris_debug(2, "Ending our Jabber client's thread due to a disconnect\n");
			pthread_exit(NULL);
		}
		...
		if (res == IKS_HOOK) 
			tris_log(LOG_WARNING, "JABBER: Got hook event.\n");
		else if (res == IKS_NET_TLSFAIL)
			tris_log(LOG_ERROR, "JABBER:  Failure in TLS.\n");
		else if (client->timeout == 0 && client->state == AJI_CONNECTED) {
			res = client->keepalive ? aji_send_raw(client, " ") : IKS_OK;
			if(res == IKS_OK)
				client->timeout = 50;
			else
				tris_log(LOG_WARNING, "JABBER:  Network Timeout\n");
		} else if (res == IKS_NET_RWERR)
			tris_log(LOG_WARNING, "JABBER: socket read error\n");
	} while (client);
	ASTOBJ_UNREF(client, aji_client_destroy);
	return 0;
}
...
static int aji_register_transport2(void *data, ikspak *pak)
{
	...
	if (client && regquery && reguser && regpass && regiq) {
		...
		res = tris_aji_send(client, regiq);
	} else
		tris_log(LOG_ERROR, "Out of memory.\n");
	if (regiq)
		iks_delete(regiq);
...
static void aji_pruneregister(struct aji_client *client)
{
	...
	ASTOBJ_CONTAINER_TRAVERSE(&client->buddies, 1, {
		...
		if (tris_test_flag(&iterator->flags, AJI_AUTOPRUNE)) { /* If autoprune is set on jabber.conf */
			res = tris_aji_send(client, iks_make_s10n(IKS_TYPE_UNSUBSCRIBE, iterator->name,
								 "GoodBye. Your status is no longer needed by Trismedia the Open Source PBX"
								 " so I am no longer subscribing to your presence.\n"));
			res = tris_aji_send(client, iks_make_s10n(IKS_TYPE_UNSUBSCRIBED, iterator->name,
								 "GoodBye.  You are no longer in the Trismedia config file so I am removing"
			...
			res = tris_aji_send(client, removeiq);
		} else if (tris_test_flag(&iterator->flags, AJI_AUTOREGISTER)) {
			res = tris_aji_send(client, iks_make_s10n(IKS_TYPE_SUBSCRIBE, iterator->name, 
								 "Greetings! I am the Trismedia Open Source PBX and I want to subscribe to your presence\n"));
			tris_clear_flag(&iterator->flags, AJI_AUTOREGISTER);
		}
		ASTOBJ_UNLOCK(iterator);
	});
...
static int aji_filter_roster(void *data, ikspak *pak)
{
		...
		while (x) {
			if (!iks_strcmp(iks_name(x), "item")) {
				if (!strcasecmp(iterator->name, iks_find_attrib(x, "jid"))) {
					flag = 1;
					tris_clear_flag(&iterator->flags, AJI_AUTOPRUNE | AJI_AUTOREGISTER);
				}
			}
			x = iks_next(x);
		}
		if (!flag)
			tris_copy_flags(&iterator->flags, &client->flags, AJI_AUTOREGISTER);
		iks_delete(x);
	...
	while (x) {
		...
		if (iks_strcmp(iks_name(x), "item") == 0) {
			...
			buddy = tris_calloc(1, sizeof(*buddy));
			if (!buddy) {
			...
			ASTOBJ_INIT(buddy);
			ASTOBJ_WRLOCK(buddy);
			tris_copy_string(buddy->name, iks_find_attrib(x, "jid"), sizeof(buddy->name));
			tris_clear_flag(&buddy->flags, TRIS_FLAGS_ALL);
			if(tris_test_flag(&client->flags, AJI_AUTOPRUNE)) {
				tris_set_flag(&buddy->flags, AJI_AUTOPRUNE);
				ASTOBJ_MARK(buddy);
				...
				tris_set_flag(&buddy->flags, AJI_AUTOREGISTER);
			}
			ASTOBJ_UNLOCK(buddy);
			if (buddy) {
...
static int aji_initialize(struct aji_client *client)
{
	...
	if (connected == IKS_NET_NOCONN) {
		tris_log(LOG_ERROR, "JABBER ERROR: No Connection\n");
		return IKS_HOOK;
	} else 	if (connected == IKS_NET_NODNS) {
		tris_log(LOG_ERROR, "JABBER ERROR: No DNS %s for client to  %s\n", client->name, S_OR(client->serverhost, client->jid->server));
		return IKS_HOOK;
	}
...
{
	if (client) {
		tris_verb(4, "JABBER: Disconnecting\n");
#ifdef HAVE_OPENSSL
...
static void aji_set_presence(struct aji_client *client, char *to, char *from, int level, char *desc)
{
	...
	if (presence && cnode && client && priority) {
		...
		iks_insert_node(presence, priority);
		iks_insert_attrib(cnode, "node", "http://www.trismedia.org/xmpp/client/caps");
		iks_insert_attrib(cnode, "ver", "trismedia-xmpp");
...
static char *aji_do_set_debug(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (!strncasecmp(a->argv[e->args - 1], "on", 2)) {
		...
		tris_cli(a->fd, "Jabber Debugging Enabled.\n");
		return CLI_SUCCESS;
		...
		tris_cli(a->fd, "Jabber Debugging Disabled.\n");
		return CLI_SUCCESS;
	}
...
static char *aji_show_clients(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_cli(a->fd, "Jabber Users and their status:\n");
	ASTOBJ_CONTAINER_TRAVERSE(&clients, 1, {
		ASTOBJ_RDLOCK(iterator);
		count++;
		...
		tris_cli(a->fd, "       User: %s     - %s\n", iterator->user, status);
		ASTOBJ_UNLOCK(iterator);
	});
	tris_cli(a->fd, "----\n");
	tris_cli(a->fd, "   Number of users: %d\n", count);
	return CLI_SUCCESS;
}
...
static char *aji_show_buddies(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	tris_cli(a->fd, "Jabber buddy lists\n");
	ASTOBJ_CONTAINER_TRAVERSE(&clients, 1, {
		tris_cli(a->fd,"Client: %s\n", iterator->user);
		client = iterator;
		ASTOBJ_CONTAINER_TRAVERSE(&client->buddies, 1, {
			ASTOBJ_RDLOCK(iterator);
			tris_cli(a->fd,"\tBuddy:\t%s\n", iterator->name);
			if (!iterator->resources)
				tris_cli(a->fd,"\t\tResource: None\n");	
			for (resource = iterator->resources; resource; resource = resource->next) {
				tris_cli(a->fd,"\t\tResource: %s\n", resource->resource);
				if(resource->cap) {
					tris_cli(a->fd,"\t\t\tnode: %s\n", resource->cap->parent->node);
					tris_cli(a->fd,"\t\t\tversion: %s\n", resource->cap->version);
					tris_cli(a->fd,"\t\t\tJingle capable: %s\n", resource->cap->jingle ? "yes" : "no");
				}
				tris_cli(a->fd,"\t\tStatus: %d\n", resource->status);
				tris_cli(a->fd,"\t\tPriority: %d\n", resource->priority);
			}
			ASTOBJ_UNLOCK(iterator);
		});
		iterator = client;
...
static char *aji_test(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	if (!(client = ASTOBJ_CONTAINER_FIND(&clients, name))) {
		tris_cli(a->fd, "Unable to find client '%s'!\n", name);
		return CLI_FAILURE;
	}

	/* XXX Does Matt really want everyone to use his personal address for tests? */ /* XXX yes he does */
	tris_aji_send_chat(client, "mogorman@astjab.org", "blahblah");
	ASTOBJ_CONTAINER_TRAVERSE(&client->buddies, 1, {
		ASTOBJ_RDLOCK(iterator);
		tris_verbose("User: %s\n", iterator->name);
		for (resource = iterator->resources; resource; resource = resource->next) {
			tris_verbose("Resource: %s\n", resource->resource);
			if(resource->cap) {
				tris_verbose("   client: %s\n", resource->cap->parent->node);
				tris_verbose("   version: %s\n", resource->cap->version);
				tris_verbose("   Jingle Capable: %d\n", resource->cap->jingle);
			}
			tris_verbose("	Priority: %d\n", resource->priority);
			tris_verbose("	Status: %d\n", resource->status); 
			tris_verbose("	Message: %s\n", S_OR(resource->description,"")); 
		}
		ASTOBJ_UNLOCK(iterator);
	});
	tris_verbose("\nOooh a working message stack!\n");
	TRIS_LIST_LOCK(&client->messages);
	TRIS_LIST_TRAVERSE(&client->messages, tmp, list) {
		tris_verbose("	Message from: %s with id %s @ %s	%s\n",tmp->from, S_OR(tmp->id,""), ctime(&tmp->arrived), S_OR(tmp->message, ""));
	}
	TRIS_LIST_UNLOCK(&client->messages);
	ASTOBJ_UNREF(client, aji_client_destroy);
...
static int aji_create_client(char *label, struct tris_variable *var, int debug)
{
	...
	if (!client) {
		flag = 1;
		client = tris_calloc(1, sizeof(*client));
		if (!client) {
			tris_log(LOG_ERROR, "Out of memory!\n");
			return 0;
		}
	...
	tris_copy_string(client->name, label, sizeof(client->name));
	tris_copy_string(client->mid, "aaaaa", sizeof(client->mid));

	/* Set default values for the client object */
	client->debug = debug;
	tris_copy_flags(&client->flags, &globalflags, TRIS_FLAGS_ALL);
	client->port = 5222;
	client->usetls = 1;
	client->usesasl = 1;
	client->forcessl = 0;
	...
	TRIS_LIST_HEAD_INIT(&client->messages);
	client->component = 0;
	tris_copy_string(client->statusmessage, "Online and Available", sizeof(client->statusmessage));
	client->priority = 0;
	...
	while (var) {
		if (!strcasecmp(var->name, "username"))
			tris_copy_string(client->user, var->value, sizeof(client->user));
		else if (!strcasecmp(var->name, "serverhost"))
			tris_copy_string(client->serverhost, var->value, sizeof(client->serverhost));
		else if (!strcasecmp(var->name, "secret"))
			tris_copy_string(client->password, var->value, sizeof(client->password));
		else if (!strcasecmp(var->name, "statusmessage"))
			tris_copy_string(client->statusmessage, var->value, sizeof(client->statusmessage));
		else if (!strcasecmp(var->name, "port"))
			client->port = atoi(var->value);
		else if (!strcasecmp(var->name, "timeout"))
			client->message_timeout = atoi(var->value);
		else if (!strcasecmp(var->name, "debug"))
			client->debug = (tris_false(var->value)) ? 0 : 1;
		else if (!strcasecmp(var->name, "type")) {
			if (!strcasecmp(var->value, "component"))
				client->component = 1;
		} else if (!strcasecmp(var->name, "usetls")) {
			client->usetls = (tris_false(var->value)) ? 0 : 1;
		} else if (!strcasecmp(var->name, "usesasl")) {
			client->usesasl = (tris_false(var->value)) ? 0 : 1;
		} else if (!strcasecmp(var->name, "forceoldssl"))
			client->forcessl = (tris_false(var->value)) ? 0 : 1;
		else if (!strcasecmp(var->name, "keepalive"))
			client->keepalive = (tris_false(var->value)) ? 0 : 1;
		else if (!strcasecmp(var->name, "autoprune"))
			tris_set2_flag(&client->flags, tris_true(var->value), AJI_AUTOPRUNE);
		else if (!strcasecmp(var->name, "autoregister"))
			tris_set2_flag(&client->flags, tris_true(var->value), AJI_AUTOREGISTER);
			...
			#else
			{
				tris_log(LOG_WARNING, "Your iksemel doesn't support invisible status: falling back to DND\n");
				client->status = IKS_SHOW_DND;
			}
			#endif
			else
				tris_log(LOG_WARNING, "Unknown presence status: %s\n", var->value);
		}
	...
	*/
	...
	tris_copy_string(client->name_space, (client->component) ? "jabber:component:accept" : "jabber:client", sizeof(client->name_space));
	client->p = iks_stream_new(client->name_space, client, aji_act_hook);
	if (!client->p) {
		tris_log(LOG_ERROR, "Failed to create stream for client '%s'!\n", client->name);
		return 0;
	}
	client->stack = iks_stack_new(8192, 8192);
	if (!client->stack) {
		tris_log(LOG_ERROR, "Failed to allocate stack for client '%s'\n", client->name);
		return 0;
	}
	client->f = iks_filter_new();
	if (!client->f) {
		tris_log(LOG_ERROR, "Failed to create filter for client '%s'\n", client->name);
		return 0;
	}
	if (!strchr(client->user, '/') && !client->component) { /*client */
		resource = NULL;
		if (asprintf(&resource, "%s/trismedia", client->user) >= 0) {
				...
				*user = '\0';
				...
				if (user && user[0] != '\0') {
					if ((pass = strchr(user, ','))) {
						*pass = '\0';
						pass++;
						tris_copy_string(buddy->pass, pass, sizeof(buddy->pass));
						tris_copy_string(buddy->user, user, sizeof(buddy->user));
						tris_copy_string(buddy->name, buddyname, sizeof(buddy->name));
						tris_copy_string(buddy->server, server, sizeof(buddy->server));
						return 1;
					}
				}
			}
...
static int aji_create_buddy(char *label, struct aji_client *client)
{
	...
	ASTOBJ_WRLOCK(buddy);
	tris_copy_string(buddy->name, label, sizeof(buddy->name));
	ASTOBJ_UNLOCK(buddy);
	if(flag)
		ASTOBJ_CONTAINER_LINK(&client->buddies, buddy);
	else {
...
static int aji_load_config(int reload)
{
	...
	if ((cfg = tris_config_load(JABBER_CONFIG, config_flags)) == CONFIG_STATUS_FILEUNCHANGED)
		return -1;

	/* Reset flags to default value */
	tris_set_flag(&globalflags, AJI_AUTOREGISTER);
	...
	for (var = tris_variable_browse(cfg, "general"); var; var = var->next) {
		if (!strcasecmp(var->name, "debug")) {
			debug = (tris_false(tris_variable_retrieve(cfg, "general", "debug"))) ? 0 : 1;
		} else if (!strcasecmp(var->name, "autoprune")) {
			tris_set2_flag(&globalflags, tris_true(var->value), AJI_AUTOPRUNE);
		} else if (!strcasecmp(var->name, "autoregister")) {
			tris_set2_flag(&globalflags, tris_true(var->value), AJI_AUTOREGISTER);
		}
	}
	...
	tris_config_destroy(cfg); /* or leak memory */
	return 1;
}
...
struct aji_client *tris_aji_get_client(const char *name)
{
	...
	if (!client && strchr(name, '@')) {
		ASTOBJ_CONTAINER_TRAVERSE(&clients, 1, {
			aux = tris_strdupa(iterator->user);
			if (strchr(aux, '/')) {
...
"Variables: \n"
"  Jabber:    Client or transport Trismedia uses to connect to JABBER\n"
...
static int aji_reload(int reload)
{
	...
	if (!(res = aji_load_config(reload))) {
		tris_log(LOG_ERROR, "JABBER: Failed to load config.\n");
		return 0;
	} else if (res == -1)
		return 1;
	...
	ASTOBJ_CONTAINER_TRAVERSE(&clients, 1, {
		...
		if(iterator->state == AJI_DISCONNECTED) {
			if (!iterator->thread)
				tris_pthread_create_background(&iterator->thread, NULL, aji_recv_loop, iterator);
...
static int unload_module(void)
{
	...
	tris_cli_unregister_multiple(aji_cli, ARRAY_LEN(aji_cli));
	tris_unregister_application(app_ajisend);
	tris_unregister_application(app_ajistatus);
	tris_manager_unregister("JabberSend");
	tris_custom_function_unregister(&jabberstatus_function);
	
	ASTOBJ_CONTAINER_TRAVERSE(&clients, 1, {
		ASTOBJ_RDLOCK(iterator);
		tris_debug(3, "JABBER: Releasing and disconnecting client: %s\n", iterator->name);
		iterator->state = AJI_DISCONNECTING;
		tris_aji_disconnect(iterator);
...
static int load_module(void)
{
	...
	tris_manager_register2("JabberSend", EVENT_FLAG_SYSTEM, manager_jabber_send,
			"Sends a message to a Jabber Client", mandescr_jabber_send);
	tris_register_application_xml(app_ajisend, aji_send_exec);
	tris_register_application_xml(app_ajistatus, aji_status_exec);
	tris_cli_register_multiple(aji_cli, ARRAY_LEN(aji_cli));
	tris_custom_function_register(&jabberstatus_function);
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_GLOBAL_SYMBOLS, "AJI - Trismedia Jabber Interface",
		.load = load_module,
