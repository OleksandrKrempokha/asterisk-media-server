...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 217036 $")
...
static char *complete_ulimit(struct tris_cli_args *a)
{
	int which = 0, i;
	...
	for (i = 0; i < ARRAY_LEN(limits); i++) {
		if (!strncasecmp(limits[i].clicmd, a->word, wordlen)) {
			if (++which > a->n)
				return tris_strdup(limits[i].clicmd);
		}
	}
	return NULL;
}

static char *handle_cli_ulimit(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	int resource;
	...
	if (a->argc == 1) {
		...
		for (resource = 0; resource < ARRAY_LEN(limits); resource++) {
			struct tris_cli_args newArgs = { .argv = newargv, .argc = 2 };
			tris_copy_string(arg2, limits[resource].clicmd, sizeof(arg2));
		...
		if (resource == -1) {
			tris_cli(a->fd, "Unknown resource\n");
			return CLI_FAILURE;
		}
		...
		if (a->argc == 3) {
...
#endif
				tris_cli(a->fd, "Resource not permitted to be set\n");
				return CLI_FAILURE;
			}
			...
			if (!getrlimit(resource, &rlimit)) {
				...
				if (rlimit.rlim_max == RLIM_INFINITY)
					tris_copy_string(printlimit, "effectively unlimited", sizeof(printlimit));
				else
					snprintf(printlimit, sizeof(printlimit), "limited to %d", (int) rlimit.rlim_cur);
				desc = str2desc(a->argv[1]);
				tris_cli(a->fd, "%c%s (%s) is %s.\n", toupper(desc[0]), desc + 1, a->argv[1], printlimit);
			} else
				tris_cli(a->fd, "Could not retrieve resource limits for %s: %s\n", str2desc(a->argv[1]), strerror(errno));
			return CLI_SUCCESS;
		}
	}
}

static struct tris_cli_entry cli_ulimit =
	TRIS_CLI_DEFINE(handle_cli_ulimit, "Set or show process resource limits");

static int unload_module(void)
{
	return tris_cli_unregister(&cli_ulimit);
}

static int load_module(void)
{
	return tris_cli_register(&cli_ulimit) ? TRIS_MODULE_LOAD_FAILURE : TRIS_MODULE_LOAD_SUCCESS;
}

TRIS_MODULE_INFO_STANDARD(TRISMEDIA_GPL_KEY, "Resource limits");
