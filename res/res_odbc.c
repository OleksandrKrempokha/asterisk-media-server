...
struct odbc_class
{
	TRIS_LIST_ENTRY(odbc_class) list;
	char name[80];
	char dsn[80];
	char *username;
	char *password;
...
static TRIS_RWLIST_HEAD_STATIC(odbc_tables, odbc_cache_tables);
...
TRIS_THREADSTORAGE(errors_buf);

static struct tris_datastore_info txn_info = {
	.type = "ODBC_Transaction",
	.destroy = odbc_txn_free,
};

struct odbc_txn_frame {
	TRIS_LIST_ENTRY(odbc_txn_frame) list;
...
static struct odbc_txn_frame *find_transaction(struct tris_channel *chan, struct odbc_obj *obj, const char *name, int active)
{
	struct tris_datastore *txn_store;
	TRIS_LIST_HEAD(, odbc_txn_frame) *oldlist;
	...
	if ((txn_store = tris_channel_datastore_find(chan, &txn_info, NULL))) {
		oldlist = txn_store->data;
	} else {
		/* Need to create a new datastore */
		if (!(txn_store = tris_datastore_alloc(&txn_info, NULL))) {
			tris_log(LOG_ERROR, "Unable to allocate a new datastore.  Cannot create a new transaction.\n");
			tris_channel_unlock(chan);
			return NULL;
		}

		if (!(oldlist = tris_calloc(1, sizeof(*oldlist)))) {
			tris_log(LOG_ERROR, "Unable to allocate datastore list head.  Cannot create a new transaction.\n");
			tris_datastore_free(txn_store);
			tris_channel_unlock(chan);
			return NULL;
		}
		...
		tris_channel_datastore_add(chan, txn_store);
	}
	...
	if (obj != NULL || active == 1) {
		TRIS_LIST_TRAVERSE(oldlist, txn, list) {
	...
	if (name && obj && (txn = tris_calloc(1, sizeof(*txn) + strlen(name) + 1))) {
		...
		TRIS_LIST_TRAVERSE(oldlist, otxn, list) {
			otxn->active = 0;
		}
		TRIS_LIST_INSERT_TAIL(oldlist, txn, list);
...
static struct odbc_txn_frame *release_transaction(struct odbc_txn_frame *tx)
{
	...
	tris_debug(2, "release_transaction(%p) called (tx->obj = %p, tx->obj->txf = %p)\n", tx, tx->obj, tx->obj ? tx->obj->txf : NULL);
	...
	if (tx->owner) {
		...
		tris_channel_lock(tx->owner);
		if ((txn_store = tris_channel_datastore_find(tx->owner, &txn_info, NULL))) {
			oldlist = txn_store->data;
			TRIS_LIST_LOCK(oldlist);
			TRIS_LIST_REMOVE(oldlist, tx, list);
			TRIS_LIST_UNLOCK(oldlist);
		}
		tris_channel_unlock(tx->owner);
		tx->owner = NULL;
	}
...
static void odbc_txn_free(void *vdata)
{
	...
	TRIS_LIST_HEAD(, odbc_txn_frame) *oldlist = vdata;

	tris_debug(2, "odbc_txn_free(%p) called\n", vdata);

	TRIS_LIST_LOCK(oldlist);
	while ((tx = TRIS_LIST_REMOVE_HEAD(oldlist, list))) {
...
static int mark_transaction_active(struct tris_channel *chan, struct odbc_txn_frame *tx)
{
	struct tris_datastore *txn_store;
	TRIS_LIST_HEAD(, odbc_txn_frame) *oldlist;
...
static void odbc_class_destructor(void *data)
{
	 ...
	 */
	if (class->username) {
		tris_free(class->username);
	}
	if (class->password) {
		tris_free(class->password);
	}
	if (class->sanitysql) {
		tris_free(class->sanitysql);
	}
...
static void odbc_obj_destructor(void *data)
{
	...
	tris_mutex_destroy(&obj->lock);
	ao2_ref(class, -1);
}

static void destroy_table_cache(struct odbc_cache_tables *table) {
	struct odbc_cache_columns *col;
	tris_debug(1, "Destroying table cache for %s\n", table->table);
	TRIS_RWLIST_WRLOCK(&table->columns);
	while ((col = TRIS_RWLIST_REMOVE_HEAD(&table->columns, list))) {
		tris_free(col);
	}
	TRIS_RWLIST_UNLOCK(&table->columns);
	TRIS_RWLIST_HEAD_DESTROY(&table->columns);
	tris_free(table);
}
 ...
 * \since 1.6.1
 */
struct odbc_cache_tables *tris_odbc_find_table(const char *database, const char *tablename)
{
	...
	struct odbc_obj *obj = tris_odbc_request_obj(database, 0);

	TRIS_RWLIST_RDLOCK(&odbc_tables);
	TRIS_RWLIST_TRAVERSE(&odbc_tables, tableptr, list) {
		if (strcmp(tableptr->connection, database) == 0 && strcmp(tableptr->table, tablename) == 0) {
			break;
		}
	}
	if (tableptr) {
		TRIS_RWLIST_RDLOCK(&tableptr->columns);
		TRIS_RWLIST_UNLOCK(&odbc_tables);
		if (obj) {
	...
	if (!obj) {
		tris_log(LOG_WARNING, "Unable to retrieve database handle for table description '%s@%s'\n", tablename, database);
		TRIS_RWLIST_UNLOCK(&odbc_tables);
		return NULL;
	}
	...
	do {
		...
		if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
			if (try == 0) {
				try = 1;
				tris_odbc_sanity_check(obj);
				continue;
			}
			tris_log(LOG_WARNING, "SQL Alloc Handle failed on connection '%s'!\n", database);
			break;
		}
		...
		if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
			...
			tris_log(LOG_ERROR, "Unable to query database columns on connection '%s'.\n", database);
			break;
		}

		if (!(tableptr = tris_calloc(sizeof(char), sizeof(*tableptr) + strlen(database) + 1 + strlen(tablename) + 1))) {
			tris_log(LOG_ERROR, "Out of memory creating entry for table '%s' on connection '%s'\n", tablename, database);
			break;
		}
		...
		TRIS_RWLIST_HEAD_INIT(&(tableptr->columns));

		while ((res = SQLFetch(stmt)) != SQL_NO_DATA && res != SQL_ERROR) {
			SQLGetData(stmt,  4, SQL_C_CHAR, columnname, sizeof(columnname), &sqlptr);

			if (!(entry = tris_calloc(sizeof(char), sizeof(*entry) + strlen(columnname) + 1))) {
				tris_log(LOG_ERROR, "Out of memory creating entry for column '%s' in table '%s' on connection '%s'\n", columnname, tablename, database);
				error = 1;
				break;
			}
			...
			tris_verb(10, "Found %s column with type %hd with len %ld, octetlen %ld, and numlen (%hd,%hd)\n", entry->name, entry->type, (long) entry->size, (long) entry->octetlen, entry->decimals, entry->radix);
			/* Insert column info into column list */
			TRIS_LIST_INSERT_TAIL(&(tableptr->columns), entry, list);
		}
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);

		TRIS_RWLIST_INSERT_TAIL(&odbc_tables, tableptr, list);
		TRIS_RWLIST_RDLOCK(&(tableptr->columns));
		break;
	} while (1);
...
struct odbc_cache_columns *tris_odbc_find_column(struct odbc_cache_tables *table, const char *colname)
{
	struct odbc_cache_columns *col;
	TRIS_RWLIST_TRAVERSE(&table->columns, col, list) {
...
int tris_odbc_clear_cache(const char *database, const char *tablename)
{
	struct odbc_cache_tables *tableptr;

	TRIS_RWLIST_WRLOCK(&odbc_tables);
	TRIS_RWLIST_TRAVERSE_SAFE_BEGIN(&odbc_tables, tableptr, list) {
		if (strcmp(tableptr->connection, database) == 0 && strcmp(tableptr->table, tablename) == 0) {
...
SQLHSTMT tris_odbc_direct_execute(struct odbc_obj *obj, SQLHSTMT (*exec_cb)(struct odbc_obj *obj, void *data), void *data)
{
	int attempt;
	SQLHSTMT stmt;
	...
	for (attempt = 0; attempt < 2; attempt++) {
		...
		if (stmt) {
			...
			tris_log(LOG_WARNING, "Failed to execute, but unable to reconnect, as we're transactional.\n");
			break;
		} else {
			obj->up = 0;
			tris_log(LOG_WARNING, "SQL Exec Direct failed.  Attempting a reconnect...\n");
...
SQLHSTMT tris_odbc_prepare_and_execute(struct odbc_obj *obj, SQLHSTMT (*prepare_cb)(struct odbc_obj *obj, void *data), void *data)
{
	int res = 0, i, attempt;
	...
	for (attempt = 0; attempt < 2; attempt++) {
		...
		if (stmt) {
			...
			if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO) && (res != SQL_NO_DATA)) {
				...
				if (obj->tx) {
					tris_log(LOG_WARNING, "SQL Execute error, but unable to reconnect, as we're transactional.\n");
					break;
				} else {
					tris_log(LOG_WARNING, "SQL Execute error %d! Attempting a reconnect...\n", res);
					SQLFreeHandle(SQL_HANDLE_STMT, stmt);
					stmt = NULL;
...
int tris_odbc_smart_execute(struct odbc_obj *obj, SQLHSTMT stmt) 
{
	int res = 0, i;
	SQLINTEGER nativeerror=0, numfields=0;
	SQLSMALLINT diagbytes=0;
...
SQLRETURN tris_odbc_tris_str_SQLGetData(struct tris_str **buf, int pmaxlen, SQLHSTMT StatementHandle, SQLUSMALLINT ColumnNumber, SQLSMALLINT TargetType, SQLLEN *StrLen_or_Ind)
{
	SQLRETURN res;

	if (pmaxlen == 0) {
		if (SQLGetData(StatementHandle, ColumnNumber, TargetType, tris_str_buffer(*buf), 0, StrLen_or_Ind) == SQL_SUCCESS_WITH_INFO) {
			tris_str_make_space(buf, *StrLen_or_Ind + 1);
		}
	} else if (pmaxlen > 0) {
		tris_str_make_space(buf, pmaxlen);
	}
	res = SQLGetData(StatementHandle, ColumnNumber, TargetType, tris_str_buffer(*buf), tris_str_size(*buf), StrLen_or_Ind);
	tris_str_update(*buf);

	return res;
}

int tris_odbc_sanity_check(struct odbc_obj *obj) 
{
	char *test_sql = "select 1";
	SQLHSTMT stmt;
	int res = 0;

	if (!tris_strlen_zero(obj->parent->sanitysql))
	...
	if (!obj->up && !obj->tx) { /* Try to reconnect! */
		tris_log(LOG_WARNING, "Connection is down attempting to reconnect...\n");
		odbc_obj_disconnect(obj);
		odbc_obj_connect(obj);
	}
	return obj->up;
...
static int load_odbc_config(void)
{
	...
	config = tris_config_load(cfg, config_flags);
	if (config == CONFIG_STATUS_FILEMISSING || config == CONFIG_STATUS_FILEINVALID) {
		tris_log(LOG_WARNING, "Unable to load config file res_odbc.conf\n");
		return -1;
	}
	for (cat = tris_category_browse(config, NULL); cat; cat=tris_category_browse(config, cat)) {
		if (!strcasecmp(cat, "ENV")) {
			for (v = tris_variable_browse(config, cat); v; v = v->next) {
				setenv(v->name, v->value, 1);
				tris_log(LOG_NOTICE, "Adding ENV var: %s=%s\n", v->name, v->value);
			}
		} else {
			...
			for (v = tris_variable_browse(config, cat); v; v = v->next) {
				if (!strcasecmp(v->name, "pooling")) {
					...
					if (tris_true(v->value) && !limit) {
						tris_log(LOG_WARNING, "Limit should be a number, not a boolean: '%s'.  Setting limit to 1023 for ODBC class '%s'.\n", v->value, cat);
						limit = 1023;
					} else if (tris_false(v->value)) {
						tris_log(LOG_WARNING, "Limit should be a number, not a boolean: '%s'.  Disabling ODBC class '%s'.\n", v->value, cat);
						enabled = 0;
						break;
					}
					...
					bse = tris_true(v->value);
				} else if (!strcasecmp(v->name, "forcecommit")) {
					forcecommit = tris_true(v->value);
				} else if (!strcasecmp(v->name, "isolation")) {
					if ((isolation = text2isolation(v->value)) == 0) {
						tris_log(LOG_ERROR, "Unrecognized value for 'isolation': '%s' in section '%s'\n", v->value, cat);
						isolation = SQL_TXN_READ_COMMITTED;
					}
				}
			}

			if (enabled && !tris_strlen_zero(dsn)) {
				...
				if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
					tris_log(LOG_WARNING, "res_odbc: Error SetEnv\n");
					ao2_ref(new, -1);
					return res;
				}
				...
				if (pooling) {
					...
					if (limit) {
						...
						tris_log(LOG_WARNING, "Pooling without also setting a limit is pointless.  Changing limit from 0 to 5.\n");
						new->limit = 5;
					}
				}
				...
				if (cat)
					tris_copy_string(new->name, cat, sizeof(new->name));
				if (dsn)
					tris_copy_string(new->dsn, dsn, sizeof(new->dsn));
				if (username && !(new->username = tris_strdup(username))) {
					ao2_ref(new, -1);
					break;
				}
				if (password && !(new->password = tris_strdup(password))) {
					ao2_ref(new, -1);
					break;
				}
				if (sanitysql && !(new->sanitysql = tris_strdup(sanitysql))) {
					ao2_ref(new, -1);
					break;
				}

				odbc_register_class(new, preconnect);
				tris_log(LOG_NOTICE, "Registered ODBC class '%s' dsn->[%s]\n", cat, dsn);
				ao2_ref(new, -1);
				new = NULL;
			}
		}
...
static char *handle_cli_odbc_show(struct tris_cli_entry *e, int cmd, struct tris_cli_args *a)
{
	...
	while ((class = ao2_iterator_next(&aoi))) {
		if ((a->argc == 2) || (a->argc == 3 && !strcmp(a->argv[2], "all")) || (!strcmp(a->argv[2], class->name))) {
			...
			if (class->haspool) {
				...
				tris_cli(a->fd, "  Pooled: Yes\n  Limit:  %d\n  Connections in use: %d\n", class->limit, class->count);

				while ((current = ao2_iterator_next(&aoi2))) {
					tris_mutex_lock(&current->lock);
#ifdef DEBUG_THREADS
					tris_cli(a->fd, "    - Connection %d: %s (%s:%d %s)\n", ++count,
						current->used ? "in use" :
						current->up && tris_odbc_sanity_check(current) ? "connected" : "disconnected",
						current->file, current->lineno, current->function);
#else
					tris_cli(a->fd, "    - Connection %d: %s\n", ++count,
						current->used ? "in use" :
						current->up && tris_odbc_sanity_check(current) ? "connected" : "disconnected");
#endif
					tris_mutex_unlock(&current->lock);
					ao2_ref(current, -1);
				}
				...
				struct ao2_iterator aoi2 = ao2_iterator_init(class->obj_container, 0);
				while ((current = ao2_iterator_next(&aoi2))) {
					tris_cli(a->fd, "  Pooled: No\n  Connected: %s\n", current->used ? "In use" :
						current->up && tris_odbc_sanity_check(current) ? "Yes" : "No");
					ao2_ref(current, -1);
				}
				ao2_iterator_destroy(&aoi2);
			}
...
static struct tris_cli_entry cli_odbc[] = {
	TRIS_CLI_DEFINE(handle_cli_odbc_show, "List ODBC DSN(s)")
};
...
static int odbc_register_class(struct odbc_class *class, int preconnect)
{
	...
	if (class) {
		...
		if (preconnect) {
			...
			obj = tris_odbc_request_obj(class->name, 0);
			if (obj) {
				tris_odbc_release_obj(obj);
			}
		}
		...
		tris_log(LOG_WARNING, "Attempted to register a NULL class?\n");
		return -1;
	}
}
...
static void odbc_release_obj2(struct odbc_obj *obj, struct odbc_txn_frame *tx)
{
	...
	tris_debug(2, "odbc_release_obj2(%p) called (obj->txf = %p)\n", obj, obj->txf);
	if (tx) {
		tris_debug(1, "called on a transactional handle with %s\n", tx->forcecommit ? "COMMIT" : "ROLLBACK");
		if (SQLEndTran(SQL_HANDLE_DBC, obj->con, tx->forcecommit ? SQL_COMMIT : SQL_ROLLBACK) == SQL_ERROR) {
			...
			for (i = 0; i < numfields; i++) {
				...
				tris_log(LOG_WARNING, "SQLEndTran returned an error: %s: %s\n", state, diagnostic);
		...
		if (SQLSetConnectAttr(obj->con, SQL_ATTR_AUTOCOMMIT, (void *)SQL_AUTOCOMMIT_ON, 0) == SQL_ERROR) {
			...
			for (i = 0; i < numfields; i++) {
				SQLGetDiagRec(SQL_HANDLE_DBC, obj->con, i + 1, state, &nativeerror, diagnostic, sizeof(diagnostic), &diagbytes);
				tris_log(LOG_WARNING, "SetConnectAttr (Autocommit) returned an error: %s: %s\n", state, diagnostic);
				if (i > 10) {
					tris_log(LOG_WARNING, "Oh, that was good.  There are really %d diagnostics?\n", (int)numfields);
					break;
				}
...
void tris_odbc_release_obj(struct odbc_obj *obj)
{
	struct odbc_txn_frame *tx = find_transaction(NULL, obj, NULL, 0);
	odbc_release_obj2(obj, tx);
}

int tris_odbc_backslash_is_escape(struct odbc_obj *obj)
{
	return obj->parent->backslash_is_escape;
}

static int commit_exec(struct tris_channel *chan, void *data)
{
	struct odbc_txn_frame *tx;
	SQLINTEGER nativeerror=0, numfields=0;
	SQLSMALLINT diagbytes=0, i;
	...
	if (tx) {
		if (SQLEndTran(SQL_HANDLE_DBC, tx->obj->con, SQL_COMMIT) == SQL_ERROR) {
			struct tris_str *errors = tris_str_thread_get(&errors_buf, 16);
			tris_str_reset(errors);
			...
			for (i = 0; i < numfields; i++) {
				...
				tris_str_append(&errors, 0, "%s%s", tris_str_strlen(errors) ? "," : "", state);
				tris_log(LOG_WARNING, "SQLEndTran returned an error: %s: %s\n", state, diagnostic);
				if (i > 10) {
					tris_log(LOG_WARNING, "Oh, that was good.  There are really %d diagnostics?\n", (int)numfields);
					break;
			...
			pbx_builtin_setvar_helper(chan, "COMMIT_RESULT", tris_str_buffer(errors));
		}
	}
	return 0;
}

static int rollback_exec(struct tris_channel *chan, void *data)
{
	struct odbc_txn_frame *tx;
	SQLINTEGER nativeerror=0, numfields=0;
	SQLSMALLINT diagbytes=0, i;
	...
	if (tx) {
		if (SQLEndTran(SQL_HANDLE_DBC, tx->obj->con, SQL_ROLLBACK) == SQL_ERROR) {
			...
			pbx_builtin_setvar_helper(chan, "ROLLBACK_RESULT", tris_str_buffer(errors));
		}
	}
	return 0;
}
...
#ifdef DEBUG_THREADS
struct odbc_obj *_tris_odbc_request_obj2(const char *name, struct tris_flags flags, const char *file, const char *function, int lineno)
#else
struct odbc_obj *tris_odbc_request_obj2(const char *name, struct tris_flags flags)
#endif
{
	...
	if (class->haspool) {
		...
		if (!obj && (class->count < class->limit)) {
			...
			tris_assert(ao2_ref(obj, 0) == 1);
			tris_mutex_init(&obj->lock);
			/* obj inherits the outstanding reference to class */
			obj->parent = class;
			class = NULL;
			if (odbc_obj_connect(obj) == ODBC_FAIL) {
				tris_log(LOG_WARNING, "Failed to connect to %s\n", name);
				ao2_ref(obj, -1);
				tris_assert(ao2_ref(class, 0) > 0);
				obj = NULL;
			} else {
				obj->used = 1;
				ao2_link(obj->parent->obj_container, obj);
				tris_atomic_fetchadd_int(&obj->parent->count, +1);
			}
		} else {
		...
		if (obj && tris_test_flag(&flags, RES_ODBC_INDEPENDENT_CONNECTION)) {
			...
			if (SQLSetConnectAttr(obj->con, SQL_ATTR_AUTOCOMMIT, (void *)SQL_AUTOCOMMIT_OFF, 0) == SQL_ERROR) {
				...
				for (i = 0; i < numfields; i++) {
					SQLGetDiagRec(SQL_HANDLE_DBC, obj->con, i + 1, state, &nativeerror, diagnostic, sizeof(diagnostic), &diagbytes);
					tris_log(LOG_WARNING, "SQLSetConnectAttr (Autocommit) returned an error: %s: %s\n", state, diagnostic);
					if (i > 10) {
						tris_log(LOG_WARNING, "Oh, that was good.  There are really %d diagnostics?\n", (int)numfields);
						break;
					}
	...
	} else if (tris_test_flag(&flags, RES_ODBC_INDEPENDENT_CONNECTION)) {
	...
	if (obj && SQLSetConnectAttr(obj->con, SQL_ATTR_TXN_ISOLATION, (void *)(long)obj->parent->isolation, 0) == SQL_ERROR) {
		...
		for (i = 0; i < numfields; i++) {
			SQLGetDiagRec(SQL_HANDLE_DBC, obj->con, i + 1, state, &nativeerror, diagnostic, sizeof(diagnostic), &diagbytes);
			tris_log(LOG_WARNING, "SetConnectAttr (Txn isolation) returned an error: %s: %s\n", state, diagnostic);
			if (i > 10) {
				tris_log(LOG_WARNING, "Oh, that was good.  There are really %d diagnostics?\n", (int)numfields);
				break;
			}
	...
	if (obj && tris_test_flag(&flags, RES_ODBC_SANITY_CHECK)) {
		tris_odbc_sanity_check(obj);
	} else if (obj && obj->parent->idlecheck > 0 && tris_tvdiff_sec(tris_tvnow(), obj->last_used) > obj->parent->idlecheck)
		odbc_obj_connect(obj);

#ifdef DEBUG_THREADS
	if (obj) {
		tris_copy_string(obj->file, file, sizeof(obj->file));
		tris_copy_string(obj->function, function, sizeof(obj->function));
		obj->lineno = lineno;
	}
#endif
	tris_assert(class == NULL);
...
#ifdef DEBUG_THREADS
struct odbc_obj *_tris_odbc_request_obj(const char *name, int check, const char *file, const char *function, int lineno)
#else
struct odbc_obj *tris_odbc_request_obj(const char *name, int check)
#endif
{
	struct tris_flags flags = { check ? RES_ODBC_SANITY_CHECK : 0 };
#ifdef DEBUG_THREADS
	return _tris_odbc_request_obj2(name, flags, file, function, lineno);
#else
	return tris_odbc_request_obj2(name, flags);
#endif
}

struct odbc_obj *tris_odbc_retrieve_transaction_obj(struct tris_channel *chan, const char *objname)
{
	struct tris_datastore *txn_store;
	TRIS_LIST_HEAD(, odbc_txn_frame) *oldlist;
	struct odbc_txn_frame *txn = NULL;
...
static odbc_status odbc_obj_disconnect(struct odbc_obj *obj)
{
	...
	if (obj->parent) {
		if (res == SQL_SUCCESS || res == SQL_SUCCESS_WITH_INFO) {
			tris_log(LOG_DEBUG, "Disconnected %d from %s [%s]\n", res, obj->parent->name, obj->parent->dsn);
		} else {
			tris_log(LOG_DEBUG, "res_odbc: %s [%s] already disconnected\n", obj->parent->name, obj->parent->dsn);
		}
	}

	if ((res = SQLFreeHandle(SQL_HANDLE_DBC, obj->con) == SQL_SUCCESS)) {
		obj->con = NULL;
		tris_log(LOG_DEBUG, "Database handle deallocated\n");
	} else {
		SQLGetDiagRec(SQL_HANDLE_DBC, obj->con, 1, state, &err, msg, 100, &mlen);
		tris_log(LOG_WARNING, "Unable to deallocate database handle? %d errno=%d %s\n", res, (int)err, msg);
	}
...
static odbc_status odbc_obj_connect(struct odbc_obj *obj)
{
	...
	if (obj->up) {
		odbc_obj_disconnect(obj);
		tris_log(LOG_NOTICE, "Re-connecting %s\n", obj->parent->name);
	} else {
		tris_log(LOG_NOTICE, "Connecting %s\n", obj->parent->name);
	}

	res = SQLAllocHandle(SQL_HANDLE_DBC, obj->parent->env, &obj->con);

	if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
		tris_log(LOG_WARNING, "res_odbc: Error AllocHDB %d\n", res);
		tris_mutex_unlock(&obj->lock);
		return ODBC_FAIL;
	}
	...
	if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
		SQLGetDiagRec(SQL_HANDLE_DBC, obj->con, 1, state, &err, msg, 100, &mlen);
		tris_mutex_unlock(&obj->lock);
		tris_log(LOG_WARNING, "res_odbc: Error SQLConnect=%d errno=%d %s\n", res, (int)err, msg);
		return ODBC_FAIL;
	} else {
		tris_log(LOG_NOTICE, "res_odbc: Connected to %s [%s]\n", obj->parent->name, obj->parent->dsn);
		obj->up = 1;
		obj->last_used = tris_tvnow();
	}
...
static int acf_transaction_read(struct tris_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(property);
		TRIS_APP_ARG(opt);
	);
	struct odbc_txn_frame *tx;
	...
	if (strcasecmp(args.property, "transaction") == 0) {
		if ((tx = find_transaction(chan, NULL, NULL, 1))) {
			tris_copy_string(buf, tx->name, len);
			return 0;
		}
	} else if (strcasecmp(args.property, "isolation") == 0) {
		if (!tris_strlen_zero(args.opt)) {
			tx = find_transaction(chan, NULL, args.opt, 0);
		} else {
			tx = find_transaction(chan, NULL, NULL, 1);
		}
		if (tx) {
			tris_copy_string(buf, isolation2text(tx->isolation), len);
			return 0;
		}
		...
		if (tx) {
			tris_copy_string(buf, tx->forcecommit ? "1" : "0", len);
			return 0;
		}
	}
	return -1;
...
static int acf_transaction_write(struct tris_channel *chan, const char *cmd, char *s, const char *value)
{
	TRIS_DECLARE_APP_ARGS(args,
		TRIS_APP_ARG(property);
		TRIS_APP_ARG(opt);
	...
	if (strcasecmp(args.property, "transaction") == 0) {
		...
		if ((tx = find_transaction(chan, NULL, value, 0))) {
			...
			struct tris_flags flags = { RES_ODBC_INDEPENDENT_CONNECTION };
			if (tris_strlen_zero(args.opt) || !(obj = tris_odbc_request_obj2(args.opt, flags))) {
				tris_log(LOG_ERROR, "Could not create transaction: invalid database specification '%s'\n", S_OR(args.opt, ""));
		...
		if (tris_true(value)) {
			tx->forcecommit = 1;
		} else if (tris_false(value)) {
			tx->forcecommit = 0;
		} else {
			tris_log(LOG_ERROR, "Invalid value for forcecommit: '%s'\n", S_OR(value, ""));
		...
		if (isolation == 0) {
			pbx_builtin_setvar_helper(chan, "ODBC_RESULT", "INVALID_VALUE");
			tris_log(LOG_ERROR, "Invalid isolation specification: '%s'\n", S_OR(value, ""));
		...
		tris_log(LOG_ERROR, "Unknown property: '%s'\n", args.property);
		return -1;
	}
}

SQLHSTMT generic_prepare(struct odbc_obj *obj, void *data)
{
...
static int check_sql_normality(char* sql, int max)
{
	while (*sql && *sql != '\0' && max > 0) {
		max--;
		if (isalnum(*sql) || isblank(*sql) || *sql == '\'' || *sql == '-' || *sql == '=' || *sql == '.' || *sql == ',' ||
				*sql == '_' || *sql == '*' || *sql == '(' || *sql == ')' || *sql == ':' || *sql == ';' || *sql == '%' ||
				 *sql == '@') {
			sql++;
			continue;
		}
...
void sql_select_query_execute(char *result,char *sql)
{
	int res;
	SQLHSTMT stmt;
	//char sql[PATH_MAX];
	...
	strcpy(result, "");
	if (check_sql_normality(sql, strlen(sql))) {
		tris_log(LOG_WARNING, "Oh, bad sql: %s\n", sql);
		return;
	}
...
int sql_query_execute(struct odbc_obj * obj, char *sql)
{
	int res = 0, i, attempt, ret = 0;
	SQLINTEGER nativeerror=0, numfields=0;
	SQLSMALLINT diagbytes=0;
	unsigned char state[10], diagnostic[256];
	SQLHSTMT stmt;
	struct generic_prepare_struct gps = {0,0,0};
	gps.sql = sql;
...
static struct tris_custom_function odbc_function = {
	.name = "ODBC",
...
static int reload(void)
{
	...
	TRIS_RWLIST_WRLOCK(&odbc_tables);
	while ((table = TRIS_RWLIST_REMOVE_HEAD(&odbc_tables, list))) {
...
static int load_module(void)
{
	...
	if (load_odbc_config() == -1)
		return TRIS_MODULE_LOAD_DECLINE;
	tris_cli_register_multiple(cli_odbc, ARRAY_LEN(cli_odbc));
	tris_register_application_xml(app_commit, commit_exec);
	tris_register_application_xml(app_rollback, rollback_exec);
	tris_custom_function_register(&odbc_function);
	tris_sql_select_query_execute = sql_select_query_execute;
#ifdef DEBUG_THREADS
	tris_odbc_connect = _tris_odbc_request_obj;
#else
	tris_odbc_connect = tris_odbc_request_obj;
#endif
	tris_odbc_disconnect = tris_odbc_release_obj;
	tris_query_execute = sql_query_execute;
	tris_log(LOG_NOTICE, "res_odbc loaded.\n");
	return 0;
}

TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_GLOBAL_SYMBOLS, "ODBC resource",
		.load = load_module,
