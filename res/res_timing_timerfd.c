...
static enum tris_timer_event timerfd_timer_get_event(int handle);
static unsigned int timerfd_timer_get_max_rate(int handle);

static struct tris_timing_interface timerfd_timing = {
	.name = "timerfd",
	.priority = 200,
...
static int timerfd_timer_open(void)
{
	...
	if (!(timer = ao2_alloc(sizeof(*timer), timer_destroy))) {
		tris_log(LOG_ERROR, "Could not allocate memory for timerfd_timer structure\n");
		return -1;
	}
	if ((handle = timerfd_create(CLOCK_MONOTONIC, 0)) < 0) {
		tris_log(LOG_ERROR, "Failed to create timerfd timer: %s\n", strerror(errno));
		ao2_ref(timer, -1);
		return -1;
	}
...
static void timerfd_timer_close(int handle)
{
	...
	if (!(our_timer = ao2_find(timerfd_timers, &find_helper, OBJ_POINTER))) {
		tris_log(LOG_ERROR, "Couldn't find timer with handle %d\n", handle);
		return;
	}
...
static void timerfd_timer_ack(int handle, unsigned int quantity)
{
	...
	if (expirations != quantity) {
		tris_debug(2, "Expected to acknowledge %u ticks but got %llu instead\n", quantity, (unsigned long long) expirations);
	}
}
...
static int load_module(void)
{
	...
	if ((fd = timerfd_create(CLOCK_MONOTONIC, 0)) < 0) {
		tris_log(LOG_ERROR, "CLOCK_MONOTONIC not supported.  Not loading.\n");
	...
	if (!(timing_funcs_handle = tris_register_timing_interface(&timerfd_timing))) {
...
TRIS_MODULE_INFO(TRISMEDIA_GPL_KEY, TRIS_MODFLAG_LOAD_ORDER, "Timerfd Timing Interface",
		.load = load_module,
