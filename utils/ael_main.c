...
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 236905 $")
...
struct tris_context *context_list;
struct tris_context *last_context;
...
const char *tris_config_TRIS_CONFIG_DIR = config_dir;
const char *tris_config_TRIS_VAR_DIR = var_dir;
...
struct tris_context * tris_context_create(void **extcontexts, const char *name, const char *registrar);
struct tris_context * tris_context_find_or_create(void **extcontexts, void *tab, const char *name, const char *registrar);
void tris_context_add_ignorepat2(struct tris_context *con, const char *value, const char *registrar);
void tris_context_add_include2(struct tris_context *con, const char *value, const char *registrar);
void tris_context_add_switch2(struct tris_context *con, const char *value, const char *data, int eval, const char *registrar);
void tris_merge_contexts_and_delete(void);
void tris_context_verify_includes(void);
struct tris_context * tris_walk_contexts(void);
void tris_cli_unregister_multiple(void);
void tris_context_destroy(void);
...
struct tris_app *pbx_findapp(const char *app)
{
	return (struct tris_app*)1; /* so as not to trigger an error */
}
...
#if !defined(LOW_MEMORY)
int tris_add_profile(const char *x, uint64_t scale)
{
	if (!no_comp)
		printf("Executed tris_add_profile();\n");
...
{
	if(!no_comp)
        	printf("Executed tris_cli_register_multiple();\n");
}
			...
			const char *registrar)
{
	...
	if(!no_comp)
		printf("Executed tris_add_extension2(context=%s, rep=%d, exten=%s, priority=%d, label=%s, callerid=%s, appl=%s, data=%s, FREE, registrar=%s);\n",
...
struct tris_context * tris_context_create(void **extcontexts, const char *name, const char *registrar)
{
	struct tris_context *x = calloc(1, sizeof(*x));
	if (!x)
		return NULL;
	x->next = context_list;
	context_list = x;
	if (!no_comp)
		printf("Executed tris_context_create(conts, name=%s, registrar=%s);\n", name, registrar);
	conts++;
	strncpy(x->name, name, sizeof(x->name) - 1);
...
struct tris_context * tris_context_find_or_create(void **extcontexts, void *tab, const char *name, const char *registrar)
{
	...
	context_list = x;
	if (!no_comp)
		printf("Executed tris_context_find_or_create(conts, name=%s, registrar=%s);\n", name, registrar);
	conts++;
	strncpy(x->name, name, sizeof(x->name) - 1);
	strncpy(x->registrar, registrar, sizeof(x->registrar) - 1);
	return x;
...
{
	if(!no_comp)
		printf("Executed tris_context_add_ignorepat2(con, value=%s, registrar=%s);\n", value, registrar);
	if( dump_extensions ) {
		struct namelist *x;
		x = create_name(value);
...
{
	if(!no_comp)
		printf("Executed tris_context_add_include2(con, value=%s, registrar=%s);\n", value, registrar);
	if( dump_extensions ) {
		struct namelist *x;
...
{
	if(!no_comp)
		printf("Executed tris_context_add_switch2(con, value=%s, data=%s, eval=%d, registrar=%s);\n", value, data, eval, registrar);
	if( dump_extensions ) {
		struct namelist *x;
		x = create_name((char*)value);
...
{
	if(!no_comp)
		printf("Executed tris_merge_contexts_and_delete();\n");
}

void tris_context_verify_includes(void)
{
	if(!no_comp)
		printf("Executed tris_context_verify_includes();\n");
}

struct tris_context * tris_walk_contexts(void)
{
	if(!no_comp)
		printf("Executed tris_walk_contexts();\n");
	return 0;
}
...
{
	if(!no_comp)
		printf("Executed tris_cli_unregister_multiple();\n");
}

void tris_context_destroy(void)
{
	if( !no_comp)
		printf("Executed tris_context_destroy();\n");
}
...
int main(int argc, char **argv)
{
	...
	if( use_curr_dir ) {
		...
		strcpy(config_dir, "/etc/trismedia");
		localized_use_conf_dir();
	}
	strcpy(var_dir, "/var/lib/trismedia");
	...
	tris_log(4, "ael2_parse", __LINE__, "main", "%d contexts, %d extensions, %d priorities\n", conts, extens, priors);
	...
	for( lp = context_list; lp; lp = lp2 ) { /* free the tris_context structs */
		lp2 = lp->next;
		lp->next = 0;
