 ...
 * locked up (in fact, what has happened is that Trismedia runs at
 ...
 * Trismedia.  Should that happen, that monitoring thread may take
 * immediate action to slow down Trismedia to regular priority,
 * thus allowing an administrator to login to the system and
 * restart Trismedia or perhaps take another course of action
 ...
 * answer is, in Trismedia, nothing.  Process scope threads are the
 * alternative, but they aren't supported in Linux.
 */

static const char explanation[] =
"This file is created when Trismedia is run with a realtime priority (-p).  It\n"
"must continue to exist, and the astcanary process must be allowed to continue\n"
"running, or else the Trismedia process will, within a short period of time,\n"
"slow itself down to regular priority.\n\n"
"The technical explanation for this file is to provide an assurance to Trismedia\n"
"that there are no threads that have gone into runaway mode, thus hogging the\n"
"CPU, and making the Trismedia machine seem to be unresponsive.  When that\n"
"happens, the astcanary process will be unable to update the timestamp on this\n"
"file, and Trismedia will notice within 120 seconds and react.  Slowing the\n"
"Trismedia process down to regular priority will permit an administrator to\n"
...
int main(int argc, char *argv[])
{
	...
	for (parent = atoi(argv[2]); parent == getppid() ;) {
		/* Update the modification times (checked from Trismedia) */
