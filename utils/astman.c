...
struct tris_chan {
	...
	char callerid[40];
	char state[10];
	TRIS_LIST_ENTRY(tris_chan) list;
};

static TRIS_LIST_HEAD_NOLOCK_STATIC(chans, tris_chan);

/* dummy functions to be compatible with the Trismedia core for md5.c */
...
int tris_add_profile(const char *s, uint64_t scale)
{
	return -1;
}

int64_t tris_profile(int, int64_t);
int64_t tris_profile(int key, int64_t val)
{
	return 0;
}
int64_t tris_mark(int, int start1_stop0);
int64_t tris_mark(int key, int start1_stop0)
{
	return 0;
}
#endif /* LOW_MEMORY */
...
static struct tris_chan *find_chan(char *name)
{
	struct tris_chan *chan;
	TRIS_LIST_TRAVERSE(&chans, chan, list) {
		if (!strcmp(name, chan->name))
			return chan;
	}
	chan = malloc(sizeof(struct tris_chan));
	if (chan) {
		memset(chan, 0, sizeof(struct tris_chan));
		strncpy(chan->name, name, sizeof(chan->name) - 1);
		TRIS_LIST_INSERT_TAIL(&chans, chan, list);
	}
	return chan;
}
...
static void del_chan(char *name)
{
	...
	TRIS_LIST_TRAVERSE_SAFE_BEGIN(&chans, chan, list) {
		if (!strcmp(name, chan->name)) {
...
static int event_newstate(struct tris_mansession *s, struct message *m)
{
	struct tris_chan *chan;
...
static int event_newexten(struct tris_mansession *s, struct message *m)
{
	struct tris_chan *chan;
	chan = find_chan(get_header(m, "Channel"));
...
static int event_newchannel(struct tris_mansession *s, struct message *m)
{
	struct tris_chan *chan;
	chan = find_chan(get_header(m, "Channel"));
	strncpy(chan->state, get_header(m, "State"), sizeof(chan->state) - 1);
...
static int event_status(struct tris_mansession *s, struct message *m)
{
	struct tris_chan *chan;
	chan = find_chan(get_header(m, "Channel"));
	strncpy(chan->state, get_header(m, "State"), sizeof(chan->state) - 1);
...
static int event_hangup(struct tris_mansession *s, struct message *m)
{
	del_chan(get_header(m, "Channel"));
	return 0;
}

static int event_ignore(struct tris_mansession *s, struct message *m)
{
	return 0;
}

static int event_rename(struct tris_mansession *s, struct message *m)
{
	struct tris_chan *chan;
...
static struct event {
	char *event;
	int (*func)(struct tris_mansession *s, struct message *m);
} events[] = {
...
static int process_message(struct tris_mansession *s, struct message *m)
{
	int x;
	char event[80] = "";
...
static int has_input(struct tris_mansession *s)
{
	int x;
	for (x=1;x<s->inlen;x++) 
		if ((s->inbuf[x] == '\n') && (s->inbuf[x-1] == '\r')) 
...
static int get_input(struct tris_mansession *s, char *output)
{
...
static int input_check(struct tris_mansession *s, struct message **mout)
{
...
static int manage_calls(char *host)
{
	...
	*/
	snprintf(tmp, sizeof(tmp), "Trismedia Manager at %s", host);
...
static int manager_login(char *hostname)
{
	...
	newtCenteredWindow(40, 10, "Trismedia Manager Login");
...
int main(int argc, char *argv[])
{
	...
	newtInit();
	newtCls();
	newtDrawRootText(0, 0, "Trismedia Manager (C)2002, Linux Support Services, Inc.");
	newtPushHelpLine("Welcome to the Trismedia Manager!");
