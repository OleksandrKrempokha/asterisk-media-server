...
#include "trismedia.h"
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 165071 $")

#include "trismedia/tris_expr.h"

#define TRIS_API_MODULE 1
...
void tris_mark_lock_acquired(void *);
void tris_mark_lock_acquired(void *foo)
{
    /* not a lot to do in a standalone w/o threading! */
}
#endif
...
/* Our own version of tris_log, since the expr parser uses it. */
...
int check_eval(char *buffer, char *error_report)
{
	...
	*ep++ = 0;
	...
	/* now, run the test */
	result = tris_expr(evalbuf, s, sizeof(s),NULL);
	if (result) {
