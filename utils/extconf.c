 ...
 * for operations outside of trismedia. A huge, awful hack.
 *
 */
#include "trismedia.h"
...
#include "trismedia/paths.h"	/* we use TRIS_CONFIG_DIR */
...
static void tris_log(int level, const char *file, int line, const char *function, const char *fmt, ...) __attribute__((format(printf, 5, 6)));
void tris_verbose(const char *fmt, ...) __attribute__((format(printf, 1, 2)));
...
#define TRIS_API_MODULE  1 /* gimme the inline defs! */
struct tris_channel 
{
...
/* IN CONFLICT: void tris_verbose(const char *fmt, ...)
   __attribute__((format(printf, 1, 2))); */

int tris_register_verbose(void (*verboser)(const char *string));
int tris_unregister_verbose(void (*verboser)(const char *string));
...
#define TRIS_MUTEX_INIT_VALUE { PTHREAD_MUTEX_INIT_VALUE, { NULL }, { 0 }, 0, { NULL }, { 0 } }
...
static inline int __tris_pthread_mutex_init_attr(const char *filename, int lineno, const char *func,
						...
						pthread_mutexattr_t *attr) 
{
	...
	if ((t->mutex) != ((pthread_mutex_t) PTHREAD_MUTEX_INITIALIZER)) {
		if ((t->mutex) != (empty_mutex)) {
			__tris_mutex_logger("%s line %d (%s): Error: mutex '%s' is already initialized.\n",
					   filename, lineno, func, mutex_name);
			__tris_mutex_logger("%s line %d (%s): Error: previously initialization of mutex '%s'.\n",
...
static inline int __tris_pthread_mutex_init(const char *filename, int lineno, const char *func,
					   const char *mutex_name, tris_mutex_t *t)
{
	...
	return __tris_pthread_mutex_init_attr(filename, lineno, func, mutex_name, t, &attr);
}
#define tris_mutex_init(pmutex) __tris_pthread_mutex_init(__FILE__, __LINE__, __PRETTY_FUNCTION__, #pmutex, pmutex)
						...
						const char *mutex_name, tris_mutex_t *t)
{
	...
	if ((res = pthread_mutex_destroy(&t->mutex)))
		__tris_mutex_logger("%s line %d (%s): Error destroying mutex: %s\n",
                                           ...
                                           const char* mutex_name, tris_mutex_t *t)
{
	...
	if (!res) {
		if (t->reentrancy < TRIS_MAX_REENTRANCY) {
                                              ...
                                              const char* mutex_name, tris_mutex_t *t)
{
	...
	if (!(res = pthread_mutex_trylock(&t->mutex))) {
		...
		__tris_mutex_logger("%s line %d (%s): Warning: '%s' was locked here.\n",
                                   t->file[t->reentrancy-1], t->lineno[t->reentrancy-1], t->func[t->reentrancy-1], mutex_name);
	}
...
#if defined(TRIS_MUTEX_INIT_W_CONSTRUCTORS)
/* If TRIS_MUTEX_INIT_W_CONSTRUCTORS is defined, use file scope
 constructors/destructors to create/destroy mutexes.  */
#define __TRIS_MUTEX_DEFINE(scope, mutex) \
	scope tris_mutex_t mutex = TRIS_MUTEX_INIT_VALUE; \
...
#define TRIS_MUTEX_DEFINE_STATIC(mutex) __TRIS_MUTEX_DEFINE(static, mutex)
...
#define __TRIS_RWLOCK_DEFINE(scope, rwlock) \
        scope tris_rwlock_t rwlock; \
...
#define __TRIS_RWLOCK_DEFINE(scope, rwlock) \
        scope tris_rwlock_t rwlock = TRIS_RWLOCK_INIT_VALUE
#endif

#define TRIS_RWLOCK_DEFINE_STATIC(rwlock) __TRIS_RWLOCK_DEFINE(static, rwlock)
...
#else
static int tris_atomic_fetchadd_int_slow(volatile int *p, int v)
{
	int ret;
	ret = *p;
	*p += v;
...
#define tris_channel_lock(x)		tris_mutex_lock(&x->lock)
/*! \brief Unlock a channel. If DEBUG_CHANNEL_LOCKS is defined 
	in the Makefile, print relevant output for debugging */
#define tris_channel_unlock(x)		tris_mutex_unlock(&x->lock)
/*! \brief Try locking a channel. If DEBUG_CHANNEL_LOCKS is defined 
	in the Makefile, print relevant output for debugging */
#define tris_channel_trylock(x)		tris_mutex_trylock(&x->lock)
#else

/*! \brief Lock AST channel (and print debugging output)
\note You need to enable DEBUG_CHANNEL_LOCKS for this function */
int tris_channel_lock(struct tris_channel *chan);

/*! \brief Unlock AST channel (and print debugging output)
\note You need to enable DEBUG_CHANNEL_LOCKS for this function
*/
int tris_channel_unlock(struct tris_channel *chan);

/*! \brief Lock AST channel (and print debugging output)
\note   You need to enable DEBUG_CHANNEL_LOCKS for this function */
int tris_channel_trylock(struct tris_channel *chan);
#endif
...
struct tris_flags {  /* stolen from utils.h */
	unsigned int flags;
};
...
TRIS_INLINE_API(
__attribute__((format(printf, 5, 6)))
int _tris_asprintf(char **ret, const char *file, int lineno, const char *func, const char *fmt, ...),
{
	int res;
	va_list ap;
...
typedef struct tris_config *config_load_func(const char *database, const char *table, const char *configfile, struct tris_config *config, int withcomments, const char *suggested_include_file);
...
static force_inline int tris_strlen_zero(const char *s)
{
	return (!s || (*s == '\0'));
}

#define S_OR(a, b)	(!tris_strlen_zero(a) ? (a) : (b))
...
TRIS_INLINE_API(
char *tris_skip_blanks(const char *str),
{
	while (*str && *str < 33)
		str++;
	return (char *)str;
...
/*!
  ...
  \param tris_trim_blanks function being used
...
static const char *tris_variable_retrieve(const struct tris_config *config, const char *category, const char *variable);
static struct tris_config *config_text_file_load(const char *database, const char *table, const char *filename, struct tris_config *cfg, int withcomments, const char *suggested_include_file);

struct tris_config *localized_config_load_with_comments(const char *filename);
static char *tris_category_browse(struct tris_config *config, const char *prev);
static struct tris_variable *tris_variable_browse(const struct tris_config *config, const char *category);
static void tris_variables_destroy(struct tris_variable *v);
static void tris_config_destroy(struct tris_config *cfg);
static struct tris_config_include *tris_include_new(struct tris_config *conf, const char *from_file, const char *included_file, int is_exec, const char *exec_file, int from_lineno, char *real_included_file_name, int real_included_file_name_size);
static struct tris_config_include *tris_include_find(struct tris_config *conf, const char *included_file);
void localized_tris_include_rename(struct tris_config *conf, const char *from_file, const char *to_file);

static struct tris_variable *tris_variable_new(const char *name, const char *value, const char *filename);
...
static struct tris_variable *tris_variable_new(const char *name, const char *value, const char *filename) 
{
	...
	if ((variable = tris_calloc(1, name_len + strlen(value) + 1 + strlen(filename) + 1 + sizeof(*variable)))) {
...
static void tris_variable_append(struct tris_category *category, struct tris_variable *variable);
...
static struct tris_category *tris_category_get(const struct tris_config *config, const char *category_name)
{
...
/* options.h declares tris_options extern; I need it static? */
...
#define tris_opt_priority_jumping	tris_test_flag(&tris_options, TRIS_OPT_FLAG_PRIORITY_JUMPING)
...
extern char debug_filename[TRIS_FILENAME_MAX];
 ...
 */
#define TRIS_LIST_REMOVE_CURRENT(head, field)						\
 ...
 */
#define TRIS_LIST_REMOVE(head, elm, field) do {			        \
	if ((head)->first == (elm)) {					\
...
/* IN CONFLICT: struct tris_var_t *tris_var_assign(const char *name, const char *value); */

static struct tris_var_t *tris_var_assign(const char *name, const char *value);

static void tris_var_delete(struct tris_var_t *var);
...
static void pbx_substitute_variables_helper(struct tris_channel *c,const char *cp1,char *cp2,int count);
 ...
 */
static void tris_shrink_phone_number(char *n)
{
	int x, y=0;
	int bracketed = 0;
...
static const char *tris_var_name(const struct tris_var_t *var)
{
	const char *name;
...
static void tris_log(int level, const char *file, int line, const char *function, const char *fmt, ...)
{
	va_list vars;
	va_start(vars,fmt);
...
static char *tris_process_quotes_and_slashes(char *start, char find, char replace_with)
{
 	char *dataPut = start;
	int inEscape = 0;
	int inQuotes = 0;
...
static int tris_true(const char *s)
{
...
static int tris_func_write(struct tris_channel *chan, const char *function, const char *value)
{
...
static unsigned int tris_app_separate_args(char *buf, char delim, char **array, int arraylen)
{
	int argc;
	char *scan;
	int paren = 0, quote = 0;
...
static void pbx_builtin_setvar_helper(struct tris_channel *chan, const char *name, const char *value)
{
	...
	TRIS_LIST_TRAVERSE (headp, newvariable, entries) {
		if (strcasecmp(tris_var_name(newvariable), nametail) == 0) {
			/* there is already such a variable, delete it */
			TRIS_LIST_REMOVE(headp, newvariable, entries);
	...
	if (value) {
		if ((option_verbose > 1) && (headp == &globals))
			tris_verbose(VERBOSE_PREFIX_2 "Setting global variable '%s' to '%s'\n", name, value);
...
static int pbx_builtin_setvar(struct tris_channel *chan, void *data)
{
	...
	if (tris_strlen_zero(data)) {
		tris_log(LOG_WARNING, "Set requires at least one variable name/value pair.\n");
		return 0;
	}

	mydata = tris_strdupa(data);
	argc = tris_app_separate_args(mydata, '|', argv, sizeof(argv) / sizeof(argv[0]));
	...
	for (x = 0; x < argc; x++) {
		...
		if ((value = strchr(name, '='))) {
			*value++ = '\0';
			...
			tris_log(LOG_WARNING, "Ignoring entry '%s' with no = (and not last 'options' entry)\n", name);
	}
...
static struct tris_category *tris_category_new(const char *name, const char *in_file, int lineno);
...
static void tris_category_append(struct tris_config *config, struct tris_category *category);
...
static void tris_category_destroy(struct tris_category *cat);
...
static struct tris_config *tris_config_internal_load(const char *filename, struct tris_config *cfg, int withcomments, const char *suggested_incl_file);
...
static struct tris_config *tris_config_internal_load(const char *filename, struct tris_config *cfg, int withcomments, const char *suggested_incl_file)
{
	...
	cfg->include_level++;
	/*  silence is golden!
		tris_log(LOG_WARNING, "internal loading file %s level=%d\n", filename, cfg->include_level);
	*/
	...
	/* silence is golden 
	   tris_log(LOG_WARNING, "finished internal loading file %s level=%d\n", filename, cfg->include_level);
	*/
...
static int process_text_line(struct tris_config *cfg, struct tris_category **cat, char *buf, int lineno, const char *configfile, int withcomments, const char *suggested_include_file)
{
	char *c;
	char *cur = buf;
	struct tris_variable *v;
		...
		*c++ = '\0';
		cur++;
 		if (*c++ != '(')
 			c = NULL;
		catname = cur;
		if (!(*cat = newcat = tris_category_new(catname, tris_strlen_zero(suggested_include_file)?configfile:suggested_include_file, lineno))) {
			return -1;
		}
		(*cat)->lineno = lineno;
		...
		if (do_include || do_exec) {
			if (c) {
				...
				if (do_exec) { 
					...
					exec_file[0] = '\0';
				/* A #include */
				/* tris_log(LOG_WARNING, "Reading in included file %s withcomments=%d\n", cur, withcomments); */
				
				/* record this inclusion */
				inclu = tris_include_new(cfg, configfile, cur, do_exec, cur2, lineno, real_inclusion_name, sizeof(real_inclusion_name));
				
				do_include = tris_config_internal_load(cur, cfg, withcomments, real_inclusion_name) ? 1 : 0;
				if(!tris_strlen_zero(exec_file))
					unlink(exec_file);
				if(!do_include)
					return 0;
				/* tris_log(LOG_WARNING, "Done reading in included file %s withcomments=%d\n", cur, withcomments); */
		...
		if (c) {
			*c = 0;
			...
			if (*c== '>') {
				...
				object = 0;
			if ((v = tris_variable_new(tris_strip(cur), tris_strip(c), configfile))) {
				v->lineno = lineno;
				v->object = object;
			...
			tris_log(LOG_WARNING, "EXTENSIONS.CONF: No '=' (equal sign) in line %d of %s\n", lineno, configfile);
		}
	}
	return 0;
}
...
static struct tris_config *config_text_file_load(const char *database, const char *table, const char *filename, struct tris_config *cfg, int withcomments, const char *suggested_include_file)
{
	...
	do {
		...
		if (option_verbose > 1) {
			tris_verbose(VERBOSE_PREFIX_2 "Parsing '%s': ", fn);
			fflush(stdout);
		}
		if (!(f = fopen(fn, "r"))) {
			if (option_debug)
		...
		while(!feof(f)) {
			...
			if (fgets(buf, sizeof(buf), f)) {
				...
				if (process_buf) {
					char *stripped_process_buf = tris_strip(process_buf);
					if (!tris_strlen_zero(stripped_process_buf)) {
...
static struct tris_config *tris_config_new(void) ;
...
struct tris_config *localized_config_load(const char *filename)
{
	...
	result = tris_config_internal_load(filename, cfg, 0, "");
	if (!result)
		tris_config_destroy(cfg);
...
struct tris_config *localized_config_load_with_comments(const char *filename)
{
	...
	result = tris_config_internal_load(filename, cfg, 1, "");
	if (!result)
		tris_config_destroy(cfg);
...
int localized_config_text_file_save(const char *configfile, const struct tris_config *cfg, const char *generator)
{
	...
	for (incl=cfg->includes; incl; incl = incl->next)
	{
		if (!incl->exec) { /* leave the execs alone -- we'll write out the #exec directives, but won't zero out the include files or exec files*/
			...
			if (f1) {
				...
				tris_verbose(VERBOSE_PREFIX_2 "Unable to write %s (%s)", fn, strerror(errno));
			}
		}
	}
	...
	if ((f = fopen(fn, "w"))) {
#endif	    
		if (option_verbose > 1)
			tris_verbose(VERBOSE_PREFIX_2 "Saving '%s': ", fn);
		...
		if (option_verbose > 1)
			tris_verbose(VERBOSE_PREFIX_2 "Unable to write (%s)", strerror(errno));
		return -1;
	}
...
static struct tris_context *local_contexts = NULL;
static struct tris_context *contexts = NULL;
struct tris_context;
struct tris_app;
#ifdef LOW_MEMORY
...
static const char *tris_get_extension_app(struct tris_exten *e)
{
	return e ? e->app : NULL;
}

static const char *tris_get_extension_name(struct tris_exten *exten)
{
...
/*! \brief  tris_change_hint: Change hint for an extension */
...
/*! \brief  tris_add_hint: Add hint to hint list, check initial extension state */
static int tris_add_hint(struct tris_exten *e)
{
	...
	TRIS_RWLIST_TRAVERSE(&hints, hint, list) {
		if (hint->exten == e) {
			if (option_debug > 1)
				tris_log(LOG_DEBUG, "HINTS: Not re-adding existing hint %s: %s\n", tris_get_extension_name(e), tris_get_extension_app(e));
			return -1;
		}
	}

	if (option_debug > 1)
		tris_log(LOG_DEBUG, "HINTS: Adding hint %s: %s\n", tris_get_extension_name(e), tris_get_extension_app(e));
...
/*! \brief  tris_remove_hint: Remove hint from extension */
static int tris_remove_hint(struct tris_exten *e)
{
	/* Cleanup the Notifys if hint is removed */
	struct tris_hint *hint;
	struct tris_state_cb *cblist, *cbprev;
	int res = -1;
	...
	TRIS_RWLIST_TRAVERSE_SAFE_BEGIN(&hints, hint, list) {
		if (hint->exten == e) {
			...
			hint->callbacks = NULL;
			TRIS_RWLIST_REMOVE_CURRENT(&hints, list);
			free(hint);
	   		res = 0;
			break;
		}
...
static struct tris_context *tris_walk_contexts(struct tris_context *con);
...
struct tris_context *localized_walk_contexts(struct tris_context *con)
{
	return tris_walk_contexts(con);
}



static struct tris_exten *tris_walk_context_extensions(struct tris_context *con,
...
struct tris_exten *localized_walk_context_extensions(struct tris_context *con,
													struct tris_exten *exten)
{
	return tris_walk_context_extensions(con,exten);
}


static struct tris_exten *tris_walk_extension_priorities(struct tris_exten *exten,
...
struct tris_exten *localized_walk_extension_priorities(struct tris_exten *exten,
													  struct tris_exten *priority)
{
	return tris_walk_extension_priorities(exten, priority);
}



static struct tris_include *tris_walk_context_includes(struct tris_context *con,
...
struct tris_include *localized_walk_context_includes(struct tris_context *con,
													struct tris_include *inc)
{
	return tris_walk_context_includes(con, inc);
}


static struct tris_sw *tris_walk_context_switches(struct tris_context *con,
													 struct tris_sw *sw);
...
struct tris_sw *localized_walk_context_switches(struct tris_context *con,
													struct tris_sw *sw)
{
	return tris_walk_context_switches(con, sw);
}


static struct tris_context *tris_context_find(const char *name);
...
/*
 * Internal function for tris_extension_{match|close}
...
static int tris_extension_match(const char *pattern, const char *data);
...
static struct tris_exten *pbx_find_extension(struct tris_channel *chan,
...
static const char *tris_get_context_name(struct tris_context *con);
 ...
 */
static int tris_context_add_include2(struct tris_context *con, const char *value,
							 const char *registrar);
									...
									const char *registrar)
{
	...
	if (option_verbose > 2)
		tris_verbose(VERBOSE_PREFIX_3 "Including context '%s' in context '%s'\n", new_include->name, tris_get_context_name(con));
...
int localized_context_add_include2(struct tris_context *con, const char *value,
								   const char *registrar)
{
	return  tris_context_add_include2(con, value, registrar);
}



static int tris_context_add_ignorepat2(struct tris_context *con, const char *value, const char *registrar);
...
int localized_context_add_ignorepat2(struct tris_context *con, const char *value, const char *registrar)
{
	return tris_context_add_ignorepat2(con, value, registrar);
}
...
static int tris_wrlock_contexts(void)
{
	return tris_rwlock_wrlock(&conlock);
}

static int tris_unlock_contexts(void)
{
	return tris_rwlock_unlock(&conlock);
}

static int tris_wrlock_context(struct tris_context *con)
{
	return tris_rwlock_wrlock(&con->lock);
}

static int tris_unlock_context(struct tris_context *con)
{
 ...
 *  EINVAL - there is no existence of context for inclusion
 */
static int tris_context_add_switch2(struct tris_context *con, const char *value,
	...
	const char *data, int eval, const char *registrar)
{
	...
	if (option_verbose > 2)
		tris_verbose(VERBOSE_PREFIX_3 "Including switch '%s/%s' in context '%s'\n", new_sw->name, new_sw->data, tris_get_context_name(con));
...
int localized_context_add_switch2(struct tris_context *con, const char *value,
										 const char *data, int eval, const char *registrar)
{
	return tris_context_add_switch2(con, value, data, eval, registrar);
}

static struct tris_context *__tris_context_create(struct tris_context **extcontexts, const char *name, const char *registrar, int existsokay)
{
	struct tris_context *tmp, **loc_contexts;
	...
	for (tmp = *loc_contexts; tmp; tmp = tmp->next) {
		if (!strcasecmp(tmp->name, name)) {
			if (!existsokay) {
				tris_log(LOG_WARNING, "Tried to register context '%s', already in use\n", name);
				tmp = NULL;
			}
			if (!extcontexts)
	...
	if ((tmp = tris_calloc(1, length))) {
		...
		*loc_contexts = tmp;
		if (option_debug)
			tris_log(LOG_DEBUG, "Registered context '%s'\n", tmp->name);
		if (option_verbose > 2)
			tris_verbose( VERBOSE_PREFIX_3 "Registered extension context '%s'\n", tmp->name);
	}
 ...
 *
 */
static int tris_add_extension2(struct tris_context *con,
	...
	const char *registrar)
{
	 ...
	 */
	struct tris_exten *tmp, *e, *el = NULL;
	int res;
	int length;
	char *p;
	...
	*/
	...
	if (option_debug) {
		if (tmp->matchcid) {
			tris_log(LOG_DEBUG, "Added extension '%s' priority %d (CID match '%s') to %s\n",
				tmp->exten, tmp->priority, tmp->cidmatch, con->name);
		} else {
			tris_log(LOG_DEBUG, "Added extension '%s' priority %d to %s\n",
				tmp->exten, tmp->priority, con->name);
		}
	}
	if (option_verbose > 2) {
		if (tmp->matchcid) {
			tris_verbose( VERBOSE_PREFIX_3 "Added extension '%s' priority %d (CID match '%s')to %s\n",
				tmp->exten, tmp->priority, tmp->cidmatch, con->name);
		} else {
			tris_verbose( VERBOSE_PREFIX_3 "Added extension '%s' priority %d to %s\n",
				tmp->exten, tmp->priority, con->name);
		}
	}
	return 0;
...
int localized_add_extension2(struct tris_context *con,
							 ...
							 const char *registrar)
{
	return tris_add_extension2(con, replace, extension, priority, label, callerid, application, data, datad, registrar);
}
								...
								const char *label, const char *callerid, enum ext_match_t action)
{
	...
	if (e) {
		...
		default:
			if (option_debug)
				tris_log(LOG_DEBUG, "Shouldn't happen!\n");
		}
...
static int tris_findlabel_extension2(struct tris_channel *c, struct tris_context *con, const char *exten, const char *label, const char *callerid);
...
static struct tris_context *tris_context_find_or_create(struct tris_context **extcontexts, void *tab, const char *name, const char *registrar)
{
	return __tris_context_create(extcontexts, name, registrar, 1);
}
...
/* chopped this one off at the knees */
static int tris_func_read(struct tris_channel *chan, const char *function, char *workspace, size_t len)
{
	tris_log(LOG_ERROR, "Function %s not registered\n", function);
	return -1;
}
...
static const char *tris_var_value(const struct tris_var_t *var)
{
 ...
 */
static void pbx_retrieve_variable(struct tris_channel *c, const char *var, char **ret, char *workspace, int workspacelen, struct varshead *headp)
{
	...
	for (i = 0; s == &not_found && i < (sizeof(places) / sizeof(places[0])); i++) {
		...
		TRIS_LIST_TRAVERSE(places[i], variables, entries) {
			if (strcasecmp(tris_var_name(variables), var)==0) {
...
static void pbx_substitute_variables_helper_full(struct tris_channel *c, struct varshead *headp, const char *cp1, char *cp2, int count)
{
	...
	*cp2 = 0; /* just in case there's nothing to do */
	...
	while (!tris_strlen_zero(whereweare) && count) {
		...
		if (nextvar) {
			...
			if (brackets)
				tris_log(LOG_NOTICE, "Error in extension logic (missing '}' in '%s')\n", cp1);
			len = vare - vars - 1;
			...
			if (isfunction) {
				...
				if (option_debug)
					tris_log(LOG_DEBUG, "Function result is '%s'\n", cp4 ? cp4 : "(null)");
			} else {
			...
			if (brackets)
				tris_log(LOG_NOTICE, "Error in extension logic (missing ']')\n");
			len = vare - vars - 1;
			...
			length = tris_expr(vars, cp2, count, NULL);

			if (length) {
				if (option_debug)
					tris_log(LOG_DEBUG, "Expression result is '%s'\n", cp2);
				count -= length;
				cp2 += length;
				*cp2 = 0;
			}
...
static int pbx_load_config(const char *config_file)
{
	...
	static_config = tris_true(tris_variable_retrieve(cfg, "general", "static"));
	write_protect_config = tris_true(tris_variable_retrieve(cfg, "general", "writeprotect"));
	if ((aft = tris_variable_retrieve(cfg, "general", "autofallthrough")))
		autofallthrough_config = tris_true(aft);
	clearglobalvars_config = tris_true(tris_variable_retrieve(cfg, "general", "clearglobalvars"));

	if ((cxt = tris_variable_retrieve(cfg, "general", "userscontext"))) 
		tris_copy_string(userscontext, cxt, sizeof(userscontext));
	else
		tris_copy_string(userscontext, "default", sizeof(userscontext));
								    
	for (v = tris_variable_browse(cfg, "globals"); v; v = v->next) {
		memset(realvalue, 0, sizeof(realvalue));
		pbx_substitute_variables_helper(NULL, v->value, realvalue, sizeof(realvalue) - 1);
		pbx_builtin_setvar_helper(NULL, v->name, realvalue);
	}
	for (cxt = NULL; (cxt = tris_category_browse(cfg, cxt)); ) {
		/* All categories but "general" or "globals" are considered contexts */
		if (!strcasecmp(cxt, "general") || !strcasecmp(cxt, "globals"))
			continue;
		con=tris_context_find_or_create(&local_contexts,NULL,cxt, global_registrar);
		if (con == NULL)
			continue;

		for (v = tris_variable_browse(cfg, cxt); v; v = v->next) {
			if (!strcasecmp(v->name, "exten")) {
				char *tc = tris_strdup(v->value);
				if (tc) {
					int ipri = -2;
					char realext[256]="";
					...
					if (cidmatch) {
						*cidmatch++ = '\0';
						tris_shrink_phone_number(cidmatch);
					}
					pri = strsep(&stringp, ",");
					if (!pri)
						pri="";
						...
						*label++ = '\0';
						...
						if (end)
							*end = '\0';
							...
							tris_log(LOG_WARNING, "Label missing trailing ')' at line %d\n", v->lineno);
					}
					plus = strchr(pri, '+');
					if (plus)
						*plus++ = '\0';
					if (!strcmp(pri,"hint"))
						...
						if (lastpri > -2)
							...
							tris_log(LOG_WARNING, "Can't use 'next' priority on the first entry!\n");
					} else if (!strcmp(pri, "same") || !strcmp(pri, "s")) {
						if (lastpri > -2)
							ipri = lastpri;
						else
							tris_log(LOG_WARNING, "Can't use 'same' priority on the first entry!\n");
					} else if (sscanf(pri, "%30d", &ipri) != 1 &&
					    (ipri = tris_findlabel_extension2(NULL, con, realext, pri, cidmatch)) < 1) {
						tris_log(LOG_WARNING, "Invalid priority/label '%s' at line %d\n", pri, v->lineno);
						ipri = 0;
					}
					appl = S_OR(stringp, "");
					...
					if (firstc && (!firstp || firstc < firstp)) {
						...
						if ((end = strrchr(data, ')'))) {
							*end = '\0';
							...
							tris_log(LOG_WARNING, "No closing parenthesis found? '%s(%s'\n", appl, data);
						}
						tris_process_quotes_and_slashes(data, ',', '|');
					}

					if (!data)
						data="";
					appl = tris_skip_blanks(appl);
					if (ipri) {
						if (plus)
							ipri += atoi(plus);
						lastpri = ipri;
						if (!tris_opt_dont_warn && !strcmp(realext, "_."))
							tris_log(LOG_WARNING, "The use of '_.' for an extension is strongly discouraged and can have unexpected behavior.  Please use '_X.' instead at line %d\n", v->lineno);
						if (tris_add_extension2(con, 0, realext, ipri, label, cidmatch, appl, strdup(data), tris_free_ptr, global_registrar)) {
							tris_log(LOG_WARNING, "Unable to register extension at line %d\n", v->lineno);
						}
					}
					free(tc);
				}
				...
				memset(realvalue, 0, sizeof(realvalue));
				pbx_substitute_variables_helper(NULL, v->value, realvalue, sizeof(realvalue) - 1);
				if (tris_context_add_include2(con, realvalue, global_registrar))
					tris_log(LOG_WARNING, "Unable to include context '%s' in context '%s'\n", v->value, cxt);
			} else if (!strcasecmp(v->name, "ignorepat")) {
				memset(realvalue, 0, sizeof(realvalue));
				pbx_substitute_variables_helper(NULL, v->value, realvalue, sizeof(realvalue) - 1);
				if (tris_context_add_ignorepat2(con, realvalue, global_registrar))
					tris_log(LOG_WARNING, "Unable to include ignorepat '%s' in context '%s'\n", v->value, cxt);
				...
				if (!strcasecmp(v->name, "switch"))
					...
					tris_copy_string(realvalue, v->value, sizeof(realvalue));
				appl = strsep(&stringp, "/");
				data = strsep(&stringp, ""); /* XXX what for ? */
				if (!data)
					data = "";
				if (tris_context_add_switch2(con, appl, data, !strcasecmp(v->name, "eswitch"), global_registrar))
					tris_log(LOG_WARNING, "Unable to include switch '%s' in context '%s'\n", v->value, cxt);
			} else {
				tris_log(LOG_WARNING, "==!!== Unknown directive: %s at line %d -- IGNORING!!!\n", v->name, v->lineno);
			}
		}
	}
	tris_config_destroy(cfg);
...
static void __tris_context_destroy(struct tris_context *con, const char *registrar)
{
	...
	for (tmp = contexts; tmp; ) {
		...
		for (; tmp; tmpl = tmp, tmp = tmp->next) {
			if (option_debug)
				tris_log(LOG_DEBUG, "check ctx %s %s\n", tmp->name, tmp->registrar);
		...
		if (option_debug)
			tris_log(LOG_DEBUG, "delete ctx %s %s\n", tmp->name, tmp->registrar);
		next = tmp->next;
		if (tmpl)
			tmpl->next = next;
		else
...
{
	tris_wrlock_contexts();
	__tris_context_destroy(con,registrar);
	tris_unlock_contexts();
}


static void tris_merge_contexts_and_delete(struct tris_context **extcontexts, const char *registrar)
{
	struct tris_context *tmp, *lasttmp = NULL;
	...
	*/
	...
	if (registrar) {
		...
		if (option_debug)
			tris_log(LOG_DEBUG, "must remove any reg %s\n", registrar);
		__tris_context_destroy(NULL,registrar);
		while (tmp) {
			lasttmp = tmp;
			tmp = tmp->next;
		}
		...
		while (tmp) {
			tris_log(LOG_WARNING, "must remove %s  reg %s\n", tmp->name, tmp->registrar);
			__tris_context_destroy(tmp,tmp->registrar);
			lasttmp = tmp;
			tmp = tmp->next;
		}
	}
	if (lasttmp) {
		...
		*extcontexts = NULL;
		...
		tris_log(LOG_WARNING, "Requested contexts didn't get merged\n");
...
void localized_merge_contexts_and_delete(struct tris_context **extcontexts, const char *registrar);

void localized_merge_contexts_and_delete(struct tris_context **extcontexts, const char *registrar)
{
	tris_merge_contexts_and_delete(extcontexts, registrar);
}

static int tris_context_verify_includes(struct tris_context *con)
{
	...
	while ( (inc = tris_walk_context_includes(con, inc)) )
		if (!tris_context_find(inc->rname)) {
			...
			if (strcasecmp(inc->rname,"parkedcalls")!=0)
				tris_log(LOG_WARNING, "Context '%s' tries to include the nonexistent context '%s'\n",
...
int localized_context_verify_includes(struct tris_context *con)
{
	return tris_context_verify_includes(con);
}
...
int localized_pbx_load_module(void)
{
	...
	if(!pbx_load_config(config_filename))
		return -1 /* TRIS_MODULE_LOAD_DECLINE*/;

	/* pbx_load_users(); */ /* does this affect the dialplan? */

	tris_merge_contexts_and_delete(&local_contexts, global_registrar);

	for (con = NULL; (con = tris_walk_contexts(con));)
