...
#include "trismedia.h"
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 163991 $")
...
int64_t tris_mark(int prof_id, int x)
{
	return 0;
}
#endif
...
static int hashtab_compare_strings_nocase(const void *a, const void *b)
{
	...
	return tris_hashtab_compare_strings_nocase(ae->key, be->key);
}
...
{
	const struct ht_element *ae = a, *be = b;
	return tris_hashtab_compare_strings(ae->key, be->key);
}

static unsigned int hashtab_hash_string(const void *obj)
{
	const struct ht_element *o = obj;
	return tris_hashtab_hash_string((const void *)o->key);
}
#endif
...
{
	const struct ht_element *o = obj;
	return tris_hashtab_hash_string_nocase((const void*)o->key);
}
...
struct tris_hashtab *glob_hashtab = 0;
...
static void add_element(void)
{
	...
	tris_hashtab_insert_immediate(glob_hashtab, x);
	els_added++;
}
...
static void traverse_elements(void)
{
	...
	int c=0;
	struct tris_hashtab_iter *it = tris_hashtab_start_write_traversal(glob_hashtab);
#ifdef DEBUG
	printf("Traverse hashtab\n");
#endif
	while ((el = tris_hashtab_next(it))) {
		c++;
	}
	tris_hashtab_end_traversal(it);
...
#endif
	lookup.key = keybuf;
	el = tris_hashtab_remove_object_via_lookup(glob_hashtab, &lookup);
...
static int lookup_element(unsigned int *seedp)
{
	...
	sprintf(keybuf, "key%08d", x);
	lookup.key = keybuf;
	el = tris_hashtab_lookup(glob_hashtab, &lookup);
	els_lookedup++;
	if (el)
		els_found++;
	if (el)
...
static void run_hashtest(int numthr)
{
	...
	glob_hashtab = tris_hashtab_create(180000, hashtab_compare_strings_nocase, tris_hashtab_resize_java, tris_hashtab_newsize_java, hashtab_hash_string_nocase, 1);
	printf("starting with %d elements in the hashtable...\n", tris_hashtab_capacity(glob_hashtab));
	/* set a random seed  */
	...
	for(i=0;i<numthr;i++)
	{
		...
		if (tris_pthread_create(&thr[i], NULL, hashtest, (void*)z)) {
	...
	tris_hashtab_get_stats(glob_hashtab, &biggest, &resize_cnt, &numobjs, &numbuckets);
