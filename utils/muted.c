 ...
 * Connects to the Trismedia Manager Interface, AMI, and listens for events
...
/*! Connect to the trismedia manager interface */
static int connect_trismedia(void)
{
	int sock;
	struct hostent *hp;
	char *ports;
...
/*! Login to the trismedia manager interface */
static int login_trismedia(void)
{
	char *welcome;
	char *resp;
	if (!(welcome = get_line())) {
