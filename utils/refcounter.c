...
#include "trismedia.h"
TRISMEDIA_FILE_VERSION(__FILE__, "$Revision: 169673 $")
...
int main(int argc,char **argv)
{
	...
	objhash = tris_hashtab_create(9000, hashtab_compare_rc, tris_hashtab_resize_java, tris_hashtab_newsize_java, hashtab_hash_rc, 1);
	...
	while (fgets(linebuffer, sizeof(linebuffer), ifile)) {
		...
		count1_obj = tris_hashtab_lookup(objhash, &lookup);
		...
		if (count1_obj) {
			...
			if (*(t+1) == '=') {
				...
				tris_hashtab_insert_safe(objhash, curr_obj);
			} else {
				if (count1_obj->last_count > 1) {
					lookup.count = count1_obj->last_count;
					curr_obj = tris_hashtab_lookup(objhash, &lookup);
				} else {
					curr_obj = count1_obj;
				}
	...
	it = tris_hashtab_start_traversal(objhash);
	while ((curr_obj = tris_hashtab_next(it))) {
...
/* stub routines to satisfy linking with trismedia subcomponents */
...
int64_t tris_mark(int x, int start1_stop0)
{
	return 0;
}
#endif
